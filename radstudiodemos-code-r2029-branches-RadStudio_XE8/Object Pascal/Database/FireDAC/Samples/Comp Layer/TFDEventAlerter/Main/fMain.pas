...
uses
  ...
  fMainCompBase,
  FireDAC.Stan.Intf, FireDAC.DatS, FireDAC.Stan.Option, FireDAC.Stan.Param, FireDAC.Stan.Error, FireDAC.Stan.Async, 
  FireDAC.Phys.Intf,
...
begin
  ...
  if qryFireEvents.SQL.Count = 0 then
    with qryFireEvents.SQL do begin
      ...
      TFDRDBMSKinds.Oracle:
        if FDEventAlerter1.Options.Kind = 'DBMS_PIPE' then begin
          Add('declare');
          Add('  i integer;');
          Add('begin');
          Add('  dbms_pipe.pack_message(''qwe'');');
          Add('  dbms_pipe.pack_message(123);');
          Add('  dbms_pipe.pack_message(sysdate());');
          Add('  i := dbms_pipe.send_message(''ev1'');');
          Add('end;');
        end
        else begin
          Add('begin');
          Add('  dbms_alert.signal(''ev1'', ''event 1'');');
          Add('  dbms_alert.signal(''ev1'', ''event 11'');');
          Add('  dbms_alert.signal(''ev2'', ''event 2'');');
          Add('end;');
        end;
      TFDRDBMSKinds.SQLAnywhere:
        Add('MESSAGE ''_AD_$$EV1$$hello world !'' TYPE INFO TO CLIENT FOR ALL');
      ...
      TFDRDBMSKinds.Advantage:
        ...
        raise Exception.Create('The selected DBMS does not support events !');
      end;
    end;
