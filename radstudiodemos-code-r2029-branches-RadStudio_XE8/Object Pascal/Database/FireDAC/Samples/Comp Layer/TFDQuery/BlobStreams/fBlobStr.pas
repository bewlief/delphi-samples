...
uses
  ...
  FireDAC.Phys, FireDAC.Phys.MSSQL, FireDAC.Phys.SQLite, FireDAC.Phys.Oracle,
    FireDAC.Phys.PG, FireDAC.Phys.IBBase, FireDAC.Phys.FB, FireDAC.Phys.ADS,
...
// "Real BLOB Stream" feature allows to read / write BLOBs without copying
// a BLOB value through client side memory. Real BLOB Stream works directly
...
//
// All Real BLOB Stream operations must be performed inside of a transaction.
// ODBC based drivers require to call TFDQuery.CloseStreams after using the
// streams. For other drivers this call is optional.
//
// Internal stream reading / writing for most drivers is unidirectional and
// stream.Size / Position provide no information / does nothing.
//
// The following drivers are supporting Real BLOB Streaming:
// - Advantage
// - DB2
// - Informix
// - InterBase / IBLite / FireBird (only external streams)
// - MSAccess
...
begin
  // All "Real BLOB Stream" operations must be performed in a transaction.
...
begin
  ...
  try
    ...
    TFDRDBMSKinds.Firebird:
      begin
        qInsert.SQL.Text := 'insert into {id FDQA_Blob} (blobdata) values (''x'') returning blobdata {into :blobdata}';
        qInsert.Params[0].ParamType := ptOutput;
      end;
    else
      qInsert.SQL.Text := 'insert into {id FDQA_Blob} (blobdata) values (:blobdata)';
    end;
    ...
    // Flush / close the streams. Mandatory for ODBC-based drivers.
    // Optional for other drivers (does nothing).
