...
uses ExpertsUIInternalWizard,
  ToolsAPI, ExpertsUIResStrs, Dialogs;
...
type
  ...
  protected
    ...
    procedure OnLeavingPage(WizardPage: IWizardPage; PageTransition: TPageTransition; var Allow: Boolean); override;    function CanFinish: Boolean; override;
...
begin
  ...
  for I := 1 to Length(LEnabledPages) - 1 do
  begin
...
begin
  ...
  begin
    ...
    begin
      LPageClass := CreatePageClass(LEnabledPages[0]);
      try
...
begin
  ...
  begin
    LPageClass := CreatePageClass(LPageArray[0]);
    try
      AID := LPageClass.Guid;
...
begin
  Assert(FFeaturesWizardPageIntf <> nil);
  ASelected := FFeaturesWizardPageIntf.Selected;
  if ASelected <> -1 then
