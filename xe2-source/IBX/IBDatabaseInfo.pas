...
type
  ...
  protected
    ...
    function GetStringDatabaseInfo(DatabaseInfoCommand: Integer): AnsiString;
  ...
  public
    ...
    property DBFileName: AnsiString read GetDBFileName;
  ...
  TIBDatabaseInfoHelper = class helper for TIBDatabaseInfo
...
function TIBDatabaseInfo.GetDBFileName: AnsiString;
var
  local_buffer: array[0..IBLocalBufferLength - 1] of AnsiChar;
  DatabaseInfoCommand: Byte;
begin
...
function TIBDatabaseInfo.GetDBSiteName: AnsiString;
var
...
var
  ...
  temp_buffer: array[0..IBLocalBufferLength - 2] of AnsiChar;
  DatabaseInfoCommand: Byte;
...
function TIBDatabaseInfo.GetStringDatabaseInfo(DatabaseInfoCommand: Integer): AnsiString;
var
  local_buffer: array[0..IBBigLocalBufferLength - 1] of AnsiChar;
  _DatabaseInfoCommand: Byte;
begin
  ...
  result := AnsiString(@local_buffer[4]);
end;
...
begin
  ...
  Call(GetGDSLibrary.isc_database_info(StatusVector, @FDatabase.Handle, 1, @DatabaseInfoCommand,
                       IBLocalBufferLength, local_buffer), True);
  if local_buffer[0] <> AnsiChar(isc_info_db_SQL_dialect) then
    result := 1
  else begin
    length := GetGDSLibrary.isc_vax_integer(@local_buffer[1], 2);
    result := GetGDSLibrary.isc_vax_integer(@local_buffer[3], length);
...
function TIBDatabaseInfoHelper.GetGDSLibrary: IGDSLibrary;
begin
