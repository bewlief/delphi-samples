...
{                                                             }
{       Copyright (c) 1998-2008 CodeGear                      }
{                                                             }
...
uses
  SysUtils, Classes, Variants, System.Types,
  {$IFDEF MSWINDOWS}
  Windows,
  {$ENDIF}
  {$IFDEF LINUX}
  Libc,
  {$ENDIF}
  IBExternals, IB, IBHeader, IBDatabase,
  IBSQL, Db, IBUtils, IBBlob, IBIntf;
...
type
  ...
  private
    ...
    function AdjustPosition(FCache: TRecordBuffer; Offset: DWORD;
                            Origin: Integer): Integer;
    procedure ReadCache(FCache: TRecordBuffer; Offset: DWORD; Origin: Integer;
                       Buffer: TRecordBuffer);
    procedure ReadRecordCache(RecordNumber: Integer; Buffer: TRecordBuffer;
                              ReadOldBuffer: Boolean);
    procedure WriteCache(FCache: TRecordBuffer; Offset: DWORD; Origin: Integer;
                        Buffer: TRecordBuffer);
...
DefaultFieldClasses: array[TFieldType] of TFieldClass = (
    ...
    nil,                { ftCursor }
    TIBStringField,     { ftFixedChar }
    TIBStringField,     { ftWideString }
...
begin
  if not Assigned(Buffer) then
    GetMem(Buffer, (Size + 1) * SizeOf(PChar));
...
begin
  ...
  begin
    ...
    with p^ do
    begin
      rdFields[j].fdDataType := Qry.Current[i].Data.SqlDef;
      ...
      case rdFields[j].fdDataType of
        ...
        begin
          ...
          begin
            ...
            if (rdFields[j].fdDataScale >= (-4)) then
            begin
            ...
            begin
              ...
              //    Only needs to be done on hte Model buffer which has a RecordNumber of -1
              ...
              begin
                ...
                if Assigned(aField) and
                  ...
                  rdFields[j].fdPersistedFloatField := true;
              end;
              if rdFields[j].fdPersistedFloatField then
              begin
              ...
              begin
                rdFields[j].fdDataSize := SizeOf(tBcd);
                if RecordNumber >= 0 then
          ...
          begin
            ...
            begin
              ...
              if (rdFields[j].fdDataType = SQL_TEXT) then
                ...
                rdFields[j].fdDataLength := min(Length(S) * SizeOf(Char), p^.rdFields[j].fdDataSize);
              LocalData := @s[1];
            end;
          end;
        end;
      ...
      begin
        if (rdFields[j].fdDataType = SQL_VARYING) or
           (rdFields[j].fdDataType = SQL_TEXT) then
        begin
  ...
  WriteRecordCache(RecordNumber, TRecordBuffer(p));
end;
...
begin
  ...
  try
    ReadRecordCache(RecordNumber, TRecordBuffer(NewBuffer), False);
    ReadRecordCache(RecordNumber, TRecordBuffer(OldBuffer), True);
    ...
    WriteRecordCache(RecordNumber, TRecordBuffer(NewBuffer));
...
function TIBCustomDataSet.IsVisible(Buffer: TRecordBuffer): Boolean;
begin
  result := True;
...
begin
  ...
  begin
    ...
    ReadRecordCache(Buff^.rdRecordNumber, TRecordBuffer(Buff), False);
    DataEvent(deRecordChange, 0);
  end;
end;

procedure TIBCustomDataSet.SaveOldBuffer(Buffer: TRecordBuffer);
var
  OldBuffer: Pointer;
        ...
        begin
          ...
          begin
            ...
            case rdFields[j].fdDataType of
              ...
              begin
                SetString(st, PChar(data), rdFields[j].fdDataLength div 2);
...
function TIBCustomDataSet.AdjustPosition(FCache: TRecordBuffer; Offset: DWORD;
                                        Origin: Integer): Integer;
var
...
procedure TIBCustomDataSet.ReadCache(FCache: TRecordBuffer; Offset: DWORD; Origin: Integer;
                                    Buffer: TRecordBuffer);
var
  pCache: TRecordBuffer;
  bOld: Boolean;
begin
  bOld := (FCache = FOldBufferCache);
  pCache := TRecordBuffer(AdjustPosition(FCache, Offset, Origin));
  if not bOld then
...
procedure TIBCustomDataSet.ReadRecordCache(RecordNumber: Integer; Buffer: TRecordBuffer;
                                          ReadOldBuffer: Boolean);
begin
...
procedure TIBCustomDataSet.WriteCache(FCache: TRecordBuffer; Offset: DWORD; Origin: Integer;
                                     Buffer: TRecordBuffer);
var
  pCache: TRecordBuffer;
  bOld: Boolean;
...
procedure TIBCustomDataSet.WriteRecordCache(RecordNumber: Integer; Buffer: TRecordBuffer);
begin
  if RecordNumber >= 0 then
  begin
    if FUniDirectional then
...
begin
  ...
  begin
    ...
    pb^[Field.Offset] := TIBBlobStream.Create(nil);
    fs := pb^[Field.Offset];
    FBlobStreamList.Add(Pointer(fs));
    fs.Mode := bmReadWrite;
    fs.Database := Database;
    ...
    if result and (Buffer <> nil) then
      ...
      begin
        ...
        begin
          ...
          if CurrentRecord^.rdFields[FMappedFieldPosition[Field.FieldNo - 1]].fdDataLength div 2 <= Field.Size then
          begin
            if (Field is TStringfield) and TStringField(Field).FixedChar then
              StrPCopy(Buffer, StringOfChar(' ', Field.Size));
            Move(Data^, Buffer^, fdDataLength);
            if (Field is TStringfield) and TStringField(Field).FixedChar then
              PChar(Buffer)[Field.Size] := #0
            else
              PChar(Buffer)[fdDataLength div 2] := #0;
...
function TIBCustomDataSet.InternalGetRecord(Buffer: TRecordBuffer; GetMode: TGetMode;
  DoCheck: Boolean): TGetResult;
begin
  result := grError;
  case GetMode of
...
begin
  ...
  pbd := PBlobDataArray(TRecordBuffer(Buff) + FBlobCacheOffset);
  j := 0;
  for i := 0 to FieldCount - 1 do
    if Fields[i].IsBlob then
    begin
...
begin
  ...
  FBEnd := 0;
  FOBEnd := 0;
  if Assigned(FBufferCache) then
    ReallocMem(FBufferCache, 0);
  if Assigned(FOldBufferCache) then
...
begin
  ...
  try
    ...
    for i := 0 to FQSelect.Current.Count - 1 do
      ...
      begin
        { Get the field name }
        FieldAliasName := String(aliasname);
        RelationName := String(relname);
        FieldName := String(sqlname);
        FieldSize := 0;
        FieldPrecision := 0;
        ...
        case sqltype and not 1 of
          ...
          SQL_VARYING, SQL_TEXT:
          begin
            FieldSize := sqllen div FQSelect.Current[i].CharsetSize;
...
procedure TIBCustomDataSet.InternalInitRecord(Buffer: TRecordBuffer);
begin
  CopyRecordBuffer(FModelBuffer, Buffer);
end;
...
procedure TIBCustomDataSet.SetBookmarkFlag(Buffer: TRecordBuffer; Value: TBookmarkFlag);
begin
    ...
    begin
      ...
      begin
        ...
        case AParams[i].DataType of
          ftBytes:
          begin
            GetMem(Buffer,AParams[i].GetDataSize);
            try
              AParams[i].GetData(Buffer);
              Param.AsPointer := Buffer;
            finally
              FreeMem(Buffer);
            end;
          end;
...
function TIBDSBlobStream.Read(var Buffer; Count: Longint): Longint;
  ...
  procedure DecodeStream(CodePage : Integer);
  var
    ss : TBytesStream;
    s : String;
  ...
  begin
    ...
    try
      ...
      s := Encoding.GetString(ss.bytes, 0, ss.Size);
...
begin
  ...
  begin
    if FBlobStream.Database.CharacterSetCodePage = 0 then
      DecodeStream(DefaultSystemCodePage)
    else
      DecodeStream(FBlobStream.Database.CharacterSetCodePage);
  end
  else
...
function TIBDSBlobStream.Seek(Offset: Longint; Origin: Word): Longint;
  ...
  function DecodeLength(CodePage : Integer) : LongInt;
  var
    ss : TBytesStream;
    s : String;
    FSS_ENC : TMBCSEncoding;
  begin
    ss := TBytesStream.Create;
    FSS_ENC := TMBCSEncoding.Create(CodePage);
    try
      FBlobStream.SaveToStream(ss);
      s := FSS_ENC.GetString(ss.bytes);
      Result := StrLen(PChar(s)) * sizeof(Char);
    finally
      FSS_ENC.Free;
      ss.free;
...
begin
  ...
  begin
    if FBlobStream.Database.CharacterSetCodePage = 0 then
      Result := DecodeLength(DefaultSystemCodePage)
    else
...
function TIBDSBlobStream.Write(const Buffer; Count: Longint): Longint;
  ...
  begin
    ...
    try
      ...
      bt := FSS_ENC.Convert(FSS_ENC.Unicode, FSS_ENC, bt);
