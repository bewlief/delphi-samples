...
begin
  ...
  try
    ...
    begin
      ...
      if DatabaseInfo.FullODS > 11.1 then
        if POS('TEMPORARY', qryTables.FieldByName('RDB$RELATION_TYPE').AsString) > 0 then
          TableBase := 'CREATE GLOBAL TEMPORARY TABLE %s';

      if NewName <> '' then  {do not localize}
        FMetaData.Add(Format(TableBase, [QuoteIdentifier(FDatabase.SQLDialect,NewName)])) {do not localize}
      else
        FMetaData.Add(Format(TableBase, [QuoteIdentifier(FDatabase.SQLDialect,RelationName)]));  {do not localize}
    ...
    {  Check first for computed fields, then domains.
      ...
      begin
        ...
        if not ((Copy(qryTables.FieldByName('RDB$FIELD_NAME1').AsTrimString, 1, 4) = 'RDB$') and {do not localize}
          CharInSet(qryTables.FieldByName('RDB$FIELD_NAME1').AsTrimString[5], ['0'..'9'])) and {do not localize}
    ...
    if (DatabaseInfo.FullODS > 11.1) then
      case qryTables.FieldByName('RDB$Flags').AsInt64 of
        11 : OnCommit := ' ON COMMIT DELETE ROWS';
        19 : OnCommit := ' ON COMMIT PRESERVE ROWS';
        else OnCommit := '';
      end;
...
var
  ...
  var
    ...
    length: Integer;
    _DatabaseInfoCommand: AnsiChar;
  begin
    _DatabaseInfoCommand := AnsiChar(DatabaseInfoCommand);
    FDatabaseInfo.Call(Database.GDSLibrary.isc_database_info(StatusVector, @FDatabase.Handle, 1, @_DatabaseInfoCommand,
                           IBLocalBufferLength, local_buffer), True);
    length := Database.GDSLibrary.isc_vax_integer(@local_buffer[1], 2);
    result := Database.GDSLibrary.isc_vax_integer(@local_buffer[3], length);
  end;
  ...
  try
    ...
    begin
      ...
      Buffer := Buffer + Format('GROUP_COMMIT_WAIT_TIME = %d',   {do not localize}
          [GetLongDatabaseInfo(isc_info_wal_ckpt_interval)]);
      Buffer := Buffer + PrintSet(SetUsed);
  ...
  try
    ...
    begin
      ...
      Line := Line + Format('FOREIGN KEY (%s) REFERENCES %s ', [    {do not localize}
        ...
        QuoteIdentifier(FDatabase.SQLDialect, Trim(qryForeign.FieldByName('RELC2_RELATION_NAME').AsTrimString))]);  {do not localize}
...
begin
  ...
  FMetaData.Clear;
  DefaultCharSet := TIBSQL.Create(FTransaction);
  try
