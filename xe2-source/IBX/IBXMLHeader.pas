...
type
 ...
 PVary = ^TVary;
  ...
  Tib_xmlda = record
    xmlda_file_name : PAnsiChar; { pointer to a char string containing the name of the
					    file used by xml_fetch(), ignored by the buffer 
					    function }
	  xmlda_header_tag : PAnsiChar; { points to the string which is printed out as the
					    header tag }
	  xmlda_database_tag : PAnsiChar; { points to the string which is printed
                                 out as the database tag in the xml file }
	  xmlda_table_tag : PAnsiChar; { points to the string which is printed
                               out as the tablename tag in the xml file }
	  xmlda_row_tag : PAnsiChar; { points to the string which is printed
    ...
    xmlda_temo_buffer : ^PAnsiChar; { internal use only, used for storing the String array
					    from fetch() }
...
type
  ...
  Tisc_dsql_xml_buffer_fetch = function(status : PISC_STATUS;
                                   stmt: PISC_STMT_HANDLE;
		                               buffer : PAnsiChar; buffer_size : Integer;
                                   da_version: USHORT;
                                   sqlda: PXSQLDA;
                                   ib_xmlda: PIB_XMLDA): Integer; stdcall;
