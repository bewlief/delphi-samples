...
uses IBUtils, IB, IBXConst, IBIntf{, Windows};
        ...
        if (Token = 'CREATE') and
          ...
          if (Token = 'DROP') and (Token1 = 'DATABASE') then    {do not localize}
            ...
            if ((Token = 'DROP') and (Token1 = 'JOURNAL')) or {do not localize}
               ...
               (FTokens.Count > 2) and (AnsiUpperCase(FTokens[2]) = 'INDEX')) then  {do not localize}
              Result := stmtDDL
            else
...
begin
  ...
  ImportIndex := 0;
  FileName := FTokens[1];
  if CharInSet(FileName[1], [QUOTE, DBL_QUOTE]) then
    Delete(FileName, 1, 1);
  if CharInSet(FileName[Length(FileName)], [QUOTE, DBL_QUOTE]) then
  ...
  if AnsiCompareText('AUTODDL', FSQLParser.CurrentTokens[1]) = 0 then    {do not localize}
    ...
    if AnsiCompareText('STATISTICS', FSQLParser.CurrentTokens[1]) = 0 then {do not localize}
      ...
      if (AnsiCompareText('NAMES', FSQLParser.CurrentTokens[1]) = 0) then  {do not localize}
          ...
          if AnsiCompareText('SAVEPOINTS', FSQLParser.CurrentTokens[1]) = 0 then {do not localize}
...
var
  ...
  function GetSavepointName : AnsiString;
  begin
    Result := '';
    if ((FSQLParser.CurrentTokens.Count = 4) and
      ...
      Result := AnsiString(FSQLParser.CurrentTokens[3]);
    ...
    if ((FSQLParser.CurrentTokens.Count = 5) and {do not localize}
      ...
      Result := AnsiString(FSQLParser.CurrentTokens[4]);
  end;
      ...
      begin
          ...
          begin
            ...
            try
              if Assigned(Encoding) then
                bt := Encoding.GetBytes(SQLText + #0)
              else
                bt := BytesOf(AnsiString(SQLText + #0));
        ...
        case AKind of
          ...
          stmtReleaseSavePoint :
            FTransaction.ReleaseSavepoint(AnsiString(FSQLParser.CurrentTokens[1]));
          stmtStartSavepoint :
            FTransaction.StartSavepoint(AnsiString(FSQLParser.CurrentTokens[1]));
          stmtBatchStart :
          begin
            InBatch := true;
            FBatchCount := 0;
...
begin
  ...
  begin
    ...
    if not Assigned(FTransaction) and Assigned(Value) then
      FTransaction := Value.DefaultTransaction;
...
begin
  ...
  if not Assigned(FDatabase) and Assigned(Value) then
    FDatabase := Value.DefaultDatabase;
  if Assigned(FDMLQuery) then
...
begin
  Result := Text;
  if CharInSet(Result[1], [Quote, DBL_QUOTE]) then
  begin
    Delete(Result, 1, 1);
