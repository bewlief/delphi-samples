...
type
  ...
  public
    ...
    constructor Create(GDSLibrary : IGDSLibrary);
...
{ TIBBlobStream }
constructor TIBBlobStream.Create(GDSLibrary : IGDSLibrary);
begin
  inherited Create;
...
begin
  ...
  Call(FBase.Database.GDSLibrary.isc_create_blob2(StatusVector, DBHandle, TRHandle, @FHandle, @FBlobID,
                       0, nil), True);
  IBBlob.WriteBlob(@FHandle, FBuffer, FBlobSize, FBase.Database.GDSLibrary);
...
begin
  ...
  try
    GetBlobInfo;
    SetSize(FBlobSize);
    IBBlob.ReadBlob(@FHandle, FBuffer, FBlobSize, FBase.Database.GDSLibrary);
  except
