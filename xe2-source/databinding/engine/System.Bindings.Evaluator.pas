...
function Compile(const Source: string; const RootScope: IScope = nil): ICompiledBinding;
begin
  with TEvaluator.Create do
    try
      Result := Compile(Source, RootScope);
    finally
      Free;
    end;
end;
...
begin
  ...
  case cp^ of
    ...
    begin
      ...
      SetString(FStringToken, start, cp - start);
      FFloatToken := StrToFloat(FStringToken);
...
begin
  ...
  if Supports(TopScope, IRttiChild, LRttiChild) and
     not TBindingSearch.PermitsObjectWrapper(LRttiChild.Member) then
    PopUntil(RootScope);
end;
