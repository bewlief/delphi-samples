...
begin
  ...
  begin
    ...
    if Member is TRttiProperty then
      LTypeKind := (Member as TRttiProperty).PropertyType.TypeKind
    else
    if Member is TRttiMethod then
      LTypeKind := (Member as TRttiMethod).ReturnType.TypeKind
    else
    if Member is TRttiIndexedProperty then
      LTypeKind := (Member as TRttiIndexedProperty).PropertyType.TypeKind
    else
    if Member is TRttiField then
      LTypeKind := (Member as TRttiField).FieldType.TypeKind;

    Result := LTypeKind = tkClass;
  end;
end;
...
var
  ...
  WrprPlaceholder: IPlaceholder;
  Wrpr: IInterface;
begin
  if Assigned(StartScopeEnumerable) then
    for Wrpr in StartScopeEnumerable do
    begin
      if Supports(Wrpr, IPlaceholder, WrprPlaceholder) then
        WrprPlaceholder.Attach(WrprPlaceholder.Attachment);
