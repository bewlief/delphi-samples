...
uses Winapi.ActiveX, Winapi.Windows, System.Variants, System.Win.ComObj, Winapi.msxml, Xml.xmldom, System.Classes;
...
var
  ...
  // Global property to true to allow XML documents referencing DTDs to be loaded (set it to false).
  MSXML6_ProhibitDTD: Boolean = true;
...
begin
  ...
  // Allow DTD's to be loaded when the user has set the MSXML_ProhibitDTD option
  if not MSXML6_ProhibitDTD then
  begin
    try
      (Result as IXMLDOMDocument2).SetProperty('ProhibitDTD', False);
    except on E: EOleError do
      MSXML6_ProhibitDTD := True;
    end;
  end;
end;
