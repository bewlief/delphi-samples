...
{$ELSE}
  AString := UTF8Encode(Value);
  TMemoryStream(FSourceStream).SetSize(Length(AString));
  FSourceStream.Write(AString[1], Length(AString));
...
begin
  ...
  try
    ...
    begin
      ...
      if URI = '' then
        URIMap[Index].URI := 'urn:' + AppURI + TypeInfoName(GetTypeData(AClass.ClassInfo).UnitName) { do not localize }
      else
...
var
  ...
  begin
    ...
    while Size >= 0  do
    begin
      Inc(IntPtr(P), Length(P^) + 1);
      Dec(Size);
    end;
    Result := TypeInfoName(P^);
  end;
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        UnitName := TypeInfoName(GetTypeData(Info).DynUnitName);
      ...
      begin
        ...
        URIMap[Index].URI := 'urn:' + AppURI + TypeInfoName(GetTypeData(Info).UnitName)
      else
    ...
    begin
      URIMap[Index].Name := TypeInfoName(Info.Name);
    end;
    URIMap[Index].ExtName := ExtName;
