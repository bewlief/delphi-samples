...
{$ENDIF}
  System.SysUtils, System.Classes, Soap.WebNode, Soap.WSDLNode, System.Types, Soap.IntfInfo, Soap.WSDLIntf, Soap.SOAPAttachIntf;
...
{$ELSE}
  {$IFDEF INDY_10}
     IdAssignedNumbers,
  {$ENDIF}
     IdIntercept, IdException, IdURI, IdGlobal, IdHeaderList, IdHTTPHeaderInfo, IdAuthenticationDigest, IdSSLOpenSSL,
{$ENDIF}
  System.Variants, Soap.SOAPConst, Xml.XMLDoc, Xml.XMLIntf, Soap.InvokeRegistry, Soap.WSDLItems,
  Soap.SOAPAttach, Soap.UDDIHelper, System.SyncObjs;
...
procedure THTTPReqRespHelper.RaiseCheck(ErrCode: DWORD; ShowSOAPAction: Boolean);
var
  S: string;
begin
  if (ErrCode <> 0) then
...
procedure THTTPReqResp.SetMimeBoundary(const Value: AnsiString);
begin
  FMimeBoundary := Value;
end;

function THTTPReqResp.GetMimeBoundary: AnsiString;
begin
...
begin
  ...
  if (HttpQueryInfo(Pointer(Context), HTTP_QUERY_CONTENT_TYPE, @FContentType[1], Size, Index)) then
  ...
  FMimeBoundary := GetMimeBoundaryFromType(AnsiString(FContentType));
...
{$IFDEF UNICODE}
      ...
      Check(not InternetReadFile(Pointer(Context), bytes, Size, Downloaded));
...
function THTTPReqResp.HandleWinInetError(LastError: DWord; Request: HINTERNET): DWord;
begin
...
{$ENDIF}
    ...
    begin
      { This instructs WinInet to pick the first (a random?) client cerficated }
