...
{$ENDIF}
    ...
    begin
      ...
      if IsNull then
        CreateNULLNode(RootNode,ElemNode, NodeName)
      else
        ElemNode := CreateScalarNodeXS(RootNode, Node, NodeName, NodeNamespace, TypeName, S);
    end;
  end;
...
begin
  ...
  begin
    XMLDoc.SaveToXML(XMLUTF8String);
    StrStr := TStringStream.Create(XMLUTF8String);
    try
...
begin
  ...
  begin
    ...
    if RegInfoStr <> '' then
    begin
      if (RegInfoStr[1] = INFOATTR_BEG_CHAR) and
...
begin
  ...
  if ReturnName <> '' then
  begin
    if (ReturnName[1] = INFOATTR_BEG_CHAR) then
...
begin
   Stream := TMemoryStream.Create;
  try
    Stream.Write(Resp[1], Length(Resp)*2);
...
{$ENDIF}
            ...
            if ExtPropName[1] = INFOATTR_BEG_CHAR then
            begin
...
{$IFDEF TIBURON_UP}
    tkUString:
      SetUnicodeStrProp(Instance, PropInfo, SoapData);
{$ENDIF}
    tkInt64:
      begin
