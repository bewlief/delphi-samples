...
procedure GenProxy(const WSDLFileName: String; OutFile: String;
                   const Directory, StringType, BaseClassName: string;
...
var
  ...
  var
    ...
    I: Integer;
    CurrCommandLineFlag, CurrDescription: string;
  begin
    OutputStr(Usage);
    ...
    begin
      ...
      begin
        ...
        // Remove pick letter indicator from descriptions
        // since they are shared with the IDE/UI
        CurrDescription := ReplaceStr(OptDescriptions[CurrFlag], '&', '');
        OutputStr(Format(sOptionUsageFormatString, [CurrCommandLineFlag, CurrDescription]));
      end;
    end;
    ...
    OutputStr(Format(sOptionUsageFormatString, [OptCommandLineFlags[wfForceSOAP11],
                                                ReplaceStr(OptDescriptions[wfForceSOAP11], '&', '')]));
    OutputStr(Format(sOptionUsageFormatString, [OptCommandLineFlags[wfForceSOAP12],
                                                ReplaceStr(OptDescriptions[wfForceSOAP12], '&', '')]));
    OutputStr(sLineBreak);
  ...
  {$ENDIF}
            ...
            begin
              try
                ...
                GenProxy(InFileName, OutFileName, Directory,
