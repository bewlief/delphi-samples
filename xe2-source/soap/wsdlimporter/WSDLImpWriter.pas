...
const
  RevString = '- $Rev: 37707 $';  { Do not localize }
  { Default Settings }
...
const
  DateString= '$Date: 2011-03-23 17:36:51 -0700 (Wed, 23 Mar 2011) $'; { Do not localize }
...
type
  ...
  private
    ...
    function  AddElement(const TypeDef: IXMLElementDef;
                         const ElementType: TElementType;
...
{$IFDEF LOG_TYPES_READING}
  LogTypeReading(Item);
{$ENDIF}
  Result := AddElement(Item, ElementType, CompositorId, Context);
end;
...
var
  AName, ANamespace: DOMString;
  isGlobal, hasName, hasRef, isAnonymous: Boolean;
  Part: TWSDLPart;
begin
...
{$IFEND}
    ...
    else if ElementType = etChoice then
      PartFlags := PartFlags + [pfOptional];
...
procedure TWSDLWriter.SetDirectory(Directory: String);
begin
...
function GetTypeKey(const AName: DOMString; ANamespace: DOMString): string; overload;
begin
