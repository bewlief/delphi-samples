...
uses
  ...
  FMX.Objects3D,  FMX.DAE.Schema,
  System.Math, FMX.Import, XML.XMLIntf;
...
type
  ...
  private
    ...
    FChilds: TDAEVisualNodeDynArray;
    FId : string;
  ...
  public
    ...
    property Childs: TDAEVisualNodeDynArray read FChilds;
    property Id: string read FId;
    destructor Destroy; override;
  end;
...
begin
  ...
  begin
    ...
    FExporter := deUnknown;
{$IFDEF FPCCOMP}
    if Pos(WideString('Maya'), LTool) > 0 then
{$ELSE}
    if Pos('Maya', LTool) > 0 then
{$ENDIF}
      FExporter := deMaya else
{$IFDEF FPCCOMP}
    if Pos(WideString('Max'), LTool) > 0 then
{$ELSE}
    if Pos('Max', LTool) > 0 then
{$ENDIF}
      FExporter := de3DStudioMax else
  end;
end;

procedure SetVetexId(var AVertex:TGEVertexID; const AMode: TSourceMode; const Aid: Integer);
begin
  case AMode of
   smVertex:
...
begin
  ...
  if (LPos > 0) then
  begin
    LFileName := Copy(AName, 0 , LPos + 3);

    if SetLocalPath(ExtractFilePath(FFileName), LFileName) then
    begin
      LNodeName := Copy(AName, LPos + 4 , Length(AName) - 3 - LPos);
...
begin
  ...
  if Pos(LFilePred, LFileName) > 0 then
  begin
...
procedure TDAEModel.ImportFromImageLib(const AImageLib
...
begin
  ...
  begin
    ANode := FAllNodes[i];
    id := Length(ANode.Childs);
    SetLength(ANode.FChilds , id + Length(ANode.FNodeUrls));
    for j := 0 to High(ANode.FNodeUrls) do
    begin
      ANode.FChilds[id] := FindNodeById(ANode.FNodeUrls[j]);
      inc(id);
    end;
  end;
end;
...
begin
  ...
  SetLength(Result.FChilds, ANode.Node.Count);
  for i := 0 to ANode.Node.Count-1 do
    Result.FChilds[i] := GetNode(ANode.Node[i]);
  ...
  begin
    ...
    UpdateMatrix(AxisRotationToMatrix3D(Vector3D(LArr[0],LArr[1],LArr[2]),LArr[3]));
  end;
