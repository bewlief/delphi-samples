...
type
  ...
  private
    ...
    FSuccess: Boolean;
    FValue: UTF8String;
    constructor Create(const AValue: UTF8String; AIndex, ALength: Integer; ASuccess: Boolean);
  ...
  private
    ...
    constructor Create(ANotifier: IInterface; const AValue: UTF8String;
  ...
  private
    FList: TArray<TMatch>;
    constructor Create(ANotifier: IInterface; const Input: UTF8String;
...
uses
  System.Classes, System.SysUtils, System.Variants, System.RegularExpressionsAPI, System.RegularExpressionsConsts;
...
function UTF8IndexToUnicode(const S: UTF8String; AIndex: Integer): Integer;
var
  I: Integer;
begin
  if AIndex > Length(S) then
...
constructor TGroup.Create(const AValue: UTF8String; AIndex, ALength: Integer; ASuccess: Boolean);
begin
  FIndex := AIndex;
  FLength := ALength;
  FSuccess := ASuccess;
...
begin
  Result := UTF8IndexToUnicode(FValue, FIndex);
end;

function TGroup.GetLength: Integer;
begin
  if (FIndex + FLength) > System.Length(FValue) then
    Result := System.Length(String(FValue)) - GetIndex + 1
  else
    Result := UTF8IndexToUnicode(FValue, FIndex + FLength) - GetIndex;
 end;

function TGroup.GetValue: string;
begin
  Result := string(Copy(FValue, FIndex, FLength));
end;
...
begin
  case VarType(Index) of
    varString, varUString, varOleStr:
      LIndex := GetRegEx(FNotifier).NamedGroup(UTF8String(Index));
...
constructor TMatch.Create(ANotifier: IInterface; const AValue: UTF8String; AIndex,
...
begin
  LRegEx := GetRegEx(FNotifier);
  LRegEx.Replacement := UTF8String(Pattern);
  Result := string(LRegEx.ComputeReplacement);
end;

{ TMatchCollection }

constructor TMatchCollection.Create(ANotifier: IInterface; const Input: UTF8String;
  AOptions: TRegExOptions; StartPos: Integer);
var
  Count: Integer;
  LResult: Boolean;
...
begin
  ...
  FRegEx.RegEx := UTF8String(Pattern);
...
begin
  FRegEx.Subject := UTF8String(Input);
  Result := FRegEx.Match;
end;
...
begin
  FRegEx.Subject := UTF8String(Input);
  FRegEx.Start := UnicodeIndexToUTF8(Input, StartPos);
...
procedure TRegEx.InternalOnReplace(Sender: TObject; var ReplaceWith: UTF8String);
var
  Match: TMatch;
begin
  ...
  begin
    ...
    ReplaceWith := UTF8String(FMatchEvaluator(Match));
  end;
end;
...
begin
  Result := TMatchCollection.Create(FNotifier, UTF8String(Input), FOptions, 1);
end;

function TRegEx.Matches(const Input: string; StartPos: Integer): TMatchCollection;
begin
  Result := TMatchCollection.Create(FNotifier, UTF8String(Input), FOptions,
...
begin
  FRegEx.Subject := UTF8String(Input);
  FRegEx.Start := UnicodeIndexToUTF8(Input, StartPos);
  FRegEx.Stop := UnicodeIndexToUTF8(Input, StartPos + Length) - 1;
  LSuccess := FRegEx.MatchAgain;
  if LSuccess then
    Result := TMatch.Create(FNotifier, FRegEx.Subject,
      FRegEx.MatchedOffset, FRegEx.MatchedLength, LSuccess)
...
begin
  FRegEx.Subject := UTF8String(Input);
  FRegEx.Replacement := UTF8String(Replacement);
  FRegEx.ReplaceAll;
...
begin
  ...
  try
    FRegEx.Subject := UTF8String(Input);
    FRegEx.SplitCapture(List, Count, UnicodeIndexToUTF8(Input, StartPos));
