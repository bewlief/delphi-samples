...
{$ENDIF MSWINDOWS}
    ...
    class function DoCopy(const SourceFileName, DestFileName: string;
        const Overwrite: Boolean): Boolean; inline; static;
...
{$IFDEF MSWINDOWS}
    class procedure Replace(const SourceFileName, DestinationFileName,
        DestinationBackupFileName: string); overload; inline; static;
...
uses
{$IFDEF POSIX}
  Posix.Base, Posix.Stdio, Posix.Stdlib, Posix.Unistd, Posix.SysStat, Posix.Time,
  Posix.Utime,
{$ENDIF}
  System.StrUtils, System.DateUtils, System.Masks, System.Math, System.RTLConsts;
  ...
  PreCallback :=
    function (const Path: string; const FileInfo: TSearchRec): Boolean
      ...
      var
        Ch: Char;
      begin
        Ch := SourceDirName[Length(SourceDirName)];
...
class procedure TDirectory.CreateDirectory(Path: string);
begin
  Path := TPath.DoGetFullPath(Path);
  CheckCreateDirectoryParameters(Path);
...
{$IFDEF POSIX}
var
  LBuffer: array[0 .. MAX_PATH] of AnsiChar;

begin
  SetLastError(0);

  { Get current directory }
  if getcwd(LBuffer, SizeOf(LBuffer)) <> nil then
...
begin
  ...
  ErrCode := __chdir(PAnsiChar(UTF8String(Path)));
...
begin
  ...
  begin
    ErrCode := stat(PAnsiChar(LFileName), LStatBuf);
  ...
  try
    ...
    utime(PAnsiChar(LFileName), LBuf);
  except
...
begin
  ...
  try
    try
      ...
      LFileStream.WriteBuffer(Pointer(Buff)^, Length(Buff));
    except
  ...
  try
    try
      ...
      begin
        ...
        UTFStr := UTF8String(AnsiString(Contents));
        if UTFStr = UTF8String(Contents) then
        begin
          LFileStream.Seek(0, TSeekOrigin.soEnd);
        ...
        begin
          ...
          LFileStream.ReadBuffer(Pointer(Buff)^, Length(Buff));
          ...
          LFileStream.WriteBuffer(Pointer(Preamble)^, Length(Preamble));
          LFileStream.WriteBuffer(Pointer(Buff)^, Length(Buff));
...
begin
  ...
  if (not DeleteFile(Path)) and
     (GetLastError = ERROR_SHARING_VIOLATION) then
    raise EInOutError.Create(SysErrorMessage(ERROR_SHARING_VIOLATION));
end;
{$ENDIF}
{$IFDEF POSIX}
var
...
begin
  ...
  if unlink(PAnsiChar(UTF8String(Path))) = -1 then
    ErrCode := GetLastError()
  else
    ErrCode := 0;

  { Throw an error only for EACCES, EBUSY and EPERM}
  if (ErrCode = EACCES) or (ErrCode = EBUSY) or (ErrCode = EPERM) then
    raise EInOutError.Create(SysErrorMessage(ErrCode));
end;
{$ENDIF}
...
begin
  ...
  try
    LSource := TFileStream.Create(SourceFileName, fmOpenRead);
  except
    SetLastError(EACCES);
    Exit(false);
  end;
...
begin
  if Exists(Path) then
    Mode := System.SysUtils.fmOpenReadWrite
  else
...
begin
  Buff := DoReadAllBytes(Path);
  BOMLength := Length(Encoding.GetPreamble);
  Result := Encoding.GetString(Buff, BOMLength, Length(Buff) - BOMLength);
end;
...
{$IFDEF POSIX}
  ...
  begin
    Result := chmod(PAnsiChar(UTF8String(FileName)), Attr);
  end;
{$ENDIF POSIX}
...
begin
  ...
  Stream.ReadBuffer(Pointer(Buff)^, Length(Buff));
...
begin
  TPath.CheckPathLength(Path, MAX_PATH{$IFDEF MSWINDOWS}- TFile.FCMinFileNameLen{$ENDIF}); // Check path length
...
{$ENDIF}
{$IFDEF POSIX}
  { On POSIX try to rename() the file. Check for EXDEV error (diff file systems) and copy/delete then }
  if __rename(PAnsiChar(UTF8String(SourceFileName)), PAnsiChar(UTF8String(DestFileName))) = EXDEV then
{$ENDIF}
  begin
...
{$ENDIF}
  ...
  case Mode of
    ...
    TFileMode.fmCreate:
      try
        Result := TFileStream.Create(Path, System.Classes.fmCreate, LFileStrmShare);
      except
        on E: EFileStreamError do
          raise EInOutError.Create(E.Message);
      end;
...
begin
  ...
  try
    ...
    LFileStream.Seek(0, TSeekOrigin.soBeginning);
    LFileStream.WriteBuffer(Pointer(Bytes)^, Length(Bytes));
  finally
    LFileStream.Free;
  end;
end;
...
begin
  ...
  begin
    ...
    if Extension <> '' then // DO NOT LOCALIZE
      if Extension[1] <> ExtensionSeparatorChar then
        CopyIdx := 1
      else
        CopyIdx := 2
  ...
  begin
    ...
    begin
      ...
      if IsDriveRooted(FileName) then
        ...
        if (FileName[SeparatorIdx - 1] = VolumeSeparatorChar) then
...
begin
  ...
  if (Path <> '') and (Path[1] = DirectorySeparatorChar) then // DO NOT LOCALIZE
...
begin
  { Check the first char of the path (must be /) for rooted paths on Unixes }
  Result := (Path <> '') and (Path[1] = DirectorySeparatorChar); // DO NOT LOCALIZE
end;
{$ENDIF POSIX}
...
{$IFDEF MSWINDOWS}
const
  CFirstDrive: AnsiChar = 'A'; // DO NOT LOCALIZE
  CLastDrive: AnsiChar = 'Z'; // DO NOT LOCALIZE
var
  PPath: PChar;
  Drives: Cardinal;
  StartIdx: Integer;
...
begin
  ...
  begin
    ...
    PathDrive := AnsiChar(UpCase(PPath[0]));

    if CharInSet(PathDrive, [CFirstDrive..CLastDrive]) then
    begin
...
begin
  TDirectory.InternalCheckDirPathParam(Path, False);
...
begin
  ...
  for i := 1 to CFileNameLen do
    ...
    begin
      ...
      with CIntervals[Interval] do
        Result := Result + Chr(BaseCharCode + Random(RandomCount));
    end
    else
      Result := Result + ExtensionSeparatorChar;
end;
...
{$IFDEF MSWINDOWS}
type
  TDriveSet = set of AnsiChar;
const
  CDriveSet: TDriveSet = ['A'..'Z'];
var
  PPath: PChar;
  StartIdx: Integer;
begin
  ...
  begin
    StartIdx := GetPosAfterExtendedPrefix(Path);
    PPath := PChar(Path) + StartIdx - 1;
    Result := CharInSet(UpCase(PPath[0]), CDriveSet) and (PPath[1] = VolumeSeparatorChar); // DO NOT LOCALIZE
  end;
end;
{$ENDIF MSWINDOWS}
{$IFDEF POSIX}
...
begin
  ...
  if Path <> '' then
  begin
    LHeader := Copy(Path, 1, Length(FCExtendedUNCPrefix));
    if not (LHeader = FCExtendedUNCPrefix) then
    begin
      LHeader := Copy(Path, 1, Length(FCExtendedPrefix));
      if LHeader = FCExtendedPrefix then
