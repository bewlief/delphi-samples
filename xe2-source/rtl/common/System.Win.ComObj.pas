...
{$IFDEF LINUX}
  WinUtils,
{$ENDIF}
  System.Variants, Winapi.Windows, Winapi.ActiveX, System.SysUtils, System.Classes;
...
uses System.Win.ComConst, System.VarUtils;
...
begin
  ...
  try
    ...
    if FShowErrors and (ExceptObject is Exception) then
      ...
      begin
        if (Message <> '') and (AnsiLastChar(AnsiString(Message)) > '.') then
          Message := Message + '.';
...
begin
  ...
  DispatchInvoke(Dispatch, @DispDesc.CallDesc, @DispIDs,
{$IFDEF CPUX86}
                 @Params, 
{$ELSE}
                 Params, 
...
begin
  ...
  // Copy back data passed by reference
  // NOTE: Not really required as all copies point to same ref.
  for I := LFistArrItem to LLastArrItem do
  begin
    LPVarArgIn := @ADispParams.rgvarg[I];
    if (LPVarArgIn.vt and varByRef) <> varByRef then
      Continue;
    POleVariant(LPVarArgIn)^ := LVarArray[LLastArrItem - I];
  end;
