...
uses
  System.SysUtils, System.Classes, System.RegularExpressionsAPI;
...
type
  TPerlRegExReplaceEvent = procedure(Sender: TObject; var ReplaceWith: UTF8String) of object;
...
type
  ...
  private    // *** Property storage, getters and setters
    ...
    function GetMatchedText: UTF8String;
    function GetMatchedLength: Integer;
    function GetMatchedOffset: Integer;
    procedure SetOptions(Value: TPerlRegExOptions);
    procedure SetRegEx(const Value: UTF8String);
    function GetGroupCount: Integer;
    function GetGroups(Index: Integer): UTF8String;
  ...
  private    // *** Variables used by PCRE
    ...
    FStoredGroups: array of UTF8String;
    function GetSubjectLeft: UTF8String;
    function GetSubjectRight: UTF8String;
  protected
    procedure CleanUp;
  ...
  public
    ...
    function NamedGroup(const Name: UTF8String): Integer;
    ...
    property MatchedText: UTF8String read GetMatchedText;
        // The matched text
    ...
    property SubjectLeft: UTF8String read GetSubjectLeft;
        // Part of the subject to the left of the match
    property SubjectRight: UTF8String read GetSubjectRight;
        // Part of the subject to the right of the match
  public
    property Options: TPerlRegExOptions read FOptions write SetOptions;
        // Options
    property RegEx: UTF8String read FRegEx write SetRegEx;
        // The regular expression to be matched
    property Replacement: UTF8String read FReplacement write FReplacement;
    ...
    property OnReplace: TPerlRegExReplaceEvent read FOnReplace write FOnReplace;
        // Triggered by Replace and ReplaceAll just before the replacement is done, allowing you to determine the new UTF8String
  end;
...
begin
  ...
  if FPattern = nil then
    raise ERegularExpressionError.CreateResFmt(@SRegExExpressionError, [ErrorOffset, AnsiString(Error)]);
  FCompiled := True
end;
...
function TPerlRegEx.ComputeReplacement: UTF8String;
var
  Mode: AnsiChar;
  S: UTF8String;
  I, J, N: Integer;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        case Mode of
          'L', 'l': Backreference := UTF8String(AnsiLowerCase(string(Backreference)));
          'U', 'u': Backreference := UTF8String(AnsiUpperCase(string(Backreference)));
          'F', 'f': Backreference := UTF8String(FirstCap(string(Backreference)));
          'I', 'i': Backreference := UTF8String(InitialCaps(string(Backreference)));
        end;
        if S <> '' then
        begin
...
function TPerlRegEx.GetMatchedText: UTF8String;
begin
...
function TPerlRegEx.GetGroups(Index: Integer): UTF8String;
begin
  if not FoundMatch then
    raise ERegularExpressionError.CreateRes(@SRegExMatchRequired);
...
function TPerlRegEx.GetSubjectLeft: UTF8String;
begin
  Result := Copy(Subject, 1, Offsets[0]-1);
end;

function TPerlRegEx.GetSubjectRight: UTF8String;
begin
...
begin
  ...
  Result := OffsetCount > 0;
  // Convert offsets into UTF8String indices
  if Result then
  begin
...
function TPerlRegEx.NamedGroup(const Name: UTF8String): Integer;
begin
  Result := pcre_get_stringnumber(FPattern, PAnsiChar(Name));
end;

function TPerlRegEx.Replace: UTF8String;
begin
  if not FoundMatch then
    raise ERegularExpressionError.CreateRes(@SRegExMatchRequired);
...
procedure TPerlRegEx.SetRegEx(const Value: UTF8String);
begin
...
procedure TPerlRegEx.SetSubject(const Value: UTF8String);
begin
  FSubject := Value;
...
begin
  ...
  if (Limit = 1) or not Match then
    Strings.Add(string(Subject))
  else
  begin
    Offset := 1;
    Count := 1;
    repeat
      Strings.Add(string(Copy(Subject, Offset, MatchedOffset - Offset)));
      Inc(Count);
      Offset := MatchedOffset + MatchedLength;
    until ((Limit > 1) and (Count >= Limit)) or not MatchAgain;
    Strings.Add(string(Copy(Subject, Offset, MaxInt)));
  end
end;
  ...
  begin
    ...
    repeat
      ...
      begin
        ...
        begin
          LUseOffset := False;
          Strings.Add(string(Copy(Subject, 1, MatchedOffset -1)));
          if Self.GroupCount > 0 then
            Strings.Add(string(Self.Groups[Self.GroupCount]));
        end;
      end
      else
      begin
        Strings.Add(string(Copy(Subject, LOffset, MatchedOffset - LOffset)));
        Inc(Count);
        if Self.GroupCount > 0 then
          Strings.Add(string(Self.Groups[Self.GroupCount]));
      end;
    ...
    until ((Limit > 1) and (Count >= Limit)) or not MatchAgain;
    Strings.Add(string(Copy(Subject, LOffset, MaxInt)));
  end
end;
...
procedure TPerlRegExList.SetSubject(const Value: UTF8String);
var
  I: Integer;
begin
