...
uses
  System.SyncObjs, System.Math, System.AnsiStrings, System.Generics.Collections, System.Variants;
...
begin
  Result := TInterlocked.Increment(inst^.RefCount);
end;

function MemRelease(inst: PSimpleInstance): Integer; stdcall;
begin
  Result := TInterlocked.Decrement(inst^.RefCount);
  if Result = 0 then
    FreeMem(inst);
end;
...
begin
  Result := (TMethod(Left).Code = TMethod(Right).Code) and (TMethod(Left).Data = TMethod(Right).Data);
end;
...
begin
  try
    ...
    Result := BobJenkinsHash(Value^, SizeOf(Variant), 0);
  end;
end;
