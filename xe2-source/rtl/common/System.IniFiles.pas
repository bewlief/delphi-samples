...
begin
  ...
  begin
    ...
    try
      ...
      BinToHex(Pointer(PByte(Stream.Memory) + Stream.Position)^, PChar(Text),
  ...
  try
    ...
    try
      ...
      begin
        ...
        SLen := Length(S);
        if (SectionLen = 0) or
          (SubSectionNamesOnly and (SLen > SectionLen) and SameText(Section, Copy(S, 1, SectionLen))) or
          (not SubSectionNamesOnly and (SLen >= SectionLen) and SameText(Section, Copy(S, 1, SectionLen))) then
        begin
          SubSectionName := Copy(S, SectionEndOfs, SLen + 1 - SectionEndOfs);
          if not Recurse and (Pos(SectionNameSeparator, SubSectionName) <> 0) then
            Continue;
