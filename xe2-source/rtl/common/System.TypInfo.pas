...
const
  ...
  (*$HPPEMIT '{'*)
  (*$HPPEMIT '  enum TParamFlag {pfVar, pfConst, pfArray, pfAddress, pfReference, pfOut, pfResult};'*)
...
type
  ...
  TTypeData = packed record
    case TTypeKind of
      tkUnknown: ();
      tkWString, tkUString, tkVariant: (AttrData: TAttrData);
      tkLString: (
        CodePage: Word
...
function HasExtendedChars(const Name: UTF8String): Boolean;
var
  I: Integer;
begin
  for I := 1 to Length(Name) do
    if (Byte(Name[I]) and $80) <> 0 then
      Exit(True);
  Result := False;
end;
...
var
  ...
  J, I: Integer;
begin
  UTF8Compare := HasExtendedChars(Name);
  if UTF8Compare then
    Utf8ToUnicode(LStr1, Length(LStr1), PAnsiChar(Name), Length(Name));
  ...
  begin
    if UTF8Compare then
    begin
      LStr2[UTF8ToUnicode(LStr2, Length(LStr2), @LName[1], LName[0])] := #0;
      if AnsiStrIComp(LStr1, LStr2) = 0 then
        Exit(I);
    end
    else
    begin
      ...
      while J > 0 do
      begin
        if ((Byte(LName[J]) xor Byte(Name[J])) and $DF) <> 0 then
          break;
        Dec(J);
      end;
      if J = 0 then
...
begin
  ...
  UTF8Compare := HasExtendedChars(PropName);
  if UTF8Compare then
    Utf8ToUnicode(LStr1, Length(LStr1), PAnsiChar(PropName), Length(PropName));
  ...
  begin
    ...
    begin
      if Length(Result^.Name) = Length(PropName) then
      if UTF8Compare then
      begin
        LStr2[UTF8ToUnicode(LStr2, Length(LStr2), PAnsiChar(@Result^.Name[1]), Cardinal(Result^.Name[0]))] := #0;
        if AnsiStrIComp(LStr1, LStr2) = 0 then Exit;
      end
      else
      begin
        J := Length(Result^.Name);
        while J > 0 do
        begin
          if ((Byte(Result^.Name[J]) xor Byte(PropName[J])) and $DF) <> 0 then break;
          Dec(J);
        end;
        if J = 0 then Exit;
      end;
...
begin
  ...
  if PropInfo^.PropType^^.Kind = tkClass then
    Result := NativeInt(TPropSet<TClass>.GetProc(Instance, PropInfo))
    ...
    otULong: Result := TPropSet<Cardinal>.GetProc(Instance, PropInfo);
  end;
end;
{$ELSE !PUREPASCAL}
asm //StackAligned
...
begin
  if PropInfo^.PropType^^.Kind = tkClass then
    TPropSet<TClass>.SetProc(Instance, PropInfo, TClass(Value))
  else case GetTypeData(PropInfo^.PropType^)^.OrdType of
...
begin
  case PropInfo^.PropType^.Kind of
    ...
    tkUString: Result := TPropSet<UnicodeString>.GetProc(Instance, PropInfo);
  else
    Result := '';
  end;
end;
