...
const
  ZLIB_VERSION: PAnsiChar = '1.2.5'; // do not localize
  ZLIB_VERNUM = $1250;
  ZLIB_VER_MAJOR = 1;
  ZLIB_VER_MINOR = 2;
  ZLIB_VER_REVSION = 5;
...
function inflateCopy(var dest, source: z_stream): Integer; cdecl;
  external {$IFDEF POSIX} libzlib name _PU + 'inflcateCopy'{$ENDIF};
(*
     Sets the destination stream as a complete copy of the source stream.
...
function gzprintf(file_: gzFile; format: PAnsiChar): Integer; cdecl; varargs;
...
function gzputs(file_: gzFile; s: PAnsiChar): Integer; cdecl;
...
function gzgets(file_: gzFile; buf: PAnsiChar; len: Integer): PAnsiChar; cdecl;
...
function gzerror(file_: gzFile; var errnum: Integer): PAnsiChar; cdecl;
...
function gzopen64(path: PAnsiChar; mode: PAnsiChar): gzFile; cdecl;
...
function zError           (Err: Integer): PAnsiChar; cdecl;
...
begin
  ...
  begin
    ...
    (((NativeUInt(offset) - FZStream.total_out) > 0) and (origin = soFromBeginning)) then
  begin
