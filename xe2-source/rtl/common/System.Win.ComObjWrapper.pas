...
uses
  Winapi.Windows, System.SysUtils, System.Classes, System.Win.ComObj, Winapi.ActiveX;
...
uses System.Contnrs, System.RTLConsts;
...
var
  COMComponents: TThreadList;
  COMServersList: TObjectList;
  COMClassesList: TClassList;
...
begin
  ...
  try
    ...
    begin
      Component := TCOMComponent(Items[I]);
...
begin
  ...
  if COMClassesList = nil then
    COMClassesList := TClassList.Create;
  ...
  begin
    ...
    if COMServersList = nil then
      COMServersList := TObjectList.Create;
    COMServersList.Add(Result);
  end
  else
    Result := TCOMServer(COMServersList.Items[Idx]);
end;
...
begin
  inherited;
  if COMComponents = nil then
    COMComponents := TThreadList.Create;
  FGUID := IUnknown;
  with COMComponents.LockList do
  try
    Add(Self);
...
begin
  ...
  try
    for I := 0 to Count - 1 do
    begin
      ComComponent := TComComponent(Items[I]);
