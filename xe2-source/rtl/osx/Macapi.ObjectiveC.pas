...
function MangleMethodName(Method: TRttiMethod): string;
...
function MangleType(RttiType: TRttiType): string; overload;
...
type
  ...
  protected
    destructor Destroy; override;
    function CreateObj(O: TOCLocal): Pointer;
...
begin
  ...
  DebugPrint('oc.import.invoke', 'self->obj 0x%x->0x%x for call to ''%s''',
             [IntPtr(PPointer(Src)^), IntPtr(Imp.GetObjectID), PAnsiChar(FSelector)]);
...
begin
  ...
  PPointer(Dest)^ := P;
  DebugPrint('oc.local.invoke', 'wrapping %s object 0x%p -> 0x%p',
             [FTypeInfo^.Name, PPointer(Src)^, P]);
end;

function HasInterfaceReturn(Method: TRttiMethod): Boolean;
var
  RetType: TRttiType;
begin
...
function IsLargeStruct(RT: TRttiType): Boolean;
begin
...
function HasLargeStructReturn(Method: TRttiMethod): Boolean;
var
  RetType: TRttiType;
begin
  RetType := Method.ReturnType;
  ...
  begin
    DebugPrint('oc.metadata.build', '    has struct return of %d bytes',
               [RetType.TypeSize]);
    Result := True;
  end
  else
...
function HasObjectiveCReturn(Method: TRttiMethod): Boolean;
var
  RetType: TRttiType;
begin
  RetType := Method.ReturnType;
  if Assigned(RetType) and (RetType.TypeKind = tkInterface) then
  begin
    DebugPrint('oc.metadata.build', '    has ObjectiveC return');
    Result := True;
  end
  else
    Result := False;
...
var
  Imp: TOCImport;
begin
  DebugPrint('oc.super.invoke', 'transforming super reference, Self = 0x%p',
             [PPointer(Src)^]);
  Imp := TOCImport(TRawVirtualClass.GetInstanceFromInterface(PPointer(Src)^));
  PPointer(Dest)^ := Imp.FID;
  PPointer(PByte(Dest) + SizeOf(Pointer))^ := Imp.FClsID;
  DebugPrint('oc.super.invoke', 'Obj ID = 0x%p, Cls ID = 0x%p',
             [Imp.FID, Imp.FClsID]);
end;
...
begin
  DebugPrint('oc.import.invoke', 'xforminterface src= 0x%x', [IntPtr(PPointer(Src)^)]);
...
begin
  ...
  DebugPrint('oc.metadata.RegisterPascal', 'retrieved FMXImplObject = 0x%x', [IntPtr(Dest^)]);
end;

function BuildBasicImportMarshalingInfo(Method: TRttiMethod;
                                        var Src: Integer;
                                        var Dest: Integer;
...
begin
  ...
  Op.Src := Src;
  Op.Dest := Dest;
  XFSelf := TSelfContext.Create(Selector);
  ContextMethod.Code := @TSelfContext.XForm;
  ContextMethod.Data := XFSelf;
  Op.XFormContextProc := TXFormWithContextProc(ContextMethod);
  Ops.Add(Op);
  Inc(Src, SizeOf(Pointer));
...
begin
  ...
  DebugPrint('oc.metadata.build.marshalinfo', 'building marshaling info for method %s', [Method.Name]);
  Ops := TList<TMarshalOp>.Create;
  ...
  for Param in Method.GetParameters do
  begin
    DebugPrint('oc.metadata.build.marshalinfo',
    ...
    case ParamType.TypeKind of
      ...
      tkInterface:
        begin
          ...
          Op.Src := Src;
          Op.Dest := Dest;
          XFConv := TConvObjID.Create(ParamType.Handle, TVTableCache.GetVTable(ParamType.Handle));
          ContextMethod.Code := @TConvObjID.XForm;
          ContextMethod.Data := XFConv;
          Op.XFormContextProc := TXFormWithContextProc(ContextMethod);
          Inc(Size, sizeof(Pointer));
          Inc(Src, sizeof(Pointer));
          Inc(Dest, sizeof(Pointer));
...
procedure MangleType(Buff: TStringBuilder; RttiType: TRttiType); overload;
var
  Kind: TTypeKind;
...
function SelectorForMethod(Method: TRttiMethod): SEL;
var
  MangledName: string;
begin
  MangledName := MangleMethodName(Method);
  Result := sel_registerName(PAnsiChar(UTF8String(MangledName)));
end;

function BuildOCMethodInvokeData(Method: TRttiMethod; IsSuperReference: Boolean):
  OCMethodInvokeData;
var
...
procedure GetMethodsInVTableOrder(RttiType: TRttiType; Methods:TList<TRttiMethod>); overload;
var
  BaseType: TRttiType;
  Method: TRttiMethod;
begin
...
function GetMethodsInVTableOrder(RttiType: TRttiType): TList<TRttiMethod>; overload;
begin
...
begin
  DebugPrint('oc.metadata.build',
             'Building invoke data for ''%s''', [P^.Name]);
  List := TList<OCMethodInvokeData>.Create;
  Context := TRttiContext.Create;
  RttiType := Context.GetType(P);
  if not Assigned(RttiType) then
    NoRtti(string(P^.Name));

  Methods := GetMethodsInVTableOrder(RttiType);
  for Method in Methods do
  begin
    DebugPrint('oc.metadata.build', '  %s (vt idx = %d)',
...
begin
  ...
  if Imp.QueryInterface(GetTypeData(P)^.Guid, Obj) <> S_OK then
    raise EObjectiveC.CreateResFmt(@SInternalBindingError, [InvokeData^.ObjectiveCType^.Name]);
end;
...
begin
  DebugPrint('oc.vtable.create', 'Creating vtable for ''%s''', [P^.Name]);
  ...
  begin
    ...
    DebugPrint('oc.vtable.create', '  %s %s', [P^.Name, GUIDToString(FGuids[I])]);
    P := GetParent(P);
  end;
...
begin
//  DebugPrint('oc.import.dtor', 'import 0x%p being destroyed', [Self]);
end;
...
var
  ...
  procedure BadType;
  begin
    raise EObjectiveC.CreateResFmt(ErrMsg, [P^.Name]);
  end;
begin
  if P^.Kind <> tkInterface then
    BadType;
  DebugPrint('oc.checkrtti.typehierarchy', '===');
  while P <> nil do
  begin
    DebugPrint('oc.checkrtti.typehierarchy', 'checking rtti for P = %s, RequiredRoot = %s', [P^.Name, RequiredRoot^.Name]);
    CheckForRtti(P);
...
begin
  ...
  begin
    ...
    ClsID := objc_getClass(PAnsiChar(UTF8Encode(ITypeInfo^.Name)));
    if not Assigned(ClsId) then
      raise EObjectiveC.CreateResFmt(@SObjectiveCClassNotFound, [ITypeInfo^.Name]);
    VTable := TOCVTable.Create(CTypeInfo, False);
    ClassImport := TOCImport.Create(ClsId, nil, VTable);
    if ClassImport.QueryInterface(GetTypeData(CTypeInfo)^.Guid, FOCClass) <> 0 then
      raise EObjectiveC.CreateResFmt(@SInternalBindingError, [ITypeInfo^.Name]);
  end;
  Result := FOCCLass;
end;
...
begin
  ITypeInfo := TypeInfo(T);
  DebugPrint('oc.import.create', 'Creating instance of ''%s''', [ITypeInfo^.Name]);
  Cls := GetOCClass;
  ObjID := Cls.alloc;
...
begin
  ITypeInfo := TypeInfo(T);
  DebugPrint('oc.import.create', 'wrapping instance of ''%s'' around 0x%x',
             [ITypeInfo^.Name, IntPtr(P)]);
...
begin
  ...
  P := Result.init;
  DebugPrint('oc.import.create', 'Value from default ctor = 0x%x', [IntPtr(P)]);
  Init(Result, P);
end;
...
begin
  ...
  DebugPrint('oc.instance.init', 'Initing ''%s'' with 0x%x.  Old value = 0x%x',
...
///  
function FindCorrespondingMethod(Method: TRttiMethod; Methods: TArray<TRttiMethod>) : TRttiMethod;
var
...
begin
  ...
  begin
    if Candidate.toString = Method.toString then
    begin
      Result := Candidate;
      break;
    end;
...
function BuildLocalInvokeData(Method: TRttiMethod): TDelphiMethodInvokeData;
var
  Ops: TArray<TMarshalOp>;
begin
...
function TRegisteredDelphiClass.CreateObj(O: TOCLocal): Pointer;
begin
  Result := objc_msgSend(FClassId, AllocSelector);
  if Assigned(Result) then
  begin
    object_setInstanceVariable(Result, 'FMXImplObject', O);
    DebugPrint('oc.metadata.RegisterPascal', 'Set FMXImplObject = 0x%x', [IntPtr(O)]);
  end;
end;
...
var
  ...
  begin
    raise EObjectiveC.CreateResFmt(@SObjCClassRegistrationFailed, [P^.Name]);
  end;
begin
  TMonitor.Enter(TypeMap);
  ...
  try
    ...
    ObjCClassName := string(P^.Name);
    
    DebugPrint('oc.metadata.RegisterPascal', 'registering meta data local object %s',
               [ObjCClassName]);
    ...
    begin
      DebugPrint('oc.metadata.RegisterPascal',  'checking protocol %s', [Protocol.Handle^.Name]);
      try
...
//          Writeln('** exception caught:' + E.ClassName);
      ...
      begin
        DebugPrint('oc.metadata.RegisterPascal', 'adding method %s', [Method.Name]);
                                                                    
        if Method.CallingConvention <> ccCdecl then
          raise EObjectiveC.CreateResFmt(@SMethodMustBeCDecl,
                                         [Method.Name, Protocol.Handle^.Name]);
    ...
    begin
      ...
      DebugPrint('oc.metadata.RegisterPascal', 'class claims to implement %s',
                 [Implementing^.Name]);
      TP := Context.GetType(Implementing);
      if not Assigned(TP) then
        NoRtti(string(Implementing^.Name));
      ...
      for Method in ImplementingType.GetMethods do
        ...
        begin
          DebugPrint('oc.metadata.RegisterPascal', '  must implement %s',
                     [Method.toString]);
          if Method.CallingConvention <> ccCdecl then
          raise EObjectiveC.CreateResFmt(@SMethodMustBeCDecl,
                                         [Method.Name, Implementing^.Name]);
          MethodsHash.AddOrSetValue(Method.toString, Method);
        end;
      ...
      DebugPrint('oc.metadata.RegisterPascal', 'super class will be %s',
                 [SuperClassName]);
      SuperClass := objc_getClass(PAnsiChar(UTF8String(SuperClassName)));
      if not Assigned(SuperClass) then
        ObjectiveCClassNotFound(SuperClassName);
    end
    else
    begin
      DebugPrint('oc.metadata.RegisterPascal', 'super class will be NSObject');
      SuperClass := objc_getClass('NSObject');
    end;

                                                                           
    ObjCClassId := objc_allocateClassPair(SuperClass, PAnsiChar(AnsiString(ObjCClassName)), 0);
    if not Assigned(ObjCClassId) then
      FailRegistration;
    
    DebugPrint('oc.metadata.RegisterPascal', 'ObjectiveC class id is 0x%x',
               [IntPtr(ObjCClassId)]);
    ...
    begin
      ...
      if not Assigned(ClassMethod) then
        raise EObjectiveC.CreateResFmt(@SMethodNotFound,
                                       [Method.ToString, P^.Name]);
      
      MethodName := MangleMethodName(ClassMethod);
      DebugPrint('oc.metadata.RegisterPascal', 'method %s -> %s', [Method.toString, MethodName]);
      MangledName := MangleMethodType(ClassMethod);
      DebugPrint('oc.metadata.RegisterPascal.mangled', 'selector = ''%s''',
                 [MangledName]);
      ...
      if class_addMethod(ObjCClassId, sel_registerName(PAnsiChar(UTF8String(MethodName))),
                         Thunk,
                         PAnsiChar(UTF8String(MangledName))) = NO then
      begin
        FailRegistration;
      end;
    end;
    ...
    for Protocol in InstanceType.GetImplementedInterfaces do
    begin
        DebugPrint('oc.metadata.registerprotocol',  'claims to implement protocol %s', [Protocol.Handle^.Name]);
        if (Protocol.GUID = ILocalObject) or (Protocol.GUID = IInterface) or (Protocol.GUID = IObjectiveC) then
          continue;
        OCProtocol := objc_getProtocol(PAnsiChar(UTF8String(Protocol.Handle^.Name)));
        if OCProtocol <> nil then
        begin
          class_addProtocol(ObjCClassId, OCProtocol);
          DebugPrint('oc.metadata.registerprotocol',  '    registering protocol %s', [Protocol.Handle^.Name]);
        end;
    end;
...
begin
  ...
  begin
    ...
    DebugPrint('oc.super', 'Setting up super class for ''%s''', [OCTypeInfo^.Name]);
    DebugPrint('oc.super', '  Pascal class is ''%s''', [Self.ClassName]);
...
begin
  ...
  DebugPrint('oc.local.create', 'Created instance of ''%s'' ObjID = 0x%x',
             [Self.ClassName, IntPtr(FObjectID)]);
  if not Assigned(FObjectID) then
    raise EObjectiveC.CreateResFmt(@SErrorCreatingOCObject, [CType^.Name]);
end;

destructor TOCLocal.Destroy;
begin
                              
  DebugPrint('oc.local.destroy', 'Destroying instance of ''%s'' ObjID = 0x%x',
    [Self.ClassName, IntPtr(FObjectID)]);
  inherited Destroy;
end;
