...
const
  ...
  { tagCLSCTX from idl/wtypes.h in very old version}
  // {$EXTERNALSYM CLSCTX_INPROC_SERVERX86}
  // CLSCTX_INPROC_SERVERX86    = $40;       // Wx86 server dll (runs in same process as caller)
  // {$EXTERNALSYM CLSCTX_INPROC_HANDLERX86}
  // CLSCTX_INPROC_HANDLERX86   = $80;       // Wx86 handler dll (runs in same process as caller)
  // {$EXTERNALSYM CLSCTX_ESERVER_HANDLER}
  // CLSCTX_ESERVER_HANDLER     = $100;      // handler dll (runs in the server process)
  // {$EXTERNALSYM CLSCTX_NO_WX86_TRANSLATION}
  // CLSCTX_NO_WX86_TRANSLATION = $800;

  { tagCLSCTX from wtypes.h }
  ...
  // CLSCTX_RESERVED1             = $0040;
  // {$EXTERNALSYM CLSCTX_RESERVED2}
  // CLSCTX_RESERVED2             = $0080;
  // {$EXTERNALSYM CLSCTX_RESERVED3}
  // CLSCTX_RESERVED3             = $0100;
  // {$EXTERNALSYM CLSCTX_RESERVED4}
  // CLSCTX_RESERVED4             = $0200;
  ...
  CLSCTX_DISABLE_AAA           = $8000;    // Disable EOAC_DISABLE_AAA capability for this activation only
  ...
  CLSCTX_PS_DLL                 = Integer($800000);

{ from objbase.h, wtypes.h }
  {$EXTERNALSYM CLSCTX_INPROC}
  CLSCTX_INPROC = CLSCTX_INPROC_SERVER or CLSCTX_INPROC_HANDLER;
  {$EXTERNALSYM CLSCTX_ALL}
  CLSCTX_ALL    = CLSCTX_INPROC_SERVER or CLSCTX_INPROC_HANDLER
               or CLSCTX_LOCAL_SERVER or CLSCTX_REMOTE_SERVER;  { _WIN32_WINNT >= 0x0400 }
  //CLSCTX_ALL    = CLSCTX_INPROC_SERVER or CLSCTX_INPROC_HANDLER or CLSCTX_LOCAL_SERVER; { else }
  {$EXTERNALSYM CLSCTX_SERVER}
  CLSCTX_SERVER = CLSCTX_INPROC_SERVER or CLSCTX_LOCAL_SERVER
               or CLSCTX_REMOTE_SERVER;  { _WIN32_WINNT >= 0x0400 }
  // CLSCTX_SERVER = CLSCTX_INPROC_SERVER or CLSCTX_LOCAL_SERVER; { else }
  ...
  FADF_RESERVED  = $F008;  { bits reserved for future use }
...
{ VARENUM usage key,
  ...
  VT_RECORD          = 36;  { [V][T][P]  user defined type           }
...
function StgSetTimes(pszName: POleStr; const ctime: TFileTime;
  const atime: TFileTime; const mtime: TFileTime): HResult; inline; overload;
function StgSetTimes(pszName: POleStr; ctime: PFileTime;
  atime: PFileTime; mtime: PFileTime): HResult; overload; stdcall;
...
function StgSetTimes(pszName: POleStr; ctime: PFileTime; atime: PFileTime;
  mtime: PFileTime): HResult;           external ole32 name 'StgSetTimes';
function StgSetTimes(pszName: POleStr; const ctime: TFileTime;
  const atime: TFileTime; const mtime: TFileTime): HResult;
begin
  Result := StgSetTimes(pszName, @ctime, @atime, @mtime);
end;
