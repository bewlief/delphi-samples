...
uses Winapi.Windows, Winapi.Messages, Winapi.MMSystem;
...
{ Structures: }
  ...
  PAbsTime = ^TAbsTime;
  tagABSTIME = record    { 2.0 absolute date/time }
...
{$EXTERNALSYM LPABSTIME}
  ...
  tagCTLINITHEDIT = record      { 2.0 init struct for (h)edit }
...
{$EXTERNALSYM LPCTLINITHEDIT}
  ...
  tagBOXLAYOUT = record         { 1.0 box edit layout }
...
{$EXTERNALSYM LPBOXLAYOUT}
  ...
  tagIMECOLORS = record         { 2.0 IME undetermined string color info. }
...
{$EXTERNALSYM LPIMECOLORS}
  ...
  tagCTLINITBEDIT = record      { 2.0 init struct for box edit }
...
{$EXTERNALSYM LPCTLINITBEDIT}
  ...
  tagBOXEDITINFO = record       { 1.1 box edit Size Info }
...
{$EXTERNALSYM LPBOXEDITINFO}
  ...
  PRectOfs = ^TRectOfs;
  tagRECTOFS = record            { 1.0 rectangle offset for nonisometric inflation }
...
{$EXTERNALSYM LPRECTOFS}
  ...
  tagPENDATAHEADER = record     { 1.0 main pen data header }
...
{$EXTERNALSYM LPPENDATAHEADER}
  ...
  tagSTROKEINFO = record        { 1.0 stroke header }
...
{$EXTERNALSYM LPSTROKEINFO}
  ...
  PPenTip = ^TPenTip;
  tagPENTIP = record            { 2.0 Pen Tip characteristics }
...
{$EXTERNALSYM ANIMATEPROC}
  ...
  tagANIMATEINFO = record       { 2.0 Animation parameters }
...
{$EXTERNALSYM LPANIMATEINFO}
  ...
  tagOEMPENINFO = record        { 1.0 OEM pen/tablet hdwe info }
...
{$EXTERNALSYM LPOEMPENPACKET}
  ...
  PPenInfo = ^TPenInfo;
  tagPENINFO = record           { 1.0 pen/tablet hdwe info }
    ...
    lPdc: LONG;                 { Pen Device Capabilities }
...
{$EXTERNALSYM LPPENINFO}
  ...
  tagCALBSTRUCT = record        { 1.0 pen calibration }
    wOffsetX: Integer;
    wOffsetY: Integer;
...
{$EXTERNALSYM LPHINKSET}
  ...
  PInterval = ^TInterval;
  tagINTERVAL = record           { 2.0 interval structure for inksets }
...
{$EXTERNALSYM LPBOXRESULTS}
  ...
  PGuide = ^TGuide;
  tagGUIDE = record                { 1.0 guide structure }
...
{$EXTERNALSYM LPGUIDE}
  ...
  tagCTLINITIEDIT = record      { 2.0 init struct for Ink Edit }
...
{$EXTERNALSYM LPCTLINITIEDIT}
  ...
  tagPDEVENT = record           { 2.0 }
    ...
    lExInfo: LONG;              { GetMessageExtraInfo() return value }
    dwReserved: DWORD;          { for future use }
  end;
...
{$EXTERNALSYM LPSTRKFMT}
  ...
  PPCMInfo = ^TPCMInfo;
  tagPCMINFO = record           { 2.0 Pen Collection Mode Information }
...
{$EXTERNALSYM LPPCMINFO}
  ...
  tagINKINGINFO = record        { 2.0 Pen Inking Information }
...
{$EXTERNALSYM LPINKINGINFO}
  ...
  PSYC = ^TSYC;
{$EXTERNALSYM PSYC}
  tagSYC = record               { 1.0 Symbol Correspondence for Ink }
...
{$EXTERNALSYM LPSYC}
  ...
  PSYE = ^TSYE;
  tagSYE = record               { 1.0 Symbol Element }
    syv: SYV;                   { symbol value }
    lRecogVal: LONG;            { for internal use by recognizer }
...
{$EXTERNALSYM LPSYE}
  ...
  PSYG = ^TSYG;
{$EXTERNALSYM PSYG}
  tagSYG = record              { 1.0 Symbol Graph }
    ...
    lRecogVal: LONG;            { reserved for use by recoognizer }
...
{$EXTERNALSYM LPSYG}
  ...
  PRC = ^TRC;
  tagRC = record                { 1.0 Recognition Context (RC) }
    ...
    lRcOptions: LONG;           { RCO_xx options }
    ...
    lpLanguage: packed array[0..cbRcLanguageMax-1] of AnsiChar;{ language strings }
    ...
    lPcm: LONG;                 { flags: PCM_xx for ending recognition }
...
{$EXTERNALSYM LPRC}
  ...
  PRCResult = ^TRCResult;
{$EXTERNALSYM PRCResult}
  tagRCRESULT = record         { 1.0 Recognition Result }
...
{$EXTERNALSYM TFNFuncResults}

  PTarget = ^TTarget;
  tagTARGET = record            { 2.0 Geometry for a single target. }
...
{$EXTERNALSYM LPTARGET}
  ...
  PTargInfo = ^TTargInfo;
  tagTARGINFO = record          { 2.0 A set of targets }
...
function SyvCharacterToSymbol(c: AnsiChar): DWORD;
{$EXTERNALSYM SyvCharacterToSymbol}
function SyvKanjiToSymbol(c: AnsiChar): DWORD;
