...
{ The following structure is used by DdeQueryConvInfo(): }
  ...
  tagCONVINFO = record
    ...
    hUser: DWORD_PTR;       { user specified field  }
...
const
  ...
  DDE_FACKRESERVED = $ffff3f00; { ~(DDE_FACK | DDE_FBUSY | DDE_FAPPSTATUS) }
  {$EXTERNALSYM DDE_FADVRESERVED}
  DDE_FADVRESERVED = $ffff3fff;
  {$EXTERNALSYM DDE_FDATRESERVED}
  DDE_FDATRESERVED = $ffff4fff;
  {$EXTERNALSYM DDE_FPOKRESERVED}
  DDE_FPOKRESERVED = $ffffdfff;
...
function DdeSetUserHandle(Conv: HConv; ID, User: DWORD_PTR): BOOL; stdcall;
...
type
  ...
  tagDDEML_MSG_HOOK_DATA = record  { new for NT }
    uiLo: UINT_PTR;      { unpacked lo and hi parts of lParam }
    uiHi: UINT_PTR;
