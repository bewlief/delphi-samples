...
uses
  Winapi.Windows, Winapi.MMSystem, Winapi.ActiveX, Winapi.DXTypes, Winapi.DirectSound;
 ...
 * method; pass NULL to have DirectMusic manage usage of Winapi.DirectSound.
 *)
...
type
  ...
  TDMusPortParams7 = _DMUS_PORTPARAMS7;
  {$EXTERNALSYM _DMUS_PORTPARAMS7}
  DMUS_PORTPARAMS7 = _DMUS_PORTPARAMS7;
 ...
 *
 * These priorities are shared with Winapi.DirectSound.
 *)
...
//Translator: IReferenceClock is already defined in Winapi.DirectSound.pas
  IReferenceClock = Winapi.DirectSound.IReferenceClock;
...
(* every DMUS_PMSG is based off of this structure. The Performance needs
  ...
  _DMUS_NOTE_PMSG = record
    ...
    cTranspose: AnsiChar;       // Transposition to add to midi note value after converted from wMusicValue.
  end;
...
// IDirectMusicPerformance8 *)
  ...
  IDirectMusicPerformance8 = interface(IDirectMusicPerformance)
    ...
    function InitAudio(
      ...
      hWnd: hWnd;                                    // HWND for Winapi.DirectSound.
...
{$IFDEF BYTE2_ALIGNMENT}{$MINENUMSIZE 1}{$ENDIF}
  ...
  _DMUS_IO_TIMESIG = record
    (* Time signatures define how many beats per measure, which note receives *)
    (* the beat, and the grid resolution. *)
    bBeatsPerMeasure: {$IFDEF USE_BYTE2_ALIGNMENT}Byte2{$ELSE}Byte{$ENDIF};   // beats per measure (top of time sig)
