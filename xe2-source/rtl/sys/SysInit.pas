...
{$ENDIF}  
  ...
  CoverageLibraryName: array [0..128] of AnsiChar = '*'; { initialized by the linker! }
{$IFDEF ELF}
...
procedure UnloadDelayLoadedDLL2(szDll: PAnsiChar); stdcall;


{ Snap load support - Load and resolve all delay-loaded entrypoints for the given dll }
procedure LoadAllImportsForDll(szDll: PAnsiChar); stdcall;
...
function GetModuleFileNameA(Module: Integer; Filename: PAnsiChar; Size: Integer): Integer; stdcall;
...
{$IFNDEF UNICODE}
function GetModuleFileName(Module: Integer; Filename: PAnsiChar; Size: Integer): Integer; stdcall;
  external kernel name 'GetModuleFileNameA';
{$ELSE}
...
function GetModuleHandleA(ModuleName: PAnsiChar): HINST; stdcall;
...
{$IFNDEF UNICODE}
function GetModuleHandle(ModuleName: PAnsiChar): HINST; stdcall;
  external kernel name 'GetModuleHandleA';
{$ELSE}
...
function SetDliNotifyHook2(HookProc: TDelayedLoadHook): TDelayedLoadHook;
begin
  Result := InterlockedExchangePointer(PPointer(pfnDliNotifyHook)^, @HookProc);
end;
...
function SetDliFailureHook2(HookProc: TDelayedLoadHook): TDelayedLoadHook;
begin
  Result := InterlockedExchangePointer(PPointer(pfnDliFailureHook)^, @HookProc);
end;
...
procedure UnloadDelayLoadedDLL2(szDll: PAnsiChar);
...
{$IF defined(PUREPASCAL) or defined(CPUX64)}
function _ExitLib: Integer; cdecl;
begin
                                               
  Result := 0;
end;
{$ELSE !(PUREPASCAL or CPUX64)}
function _ExitLib: Integer; cdecl;
asm
{$IFDEF DEBUG_STARTUP}
        INT 3
...
begin
  ...
  while P^ <> 0 do
  begin
    P^ := NativeInt(dlsym(0, PAnsiChar(P^)));
    Inc(P);
  end;
end;
{$ENDIF LINUX}
...
{$ENDIF TRIAL_EDITION}
  ...
  Copyright name 'Portions Copyright(c) 1995-2011 Embarcadero Technologies, Inc.';                     // do not localize
