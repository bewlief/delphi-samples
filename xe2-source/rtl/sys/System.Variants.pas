...
{$IFDEF EMPTYPARAM_VAR}         // Causes problems - TODO - FIXME
var
...
procedure _DispInvoke(Dest: PVarData; const Source: TVarData;
  CallDesc: PCallDesc; Params: Pointer); varargs;
{$ELSE !CPUX64}
procedure _DispInvoke(Dest: PVarData; const Source: TVarData;
  CallDesc: PCallDesc; Params: Pointer); cdecl;
{$ENDIF !CPUX64}
...
type
  ...
  //TVarDynArray = array of OleVariant;
...
begin
  LStringPtr := nil;
  OleStrToStrVar(Source.VOleStr, AnsiString(LStringPtr));
  _VarClear(Dest);
  Dest.VType := varString;
...
{$IFDEF PUREPASCAL}
begin
  _VarOleStrToString(TVarData(Dest), TVarData(Source));
end;
{$ELSE !PUREPASCAL}
{$IFDEF X86ASM}
asm
...
begin
  ...
  else if TVarData(Dest).VType = varUString then
...
procedure TCustomVariantType.VarDataFromLStr(var V: TVarData;
...
{$IFNDEF EMPTYPARAM_VAR}         // Causes problems - TODO - FIXME
