...
function pthread_getname_np(Thread: pthread_t; Name: PAnsiChar;
  len: size_t): Integer;
{$EXTERNALSYM pthread_getname_np}

function pthread_setname_np(Name: PAnsiChar): Integer;
...
function InterlockedCompareExchange(var Target: THandle; Value, Comparand: THandle): THandle;
asm
  XCHG EAX,EDX
  XCHG EAX,ECX
...
begin
  ...
  begin
    ...
    if InterlockedCompareExchange(HPThread, Result, 0) <> 0 then
      dlclose(Result);
  end;
  Result := HPThread;
end;
...
type
  Tpthread_getname_np = function(Thread: pthread_t; Name: PAnsiChar;
...
type
  Tpthread_setname_np = function(Name: PAnsiChar): Integer; cdecl;
var
