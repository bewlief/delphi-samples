...
uses System.Variants, System.Classes, Web.HTTPApp, Data.DB, Datasnap.DBClient, Datasnap.Provider,
  WebComp, System.Masks, Datasnap.Midas, Web.AutoDisp;
...
{$IFDEF MSWINDOWS}
uses Winapi.Windows, Winapi.Messages, Data.DBConsts, Datasnap.MidConst, Winapi.ActiveX, System.Win.ComObj, Web.WebConst, System.SysUtils, Web.DBWeb,
  Datasnap.DSIntf, WbmConst;
{$ENDIF}
{$IFDEF LINUX}
uses  DbConsts, MidConst, { ActiveX, ComObj,}  WebConst, sysutils, DBWeb,
  DsIntf, WbmConst;
{$ENDIF}
...
begin
  Result := string(Datasnap.DSIntf.VariantArrayToString(V));
end;

function StringToVariantArray(const S: string): OleVariant;
begin
  Datasnap.DSIntf.StringToVariantArray(AnsiString(S));
end;
...
//{$DEFINE DEBUG}
{$IFDEF DEBUG}
procedure DumpXML(const FileName, XML: string);
var
  F: TFileStream;
begin
  F := TFileStream.Create(FileName, fmCreate or fmShareDenyNone);
  try
    F.Write(PChar(XML)^, Length(XML));
  finally
    F.Free;
  end;
end;
...
begin
{$IFDEF DEBUG}
  DumpXML('\xmldelta.txt', Delta);
{$ENDIF}
  XMLDelta := StringToVariantArray('<?xml version="1.0" standalone="yes"?>' + Delta);
{$IFDEF DEBUG}
  DumpXML('\xmlconvert.txt', VariantArrayToString(XMLDelta));
{$ENDIF}
  if Delta = '' then
    ErrorCount := 0
  else
  ...
  begin
{$IFDEF DEBUG}
    DumpXML('\xmlerrors.txt', VariantArrayToString(XMLErrors));
{$ENDIF}
...
function TXMLBroker.GetXMLStream(var Owned: Boolean): TStream;
var
  RecsOut: Integer;
  OwnerData: OleVariant;
  S: string;
begin
  S := GetXMLRecords(RecsOut, OwnerData, []);
  Owned := True;
