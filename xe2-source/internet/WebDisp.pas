...
{$IFDEF MSWINDOWS}
uses System.Classes, Winapi.Messages, Web.HTTPApp, Web.HTTPProd, Web.WebCntxt,
  System.Masks, WebComp, System.Contnrs, System.SysUtils, SiteComp;
{$ENDIF}
{$IFDEF LINUX}
...
{$IFDEF MSWINDOWS}
uses Web.WebConst, Winapi.Windows,
 WebScript, WebSess, System.Variants, AdaptReq, SiteConst;
{$ENDIF}
{$IFDEF LINUX}
uses WebConst,
...
begin
  if WebContext.Request <> nil then
    Result := string(WebContext.Request.InternalScriptName)
  else
    Result := '';
end;
...
begin
  ...
  begin
    ...
    Result := Result +
      Format('%s=%s', [HTTPEncode(AnsiString(AParams.Names[I])), HTTPEncode(AnsiString(ExtractStringValue(AParams[I])))]);
  end;
  Result := '/?' + Result;
end;

function LocalHTTPEnc(const AStr: AnsiString): AnsiString;
// This routine is identical to Web.HTTPApp.HTTPEncode except that it also encodes
...
function EncodeParamNameValues(ParamValues: TStrings): string;
  procedure AddNameValue(const AName: string; AValue: AnsiString);
  begin
...
begin
  ...
  for I := 0 to ParamValues.Count - 1 do
    AddNameValue(ParamValues.Names[I], LocalHTTPEnc(AnsiString(ExtractStringValue(ParamValues[I]))));
end;
...
begin
  ...
  begin
    ...
    Len := StrToInt(S);
    Delete(Temp, 1, P);
    Value :=  string(HTTPDecode(AnsiString(Copy(Temp, 1, Len))));
    Delete(Temp, 1, Len);
...
begin
  ...
  if dpPublished in AFlags then
    ...
    FindOptions := [];
  if Web.WebCntxt.WebContext.FindPage(APageName, FindOptions, PageComponent) then
  begin
...
begin
  if Assigned(OnActionRequestNotHandled) then
    if GetActionRequestParams(S) then
      try
        OnActionRequestNotHandled(Self, AObject, S, AHandled);
      finally
        S.Free;
      end;
  if Assigned(OnImageRequestNotHandled) then
    if GetImageRequestParams(S) then
      try
...
begin
  ...
  Page := string(Request.InternalPathInfo);
  P := Pos('?', Page);
  if P <> 0 then
...
begin
  for I := 0 to AResponse.Cookies.Count - 1 do
    with AResponse.Cookies[I] do
      if AnsiStrComp(PAnsiChar(Name), PAnsiChar(sSessionCookieName)) = 0 then
      begin
