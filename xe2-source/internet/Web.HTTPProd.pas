...
uses System.SysUtils, System.Classes, Web.HTTPApp, System.Masks, System.Contnrs;
...
{ TPageProducer }
  ...
  ['{53324483-A14A-497B-81DF-CA51668F36B9}']
    ...
    property HTMLBlocks[I: Integer]: string read GetHTMLBlock;
  ...
  TAbstractScriptEnginesList = class(TObject)
  public
    constructor Create;
    function FindScriptProducerClass(const ALanguageName: string): TScriptProducerClass; virtual; abstract;
  end;
...
const
  ...
  HTMLVAlign: array[THTMLVAlign] of string =
    ('',
     ...
     ' VAlign="baseline"');
var
  ScriptEnginesList: TAbstractScriptEnginesList = nil;
...
function FindScriptProducerClass(const AEngine: string): TScriptProducerClass;
begin
  if ScriptEnginesList <> nil then
    Result := ScriptEnginesList.FindScriptProducerClass(AEngine)
  else
    Result := nil;
end;
...
var
  ScriptProducerClass: TScriptProducerClass;
begin
  if UseScriptEngine then
    ScriptProducerClass := FindScriptProducerClass(ScriptEngine)
  else
    ScriptProducerClass := nil;
  if ScriptProducerClass <> nil then
    Result := Web.HttpProd.ContentFromScriptStream(Stream, WebModuleContext, StripParamQuotes, HandleTag,
...
begin
  ...
  begin
    ...
    if not (((Length(Result) >= 3) and (Result[2] = ':')) or
      ((Length(Result) >= 2) and (Result[1] = PathDelim) and (Result[2] = PathDelim))) then
      if not ((Length(Result) >= 1) and (Result[1] = PathDelim)) then
...
begin
  Result := '';
  ScriptProducerClass := FindScriptProducerClass(AScriptEngine);
    ...
    begin
      ...
      begin
        ...
        for I := 1 to Length(Signature) do
          if Bytes[I - 1] <> Signature [I - 1] then
          begin
            ASignatureSize := 0;
            Result := False;
            Break;
...
constructor TAbstractScriptEnginesList.Create;
begin
  ScriptEnginesList := Self;
end;
