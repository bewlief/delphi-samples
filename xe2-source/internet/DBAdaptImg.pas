...
{$IFDEF MSWINDOWS}
uses System.Classes, Winapi.Messages, Web.HTTPApp, WebComp, Data.DB, SiteComp, System.SysUtils,
...
uses System.Variants, Web.WebCntxt,
  AutoAdap,
  {$IFDEF MSWINDOWS}
...
function TCustomDataSetAdapterImageField.GetAdapterImage: TComponent;
begin
    ...
    begin
      ...
      begin
        ...
        try
          ...
          {$IFDEF MSWINDOWS}
          begin
            ContentStream.Seek(0, soFromBeginning); // Return to the start of the file
            ConvertBitmapToJpeg
          end
          {$ENDIF MSWINDOWS}
          {$IFDEF LINUX}
            raise EAdapterFieldException.Create(Format(sIncorrectImageFormat,
                ['image/bmp', FieldName]), FieldName)  { Do not localize }
{$ENDIF LINUX}
          else
            raise EAdapterFieldException.Create(Format(sIncorrectImageFormat,
              [sUnknownImageType, FieldName]), FieldName)
        except
    ...
    begin
      ...
      begin
        ...
        if (FieldValue.Files[0].ContentType = 'image/jpeg') or
          ...
          raise EAdapterFieldException.Create(Format(sIncorrectImageFormat, [FieldValue.Files[0].ContentType, FieldName]),
            FieldName);
      end
      else
        Result := True;
