...
uses System.Classes, Web.HTTPApp, Web.HTTPProd, PagItems, MidProd,
  CompProd, WebComp, System.SysUtils, Web.DSProd, SiteComp, System.Contnrs, WebForm;
...
uses Web.WebConst,
  Data.DB, Datasnap.DBClient, MidItems, WbmConst;
...
begin
  ...
  begin
    ...
    if Result then
      ReplaceString := SLineBreak + PageElements.ServerVariablesContent + PageElements.ServerFunctionsContent + PageElements.ServerScriptContent;
  end;
end;
...
begin
  ...
  for I := 0 to AGeneratedFunctions.ItemCount - 1 do
    Result := Result + AGeneratedFunctions.Script[I] + SLineBreak;
end;
...
begin
  ...
  for I := 0 to AVariables.Count - 1 do
    Result := Result + AVariables[I] + SLineBreak;
  if Result <> '' then
...
begin
  Result := Format('<html>' + SLineBreak +                { do not localize }
             ...
             '<#%0:s><#%1:s><#%2:s>' + SLineBreak +
             '</body>' + SLineBreak +                     { do not localize }
             '</html>' + SLineBreak, [sStylesTag,         { do not localize }
