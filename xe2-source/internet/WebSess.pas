...
uses
  System.Classes, System.Contnrs, System.SysUtils, SiteComp, WebContnrs, Web.HTTPApp, SessColn, WebDisp;
...
type
  ...
  protected
    ...
    procedure DoEndSession(ASession: TAbstractWebSession;
      AReason: TEndSessionReason); virtual;
  public
...
procedure TCustomSessionsService.DoStartSession(ASession: TAbstractWebSession);
begin
  if Assigned(OnStartSession) then
    OnStartSession(Self, ASession);
end;

procedure TCustomSessionsService.DoEndSession(ASession: TAbstractWebSession; AReason: TEndSessionReason);
begin
  if Assigned(OnEndSession) then
    OnEndSession(Self, ASession, AReason);
end;
...
begin
  S := TWebSession.Create(FComponent, AID);
  try
    FComponent.DoEndSession(S, esTerminate);
  finally
    S.Free;
  end;
  AHandled := True;
...
begin
  S := TWebSession.Create(FComponent, AID);
  try
    FComponent.DoEndSession(S, esTimeOut);
  finally
    S.Free;
  end;
  AHandled := True;
