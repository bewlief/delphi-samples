...
{$IFDEF MSWINDOWS}
uses System.Classes, Winapi.Messages, Web.HTTPApp, Web.HTTPProd, WebComp, Data.DB, SiteComp, WebModu,
  MidItems, System.Contnrs;
{$ENDIF}
{$IFDEF LINUX}
...
uses System.SysUtils, System.Variants, Web.WebCntxt, WebDisp, System.Math, SiteConst, AdaptReq, Winapi.Windows;
...
const
  sIfCan =
'<%%%% if (%0:s)' + sLineBreak +  // Do not localize
...
'<%%%% } %%%%>' + sLineBreak;
  ...
  sAccessBased =
'<%% if (%0:s.CanModify)' + sLineBreak +
'   { %%>' + sLineBreak +           // Do not localize
'%3:s%1:s' + sLineBreak +                                      // Do not localize
'<%% } %%>' + sLineBreak +                                     // Do not localize
'<%% else if (%0:s.CanView)' + sLineBreak +                    // Do not localize
'   { %%>' + sLineBreak +              // Do not localize
'%3:s%2:s' + sLineBreak +                                      // Do not localize
'<%% } %%>' + sLineBreak;
...
const
  sSetAdapterMode = '<%%' + sLineBreak +
                     '{' + sLineBreak +                            // Do not localize
                     '   var lastMode=%0:s.Mode' + sLineBreak +         // Do not localize
                     '   if(lastMode=="")' + sLineBreak +     // Do not localize
                     '   {' + sLineBreak +         // Do not localize
                     '     %0:s.Mode=''%1:s''' + sLineBreak +          // Do not localize
                     '   } %%>' + sLineBreak +                          // Do not localize
                     '%2:s' + sLineBreak +                           // Do not localize
                    '<%% %0:s.Mode=lastMode' + sLineBreak +         // Do not localize
                     '}  %%>' + sLineBreak;                           // Do not localize
...
begin
  ...
  if Supports(ParentLayout, ILayoutWebContent, Intf) then
    ...
    begin
      Result := Result + ChildTableIndent(GetParentComponent) + '</table>' + sLineBreak;  { do not localize }
      Result := Intf.LayoutTable(ChildTableIndent(GetParentComponent) + Result, GetLayoutAttributes)
    end
    else
    begin
      Result := Result + '</table>' + sLineBreak;  { do not localize }
      ...
      try
        ...
        begin
          ...
          Result := Intf.LayoutTable(sLineBreak + Result, A);
        end
      finally
        A.Free;
      end;
...
const
  ...
  sWriteHiddenFields =
  '<%%%2:s if (%0:s.%1:s != null)' + sLineBreak +                                      { do not localize }
   '%2:s   {' + sLineBreak +                                                           { do not localize }
   '%2:s     %0:s.%1:s.WriteFields(Response)' + sLineBreak +                           { do not localize }
   '%2:s   } %%>' + sLineBreak;                                                        { do not localize }
  ...
  sJSGridForEach =
  '<%% {' + sLineBreak +                                                               { do not localize }
   '   var ri = 0' + sLineBreak +                                                      { do not localize }
   '   var e = new Enumerator(%0:s.Records)' + sLineBreak +                            { do not localize }
   '   for (; !e.atEnd(); e.moveNext())' + sLineBreak +                                { do not localize }
   '   { %%>' + sLineBreak +                                                           { do not localize }
   '%1:s' +                                                                     { do not localize }
  '<%%  ri++' + sLineBreak +                                                           { do not localize }
   '   }' + sLineBreak +                                                               { do not localize }
   '   %0:s.EndIterating()' + sLineBreak +                                                   { do not localize }
   '   } %%>' + sLineBreak;                                                            { do not localize }
...
function TCustomAdapterGrid.FormatTable(Layout: TLayoutWebContent;
  ...
  begin
    ...
    Result := Format('<table%s>' + sLineBreak, [Attribs]);   { do not localize }
  end;
  ...
  begin
    ...
    Result := FieldIndent(Self) + Format('<tr%s>' + sLineBreak, [Attribs]);  { do not localize }
  end;
...
begin
  ...
  Result := Result + FieldIndent(Self) + '</tr>' + sLineBreak;  { do not localize }

  VarName := MakeAdapterVariableName(Self);
  OneRow := OneRow + FieldIndent(Self) + '</tr>' + sLineBreak + FormatHiddenRecordFields(VarName);  { do not localize }
  if VarName <> '' then
...
const
  ...
  'function %s(t)' + sLineBreak +  { do not localize }
  '{' + sLineBreak +
  '   Response.Write((t!="")?t:"&nbsp")' + sLineBreak +  { do not localize }
  '}' + sLineBreak;
...
const
  ...
  sListValuesFunction =
  'function %s(f)' + sLineBreak +
  '{' + sLineBreak +
  '   var s=''''' + sLineBreak +
  '   var v=''''' + sLineBreak +
  '   var n=''''' + sLineBreak +
  '   var c=0;' + sLineBreak +
  '   if (f.Values == null) return s;' + sLineBreak +
  '   var e = new Enumerator(f.Values.Records)' + sLineBreak +
  '   for (; !e.atEnd(); e.moveNext())' + sLineBreak +
  '   {' + sLineBreak +
  '     s+= ''<li>''' + sLineBreak +
  '     s += f.Values.ValueField.DisplayText;' + sLineBreak +
  '     s += ''</li>''' + sLineBreak +
  '     c++' + sLineBreak +
  '   }' + sLineBreak +
  '   e.moveFirst()' + sLineBreak +
  '   r = new Object;' + sLineBreak +
  '   r.text = s' + sLineBreak +
  '   r.count = c' + sLineBreak +
  '   return r;' + sLineBreak +
  ' }' + sLineBreak;
  ...
  '   var n=''''' + sLineBreak +
  '   var c=0' + sLineBreak +
  '   if (f.ValuesList != null)' + sLineBreak +
  '   {' + sLineBreak +
  '     var e = new Enumerator(f.ValuesList.Records)' + sLineBreak +
  '     for (; !e.atEnd(); e.moveNext())' + sLineBreak +
  '     {' + sLineBreak +
  '       s+= ''<option''' + sLineBreak +
  '       v = f.ValuesList.Value;' + sLineBreak +
  '       var selected' + sLineBreak +
  '       if (f.Values == null)' + sLineBreak +
  '          selected = f.IsEqual(v)' + sLineBreak +
  '       else' + sLineBreak +
  '          selected = f.Values.HasValue(v)' + sLineBreak +
  '       if (selected)' + sLineBreak +
  '          s += '' selected''' + sLineBreak +
  '       n = f.ValuesList.ValueName;' + sLineBreak +
  '       if (n=='''')' + sLineBreak +
  '       {' + sLineBreak +
  '         n = v' + sLineBreak +
  '         v = ''''' + sLineBreak +
  '       }' + sLineBreak +
  '       if ((typeof(v) != ''string'') || (v != ''''))' + sLineBreak +
  '          s += '' value="'' + v + ''"''' + sLineBreak +
  '       s += ''>'' + n + ''</option>\r\n''' + sLineBreak +
  '       c++' + sLineBreak +
  '     }' + sLineBreak +
  '     e.moveFirst()' + sLineBreak +
  ...
  sInputGroupFunction =
  'function %0:s(f,nm,cols,attr)' + sLineBreak +
  '{' + sLineBreak +
  '   var s=''''' + sLineBreak +
  '   var v=''''' + sLineBreak +
  '   var n=''''' + sLineBreak +
  ...
  '   if (f.ValuesList == null)' + sLineBreak +
  '   {' + sLineBreak +
  '     s+= ''<input type="%1:s"''' + sLineBreak +
  '     if (f.IsEqual(true)) s+= '' checked''' + sLineBreak +
  '     s += '' value="true"'' + '' name="'' + nm + ''"''' + sLineBreak +
  '     if (attr!='''') s+= '' '' + attr' + sLineBreak +
  '     s += ''>\r\n''' + sLineBreak +
  '     c = 1' + sLineBreak +
  '   }' + sLineBreak +
  '   else' + sLineBreak +
  '   {' + sLineBreak +
  '     s += ''<table><tr>''' + sLineBreak +
  '     var e = new Enumerator(f.ValuesList.Records)' + sLineBreak +
  '     for (; !e.atEnd(); e.moveNext())' + sLineBreak +
  '     {' + sLineBreak +
  '       if (c %% cols == 0 && c != 0) s += ''</tr><tr>''' + sLineBreak +
  '       s+= ''<td><input type="%1:s"''' + sLineBreak +
  '       v = f.ValuesList.Value;' + sLineBreak +
  '       var checked' + sLineBreak +
  '       if (f.Values == null)' + sLineBreak +
  '         checked = (f.IsEqual(v))' + sLineBreak +
  '       else' + sLineBreak +
  '         checked = f.Values.HasValue(v)' + sLineBreak +
  '       if (checked)' + sLineBreak +
  '         s+= '' checked''' + sLineBreak +
  '       n = f.ValuesList.ValueName;' + sLineBreak +
  '       if (n=='''')' + sLineBreak +
  '       {' + sLineBreak +
  '       n = v' + sLineBreak +
  '       }' + sLineBreak +
  '       s += '' value="'' + v + ''"'' + '' name="'' + nm + ''"''' + sLineBreak +
  '       if (attr!='''') s+= '' '' + attr' + sLineBreak +
  '       s += ''>'' + n + ''</td>\r\n''' + sLineBreak +
  '       c++' + sLineBreak +
  '     }' + sLineBreak +
  '     e.moveFirst()' + sLineBreak +
  '     s += ''</tr></table>''' + sLineBreak +
  '   }' + sLineBreak +
  '   r = new Object;' + sLineBreak +
  '   r.text = s' + sLineBreak +
  '   r.count = c' + sLineBreak +
...
begin
  ...
  Result := Result + Format('<ul %0:s>' + sLineBreak + '%1:s</ul>',   { do not localize }
                   [Attrs,
...
begin
  ...
  S := Self.Caption;
  Result := Format('<th%s>%s</th>' + sLineBreak, [Attribs, S]);   { do not localize }
...
begin
  Result := Format('<%%' + sLineBreak + sWriteColTextFunction + '%%>', [sWriteColTextFunctionName]);
end;
...
begin
  ...
  if FBreakButtons then
    Result := Format('%0:s%3:s>%1:s</td>' + sLineBreak, [Result, HTMLButton, FieldIndent, Attributes.ControlAttributes])  { do not localize }
  else if FButtonIndex = 1 then
    Result := Format('%0:s%3:s>%1:s</td>' + sLineBreak, [Result, HTMLButton, FieldIndent, Attributes.ControlAttributes])  { do not localize }
  else
    Result := Format('%0:s%2:s<td%3:s>%1:s</td>' + sLineBreak,     { do not localize }
...
begin
  ...
  if Assigned(Attributes) then
    case Attributes.LabelPosition of
      ...
      begin
        Result := StartFields(2) +
           Format('%0:s>%1:s</td>' + sLineBreak + FieldIndent + '<td %2:s>%3:s</td>' + sLineBreak, { do not localize }
            [Attributes.LabelAttributes, HTMLLabel, Attributes.ControlAttributes, HTMLField]);
      end;
      lposRight:
      begin
        Result := StartFields(2) +
          Format('%0:s>%1:s</td>' + sLineBreak + '<td %2:s>%3:s</td>' + sLineBreak,  { do not localize }
            [Attributes.ControlAttributes, HTMLField, Attributes.LabelAttributes, HTMLLabel]);
      end;
      lposAbove:
        Result :=
          StartFields(1) +
           Format('%0:s>%1:s</td></tr>' + sLineBreak + '<tr><td ' + sColSpanAttr + '="2" %2:s>%3:s</td>' + sLineBreak, { do not localize }
            [Attributes.LabelAttributes, HTMLLabel, Attributes.ControlAttributes, HTMLField]);
      lposBelow:
        Result :=
          StartFields(1) +
    ...
    Result :=
        StartFields(2) + Format('>%0:s</td>' + sLineBreak + '<td>%1:s</td>' + sLineBreak,  { do not localize }
          [HTMLLabel, HTMLField])
end;
...
begin
  if Assigned(Attributes) then
    Result := StartFields(1) + Format('%0:s>' + sLineBreak + '%1:s' + FieldIndent + '</td>' + sLineBreak, [Attributes.ControlAttributes, HTMLTable])  { do not localize }
  else
    Result := StartFields(1) + Format('>' + sLineBreak + '%0:s' + FieldIndent + '</td>' + sLineBreak, [HTMLTable]);  { do not localize }
...
begin
  ...
  if (FLayoutState = lsButtons) and (not FBreakButtons) and (FButtonIndex = 0) then
    ...
    begin
      Result := FieldIndent + '</tr>' + sLineBreak;  { do not localize }
      FColumnIndex := 0;
      FInRow := False;
    end;
end;
...
begin
  if FInTable then
  begin
    Result := EndRow + TableIndent + '</table>' + sLineBreak;   { do not localize }
    Dec(FTableLevel);
    FInTable := False;
  end
  else
...
begin
  ...
  begin
    FInTable := True;
    Inc(FTableLevel);
    Result := TableIndent + FTableHeader + sLineBreak;
  end
  else
    Result := '';
end;
...
procedure FillBlanks(var AValue: string);
var
  I: Integer;
begin
...
begin
  ...
  if (WebContext <> nil) and (WebContext.Request <> nil) then
    AddQuotedAttrib(Attribs, sActionAttr,
      string(WebContext.Request.InternalScriptName +
...
function TCustomAdapterForm.FormatForm(ParentLayout: TLayout;
  ...
  begin
    Result := Format('<input ' + sTypeAttr + '="hidden" ' + sNameAttr + '="%s">' + sLineBreak, [sActionRequest]);  { do not localize }
  end;
...
begin
  ...
  try
    ...
    Result := Result +  sLineBreak + FFormLayout.FieldIndent +
      FormatActionField;
...
begin
  Result := Format('<%%' + sLineBreak + sSelOptionsFunction + '%%>', [sSelOptionsFunctionName]);
end;

function ListValuesFunction(AComponent: TComponent): string;
begin
  Result := Format('<%%' + sLineBreak + sListValuesFunction + '%%>', [sListValuesFunctionName]);
end;

function RadioGroupFunction(AComponent: TComponent): string;
begin
  Result := Format('<%%' + sLineBreak + sInputGroupFunction + '%%>', [sRadioGroupFunctionName, 'radio']);  { do not localize }
end;

function CheckBoxGroupFunction(AComponent: TComponent): string;
begin
  Result := Format('<%%' + sLineBreak + sInputGroupFunction + '%%>', [sCheckBoxGroupFunctionName, 'checkbox']);  { do not localize }
end;
...
const
  ...
  // 3 - Indent
  sIfEnabledElse =    '<%% if (%0:s.Enabled)' + sLineBreak +
                       '   { %%>' + sLineBreak +
                       '%3:s%1:s' + sLineBreak +
                      '<%% }' + sLineBreak +
                       '   else' + sLineBreak +
...
function TCustomAdapterActionButton.ImplContent(Options: TWebContentOptions;
  ParentLayout: TLayout): string;
var
  Condition: string;
  ...
  begin
    ...
    // 3 - Indent
    Result := Format(
  '<%% if (%0:s.Array != null)' + sLineBreak +                                         { do not localize }
   '   {' + sLineBreak +                                                               { do not localize }
   '     var c=0' + sLineBreak +                                                       { do not localize }
   '     var cols=%1:d %%>' + sLineBreak +                                             { do not localize }
   '%3:s<table><tr>' + sLineBreak +                                                    { do not localize }
  '<%%   var e = new Enumerator(%0:s.Array)' + sLineBreak +                            { do not localize }
   '     for (; !e.atEnd(); e.moveNext())' + sLineBreak +                              { do not localize }
   '     {' + sLineBreak +                                                             { do not localize }
   '       if (cols > 0 && c %% cols == 0 && c != 0)' + sLineBreak +                   { do not localize }
   '       { %%>' + sLineBreak +                                                       { do not localize }
   '%3:s  </tr><tr>' + sLineBreak +                                                    { do not localize }
  '<%%     } %%>' + sLineBreak +                                                       { do not localize }
   '%3:s  <td>' + sLineBreak +                                                         { do not localize }
   '%3:s  %2:s' + sLineBreak +                                                         { do not localize }
   '%3:s  </td>' + sLineBreak +                                                        { do not localize }
  '<%%     c++' + sLineBreak +                                                         { do not localize }
   '     } ' + sLineBreak +                                                            { do not localize }
   '     e.moveFirst() %%>' + sLineBreak +                                             { do not localize }
   '%3:s</tr></table>' + sLineBreak +                                                  { do not localize }
  '<%% } %%>' + sLineBreak, [VarName, DisplayColumns, HTMLElement, Indent]);           { do not localize }
  end;
  ...
  begin
    Result := Format(
  '<%%   {' + sLineBreak +                                                             { do not localize }
   '     var Image' + sLineBreak +                                                     { do not localize }
   '     Image = %0:s.GetEventImage(''%1:s'',''%3:s'', %4:s)' + sLineBreak +               { do not localize }
   '     if (Image != null)' + sLineBreak +                                            { do not localize }
   '     { %%>' + sLineBreak +                                                         { do not localize }
   '%5:s%3:s="if (document.images) document.images.%2:s.src=''<%%= Image.AsHREF %%>''"' + sLineBreak +  { do not localize }
  '<%%   }' + sLineBreak +                                                             { do not localize }
   '     } %%>' + sLineBreak, [ProducerVarName, VarName, ImageName, Event, Text, Indent]);  { do not localize }
  end;
  ...
  begin
    ...
    begin
      ...
      if IsList then
        ...
        Text := Format('''%s''', [Self.Caption]); { do not localize }
      HREF := Format('<%%=(%1:s.GetImage(''%0:s'', %2:s)!=null)?%1:s.GetImage(''%0:s'', %2:s).AsHREF:''''%%>', { do not localize }
        [VarName, ProducerVarName, Text]);
      if UseEvents then
      begin
        ...
        if Events <> '' then
          Events := sLineBreak + Events + ChildTableIndent(Self);
    ...
    Result := Format(sLineBreak + sIfEnabledElse,
      [VarName,
    ...
    if IsList then
      Result := sLineBreak + FormatList(VarName, Result, ChildTableIndent(Self));
  ...
  begin
    ...
    Result :=
      Format('<input type="submit"%0:s>',  { do not localize }
       [Attrs]);
    if IsList then
      // Result := sLineBreak + FormatList(VarName, Result, FieldIndent(Self));
      Result := sLineBreak + FormatList(VarName, Result, ChildTableIndent(Self));
  end;
...
begin
  if Assigned(FAdapter) then
    Result := Format(
     sLineBreak +
     '<%% {' + sLineBreak +                                                             { do not localize }
     '   var e = new Enumerator(%0:s.Errors)' + sLineBreak +                           { do not localize }
     '   for (; !e.atEnd(); e.moveNext())' + sLineBreak +                              { do not localize }
     '   {' + sLineBreak +                                                             { do not localize }
     '     Response.Write("<li>" + e.item().Message)' + sLineBreak +                   { do not localize }
     '   }' + sLineBreak +                                                             { do not localize }
     '   e.moveFirst()' + sLineBreak +                                                 { do not localize }
     '   } %%>' + sLineBreak + ChildTableIndent(Self),                                 { do not localize }
  ...
  if Result <> '' then
    if Supports(Layout, ILayoutWebContent, Intf) then
      if Layout is TAdapterFormLayout then
        Result := Intf.LayoutField(Result, GetLayoutAttributes) + sLineBreak
      else
      begin
        A := TLayoutAttributes.Create;
        try
          ...
          begin
            ...
            AddQuotedAttrib(ControlAttributes, sVAlignAttr, HTMLVAlignValues[haTop]);
            // Work better with a layout group
            Result := Intf.LayoutField(sLineBreak + Result + sLineBreak, A);
          end
        finally
          A.Free;
        end;
...
begin
  AddQuotedAttrib(Attribs, sAlignAttr, HTMLAlignValues[Align]);
  AddQuotedAttrib(Attribs, sVAlignAttr, HTMLVAlignValues[VAlign]);
  Result := Format(FieldIndent(Self) + '<td%0:s><div>' + sLineBreak + '%1:s%2:s</div></td>' + sLineBreak, [Attribs,  { do not localize }
...
begin
  ...
  AddQuotedAttrib(Attribs, sVAlignAttr, HTMLVAlignValues[Self.CaptionAttributes.VAlign]);
  AddQuotedAttrib(Attribs, sAlignAttr, HTMLAlignValues[Self.CaptionAttributes.Align]);
  Result := Format(FieldIndent(Self) + '<th%0:s>%1:s</th>' + sLineBreak, [Attribs, Caption]);  { do not localize }
end;
