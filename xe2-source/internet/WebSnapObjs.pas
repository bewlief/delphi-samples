...
uses
  System.SysUtils, {$IFDEF MSWINDOWS}Winapi.Windows, Winapi.ActiveX,{$ENDIF}
  Web.WebCntxt, SiteComp, System.Classes, Web.HTTPProd, Web.HTTPApp, System.Win.ObjComAuto;
...
uses
  SiteConst, System.Variants, WebComp, WebDisp, System.ObjAuto, WebScript, WebAuto;
  ...
  begin
    ...
    try
      ...
      if FFailurePage <> '' then
        TempList.Add(sFailurePageParamName + '=' + FFailurePage);
...
begin
  if Request <> nil then
    Result := WideString(Request.Host)
  else
    Result := '';
end;
...
begin
  if Request <> nil then
    Result := WideString(Request.Method)
  else
    Result := '';
end;
...
begin
  if Request <> nil then
    Result := WideString(Request.InternalPathInfo)
  else
    Result := '';
end;
...
begin
  if Request <> nil then
    Result := WideString(Request.PathTranslated)
  else
    Result := '';
end;
...
begin
  if Request <> nil then
    Result := WideString(Request.ProtocolVersion)
  else
    Result := '';
end;
...
begin
  if Request <> nil then
    Result := WideString(Request.Query)
  else
    Result := ''
end;
...
begin
  if Request <> nil then
    Result := WideString(Request.InternalScriptName)
  else
    Result := '';
end;
...
begin
  Result := string(WebContext.Response.Allow);
end;

function TResponseObj.GetContentEncoding: string;
begin
  Result := string(WebContext.Response.ContentEncoding);
end;
...
begin
  Result := string(WebContext.Response.ContentType);
end;

function TResponseObj.GetContentVersion: string;
begin
  Result := string(WebContext.Response.ContentVersion);
end;

function TResponseObj.GetLocation: string;
begin
  Result := string(WebContext.Response.Location);
end;

function TResponseObj.GetRealm: string;
begin
  Result := string(WebContext.Response.Realm);
end;

function TResponseObj.GetReasonString: string;
begin
  Result := string(WebContext.Response.ReasonString);
end;

function TResponseObj.GetServer: string;
begin
  Result := string(WebContext.Response.Server);
end;
...
begin
  Result := string(WebContext.Response.Title);
end;

function TResponseObj.GetVersion: string;
begin
  Result := string(WebContext.Response.Version);
end;

function TResponseObj.GetWWWAuthenticate: string;
begin
  Result := string(WebContext.Response.WWWAuthenticate);
end;

procedure TResponseObj.SetAllow(const Value: string);
begin
  WebContext.Response.Allow := AnsiString(Value);
end;

procedure TResponseObj.SetContentEncoding(const Value: string);
begin
  WebContext.Response.ContentEncoding := AnsiString(Value);
end;
...
begin
  WebContext.Response.ContentType := AnsiString(Value);
end;

procedure TResponseObj.SetContentVersion(const Value: string);
begin
  WebContext.Response.ContentVersion := AnsiString(Value);
end;

procedure TResponseObj.SetLocation(const Value: string);
begin
  WebContext.Response.Location := AnsiString(Value);
end;

procedure TResponseObj.SetRealm(const Value: string);
begin
  WebContext.Response.Realm := AnsiString(Value);
end;

procedure TResponseObj.SetReasonString(const Value: string);
begin
  WebContext.Response.ReasonString := AnsiString(Value);
end;

procedure TResponseObj.SetServer(const Value: string);
begin
  WebContext.Response.Server := AnsiString(Value);
end;
...
begin
  WebContext.Response.Title := AnsiString(Value);
end;

procedure TResponseObj.SetVersion(const Value: string);
begin
  WebContext.Response.Version := AnsiString(Value);
end;

procedure TResponseObj.SetWWWAuthenticate(const Value: string);
begin
  WebContext.Response.WWWAuthenticate := AnsiString(Value);
end;
...
begin
  if not VarIsEmpty(Value) then
    FScriptProducer.Write(FScriptProducer.ReplaceTags(Value));
end;
...
const
  cKeywords: array[0..1] of string = ('Published', 'Array'); // Do not localize
  cExtraKeywords: array[0..1] of string = ('Name', 'Count'); // Do not localize
var
  I: Integer;
  Name: string;
  NewNames: Pointer;
  ...
  begin
    ...
    if NameCount = 1 then
    begin
      IdLocation := FLookupList.IndexOf(string(Name));
      if IdLocation < 0 then
      begin
        Disp := DispatchOfName(string(Name));
        if Disp <> nil then
        begin
