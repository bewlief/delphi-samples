...
{$IFDEF MSWINDOWS}
  Winapi.Windows,
{$ENDIF} 
  System.TypInfo, WebComp, System.SysUtils, Web.WebCntxt, SiteComp;
...
begin
  if WebContext.Request <> nil then
    Result := string(WebContext.Request.InternalScriptName) + '/' + PageName
  else
    Result := '';
end;
...
begin
  ...
  if (FFile <> '') then
    if AnsiCompareFileName(ExtractFileExt(FFile), FFile) = 0 then
      // FFile is a file ext
      FFile := ChangeFileExt(Factory.ComponentClass.UnitName, FFile)
end;
