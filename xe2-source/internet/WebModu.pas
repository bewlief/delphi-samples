...
uses
  System.Classes, Web.HTTPApp, Web.HTTPProd, System.Contnrs, WebComp,
  WebDisp, SiteComp, System.SysUtils, WebSnapObjs;
...
type
  ...
  TCustomWebDataModule = class(TDataModule,
    IWebVariablesContainer, INotifyWebActivate, IIterateObjectSupport,
    IWebSnapObjClass)
  private
...
uses Web.WebConst, Web.WebCntxt, WebAuto;
...
begin
  ...
  begin
    ...
    begin
      ...
      if Supports(IInterface(Result), IWebVariableName, WebVariableName) then
...
function TCustomWebDataModule.GetWebSnapObjClass: TObjectScripterClass;
begin
  Result := TModuleObj;
end;
...
begin
  ...
  try
    ...
    if not Result then
    begin
      R := string(AResponse.HTTPRequest.InternalScriptName) + '/' + APageName + EncodeParamsAsHTTPQuery(S);
      if Response.Cookies.Count > 0 then
        SafeRedirect(Response, R)
      else
        Response.SendRedirect(AnsiString(R));
      //AResponse.SendRedirect(
      //  AResponse.HTTPRequest.InternalScriptName + '/' + APageName + P);
      Result := True;
...
begin
  ...
  if S <> nil then
    try
      ...
      try
        Result := TStringStream.Create(HTMLFromTemplate(S), TEncoding.UTF8);
        AOwned := True;
      finally
