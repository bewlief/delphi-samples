...
{$IFDEF MSWINDOWS}
uses System.Classes, Web.HTTPApp, WebComp, Data.DB, SiteComp, System.SysUtils,
  WebContnrs, System.Contnrs, AdaptReq, Web.HTTPProd, Web.WebCntxt, WebSnapObjs;
{$ENDIF}
{$IFDEF LINUX}
...
type
  ...
  TObjectList = System.Contnrs.TObjectList;
  ...
  TAdapterNamedField = class(TWebContainedComponent, IWebVariableName, IWebInputName,
    ...
    IWebGetDisplayWidth, IWebGetDisplayLabel, IWebGetMaxLength, 
    IWebSnapObjClass, IAdapterFieldAccess, IAdapterFieldAttributes)
  private
    FFieldName: string;
 ...
 protected
  ...
  protected
    ...
    function ImplGetListImageOfValue(Value: Variant): TComponent; virtual;
  ...
  {$EXTERNALSYM EAdapterFieldException}
...
uses System.Variants, WebDisp,
  Web.WebConst, SiteConst, MidItems, AutoAdap;
...
function MakeValidAdapterFieldName(const AFieldName: string): string;
  ...
  begin
    ...
    begin
      if AnsiChar(Result[I]) in ['A'..'Z','a'..'z','_','0'..'9'] then
        Inc(I)
      else if AnsiChar(Result[I]) in LeadBytes then
        Delete(Result, I, 2)
      else
        Delete(Result, I, 1);
    end;
...
begin
  ...
  begin
    CrunchFieldName;
    if (Result = '') or (AnsiChar(Result[1]) in ['0'..'9']) then
      Result := '_' + Result;
  end;
end;
...
function TCustomAdapter.GetWebSnapObjClass: TObjectScripterClass;
begin
  Result := TAdapterObj;
end;
...
function TAdapterNamedField.GetAdapter: TCustomAdapter;
begin
  if FAdapter = nil then
    FAdapter := FindAdapterInParent(Self) as TCustomAdapter;
  Result := FAdapter;
end;
...
function TAdapterNamedField.GetWebSnapObjClass: TObjectScripterClass;
begin
  Result := TAdapterFieldObj;
end;
...
function TBaseAdapterAction.GetWebSnapObjClass: TObjectScripterClass;
begin
  Result := TAdapterActionObj;
end;
...
begin
  ...
  begin
    ...
    if SameText(WebContext.DispatchedPageName, FNextPage) then
      // Don't use the login page as the next page.
      FNextPage := '';
  end;
  if FNextPage = '' then
...
begin
  Response := Web.WebCntxt.WebContext.Response;
  Result := False;
  ...
  begin
    if LoginPage <> '' then
    begin
      if Web.WebCntxt.WebContext.FindPage(LoginPage, [], PageComponent) then
      begin
...
function TCustomApplicationAdapter.GetWebSnapObjClass: TObjectScripterClass;
begin
  Result := TApplicationObj;
end;
...
function TBaseValuesListAdapter.GetListImage: TComponent;
begin
  Result := nil;
end;

function TBaseValuesListAdapter.GetListImageOfValue(
  Value: Variant): TComponent;
begin
  Result := ImplGetListImageOfValue(Value);
end;

function TBaseValuesListAdapter.ImplGetListImage: TComponent;
begin
  Result := nil;
end;
...
function TBaseValuesListAdapter.GetWebSnapObjClass: TObjectScripterClass;
begin
  Result := TAdapterValuesListObj;
end;
...
function TCustomAdapterImageField.GetAdapterImage: TComponent;
begin
  Result := Self;
end;
...
begin
  ...
  begin
    ...
    try
      ...
      Assert(MimeType <> '');
      Response.ContentType := AnsiString(MimeType);
...
begin
  ...
  begin
    Value := ActionValue;
    if Value <> nil then
      Result := string(Value.Files[I].FileName)
    else
      Result := '';
  end
  else
