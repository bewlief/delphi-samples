...
uses System.SysUtils, System.Classes, System.Masks, System.Contnrs, Web.WebFileDispatcher;
...
{$IFDEF POSIX}
  Posix.String_,
{$ENDIF}
  Web.WebConst, Web.BrkrConst, System.StrUtils;
...
begin
  if AnsiStrComp(PAnsiChar(Method), 'GET') = 0 then  { do not localize }
    FMethodType := mtGet
  else if AnsiStrComp(PAnsiChar(Method), 'PUT') = 0 then   { do not localize }
    FMethodType := mtPut
  else if AnsiStrComp(PAnsiChar(Method), 'POST') = 0 then  { do not localize }
    FMethodType := mtPost
  else if AnsiStrComp(PAnsiChar(Method), 'HEAD') = 0 then  { do not localize }
    FMethodType := mtHead;
end;
...
begin
  ...
  for I := 1 to Length(S) do
    if S[I] = '-' then
      S[I] := ':'; // Change delimiter to work with TParser
