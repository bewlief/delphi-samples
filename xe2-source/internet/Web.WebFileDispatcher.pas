...
begin
  ...
  begin
    ...
    begin
      ...
      J := 0;
    while (I <= Length(Extensions)) and (Extensions[I] <> ';') do
    begin
...
begin
  if (Length(AFileName) > 0) and (AFileName[1] = PathDelim) then
      Delete(AFileName, 1, 1);
  Result := AFileName;
  LRootDirectory := IncludeTrailingPathDelimiter(FProperties.FRootDirectory);
  if not (((Length(LRootDirectory) >= 3) and (LRootDirectory[2] = ':')) or
    ((Length(LRootDirectory) >= 2) and (LRootDirectory[1] = PathDelim) and (Result[2] = PathDelim))) then
    if not ((Length(LRootDirectory) >= 1) and (LRootDirectory[1] = PathDelim)) then
