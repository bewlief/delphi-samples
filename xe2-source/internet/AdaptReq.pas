...
uses Web.WebConst, Winapi.Windows,
 WebScript, WebSess, System.Variants, WebDisp;
{$ENDIF}
{$IFDEF LINUX}
uses WebConst, 
  WebSess, Variants, WebDisp;
{$ENDIF}
  ...
  case RespondWith of
    ...
    begin
      ...
      if Response.Cookies.Count > 0 then
        ...
        Response.SendRedirect(AnsiString(FURL));
    end;
    rwPage:
    begin
...
var
  ...
  begin
    ...
    begin
      if (AAdapterStrings.Count > 0) and (AAdapterStrings.Objects[0] = nil) then
        // Detect case where there is a grid and a field on the same adapter.  We
        // don't support so we remove all of the field values.
        AAdapterStrings.Clear;
