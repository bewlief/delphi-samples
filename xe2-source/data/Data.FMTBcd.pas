...
begin
  ...
  PStr := @AValue[1];
  while CharInSet(PStr^, spaceChars) do
    Inc(PStr);
  Negative := PStr^ = '-';
  ...
  begin
    ...
    if CharInSet(PStr^, spaceChars) or CharInSet(PStr^, ['E', 'e']) then
      Break;
...
@@1:    SUB     EDX,4
        ...
        FILD    Curr
{$IFDEF PIC}
        FDIV    Power10.Single[EBX+EDX*4+16]
{$ELSE}
...
begin
  DecPos := Pos(DecSep, S);
  if (Precision <= 0) and ((DecPos = 0) or ((Length(S) - DecPos) <= -Precision)) then
     Exit;

  Negative := S[1] = '-';
  if Negative then
    buffer := ' ' + Copy(S, 2, length(S))
  else
    buffer := ' ' + Copy(S, 1, length(S));
  DecPos := Pos(DecSep, Buffer);
  if DecPos = 0 then DecPos := Length(Buffer) + 1;
  ...
  FillZeros(P);
  if Pos(DecSep, Buffer) <> 0 then
  begin
    P := @Buffer[Length(Buffer)];
    while P^ = '0' do
      Dec(P);
    if P^ = DecSep then
      P^ := #0
...
begin
  ...
  if Negative then
    Delete(S, 1, 1);
  Exponent := Pos(DecSep, S) -2;
  if Exponent = -2 then
    Exponent := Length(S) - 1
  else
  begin
    Delete(S, Exponent+2, 1);
  ...
  if Length(S) < Precision then
    S := S + StringOfChar('0', Precision - Length(S));
  if Length(S) > Precision then
    Delete(s, Precision+1, Length(S));
  ...
  if Negative then
    S := '-' + S;
  if (Exponent >= 0) and (Format = ffExponent) then
    S := S + System.SysUtils.Format('E+%.*d', [Digits, Exponent])
  else
    S := S + System.SysUtils.Format('E%.*d', [Digits, Exponent])
end;
...
begin
  ...
  DecPos := Pos(FS.DecimalSeparator, buf);
  if DecPos = 0 then
  begin
    DecPos := Length(buf)+1;
    if Digits > 0 then
      S := buf + FS.DecimalSeparator + StringOfChar('0', Digits)
    else
      S := Buf;
  end
  else
    S := buf + StringOfChar('0', Digits - (Length(buf) - DecPos));
  ...
  begin
    Dec(DecPos, 3);
    while (DecPos > 1) and (S[DecPos-1] <> '-') do
    begin
      Insert(FS.ThousandSeparator, S, DecPos);
      Dec(DecPos, 3);
    end;
  end;
...
begin
  ...
  case Format of
    ffGeneral: if ((DecPos = 0) and (Length(Result) > LPrecision)) or (DecPos - 1 > LPrecision) then
...
procedure ScanSection(Section: PWideChar; var Options: TFormatFloatOptions);
var
  QuoteChar: Char;
begin
...
begin
  ...
  Power := Pos(options.FormatSettings.DecimalSeparator, lstr) -2;
  if Power = -2 then
    Power := length(LStr)-1;
  Delete(LStr, Power+2, 1);
  ...
  LPStr := @LStr[1];
  if (Options.FirstDigit = 0) and (LStr[1] = '0') then
  begin
    Inc(LPStr);
    Dec(Power);
  end;
      ...
      begin
        ...
        begin
          ...
          if Digit > 4 then
            Digit := 4;
          Dec(Section);
          LExp := System.SysUtils.Format('%.*d', [Digit, Power]);
          Move(LExp[1], Buf^, length(LExp)* Sizeof(Char));
          Inc(Buf, Length(LExp));
        end
      end;
      ',', '.': ;
...
begin
  ...
  begin
    Section := FindSection(fsNegative, @Format[1]);
    if Section = @Format[1] then // Negative section was blank, put '-'
      PutChar(LBuffer, '-');
  end
  else if IsZero(Bcd) then
    Section := FindSection(fsZero, @Format[1])
  else
    Section := FindSection(fsPositive, @Format[1]);
  ScanSection(Section, Options);
  Options.FormatSettings := FS;
  ApplyFormat(Section, LBuffer, Bcd, Options);

  SetString(Result, Buffer, LBuffer - @Buffer[0]);
end;
...
begin
  if Indirect and VarDataIsByRef(Source) then
    ...
    begin
      VType := VarType;
      VBcd := TFMTBcdData.Create(TFMTBcdVarData(Source).VBcd.Bcd);
    end;
end;
...
begin
  if Source.VType = VarType then
    case AVarType of
      ...
      varUString:
        VarDataFromStr(Dest, string(TFMTBcdVarData(Source).VBcd.bcd));
    else
      VarDataInit(LTemp);
      try
...
begin
  case Operator of
    opAdd:
      BcdAdd(TFmtBcdVarData(Left).VBcd.Bcd, TFmtBcdVarData(Right).VBcd.Bcd,
        TFmtBcdVarData(Left).VBcd.FBcd);
    opSubtract:
      BcdSubtract(TFmtBcdVarData(Left).VBcd.Bcd, TFmtBcdVarData(Right).VBcd.Bcd,
        TFmtBcdVarData(Left).VBcd.FBcd);
    opMultiply:
      BcdMultiply(TFmtBcdVarData(Left).VBcd.Bcd, TFmtBcdVarData(Right).VBcd.Bcd,
        TFmtBcdVarData(Left).VBcd.FBcd);
    opDivide:
      BcdDivide(TFmtBcdVarData(Left).VBcd.Bcd, TFmtBcdVarData(Right).VBcd.Bcd,
        TFmtBcdVarData(Left).VBcd.FBcd);
  else
    RaiseInvalidOp;
  end;
...
begin
  ...
  else if VarType(AValue) = varCurrency then
