...
uses System.Variants, Data.DB, Data.DBCommon;
...
{$IFDEF POSIX}
  DSProps = record
    ...
    bReadOnly        : LongBool;         { Dataset is not updateable }
    szName           : MIDASPATH;
    szMorePad        : packed array[0..2] of AnsiChar;  { GCC pads this struct to 4 byte boundary }
  end;
{$ENDIF}
...
{ Callbacks }
  ...
  pfDSReconcile = function(         { Reconcile callback }
    ...
    pErrMessage   : PAnsiChar;      { Native errormessage if any }
    pErrContext   : PAnsiChar;      { 1-level error context, if any }
  ...
  pfDSReconcile_MD = function(
    ...
    pErrMessage   : PAnsiChar;    { Native errormessage, if any (otherwise NULL) }
...
type
  ...
  IDSBase = interface(IUnknown)
    ...
    function MakeEmbedded(
        ...
        pMasterFieldName  : PAnsiChar;   { Name of new link field in master, NULL if using default name }
        pDetailFieldName  : PAnsiChar    { Name of new link field in detail, NULL if using defaultname }
    ): DBResult; stdcall;
    ...
    function DropOptParameter(
           iFldNo: LongWord;     { 0 if not field attribute }
           pName: PAnsiChar      { Name of attribute to delete }
        ): DBResult; stdcall;
  end;
...
{$ENDIF}
  Datasnap.MidConst, System.SysUtils, System.VarUtils, System.Types
{$IFDEF POSIX}
  , Posix.Dlfcn
{$ENDIF}
  ;
...
var
  ...
  DllGetClassObject: function(const CLSID, IID: TGUID; var Obj): HResult; {$IFDEF POSIX}cdecl{$ELSE}stdcall{$ENDIF};
