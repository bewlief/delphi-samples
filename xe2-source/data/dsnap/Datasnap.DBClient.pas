...
uses System.SysUtils, System.Variants, System.Classes, Data.DB, Datasnap.DSIntf, Data.DBCommon, Data.DBCommonTypes, Datasnap.Midas, Data.SqlTimSt;
...
{ TClientDataSet }
  ...
  private
    ...
    FRanged: Boolean;
    procedure AddExprFilter(const Expr: WideString; Options: TFilterOptions);
    procedure AddFuncFilter;
    function CalcFieldsCallBack(RecBuf: TRecordBuffer): DBResult; stdcall;
    ...
    procedure EncodeIndexDesc(var IndexDesc: DSIDXDesc;
      const Name, Fields, DescFields, CaseInsFields: WideString; Options: TIndexOptions);
    function FilterCallback(RecBuf: TRecordBuffer): LongBool; stdcall;
    ...
    procedure GetAggFieldData(Buffer: TRecordBuffer);
    ...
    function ReconcileCallback(iRslt: Integer; iUpdateKind: DSAttr;
      iResAction: dsCBRType; iErrCode: Integer; pErrMessage, pErrContext: PAnsiChar;
  ...
  protected
    ...
    procedure RefreshInternalCalcFields(Buffer: TRecordBuffer); override;
  ...
  public
    ...
    procedure SetAltRecBuffers(Old, New, Cur: TRecordBuffer);
    procedure SetKey;
...
begin
  ...
  Result.SetProp(dspropANSICODEPAGE, DefaultSystemCodePage);
  Result.SetProp(dspropUTF8METADATA, NativeUInt(True));
  Result.SetProp(dspropUTF8ERRORMSG, NativeUInt(True));
end;
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        StrValue := UTF8Encode(Value);
        PValue := PAnsiChar(StrValue);
        ParamLen := Length(StrValue) + 1;
      end
      ...
      if (VarType(Value) and varByRef) = varByRef then
        PValue := TVarData(Value).VPointer else
      ...
      Name := MetaDataFromUnicode(ParamName);
      PNameOut := Pointer(Name); // Passed as "var" to GetOptParameter so we need a temp.
      if FDSBase.GetOptParameter(0, FieldNo, PNameOut, TypeOut, LenOut, PValueOut) = 0 then
        Check(FDSBase.DropOptParameter(FieldNo, Pointer(Name)));
      Check(FDSBase.AddOptParameter(FieldNo, Pointer(Name), ParamType, ParamLen, PValue));
    finally
      if Unlock then
        VarArrayUnlock(Value);
    end;
...
begin
  ...
  if (ParamType = dsfldBYTES) or
    ...
    begin
      ...
      try
        Move((TRecordBuffer(PValue) + SizeOf(Integer))^, PVarData^, ParamLen-SizeOf(Integer));
      finally
...
var
  ...
  ErrMsg: string;
  ErrMsgBuffer: array[0..2048] of AnsiChar;
begin
  if Status <> 0 then
  begin
    FDSBase.GetErrorString(Status, ErrMsgBuffer);
    FDSBase.GetProp(dspropUTF8ERRORMSG, @UTF8ErrorMessage);
    if LongBool(UTF8ErrorMessage) then
      ErrMsg := UTF8ToString(ErrMsgBuffer)
    else
      ErrMsg := string(ErrMsgBuffer);
...
var
  ...
  CursorProps: DSProps;
begin
  FDSBase.SetProp(dspropCOMPRESSARRAYS, NativeInt(True));
...
begin
  ...
  begin
    ...
    CheckMasterRange;
    if DisableStringTrim then FDSBase.SetProp(dspropDISABLESTRINGTRIM, NativeUInt(True));
    if FReadOnly then FDSBase.SetProp(dspropREADONLY, NativeUInt(True));
...
begin
  ...
  begin
    ...
    begin
      FDSBase.GetProp(dspropRECORDSINDS, @RecCountProp);
...
begin
  ...
  begin
    CheckBrowseMode;
    FDSBase.SetProp(dspropXML_STREAMMODE, xmlUTF8);
    Check(FDSBase.StreamDS(DataPacket));
    DataPacketToVariant(DataPacket, VarPacket);
    Result := UTF8ToString(VariantArrayToString(VarPacket));
  end;
end;
...
begin
  // Sanity check.
  if (Length(Value) > 1) and (Copy(Value, 1, 1) <> UTF16BOM) then
    U := UTF8Encode(#$FEFF + Value)
  else
    U := UTF8Encode(Value);
  SetData(StringToVariantArray(U));
end;
...
procedure TCustomClientDataSet.SetAltRecBuffers(Old, New, Cur: TRecordBuffer);
begin
  FOldValueBuffer := Old;
  FNewValueBuffer := New;
  FCurValueBuffer := Cur;
...
function TCustomClientDataSet.ReconcileCallback(
    ...
    pErrContext   : PAnsiChar;     { 1-level error context, if any (otherwise Null) }
...
begin
  ...
  try
    ...
    E := EReconcileError.Create(string(pErrMessage), string(pErrContext), iErrCode, iRslt);
    try
...
begin
  ...
  begin
    ...
    StrLCopy(szName, PAnsiChar(MetaDataFromUnicode(Name)), SizeOf(szName)-1);
...
begin
  ...
  FDSBase := CreateDSBase;
  try
    Check(FDSBase.Create(FieldDefCount, pDSFLDDesc(FieldDescs), PAnsiChar(MetaDataFromUnicode(Name))));
    CheckLongFieldNames;
    CreateIndexes;
  except
    FDSBase := nil;
...
begin
  CheckBrowseMode;
  Check(FDSBase.Reset);
  FDSBase.SetProp(dspropDATAHASCHANGED, NativeUInt(True));
  ProviderEOF := True;
  Resync([]);
...
procedure TCustomClientDataSet.ClearCalcFields(Buffer: TRecordBuffer);
begin
      ...
      DBERR_NONE:
        begin
          ...
          begin
            ...
            GetSequenceNumber(LongWord(RecordNumber));
...
function TCustomClientDataSet.GetActiveRecBuf(var RecBuf: TRecordBuffer): Boolean;
...
begin
  case State of
    ...
    dsSetKey: RecBuf := TRecordBuffer(FKeyBuffer) + SizeOf(TKeyBuffer);
    dsCalcFields,
...
function TCustomClientDataSet.GetStateFieldValue(State: TDataSetState; Field: TField): Variant;
  ...
  function CheckNotChanged(Buffer: TRecordBuffer): Variant;
  var
    IsBlank: Integer;
  begin
    if (Buffer = nil) then
...
function TCustomClientDataSet.CalcFieldsCallBack(RecBuf: TRecordBuffer): DBResult;
var
...
begin
  ResetCursorRange;
  Status := FDSCursor.UseIndexOrder(PAnsiChar(MetaDataFromUnicode(IndexName)));
  if Status <> DBERR_NONE then
  begin
    ...
    begin
      ...
      begin
        EncodeIndexDesc(IndexDesc, Name, Fields, DescFields, CaseInsFields, Options);
        Check(FDSBase.CreateIndex(IndexDesc));
        Check(FDSCursor.UseIndexOrder(PAnsiChar(MetaDataFromUnicode(IndexName))));
      end;
    end else
      Check(Status);
  end;
...
procedure TCustomClientDataSet.SetIndex(const Value: Widestring; FieldsIndex: Boolean);
var
  SeqNo: LongWord;
begin
  if Active then
...
begin
  ...
  Check(FDSBase.RemoveIndex(PAnsiChar(MetaDataFromUnicode(Name))));
  IndexDefs.Updated := False;
end;
...
function TCustomClientDataSet.GetIndexName: Widestring;
begin
...
procedure TCustomClientDataSet.SetIndexName(const Value: Widestring);
begin
...
function TCustomClientDataSet.GetIndexFieldNames: Widestring;
begin
  if FFieldsIndex then Result := FIndexName else Result := '';
end;

procedure TCustomClientDataSet.SetIndexFieldNames(const Value: Widestring);
begin
...
begin
  ...
  Check(FDSCursor.InitRecord(TRecordBuffer(Buffer) + SizeOf(TKeyBuffer)));
  Result := Buffer;
end;
...
begin
  ...
  begin
    ...
    StartKey := TRecordBuffer(RangeStart) + SizeOf(TKeyBuffer);
    if not RangeStart.Modified then
      StartKey := nil;
    EndKey := TRecordBuffer(RangeEnd) + SizeOf(TKeyBuffer);
...
begin
  ...
  RecBuffer := TRecordBuffer(KeyBuffer) + SizeOf(TKeyBuffer);
...
function TCustomClientDataSet.GetMasterFields: Widestring;
begin
  Result := FMasterLink.FieldNames;
end;

procedure TCustomClientDataSet.SetMasterFields(const Value: Widestring);
begin
...
function TCustomClientDataSet.FilterCallback(RecBuf: TRecordBuffer): LongBool;
var
  SaveState: TDataSetState;
  Accept: Boolean;
begin
  ...
  case Agg.DataType of
    ...
    ftWideString:
      begin
        ...
        begin
          VType := varUString;
          SetString(PUnicodeString(@VUString)^, PChar(Buffer)+1, Word(Buffer^) div 2);
        end;
      end;
    else
      Result := Null;
...
procedure TCustomClientDataSet.GetAggFieldData(Buffer: TRecordBuffer);
type
...
type
  ...
  const
    TypeStr: array[TConstraintType] of PAnsiChar = (szBDEDOMCL, szBDERECCL, szBDEDEFCL);
    ...
    try
      ...
      begin
        if (ConstraintType = ctField) and (FieldName <> '') then
          ErrorStr := UTF8Encode(Format('%s %s: %s %s',[SConstraintFailed, SField, FieldName, ExprText])) else
          ErrorStr := UTF8Encode(Format('%s %s',[SConstraintFailed, ExprText]));
      end else
        ErrorStr := UTF8Encode(ExprErrMsg);
      Len := Length(ErrorStr);
      ...
      if Len > 0 then
        StrLCopy(@FBuffer[SizeOf(TSQLExprInfo)], PAnsiChar(ErrorStr), Length(FBuffer) - SizeOf(TSQLExprInfo) - 1);
