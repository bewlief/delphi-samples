...
type
  ...
  private
    ...
    FSqlIdentifierQuoteChar: UnicodeString;
    FSqlIdentifierCase: Word;
    FSupportsCatalogNames: Boolean;
    ...
    procedure SetReservedWords(Buffer: TBytes);
...
uses
  Data.DBXCommonResStrs, Data.DBXMetaDataCommandFactory, Data.DBXReaderTableStorage, System.Odbc;
...
begin
  ...
  begin
    ...
    try
      FMethodTable.RaiseError(Context, ReturnValue, SQL_HANDLE_DBC, FConnectionHandle);
    finally
      Context.Free;
    end;
  end;
...
begin
  ...
  begin
    ...
    FMethodTable := FOdbcConnection.MethodTable;
  end;
  //(Character + null) * SizeOf(Char)
  SetLength(Buffer, 4);
  ErrorCode := FMethodTable.SQLGetInfo(FConnectionHandle, SQL_IDENTIFIER_QUOTE_CHAR,
                                       SQLPOINTER(@Buffer[0]), Length(Buffer), ReturnLength);
  CheckResult(ErrorCode);
  FSqlIdentifierQuoteChar := Trim(TEncoding.ANSI.GetString(Buffer));
  ErrorCode := FMethodTable.SQLGetInfo(FConnectionHandle, SQL_DESCRIBE_PARAMETER,
                                       SQLPOINTER(@Buffer[0]), Length(Buffer), ReturnLength);
  CheckResult(ErrorCode);
  ...
  ErrorCode := FMethodTable.SQLGetInfo(FConnectionHandle, SQL_MULTIPLE_ACTIVE_TXN,
                                       SQLPOINTER(@Buffer[0]), Length(Buffer), ReturnLength);
  CheckResult(ErrorCode);
  if PChar(@Buffer[0])^ = 'Y' then
  ...
  ErrorCode := FMethodTable.SQLGetInfo(FConnectionHandle, SQL_CATALOG_NAME,
                                       SQLPOINTER(@Buffer[0]), Length(Buffer), ReturnLength);
  CheckResult(ErrorCode);
  if PChar(@Buffer[0])^ = 'Y' then
    FSupportsCatalogNames := True
  else
    FSupportsCatalogNames := False;

  ErrorCode := FMethodTable.SQLGetInfo(FConnectionHandle, SQL_INDEX_KEYWORDS,
                                       SQLPOINTER(@Buffer[0]), Length(Buffer), ReturnLength);
  CheckResult(ErrorCode);
  FIndexKeyword := PWord(@Buffer[0])^;

  SetLength(Buffer, SizeOf(Word));
  ErrorCode := FMethodTable.SQLGetInfo(FConnectionHandle, SQL_IDENTIFIER_CASE,
                                       SQLPOINTER(@Buffer[0]), Length(Buffer), ReturnLength);
  CheckResult(ErrorCode);
  FSqlIdentifierCase := PWord(@Buffer[0])^;

  ErrorCode := FMethodTable.SQLGetInfo(FConnectionHandle, SQL_MAX_CONCURRENT_ACTIVITIES,
                                       SQLPOINTER(@Buffer[0]), Length(Buffer), ReturnLength);
  CheckResult(ErrorCode);
  FMaxConcurrentActivities := PWord(@Buffer[0])^;
  ErrorCode := FMethodTable.SQLGetInfo(FConnectionHandle, SQL_TXN_CAPABLE,
                                       SQLPOINTER(@Buffer[0]), Length(Buffer), ReturnLength);
  CheckResult(ErrorCode);
  FTransactionCapable := PWord(@Buffer[0])^;

  ErrorCode := FMethodTable.SQLGetInfo(FConnectionHandle, SQL_KEYWORDS,
...
var
  ...
  Reader: TDBXReader;
  DataTypeCount, ArrayLength: Integer;
  TypeName, CreateFormat, CreateParams, LiteralPrefix, LiteralSuffix, MaxVersion, MinVersion: UnicodeString;
  DataType, MaxScale, MinScale, Flags: Integer;
  SubType: SmallInt;
  ColumnSize: Int64;
begin
  ArrayLength := 20;
  ...
  Context := TDBXContext.Create;
  try
    Command := TDBXOdbcCommand.Create(Context, FOdbcConnection);
    try
...
procedure TDBXOdbcMetaDataReader.SetReservedWords(Buffer: TBytes);
begin
  FReservedWords := TStringList.Create;
  FReservedWords.Delimiter := ',';
  FReservedWords.DelimitedText := TEncoding.Unicode.GetString(Buffer);
end;
  ...
  try
    ...
    try
      ...
      begin
        ...
        Columns[3] := TDBXMetaDataCollectionColumns.CreateValueType('TableType', 'TABLE_TYPE', TDBXDataTypes.WideStringType);
...
function TDBXOdbcMetaDataReader.GetProductName: UnicodeString;
begin
  Result := 'Odbc';
end;
...
function TDBXOdbcMetaDataReader.GetSqlIdentifierQuoteChar: UnicodeString;
begin
  CheckInitialized;
  Result := FSqlIdentifierQuoteChar;
end;

function TDBXOdbcMetaDataReader.GetSqlIdentifierQuotePrefix: UnicodeString;
begin
  CheckInitialized;
  Result := FSqlIdentifierQuoteChar;
end;

function TDBXOdbcMetaDataReader.GetSqlIdentifierQuoteSuffix: UnicodeString;
begin
  CheckInitialized;
  Result := FSqlIdentifierQuoteChar;
end;

function TDBXOdbcMetaDataReader.GetSqlProcedureQuoteChar: UnicodeString;
begin
  CheckInitialized;
  Result := FSqlIdentifierQuoteChar;
end;
...
function TDBXOdbcMetaDataReader.FetchColumns(const Catalog: UnicodeString; const Schema: UnicodeString; const Table: UnicodeString): TDBXTable;
var
  Context: TDBXContext;
  Command: TDBXOdbcCommand;
  ...
  try
    ...
    try
      ...
      begin
        ...
        Columns[1] := TDBXMetaDataCollectionColumns.CreateValueType(TDBXIndexesColumns.SchemaName, 'TABLE_SCHEM', TDBXDataTypes.WideStringType);
        Columns[2] := TDBXMetaDataCollectionColumns.CreateValueType(TDBXIndexesColumns.TableName, 'TABLE_NAME', TDBXDataTypes.WideStringType);
        Columns[3] := TDBXMetaDataCollectionColumns.CreateValueType(TDBXIndexesColumns.ColumnName, 'COLUMN_NAME', TDBXDataTypes.WideStringType);
        Columns[4] := TDBXMetaDataCollectionColumns.CreateValueType('DATA_TYPE', 'DATA_TYPE', TDBXDataTypes.Int16Type);
        Columns[5] := TDBXMetaDataCollectionColumns.CreateValueType('TypeName', 'TYPE_NAME', TDBXDataTypes.WideStringType);
        Columns[6] := TDBXMetaDataCollectionColumns.CreateValueType('COLUMN_SIZE', 'COLUMN_SIZE', TDBXDataTypes.Int32Type);
        Columns[7] := TDBXMetaDataCollectionColumns.CreateValueType('BUFFER_LENGTH', 'BUFFER_LENGTH', TDBXDataTypes.Int32Type);
        Columns[8] := TDBXMetaDataCollectionColumns.CreateValueType('DECIMAL_DIGITS', 'DECIMAL_DIGITS', TDBXDataTypes.Int16Type);
        Columns[9] := TDBXMetaDataCollectionColumns.CreateValueType('NUM_PREC_RADIX', 'NUM_PREC_RADIX', TDBXDataTypes.Int16Type);
        Columns[10] := TDBXMetaDataCollectionColumns.CreateValueType('NULLABLE', 'NULLABLE', TDBXDataTypes.Int16Type);
        Columns[11] := TDBXMetaDataCollectionColumns.CreateValueType('REMARKS', 'REMARKS', TDBXDataTypes.WideStringType);
        Columns[12] := TDBXMetaDataCollectionColumns.CreateValueType('COLUMN_DEF', 'COLUMN_DEF', TDBXDataTypes.WideStringType);
        ...
        Columns[16] := TDBXMetaDataCollectionColumns.CreateValueType('ORDINAL_POSITION', 'ORDINAL_POSITION', TDBXDataTypes.Int32Type);
...
function TDBXOdbcMetaDataReader.FetchForeignKeyColumns(const Catalog: UnicodeString; const Schema: UnicodeString; const Table: UnicodeString; const ForeignKeyName: UnicodeString; const PrimaryCatalog: UnicodeString; const PrimarySchema: UnicodeString; const PrimaryTable: UnicodeString; const PrimaryKeyName: UnicodeString): TDBXTable;
var
  Columns: TDBXValueTypeArray;
begin
  CheckInitialized;
  ...
  try
    ...
    try
      ...
      begin
        ...
        Columns[0] := TDBXMetaDataCollectionColumns.CreateValueType('PKTABLE_CAT', 'PKTABLE_CAT', TDBXDataTypes.WideStringType);
        Columns[1] := TDBXMetaDataCollectionColumns.CreateValueType('PKTABLE_SCHEM', 'PKTABLE_SCHEM', TDBXDataTypes.WideStringType);
        Columns[2] := TDBXMetaDataCollectionColumns.CreateValueType('PKTABLE_NAME', 'PKTABLE_NAME', TDBXDataTypes.WideStringType);
        Columns[3] := TDBXMetaDataCollectionColumns.CreateValueType('PKCOLUMN_NAME', 'PKCOLUMN_NAME', TDBXDataTypes.WideStringType);
        Columns[4] := TDBXMetaDataCollectionColumns.CreateValueType('FKTABLE_CAT', 'FKTABLE_CAT', TDBXDataTypes.WideStringType);
        Columns[5] := TDBXMetaDataCollectionColumns.CreateValueType('FKTABLE_SCHEM', 'FKTABLE_SCHEM', TDBXDataTypes.WideStringType);
        Columns[6] := TDBXMetaDataCollectionColumns.CreateValueType('FKTABLE_NAME', 'FKTABLE_NAME', TDBXDataTypes.WideStringType);
        Columns[7] := TDBXMetaDataCollectionColumns.CreateValueType('FKCOLUMN_NAME', 'FKCOLUMN_NAME', TDBXDataTypes.WideStringType);
        Columns[8] := TDBXMetaDataCollectionColumns.CreateValueType('KEY_SEQ', 'KEY_SEQ', TDBXDataTypes.Int16Type);
        ...
        Columns[11] := TDBXMetaDataCollectionColumns.CreateValueType('FK_NAME', 'FK_NAME', TDBXDataTypes.WideStringType);
        Columns[12] := TDBXMetaDataCollectionColumns.CreateValueType('PK_NAME', 'PK_NAME', TDBXDataTypes.WideStringType);
...
function TDBXOdbcMetaDataReader.FetchIndexColumns(const Catalog: UnicodeString; const Schema: UnicodeString; const Table: UnicodeString; const Index: UnicodeString): TDBXTable;
var
  Columns: TDBXValueTypeArray;
begin
  CheckInitialized;
...
function TDBXOdbcMetaDataReader.FetchIndexes(const Catalog: UnicodeString; const Schema: UnicodeString; const Table: UnicodeString): TDBXTable;
var
  Context: TDBXContext;
  Command: TDBXOdbcCommand;
  TableParam: TDBXParameter;
...
function TDBXOdbcMetaDataReader.FetchPackageProcedureParameters(const Catalog: UnicodeString; const Schema: UnicodeString; const PackageName: UnicodeString; const ProcedureName: UnicodeString; const ParameterName: UnicodeString): TDBXTable;
var
  Columns: TDBXValueTypeArray;
begin
  CheckInitialized;
...
function TDBXOdbcMetaDataReader.FetchPackageProcedures(const Catalog: UnicodeString; const Schema: UnicodeString; const PackageName: UnicodeString; const ProcedureName: UnicodeString; const ProcedureType: UnicodeString): TDBXTable;
var
  Columns: TDBXValueTypeArray;
begin
  CheckInitialized;
...
function TDBXOdbcMetaDataReader.FetchPackages(const Catalog: UnicodeString; const Schema: UnicodeString; const PackageName: UnicodeString): TDBXTable;
var
  Columns: TDBXValueTypeArray;
begin
  CheckInitialized;
...
function TDBXOdbcMetaDataReader.FetchPackageSources(const Catalog: UnicodeString; const Schema: UnicodeString; const PackageName: UnicodeString): TDBXTable;
var
  Columns: TDBXValueTypeArray;
begin
  CheckInitialized;
...
function TDBXOdbcMetaDataReader.FetchProcedures(const Catalog: UnicodeString; const Schema: UnicodeString; const ProcedureName: UnicodeString; const ProcedureType: UnicodeString): TDBXTable;
var
  Context: TDBXContext;
  Command: TDBXOdbcCommand;
  Reader: TDBXReader;
...
function TDBXOdbcMetaDataReader.FetchProcedureParameters(const Catalog: string; const Schema: string; const &Procedure: string; const Parameter: string): TDBXTable;
var
  Context: TDBXContext;
  Command: TDBXOdbcCommand;
...
begin
  ...
  try
    ...
    try
      ...
      Command.Parameters[0].Value.SetWideString(&Procedure);
      Reader := Command.ExecuteQuery;
      if Reader = nil then
        Result := nil
      else
      begin
        ...
        Columns[4] := TDBXMetaDataCollectionColumns.CreateValueType('ColumnType', 'COLUMN_TYPE', TDBXDataTypes.Int16Type);
        Columns[5] := TDBXMetaDataCollectionColumns.CreateValueType('DATA_TYPE', 'DATA_TYPE', TDBXDataTypes.Int16Type);
        Columns[6] := TDBXMetaDataCollectionColumns.CreateValueType('TYPE_NAME', 'TYPE_NAME', TDBXDataTypes.WideStringType);
        ...
        Columns[11] := TDBXMetaDataCollectionColumns.CreateValueType('NULLABLE', 'NULLABLE', TDBXDataTypes.Int16Type);
...
function TDBXOdbcMetaDataReader.FetchProcedureSources(const Catalog: UnicodeString; const Schema: UnicodeString; const &Procedure: UnicodeString): TDBXTable;
var
  Columns: TDBXValueTypeArray;
begin
  CheckInitialized;
...
function TDBXOdbcMetaDataReader.FetchSchemas(const Catalog: UnicodeString): TDBXTable;
begin
  Result := SqlTablesInvocation('GetSchemas', Catalog, '', '', '');
end;

function TDBXOdbcMetaDataReader.FetchSynonyms(const Catalog: UnicodeString; const Schema: UnicodeString; const Synonym: UnicodeString): TDBXTable;
begin
  Result := SqlTablesInvocation('GetSynonyms', Catalog, Schema, Synonym, '');
end;

function TDBXOdbcMetaDataReader.FetchTables(const Catalog: UnicodeString; const Schema: UnicodeString; const TableName: UnicodeString; const TableType: UnicodeString): TDBXTable;
begin
