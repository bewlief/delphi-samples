...
type
  ...
  public
    ...
    constructor Create(const Key: UnicodeString); overload;
    ...
    function SetEncryptionKey(const Key: UnicodeString): Boolean;
    ...
    procedure Reset;
  protected
    function GetEncryptionKey: UnicodeString;
  private
  ...
  public
    property EncryptionKey: UnicodeString read GetEncryptionKey;
  private
    const Keylen = 16;
  end;
...
constructor TPC1Cypher.Create(const Key: UnicodeString);
begin
  SetLength(FX1a0,8);
  SetLength(FCle,Keylen);
  inherited Create;
...
var
  ...
  Len: Integer;
begin
  Len := Min(Length(FCypherKey),Keylen);
  for i := 0 to Len - 1 do
    FCle[I] := Ord((FCypherKey[1+I]));
  for i := Len to 15 do
    FCle[I] := Byte(32);
...
function TPC1Cypher.SetEncryptionKey(const Key: UnicodeString): Boolean;
var
  I: Integer;
begin
  ...
  begin
    TEMPXORVeri := TEMPXORVeri xor Ord(Key[I]);
  end;
end;

function TPC1Cypher.GetEncryptionKey: UnicodeString;
begin
  Result := FCypherKey;
end;
