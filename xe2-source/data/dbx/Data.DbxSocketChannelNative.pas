...
TDBXIdTCPLayer = class(TDBXCommunicationLayer)
  ...
  protected
    function ReadData(const Buffer: TBytes; const Offset: Integer; const Count: Integer): Integer;
...
function TDBXSocketChannel.Read(const Buffer: TBytes; const Offset,
...
function TDBXSocketChannel.Write(const Buffer: TBytes; const Offset,
  Count: Integer): Integer;
begin
//  Assert(Offset = 0);
...
function TDBXTCPLayer.Read(const Buffer: TBytes; const Offset,
  Count: Integer): Integer;
begin
...
function TDBXTCPLayer.Write(const Buffer: TBytes; const Offset,
  Count: Integer): Integer;
begin
//  Assert(Offset = 0);
  if FTcpClient <> nil then
...
function TDBXIdTCPLayer.Read(const Buffer: TBytes; const Offset,
  Count: Integer): Integer;
begin
  if Terminated then
    exit(-1);
...
function TDBXIdTCPLayer.ReadData(const Buffer: TBytes; const Offset,
  Count: Integer): Integer;
var
...
function TDBXIdTCPLayer.Write(const Buffer: TBytes; const Offset,
  Count: Integer): Integer;
begin
  if Terminated then
    exit(-1)
  ...
  begin
    FIdSocket.IOHandler.Write(Buffer, Count, Offset);
    exit(Count);
  end
  else
    exit(-1);
