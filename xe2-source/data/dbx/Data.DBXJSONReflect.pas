...
/// on the strcucture of the user object. Based on the events and their
...
type
  ...
  public
    ...
    property ObjectsConverter
      : TObjectsConverter read FObjectsConverter write SetObjectsConverter;
    property StringsConverter
      : TStringsConverter read FStringsConverter write SetStringsConverter;
    property TypeObjectsConverter
      : TTypeObjectsConverter read FTypeObjectsConverter write
      SetTypeObjectsConverter;
    property TypeStringsConverter
      : TTypeStringsConverter read FTypeStringsConverter write
      SetTypeStringsConverter;
    property ObjectConverter
      : TObjectConverter read FObjectConverter write SetObjectConverter;
    property StringConverter
      : TStringConverter read FStringConverter write SetStringConverter;
    property TypeObjectConverter
      : TTypeObjectConverter read FTypeObjectConverter write
      SetTypeObjectConverter;
    property TypeStringConverter
      : TTypeStringConverter read FTypeStringConverter write
      SetTypeStringConverter;
  ...
  public
    ...
    property ObjectsReverter
      : TObjectsReverter read FObjectsReverter write SetObjectsReverter;
    property StringsReverter
      : TStringsReverter read FStringsReverter write SetStringsReverter;
    property TypeObjectsReverter
      : TTypeObjectsReverter read FTypeObjectsReverter write
      SetTypeObjectsReverter;
    property TypeStringsReverter
      : TTypeStringsReverter read FTypeStringsReverter write
      SetTypeStringsReverter;
    property ObjectReverter
      : TObjectReverter read FObjectReverter write SetObjectReverter;
    property StringReverter
      : TStringReverter read FStringReverter write SetStringReverter;
    property TypeObjectReverter
      : TTypeObjectReverter read FTypeObjectReverter write
      SetTypeObjectReverter;
    property TypeStringReverter
      : TTypeStringReverter read FTypeStringReverter write
      SetTypeStringReverter;
  ...
  public
    ...
    property ConverterType
      : TConverterType read FConverterType write FConverterType;
  ...
  TTypeMarshaller<TSerial: class> = class
  private
    ...
    FConverters: TObjectDictionary<String, TConverterEvent>;
  ...
  private
    ...
    FReverters: TObjectDictionary<String, TReverterEvent>;
    FRTTICtx: TRTTIContext;
  ...
  protected
    ...
    /// <param name="Data">object instance</param>
    procedure PopulateFields(JsonFields: TJSONObject; Data: TObject);
...
procedure TTypeMarshaller<TSerial>.AddWarning(Data: TObject;
  FieldClassUnit, FieldClassName, FieldTypeUnit, FieldTypeName,
  FieldName: string);
var
  idx: Integer;
begin
...
procedure TTypeMarshaller<TSerial>.ClearWarnings(OwnWarningObject: boolean);
var
  I: Integer;
begin
...
class function TTypeMarshaller<TSerial>.ComposeKey(clazz: TClass;
  Field: string): string;
begin
  if clazz <> nil then
...
begin
  // find the last .
  DotPos := LastDelimiter(SEP_DOT, TypeName);
  if DotPos > 0 then
  begin
    UnitName := LeftStr(TypeName, DotPos - 1);
    ClassName := RightStr(TypeName, Length(TypeName) - DotPos);
  end;
end;
...
function TTypeMarshaller<TSerial>.HasWarnings: boolean;
begin
...
begin
  ...
  try
    ...
    if FConverter.IsConsistent then
      ...
      raise EConversionError.Create(SInconsistentConversion) finally begin
      // clear
      FObjectHash.Clear;
    FPointerHash.Clear;
    FId := 1;
    ...
    case Value.Kind of
      ...
      TTypeKind.tkEnumeration:
        if ((fieldRtti <> nil) and
           (CompareText('Boolean', fieldRtti.FieldType.Name) = 0)) or
           ((fieldRtti = nil) and (CompareText('Boolean', String(Value.typeInfo.Name)) = 0)) then
...
begin
  ...
  begin
    ...
    for rttiField in rttiType.GetFields do
    begin
      if not JSONBooleanAttributeValue(rttiField,JSONMarshalled,true) then
        Continue;
...
begin
  ...
  begin
    ...
    if customizer = nil then // Use default population customizer to prevent leaked memory
      customizer := TJSONPopulationCustomizer.Create;
    try
      customizer.PrePopulate(Obj,FRTTICtx);
      PopulateFields(objFields, Obj);
      ...
      TTypeKind.tkRecord:
        begin
          ...
          begin
            ...
            begin
              if HasReverter(TJSONMarshal.ComposeKey(ClassType, FIELD_ANY)) then
                RevertType(recField, Instance,
                  Reverter(TJSONMarshal.ComposeKey(ClassType, FIELD_ANY)),
                  jsonFieldVal)
              else
              begin
...
begin
  ...
  begin
    ...
    // check for reverters
    if HasReverter(TJSONMarshal.ComposeKey(ClassType, FieldName)) then
    begin
      revEv := Reverter(TJSONMarshal.ComposeKey(ClassType, FieldName));
      case revEv.ReverterType of
        rtTypeObjects:
          begin
            if jsonFieldVal is TJSONArray then
    ...
    else if HasReverter(TJSONMarshal.ComposeKey(ClassTypeOf(Data, FieldName),
        FIELD_ANY)) then
    begin
      revEv := Reverter(TJSONMarshal.ComposeKey(ClassTypeOf(Data, FieldName),
          FIELD_ANY));
      case revEv.ReverterType of
        rtTypeObjects:
          begin
    ...
    begin
      ...
      begin
        ...
        begin
          // object...
          SetField(Data, FieldName, CreateObject(TJSONObject(jsonFieldVal)));
        end
...
begin
  TMonitor.Enter(FReverters);
  try
    FReverters.AddOrSetValue(TJSONMarshal.ComposeKey(clazz, Field), Reverter);
  finally
    TMonitor.Exit(FReverters);
  end;
end;
...
begin
  ...
  try
    Result := CreateObject(Root) finally FObjectHash.Clear;
  end;
end;
...
begin
  ...
  for rttiField in rttiType.GetFields do
    if (rttiField.FieldType <> nil)
        ...
        and (rttiField.GetValue(Data).AsObject <> nil) then
        begin
          rttiField.GetValue(Data).AsObject.Free;
