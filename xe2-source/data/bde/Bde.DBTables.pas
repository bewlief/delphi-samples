...
uses System.Variants, Winapi.Windows, System.SysUtils, System.Classes, Data.DB, Data.DBCommon, Data.DBCommonTypes,
  BDE, Bde.SMINTF;
...
{$HPPEMIT '#pragma link "vcl.dblogdlg.obj"'}
...
{ TTable }
  ...
  private
    ...
    function GetDriverTypeName(Buffer: PAnsiChar): PAnsiChar;
...
{ TBatchMove }
  ...
  private
    ...
    function ConvertName(const Name: TFileName; Buffer: PAnsiChar): PAnsiChar;
...
{ TQuery }
  ...
  private
    ...
    procedure PrepareSQL(Value: PAnsiChar);
  ...
  protected
    ...
    function GetParamsCount: Word;
    function SetDBFlag(Flag: Integer; Value: Boolean): Boolean; override;
    procedure GetStatementHandle(SQLText: PAnsiChar); virtual;
...
function NativeCompareTextBuf(Locale: TLocale; const S1, S2: PAnsiChar; Len: Integer): Integer;
...
uses Data.DBConsts, Bde.Bdeconst, Winapi.ActiveX;

const
  TableTypeDriverNames: array[TTableType] of PAnsiChar =
...
begin
  Result := False;
  Field := ADataSet.FindField(string(ADataSources.szSourceFldName));
  ...
  Dec(I);
  StrCopy(@ADataSources.szOrigFldName[0], PopValue);
  StrCopy(@ADataSources.szTblName[0], PopValue);
  StrCopy(@ADataSources.szDbName[0], PopValue);
...
const
  WideDataTypes= [ftWideString, ftFixedWideChar, ftWideMemo];

function GetParamDataSize(Param: TParam): Integer;
begin
  with Param do
  begin
    if ((DataType in [ftString, ftFixedChar]) and (Length(AsAnsiString) > 255)) or
       (DataType in [ftBlob..ftTypedBinary,ftOraBlob,ftOraClob]) then
      Result := SizeOf(BlobParamDesc)
    else if (DataType in WideDataTypes) then
      Result := Length(AsAnsiString)
    else
      Result := GetDataSize;
  end;
end;
...
procedure GetParamData(Param: TParam; Buffer: Pointer; const DrvLocale: TLocale);
  ...
  // Helper routine to store data into a string without doing any
  // data conversion.  This is needed becasue NativeStr is defined as
  // a string instead of an AnsiString or a TBytes.
  function MakeNativeStr(ByteData: Pointer; Len: Integer): string;
  begin
    SetLength(Result, (Len+1) div SizeOf(Char));
    Move(ByteData^, Pointer(Result)^, Len);
  end;
...
begin
  with Param do
    if DataType in [ftString, ftFixedChar, ftMemo] + WideDataTypes then
    begin
      AnsiNativeStr := VarToAnsiStr(Value);
      if (Length(AnsiNativeStr) > 255) or
         (DataType = ftMemo) or
         (DataType = ftWideMemo)then
      begin
        with BlobParamDesc(Buffer^) do
        begin
          if DrvLocale <> nil then
            AnsiToNativeBuf(DrvLocale, PAnsiChar(AnsiNativeStr), PAnsiChar(AnsiNativeStr), Length(AnsiNativeStr));
          // Make sure the string data pointed to by pBlobBuffer is reference counted.
          NativeStr := MakeNativeStr(Pointer(AnsiNativeStr), Length(AnsiNativeStr));
          pBlobBuffer := Pointer(NativeStr);
          ulBlobLen := Length(AnsiNativeStr);
        end;
      end else
      begin
        if (DrvLocale <> nil) then
          AnsiToNativeBuf(DrvLocale, PAnsiChar(AnsiNativeStr), Buffer, Length(AnsiNativeStr) + 1)
        else if DataType in WideDataTypes then
          Move(Pointer(AnsiNativeStr)^, Buffer^, Length(AnsiNativeStr))
        else
          GetData(Buffer);
      end;
    end
    ...
    begin
      ...
      begin
        NativeBytes := AsBytes;
        // Make sure the string data pointed to by pBlobBuffer is reference counted.
        NativeStr := MakeNativeStr(Pointer(NativeBytes), Length(NativeBytes));
        ulBlobLen := Length(NativeBytes);
        pBlobBuffer := Pointer(NativeStr);
      end;
    end else
      GetData(Buffer);
...
var
  ...
  RecBuffer: TBytes;
  BufStart, CurPtr, NullPtr: TRecordBuffer;
  DrvLocale: TLocale;
begin
  ...
  SetLength(RecBuffer, NumBytes);
  BufStart := @RecBuffer[0];
  CurPtr := BufStart;
  NullPtr := BufStart + NumBytes - SizeOf(SmallInt);
  ...
  try
    for I := 0 to Params.Count - 1 do
      ...
      begin
        ...
        iOffset := CurPtr - BufStart;
        if IsNull then
          iNullOffset := NullPtr - BufStart
        else if iFldType = fldZString then
        begin
          if DataType in WideDataTypes then
            iUnits1 := Length(AsAnsiString)
          else
...
// Helpers to reduce the need for AnsiString temps and casts.
function AnsiToNative(Locale: TLocale; const UStr: UnicodeString;
  NativeStr: PAnsiChar; MaxLen: Integer): PAnsiChar; overload;
begin
  Result := AnsiToNative(Locale, AnsiString(UStr), NativeStr, MaxLen);
end;

procedure NativeToAnsi(Locale: TLocale; NativeStr: PAnsiChar; var UStr: UnicodeString); overload;
var
  TempAnsiStr: AnsiString;
begin
  TempAnsiStr := AnsiString(UStr);
  NativeToAnsi(Locale, NativeStr, TempAnsiStr);
  UStr := string(TempAnsiStr);
end;
...
begin
  if Len > 0 then
    if Locale <> nil then
    begin
      TMonitor.Enter(CSAnsiToNative);
      try
        DbiAnsiToNative(Locale, Dest, Source, Len, DataLoss);
      finally
        TMonitor.Exit(CSAnsiToNative);
      end;
    end else
      CharToOemBuffA(Source, Dest, Len);
end;
...
begin
  if Len > 0 then
    if Locale <> nil then
    begin
      TMonitor.Enter(CSNativeToAnsi);
      try
        DbiNativeToAnsi(Locale, Dest, Source, Len, DataLoss);
      finally
        TMonitor.Exit(CSNativeToAnsi);
      end;
    end else
      OemToCharBuffA(Source, Dest, Len)
end;

function NativeCompareStr(Locale: TLocale; const S1, S2: AnsiString; Len: Integer): Integer;
begin
  Result := NativeCompareStrBuf(Locale, PAnsiChar(S1), PAnsiChar(S2), Len);
end;
...
function NativeCompareText(Locale: TLocale; const S1, S2: AnsiString; Len: Integer): Integer;
begin
  Result := NativeCompareTextBuf(Locale, PAnsiChar(S1), PAnsiChar(S2), Len);
end;
...
function IsStandardType(AType: PAnsiChar): Boolean;
begin
...
function StringListToParams(List: TStrings): AnsiString;
var
  S: string;
  P, I: Integer;
begin
  ...
  begin
    ...
    if (P >= 0) and (P < Length(S)) then
      Result := AnsiString(Format('%s%s:"%s";', [Result, Copy(S, 1, P-1), Copy(S, P+1, 255)]));
  end;
...
begin
  ...
  begin
    ...
    TDBError.Create(Self, ErrorCode, 0, Message);
  end else
  begin
    TDBError.Create(Self, ErrorCode, 0, GetErrorString(ErrorCode));
    ...
    try
      ...
      begin
        ...
        if (EntryCode = DBIERR_NONE) or (EntryCode = DBIERR_NOTINITIALIZED) then
          Break;
        ContextBufStr := string(ContextBuf);
        TDBError.Create(Self, EntryCode, NativeError, ContextBufStr);
        if (NativeError = 0) and (ErrorCode <> DBIERR_USERCONSTRERR) then
          AddMessage(GetErrorString(EntryCode));
        AddMessage(Trim(ContextBufStr));
        Inc(ErrorIndex);
      end;
...
begin
  ...
  try
    ...
    try
      ...
      Check(DbiAddDriver(nil, PAnsiChar(StrToOem(AnsiString(Name))), PAnsiChar(Params), Bool(-1)));
    finally
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        Database := OpenDatabase(string(szDbName));
      ...
      begin
        AliasParams := TStringList.Create;
        try
          GetAliasParams(string(szDbName), AliasParams);
        ...
        if Assigned(LoginDialogExProc) then
          if LoginDialogExProc(string(szDbName), UserName, Password, True) then
          begin
    ...
    begin
      Database := FindDatabase(string(szDbName));
...
begin
  ...
  LockSession;
  try
    DbiDeleteDriver(nil, PAnsiChar(StrToOem(AnsiString(Name))), False);
  finally
    UnlockSession;
  end;
end;
...
begin
  ...
  if DbiCfgPosition(nil, PAnsiChar(AnsiString(CfgPath))) = 0 then
    Check(DbiCfgDropRecord(nil, PAnsiChar(AnsiString(CfgPath))));
end;
...
begin
  LockSession;
  try
    if DbiGetDatabaseDesc(PAnsiChar(StrToOem(AnsiString(AliasName))), @Desc) <> 0 then
      DatabaseErrorFmt(SInvalidAliasName, [AliasName]);
  finally
    UnlockSession;
  end;
  if Desc.szDBType[Length(Desc.szDBType) - 1] <> #0 then
    Desc.szDBType[Length(Desc.szDBType) - 1] := #0;
  OemToCharA(Desc.szDBType, Desc.szDBType);
  Result := string(Desc.szDBType);
end;
...
begin
  ...
  try
    ...
    try
      ...
      begin
        OemToCharA(Desc.szName, Desc.szName);
        List.Add(string(Desc.szName));
      end;
    finally
...
begin
  LockSession;
  try
    Check(DbiOpenCfgInfoList(nil, dbiREADONLY, cfgPERSISTENT, PAnsiChar(AnsiString(Format(Path,
      [Section]))), Cursor));
  finally
    UnlockSession;
  end;
...
begin
  ...
  try
    ...
    begin
      ...
      GetConfigParams('\DRIVERS\%s\DB OPEN', string(StrToOem(AnsiString(DriverName))), List);
  finally
    List.EndUpdate;
  end;
end;
...
begin
  ...
  end else if Assigned(Data.DB.PasswordDialog) then
    Result := Data.DB.PasswordDialog(Self as IDBSession)
  else
    Result := False;
end;
...
begin
  ...
  try
    ...
    Database := OpenDatabase(DatabaseName);
    try
      Check(DbiOpenFieldList(Database.Handle, PAnsiChar(AnsiString(TableName)), '', False, Cursor));
      try
        while DbiGetNextRecord(Cursor, dbiNOLOCK, @Desc, nil) = 0 do
          with Desc do
          begin
...
begin
  ...
  try
    ...
    StrPLCopy(Env.szLang, AnsiString(SIDAPILangID), SizeOf(Env.szLang) - 1);
    Status := DbiInit(@Env);
...
begin
  ...
  try
    ...
    try
      ...
      Check(DbiAddAlias(nil, PAnsiChar(StrToOem(AnsiString(Name))),
        PAnsiChar(StrToOem(AnsiString(DrvName))), PAnsiChar(AnsiString(Params)), Bool(-1)));
    finally
...
begin
  ...
  try
    ...
    try
      CheckConfigMode(CfgMode);
      DeleteConfigPath('\DATABASES\%s', string(StrToOem(AnsiString(Name))));
    finally
      if RestoreMode then ConfigMode := cfgModeSave;
    end;
  finally
...
begin
  ...
  try
    Result := (Name <> '') and
      (DbiCfgPosition(nil, PAnsiChar(AnsiString(Format('\DATABASES\%s', [StrToOem(AnsiString(Name))])))) = 0);
  finally
    UnlockSession;
  end;
end;
...
begin
  ...
  try
    ...
    try
      ...
      OemName := string(StrToOem(AnsiString(Name)));
...
begin
  ...
  try
    ...
    try
      ...
      if IsStandardType(PAnsiChar(AnsiString(Name))) then
...
begin
  ...
  try
    with ParamList do
      Check(DbiCfgModifyRecord(nil, PAnsiChar(AnsiString(Format(Path, [Node]))), FieldCount,
...
begin
  try
    ...
    FSMClient.AddStatement(Data, StrLen(PAnsiChar(Data)));
  except
    SetTraceFlags([]);
  end;
  Result := cbrUSEDEF;
    ...
    begin
      ...
      if P <> 0 then
        ...
        begin
          ...
          iLen := Length(S) - P + 1;
          StrCopy(BufPtr, PAnsiChar(Copy(S, P + 1, 255)));
          CharToOemA(BufPtr, BufPtr);
          Inc(BufPtr, iLen);
        end;
    end;
  end;
...
function TDatabase.Execute(const SQL: string; Params: TParams = nil;
  ...
  begin
    ...
    begin
      ...
      if (Info.HashCode = HashCode) and
         SameText(Info.SQLText, SQL) then
      begin
        Result := Info;
        break;
      end;
  ...
  begin
    ...
    begin
      ...
      while True do
      begin
        Status := DbiQPrepare(Result, PAnsiChar(AnsiString(SQL)));
        case Status of
        DBIERR_NONE: break;
...
begin
  ...
  begin
    ...
    Check(DbiQExecDirect(Handle, qrylangSQL, PAnsiChar(AnsiString(SQL)), Cursor));
...
begin
  ...
  begin
    ...
    OemToCharA(SDirectory, SDirectory);
    Result := string(AnsiString(SDirectory));
  end else
    Result := '';
end;
    ...
    begin
      ...
      begin
        ...
        if Desc.szDBType[sizeOf(Desc.szDBType) - 1] <> #0 then
          Desc.szDBType[sizeOf(Desc.szDBType) - 1] := #0;
        OemToCharA(Desc.szDbType, Desc.szDbType);
        DriverType := string(Desc.szDbType);
        ...
        try
          ...
          try
            if (FDatabaseType = '') and IsDirectory(FDatabaseName) then
              DBName := '' else
              DBName := StrToOem(AnsiString(FDatabaseName));
            ...
            Check(DbiOpenDatabase(PAnsiChar(DBName), nil,
              DBIOpenMode(Integer(OpenModes[FReadOnly]) or OpenModeFlag),
              ShareModes[FExclusive], PAnsiChar(StrToOem(AnsiString(DBPassword))), OptParamCount, OptFldDesc,
              OptParam, FHandle));
...
begin
  if Handle <> nil then
    Check(DbiSetDirectory(Handle, PAnsiChar(StrToOem(AnsiString(Value)))));
end;
...
begin
  if Connected and IsSQLBased then
    Check(DbiSchemaCacheFlush(FHandle, PAnsiChar(AnsiString(TableName))));
end;
...
begin
  ...
  FBookmarkOfs := FRecInfoOfs + SizeOf(TBDERecInfo);
...
begin
  if FCacheBlobs then
    for I := 0 to BlobFieldCount - 1 do
      SetLength(TBlobDataArray(Buffer + FBlobCacheOfs)[I], 0);
end;
...
begin
  case State of
    ...
    dsSetKey: RecBuf := TRecordBuffer(FKeyBuffer) + SizeOf(TBDEKeyBuffer);
...
begin
  ...
  begin
    ...
    begin
      Inc(I);
      FName := AnsiString(Format('%s_%d', [FieldName, I]));
    end;
...
function TBDEDataSet.GetBlobData(Field: TField; Buffer: TRecordBuffer): TBlobData;
begin
  Result := TBlobDataArray(Buffer + FBlobCacheOfs)[Field.Offset];
end;

procedure TBDEDataSet.SetBlobData(Field: TField; Buffer: TRecordBuffer; Value: TBlobData);
begin
...
function TBDEDataSet.Translate(Src, Dest: PAnsiChar; ToOem: Boolean): Integer;
begin
  Result := StrLen(PAnsiChar(Src));
  if ToOem then
...
procedure TBDEDataSet.InternalGotoBookmark(Bookmark: Pointer);
begin
...
procedure TBDEDataSet.GetBookmarkData(Buffer: TRecordBuffer; Data: Pointer);
begin
  Move(Buffer[FBookmarkOfs], Data^, BookmarkSize);
end;

procedure TBDEDataset.SetBookmarkData(Buffer: TRecordBuffer; Data: Pointer);
begin
...
begin
  ResetCursorRange;
  UpdateCursorPos;
  Status := DbiSwitchToIndex(FHandle, PAnsiChar(AnsiString(IndexName)),
    PAnsiChar(AnsiString(TagName)), 0, True);
  if Status = DBIERR_NOCURRREC then
    Status := DbiSwitchToIndex(FHandle, PAnsiChar(AnsiString(IndexName)),
    PAnsiChar(AnsiString(TagName)), 0, False);
  Check(Status);
  FKeySize := 0;
  FExpIndex := False;
  FCaseInsIndex := False;
  ...
  try
    for KeyIndex := Low(TKeyIndex) to High(TKeyIndex) do
      FKeyBuffers[KeyIndex] := InitKeyBuffer(
        AllocMem(SizeOf(TBDEKeyBuffer) + FRecordSize));
  except
    FreeKeyBuffers;
    raise;
  end;
...
begin
  for KeyIndex := Low(TKeyIndex) to High(TKeyIndex) do
    DisposeMem(FKeyBuffers[KeyIndex], SizeOf(TBDEKeyBuffer) + FRecordSize);
end;

function TBDEDataSet.InitKeyBuffer(Buffer: PKeyBuffer): PKeyBuffer;
begin
  FillChar(Buffer^, SizeOf(TBDEKeyBuffer) + FRecordSize, 0);
  DbiInitRecord(FHandle, TRecordBuffer(Buffer) + SizeOf(TBDEKeyBuffer));
  Result := Buffer;
end;
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        StartKey := TRecordBuffer(RangeStart) + SizeOf(TBDEKeyBuffer);
      ...
      begin
        EndKey := TRecordBuffer(RangeEnd) + SizeOf(TBDEKeyBuffer);
      ...
      Move(FKeyBuffers[kiRangeStart]^, FKeyBuffers[kiCurRangeStart]^,
        SizeOf(TBDEKeyBuffer) + FRecordSize);
...
begin
  ...
  Move(FKeyBuffer^, FKeyBuffers[kiSave]^, SizeOf(TBDEKeyBuffer) + FRecordSize);
...
begin
  ...
  if Commit then
    ...
    Move(FKeyBuffers[kiSave]^, FKeyBuffer^, SizeOf(TBDEKeyBuffer) + FRecordSize);
  SetState(dsBrowse);
  DataEvent(deDataSetChange, 0);
end;
...
function TBDEDataSet.ConstraintCallBack(Req: DsInfoReq;
  ...
  begin
    Result := False;
    Field := FindField(string(ADataSources.szSourceFldName));
    if (Field <> nil) and (Field.Required or (Field.ImportedConstraint <> '') or
      (Field.CustomConstraint <> '')) then
    begin
      StrCopy(ADataSources.szSQLExprImport, PAnsiChar(AnsiString(Field.ImportedConstraint)));
      StrCopy(ADataSources.szSQLExprCustom, PAnsiChar(AnsiString(Field.CustomConstraint)));
      StrCopy(ADataSources.szErrStrCustom, PAnsiChar(AnsiString(Field.ConstraintErrorMessage)));
      StrCopy(ADataSources.szErrStrImport, PAnsiChar(AnsiString(Field.ConstraintErrorMessage)));
  ...
  begin
    ...
    begin
      StrCopy(szSQLExprImport, PAnsiChar(AnsiString(ImportedConstraint)));
      StrCopy(szSQLExprCustom, PAnsiChar(AnsiString(CustomConstraint)));
      StrCopy(szErrStrCustom, PAnsiChar(AnsiString(ErrorMessage)));
      StrCopy(szErrStrImport, PAnsiChar(AnsiString(ErrorMessage)));
    end;
  end;
  ...
  begin
    ...
    begin
      StrCopy(ADataSources.szSQLExprImport, PAnsiChar(AnsiString(Field.DefaultExpression)));
      Result := True;
    end;
  end;
...
begin
  ...
  if Status <> DBIERR_NONE then
    if Status = DBIERR_UPDATEABORT then System.SysUtils.Abort
...
procedure TBDEDataSet.DataEvent(Event: TDataEvent; Info: NativeInt);
...
begin
  ...
  begin
    ...
    Check(DbiSetProp(hDBIObj(FHandle), curREFINSERTTABLENAME,
      Integer(PAnsiChar(AnsiString(TReferenceField(DataSetField).ReferenceTableName)))));
  end;
  inherited;
end;
...
function TBatchMove.ConvertName(const Name: TFileName; Buffer: PAnsiChar): PAnsiChar;
begin
  if Name <> '' then
...
begin
  ...
  if IsXBaseTable then
    ...
    begin
      CharToOemA(PAnsiChar(AnsiString(IndexFiles[I])), SIndexName);
...
begin
  ...
  begin
    ...
    if (IndexName <> '') and IsProductionIndex(IndexName) then
       Check(DbiSwitchToIndex(Result, PAnsiChar(AnsiString(IndexName)), PAnsiChar(AnsiString(IndexTag)), 0, False));
  end;
end;
...
begin
  ...
  try
    if Database.IsSQLBased then
    begin
      DriverName := PAnsiChar(AnsiString(DBSession.GetAliasDriverName(DatabaseName)));
...
begin
  ...
  begin
    ...
    if (LName <> '') and (OsLdLoadBySymbName(PAnsiChar(AnsiString(LName)), TempLocale) = 0) then
...
begin
  ...
  begin
    ...
    CheckBrowseMode;
    Check(DbiDeleteIndex(DBHandle, Handle, nil, nil, PAnsiChar(AnsiString(IndexName)),
      PAnsiChar(AnsiString(IndexTag)), 0));
  end else
  begin
    GetIndexParams(Name, False, IndexName, IndexTag);
    SetDBFlag(dbfTable, True);
...
begin
  ...
  if IsXBaseTable and (IndexName <> '') then
    if AnsiUpperCase(ExtractFileExt(IndexName)) = '.NDX' then
      Result := False
    else Result := AnsiUpperCaseFileName(ChangeFileExt(string(NativeTableName), '')) =
...
begin
  ...
  try
    RecBuffer := TRecordBuffer(KeyBuffer) + SizeOf(TBDEKeyBuffer);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Check(DbiSwitchToIndex(FLookupHandle, PAnsiChar(AnsiString(IndexName)),
          PAnsiChar(AnsiString(IndexTag)), 0, False));
      end;
...
var
  ...
  begin
    ...
    begin
      ...
      begin
        iOptParams := 1;
        StrCopy(@Level, PAnsiChar(AnsiString(IntToStr(FTableLevel))));
        pOptData := @Level;
...
procedure TTable.DataEvent(Event: TDataEvent; Info: NativeInt);
begin
...
function TTable.GetDriverTypeName(Buffer: PAnsiChar): PAnsiChar;
var
  Length: Word;
begin
  Result := Buffer;
...
function TTable.GetTableTypeName: PAnsiChar;
begin
...
function TTable.NativeTableName: PAnsiChar;
begin
    ...
    while DbiGetNextRecord(Cursor, dbiNOLOCK, @Desc, nil) = 0 do
      ...
      begin
        ...
        if (TParamType(eParamType) = ptResult) and (ParamName = '') then
          ParamName := AnsiString(SResultName);
...
begin
  ...
  begin
    CurPtr := PAnsiChar(FRecordBuffer);
...
begin
  ...
  NumBytes := High(FRecordBuffer) + 1;
  CurPtr := TRecordBuffer(FRecordBuffer);
  DrvLocale := GetStatementLocale(StmtHandle);
  try
    for I := 0 to FParams.Count - 1 do
    begin
...
begin
  SQLBinary := AnsiStrAlloc(Stream.Size);
...
begin
  ...
  begin
    ...
    begin
      ...
      if Length(Text) > 1 then PrepareSQL(PAnsiChar(AnsiString(Text)))
...
procedure TQuery.PrepareSQL(Value: PAnsiChar);
begin
...
procedure TQuery.GetStatementHandle(SQLText: PAnsiChar);
const
  ...
  begin
    ...
    begin
      FCached := FDataSet.FCacheBlobs and (FBuffer = FDataSet.ActiveBuffer) and
        (FField.IsNull or (FDataSet.GetBlobData(FField, FBuffer) <> nil));
      OpenMode := dbiReadOnly;
    end else
    begin
...
begin
  ...
  begin
    ...
    begin
      ...
      if Result > 0 then
      begin
        Move(TRecordBuffer(FDataSet.GetBlobData(FField, FBuffer))[FPosition], Buffer, Result);
        ...
        DBIERR_NONE, DBIERR_ENDOFBLOB:
          begin
            ...
            begin
              ...
              Move(Buffer, FBlobData[FPosition], Result);
