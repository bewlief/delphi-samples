...
uses Winapi.Windows, System.SysUtils, System.Classes, BDE, Data.DB, Bde.DBTables;
...
begin
  Check(DbiDROpenRelTypeList(DRHandle, PAnsiChar(AnsiString(ObjectTypeName)), Result));
end;
...
begin
  Check(DbiDROpenAttrTypeList(DRHandle, PAnsiChar(AnsiString(TypeName)), Result));
end;
...
begin
  Check(DbiDROpenObjSet(DRHandle, PAnsiChar(AnsiString(ObjectTypeName)), @FSource, Pointer(FRelName),
...
begin
  ...
  Check(DbiDROpenRelSet(DRHandle, PAnsiChar(AnsiString(RelationshipTypeName)), PS, PT,
    Pointer(Condition), Result));
end;
