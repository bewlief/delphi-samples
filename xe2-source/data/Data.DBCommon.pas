...
uses Data.DBConsts, Data.FMTBcd, System.WideStrUtils;
...
begin
  if VarType(Value) = varString then
    C := StrToCurr(String(AnsiString(TVarData(Value).VString))) else
    C := Value;
...
begin
  if VarType(Value) = varString then
    BCD := StrToBcd(string(AnsiString(TVarData(Value).VString))) else
    BCD := VarToBcd(Value);
  Result := PutConstNode(ftBCD, @BCD, 18);
end;
...
begin
  if VarType(Value) = varString then
  begin
    if not TryStrToDate(string(AnsiString(TVarData(Value).VString)), DateTime) then
...
begin
  if VarType(Value) = varString then
    TimeStamp := StrToSQLTimeStamp(string(AnsiString(TVarData(Value).VString))) else
...
begin
  if VarType(Value) = varString then
    TimeStampOffset := StrToSQLTimeStampOffset(string(AnsiString(TVarData(Value).VString))) else
...
begin
  if VarType(Value) = varString then
    F := StrToFloat(string(AnsiString(TVarData(Value).VString))) else
    F := Value;
...
begin
  if VarType(Value) = varString then
    I := StrToInt(string(AnsiString(TVarData(Value).VString))) else
    I := Value;
  Size := 2;
  case DataType of
    ftByte:
...
begin
  if VarType(Value) = varString then
    DateTime := StrToTime(string(AnsiString(TVarData(Value).VString))) else
    DateTime := VarToDateTime(Value);
  TimeStamp := DateTimeToTimeStamp(DateTime);
...
begin
  FDecimalSeparator := WideChar(FormatSettings.DecimalSeparator);
  FFieldMap := FieldMap;
  FStrTrue := STextTrue;
...
begin
  ...
  else if (CompareText(Node^.FData , 'UPPER') = 0) or
          (CompareText(Node^.FData , 'LOWER') = 0 ) or
          (CompareText(Node^.FData , 'SUBSTRING') = 0) then
  begin
    Node^.FDataType := PExprNode(Node^.FArgs.Items[0])^.FDataType;
  end
  else if (CompareText(Node^.FData , 'TRIM') = 0) or
          (CompareText(Node^.FData , 'TRIMLEFT') = 0 ) or
          (CompareText(Node^.FData , 'TRIMRIGHT') = 0 ) then
  begin
    Node^.FDataType := PExprNode(Node^.FArgs.Items[0])^.FDataType;
...
begin
  ...
  begin
    ...
    else if ((FLeft.FDataType in UnicodeStringFieldTypes) or
             (FRight.FDataType in UnicodeStringFieldTypes)) and (FOperator = coADD) then
    begin
      if not (FLeft.FDataType in UnicodeStringFieldTypes) and (FLeft.FKind = enConst) then
        FLeft.FDataType := ftWideString;
      if not (FRight.FDataType in UnicodeStringFieldTypes) and (FRight.FKind = enConst) then
        FRight.FDataType := ftWideString;
      FDataType := ftWideString;
    end
    else
      DatabaseError(SExprTypeMis);
