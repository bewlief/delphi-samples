...
uses
   System.SysUtils, System.Classes, System.Variants, System.MaskUtils, System.WideStrings,
   Data.SqlTimSt, Data.FMTBcd, Data.DBCommonTypes;
...
{ TField }
  ...
  protected
    ...
    function GetAsWideString: UnicodeString; virtual;
    ...
    procedure GetWideText(var Text: UnicodeString; DisplayText: Boolean); virtual;
    ...
    procedure SetAsWideString(const Value: UnicodeString); virtual;
    ...
    procedure SetWideText(const Value: UnicodeString); virtual;
...
{ TParam }
  ...
  private
    ...
    procedure GetStreamData(Buffer: Pointer);
  protected
...
{ IProviderSupport interface }
  ...
  ['{BA98319B-A3FD-4A6A-AC15-FAD83C28E253}']
    procedure PSEndTransaction(Commit: Boolean);
    procedure PSExecute;
    ...
    function PSGetCommandType: TPSCommandType;
    function PSGetDefaultOrder: TIndexDef;
    function PSGetKeyFields: WideString;
    function PSGetParams: TParams;
    function PSGetQuoteChar: WideString;
    function PSGetTableName: WideString;
    function PSGetIndexDefs(IndexTypes: TIndexOptions = [ixPrimary..ixNonMaintained]): TIndexDefs;
    function PSGetUpdateException(E: Exception; Prev: EUpdateError): EUpdateError;
    function PSInTransaction: Boolean;
    function PSIsSQLBased: Boolean;
    ...
    procedure PSReset;
    procedure PSSetParams(AParams: TParams);
    procedure PSSetCommandText(const CommandText: WideString);
    procedure PSStartTransaction;
    function PSUpdateRecord(UpdateKind: TUpdateKind; Delta: TDataSet): Boolean;
  end deprecated;
...
{ TDataSet }
  ...
  private
    ...
    function GetBuffer(Index: Integer): TRecordBuffer;
  ...
  protected
    { IProviderSupport }
    function PSExecuteStatement(const ASQL: string; AParams: TParams;
      ResultSet: Pointer = nil): Integer; overload; virtual;
    function PSExecuteStatement(const ASQL: WideString; AParams: TParams;
      ResultSet: Pointer = nil): Integer; overload; virtual; deprecated;
    function PSGetCommandText: string; virtual;
    function PSGetCommandTextW: WideString; virtual; deprecated;
    ...
    function PSGetQuoteCharW: WideString; virtual; deprecated;
    function PSGetTableName: string; virtual;
    function PSGetTableNameW: WideString; virtual; deprecated;
    procedure PSSetCommandText(const CommandText: string); overload; virtual;
    procedure PSSetCommandText(const CommandText: WideString); overload; virtual; deprecated;
    procedure PSEndTransaction(Commit: Boolean); virtual;
    procedure PSExecute; virtual;
    procedure PSGetAttributes(List: TList); virtual;
    function IProviderSupport.PSGetCommandText = PSGetCommandText;
    function PSGetCommandType: TPSCommandType; virtual;
    function PSGetDefaultOrder: TIndexDef; virtual;
    function IProviderSupport.PSGetKeyFields = PSGetKeyFields;
    function PSGetParams: TParams; virtual;
    function IProviderSupport.PSGetQuoteChar = PSGetQuoteChar;
    function IProviderSupport.PSGetTableName = PSGetTableName;
  ...
  protected
    ...
    function TempBuffer: TRecordBuffer; inline;
    ...
    property Buffers[Index: Integer]: TRecordBuffer read GetBuffer;
  ...
  protected { Method stubs for UniDirectional/Readonly/Unbuffered datasets }
    ...
    procedure SetBookmarkData(Buffer: TRecordBuffer; Data: Pointer); overload; virtual;
  ...
  public
    ...
    procedure GetFieldNames(List: TStrings); overload; virtual;
    procedure GetFieldNames(List: TWideStrings); overload; virtual;
...
uses System.RTLConsts, System.Types, System.WideStrUtils, Data.DBConsts;
...
function TParamStreamObject.GetBytes: TBytes;
const
  MaxGrowth = $2000;
var
  Stream: TStream;
...
procedure TDefCollection.GetItemNames(List: TWideStrings);
var
  I: Integer;
begin
  List.BeginUpdate;
...
begin
  if Collection is TFieldDefs then
    Result := TFieldDefs(Collection).DataSet.GetFieldClass(Self) else
    Result := nil;
end;
...
procedure TFields.GetFieldNames(List: TWideStrings);
var
  I: Integer;
begin
  List.BeginUpdate;
...
function TField.GetAsWideString: UnicodeString;
begin
...
procedure TField.GetWideText(var Text: UnicodeString; DisplayText: Boolean);
var
  AText: string;
begin
...
begin
  if ValidChars <> [] then
    Result := CharInSet(InputChar, ValidChars)
  else
    Result := True;
end;
...
procedure TField.SetWideText(const Value: UnicodeString);
begin
  SetText(Value);
end;
...
begin
  SetData(PChar(Value), False);
end;

procedure TWideStringField.SetAsWideString(const Value: UnicodeString);
begin
  SetData(PChar(Value), False);
end;
...
begin
  ...
  if Result then
    case DataType of
      ftShortint: Value := Data.S;
      ftByte: Value := Data.B;
      ftSmallint: Value := Data.I;
      ftWord: Value := Data.W;
      ftLongWord: Value := Data.U32;
    else
      Value := Data.L;
    end;
end;
...
procedure TBinaryField.SetAsBytes(const Value: TBytes);
var
  Len: Integer;
  Data: Variant;
  PData: Pointer;
...
begin
  ...
  begin
    LoadFromStrings(TWideStrings(Source));
    Exit;
  end;
...
begin
  ...
  begin
    SaveToStrings(TWideStrings(Dest));
    Exit;
  end;
...
function TBlobField.GetAsWideString: UnicodeString;
var
  Len: Integer;
begin
...
procedure TBlobField.LoadFromStrings(Strings: TWideStrings);
var
  BlobStream: TStream;
begin
  BlobStream := DataSet.CreateBlobStream(Self, bmWrite);
...
procedure TBlobField.SaveToStrings(Strings: TWideStrings);
var
  BlobStream: TStream;
begin
  BlobStream := DataSet.CreateBlobStream(Self, bmRead);
...
procedure TBlobField.SetAsWideString(const Value: UnicodeString);
begin
  SetData(Pointer(Value), Length(Value) * SizeOf(Char));
end;

procedure TBlobField.SetAsAnsiString(const Value: AnsiString);
begin
  SetData(Pointer(Value), Length(Value) * SizeOf(AnsiChar));
end;

procedure TBlobField.SetAsBytes(const Value: TBytes);
begin
  SetData(Pointer(Value), Length(Value));
end;

procedure TBlobField.SetData(Buffer: Pointer; Len: Integer);
begin
  with DataSet.CreateBlobStream(Self, bmWrite) do
    try
...
begin
  ...
  begin
    case VarType(Value) of
      ...
      varOleStr: SetAsWideString(UnicodeString(Value));
    else
...
function TMemoField.GetAsWideString: UnicodeString;
begin
  Result := UnicodeString(GetAsAnsiString);
end;
...
procedure TMemoField.SetAsWideString(const Value: UnicodeString);
begin
  SetAsAnsiString(AnsiString(Value));
end;
...
begin
  SetAsWideString(UnicodeString(Value));
end;
...
begin
  ...
  case DataType of
    ...
    ftWideString, ftWideMemo, ftFixedWideChar:
      Result := (Length(VarToStr(FData)) + 1) * 2;
    ...
    ftBlob, ftGraphic..ftTypedBinary,ftOraClob,ftOraBlob:
      if VarIsArray(FData) then
        ...
        Result := Length(VarToStr(FData)) * 2
...
begin
  case DataType of
    ...
    begin
      ...
      Move(PChar(TempWideStr)^, Buffer^, (Length(TempWideStr) + 1) * SizeOf(Char));
    end;
...
procedure TParam.SetAsWideString(const Value: UnicodeString);
begin
  if FDataType <> ftFixedWideChar then FDataType := ftWideString;
  Self.Value := Value;
end;
...
procedure TParam.SetAsBytes(const Value: TBytes);
begin
...
function TParam.GetAsWideString: UnicodeString;
begin
  if VarType(FData) = varUString then
    Result := UnicodeString(FData)
  else
    Result := GetAsString;
end;
...
begin
  ...
  begin
    case VarType(FData) of
      varOleStr:
        begin
          Len := Length(UnicodeString(FData)) * SizeOf(Char);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        else if (DataType = ftFMTBcd) and (F is TFMTBCDField) then
...
procedure TDataSet.GetFieldNames(List: TWideStrings);
begin
  if FFields.Count > 0 then
    List.Assign(FieldList)
  else
...
begin
  case Field.DataType of
    ...
    ftWideString, ftFixedWideChar:
      WStrLCopy(PWideChar(Dest), PWideChar(Source), Field.Size+1);
  end;
end;
...
function TDataSet.GetBuffer(Index: Integer): TRecordBuffer;
begin
...
begin
  for I := 0 to FFields.Count - 1 do
    with FFields[I] do
      ...
      begin
        FocusControl;
        DatabaseErrorFmt(SFieldRequired, [DisplayName]{, Self});
      end;
end;
...
function TDataSet.PSExecuteStatement(const ASQL: WideString; AParams: TParams;
  ResultSet: Pointer = nil): Integer;
begin
  Result := 0;
  DatabaseError(SProviderSQLNotSupported, Self);
...
function TDataSet.PSGetCommandTextW: WideString;
begin
...
function TDataSet.PSGetKeyFieldsW: WideString;
begin
...
function TDataSet.PSGetQuoteCharW: WideString;
begin
...
function TDataSet.PSGetTableNameW: WideString;
begin
...
procedure TDataSet.PSSetCommandText(const CommandText: WideString);
begin
end;
...
function TWideDataSet.PSExecuteStatement(const ASQL: string; AParams: TParams;
  ResultSet: Pointer = nil): Integer;
begin
  Result := PSExecuteStatement(WideString(ASQL), AParams, ResultSet);
end;

function TWideDataSet.PSExecuteStatement(const ASQL: WideString; AParams: TParams;
  ResultSet: Pointer = nil): Integer;
begin
  Result := 0;
  DatabaseError(SProviderSQLNotSupported, Self);
...
function TWideDataSet.PSGetCommandText: string;
begin
  Result := PSGetCommandTextW;
end;

function TWideDataSet.PSGetCommandTextW: WideString;
begin
  Result := '';
end;

function TWideDataSet.PSGetKeyFields: string;
begin
  Result := PSGetKeyFieldsW;
end;

function TWideDataSet.PSGetKeyFieldsW: WideString;
var
  i: integer;
begin
  for i := 0 to Fields.Count - 1 do
...
function TWideDataSet.PSGetQuoteChar: string;
begin
  Result := PSGetQuoteCharW;
end;

function TWideDataSet.PSGetQuoteCharW: WideString;
begin
  Result := '';
end;

function TWideDataSet.PSGetTableName: string;
begin
  Result := PSGetTableNameW;
end;

function TWideDataSet.PSGetTableNameW: WideString;
begin
  Result := '';
end;

procedure TWideDataSet.PSSetCommandText(const CommandText: string);
begin
  PSSetCommandText(WideString(CommandText));
end;

procedure TWideDataSet.PSSetCommandText(const CommandText: WideString);
begin
end;
