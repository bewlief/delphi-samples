...
uses System.Classes, System.SysUtils, Data.DBXJSON, IdAuthentication,
  IdHTTP,
  IdHTTPServer,
  IdAssignedNumbers,
  IdContext,
...
type
  ...
  protected
    ...
    function HMAC_SHA256(const StringToSign: String): TBytes;
  public
  ...
  /// <summary>Before 2009-09-19 the authentication is done against:
  /// For a queue/blob, you need to use this Canonical form:
  ///  VERB + "\n" +
  ///  Content - MD5 + "\n" +
  ///  Content - Type + "\n" +
  ///  Date + "\n" +
  ...
  public
    ...
    function PutBlockBlob(const ContainerName, BlobName: String; Content: TBytes;
    ...
    function PutBlock(const ContainerName: String; const BlobName: String; const BlockId: String;
                      Content: TBytes;  const LeaseId: String = '';  const ContentMD5: String = ''): boolean;
    function PutPage(const ContainerName: String; const BlobName: String; Content: TBytes; StartByte: int64;
  ...
  ///  </summary>
  ///  <remarks>
  ///   Beginning with version 2009-09-19, the Table service requires that all
  ///   REST calls include the DataServiceVersion and MaxDataServiceVersion
  ///   headers (set to 1.0;NetFx)
  ///  </remarks>
...
function EncodeBytes64(asBytes: TBytes): String;
var
  bitIdx: Integer;
  data: Integer;
  remainder: Integer;
...
function DecodeBytes64(S: String): TBytes;
var
  i: Integer;
  a: Integer;
  enc: Integer;
...
function TAzureBlobService.PutBlock(const ContainerName, BlobName,
  BlockId: String; Content: TBytes; const LeaseId, ContentMD5: String): boolean;
var
...
function TAzureBlobService.PutBlockBlob(const ContainerName, BlobName: String; Content: TBytes; const LeaseId: String;
...
function TAzureBlobService.PutPage(const ContainerName, BlobName: String;
   Content: TBytes; StartByte, EndByte: int64; PageWrite: String; const ContentMD5, LeaseId, IfSeqNbLte,
...
function TSharedKeyAuthorization.HMAC_SHA256(const StringToSign: String): TBytes;
var
  hmacInst: TIdHMACIntCtx;
  utf8Data: TBytes;
begin
  // get the HMAC-SHA256 for the key
  hmacInst := GetHMACSHA256HashInst(FSHAKey);
  // convert StringToSign into UTF8
  utf8Data := TEncoding.UTF8.GetBytes(StringToSign);
  // Sign it
  UpdateHMACInst(hmacInst, utf8Data);
  // build the Base64 result
  Result := FinalHMACInst(hmacInst);
end;
...
function TSignedIdentifier.HMAC_SHA256(const StringToSign: String): TBytes;
var
  hmacInst: TIdHMACIntCtx;
  key: TBytes;
begin
  ...
  // get the HMAC-SHA256 for the key
  hmacInst := GetHMACSHA256HashInst(key);
  // convert StringToSign into UTF8
  key := TEncoding.UTF8.GetBytes(StringToSign);
  UpdateHMACInst(hmacInst, key);
  Result := FinalHMACInst(hmacInst);
end;
...
begin
  ...
  for idx := 0 to entity.Size - 1 do
  begin
    jsonProp := entity.Get(idx);
    propName := jsonProp.JsonString.Value;
    if jsonProp.JsonValue is TJSONArray then
    begin
      propValue := ((jsonProp.JsonValue as TJSONArray).Get(0) as TJSONString).Value;
      propType  := ((jsonProp.JsonValue as TJSONArray).Get(1) as TJSONString).Value;
    end
    else
    begin
