...
type
  ...
  public
    ...
    /// <summary>The proxy host's port to use for the HTTP requests to the cloud</summary>
    property RequestProxyPort: Integer read FRequestProxyPort write FRequestProxyPort;
    /// <summary>True to use the default service URLs, false to use custom ones.</summary>
  ...
  protected
    ...
    function SignString(const StringToSign: String): TBytes; virtual; abstract;
  public
  ...
  protected
    ...
    function SignString(const StringToSign: String): TBytes; override;
  public
  ...
  /// <param name="Str">The string to Base64 decode.</param>
  /// <returns>The decoded version of the string.</returns>
  function DecodeBytes64(Str: String): TBytes;
...
begin
  ...
  if Str[encLen] = Pad then
  begin
    byteLen := byteLen - 1;
    if Str[encLen-1] = Pad then
      byteLen := byteLen - 1;
  end;
...
function TCloudSHA256Authentication.SignString(const StringToSign: String): TBytes;
var
  hmacInst: TIPHMACIntCtx;
  utf8Data: TBytes;
begin
  // get the HMAC-SHA256 for the key
  hmacInst := IPProcs.GetHMACSHA256HashInst(FSHAKey);
  // convert StringToSign into UTF8
  utf8Data := TEncoding.UTF8.GetBytes(StringToSign);
  // Sign it
  IPProcs.UpdateHMACInst(hmacInst, utf8Data);
  // build the Base64 result
  Result := IPProcs.FinalHMACInst(hmacInst);
end;
...
function TCloudSHA1Authentication.SignString(const StringToSign: String): TBytes;
var
  hmacInst: TIPHMACIntCtx;
  utf8Data: TBytes;
begin
  // get the HMAC-SHA256 for the key
  hmacInst := IPProcs.GetHMACSHA1HashInst(FSHAKey);
  // convert StringToSign into UTF8
  utf8Data := TEncoding.UTF8.GetBytes(StringToSign);
  // Sign it
  IPProcs.UpdateHMACInst(hmacInst, utf8Data);
...
begin
  FColumns := TList<TCloudTableColumn>.Create;
...
begin
  ...
  if (Index > 0) and (Index < Length(URL)) then
  begin
    Inst.Name := Copy(URL, Index + 1);
  end
  else
    Inst.Name := URL;
  Result := Inst;
