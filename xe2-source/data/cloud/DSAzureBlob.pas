...
type
  ...
  private
    ...
    function ContentOf(const FileName: String): TBytes;
  protected
...
uses Winapi.ActiveX, Vcl.ClipBrd, System.DateUtils, Data.DBXClientResStrs, Vcl.Dialogs, Vcl.Graphics, Xml.XMLDoc;
...
begin
  ...
  begin
    with BlobService do
    try
      LeaseBlob(container.Name, blob.Name);
      leaseId := ResponseHeader['x-ms-lease-id'];
      duration := 60;
      ...
      // display error
      ShowErrorMessage(SErrorGeneric + ResponseText);
...
begin
  ...
  begin
    with BlobService do
    try
      LeaseBlob(container.Name, blob.Name, 'break', '');
      duration := StrToInt(ResponseHeader['x-ms-lease-time']);
...
begin
  ...
  begin
    with BlobService do
    try
      LeaseBlob(container.Name, blob.Name, 'release', blob.LeaseID);
...
begin
  ...
  begin
    with BlobService do
    try
      LeaseBlob(container.Name, blob.Name, 'renew', blob.LeaseID);
      leaseId := ResponseHeader['x-ms-lease-id'];
      duration := 60;
      leaseExpires := IncSecond(Now, duration);
      blob.SetLeaseInfo(leaseId, leaseExpires);
...
begin
  ...
  with BlobService do
  try
    xmlStr := GetBlockList(container.Name, blob.Name, 'all');
    size := ResponseHeader['x-ms-blob-content-length'];
    Free;
  except
    // display error
    MessageDlg(SErrorGeneric + ResponseText, mtError, [mbOK], 0);
    Free;
    ...
    begin
      ...
      begin
        try
          ...
          with BlobService do
          try
            if not PutBlockList(container.Name, blob.Name, xmlList, '', cacheControl,
                                contentType, encoding, language, md5, blob.LeaseID, metaHeaders) then
            begin
               // display error
               ShowErrorMessage(Format(SCommitBlockListFailure,
                                       [container.Name, blob.Name, ReturnCode]) + ResponseText);
               exit;
            end
          finally
            Free;
...
function TAzureBlobManagement.ContentOf(const FileName: String): TBytes;
var
  fs: TFileStream;
begin
...
begin
  ...
  begin
    ...
    begin
        with BlobService do
        try
          CopyBlob(destCont, destBlob, container.Name, blob.Name, blob.Snapshot,
          ...
          if NeedsLoading(destContNode) then
            ...
            xmlStr := ListBlobs(destCont, ['prefix', 'maxresults', 'include'],
                       [destBlob, '1', 'metadata']);
    ...
    begin
      ...
      begin
        ...
        try
          ...
          PutBlockBlob(container.Name, blobName, content, EmptyStr, metaHeaders,
                       contentType, contentEncoding, contentLanguage, contentMD5);
          xmlStr := ListBlobs(container.Name, ['prefix', 'maxresults', 'include'],
                       [blobName, '1', 'metadata']);
  ...
  begin
    ...
    begin
      try
        ...
        with BlobService do
        try
          if not CreateContainer(containerName, metaHeaders, access) then
            raise Exception.Create(Format(SContainerCreateFailure, [containerName, ReturnCode]));
          xmlStr := ListContainers(['prefix', 'maxresults', 'include'], [containerName, '1', 'metadata']);
          Free;
        except
          ShowErrorMessage(SErrorGeneric + ResponseText);
          Free;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        with BlobService do
        try
          PutPageBlob(container.Name, blobName, contentLength, seqNb, metaHeaders,
                      contentType, contentEncoding, contentLanguage, contentMD5);
          xmlStr := ListBlobs(container.Name, ['prefix', 'maxresults', 'include'],
                       [blobName, '1', 'metadata']);
  ...
  begin
    ...
    begin
      try
        ...
        with BlobService do
        try
          if not CreateContainer('$root', metaHeaders, access) then
            raise Exception.Create(Format(SRootContainerCreateFailure, [ReturnCode]));
          Free;
        except
          ShowErrorMessage(SErrorGeneric + ResponseText);
          Free;
  ...
  begin
    ...
    begin
      try
        with BlobService do
        try
          snapshot := SnapshotBlob(container.Name, blob.Name, modSince, unmodSince,
                                   match, noMatch, '', metaHeaders);
          xmlStr := ListBlobs(container.Name, ['prefix', 'include', 'include'],
...
begin
  ...
  begin
    ...
    begin
        with BlobService do
        try
          if not DeleteBlob(container.Name, blob.Name, blob.Snapshot, blob.LeaseID) then
            raise Exception.Create(Format(SDeleteBlobFailure, [container.Name, blob.Name, ReturnCode]));
          Free;
        except
          ShowErrorMessage(SErrorGeneric + ResponseText);
          Free;
...
begin
  ...
  begin
    ...
    begin
        with BlobService do
        try
          if not DeleteContainer(container.Name) then
          begin
            MarkAsAvailable(containerNode);
            ShowErrorMessage(Format(SDeleteContainerFailure, [container.Name, ReturnCode]) + ResponseText);
            exit;
          end
        finally
          Free;
...
begin
  ...
  begin
      ...
      with BlobService do
      try
        if not DeleteContainer('$root') then
        begin
          MarkAsAvailable(FRootNode);
          ShowErrorMessage(Format(SRootContainerDeleteFailure, [ReturnCode]) + ResponseText);
          exit;
        end;
      finally
        Free;
  ...
  begin
    ...
    TAnonymousThread.Create(procedure begin
      try
        with BlobService do
        try
          if not GetBlob(container.Name, blob.Name, stream, blob.Snapshot) then
            ShowErrorMessage(SErrorGeneric + ResponseText);
        finally
          Free;
        end
...
begin
  ...
  begin
    try
      ...
      with BlobService do
      try
        xmlStr := GetContainerACL(container.Name);
        access := ResponseHeader['x-ms-blob-public-access'];
        Free;
      except
        MarkAsAvailable(containerNode);
        ShowErrorMessage(SErrorGeneric + ResponseText);
            ...
            try
              for i := 0 to count - 1 do
                if FACL.RawIdentifier[i] <> EmptyStr then
                  if not SetContainerACL(container.Name, FACL.Identifier[i], FACL.AccessPolicy[i],
...
begin
  ...
  TAnonymousThread.Create(procedure begin
    ...
    with BlobService do
    try
      if not GetBlobMetadata(container.Name, blob.Name, blob.Snapshot) then
      begin
        MarkAsAvailable(blobNode);
        ShowErrorMessage(Format(SMetadataBlobFailure, [container.Name, blob.Name, ReturnCode]) + ResponseText);
        exit;
      end;
      meta := TStringList.Create;
      PopulateContainer('x-ms-meta-', meta);
    finally
      Free;
    end;
    TThread.Queue(nil, procedure begin
        ...
        begin
          ...
          try
            with BlobService do
            try
              if not SetBlobMetadata(container.Name, blob.Name, meta, blob.LeaseID) then
              begin
                MarkAsAvailable(blobNode);
                MessageDlg(Format(SMetadataBlobFailure, [container.Name, blob.Name, ReturnCode]) +
                           ResponseText, mtError, [mbOK], 0);
                exit;
              end
            finally
              Free;
...
begin
  ...
  TAnonymousThread.Create(procedure begin
    try
      ...
      with BlobService do
      try
        if not GetBlobProperties(container.Name, blob.Name, blob.Snapshot) then
        begin
          MarkAsAvailable(blobNode);
          ShowErrorMessage(Format(SPropertiesFailure, [container.Name, blob.Name, ReturnCode]) + ResponseText);
          exit;
        end;
        md5 := ResponseHeader['Content-MD5'];
        contentType := ResponseHeader['Content-Type'];
        encoding := ResponseHeader['Content-Encoding'];
        language := ResponseHeader['Content-Language'];
      ...
      TThread.Queue(nil, procedure begin
        ...
        begin
          ...
          try
            with BlobService do
            try
              if not SetBlobProperties(container.Name, blob.Name, cache,
                             contentType, encoding, language, md5, blob.LeaseID) then
              begin
                MessageDlg(Format(SPropertiesFailure, [container.Name, blob.Name, ReturnCode]) +
                           ResponseText, mtError, [mbOK], 0);
                exit
              end
            finally
...
begin
  ...
  TAnonymousThread.Create(procedure begin
    try
      ...
      with BlobService do
      try
        if not GetContainerMetadata(container.Name) then
        begin
          MarkAsAvailable(containerNode);
          ShowErrorMessage(Format(SMetadataContainerFailure, [container.Name, ReturnCode]) + ResponseText);
          exit;
        end;
        meta := TStringList.Create;
        PopulateContainer('x-ms-meta-', meta);
      ...
      TThread.Queue(nil, procedure begin
        ...
        begin
          ...
          try
            with BlobService do
            try
              if not SetContainerMetadata(container.Name, meta) then
              begin
                MessageDlg(Format(SMetadataContainerFailure, [container.Name, ReturnCode]) +
                           ResponseText, mtError, [mbOK], 0);
                exit
              end;
            finally
  ...
  TAnonymousThread.Create(procedure begin
    try
      ...
      try
        ...
        language := ResponseHeader['Content-Language'];
        cache := ResponseHeader['Cache-Control'];
        length := ResponseHeader['Content-Length'];
        sequence := ResponseHeader['x-ms-blob-sequence-number'];
      finally
        Free;
      end;
      ...
      TThread.Queue(nil, procedure begin
        ...
        begin
          with BlobService do
          try
            if not SetPageBlobProperties(container.Name, blob.Name, FPageProps.Cache,
  ...
  begin
    ...
    TAnonymousThread.Create(procedure
      ...
      begin
        with BlobService do
        try
          if not PutBlock(container.Name, blob.Name, blockId, blockContent, leaseID,
                          MD5) then
          begin
            MarkAsAvailable(blobNode);
            ShowErrorMessage(SErrorGeneric + ResponseText);
  ...
  begin
    ...
    TAnonymousThread.Create(procedure
      ...
      begin
        with BlobService do
        try
          if not PutPage(container.Name, blob.Name, pageContent, firstByte, endByte,
...
begin
  ...
  begin
    try
      ...
      with BlobService do
        try
          xmlStr := ListBlobs(ContainerName, ['include', 'include'], ['metadata', 'snapshots']);
          Free;
        except
          if Mark then
            MarkAsAvailable(ContainerNode);
...
begin
  ...
  begin
    try
      ...
      with BlobService do
        try
          xmlStr := GetPageRegions(container, blob, snapshot);
          size := ResponseHeader['x-ms-blob-content-length'];
          Free;
        except
...
begin
  ...
  begin
    ...
    try
      if BlobObj <> nil then
        Success := GetBlobMetadata(Container.Name, BlobObj.Name, BlobObj.Snapshot)
      else
        Success := GetContainerMetadata(Container.Name);

      if Success then
      begin
        props := TStringList.Create;
        PopulateContainer('x-ms-meta-', props);
...
begin
  ...
  begin
    ...
    try
      if BlobObj <> nil then
        Success := GetBlobProperties(Container.Name, BlobObj.Name, BlobObj.Snapshot)
      else
        Success := GetContainerProperties(Container.Name);

      if Success then
      begin
        props := TStringList.Create;
        PopulateContainerProperties(props);
...
begin
  ...
  begin
    try
      ...
      with BlobService do
        try
          xmlStr := GetBlockList(container, blob, 'all', snapshot);
          size := ResponseHeader['x-ms-blob-content-length'];
          Free;
        except
    ...
    TAnonymousThread.Create(procedure
      ...
      begin
        ...
        try
          ...
          if not PutBlockBlob(container.Name, blob.Name, content, blob.LeaseID, metaHeaders,
          ...
          xmlStr := ListBlobs(container.Name, ['prefix', 'maxresults'],
                                              [blob.Name, '1']);
...
begin
  ...
  begin
    ...
    //addresses inconsistency in the Windows Azure REST api, where
