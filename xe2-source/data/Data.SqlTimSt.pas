...
uses
  System.VarUtils, System.DateUtils, System.SysConst, Data.DBConsts, System.TypInfo, System.Classes, System.StrUtils
...
{ Helper record that helps crack open TSQLTimeStampObject }
  ...
  TSQLTimeStampVarData = record
    ...
    case Integer of
    0: (VTimeStamp: TSQLTimeStampData);
...
{ Helper record that helps crack open TSQLTimeStampOffsetObject }
  ...
  TSQLTimeStampOffsetVarData = record
    ...
    Reserved1, Reserved2, Reserved3: Word;
    case Integer of
    0: (VDateTimeOffset: TSQLTimeStampOffsetData);
    // to match size as Variant/TVarData type
    1: (VInt64: Int64);
    2: (VRecord: TVarRecord);
  end;
...
begin
  ...
  begin
    ...
    if CurPos = 0 then
	  Exit(0);
    TimeStr := MidStr(S, CurPos+Length(Pattern)-1, MaxInt);

    Pattern := IntToStr(Min)+FormatSettings.TimeSeparator;
    CurPos := Pos(Pattern, TimeStr);
    if CurPos = 0 then
	  Exit(0);
    TimeStr := MidStr(TimeStr, CurPos+Length(Pattern)-1, MaxInt);

    Pattern := FormatSettings.TimeSeparator+IntToStr(Sec);
    CurPos := Pos(Pattern, TimeStr);
    if (CurPos = 0) and (Length(Pattern) = 2) then
    begin
    ...
    if CurPos = 0 then
	  Exit(0);
    TimeStr := MidStr(TimeStr, CurPos+Length(Pattern), MaxInt);

    Pattern := '';
    if (Length(TimeStr) > 1) and (TimeStr[1] = FormatSettings.TimeSeparator) then
    begin
      CurPos := 2;
      while (CurPos <= Length(TimeStr)) and CharInSet(TimeStr[CurPos], ['0'..'9']) do
      begin
        Pattern := Pattern + TimeStr[CurPos];
        Inc(CurPos);
      end;
    end;
