...
{$IFDEF CLR}
procedure TDbxDataGenerator.AddParameters(const Command: DbCommand);
var
  ColumnCount: Integer;
  Ordinal: Integer;
  Column: TDBXMetadataColumn;
...
begin
  ...
  begin
    Column := GetColumn(Ordinal).MetaDataColumn;
    Parameter := Command.CreateParameter;
    Parameter.DbType := TAdoDbxProviderFactory.DbxToAdoType(Column.DataType, 0);
...
{$IFDEF CLR}
procedure TDbxDataGenerator.SetInsertParameters(const Command: DbCommand;
  Row: Integer);
var
  ColumnCount: Integer;
  Ordinal: Integer;
...
begin
  ...
  begin
    ...
    case Column.Datatype of
      TDBXDataTypes.WideStringType,
      TDBXDataTypes.AnsiStringType: Command.Parameters[Ordinal].Value := Column.GetString(Row);
      TDBXDataTypes.BooleanType: Command.Parameters[Ordinal].Value := TObject(Column.GetBoolean(Row));
      TDBXDataTypes.UInt8Type: Command.Parameters[Ordinal].Value := TObject(Column.GetInt8(Row));
      TDBXDataTypes.Int8Type: Command.Parameters[Ordinal].Value := TObject(Column.GetInt8(Row));
      TDBXDataTypes.UInt16Type: Command.Parameters[Ordinal].Value := TObject(Column.GetInt16(Row));
      TDBXDataTypes.Int16Type: Command.Parameters[Ordinal].Value := TObject(Column.GetInt16(Row));
      TDBXDataTypes.Int32Type: Command.Parameters[Ordinal].Value := TObject(Column.GetInt32(Row));
      TDBXDataTypes.Int64Type: Command.Parameters[Ordinal].Value := TObject(Column.GetInt64(Row));
      TDBXDataTypes.SingleType: Command.Parameters[Ordinal].Value := TObject(Column.GetSingle(Row));
      TDBXDataTypes.DoubleType: Command.Parameters[Ordinal].Value := TObject(Column.GetDouble(Row));
      TDBXDataTypes.BcdType: Command.Parameters[Ordinal].Value := TObject(StrToBcd(Column.GetDecimal(Row)));
      TDBXDataTypes.DateType:
        begin
          DateTime := EncodeDate(Column.GetYear(Row), Column.GetMonth(Row), Column.GetDay(Row));
          TimeStamp := DateTimeToTimeStamp(DateTime);
          Command.Parameters[Ordinal].Value := TObject(TimeStamp.Date);
        end;
      TDBXDataTypes.TimeType:
        begin
          DateTime := EncodeTime(Column.GetHour(Row), Column.GetMinute(Row), Column.GetSeconds(Row), 0);
          Command.Parameters[Ordinal].Value := TObject(DateTime);
        end;
      TDBXDataTypes.TimeStampType:
        begin
          SqlTimeStamp.Year       := Column.GetYear(Row);
          ...
          SqlTimeStamp.Second     := Column.GetSeconds(Row);
          SqlTimeStamp.Fractions  := Column.GetMilliseconds(Row);
          Command.Parameters[Ordinal].Value := SqlTimeStamp;
        end;
      TDBXDataTypes.TimeStampOffsetType:
        begin
          SqlTimeStampOffset.Year       := Column.GetYear(Row);
          ...
          SqlTimeStampOffset.TimeZoneHour := 0;
          SqlTimeStampOffset.TimeZoneMinute := 0;
          Command.Parameters[Ordinal].Value := SqlTimeStampOffset;
        end;
      TDBXDataTypes.BlobType, TDBXDataTypes.VarBytesType, TDBXDataTypes.BytesType:
        begin
          Buffer := Column.GetBytes(Row);
          Command.Parameters[Ordinal].Value := Buffer;
        end
      else
        raise TDbxDataGeneratorException.Create('Unknown DataType: ' + IntToStr(Column.DataType) + '.  Parameter will not be set');
    end;
...
class function TDbxDataGenerator.BytesEquals(const Value1, Value2: TBytes;
...
{$IFDEF CLR}
      TDBXDataTypes.WideStringType: Param.AsString := Column.GetString(Row);
{$ELSE}
...
class procedure TDbxDataGenerator.ValidateBytes(Value1, Value2: TBytes);
begin
  if (Length(Value1) <> Length(Value2)) or not BytesEquals(Value1, Value2, Length(Value2)) then
  begin
    raise TDBXDataGeneratorException.Create('TBytes values do not match');
  end;
end;
...
begin
  ...
  begin
    ...
    DbxValue := Command.Parameters[Ordinal].Value;
    case Column.Datatype of
      TDBXDataTypes.AnsiStringType: ValidateAnsiString(string(DbxValue.GetAnsiString), Column.GetString(Row));
      ...
      ftWideMemo:
        ...
        begin
          if MetaDataProvider.DatabaseProduct = 'MySQL' then
            ValidateWideString(TEncoding.ASCII.GetString(DataSet.Fields[I].AsBytes), GetColumn(I).GetString(Row))
          else
            ValidateWideString(DataSet.Fields[I].AsString, GetColumn(I).GetString(Row));
        end
        else
                                ...
                                begin
                                  ...
                                  begin
                                    ...
                                    begin
                                      ...
                                      ValidateWideString(TEncoding.Unicode.GetString(Buffer), GetColumn(I).GetString(Row));
                                    end
                                    else
...
{$IFDEF CLR}
function TDbxDataGenerator.ValidateRow(const Reader: DbDataReader; Row: Integer): Boolean;
var
  I, Size: Integer;
  SQLTimeStamp: TSQLTimeStamp;
  DateTimeString: WideString;
  Buffer: TBytes;
begin
  for I := 0 to Reader.FieldCount - 1 do
  begin
    case System.&Type.GetTypeCode(Reader.GetValue(I).GetType) of
      TypeCode.Byte: if (Reader.GetByte(I) <> GetColumn(I).GetInt8(Row)) then
                        raise TDBXDataGeneratorException.Create('UInt8 values do not match');
      TypeCode.SByte: if (Convert.ToSByte(Reader.GetValue(I)) <> GetColumn(I).GetInt8(Row)) then
                        raise TDBXDataGeneratorException.Create('Int8 values do not match');
      TypeCode.UInt16, TypeCode.Int16: if (Reader.GetInt16(I) <> GetColumn(I).GetInt16(Row)) then
                        raise TDBXDataGeneratorException.Create('Int16 values do not match');
      TypeCode.Int32: if (Reader.GetInt32(I) <> GetColumn(I).GetInt32(Row)) then
                        raise TDBXDataGeneratorException.Create('Int32 values do not match');
      TypeCode.Int64: ValidateInt64(Reader.GetInt64(I), GetColumn(I).GetInt64(Row));
      TypeCode.Decimal: if GetColumn(I).MetaDataColumn.Scale = 0 then
                          ValidateBcd(StrToBcd(Reader.GetValue(I).ToString), IntegerToBcd(GetColumn(I).GetInt32(Row)))
                        else
                          ValidateBcd(StrToBcd(Reader.GetValue(I).ToString), StrToBcd(GetColumn(I).GetDecimal(Row)));
      TypeCode.Single: ValidateSingle(Reader.GetFloat(I), GetColumn(I).GetSingle(Row));
      TypeCode.Double: ValidateDouble(Reader.GetDouble(I), GetColumn(I).GetDouble(Row));
      TypeCode.String:  if GetColumn(I).DataType = TDBXDataTypes.WideStringType then
                          ValidateWideString(Reader.GetString(I), GetColumn(I).GetString(Row))
                        else if GetColumn(I).DataType = TDBXDataTypes.AnsiStringType then
                        begin
                          if Assigned(MetaDataProvider) then
                          begin
                            if ((MetaDataProvider.DatabaseProduct = 'MySQL') or
                                (MetaDataProvider.DatabaseProduct = 'Sybase SQL Server')) then
                              ValidateAnsiString(AnsiString(Reader.GetString(I)), Trim(GetColumn(I).GetString(Row)))
                            else
                              ValidateAnsiString(AnsiString(Reader.GetString(I)), GetColumn(I).GetString(Row));
                          end
                          else
                            ValidateAnsiString(AnsiString(Reader.GetString(I)), GetColumn(I).GetString(Row));
                        end
                        else
                          raise TDBXDataGeneratorException.Create('String type, but not WideStringType nor AnsiStringType');
      TypeCode.Boolean: ValidateBoolean(Reader.GetBoolean(I),GetColumn(I).GetBoolean(Row));
      TypeCode.DateTime: if GetColumn(I).DataType = TDBXDataTypes.DateType then
                           ValidateDateTime(Reader.GetDateTime(I), EncodeDate(GetColumn(I).GetYear(Row), GetColumn(I).GetMonth(Row), GetColumn(I).GetDay(Row)))
                         else if GetColumn(I).DataType = TDBXDataTypes.TimeType then
                           ValidateDateTime(Reader.GetDateTime(I), EncodeTime(GetColumn(I).GetHour(Row), GetColumn(I).GetMinute(Row), GetColumn(I).GetSeconds(Row), 0{GetColumn(I).GetMilliseconds(Row)}))
                         else if GetColumn(I).DataType = TDBXDataTypes.DateTimeType then
                           raise Exception.Create('DateTime type actually found')
                         else if GetColumn(I).DataType = TDBXDataTypes.TimeStampType then
                         begin
                           SqlTimeStamp.Year       := GetColumn(I).GetYear(Row);
                           SqlTimeStamp.Month      := GetColumn(I).GetMonth(Row);
                           SqlTimeStamp.Day        := GetColumn(I).GetDay(Row);
                           ...
                           if IsFractionsSupported then
                           begin
                             DateTimeToString(DateTimeString,'yyyy-mm-dd hh:nn:ss:zzz',Reader.GetDateTime(I));
                             ValidateAnsiString(DateTimeString, SQLTimeStampToStr('yyyy-mm-dd hh:nn:ss:zzz',SqlTimeStamp));
                           end
                           else
                           begin
                             DateTimeToString(DateTimeString,'yyyy-mm-dd hh:nn:ss',Reader.GetDateTime(I));
                             ValidateAnsiString(DateTimeString, SQLTimeStampToStr('yyyy-mm-dd hh:nn:ss',SqlTimeStamp));
                           end;
                         end
                         else raise Exception.Create('System.DateTime type, but not Time,Date,TimeStamp, or DateTime');
      TypeCode.&Object: if GetColumn(I).DataType = TDBXDataTypes.AnsiStringType then
                          ValidateAnsiString(Reader.GetString(I), GetColumn(I).GetString(Row))
                        else
                        begin
                          if ((MetaDataProvider.DatabaseProduct <> 'Adaptive Server Anywhere') or
                              ((MetaDataProvider.DatabaseProduct = 'Adaptive Server Anywhere') and
                              (GetColumn(I).DataType <> TDBXDataTypes.BlobType))) then
                          begin
                            Size:= Length(GetColumn(I).GetBytes(Row));
                            SetLength(Buffer, Size);
                            Reader.GetBytes(I, 0, Buffer, 0, Size);
                            ValidateBytes(Buffer, GetColumn(I).GetBytes(Row));
                          end;
                        end
    else
      raise TDBXDataGeneratorException.Create('Unsupported type ' +Reader.GetValue(I).GetType.ToString);
    end;
  end;
  Result:= True;
end;
