...
var
  //used to store reference to events added to TDSSessionManager, so they can later be removed
                                                                                                
  FEventDictionary: TObjectDictionary<TDBXJSonServerProtocolHandler, TDSSessionEvent> = nil;

//note, each TDBXJSonServerProtocolHandler instance can store only one event
function AddToEventDictionary(const Handler: TDBXJSonServerProtocolHandler;
                               const Event: TDSSessionEvent): Boolean;
begin
  Result := False;
  if Handler <> nil then
  begin
    if FEventDictionary = nil then
      FEventDictionary := TObjectDictionary<TDBXJSonServerProtocolHandler, TDSSessionEvent>.Create;

    TMonitor.Enter(FEventDictionary);
    try
      if not FEventDictionary.ContainsKey(Handler) then
      begin
        FEventDictionary.Add(Handler, Event);
        Result := True;
      end;
    finally
      TMonitor.Exit(FEventDictionary);
    end;
  end;
end;

procedure RemoveFromEventDictionary(const Handler: TDBXJSonServerProtocolHandler);
var
  LEvent: TDSSessionEvent;
begin
  LEvent := nil;
  if (Handler <> nil) and (FEventDictionary <> nil) then
  begin
    TMonitor.Enter(FEventDictionary);
    try
      if FEventDictionary.TryGetValue(Handler, LEvent) then
      begin
        FEventDictionary.Remove(Handler);
        TDSSessionManager.Instance.RemoveSessionEvent(LEvent);
      end;

      if FEventDictionary.Count = 0 then
        FreeAndNil(FEventDictionary);
    finally
      if FEventDictionary <> nil then
        TMonitor.Exit(FEventDictionary);
    end;
...
function TDBXJSonServerInputConnectionHandler.GetProductName: UnicodeString;
begin
...
function TDBXJSonServerOutputConnectionHandler.GetProductName: UnicodeString;
begin
  Result := NullString;
end;
...
begin
  RemoveFromEventDictionary(Self);
...
begin
  ...
  if AddToEventDictionary(Self, LSessionEvent) then
