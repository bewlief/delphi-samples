...
{$HPPEMIT '#pragma link "Datasnap.DSProxyCpp"'}    {Do not Localize}
...
function TDSCustomCppProxyWriter.GetDelphiTypeName(const Param: TDSProxyParameter): UnicodeString;
var
  Name: UnicodeString;
begin
  Name := Param.TypeName;
...
procedure TDSCustomCppProxyWriter.WriteOutgoingParameter(const Lhs: UnicodeString; const InRhs: UnicodeString; const Param: TDSProxyParameter; const CommandName: UnicodeString; const ParamName: UnicodeString);
var
  Rhs: UnicodeString;
  LTypeName: string;
begin
...
function TDSCustomCppProxyWriter.GetAssignmentString: UnicodeString;
begin
  Result := '=';
end;

function TDSCustomCppProxyWriter.GetCreateDataSetReader(const Param: TDSProxyParameter): UnicodeString;
begin
  Result := '(new TDBXDataSetReader(' + Param.ParameterName + ', FInstanceOwner), True)';
end;

function TDSCustomCppProxyWriter.GetCreateParamsReader(const Param: TDSProxyParameter): UnicodeString;
begin
...
procedure TDSCppProxyWriter.DerivedWrite(const Line: UnicodeString);
begin
