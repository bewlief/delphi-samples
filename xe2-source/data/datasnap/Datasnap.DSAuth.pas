...
type
  ...
  ///  <summary>
  ///  Custom attribute for annotating language constructs for specific role authorization.
  ///  </summary>
  ...
  /// <summary>
  ///   A role item which will be contained by a collection attached to a TDBXAuthenticationManager.
  ///   This role item contains a list of authorized and denied roles, as well as a set
  ///   of entities which the roles apply to, and a set of entities to exclude from the set the roles apply to.
  /// </summary>
  /// <remarks>
  ///   The entities in applies to (and exclude) should be either a Class name, Method name, or ClassName.MethodName.
  ///   If only the class is specified, then the role will apply to any server method of that class.
  ///   If only the method name is specified, then the roles will apply to a method of any class with
  ///   the given method name.
  ///   If an entity is specified as ClassName.MethodName, the roles apply to that specific method of that class.
  ///
  ...
  ///     denied: rolea, roleb
  ///     ApplyTo: TServerClass1, SomeMethod, TServerClass2.SomeOtherMethod
  ///     Exclude: TServerClass1, SomeMethod, TServerClass2.SomeOtherMethod
  ///
  /// </remarks>
  ...
  TDSAuthenticationEvent = procedure(Sender: TObject;
                                    const Protocol: UnicodeString;
                                    const Context: UnicodeString;
                                    const User: UnicodeString;
  ...
  protected
    ...
    function Authenticate(const AuthenticateEventObject: TDSAuthenticateEventObject; const Protocol: UnicodeString; const Context: UnicodeString;
                          const User: UnicodeString; const Password: UnicodeString;
  ...
  public
    ...
    function Authenticate(const Protocol: UnicodeString; const Context: UnicodeString; const AuthUserName, AuthPassword: string;
...
uses Datasnap.DSService, Datasnap.DSServerResStrs, System.SysUtils;
...
function TDSCustomAuthenticationManager.Authenticate(const AuthenticateEventObject: TDSAuthenticateEventObject; const Protocol, Context, User,
  Password: UnicodeString; UserRoles: TStrings): Boolean;
begin
  Result := true;
...
function TDSCustomAuthenticationManager.Authenticate(const Protocol,
  Context: UnicodeString; const AuthUserName, AuthPassword: string;
