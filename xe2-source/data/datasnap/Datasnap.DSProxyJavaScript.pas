...
{$HPPEMIT '#pragma link "Datasnap.DSProxyJavaScript"'}    {Do not Localize}
...
type
  ...
  protected
    ...
    procedure WriteJSLine(const Line: UnicodeString); overload; virtual;
    ...
    function GetMethodRequestType(const ProxyClass: TDSProxyClass; const Method: TDSProxyMethod): UnicodeString; virtual;
    function GetMethodRequestName(const ProxyClass: TDSProxyClass; const Method: TDSProxyMethod): UnicodeString; virtual;
...
function TDSCustomJavaScriptProxyWriter.GetAssignmentString: UnicodeString;
begin
  Result := '=';
end;

function TDSCustomJavaScriptProxyWriter.GetCreateDataSetReader(const Param: TDSProxyParameter): UnicodeString;
begin
  Result := NullString;
end;

function TDSCustomJavaScriptProxyWriter.GetCreateParamsReader(const Param: TDSProxyParameter): UnicodeString;
begin
  Result := NullString;
end;
...
procedure TDSCustomJavaScriptProxyWriter.WriteJSLine(const Line: UnicodeString);
begin
...
begin
  ...
  begin
    ...
    if CompareText(sDSMetadataClassName, ProxyClass.ProxyClassName) = 0 then
      Exit(False);
...
begin
  ...
  begin
    Line := UnicodeString(ProxyClassNameList[I]);
...
var
  ClassMethodString: TDBXStringBuffer;
  Methods: TDSProxyMethod;
  FirstMethod: Boolean;
begin
  ClassMethodString := TDBXStringBuffer.Create;
  try
...
function TDSCustomJavaScriptProxyWriter.GetMethodRequestType(const ProxyClass: TDSProxyClass; const Method: TDSProxyMethod): UnicodeString;
var
  MName: UnicodeString;
  Param: TDSProxyParameter;
...
begin
  ...
  if System.StrUtils.StartsText(FPutPrefix,MName) and (Length(MName) > Length(FPutPrefix)) then
    Exit(Put);
  if System.StrUtils.StartsText(FPostPrefix,MName) and (Length(MName) > Length(FPostPrefix)) then
    Exit(Post);
  if System.StrUtils.StartsText(FDeletePrefix,MName) and (Length(MName) > Length(FDeletePrefix)) then
    Exit(Delete);
  if (CompareText(sDSAdminClassName, ProxyClass.ProxyClassName) = 0) and (Method.ParameterCount > 0) then
  begin
    HadIn := False;
    Param := Method.Parameters;
    Aux := Param;
...
function TDSCustomJavaScriptProxyWriter.GetMethodRequestName(const ProxyClass: TDSProxyClass; const Method: TDSProxyMethod): UnicodeString;
var
  MType: UnicodeString;
  MName: UnicodeString;
...
var
  ...
  InputParamCSV: UnicodeString;
  OutIndex: Integer;
  PCount: Integer;
  LineToAdd: UnicodeString;
  Name: UnicodeString;
  MethodName: UnicodeString;
  HasReturn: Boolean;
  ReturnVarPrefix: UnicodeString;
  I: Integer;
  HasComplexInput: Boolean;
  HasComplexResult: Boolean;
  MethodNameQuoted: String;
  RequestTypeToUse: String;
begin
  ...
  try
    ...
    begin
      ...
      if (Param.ParameterDirection = TDBXParameterDirections.InOutParameter) or (Param.ParameterDirection = TDBXParameterDirections.InParameter) then
      begin
        InputParamCSV := C_Conditional(StringIsNil(InputParamCSV), '', InputParamCSV + ', ') + Param.ParameterName;
        IncrAfter(PCount);
      ...
      if not StringIsNil(LineToAdd) then
    ...
    begin
      ...
      for I := 0 to ResultObjectLines.Count - 1 do
        WriteJSLine(UnicodeString(ResultObjectLines[I]));
...
procedure TDSJavaScriptProxyWriter.DerivedWrite(const Line: UnicodeString);
begin
...
var
  StartUnitNamePos: Integer;
  EndUnitNamePos: Integer;
begin
  Result := OutputFile;
  StartUnitNamePos := LastDelimiter(':\/', Result) + 1;
  if StartUnitNamePos < 1 then
    StartUnitNamePos := 1;
  EndUnitNamePos := LastDelimiter('.', Result);
  TargetUnitName := Copy(Result, StartUnitNamePos, EndUnitNamePos - StartUnitNamePos);
end;
