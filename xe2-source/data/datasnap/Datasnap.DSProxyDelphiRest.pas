...
{$HPPEMIT '#pragma link "Datasnap.DSProxyDelphiRest"'}    {Do not Localize}
...
var
  ...
  Line: UnicodeString;
begin
  Line := 'uses Datasnap.DSProxyRest, Datasnap.DSClientRest, Data.DBXCommon, Data.DBXClient, Data.DBXDataSnap, Data.DBXJSON, Datasnap.DSProxy, System.Classes, System.SysUtils, Data.DB, Data.SqlExpr, Data.DBXDBReaders,';
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            UnitName := ' ' + UnitName + ',';
            if StringIndexOf(Line,UnitName) <= 0 then
              Line := Line + UnitName;
          end;
          PParam := PParam.Next;
        end;
...
procedure TDSCustomDelphiRestProxyWriter.WriteOutgoingParameter(const Lhs: UnicodeString; const InRhs: UnicodeString; const Param: TDSProxyParameter; const CommandName: UnicodeString; const ParamName: UnicodeString);
var
  Rhs: UnicodeString;
begin
  Rhs := InRhs;
...
begin
  ...
  begin
    ...
    if IncludeCacheMethod(Methods) then
      WriteLine('FreeAndNil(F' + Methods.ProxyMethodName + 'Command_Cache);');
    Methods := Methods.Next;
  end;
  WriteLine('inherited;');
  Outdent;
...
function TDSCustomDelphiRestProxyWriter.GetAssignmentString: UnicodeString;
begin
  Result := ':=';
end;
...
function TDSCustomDelphiRestProxyWriter.GetCreateDataSetReader(const Param: TDSProxyParameter): UnicodeString;
begin
  Result := '(TDBXDataSetReader.Create(' + Param.ParameterName + ', FInstanceOwner), True)';
end;

function TDSCustomDelphiRestProxyWriter.GetCreateParamsReader(const Param: TDSProxyParameter): UnicodeString;
begin
...
procedure TDSDelphiRestProxyWriter.DerivedWrite(const Line: UnicodeString);
begin
...
initialization
  TDSProxyWriterFactory.RegisterWriter(sDelphiRestProxyWriter, TDSClientProxyWriterDelphi);
finalization
