...
(*$HPPEMIT '#define NO_USING_NAMESPACE_FMX_PLATFORM_WIN' *)

interface

uses
  Winapi.Windows, FMX_Forms, FMX_Platform, FMX_Types;
...
uses
  ...
  Types, Variants, SysUtils, Classes,
  UITypes, Math, StrUtils, System.SyncObjs,
  FMX_Consts, FMX_Menus, FMX_Dialogs, FMX_Printer, FMX_Printer_Win,
  FMX_Canvas_D2D, System.Generics.Collections;
...
{ TPlatformWin }
  ...
  public
    ...
    function TextToShortCut(Text: WideString):integer; override;
...
var
  WindowAtom: TAtom;
  WindowAtomString: WideString;
  DropAtom: TAtom;
  DropAtomString: WideString;
  CF_OBJECT: Longint;
...
begin
  ...
  WindowAtom := GlobalAddAtomW(PWideChar(WindowAtomString));
  DropAtomString := Format('FIREMONKEYDROP%.8X', [GetCurrentProcessID]);
  DropAtom := GlobalAddAtomW(PWideChar(DropAtomString));
  CF_OBJECT := RegisterClipBoardFormatW(PWideChar('WindowAtomString'));
...
function TTextServiceWin.GetText: WideString;
begin
  Result := FText;
end;

procedure TTextServiceWin.SetText(const Value: WideString);
begin
  FText := Value;
end;
...
procedure TTextServiceWin.InternalSetMarkedText( const AMarkedText: WideString );
begin
...
function TTextServiceWin.InternalGetMarkedText: WideString;
begin
  Result := FMarkedText;
end;

function TTextServiceWin.CombinedText: WideString;
begin
  if FMarkedText <> '' then
...
procedure TTextServiceWin.DrawSingleLine2( Canvas: TCanvas; const S: WideString;
      const ARect: TRectF;
      const Font: TFont;
      const AOpacity: Single; const Flags: TFillTextFlags; const ATextAlign: TTextAlign;
      const AVTextAlign: TTextAlign = TTextAlign.taCenter );
...
begin
  ...
  if (Handle <> 0) then
  begin
    if GlobalFindAtomW(PWideChar(WindowAtomString)) = WindowAtom then
...
begin
  ...
  if (Handle <> 0) then
  begin
    if GlobalFindAtomW(PWideChar(DropAtomString)) = DropAtom then
  ...
  begin
    ...
    begin
      try
        ...
        ImmGetCompositionString(IMC, GCS_RESULTSTR, PWideChar(S), Size);
      finally
...
//          TTextServiceWin(IMEObj.GetTextService).InternalSetMarkedText(S);
          ...
          SetLength(S, Size div sizeof(Char));
          ImmGetCompositionString(IMC, GCS_COMPSTR, PWideChar(S), Size);
...
var
  ...
  Wnd: HWND;
  ParentWnd: HWND;
  WndClassName: WideString;
begin
  WndClassName := 'FM' + AForm.ClassName;
  if not GetClassInfo(hInstance, PWideChar(WndClassName), WindowClass) then
  begin
    ...
    WindowClass.hIcon := LoadIconW(MainInstance, PWideChar('MAINICON'));
    WindowClass.hCursor := LoadCursorW(0, PWideChar(IDC_ARROW));
  ...
  Wnd := CreateWindowEx(ExStyle, WindowClass.lpszClassName, PWideChar(AForm.Caption), Style,
...
procedure TPlatformWin.SetWindowCaption(AForm: TCommonCustomForm; const ACaption: WideString);
begin
...
var
  P: TPoint;
begin
  P := Types.Point(Round(Point.X), Round(Point.Y));
...
function TPlatformWin.ShortCutToText(ShortCut: TShortCut): WideString;

  function GetSpecialName(ShortCut: TShortCut): WideString;
  var
    ScanCode: Integer;
...
function TPlatformWin.TextToShortCut(Text: WideString): integer;
 { If the front of Text is equal to Front then remove the matching piece
  ...
  begin
    ...
    if (Length(Text) >= Length(Front)) and
      (AnsiStrLIComp(PWideChar(Text), PWideChar(Front), Length(Front)) = 0) then
    begin
      Result := True;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        AppendMenu(Parent, MF_STRING or MF_POPUP, PopupMenu, PWideChar(Item.Text));
      ...
      begin
        ...
        if (Item.Text = '-')  then
          ...
          if (ShortCutToText(Item.ShortCut) <> '') and  not(Item.Parent is TMainMenu)  then
            AppendMenu(Parent, Flags or MF_STRING, THandle(Item), PWideChar(Item.Text+ #9 + ShortCutToText(Item.ShortCut)))
          else
            AppendMenu(Parent, Flags or MF_STRING, THandle(Item), PWideChar(Item.Text));
          end;
        Item.Handle := Parent;
      end;
    end;
    ...
    begin
      ...
      begin
        ...
        if AItem.Enabled then
          ...
          Flags := Flags or MF_DISABLED;
        ModifyMenuW(AItem.Handle, THandle(AItem), MF_BYCOMMAND or MF_STRING or Flags, THandle(AItem), PWideChar(AItem.Text));
      end;
    end;
  end;
end;
...
const
  SID_IDropTargetHelper = '{45A188FD-1C29-4787-B06B-3BE727F5DB8B}';
...
const
  CursorMap: array[crSizeAll..crArrow] of PWideChar = (
...
function TPlatformWin.GetCurrentLangID: WideString;
var
...
function TPlatformWin.GetDefaultFontFamilyName: WideString;
var
  VI: TOSVersionInfo;
begin
...
function TPlatformWin.GetLocaleFirstDayOfWeek: WideString;
var
...
procedure GetFileNames(var OpenFileName: TOpenFilenameW; var AFileName: TFileName; var AFiles: TWideStrings; AOptions: TOpenOptions);
var
  Separator: WideChar;

  function ExtractFileName(P: PWideChar; var S: TFilename): PWideChar;
  begin
...
begin
  ...
  Move(PWideChar(AFileName)^, FileNameBuffer^, FileNameBufferSize * 2);
...
begin
  ...
  begin
    ...
    try
      ...
      with DevNames^ do
      begin
        wDriverOffset := Offset - PWideChar(DevNames);
        Offset := StrECopy(Offset, Driver) + 1;
        wDeviceOffset := Offset - PWideChar(DevNames);
        Offset := StrECopy(Offset, Device) + 1;
        wOutputOffset := Offset - PWideChar(DevNames);;
        StrCopy(Offset, Port);
      end;
    finally
...
begin
  ...
  try
    with DevNames^ do
      (Printer as TPrinterWin).SetPrinter(PWideChar(DevNames) + wDeviceOffset,
        PWideChar(DevNames) + wDriverOffset,
        PWideChar(DevNames) + wOutputOffset, DeviceMode);
  finally
    GlobalUnlock(DeviceNames);
