...
uses
  ...
  FMX_Forms, FMX_Types, FMX_Objects, FMX_Ani;
...
uses
  Math, TypInfo, SysUtils, Variants, FMX_Layouts;
...
procedure TLabel.SetText(const Value: WideString);
begin
  if Value <> FText then
  begin
    inherited;
  ...
  case Orientation of
    ...
    begin
      if FViewportSize > 0 then
        ...
        lSize := Height;
      Result := Round(Math.Min(Math.MaxValue([lSize, Height / 2, 5]), Width));
    end;
    TOrientation.orVertical:
    begin
      if FViewportSize > 0 then
        ...
        lSize := Width;
      Result := Round(Math.Min(Math.MaxValue([lSize, Width / 2, 5]), Height));
    end;
  end;
  if Result < 5 then
    Result := 0;
...
begin
  ...
  lValue := Math.Max(FMin,
                            Math.MinValue([FMax - FViewportSize,
                                                  FMax,
                                                  lValue]));
...
function TBitmapTrackBar.GetDefaultStyleLookupName: WideString;
begin
