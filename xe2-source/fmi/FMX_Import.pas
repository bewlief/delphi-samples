...
uses
  Classes, SysUtils, Types, FMX_Types, FMX_Types3D,
  FMX_Objects3D;
...
type
  ...
  IModelImporter = interface
    ...
    function LoadFromFile(const AFileName: WideString; out AMesh: TMeshDynArray;
  ...
  published
    ...
    class function GetFileExt(const index: integer): WideString;
    class function GetFileDescription(const index: integer): WideString;
    ...
    class function LoadFromFile(const AFileName: WideString;
  ...
  public
    ...
    function GetExt: WideString; virtual; abstract;

    function LoadFromFile(const AFileName: WideString;
      out AMesh: TMeshDynArray; AOwner: TComponent): Boolean; virtual; abstract;
  end;

  TCustomModel = class
  public
    procedure LoadFromFile(const AFileName: WideString); virtual; abstract;
  end;
  ...
  private
    FFileName: WideString;
  public
    constructor CreateFromFile(const AFileName: WideString); override;
  ...
  private
    ...
    function CreateBitmap(const AFileName: WideString): TGEBitmap;
    procedure SetLocalTexturePath(const APath: WideString);
  ...
  public
    ...
    procedure LoadImages(const APath : WideString);
    function Add(AMaterial: TGEMaterial): TGEMaterial;
...
function SetLocalPath(const APath: WideString; var AFileName: WideString):boolean;
function FloatStringsToSingleDynArray(const AStr: WideString): TSingleDynArray;
function IntStringsToIntegerDynArray(const AStr: WideString): TIntegerDynArray; overload;
function StringsToStringDynArray(const AStr: WideString): TStringDynArray; overload;
...
class function TModelImportServices.GetFileExt(const index: integer): WideString;
begin
...
class function TModelImportServices.LoadFromFile(const AFileName: WideString;
  out AMesh: TMeshDynArray; AOwner: TComponent): Boolean;
var
...
var
  ...
  LNormal: TVector3D;
begin
  LPos1 := Point3DToVector3D(Av1.Pos);
  LPos2 := Point3DToVector3D(Av2.Pos);
  LPos3 := Point3DToVector3D(Av3.Pos);
  LTangent1 := Vector3DSubtract(LPos1, LPos2);
  NormalizeVector3D(LTangent1);
  LTangent2 := Vector3DSubtract(LPos1, LPos3);
  NormalizeVector3D(LTangent2);
  LNormal   := Vector3DCrossProduct(LTangent2, LTangent1);
  NormalizeVector3D(LNormal);

  Av1.Nor := Vector3DToPoint3D(LNormal);
  Av2.Nor := Vector3DToPoint3D(LNormal);
  Av3.Nor := Vector3DToPoint3D(LNormal);
end;
...
begin
  ...
  begin
    LVec := Point3DToVector3D(LNewTriangles[v].Pos);
    LVec := Vector3DTransform(LVec, ATransform);
    LMesh.Data.VertexBuffer.Vertices[v] := Vector3DToPoint3D(LVec);

    LVec := Point3DToVector3D(LNewTriangles[v].Nor);
    LVec := Vector3DTransform(LVec, ANormalTransform);
    NormalizeVector3D(LVec);
    LMesh.Data.VertexBuffer.Normals[v] := Vector3DToPoint3D(LVec);
  ...
  begin
    ...
    begin
      ...
      begin
        LMesh.Material.Texture := AMaterial.FDiffuseBitmap;
        LMesh.Material.Diffuse := MakeColor(255, 255, 255, 255);
      end else
      begin
        LMesh.Material.Diffuse := Vector3DToColor(AMaterial.FDiffuse);
        LMesh.Material.Specular := Vector3DToColor(AMaterial.FSpecular);
        LMesh.Material.Ambient := Vector3DToColor(AMaterial.FAmbient);
      end;
    end;
  end;
...
constructor TGEBitmap.CreateFromFile(const AFileName: WideString);
begin
  inherited;
  FFileName := AFileName;
end;
...
function TGEMaterials.Add(AMaterial: TGEMaterial): TGEMaterial;
var
  LLength: Integer;
begin
...
function TGEMaterials.CreateBitmap(const AFileName: WideString): TGEBitmap;
var
  LLength: Integer;
begin
...
procedure TGEMaterials.LoadImages(const APath: WideString);
var
...
var
  ...
  function SetFileExists(const ATestFileName: WideString):boolean;
  begin
...
{$IFDEF MSWINDOWS}
  LFileName := StringReplace(AFileName, '/', '\', [rfReplaceAll]);
{$ELSE}
  LFileName := StringReplace(AFileName, '\', '/', [rfReplaceAll]);
{$ENDIF}
...
procedure TGEMaterials.SetLocalTexturePath(const APath: WideString);
var
  i : Integer;
begin
  for i := 0 to High(FMaterials) do
...
function IntStringsToIntegerDynArray(const AStr: WideString): TIntegerDynArray;
var
  str: WideString;
  c: PWideChar;
