...
uses
  Winapi.Windows, Winapi.WinSpool, Winapi.Messages, Classes, UITypes,
  FMX_Types, FMX_Canvas_GDIP, FMX_Forms, FMX_Printer;
...
{ TPrinterWin }
  ...
  public
    ...
    procedure GetPrinter(ADevice, ADriver, APort: PWideChar; var ADeviceMode: THandle);
    procedure SetPrinter(ADevice, ADriver, APort: PWideChar; ADeviceMode: THandle);
...
type
  ...
  protected
    ...
    procedure PrinterSettingsReadError(const AdditionalMsg: WideString = '');
    procedure PrinterSettingsWriteError(const AdditionalMsg: WideString = '');

    procedure RefreshDPIList; override;
    procedure ActiveDPIChanged; override;
  public
    constructor Create(const ATitle, ADriver, ADevice, APort: WideString);
...
constructor TWinPrinterDevice.Create(const ATitle, ADriver, ADevice, APort: WideString);
begin
...
function TWinPrinterDevice.GetTitle: WideString;
begin
  Result := FTitle;
end;

procedure TWinPrinterDevice.PrinterSettingsReadError(
  const AdditionalMsg: WideString);
begin
...
begin
  ...
  Count := DeviceCapabilities(PWideChar(Device), nil, DC_ENUMRESOLUTIONS, nil, nil);
  SetLength(DPIs, Count);
  DeviceCapabilities(PWideChar(Device), nil, DC_ENUMRESOLUTIONS, PWideChar(DPIs), nil);
...
type
  TCreateHandleFunc = function(DriverName, DeviceName, Output: PWideChar; InitData: PDeviceMode): HDC stdcall;
var
...
begin
  ...
  begin
    ...
    if Assigned(CreateHandleFunc) then
      with ActivePrinter do
      begin
        FDC := CreateHandleFunc(PWideChar(Driver), PWideChar(Device), PWideChar(Port), FDevMode);
        if FDC = 0 then
...
begin
  if FCanvas = nil then
  begin
    FCanvas := DefaultPrinterCanvasClass.CreateFromPrinter(Self);
...
begin
  TWideStrings(Data).Add(LogFont.lfFaceName);
  Result := 1;
end;
...
procedure TPrinterWin.GetPrinter(ADevice, ADriver, APort: PWideChar; var ADeviceMode: THandle);
begin
  with ActivePrinter do
  begin
    StrCopy(ADevice, PWideChar(Device));
    StrCopy(ADriver, PWideChar(Driver));
    StrCopy(APort, PWideChar(Port));
  end;
  ADeviceMode := FDeviceMode;
end;

procedure TPrinterWin.SetPrinter(ADevice, ADriver, APort: PWideChar; ADeviceMode: THandle);
var
  I, J: Integer;
begin
  CheckPrinting(False);
...
function FetchStr(var Str: PWideChar): PWideChar;
var
  P: PWideChar;
begin
  Result := Str;
...
begin
  ...
  SetState(TPrinterState.psNoHandle);
  LDevice := PWideChar(ActivePrinter.Device);
  ...
  try
    ...
    begin
      if Level = 4 then
        ...
        begin
          FPrinters.Add(TWinPrinterDevice.Create(pPrinterName, '', pPrinterName, ''));
        ...
        begin
          ...
          while Port^ <> #0 do
          begin
            FPrinters.Add(TWinPrinterDevice.Create(Format(SDeviceOnPort, [pPrinterName, Port]),
              '', pPrinterName, Port));
            Port := FetchStr(LineCur);
          end;
