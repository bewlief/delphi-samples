...
{$ENDIF}
  UITypes, Types, SysUtils, Classes, FMX_Types;
...
{$ENDIF}
  ...
  public
    constructor Create(const ADriver, ADevice, APort: WideString);
    destructor Destroy; override;

    function Equals(Obj: TObject): Boolean; overload; override;
    function Equals(const ADriver, ADevice, APort: WideString): Boolean; reintroduce; overload;
    ...
    property Device: WideString read FDevice;
    property Driver: WideString read FDriver;
...
{$ENDIF}
  protected
    ...
    function GetFonts: TWideStrings;
  ...
  public
    ...
    property Fonts: TWideStrings read GetFonts;
...
uses
{$IFDEF MSWINDOWS}
  System.Character, FMX_Consts, FMX_Printer_Win, FMX_Canvas_GDIP;
{$ENDIF}

{$IFDEF MACOS}
  System.Character, FMX_Consts, FMX_Printer_Mac;
{$ENDIF}
...
function TPrinter.GetFonts: TWideStrings;
begin
  if FFonts = nil then
  try
    FFonts := TWideStringList.Create;
    RefreshFonts;
  except
    FreeAndNil(FFonts);
    raise;
...
procedure TPrinter.RaiseError(const Msg: WideString);
begin
...
constructor TPrinterDevice.Create(const ADriver, ADevice, APort: WideString);
begin
  inherited Create;
  FDriver := ADriver;
  FDevice := ADevice;
...
function TPrinterDevice.Equals(const ADriver, ADevice, APort: WideString): Boolean;
begin
...
function TPrinterDevice.GetTitle: WideString;
begin
...
{$IFNDEF FPC}
  function CharPrev(Start, Current: PWideChar): PWideChar;
  begin
...
{$ENDIF}
  ...
  procedure TextExtent(Text: PWideChar; Len: Integer; var R: TRectF);
  var
    S: WideString;
  begin
...
{$IFNDEF FPC}
  ...
  begin
    ...
    SetLength(S, L);
    Len := UnicodeFromLocaleChars(CodePage, 0, Text, Len, PWideChar(S), Length(S));
    LText := PWideChar(S);
    while Len > 0 do
    begin
      L := Len;
