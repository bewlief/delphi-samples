...
uses
  ...
  FMX_Types, FMX_Forms, FMX_Printer;
...
{ TOpenDialog }
  ...
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    property Files: TWideStrings read GetFiles;
    property HistoryList: TWideStrings read FHistoryList write SetHistoryList;
  published
...
function InputQuery(const ACaption: WideString; const APrompts: array of string; var AValues: array of string; CloseQueryFunc: TInputCloseQueryFunc = nil): Boolean; overload;
function InputQuery(const ACaption: WideString; const APrompts: array of string; var AValues: array of string; CloseQueryEvent: TInputCloseQueryEvent; Context: TObject = nil): Boolean; overload;
function InputQuery(const ACaption, APrompt: WideString; var Value: WideString): Boolean; overload;
{$ENDIF}

implementation

uses Math, FMX_Consts, FMX_Platform, FMX_Controls, FMX_Objects, FMX_Layouts, FMX_Edit;
...
type
  ...
  protected
    ...
    procedure WriteToClipBoard(const Text: WideString);
...
procedure TMessageForm.WriteToClipBoard(const Text: WideString);
begin
  Platform.SetClipboard(Text);
end;

function TMessageForm.GetFormText: WideString;
var
...
function StripHotKey(const AText: WideString): WideString;
begin
  Result := AText;
  if Pos('&', AText) <>0 then
...
var
  ...
  TextRect: TRectF;
  LButton: TButton;
  IconStyle: WideString;
  S: TFmxObject;
  NewMsg: WideString;
  lblMessage: TLabel;
  function CreateLabel(Form: TForm; Msg: WideString): TLabel;
  begin
...
begin
  inherited Create(AOwner);
  FHistoryList := TWideStringList.Create;
...
function TOpenDialog.GetFiles: TWideStrings;
begin
  Result := FFiles;
end;
...
function TOpenDialog.GetInitialDir: WideString;
begin
  Result := FInitialDir;
end;

function TOpenDialog.GetTitle: WideString;
begin
  Result := FTitle;
end;
...
procedure TOpenDialog.SetHistoryList(Value: TWideStrings);
begin
  FHistoryList.Assign(Value);
end;

procedure TOpenDialog.SetInitialDir(const Value: WideString);
var
  L: Integer;
begin
  L := Length(Value);
...
procedure TOpenDialog.SetTitle(const Value: WideString);
begin
...
function InputQuery(const ACaption: WideString; const APrompts: array of string; var AValues: array of string; CloseQueryFunc: TInputCloseQueryFunc = nil): Boolean;
var
  I, J: Integer;
  ...
  function GetPromptCaption(const ACaption: WideString): WideString;
  begin
  ...
  function GetPasswordChar(const ACaption: WideString): WideChar;
  begin
    if (Length(ACaption) > 1) and (ACaption[1] < #32) then
      Result := '*'
    else
...
function InputQuery(const ACaption: WideString; const APrompts: array of string; var AValues: array of string; CloseQueryEvent: TInputCloseQueryEvent; Context: TObject = nil): Boolean;
begin
