...
{$IF NOT DEFINED(CLR)}
  Winapi.OleAcc,
{$IFEND}
  Winapi.Windows, Winapi.Messages, System.SysUtils, System.Classes, Vcl.Forms, Vcl.Menus,
...
{$ELSE}
  System.Character,
{$IFEND}
  Vcl.Dialogs, Winapi.CommCtrl, System.Math, System.Types, Vcl.GraphUtil, Vcl.Themes, Vcl.ActnMenus, Vcl.Consts,
...
begin
  ...
  begin
    ...
    if (Action <> nil) and (Action.ActionList <> nil) then
      LImageList := Action.ActionList.Images;
...
procedure TCustomActionBar.CMThemeChanged(var Message: TMessage);
begin
  SetColorMap(nil);
  RecreateControls;
end;
...
begin
  ...
  begin
    ...
    if Assigned(ActionClient.Action) and Assigned(ActionClient.Action.ActionList) then
      LEnabledImages := ActionClient.Action.ActionList.Images
    else
      LEnabledImages := ActionClient.OwningCollection.ActionManager.Images;
  end;
