...
uses
  Winapi.Windows, Winapi.Messages, System.Classes, System.Contnrs, Vcl.Buttons, Vcl.Graphics, Vcl.Forms, Vcl.Controls,
...
{$IFEND}
  System.Types, System.SysUtils, Vcl.Dialogs, Vcl.Consts, Winapi.MMSystem, Vcl.GraphUtil,
...
procedure TCustomActionMenuBar.Select(const Forward: Boolean);
  ...
  begin
    if AllowScrolling and (AnItem.Control.Top = -MAXINT) then
    begin
      if Forward then
      begin
        LBtn := FScrollDown;
      ...
      while (AnItem.Control.Top = -MAXINT) do
      begin
        LBtn.Click;
        if not LBtn.Visible then
          Break;
