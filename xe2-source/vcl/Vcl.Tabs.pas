...
uses Winapi.Windows, System.Classes, Vcl.Graphics, Vcl.Forms, Vcl.Controls, Winapi.Messages, Vcl.ImgList,
...
{$IFEND}
  System.SysUtils, System.Types, System.Math, Vcl.GraphUtil, Vcl.Consts;
  ...
  begin
    ...
    if FHover and CanScrollLeft and (R.Contains(FHoverPoint)) and not FDown then
      StyleServices.DrawElement(Canvas.Handle,
        StyleServices.GetElementDetails(tsDownHorzHot), R)
    else
      StyleServices.DrawElement(Canvas.Handle,
        StyleServices.GetElementDetails(LeftButton[CanScrollLeft, FDown and (FCurrent = sbLeft)]), R);

    R := TRect.Create(FWidth div 2, 0, FWidth, FHeight);
    if FHover and CanScrollRight and (R.Contains(FHoverPoint))and not FDown then
      StyleServices.DrawElement(Canvas.Handle,
        StyleServices.GetElementDetails(tsUpHorzHot), R)
    else
      StyleServices.DrawElement(Canvas.Handle,
        StyleServices.GetElementDetails(RightButton[CanScrollRight, FDown and (FCurrent = sbRight)]), R);
  end
  else
    with Canvas do
    begin
...
type
  TTabItem = TCustomData; // TObject in .NET and Pointer in Win32

function TabPosCompare(Item1, Item2: TTabItem): Integer;
var
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        FTabPositions.Add(TTabItem(TabPos));    { add to list }
        Inc(Index);
      end;
      FSortedTabPositions.Add(TTabItem(TabPos));
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        FSortedTabPositions[I] := TTabItem(TabPos);
...
begin
  ...
  begin
    TabPos := TTabPos(FTabPositions[Index]);
...
begin
  ...
  begin
    ...
    if GetThemeColor(LDetails, ecEdgeShadowColor, LColor) then
      Pen.Color := LColor
    else
    ...
    begin
      if GetThemeColor(LDetails, ecEdgeDkShadowColor, LColor) then
        Pen.Color := LColor
      else
  ...
  begin
    TabPos := TTabPos(FTabPositions[Tab]);

    Inc(TabPos.Size, 1);
    Inc(TabPos.StartPos, Tab - 1);
    ...
    if isSelected then
      if GetThemeColor(LDetails, ecFillColor, LColor) then
    ...
    begin
      ...
      begin
        ...
        begin
          if GetThemeColor(StyleServices.GetElementDetails(TabState[isSelected]), ecTextColor, LColor) then
...
var
  ...
  begin
    ...
    begin
      ...
      if SoftTop then
      begin
        if GetThemeColor(LBkDetails, ecFillColor, LColor) then
          Pen.Color := LColor
        else
          Pen.Color := BackgroundColor;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if GetThemeColor(LDetails, ecEdgeHighLightColor, LColor) then
          Pen.Color := LColor
        else
...
begin
  ...
  begin
    Brush.Style := bsSolid;
    if GetThemeColor(StyleServices.GetElementDetails(tbsBackground), ecFillColor, LColor) then
      Brush.Color := LColor
    else
      Brush.Color := FBackgroundColor;
    FillRect(TRect.Create(0, 0, Width, Height));
    if FDitherBackground and not TStyleManager.IsCustomStyleActive then
      for Y := 0 to 7 do
        for X := 0 to 7 do
...
begin
  ...
  begin
    ...
    if StyleServices.Enabled then
      LDetails := StyleServices.GetElementDetails(tbsTabNormal);
    if GetThemeColor(LDetails, ecEdgeDkShadowColor, LColor) then
      Pen.Color := LColor
    else
      Pen.Color := clWindowFrame;
  ...
  begin
    ...
    begin
      if StyleServices.Enabled then
        LDetails := StyleServices.GetElementDetails(tbsTabSelected);
...
begin
  ...
  begin
    ...
    with FMemBitmap.Canvas do
    begin
      if GetThemeColor(StyleServices.GetElementDetails(TabState[False]), ecEdgeFillColor, LColor) then
        Pen.Color := LColor
      else
        Pen.Color := clBtnShadow;
      Pen.Width := 1;
      if isSelected then
      begin
        if GetThemeColor(StyleServices.GetElementDetails(TabState[isSelected]), ecFillColor, LColor) then
          Brush.Color := LColor
        else
