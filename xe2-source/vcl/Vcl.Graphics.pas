...
{$IF NOT DEFINED(CLR)}
  System.Win.Registry, 
{$IFEND}
  System.Types, System.UIConsts, Vcl.Consts, Winapi.ActiveX, System.TypInfo;
...
[RegistryPermission(SecurityAction.Assert, Read='HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows NT\CurrentVersion\FontSubstitutes\')]
...
begin
  ...
  try
    ...
    begin
      FName := Reg.ReadString('MS Shell Dlg 2'); { do not localize }
      Reg.CloseKey;
    end;
  finally
    Reg.Free;
  ...
  if FName <> '' then
    DefFontData.Name := UTF8EncodeToShortString(FName);
{$IFEND}
end;
