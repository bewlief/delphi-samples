...
uses
  Winapi.Windows, Winapi.Messages, Vcl.Graphics, System.SysUtils, System.Classes, Vcl.Controls, System.Types, Vcl.ExtCtrls,
...
procedure TCustomRibbon.CMThemeChanged(var Message: TMessage);
begin
  inherited;
...
begin
  ...
  begin
    if PtInRect(GetTabRect(I), Point(X, Y)) then
    begin
      Result := I;
      Exit;
    end;
...
begin
  ...
  begin
    ...
    if (tsbTabRight in FTabData.ScrollButtons) and
       PtInRect(FTabData.RightTabScrollButtonRect, LPt) then
    begin
    ...
    if (tsbTabLeft in FTabData.ScrollButtons) and
       PtInRect(FTabData.LeftTabScrollButtonRect, LPt) then
    begin
...
begin
  ...
  begin
    ...
    if PtInRect(LButtonRect, LPt) then
    begin
      if IsLeftMouseButtonPressed(Self) then
...
begin
  if not HasQuickAccessToolbar or (ActionManager = nil) or HideRibbon or
    PtInRect(FHeaderRect, MousePos) then
    Exit;
  ...
  begin
    ...
    begin
      ...
      begin
        if (LGroup.DialogAction <> nil) and PtInRect(LGroup.GetDialogButtonRect, LCtrlPt) then
    ...
    begin
      ...
      begin
        ...
        if PtInRect(QuickAccessToolbar.ActionBar.BoundsRect, LPt) then
        begin
...
begin
  ...
  if IsLeftMouseButtonPressed(Self) then
  begin
    MouseInControl := PtInRect(ClientRect, Point(X, Y));
  end;
end;
...
begin
  inherited;
  LPt := ScreenToClient(Mouse.CursorPos);
  if not PtInRect(ClientRect, LPt) then
  MouseInControl := False;
...
begin
  inherited;
  if (FDialogAction <> nil) and PtInRect(GetDialogButtonRect, Point(X, Y)) then
...
begin
  ...
  begin
    LPt := Point(X, Y);
    DialogButtonHot := not DesignMode and PtInRect(GetDialogButtonRect, LPt);
  end;
end;
...
begin
  ...
  if (Button = mbLeft) then
  begin
    if (DialogAction <> nil) and PtInRect(GetDialogButtonRect, Point(X, Y)) then
    begin
