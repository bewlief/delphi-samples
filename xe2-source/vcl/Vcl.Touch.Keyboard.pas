...
procedure TCustomTouchKeyboard.Initialize;
begin
  if not FInitialized and CheckWin32Version(6, 1) and RegisterTouchWindow(Handle, 0) then
    FInitialized := True;
end;
...
begin
  ...
  if (Message.Msg = WM_DESTROY) and FInitialized then
  begin
...
procedure TCustomTouchKeyboard.Loaded;
begin
  inherited;
end;
...
procedure TCustomTouchKeyboard.ProcessKeyPress(const Msg: TWmMouse; ID: Integer);
  ...
  begin
    ...
    for Index := 0 to FButtons.Count - 1 do
    begin
      if PtInRect(FButtons[Index].BoundsRect, Point) then
      begin
        Result := FButtons[Index];
        Break;
      end;
