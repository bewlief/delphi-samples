...
protected
  ...
  TEditStyleHook = class(TStyleHook)
  strict private
...
begin
  if Observers.IsObserving(TObserverMapping.EditLinkID) then
  begin
    if (TLinkObservers.EditLinkIsReadOnly(Observers) <> Value) then
    begin
...
begin
  Result.X := (SendMessage(Handle, EM_GETSEL, 0, 0) shr 16) and $FFFF;
...
{$ELSE}
  ...
  UniqueString(LString);
{$IFEND}
  LResult := SendTextMessage(ComboBox.Handle, CB_INSERTSTRING, Index, LString);
  if LResult < 0 then
    raise EOutOfResources.Create(SInsertLineError);
end;
...
begin
  ...
  begin
    ...
    begin
      if not TLinkObservers.EditLinkEdit(Observers) and (Key in [VK_UP, VK_DOWN]) then
        Key := 0;
      if TLinkObservers.EditLinkIsEditing(Observers) then
        TLinkObservers.EditLinkModified(Observers);
    end;
...
begin
  ...
  if ID = TObserverMapping.EditLinkID then
    Result := True
  else if ID = TObserverMapping.EditLinkID then
    Result := True;
...
begin
  ...
  begin
    ...
    else if PtInRect(VertDownButtonRect, P) then
    begin
      FVDownState := tsArrowBtnDownPressed;
      PaintScrollBar;
    end
    else if PtInRect(VertSliderRect, P) then
    begin
  ...
  begin
    ...
    else if PtInRect(HorzDownButtonRect, P) then
    begin
      FHDownState := tsArrowBtnRightPressed;
      PaintScrollBar;
    end
    else if PtInRect(HorzSliderRect, P) then
    begin
...
begin
  Self.CallDefaultProc(TMessage(Message));
  Handled := True;
end;
...
begin
  ...
  try
    ...
    if (Style <> csSimple) and (FEditHandle = 0) and (SendMessage(Handle, CB_GETCURSEL, 0, 0) >= 0) then
    begin
      R := Control.ClientRect;
      InflateRect(R, -3, -3);
      if Control.BiDiMode <> bdRightToLeft then
...
var
  ...
  begin
    ...
    if PtInRect(ListBoxVertScrollArea, P) then
    begin
      if PtInRect(ListBoxVertUpButtonRect, Point(Msg.XPos, Msg.YPos)) then
      begin
        SendMessage(FListHandle, WM_SETREDRAW, 0, 0);
      ...
      if PtInRect(ListBoxVertDownButtonRect, Point(Msg.XPos, Msg.YPos)) then
      begin
        SendMessage(FListHandle, WM_SETREDRAW, 0, 0);
        SendMessage(FListHandle, LB_SETTOPINDEX, SendMessage(FListHandle, LB_GETTOPINDEX, 0, 0) + 1, 0);
        SendMessage(FListHandle, WM_SETREDRAW, 1, 0);
  ...
  begin
    ...
    begin
      ...
      FDownPos := P;
      if PtInRect(ListBoxVertTrackRectUp, P) then
      begin
      ...
      else if PtInRect(ListBoxVertTrackRectDown, P) then
      begin
        ItemHeight := SendMessage(FListHandle, LB_GETITEMHEIGHT, 0, 0);
        if ItemHeight > 0 then
          VisibleCount := ListBoxClientRect.Height div ItemHeight
      ...
      else if PtInRect(ListBoxVertSliderRect, P) then
      begin
        FVSliderState := tsThumbBtnVertPressed;
        FDownSliderPos := FDownPos.Y - ListBoxVertSliderRect.Top;
        DrawListBoxVertScroll(0);
      end
      else if PtInRect(ListBoxVertDownButtonRect, P) then
      begin
      ...
      else if PtInRect(ListBoxVertUpButtonRect, P) then
      begin
  ...
  begin
    ...
    if FListBoxUpBtnDown and not PtInRect(ListBoxVertUpButtonRect, P) and (FVUpState = tsArrowBtnUpPressed)
    then
    begin
      FVUpState := tsArrowBtnUpNormal;
      DrawListBoxVertScroll(0);
    ...
    if FListBoxUpBtnDown and PtInRect(ListBoxVertUpButtonRect, P) and (FVUpState = tsArrowBtnUpNormal)
    then
    begin
      FVUpState := tsArrowBtnUpPressed;
      DrawListBoxVertScroll(0);
    ...
    if FListBoxDownBtnDown and not PtInRect(ListBoxVertDownButtonRect, P) and (FVDownState = tsArrowBtnDownPressed)
    then
    begin
      FVDownState := tsArrowBtnDownNormal;
      DrawListBoxVertScroll(0);
    ...
    if FListBoxDownBtnDown and PtInRect(ListBoxVertDownButtonRect, P) and (FVDownState = tsArrowBtnDownNormal)
    then
    begin
      FVDownState := tsArrowBtnDownPressed;
      DrawListBoxVertScroll(0);
    ...
    if PtInRect(ListBoxVertScrollArea, P) then
    begin
      if PtInRect(ListBoxVertSliderRect, P) and (FVSliderState = tsThumbBtnVertNormal) then
      begin
        FVSliderState := tsThumbBtnVertHot;
        DrawListBoxVertScroll(0);
      end
      else if not PtInRect(ListBoxVertSliderRect, P) and (FVSliderState = tsThumbBtnVertHot) then
      begin
        FVSliderState := tsThumbBtnVertNormal;
        DrawListBoxVertScroll(0);
      end
      else if PtInRect(ListBoxVertUpButtonRect, P) and (FVUpState = tsArrowBtnUpNormal) then
      begin
        FVUpState := tsArrowBtnUpHot;
        DrawListBoxVertScroll(0);
      end
      else if not PtInRect(ListBoxVertUpButtonRect, P) and (FVUpState = tsArrowBtnUpHot) then
      begin
        FVUpState := tsArrowBtnUpNormal;
        DrawListBoxVertScroll(0);
      end
      else if PtInRect(ListBoxVertDownButtonRect, P) and (FVDownState = tsArrowBtnDownNormal) then
      begin
        FVDownState :=  tsArrowBtnDownHot;
        DrawListBoxVertScroll(0);
      end
      else if not PtInRect(ListBoxVertDownButtonRect, P) and (FVDownState =  tsArrowBtnDownHot) then
      begin
        FVDownState :=  tsArrowBtnDownNormal;
        DrawListBoxVertScroll(0);
      end;
  ...
  begin
    ...
    if (Style = csSimple) and PtInRect(ListBoxVertScrollArea, FDownPos)
    then
      ReleaseCapture;


    if PtInRect(ListBoxVertSliderRect, P) then
      FVSliderState := tsThumbBtnVertHot
    else
      FVSliderState := tsThumbBtnVertNormal;

    if PtInRect(ListBoxVertUpButtonRect, P) then
      FVUpState := tsArrowBtnUpHot
    else
      FVUpState := tsArrowBtnUpNormal;

    if PtInRect(ListBoxVertDownButtonRect, P) then
      FVDownState := tsArrowBtnDownHot
    else
      FVDownState := tsArrowBtnDownNormal;
    ...
    MsgHandled := PtInRect(ListBoxVertScrollArea, P);
  end;
  ...
  begin
    case FListBoxTimerCode of
      ...
      7:
        begin
          if PtInRect(ListBoxVertSliderRect, FMovePos) or (FMovePos.Y > ListBoxVertSliderRect.Bottom) then
          begin
            ListBoxStopTimer;
            Exit;
          end;
      ...
      8:
        begin
          if PtInRect(ListBoxVertSliderRect, FMovePos) or (FMovePos.Y < ListBoxVertSliderRect.Top) then
          begin
            ListBoxStopTimer;
            Exit;
          end;
...
begin
  ...
  begin
    ...
    begin
      ...
      Inc(DrawRect.Top, 15);
      Inc(DrawRect.Left, 5);
      Canvas.Font := TCustomButton(Control).Font;
      Canvas.Font.Style := [];
      Canvas.Font.Size := 12;
      TextFormat := TTextFormatFlags(DT_LEFT);
      if StyleServices.GetElementColor(Details, ecTextColor, ThemeTextColor) then
         Canvas.Font.Color := ThemeTextColor;
      StyleServices.DrawText(Canvas.Handle, Details, BCaption, DrawRect, TextFormat, Canvas.Font.Color);
      ...
      begin
        ...
        begin
          ...
          Canvas.Font.Size := 8;
          StyleServices.DrawText(Canvas.Handle, Details, Buffer, DrawRect,
    ...
    begin
      ...
      DrawControlText(Canvas, Details, Text, DrawRect, DT_VCENTER or DT_CENTER);
      if FDropDown then
      begin
        Details := StyleServices.GetElementDetails(tbPushButtonPressed);
        SaveIndex := SaveDC(Canvas.Handle);
      ...
      if (Control is TCustomButton) and TCustomButton(Control).WordWrap then
        DrawControlText(Canvas, Details, BCaption, DrawRect, DT_VCENTER or DT_CENTER or DT_WORDBREAK)
      else
        DrawControlText(Canvas, Details, BCaption, DrawRect, DT_VCENTER or DT_CENTER);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      R := Rect(0, 0, BoxSize.X, BoxSize.Y);
      RectVCenter(R, Rect(0, 0, Control.Width, Control.Height));
    end
    else
    begin
      R := Rect(Control.Width - BoxSize.X - 1, 0, Control.Width, Control.Height);
      RectVCenter(R, Rect(Control.Width - BoxSize.X - 1, 0, Control.Width, Control.Height));
    end;

    StyleServices.DrawElement(Canvas.Handle, Details, R);
    Canvas.Font := TWinControlClass(Control).Font;

    R := Rect(0, 0, Control.Width - BoxSize.X - 10, Control.Height);
    LCaption := Text;
    DrawText(Canvas.Handle, PWideChar(LCaption), Length(LCaption), R, Control.DrawTextBiDiModeFlags(DT_CALCRECT or DT_EXPANDTABS));

    if not RightAlignment then
      RectVCenter(R, Rect(BoxSize.X + Spacing, 0, Control.Width, Control.Height))
    else
     begin
       if Control.BiDiMode <> bdRightToLeft then
         RectVCenter(R, Rect(3, 0,
           Control.Width - BoxSize.X - Spacing, Control.Height))
       else
         RectVCenter(R, Rect(Control.Width - BoxSize.X - Spacing - R.Right, 0,
           Control.Width - BoxSize.X - Spacing, Control.Height));
     end;
...
begin
  ...
  if Control.BiDiMode <> bdRightToLeft then
    Result := Rect(FCaptionMargin, 0,
      FCaptionMargin + Canvas.TextWidth(Text), Canvas.TextHeight(Text))
  else
    Result := Rect(Control.Width - Canvas.TextWidth(Text) - FCaptionMargin, 0,
     Control.Width  - FCaptionMargin, Canvas.TextHeight(Text))
end;

function TGroupBoxStyleHook.GetBoxRect(Canvas: TCanvas): TRect;
begin
  with Canvas do
    Result := Rect(0, TextHeight(Text) div 2 + 1, Control.Width - 0, Control.Height - 0);
end;
