...
uses 
  Winapi.Windows, Winapi.Messages, System.SysUtils, System.Classes, System.Variants, Vcl.Controls, Vcl.Grids, Vcl.StdCtrls;
...
{$IFEND}
  Vcl.Forms, Vcl.Dialogs, System.TypInfo, Vcl.Consts, Vcl.Themes;
...
begin
  ...
  FTitleCaptions := TValueListTitles.Create(Self);
...
begin
  if goColMoving in Value then
    raise Exception.CreateRes({$IFNDEF CLR}@{$ENDIF}SNoColumnMoving);
...
begin
  Result := True;
  if Pos('=', Key) <> 0 then
    raise Exception.CreateRes({$IFNDEF CLR}@{$ENDIF}SNoEqualsInKey);
  ...
  begin
    ...
    begin
      ...
      if not Result and RaiseError then
        raise Exception.CreateResFmt({$IFNDEF CLR}@{$ENDIF}SKeyConflict, [Key]);
    end;
  end;
end;
...
{$IFEND}
    ...
    begin
{$IF DEFINED(CLR)}
      OldIndex := OldOrder.IndexOf(TObject(Get(I)));
{$ELSE}
...
begin
  ...
  begin
    ...
    begin
{$IF DEFINED(CLR)}
      Index := IndexOfName(VarToStr(KeyOrIndex));
{$ELSE}
      Index := IndexOfName(KeyOrIndex);
{$IFEND}
      if Create and (Index = -1) then
        raise Exception.CreateResFmt({$IFNDEF CLR}@{$ENDIF}SKeyNotFound, [KeyOrIndex]);
    end;
...
constructor TValueListTitles.Create(AEditor: TValueListEditor);
begin
  FEditor := AEditor;
  inherited Create;
end;
