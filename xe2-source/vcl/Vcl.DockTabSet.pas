...
begin
  ...
  Visible := False;
  FGrabberSize := 23; // This should match value in CaptionedDockTree.pas
  FSplitterWidth := 3;
  FAnimateSpeed := 200;
...
begin
  if FIsNotXP or
    ((not FInAnimation or FFirstErase) or (not StyleServices.Enabled)) then
  begin
    inherited;
    FFirstErase := False;
  end;
...
begin
  ...
  with FDockClientInfo do
    if Application.Active and { Not in the panel rect }
       (not System.Types.PtInRect(Rect(0, 0, Width, Height),
        ScreenToClient(Mouse.CursorPos))) and { Not in the tab dock }
       (not System.Types.PtInRect(Rect(0, 0, DockTabSet.Width, DockTabSet.Height),
