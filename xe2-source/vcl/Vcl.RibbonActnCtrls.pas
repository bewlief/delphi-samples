...
begin
  Result := PtInRect(FSplitRect, Point(X, Y));
end;
...
begin
  ...
  begin
    ...
    if ActionClient.Action.ActionList.Images.Width > 16 then
      Canvas.RoundRect(2, 2, 42, 42, 2, 2)
    else
      Canvas.RoundRect(2, 2, 26, 26, 2, 2);
  end;
  NewLocation := Location;
  if (ActionClient.Action <> nil) and (ActionClient.Action.ActionList.Images.Width > 16) then
  begin
...
begin
  ...
  begin
    ...
    try
      ...
      if (ActionClient <> nil) and (ActionClient.Action <> nil) and
        (ActionClient.Action.ActionList <> nil) and
        (ActionClient.Action.ActionList.Images <> nil) then
        Result := Max(result, ActionClient.Action.ActionList.Images.Height + 8);
    finally
      LCanvas.Handle := 0;
      LCanvas.Free;
    end;
...
begin
  Result := PtInRect(FSplitRect, Pt);
end;

procedure TRibbonSplitMenuItem.MouseMove(Shift: TShiftState; X, Y: Integer);
begin
  inherited;
  MouseInSplit := PtInRect(FSplitRect, Point(X, Y));
end;
...
begin
  ...
  begin
    LPt := ScreenToClient(Mouse.CursorPos);
    LMouseInBtn := PtInRect(FBtnRect, LPt);
...
begin
  inherited;
  CursorInBtn := PtInRect(FBtnRect, Point(X, Y));
end;
...
begin
  Result := CharInSet(Key, [AnsiChar(DecimalSeparator), '+', '-', '0'..'9']) or
