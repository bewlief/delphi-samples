...
type
  ...
  strict protected
    procedure DrawHeaderSection(Canvas: TCanvas; R: TRect; Index: Integer;
      const Text: string; IsPressed, IsBackground: Boolean); virtual;
...
var
  LItemClass: TCustomComboBoxStringsClass;
begin
{$IFDEF CLR}
...
{$ENDIF}
  LItemClass := GetItemsClass;
  if LItemClass.InheritsFrom(TComboBoxExStrings) then
    Items := TComboBoxExStringsClass(LItemClass).Create(Self)
  else
    Items := LItemClass.Create;
...
begin
  inherited;
  OverridePaintNC := not TStyleManager.SystemStyle.Enabled;
  FDroppedDown := False;
...
begin
  inherited;
  if not FDroppedDown and FMouseOnButton then
  begin
    FMouseOnButton := False;
    Invalidate;
  end;
...
begin
  if StyleServices.Available then
  begin
    Detail := teEditBorderNoScrollNormal;
...
begin
  ...
  begin
    SendMessage(Handle, TBM_GETTHUMBRECT, 0, IntPtr(@R));
    NewValue := PtInRect(R, Point(Message.XPos, Message.YPos));
...
begin
  ...
  for I := 0 to GetButtonCount - 1 do
  begin
    if PtInRect(GetItemRect(I), Point(Message.XPos, Message.YPos)) then
    begin
      Info := GetItemInfo(I, @Buffer, BufSize);
