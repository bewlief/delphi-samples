...
{$IFEND}
  Vcl.Consts, System.SysUtils, Vcl.ActnList, Vcl.Themes, Winapi.UxTheme, Winapi.DwmApi;
...
procedure TButtonGlyph.DrawButtonText(Canvas: TCanvas; const Caption: string;
  ...
  begin
    if FThemesEnabled then
    begin
      if (State = bsDisabled) or not StyleServices.IsSystemStyle then
      begin
...
{$IFEND}
  ...
  FSpacing := 4;
  FMargin := -1;
  ControlStyle := ControlStyle + [csReflector, csPaintBlackOpaqueOnGlass, csOverrideStylePaint];
  DoubleBuffered := True;
end;
