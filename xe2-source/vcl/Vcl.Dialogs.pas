...
begin
  if (Win32MajorVersion >= 6) and UseLatestCommonDialogs and
     StyleServices.Enabled and (Template = '') and
    ...
    try
      ...
      begin
        ...
        if FDefaultExtension <> '' then
          SetDefaultExtension({$IFNDEF CLR}PWideChar{$ENDIF}(WideString(FDefaultExtension)));
        if FFileName <> '' then
          SetFileName({$IFNDEF CLR}PWideChar{$ENDIF}(WideString(FFileName)));
        if FFileNameLabel <> '' then
          SetFileNameLabel({$IFNDEF CLR}PWideChar{$ENDIF}(WideString(FFileNameLabel)));
        if FOkButtonLabel <> '' then
          SetOkButtonLabel({$IFNDEF CLR}PWideChar{$ENDIF}(WideString(FOkButtonLabel)));
        if FTitle <> '' then
          SetTitle({$IFNDEF CLR}PWideChar{$ENDIF}(WideString(FTitle)));

        // DefaultFolder
        if FDefaultFolder <> '' then
        begin
          if Succeeded(SHCreateItemFromParsingName({$IFNDEF CLR}PWideChar{$ENDIF}(WideString(FDefaultFolder)),
        ...
        for LPlace in FFavoriteLinks do
          if Succeeded(SHCreateItemFromParsingName({$IFNDEF CLR}PWideChar{$ENDIF}(WideString(LPlace.Location)),
             nil, StringToGUID(SID_IShellItem), LShellItem)) then
