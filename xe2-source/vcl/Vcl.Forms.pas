...
{$IFDEF LINUX}
  WinUtils, libc,
{$ENDIF}
  Winapi.Messages, Winapi.Windows, System.UITypes, System.SysUtils, System.Classes, Vcl.Graphics, Vcl.Menus,
  Vcl.Controls, Winapi.Imm, Vcl.ActnList, Winapi.MultiMon, System.HelpIntfs, Winapi.DwmApi, Vcl.Themes;
...
type
  ...
  strict private type
    ...
    strict private
      ...
      class function PopupMenuHook(Code: Integer; WParam: WPARAM; var Msg: TMsg): Longint; stdcall; static;
    strict private
      FActiveItem: Integer;
      FBoundsRect: TRect;
...
{$IFEND}
  ...
  System.SysConst, Vcl.Printers, Vcl.Consts, Vcl.StdActns, Vcl.StdCtrls,
  Winapi.UxTheme;
...
begin
  ...
  if (igPan in Gestures) and not FAutoScroll then
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            X := (LRect.Right - LRect.Left - Width) div 2;
            Y := (LRect.Bottom - LRect.Top - Height) div 2;
          end;
...
begin
  ...
  begin
    ...
    if Cardinal(Message.Msg) = RM_TaskbarCreated then
    begin
      Screen.ResetFonts;
  ...
  begin
    ...
    if (FMainForm <> nil) then
      ...
      begin
        ...
        SetWindowPos(Handle, 0, GetSystemMetrics(SM_CXSCREEN) div 2,
...
begin
  ...
  begin
    if (NewHintControl <> FHintControl) or
      (not PtInRect(FHintCursorRect, Control.ClientToScreen(SmallPointToPoint(MouseMsg.Pos)))) then
    begin
...
begin
  ...
  begin
    ...
    begin
      OffsetRect(R, -R.Left, 0);
      if HasBorder then OffsetRect(R, 2, 0);
    end;
...
begin
  ...
  if PtInRect(VertDownButtonRect, P) and (VertSliderRect.Height > 0) then
    FVertDownState := tsArrowBtnDownPressed;

  if PtInRect(VertUpButtonRect, P) and (VertSliderRect.Height > 0) then
    FVertUpState := tsArrowBtnUpPressed;

  if PtInRect(HorzDownButtonRect, P) and (HorzSliderRect.Width > 0)  then
    FHorzDownState := tsArrowBtnRightPressed;

  if PtInRect(HorzUpButtonRect, P) and (HorzSliderRect.Width > 0) then
...
begin
  ...
  if VertSliderRect.Height > 0 then
  if PtInRect(VertSliderRect, P) and (VertSliderState = tsThumbBtnVertNormal) then
  begin
    FVertSliderState := tsThumbBtnVertHot;
    MustUpdateScroll := True;
  end
  else if not PtInRect(VertSliderRect, P) and (VertSliderState = tsThumbBtnVertHot) then
  begin
    FVertSliderState := tsThumbBtnVertNormal;
     MustUpdateScroll := True;
  end;
  ...
  if HorzSliderRect.Width > 0 then
    if PtInRect(HorzSliderRect, P) and (FHorzSliderState = tsThumbBtnHorzNormal) then
    begin
      FHorzSliderState := tsThumbBtnHorzHot;
      MustUpdateScroll := True;
    end
    else if not PtInRect(HorzSliderRect, P) and (FHorzSliderState = tsThumbBtnHorzHot) then
    begin
      FHorzSliderState := tsThumbBtnHorzNormal;
      MustUpdateScroll := True;
    end;
  ...
  if VertSliderRect.Height > 0 then
  if PtInRect(VertDownButtonRect, P) and (FVertDownState = tsArrowBtnDownNormal) then
  begin
    FVertDownState := tsArrowBtnDownHot;
    MustUpdateScroll := True;
  end
  else if not PtInRect(VertDownButtonRect, P) and (FVertDownState = tsArrowBtnDownHot) then
  begin
    FVertDownState := tsArrowBtnDownNormal;
    MustUpdateScroll := True;
  end;
  ...
  if VertSliderRect.Height > 0 then
    if PtInRect(VertUpButtonRect, P) and (FVertUpState = tsArrowBtnUpNormal) then
    begin
      FVertUpState := tsArrowBtnUpHot;
      MustUpdateScroll := True;
    end
    else if not PtInRect(VertUpButtonRect, P) and (FVertUpState = tsArrowBtnUpHot) then
    begin
      FVertUpState := tsArrowBtnUpNormal;
      MustUpdateScroll := True;
    end;
  ...
  if HorzSliderRect.Width > 0 then
    if PtInRect(HorzDownButtonRect, P) and (FHorzDownState = tsArrowBtnRightNormal) then
    begin
      FHorzDownState := tsArrowBtnRightHot;
      MustUpdateScroll := True;
    end
    else if not PtInRect(HorzDownButtonRect, P) and (FHorzDownState = tsArrowBtnRightHot) then
    begin
      FHorzDownState := tsArrowBtnRightNormal;
      MustUpdateScroll := True;
    end;
  ...
  if HorzSliderRect.Width > 0 then
    if PtInRect(HorzUpButtonRect, P) and (FHorzUpState = tsArrowBtnLeftNormal) then
    begin
      FHorzUpState := tsArrowBtnLeftHot;
      MustUpdateScroll := True;
    end
    else if not PtInRect(HorzUpButtonRect, P) and (FHorzUpState = tsArrowBtnLeftHot) then
    begin
      FHorzUpState := tsArrowBtnLeftNormal;
      MustUpdateScroll := True;
    end;
...
begin
  Result := -1;
  for I := 0 to 2 do
    if PtInRect(FMDIButtons[I].ItemRect, Point(X, Y)) then
...
begin
  ...
  for I := 0 to FItemCount - 1 do
    if FItems[I].MenuItem.Visible and FItems[I].MenuItem.Enabled and
       PtInRect(FItems[I].ItemRect, Point(X, Y)) then
      Exit(FItems[I].Index);
end;
...
begin
  ...
  if FFormHook.Form.FormStyle = fsMDIForm then
    with FFormHook.Form do
      if (ActiveMDIChild <> nil) and (ActiveMDIChild.Active) and
         (ActiveMDIChild.Menu <> nil) and
         (ActiveMDIChild.Menu.Items.Count > 0) and
         (ActiveMDIChild.Handle <> FFormHook.FChangeVisibleChildHandle) then
        FChildMenu := ActiveMDIChild.Menu;
...
class function TFormStyleHook.TMainMenuBarStyleHook.PopupMenuHook(Code: Integer; WParam: WPARAM; var Msg: TMsg): Longint;
var
  FItem: Integer;
...
begin
  ...
  begin
    if FShowMDIButtons and CanTrackMDISystemMenu and PtInRect(Rect(0, 0,
       GetSystemMetrics(SM_CYMENU), GetSystemMetrics(SM_CYMENU)), Point(X, Y)) then
      TrackMDIChildSystemMenu;
  end;
...
begin
  ...
  begin
    if (FMainMenuBarHook <> nil) and PtInRect(FMainMenuBarHook.BoundsRect, P) then
      Exit(HTMENU)
    else
      Exit;
  end;
  ...
  if (FMainMenuBarHook <> nil) and PtInRect(FMainMenuBarHook.BoundsRect, P) then
    Exit(HTMENU)
  else if PtInRect(FHitCaptionRect, P) then
    Exit(HTCAPTION)
  ...
  begin
    if PtInRect(Rect(FBorderSize.Left, FBorderSize.Top,
       FWidth - FBorderSize.Right, FHeight - FBorderSize.Bottom), P) then
      Exit(HTCLIENT)
    else
      Exit(HTBORDER);
  end;
...
begin
  if (FMainMenuBarHook <> nil) and FMainMenuBarHook.CheckHotKeyItem(Message.CharCode) then
  begin
    Message.Result := 1;
    Handled := True;
  end;
...
begin
  if not (fsShowing in Form.FormState) then
    SetRedraw(False);
...
begin
  if WPARAM(GetMenu(Control.Handle)) = Message.wParam then
...
begin
  ...
  begin
    ...
    if not TStyleManager.SystemStyle.Enabled and (GetWindowLong(Handle, GWL_STYLE) and WS_CAPTION <> 0) and
