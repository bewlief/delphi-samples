...
{$IFEND}
  ...
  System.Types, Vcl.ListActns, Vcl.ActnColorMaps, Vcl.Themes;
...
begin
  ...
  try
   if TStyleManager.IsCustomStyleActive then
    begin
      if Enabled then
        TempCanvas.Pen.Color := StyleServices.GetSystemColor(clBtnText)
      else
        TempCanvas.Pen.Color := StyleServices.GetSystemColor(clGrayText);
    end
    else
    if Enabled then
...
begin
  Result := PtInRect(System.Types.Rect(ClientWidth - GetDropDownButtonWidth,
    0, ClientWidth, ClientHeight), ScreenToClient(Mouse.CursorPos));
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      // tweak text
      LCaption := Text;
      if (tfCalcRect in LFormats) and ( (LCaption = '') or (LCaption[1] = '&') and (LCaption[2] = #0) ) then
        LCaption := LCaption + ' ';

      // Draw menu item text
      StyleServices.DrawText(Canvas.Handle, LDetails, LCaption, TextRect, LFormats, LColor);
    end;
    // draw glyph
    if TActionBarClass(ActionBar).HasItems then
      if TActionBarClass(ActionBar).Items.SmallIcons then
        DrawGlyph(GlyphPos)
      else
        DrawLargeGlyph(GlyphPos);
  end
  else
    inherited Paint;
  DrawDropDownArrow;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        NewTop := -MAXINT; // Make the control not appear in the client area
      end
      else
