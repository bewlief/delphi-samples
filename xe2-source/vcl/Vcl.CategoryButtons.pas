...
{$IF DEFINED(CLR)}
  WinUtils,
{$IFEND}
  Winapi.Windows, Winapi.Messages, System.SysUtils, System.Classes, Vcl.ImgList, Vcl.Forms, Vcl.Controls,
  Vcl.Graphics, Vcl.StdCtrls, Vcl.GraphUtil, Vcl.ActnList, Vcl.Themes;
...
begin
  ...
  if (FSelectedItem <> Category) and
     ((boGradientFill in FButtonOptions) or LStyle.Enabled) then
  begin
    if LStyle.Enabled then
    begin
      LDetails := LStyle.GetElementDetails(tcbCategoryNormal);
  ...
  begin
    if LStyle.Enabled and LStyle.GetElementColor(LDetails, ecEdgeShadowColor, LColor) and (LColor <> clNone) then
