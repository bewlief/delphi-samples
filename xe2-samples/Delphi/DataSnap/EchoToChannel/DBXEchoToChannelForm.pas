...
//---------------------------------------------------------------------------
// Demonstrates how to register a client channel and callback.
// In this example the callback and channel have the same name, since
// the client only registers a single callback.
//
// Known issue.  If the server is shutdown while the client is
// running, the client will not be able to re-establish a channel
// connection even after the server is restarted.
// To work around this issue, the client would need to
// destroy and recreate TDSClientCallbackChannelManager.
...
uses
  ...
  Dialogs, StdCtrls, DSHTTPCommon, DBXJson, AppEvnts;
