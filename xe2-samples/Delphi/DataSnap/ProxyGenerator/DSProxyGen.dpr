    ...
    try
      ...
      try
       ...
       // Set default values for connection string
        if not StrUtils.ContainsText(ConnectionString, TDBXPropertyNames.DriverName) then
          ConnectionString := ConnectionString + ';' + TDBXPropertyNames.DriverName+'=DataSnap';
        if not StrUtils.ContainsText(ConnectionString, TDBXPropertyNames.HostName) then
          ConnectionString := ConnectionString + ';' + TDBXPropertyNames.HostName+'=localhost';
        if not StrUtils.ContainsText(ConnectionString, TDBXPropertyNames.Port) then
