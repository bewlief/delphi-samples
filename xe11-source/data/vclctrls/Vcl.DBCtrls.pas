...
function OkToChangeFieldBidiMode(AField: TField): Boolean;
function DBUseRightToLeftBidiMode(AControl: TControl; AField: TField): Boolean;
...
const
  ...
  FieldAlignmentTbl: array[TFieldType] of Boolean= (
    ...
    False, False, True,  True,   { ftTimeStamp, ftFMTBcd, ftFixedWideChar, ftWideMemo}
    False, False,                { ftOraTimeStamp, ftOraInterval }
    False, False, False, False,  { ftLongWord, ftShortint, ftByte, ftExtended}
    True,  True,  True,  False,  { ftConnection, ftParams, ftStream, ftTimeStampOffset}
    True,  False);               { ftObject, ftSingle }
...
function DBUseRightToLeftBidiMode(AControl: TControl; AField: TField): Boolean;
begin
  Result := (SysLocale.MiddleEast) and Assigned(AControl) and
    (AControl.BiDiMode = bdRightToLeft) and (OkToChangeFieldBidiMode(AField));
end;
...
{ Since edit controls do not handle justification unless multi-line (and
  ...
  try
    ...
    begin
      ...
      if (S = '') and (TextHint <> '') and not Focused then
      begin
        S := TextHint;
        AAlignment := taLeftJustify;
        if IsCustomStyleActive and (seClient in StyleElements) then
          Font.Color := LStyle.GetStyleFontColor(StyleFontColor[False])
        else
          Font.Color := clGrayText;
      end;
...
var
  I: TNavigateBtn;
begin
  if TDBNavigator.FButtonsImageCollection <> nil then
    Exit;
  TDBNavigator.FButtonsImageCollection := TImageCollection.Create(nil);
  for I := Low(BtnTypeName) to High(BtnTypeName) do
    TDBNavigator.FButtonsImageCollection.Add(BtnTypeName[I], HInstance,
      'DBN_' + BtnTypeName[I], ['', '_20X']);
end;
...
begin
  ...
  if Kind = dbnHorizontal then
  begin
    W := Max(W, Count * ScaleValue(MinBtnSize.X));
    H := Max(H, ScaleValue(MinBtnSize.Y));
    if Align = alNone then W := (W div Count) * Count;
  end
  else
  begin
    W := Max(W, ScaleValue(MinBtnSize.X));
    H := Max(H, Count * ScaleValue(MinBtnSize.Y));
...
begin
  ...
  begin
    try
      ...
      begin
        PrevReadOnly := inherited ReadOnly;
...
begin
  ...
  begin
    ...
    try
      ...
      FDataLink.Field.Text := Stream.DataString;
    finally
      Stream.Free;
    end;
  end else
