...
uses System.Math, System.Types, System.UITypes, Data.DBConsts, Vcl.VDBConsts, Vcl.Dialogs,
  System.RTLConsts, Vcl.Themes, Winapi.UxTheme;
...
function GetColumnBiDiMode(AColumn: TColumn): TBiDiMode;
var
  Grid: TCustomDBGrid;
begin
  Grid := AColumn.GetGrid;
  if Assigned(Grid) then
  begin
    if Assigned(AColumn.Field) then
    begin
      if DBUseRightToLeftBidiMode(Grid, AColumn.Field) then
        Result := bdRightToLeft
      else
        Result := bdLeftToRight;
    end
    else
      Result := Grid.BiDiMode;
  end
  else
    Result := bdLeftToRight;
end;
...
begin
  ...
  BiDiMode := GetColumnBiDiMode(Column);
end;
...
begin
  ...
  begin                       { Use ExtTextOut for solid colors }
    ...
    SetIgnoreLang := not ARightToLeft and
      (ACanvas.TextFlags and (ETO_RTLREADING or ETO_IGNORELANGUAGE) = ETO_RTLREADING);
    if SetIgnoreLang then
      ACanvas.TextFlags := ACanvas.TextFlags or ETO_IGNORELANGUAGE;
    case Alignment of
      taLeftJustify:
        Left := ARect.Left + DX;
      taRightJustify:
    ...
    if SetIgnoreLang then
      ACanvas.TextFlags := ACanvas.TextFlags and not ETO_IGNORELANGUAGE;
  end
...
procedure InitImageCollection;

  procedure LoadItem(const AName: String);
  begin
    TCustomDBGrid.FIndicatorImageCollection.Add(AName, HInstance, AName, ['', '_20X']);
  end;

begin
  if TCustomDBGrid.FIndicatorImageCollection <> nil then
    Exit;
  TCustomDBGrid.FIndicatorImageCollection := TImageCollection.Create(nil);
  LoadItem(bmArrow);
  LoadItem(bmEdit);
  LoadItem(bmInsert);
  LoadItem(bmMultiDot);
...
begin
  ...
  begin
    ...
    begin
      if (cvFont in FColumns[I].AssignedValues) and not FColumns[I].IsFontScaled then
        FColumns[I].Font.Height := MulDiv(FColumns[I].Font.Height, M, D);
      if (cvTitleFont in FColumns[I].AssignedValues) and not FColumns[I].Title.IsFontScaled then
    ...
    if FTitleFont.Handle <> Font.Handle then
    begin
...
begin
  ...
  WriteText(Canvas, Rect, 3, 2, Value, Column.Alignment,
    (GetColumnBiDiMode(Column) = bdRightToLeft) and
      ...
      try
        ...
        if FDefaultDrawing then
          WriteText(Canvas, ARect, 3, 2, Value, DrawColumn.Alignment,
            (GetColumnBiDiMode(DrawColumn) = bdRightToLeft) and
...
begin
  ...
  DoubleBuffered := not UseRightToLeftAlignment and (FDatalink.Dataset <> nil) and FDatalink.Dataset.ObjectView;
...
class destructor TDBGrid.Destroy;
begin
  TCustomStyleEngine.UnRegisterStyleHook(TDBGrid, TScrollingStyleHook);
end;
