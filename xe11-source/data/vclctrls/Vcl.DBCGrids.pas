...
uses System.SysUtils, Winapi.Windows, Winapi.Messages, System.Classes, Vcl.Controls, Vcl.Forms,
  Vcl.Graphics, Vcl.Menus, Data.DB;
...
uses
  Data.DBConsts, Vcl.VDBConsts, Vcl.Themes, Vcl.GraphUtil, System.Types;
...
begin
  if FDBCtrlGrid.FDataLink.Active then
  begin
    Selected := (FDBCtrlGrid.FDataLink.ActiveRecord = FDBCtrlGrid.FPanelIndex);
    FDBCtrlGrid.DrawPanelBackground(DC, Self.ClientRect, True, Selected);
    FDBCtrlGrid.FCanvas.Handle := DC;
    try
      FDBCtrlGrid.FCanvas.Font := FDBCtrlGrid.Font;
      FDBCtrlGrid.FCanvas.Brush.Style := bsSolid;
      FDBCtrlGrid.FCanvas.Brush.Color := FDBCtrlGrid.Color;
      FDBCtrlGrid.PaintPanel(FDBCtrlGrid.FDataLink.ActiveRecord);
      if FDBCtrlGrid.FShowFocus and FDBCtrlGrid.FFocused and Selected then
      begin
        R := Self.ClientRect;
        if FDBCtrlGrid.FPanelBorder = gbRaised then
          InflateRect(R, -2, -2);
        if IsCustomStyleActive and (seClient in FDBCtrlGrid.StyleElements) then
        begin
          LStyle := StyleServices(Self);
          FDBCtrlGrid.FCanvas.Brush.Color := LStyle.GetSystemColor(clBtnFace);
          FDBCtrlGrid.FCanvas.Font.Color := LStyle.GetSystemColor(clBtnText);
          WinApi.Windows.DrawFocusRect(FDBCtrlGrid.FCanvas.Handle, R);
        end
        else
        begin
          FDBCtrlGrid.FCanvas.Brush.Color := FDBCtrlGrid.Color;
          FDBCtrlGrid.FCanvas.DrawFocusRect(R);
        end;
      end;
    finally
      FDBCtrlGrid.FCanvas.Handle := 0;
    end;
  end
  else
    FDBCtrlGrid.DrawPanelBackground(DC, Self.ClientRect, True, csDesigning in ComponentState);
end;
...
class constructor TDBCtrlGrid.Create;
begin
...
class destructor TDBCtrlGrid.Destroy;
begin
  TCustomStyleEngine.UnRegisterStyleHook(TDBCtrlGrid, TScrollingStyleHook);
end;
...
begin
  ...
  begin
    ...
    case Key of
      ...
      gkHome: FDataLink.DataSet.First;
      gkEnd: FDataLink.DataSet.Last;
      gkInsert:
        if FAllowInsert and FDataLink.DataSet.CanModify then
        begin
      ...
      gkDelete:
        if FAllowDelete and FDataLink.DataSet.CanModify then
        begin
...
begin
  ...
  if Erase then
  begin
    if Selected and not (IsCustomStyleActive and (seClient in StyleElements)) then
      FPanel.Color := FSelectedColor
    else
    begin
      if IsCustomStyleActive and (seClient in StyleElements) then
        FPanel.Color := LStyle.GetStyleColor(scPanel)
      else
        FPanel.Color := Color;
    end;
    Brush := CreateSolidBrush(ColorToRGB(FPanel.Color));
    FillRect(DC, R, Brush);
    if IsCustomStyleActive and (seClient in StyleElements) and Selected then
    begin
      ...
      try
        ...
        Canvas.Pen.Color := LStyle.GetSystemColor(clHighLight);
        Canvas.MoveTo(LRect.Right - 1, LRect.Top);
        Canvas.LineTo(LRect.Left, LRect.Top);
        Canvas.LineTo(LRect.Left, LRect.Bottom - 1);
        Canvas.LineTo(LRect.Right - 1, LRect.Bottom - 1);
        Canvas.LineTo(LRect.Right - 1, LRect.Top);
      finally
        Canvas.Handle := 0;
        Canvas.Free;
      end;
  ...
  begin
    ...
    begin
      ...
      Canvas.Handle := DC;
      try
        TopColor := LStyle.GetSystemColor(clBtnHighLight);
        BottomColor := LStyle.GetSystemColor(clBtnShadow);
...
begin
  ...
  begin
    ...
    begin
      if IsCustomStyleActive and (seClient in StyleElements) then
        Brush := CreateSolidBrush(ColorToRGB(StyleServices(Self).GetStyleColor(scPanel)))
      else
...
begin
  Result := (FPanelCount > 0) and GetPanelBounds(FPanelIndex).Contains(
    SmallPointToPoint(P));
end;
...
begin
  ...
  begin
    ...
    for I := 0 to FPanelCount - 1 do
      if (I <> FPanelIndex) and GetPanelBounds(I).Contains(P) then
      begin
        FClicking := True;
        try
          SetPanelIndex(I);
        ...
        begin
          Winapi.Windows.ScreenToClient(Window, P);
