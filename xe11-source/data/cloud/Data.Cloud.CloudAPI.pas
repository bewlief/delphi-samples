...
var
  ...
  LReaded: LongInt;
begin
  SetLength(LBytes, CBuffSize);
  Content.Position := 0;
  Hash := THashSHA2.Create;
  try
    while True do
    begin
      LReaded := Content.Read(LBytes[0], Length(LBytes));
      if LReaded = 0 then
        Break;
      Hash.Update(LBytes, LReaded);
    end;
  finally
    Content.Position := 0;
  end;
...
begin
  ...
  // Cloud services (AWS, Azure) normally require minimum TLS1.0, but suggest to use TLS1.2
  if SameText(ConnectionInfo.Protocol, 'https') then
    Result.Client.SecureProtocols := [THTTPSecureProtocol.TLS11, THTTPSecureProtocol.TLS12];
