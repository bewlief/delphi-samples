...
{ Dataset and Cursor Properties }
  ...
  DSProp = (
    ...
    dspropUPDATESPENDING            { r  LongBool,   Dataset has changes }
  );
...
type
  ...
  IDSBase = interface(IUnknown)
    ...
    function Create(                { Create empty dataset }
        iFields  : Cardinal;        { Number of fields }
    ...
    function GetOptParameter(       { Returns optional parameter (unknown to dataset) }
        iNo      : Cardinal;        { Number 1..iOptAttr }
        iFldNo   : Cardinal;        { 0 if not field attribute }
    var ppName   : Pointer;         { returns ptr to name }
    var piType   : Cardinal;        { returns type }
    var piLen    : Cardinal;        { returns length }
    ...
    function PutBlank(              { Put blank value }
        ...
        iRecNo       : Cardinal;    { iRecNo }
        iFldNo       : Cardinal;
        iBlankValue  : Cardinal
    ): DBResult; stdcall;
    ...
    function GetEmbeddedDS(
        iFieldID  : Cardinal;       { FieldID of embedded table (0 : get the first one) }
    out DsDet     : IDSBase         { Returns the ds of the embedded table }
    ): DBResult; stdcall;

    function MakeEmbedded(
        DsDet             : IDSBase;         { Embed this dataset }
        iFieldsLink       : Cardinal;
        piFieldsM         : PCardinal;       { Fields in Master }
        piFieldsD         : PCardinal;       { Fields in Detail }
    ...
    function RefreshRecords(            { Refresh specific records }
        ...
        iRecNo          : Cardinal;     { Refresh this specific record (0 if more than one.Unique key req.) }
        iClientData     : NativeUInt;
        pfReconcile     : pfDSReconcile { (NULL) Callback for resolving conflicts }
    ): DBResult; stdcall;

    function ReleaseBlobs(             { Release all uncommitted blobs }
        iBlobId  : Cardinal            { 0: all uncommitted blobs, otherwise : specific blob }
    ): DBResult; stdcall;

    function Clone(                     { Clones the structure of the dsbase, including details if any }
         iPType : Cardinal;             { 0:normal-ds, 1:delta-ds, 2:error-ds }
...
{ IDSCursor }
  ...
  IDSCursor = interface(IUnknown)
    ...
    function MoveToSeqNo(i: Cardinal): DBResult; stdcall;
    ...
    function GetBlobLen(            { Return length of blob }
        pRecBuf   : Pointer;
        iFieldNo  : Cardinal;
    var iLength   : Cardinal
    ): DBResult; stdcall;

    function GetBlob(               { Return blob }
        pRecBuf   : Pointer;
        iFieldNo  : Cardinal;
        iOffSet   : Cardinal;       { Starting position }
        pBuf      : Pointer;
    var iLength   : Cardinal        { No of bytes to be read/ returns number read }
    ): DBResult; stdcall;
    ...
    function AddFilter(             { Add a canexpr-filter to this cursor }
        ...
        iLen      : Cardinal;       { Length of canexpr }
    ...
    function AddAggregate(
        iFlds     : Cardinal;       { Defines grouping  (0 if global) }
        iCanLen   : Cardinal;       { Length of canexpr (0 if grouping only) }
...
{ IDSWriter }
  ...
  IDSWriter = interface(IUnknown)
    ...
    function GetColumnCount(var Count: Cardinal): DBResult; stdcall;
   ...
   function GetXMLMode: Cardinal; stdcall;
...
type
  // IFOPT is here because CreateInstance contains an untyped argument and
  // M+ will produce error "Type '<void>' has no type info.". Anyway RTTI
  // here is unnecessary because it is in the implementation section.
{$IFOPT M+}
  {$M-}
  {$DEFINE M_ON}
{$ENDIF}
...
{$IF (DEFINED(IOS) or DEFINED(ANDROID)) and DEFINED(CPUARM))}
function DllGetDataSnapClassObject(const [REF] CLSID, [REF] IID: TGUID; var Obj): HResult; cdecl;
  external 'libmidas.a' name 'DllGetDataSnapClassObject' dependency LibCpp
  {$IF DECLARED(LibCPP_ABI)}, LibCPP_ABI {$ENDIF};
{$ENDIF (IOS or ANDROID) and CPUARM}
...
begin
  case Status of
    ...
    ERRCODE_UNABLETOLOADICU: raise Exception.Create(SUnableToLoadICU);
  end;
end;
