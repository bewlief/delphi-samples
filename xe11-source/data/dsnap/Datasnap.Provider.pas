...
begin
  ...
  begin
    ...
    begin
      ...
      if Length(FBuffer) < Info.Size + 1 then
...
{$IFNDEF NEXTGEN}
    if TStringField(Info.Field).Transliterate then
      LSize := Info.DataSet.Translate(PAnsiChar(FBuffer), PAnsiChar(FBuffer), False)
    else
      LSize := StrLen(PAnsiChar(FBuffer));
{$ELSE}
    LSize := TMarshal.ReadStringAsAnsi(TPtrWrapper.Create(FBuffer)).Length;
{$ENDIF !NEXTGEN}
    Check(DSWriter.PutField(fldIsChanged, LSize, PByte(FBuffer)));
  end
  else
    Check(DSWriter.PutField(fldIsNull, 0, nil));
end;
...
begin
  ...
  begin
    LSize := PWord(FBuffer)^;
    Check(DSWriter.PutField(fldIsChanged, LSize, @FBuffer[SizeOf(Word)]))
  end
  else
    Check(DSWriter.PutField(fldIsNull, 0, nil));
end;
...
var
  Value: NativeUInt;
begin
  Check(DSBase.GetProp(dspropDONTINCLMETADATA, @Value));
  Result := Value <> 0;
end;
...
begin
  ...
  begin
    ...
    FDataDS.LogChanges := True;
    if FDataDS.DSBase.GetProps(rDSProps) = DBERR_NONE then
      PrevOptParams := rDSProps.iOptParams
    else
      PrevOptParams := -1;
    ...
    else if (PrevOptParams <> -1) and
            (FDataDS.DSBase.GetProps(rDSProps) = DBERR_NONE) and
            (rDSProps.iOptParams <> PrevOptParams) then
      Data := FDataDS.Data;
