...
uses
  System.VarUtils, System.Variants, Winapi.Windows, Winapi.Messages, System.Classes, System.SysUtils, Datasnap.Win.MConnect,
  System.Win.ScktComp, Winapi.WinSock, Winapi.WinInet, System.Win.ComObj;
...
uses
  Winapi.ActiveX, Datasnap.MidConst, System.RTLConsts, System.Generics.Collections, System.Types;
...
procedure GetDataBrokerList(List: TStringList; const RegCheck: string);
  ...
  begin
    if Winapi.Windows.RegOpenKeyEx(Key, PChar(SubKey), 0, KEY_READ, Result) <> 0 then Result := 0;
  end;
  ...
  begin
    Result := False;
    if Winapi.Windows.RegEnumKey(Key, Index, Buffer, SizeOf(Buffer)) = 0 then
    begin
      Value := Buffer;
      Result := True;
    end;
  ...
  begin
    ...
    if Winapi.Windows.RegQueryValue(Key, PChar(SubKey), Buffer, BufSize) = 0 then
    begin
      Value := Buffer;
      Result := True;
    end;
...
begin
  ...
  if DataLen < MINDATAPACKETSIZE then
    raise Exception.CreateRes(@SInvalidDataPacket);
  Sig := Integer(P^);
  P := Pointer(IntPtr(Data) + SizeOf(Sig));
  CheckSignature(Sig);
  Signature := Sig;
  Result := Integer(P^);
  P := Pointer(IntPtr(P) + SizeOf(Result));
  if CheckLen then
  begin
...
const
  ...
  VariantSize: array[0..varUInt64] of Word  = (0, 0, SizeOf(SmallInt), SizeOf(Integer),
    ...
    SizeOf(Word), SizeOf(LongWord), SizeOf(Int64), SizeOf(UInt64));
...
begin
  ...
  for I := 0 to PDispParams(@Params).cArgs - 1 do
    if PDispParams(@Params)^.rgvarg^[I].vt and varByRef = varByRef then
    begin
      if PDispParams(@Params)^.rgvarg^[I].vt = (varByRef or varVariant) then
        PDest := @TVarData(TVarData(PDispParams(@Params)^.rgvarg^[I]).VPointer^)
      else
        PDest := @TVarData(PDispParams(@Params)^.rgvarg^[I]);
...
procedure TTransportThread.Execute;
  ...
  begin
    ...
    PostMessage(FParentHandle, THREAD_EXCEPTION, 0, LPARAM(Pointer(SendException)));
  end;
        ...
        try
          case MsgWaitForMultipleObjects(1, Event, False, INFINITE, QS_ALLINPUT) of
            ...
            begin
              ...
              begin
                Data._AddRef;
                PostMessage(FParentHandle, THREAD_RECEIVEDSTREAM, 0, LPARAM(Pointer(Data)));
                Data := nil;
              end;
            end;
            WAIT_OBJECT_0 + 1:
...
function TSocketTransport.Receive(WaitForInput: Boolean; Context: NativeInt): IDataBlock;
var
...
begin
  ...
  if (FEvent <> 0) and (StreamLen > 0) then
...
function TSocketTransport.Send(const Data: IDataBlock): NativeInt;
var
  P: Pointer;
begin
  Result := 0;
...
function TWebConnection.Receive(WaitForInput: Boolean; Context: NativeInt): IDataBlock;
const
...
function TWebConnection.Send(const Data: IDataBlock): NativeInt;
var
  Request: HINTERNET;
  RetVal, Flags: DWord;
  P: Pointer;
...
begin
  ...
  Request := HttpOpenRequest(FInetConnect, 'POST', PChar(FURLSite), nil,
    nil, Pointer(AcceptTypes), Flags, DWORD_PTR(Self));
...
//  I: integer;
begin
{ // QC 2297 Adding this code is causing Free to be called on the server twice
