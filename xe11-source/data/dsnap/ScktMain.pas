...
{*******************************************************}
{           CodeGear Socket Server source code          }
{*******************************************************}
...
uses
  Vcl.SvcMgr, Winapi.Windows, Winapi.Messages, System.SysUtils, System.Classes, Vcl.Graphics, Vcl.Controls, Vcl.Forms,
  Vcl.Dialogs, Vcl.Menus, Winapi.ShellAPI, Vcl.ExtCtrls, Vcl.StdCtrls, Vcl.ComCtrls, System.Win.ScktComp, System.Win.Registry,
...
uses ScktCnst, Datasnap.Win.SConnect, Winapi.ActiveX, Datasnap.MidConst, System.UITypes;
...
begin
  ...
  if Selected then
  begin
    PortUpDown.Position := TSocketDispatcher(PortList.Items.Objects[FCurItem]).Port;
    ThreadUpDown.Position := TSocketDispatcher(PortList.Items.Objects[FCurItem]).ThreadCacheSize;
    Self.InterceptGUID.Text := TSocketDispatcher(PortList.Items.Objects[FCurItem]).FInterceptGUID;
    TimeoutUpDown.Position := TSocketDispatcher(PortList.Items.Objects[FCurItem]).Timeout;
    ClearModifications;
  end;
...
procedure TSocketForm.Initialize(FromService: Boolean);
  ...
  begin
    Result := False;
    if RegOpenKeyEx(HKEY_LOCAL_MACHINE, KEY_IE, 0, KEY_READ, RegKey) = ERROR_SUCCESS then
    try
...
begin
  if not NT351 then
  begin
    FIconData.cbSize := System.SizeOf(FIconData);
    FIconData.Wnd := Self.Handle;
    FIconData.uID := $DEDB;
    FIconData.uFlags := NIF_MESSAGE or NIF_ICON or NIF_TIP;
    FIconData.hIcon := Vcl.Forms.Application.Icon.Handle;
    FIconData.uCallbackMessage := WM_MIDASICON;
...
begin
  if MessageDlg(SQueryDisconnect, mtConfirmation, [mbYes, mbNo], 0) = mrNo then
    Exit;
  SelectedSocket.Socket.Lock;
  try
    for I := 0 to ConnectionList.Items.Count - 1 do
      if ConnectionList.Items[I].Selected then
        TServerClientThread(SelectedSocket.Socket.Data).ClientSocket.Close;
  finally
    SelectedSocket.Socket.Unlock;
  end;
end;
...
//  begin
    if TSocketDispatcher(SelectedSocket).Socket.ActiveConnections > 0 then
      if MessageDlg(SErrChangeSettings, mtConfirmation, [mbYes, mbNo], 0) = idNo then
        Exit;
    TSocketDispatcher(SelectedSocket).Close;
    TSocketDispatcher(SelectedSocket).Port := StrToInt(PortNo.Text);
    PortList.Items[ItemIndex] := PortNo.Text;
    TSocketDispatcher(SelectedSocket).ThreadCacheSize := StrToInt(ThreadSize.Text);
    TSocketDispatcher(SelectedSocket).InterceptGUID := Self.InterceptGUID.Text;
    TSocketDispatcher(SelectedSocket).Timeout := StrToInt(Self.Timeout.Text);
    TSocketDispatcher(SelectedSocket).Open;
//  end;
  ClearModifications;
end;
