...
{$ENDIF !NEXTGEN}
    ...
    procedure SetIncludeBlobsInDelta(Value: Boolean);
  protected
    { IProviderSupport }
...
{$ENDIF NEXTGEN}
    ...
    procedure ClearCalcFields(Buffer: TRecBuf); overload; override;
{$IFNDEF NEXTGEN}
    procedure ClearCalcFields(Buffer: TRecordBuffer); overload; override; deprecated 'Use overloaded method instead';
{$ENDIF !NEXTGEN}
...
{$ENDIF !NEXTGEN}
    ...
    property IncludeBlobsInDelta: Boolean read FIncludeBlobsInDelta write SetIncludeBlobsInDelta default False;
    [Stored('FStoreDefs')]
...
procedure TCustomClientDataSet.SetIncludeBlobsInDelta(Value: Boolean);
begin
  CheckInactive;
  FIncludeBlobsInDelta := Value;
end;
...
var
  ParamType, ParamLen, TypeOut, LenOut: Cardinal;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        ParamLen := Length(TEncoding.UTF8.GetBytes(String(Value))) + 1;
      end
...
begin
  ...
  begin
    ...
    if ReadOnly then Check(FDSBase.SetProp(dspropREADONLY, NativeUInt(True)));
    if IncludeBlobsInDelta then Check(FDSBase.SetProp(dspropINCLBLOBSINDELTA, MaxInt));
...
begin
  ...
  if not RootDataset.UpdatesPending then
    Result := 0
  else
...
begin
  ...
  if (FieldDesc.iFldSubType = 0) and (faFixed in Attributes) then
    FieldDesc.iFldSubType := fldstFIXED;
...
begin
  Result := not UpdatesPending and HasAppServer;
end;
...
procedure TCustomClientDataSet.ClearCalcFields(Buffer: TRecBuf);
begin
  ...
  case Status of
    DBERR_NONE:
      begin
        ...
        FDSCursor.GetSequenceNumber(Cardinal(PRecInfo(PByte(Buffer) + FRecInfoOfs)^.RecordNumber));
...
function TCustomClientDataSet.GetUpdatesPending: Boolean;
var
  UpdatesPending: NativeUInt;
begin
  Result := False;
  if Active then
  begin
    Check(FDSBase.GetProp(dspropUPDATESPENDING, @UpdatesPending));
    Result := Boolean(UpdatesPending);
  end;
end;
...
begin
  ...
  if Length(MarshaledAString(@FieldDesc.szName[0])) = SizeOf(MIDASNAME) - 1 then
  begin
  ...
  case FieldDesc.iFldType of
    ...
    fldBLOB:
      begin
        if FieldDesc.iFldSubType = fldstWIDEMEMO then
          LSize := FieldDesc.iUnits1 div 2
        else
          LSize := FieldDesc.iUnits1;
    ...
    fldDATETIME:
      if FieldDesc.iFldSubType = fldstORATIMESTAMP then LType := ftOraTimeStamp;
  end;
...
begin
  ...
  begin
    ...
    if (State = dsSetKey) and (Field.FieldNo > 0) then
      for I := 0 to FIndexFieldCount - 1 do
        if FIndexFieldMap[I] = Field.FieldNo then
        begin
          if I + 1 > FKeyBuffer^.FieldCount then
            FKeyBuffer^.FieldCount := I + 1;
          Break;
        end;
  end
  else
...
function TCustomClientDataSet.GetModifiedBlobCached: Boolean;
begin
  Result := False;
end;
...
procedure TCustomClientDataSet.GetDetailLinkFields(MasterFields, DetailFields: TList<TField>);
var
  Fields: String;
  I: Integer;
begin
  ...
  if (MasterSource <> nil) and (MasterSource.DataSet <> nil) then begin
    Fields := FMasterLink.FieldNames;
    if Fields <> '' then
      MasterSource.DataSet.GetFieldList(MasterFields, Fields);
    Fields := '';
    for I := 0 to FMasterLink.Fields.Count - 1 do
    begin
      if Fields <> '' then
        Fields := Fields + ';';
      Fields := Fields + IndexFields[I].FieldName;
    end;
    if Fields <> '' then
      GetFieldList(DetailFields, Fields);
  end;
end;
...
begin
  ...
  try
    Buff := Memory;
    if (Buff = nil) and (Size = 0) and FWritten then
      Buff := Self;
    FDataSet.Check(FDataSet.FDSCursor.PutBlob(Pointer(FBuffer), FFieldNo, 0, Buff, Size));
...
function TClientBlobStream.Write(const ABuffer: TBytes; AOffset, ACount: Longint): Longint;
begin
