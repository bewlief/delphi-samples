...
begin
  ...
  begin
    if (Index >= FSparseFields) or (Index < 0) then
...
begin
  ...
  if FValidating and not (FDataSet.State in [dsOldValue, dsCurValue]) then
  begin
...
begin
  CheckInactive;
  if AnsiCompareText(Value, FieldName) <> 0 then
  begin
    if FFields <> nil then
    begin
...
begin
  ...
  if Result then
    Value := TMarshal.ReadStringAsUnicode(TPtrWrapper.Create(FIOBuffer));
end;
