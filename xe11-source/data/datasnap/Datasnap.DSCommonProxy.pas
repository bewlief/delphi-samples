...
type
  ...
  protected
    ...
    function GetParameterDBXSubDataType: Integer;
    procedure SetParameterDBXSubDataType(const DbxSubDataType: Integer);
  ...
  public
    ...
    property ParameterDBXSubDataType: Integer read GetParameterDBXSubDataType write SetParameterDBXSubDataType;
...
begin
  ...
  DbxDataType.Size := 4;
  DbxSubDataType := TDBXValueType.Create;
  DbxSubDataType.Name := TDSMethodParametersColumns.DbxSubDataType;
  DbxSubDataType.DisplayName := TDSMethodParametersColumns.DbxSubDataType;
  DbxSubDataType.DataType := TDBXDataTypes.Int32Type;
  DbxSubDataType.Size := 4;
...
function TDSMethodParametersEntity.GetParameterDBXSubDataType: Integer;
var
  Value: TDBXValue;
begin
  try
    Value := Table.Value[Table.GetOrdinal(TDSMethodParametersColumns.DbxSubDataType)];
    if Value.IsNull then
      Result := TDBXDataTypes.UnknownType
    else
      Result := Value.AsInt32;
  except
    // DbxSubDataType does not exist in XE9 and earlier
    Result := TDBXDataTypes.UnknownType
  end;
end;
...
procedure TDSMethodParametersEntity.SetParameterDBXSubDataType(const DbxSubDataType: Integer);
begin
  Table.Value[Table.GetOrdinal(TDSMethodParametersColumns.DbxSubDataType)].AsInt32 := DbxSubDataType;
end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          ProxyParam.SubDataType := MethodParameters.ParameterDBXSubDataType;
