...
type
  ...
  public
    ...
    /// <summary>Assign property TDSHTTP.ConnectTimeout. Get or set the timeout when making an HTTP connection </summary>
    ...
    /// <summary>Assign property TDSHTTP.ReadTimeout. Get or set the timeout when reading a HTTP response</summary>
    property ReadTimeout: Integer read GetReadTimeout write SetReadTimeout default TURLClient.DefaultResponseTimeout;
  end;
...
function EncodeURIComponent(const AStr: string; AHost: Boolean = False): string; overload;
const
  UnsafeChars: TURLEncoding.TUnsafeChars =
    [Ord('|'), Ord('`'), Ord('{'), Ord('}'), Ord('^'),
     ...
     Ord('%'), Ord(':')];
var
  LUnsafe: TURLEncoding.TUnsafeChars;
begin
  LUnsafe := UnsafeChars;
  if not AHost then
    Include(LUnsafe, Ord('.'));
  Result := TNetEncoding.URL.Encode(AStr, LUnsafe, [TURLEncoding.TEncodeOption.EncodePercent]);
end;
...
begin
  ...
  LUrl := LProtocol + '://' + EncodeURIComponent(LHost, True) + LPortString + LPathPrefix + '/' + LContext +
...
begin
  ...
  begin
    ...
    TDSCustomRestConnection(Dest).SecureProtocols := SecureProtocols;
    TDSCustomRestConnection(Dest).ConnectTimeout := ConnectTimeout;
    TDSCustomRestConnection(Dest).ReadTimeout := ReadTimeout;
  end
  else
    inherited;
end;
...
begin
  ...
  FConnectTimeout := TURLClient.DefaultConnectionTimeout;
  FReadTimeout := TURLClient.DefaultResponseTimeout;
end;
...
begin
  if FHTTP = nil then
  begin
    FHTTP := TDSHTTP.Create; // call virtual constructor
    ...
    FHTTP.SecureProtocols := FSecureProtocols;
    FHTTP.ConnectTimeout := FConnectTimeout;
    FHTTP.ReadTimeout := FReadTimeout;
...
function TDSCustomRestConnection.GetSecureProtocols: THTTPSecureProtocols;
begin
  if FHTTP <> nil then
...
procedure TDSCustomRestConnection.SetSecureProtocols(const Value: THTTPSecureProtocols);
begin
  FSecureProtocols := Value;
  if FHTTP <> nil then
    FHTTP.SecureProtocols := Value;
end;
...
function TDSCustomRestConnection.GetConnectTimeout: Integer;
begin
  if FHTTP <> nil then
...
function TDSCustomRestConnection.GetReadTimeout: Integer;
begin
  if FHTTP <> nil then
...
procedure TDSCustomRestConnection.SetConnectTimeout(const Value: Integer);
begin
...
procedure TDSCustomRestConnection.SetReadTimeout(const Value: Integer);
begin
  FReadTimeout := Value;
  if FHTTP <> nil then
  ...
  try
    ...
    begin
      ...
      try
        ...
        if (LJSONValue <> nil) and (LJSONValue is TJSONObject) and (TJSONObject(LJSONValue).Get('SessionExpired') <> nil) then
    ...
    begin
      LJSONValue := TJSONObject.ParseJSONValue(LErrorMessage);
      try
        if (LJSONValue <> nil) and (LJSONValue is TJSONObject) then
        begin
          LJSONError := TJSONObject(LJSONValue).GetValue('error');
          if LJSONError = nil then
            LJSONError := TJSONObject(LJSONValue).GetValue('SessionExpired');
          if (LJSONError <> nil) and (LJSONError.Value <> '') then
            LResponseText := LResponseText + ':' + sLineBreak + LJSONError.Value;
        end;
      finally
        FreeAndNil(LJSONValue);
      end;
...
procedure TDSRestRequest.DoRequest(var response: string; out responseCode: Integer;
  out errorMessage: string; ACallBack: TProc<TDSHTTP>);
var
  HTTP: TDSHTTP;
  LPragma: string;
  LPos: Integer;
