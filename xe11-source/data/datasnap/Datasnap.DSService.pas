...
type
  ...
  ///  <summary> Event that returns the name of the Datasnap Server method given
  ///    the actual context.
  ///  </summary>
  ///  <param name="Sender">Event owner</param>
  ///  <param name="RequestType">REST method name as present in the request</param>
  ///  <param name="ClassName">DS class name as present in the request</param>
  ///  <param name="MethodName">DS method name, as present in the request</param>
  ///  <param name="DSMethodName">DS method name used to find and execute</param>
  ...
  private
    ...
    procedure ProcessException(const AResponseHandler: TRequestCommandHandler;
      const AExc: Exception);
  protected
  ...
  public
    ...
    ///  <param name="NameMapEvent">optional event that returns the name of the Datasnap Server method given the actual context.</param>
    ...
    procedure ProcessGETRequest(const Request: string; Params: TStrings; Content: TArray<Byte>;
      const NameMapEvent: TDSRESTMethodNameMapEvent; ResponseHandler: TRequestCommandHandler); virtual; abstract;
  ...
  private
    ...
    procedure SetMethodNameWithPrefix(const RequestType, ClassName, MethodName: string;
      const NameMapEvent: TDSRESTMethodNameMapEvent; out DSMethodName: string);
  ...
  public
    ...
    procedure ProcessGETRequest(const Request: string; Params: TStrings; Content: TArray<Byte>;
      const NameMapEvent: TDSRESTMethodNameMapEvent; ResponseHandler: TRequestCommandHandler); override;
    procedure ProcessPUTRequest(const Request: string; Params: TStrings; Content: TArray<Byte>;
      const NameMapEvent: TDSRESTMethodNameMapEvent; ResponseHandler: TRequestCommandHandler); override;
    procedure ProcessPOSTRequest(const Request: string; Params: TStrings; Content: TArray<Byte>;
      const NameMapEvent: TDSRESTMethodNameMapEvent; ResponseHandler: TRequestCommandHandler); override;
...
procedure TDSService.ProcessException(const AResponseHandler: TRequestCommandHandler; const AExc: Exception);
var
  iCode: Integer;
begin
  if AExc is TDBXError then
  begin
    iCode := -1;
    case TDBXError(AExc).ErrorCode of
    TDBXErrorCodes.AuthorizationFailed: iCode := 403;
    end;
    if (iCode <> -1) and (GetInvocationMetadata.ResponseCode = 0) then
      GetInvocationMetadata.ResponseCode := iCode;
  end;
  AResponseHandler.AddCommandError(AExc.Message);
end;
...
begin
  ...
  FStreamAsJSON := True;    // Binary stream response represented by JSON array
end;
...
begin
  ...
  if Assigned(NameMapEvent) then
    NameMapEvent(Self, RequestType, ClassName, MethodName, DSMethodName);
end;
...
begin
  if (Content <> nil) and (Length(Content) > 1) and
    ...
    ProcessREST('POST', Request, BytesOf(Params.Values[Params.Names[0]]), NameMapEvent, ResponseHandler)
  else
    ProcessREST('POST', Request, Content, NameMapEvent, ResponseHandler);
end;

procedure TDSRESTService.ProcessPUTRequest(const Request: string; Params: TStrings; Content: TArray<Byte>;
  const NameMapEvent: TDSRESTMethodNameMapEvent; ResponseHandler: TRequestCommandHandler);
begin
  ProcessREST('PUT', Request, Content, NameMapEvent, ResponseHandler);
end;

procedure TDSRESTService.ProcessDELETERequest(const Request: string; Params: TStrings; Content: TArray<Byte>;
  const NameMapEvent: TDSRESTMethodNameMapEvent; ResponseHandler: TRequestCommandHandler);
begin
  ProcessREST('DELETE', Request, nil, NameMapEvent, ResponseHandler);
end;
...
procedure TDSJSONService.ProcessPOSTRequest(const Request: string; Params: TStrings; Content: TArray<Byte>;
  const NameMapEvent: TDSRESTMethodNameMapEvent; ResponseHandler: TRequestCommandHandler);
begin
  if (Content <> nil) and (Length(Content) > 0) then
...
procedure TDSJSONService.ProcessGETRequest(const Request: string; Params: TStrings; Content: TArray<Byte>;
  const NameMapEvent: TDSRESTMethodNameMapEvent; ResponseHandler: TRequestCommandHandler);
begin
  raise TDSServiceException.Create(Format(SPOSTExpected, ['GET']));
end;

procedure TDSJSONService.ProcessPUTRequest(const Request: string; Params: TStrings; Content: TArray<Byte>;
  const NameMapEvent: TDSRESTMethodNameMapEvent; ResponseHandler: TRequestCommandHandler);
begin
  raise TDSServiceException.Create(Format(SPOSTExpected, ['PUT']));
end;

procedure TDSJSONService.ProcessDELETERequest(const Request: string; Params: TStrings; Content: TArray<Byte>;
  const NameMapEvent: TDSRESTMethodNameMapEvent; ResponseHandler: TRequestCommandHandler);
begin
