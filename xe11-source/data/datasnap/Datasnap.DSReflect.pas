...
{$ENDIF}
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                SetAllocatedObject(I, LStream, True);
              end
              else
                FMethodValues[I] := TValue.Empty;
            end
            ...
            begin
              ...
              LStream := TDBXJSONStream.Create(StreamValue, not FServerSideCommand);
        ...
        begin
          LVariant := ParameterValue.AsVariant;
          if VarIsByRef(LVariant) or ((TVarData(LVariant).VType and varArray) = varArray) then
            FMethodValues[I] := TValue.From<Variant>(LVariant)
          else
            FMethodValues[I] := TValue.FromVariant(LVariant);
        end;
            ...
            begin
              ...
              begin
                ...
                begin
                  ...
                  JSONValue := ParameterValue.GetJSONValue;
              ...
              begin
                if not FServerSideCommand then
                  ...
                  JSONValue := ParameterValue.GetJSONValue;
                SetAllocatedObject(I, JSONValue, not FServerSideCommand);
...
begin
  ...
  begin
      if (FAllocatedObjects[Ordinal].Key <> AllocatedObject) and
         (not FServerSideCommand or FAllocatedObjects[Ordinal].Value) then
...
begin
  ...
  RMethod := RType.GetMethod(FMethodInfoHeader.NameFld.ToString);

  // correct var, out parameters
  RParams := RMethod.GetParameters;
  for I := 0 to Min(Length(RParams), Length(Params)) - 1 do
  begin
    RParam := RParams[I];
    if (RParam.Flags * [pfVar, pfReference, pfOut] <> []) then
    begin
      if RParam.ParamType.Handle <> Params[I].TypeInfo then
      begin
        Params[I] := Params[I].Cast(RParam.ParamType.Handle);
      end;
    end;
  end;

  MethodValues.ReturnValue := RMethod.Invoke(MethodInstance, Params);
end;
...
begin
  ...
  if ClassName = 'UnicodeString' then
    ...
    FClassName := 'Int64'
  else if ClassName = 'unsigned __int64' then
    FClassName := 'UInt64'
    ...
    FClassName := 'Integer'
  else if ClassName = 'unsigned int' then
    FClassName := 'Cardinal'
...
begin
  ...
  begin
    ...
    Result := TDBXDataTypes.Int32Type
  else if SameText(FClassName, 'Cardinal') then
...
//    Result := TDBXDataTypes.TimeStampOffsetType
    ...
    Result := TDBXDataTypes.Int64Type
  else if SameText(FClassName, 'UInt64') then
