...
type
  ...
  private
    [weak] FServerCon: TDSServerConnectionHandler;
  ...
  private
    ...
    [weak] FServerConnectionHandler: TDSServerConnectionHandler;
  public
    property DbxContext: TDBXContext read FDbxContext;
  ...
  private
    ...
    [weak] FServerCon: TDSServerConnection;
  ...
  public
    ...
    property BufferKBSize: Integer read FBufferKBSize write SetBufferKBSize default DefBufferKBSize;
...
begin
  Result := FServerCon.ClearResources;
end;
...
begin
  FMethodAlias := MethodAlias;
  FSearchName := UpperCase(MethodAlias);
end;
...
begin
  if FUserName <> '' then
    ...
    if (ServerConnectionHandler <> nil) and (ServerConnectionHandler.ConProperties <> nil) then
      Result := ServerConnectionHandler.ConProperties.Values[TDBXPropertyNames.DSAuthenticationUser]
    else
      Result := '';
end;
...
begin
  ...
  try
    try
      ...
      on E: Exception do
      begin
        Error := TDBXErrorMessage.Create;
        Error.Error := E;
        Error.HandleMessage(FConHandler);
      end;
    end;
  finally
    Error.Free;
...
begin
  ...
  if Message.Error <> FErrorEventObject.Error then
    Message.Error := FErrorEventObject.Error;
  if Message.ErrorMessage <> FErrorEventObject.Error.Message then
    Message.ErrorMessage := FErrorEventObject.Error.Message;
...
begin
  Result := FMethodValues.ClearReferenceParameters;
end;
...
begin
  if BufferKBSize < 1 then
    FBufferKBSize := 1
  else if BufferKBSize > $80000 then
    FBufferKBSize := $80000
  else
    FBufferKBSize := BufferKBSize;
end;
...
begin
  ...
  begin
    ...
    TempData := FDestroyEventObject.ServerClassInstance;
    FDestroyEventObject.ServerClassInstance := nil;
    TempData.DisposeOf;
  end;
  FInstance := nil;
end;
