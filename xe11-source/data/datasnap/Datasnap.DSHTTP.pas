...
type
  ...
  private
    ...
    FOnNameMap: TDSRESTMethodNameMapEvent;
  ...
  public
    ...
    property OnNameMap: TDSRESTMethodNameMapEvent read FOnNameMap write FOnNameMap;
  end;
...
//    FSessionTimeout: Integer;
  ...
  private
    ...
    function GetOnNameMap: TDSRESTMethodNameMapEvent;
    procedure SetOnNameMap(const Value: TDSRESTMethodNameMapEvent);
  protected
  ...
  public
    ...
    property OnNameMap: TDSRESTMethodNameMapEvent read GetOnNameMap write SetOnNameMap;
  end;
  ...
  private
    ...
    function GetSSLVersion: TIPSSLVersionPeer;
    procedure SetSSLVersion(const Value: TIPSSLVersionPeer);
  published
  ...
  public
    constructor Create(AOwner: TComponent); override;
    /// <summary> HTTPS SSL/TLS version </summary>
    property SSLVersion: TIPSSLVersionPeer read GetSSLVersion write SetSSLVersion default TIPSSLVersionPeer.sslvTLSv1;
...
begin
  ...
  FResponseInfo.WWWAuthenticate.Add('Basic realm="' + Realm + '", charset="UTF-8"');
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      RawStr := RawStr + UTF8Char(CharCode);
    end;
...
constructor TDSCustomCertFiles.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  FSSLVersion := TIPSSLVersionPeer.sslvTLSv1;
end;
...
function TDSCustomCertFiles.GetSSLVersion: TIPSSLVersionPeer;
begin
  Result := FSSLVersion;
end;

procedure TDSCustomCertFiles.SetSSLVersion(const Value: TIPSSLVersionPeer);
begin
  FSSLVersion := Value;
end;
...
constructor TDSHTTPSServerIndy.Create(const ADSServer: TDSCustomServer;
...
function TDSHTTPSServerIndy.GetSSLVersion: TIPSSLVersionPeer;
begin
  if SSLOptions <> nil then
    Result := SSLOptions.Method
  else
    Result := FSSLVersion;
end;
...
begin
  ...
  begin
    ...
    LIOHandler.SSLOptions.Method := FSSLVersion;
...
procedure TDSHTTPSServerIndy.SetSSLVersion(const Value: TIPSSLVersionPeer);
begin
  if SSLOptions <> nil then
    SSLOptions.Method := Value
  else
    FSSLVersion := Value;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      try
        try
          RESTService.ProcessGETRequest(CloseTunnelRequest, nil, nil, nil, RespHandler);
        except
        end;
      finally
...
begin
  Server := nil;
  // Break ARC cycle, which is result of HTTPServer calls
  // TDSSessionManager.AddSessionEvent and keeps reference to event
  TempData := FRestServer;
  FRestServer := nil;
  TempData.DisposeOf;
...
begin
  ...
  FRESTServer.OnNameMap := FOnNameMap;
  UpdateDSServerName;
end;
...
function TCustomDSRESTServerTransport.GetOnNameMap: TDSRESTMethodNameMapEvent;
begin
  if FRESTServer <> nil then
    Result := FRESTServer.OnNameMap
  else
    Result := FOnNameMap;
end;

procedure TCustomDSRESTServerTransport.SetOnNameMap(
  const Value: TDSRESTMethodNameMapEvent);
begin
  FOnNameMap := Value;
  if FRESTServer <> nil then
    FRESTServer.OnNameMap := Value;
end;
...
begin
  ...
  begin
    ...
    ResponseStream := Parameter.Value.GetStream(False);
    if ResponseStream = nil then
