...
var
  ...
  AutomaticChannelRecovery: Boolean = False;
...
begin
  try
    ...
    if State in [ctsStarting, ctsStarted] then
    begin
...
begin
  ...
  begin
    Result.Values[TDBXPropertyNames.ConnectTimeout] := '-1';
    Result.Values[TDBXPropertyNames.CommunicationTimeout] := '-1';
  end
end;
...
begin
  if not NoTimeout then
    while (FDSChannelThread <> nil) and (FState = ctsStarting) do
      Sleep(1);
    ...
    begin
      ...
      begin
        ...
        FDSChannelThread := TDSChannelThread.Create(
          ...
          begin
            repeat
              if AutomaticChannelRecovery then
                while not FDSChannelThread.Terminated and (FState = ctsFailed) do
                  try
                    Sleep(100);
                    ExecuteRemote('DSAdmin', 'CloseClientChannel',
                      procedure (Params: TDBXParameterList) begin
                    ...
                    on E: Exception do
                      if not E.ClassNameIs('EIdSocketError') then
                      begin
                        FState := ctsStarting;
                        FStateError := EmptyStr;
                      end;
              ...
              try
                ...
                on E: Exception do
                begin
                  if AutomaticChannelRecovery and E.ClassNameIs('EIdSocketError') then
                    LDone := False
                  else
