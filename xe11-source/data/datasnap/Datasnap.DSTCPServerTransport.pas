...
begin
  Result := TDSTCPChannel.Create(AContext);
end;
...
begin
  ...
  // Break ARC cycle, which is result of ProtocolHandler calls
...
begin
  ...
  if LHandler.InputBuffer.Size = 0 then
    LHandler.CheckForDataOnSource(1000);
...
procedure TDSTCPChannel.ChannelSessionEvent(Sender: TObject;
...
begin
  ...
  // wait for data
  LHandler := FContext.Connection.IOHandler;
  while LHandler.InputBuffer.Size = 0 do
  ...
  if LHandler.InputBuffer.Size < Count then
  ...
  LHandler.ReadBytes(TIPBytesPeer(FReadBuffer), ActualReadCount, False);
