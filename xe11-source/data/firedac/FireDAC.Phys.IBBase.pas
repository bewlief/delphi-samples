...
begin
  ...
  TIBBackup(AService).SkipData := SkipData;
end;
...
begin
  ...
  FLib := TIBLibExt.Create(GetBaseDriverID, FDPhysManagerObj);
end;
...
begin
  ...
  if (AKeys <> nil) and (
    ...
    Result.Rows.Add([Unassigned, S_FD_ConnParam_IB_PageSize, '1024;2048;4096;8192;16384;32768', '4096', S_FD_ConnParam_IB_PageSize, -1]);
...
begin
  Result := TFDPhysIBMetadataExt.Create(Self, FEnv.Lib.Brand, ServerVersion,
...
begin
  ...
  if InternalGetSharedCliHandle() <> nil then begin
    ...
    FDatabase := TIBDatabaseExt.CreateUsingHandle(FEnv, pCliHandles^[0], Self);
    FSharedTxHandle := pCliHandles^[1];
  end
  else begin
    FDatabase := TIBDatabaseExt.Create(FEnv, Self);
    FSharedTxHandle := nil;
  end;
{$IFDEF FireDAC_MONITOR}
...
begin
  ...
  try
    oStmt := TIBStatementExt.Create(FDatabase, oTX, Self);
    try
  ...
  case AType of
    ...
    dtFmtBCD:
      begin
        ...
        if (APrec > 18) and (IBConnection.FEnv.Lib.Brand = ibFirebird) and (IBConnection.FEnv.Lib.Version >= ivFB040000) then
          ASQLDataType := SQL_INT128
        else
          ASQLDataType := SQL_INT64;
        ASQLPrec := APrec;
        ASQLScale := -AScale;
...
begin
  ...
  FStmt := TIBStatementExt.Create(IBConnection.FDatabase,
    TFDPhysIBTransactionBase(FTransactionObj).FTransaction, Self);
