...
{$ENDIF}
  ...
  TODBCDriverKind = (dkOther, dkSQLSrv, dkSQLNC, dkSQLOdbc, dkFreeTDS, dkSQLSrvOther,
    ...
    dkDB2_400, dkQuickBooks, dkExcel);
...
{$ENDIF}
    ...
    property SEARCH_PATTERN_ESCAPE: Char read FSearchPatternEscape;
    // Events
...
{$ENDIF}
  ...
  public
    ...
    property RowSetSupported: Boolean read FRowSetSupported write FRowSetSupported;
    // ODBC ----------
...
{$ENDIF}
  ...
  private
    ...
    FTableTypes: String;
    function EscapeSpecChars(const AStr: String): String;
  public
  ...
  if AStatus <> SQL_SUCCESS then begin
    ...
    while True do begin
      ...
      if not (oHandle.Lib.SQLGetDiagRec(oHandle.HandleType, oHandle.Handle,
        ...
        Break;
      // Oracle: workaround for bug, when iDiagMesLen is number of bytes instead of characters
      if aDiagMesBuffer[iDiagMesLen div SizeOf(SQLChar)] = #0 then
        iDiagMesLen := iDiagMesLen div SizeOf(SQLChar);
...
begin
  ...
  Resume;
  if ekVendor in AMonitor.EventKinds then begin
    Sleep(200);
    Inc(FLockOutput);
    try
...
begin
  // Informix: after connection losting releasing ENV handle mai raise exception
  IgnoreErrors := True;
  inherited Destroy;
end;
...
begin
  ...
  else if Match('ODBCJT') then
    if sDbms = 'DBASE' then
      FDriverKind := dkDBF
    else if sDbms = 'EXCEL' then
      FDriverKind := dkExcel
    else
      FDriverKind := dkMSAccessJet
  else if Match('ACEODBC') then
    if sDbms = 'DBASE' then
      FDriverKind := dkDBF
    else if sDbms = 'EXCEL' then
      FDriverKind := dkExcel
  ...
  else if Match('FQQB') then
    FDriverKind := dkQuickBooks;
  ...
  sEsc := GetInfoOptionStr(SQL_SEARCH_PATTERN_ESCAPE);
  if Length(sEsc) = 1 then
    FSearchPatternEscape := sEsc[1]
  else
    FSearchPatternEscape := #0;
end;
...
begin
  ...
  try
    ...
    if (Lib.SQLBrowseConnect(Handle, PSQLChar(AConnString), SQL_NTS,
                             ...
                             iOutLen)) <> SQL_NEED_DATA then

      Exit;
    if C_RETURNED_STRING_MAXLEN < iOutLen then begin
      SetLength(sOutStr, iOutLen + 1);
      if (Lib.SQLBrowseConnect(Handle, PSQLChar(AConnString), SQL_NTS,
                               PSQLChar(sOutStr), iOutLen + 1,
                               iOutLen)) <> SQL_NEED_DATA then
        Exit;
    end;
    ODBCSetLength(sOutStr, iOutLen);
    i1 := Pos('{', sOutStr);
    i2 := Pos('}', sOutStr, i1 + 1);
...
begin
  ...
  if (DriverKind in [dkMSAccessJet, dkMSAccessACE, dkExcel]) and (Pos(',TEXT,', Result) = 0) then
...
begin
  ...
  if ((SQLDataType = SQL_DECIMAL) or (SQLDataType = SQL_NUMERIC) or
      (SQLDataType = SQL_DECFLOAT)) and
...
begin
  if LongData and IsParameter then begin
    ...
    if SQLDataType = SQL_SS_TIME2 then begin
      ...
      if lIsNull then begin
        ...
        if Statement.StrsEmpty2Null and (iSize = 0) then
          ...
          if (iSize > ColumnSize) and not LongData then
            if Statement.StrsTrim2Len then
              iSize := ColumnSize
            else
              ErrorDataTooLarge(ColumnSize, iSize);
...
begin
  ...
  // Allocate 1 byte long buffer when blob size=0 to avoid future conversion
  // of nil buffer to NULL
  if ASize = 0 then
    ASize := 1;
  if pBuffer = nil then
    GetMem(Result^, ASize)
...
begin
  ...
  if SQLDataType = SQL_SS_VARIANT then
    ...
    FMSSQLVariantBinary := Connection.MSSQLVariantBinary and
     ...
     (SQLDataType = SQL_LONGVARGRAPHIC) then begin
    ...
    if Assigned(oConn.OnGetMaxSizes) and (FColumnSize <> MAXINT) then begin
      ...
      if (iMaxCharSize > 0) and ((FColumnSize <= 0) or (FColumnSize > iMaxCharSize)) then begin
        FColumnSize := iMaxCharSize;
        FLimitedToMaxLen := True;
      end;
      if (iMaxByteSize > 0) and ((FDataSize <= 0) or (FDataSize > iMaxByteSize)) then begin
    ...
    if FColumnSize = 0 then begin
      ...
      if (oConn.DriverKind = dkDBF) and (FColumnSize > 254) then begin
        FColumnSize := 254;
        FLimitedToMaxLen := True;
      end;
      FDataSize := CalcDataSize(FColumnSize);
  ...
  else if (SQLDataType = SQL_NUMERIC) or (SQLDataType = SQL_DECIMAL) or
          (SQLDataType = SQL_DECFLOAT) then begin
    ...
    FScale := 0;
    FColumnSize := SQLULen(High(UDword));
    FDataSize := 0;
  end
...
begin
  ...
  if FBinded then begin
    if Name <> '' then
      oStmt.Check(oStmt.Lib.SQLSetDescField(oStmt.IMP_PARAM_DESC, Position,
        SQL_DESC_NAME, PSQLChar(Name), Length(Name) * SizeOf(Char)));
...
begin
  if SQLDataType = SQL_DECFLOAT then
    Result := '.'
  else
...
begin
  ...
  for i := 0 to FVariableList.Count - 1 do begin
    ...
    if oVar.LongData then begin
      ...
      GetMem(pLocal, oVar.DataSize * NativeInt(RowCount));
...
begin
  ...
  else if oStmt.RowSetSupported then begin
    // +1 is a workaround for AV on fetching using Parkway Micro Focus ODBC driver
    SetLength(FRowStatusArr, FRowCount + 1);
...
begin
  ...
  if AVar.LongData then
    ApData := SQLPointer(NativeInt(AVar.LocalBuffer) + NativeInt(AIndex) * SizeOf(Pointer))
  else
    ApData := SQLPointer(NativeInt(AVar.LocalBuffer) + NativeInt(AIndex) * AVar.DataSize);
  ApInd := PSQLLen(NativeInt(AVar.LocalBufIndicator) + NativeInt(AIndex) * SizeOf(SQLLen));
end;
...
begin
  ...
  FRowSetSupported := True;
end;

{-------------------------------------------------------------------------------}
destructor TODBCStatementBase.Destroy;
begin
  // MSSQL: after connection losting releasing ENV handle may raise exception
  IgnoreErrors := True;
  Unprepare;
  FDFreeAndNil(FColumnList);
  inherited Destroy;
...
begin
  ...
  if pInd^ = SQL_NULL_DATA then
    iSize := SQL_NULL_DATA;
  // ASA: SQLPutData(..., nil, 0) raises "invalid string or buffer length".
...
begin
  ...
  repeat
    ...
    if (TODBCConnection(Owner).DriverKind = dkTeradata) and
      ...
      if oPar.SQLDataType = SQL_SS_TABLE then begin
        PutTableParam(oPar, iIndex, oTVPPar <> oPar);
        oTVPPar := oPar;
      end
...
var
  ...
  begin
    ...
    // SQLRowCount returns "Function sequence error".
      ...
      if TODBCConnection(Owner).DriverKind in [dkMSAccessJet, dkMSAccessACE, dkExcel] then
...
var
  ...
  begin
    FDException(oStmt.OwningObj, [S_FD_LPhys, S_FD_ODBCId],
      er_FD_AccLongDataStream, [AMsg, Param.DumpLabel]);
  end;
...
begin
  ...
  if (@Buffer = nil) and (Count = 0) then
    ...
    pData := @Buffer;
  oStmt.Check(oStmt.Lib.SQLPutData(oStmt.FHandle, pData, SQLLen(Count)));
  Inc(FPosition, Count);
  FModified := True;
  Result := Count;
end;
...
{-------------------------------------------------------------------------------}
procedure TODBCLongDataStream.Flush;
begin
  if (FMode = smOpenWrite) and not FModified then
    Write(nil^, 0);
end;
...
{-------------------------------------------------------------------------------}
function TODBCMetaInfoStatement.EscapeSpecChars(const AStr: String): String;
var
  pSrc, pDest: PChar;
  cEscChar: Char;
begin
  cEscChar := Connection.SEARCH_PATTERN_ESCAPE;

  pSrc := PChar(AStr);
  while not CharInSet(pSrc^, [#0, '_', '%']) and (pSrc^ <> cEscChar) do
    Inc(pSrc);
  if pSrc^ = #0 then
    Exit(AStr);
  ...
  while pSrc^ <> #0 do begin
    if (pSrc^ = '_') or (pSrc^ = '%') or (pSrc^ = cEscChar) then begin
      pDest^ := cEscChar;
      Inc(pDest);
    end;
    pDest^ := pSrc^;
  ...
  SetLength(Result, pDest - PChar(Pointer(Result)));
end;
...
begin
  ...
  FResultCols := -1;
  FCatalog := ACatalog;
  FSchema := EscapeSpecChars(ASchema);
  FObject := EscapeSpecChars(ATable);
  FColumn := AColumn;
end;
