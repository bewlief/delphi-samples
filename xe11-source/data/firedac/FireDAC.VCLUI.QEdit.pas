...
begin
  if CompareText(AValue, '<unassigned>') = 0 then
    Result := Unassigned
  else if (CompareText(AValue, '<null>') = 0) or
          (AValue = '') and not ((AVarType = varString) or (AVarType = varOleStr) or (AVarType = varUString)) then
    Result := Null
...
begin
  if Assigned(FDGUIxCreateMemoFunc) then
    mmSQL := TFDGUIxFormsMemo(FDGUIxCreateMemoFunc(Self))
  else
    mmSQL := TFDGUIxFormsMemo.Create(Self);
  Supports(mmSQL, IFDGUIxFormsMemo, mmSQLIntf);
  mmSQL.Parent := tsSQL;
  mmSQL.Left := ScaleValue(5);
  mmSQL.Top := ScaleValue(5);
  mmSQL.Width := tsSQL.ClientWidth - ScaleValue(11);
  mmSQL.Height := tsSQL.ClientHeight - ScaleValue(11);
  mmSQL.Align := alNone;
  mmSQL.Anchors := [akLeft, akTop, akRight, akBottom];
  if Assigned(mmSQLIntf) then
    mmSQLIntf.SetBothScrollBars
  else
    TFDGUIxFormsMemo(mmSQL).ScrollBars := ssBoth;
  mmSQL.TabOrder := 0;
  if Assigned(mmSQLIntf) then begin
    mmSQLIntf.SetOnChange(mmSQLChange);
    mmSQLIntf.SetOnKeyDown(mmSQLKeyDown);
//    mmSQLIntf.GetFont.Height := 9;
  end
  else begin
    TFDGUIxFormsMemo(mmSQL).OnChange := mmSQLChange;
    TFDGUIxFormsMemo(mmSQL).OnKeyDown := mmSQLKeyDown;
    TFDGUIxFormsMemo(mmSQL).Font.Size := 9;
  end;
...
begin
  ...
  if Assigned(mmSQLIntf) then
    mmSQLIntf.GetLines.SetStrings(AQuery.SQL)
  else
    TFDGUIxFormsMemo(mmSQL).Lines.SetStrings(AQuery.SQL);
  oConn := AQuery.PointedConnection;
  if oConn <> nil then
    if Assigned(mmSQLIntf) then
      mmSQLIntf.SetRDBMSKind(oConn.RDBMSKind)
    else
      TFDGUIxFormsMemo(mmSQL).RDBMSKind := oConn.RDBMSKind;
...
begin
  if FDefinitionChanged then begin
    FDefinitionChanged := False;
    if Assigned(mmSQLIntf) then
      FQuery.SQL.SetStrings(mmSQLIntf.GetLines)
    else
      FQuery.SQL.SetStrings(TFDGUIxFormsMemo(mmSQL).Lines);
...
begin
  if Assigned(mmSQLIntf) then
    acExecute.Enabled := mmSQLIntf.GetLines.Count > 0
  else
    acExecute.Enabled := TFDGUIxFormsMemo(mmSQL).Lines.Count > 0;
end;
