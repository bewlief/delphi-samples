...
{$ENDIF}
  ...
  protected
    ...
    procedure DoGetStorageMergeModes(var AData: TFDMergeDataMode;
      var AMeta: TFDMergeMetaMode); override;
...
{-------------------------------------------------------------------------------}
type
  TFDCommandTextReader = class(TObject)
  public
    FCommand: TFDCustomCommand;
    procedure ReadCommandText(AReader: TReader);
  end;
...
procedure TFDCommandTextReader.ReadCommandText(AReader: TReader);
var
  iPrevPos: Integer;
  lIsString: Boolean;
begin
  iPrevPos := AReader.Position;
  try
    lIsString := AReader.ReadValue in [vaWString, vaUTF8String, vaString, vaLString];
  finally
    AReader.Position := iPrevPos;
  end;
  if lIsString then
    FCommand.CommandText.Text := AReader.ReadString
  else
    __TStrings(FCommand.CommandText).DefineProperties(AReader);
end;
...
begin
  ...
  oCmdRdr := TFDCommandTextReader.Create;
  try
    AFiler.DefineProperty('CommandText', oCmdRdr.ReadCommandText, nil, False);
  finally
    oCmdRdr.Free;
  end;
  if FOwner = nil then
...
{-------------------------------------------------------------------------------}
procedure TFDAdaptedDataSet.DoGetStorageMergeModes(var AData: TFDMergeDataMode;
  var AMeta: TFDMergeMetaMode);
begin
  inherited DoGetStorageMergeModes(AData, AMeta);
  if (AData = dmNone) and (AMeta = mmNone) and
     (Adapter <> nil) and (Adapter.SchemaAdapter <> nil) and
     (Adapter.SchemaAdapter.Count > 1) then begin
    AData := dmDataSet;
    AMeta := mmAdd;
  end;
end;
...
begin
  FContentModified := Active and AValue;
end;

{-------------------------------------------------------------------------------}
procedure TFDAdaptedDataSet.RefreshContent;
begin
  if (State = dsBrowse) and not UpdatesPending then begin
...
var
  lRestore: Boolean;
begin
  lRestore := ([csLoading, csDestroying, csDesigning] * ComponentState = [csLoading, csDesigning]) and
              (dfStreamedActive in FFlags) and DoStoredActivation and (PointedConnection = nil);
  if lRestore then
...
begin
  if Connection <> AValue then begin
    ...
    if ([csLoading, csDestroying, csDesigning] * ComponentState = [csDesigning]) and
       (dfStreamedActive in FFlags) and (PointedConnection <> nil) then
      try
        Active := True;
      finally
        Exclude(FFlags, dfStreamedActive);
...
begin
  ...
  if FTableName <> AValue then begin
    if UpdateOptions.UpdateTableName = FTableName then
...
{-------------------------------------------------------------------------------}
function TFDTable.GetFullTableName: String;
var
  oConn: TFDCustomConnection;
...
begin
  ...
  try
    ...
    if (sBObj <> '') and (sObj = '') then
      sObj := sBObj;
    Result := oConn.EncodeObjectName(sCat, sSch, sBObj, sObj);
  finally
    Command.ReleaseConnection(oConn);
  end;
end;
...
begin
  ...
  Adapter.UpdateTableName := FullTableName;
...
begin
  ...
  try
    ...
    if (FTableParams.FLocateRow = nil) and (Table <> nil) and (Table.Rows.Count > 0) then
      FTableParams.FLocateRow := Table.Rows[0];
...
begin
  ...
  try
    ...
    try
      ...
      // Prepare table name
      Table.SourceName := FullTableName;
