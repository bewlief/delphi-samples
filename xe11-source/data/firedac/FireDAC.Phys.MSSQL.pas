...
begin
  ...
  if ((CompareText(ODBCDriver, C_2012_ODBC) = 0) or
      (CompareText(ODBCDriver, C_2016_ODBC) = 0) or
      (CompareText(ODBCDriver, C_2017_ODBC) = 0) or
      (CompareText(ODBCDriver, C_2018_ODBC) = 0)
{$IFDEF MSWINDOWS}
      or (CompareText(ODBCDriver, C_2005) = 0) or
      (CompareText(ODBCDriver, C_2008) = 0) or
      (CompareText(ODBCDriver, C_2012_NC) = 0)
{$ENDIF}
     ) and
...
begin
  ...
  Result.Rows.Add([Unassigned, S_FD_ConnParam_MSSQL_StringFormat, S_FD_Choose + ';' + S_FD_ANSI, S_FD_Choose, S_FD_ConnParam_MSSQL_StringFormat, -1]);
...
{-------------------------------------------------------------------------------}
function TFDPhysMSSQLConnection.GetStrsType: TFDDataType;
begin
  if FUseWCHAR then
    Result := dtUnknown
  else
...
begin
  ...
  if ConnectionDef.HasValue(S_FD_ConnParam_MSSQL_StringFormat) then
    FUseWCHAR := SameText(ConnectionDef.AsString[S_FD_ConnParam_MSSQL_StringFormat], S_FD_Choose)
  else
    FUseWCHAR := True;
end;
...
begin
  ...
  try
    ...
    if FUseWCHAR then begin
      ...
      oCol1 := oStmt.AddCol(1, SQL_VARCHAR, SQL_C_CHAR, 128);
      oCol2 := oStmt.AddCol(2, SQL_VARCHAR, SQL_C_CHAR, 128);
      oCol3 := oStmt.AddCol(3, SQL_VARCHAR, SQL_C_CHAR, 128);
    end;
