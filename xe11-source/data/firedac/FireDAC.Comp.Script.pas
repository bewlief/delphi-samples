...
var
  sPrecSQLScriptFileName: string;
begin
  sPrecSQLScriptFileName := SQLScriptFileName;
  try
    ...
    SQLScriptFileName := sPrecSQLScriptFileName;
  end;
end;
...
begin
  sUniqueName := 'SQL';
  i := 0;
  while SQLScripts.FindScript(sUniqueName) <> nil do begin
    Inc(i);
    sUniqueName := 'SQL_' + i.ToString;
  end;
  ...
  try
    oScript.SQL := AScript;
    oScript.Name := sUniqueName;
    SQLScriptFileName := sUniqueName;
    ValidateAll;
    if Status = ssFinishSuccess then
      ExecuteAll;
  finally
