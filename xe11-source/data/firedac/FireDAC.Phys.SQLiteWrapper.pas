...
type
  ...
  TSQLiteLib = class;
  TSQLiteLibClass = class of TSQLiteLib;
  TSQLiteLibDyn = class;
  TSQLiteHandle = class;
  ...
  TSQLiteEngineLinkage = (slDefault, slStatic, slDynamic, slSEEStatic);
  ...
  protected
    ...
    procedure InternalAfterLoad(AEnableSharedCache: Boolean);
    procedure InternalBeforeUnload;
    function GetDefaultSharedCacheMode: Integer; virtual; abstract;
  protected
    class var
      Gsqlite3_user_data: Tsqlite3_user_data;
  public
    class var
      GLibClasses: array [TSQLiteEngineLinkage] of TSQLiteLibClass;
  public
    ...
    Fsqlite3_errstr: Tsqlite3_errstr;
    ...
    Fsqlite3_activate_see: Tsqlite3_activate_see;
    Fsqlite3_key: Tsqlite3_key;
    Fsqlite3_rekey: Tsqlite3_rekey;
    ...
    Fad_sqlite3GetCacheSize: Tad_sqlite3GetCacheSize;
    Fad_sqlite3GetEncoding: Tad_sqlite3GetEncoding;
    Fad_sqlite3GetEncryptionMode: Tad_sqlite3GetEncryptionMode;
    Fad_sqlite3GetEncryptionError: Tad_sqlite3GetEncryptionError;
    Fad_sqlite3Error: Tad_sqlite3Error;
    constructor Create(AOwningObj: TObject = nil);
    destructor Destroy; override;
    procedure Load(const AVendorHome, AVendorLib: String); overload; virtual; abstract;
    procedure ReleaseMemory(AAmount: Integer);
    procedure SoftHeapLimit(AAmount: Integer);
    procedure ActivateSEE(const AKey: String);
...
{$ENDIF}
  ...
  private
    ...
    [unsafe] FCurStatement: TSQLiteStatement;
...
{$ENDIF}
    function GetKeyEncoding: TFDEncoding;
...
{$ENDIF}
  ...
  TSQLiteVConstraintOp = (coEqual, coGreater, coLess, coLessOrEqual,
    coGreaterOrEqual, coMatch, coLike, coGLOB, coRegExp, coNotEqual,
    coIsNot, coIsNotNull, coIsNull, coIs);
...
const
  ...
  ssqlite3_activate_see: String = 'sqlite3_activate_see';
  ...
  sad_sqlite3GetCacheSize: String = 'ad_sqlite3GetCacheSize';
  sad_sqlite3GetEncoding: String = 'ad_sqlite3GetEncoding';
  sad_sqlite3GetEncryptionMode: String = 'ad_sqlite3GetEncryptionMode';
  sad_sqlite3GetEncryptionError: String = 'ad_sqlite3GetEncryptionError';
  sad_sqlite3Error: String = 'ad_sqlite3Error';
...
{-------------------------------------------------------------------------------}
procedure TSQLiteLib.ActivateSEE(const AKey: String);
begin
  if Assigned(Fsqlite3_activate_see) then
    Fsqlite3_activate_see(PFDAnsiString(PByte(TFDEncoder.Enco(AKey, ecANSI))));
end;
...
{-------------------------------------------------------------------------------}
procedure TSQLiteLib.InternalAfterLoad(AEnableSharedCache: Boolean);
begin
                                                                       
  if Assigned(Fsqlite3_initialize) then
    Check(Fsqlite3_initialize());

  if AEnableSharedCache then
...
{-------------------------------------------------------------------------------}
procedure TSQLiteLib.InternalBeforeUnload;
begin
...
{ TSQLiteLibDyn                                                                 }
{-------------------------------------------------------------------------------}
function TSQLiteLibDyn.GetDefaultSharedCacheMode: Integer;
begin
  Result := SQLITE_OPEN_SHAREDCACHE;
end;

{-------------------------------------------------------------------------------}
procedure TSQLiteLibDyn.Load(const AVendorHome, AVendorLib: String);
var
  sDLLName: String;
...
{$ENDIF}
  ...
  InternalAfterLoad(False);
end;

{-------------------------------------------------------------------------------}
procedure TSQLiteLibDyn.Unload;
begin
  if hDLL <> 0 then
...
{-------------------------------------------------------------------------------}
function TSQLiteLibDyn.GetSQLiteProcAddress(const AProcName: String;
  A16: Boolean; AVer: Integer; ARequired: Boolean): Pointer;
var
  sName: String;
begin
  if A16 then
...
{-------------------------------------------------------------------------------}
procedure TSQLiteLibDyn.LoadEntries;
begin
  ...
  if Version >= svSQLite030600 then begin
    ...
    @Fsqlite3_shutdown := GetSQLiteProcAddress(ssqlite3_shutdown);
  ...
  @Fsqlite3_errstr := GetSQLiteProcAddress(ssqlite3_errmsg, False, 0, False);
  ...
  @Fsqlite3_activate_see := GetSQLiteProcAddress(ssqlite3_activate_see, False, 0, False);
  ...
  @Fsqlite3_trace := GetSQLiteProcAddress(ssqlite3_trace, False, 0, False);
  @Fsqlite3_profile := GetSQLiteProcAddress(ssqlite3_profile, False, 0, False);
  ...
  @Fad_sqlite3GetCacheSize := GetSQLiteProcAddress(sad_sqlite3GetCacheSize, False, 0, False);
  @Fad_sqlite3GetEncoding := GetSQLiteProcAddress(sad_sqlite3GetEncoding, False, 0, False);
  @Fad_sqlite3GetEncryptionMode := GetSQLiteProcAddress(sad_sqlite3GetEncryptionMode, False, 0, False);
  @Fad_sqlite3GetEncryptionError := GetSQLiteProcAddress(sad_sqlite3GetEncryptionError, False, 0, False);
  @Fad_sqlite3Error := GetSQLiteProcAddress(sad_sqlite3Error, False, 0, False);
end;
...
{$ENDIF}
  ...
  if AMsg <> '' then
    sMsg := AMsg
  else if oDB.Handle <> nil then
    if Assigned(oDB.Lib.Fad_sqlite3GetEncryptionError) and
       (oDB.Lib.Fad_sqlite3GetEncryptionError(oDB.Handle, pEncError, iEncLen, iEncError) = SQLITE_ERROR) then begin
  ...
  else if Assigned(oDB.Lib.Fsqlite3_errstr) then
    sMsg := oDB.Encoder.Decode(oDB.Lib.Fsqlite3_errstr(ACode), -1, ecUTF8)
  else
    sMsg := 'code ' + IntToStr(ACode);
...
{$IFDEF FireDAC_MONITOR}
  procedure Trace1;
  begin
    Trace(ssqlite3_open_v2, ['filename', AFilename, 'mode', iFlags]);
  end;
...
begin
  try
    if Assigned(Lib.Fsqlite3_open_v2) then begin
      iFlags := aModes[AMode];
      if ASharedCache = scDefault then
        iFlags := iFlags or Lib.GetDefaultSharedCacheMode
      else
        iFlags := iFlags or aCaches[ASharedCache];
{$IFDEF FireDAC_MONITOR}
      if Tracing then Trace1;
{$ENDIF}
      pFileName := PByte(Encoder.Encode(AFilename, ecUTF8));
      Check(Lib.Fsqlite3_open_v2(PUtf8(pFileName), FHandle, iFlags, nil));
    end
    else begin
{$IFDEF FireDAC_MONITOR}
...
{-------------------------------------------------------------------------------}
function TSQLiteDatabase.GetKeyEncoding: TFDEncoding;
begin
  if Assigned(Lib.Fad_sqlite3GetEncryptionMode) then
    Result := ecUTF8
  else
    Result := ecANSI;
end;
...
begin
  if Assigned(Lib.Fsqlite3_key) then begin
    sKey := Encoder.Encode(AKey, GetKeyEncoding);
{$IFDEF FireDAC_MONITOR}
    if Tracing then Trace1;
{$ENDIF}
    Check(Lib.Fsqlite3_key(Handle, PByte(sKey), Encoder.EncodedLength(sKey, GetKeyEncoding)));
  end
  else
...
{$IFDEF FireDAC_MONITOR}
    if Tracing then Trace1;
{$ENDIF}
    Check(Lib.Fsqlite3_rekey(Handle, PByte(sKey), Encoder.EncodedLength(sKey, GetKeyEncoding)));
  end
  else
    FDCapabilityNotSupported(nil, [S_FD_LPhys, S_FD_SQLiteId, 'sqlite3']);
end;
...
begin
  if Handle <> nil then
    if Tracing then begin
      if Assigned(Lib.Fsqlite3_trace) then
        Lib.Fsqlite3_trace(Handle, @FDsqlite3_trace, Self);
      if Assigned(Lib.Fsqlite3_profile) then
...
begin
  if Assigned(Lib.Fsqlite3_table_column_metadata) then begin
    ...
    // When ADBName='', then most probably it is calculated column in CTE
    else if ARaiseError and (ADBName <> '') then
      Check(iCode);
  end
  else
    Result := False;
...
begin
  if Assigned(Lib.Fad_sqlite3GetCacheSize) then
    Result := Lib.Fad_sqlite3GetCacheSize(Handle)
  else
    Result := 0;
end;
...
begin
  if Assigned(Lib.Fad_sqlite3GetEncoding) then
    case Lib.Fad_sqlite3GetEncoding(Handle) of
    SQLITE_UTF16NATIVE,
...
begin
  ...
  if Assigned(Lib.Fad_sqlite3GetEncryptionMode) and
     (Lib.Fad_sqlite3GetEncryptionMode(Handle, pName, iLen) = SQLITE_OK) then
...
var
  i: Integer;
  oInst: TSQLiteExtensionInstance;
begin
  Lib.FLock.Enter;
  try
    for i := ADatabase.FExtensionInstances.Count - 1 downto 0 do begin
      oInst := TSQLiteExtensionInstance(ADatabase.FExtensionInstances[i]);
      if (oInst.Extension = Self) and (oInst.Database = ADatabase) then
        FDFree(oInst);
    end;
  finally
    Lib.FLock.Leave;
...
begin
  ...
  iLen := Database.Encoder.EncodedLength(sb, ecANSI);
  if iLen = 0 then
    pData := Self
  else
    pData := PByte(sb);
  SetData(pData, iLen, etString);
end;
    ...
    if AValue <> nil then
      ...
      SQLITE_BLOB:
        begin
          ...
          // If AValue=nil and ASize=0 then BLOB value will be
          // set to NULL instead of zero length BLOB
...
{-------------------------------------------------------------------------------}
procedure TSQLiteVariables.Clear(ACount: Integer);
var
  i: Integer;
begin
  for i := FList.Count - 1 downto ACount do
...
begin
  // SQLite: To avoid Invalid Floating Point Operation exception on executing SELECT query
...
begin
  ...
  Result := FDHasKW(sSQL, 'DROP') and FDHasKW(sSQL, 'TABLE');
end;
...
begin
  TSQLiteFunctionInstance(TSQLiteLib.Gsqlite3_user_data(context)).DoCalculate(context, nargs, args);
end;

{-------------------------------------------------------------------------------}
procedure FDsqlite3_final(context: psqlite3_context); cdecl;
begin
  TSQLiteFunctionInstance(TSQLiteLib.Gsqlite3_user_data(context)).DoFinalize(context);
end;
...
begin
  for i := 0 to FDExpressionManager().FunctionCount - 1 do begin
    ...
    if (oDesc.ScopeKind in [ckUnknown, ckConst]) and (oDesc.ArgMax <> MAXINT) and
       not ((ALib.Version >= svSQLite033200) and (CompareText('iif', oDesc.Name) = 0)) then begin
      lIgnore := False;
      ...
      if not lIgnore then
        for j := oDesc.ArgMin to oDesc.ArgMax do begin
          ...
          oFunc.Deterministic := not (oDesc.ScopeKind in [ckField, ckAgg]) and
                                                                              
            (CompareText('newguid', oDesc.Name) <> 0);
        end;
    end;
  end;
end;
...
begin
  for i := 0 to Inputs.Count - 1 do begin
    ...
    if oIn.IsNull then
      ...
      C_FD_FBool:  FFuncArgs[i] := oIn.AsBoolean;
      end;
  end;
...
begin
  ...
  for i := Low(ACols) to High(ACols) do begin
    oDef := ACols[i];
    Result := Result + '[' + oDef.FName + '] ' + C_HIDDEN[oDef.FHidden] +
      oDef.FDataType + C_COLON[i <> High(ACols)];
  end;
  Result := Result + ')';
end;
  ...
  for i := 0 to Length(FConstraints) - 1 do begin
    ...
    if i < info^.nConstraint then begin
      if pConstr^.usable = SQLITE_TRUE then begin
        ...
        SQLITE_INDEX_CONSTRAINT_LIKE:     FConstraints[i].FOp := coLike;
        SQLITE_INDEX_CONSTRAINT_GLOB:     FConstraints[i].FOp := coGLOB;
        SQLITE_INDEX_CONSTRAINT_REGEXP:   FConstraints[i].FOp := coRegExp;
        SQLITE_INDEX_CONSTRAINT_NE:       FConstraints[i].FOp := coNotEqual;
        SQLITE_INDEX_CONSTRAINT_ISNOT:    FConstraints[i].FOp := coIsNot;
        SQLITE_INDEX_CONSTRAINT_ISNOTNULL:FConstraints[i].FOp := coIsNotNull;
        SQLITE_INDEX_CONSTRAINT_ISNULL:   FConstraints[i].FOp := coIsNull;
        SQLITE_INDEX_CONSTRAINT_IS:       FConstraints[i].FOp := coIs;
        end;
      end;
...
initialization
  TSQLiteLib.Gsqlite3_user_data := nil;

  if TSQLiteLib.GLibClasses[slDefault] = nil then
    TSQLiteLib.GLibClasses[slDefault] := TSQLiteLibDyn;
  TSQLiteLib.GLibClasses[slDynamic] := TSQLiteLibDyn;
  TSQLiteLib.GLibClasses[slSEEStatic] := nil;
