...
function TFDPhysCommandGenerator.ColumnStorable(AColumn: TFDDatSColumn): Boolean;
  ...
  function GetParentColumnAttributes(AColumn: TFDDatSColumn): TFDDataAttributes;
  var
    eAttrs: TFDDataAttributes;
    eOpts: TFDDataOptions;
    sName, sAlias: String;
    oOrigCol, oParCol: TFDDatSColumn;
  begin
    sName := '';
    sAlias := '';
    eAttrs := [];
    ...
    oOrigCol := AColumn;
    while True do begin
      oParCol := AColumn.ParentColumn;
      if oParCol = nil then
        Break;
      AColumn := oParCol;
    end;
    if oOrigCol = AColumn then
      Result := []
    else begin
      GetColumnAttributes(AColumn, eAttrs, eOpts, sName, sAlias);
      Result := eAttrs * [caAllowNull, caReadOnly, caCalculated, caBase, caExpr];
    end;
  end;
...
begin
  ...
  eOpts := [];
  GetColumnAttributes(AColumn, eAttrs, eOpts, sName, sAlias);
  if ctInvars in AColumn.ColumnList.HasThings then
    eAttrs := eAttrs + GetParentColumnAttributes(AColumn);
  Result :=
...
begin
  if ARow = nil then
    Result := True
  else if AColumn.DataType in [dtArrayRef, dtRowRef] then begin
...
begin
  ...
  if lBeautify then begin
    // SQLite does not support quoted parameter names and will return "invalid token ':'" error
    if FConnMeta.Kind = TFDRDBMSKinds.SQLite then
      Result := ':' + Result
    else
...
begin
  ...
  // Build FROM
  if (ATableParams.FTable <> '') and (ATableParams.FTable[1] = '{') then
    sSQLFrom := ATableParams.FTable
  else begin
  ...
  if sIndexFields <> '' then begin
    ...
    while i1 <= Length(sIndexFields) do begin
      ...
      // Build ORDER BY
      // Note, when table has a record where all index fields are null's,
      ...
      if not (ATableParams.FTableCommand in [tcGetRecNo, tcGetRowCount]) then begin
        ...
        if (Pos(';' + UpperCase(sField) + ';', sNocaseFields) <> 0) and
           ((ATableParams.FLocateRow = nil) or
            (ATableParams.FLocateRow.Table.Columns.ColumnByName(sField).DataType in C_FD_CharTypes)) then
...
begin
  ...
  lMySQL := FConnMeta.Kind = TFDRDBMSKinds.MySQL;
  case AKind of
  mkForeignKeys:
    begin
  ...
  mkForeignKeyFields:
    begin
      ...
      if not lMySQL then begin
        ...
        if FConnMeta.Kind = TFDRDBMSKinds.MSSQL then
          Result := Result + ' AND E.ORDINAL_POSITION = D.ORDINAL_POSITION'
        else
          Result := Result + ' AND E.ORDINAL_POSITION = D.POSITION_IN_UNIQUE_CONSTRAINT';
      end;
      if (ACatalog <> '') and (npCatalog in eParts) then
        AddWhere(GetFK_TABLE_CATALOG + ' = ', 'CAT');
      if (ASchema <> '') and (npSchema in eParts) then
