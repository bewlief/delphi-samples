...
type
  ...
  TFDPhysManager = class(TFDObject, IUnknown,
  ...
  TFDPhysConnectionHost = class(TInterfacedObject, IUnknown, 
    IFDStanObjectHost)
  private
...
{$ENDIF}
  ...
  TFDPhysConnection = class(TInterfacedObject, IUnknown,
...
{$ENDIF}
    ...
    procedure RecoverConnection(const AInitiator: IFDStanObject; AOnLogin, AForceRaise: Boolean);
  public
    // IUnknown
    function _AddRef: Integer; stdcall;
    function _Release: Integer; stdcall;
...
{$ENDIF}
  ...
  protected
    class function ThreadOwned: Boolean; override;
  public
...
{$ENDIF}
  ...
  TFDPhysCommand = class(TInterfacedObject, IUnknown,
    IFDMoniAdapter, IFDStanObject, IFDStanOptions, IFDStanErrorHandler,
    ...
    try
      ...
      on E: EFDDBEngineException do
        if E.Kind = ekServerGone then
          RecoverConnection(Self, True, False)
        else
          raise;
    end;
  until lConnected;
...
begin
  ...
  iVer := Int64(oConnMeta.ServerVersion div 100000000) - Int64(oConnMeta.ClientVersion div 100000000);
  if iVer < 0 then
    iVer := -iVer;
  if iVer > 1 then
...
{-------------------------------------------------------------------------------}
procedure TFDPhysConnection.RecoverConnection(const AInitiator: IFDStanObject;
  AOnLogin, AForceRaise: Boolean);
var
  oExc: TObject;
        ...
        try
          ...
          on E: EFDDBEngineException do
            if E.Kind = ekServerGone then begin
              RecoverConnection(Self, False, False);
...
begin
  if not (GetState in [tsInactive, tsCommiting, tsRollingback]) and
        ...
        try
          ...
          on E: EFDDBEngineException do
            if E.Kind = ekServerGone then begin
              ...
              FConnectionObj.RecoverConnection(Self, False, False);
...
{-------------------------------------------------------------------------------}
class function TFDPhysEventMessage.ThreadOwned: Boolean;
begin
  Result := False;
end;
...
begin
  if GetSubscriptionName <> '' then begin
    lFound := False;
    for i := 0 to FChangeHandlerNames.Count - 1 do
      if AnsiCompareText(AEventName, FChangeHandlerNames[i]) = 0 then begin
        lFound := True;
        oHnlr := FChangeHandlers[i] as IFDPhysChangeHandler;
...
procedure DumpParams(Self: TFDPhysCommand; AKind: TFDMoniEventKind; ATypes: TParamTypes);
var
  i: Integer;
  oPars: TFDParams;
  s: String;
begin
  with Self do begin
    oPars := GetParams;
    for i := 0 to oPars.Count - 1 do
      if oPars[i].ParamType in ATypes then begin
        s := 'VAR ' + oPars[i].SQLName + ' ';
        case oPars[i].ParamType of
        ptUnknown,
        ptInput:
          s := s + 'IN';
        ptOutput,
        ...
        s := s + ' = ';
        if oPars[i].IsNull then
          s := s + 'NULL'
        else if oPars[i].ArraySize <> 1 then
          s := s + '<array>'
        else if oPars[i].IsObject then
          s := s + '<object>'
        else if oPars[i].IsStream then
          s := s + '<stream>'
        else if oPars[i].IsDataSet then
          s := s + '<dataset>'
        else if oPars[i].DataType in (ftNonTextTypes - [ftMemo, ftFmtMemo]) then
          s := s + '<binary>'
        else
          try
            s := s + oPars[i].AsString;
          except
            s := s + '<...>';
          end;
        Trace(AKind, esProgress, s, []);
      end;
  end;
end;
{$ENDIF}
...
{$IFDEF FireDAC_MONITOR}
  ...
  begin
    ...
    Trace(ekCmdExecute, esStart, 'Open', ['Command', sCmdText]);
    Trace(ekSQL, esStart, '', []);
    if ekSQLVarIn in FConnectionObj.GetMonitor.EventKinds then
      DumpParams(Self, ekSQLVarIn, [ptUnknown, ptInput, ptInputOutput]);
    Trace(ekSQL, esProgress, sCmdText, [FMetaInfoKind = mkNone]);
    rTime := TStopwatch.StartNew;
  end;
  ...
  begin
    rTime.Stop;
    if AUpdateMonitor then UpdateMonitor;
    if ekSQLVarOut in FConnectionObj.GetMonitor.EventKinds then
      DumpParams(Self, ekSQLVarOut, [ptOutput, ptInputOutput, ptResult]);
    Trace(ekSQL, esEnd, '', [rTime.Elapsed.ToString]);
    Trace(ekCmdExecute, esEnd, 'Open', ['Command', sCmdText]);
  end;
{$ENDIF}
...
{$IFDEF FireDAC_MONITOR}
  ...
  begin
    sCmdText := GetTraceCommandText;
    Trace(ekCmdExecute, esStart, 'Execute', ['Command', sCmdText,
      'ATimes', ATimes, 'AOffset', AOffset]);
    Trace(ekSQL, esStart, '', []);
    if ekSQLVarIn in FConnectionObj.GetMonitor.EventKinds then
      DumpParams(Self, ekSQLVarIn, [ptUnknown, ptInput, ptInputOutput]);
    Trace(ekSQL, esProgress, FDbCommandText, [True]);
    rTime := TStopwatch.StartNew;
  end;
  ...
  begin
    ...
    if ekSQLVarOut in FConnectionObj.GetMonitor.EventKinds then
      DumpParams(Self, ekSQLVarOut, [ptOutput, ptInputOutput, ptResult]);
    Trace(ekSQL, esEnd, '', [rTime.Elapsed.ToString]);
    Trace(ekCmdExecute, esEnd, 'Execute', ['Command', sCmdText,
    ...
    repeat
      ...
      if ARowsetSize > 0 then begin
        try
          ...
          on E: EFDDBEngineException do
            if E.Kind = ekServerGone then
              FConnectionObj.RecoverConnection(Self, False, True)
            else
              raise;
        end;
