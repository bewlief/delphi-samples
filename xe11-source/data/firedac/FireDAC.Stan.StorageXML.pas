...
begin
  ...
  dtDateTime:
    begin
      ...
      if FStreamVersion >= 16 then
        sVal := Format(CDateTimeISO8601FormatWithMS, [y, mo, d, h, mi, se, ms])
      else if FStreamVersion >= 9 then
  ...
  dtDateTimeStamp:
    begin
      ...
      if FStreamVersion >= 16 then
        AddAttribute(APropName, Format(CDateTimeISO8601FormatWithMS, [pTS^.Year,
          pTS^.Month, pTS^.Day, pTS^.Hour, pTS^.Minute, pTS^.Second, pTS^.Fractions]))
      else if FStreamVersion >= 9 then
  ...
  dtTime:
    begin
      ...
      if FStreamVersion >= 16 then
        AddAttribute(APropName, Format(CTimeFormatWithMS, [sSign, rTS.Hour,
          rTS.Minute, rTS.Second, rTS.Fractions]))
      else
        AddAttribute(APropName, Format(CTimeFormat, [sSign, rTS.Hour,
          rTS.Minute, rTS.Second]));
    end;
  dtDate:
    begin
      rTS := FDDate2SQLTimeStamp(PInteger(ABuff)^);
      AddAttribute(APropName, Format(CDateFormat, [rTS.Year, rTS.Month, rTS.Day]));
    end;
  dtAnsiString:
    begin
  ...
  dtHBFile:
    ...
    dtDateTime:
      begin
        ...
        if (i = 0) or ((pCh + 8)^ = 'T') then begin
          FDStr2Int(pCh + 8 + i, 2, @rTS.Hour, SizeOf(rTS.Hour), True);
          FDStr2Int(pCh + 10 + i, 2, @rTS.Minute, SizeOf(rTS.Minute), True);
          FDStr2Int(pCh + 12 + i, 2, @rTS.Second, SizeOf(rTS.Second), True);
          if FStreamVersion >= 16 then
            FDStr2Int(pCh + 15 + i, 3, @rTS.Fractions, SizeOf(rTS.Fractions), True)
          else
            rTS.Fractions := 0;
        end
        else begin
    ...
    dtDateTimeStamp:
      begin
        ...
        if (i = 0) or ((pCh + 8)^ = 'T') then begin
          FDStr2Int(pCh + 8 + i, 2, @pTS^.Hour, SizeOf(pTS^.Hour), True);
          FDStr2Int(pCh + 10 + i, 2, @pTS^.Minute, SizeOf(pTS^.Minute), True);
          FDStr2Int(pCh + 12 + i, 2, @pTS^.Second, SizeOf(pTS^.Second), True);
          if FStreamVersion >= 16 then
            FDStr2Int(pCh + 15 + i, 3, @pTS^.Fractions, SizeOf(pTS^.Fractions), True)
          else
            pTS^.Fractions := 0;
        end
        else begin
