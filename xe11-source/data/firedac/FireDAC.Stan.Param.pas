...
begin
  if FHasObjects then
    for i := 0 to Count - 1 do begin
      oPar := Items[i];
      if oPar.IsObject and not oPar.IsDataset then
        oPar.Clear();
    end;
end;
...
begin
  ...
  case DataType of
    ...
    ftLongWord:
      PCardinal(ABuffer)^ := AsLongWords[AIndex];
    ftLargeint:
      PInt64(ABuffer)^ := AsLargeInts[AIndex];
    ftTime:
      if AsDateTimes[AIndex] < 0 then
...
{$ENDIF}
      ...
      ftLongWord:
        AsLongWords[AIndex] := PCardinal(ABuffer)^;
      ftLargeint:
        AsLargeInts[AIndex] := PInt64(ABuffer)^;
      ftTime:
        begin
          TimeStamp.Date := DateDelta;
          if PInteger(ABuffer)^ < 0 then begin
...
{$IFNDEF NEXTGEN}
    varString:
      begin
        APtr := PByte(PAnsiChar(AnsiString(TVarData(FValue[AIndex]).VString)));
...
function TFDParam.HasParamObjectBase(const AData: TVarData; const AObjectType: TClass): Boolean;
  ...
  var
    oObj: IParamObject;
  begin
    Result := Supports(IUnknown(AData.VUnknown), IParamObject, oObj);
    if Result then
      if AObjectType = TObject then
        Result := oObj.GetInstance(False) <> nil
      else
        Result := oObj.InstanceIsType(AObjectType);
  end;
...
begin
  if not (DataType in [ftWideString, ftFixedWideChar, ftString, ftFixedChar]) then
    DataType := ftWideString;
  Values[AIndex] := AValue;
end;
...
begin
  ...
  if not IsUnicode and ((tp = varOleStr) or (tp = varUString)) then
    FValue[AIndex] := VarAsType(AValue, varFDAString)
  else if IsUnicode and (tp = varString) then
    FValue[AIndex] := VarAsType(AValue, varUString)
