...
{$IFDEF NEXTGEN}
            ...
            iLen := Length(s);
            FBuffer.Check((iLen + 1) * SizeOf(TFDAnsiChar));
...
{$ENDIF}
        ...
        ftDBaseOle:
          begin
            ...
            iLen := Length(s);
            FBuffer.Check((iLen + 1) * SizeOf(WideChar));
            if iLen <> 0 then
          ...
          begin
            ...
            if iLen = LongWord(-1) then begin
              ...
              repeat
                ...
                iLen := oDbxParam.Value.GetBytes(iPos, aBytes, iPos, C_FD_DefPieceBuffLen);
                Inc(iPos, iLen);
  ...
  TDBXDataTypes.BcdType:
    begin
      ...
      if ADbxPrec < AScale then
        ...
        APrec := ADbxPrec;
      oFmtOpts := GetOptions.FormatOptions;
      if oFmtOpts.IsFmtBcd(APrec, AScale) then
        AType := dtFmtBCD
      else
        AType := dtBCD;
    end;
...
{-------------------------------------------------------------------------------}
function TFDPhysTDBXCommand.InternalOpen(var ACount: TFDCounter): Boolean;
var
  i, wCols: Word;
  wType: TDBXType;
  siPrec: SmallInt;
  ...
  if FDBXReader = nil then begin
    if GetMetaInfoKind = mkNone then begin
      ...
      try
        ...
        ACount := FDBXCommand.RowsAffected;
        TDBXConn.FCurrentCommand := nil;
      end;
