...
begin
  ...
  if AMode = smWrite then begin
    SetLength(FStack, 0);
    FStackIndex := -1;
    FDirectWriting := AFileName = C_FD_SysNamePrefix + '[';
    if FDirectWriting then
      WriteObjectBegin('', osFlatArray)
    else begin
    ...
    if TestObject('FDBS') then begin
      ...
      FStreamVersion := ReadInteger('Version', C_FD_StorageVer112);
    end;
  end;
end;
...
begin
  ...
  dtDateTime:
    begin
      ...
      FDStr2Int(pCh + 0, 4, @rTS.Year, SizeOf(rTS.Year), True);
      FDStr2Int(pCh + 4, 2, @rTS.Month, SizeOf(rTS.Month), True);
      FDStr2Int(pCh + 6, 2, @rTS.Day, SizeOf(rTS.Day), True);
      if (pCh + 8)^ = 'T' then begin
        FDStr2Int(pCh + 9, 2, @rTS.Hour, SizeOf(rTS.Hour), True);
        FDStr2Int(pCh + 11, 2, @rTS.Minute, SizeOf(rTS.Minute), True);
        FDStr2Int(pCh + 13, 2, @rTS.Second, SizeOf(rTS.Second), True);
        if FStreamVersion >= 16 then
          FDStr2Int(pCh + 16, 3, @rTS.Fractions, SizeOf(rTS.Fractions), True)
        else
          rTS.Fractions := 0;
      end
      else begin
  ...
  dtDateTimeStamp:
    begin
      ...
      pCh := PChar(FTokenString);
      FDStr2Int(pCh + 0, 4, @pTS^.Year, SizeOf(pTS^.Year), True);
      FDStr2Int(pCh + 4, 2, @pTS^.Month, SizeOf(pTS^.Month), True);
      FDStr2Int(pCh + 6, 2, @pTS^.Day, SizeOf(pTS^.Day), True);
      if (pCh + 8)^ = 'T' then begin
        FDStr2Int(pCh + 9, 2, @pTS^.Hour, SizeOf(pTS^.Hour), True);
        FDStr2Int(pCh + 11, 2, @pTS^.Minute, SizeOf(pTS^.Minute), True);
        FDStr2Int(pCh + 13, 2, @pTS^.Second, SizeOf(pTS^.Second), True);
        if FStreamVersion >= 16 then
          FDStr2Int(pCh + 16, 3, @pTS^.Fractions, SizeOf(pTS^.Fractions), True)
        else
          pTS^.Fractions := 0;
      end
      else begin
  ...
  dtTime:
    begin
      ...
      if pCh^ = '-' then begin
        ...
        iSign := 1;
      FDStr2Int(pCh + 0, 2, @rTS.Hour, SizeOf(rTS.Hour), True);
      FDStr2Int(pCh + 2, 2, @rTS.Minute, SizeOf(rTS.Minute), True);
      FDStr2Int(pCh + 4, 2, @rTS.Second, SizeOf(rTS.Second), True);
      if FStreamVersion >= 16 then
        FDStr2Int(pCh + 7, 3, @rTS.Fractions, SizeOf(rTS.Fractions), True)
      else
        rTS.Fractions := 0;
      PInteger(ABuff)^ := iSign * FDSQLTimeStamp2Time(rTS);
    end;
  dtDate:
    begin
      pCh := PChar(FTokenString);
...
*)
  ...
  if FStackIndex = -1 then
    if FDirectWriting and (AObjectName = '') then
      eParent := osFlatArray
    else
      eParent := osObject
  else
...
const
  ...
  CDateTimeISO8601FormatWithMS: String = '%.4d%.2d%.2dT%.2d%.2d%.2d.%.3d';
  CTimeFormat: String = '%s%.2d%.2d%.2d';
  CTimeFormatWithMS: String =  '%s%.2d%.2d%.2d.%.3d';
...
begin
  ...
  dtDateTime:
    begin
      ...
      if FStreamVersion >= 16 then
        iLen := WideFormatBuf(pDest^, iLen, Pointer(CDateTimeISO8601FormatWithMS)^,
          Length(CDateTimeISO8601FormatWithMS), [y, mo, d, h, mi, se, ms], FormatSettings)
      else
  ...
  dtDateTimeStamp:
    begin
      ...
      if FStreamVersion >= 16 then
        iLen := WideFormatBuf(pDest^, iLen, Pointer(CDateTimeISO8601FormatWithMS)^,
          Length(CDateTimeISO8601FormatWithMS), [pTS^.Year, pTS^.Month, pTS^.Day, pTS^.Hour,
          pTS^.Minute, pTS^.Second, pTS^.Fractions], FormatSettings)
      else
        iLen := WideFormatBuf(pDest^, iLen, Pointer(CDateTimeISO8601Format)^,
  ...
  dtTime:
    begin
      ...
      if iVal < 0 then begin
        ...
        sSign := '';
      rTS := FDTime2SQLTimeStamp(iVal);
      if FStreamVersion >= 16 then
        iLen := WideFormatBuf(pDest^, iLen, Pointer(CTimeFormatWithMS)^,
          Length(CTimeFormatWithMS), [sSign, rTS.Hour, rTS.Minute, rTS.Second,
          rTS.Fractions], FormatSettings)
      else
        iLen := WideFormatBuf(pDest^, iLen, Pointer(CTimeFormat)^,
          Length(CTimeFormat), [sSign, rTS.Hour, rTS.Minute, rTS.Second], FormatSettings);
      AddAttribute(APropName, pDest, iLen, True);
    end;
  dtDate:
    begin
