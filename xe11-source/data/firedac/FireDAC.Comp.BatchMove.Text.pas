...
begin
  if ASource is TFDTextField then begin
    ...
    Precision := TFDTextField(ASource).Precision;
  end
  else
  ...
  begin
    ...
    if (AItem = '') or (DataDef.FormatSettings.NullVals.IndexOf(AItem) >= 0) then
      ...
      Result := atLongInt
                            ...
                            ['0'..'9', DataDef.FormatSettings.ThousandSeparator, DataDef.FormatSettings.DecimalSeparator]) and
            TryStrToFloat(FDRemoveChar(AItem, DataDef.FormatSettings.ThousandSeparator), eVal, DataDef.FormatSettings.FS) then
      Result := atFloat
...
var
  ...
  begin
    if FDInSet(AStr[1], ['T', 't', 'Y', 'y']) then
      ...
      Result := FDStr2Float(FDRemoveChar(AStr, DataDef.FormatSettings.ThousandSeparator), AFS.DecimalSeparator);
  end;
  ...
  function CheckTimeNotEmpty(const AStr: String): Boolean;
  var
    pCh: PChar;
    oFS: TFDTextFormatSettings;
  begin
    ...
    oFS := DataDef.FormatSettings;
    while pCh^ <> #0 do
      if not ((pCh^ = ' ') or (pCh^ = oFS.TimeSeparator)) then
        Break
      else
        Inc(pCh);
    Result := pCh^ <> #0;
    ...
    if (S = '') and ((oFld.DataType <> atString) or DataDef.StrsEmpty2Null) or
      ...
      try
        ...
        atString:
          ATxtFld.FValue := S;
        atFloat:
          ATxtFld.FValue := StrAndBoolToFloat(S, DataDef.FormatSettings.FS);
        atNumber:
          ATxtFld.FValue := StrAndBoolToInt(S);
        atBool:
          if DataDef.FormatSettings.BoolTrueVals.IndexOf(S) >= 0 then
            ATxtFld.FValue := True
          else if DataDef.FormatSettings.BoolFalseVals.IndexOf(S) >= 0 then
            ATxtFld.FValue := False
          else
            ATxtFld.FValue := StrAndBoolToInt(S) <> 0;
        atLongInt:
          ATxtFld.FValue := StrAndBoolToInt(S);
        atDate:
          if CheckDateNotEmpty(S) then
            ATxtFld.FValue := StrToDate(S, DataDef.FormatSettings.FS)
          else
            ATxtFld.FValue := Null;
        atTime:
          if CheckTimeNotEmpty(S) then
            ATxtFld.FValue := StrToTime(S, DataDef.FormatSettings.FS)
          else
            ATxtFld.FValue := Null;
        atDateTime:
          if CheckDateNotEmpty(S) then
            ATxtFld.FValue := StrToDateTime(S, DataDef.FormatSettings.FS)
          else
            ATxtFld.FValue := Null;
        atBlob:
          ATxtFld.FValue := FDHex2Bin(S);
        atMemo:
          ATxtFld.FValue := S;
        end;
      except
        on E: EConvertError do
          FDException(BatchMove, [S_FD_LComp, S_FD_LComp_PDM], er_FD_DPBadTextFmt,
            [AStr, ATxtFld.DisplayName, E.Message]);
      end;
  end;
...
{-------------------------------------------------------------------------------}
function TFDBatchMoveTextReader.GetFieldValue(AField: TObject): Variant;
begin
  Result := TFDTextField(AField).FValue;
end;
...
{-------------------------------------------------------------------------------}
procedure TFDBatchMoveTextWriter.SetFieldValue(AField: TObject;
  const AValue: Variant);
begin
  TFDTextField(AField).FValue := AValue;
end;
