    ...
    if piCreateParams in Instrs then begin
      ...
      if oPar.ParamType in [ptUnknown, ptInput] then
        if FInIntoEscape then
          if (iPar = -1) and (ConnMetadata.Kind = TFDRDBMSKinds.Firebird) then
...
begin
  ...
  if cCh = '?' then
    ...
    if piExpandParams in Instrs then
      if oPar = nil then
        oPar := FParams.FindParam(Params.Markers.Count + 1);
      if oPar <> nil then
        Params.Markers.Add(oPar.Name)
      else
        Params.Markers.Add('');
      case FParamMark of
      prName,
    ...
    #13, #10:
      begin
        if not FInComment1 and FInComment2 then
          FInComment2 := False;
        if not FInStr1 and not FInStr2 and (piTransformEOLs in Instrs) then
...
begin
  ...
  while iPos <= iLen do begin
    ...
    '/':
      begin
        Inc(iPos);
        if not FInComment1 and not FInComment2 and not FInStr1 and not FInStr2 and (FInNames = []) and (iPos <= iLen) and (FDestination[iPos] = '*') then begin
    ...
    '*':
      begin
        Inc(iPos);
        if not FInComment2 and not FInStr1 and not FInStr2 and (FInNames = []) and (iPos <= iLen) and (FDestination[iPos] = '/') then
          if FInMySQLConditional then
            FInMySQLConditional := False
          else
            FInComment1 := False
    ...
    '-':
      begin
        Inc(iPos);
        if not FInComment1 and not FInStr1 and not FInStr2 and (FInNames = []) and (iPos <= iLen) and (FDestination[iPos] = '-') then begin
          if (FBraceLevel = 0) and (iFromStartedPos <> 0) and (iFromEndedPos = 0) then
            iFromEndedPos := iPos - 2;
          FInComment2 := True;
        end
    ...
    'A' .. 'Z', 'a' .. 'z':
      if not FInComment1 and not FInComment2 and not FInStr1 and not FInStr2 and (FInNames = []) and
         (FBraceLevel = 0) and
         ((iPos = 1) or not FDInSet(FDestination[iPos - 1], ['a' .. 'z', 'A' .. 'Z', '0' .. '9', '#', '$', '_', '@'])) then begin
