...
{$ENDIF}
  ...
  if DriverKind in [dkSQLSrv, dkSQLNC, dkSQLOdbc, dkFreeTDS, dkSQLSrvOther, dkSQLAnywhere,
                    dkAdvantage, dkMSAccessJet, dkMSAccessACE, dkExcel, dkInformix, dkSybaseASE] then
...
begin
  if ODBCConnection.DriverKind = dkQuickBooks then begin
...
begin
  ...
  dkExcel:
    if Result = SQL_DECIMAL then
      ...
      Result := SQL_SMALLINT
    else if Result = SQL_TINYINT then
      Result := SQL_SMALLINT;
  dkDB2,
  dkDB2_400:
  ...
  SQL_DECFLOAT:
    begin
      ...
      if (soMoney in ODBCConnection.FSupportedOptions) and
        ...
        AType := dtCurrency
      else if oFmt.IsFmtBcd(ASQLSize, ASQLDecimals) then
        AType := dtFmtBCD
      else
        AType := dtBCD;
    end;
...
var
  ...
  lMetadata, lInfxBigSer, lDB2AlwaysIdent: Boolean;
...
begin
  ...
  for i := 1 to iNumCols do begin
    ...
    try
      ...
      if (ODBCConnection.ODBCConnection.DriverODBCVersion >= ovODBC3) and
         oConnMeta.ColumnOriginProvided then begin
      ...
      if lMetaData and not (eStrsType in [dtMemo, dtWideMemo]) then
        ...
        if eStrsType in [dtWideString, dtWideMemo] then
          case pInfo^.FSrcSQLDataType of
        ...
        else if eStrsType in [dtAnsiString, dtMemo] then
          case pInfo^.FSrcSQLDataType of
          SQL_WCHAR,
      ...
      if oConnMeta.ColumnOriginProvided then
      ...
      // Informix: driver returns improper UPDATABLE and UNSIGNED
      ...
      // DB2: driver describes only GENERATED ALWAYS AS IDENTITY as
      // AUTO_UNIQUE_VALUE = SQL_TRUE, but "ALWAYS" must be read-only
      lDB2AlwaysIdent := (ODBCConnection.DriverKind in [dkDB2, dkDB2_400]) and
        (caAutoInc in pInfo^.FAttrs);
      if (oSelItem.UPDATABLE = SQL_ATTR_READONLY) or lInfxBigSer or lDB2AlwaysIdent then
      ...
      // MSAcc: ACEODBC 2010-2016 can raise exception in ROWVER
      if (ODBCConnection.DriverKind <> dkMSAccessACE) and (oSelItem.ROWVER = SQL_TRUE) then
      ...
      if lMetaData and not (eStrsType in [dtMemo, dtWideMemo]) then
        ...
        if (pInfo^.FSrcDataType in C_FD_BlobTypes) and (pInfo^.FDestDataType in C_FD_StrTypes) and
           (iLen <> 0) then
          pInfo^.FColSize := iLen;
      end;
...
begin
  ...
  lUseNames := ((oConnMeta.Kind = TFDRDBMSKinds.MSSQL) or (ODBCConnection.DriverKind = dkSybaseASE)) and
  ...
  for i := 0 to n - 1 do begin
    ...
    if eStrsType = dtMemo then
      case pInfo^.FDestDataType of
      dtWideString: pInfo^.FDestDataType := dtAnsiString;
      dtWideMemo:   pInfo^.FDestDataType := dtMemo;
      dtWideHMemo:  pInfo^.FDestDataType := dtHMemo;
      end;
    ...
    if lUseNames and (pInfo^.FName <> '') and (pInfo^.FName[1] = '@') then
...
begin
  ...
  if fiBlobs in oFtchOpts.Items then
    Include(FStatementProps, cpLateBindedData);
  // ASA: $8000 size of a piece drammatically increases BLOB fetching performance
  if ODBCConnection.DriverKind in [dkSQLAnywhere] then
    AStmt.PieceBuffLen := C_FD_DefMaxStrSize + 1
  else
...
{$ENDIF}
    ...
    dkQuickBooks:
      ...
      // MSAcc: ACEODBC 2010-2016 can raise exception in CURSOR_TYPE
      if not (ODBCConnection.DriverKind in [dkMSAccessACE, dkExcel]) and
         (cpLateBindedData in FStatementProps) then
        AStmt.CURSOR_TYPE := SQL_CURSOR_DYNAMIC;
    end;
  end;

  if not (cpDefaultCrs in FStatementProps) then begin
    if (ODBCConnection.DriverKind <> dkOther) and
       not (GetCommandKind in [skSelectForLock, skSelectForUnLock]) then
...
begin
  ...
  AStmt.RowSetSupported := cpRowSet in FStatementProps;
end;
      ...
      if oParam.DataType <> ftCursor then begin
        ...
        if oVar <> nil then begin
          ...
          if ((iMaxNumBefDot >= 0) or (iMaxNumScale >= 0)) and (
              ...
              (oVar.ColumnSize - SQLULen(SQLLen(oVar.Scale)) < iMaxNumBefDot)) then begin
          ...
          if (iMaxStrSize >= 0) and (oVar.ColumnSize < iMaxStrSize) and not oVar.LimitedToMaxLen then begin
...
var
  ...
  oStr: TStream;
begin
  if FCommandStatement <> nil then begin
    for i := 0 to GetParams().Count - 1 do begin
      oPar := GetParams()[i];
      for j := 0 to oPar.ArraySize - 1 do
        if oPar.IsStreams[j] then begin
          oStr := oPar.AsStreams[j];
          if (oStr is TODBCLongDataStream) and
             (TODBCLongDataStream(oStr).Param.Statement.OwningObj = Self) then
            TODBCLongDataStream(oStr).Flush;
        end;
    end;
...
{-------------------------------------------------------------------------------}
function TFDPhysODBCCommand.InternalOpen(var ACount: TFDCounter): Boolean;
var
  sCmd: String;
begin
  ACount := 0;
  ...
  try
    if UseStatement then begin
      FCommandStatement.Execute(GetParams.ArraySize, 0, ACount,
    ...
    Result := Result and (GetState <> csAborting) and
      GetCursor(ACount);

    if Result then begin
      if UseStatement and (cpOutResParams in FStatementProps) then
        GetParamValues(-1, 0, -1);
      if Length(FColInfos) <> FStatement.NumResultCols then begin
        if Length(FColInfos) <> 0 then
          DestroyDescribeInfos;
        CreateDescribeInfos;
...
begin
  if (AForceClose or not GetNextRecordSet) and
    ...
    if (cpOutResParams in FStatementProps) and
       ((ODBCConnection.DriverKind <> dkTeradata) or not FStatement.NoMoreResults) then begin
      ODBCConnection.CreateMetadata(oConnMeta);
      // ASA: If after SQLExecute & paramsetsize > 1 & failed row with index < paramsetsize - 1,
    ...
    // MSSQL: No Check call is required to fix potential SQL_ERROR
        ...
        if pInfo^.FVar <> nil then begin
          ...
          if pInfo^.FOutDataType = pInfo^.FDestDataType then
            ...
            if lIsVal then begin
              if pInfo^.FOutDataType in C_FD_WideTypes then
                iSize := iSize * SizeOf(WideChar);
              FBuffer.Check(iSize);
  ...
  begin
    ...
    if ODBCConnection.DriverKind = dkInformix then begin
      if rName.FBaseObject <> '' then
        rName.FBaseObject := '"' + rName.FBaseObject + '"';
      if rName.FObject <> '' then
        rName.FObject := '"' + rName.FObject + '"';
    end;
  ...
  try
    ...
    mkTables:
      begin
        ...
        if ((ODBCConnection.DriverKind = dkExcel) or (osSystem in GetObjectScopes)) and
           (tkTable in GetTableKinds) then
    ...
    on E: EODBCNativeException do begin
      ODBCConnection.CreateMetadata(oConnMeta);
      if (ODBCConnection.DriverKind in [dkMSAccessJet, dkMSAccessACE, dkExcel]) and (E.Errors[0].ErrorCode = -1034) or
...
var
  ...
  begin
    if not (npSchema in oConnMeta.NameParts) then
      ...
      if AnsiCompareText(sSch, oConnMeta.UnQuoteObjName(ODBCConnection.GetCurrentSchema)) = 0 then
        AScope := osMy
  ...
  function AdjustOverloaded(var AOverload: Integer): Boolean;
  var
    pCh: PWideChar;
  begin
    Result := False;
    ...
    while (pCh >= pData) and (pCh^ <> ';') do
      Dec(pCh);
    if (pCh >= pData) and (pCh^ = ';') then begin
      Inc(pCh);
      if (pCh^ = '0') or (pCh^ = '1') then
        Dec(iSize, 2)
      else begin
        Result := True;
        FDStr2Int(pCh, iSize - (pCh - PWideChar(pData)), @AOverload, SizeOf(AOverload));
      end;
    end;
  end;

  function GetStmtData(AColIndex: Integer; ATrimZeros: Boolean = False): Boolean;
  var
    pDest: Pointer;
  begin
    pData := FBuffer.Ptr;
    if FStatement.ColumnList[AColIndex].GetData(ARowIndex, pData, iSize) then begin
      Result := True;
      if ATrimZeros then begin
        // Workaround for some ODBC drivers, which return #0 instead of NULL
      ...
      if Result and (FStatement.ColumnList[AColIndex].CDataType = SQL_C_CHAR) then begin
        pDest := nil;
        iSize := ODBCConnection.ODBCConnection.Encoder.Encode(pData, iSize, pDest, ecANSI, ecUTF16);
        pData := pDest;
      end;
    end
    else
    ...
    mkTables:
      begin
        ...
        if GetStmtData(1, True) then
          oRow.SetData(2, pData, iSize);
        iSize := 0;
        if GetStmtData(2) then
          oRow.SetData(3, pData, iSize);
        iSize := 0;
        eTableKind := tkTable;
        eScope := osOther;
        if GetStmtData(3) then begin
          ...
          if StrLIComp(PWideChar(pData), 'TABLE', iSize) = 0 then
            ...
            eTableKind := tkTable;
            if ODBCConnection.DriverKind <> dkExcel then
              eScope := osSystem;
          end
    ...
    mkTableTypeFields:
      begin
        ...
        if GetStmtData(2) then
          oRow.SetData(3, pData, iSize);
        iSize := 0;
        if GetStmtData(3) then
          oRow.SetData(4, pData, iSize);
        iSize := 0;
        if FStatement.ColumnList.Count >= 17 then begin
          if GetStmtData(16) then begin
            // Informix: ORDINAL_POSITION is defined as SQL_SMALLINT instead of SQL_INTEGER.
            // TData: ORDINAL_POSITION is defined as SQL_BIGINT instead of SQL_INTEGER.
            case FStatement.ColumnList[16].CDataType of
            SQL_C_STINYINT,
            SQL_C_UTINYINT,
            SQL_C_TINYINT:
              oRow.SetData(5, PSQLByte(pData)^);
            SQL_C_SSHORT,
            SQL_C_USHORT,
            SQL_C_SHORT:
              oRow.SetData(5, PSQLSmallint(pData)^);
            SQL_C_SLONG,
            SQL_C_ULONG,
            SQL_C_LONG:
            ...
            SQL_C_SBIGINT,
            SQL_C_UBIGINT:
              oRow.SetData(5, PSQLBigInt(pData)^);
            else
              ASSERT(False);
            end;
          end;
                                                    ...
                                                    
  {
    ...
    mkIndexFields:
      begin
        ...
        if GetStmtData(6) and
          ...
          iSize := 0;
          if GetStmtData(7) then
            oRow.SetData(6, PSQLSmallint(pData)^);
          iSize := 0;
          if GetStmtData(9) then
            oRow.SetData(7, pData, iSize);
          iSize := 0;
          if GetStmtData(12) then
        ...
        if soPKFunction in ODBCConnection.FSupportedOptions then begin
          ...
          if not lDeleteRow then begin
            ...
            oConnMeta.DecodeObjName(Trim(GetCatalogName), rName, Self,
              [doNormalize, doUnquote]);
            if rName.FObject = '' then
              rName.FObject := ODBCConnection.ODBCConnection.CURRENT_CATALOG;
    ...
    mkForeignKeys:
      begin
        oRow.SetData(0, iRecNo);
        iSize := 0;
        if GetStmtData(4, True) then
          oRow.SetData(1, pData, iSize);
        iSize := 0;
        if GetStmtData(5, True) then
          oRow.SetData(2, pData, iSize);
        iSize := 0;
        if GetStmtData(6) then
          oRow.SetData(3, pData, iSize);
        iSize := 0;
        if GetStmtData(11) then
          oRow.SetData(4, pData, iSize);
        iSize := 0;
        if GetStmtData(0) then
          oRow.SetData(5, pData, iSize);
        iSize := 0;
        if GetStmtData(1) then
    ...
    mkProcs:
      begin
        ...
        if GetStmtData(1, True) then
          oRow.SetData(2, pData, iSize);
        iSize := 0;
        if GetStmtData(2) then begin
          if (oConnMeta.Kind = TFDRDBMSKinds.MSSQL) and AdjustOverloaded(i) then
            oRow.SetData(5, @i, SizeOf(i));
          oRow.SetData(4, pData, iSize);
        end;
        iSize := 0;
        if GetStmtData(7) and
    ...
    mkProcArgs:
      begin
        ...
        if FStatement.ColumnList.Count >= 14 then begin
          if GetStmtData(13) and (iSize > 0) then
            Include(eAttrs, caDefault);
        end;
        iSize := 0;
        if not GetStmtData(11) or
           (PSQLSmallint(pData)^ <> SQL_NO_NULLS) then
          Include(eAttrs, caAllowNull);
        iSize := 0;
        if GetStmtData(5) then
