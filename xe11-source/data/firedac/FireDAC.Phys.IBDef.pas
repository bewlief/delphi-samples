...
uses
  System.Types, System.SysUtils, System.Classes, FireDAC.Stan.Intf, FireDAC.Phys.IBWrapper;
...
type
  ...
  private
    ...
    function GetGUIDEndian: TEndian;
    procedure SetGUIDEndian(const AValue: TEndian);
  published
    ...
    property GUIDEndian: TEndian read GetGUIDEndian write SetGUIDEndian stored False;
  end;
...
begin
  FDef.AsString[S_FD_ConnParam_IB_PageSize] := C_PageSize[Integer(AValue)];
end;
...
{-------------------------------------------------------------------------------}
function TFDPhysIBConnectionDefParams.GetGUIDEndian: TEndian;
var
  s: String;
begin
  s := FDef.AsString[S_FD_ConnParam_Common_GUIDEndian];
  if CompareText(s, S_FD_Little) = 0 then
    Result := Little
  else if CompareText(s, S_FD_Big) = 0 then
    Result := Big
  else
    Result := Little;
end;
...
{-------------------------------------------------------------------------------}
procedure TFDPhysIBConnectionDefParams.SetGUIDEndian(const AValue: TEndian);
const
  C_GUIDEndian: array[TEndian] of String = (S_FD_Big, S_FD_Little);
begin
  FDef.AsString[S_FD_ConnParam_Common_GUIDEndian] := C_GUIDEndian[AValue];
end;
