...
begin
  ...
  if Assigned(FDGUIxCreateMemoFunc) then
    mmScript := TFDGUIxFormsMemo(FDGUIxCreateMemoFunc(Self))
  else
    mmScript := TFDGUIxFormsMemo.Create(Self);
  Supports(mmScript, IFDGUIxFormsMemo, mmScriptIntf);
  mmScript.Parent := pnlSQL;
  mmScript.Align := alClient;
  if Assigned(mmScriptIntf) then begin
    mmScriptIntf.SetOnKeyUp(mmScriptKeyUp);
    mmScriptIntf.SetOnMouseUp(mmScriptMouseUp);
//    mmScriptIntf.GetFont.Size := 9;
  end
  else begin
    TFDGUIxFormsMemo(mmScript).OnKeyUp := mmScriptKeyUp;
    TFDGUIxFormsMemo(mmScript).OnMouseUp := mmScriptMouseUp;
    TFDGUIxFormsMemo(mmScript).Font.Size := 9;
  end;
...
begin
  ...
  oCanvas.TextRect(Rect, Rect.Left + 6, Rect.Top + 3, oGrid.Cells[ACol, ARow]);
end;
...
begin
  if Assigned(mmScriptIntf) then begin
    scrEngine.SQLScripts[0].SQL.Text := mmScriptIntf.GetLines.Text;
    scrEngine.Position := mmScriptIntf.GetCaretPos;
  end
  else begin
    scrEngine.SQLScripts[0].SQL.Text := TFDGUIxFormsMemo(mmScript).Lines.Text;
    scrEngine.Position := TFDGUIxFormsMemo(mmScript).CaretPos;
  end;
end;
...
begin
  if Assigned(mmScriptIntf) then begin
    if not scrEngine.Eof then
      mmScriptIntf.SetCaretPos(scrEngine.Position)
    else
      mmScriptIntf.SetSelStart(mmScript.GetTextLen);
  end
  else begin
    if not scrEngine.Eof then
      TFDGUIxFormsMemo(mmScript).CaretPos := scrEngine.Position
    else
      TFDGUIxFormsMemo(mmScript).SelStart := mmScript.GetTextLen;
  end;
end;
...
begin
  if (mmLog.CaretPos.Y >= 0) and (mmLog.CaretPos.Y < mmLog.Lines.Count) then begin
    ...
    if i >= 0 then
      if Assigned(mmScriptIntf) then
        mmScriptIntf.SetCaretPos(Point(0, i))
      else
        TFDGUIxFormsMemo(mmScript).CaretPos := Point(0, i);
  end;
end;
...
begin
  if Assigned(mmScriptIntf) then
    mmScriptIntf.SetCaretPos(Point(0, 0))
  else
    TFDGUIxFormsMemo(mmScript).CaretPos := Point(0, 0);
...
begin
  if Assigned(mmScriptIntf) then
    rPos := mmScriptIntf.GetCaretPos
  else
    rPos := TFDGUIxFormsMemo(mmScript).CaretPos;
  scrEngine.ExecuteStep;
  if scrEngine.TotalErrors <> 0 then
    if Assigned(mmScriptIntf) then
      mmScriptIntf.SetCaretPos(rPos)
    else
      TFDGUIxFormsMemo(mmScript).CaretPos := rPos;
end;
...
begin
  ...
  iHeight := ScaleValue(C_TitleHeight);
  BorderStyle := bsNone;
  sgParams.Top := sgParams.Top - (pnlTitle.Height - iHeight);
  sgParams.Height := sgParams.Height + (pnlTitle.Height - iHeight);
  sgParams.Color := clBtnFace;
  pnlTitle.Height := iHeight;
