...
begin
  ...
  if (AServerVersion = 0) or (AServerVersion >= mvMySQL050600) then
    sKwd := sKwd +
      ...
      'WORK,X509,XML,' +
      'YEAR';
  if (AServerVersion = 0) or (AServerVersion >= mvMariaDB100300) then
    sKwd := sKwd +
      ',INTERSECT,EXCEPT';
...
begin
  ...
  lPrepStmt := AProc[1] = '*';
  if lPrepStmt then
    rName.FObject := Copy(AProc, 2, MaxInt)
  else
    rName.FObject := AProc;
  ...
  for i := 0 to FParams.Count - 1 do begin
    ...
    if lPrepStmt then
      ...
      if oParam.ParamType in [ptInputOutput, ptOutput] then begin
        ...
        ftSmallint, ftInteger, ftAutoInc, ftLargeint, ftShortint:
          sCast := 'SIGNED';
        ftWord, ftLongWord, ftByte:
          sCast := 'UNSIGNED';
    ...
    if lPrepStmt and (oParam.ParamType <> ptResult) or
       not lPrepStmt and (oParam.ParamType in [ptUnknown, ptInput]) then
      if sFUNC = '' then
        sFUNC := sName + '(' + sVar
      else
        sFUNC := sFUNC + ', ' + sVar;
...
begin
  ...
  mkPrimaryKey:
    begin
      ...
      Result := Result + ' FROM ' + EncodeName();
      if AWildcard <> '' then
        Result := Result + ' WHERE Key_name LIKE ''' + AWildcard + '''';
    end;
  mkIndexFields,
  mkPrimaryKeyFields:
    begin
