    ...
    efTIMESTAMPDIFF:
      begin
        ...
        if (A1 = 'MONTH') and FUseMonthsBetween then
          ...
          Result := 'ROUND(EXTRACT(EPOCH FROM CAST(' + A3 + ' AS TIMESTAMP)) / 3600 - ' +
                          'EXTRACT(EPOCH FROM CAST(' + A2 + ' AS TIMESTAMP)) / 3600)'
        else if A1 = 'MINUTE' then
          Result := 'ROUND(EXTRACT(EPOCH FROM CAST(' + A3 + ' AS TIMESTAMP)) / 60 - ' +
                          'EXTRACT(EPOCH FROM CAST(' + A2 + ' AS TIMESTAMP)) / 60)'
        else if A1 = 'SECOND' then
          Result := '(EXTRACT(EPOCH FROM CAST(' + A3 + ' AS TIMESTAMP)) - ' +
                     'EXTRACT(EPOCH FROM CAST(' + A2 + ' AS TIMESTAMP)))'
        else if A1 = 'FRAC_SECOND' then
          Result := '((EXTRACT(EPOCH FROM CAST(' + A3 + ' AS TIMESTAMP)) - ' +
                     'EXTRACT(EPOCH FROM CAST(' + A2 + ' AS TIMESTAMP))) * 1000000)'
        else
...
function TFDPhysPgCommandGenerator.GetStoredProcCall(const ACatalog, ASchema,
  ...
  procedure ErrorCantResult;
  begin
    FDException(Self, [S_FD_LPhys, S_FD_PGId], er_FD_PgProcCantResult, [AProc]);
  end;
...
var
  ...
  begin
    ...
    if FConnMeta.ServerVersion >= svPGSQL100000 then
      sAdsrc := 'PG_GET_EXPR(D.ADBIN, D.ADRELID)'
    else
      sAdsrc := 'D.ADSRC';
    Result := Result +
      ...
      'CASE WHEN UPPER(SUBSTR(' + sAdsrc + ', 1, 8)) = ''NEXTVAL('' THEN ' + Attrs([caAutoInc]) +
  ...
  function EncodeProcKind: String;
  begin
    if FConnMeta.ServerVersion >= svPGSQL110000 then
      Result := 'CASE WHEN P.PROKIND = ''p'' THEN ' + IntToStr(Integer(pkProcedure)) +
        ' ELSE ' + IntToStr(Integer(pkFunction)) + ' END'
    else
      Result := IntToStr(Integer(pkFunction));
  end;
...
begin
  ...
  case AKind of
    ...
    mkTables:
      begin
        Result := 'SELECT NULL::INTEGER AS RECNO, CURRENT_DATABASE() AS CATALOG_NAME, ' +
          ...
          ' WHEN C.RELKIND = ''v'' OR C.RELKIND = ''m'' THEN ' + IntToStr(Integer(tkView)) +
          ' ELSE 0 ' +
    ...
    mkProcArgs:
      begin
        Result := 'SELECT P.PRORETSET, ';
        if FConnMeta.ServerVersion >= svPGSQL110000 then
          Result := Result + 'CASE WHEN P.PROKIND = ''p'' THEN 0::oid ELSE P.PRORETTYPE END AS PRORETTYPE'
        else
          Result := Result + 'P.PRORETTYPE';
        Result := Result + ',' +
