...
type
  ...
  TFDPhysIBMetadataExt = class (TFDPhysIBMetadata)
  protected
...
begin
  ...
  if FBrand = ibFirebird then begin
    sKwds := sKwds + ',' +
      ...
      // Firebird 4.0
      'BINARY,DECFLOAT,INT128,LATERAL,LOCAL,LOCALTIME,LOCALTIMESTAMP,PUBLICATION,' +
      'RDB$GET_TRANSACTION_CN,RDB$ERROR,RDB$ROLE_IN_USE,RDB$SYSTEM_PRIVILEGE,' +
      'RESETTING,TIMEZONE_HOUR,TIMEZONE_MINUTE,UNBOUNDED,VARBINARY,WINDOW,WITHOUT';
   end;
...
{-------------------------------------------------------------------------------}
function TFDPhysIBMetadataExt.GetAsyncNativeTimeout: Boolean;
begin
  Result := (FBrand = ibFirebird) and (GetClientVersion >= ivFB040000);
end;
