...
{$ENDIF}
  ...
  if InternalGetSharedCliHandle() <> nil then begin
    ...
    FServerVersion := FServer.ServerVersion;
  end
...
var
  oStmt: TOCIStatement;
  oVar1, oVar2, oVar3, oVar4, oVar5, oVar6: TOCIVariable;
  s: String;
...
{$ENDIF}
  try
    ...
    try
      ...
      oVar2 := oStmt.AddVar(':NUM', odOut, otNString, 64);
      oVar3 := oStmt.AddVar(':BTS', odOut, otInt32);
      oVar4 := oStmt.AddVar(':USR', odOut, otNString, 64);
      oVar5 := oStmt.AddVar(':CHS', odOut, otNString, 64);
      oVar6 := oStmt.AddVar(':NCS', odOut, otNString, 64);
      oStmt.Execute(1, 0, False, False);
      FUseDBAViews := (oVar1.AsInteger = 2);
      s := oVar2.AsString;
      if Length(s) > 0 then
        FDecimalSep := s[1];
      FService.BytesPerChar := ub1(oVar3.AsInteger);
      if FCurrentSchema = '' then
        FCurrentSchema := oVar4.AsString;
      FDBCharacterSet := oVar5.AsString;
      FNDBCharacterSet := oVar6.AsString;
    finally
      FDFree(oVar1);
      FDFree(oVar2);
      FDFree(oVar3);
...
{$ELSE}
        AValue := AdjustLineBreaks(FServer.ServerVersionStr, tlbsCRLF);
{$ENDIF}
        AKind := ikSessionInfo;
      end;
    6:
...
{$IFDEF MSWINDOWS}
  // 6. 12.1 client has major resource & memory leak on Windows
  if (FEnv.Lib.Version >= cvOracle121000) and (FEnv.Lib.Version < cvOracle122000) then
    AMessages.Add(Format(S_FD_OracleWarnLeak, [FEnv.Lib.VersionStr]));
{$ENDIF}
end;
...
{-------------------------------------------------------------------------------}
procedure TFDPhysOracleTransaction.UpdateInProgress;
begin
  if (OraConnection.FServerVersion >= cvOracle120000) and
     (OraConnection.FEnv.Lib.Version >= cvOracle120000) then
    FInProgress := OraConnection.FSession.TRANSACTION_IN_PROGRESS <> 0;
end;
...
begin
  ...
  if (OraConnection.FServerVersion >= cvOracle120000) and
    ...
    lInTx := FInProgress;
    UpdateInProgress;
    if (lInTx <> FInProgress) and (GetActive <> FInProgress) then
      if FInProgress then
        TransactionStarted
      else
        TransactionFinished;
...
begin
  ...
  otDouble:
    begin
      ...
      if (AType = dtFmtBCD) and not oFmt.IsFmtBcd(APrec, AScale) then
        AType := dtBCD;
    end;
  otDateTime:
    AScale := 1000;
...
function TFDPhysOracleCommand.FDType2OCIType(AFDType: TFDDataType;
  AFixedLen, APLSQL, AInput: Boolean): TOCIVarDataType;
var
...
begin
  if AFDType = dtBoolean then begin
    ...
    Result := addt2ovdt[AFDType];
    if AFixedLen and not (APLSQL and AInput) then
...
begin
  ...
  try
    oDescr.BoolType := FDType2OCIType(dtBoolean, False, True, True);
    oDescr.BindByName := GetParams.BindMode = pbByName;
    oDescr.MaxNameLength := oConnMeta.ParamNameMaxLength;
  ...
  for i := 1 to Length(ACrsInfo^.FColInfos) do begin
    ...
    try
      ...
      if lMetadata then
        ...
        else if pInfo^.FSrcType = dtMemo then
          pInfo^.FDestType := dtWideMemo
        else
      ...
      pInfo^.FOOutputType := FDType2OCIType(pInfo^.FDestType,
        pInfo^.FOSrcType in [otChar, otNChar], False, False);
...
begin
  ...
  for i := 0 to Length(FParInfos) - 1 do begin
    ...
    pInfo^.FOSrcType := FDType2OCIType(pInfo^.FSrcType,
      ...
      oParam.ParamType in [ptInput, ptInputOutput]);
    pInfo^.FOOutputType := FDType2OCIType(pInfo^.FDestType,
    ...
    if (pInfo^.FOOutputType in otHBlobs) and (pInfo^.FVarType in [odIn, odOut, odInOut]) and
       ((TFDPhysOracleConnection(FConnectionObj).FEnv.Lib.Version < cvOracle81000) or
        oFmtOpts.DataSnapCompatibility) and
       (FBase.FStmt.STMT_TYPE in [OCI_STMT_UPDATE, OCI_STMT_INSERT]) and
       (FBase.FStmt.IS_RETURNING <> 0) then begin
...
begin
  ...
  AStmt.MapWC2NChar := not oFmtOpts.ADOCompatibility;
end;
...
{-------------------------------------------------------------------------------}
function TFDPhysOracleCommand.InternalOpen(var ACount: TFDCounter): Boolean;
var
  oFO: TFDFetchOptions;
  pCrsInfo: PFDOraCrsDataRec;
...
{-----------------------------------------------------------------------------}
initialization
  FDRegisterDriverClass(TFDPhysOracleDriver);

finalization
  FDUnregisterDriverClass(TFDPhysOracleDriver);
