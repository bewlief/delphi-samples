...
{-------------------------------------------------------------------------------}
function TFDPhysMSSQLMetadata.GetLimitOptions: TFDPhysLimitOptions;
begin
...
{-------------------------------------------------------------------------------}
function TFDPhysMSSQLMetadata.InternalDecodeObjName(const AName: String;
  out AParsedName: TFDPhysParsedName; const ACommand: IFDPhysCommand;
...
begin
  ...
  if Result then begin
    i := Pos(';', AParsedName.FObject);
    if i > 0 then begin
      AParsedName.OverInd := StrToInt(Trim(Copy(AParsedName.FObject, i + 1, MaxInt)));
      AParsedName.FObject := Copy(AParsedName.FObject, 1, i - 1);
    end;
  end;
end;

{-------------------------------------------------------------------------------}
function TFDPhysMSSQLMetadata.InternalEncodeObjName(
  const AParsedName: TFDPhysParsedName; const ACommand: IFDPhysCommand): String;
begin
  Result := inherited InternalEncodeObjName(AParsedName, ACommand);
  if AParsedName.OverInd > 1 then
    Result := Result + ';' + IntToStr(AParsedName.OverInd);
end;
...
begin
  if (GetSQLOrderByPos > 0) and (FConnMeta.ServerVersion >= svMSSQL2012) then begin
    s := UpperCase(ASQL);
    if not (FDHasKW(s, 'UNION') or FDHasKW(s, 'INTERSECT') or FDHasKW(s, 'EXCEPT')) then
      ...
      Result := inherited GetLimitSelect(ASQL, ASkip, ARows, AOptions);
  end
...
begin
  ...
  maInsertIgnore:
    Result := 'IF NOT EXISTS (SELECT 1 FROM ' + GetFrom() + ' WHERE ' +
