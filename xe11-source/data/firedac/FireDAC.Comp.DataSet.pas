...
type
  ...
  private
    ...
    function CheckDetailsFetched: Boolean;
  protected
...
{$ENDIF !NEXTGEN}
    ...
    function GetModifiedBlobCached: Boolean; override;
...
{$ENDIF !NEXTGEN}
    ...
    procedure DoAfterOpenOrExecute; virtual;
    ...
    procedure DoMasterClearDetails(AAll: Boolean); virtual;
    ...
    procedure DoGetStorageMergeModes(var AData: TFDMergeDataMode;
      var AMeta: TFDMergeMetaMode); virtual;
    ...
    procedure DoResetAtLoading; virtual;
...
{$ENDIF}
    ...
    procedure ParseIndexFieldNames(out AOrderFields, ADescFields,
      ANoCaseFields: String);
...
begin
  ...
  if (Event = deCheckBrowseMode) and
     (DetailDataSet.State in dsEditModes) and (DataSet.State in dsEditModes) then
    Exit;
...
begin
  oFld := FDataSet.GetColumnField(AColumn);
  if oFld <> nil then begin
    if (oFld.DataType = ftAutoInc) or (oFld.AutoGenerateValue <> Data.DB.arNone) then begin
...
begin
  if fiDetails in FetchOptions.Cache then begin
    if LocalIndexName <> '' then begin
      ...
      ParseIndexFieldNames(Result, sTemp, sTemp);
  end
  else
    Result := DetailFields;
end;
...
{-------------------------------------------------------------------------------}
function TFDDataSet.CheckDetailsFetched: Boolean;
var
  ePrevFilter: TFDDatSRowStates;
begin
  Result := FSourceView.Rows.Count <> 0;
  if not Result and UpdatesPending then begin
    ePrevFilter := FSourceView.RowStateFilter;
    try
      FSourceView.RowStateFilter := [rsModified, rsInserted, rsDeleted, rsUnchanged];
      Result := FSourceView.Rows.Count <> 0;
    finally
      FSourceView.RowStateFilter := ePrevFilter;
    end;
  end;
end;
...
begin
  if not FetchOptions.DetailOptimize or Assigned(OnMasterSetValues) or
     not CheckDetailsFetched then begin
    Result := True;
    Exit;
  end;
  Result := False;
...
{-------------------------------------------------------------------------------}
procedure TFDDataSet.DoMasterClearDetails(AAll: Boolean);
begin
  ClearBuffers;
  if (FTable <> nil) and (FSourceView <> nil) then
    if AAll or not DoMasterParamDependent(FMasterLink.Fields) then
      FTable.Clear
    else
      FSourceView.DeleteAll(True);
  FSourceEOF := True;
end;
...
procedure TFDDataSet.CheckDetailRecords(AEnforce: Boolean);
  ...
  begin
    Result := False;
    if not DoIsSourceOnline then
      Exit;

    if DoMasterParamDependent(FMasterLink.Fields) then begin
      if not DoMasterParamChanged(FMasterLink.Fields) then
        Exit;
    end
    else begin
      if not ((State = dsInactive) or AEnforce or
              not (fiDetails in FetchOptions.Cache) and not CheckDetailsFetched) then
        Exit;
    end;

    if State <> dsInactive then
      DoBeforeClose;
    DoMasterClearDetails(not (fiDetails in FetchOptions.Cache));
    DoCloseSource;
...
begin
  ...
  if DataSetField = nil then begin
    if AEnforce or not FetchOptions.DetailOptimize or
       not MasterSource.DataSet.IsEmpty and
       not (MasterSource.DataSet.State in [dsInsert, dsSetKey]) then begin
      if fiDetails in FetchOptions.Cache then begin
        // Rows were assigned using Data property
        if not Active and (FTable <> nil) and (FTable.Rows.Count <> 0) then
          LChanged := True
        else if not Active or (FSourceView = nil) or not CheckDetailsFetched then
      ...
      DoMasterClearDetails(True);
      lChanged := True;
    end;
    if Active and lChanged then begin
      if IsUniDirectional and IsEmpty then
        ActivateBuffers;
      First;
    end;
  end
...
begin
  if IsLinkedDetail then begin
    ...
    if (fiDetails in FetchOptions.Cache) or not DoMasterParamDependent(FMasterLink.Fields) then begin
...
var
  ...
  oMechChilds: TFDDatSMechChilds;
begin
  if not Active then
    Exit;
  oParentRow := GetParentRow;
  if oParentRow <> FLastParentRow then begin
    if FetchOptions.Mode in [fmOnDemand, fmAll, fmExactRecsMax] then
      CheckDetailRecords(False);
    if (FFilterView <> nil) and (FFilterView.Mechanisms[0] is TFDDatSMechChilds) then begin
      FLastParentRow := oParentRow;
      oMechChilds := TFDDatSMechChilds(FFilterView.Mechanisms[0]);
      oMechChilds.RebuildForRow(oParentRow);
    end
    else
...
begin
  ...
  if (FetchOptions.Mode in [fmOnDemand, fmAll, fmExactRecsMax]) and
     (State <> dsOpening) and lDeferOpen then begin
    MasterChanged(Self);
    FSourceEOF := False;
  end;
...
begin
  if FetchOptions.RecordCountMode = cmTotal then
    GetRecordCount;
  if (dfCalcFieldInIndex in FFlags) or (IsEmpty and (FSourceView.Rows.Count > 0)) then
    Resync([]);
  for i := 0 to FEventListeners.Count - 1 do
...
begin
  if DoIsSourceAsync then
    try
      ...
      TThread.Synchronize(nil, OpenCursorComplete);
    except
...
begin
  if DoIsSourceAsync then
    TThread.Synchronize(nil, DoAfterExecute);
end;

{-------------------------------------------------------------------------------}
procedure TFDDataSet.DoAfterOpenOrExecute;
begin
  // nothing
end;
...
begin
  ...
  try
    if Active and not SourceEOF and DoIsSourceOpen and not IsUnidirectional and
...
{$IFNDEF NEXTGEN}
procedure TFDDataSet.ClearCalcFields(Buffer: TRecordBuffer);
begin
  ClearCalcFields(TRecBuf(Buffer));
end;
{$ENDIF !NEXTGEN}
...
{$IFNDEF NEXTGEN}
procedure TFDDataSet.InternalInitRecord(Buffer: TRecordBuffer);
begin
...
begin
  ...
  if (pInfo^.FBookmarkFlag in [bfBOF, bfEOF, bfInserted]) and
     ...
     (FNewRow <> nil) then
    Result := FNewRow
  else if (pInfo^.FRow <> nil) and (pInfo^.FRow.RowState = rsDestroying) and not (dfDeleting in FFlags) then
    Result := nil
  else
    Result := pInfo^.FRow;
end;
  ...
  cmTotal:
    begin
      if FRecordCount = -1 then begin
        UpdateParentCursorPos;
        if not (State in [dsInactive, dsOpening]) and (
             (FParentDataSet <> nil) and (FTable.Manager <> nil) or
...
begin
  Dec(AValue);
  if AValue < FUnidirRecsPurged then
    FRecordIndex := 0
  else begin
    while (AValue >= FSourceView.Rows.Count + FUnidirRecsPurged) and
          InternalFetchRows(False, True) do
      ;
    if AValue >= FSourceView.Rows.Count + FUnidirRecsPurged then
...
var
  ...
  procedure ErrorVarLenDataMismatch(ADataLen, AMaxLen: Integer);
  begin
    FDException(Self, [S_FD_LDatS], er_FD_VarLenDataMismatch, [oColumn.Name, ADataLen, AMaxLen]);
  end;
  ...
  begin
    ...
    else if LongWord(ADataLen) > oColumn.Size then
      if FormatOptions.StrsTrim2Len then
        Result := oColumn.Size
      else
        ErrorVarLenDataMismatch(ADataLen, oColumn.Size);
  end;
    ...
    if caFixedLen in oColumn.Attributes then begin
      ...
      if (ADataLen = 0) and FormatOptions.StrsEmpty2Null then
        ...
        if ADataLen > High(Word) then
          ErrorVarLenDataMismatch(ADataLen, High(Word));
...
begin
  ...
  dtRowRef, dtArrayRef:
    ...
    if Result and (ABuffer <> nil) then
      ...
      dtInt32:            PInteger(ABuffer)^ := PInteger(pData)^;
      dtUInt32:           PCardinal(ABuffer)^ := PCardinal(pData)^;
...
begin
  lCheckUpdatable := (FDataLoadSave = nil) and not IsForceRowMode and UpdateOptions.CheckUpdatable;
  ...
  if iFieldNo > 0 then begin
    if lCheckUpdatable then begin
      if (State = dsCalcFields) and (AField.FieldKind <> fkInternalCalc) then
        NotEditingError;
      if not (State in [dsSetKey, dsFilter]) then begin
        if AField.ReadOnly and UpdateOptions.CheckReadOnly then
          FieldROError;
    ...
    if GetFieldColumn(pRecBuf, iFieldNo, oColumn, iColIndex, oRow, True) then begin
      ...
      if lEndEdit then
        oRow.EndEdit(True);
      if (State = dsSetKey) and (oRow <> GetLocateRow) then begin
...
{-------------------------------------------------------------------------------}
function TFDDataSet.GetModifiedBlobCached: Boolean;
begin
  Result := False;
end;
...
begin
  ...
  if ((FParentDataSet <> nil) or not SourceEOF) and
    ...
    if FParentDataSet <> nil then
      ...
      try
        if DoIsSourceAsync then
          TThread.Synchronize(nil, DoBeforeGetRecords)
        else
          DoBeforeGetRecords;
        ...
        if FTable <> nil then begin
          if not lFetched and (ADirection = fdDown) then
            FSourceEOF := True;
          if lUnidir and (FSourceView.Rows.Count = 0) then begin
            ClearBuffers;
            SetBof(FUnidirRecsPurged = 0);
          end;
          iToRec := FTable.Rows.Count - 1;
          if DoIsSourceAsync then
            TThread.Synchronize(nil, DoAfterGetRecords2)
          else
...
begin
  ...
  if Result then begin
    if not (dfOpenBlob in FFlags) then begin
...
begin
  Result := ((DataSetField = nil) or DataSetField.CanModify) and
    ((FDataLoadSave <> nil) or not UpdateOptions.CheckReadOnly or not UpdateOptions.ReadOnly);
end;
  ...
  try
    ...
    if coStructure in AOptions then begin
      ...
      try
        ...
        FormatOptions.MaxBcdScale := MaxInt div 2;
        ...
        if (ParentDataSet = nil) and (DataSetField = nil) then
          Active := True;
        if Active and not (ASource is TFDDataSet) then begin
    ...
    // If process the data, then correct options, setup key fields.
    if [coAppend, coEdit, coDelete, coRefresh] * AOptions <> [] then begin
      ...
      if [coEdit, coDelete, coRefresh] * AOptions <> [] then begin
        ...
        if (oKeyFields.Count = 0) or (sKeyFields = '') then
          AOptions := AOptions - [coEdit, coDelete, coRefresh]
        else
          vKeyValues := VarArrayCreate([0, oKeyFields.Count - 1], varVariant);
      end;
    end;
    // If do not process the data, then exit.
    if [coAppend, coEdit, coDelete, coRefresh] * AOptions = [] then
      Exit;

    // Run batch update.
    BeginBatch((ctInvars in Table.Columns.HasThings) or
               ([coEdit, coDelete, coRefresh] * AOptions <> []));
    try
...
begin
  ...
  if IsRowValid(pBmk^.FRow, pBmk^.FResultSetNum) then
    i := FSourceView.IndexOf(pBmk^.FRow, rvDefault, pBmk^.FRowIndex)
  else if (pBmk^.FRowIndex >= 0) and (pBmk^.FRowIndex < FSourceView.Rows.Count) then
    i := pBmk^.FRowIndex
  else
    i := -1;
...
{$IFNDEF NEXTGEN}
procedure TFDDataSet.SetBookmarkFlag(Buffer: TRecordBuffer;
  AValue: TBookmarkFlag);
begin
  SetBookmarkFlag(TRecBuf(Buffer), AValue);
end;
{$ENDIF !NEXTGEN}
...
{-------------------------------------------------------------------------------}
procedure TFDDataSet.ParseIndexFieldNames(out AOrderFields, ADescFields, ANoCaseFields: String);
var
...
begin
  ...
  if AKeyFields = '#' then begin
    AIndex := Table.Rows.IndexOf(AKeyValues);
    if AIndex <> -1 then
      AIndex := FSourceView.IndexOf(Table.Rows[AIndex]);
    Result := AIndex <> -1;
    Exit;
  end;
...
var
  eOpts: TFDDataSetLocateOptions;
begin
  eOpts := [];
  if foCaseInsensitive in FilterOptions then
    Include(eOpts, lxoCaseInsensitive);
  Result := LookupEx(AKeyFields, AKeyValues, AResultFields, eOpts);
end;
...
begin
  ...
  if iSaveState <> dsCalcFields then begin
    EndForceRow;
    RestoreState(iSaveState);
  end;
end;
...
{-------------------------------------------------------------------------------}
function TFDDataSet.GetClonedDataSet(AWithSettings: Boolean): TDataSet;
begin
  if not ClientCursor then
    Exit(nil);
  Result := NestedDataSetClass.Create(nil);
  try
    (Result as TFDDataSet).CloneCursor(Self, False, not AWithSettings);
  except
    FDFree(Result);
    raise;
  end;
...
{-------------------------------------------------------------------------------}
procedure TFDDataSet.DoResetAtLoading;
begin
  Close;
  FieldDefs.Updated := False;
  IndexDefs.Updated := False;
...
{-------------------------------------------------------------------------------}
procedure TFDDataSet.DoGetStorageMergeModes(var AData: TFDMergeDataMode;
  var AMeta: TFDMergeMetaMode);
begin
  AData := ResourceOptions.StoreMergeData;
  AMeta := ResourceOptions.StoreMergeMeta;
end;

{-------------------------------------------------------------------------------}
procedure TFDDataSet.InternalLoadFromStorage;
var
  eDataMode: TFDMergeDataMode;
  eMetaMode: TFDMergeMetaMode;
  oTabs: TFDDatSTableArray;
  i: Integer;
begin
  DoGetStorageMergeModes(eDataMode, eMetaMode);
  if FStorage.IsStored(siMeta) then begin
    DatSManager.LoadFromStorage(FStorage, eDataMode, eMetaMode, [moPreserveState]);
    if ResetAtLoading then begin
      i := -1;
      if Name <> '' then begin
        i := DatSManager.Tables.IndexOfName(Name);
        if i >= 0 then
          AttachTable(DatSManager.Tables.ItemsI[i], nil);
      end;
      if i < 0 then begin
  ...
  if Updates <> nil then begin
    if not CachedUpdates and Updates.HasChanges(FTable) then
      if FStorage.IsStored(siDelta) then
        CachedUpdates := True
      else
        Updates.AcceptChanges(FTable);
    FLastSavePoint := Updates.SavePoint;
  end
  else
    FLastSavePoint := 0;
  FSourceEOF := True;
...
begin
  try
    ...
    if FStorage.IsStored(siCurrent) and (GetRow() <> nil) then
      FStorage.AddFilterObj(Table.Rows, GetRow());
    DatSManager.SaveToStorage(FStorage);
    if Updates <> nil then
      FLastSavePoint := Updates.SavePoint;
  finally
    FStorage.RemoveFilterObj(Table);
    FStorage.RemoveFilterObj(Table.Rows);
  end;
end;
...
begin
  Result := (DataSetField = nil) and (ParentDataSet = nil) and (DatSManager <> nil) and
    (FStorage = nil) and (FCloneSource = nil) and
    ResourceOptions.Persistent and FileExists(ResourceOptions.ResolveFileName());
end;
...
begin
  Result := (DataSetField = nil) and (ParentDataSet = nil) and (DatSManager <> nil) and
    (FStorage = nil) and (FCloneSource = nil) and
    ResourceOptions.Persistent and (ResourceOptions.ResolveFileName() <> '') and
    ((Updates <> nil) and (FLastSavePoint < Updates.SavePoint) or
...
// 10.4 1 0 1 0
begin
  Result := not ResourceOptions.StoreMerge and (siMeta in ResourceOptions.StoreItems) or
    not Active;
end;
...
begin
  ...
  if Active and (Updates <> nil) then
    Result := Updates.HasChanges(FTable);
end;
...
procedure TFDDataSet.BeginBatch(ARegular: Boolean = False);
var
...
begin
  inherited PSReset;
  if Active and FetchOptions.AutoClose and not UpdatesPending then begin
    Close;
    Open;
  end;
end;
...
var
  iPrevErrCode, iErrCode: Integer;
begin
  if Prev <> nil then
    iPrevErrCode := Prev.ErrorCode
  else
    iPrevErrCode := 0;
  if (E is EFDDBEngineException) and (EFDDBEngineException(E).ErrorCount > 0) then begin
    iErrCode := EFDDBEngineException(E).Errors[0].ErrorCode;
    if iErrCode = 0 then
      iErrCode := EFDDBEngineException(E).FDCode;
    Result := EUpdateError.Create(E.Message, '', iErrCode, iPrevErrCode, E);
  end
  else
...
{-------------------------------------------------------------------------------}
function TFDBlobStream.Realloc(var ANewCapacity: NativeInt): Pointer;
begin
...
begin
  if (FView <> nil) and ((DataSet = nil) or (DataSet.SortView <> FView)) then begin
    FView.RemRef;
    FView := nil;
  end;
end;
