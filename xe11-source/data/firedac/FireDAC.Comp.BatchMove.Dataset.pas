...
uses
  System.SysUtils,
  FireDAC.Stan.Util, FireDAC.Stan.Consts, FireDAC.Stan.Error,
...
begin
  ...
  try
    if Provider <> nil then begin
      oIndDefs := Provider.PSGetIndexDefs();
      sKeyFields := Provider.PSGetKeyFields();
    end;
    if oIndDefs = nil then
      AIndexDefs.Clear
    else
      AIndexDefs.Assign(oIndDefs);
  finally
    FDFree(oIndDefs);
  end;
...
begin
  ...
  if Optimise and not DataSet.Active and
     (DataSet is TFDAdaptedDataSet) and not IsMemTable and FDDataSet.ClientCursor then begin
    oFO := FDDataSet.FetchOptions;
    FPrevOptions := TFDOptionsContainer.Create(Self, TFDFetchOptions,
      TFDBottomUpdateOptions, TFDBottomResourceOptions, nil);
    FPrevOptions.FetchOptions.Assign(oFO);
    oFO.Items := [fiBlobs, fiDetails];
    oFO.Mode := fmOnDemand;
    if oFO.RowsetSize < BatchMove.CommitCount then
      oFO.RowsetSize := BatchMove.CommitCount;
    oFO.Cache := [];
    if (FDDataSet.IndexName = '') and (FDDataSet.IndexFieldNames = '') and
       ((BatchMove.CommitCount <= 0) or (BatchMove.CommitCount = MaxInt)) then
...
{-------------------------------------------------------------------------------}
procedure TFDBatchMoveDataSetReader.Close(AStopTxError: Boolean);
begin
  if FPrevOptions <> nil then begin
    FDDataSet.FetchOptions.RestoreDefaults;
    FDDataSet.FetchOptions.Assign(FPrevOptions.FetchOptions);
    FPrevOptions := nil;
  end;
...
{-------------------------------------------------------------------------------}
function TFDBatchMoveDataSetReader.GetFieldValue(AField: TObject): Variant;
begin
...
begin
  ...
  if DataSet is TFDAdaptedDataSet then begin
    if Optimise then begin
      FPrevOptions := TFDOptionsContainer.Create(Self, TFDFetchOptions,
        TFDBottomUpdateOptions, TFDBottomResourceOptions, nil);
      FPrevOptions.FetchOptions.Assign(FDDataSet.FetchOptions);
      FPrevOptions.UpdateOptions.Assign(FDDataSet.UpdateOptions);
...
begin
  if FPrevOptions <> nil then begin
    FDDataSet.FetchOptions.RestoreDefaults;
    FDDataSet.FetchOptions.Assign(FPrevOptions.FetchOptions);
    FDDataSet.UpdateOptions.RestoreDefaults;
    FDDataSet.UpdateOptions.Assign(FPrevOptions.UpdateOptions);
    FPrevOptions := nil;
  end;
  inherited Close(AStopTxError);
...
begin
  if (Direct or ANoUndo) and (DataSet is TFDAdaptedDataSet) then begin
...
{-------------------------------------------------------------------------------}
procedure TFDBatchMoveDataSetWriter.SetFieldValue(AField: TObject;
  const AValue: Variant);
begin
  TField(AField).Value := AValue;
end;
