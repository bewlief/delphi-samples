...
{-------------------------------------------------------------------------------}
function TFDPhysConnectionMetadata.QueryInterface(const IID: TGUID; out Obj): HResult;
begin
...
begin
  ...
  if AParsedName.OverInd = 0 then
    AParsedName.OverInd := ACommand.Overload;
  if FConnectionObj <> nil then
...
begin
  ...
  try
    ...
    oFtchOpts := oCommand.Options.FetchOptions;
    oFtchOpts.RecsSkip := -1;
    oFtchOpts.RecsMax := -1;
...
begin
  ...
  if (oCommand.RowsAffected = 0) and
     (AMetaKind in [mkIndexes, mkPrimaryKey, mkPrimaryKeyFields, mkForeignKeys, mkProcArgs]) then
  begin
    ParseMetaInfoParams(ACatalog, ASchema, ABaseObject, AObject, rName);
    ADataTable.EnforceConstraints := False;
    try
      oRow := ADataTable.NewRow;
      oRow.ValueS['RECNO'] := -1;
      oRow.ValueS['CATALOG_NAME'] := rName.FCatalog;
      oRow.ValueS['SCHEMA_NAME'] := rName.FSchema;
      case AMetaKind of
      mkIndexes,
      mkPrimaryKey,
      mkForeignKeys:
        oRow.ValueS['TABLE_NAME'] := rName.FObject;
      mkPrimaryKeyFields:
        oRow.ValueS['TABLE_NAME'] := rName.FBaseObject;
      mkProcArgs:
        begin
          if rName.FBaseObject <> '' then
            oRow.ValueS['PACK_NAME'] := rName.FBaseObject;
          oRow.ValueS['PROC_NAME'] := rName.FObject;
          if rName.FOverload <> '' then
            oRow.ValueS['OVERLOAD'] := rName.FOverload;
        end;
      end;
      ADataTable.Rows.Add(oRow);
    finally
      ADataTable.EnforceConstraints := True;
    end;
  end;
end;
...
begin
  ...
  if Result then
    AView.RowFilter := AFilter + ' AND RECNO >= 0'
  else if (AView.Rows.Count = 1) and (AView.Rows[0].GetData(0) < 0) then
    AView.Rows.RemoveAt(0, True);
end;
...
begin
  ...
  if (AParsedName.FCatalog = '') and (ACatalog <> '') then
    AParsedName.FCatalog := NormObjName(ACatalog, npCatalog);
  if (AParsedName.FSchema = '') and (ASchema <> '') then
    AParsedName.FSchema := NormObjName(ASchema, npSchema);
  if FConnectionObj <> nil then
    __TADPhysConnection(FConnectionObj).InternalOverrideNameByCommand(AParsedName, nil);
  if AParsedName.FCatalog <> '' then
    AParsedName.FCatalog := UnQuoteObjName(AParsedName.FCatalog);
  if AParsedName.FSchema <> '' then
    AParsedName.FSchema := UnQuoteObjName(AParsedName.FSchema);
...
begin
  ...
  if (GetKind = TFDRDBMSKinds.MSSQL) and (Pos(';', AProc) = 0) and (AOverload > 0) then
    sProc := rName.FObject + ';' + IntToStr(AOverload)
  else
    sProc := rName.FObject;
