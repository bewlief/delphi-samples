...
{$ENDIF}
  ...
  public
    ...
    constructor Create(AObj: TOCIHandle; AErrorCode: sb4; const AMessage: String); overload;
  end;
  ...
  public
    ...
    property ServerVersion: TFDVersion read FServerVersion write FServerVersion;
  end;
...
//      var BufLen: pUb4; var PieceStatus: ub1; var Ind: pointer): sb4;
  ...
  public
    ...
    property MapWC2NChar: Boolean read FMapWC2NChar write FMapWC2NChar;
    // general attrs
    property ENV_: pOCIHandle index OCI_ATTR_ENV read GetHandleAttr;
    // parse info attrs
    ...
    property IS_RETURNING: ub1 index OCI_ATTR_STMT_IS_RETURNING read GetUB1Attr;
    // fetch setup attrs
  ...
  private
    ...
    function GetStatement: TOCIStatement; inline;
    function GetMaxStrSize: ub4;
  protected
...
{$ENDIF}
    ...
    property Statement: TOCIStatement read GetStatement;
  ...
  public
    ...
    property IOMODE: ub4 index OCI_ATTR_IOMODE read GetUB4Attr;
  ...
  public
    ...
    property MaxNameLength: Integer read FMaxNameLength write FMaxNameLength;
  end;
...
begin
  ...
  if sExplicitLib <> '' then
    ...
    // Use OCI.dll, do not use OraClientNN.dll because it does not export
    // OCICollXxxx, OCIIterXxxx and some other required entry points.
...
begin
  ...
  while True do begin
    errCode := 0;
    SetLength(errBuf, OCI_ERROR_MAXMSG_SIZE2 * SizeOf(Char));
...
begin
  ...
  FHandle := AHandle;
  FOwnHandle := False;
  TOCIService(FOwner).ServerVersion := ServerVersion;
end;
...
{-------------------------------------------------------------------------------}
function TOCIServer.GetServerVersionStr: String;
{$IFDEF FireDAC_MONITOR}
  procedure ProcTrace;
  begin
    Trace(sOCIServerVersion, []);
...
var
  sbVer: TFDByteString;
begin
  if FServerVersionStr = '' then begin
{$IFDEF FireDAC_MONITOR}
    if Env.Tracing then ProcTrace;
{$ENDIF}
    ...
    FServerVersionStr := Env.MetaEncoder.Decode(sbVer);
  end;
  Result := FServerVersionStr;
end;

{-------------------------------------------------------------------------------}
function TOCIServer.GetServerVersion: TFDVersion;
var
  s: String;
  i, iFrom: Integer;
begin
  if FServerVersion = 0 then begin
    s := ServerVersionStr;
  ...
  TOCIService(FOwner).ServerVersion := Result;
end;
...
{$ENDIF}
      ...
      if AData.FVar.GetRawData(AData.FIter, AData.FBuff, AData.FPieceSize) then
        AData.FInd := 1
      else
        AData.FInd := -1;
      if AData.FVar.DataType in [otNString, otNChar, otNLong, otNCLOB] then
...
{-------------------------------------------------------------------------------}
function TOCIVariable.GetStatement: TOCIStatement;
begin
  Result := TOCIStatement(FOwner);
end;

{-------------------------------------------------------------------------------}
// Buffer management

function TOCIVariable.GetMaxStrSize: ub4;
begin
  if Statement.STMT_TYPE in [OCI_STMT_BEGIN, OCI_STMT_DECLARE] then
    Result := C_FD_MaxPLSQLDataSize
  else if (DataType in [otChar, otNChar, otRaw]) and
          (Env.Lib.Version < cvOracle120000) then
    Result := 2000
  else
    Result := 4000;
end;
...
begin
  ...
  FreeBuffer;
  lSetMax := False;
  iBPC := Statement.FService.BytesPerChar;
  ...
  if (DataType in otVBlobs) and (VarType in [odIn, odOut, odInOut]) and
     (Statement.STMT_TYPE in [OCI_STMT_BEGIN, OCI_STMT_DECLARE]) and
    ...
    if (DataType in [otRaw]) and (DataSize = C_FD_DefStrSize) or
      ...
      lSetMax := True;
      iMaxSize := GetMaxStrSize();
      if not (DataType in [otNString, otNChar, otRaw]) then
  ...
  // map otString/otNString to SQLT_AVC instead of SQLT_CHR. SQLT_AVC parameter
  // allows comparision with CHAR column without need to pad by spaces.
  if (FDataType in [otString, otNString]) and (VarType = odIn) and (CharSize <= 2000) then begin
    FValue_ty := SQLT_AVC;
    if FDataType = otString then
      Inc(FValue_byte_sz, SizeOf(TFDAnsiChar))
    else
      Inc(FValue_byte_sz, SizeOf(WideChar));
    lSetMax := False;
  end;
...
{$ENDIF}
            Check(Env.Lib.OCILobFreeTemporary(Statement.FService.Handle,
...
{$IFDEF FireDAC_MONITOR}
          if Env.Tracing then ProcTrace3;
{$ENDIF}
          Check(Env.Lib.OCILobCreateTemporary(Statement.FService.FHandle,
...
{$ENDIF}
    ...
    if (FHandle = nil) or (FBindData^.FBuffer <> FLastBindedBuffer) then begin
      ...
      if FIsPLSQLTable and
         (Statement.Stmt_Type in [OCI_STMT_BEGIN, OCI_STMT_DECLARE]) then begin
        v1 := FMaxArr_len;
        v2 := @FCurEle;
      end
      else begin
...
{$ENDIF}
  ...
  otNLong:
    begin
      // With different Oracle versions and DB character sets, this may lead
      // to different opposite test results. Two options:
      // * for national characters - always set CHARSET_FORM (RS-81597)
      // * for new 32k characters - do not set (TFDQACompQRYTsHolder.TestChar32k)
      if Statement.MapWC2NChar and (
          (VarType = odDefine) or (
            (sz <= sb4(GetMaxStrSize())) and
            (Statement.Service.ServerVersion >= cvOracle90000) and (Env.Lib.Version >= cvOracle90000)
          )) then
    ...
    case DataType of
      ...
      otString, otChar, otLong:
        begin
          ...
          if (FValue_ty = SQLT_AVC) and (PFDAnsiString(pBuff)[ASize - 1] = TFDAnsiChar(#0)) then
            Dec(ASize);
          if (DataType = otChar) and Statement.StrsTrim then
          ...
          if (ASize = 0) and Statement.StrsEmpty2Null then begin
            ...
            Move(pBuff^, PFDAnsiString(ABuff)^, ASize * SizeOf(TFDAnsiChar));
            if (DataType = otChar) and not Statement.StrsTrim and
      ...
      otNString, otNChar, otNLong:
        begin
          ...
          if ByteSemantic then
            ASize := ASize div SizeOf(WideChar);
          if (FValue_ty = SQLT_AVC) and (PWideChar(pBuff)[ASize - 1] = #0) then
            Dec(ASize);
          if (DataType = otNChar) and Statement.StrsTrim then
          ...
          if (ASize = 0) and Statement.StrsEmpty2Null then begin
            ...
            if (DataType = otNChar) and not Statement.StrsTrim and
               (ASize < FValue_char_sz) then
...
begin
  ...
  if ABuff = nil then
    ...
    case DataType of
      ...
      otRaw, otLongRaw:
        begin
          if Statement.StrsEmpty2Null and (ASize = 0) then
            FBindData^.FInd[AIndex] := -1
          else begin
        ...
        begin
          ...
          if Statement.StrsEmpty2Null and (ASize = 0) then
            ...
            if LongData then begin
              ...
              Move(PFDAnsiString(ABuff)^, Buffer[AIndex]^, ASize * SizeOf(TFDAnsiChar));
              if FValue_ty = SQLT_AVC then begin
                PFDAnsiString(PByte(Buffer[AIndex]) + ASize * SizeOf(TFDAnsiChar))^ := TFDAnsiChar(#0);
                Inc(ASize);
              end;
        ...
        begin
          ...
          if Statement.StrsEmpty2Null and (ASize = 0) then
            ...
            if LongData then begin
              ...
              if FValue_ty = SQLT_AVC then begin
                PWideChar(PByte(Buffer[AIndex]) + ASize * SizeOf(WideChar))^ := #0;
                Inc(ASize, SizeOf(WideChar));
              end;
...
begin
  Result :=
    (FOwner.Env.DataEncoder.Encoding in [ecUTF8, ecUTF16]) or
    (CHARSET_FORM in [SQLCS_NCHAR, SQLCS_EXPLICIT]) or
    // RS-78670
    (CHARSET_FORM = SQLCS_IMPLICIT) and (DATA_SIZE <> 0) and (CHAR_SIZE <> 0) and
      (DATA_SIZE > CHAR_SIZE);
end;
...
begin
  ...
  case IOMODE of
  OCI_TYPEPARAM_OUT,
  OCI_TYPEPARAM_OUTNCPY:   Result := odOut;
  OCI_TYPEPARAM_IN:        Result := odIn;
  OCI_TYPEPARAM_INOUT,
  OCI_TYPEPARAM_BYREF,
  OCI_TYPEPARAM_INOUTNCPY: Result := odInOut;
  end;
end;
...
var
  ...
  var
    i: Integer;
  begin
    if Length(AName) > MaxNameLength then begin
      i := oPars.IndexOfName(AName);
      if i >= 0 then
        Result := oPars.ValueFromIndex[i]
      else begin
        Result := Copy(AName, 1, MaxNameLength - 1 - Length(IntToStr(iParCnt + 1))) + '_' + IntToStr(iParCnt + 1);
