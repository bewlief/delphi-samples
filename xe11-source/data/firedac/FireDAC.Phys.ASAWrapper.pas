...
{     FireDAC SAP SQL Anywhere API wrapping classes     }
{                                                       }
...
{$ENDIF}
  ...
  public
    ...
    procedure Load(const AToolHome, AToolLib: String);
  end;
...
{-------------------------------------------------------------------------------}
procedure TASAToolLib.Load(const AToolHome, AToolLib: String);
var
...
begin
  if AToolLib <> '' then begin
    ...
    if AToolHome <> '' then
      sDLLName := FDNormPath(FDNormPath(AToolHome) + C_ASABinFolder)
    else
      sDLLName := '';
    SetLength(aTools, 9);
    for i := 12 downto 5 do
      aTools[12 - i + 1] := sDLLName + Format(C_DBTool, [i]);
    aTools[0] := sDLLName + Format(C_DBTool, [16]);
  end;
