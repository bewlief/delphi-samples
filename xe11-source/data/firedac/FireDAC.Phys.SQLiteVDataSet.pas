...
begin
  ...
  FUseIntRowid := FDataSet is TFDDataSet;
  if not FUseIntRowid then
...
begin
  ...
  if not FUseIntRowid then begin
    ...
    for i := 0 to Length(FConstraints) - 1 do begin
      ...
      if pConstr^.FColumn <> C_FD_SQLiteUndefRowid then begin
        if pConstr^.FColumn = -1 then
          ...
          ACost := ACost + 10;
        // don't include these op's, as we don't know its implementation
        if not (pConstr^.FOp in [coMatch, coGLOB, coRegExp, coIs, coIsNot]) then begin
          pConstr^.FOmit := True;
...
begin
  ...
  else if (AOp = moUpdate) and (ARowid1 <> C_FD_SQLiteUndefRowid) and (ARowid1 <> ARowid2) then
    if FUseIntRowid or
...
var
  ...
  i: Integer;
begin
  if FUseIntRowid then
                                                                                       
    Exit(TFDDataSet(ADataSet).GetRow().RowID);
...
var
  pKeys: PSQLiteVDSBookmark;
begin
  if FUseIntRowid then begin
                                                                                       
    if not FDataSet.Locate('#', ARowid, []) then
      VDSError(Self, er_FD_SQLiteVTabDSNoRowExists, []);
    Exit;
  end;
...
constructor TSQLiteVDSCursor.Create(ATable: TSQLiteVDSTable);
  ...
  function HasCalculatedFields(ADataSet: TDataSet): Boolean;
  var
    I: Integer;
  begin
    for I := 0 to ADataSet.Fields.Count - 1 do
      if ADataSet.Fields[I].FieldKind in [fkCalculated, fkLookup, fkAggregate] then
        Exit(True);
    Result := False;
  end;
...
begin
  ...
  if ATable.FMultipleCursors then begin
    FClonedDataSet := nil;
    if not Table.IsUnidirectional and not HasCalculatedFields(Table.FDataSet) then
    ...
    FDataSet := FClonedDataSet;
    FAdapter := nil;
    if not Supports(FDataSet, IFDPhysLocalSQLAdapter, FAdapter) then
                                                                           
                                     
      if Supports(TObject(Table.FAdapter).ClassType.Create, IFDPhysLocalSQLAdapter, FAdapter) then
...
begin
  ...
  FUseRanges := (Table.FMultipleCursors or (afRanges in FAdapter.Features)) and
    (afIndexFieldNames in FAdapter.Features) and (Pos('=', AIndexStr) > 0);
  lHasOrderBy := False;
  iConstrs := 0;
  i := 1;
  while i <= Length(AIndexStr) do begin
    ...
    if pConstr^.FPredicate then begin
      ...
      Inc(iConstrs);
      if FUseRanges and (pConstr^.FOp in [coEqual, coLess, coLessOrEqual, coGreater, coGreaterOrEqual]) then begin
  ...
  begin
    ...
    if oIn.ExtDataType = etUString then begin
      ...
      if oFld.DataType = ftDate then
        ...
        UnicodeString(AVar.VUnicodeString) := __TBooleanField(oFld).FTextValues[oIn.AsBoolean]
...
begin
  ...
  if AIndexStr = '' then begin
    ...
    FDataSet.Filter := '';
    if FClonedDataSet <> nil then
      FClonedDataSet.First
    else
      Table.Rewind;
    Exit;
  end
  ...
  for i := 0 to FFilterCols.Count - 1 do begin
    ...
    lStrings := (oFld.DataType in (CStringTypes + CWideStringTypes +
      CBlobTypes + CDateTimeTypes)) or (pConstr^.FOp = coLike);
    if oFld.DataType = ftDate then
    ...
    else if oFld.DataType in (CStringTypes + CWideStringTypes + CBlobTypes) then begin
      sVal := AnsiQuotedStr(oIn.AsString, '''');
      lStrings := False;
    end
    else
      sVal := oIn.AsString;
    sFilter := sFilter + C_AND[i > 0] + '[' + oFld.FieldName + '] ' + C_OP[pConstr^.FOp];
    if not (pConstr^.FOp in [coIsNotNull, coIsNull]) then
      sFilter := sFilter + ' ' + C_QUOTES[lStrings] + sVal + C_QUOTES[lStrings];
  end;
