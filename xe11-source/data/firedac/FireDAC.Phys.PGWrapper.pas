...
{$IFDEF MSWINDOWS}
  Winapi.Windows,
{$ENDIF}
  System.Types, System.Classes, System.SysUtils, System.Math, Data.FmtBCD,
...
type
  ...
  public
    ...
    FPQbinaryTuples: TPQbinaryTuples;
    ...
    FPQencryptPassword: TPQencryptPassword;
    FPQencryptPasswordConn: TPQencryptPasswordConn;
...
{$ENDIF}
  ...
  TPgTypeAttrs = set of (paString, paFixed, paBlob, paOID, paArray,
    paRecord, paEnum, paRange, paCast, paCustom);
  ...
  public
    ...
    destructor Destroy; override;
    class procedure GetDefaults(AFormat: Integer; ATypeOid: Oid;
  ...
  public
    ...
    function EncryptPassword(const APassword, AUser, AAlgorythm: String): String;
    // R/O
...
const
  ...
  sPQbackendPID: String = 'PQbackendPID';
  ...
  sPQbinaryTuples: String = 'PQbinaryTuples';
  ...
  sPQgetCopyData: String = 'PQgetCopyData';
  ...
  sPQencryptPassword: String = 'PQencryptPassword';
  sPQencryptPasswordConn: String = 'PQencryptPasswordConn';
  ...
  sClosePrefix: String = 'CLOSE "';
  sDeallocatePrefix: String = 'DEALLOCATE "';
...
{$IFNDEF FireDAC_PGSQL_STATIC}
  ...
  if sDLLName <> '' then
    sDLLName := FDNormPath(FDNormPath(sDLLName) + C_PgDllFolder);
  if AVendorLib <> '' then
...
{$IFDEF FireDAC_PGSQL_STATIC}
  ...
  FPQbinaryTuples := @PQbinaryTuples;
  ...
  FPQgetCopyData := @FPQgetCopyData;                // not used
  ...
  FPQencryptPassword := @FPQencryptPassword;
  FPQencryptPasswordConn := @FPQencryptPasswordConn;
 ...
 {$ELSE}
  ...
  FPQbackendPID := GetProc(sPQbackendPID);                   // not used
  ...
  FPQbinaryTuples := GetProc(sPQbinaryTuples);
  ...
  FPQgetCopyData := GetProc(sPQgetCopyData);                 // not used
  ...
  if FBrand = pbEnterprise then
    FPQexecBulkPrepared := GetProc(sPQexecBulkPrepared)
  else
    FPQexecBulkPrepared := nil;

  FPQencryptPassword := GetProc(sPQencryptPassword, False);
  FPQencryptPasswordConn := GetProc(sPQencryptPasswordConn, False);
{$ENDIF}

{$IFDEF POSIX}
  if Assigned(FPQencryptPasswordConn) then
    FVersion := svPGSQL100000
  else if Assigned(Flo_truncate) then
    FVersion := svPGSQL080300
  else if Assigned(FPQprepare) then
...
begin
  ...
  oEx.AppendError(AMessage).ErrorCode := AStatus;
{$IFDEF FireDAC_MONITOR}
  Env.Trace(ekError, esProgress, 'ERROR: ' + oEx[0].Message, ['Status', AStatus]);
{$ENDIF}
...
begin
  ...
  AddBase(SQL_CHAR,       'char',           1,  0,                        0,                  [paString, paFixed]);
  ...
  AddBase(SQL_CIDR,       'cidr',           0,  20,                       20 {49},            [paString, paCast]);
  ...
  AddBase(SQL_MACADDR8,   'macaddr8',       0,  8,                        8 {23},             [paString, paFixed, paCast]);
  AddBase(SQL_CASH,       'money',          0,  0,                        SizeOf(Currency),   []);
  AddBase(SQL_MACADDR,    'macaddr',        0,  6,                        6 {17},             [paString, paFixed, paCast]);
  AddBase(SQL_INET,       'inet',           0,  20,                       20 {49},            [paString, paCast]);
  ...
  AddArray(SQL_MACADDR8 + 1,'_macaddr8',     SQL_MACADDR8);
...
{-------------------------------------------------------------------------------}
class procedure TPgTypesManager.GetDefaults(AFormat: Integer; ATypeOid: Oid;
...
begin
  ...
  SQL_MACADDR: if AFormat = 0 then ALen := 17 else ALen := 6;
  SQL_MACADDR8:if AFormat = 0 then ALen := 23 else ALen := 8;
  SQL_CIDR,
  SQL_INET:    if AFormat = 0 then ALen := 49 else ALen := 20;
...
begin
  ...
  try
    sId := IntToStr(AOid);
    oStmt.PrepareSQL('SELECT TYPNAME, TYPRELID, TYPELEM, TYPARRAY, TYPBASETYPE, TYPTYPE, TYPCATEGORY ' +
    ...
    sCat := oStmt.Fields[6].GetAsString;

    if iBase <> 0 then
      Result := Types[iBase]
    else if (iRelId = 0) and (iElem = 0) and (sType <> 'e') and (sType <> 'r') and (sType <> 'b') then
      if FConnection.UnknownFormat = 0 then
        DescribeError(sName, 'Type must be an array, composite, enum, range or base type')
      else
        Result := Types[FConnection.UnknownFormat];
    if Result <> nil then
      Exit;
    ...
    // range type
      ...
      for i := 0 to 2 do begin
        ...
        GetDefaults(1, oMbr.FOid, oMbr.FLen, oMbr.FPrec, oMbr.FScale);
    ...
    // base type
      ...
      if sCat = 'B' then
        ...
        iBase := SQL_BYTEA
      else if FConnection.UnknownFormat = 0 then
        if sCat = 'U' then begin
          AddBase(AOid, sName, 0, 0, 0, [paBlob, paString, paCast, paCustom]);
          iBase := AOid;
        end
        else
          DescribeError(sName, 'Base type must be simple or custom type')
      else
        iBase := FConnection.UnknownFormat;
      Result := Types[iBase];
    end
...
procedure TPgConnection.Check;
  ...
  begin
{$IFDEF MSWINDOWS}
    if Lib.FPQstatus(FHandle) = CONNECTION_BAD then
      eSrcEnc := ecANSI
    else
{$ENDIF}
      eSrcEnc := ecDefault;
    DoError(Encoder.Decode(Lib.FPQerrorMessage(FHandle), -1, eSrcEnc), Lib.FPQstatus(FHandle));
  end;

var
  pMsg: PByte;
begin
  if Lib.FPQstatus(FHandle) = CONNECTION_OK then begin
    pMsg := Lib.FPQerrorMessage(FHandle);
    if (pMsg = nil) or (pMsg^ = 0) then
      Exit;
  end;
  Error;
end;
...
begin
  ...
  ).ErrorCode := Lib.FPQresultStatus(AHandle);
end;
...
var
  sConnBytes: TFDByteString;
begin
  FIntegerDateTimes := MAXINT;
  FServerVersion := 0;
  try
    sConnBytes := Encoder.Encode(AConnStr);
    FHandle := Lib.FPQconnectdb(PFDAnsiString(PByte(sConnBytes)));
{$IFDEF FireDAC_MONITOR}
...
{$IFDEF FireDAC_MONITOR}
  if Env.Tracing then Trace1;
{$ENDIF}
  if (StrLComp(PChar(ASQL), PChar(sClosePrefix), Length(sClosePrefix)) <> 0) and
     (StrLComp(PChar(ASQL), PChar(sDeallocatePrefix), Length(sDeallocatePrefix)) <> 0) then
    ClearNotices;
...
{-------------------------------------------------------------------------------}
function TPgConnection.EncryptPassword(const APassword, AUser, AAlgorythm: String): String;
begin
  if Assigned(Lib.FPQencryptPasswordConn) then
    Result := Encoder.Decode(PByte(Lib.FPQencryptPasswordConn(FHandle,
      PFDAnsiString(PByte(Encoder.Encode(APassword))),
      PFDAnsiString(PByte(Encoder.Encode(AUser))),
      PFDAnsiString(PByte(Encoder.Encode(AAlgorythm))))), -1)
  else if Assigned(Lib.FPQencryptPassword) then
    Result := Encoder.Decode(PByte(Lib.FPQencryptPassword(
      PFDAnsiString(PByte(Encoder.Encode(APassword))),
      PFDAnsiString(PByte(Encoder.Encode(AUser))))), -1)
  else
...
var
  ...
  procedure Str2Guid(ASource: Pointer; ASourceLen: Integer; out ADest: TGUID);
  var
    s: String;
  begin
    SetString(s, PChar(ASource), ASourceLen);
    ADest := StringToGUID('{' + s + '}');
    // Change GUID mixed-endian to UUID big-endian
    ADest.D1 := Cardinal(ReadInteger(@ADest.D1));
    ADest.D2 := ReadWord(@ADest.D2);
    ADest.D3 := ReadWord(@ADest.D3);
  end;

  procedure Str2Curr(ASource: Pointer; ASourceLen: Integer; out ADest: Currency);
  var
    pSrc, pDest: PChar;
    lClear: Boolean;
  begin
    ...
    while pSrc^ <> #0 do begin
      ...
      '.', ',':
        if not lClear then begin
          pDest^ := FormatSettings.DecimalSeparator;
          Inc(pDest);
        end;
      ' ', #160:
        ;
    ...
    pDest^ := #0;
    if not TextToFloat(PChar(ASource), ADest, fvCurrency) then
      raise EConvertError.CreateResFmt(@SInvalidFloat, [PChar(ASource)]);
  end;

  procedure Str2Bin(ASource: Pointer; ASourceLen: Integer;
    ADest: Pointer; out ADestLen: LongWord);
  var
    i: Integer;
    lOn: Boolean;
    B, M: Byte;
  begin
    ADestLen := (ASourceLen + 7) div 8;
    for i := 0 to ASourceLen - 1 do begin
      lOn := (PChar(ASource) + i)^ = '1';
      if i mod 8 = 0 then
        B := 0
      else
        B := (PByte(ASource) + (i div 8))^;
      M := 1 shl (i mod 8);
      if lOn then
        B := B or M
      else
        B := B and not M;
      (PByte(ASource) + (i div 8))^ := B;
    end;
  end;

  procedure Str2Interval(ASource: Pointer; ASourceLen: Integer; out ADest: TFDSQLTimeInterval);
  var
    s: String;
  begin
    SetString(s, PChar(ASource), ASourceLen);
    ADest := FDStr2SQLTimeInterval(s);
  end;

  procedure Hex2Bin(ASource: Pointer; ASourceLen: Integer;
    ADest: Pointer; out ADestLen: LongWord);
{$IFDEF NEXTGEN}
  var
    aBuff: TBytes;
{$ENDIF}
  begin
    if (PChar(ASource)^ = '\') and ((PChar(ASource) + 1)^ = 'x') then begin
      Inc(PChar(ASource), 2);
      Dec(ASourceLen, 2);
    end;
    ADestLen := (ASourceLen + 1) div 2;
{$IFNDEF NEXTGEN}
    HexToBin(PChar(ASource), ADest^, ADestLen);
{$ELSE}
    SetLength(aBuff, ADestLen);
    HexToBin(PChar(ASource), 0, aBuff, 0, ADestLen);
    Move(aBuff[0], ADest^, ADestLen);
{$ENDIF}
  end;

  procedure Str2TimeStamp(ASource: Pointer; ASourceLen: Integer; out ADest: TSQLTimeStamp);
  var
    c: Char;
  begin
    c := (PChar(ASource) + ASourceLen - 3)^;
    if (c = '+') or (c = '-') then
      Dec(ASourceLen, 3);
    FDISOStr2TimeStamp(ASource, ASourceLen, ADest);
  end;
...
begin
  ...
  if [paCustom, paBlob] * AType.Attrs = [paCustom, paBlob] then
    if FFormat = 0 then
      iId := SQL_TEXT
    else
      iId := SQL_BYTEA
  ...
  if FFormat = 0 then
    case iId of
      ...
      SQL_VOID:
        ...
        pWC := nil;
        ASourceLen := Statement.FConnection.Encoder.Decode(ASource, ASourceLen, pWC, ecUTF16);
        ASource := pWC;
      end;
    end;
  ...
  SQL_SMGR:
    if FFormat = 1 then
      ...
      FDStr2Int(ASource, ASourceLen, ADest, ADestLen, False, 0);
  SQL_FLOAT4,
  SQL_FLOAT8:
    if FFormat = 1 then
      MoveRotate(ASource, ADest, ADestLen)
    else
      FDStr2Float(ASource, ASourceLen, ADest, ADestLen, '.');
  SQL_CASH:
    if FFormat = 1 then
      GetPgMoney(ASource, ADestLen, PCurrency(ADest)^)
    else
      Str2Curr(ASource, ASourceLen, PCurrency(ADest)^);
  SQL_NAME,
  SQL_CHAR,
  SQL_BPCHAR,
  SQL_VARCHAR,
  ...
  SQL_VOID:
    begin
      if (iId = SQL_JSONB) and (FFormat = 1) then begin
  ...
  SQL_INET:
    if FFormat = 1 then
      ...
      if iId = SQL_BYTEA then
        Hex2Bin(ASource, ASourceLen, ASource, ADestLen)
      else
        ADestLen := ASourceLen;
      if AByRef then
        ADest := ASource
  ...
  SQL_VARBIT:
    if FFormat = 1 then begin
      ...
      Str2Bin(ASource, ASourceLen, ASource, ADestLen);
      if AByRef then
        ADest := ASource
      else if ADest <> nil then
        Move(ASource^, ADest^, ADestLen);
  ...
  SQL_DATE:
    if FFormat = 1 then
      ...
      PInteger(ADest)^ := FDISOStr2Date(ASource, ASourceLen);
  SQL_TIME,
  SQL_TIMETZ:
    if FFormat = 1 then
      GetPgTime(ASource, PInteger(ADest)^)
    else
      PInteger(ADest)^ := FDISOStr2Time(ASource, ASourceLen);
  SQL_TIMESTAMP:
    if FFormat = 1 then
      GetPgTimeStamp(ASource, PSQLTimeStamp(ADest)^, 0)
    else
      FDISOStr2TimeStamp(ASource, ASourceLen, PSQLTimeStamp(ADest)^);
  SQL_TIMESTAMPTZ:
    if FFormat = 1 then
      GetPgTimeStamp(ASource, PSQLTimeStamp(ADest)^, Statement.Connection.TimeZoneOffset)
    else
      Str2TimeStamp(ASource, ASourceLen, PSQLTimeStamp(ADest)^);
  SQL_RELTIME:
    if FFormat = 1 then
      GetPgRelTime(ASource, PSQLTimeStamp(ADest)^)
    else
      PSQLTimeStamp(ADest)^ := FDTime2SQLTimeStamp(FDISOStr2Time(ASource, ASourceLen));
  SQL_ABSTIME:
  ...
  SQL_INTERVAL:
    if FFormat = 1 then
      ...
      Str2Interval(ASource, ASourceLen, PFDSQLTimeInterval(ADest)^);
  SQL_NUMERIC:
    if FFormat = 1 then
      GetPgNumeric(ASource, PBcd(ADest)^)
    else
      FDStr2BCD(ASource, ASourceLen, PBcd(ADest)^, '.');
  SQL_BOOL:
    if FFormat = 1 then
      PWordBool(ADest)^ := WordBool(PByte(ASource)^)
    else
      PWordBool(ADest)^ := (PChar(ASource)^ = 't') or (PChar(ASource)^ = 'T');
  SQL_UUID:
    if FFormat = 1 then
      if Statement.GUIDEndian = Big then
        PGUID(ADest)^ := TGUID.Create(PByte(ASource)^, True)
      else
        PGUID(ADest)^ := PGUID(ASource)^
    else begin
      Str2Guid(ASource, ASourceLen, PGUID(ADest)^);
      if Statement.GUIDEndian = Big then
        PGUID(ADest)^ := TGUID.Create(PGUID(ADest)^, True);
    end;
  else
    ASSERT(False);
  end;
    ...
    begin
      ...
      if Result then begin
        if (FEncoding = ecANSI) and (Statement.FConnection.Encoder.Encoding = ecANSI) then begin
          ...
          iMaxChars := ADestLen;
          if Statement.FConnection.FEncoder.Encoding = ecUTF8 then
            iMaxChars := iMaxChars div C_FD_MaxUTF8Len;
          if ASourceLen > Integer(iMaxChars) then
            if Statement.StrsTrim2Len then
              ASourceLen := iMaxChars
            else
              ErrorDataTooLarge(iMaxChars, ASourceLen);
          pRes := nil;
...
begin
  if (ABuffer = nil) and (ALen = 0) then
    ...
    SQL_INET:
      if FFormat = 0 then
        if Statement.FConnection.Encoder.Encoding = ecANSI then
          Result := '''' + Statement.FConnection.Encoder.Decode(ABuffer, ALen, ecANSI) + ''''
        else begin
          SetString(Result, PWideChar(ABuffer), ALen);
          Result := '''' + Result + '''';
        end
...
var
  ...
  iLen: LongWord;
begin
  if (TypeRef = nil) or not (paArray in TypeRef.Attrs) or (paFixed in TypeRef.Attrs) then
    Exit;
...
var
  i: Integer;
begin
  if (FFormat = 0) and ([paArray, paRecord, paRange] * TypeRef.Attrs <> []) then
    FDException(Statement.OwningObj, [S_FD_LPhys, S_FD_PGId], er_FD_PgUnsupTextType,
      [DumpLabel]);
  Statement.Connection.TypesManager.GetDefaults(FFormat, FTypeOid, FLen, FPrec, FScale);
  ...
  if TypeOid = SQL_PATH then begin
    ...
    if iDims = 0 then begin
      ...
      if (Length(TypeRef.Members) <> 1) or
         ([paArray, paRange] * TypeRef.Members[0].TypeRef.Attrs <> []) or
...
begin
  Reset;
  try
    if (FDStartsWithKW(ASQL, 'SELECT', iAfter) or
        FDStartsWithKW(ASQL, 'WITH', iAfter)) and
...
{$ENDIF}
  for i := 0 to FFields.Count - 1 do begin
    ...
    oFld.SetDefaults;
    if (oFld.TypeRef.Attrs = [paArray]) and (oFld.Fields.Count = 1) then
      oFld := oFld.Fields[0];
...
{$ENDIF}
    ...
    if (FCursorName <> '') and
      ...
      FConnection.ExecuteQuery(sClosePrefix + FCursorName + '"');
  except
    on E: EPgNativeException do
...
{$IFDEF FireDAC_MONITOR}
      if Env.Tracing then Trace1;
{$ENDIF}
      FConnection.ExecuteQuery(sDeallocatePrefix + sTmpStmtName + '"');
    end;
  except
    on E: EPgNativeException do
      if E.Kind <> ekServerGone then
