...
begin
  ...
  FLib.LoadIB(sHome, sLib, GetLite(), GetThreadSafe());
end;
...
begin
  ...
  while iCurOffset < ATimes do begin
    ...
    try
      try
        if FStmt.InVars.RowCount = 1 then
          FStmt.Execute(False)
        else
          FStmt.ExecuteBatch;
      finally
...
{-------------------------------------------------------------------------------}
initialization
  FDRegisterDriverClass(TFDPhysIBDriver);
  FDRegisterDriverClass(TFDPhysIBLiteDriver);

finalization
  FDUnregisterDriverClass(TFDPhysIBDriver);
  FDUnregisterDriverClass(TFDPhysIBLiteDriver);
