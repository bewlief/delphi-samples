...
begin
  ...
  // OCTET_LENGTH for ASCII characters must return 1
  if SameText(FCharacterSet, 'UTF16') then
    FCharacterSet := 'UTF8';
end;
...
begin
  ...
  efBIT_LENGTH:
    begin
      Result := '(OCTET_LENGTH(' + A1;
      if FCharacterSet <> '' then
        Result := Result + ', ' + FCharacterSet;
      Result := Result + ') * 8)';
    end;
  ...
  efWEEK:        Result := '(((' + A1 + ') - ((EXTRACT(YEAR FROM (' + A1 + ')) - 1900) * 10000 + 0101 (DATE))) - (((' + A1 + ') - DATE ''0001-01-07'') MOD 7)  + 13) / 7';
...
begin
  if (ASkip + ARows <> MAXINT) and
    ...
    else if FDHasKW(s, 'UNION') or FDHasKW(s, 'MINUS') or FDHasKW(s, 'INTERSECT') or
       FDHasKW(s, 'EXCEPT') or FDHasKW(s, 'DISTINCT') then
