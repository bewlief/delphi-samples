...
begin
  ...
  if C <> '' then
    if Length(C) = 2 then begin
      ...
      FNameQuoteChar := C[1];
      if oConn.DriverKind in [dkMSAccessJet, dkMSAccessACE, dkExcel] then begin
        FNameQuoteChar1 := '[';
        FNameQuoteChar2 := ']';
      end;
    end;
  C := oConn.CATALOG_NAME_SEPARATOR;
  if (C <> '') and (Length(C) = 1) then
...
{------------------------------------------------------------------------------}
function TFDPhysODBCMetadata.IsNameValid(const AName: String): Boolean;
begin
  if FParentMeta <> nil then
