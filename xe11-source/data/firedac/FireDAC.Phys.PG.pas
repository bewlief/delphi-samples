...
{$ENDIF}
  ...
  private
    ...
    procedure SQL2FDColInfo(ASQLTypeOid: Oid; ASQLLen: LongWord; ASQLPrec,
      ASQLScale: Integer; AParamType: TParamType; out AType: TFDDataType;
      var AAttrs: TFDDataAttributes; out ALen: LongWord; out APrec, AScale: Integer;
...
function TFDPhysPgConnection.BuildPgConnectString(const AConnectionDef: IFDStanConnectionDef): String;
  ...
  begin
    iCnt := 0;
    aStr := AAddr.Split(['.'], TStringSplitOptions.ExcludeEmpty);
  ...
  begin
    i := Pos('=', AParam);
    if i > 0 then begin
      sName := Copy(AParam, 1, i - 1);
      sValue := Copy(AParam, i + 1, MaxInt);
...
var
  ...
  procedure SetEncoder(const AEncoding: String);
  begin
    if SameText(AEncoding, 'UTF8') or SameText(AEncoding, 'UNICODE') then
...
{$ENDIF}
  ...
  if InternalGetSharedCliHandle() = nil then begin
    SetEncoder(ConnectionDef.AsString[S_FD_ConnParam_Common_CharacterSet]);
    ...
    if oParams.NewPassword <> '' then begin
      sEncPassword := FConnection.EncryptPassword(oParams.NewPassword, oParams.UserName, '');
      InternalExecuteDirect('ALTER USER ' + oParams.UserName +
        ' PASSWORD ''' + sEncPassword + '''', nil);
    end;
  end;

  FServerVersion := FConnection.ServerVersion;
  SetEncoder(FConnection.ClientEncoding);
...
begin
  ...
  try
    ...
    sEncPassword := oConn.EncryptPassword(ANewPassword, oConnDef.Params.UserName, '');
    oConn.ExecuteQuery('ALTER USER ' + oConnDef.Params.UserName +
      ' PASSWORD ''' + sEncPassword + '''');
  finally
    FDFree(oConn);
  end;
end;
  ...
  try
    ...
    if oStmt.ResultFormat = 1 then
      if oStmt.Fields[2].GetData(@rTS, iLen) then begin
        dt := dt - SQLTimeStampToDateTime(rTS);
        if dt >= 0 then
          dt := dt + OneSecond
        else
          dt := dt - OneSecond;
        FConnection.TimeZoneOffset := Trunc(dt / OneHour);
...
{-------------------------------------------------------------------------------}
procedure TFDPhysPgSPDescriber.CheckProcedure;
var
  oProcArgs: TFDProcArgRecs;
begin
  ReadProcArgs(oProcArgs);
...
begin
  ...
  iScale := 0;
  eSrcAttrs := [];
  FCommand.SQL2FDColInfo(ATypeOid, 0, 0, 0, AParamType, eSrcFDDataType, eSrcAttrs,
  ...
  if eSrcFDDataType <> dtUnknown then begin
    ...
    try
      ...
      iDestPrec := 0;
      iDestScale := 0;
      oFmt.ResolveDataType(AName, oType.Name, eSrcFDDataType, uiSize, iPrec, iScale,
        eDestFDDataType, uiSize, True);
    ...
    if (paArray in oType.Attrs) and (Length(oType.Members) = 1) then
      oParam.ArrayType := atTable;
  end
  else
...
begin
  ...
  try
    ...
    if oPgStmt.Fields[1].GetData(@iResTypeOid) then begin
      FIsProcedure := iResTypeOid = 0;
      if not bHasOutParams and not FIsProcedure and (iResTypeOid <> SQL_VOID) then begin
        SetLength(AProcArgs, iIndex + 1);
...
begin
  ...
  // generate stored proc call SQL command
    ...
    FDbCommandText := '';
    bMeta := fiMeta in FOptions.FetchOptions.Items;
    eKind := GetCommandKind;
    if bMeta or (PgConnection.FServerVersion >= svPGSQL110000) then begin
      ...
      try
        if bMeta then begin
          ...
          oDesc.CheckProcedure;
        if oDesc.IsProcedure then
          eKind := skExecute;
      finally
        FDFree(oDesc);
      end;
    ...
    if FDbCommandText = '' then begin
      ...
      GenerateStoredProcCall(rName, eKind);
    end;
  end;
...
procedure TFDPhysPgCommand.SQL2FDColInfo(ASQLTypeOID: Oid; ASQLLen: LongWord;
  ASQLPrec, ASQLScale: Integer; AParamType: TParamType; out AType: TFDDataType;
  var AAttrs: TFDDataAttributes; out ALen: LongWord; out APrec, AScale: Integer;
  AFmtOpts: TFDFormatOptions);
var
  oType: TPgType;
begin
  ...
  SQL_NUMERIC:
    begin
      APrec := ASQLPrec;
      AScale := ASQLScale;
      if AFmtOpts.IsFmtBcd(APrec, AScale) or
  ...
  SQL_TIMETZ:
    begin
      ...
      else if ASQLScale > 3 then
        // caVirtual processed in InternalColInfoGet to exclude coInWhere
        Include(AAttrs, caVirtual);
    end;
  SQL_TIMESTAMP,
  SQL_TIMESTAMPTZ,
  ...
  SQL_UUID:
    ...
    if paArray in oType.Attrs then
      SQL2FDColInfo(oType.Members[0].TypeOid, ASQLLen, ASQLPrec, ASQLScale,
        AParamType, AType, AAttrs, ALen, APrec, AScale, AFmtOpts)
      ...
      ASQLTypeOid := PgConnection.FConnection.UnknownFormat;
      if (ASQLTypeOid = 0) and ([paBlob, paCustom] * oType.Attrs <> []) then
        if (paString in oType.Attrs) and UseExecDirect then
...
begin
  ...
  Result.StrsTrim2Len := oFmt.StrsTrim2Len;
  Result.GUIDEndian := PgConnection.FGUIDEndian;
    ...
    if oParam.ParamType in [ptUnknown, ptInput, ptInputOutput] then begin
      ...
      if (iPgParRef >= 0) and (iPgParRef < FStmt.Params.Count) then begin
        ...
        SQL2FDColInfo(pInfo^.FOutSQLDataType, pInfo^.FSrcSize, pInfo^.FSrcPrec,
          pInfo^.FSrcScale, pInfo^.FParamType, pInfo^.FOutDataType, eAttrs,
          iLen, iPrec, iScale, oFmtOpts);
    ...
    for i := 0 to nFields - 1 do begin
      ...
      if [paArray, paRecord, paRange] * oMember.TypeRef.Attrs <> [] then begin
        ...
        SQL2FDColInfo(oMember.TypeOid, Def(oMember.Len, oField.Len),
          ...
          ptUnknown, pInfo^.FSrcDataType, pInfo^.FAttrs, pInfo^.FLen,
          pInfo^.FPrec, pInfo^.FScale, oFmtOpts);
    end;
  end;
  ...
  for i := 0 to Length(FBaseRecInfo.FColInfos) - 1 do begin
    ...
    if [paArray, paRecord, paRange] * oField.TypeRef.Attrs <> [] then begin
      ...
      SQL2FDColInfo(pInfo^.FSrcSQLDataType, oField.Len, oField.Prec, oField.Scale,
        ptUnknown, pInfo^.FSrcDataType, pInfo^.FAttrs, pInfo^.FLen, pInfo^.FPrec,
        ...
        if (pInfo^.FTableOid <> InvalidOid) and (pInfo^.FTableCol >= 0) then begin
          ...
          if j <> -1 then begin
            ...
            if not VarIsNull(V) then begin
              ...
              if (iBaseTableOid <> InvalidOid) and (iBaseTableOid <> pInfo^.FTableOid) then
                pInfo^.FAttrs := pInfo^.FAttrs + [caAllowNull];
            end;
...
begin
  if (FCurrentRecInfo^.FFixedLength > 0) and
    ...
    if caVirtual in pColInfo^.FAttrs then begin
      AColInfo.FAttrs := pColInfo^.FAttrs - [caVirtual];
      AColInfo.FForceRemOpts := [coInWhere];
    end
    else begin
...
begin
  if ATimes - AOffset > 1 then begin
    if (PgConnection.FServerVersion >= svPGSQL080100) and
       (GetCommandKind = skInsert) and (FSQLValuesPos > 0) then begin
...
{-------------------------------------------------------------------------------}
function TFDPhysPgCommand.InternalOpen(var ACount: TFDCounter): Boolean;
begin
  ACount := 0;
  ...
  try
    try
      if UseExecDirect and (GetCommandKind <> skStoredProcWithCrs) then
        FStmt.ExecuteDirect(FDbCommandText)
      else
        FStmt.Execute;
    finally
...
var
  ...
  begin
    ...
    // constrained array
      ...
      if iHBound - iLBound >= 0 then begin
        oNestedTable := ARow.Table.Columns[AColIndex].NestedTable;
        if oNestedTable <> nil then begin
  ...
  begin
    ...
    if (AColIndex = 0) or (AColIndex = 6) and (GetMetaInfoKind in [mkIndexFields, mkPrimaryKeyFields]) then
      ARow.SetData(AColIndex, ATable.Rows.Count + 1)
...
begin
  ...
  SQL2FDColInfo(iTypeOid, oRow.GetData(11), oRow.GetData(9), oRow.GetData(10),
    ptUnknown, eType, eAttrs, iLen, iPrec, iScale, FOptions.FormatOptions);
...
var
  ...
  begin
    ...
    SQL2FDColInfo(ATypeOID, 0, 0, 0, AParamType, eType, eAttrs, uiLen, iPrec,
...
{-------------------------------------------------------------------------------}
initialization
  FDRegisterDriverClass(TFDPhysPgDriver);

finalization
  FDUnregisterDriverClass(TFDPhysPgDriver);
