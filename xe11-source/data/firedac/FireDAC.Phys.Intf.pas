...
procedure FDRegisterDriverClass(ADriverClass: TClass);
procedure FDUnregisterDriverClass(ADriverClass: TClass);
...
{ TFDPhysParsedName                                                             }
{-------------------------------------------------------------------------------}
function TFDPhysParsedName.GetOverInd: Integer;
begin
  Result := StrToIntDef(FOverload, 0);
end;

{-------------------------------------------------------------------------------}
procedure TFDPhysParsedName.SetOverInd(const Value: Integer);
begin
  FOverload := IntToStr(Value);
end;
...
procedure FDRegisterDriverClass(ADriverClass: TClass);
begin
  FDPhysManager().RegisterDriverClass(ADriverClass);
end;
...
begin
  oMan := FDPhysManager();
  // After calling FDTerminate the FDPhysManager returns nil
  if oMan <> nil then
    oMan.UnregisterDriverClass(ADriverClass);
end;
