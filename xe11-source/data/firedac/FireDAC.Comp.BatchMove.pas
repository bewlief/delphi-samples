...
type
  ...
  IFDBatchMoveDriver = interface(IUnknown)
    ...
    // public
    function GetBatchMove: TFDBatchMove;
  ...
  private
    ...
    function GetBatchMove: TFDBatchMove; inline;
  protected
  ...
  private
    ...
    FKeyFields: String;
    FKeyValues: Variant;
    function GetItem(AIndex: Integer): TFDBatchMoveMappingItem; inline;
    procedure SetItem(AIndex: Integer; const AValue: TFDBatchMoveMappingItem); inline;
  ...
  published
    ...
    property Options: TFDBatchMoveOptions read FOptions write FOptions
      default [poIdentityInsert, poCreateDest, poSkipUnmatchedDestFields, poUseTransactions];
...
{ TFDBatchMoveDriver                                                            }
{-------------------------------------------------------------------------------}
function TFDBatchMoveDriver.GetBatchMove: TFDBatchMove;
begin
  Result := FBatchMove;
end;
...
{-------------------------------------------------------------------------------}
function TFDBatchMoveMappingItem.GetBatchMove: TFDBatchMove;
begin
  Result := TFDBatchMoveMappings(Collection).FBatchMove;
end;
...
begin
  ...
  if DestinationFieldName <> '' then
    ...
    iDestInd := Index;
  oObj := oBM.Writer.GetFieldInfo(iDestInd, eTmp, ilTmp, iTmp, iTmp, FInKey, FIsIdentity);
  if oObj is TField then
    FDestField := TField(oObj)
  else
    FDestFieldObj := oObj;
  ...
  if (SourceFieldName <> '') or (SourceExpression = '') then begin
    if SourceFieldName <> '' then
      iSrcIndex := oBM.Reader.GetFieldIndex(SourceFieldName, True)
    else
      iSrcIndex := iDestInd;
    try
      oObj := oBM.Reader.GetFieldInfo(iSrcIndex, eTmp, ilTmp, iTmp, iTmp, lTmp, lTmp);
      if oObj is TField then
        FSourceField := TField(oObj)
      else
        FSourceFieldObj := oObj;
    except
...
begin
  if FSourceFieldObj <> nil then
    Result := BatchMove.Reader.GetFieldValue(FSourceFieldObj)
...
begin
  ...
  oBM := BatchMove;
  if Assigned(oBM.OnWriteValue) then
    oBM.OnWriteValue(oBM, Self, Result);
end;
...
begin
  if FDestFieldObj <> nil then
    BatchMove.Writer.SetFieldValue(FDestFieldObj, ItemValue)
  else if FDestField.ReadOnly then
    Exit
  else if (FSourceField <> nil) and (FSourceEvaluator = nil) and
          not Assigned(BatchMove.OnWriteValue) then begin
    if FIsIdentity and (poIdentityInsert in BatchMove.Options) and
    ...
    if VarIsNull(V) then begin
      ...
      if FIsIdentity and (poIdentityInsert in BatchMove.Options) then
        SetupIdentityInsertion;
      if FDestField is TBinaryField then
        if BatchMove.Reader.IsUnicode then
  ...
  try
    for i := 0 to BatchMove.Writer.FieldCount - 1 do begin
      ...
      if BatchMove.Reader.GetFieldIndex(sDestName, False) <> -1 then
        ...
        // SQL Writer may have parameter names like NAME$#1,
        // then it must match to NAME source field.
        j := Pos('$#', sDestName);
        if j <> 0 then begin
          sDestName := Copy(sDestName, 1, j - 1);
          if BatchMove.Reader.GetFieldIndex(sDestName, False) <> -1 then
            oItem.SourceFieldName := sDestName;
        end;
      end;
      if (poSkipUnmatchedDestFields in BatchMove.Options) and
         (oItem.SourceFieldName = '') and (oItem.SourceExpression = '') then
        FDFree(oItem);
    end;
  finally
    if not lReaderOpen then
...
begin
  try
    ...
    if Count = 0 then
      FDException(BatchMove, [S_FD_LComp, S_FD_LComp_PDM], er_FD_DPMapUndefined, []);
    ...
    if (BatchMove.Mode in [dmAppend, dmUpdate, dmAppendUpdate, dmDelete]) and
       ...
       (FKeyFields = '') then
      FDException(BatchMove, [S_FD_LComp, S_FD_LComp_PDM], er_FD_DPDestNoKeyFields, []);
  except
    Unprepare;
    raise;
  end;
...
begin
  ...
  FOptions := [poIdentityInsert, poCreateDest, poSkipUnmatchedDestFields, poUseTransactions];
  FCommitCount := 100;
...
begin
  if Operation = opRemove then begin
    ...
    if ((AComponent.Owner = Self) or (Reader = nil)) and
       Supports(AComponent, IFDBatchMoveReader, oRdr) and (oRdr.GetBatchMove = nil) then
      Reader := oRdr
    else if ((AComponent.Owner = Self) or (Writer = nil)) and
       Supports(AComponent, IFDBatchMoveWriter, oWrt) and (oWrt.GetBatchMove = nil) then
      Writer := oWrt;
  end;
...
begin
  if ADriver <> AValue then begin
    ...
    ADriver := AValue;
    if ADriver <> nil then begin
      oBM := ADriver.GetBatchMove;
      if oBM <> nil then
        if oBM.Reader = AValue then
          oBM.Reader := nil
        else if oBM.Writer = AValue then
          oBM.Writer := nil;
      ...
      if eInitAction <> paSkip then begin
        ...
        while not Reader.Eof and not FAborting do begin
          ...
          try
            ...
            Reader.NextRecord;
            if (CommitCount > 0) and (CommitCount <> MaxInt) and (WriteCount mod CommitCount = 0) then
