...
begin
  ...
  if (InternalGetSharedCliHandle() = nil) and FTxSupported then begin
    ...
    if (FServerVersion >= ivInfx1110) and oPars.TxLastCommitted then
      InternalExecuteDirect('SET ENVIRONMENT USELASTCOMMITTED ''COMMITTED READ''', nil);
  end;
end;
...
{-------------------------------------------------------------------------------}
function TFDPhysInfxCommand.InternalOpen(var ACount: TFDCounter): Boolean;
begin
  if FOptions.UpdateOptions.LockWait <> TFDPhysInfxConnection(ODBCConnection).FWaitLock then
    UpdateWaitLock;
...
begin
  while not Terminated and FAlerter.IsRunning do
    try
      ...
      if not Terminated and (FAlerter.FWaitCommand.Params[2].AsInteger = 0) then
        FAlerter.DoFired;
    except
...
begin
  ...
  FWaitCommand.Options.ResourceOptions.PreprocessCmdText := False;
end;
...
var
  ...
  oPar: TFDParam;
begin
  FWaitCommand.CommandText := '{CALL dbms_alert_register(?)}';

  FWaitCommand.Params.Clear;
  oPar := FWaitCommand.Params.Add;
  oPar.Position := 1;
  oPar.Name := 'name';
  ...
  FWaitCommand.CommandText := '{CALL dbms_alert_waitany(?, ?, ?, ?)}';
...
begin
  ...
  if FWaitCommand <> nil then
    try
      FWaitCommand.CommandText := '{CALL dbms_alert_remove(?)}';
...
begin
  if FSignalCommand = nil then begin
    ...
    FSignalCommand.Options.ResourceOptions.PreprocessCmdText := False;

    FSignalCommand.CommandText := '{CALL dbms_alert_signal(?, ?)}';

    FSignalCommand.Params.Clear;
    oPar := FSignalCommand.Params.Add;
    oPar.Position := 1;
    oPar.Name := 'name';
    oPar.DataType := ftString;
    oPar.Size := 128;
