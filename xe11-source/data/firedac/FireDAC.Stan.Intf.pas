...
const
  ...
  C_FD_NonSearchableDataTypes = C_FD_InvariantDataTypes;
  ...
  siCurrent: TFDStoreItem = TFDStoreItem(Integer(High(TFDStoreItem)) + 1);

                                                                                                      
  ekSQL: TFDMoniEventKind = TFDMoniEventKind(Integer(High(TFDMoniEventKind)) + 1);
  ekSQLVarIn: TFDMoniEventKind = TFDMoniEventKind(Integer(High(TFDMoniEventKind)) + 2);
  ekSQLVarOut: TFDMoniEventKind = TFDMoniEventKind(Integer(High(TFDMoniEventKind)) + 3);
...
begin
  FLock.Enter;
  try
    i := FClasses.IndexOf(AStreamingName);
    if i <> -1 then
      FClasses.Delete(i);
