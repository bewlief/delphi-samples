...
begin
  ...
  FReadQuery.UpdateOptions.CheckReadOnly := True;
  DataSet := FReadQuery;
...
{-------------------------------------------------------------------------------}
function TFDBatchMoveSQLReader.GetFieldValue(AField: TObject): Variant;
begin
  Result := TField(AField).Value;
end;
...
var
  sName, sCat, sSch, sBObj, sObj: String;
  oGen: IFDPhysCommandGenerator;
  i: Integer;
  oPar: TFDParam;
  oFld: TField;
  lAvoidArrayDML: Boolean;
  rTable: TFDPhysTableParams;
begin
  inherited Open(AStartTx);
  FRecIndex := 0;

  if (ReadSQL = '') or (WriteSQL = '') then begin
    FActualConnection.CheckActive;
  ...
  if ReadSQL <> '' then
    ...
    if (sName <> '') and (sName[1] = '{') then begin
      ...
      FActualConnection.DecodeObjectName(sName, sCat, sSch, sBObj, sObj);
      if (sBObj <> '') and (sObj = '') then
        sObj := sBObj;
    end;
    FillChar(rTable, SizeOf(TFDPhysTableParams), #0);
    rTable.FTableCommand := tcBof;
    rTable.FCatalog := sCat;
    rTable.FSchema := sSch;
    rTable.FTable := sObj;
    rTable.FCustomWhere := '0=1';
    rTable.FReadOnly := True;
    for i := 0 to BatchMove.Mappings.Count - 1 do begin
      if i > 0 then
        rTable.FSelectFields := rTable.FSelectFields + ';';
      if BatchMove.Mappings[i].DestinationFieldName <> '' then
        rTable.FSelectFields := rTable.FSelectFields + BatchMove.Mappings[i].DestinationFieldName
      else
        rTable.FSelectFields := rTable.FSelectFields + BatchMove.Mappings[i].SourceFieldName;
    end;
    FReadQuery.SQL.Text := oGen.GenerateSelectTable(rTable);
  end;
  ...
  if poIdentityInsert in BatchMove.Options then
    for i := 0 to FReadQuery.FieldCount - 1 do begin
      ...
      else if caAutoInc in TFDDataSet(DataSet).GetFieldColumn(oFld).ActualAttributes then begin
        oFld.ReadOnly := False;
  ...
  if WriteSQL <> '' then
    ...
    FWriteQuery.Command.CommandKind := oGen.CommandKind;
  end;

  lAvoidArrayDML := False;
  ...
  for i := 0 to FWriteQuery.ParamCount - 1 do begin
    ...
    if oFld <> nil then begin
      ...
      lAvoidArrayDML := lAvoidArrayDML or
        (oPar.ArrayType = atTable) or
        (oPar.DataType in [ftBlob, ftMemo, ftGraphic, ftFmtMemo, ftParadoxOle,
                           ftDBaseOle, ftTypedBinary, ftOraBlob, ftOraClob, ftWideMemo]) or
        (oPar.DataType in [ftVarBytes, ftBytes, ftString, ftFixedChar,
                           ftWideString, ftFixedWideChar]) and
          (LongWord(oPar.Size) >= FWriteQuery.FormatOptions.MaxStringSize);
    end;
  end;
  if not lAvoidArrayDML then
...
begin
  ...
  if WriteSQL = '' then
    FWriteQuery.Command.CommandKind := skUnknown;
  inherited Close(AStopTxError);
...
begin
  if poUseTransactions in BatchMove.Options then
...
begin
  if poUseTransactions in BatchMove.Options then
    ...
    else if FActualConnection <> nil then begin
...
{-------------------------------------------------------------------------------}
procedure TFDBatchMoveSQLWriter.SetFieldValue(AField: TObject;
  const AValue: Variant);
begin
  TFDParam(AField).Values[FRecIndex] := AValue;
end;
