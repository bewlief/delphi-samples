...
type
  ...
  IFDDatSMechSort = interface (IUnknown)
    ...
    property CompareDataOptions: TFDCompareDataOptions read GetCompareDataOptions;
  end;
  ...
  public
    ...
    procedure Assign(AObj: TFDDatSObject); override;
    procedure RebuildForRow(AParentRow: TFDDatSRow);
    property ParentRow: TFDDatSRow read FParentRow write SetParentRow;
  end;
...
{$ENDIF}
    ...
    procedure SaveActualToStorage(const AStorage: IFDStanStorage);
    // R/O
...
{$WARNINGS ON}
  ...
  private
    ...
    function GetResultSetNum: Integer;
  protected
    procedure HandleNotification(AParam: PFDDatSNotifyParam); override;
    function GetManager: TFDDatSManager; override;
    procedure InternalSaveToStorage(const AStorage: IFDStanStorage); override;
...
var
  ...
  /// <summary> Controls maximum difference when comparing ftSingle field values.
  /// The default value is 0.00001. </summary>
  FDSingleDelta: Single = 0.00001;
  /// <summary> Controls maximum difference when comparing ftFloat field values.
  /// The default value is 0.0000000000001. </summary>
  FDDoubleDelta: Double = 0.0000000000001;
  /// <summary> Controls maximum difference when comparing ftExtended field values.
  /// The default value is 0.0000000000001. </summary>
...
{-------------------------------------------------------------------------------}
function TFDDatSBindedList.FindAnyName(const AName, ASourceName: String;
  out AIndex: Integer): Boolean;
begin
  if not FindRealIndex(AName, AIndex) then
    if ASourceName <> '' then
      AIndex := IndexOfSourceName(ASourceName)
    else
      AIndex := -1;
  Result := AIndex >= 0;
end;
...
procedure TFDDatSColumnList.FixAutoIncs(ARow: TFDDatSRow);
  ...
  begin
    for i := 0 to Count - 1 do
      if ItemsI[i].AutoIncrement then begin
        V := ARow.GetData(i);
        if not VarIsNull(V) then
          if ItemsI[i].AutoIncrementStep > 0 then begin
            if FAutoIncs[i] < V then
              FAutoIncs[i] := V;
          end
          else begin
            if FAutoIncs[i] > V then
...
begin
  ...
  if Result = nil then begin
    ...
    try
      Result.Mechanisms.AddStates(C_AllViewRows);
...
begin
  ...
  if soDigitsAsNumbers in SortOptions then
    Include(Result, coDigitsAsNumbers);
end;
...
begin
  ...
  if SortMech <> nil then begin
    eOpts := SortMech.CompareDataOptions;
    if Bottom <> nil then begin
...
{-------------------------------------------------------------------------------}
procedure TFDDatSMechChilds.RebuildForRow(AParentRow: TFDDatSRow);
begin
  LockNotification;
  try
    ParentRow := AParentRow;
...
begin
  ...
  if lIsNull1 and lIsNull2 or lIsEmpty1 and lIsEmpty2 then
    ...
    Result := (CompareData(AColumn, pBuff1, iLen1, pBuff2, iLen2, [coExactMatch]) = 0)
  else
    Result := False;
end;
...
function TFDDatSRow.CompareData(AColumn: Integer; ABuff1: Pointer;
  ...
  function CmpBCDAsExt(ABuff1, ABuff2: Pointer): TValueRelationship;
  var
  ...
  begin
    ...
    Result := CompareValue(FDStr2Float(s1, FormatSettings.DecimalSeparator),
      FDStr2Float(s2, FormatSettings.DecimalSeparator), FDExtendedDelta);
  end;
...
begin
  ...
  dtUInt64:
    if PUInt64(ABuff1)^ > PUInt64(ABuff2)^ then
      ...
      Result := 0;
  dtSingle:
    if FDSingleDelta = 0.0 then
      if PSingle(ABuff1)^ > PSingle(ABuff2)^ then
        Result := 1
      else if PSingle(ABuff1)^ < PSingle(ABuff2)^ then
        Result := -1
      else
        Result := 0
    else
      Result := CompareValue(PSingle(ABuff1)^, PSingle(ABuff2)^, FDSingleDelta);
  dtDouble:
    if FDDoubleDelta = 0.0 then
      if PDouble(ABuff1)^ > PDouble(ABuff2)^ then
        Result := 1
      else if PDouble(ABuff1)^ < PDouble(ABuff2)^ then
        Result := -1
      else
        Result := 0
    else
      Result := CompareValue(PDouble(ABuff1)^, PDouble(ABuff2)^, FDDoubleDelta);
  dtExtended:
    if FDExtendedDelta = 0.0 then
      if PExtended(ABuff1)^ > PExtended(ABuff2)^ then
        Result := 1
      else if PExtended(ABuff1)^ < PExtended(ABuff2)^ then
        Result := -1
      else
        Result := 0
    else
      Result := CompareValue(PExtended(ABuff1)^, PExtended(ABuff2)^, FDExtendedDelta);
  dtCurrency:
  ...
  dtFmtBCD:
    begin
      ...
      if Result = 0 then
        Result := CmpBCDAsExt(ABuff1, ABuff2);
    end;
  dtDate,
  dtTime:
    begin
  ...
  dtHMemo:
    Result := FDCompareAnsiStr(PFDAnsiString(ABuff1), PFDAnsiString(ABuff2),
      ADataLen1, ADataLen2, Self.Table.Locale, AOptions);
  dtWideString,
  dtWideMemo,
  dtWideHMemo,
  dtXML:
...
begin
  ...
  else if (RowState = rsInserted) or ((RowState = rsEditing) and (RowPriorState = rsInserted)) then begin
    if RowState = rsEditing then
      CancelEdit;
    UnregisterChange;
    oList := RowList;
...
{-------------------------------------------------------------------------------}
procedure TFDDatSRow.SaveActualToStorage(const AStorage: IFDStanStorage);
begin
  if FpProposed <> nil then
    SaveRowVersion(AStorage, '', FpProposed, rsImportingProposed)
  else if FpCurrent <> nil then
    SaveRowVersion(AStorage, '', FpCurrent, rsImportingCurent)
  else if FpOriginal <> nil then
    SaveRowVersion(AStorage, '', FpOriginal, rsImportingOriginal);
end;
...
begin
  ...
  AOpts := AOpts + [coNullsNotEqual] - [coNoSymbols, coDigitsAsNumbers];
...
begin
  ...
  if AStorage.IsStored(siCurrent) then begin
    oFilter := AStorage.GetFilterObj(Self);
    if (oFilter <> nil) and (oFilter is TFDDatSRow) and
       (IndexOf(TFDDatSRow(oFilter)) <> -1) then begin
      oRow := TFDDatSRow(oFilter);
...
{-------------------------------------------------------------------------------}
procedure TFDDatSRowListWithAggregates.Clear;
var
  i: Integer;
begin
  inherited Clear;
  if FAggregateValues <> nil then
...
begin
  ...
  FLastRowCount := -1;
  FRowIDOrdered := True;
  if (Table = nil) or (Table.Manager = nil) then
    Inc(FResultSetNum);
end;
...
begin
  ...
  for i := 0 to Count - 1 do begin
    ...
    if oView.Actual then
      if oView.Mechanisms.Count = 1 then
        oMech := oView.Mechanisms.ItemsI[0]
      else if oView.Mechanisms.Count = 2 then begin
        oMech := oView.Mechanisms.ItemsI[0];
        if (oMech is TFDDatSMechRowState) and (TFDDatSMechRowState(oMech).RowStates = C_AllViewRows) then
          oMech := oView.Mechanisms.ItemsI[1]
        else
          oMech := nil;
      end;

    if (oMech <> nil) and (oMech.Kinds = [mkSort]) and
       Supports(oMech, IFDDatSMechSort, oSortMech) and
...
begin
  ...
  if Manager <> nil then begin
    ...
    Rows.FResultSetNum := Manager.GetResultSetNum;
  end;
end;
...
{-------------------------------------------------------------------------------}
procedure TFDDatSTable.Notify(AParam: PFDDatSNotifyParam);
begin
  inherited Notify(AParam);
  if (AParam^.FKind = snObjectNameChanged) and (AParam^.FParam1 = NativeInt(Self)) then begin
    if (FSourceName = PFDDatSNamedObjectUpdateInfo(AParam^.FParam2)^.FPrevName) and
       (FSourceName = GetDefaultName()) then
      FSourceName := Name;
  end;
end;
...
begin
  ...
  if AStorage.IsStored(siData) or AStorage.IsStored(siDelta) then
    if AStorage.IsStored(siVisible) then begin
      oFilter := AStorage.GetFilterObj(Self);
      if (oFilter <> nil) and (oFilter is TFDDatSView) and
         (Views.IndexOf(TFDDatSView(oFilter)) <> -1) then
...
{-------------------------------------------------------------------------------}
function TFDDatSManager.GetResultSetNum: Integer;
begin
  Inc(FResultSetNum);
  Result := FResultSetNum;
end;
...
begin
  ...
  for i := 0 to AManager.Tables.Count - 1 do begin
    ...
    oDestTab := nil;
    if not Tables.FindAnyName(oSrcTab.Name, oSrcTab.SourceName, iTab) then begin
  ...
  if AMeta in [mmMerge, mmAdd] then
    for i := 0 to AManager.Relations.Count - 1 do begin
      ...
      if not Relations.FindRealIndex(oSrcRel.Name, iRel) then begin
        oParTab := nil;
        oChldTab := nil;
        if (oSrcRel.ParentTable <> nil) and Tables.FindAnyName(oSrcRel.ParentTable.Name, oSrcRel.ParentTable.SourceName, iPar) then
          oParTab := Tables.ItemsI[iPar];
        if (oSrcRel.ChildTable <> nil) and Tables.FindAnyName(oSrcRel.ChildTable.Name, oSrcRel.ChildTable.SourceName, iChld) then
