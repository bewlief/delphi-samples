...
resourcestring
  ...
  S_FD_Docu_MonitorLink = 'http://docwiki.embarcadero.com/RADStudio/en/FDMonitor';
  ...
  S_FD_AccCommandMBFilled = 'Command [%s] text must be not empty';
  S_FD_AccEscapeEmptyName = 'Escape function name must be not empty';
  ...
  S_FD_AccUnknownOleError = 'Unknown OLE error [%s]';
  ...
  S_FD_AccLongDataStream = 'Cannot %s stream for variable [%s]';
  S_FD_AccArrayDMLWithIntStr = 'Internal streams cannot be used in Array DML';
  ...
  S_FD_DPNoJsonDest = 'Destination for JSON output must be specified';
  S_FD_DPNoJsonSrc = 'Source of JSON input must be specified';
  S_FD_DPNoJsonFld = 'JSON field [%s] is not found';
  S_FD_DPJsonFldDup = 'JSON field [%s] name is Duplicated';
  S_FD_DPMapUndefined = 'Mapping of source to destination fields is not defined';
  ...
  S_FD_MySQLBadVersion = 'Unsupported MySQL version [%d]. Supported are client and server from v 3.20';
  ...
  S_FD_MySQLFieldDataTypeUnsup = 'Field [%d] data type [%d] is not supported';
  ...
  S_FD_PgUnsupTextType = 'Direct execution does not support type of variable [%s]. Only simple types are supported';
  S_FD_PgProcCantResult = 'Stored procedure [%s] cannot have a result parameter';
  ...
  S_FD_SQLiteLinkageNotSupported = 'SQLite library linkage [%s] is not found. Hint: If static linkage is requested and supported, include [FireDAC.Phys.SQLiteWrapper.Stat] into your project';
  ...
  S_FD_FindNotFoundRestart = 'Text not found. Restart search ?';
  S_FD_FindNotFound = 'Text not found';
  S_FD_GotoCaption = 'Go to Line Number';
  S_FD_GotoPrompt = 'Enter new line number';
  ...
  S_FD_MSSQLWarnSQLSRV = 'Warning: "SQL Server" ODBC driver is deprecated. Upgrade to newer SQL Server ODBC driver.';
  ...
  S_FD_MSSQLWarn2106Dt = 'Warning: SQL Server 2016 and compatibility level >= 130 may lead to DATETIME comparision failure.';
  ...
  S_FD_MySQLWarnNoMR = 'Warning: The client [%s] version or less does not support ' +
                       'multiple result sets or output parameters of prepared statement.';
  ...
  S_FD_OracleWarnLeak = 'Warning: The client version [%s] may have major resources and memory leaks.';
