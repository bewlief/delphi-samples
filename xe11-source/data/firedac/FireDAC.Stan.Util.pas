...
{$ENDIF}
  ...
  protected
    class function ThreadOwned: Boolean; virtual;
  public
...
*)
  ...
  public
    ...
    constructor Create(AStream: TStream; AOwned, ARead, AAppend: Boolean;
      AEncoding: TFDEncoding; AEndOfLine: TFDTextEndOfLine); overload;
    destructor Destroy; override;
    class function GetEOLStr(AEndOfLine: TFDTextEndOfLine): String; static;
...
function FDRemoveChar(const AStr: String; AChar: Char): String;
...
function FDStartsWithKW(const ASQL, AKw: String; var AAfterKw: Integer): Boolean;
function FDHasKW(const ASQL, AKW: String): Boolean;
...
function FDISOStr2Date(AValue: PChar; ALen: Integer): Integer;
function FDISOStr2Time(AValue: PChar; ALen: Integer): Integer;
function FDISOStr2TimeStamp(AValue: PChar; ALen: Integer; var ATimestamp: TSQLTimeStamp): Boolean;
function FDISOStr2DateTime(AValue: PChar; ALen: Integer; var ADatetime: Comp): Boolean;
...
{-------------------------------------------------------------------------------}
function TFDReadSharedMemoryStream.Realloc(var NewCapacity: NativeInt): Pointer;
begin
  if not FShared then
...
{-------------------------------------------------------------------------------}
class function TFDThreadMsgBase.ThreadOwned: Boolean;
begin
  Result := True;
end;
...
{$IFDEF MSWINDOWS}
    repeat
      if (iCurrTID = MainThreadID) and AWait then
        ...
        Sleep(10);
      iExitCode := 0;
    until not (AWait and GetExitCodeThread(hMyHandle, iExitCode) and (iExitCode = STILL_ACTIVE));
{$ENDIF}
{$IFDEF POSIX}
    repeat
      if (iCurrTID = MainThreadID) and AWait then
        ...
        Sleep(10);
    until not (AWait and (pthread_kill(pthread_t(iMyTID), 0) = 0));
{$ENDIF}
end;
...
begin
  ...
  if FBufferSize < ALen then begin
    ...
    lAlloc := (FOrigBuffer = nil);
  if lAlloc then
    ReallocMem(FOrigBuffer, FBufferSize);
  FBuffer := FOrigBuffer;
  Result := FOrigBuffer;
end;
...
begin
  if FOrigBuffer <> nil then begin
    FreeMem(FOrigBuffer, FBufferSize);
    FOrigBuffer := nil;
  end;
  FBuffer := nil;
  ...
  case ADestEncoding of
    ecANSI:
      begin
        ...
        iRes := WideCharToMultiByte(FCodePage, 0, PWideChar(AStr),
        ...
        if iRes = 0 then begin
          ...
          WideCharToMultiByte(FCodePage, 0, PWideChar(AStr),
  ...
  case ASrcEncoding of
    ecANSI:
      begin
        ...
        MultiByteToWideChar(FCodePage, 0, LPCSTR(PByte(AStr)),
    ...
    case ADestEncoding of
      ecANSI:
        begin
          ...
          if ADest = nil then
            ADest := Alloc(iMaxLen);
          Result := LocaleCharsFromUnicode(FCodePage, 0, PWideChar(ASrc),
    ...
    case ADestEncoding of
      ...
      ecUTF8:
        begin
          ...
          iLen := UnicodeFromLocaleChars(FCodePage, 0, MarshaledAString(ASrc),
            ASrcLen, pWC, ASrcLen);
      ...
      ecUTF16:
        begin
          ...
          Result := UnicodeFromLocaleChars(FCodePage, 0, MarshaledAString(ASrc),
      ...
      ecUTF8:
        begin
          ...
          iLen := UnicodeFromLocaleChars(CP_UTF8, 0, MarshaledAString(ASrc),
            ASrcLen, pWC, ASrcLen);
          Result := LocaleCharsFromUnicode(FCodePage, 0, pWC, iLen,
...
begin
  ...
  if not ARead then begin
    ...
    oStream := TBufferedFileStream.Create(AFileName, fmOpenReadWrite or fmShareDenyWrite);
  end
  else
...
{$IFDEF MSWINDOWS}
  GetCPInfo(CP_ACP, rInfo);
  FANSIBytesPerChar := rInfo.MaxCharSize;
{$ELSE}
  FANSIBytesPerChar := 1;
{$ENDIF}
...
{-------------------------------------------------------------------------------}
class function TFDTextFile.GetEOLStr(AEndOfLine: TFDTextEndOfLine): String;
begin
  case AEndOfLine of
  elDefault: {$IFDEF MSWINDOWS} Result := C_FD_WinEOL {$ENDIF}
             {$IFDEF POSIX}     Result := C_FD_PosixEOL {$ENDIF};
  elWindows: Result := C_FD_WinEOL;
  elPosix:   Result := C_FD_PosixEOL;
  end;
end;
...
begin
  ...
  ecANSI:
    begin
{$IFDEF MSWINDOWS}
      if FANSIBytesPerChar = 1 then
{$ENDIF}
        iBytes := FStream.Read(aBuff[0], ACount)
{$IFDEF MSWINDOWS}
      else begin
        iBytes := 0;
        iChars := 0;
        while (iBytes < 4095) and (iChars < ACount) do begin
          ...
          c := Byte(aBuff[iBytes]);
          Inc(iBytes);
          if IsDBCSLeadByte(c) then begin
            if FStream.Read(aBuff[iBytes], 1) <> 1 then
              Break;          // incomplete DBCS char
            Inc(iBytes);
          end;
          Inc(iChars);
        end;
...
function FDStrLike(const AStr, AMask: String; ANoCase: Boolean = False;
  ...
  begin
    Result := True;
    while (pStr^ <> #0) and (pMask^ <> #0) and (pMask^ <> AManyCharsMask) and
          ((pStr^ = pMask^) or (pMask^ = AOneCharMask) or (pMask^ = AEscapeChar)) do begin
      if pMask^ = AEscapeChar then begin
        Inc(pMask);
        if pStr^ <> pMask^ then begin
          Dec(pMask);
          Break;
        end;
      end;
    ...
    if pMask^ = AManyCharsMask then begin
      ...
      if pMask^ <> #0 then begin
        while (pStr^ <> #0) and not InternalScan(pStr, pMask) do
          Inc(pStr);
        Result := pStr^ <> #0;
      end;
    end
...
begin
  i := 1;
  while (i <= Length(ASQL)) and FDInSet(ASQL[i], [' ', #13, #10, #9]) do
    Inc(i);
  Result := StrLIComp(PChar(ASQL) + i - 1, PChar(AKw), Length(AKw)) = 0;
  if Result then
    AAfterKw := i + Length(AKw);
end;
...
{$IFDEF MSWINDOWS}
  function CompareAsWideStr: Integer;
  var
    s1, s2: UnicodeString;
  begin
    s1 := TFDEncoder.Deco(AStr1, ALen1, ecANSI);
    s2 := TFDEncoder.Deco(AStr2, ALen2, ecANSI);
    Result := FDCompareWideStr(PWideChar(s1), PWideChar(s2), ALen1, ALen2, ALocale, AOptions);
  end;
{$ENDIF}
...
{$IFDEF MSWINDOWS}
  if coExactMatch in AOptions then
    ...
    if coNoCase in AOptions then
      ...
      Result := {$IFNDEF NEXTGEN} System.AnsiStrings. {$ENDIF}
        StrLComp(PFDAnsiString(AStr1), PFDAnsiString(AStr2), iLen)
  else if coDigitsAsNumbers in AOptions then
    // CompareStringA always returns error=1004 (invalid flags) when SORT_DIGITSASNUMBERS in flags
    Result := CompareAsWideStr
  else begin
    if coNoCase in AOptions then
      iFlag := NORM_IGNORECASE or SORT_STRINGSORT
    else
      iFlag := SORT_STRINGSORT;
    if coNoSymbols in AOptions then
      iFlag := iFlag or NORM_IGNORESYMBOLS or NORM_IGNORENONSPACE;
    SetLastError(0);
    if coPartial in AOptions then
...
{$ENDIF}
{$IFDEF POSIX}
  if coNoCase in AOptions then begin
...
{$ENDIF}
  if Result > 0 then
    ...
    if ALen1 < ALen2 then
      Result := -1
    else if (ALen1 > ALen2) and not (coPartial in AOptions) then
      Result := 1;
end;
...
{$IFDEF MSWINDOWS}
  ...
  begin
    ...
    if coNoSymbols in AOptions then
      iFlag := iFlag or NORM_IGNORESYMBOLS or NORM_IGNORENONSPACE;
    if coDigitsAsNumbers in AOptions then
      iFlag := iFlag or SORT_DIGITSASNUMBERS;
    SetLastError(0);
    if coPartial in AOptions then
...
{$IFDEF POSIX}
  ...
  // See SVN # 874
  if coPartial in AOptions then begin
...
begin
  ...
  p2 := Pointer(Result);
  while p1^ <> #0 do begin
    if p1^ <> AChar then begin
      p2^ := p1^;
      Inc(p2);
    end;
    Inc(p1);
  end;
  SetLength(Result, p2 - PChar(Pointer(Result)));
end;
...
begin
  FDStr2Int(AValue,     4, @wYear, SizeOf(Word), True);
  FDStr2Int(AValue + 5, 2, @wMonth, SizeOf(Word), True);
  FDStr2Int(AValue + 8, 2, @wDay, SizeOf(Word), True);
  if (wYear = 0) and (wMonth = 0) and (wDay = 0) then
    Result := 0
  else
    Result := FDDateTime2Date(EncodeDate(wYear, wMonth, wDay));
end;
...
begin
  if AValue^ = '-' then begin
    ...
    lNeg := False;
  FDStr2Int(AValue,     2, @wHour, SizeOf(Word), True);
  FDStr2Int(AValue + 3, 2, @wMin, SizeOf(Word), True);
  FDStr2Int(AValue + 6, 2, @wSec, SizeOf(Word), True);
  if (ALen >= 9) and (AValue[8] = '.') then begin
    FDStr2Int(AValue + 9, ALen - 9, @lwFrac, SizeOf(Cardinal), True);
    if ALen = 10 then
      wMSec := lwFrac * 100
    else if ALen <= 11 then
      wMSec := lwFrac * C_FD_ScaleFactor[ALen - 10]
    else if ALen >= 13 then
      wMSec := lwFrac div C_FD_ScaleFactor[ALen - 12]
    else
      wMSec := lwFrac;
  end
  else
    wMSec := 0;
  Result := FDDateTime2Time(EncodeTime(wHour, wMin, wSec, wMSec));
...
begin
  Result := True;
  if ALen >= 19 then begin
    FDStr2Int(AValue,     4, @wYear, SizeOf(Word), True);
    FDStr2Int(AValue + 5, 2, @wMonth, SizeOf(Word), True);
    FDStr2Int(AValue + 8, 2, @wDay, SizeOf(Word), True);
    FDStr2Int(AValue + 11,2, @wHour, SizeOf(Word), True);
    FDStr2Int(AValue + 14,2, @wMin, SizeOf(Word), True);
    FDStr2Int(AValue + 17,2, @wSec, SizeOf(Word), True);
    if (ALen >= 20) and (AValue[19] = '.') then begin
      FDStr2Int(AValue + 20, ALen - 20, @lwFrac, SizeOf(Cardinal), True);
      if ALen = 21 then
        wMSec := lwFrac * 100
      else if ALen <= 22 then
        wMSec := lwFrac * C_FD_ScaleFactor[ALen - 21]
      else if ALen >= 24 then
        wMSec := lwFrac div C_FD_ScaleFactor[ALen - 23]
      else
        wMSec := lwFrac;
    end
    else
    ...
    wSec := 0;
    wMSec := 0;
    if (ALen = 14) or (ALen = 8) then begin
      FDStr2Int(AValue, 4, @wYear, SizeOf(Word), True);
      iBase := 4;
    end
    else begin
      FDStr2Int(AValue, 2, @wYear, SizeOf(Word), True);
      iBase := 2;
    end;
    if ALen > 2 then begin
      FDStr2Int(AValue + iBase, 2, @wMonth, SizeOf(Word), True);
      if ALen > 4 then begin
        FDStr2Int(AValue + iBase + 2, 2, @wDay, SizeOf(Word), True);
        if ALen >= 10 then begin
          FDStr2Int(AValue + iBase + 4, 2, @wHour, SizeOf(Word), True);
          FDStr2Int(AValue + iBase + 6, 2, @wMin, SizeOf(Word), True);
          if ALen > 10 then
            FDStr2Int(AValue + iBase + 8, 2, @wSec, SizeOf(Word), True);
        end;
      end;
    end;
  end;
  if (wYear = 0) and (wMonth = 0) and (wDay = 0) then begin
    FillChar(ATimestamp, SizeOf(TSQLTimeStamp), 0);
    Result := False;
  end
  else begin
    ATimestamp.Year := wYear;
    ATimestamp.Month := wMonth;
    ATimestamp.Day := wDay;
    ATimestamp.Hour := wHour;
    ATimestamp.Minute := wMin;
    ATimestamp.Second := wSec;
    ATimestamp.Fractions := wMSec;
  end;
end;
...
begin
  ...
  if (wYear = 0) and (wMonth = 0) and (wDay = 0) then begin
    ...
    ADatetime := FDDateTime2MSecs(EncodeDateTime(wYear, wMonth, wDay, wHour, wMin,
      wSec, wMSec));
  end;
end;
...
begin
  ...
  if (iExponent < 0) and (-iExponent + iSize >= SizeOf(ABcd.Fraction) * 2) then begin
    iSize := SizeOf(ABcd.Fraction) * 2 + iExponent - 1;
    if iSize < 0 then
      iSize := 0;
  end
  else if (iExponent > 0) and (iExponent > SizeOf(ABcd.Fraction) * 2) then
    // iSize := 0;
                                                                                      
    raise EBcdOverflowException.Create('BCD overflow'); // do not translate
