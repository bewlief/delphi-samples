...
type
  ...
  private
    FAGKR: Integer;
    FAutoGenCol: TODBCColumn;
    procedure DoExtendedDescribe;
    procedure DoINTODescribe;
    procedure UpdateInfoAfterExecution;
  protected
    // TFDPhysODBCCommand
...
begin
  ...
  Result := TFDPhysTDataMetadata.Create(Self, iSrvVer, iClntVer, GetKeywords,
    FExtendedMetadata, FSessionMode, FCharacterSet);
end;
...
begin
  ...
  if UpperCase(Trim(QueryValue('SELECT transaction_mode FROM dbc.SessionInfoV ' +
    ...
    FSessionMode := tmANSI;
  FCharacterSet := ConnectionDef.AsString[S_FD_ConnParam_Common_CharacterSet];
end;
...
{-------------------------------------------------------------------------------}
function TFDPhysTDataConnection.GetLastAutoGenValue(const AName: String): Variant;
begin
  if AName = '' then
...
begin
  FAGKR := SQL_AGKR_NO;
  if (GetMetaInfoKind = mkNone) and (FOptions.UpdateOptions.RefreshMode <> rmManual) then
    case GetCommandKind of
    skSelect,
    skSelectForLock,
    skSelectForUnLock:
      if (StrLIComp(PChar(FDbCommandText), PChar('INSERT'), 6) = 0) or
...
{-------------------------------------------------------------------------------}
procedure TFDPhysTDataCommand.InternalUnprepare;
begin
  FAutoGenCol := nil;
...
begin
  ...
  if (FAGKR = SQL_AGKR_NO) and (GetMetaInfoKind = mkNone) and
...
{-------------------------------------------------------------------------------}
procedure TFDPhysTDataCommand.UpdateInfoAfterExecution;
var
  pData: SQLPointer;
  iLen: SQLLen;
begin
  TFDPhysTDataConnection(ODBCConnection).SaveLastAutoGenValue(Null);
  case GetCommandKind of
  skSetSchema:
    TFDPhysTDataConnection(ODBCConnection).UpdateCurrentSchema;
  skInsert,
  skMerge:
    if (FAGKR = SQL_AGKR_IDENTITY_COLUMN) and
       (ODBCStatement.NumResultCols = 1) and (Length(FColInfos) = 0) then begin
      if FAutoGenCol = nil then
        FAutoGenCol := ODBCStatement.AddCol(1, SQL_INTEGER, SQL_C_LONG);
      if (ODBCStatement.Fetch(1) = 1) and FAutoGenCol.GetData(0, pData, iLen, True) then
        TFDPhysTDataConnection(ODBCConnection).SaveLastAutoGenValue(PSQLInteger(pData)^);
    end;
  end;
end;
...
{-------------------------------------------------------------------------------}
function TFDPhysTDataCommand.InternalOpen(var ACount: TFDCounter): Boolean;
begin
