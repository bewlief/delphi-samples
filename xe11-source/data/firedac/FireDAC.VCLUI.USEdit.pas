...
var
  mmSQLIntfs: array [0..5] of IFDGUIxFormsMemo;
...
begin
  for i := 0 to 5 do begin
    if Assigned(FDGUIxCreateMemoFunc) then
      oSQL := TFDGUIxFormsMemo(FDGUIxCreateMemoFunc(Self))
    else
      oSQL := TFDGUIxFormsMemo.Create(Self);
    Supports(oSQL, IFDGUIxFormsMemo, mmSQLIntfs[i]);
    ...
    oSQL.Align := alNone;
    oSQL.Visible := True;
    if Assigned(mmSQLIntfs[i]) then begin
      mmSQLIntfs[i].SetOnExit(mmSQLExit);
      mmSQLIntfs[i].SetOnKeydown(mmSQLKeyDown);
    end
    else begin
      TFDGUIxFormsMemo(oSQL).OnExit := mmSQLExit;
      TFDGUIxFormsMemo(oSQL).OnKeyDown := mmSQLKeyDown;
    end;
  end;
end;
...
begin
  Result := TFDGUIxFormsMemo(pcSQL.Pages[AIndex].Controls[0]);
end;
...
begin
  for i := 0 to 5 do begin
    ...
    if Assigned(mmSQLIntfs[i]) then
      lEmpty := mmSQLIntfs[i].GetLines.Count = 0
    else
      lEmpty := GetSQL(i).Lines.Count = 0;
...
begin
  ...
  try
    ...
    if FOpts.UpdateOptions.EnableInsert then
      if Assigned(mmSQLIntfs[0]) then
        mmSQLIntfs[0].GetLines.Text := oCmdGen.GenerateInsert
      else
        GetSQL(0).Lines.Text := oCmdGen.GenerateInsert;
    if FOpts.UpdateOptions.EnableUpdate then
      if Assigned(mmSQLIntfs[1]) then
        mmSQLIntfs[1].GetLines.Text := oCmdGen.GenerateUpdate
      else
        GetSQL(1).Lines.Text := oCmdGen.GenerateUpdate;
    if FOpts.UpdateOptions.EnableDelete then
      if Assigned(mmSQLIntfs[2]) then
        mmSQLIntfs[2].GetLines.Text := oCmdGen.GenerateDelete
      else
        GetSQL(2).Lines.Text := oCmdGen.GenerateDelete;
    if FOpts.UpdateOptions.LockMode <> lmNone then begin
      if Assigned(mmSQLIntfs[3]) then
        mmSQLIntfs[3].GetLines.Text := oCmdGen.GenerateLock
      else
        GetSQL(3).Lines.Text := oCmdGen.GenerateLock;
      if Assigned(mmSQLIntfs[4]) then
        mmSQLIntfs[4].GetLines.Text := oCmdGen.GenerateUnLock
      else
        GetSQL(4).Lines.Text := oCmdGen.GenerateUnLock;
    end;
    if Assigned(mmSQLIntfs[5]) then
      mmSQLIntfs[5].GetLines.Text := oCmdGen.GenerateSelect(False)
    else
...
begin
  ...
  try
    ...
    for i := 0 to 5 do
      if Assigned(mmSQLIntfs[i]) then
        mmSQLIntfs[i].SetRDBMSKind(FConnection.RDBMSKind)
      else
        GetSQL(i).RDBMSKind := FConnection.RDBMSKind;
    if Assigned(mmSQLIntfs[0]) then begin
      mmSQLIntfs[0].GetLines.SetStrings(AUpdSQL.InsertSQL);
      mmSQLIntfs[1].GetLines.SetStrings(AUpdSQL.ModifySQL);
      mmSQLIntfs[2].GetLines.SetStrings(AUpdSQL.DeleteSQL);
      mmSQLIntfs[3].GetLines.SetStrings(AUpdSQL.LockSQL);
      mmSQLIntfs[4].GetLines.SetStrings(AUpdSQL.UnlockSQL);
      mmSQLIntfs[5].GetLines.SetStrings(AUpdSQL.FetchRowSQL);
    end
    else begin
  ...
  if Result then
    if Assigned(mmSQLIntfs[0]) then begin
      AUpdSQL.InsertSQL.SetStrings(mmSQLIntfs[0].GetLines);
      AUpdSQL.ModifySQL.SetStrings(mmSQLIntfs[1].GetLines);
      AUpdSQL.DeleteSQL.SetStrings(mmSQLIntfs[2].GetLines);
      AUpdSQL.LockSQL.SetStrings(mmSQLIntfs[3].GetLines);
      AUpdSQL.UnlockSQL.SetStrings(mmSQLIntfs[4].GetLines);
      AUpdSQL.FetchRowSQL.SetStrings(mmSQLIntfs[5].GetLines);
    end
    else begin
