...
begin
  if GetServerVersion >= cvOracle122000 then
    Result := 128
  else
    Result := 30;
end;
  ...
  efTIMESTAMPDIFF:
    begin
      ...
      if A1 = 'YEAR' then
        ...
        Result := 'ROUND(((' + A3 + ') - (' + A2 + ')) * (24.0 * 60.0 * 60.0 * 1000000.0))'
      else
        UnsupportedEscape(ASeq);
    end;
...
begin
  ...
  if FConnMeta.ServerVersion >= cvOracle122000 then
    sMaxLen := '128'
  else
    sMaxLen := '30';
  ...
  mkIndexFields:
    begin
      Result := 'SELECT TO_NUMBER(NULL) AS RECNO, TO_CHAR(NULL) AS CATALOG_NAME, ' +
        ...
        'SUBSTR(C.COLUMN_NAME, 1, ' + sMaxLen + ') AS COLUMN_NAME, C.COLUMN_POSITION, ';
      if FConnMeta.ServerVersion >= cvOracle81500 then
  ...
  mkPrimaryKeyFields:
    begin
      ...
      GetIndexExpressions(sIndexName, sOwnerName, sToChar, 'C');
      Result := 'SELECT TO_NUMBER(NULL) AS RECNO, TO_CHAR(NULL) AS CATALOG_NAME, ' +
        'INDEX_OWNER AS SCHEMA_NAME, TABLE_NAME, INDEX_NAME, SUBSTR(COLUMN_NAME, 1, ' + sMaxLen + ') AS COLUMN_NAME, ' +
        'COLUMN_POSITION, ';
      if FConnMeta.ServerVersion >= cvOracle81500 then
...
begin
  ...
  if (Pos('NEXTVAL', sSQL) = 0) and (Pos('CURRVAL', sSQL) = 0) then
    if FConnMeta.ServerVersion >= cvOracle121000 then
      if (ASkip > 0) and (ARows + ASkip <> MAXINT) then
        ...
        Result := ASQL + BRK + 'FETCH FIRST ' + IntToStr(ARows) + ' ROWS ONLY'
      else begin
        Result := ASQL;
        AOptions := [];
      end
