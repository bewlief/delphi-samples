...
uses
  System.Types, System.Classes, System.SyncObjs, Data.SqlTimSt,
...
type
  ...
  TIBUpdateStatus = set of (iuModified, iuUpdated, iuInserted, iuDeleted, iuTruncated);
  ...
  private
    ...
    procedure LoadBase(const AVendorHome, AVendorLib: String; AThreadSafe: Boolean
  ...
  public
    ...
    procedure LoadFB(const AVendorHome, AVendorLib: String; AEmbedded, AThreadSafe: Boolean);
  ...
  private
    ...
    function GetAsBlock: TFDByteString; inline;
    function GetAsPByte: PISC_SCHAR; inline;
    function GetLen: SmallInt; inline;
  public
    constructor Create(ALib: TIBLib; AEncoder: TFDEncoder);
...
{$ENDIF}
  ...
  public
    ...
    // FB30x
    property crypt_state: Integer index fb_info_crypt_state read GetDatabaseInfoInt;
  end;
...
{$ENDIF}
  ...
  public
    ...
    property GUIDEndian: TEndian read FGUIDEndian write FGUIDEndian;
...
*)
  ...
  public
    ...
    property SkipData: String read FSkipData write FSkipData;
  ...
  TIBOnlineValidate = class (TIBService)
  private
    FDatabaseName: String;
  ...
  protected
    ...
    procedure QueryInfo(const ARequest: TRequestProc; const AParse: TParseFunc);
  public
  ...
  public
    ...
    FFBUtil: IUtil;
    FFBInt128: IInt128;
    FFBDec16: IDecFloat16;
    FFBDec34: IDecFloat34;

    Ffb_get_master_interface: Tfb_get_master_interface;
    Ffb_dsql_set_timeout: Tfb_dsql_set_timeout;
    Ffb_get_statement_interface: Tfb_get_statement_interface;
  ...
  TIBDatabaseExt = class(TIBDatabase)
  private
    FIStatus: IStatus;
  public
    constructor Create(AEnv: TIBEnv; AOwningObj: TObject);
    constructor CreateUsingHandle(AEnv: TIBEnv; AHandle: isc_db_handle;
      AOwningObj: TObject);
    destructor Destroy; override;
    property IStatus: IStatus read FIStatus;
  end;

                                                                                     
  TIBVariablesExt = class(TIBVariables)
  private
    FFBMeta: IMessageMetadata;
  public
    procedure Unprepare;
  end;

                                                                                     
  TIBStatementExt = class(TIBStatement)
  private
  ...
  public
    ...
    property sql_stmt_blob_align: Integer index isc_info_sql_stmt_blob_align read GetSQLInfoInt;
  end;
...
const
  ...
  sfb_get_master_interface: String = 'fb_get_master_interface';
  sfb_dsql_set_timeout: String = 'fb_dsql_set_timeout';
  sfb_get_statement_interface: String = 'fb_get_statement_interface';
...
begin
  if (FhDLL <> 0) and not IsLibrary and
     (FBrand = ibFirebird) and FEmbedded and
     Assigned(Ffb_shutdown) then
    Ffb_shutdown(C_FD_PhysManagerShutdownTimeout, 1);
{$IFDEF LINUX}
  // temporary workaround for libgds.so issue
  // do not unload InterBase client library - let process exit take care of the cleanup
...
{$IFDEF USE_API}
  ...
  if Assigned(Fisc_get_client_version) then begin
    ...
    if Brand <> ibInterbase then begin
      aStr := sVer2.Split([' ', '-', '.'], TStringSplitOptions.ExcludeEmpty);
...
{-------------------------------------------------------------------------------}
procedure TIBLibExt.LoadFB(const AVendorHome, AVendorLib: String; AEmbedded,
  AThreadSafe: Boolean);
begin
  inherited LoadFB(AVendorHome, AVendorLib, AEmbedded, AThreadSafe);
  if Assigned(Ffb_get_master_interface) and (Version >= ivFB040000) then begin
    FFBMaster := Ffb_get_master_interface();
    FFBUtil := FFBMaster.getUtilInterface();
    FFBInt128 := FFBUtil.getInt128(nil);
    FFBDec16 := FFBUtil.getDecFloat16(nil);
    FFBDec34 := FFBUtil.getDecFloat34(nil);
  end;
end;

{-------------------------------------------------------------------------------}
procedure TIBLibExt.Unload;
begin
  FFBDec16 := nil;
  FFBDec34 := nil;
  FFBInt128 := nil;
...
{-------------------------------------------------------------------------------}
procedure TIBLibExt.LoadEntries;
begin
  inherited LoadEntries;
{$IFDEF FireDAC_IB_STATIC}
  {$IFDEF FireDAC_IB_STATIC_FBBRAND}
  @Ffb_get_master_interface := @fb_get_master_interface;
  @Ffb_dsql_set_timeout := @fb_dsql_set_timeout;
  @Ffb_get_statement_interface := @fb_get_statement_interface;
  {$ELSE}
  @Ffb_get_master_interface := nil;
  @Ffb_dsql_set_timeout := nil;
  @Ffb_get_statement_interface := nil;
  {$ENDIF}
{$ELSE}
  @Ffb_get_master_interface := GetProc(sfb_get_master_interface, False);
  @Ffb_dsql_set_timeout := GetProc(sfb_dsql_set_timeout, False);
  @Ffb_get_statement_interface := GetProc(sfb_get_statement_interface, False);
{$ENDIF}
end;
...
begin
  if (AStatus = 0) and not Error.IsWarning() then
    Exit;
...
procedure CheckIError(ASelf: TIBObject; AStatus: IStatus);
begin
                                                 
  if ((AStatus.getState and AStatus.STATE_ERRORS) <> 0) then begin
    ASelf.Error.FStatus := AStatus.getState;
    Move(Pointer(AStatus.getErrors)^, ASelf.Error.FStatusVector, SizeOf(TISCStatusVector));
    ASelf.Error.Check(AStatus.getState);
  end;
end;
...
var
  i, iLen, iExcCode, iSQLCode, iStatusLen, iTakeObjName, i1, i2, iFrom: Integer;
...
begin
  if (AStatus = 0) and IsWarning() then begin
    FStatus := FStatusVector[3];
    ClearInfo;
    FInfo := EIBNativeException.Create(Self);
  end
  else if AStatus <> 0 then begin
    FStatus := AStatus;
...
{-------------------------------------------------------------------------------}
function TIBError.IsWarning: Boolean;
begin
  Result := (FStatusVector[2] = isc_arg_warning) and
    (FStatusVector[1] = 0) and
...
begin
  ...
  Result := PByte(FBuffer) + FFinalSize - ABytesToAdd;
end;
...
begin
  ...
  (PByte(FBuffer) + FFinalSize - l - SizeOf(Byte))^ := Byte(l);
...
begin
  ...
  (PByte(FBuffer) + FFinalSize - l - SizeOf(Word))^ := Byte(l);
  (PByte(FBuffer) + FFinalSize - l - SizeOf(Word) + 1)^ := Byte(l shr 8);
  sb := FEncoder.Encode(AString, ecANSI);
  Move(PByte(sb)^, (PByte(FBuffer) + FFinalSize - l)^, l * SizeOf(TFDAnsiChar));
end;
...
const
  ...
  DPBInfos: array[1 .. DPBInfo_Items] of TIBPBInfo = (
    ...
    // FB30x
    (Name: 'config';                 ParamType: prStrg; Code: isc_dpb_config),                // ok
                                                  
    // FB40
    (Name: 'session_time_zone';      ParamType: prStrg; Code: isc_dpb_session_time_zone),     // ok
    (Name: 'set_db_replica';         ParamType: prCard; Code: isc_dpb_set_db_replica),        // ok
    (Name: 'set_bind';               ParamType: prStrg; Code: isc_dpb_set_bind),              // ok
    (Name: 'decfloat_round';         ParamType: prStrg; Code: isc_dpb_decfloat_round),        // ok
    (Name: 'decfloat_traps';         ParamType: prStrg; Code: isc_dpb_decfloat_traps)         // ok
  );
...
{-------------------------------------------------------------------------------}
constructor TIBDatabaseExt.Create(AEnv: TIBEnv; AOwningObj: TObject);
begin
  inherited Create(AEnv, AOwningObj);
                                                                                  
                                                                
  if TIBLibExt(Lib).FFBMaster <> nil then
    FIStatus := TIBLibExt(Lib).FFBMaster.getStatus;
end;

{-------------------------------------------------------------------------------}
constructor TIBDatabaseExt.CreateUsingHandle(AEnv: TIBEnv;
  AHandle: isc_db_handle; AOwningObj: TObject);
begin
  inherited CreateUsingHandle(AEnv, AHandle, AOwningObj);
  if TIBLibExt(Lib).FFBMaster <> nil then
    FIStatus := TIBLibExt(Lib).FFBMaster.getStatus;
end;
...
const
  ...
  TRPBInfos: array[1 .. TRPBInfo_Items] of TIBPBInfo = (
    ...
    // FB40
    (Name: 'read_consistency';   ParamType: prNoneNoZ; Code: isc_tpb_read_consistency),
    (Name: 'at_snapshot_number'; ParamType: prCard;    Code: isc_tpb_at_snapshot_number)
   );
...
begin
  Result := AInfo.ParamType;
  if (AInfo.Name = 'lock_timeout') and (AInfo.Code = isc_tpb_lock_timeout) then
    if not ((Env.Lib.Brand = ibFirebird) and (Env.Lib.Version >= ivFB020000)) then
      Result := prIgno;
  if (AInfo.Name = 'no_savepoint') and (AInfo.Code = isc_tpb_no_savepoint) then
    ...
    if V2 then begin
      ...
      for i := 0 to VarCount - 1 do begin
        ...
        if oVar.SQLDataType = SQL_NULL then
          ...
          PXSQLVAR_V2(pVar)^.sqldata := pBuff + C_VarIndSize;
      ...
      for i := 0 to VarCount - 1 do begin
        ...
        if oVar.SQLDataType = SQL_NULL then
          ...
          PXSQLVAR_V1(pVar)^.sqldata := PISC_SCHAR(pBuff + C_VarIndSize);
...
{-------------------------------------------------------------------------------}
procedure TIBVariablesExt.Unprepare;
begin
  if FFBMeta <> nil then begin
    FFBMeta.release;
    FFBMeta := nil;
  end;
end;
...
begin
  ...
  Result := [];
  pVarV2 := DataXSQLVarV2[ARowIndex];
  if (pVarV2^.sqlind^ <> -1) and (pVarV2^.sqlind^ and SQLIND_CHANGE_VIEW <> 0) then begin
    if pVarV2^.sqlind^ and SQLIND_INSERT <> 0 then
      Include(Result, iuInserted);
    if pVarV2^.sqlind^ and SQLIND_UPDATE <> 0 then
      Include(Result, iuUpdated);
    if pVarV2^.sqlind^ and SQLIND_DELETE <> 0 then
      Include(Result, iuDeleted);
    if pVarV2^.sqlind^ and SQLIND_TRUNCATE <> 0 then
      Include(Result, iuTruncated);
    if pVarV2^.sqlind^ and SQLIND_CHANGE <> 0 then
      Result := [iuModified];
  end;
end;
...
begin
  ...
  else if TIBVariablesExt(FVars).FFBMeta <> nil then
    Result := Decode(TIBVariablesExt(FVars).FFBMeta.getAlias(
      TIBDatabaseExt(FVars.FStatement.Database).FIStatus, Index), -1)
  else  begin
...
begin
  if FVars.V2 then begin
    ...
    Result := Decode(TIBVariablesExt(FVars).FFBMeta.getOwner(
      TIBDatabaseExt(FVars.FStatement.Database).FIStatus, Index), -1)
  else begin
    pVarV1 := BaseXSQLVarV1;
...
begin
  if FVars.V2 then begin
    ...
    Result := Decode(TIBVariablesExt(FVars).FFBMeta.getRelation(
      TIBDatabaseExt(FVars.FStatement.Database).FIStatus, Index), -1)
  else begin
    pVarV1 := BaseXSQLVarV1;
...
begin
  if FVars.V2 then begin
    ...
    Result := Decode(TIBVariablesExt(FVars).FFBMeta.getField(
      TIBDatabaseExt(FVars.FStatement.Database).FIStatus, Index), -1)
  else begin
    pVarV1 := BaseXSQLVarV1;
...
begin
  ...
  SQL_INT128:     Result := SizeOf(FB_I128);
  SQL_FLOAT:      Result := SizeOf(Single);
  SQL_D_FLOAT,
  SQL_DOUBLE:     Result := SizeOf(Double);
  SQL_DEC16:      Result := SizeOf(FB_DEC16);
  SQL_DEC34:      Result := SizeOf(FB_DEC34);
  SQL_BOOLEAN_IB: Result := SizeOf(ISC_BOOLEAN);
  SQL_BOOLEAN_FB: Result := SizeOf(Boolean);
  SQL_TIMESTAMP:  Result := SizeOf(ISC_TIMESTAMP);
  SQL_TIMESTAMP_TZ:Result := SizeOf(ISC_TIMESTAMP_TZ);
  SQL_TIMESTAMP_TZ_EX:Result := SizeOf(ISC_TIMESTAMP_TZ_EX);
  SQL_TYPE_TIME:  Result := SizeOf(ISC_TIME);
  SQL_TIME_TZ:    Result := SizeOf(ISC_TIME_TZ);
  SQL_TIME_TZ_EX: Result := SizeOf(ISC_TIME_TZ_EX);
...
var
  ...
  procedure Int1282BCD;
  var
    aABuff: array[0..63] of AnsiChar;
    aWBuff: array[0..63] of Char;
    pBuff: Pointer;
    iLen: Integer;
  begin
    TIBLibExt(FVars.Statement.Env.Lib).FFBInt128.toString(
      TIBDatabaseExt(FVars.Statement.Database).FIStatus,
      PFB_I128(pData), iScale, SizeOf(aABuff), @aABuff[0]);
    CheckIError(FVars.Statement, TIBDatabaseExt(FVars.Statement.Database).FIStatus);
    pBuff := @aWBuff[0];
    iLen := FVars.Statement.Database.Encoder.Encode(@aABuff[0], -1, pBuff, ecANSI, ecUTF16);
    FDStr2BCD(pBuff, iLen, PBcd(ApData)^, '.');
  end;
  ...
  var
    ...
    iLen: Integer;
  begin
    TIBLibExt(FVars.Statement.Env.Lib).FFBDec16.toString(
      TIBDatabaseExt(FVars.Statement.Database).FIStatus,
      PFB_DEC16(pData), SizeOf(aABuff), @aABuff[0]);
    CheckIError(FVars.Statement, TIBDatabaseExt(FVars.Statement.Database).FIStatus);
    pBuff := @aWBuff[0];
    iLen := FVars.Statement.Database.Encoder.Encode(@aABuff[0], -1, pBuff, ecANSI, ecUTF16);
    FDStr2BCD(pBuff, iLen, PBcd(ApData)^, '.');
  ...
  var
    ...
    iLen: Integer;
  begin
    TIBLibExt(FVars.Statement.Env.Lib).FFBDec34.toString(
      TIBDatabaseExt(FVars.Statement.Database).FIStatus,
      PFB_DEC34(pData), SizeOf(aABuff), @aABuff[0]);
    CheckIError(FVars.Statement, TIBDatabaseExt(FVars.Statement.Database).FIStatus);
    pBuff := @aWBuff[0];
    iLen := FVars.Statement.Database.Encoder.Encode(@aABuff[0], -1, pBuff, ecANSI, ecUTF16);
    FDStr2BCD(pBuff, iLen, PBcd(ApData)^, '.');
    ...
    begin
      ...
      if iSubType = csIDOctets then begin
        ...
        if (ALen = 0) and FVars.Statement.StrsEmpty2Null then begin
          ...
          if (FDDataType = dtGUID) and (FVars.Statement.GUIDEndian = Big) then
            PGUID(ApData)^ := TGUID.Create(PByte(pData)^, True)
          else
...
var
  ...
  var
    ...
    iLen: Integer;
  begin
    FDBCD2Str(aWBuff, iLen, PBcd(ApData)^, '.');
    pBuff := @aABuff[0];
    iLen := FVars.Statement.Database.Encoder.Decode(@aWBuff[0], iLen, pBuff, ecANSI, ecUTF16);
    aABuff[iLen] := #0;
    TIBLibExt(FVars.Statement.Env.Lib).FFBDec16.fromString(
      TIBDatabaseExt(FVars.Statement.Database).FIStatus,
      @aABuff[0], PFB_DEC16(pData));
    CheckIError(FVars.Statement, TIBDatabaseExt(FVars.Statement.Database).FIStatus);
  end;
  ...
  begin
    ...
    iLen := FVars.Statement.Database.Encoder.Decode(@aWBuff[0], iLen, pBuff, ecANSI, ecUTF16);
    aABuff[iLen] := #0;
    TIBLibExt(FVars.Statement.Env.Lib).FFBDec34.fromString(
      TIBDatabaseExt(FVars.Statement.Database).FIStatus,
      @aABuff[0], PFB_DEC34(pData));
    CheckIError(FVars.Statement, TIBDatabaseExt(FVars.Statement.Database).FIStatus);
  end;

  procedure BCD2Int128;
  var
    aABuff: array[0..63] of AnsiChar;
    aWBuff: array[0..63] of Char;
    pBuff: Pointer;
  ...
  begin
    ...
    iLen := FVars.Statement.Database.Encoder.Decode(@aWBuff[0], iLen, pBuff, ecANSI, ecUTF16);
    aABuff[iLen] := #0;
    TIBLibExt(FVars.Statement.Env.Lib).FFBInt128.fromString(
      TIBDatabaseExt(FVars.Statement.Database).FIStatus,
      iScale, @aABuff[0], PFB_I128(pData));
    CheckIError(FVars.Statement, TIBDatabaseExt(FVars.Statement.Database).FIStatus);
  end;

  procedure Int642Int128(const AValue: Int64);
  var
    aWBuff: array[0..63] of Char;
    pWBuff: PChar;
    aABuff: array[0..63] of AnsiChar;
  ...
  begin
    pWBuff := @aWBuff[0];
    iLen := High(aWBuff) + 1;
    FDInt2Str(@AValue, SizeOf(Int64), pWBuff, iLen, False, 0);
    pABuff := @aABuff[0];
    iLen := FVars.Statement.Database.Encoder.Decode(pWBuff, iLen, Pointer(pABuff), ecANSI, ecUTF16);
    aABuff[iLen] := #0;
    TIBLibExt(FVars.Statement.Env.Lib).FFBInt128.fromString(
      TIBDatabaseExt(FVars.Statement.Database).FIStatus,
      iScale, pABuff, PFB_I128(pData));
    CheckIError(FVars.Statement, TIBDatabaseExt(FVars.Statement.Database).FIStatus);
  end;
...
begin
  ...
  SQL_TEXT:
    if iSubType = csIDOctets then begin
      ...
      if not lIsNull then
        if (FDDataType = dtGUID) and (FVars.Statement.GUIDEndian = Big) then
          PGUID(pData)^ := TGUID.Create(PByte(ApData)^, True)
        else
  ...
  SQL_INT128:
    if lIsNull then begin
      PFB_I128(pData)^.fb_data[0] := 0;
      PFB_I128(pData)^.fb_data[1] := 0;
    end
    else begin
      GetIntDataType(dtFmtBCD);
      case FDDataType of
      dtInt64:
        Int642Int128(PInt64(ApData)^);
      dtCurrency:
                                                                     
        Int642Int128(Int64(Trunc(PCurrency(ApData)^ * C_FD_ScaleFactor[- iScale])));
      dtBCD,
      dtFmtBCD:
        BCD2Int128;
      end;
  ...
  SQL_DEC16:
    if lIsNull then
      PFB_DEC16(pData)^.fb_data[0] := 0
    else
      BCD2Dec16;
  SQL_DEC34:
    if lIsNull then begin
      PFB_DEC34(pData)^.fb_data[0] := 0;
      PFB_DEC34(pData)^.fb_data[1] := 0;
    end
    else
      BCD2Dec34;
  SQL_BOOLEAN_IB:
...
begin
  ...
  if (SQLDataType = SQL_VARYING) or (SQLDataType = SQL_TEXT) then begin
    if not GetData(ARowIndex, pData, iLen, True) then
      ...
      if iLen > 1024 then begin
        Result := FVars.Statement.Database.Encoder.Decode(pData, 1024, ecANSI);
        Result := '(truncated at 1024) ''' + Result + ' ...''';
      end
      else begin
        Result := FVars.Statement.Database.Encoder.Decode(pData, iLen, ecANSI);
        Result := '''' + Result + '''';
      end;
  end
...
begin
  ...
  blr_int128:    Result := SQL_INT128;
  blr_float:     Result := SQL_FLOAT;
  blr_double:    Result := SQL_D_FLOAT;
  blr_d_float:   Result := SQL_DOUBLE;
  blr_dec64:     Result := SQL_DEC16;
  blr_dec128:    Result := SQL_DEC34;
  blr_timestamp: Result := SQL_TIMESTAMP;
  blr_timestamp_tz: Result := SQL_TIMESTAMP_TZ;
  blr_ex_timestamp_tz: Result := SQL_TIMESTAMP_TZ_EX;
  blr_varying,
  blr_varying2,
  blr_cstring,
  ...
  blr_sql_time_tz: Result := SQL_TIME_TZ;
  blr_ex_time_tz:Result := SQL_TIME_TZ_EX;
...
begin
  ...
  SQL_INT128:     iDType := blr_int128;
  SQL_FLOAT:      iDType := blr_float;
  SQL_DOUBLE:     iDType := blr_double;
  SQL_DEC16:      iDType := blr_dec64;
  SQL_DEC34:      iDType := blr_dec128;
  SQL_TIMESTAMP:  iDType := blr_timestamp;
  SQL_TIMESTAMP_TZ: iDType := blr_timestamp_tz;
  SQL_TIMESTAMP_TZ_EX: iDType := blr_ex_timestamp_tz;
  ...
  SQL_TIME_TZ:    iDType := blr_sql_time_tz;
  SQL_TIME_TZ_EX: iDType := blr_ex_time_tz;
...
begin
  ...
  blr_varying2:
    begin
      ...
      ApData := ASrc;
      ALen := 0;
      while (ALen < sqllen) and ((PByte(ASrc) + ALen)^ <> 0) do
        Inc(ALen);
    end;
  blr_short:
    begin
...
begin
  ...
  blr_varying2:
    begin
      ...
      if iLen < sqllen then begin
        if sqldtype in [blr_text, blr_text2] then
          bPad := Ord(' ')
        else
          bPad := 0;
        FillChar((PByte(ADest) + iLen)^, sqllen - iLen, bPad);
      end;
      if iLen > 0 then
...
begin
  ...
  FDatabase := ADatabase;
  FTransaction := ATransaction;
  FOutVars := TIBVariablesExt.Create(Self);
  FInVars := TIBVariablesExt.Create(Self);
  FDialect := 3;
  FGUIDEndian := Big;
end;
...
var
  // Maximum length according to http://tracker.firebirdsql.org/browse/CORE-2370
  buff: array [0..65535-2] of Byte;
  iLen1: Integer;
begin
  GetSQLInfo(AInfo, @buff, SizeOf(buff));
  if buff[0] <> AInfo then
    Result := ''
  else begin
    iLen1 := Integer(Env.Lib.Fisc_portable_integer(@buff[1], 2));
...
{-------------------------------------------------------------------------------}
procedure TIBStatementExt.UpdateTimeout;
{$IFDEF FireDAC_MONITOR}
  procedure Trace1;
  begin
    Env.Trace(sfb_dsql_set_timeout, ['stmt_handle', FSTHandle, 'timeout', FTimeout]);
  end;
{$ENDIF}
begin
  if (FSTHandle <> nil) and Assigned(TIBLibExt(Env.Lib).Ffb_dsql_set_timeout) then begin
{$IFDEF FireDAC_MONITOR}
    if Env.Tracing then Trace1;
{$ENDIF}
    Check(TIBLibExt(Env.Lib).Ffb_dsql_set_timeout(@Error.FStatusVector, @FSTHandle, FTimeout));
  end;
end;

{-------------------------------------------------------------------------------}
procedure TIBStatementExt.SetTimeout(const AValue: Cardinal);
begin
...
{$ENDIF}
  ...
  iLen := Database.Encoder.EncodedLength(sb);
  if iLen > $FFFF then
    iLen := 0;
  Check(Lib.Fisc_dsql_prepare(@Error.FStatusVector, @FTransaction.FTRHandle,
    @FSTHandle, iLen, PISC_SCHAR(PByte(sb)), Dialect, OutVars.FXSQLDA));
  Fsql_stmt_type := GetSQLInfoInt(isc_info_sql_stmt_type);
  if Self is TIBStatementExt then
    if TIBStatementExt(Self).Timeout <> 0 then
      TIBStatementExt(Self).UpdateTimeout;
end;
...
{$ENDIF}
    Check(Lib.Fisc_dsql_describe(@Error.FStatusVector, @FSTHandle, Dialect,
      OutVars.FXSQLDA));
  end;
  if Assigned(TIBLibExt(Lib).Ffb_get_statement_interface) and
     (TIBVariablesExt(FOutVars).FFBMeta = nil) then begin
    Check(TIBLibExt(Lib).Ffb_get_statement_interface(@Error.FStatusVector,
      @TIBStatementExt(Self).FFBStatement, @FSTHandle));
    TIBVariablesExt(FOutVars).FFBMeta := TIBStatementExt(Self).FFBStatement.
      getOutputMetadata(TIBDatabaseExt(Database).FIStatus);
    CheckIError(Self, TIBDatabaseExt(Database).FIStatus);
  end;
end;
...
{$ENDIF}
  ...
  Check(Lib.Fisc_dsql_execute_immediate(@Error.FStatusVector,
    @FDatabase.FDBHandle, @FTransaction.FTRHandle, iLen,
...
{$ENDIF}
    try
      TIBVariablesExt(OutVars).Unprepare;
      TIBVariablesExt(InVars).Unprepare;
      if TIBStatementExt(Self).FFBStatement <> nil then begin
        TIBStatementExt(Self).FFBStatement.release;
        TIBStatementExt(Self).FFBStatement := nil;
      end;
...
{$ENDIF}
  ...
  begin
    if Port <> 0 then
      Result := APrefix + IntToStr(Port)
    else if InstanceName <> '' then
      Result := APrefix + InstanceName
    else
      Result := '';
  end;
...
{$ENDIF}
        ;
        sSvcName := Encoder.Encode(sProt + 'service_mgr');
      end;
    ipTCPIP:
      sSvcName := Encoder.Encode(Host + GetInstance('/') + ':service_mgr');
    ipNetBEUI:
      sSvcName := Encoder.Encode('\\'+ Host + GetInstance('@') + '\service_mgr');
    else
      SetLength(sSvcName, 0);
    end;
  ...
  bmBackup:
    begin
      ...
      if Options <> [] then begin
        ...
        if (FEnv.Lib.Brand = ibFirebird) and (FEnv.Lib.Version >= ivFB030000) then begin
          if boNoTriggers in Options then
            iOpts := iOpts or isc_spb_bkp_no_triggers;
          if boZip in Options then
            iOpts := iOpts or isc_spb_bkp_zip;
        end;
        ABld.AddDWord(iOpts);
      end;
      ...
      if (FEnv.Lib.Brand = ibFirebird) and (FEnv.Lib.Version >= ivFB030000) then begin
        if Statistics <> [] then begin
          ABld.AddByte(isc_spb_bkp_stat);
          s := '';
          if bsTime in Statistics then
            s := s + 'T';
          if bsDelta in Statistics then
            s := s + 'D';
          if bsReads in Statistics then
            s := s + 'R';
          if bsWrites in Statistics then
            s := s + 'W';
          ABld.AddLongString(s);
        end;

        if SkipData <> '' then begin
          ABld.AddByte(isc_spb_bkp_skip_data);
          ABld.AddLongString(SkipData);
        end;
      end;
    end;
...
const
  C_Direct: array[TFBNBackupDirect] of String = ('', 'ON', 'OFF');
var
  iOpts: Cardinal;
begin
  ...
  if Direct <> ndDefault then begin
    ABld.AddByte(isc_spb_nbk_direct);
    ABld.AddLongString(C_Direct[Direct]);
  end;
...
begin
  ...
  rmRestore:
    begin
      ...
      if (Statistics <> []) and
         (FEnv.Lib.Brand = ibFirebird) and (FEnv.Lib.Version >= ivFB030000) then begin
        ABld.AddByte(isc_spb_res_stat);
        s := '';
        if bsTime in Statistics then
          s := s + 'T';
        if bsDelta in Statistics then
...
{ TIBOnlineValidate                                                                   }
{-------------------------------------------------------------------------------}
constructor TIBOnlineValidate.Create(AEnv: TIBEnv; AOwningObj: TObject);
begin
  inherited Create(AEnv, AOwningObj);
  FLockTimeout := 10;
end;
...
{-------------------------------------------------------------------------------}
procedure TIBOnlineValidate.GetServiceParams(ABld: TIBPBWriter);
begin
  ABld.AddByte(isc_action_svc_validate);

  ABld.AddByte(isc_spb_dbname);
  ABld.AddLongString(DatabaseName);

  if TablesInclude <> '' then begin
    ABld.AddByte(isc_spb_val_tab_incl);
    ABld.AddLongString(TablesInclude);
  end;
  if TablesExclude <> '' then begin
    ABld.AddByte(isc_spb_val_tab_excl);
    ABld.AddLongString(TablesExclude);
  end;
  if IndexesInclude <> '' then begin
    ABld.AddByte(isc_spb_val_idx_incl);
    ABld.AddLongString(IndexesInclude);
  end;
  if IndexesExclude <> '' then begin
    ABld.AddByte(isc_spb_val_idx_excl);
    ABld.AddLongString(IndexesExclude);
  end;

  if LockTimeout <> 10 then begin
    ABld.AddByte(isc_spb_val_lock_timeout);
    ABld.AddDWord(Cardinal(LockTimeout));
  end;
...
{ TIBInfo                                                                       }
{-------------------------------------------------------------------------------}
procedure TIBInfo.QueryInfo(const ARequest: TRequestProc; const AParse: TParseFunc);
