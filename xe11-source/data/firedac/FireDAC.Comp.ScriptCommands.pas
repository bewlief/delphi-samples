  ...
  for i := 0 to AParams.Count - 1 do begin
    ...
    if (Length(s) = 18) and FDInSet(s[1], ['h', 'H']) and (s[10] = '_') then begin
      ...
      try
        oStr := oText.Stream;
        oStr.Position := iPos;
        pData := AParams[i].SetBlobRawData(iLen, nil, 0);
        if iLen > 0 then
          oStr.ReadBuffer(pData^, iLen);
      finally
...
begin
  ...
  try
    ...
    on E: EFDDBEngineException do begin
      if (E.Kind = ekObjNotExists) and Engine.ScriptOptions.DropNonexistObj and
         ((eCmdKind = skUnknown) and (oCmd.CommandKind = skDrop) or (eCmdKind = skDrop)) then begin
        if Engine.ScriptOptions.FeedbackCommands then
          if E[0].ObjName <> '' then
  ...
  if Engine.ScriptOptions.FeedbackCommands then begin
    ...
    if not lWasRowset then
      if oCmd.RowsAffectedReal and not (eCmdKind in [skCreate, skAlter, skDrop,
       skStartTransaction, skCommit, skRollback, skSet, skSetSchema]) then
...
begin
  ...
  if Result then
    Vars.AddStrings(Parser.GetLine().Split([',', ' ', #9], TStringSplitOptions.ExcludeEmpty));
end;
...
var
  ...
  begin
    ...
    oDef.FormatSettings.ShortTimeFormat := 'hh:nn:ss';
  end;
