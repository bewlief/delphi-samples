...
{$ENDIF}
  ...
  private
    ...
    FMode: TFDStreamMode;
    FModified: Boolean;
    constructor Create(AObj: TADSHandle; AVariable: TADSVariable; AMode: TFDStreamMode);
  protected
...
const
  C_ADSDllFolder: String = 'acesdk';
var
  sDLLName: String;
begin
  sDLLName := AVendorHome;
  if sDLLName <> '' then
    sDLLName := FDNormPath(FDNormPath(sDLLName) + C_ADSDllFolder);
  if AVendorLib <> '' then
...
{-------------------------------------------------------------------------------}
procedure TADSBLOBStream.Flush;
begin
...
begin
  ...
  ADS_MONEY:
    begin
      ulLen := SizeOf(aBuff);
      FObj.Check(FObj.Lib.FAdsGetField(FObj.Handle, FField, @aBuff[0], @ulLen, ADS_TRIM));
      pText := nil;
      ASize := FObj.Encoder.Decode(@aBuff[0], ulLen, pText);
...
{-------------------------------------------------------------------------------}
function TADSVariable.CreateBlobStream(AMode: TFDStreamMode): TADSBLOBStream;
begin
  Result := TADSBLOBStream.Create(FObj, Self, AMode);
end;
