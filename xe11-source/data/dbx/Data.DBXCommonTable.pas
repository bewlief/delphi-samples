...
type
  ...
  /// <summary>Represents a persisted table model.</summary>
...
begin
  ...
  begin
    FColumnCount := TJSONArray(MetaPair.JsonValue).Count;
    SetLength(FValueTypes, FColumnCount);
    SetLength(FRow, FColumnCount);
    SetLength(LValues, FColumnCount);
    for I := 0 to FColumnCount - 1 do
    begin
      FValueTypes[I] := TDBXJSONTools.JSONToValueType(TJSONArray(TJSONArray(MetaPair.JsonValue).Items[I]));
...
begin
  ...
  if JsonPair <>  nil then
    ...
    begin
      ColMetaData := TJSONArray(JsonPair.JsonValue).Items[AIndex];
      if ColMetaData is TJSONArray then
      begin
        ColName := TJSONArray(ColMetaData).Items[0];
...
begin
  ...
  if Col1 = nil then
    Exit(False);
  if FRowNb < TJSONArray(Col1.JsonValue).Count - 1 then
  begin
    IncrAfter(FRowNb);
    CleanRowValues;
    Result := True;
  ...
  begin
    ...
    begin
      ...
      if (JsonPair <> nil) and (JsonPair.JsonValue is TJSONArray) then
        JCell := TJSONArray(JsonPair.JSonValue).Items[FRowNb];
      if JCell = nil then
        Exit(nil);
