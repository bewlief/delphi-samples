...
type
  ...
  public
    ...
    procedure OnRefType(const TypeName: string; id: Integer); virtual; abstract;
    ...
    procedure OnTypeStart(const TypeName: string; id: Integer); virtual; abstract;
    ...
    procedure OnString(const Data: string); virtual; abstract;
    /// <summary> Number value event </summary>
    /// <remarks> The event was precedeed by a OnFieldStart (eventually an
    /// OnListStart) open event.</remarks>
    /// <param name="Data">Field or array element value as a number</param>
    procedure OnNumber(const Data: string); virtual; abstract;
  ...
  private
    ...
    FCanReset: TJSONCanPopulateProc;
  protected
    function CanPopulate(Data: TObject; rttiField: TRttiField): Boolean; virtual;
    function CanReset(Data: TObject; rttiField: TRttiField): Boolean; virtual;
  ...
  public
    constructor Create(const ACanPopulate: TJSONCanPopulateProc); overload;
    constructor Create(const ACanPopulate, ACanReset: TJSONCanPopulateProc); overload;
  ...
  protected
    ...
    procedure MarshalConverter(Data: TObject; const Field: string;
    ...
    procedure MarshalTypeConverter(Data: TObject; const Field: string;
      ConverterEvent: TConverterEvent); overload;
  ...
  public
    ...
    procedure OnString(const Data: string); override;
  ...
  private
    ...
    class function ObjectType(Ctx: TRTTIContext; const TypeName: string): TRttiType;
  public
  ...
  protected
    /// <summary>returns true if a reverter matching the given meta class and field was registered
    /// </summary>
    /// <param name="clazz">Object type</param>
    /// <param name="field">field name</param>
    ...
    /// <summary>Returns the reverter registered with the given meta class and field</summary>
    /// <param name="clazz">Object type</param>
    /// <param name="field">field name</param>
    ...
    /// <summary>returns true if a customizer should reset specified field to empty value</summary>
    /// <param name="Data">TObject - current data instance</param>
    /// <param name="Field">TRTTIField instance associated with object field to be reset</param>
    /// <returns>true if a field should be reset</returns>
  ...
  public
      ...
      class procedure AddMarshalFlag(AClass: TClass; const AField: string; Marshal: Boolean);
      class procedure ClearMarshalFlag(AClass: TClass; const AField: string);
  end;
...
{$HINTS ON}
  ...
  private
    ...
    FSkipping: Integer;
  public
    constructor Create(const ARTTICtx: TRttiContext);
    class function ShouldEdit(Data: TObject): Boolean; static;
    ...
    function ShouldPrePopulate(Data: TObject; rttiField: TRttiField): Boolean;
  end;
...
type
  ...
  public
    constructor Create(ACanPopulate, ACanReset: TJSONCanPopulateProc);
    destructor Destroy; override;
...
type
  __TStrings = class(TStrings);

procedure StringsEncodingReverter(Data: TObject; Field: string; Arg: string);
var
  Enc: TEncoding;
begin
  Assert(Data is TStrings);
  if Arg = '' then
    Enc := nil
  else
    Enc := TEncoding.GetEncoding(Arg.ToInteger);
  try
    if SameText(Field, 'FEncoding') then
    begin
      if TStrings(Data).Encoding <> Enc then
        __TStrings(Data).SetEncoding(Enc);
    end
    else if SameText(Field, 'FDefaultEncoding') then
    begin
      if TStrings(Data).DefaultEncoding <> Enc then
        TStrings(Data).DefaultEncoding := Enc;
    end;
  finally
    Enc.Free;
  end;
...
function StringsEncodingConverter(Data: TObject; Field: string): string;
var
  Enc: TEncoding;
begin
  Assert(Data is TStrings);
  if SameText(Field, 'FEncoding') then
    Enc := TStrings(Data).Encoding
  else if SameText(Field, 'FDefaultEncoding') then
    Enc := TStrings(Data).DefaultEncoding
  else
    Enc := nil;
  if Enc = nil then
    Result := ''
  else
    Result := Enc.CodePage.ToString;
end;
...
procedure TJSONConverter.OnBoolean(const Data: Boolean);
begin
  if Data then
...
procedure TJSONConverter.OnFieldEnd(const FieldName: string);
begin
...
procedure TJSONConverter.OnFieldStart(const FieldName: string);
begin
...
procedure TJSONConverter.OnNumber(const Data: string);
begin
  ProcessData(TJSONNumber.Create(Data));
end;

procedure TJSONConverter.OnRefType(const TypeName: string; id: Integer);
begin
...
procedure TJSONConverter.OnString(const Data: string);
begin
  ProcessData(TJSONString.Create(Data));
end;

procedure TJSONConverter.OnTypeEnd(const TypeName: string; id: Integer);
begin
  FStack.Pop;
...
procedure TJSONConverter.OnTypeStart(const TypeName: string; id: Integer);
begin
  ProcessData(TJSONObject.Create);
  OnFieldStart(TYPE_NAME);
  OnString(TypeName);
...
function TTypeMarshaller<TSerial>.Converter(clazz: TClass;
  const Field: string): TConverterEvent;
var
...
begin
  ...
  try
    repeat
      FConverters.TryGetValue(ComposeKey(clazz, Field), Result);
      if (clazz <> nil) and (Result = nil) then
        clazz := clazz.ClassParent;
    until (Result <> nil) or (clazz = nil) {or (clazz.FieldAddress(Field) = nil)};
  finally
...
class procedure TTypeMarshaller<TSerial>.DecomposeTypeName(const TypeName: string;
...
begin
  ...
  try
    repeat
      ...
      if (clazz <> nil) and not Result then
        clazz := clazz.ClassParent;
    until Result or (clazz = nil) {or (clazz.FieldAddress(Field) = nil)};
  finally
    TMonitor.Exit(FConverters);
  end;
end;
...
procedure TTypeMarshaller<TSerial>.MarshalConverter(Data: TObject;
  const Field: string; ConverterEvent: TConverterEvent);
var
  ObjItem: TObject;
  StrItem: string;
  NbrItem: string;
  ...
  case ConverterEvent.ConverterType of
    ...
    ctObject:
      begin
        ...
        MarshalData(ObjItem);
        // Marshal TJSONString.FValue as "FStrBuffer: TStringBuilder",
        // ObjItem must be destroyed
        if (Data is TJSONString) and (ObjItem is TStringBuilder) then
          ObjItem.Free;
      end;
    ctString:
...
procedure TTypeMarshaller<TSerial>.MarshalConverter(Data: TObject;
  const Field: string; ConverterEvent: TJSONInterceptor);
var
  ObjItem: TObject;
  StrItem: string;
  NbrItem: string;
...
var
  ...
  valArr: TValue;
begin
  if Value.IsEmpty and not Value.IsArray then
    FConverter.OnNull
  else
    case Value.Kind of
      TTypeKind.tkInteger:
...
begin
  ...
  begin
    ...
    if TListTFieldsEditor.ShouldEdit(Data) then
      LEditor := TListTFieldsEditor.Create(FRTTICtx)
    else
      LEditor := nil;
    try
      for rttiField in rttiType.GetFields do
      begin
        ...
        // Marshal TJSONString.FValue as "FStrBuffer: TStringBuilder", skip TJSONString.FIsNull using 10.2 layout
        if (Data is TJSONString) and (JSONSerializationVersion <= 32) then
          if SameText(rttiField.Name, 'FValue') then
            LFieldName := 'FStrBuffer'
          else if SameText(rttiField.Name, 'FIsNull') then
            continue;
        ...
        begin
            ...
            if lConverter.IsTypeConverter then
              if (rttiField.FieldType <> nil) and
                ...
                begin
                  FConverter.OnFieldStart(LFieldName);
                  MarshalTypeConverter(rttiField.GetValue(Data).AsObject,
                                       LFieldName, lConverter);
                  FConverter.OnFieldEnd(LFieldName)
                end
                else
            ...
            begin
              ...
              if convEv <> nil then
                ...
                AddWarning(Data, Data.ClassType.UnitName, Data.ClassName,
                  EmptyStr, rttiField.FieldType.Name, LFieldName)
            end;
          end;
        end
        else if not MarshalSimpleField(rttiField, Data) then
          AddWarning(Data, Data.ClassType.UnitName, Data.ClassName, EmptyStr,
            rttiField.FieldType.Name, LFieldName);
      end;
    finally
      LEditor.Free;
    end;
  ...
  case rttiField.FieldType.TypeKind of
    ...
    TTypeKind.tkRecord, TTypeKind.tkMRecord:
      begin
        ...
        if (rttiField.FieldType.Handle = TypeInfo(System.Generics.Collections.TListHelper)) and
           (JSONSerializationVersion <= 32) then
        begin
          valArr := GetArrayValueFromTListHelperValue(FRTTICtx, rttiField.GetValue(Data), Len);
          LItemsField := TRttiRecordType(rttiField.FieldType).GetDeclaredFields()[0];
          LLenField := TRttiRecordType(rttiField.FieldType).GetDeclaredFields()[1];
          // "listHelper":[2]
          FConverter.OnFieldStart(rttiField.Name);
          FConverter.OnListStart;
          ...
          FConverter.OnFieldEnd(rttiField.Name);
          // "items":[{},{}]
          FConverter.OnFieldStart(LItemsField.Name);
          FConverter.OnListStart;
          for I := 0 to Len - 1 do
            MarshalValue(valArr.GetArrayElement(I));
          FConverter.OnListEnd;
          FConverter.OnFieldEnd(LItemsField.Name);
        end
        else
        begin
          FConverter.OnFieldStart(rttiField.Name);
...
procedure TTypeMarshaller<TSerial>.RegisterConverter(clazz: TClass;
  const Field: string; func: TStringsConverter);
var
...
procedure TTypeMarshaller<TSerial>.RegisterConverter(clazz: TClass;
  const Field: string; func: TObjectConverter);
var
  ConverterEvent: TConverterEvent;
begin
  ConverterEvent := TConverterEvent.Create;
...
procedure TTypeMarshaller<TSerial>.RegisterConverter(clazz: TClass;
  const Field: string; func: TStringConverter);
var
  ConverterEvent: TConverterEvent;
begin
  ConverterEvent := TConverterEvent.Create;
...
procedure TTypeMarshaller<TSerial>.RegisterConverter(clazz: TClass;
  const Field: string; func: TObjectsConverter);
var
  ConverterEvent: TConverterEvent;
begin
  ConverterEvent := TConverterEvent.Create;
...
procedure TTypeMarshaller<TSerial>.RegisterConverter(clazz: TClass;
  const Field: string; Converter: TConverterEvent);
begin
  TMonitor.Enter(FConverters);
  try
...
procedure TTypeMarshaller<TSerial>.RegisterConverter(clazz: TClass;
  const func: TTypeStringsConverter);
var
  ConverterEvent: TConverterEvent;
begin
  ConverterEvent := TConverterEvent.Create;
...
procedure TTypeMarshaller<TSerial>.RegisterConverter(clazz: TClass;
  const func: TTypeObjectConverter);
var
  ConverterEvent: TConverterEvent;
begin
  ConverterEvent := TConverterEvent.Create;
...
procedure TTypeMarshaller<TSerial>.RegisterConverter(clazz: TClass;
  const func: TTypeObjectsConverter);
var
  ConverterEvent: TConverterEvent;
begin
  ConverterEvent := TConverterEvent.Create;
...
procedure TTypeMarshaller<TSerial>.RegisterConverter(clazz: TClass;
  const func: TTypeStringConverter);
var
  ConverterEvent: TConverterEvent;
begin
  ConverterEvent := TConverterEvent.Create;
...
procedure TConverterEvent.SetTypeObjectConverter
  (const Converter: TTypeObjectConverter);
begin
...
procedure TConverterEvent.SetTypeObjectsConverter
  (const Converter: TTypeObjectsConverter);
begin
...
procedure TConverterEvent.SetTypeStringConverter
  (const Converter: TTypeStringConverter);
begin
...
procedure TConverterEvent.SetTypeStringsConverter
  (const Converter: TTypeStringsConverter);
begin
...
function TJSONUnMarshal.FieldReverter(Data: TObject;
  const Field: string): TJSONInterceptor;
begin
...
function TJSONUnMarshal.ClassTypeOf(Data: TObject; const Field: string): TClass;
var
  tRtti: TRttiType;
  fRtti: TRTTIField;
begin
...
function TJSONUnMarshal.ShouldReset(Data: TObject;
  Field: TRTTIField): Boolean;
var
  lReverter: TReverterEvent;
  lInterceptor: TJSONInterceptor;
begin
  lReverter := Reverter(Data.ClassType, Field.Name);
  if lReverter <> nil then
    Result := lReverter.IsTypeReverter
  else
  begin
    lInterceptor := FieldReverter(Data, Field.Name);
    if lInterceptor = nil then
      lInterceptor := FieldTypeReverter(Data, Field.Name);
    Result := (lInterceptor = nil) or lInterceptor.IsTypeReverter;
    lInterceptor.Free;
  end;
end;
...
begin
  ...
  RegisterReverter(TStrings, 'FEncoding', StringsEncodingReverter);
  RegisterReverter(TStrings, 'FDefaultEncoding', StringsEncodingReverter);
end;
  ...
  begin
    ...
    if customizer = nil then // Use default population customizer to prevent leaked memory
      customizer := TInternalJSONPopulationCustomizer.Create(
        ...
        function(AData: TObject; AField: TRttiField): Boolean
        begin
          Result := ShouldReset(AData, AField);
        end);
    try
      try
...
function TJSONUnMarshal.TryCreateObject(Data: TObject; const Field: string; JsonObj: TJSONObject; out MarshalledObj: TObject): Boolean;
var
  objType: string;
  ObjId: string;
  objFields: TJSONObject;
...
function TJSONUnMarshal.GetFieldType(Data: TObject; const Field: string): TRTTIField;
var
  rType: TRttiType;
begin
...
function TJSONUnMarshal.HasReverter(clazz: TClass; const Field: string): Boolean;
begin
  TMonitor.Enter(FReverters);
  try
    repeat
      Result := FReverters.ContainsKey(ComposeKey(clazz, Field));
      if (clazz <> nil) and not Result then
        clazz := clazz.ClassParent;
    until Result or (clazz = nil) {or (clazz.FieldAddress(Field) = nil)};
  finally
      ...
      TTypeKind.tkRecord, TTypeKind.tkMRecord:
        begin
          ...
          begin
            ...
            if I >= TJSONArray(JsonValue).Count  then
              jsonFieldVal := nil
            else
...
class function TJSONUnMarshal.ObjectType(Ctx: TRttiContext; const TypeName: string) : TRttiType;
begin
...
procedure TJSONUnMarshal.PopulateFields(JsonFields: TJSONObject; Data: TObject; JsonCustomizer: TJSONPopulationCustomizer);
  ...
  function HasField(Data: TObject; Field: string): Boolean;
  begin
    Result := FRTTICtx.GetType(Data.ClassType).GetField(Field) <> nil;
  end;
...
begin
  ...
  begin
    ...
    // check for reverters
    if HasReverter(ClassType, FieldName) then
    begin
      revEv := Reverter(ClassType, FieldName);
      case revEv.ReverterType of
        rtTypeObjects:
          begin
            if jsonFieldVal is TJSONArray then
        ...
        rtObjects:
          begin
            LPopulated := HasField(Data, FieldName);
            if jsonFieldVal is TJSONArray then
    ...
    else if HasReverter(ClassTypeOf(Data, FieldName),
        FIELD_ANY) then
    begin
      revEv := Reverter(ClassTypeOf(Data, FieldName),
          FIELD_ANY);
      case revEv.ReverterType of
        rtTypeObjects:
          begin
    ...
    begin
      if not TListTFieldsEditor.ShouldEdit(Data) then
      begin
...
procedure TJSONUnMarshal.RegisterReverter(clazz: TClass; const Field: string;
  const func: TObjectReverter);
var
...
procedure TJSONUnMarshal.RegisterReverter(clazz: TClass;
  const func: TTypeStringsReverter);
var
  ReverterEvent: TReverterEvent;
begin
  ReverterEvent := TReverterEvent.Create;
...
procedure TJSONUnMarshal.RegisterReverter(clazz: TClass;
  const func: TTypeStringReverter);
var
  ReverterEvent: TReverterEvent;
begin
  ReverterEvent := TReverterEvent.Create;
...
procedure TJSONUnMarshal.RegisterReverter(clazz: TClass;
  const func: TTypeObjectsReverter);
var
  ReverterEvent: TReverterEvent;
begin
  ReverterEvent := TReverterEvent.Create;
...
procedure TJSONUnMarshal.RegisterReverter(clazz: TClass;
  const func: TTypeObjectReverter);
var
  ReverterEvent: TReverterEvent;
begin
  ReverterEvent := TReverterEvent.Create;
...
function TJSONUnMarshal.Reverter(clazz: TClass; const Field: string): TReverterEvent;
begin
  TMonitor.Enter(FReverters);
  try
    repeat
      FReverters.TryGetValue(ComposeKey(clazz, Field), Result);
      if (clazz <> nil) and (Result = nil) then
        clazz := clazz.ClassParent;
    until (Result <> nil) or (clazz = nil) {or (clazz.FieldAddress(Field) = nil)};
  finally
...
procedure TJSONUnMarshal.SetField(Data: TObject; const Field, Value: string);
var
  rField: TRTTIField;
begin
...
procedure TJSONUnMarshal.SetFieldArray(Data: TObject; const Field: string;
  Value: TJSONArray);
var
  rField: TRTTIField;
  LValue: TValue;
begin
  ...
  case rField.FieldType.TypeKind of
    ...
    TTypeKind.tkRecord, TTypeKind.tkMRecord:
      ...
      if rField.FieldType.Handle = TypeInfo(System.Generics.Collections.TListHelper) then
      begin
...
procedure TJSONUnMarshal.StoreObject(const ObjId: string; Obj: TObject);
begin
  FObjectHash.Add(ObjId, Obj);
end;

function TJSONUnMarshal.StringToTValue(const Value: string;
...
begin
  ...
  if TListTFieldsEditor.ShouldEdit(Data) then
    LEditor := TListTFieldsEditor.Create(rttiContext)
  else
    LEditor := nil;
  try
    ...
    begin
      ...
      if (rttiField.FieldType <> nil)
          ...
          and (rttiField.GetValue(Data).AsObject <> nil) and
...
begin
  ...
  begin
    if not CanPopulate(Data, rttiField) or not CanReset(Data, rttiField) then
      Exit;
...
function TJSONPopulationCustomizer.CanReset(Data: TObject;
  rttiField: TRttiField): Boolean;
begin
  if Assigned(FCanReset) then
    Result := FCanReset(Data, rttiField)
  else
    Result := JSONBooleanAttributeValue(rttiField,JSONOwned,true);
end;

constructor TJSONPopulationCustomizer.Create(const ACanPopulate: TJSONCanPopulateProc);
begin
  Create(ACanPopulate, nil);
end;

constructor TJSONPopulationCustomizer.Create(const ACanPopulate, ACanReset: TJSONCanPopulateProc);
begin
  inherited Create;
...
begin
  ...
  RegisterConverter(TStrings, 'FEncoding', StringsEncodingConverter);
  RegisterConverter(TStrings, 'FDefaultEncoding', StringsEncodingConverter);
end;
...
begin
  ...
  try
    ...
    if CFRegConverters.Count = 0 then
    begin
      // add JSON converters
      ...
      Result.RegisterConverter(TStrings, 'FEncoding', StringsEncodingConverter);
      Result.RegisterConverter(TStrings, 'FDefaultEncoding', StringsEncodingConverter);
    end;
  finally
...
begin
  ...
  try
    ...
    if CFRegReverters.Count = 0 then
    begin
      // add JSON reverters
      ...
      Result.RegisterReverter(TStrings, 'FEncoding', StringsEncodingReverter);
      Result.RegisterReverter(TStrings, 'FDefaultEncoding', StringsEncodingReverter);
    end;
  finally
...
class procedure TJSONConverters.AddMarshalFlag(AClass: TClass; const AField: string;
  Marshal: Boolean);
begin
  TMonitor.Enter(CFRegMarshal);
  try
...
class procedure TJSONConverters.ClearMarshalFlag(AClass: TClass; const AField: string);
var
  LKey: string;
begin
  TMonitor.Enter(CFRegMarshal);
...
procedure TMarshalUnmarshalBase.AddWarning(Data: TObject; const FieldClassUnit,
...
procedure TMarshalUnmarshalBase.RegisterJSONMarshalled(const AComposeKey: string;
  Marshal: Boolean);
begin
...
procedure TMarshalUnmarshalBase.RegisterJSONMarshalled(clazz: TClass;
  const Field: string; Marshal: Boolean);
begin
...
constructor TInternalJSONPopulationCustomizer.Create(ACanPopulate, ACanReset: TJSONCanPopulateProc);
begin
  inherited Create(ACanPopulate, ACanReset);
...
procedure TInternalJSONPopulationCustomizer.PrePopulateObjField(Data: TObject;
  rttiField: TRttiField);
begin
  if (rttiField <> nil) and CanReset(Data, rttiField) then
  begin
...
constructor TListTFieldsEditor.Create(const ARTTICtx: TRttiContext);
begin
  inherited Create;
  FRTTICtx := ARTTICtx;
end;
...
function TListTFieldsEditor.ShouldMarshal(Data: TObject; const [ref] rttiField: TRttiField): Boolean;
type
...
begin
  ...
  begin
    FSkipping := 1;
    PRttiField(@rttiField)^ := FRTTICtx.GetType(TListCvt).GetField('FListHelper');
  end
...
begin
  Result := False;
  rField := FRTTICtx.GetType(TListCvt).GetField('FListHelper');
  ...
  if (rField <> nil) and
     ...
     (string.CompareText('FListHelper', FieldName) = 0) then
  begin
    // "listHelper":[2] - ignore
    if (I < JsonFields.Count - 1) and
       (string.CompareText('FItems', JsonFields.Pairs[I + 1].JsonString.Value) = 0) then
      Inc(I);
    // "items":[{},{}]
...
function TListTFieldsEditor.ShouldPrePopulate(Data: TObject; rttiField: TRttiField): Boolean;
begin
