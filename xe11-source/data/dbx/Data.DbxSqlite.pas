...
function sqlite3_column_origin_name16; external sqlitelib name _PU + 'sqlite3_column_origin_name16';
function sqlite3_column_database_name16; external sqlitelib name _PU + 'sqlite3_column_database_name16';
function sqlite3_column_table_name16; external sqlitelib name _PU + 'sqlite3_column_table_name16';
function sqlite3_table_column_metadata; external sqlitelib name _PU + 'sqlite3_table_column_metadata';
...
begin
  ...
  Values[TDBXPropertyNames.Database] := '';
end;
...
begin
  try
    ...
    TDBXError.Create(TDBXErrorCodes.DriverInitFailed, SLibraryNotFound);
  end;
  if ConnectionProperties.Values['ColumnMetaDataSupported'] = '' then
  begin
    if sqlite3_compileoption_used(MarshaledAString(Utf8Encode('SQLITE_ENABLE_COLUMN_METADATA'))) = 1 then
  ...
  try
    ...
    raise TDBXError.Create(TDBXErrorCodes.DriverInitFailed, SLibraryNotFound);
  end;
  CheckError(ErrorCode, FConnectionHandle, FDBXContext);
{$IF DEFINED(MACOS) and not DEFINED(CPUARM)}
