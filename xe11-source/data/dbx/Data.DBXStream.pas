...
type
  ...
  public
    ...
    procedure CheckBuf(const BytesNeeded: Integer);
    procedure TrimBuf;
  ...
  public
    ...
    procedure ReadDataBytes(const Buffer: TArray<Byte>; const InReadOff: NativeInt; const InCount: NativeInt);
    function ReadBoolean: Boolean;
  ...
  private
    class var
      Hex: TArray<Byte>;
      MethodStart: TArray<Byte>;
      ParamsStart: TArray<Byte>;
      Connect: TArray<Byte>;
      Disconnect: TArray<Byte>;
      Prepare: TArray<Byte>;
      Parameters: TArray<Byte>;
      ParameterMetadataStart: TArray<Byte>;
      Execute: TArray<Byte>;
      CommandClose: TArray<Byte>;
      ReaderClose: TArray<Byte>;
      ResultStart: TArray<Byte>;
      ErrorStart: TArray<Byte>;
      CodeLabel: TArray<Byte>;
      MessageLabel: TArray<Byte>;
      NullValue: TArray<Byte>;
      TrueValue: TArray<Byte>;
      FalseValue: TArray<Byte>;
      ColumnsStart: TArray<Byte>;
      TableStart: TArray<Byte>;
      SessionStart: TArray<Byte>;
      RowsStart: TArray<Byte>;
      DataStart: TArray<Byte>;
      Next: TArray<Byte>;
      NextStart: TArray<Byte>;
      HandleStart: TArray<Byte>;
      FieldsStart: TArray<Byte>;
      Txbegin: TArray<Byte>;
      Txend: TArray<Byte>;
      MoreBlob: TArray<Byte>;
      MoreBlobStart: TArray<Byte>;
      NextResult: TArray<Byte>;
      VendorProperty: TArray<Byte>;
      CallbackStart: TArray<Byte>;
      MetadataStart: TArray<Byte>;
  public
  ...
  public
    ...
    function Read(const Buf: TArray<Byte>; const InOffset: NativeInt; const InSize: NativeInt): NativeInt; override;
...
begin
  SetLength(FBuf, FBufLength + BytesNeeded);
  FBufLength := Length(FBuf);
end;

procedure TDBXBinaryStream.CheckBuf(const BytesNeeded: Integer);
begin
  if FOff + BytesNeeded > FBufLength then
    Growbuf(FOff + BytesNeeded - FBufLength);
end;

procedure TDBXBinaryStream.TrimBuf;
begin
  if FOff < FBufLength then
  begin
    if FOff > FMinBufSize then
      FBufLength := FOff
    else
      FBufLength := FMinBufSize;
    SetLength(FBuf, FBufLength);
  end;
end;
...
begin
  ...
  begin
    Inc(BufOff);
    Ch := WideChar((((Integer((Buf[BufOff] and 255))) shl 8) or (Integer(Buf[BufOff+1]) and 255)));
    Inc(BufOff);
...
procedure TDBXJSonStreamReader.ReadDataBytes(const Buffer: TArray<Byte>; const InReadOff: NativeInt; const InCount: NativeInt);
var
  MaxCopy: NativeInt;
  ReadOff: NativeInt;
  Count: NativeInt;
...
begin
  ...
  SetLength(FCharBuffer, NewCharBufferSize);
...
begin
  ...
  SetLength(FByteBuffer,NewByteBufferSize);
...
function TDBXRowBlobStreamReader.Read(const Buf: TArray<Byte>; const InOffset, InSize: NativeInt): NativeInt;
var
  Offset: NativeInt;
  Size: NativeInt;
  ReturnBytes: NativeInt;
  TransferBytes: NativeInt;
  AvailableBytes: NativeInt;
begin
  Offset := InOffset;
  Size := InSize;
  ReturnBytes := 0;
...
begin
  SetLength(FRowOffsets,FRowCount + 16);
end;
...
const
  sReadSize = 8196;
begin
  CheckBuf(TDBXFieldHeader.FBigHeaderLength + FMaxInline);
...
begin
  CheckBuf(EstimatedSize + TDBXFieldHeader.FJSONBlockLength);
...
begin
  ...
  CheckBuf(TDBXFieldHeader.FMaxHeaderLength + FMaxInline);
...
begin
  ...
  if FEncoder.EncodeOff - FLastInlinByteCount > 0 then
  begin
...
begin
  ...
  begin
    SetLength(FDecodeByteBuf,NewSize);
    FDecodeLength := NewSize;
  end;
end;
...
begin
  ...
  begin
    Inc(FDecodeByteOff);
    if FDecodeByteOff >= FDecodeByteLength then
    begin
      FDecodeByteOff := OutOff;
      Exit(False);
    end;
    if (OutOff + 2) >= FDecodeByteLength then
      GrowDecodeByteBuf;
    Inc(OutOff);
    DecodeByteBuf[OutOff] := Byte(FHalfChar);
    Inc(OutOff);
    Inc(Off);
    DecodeByteBuf[OutOff] := Byte((InBuf[Off] and 255));
    FHasHalfChar := False;
  end;
  while Off < MaxOff do
  begin
    Inc(Off);
    Ch := WideChar(((Integer((InBuf[Off] and 255))) shl 8 or (Integer(InBuf[Off+1]) and 255)));
    Inc(Off);
    if (Ch = #$0) and FNullTerminated then
    begin
      FDecodeByteOff := OutOff;
    ...
    if (OutOff + 2) >= FDecodeByteLength then
      GrowDecodeByteBuf;
    Inc(OutOff);
    DecodeByteBuf[OutOff] := Byte((Ord(Ch) and 255));
    Inc(OutOff);
    DecodeByteBuf[OutOff] := Byte((Ord(Ch) shr 8));
  end;
  if Off = MaxOff then
  begin
    Inc(Off);
