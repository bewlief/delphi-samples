...
type
  ...
  /// <summary>Holds information about a connected client: its IP address,
  /// communication port and protocol, and application name.</summary>
  ...
  /// <summary>Represents a factory of various implementations of the
  ...
  private
    class var
      FSingleton: TDBXCommunicationLayerFactory;
      FRegisteredLayers: TDictionary<string,TObjectClass>;
  end;

  
  /// <summary>Represents the channel for interfacing in-process data
  /// processing. TDBXLocalChannel implements a two-process producer and
  /// consumer paradigm, where data is exchanged in alternate steps.</summary>
  ...
  /// <summary>Transport channel information for socket-based
  /// transport.</summary>
  ...
  /// <summary>Represents a collection of strings specifying the properties of a
  /// TTransportFilterItem instance.</summary>
  TFilterProperties = class(TBaseFilterProperties)
  end;

  
  /// <summary>Parent class for transport filters that are invokable if present
  /// in the transport chain of the command filter list.</summary>
  ...
  /// <summary>Implements a dbExpress transport encryption filter based on the
  /// PC1 algorithm.</summary>
  ...
  /// <summary>Represents a filter factory for transport filters.</summary>
  ...
  private
    class var
      FSingleton: TTransportFilterFactory;
      FRegisteredFilters: TDictionary<string,TObjectClass>;
  end;


  /// <summary>Represents a transport filter.</summary>
  ...
  /// <summary>Represents the utility class for data processing.</summary>
...
begin
  ...
  if Pos >= Size then
    Exit('');
  StrBuf := TStringBuilder.Create((Size - Pos) * 2);
  try
    for I := Pos to Size - 1 do
    begin
      HexDig := (Data[I] and 240) shr 4;
      StrBuf.Append(Char(DecimalToHex(HexDig)));
      HexDig := Data[I] and 15;
      StrBuf.Append(Char(DecimalToHex(HexDig)));
    end;
    Result := StrBuf.ToString(True);
  finally
    StrBuf.Free;
  end;
end;
...
begin
  ...
  begin
    Val := HexToDecimal(Ord(StrData.Chars[IncrAfter(I)])) shl 4;
    Val := Val or HexToDecimal(Ord(StrData.Chars[IncrAfter(I)]));
...
begin
  ...
  begin
    ...
    for i := 0 to JsonCollection.Count - 1 do
    begin
      FilterPos := Filters.AddFilter(JsonCollection.Pairs[I].JsonString.Value);
      Props := TFilterProperties.Create;
      FilterProps := TJSONObject(JsonCollection.Pairs[I].JsonValue);
      for j := 0 to FilterProps.Count - 1 do
      begin
        PropName := FilterProps.Pairs[J].JsonString.Value;
        PropValue := TJSONString(FilterProps.Pairs[J].JsonValue).Value;
...
begin
  ...
  if FSingleton = nil then
    Exit(nil);
  if Instance.FRegisteredFilters.TryGetValue(FilterId,Clazz) then
  begin
    if Clazz <> nil then
    begin
      try
    ...
    raise TDBXError.Create(TDBXErrorCodes.InvalidReference, FilterId+' Filter not found');
end;
