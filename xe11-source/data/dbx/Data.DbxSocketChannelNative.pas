...
begin
  ...
  LTimeout := DbxProperties[TDBXPropertyNames.ConnectTimeout];
  if LTimeout = '' then
    ConnectTimeout := 0
  else
  begin
    ConnectTimeout := StrToInt(LTimeout);
    if ConnectTimeout < 0 then
      ConnectTimeout := 0
    else
      FIdSocket.ConnectTimeout := ConnectTimeout;
  end;
  LTimeout := DbxProperties[TDBXPropertyNames.CommunicationTimeout];
  if LTimeout = '' then
    CommunicationTimeout := 0
  else
  begin
    CommunicationTimeout := StrToInt(LTimeout);
