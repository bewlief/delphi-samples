...
{$ENDIF}
{$IFDEF ANDROID}
  LibraryPreferences = '/'; { Do not localize }
{$ENDIF}
{$IFDEF Linux64}
  LibraryPreferences = '/etc/'; { Do not localize }  //  usr/lib,  var/lib
{$ENDIF}
  TDBXRegistryKey               = '\Software\Embarcadero\BDS\22.0\dbExpress'; { Do not localize }
...
type
  ...
  ///  <summary> Certificate validation with error number callback signature</summary>
  ///  <param name="Owner">object that invoked the callback</param>
  ///  <param name="Certificate">X509 certificate</param>
  ///  <param name="ADepth">number of certificates to be validated</param>
  ///  <param name="AError">Error number reported by the server</param>
  ///  <param name="Ok">boolean, expected to be true if the certificate is valid</param>
  TValidateCertificateErr = procedure(Owner: TObject; Certificate: TX509Certificate;
                                   const ADepth, AError: Integer; var Ok: Boolean) of Object;
...
//      SUB_TYPE_PASSWORD      = 1;               { Password }
      ...
      ///  <summary>Works in tandem with TStream signaling the case where
      ///  TDBXJSONStream parameter is used to read or write to a stream parameter
      ///  </summary>
      JSONStreamSubType = 41;
  end;

  /// <summary>Data types supported by the dbExpress framework (TDBX).</summary>
...
//      LOCKINFO        = 16;              { Look for LOCKINFO typedef }
  ...
  TDBXType = TInt32;
  /// <summary>Type for TDBXDataTypes.DateType. Generally used as the Date
  /// portion of a TTimeStamp which is defined as one plus the number of days
  /// since 1/1/0001.</summary>
  TDBXDate  = type TInt32;
  /// <summary>Type for TDBXDataTypes.TimeType. Generally used as the Time
  /// portion of a TTimeStamp which is defined as the number of milliseconds
  /// since midnight.</summary>
...
///  <summary>Error codes for DBXError exceptions</summary>
  ...
  /// <summary>Stores an error code for DBX exceptions.</summary>
  TDBXErrorCode = Longint;
...
//{$SCOPEDENUMS OFF}
  ...
  /// <summary>Flags used by TDBXValueType.</summary>
...
//      BlobSizeExact   = $10;
  ...
  /// <summary>Flags to indicate tracing type.</summary>
  ...
  /// <summary>Property names for connections and drivers.</summary>
  ...
  /// <summary>Type of command to be executed by a TDBXCommand</summary>
  ...
  /// <summary>List of available metadata commands to get metadata.</summary>
  ...
  /// <summary>Constants for the GetTables metadata command</summary>
  ...
  /// <summary>Constants passed to TDBXMetaDataCommands.GetProcedures metadata
  /// command.</summary>
  ...
  /// <summary>A record that holds the constants that represent supported SSL
  /// certificate file types.</summary>
  TDBXX509_Filetypes = record
  const
    SSL_FILETYPE_DER = 1;
    SSL_FILETYPE_PEM = 2;
    SSL_FILETYPE_P12 = 3;
  ...
  /// <summary>Contains trace information for DBX database drivers.</summary>
  ...
  /// <summary>Implements a collection of event procedures.</summary>
  ...
  TNameValuesStringList = class(TStringList)
  public
    procedure AddNameValues(const AStrings: TStrings); virtual;
    procedure MergeNameValues(const AStrings: TStrings); virtual;
  end;

  /// <summary>TStringList with a dictionary to improve performance for managing
  /// key/value pairs. This TStringList implementation does not support
  /// operations that change the location of a key in the list, such as Delete,
  /// Insert, and Exchange.</summary>
  ...
  public
    ...
    procedure AddNameValues(const AStrings: TStrings); override;
    procedure Exchange(Index1, Index2: Integer); override;
    procedure Insert(Index: Integer; const S: string); override;
    property Values[const Name: string]: string read GetValue write SetValue;
  end;

//{$DEFINE USEDICTIONARYSTRINGLIST}
  ...
  private type
{$IFDEF USEDICTIONARYSTRINGLIST}
    TStringListClass = TDictionaryStringList; // TFasterStringList;
{$ELSE}
    TStringListClass = TNameValuesStringList; // TFasterStringList;
{$ENDIF}
  private
    [Weak]FDBXContext:        TDBXContext;
    FProperties:              TStringListClass;
    [Weak]FOwner:             TObject;
...
//    procedure CreateDelegatePaths;
  ...
  protected
    ...
    procedure UnloadDriver(DriverName: string); virtual;
...
//    property ConnectionFactory: TDBXConnectionFactory read GetConnectionFactory write SetConnectionFactory;
  ...
  /// <summary>Creates new instances of TDBXConnection from the dbxdrivers.ini
  /// and dbxconnections.ini files</summary>
...
//    class function LoadIniFile(FileName: string): TMemIniFile; static;
  ...
  /// <summary>Implementation of TDBXConnectionFactory that does not load driver
  /// and connection properties.</summary>
  ...
  /// <summary>Collection of parameters needed when loading a driver.</summary>
...
//    property MetaDataReader: TObject read FMetaDataReader write FMetaDataReader;
  ...
  /// <summary>Used by driver implementations to morph from one command type to
  /// another.</summary>
...
//    property BlobSizeExact: Boolean read IsBlobSizeExact;
  ...
  /// <summary>
  /// A TDBXJSONStream type may be used as a parameter or return value in server methods.  Use this
  /// type to pass a JSON stream between DataSnap servers and clients.  A TDBXJSONStream is either writable or readable, but not both. IN parameters are used to pass a readable TDBXJSONStream to a server method.  OUT parameters and return values are used to pass a TDBXJSONStream that the server method has written.
  /// </summary>
  TDBXJSONStream = class
  private
  ...
  public
    /// <summary>Create a writable stream</summary>
    constructor Create; overload;
    /// <summary>Create a readable stream</summary>
    constructor Create(const AStream: TStream; AOwnsStream: Boolean); overload;
    destructor Destroy; override;
    /// Close the reader or writer
    procedure Close;
    /// <summary>Remove the stream and take ownership</summary>
    function ExtractStream: TStream;
    /// <summary>Retrieve a TJSONReader.  An exception will be raised if the stream is not readable</summary>
    property Reader: TJSONReader read GetJSONReader;
    /// <summary>Retrieve a TJSONWriter.  An exception will be raised if the stream is not writable</summary>
    property Writer: TJSONWriter read GetJSONWriter;
    /// <summary>Get the stream used for reading or writing.</summary>
  ...
  TDBXValue = class
    private
      // Note this reference can create an ARC cycle.  To break cycle, DisposeOf must be used
      // to release TDBXValueType.  See TDBXValueTypeList.Clear.
...
{$ENDIF !NEXTGEN}
      ...
      ///<summary>
      ///  Gets the TStream value for binary data types as a JDBXJSONStream.
      ///  Throws a TDBXError exception if TDBXValue.IsNull is true.
      ///</summary>
...
{$ENDIF !NEXTGEN}
      ...
      ///<summary>
      ///  Sets the TStream value for binary data types as a TDBXJSONStrer.
      ///</summary>
      procedure SetJSONStream(const Value: TDBXJSONStream; AInstanceOwner: Boolean); virtual;
      ///<summary>
  ...
  /// <summary>Implementation of TDBXWritableValue for Parameters that have no
  /// type.</summary>
...
{$ENDIF}
  ...
  /// <summary>Implementation of TDBXWritableValue for
  /// TDBXDataTypes.CursorType.</summary>
  ...
  /// <summary>Implementation of TDBXWritableValue for Int8Type
  /// values.</summary>
  ...
  /// <summary>Implementation of TDBXWritableValue for TDBXDataTypes.SingleType
  /// values.</summary>
  ...
  public
    function Read(const Buffer: TArray<Byte>; const Offset: NativeInt; const Size: NativeInt): NativeInt; virtual; abstract;
    function Eos: Boolean; virtual; abstract;
    function Size: Int64; virtual; abstract;
    procedure Close; virtual; abstract;
  end;

  /// <summary>Used internally by driver implementations and
  /// TDBXStreamValue.</summary>
  TDBXLookAheadStreamReader = class(TDBXStreamReader)
    private
      ...
      FConvertedStream:   TStream;
      FConvertedStreamOwner: Boolean;
    protected
      ///<summary>Used internally to prevent streams from being freed until the
      /// TDBXStreamValue is freed.
      ///</summary>
      function ExtractOwnedStreams: TArray<TStream>;
    public
      destructor Destroy; override;
      procedure SetStream(const Stream: TStream; const AInstanceOwner: Boolean);
      function Read(const Buffer: TArray<Byte>; const Offset: NativeInt; const Count: NativeInt): NativeInt; override;
  ...
  TDBXByteStreamReader = class(TDBXStreamReader)
    private
      ...
      FPosition:          NativeInt;
    public
      constructor Create(Bytes: TArray<Byte>; Offset: NativeInt; Count: NativeInt);
      destructor Destroy; override;
      function Read(const Buffer: TArray<Byte>; const Offset: NativeInt; const Count: NativeInt): NativeInt; override;
      function Eos: Boolean; override;
      function Size: Int64; override;
  ...
  /// <summary>Implementation of TDBXWritableValue for
  /// TDBXDataTypes.ByteArrayType values.</summary>
  TDBXStreamValue = class(TDBXByteArrayValue)
    ...
    public
      ...
      procedure SetJSONStream(const Value: TDBXJSONStream; AInstanceOwner: Boolean); override;
      procedure SetValue(const Value: TDBXValue); override;
  end;

  /// <summary>Implementation of TDBXWritableValue for WideStringType
  /// values.</summary>
  ...
  /// <summary>Implementation of TDBXWritableValue for TDBXAnsiChars
  /// values.</summary>
...
//      procedure SetAsDateTime(const Value: TDateTime); override;
  ...
  /// <summary>Implementation of TDBXWritableValue for
  /// TDBXDataTypes.DbxTableType.</summary>
  ...
  /// <summary>Implementation of TDBXWritableValue for DBXConnectionType
  /// values.</summary>
...
{$ENDIF !NEXTGEN}
  ...
  /// <summary>Implementation of TDBXByteReader.</summary>
...
//------------------------------------------------------------------------------
  ...
  /// <summary>Used by driver implementors to access private or protected
  /// API.</summary>
  ...
  /// <summary>Registry for TDBXDriverLoader keyed by driver name.</summary>
  TDBXDriverRegistry = class
    ...
    public
      ...
      class procedure UnloadDriver(DriverName: string); static;
...
{$ENDIF}
  ...
  /// <summary>Implements the API communication layer.</summary>
  ...
  /// <summary>Anonymous procedure used with the TDBXScheduler.</summary>
  TDBXScheduleEvent = reference to procedure;

  /// <summary>TDBXScheduler is a lightweight transient scheduler for user
  ...
  private
    ...
    private
      ...
      FLastFailure: TDateTime;  // Not used
    protected
{$IFNDEF POSIX}
...
{$ENDIF}
  ...
  /// <summary>Represents the abstract notation of a principal.</summary>
  TPrincipal = class abstract
  public
    function GetName: string; virtual; abstract;
  end;

  /// <summary>Represents an X.500 principal.</summary>
  ...
  /// <summary>Base class for all keys.</summary>
  TKey = class abstract
  public
  ...
  /// <summary>Subclass of TKey, used specifically for public keys.</summary>
  TPublicKey = class abstract(TKey)
  end;

  /// <summary>Abstract class for managed certificates.</summary>
  ...
  /// <summary>Abstract class for common functionality of Command
  /// Handlers.</summary>
  ...
  /// <summary>An abstract class for a result command handler.</summary>
...
{$ENDIF !NEXTGEN}
  ...
  TDBXJSONWriter = class(TJSONTextWriter)
  private
  ...
  TDBXJSONReader = class(TJSONTextReader)
  private
...
procedure TDBXConnectionFactory.UnloadDriver(DriverName: string);
begin
  if FDrivers.ContainsKey(DriverName) then
    FDrivers.Remove(DriverName);
end;
...
{$ENDIF}
  ...
  begin
    ...
    begin
      ...
      DriverFileName := AppDir + PathDelim + TDBXDriverFile;
      ConnectionFileName := AppDir + PathDelim + TDBXConnectionFile;
    end;
    Result := ConfigFilesFound;
  end;
...
{$IFDEF POSIX}
  ...
  begin
    ...
    DriverFilename := GetHomePath + LibraryPreferences + TDBXDriverFile;
    ConnectionFilename := GetHomePath + LibraryPreferences + TDBXConnectionFile;
    if FileExists(ConnectionFileName) or FileExists(DriverFileName) then
    begin
      Result := True;
      Exit;
...
var
  ...
  TrimProperties : TDBXProperties.TStringListClass;
  Index: TInt32;
begin
  Properties := nil;
  ...
  try
    ...
    TrimProperties := TDBXProperties.TStringListClass.Create;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        FConnectionProperties.AddOrSetValue(SectionList.Strings[Index], TDBXPropertiesItem.Create(DbxProperties));
      end;
    finally
      ConnectionFile.Free;
      SectionList.Free;
...
var
  Index: Integer;
begin
  if List is TDBXProperties.TStringListClass then
  begin
    FProperties.BeginUpdate;
    try
      FProperties.AddNameValues(TDBXProperties.TStringListClass(List));
    finally
  ...
  begin
    FProperties.BeginUpdate;
    try
      if Self.FProperties.Count = 0 then
        FProperties.Assign(List)
      else
...
begin
  ...
  FProperties := TDBXProperties.TStringListClass.Create;
  FOnChange := nil;
  FProperties.OnChange := DoOnChange;
...
function TDBXProperties.GetProperties: TStrings;
begin
  Result := FProperties;
end;
...
begin
  ...
  begin
    FProperties.BeginUpdate;
    try
      FProperties.MergeNameValues(TDBXProperties.TStringListClass(List));
    finally
      FProperties.EndUpdate;
    end;
  end
...
begin
  Create(ErrorCode, ErrorMessage);  // Delphi runtime automatically sets the InnerException.
end;
...
class procedure TDBXDriverRegistry.UnloadDriver(DriverName: string);
var
  Ind: Integer;
  List: TList<TDBXDriver>;
begin
  ...
  begin
    DBXDriverRegistry.FDriverClasses.Remove(DriverName);
    TDBXConnectionFactory.GetConnectionFactory.UnLoadDriver(DriverName);

    List := DBXDriverRegistry.FDrivers.LockList;
    try
      Ind := DBXDriverRegistry.FindDriver(List, DriverName);
      if Ind >= 0 then
        DBXDriverRegistry.FreeDriver(List[Ind]);
    finally
      DBXDriverRegistry.FDrivers.UnLockList;
    end;
  end;
end;
...
procedure TDBXWritableValue.SetJSONStream(const Value: TDBXJSONStream; AInstanceOwner: Boolean);
begin
  TDBXValueType.InvalidTypeAccess(ValueType.DataType, TDBXDataTypes.BytesType);
end;
...
begin
  ...
  begin
    // Use DisposeOf to break ARC cycle.
    // A TDBXParameter, may have reference to a TDBXValue which, in turn,
    // has a reference back to the TDBXParameter.
    FValueTypeArray[Index].DisposeOf;
  end;
...
function TDBXValue.GetJSONStream(AInstanceOwner: Boolean): TDBXJSONStream;
begin
  TDBXValueType.InvalidTypeAccess(ValueType.DataType, TDBXDataTypes.BytesType);
  Result := nil;
end;
...
begin
  ...
  begin
    ...
    begin
      LValue := TDBXStreamValue(Value);
      if LValue.FStreamStreamReader <> nil then
      begin
        if FStreamStreamReader <> nil then
        begin
          // Take ownership of stream
          FStreamStreamReader.FConvertedStream :=
            LValue.FStreamStreamReader.FConvertedStream;
          FStreamStreamReader.FConvertedStreamOwner :=
            True;
          LValue.FStreamStreamReader.FConvertedStream := nil;
        end;
      end;
    end;
...
begin
  ...
  begin
    if FStreamStreamReader <> nil then
    begin
      if FStreamStreamReader.FStream = FLastReadStream then
        FStreamStreamReader.FStream := nil;
      if FStreamStreamReader.FConvertedStream = FLastReadStream then
        FStreamStreamReader.FConvertedStream := nil;
    end;
    if (FOwnedStreams <> nil) and (FOwnedStreams.Remove(FLastReadStream) >= 0) then
      FLastReadStream := nil
    else
...
begin
  ...
  begin
    ...
    if AInstanceOwner then
      FStreamStreamReader.FConvertedStreamOwner := AInstanceOwner;
  end;
end;

function TDBXStreamValue.GetJSONStream(AInstanceOwner: Boolean): TDBXJSONStream;
var
  LStream: TStream;
begin
  LStream := GetStream(AInstanceOwner);
  if LStream = nil then
    Result := nil
  else
    if AInstanceOwner then
    begin
      // False means do not free stream
      Result := TDBXJSONStream.Create(LStream, False);
    ...
    begin
      // Free stream
      Result := TDBXJSONStream.Create(LStream, True);
    end
end;
...
begin
  ...
  else if FSetPending and not FIsNull and (FByteStreamReader <> nil) then
  begin
    Result := TBytesStream.Create(FByteStreamReader.FBytes);
    LNewInstance := True;
  end
...
begin
  ...
  //  ValueType.FailIfReadOnly;
    ...
    if FStreamStreamReader <> nil then
    begin
      LStreams := FStreamStreamReader.ExtractOwnedStreams;
      FreeAndNil(FStreamStreamReader);
      if LStreams <> nil then
      begin
        // Take ownership of streams
        if FOwnedStreams = nil then
          FOwnedStreams := TObjectList<TStream>.Create;
        FOwnedStreams.AddRange(LStreams);
      end;
    end;
...
begin
  ...
  begin
    ...
    else {if Length(FBytes) = FByteLength then}
      inherited SetRowValue;
  end else
    inherited SetRowValue;
end;

procedure TDBXStreamValue.SetJSONStream(const Value: TDBXJSONStream; AInstanceOwner: Boolean);
begin
  if Value = nil then
    SetStream(nil, AInstanceOwner)
  else
  begin
    if AInstanceOwner then
    begin
      Value.Close;
      SetStream(Value.BaseStream, True);
      if FFreeList = nil then
        FFreeList := TObjectList<TObject>.Create;
      FFreeList.Add(Value);
      Value.FOwnsStream := False;
    end
    else
    begin
      Value.Close;
      SetStream(Value.BaseStream, False);
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      FConvertedStream := FStream;
      FConvertedStreamOwner := FInstanceOwner;
      FStream := Stream;
      FInstanceOwner := True; // Own this stream since we created it
    end;
...
begin
  if FConvertedStreamOwner then
    FConvertedStream.Free;
  if FInstanceOwner then
    FStream.Free;
  inherited;
end;
...
function TDBXLookAheadStreamReader.ExtractOwnedStreams: TArray<TStream>;
var
  LList: TList<TStream>;
begin
  LList := TList<TStream>.Create;
  try
    if FInstanceOwner and (FStream <> nil) then
    begin
      LList.Add(FStream);
      FStream := nil;
    end;
    if FConvertedStreamOwner and (FConvertedStream <> nil) then
    begin
      LList.Add(FConvertedStream);
      FConvertedStream := nil;
    end;
...
begin
  if FInstanceOwner and (FStream <> Stream) then
    FreeAndNil(FStream);
  FInstanceOwner    := AInstanceOwner;
  FEOS              := false;
  FHasLookAheadByte := false;
...
constructor TDBXByteStreamReader.Create(Bytes: TArray<Byte>; Offset, Count: NativeInt);
begin
  inherited Create;
  FBytes := Bytes;
  FOffset := Offset;
...
function TDBXStream.Seek(Offset: LongInt; Origin: Word): Longint;
begin
...
function TDBXStreamReaderStream.Read(var Buffer; Count: Longint): Longint;
var
  Available, Remaining, TransferBytes: NativeInt;
  DestPointer: PByte;
begin
  Result := 0;
...
function TDBXStreamReaderStream.Seek(Offset: Longint; Origin: Word): Longint;
var
  LVal: Integer;
begin
...
function TDBXStreamReaderStream.Write(const Buffer; Count: Longint): Longint;
begin
  Result := 0;
end;
...
function TDBXBytesStream.Read(var Buffer; Count: Longint): Longint;
var
...
function TDBXBytesStream.Write(const Buffer; Count: Longint): Longint;
begin
  Result := 0;
end;

function TDBXBytesStream.Seek(const Offset: Int64; Origin: TSeekOrigin): Int64;
begin
  case Origin of
    soBeginning:
      FOffset := Offset;
...
{$ENDIF}
  ...
  begin
    ...
    begin
      TMonitor.Enter(FEventSemaphore);
      try
        // Wait for change to events or thread stop
        TMonitor.Wait(FEventSemaphore, INFINITE);
      finally
        TMonitor.Exit(FEventSemaphore);
      end;
    end;
       ...
       // calculate the miliseconds
        ...
        if not Terminated then
          ...
          begin
            ...
            if TDBXScheduler.Instance.PeekTime = 0 then
              // Event cancelled
...
var
  Pair: TPair<string, TEventPointer>;
begin
  ...
  begin
    for Pair In Collection do
      FEvents.Add(Pair.Key, Pair.Value);
  end;
end;

{ TDictionaryStringList }

procedure TDictionaryStringList.AddNameValues(const AStrings: TStrings);
var
  LPair: TPair<string, integer>;
  LValue: string;
  LKey: string;
  LKeyLength: Integer;
  LStrings: TDictionaryStringList;
begin
  if (Count <> 0) and (AStrings is TDictionaryStringList) then
  begin
    LStrings := TDictionaryStringList(AStrings);
    for LPair in LStrings.FDictionary do
    begin
      LKeyLength := Length(LPair.Key);
      LValue := LStrings[LPair.Value].Substring(LKeyLength+1);
      LKey := LStrings[LPair.Value].Substring(0, LKeyLength);
      if LValue <> '' then
        Values[LKey] := LValue;
    end;
  end
...
function TDictionaryStringList.AddObject(const S: string;
...
begin
  ...
  begin
    I := AnsiPos(NameValueSeparator, S);
    if I > 0 then
      FDictionary.AddOrSetValue(UpperCase(Copy(S, 1, I-1)), Result);
  end;
end;

procedure TDictionaryStringList.Assign(APersistent: TPersistent);
var
  LPair: TPair<string, Integer>;
begin
  if APersistent is TDictionaryStringList then
  begin
    FAssigning := True;
    try
      FDictionary.Clear;
      ...
      for LPair in TDictionaryStringList(APersistent).FDictionary do
        FDictionary.Add(LPair.Key, LPair.Value);
    finally
      FAssigning := False;
    end;
  end
...
begin
  raise TDBXError.Create(SNotImplemented); // not supported
  inherited;
end;
...
function TDictionaryStringList.GetValue(const Name: string): string;
begin
  Result := inherited Values[Name];
end;

function TDictionaryStringList.IndexOfName(const Name: string): Integer;
begin
  if not FDictionary.TryGetValue(UpperCase(Name), Result) then
    Result := -1;
...
procedure TDictionaryStringList.SetValue(const Name, Value: string);
var
  I: Integer;
begin
  Assert(not FAssigning);
  I := IndexOfName(Name);
  if I >= 0 then
  begin
    if Value <> Strings[I].Substring(Length(Name)+1) then
      Put(I, Name + NameValueSeparator + Value);
  end
  else
  begin
    I := Count;
    InsertItem(I, Name + NameValueSeparator + Value, nil);
    FDictionary.AddOrSetValue(UpperCase(Name), I);
  end;
end;

{ TNameValuesStringList }

procedure TNameValuesStringList.AddNameValues(const AStrings: TStrings);
var
  Index: Integer;
begin
  if Count = 0 then
    Self.Assign(AStrings)
  else
    for Index := 0 to AStrings.Count - 1 do
      Self.Add(AStrings.Names[Index] + '=' + Trim(AStrings.ValueFromIndex[Index]));
end;

procedure TNameValuesStringList.MergeNameValues(const AStrings: TStrings);
var
  Index: Integer;
  Pos: Integer;
  Key: string;
...
begin
  ...
  try
    ...
    begin
      LUpperKey := Uppercase(Self.Names[Index]);
      if not LDictionary.ContainsKey(LUpperKey) then
        LDictionary.Add(LUpperKey, Index);
    end;
    for Index := 0 to AStrings.Count - 1 do
    begin
      Key := AStrings.Names[Index];
      LUpperKey := Uppercase(Key);
      if not LDictionary.TryGetValue(LUpperKey, Pos) then
      begin
        Pos := Self.Add(Key + '=' + AStrings.ValueFromIndex[Index].Trim);
        Assert(Pos = Self.Count - 1);
        LDictionary.Add(LUpperKey, Pos);
      end
      else
        Self[Pos] := Key + '=' + AStrings.ValueFromIndex[Index];
    end;
  finally
    LDictionary.Free;
  end;
...
constructor TDBXJSONWriter.Create(const AStream: TStream);
begin
  FStreamWriter := TStreamWriter.Create(AStream);
  inherited Create(FStreamWriter);
end;

destructor TDBXJSONWriter.Destroy;
begin
  inherited;
  FStreamWriter.Free;
end;
...
constructor TDBXJSONReader.Create(const AStream: TStream);
begin
  FStreamReader := TStreamReader.Create(AStream);
...
constructor TDBXJSONStream.Create;
begin
  FBaseStream := TMemoryStream.Create;
  FOwnsStream := True;
  FJSONWriter := TDBXJSONWriter.Create(FBaseStream);
end;

procedure TDBXJSONStream.Close;
begin
...
constructor TDBXJSONStream.Create(const AStream: TStream; AOwnsStream: Boolean);
begin
  FBaseStream := AStream;
  FOwnsStream := AOwnsStream;
  FJSONReader := TDBXJSONReader.Create(FBaseStream);
end;

destructor TDBXJSONStream.Destroy;
begin
  FJSONReader.Free;
  FJSONWriter.Free;
  if FOwnsStream then
...
function TDBXJSONStream.ExtractStream: TStream;
begin
  Result := FBaseStream;
  FBaseStream := nil;
...
function TDBXJSONStream.GetJSONReader: TJSONReader;
begin
  Result := FJSONReader;
  if Result = nil then
    raise TDBXError.Create(TDBXErrorCodes.NotSupported, sReaderNotSupported);
end;

function TDBXJSONStream.GetJSONWriter: TJSONWriter;
begin
  Result := FJSONWriter;
  if Result = nil then
    raise TDBXError.Create(TDBXErrorCodes.NotSupported, sWriterNotSupported);
end;
