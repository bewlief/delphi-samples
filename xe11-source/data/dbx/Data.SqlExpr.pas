...
//    TDBXTypes.SUB_TYPE_DBSOLEOBJ, TDBXTypes.SUB_TYPE_TYPEDBINARY, 0, TDBXTypes.SUB_TYPE_FIXED, 0, // 20..24
  ...
  /// <summary>Maps the TDBXDataTypes into a FldTypeMap.</summary>
...
{ Forward declarations }
  ...
  TProcParamList = TList<SPParamDesc>;

{ TSQLBlobStream }

  TSQLBlobStream = class(TMemoryStream)
  private
    [Weak]FDataSet: TCustomSQLDataSet;
...
{$ENDIF !NEXTGEN}
  ...
  private
    ...
    [Weak]FParentData: TConnectionData;
  ...
  private
    ...
    FAutoClone: Boolean;
    [Weak]FCloneParent: TSQLConnection;
    ...
    FMonitorUsers: TList<TSQLMonitor>;
...
{$ENDIF !NEXTGEN}
    ...
    function OpenSchemaTable(eKind: TSchemaType; SInfo: string; SQualifier: string = ''; SPackage: string = ''; SSchemaName: string = ''): TCustomSQLDataSet;
...
{$IFNDEF NEXTGEN}
    function Execute(const SQL: string; Params: TParams;
      ResultSet: Pointer): Integer; overload; deprecated 'Use overloaded method instead';
{$ENDIF !NEXTGEN}
...
{ TSQLDataLink }
  ...
  private
    [Weak]FSQLDataSet: TCustomSQLDataSet;
  protected
...
{ TSQLTable }
  ...
  /// <summary>TDataSet to call DataSnap server methods and retrieve results
  /// from them.</summary>
...
begin
  ...
  try
    ...
    begin
      ...
      FParams.SetStrings(ConnectionProps.Properties);
      FDriverName := ConnectionProps[TDBXPropertyNames.DriverName];
    end else
    begin
...
{$ENDIF !NEXTGEN}
      ...
      TDBXDataTypes.WideStringType:
        begin
          ...
          if not LBlank then
          begin
            if FieldDataSize > Length(Buffer) then
              FieldDataSize := Length(Buffer);
...
{$IFDEF NEXTGEN}
function TCustomSQLDataSet.GetCalculatedField(Field: TField; Buffer: TValueBuffer): Boolean;
{$ELSE}
function TCustomSQLDataSet.GetCalculatedField(Field: TField; Buffer: Pointer): Boolean;
{$ENDIF}
var
  RecBuf: TArray<Byte>;
begin
    ...
    begin
      ...
      begin
        ...
        if Param.IsNull then
          ...
          ArgDesc.iLen := Param.GetDataSize;
        if (Param.ParamType in [ptOutput, ptResult, ptInputOutput]) and
           (Param.DataType in [ftString, ftWideString, ftFixedChar, ftFixedWideChar, ftBytes, ftVarBytes]) and
           (Param.Size > Integer(ArgDesc.iLen)) then
          ArgDesc.iLen := Param.Size;
      end;
      if (Param.ParamType in [ptOutput, ptResult, ptInputOutput]) and
      ...
      begin
        ...
        case Param.DataType of
          ...
          begin
            ...
            begin
              FDBXCommand.Parameters[I].Size := Param.Size;
            ...
            begin
              ...
              try
                IndexColumns.FGetIndexesText := DataSet.FDBXCommand.Text + '.' +
                  AddMetadataQuotes(IdxName, False);
...
function TConnectionData.GetConnectionName: string;
begin
  if Assigned(FConnection) then
...
begin
  BeginUpdate;
  try
    if not Assigned(FProperties) or not Assigned(Value) or (FProperties.ClassType <> Value.ClassType) then
    begin
      FreeAndNil(FProperties);
