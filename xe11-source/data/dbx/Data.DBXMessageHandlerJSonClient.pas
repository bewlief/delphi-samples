...
begin
  ...
  try
    ...
    try
      Count := StrToInt((TJSONNumber(TJSONArray(CallbackParam).Items[0])).Value);
      ArgType := TJSONNumber(TJSONArray(CallbackParam).Items[1]).AsInt;
      ...
      OutputArg := InvokeClientCallback(ArgType, Parameter, TJSONArray(CallbackParam).Items[2]);
    finally
      CallbackParam.Free;
    end;
