...
{$ENDIF !NEXTGEN}
  ...
  /// <summary>Used to keep metadata passed back from the execution of a server
  /// function, other than any output or return parameters in the signature of
  /// the function. For example, to specify a custom HTTP status code, message
  /// or content.</summary>
...
type
  /// <summary>Represents the base class for DataSnap filter
  /// collection.</summary>
  ...
  /// <summary>Base class for DataSnap filters.</summary>
...
{$ELSE}
    class procedure CopyStringToBuilder(const Source: String; AnsiStringBuilderSize: Integer; var Value: TDBXAnsiStringBuilder); static;
{$ENDIF !NEXTGEN}
...
{$ENDIF !NEXTGEN}
  ...
  /// <summary>Returns the stored invocation metadata for the current thread. If
  /// there is no stored invocation metadata, it returns a new instance of
  /// TDSInvocationMetadata or nil if CreateIfNil is False.</summary>
  function GetInvocationMetadata(CreateIfNil : Boolean = True): TDSInvocationMetadata;

  /// <summary>Stores the specified instance of invocation metadata for the
  /// current thread.</summary>
  procedure SetInvocationMetadata(MetaData: TDSInvocationMetadata);

  /// <summary>Frees and nils the invocation metadata stored for the current
  /// thread.</summary>
...
class procedure TDBXPlatform.CopyByteArray(const Source: TArray<Byte>;
  SourceOffset: NativeInt; const Dest: TArray<Byte>; DestOffset, Count: NativeInt);
begin
  Assert(Length(Dest) >= (Count+DestOffset));
  if Count > 0 then
...
class procedure TDBXPlatform.CopyStringToBuilder(const Source: String;
  AnsiStringBuilderSize: Integer; var Value: TDBXAnsiStringBuilder);
var
  Count: Integer;
  LBytes: TBytes;
begin
  // need room for null terminator.
  Count := Length(Source);
  if Count >= AnsiStringBuilderSize then
    UnexpectedStringOverflow(Source, AnsiStringBuilderSize);
  if Count > 0 then
  begin
    LBytes := TEncoding.ANSI.GetBytes(Source);
    Move(LBytes[0], Value^, Count);
  end;
  PByte(Value + Count)^ := 0;
end;
{$ENDIF !NEXTGEN}
