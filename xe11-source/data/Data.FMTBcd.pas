...
function StrToBcd(const AValue: string; const Format: TFormatSettings): TBcd; overload;
function TryStrToBcd(const AValue: string; var Bcd: TBcd): Boolean; overload;
function TryStrToBcd(const AValue: string; var Bcd: TBcd; const Format: TFormatSettings): Boolean; overload;
...
function BcdToStr(const Bcd: TBcd; const Format: TFormatSettings): string; overload;
function BcdToDouble(const Bcd: TBcd): Double;
function BcdToInteger(const Bcd: TBcd; Truncate: Boolean = False): Integer;
function BcdToInt64(const Bcd: TBcd): Int64;
...
function BCDToCurrency(const BCD: TBcd): Currency; Inline;
...
function BcdToStrF(const Bcd: TBcd; Format: TFloatFormat;
  const Precision, Digits: Integer; const FS: TFormatSettings): string; overload;
function FormatBcd(const Format: string; const Bcd: TBcd): string; overload;
function FormatBcd(const Format: string; const Bcd: TBcd; const FS: TFormatSettings): string; overload;
...
procedure RequiredDigits(const ABcd: TBcd; out Left, Right: Integer);
var
  I: Integer;
  B: Byte;
begin
...
function BcdToStr(const Bcd: TBcd; const DecimalSeparator: Char): string; overload;
var
...
begin
  ...
  while PBuf^ = '0' do
    Inc(PBuf);
  if (PBuf^ = #0) or (PBuf^ = DecimalSeparator) then
    Dec(PBuf);
...
begin
  Result := BcdToStr(Bcd, Format.DecimalSeparator);
end;
...
function CurrToBCD(const Curr: Currency; var BCD: TBcd; Precision: Integer = 32;
  Decimals: Integer = 4): Boolean;
begin
  NormalizeBcd(TBcd(CurrToStr(Curr)), Bcd, Precision, Decimals);
  Result := True;
end;

procedure Round(var S: string; DecSep: Char; Precision: Integer); forward;
...
begin
  ...
  begin
    // Clean extra two nibbles, because they can contain garbage
    I := (B.Precision - 1) div 2;
    B.Fraction[I + 1] := 0;
    B.Fraction[I + 2] := 0;
    Inc(B.Precision, 4);
  end
...
procedure BcdNumberString(var S: string; Format: TFloatFormat;
  Precision, Digits: Integer; const FS: TFormatSettings);
var
  Negative: Boolean;
  DecPos: Integer;
  Buf: string;
  ...
  begin
    ...
    if (Power = -1) and
       ( (Options.LastDigit <> 0) or
         ((LPStr^ <> #0) and ((Options.FirstDigit <> 0) or (Options.DecimalIndex > 0))) ) then
...
begin
  ...
  if Options.FormatLength = 0 then
  begin
    StrPCopy(Buf, LStr);
    Inc(Buf, LStr.Length);
    Exit;
...
begin
  ...
  begin
    Section := FindSection(fsNegative, PChar(Format));
    if Section = PChar(Format) then // Negative section was blank, put '-'
      PutChar(LBuffer, '-');
  end
  else if IsZero(Bcd) then
    Section := FindSection(fsZero, PChar(Format))
  else
    Section := FindSection(fsPositive, PChar(Format));
...
{$ENDIF !NEXTGEN}
      ...
      begin
        ...
        try
          ...
          LTemp.VInt64 := BcdToInt64(TFMTBcdData(TFMTBcdVarData(Source).VBcd).Bcd);
