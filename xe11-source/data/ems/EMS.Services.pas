...
type
  ...
  TEMSLoggingOutputProc = reference to procedure(const ACategory: string; const AJSON: TJSONObject);
  TEMSLoggingEnabledFunc = TFunc<Boolean>;
  IEMSLoggingService = interface
  ...
  IEMSLoggingServiceExt = interface
    ['{261E2580-D991-4E1B-9FFF-D68EBD2DEB70}']
  ...
  IEMSResourceRequest = interface
    ...
    function Post(const AContext: IEMSResourceRequestContext; const AResourceName: string;
      const ASegments: TEMSResourceRequestSegments; const AQueryParams: TEMSResourceRequestQueryParams;
      const ARequestContent: IEMSResourceRequestContent): IEMSResourceResponseContent;
    ...
    function Put(const AContext: IEMSResourceRequestContext; const AResourceName: string;
      const ASegments: TEMSResourceRequestSegments; const AQueryParams: TEMSResourceRequestQueryParams;
      const ARequestContent: IEMSResourceRequestContent): IEMSResourceResponseContent;
    /// <summary>Execute HEAD resource request.</summary>
    function Head(const AContext: IEMSResourceRequestContext; const AResourceName: string;
      const ASegments: TEMSResourceRequestSegments; const AQueryParams: TEMSResourceRequestQueryParams): IEMSResourceResponseContent;
  end;
...
begin
  ...
  try
    ...
    if LResponse.TryGetObject(LJSONObject) then
      if LJSONObject.TryGetValue<TJSONArray>(TJSONNames.GroupUsers, LTemp) then
    ...
    try
      LJSONObject.AddPair(TJSONNames.GroupUsers, LJSONArray);
      LJSONArray := nil;
...
begin
  ...
  if LResponse.TryGetObject(LJSONObject) then
  begin
    if LJSONObject.TryGetValue<TJSONArray>(TJSONNames.GroupUsers, LJSONArray) then
    begin
...
begin
  ...
  try
    LRequest := FService.CreateRequestContent(LObj);
                                                                                 
                                                                 
    Result := FService.Post(FContext, TSegmentNames.Users, TEMSResourceRequestSegments.Create(CLogout), nil, LRequest);
  finally
    LObj.Free;
  end;
end;
