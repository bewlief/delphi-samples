...
uses
  System.SysUtils, System.Generics.Collections, System.JSON, System.Classes,
  System.Rtti;
...
type
  ...
  public
    ...
    procedure Write(const AStrings: TStrings;
      out AParameters: TParameters); overload;
    procedure Write(const AStrings: TStringWriter;
      out AParameters: TParameters); overload; virtual; abstract;
  end;
  ...
  TQueryToJSON = class
  private
    class procedure WriteFilter(const AFilter: TFilterItem;
  ...
  public
    class procedure WriteFilters(const AFilters: TFilters;
      const AJSON: TJSONObject); static;
  end;
  ...
  public
    ...
    // Internal methods
    class procedure Parse(const AJSONObject: TJSONObject;
      const AFilters: TFilters; const AUtcOffset: Integer); static;
  end;
...
uses
  System.TypInfo, System.StrUtils, System.DateUtils, System.TimeSpan;
...
procedure TQueryWriter.Write(const AStrings: TStrings;
  out AParameters: TArray<TParameter>);
var
...
class procedure TQueryToJSON.WriteSimpleFilter(const AFilter: TSimpleFilter;
  const AJSON: TJSONObject);
var
  LOp: string;
begin
  ...
  if LOp <> '' then
    AJSON.AddPair(AFilter.Attribute, TJSONObject.Create(TJSONPair.Create(LOp,
      ValueToJSON(AFilter.Value))))
  else
    AJSON.AddPair(AFilter.Attribute, ValueToJSON(AFilter.Value));
end;

class procedure TQueryToJSON.WriteExistsFilter(const AFilter: TExistsFilter;
  const AJSON: TJSONObject);
begin
  if AFilter.Exists then
    AJSON.AddPair(AFilter.Attribute,
      TJSONObject.Create(TJSONPair.Create(TJSONOpNames.OpExists,
      TJSONTrue.Create)))
  else
    AJSON.AddPair(AFilter.Attribute,
      TJSONObject.Create(TJSONPair.Create(TJSONOpNames.OpExists,
...
class procedure TQueryToJSON.WriteRangeFilter(const AFilter: TRangeFilter;
  const AJSON: TJSONObject);

  procedure WriteOne(const AJSON: TJSONObject; AOp: TRangeFilter.TOperator;
    const AValue: TValue);
  var
    LOp: string;
  begin
...
class procedure TQueryToJSON.WriteOrFilter(const AFilter: TOrFilter;
  const AJSON: TJSONObject);
var
  LArray: TJSONArray;
  LJSON: TJSONObject;
...
class procedure TQueryToJSON.WriteAndFilter(const AFilter: TAndFilter;
  const AJSON: TJSONObject);
var
  LArray: TJSONArray;
  LJSON: TJSONObject;
...
class procedure TQueryToJSON.WriteInFilter(const AFilter: TInFilter;
  const AJSON: TJSONObject);
var
  LArray: TJSONArray;
  LValue: TValue;
...
procedure ParseJSON(const AJSONObject: TJSONObject;
  const AFilters: TFilters); forward;
...
procedure ParseOr(const AJSONArray: TJSONArray;
  const AFilter: TOrFilter); forward;
procedure ParseAnd(const AJSONArray: TJSONArray;
  const AFilter: TAndFilter); forward;
procedure ParseNameValue(const AName: string; AJSONValue: TJSONValue;
  const AFilters: TFilters); forward;
procedure ParseSimpleFilter(const AName: string; AOp: TSimpleFilter.TOperator;
  const AValue: TJSONValue; const AFilters: TFilters); forward;
procedure ParseOneOperator(const AName, AOp: string;
  const AJSONValue: TJSONValue; const AFilters: TFilters); forward;
function ParseValue(const AJSONValue: TJSONValue): TValue; forward;
procedure ParseTwoOperators(const AName, AOp1: string;
  const AJSONValue1: TJSONValue; const AOp2: string;
  const AJSONValue2: TJSONValue; const AFilters: TFilters); forward;
...
begin
  raise EFilterParserException.Create('Unexpected JSON: ' +
    AJSONValue.ToString);
end;
...
procedure ParseExistsFilter(const AName: string; const AValue: TJSONValue;
...
procedure ParseSimpleFilter(const AName: string; AOp: TSimpleFilter.TOperator;
  const AValue: TJSONValue; const AFilters: TFilters);
var
...
procedure ParseInFilter(const AName: string; AOp: TInFilter.TOperator;
  const AValue: TJSONValue; const AFilters: TFilters);
var
  LInFilter: TInFilter;
...
procedure ParseRangeFilter(const AName: string; AOp1: TRangeFilter.TOperator;
  const AValue1: TJSONValue; AOp2: TRangeFilter.TOperator;
  const AValue2: TJSONValue; const AFilters: TFilters);
var
...
procedure ParseOneOperator(const AName, AOp: string;
  const AJSONValue: TJSONValue; const AFilters: TFilters);
var
...
procedure ParseTwoOperators(const AName, AOp1: string;
  const AJSONValue1: TJSONValue; const AOp2: string;
  const AJSONValue2: TJSONValue; const AFilters: TFilters);
var
...
function ParseValue(const AJSONValue: TJSONValue): TValue;
  ...
  function IsValidDateString(const ADateStr: string;
  ...
  begin
    LServerTimeZone := Trunc(TTimeZone.Local.UtcOffset.Hours * 60 +
...
procedure ParseCompare(const AName: string; const AJSONValue: TJSONValue;
  const AFilters: TFilters);
var
...
begin
  ...
  begin
    ...
    begin
      ParseOneOperator(AName, LJSONObject.Pairs[0].JsonString.Value,
        LJSONObject.Pairs[0].JsonValue, AFilters);
    end
    else if LJSONObject.Count = 2 then
      ParseTwoOperators(AName, LJSONObject.Pairs[0].JsonString.Value,
        LJSONObject.Pairs[0].JsonValue, LJSONObject.Pairs[1].JsonString.Value,
        LJSONObject.Pairs[1].JsonValue, AFilters)
    else
