...
uses
  ...
  RSConsole.Data, System.JSON, System.Rtti, FMX.Grid, FMX.Layouts,
  RSConsole.Types, FMX.Memo, FMX.ListBox, IPPeerClient,
  ...
  FMX.ComboEdit, FMX.ListView.Types, FMX.ListView, FMX.Grid.Style,
  FMX.ScrollBox;
...
type
  ...
  private
    ...
    function AddChekBox(AVertScrollBox: TVertScrollBox; const AName: string)
      : TCheckBox overload;
  ...
  public
    ...
    property ChannlesList: TArray<string> read FChannlesList
      write FChannlesList;
    property ActiveGrid: TStringGrid read FActiveGrid;
    property InitialsChannels: integer read FInitialsChannels
      write FInitialsChannels;
    property GridModified: Boolean read FGridModified write FGridModified;
  end;

implementation

uses RSConsole.Consts, RSConsole.Form,
  RSConsole.ModuleBackend, RSConsole.FrameViews,
  REST.Backend.EMSApi, REST.Backend.EMSProvider, RSConsole.TypesViews;
...
begin
  ...
  begin
    ...
    ChannelsListBox.RemoveObject
      (ChannelsListBox.ItemByIndex(ChannelsListBox.Items.IndexOf
      (AChannelsArray[I])));
  end;
...
begin
  ...
  try
    ...
    begin
      ...
      if (LGroupsNamesList <> nil) and
        (LGroupsNamesList.IndexOf(FGroupsList[I]) >= 0) then
...
begin
  ...
  else if FEMSConsoleData.AddInstallationData(DeviceTypeCombo.Selected.Text,
    TokenEdit.Text, CustomInstallGrid, AddChannelsListBox, LInstallId) then
    CloseModal;
end;
...
begin
  ...
  try
    if (AUsersList <> nil) then
      ...
      begin
        (BackendDM.BackendPush1.ProviderService as IGetEMSApi)
          .EMSApi.RetrieveUser(AUsersList[I], LUserOut, nil);
...
function TAddFrame.AddChekBox(AVertScrollBox: TVertScrollBox;
  const AName: string): TCheckBox;
begin
    ...
    for I := 0 to GroupBoxUsers.ComponentCount - 1 do
      if GroupBoxUsers.Components[I] is TCheckBox then
        if (TCheckBox(GroupBoxUsers.Components[I]).IsChecked = IsSelected) and
          FModifiedCheckBox.Contains(TCheckBox(GroupBoxUsers.Components[I]))
        then
    ...
    for I := 0 to GroupBoxGroups.ComponentCount - 1 do
      if GroupBoxGroups.Components[I] is TCheckBox then
        if (TCheckBox(GroupBoxGroups.Components[I]).IsChecked = IsSelected) and
          FModifiedCheckBox.Contains(TCheckBox(GroupBoxGroups.Components[I]))
        then
