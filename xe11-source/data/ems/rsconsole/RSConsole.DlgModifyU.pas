...
uses
  ...
  RSConsole.FrameAdd, System.Generics.Collections,
  RSConsole.Types, RSConsole.Data;
...
type
  ...
  public
    { Public declarations }
    constructor Create(AOwner: TComponent; AEMSConsoleData: TEMSConsoleData);
    ...
    property InstallationInfo: TInstallationInfo read FInstallationInfo
      write FInstallationInfo;
    property EdgeModuleInfo: TEdgeModuleInfo read FEdgeModuleInfo
      write FEdgeModuleInfo;
    property ResourceInfo: TResourcesInfo read FResourceInfo
      write FResourceInfo;
  end;

implementation

uses RSConsole.Consts, System.JSON, FMX.TabControl;

{$R *.fmx}

constructor TFormAddDlg.Create(AOwner: TComponent;
  AEMSConsoleData: TEMSConsoleData);
begin
...
procedure TFormAddDlg.GridEditingDone(Sender: TObject; const Col, Row: Integer);
var
  LCell: TCell;
...
begin
  ...
  begin
    ...
    MessageDlg(strUser + ': ' + FUserInfo.UserName + ' ' + strNotUpdated,
  ...
  try
    ...
    begin
      for I := 0 to LCellList.Count - 1 do
        if (AddFrame.CustomFieldsGrid.Cells[0, LCellList.Items[I].Row] <> '') and 
...
begin
  ...
  begin
    ...
    MessageDlg(strGroup + ': ' + FGroupInfo.GroupName + ' ' + strNotUpdated,
      TMsgDlgType.mtWarning, [TMsgDlgBtn.mbOK], 0);
end;
...
begin
  ...
  try
    ...
    begin
      for I := 0 to LCellList.Count - 1 do
        if (AddFrame.CustomEdgeModuleStringGrid.Cells[0, LCellList.Items[I].Row] <> '') and 
           (AddFrame.CustomEdgeModuleStringGrid.Cells[1, LCellList.Items[I].Row] <> '') then
      ...
      AddFrame.EMSConsoleData.UpdateEdgeModule(FEdgeModuleInfo.ModuleID,
        FEdgeModuleInfo.ModuleName, FEdgeModuleInfo.Protocol,
...
begin
  ...
  begin
    ...
    MessageDlg(strEdgeModule + ': ' + FEdgeModuleInfo.ModuleName + ' ' +
      strNotUpdated, TMsgDlgType.mtWarning, [TMsgDlgBtn.mbOK], 0);
end;
...
begin
  ...
  try
    ...
    begin
      for I := 0 to LCellList.Count - 1 do
        if (AddFrame.CustomGroupFieldsGrid.Cells[0, LCellList.Items[I].Row] <> '') and 
           (AddFrame.CustomGroupFieldsGrid.Cells[1, LCellList.Items[I].Row] <> '') then
...
begin
  ...
  begin
    ...
    MessageDlg(strInstallation + ': ' + FInstallationInfo.DeviceToken + ' ' +
      strNotUpdated, TMsgDlgType.mtWarning, [TMsgDlgBtn.mbOK], 0);
end;
...
begin
  ...
  try
    ...
    begin
      for I := 0 to LCellList.Count - 1 do
        if (AddFrame.CustomResourcesStringGrid.Cells[0, LCellList.Items[I].Row] <> '') and 
           (AddFrame.CustomResourcesStringGrid.Cells[1, LCellList.Items[I].Row] <> '') then
      ...
      AddFrame.EMSConsoleData.UpdateResource(FResourceInfo.ModuleID,
        FResourceInfo.ResourceName, LJSON)
    end;
    Result := True;
  finally
    LJSON.Free;
...
begin
  ...
  try
    ...
    begin
      LJSON.AddPair(TInstallationInfo.cDeviceType,
        AddFrame.DeviceTypeCombo.Selected.Text);
      if FDataModified.TryGetValue(FInstallationInfo.InstallationID, LCellList)
      then
      begin
        for I := 0 to LCellList.Count - 1 do
          if (AddFrame.CustomInstallGrid.Cells[0, LCellList.Items[I].Row] <> '') and 
             (AddFrame.CustomInstallGrid.Cells[1, LCellList.Items[I].Row] <> '') then
