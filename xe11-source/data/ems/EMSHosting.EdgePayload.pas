...
type
  ...
  ['{561B2566-0DEA-40B9-9E6D-9588A9102A37}']
    ...
    property TenantId: string read GetTenantId;
  end;
  ...
  protected
    ...
    function GetTenantId: string;
  public
    constructor Create(const AMethod, AServerHost, AClientHost, ABasePath, APath: string; const AQuery: TStringKeyValues; const ASessionToken, ATenantId: string);
  end;
...
constructor TEdgeRequest.Create(const AMethod, AServerHost, AClientHost, ABasePath, APath: string; const AQuery: TStringKeyValues;
  const ASessionToken, ATenantId: string);
begin
  FMethod := AMethod;
  FPath := APath;
  FBasePath := ABasePath.Replace(FPath, '');
  FServerHost := AServerHost;
  FClientHost := AClientHost;
  FQuery := AQuery;
...
function TEdgeRequest.GetClientHost: string;
begin
  Result := FClientHost;
end;
...
function TEdgeRequest.GetTenantId: string;
begin
  Result := FTenantId;
end;
...
var
  LMethod, LPath, LSessionToken, LBasePath, LServerHost, LTenantId: string;
  LVersion: TEdgeVersion;
...
begin
  ...
  Result := TEdgeRequest.Create(LMethod, LServerHost, '', LBasePath, LPath, LQuery, LSessionToken, LTenantId);
end;
