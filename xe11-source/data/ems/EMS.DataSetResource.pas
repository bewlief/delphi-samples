...
type
  ...
  TEMSDataSetResouceOption = (roEnableParams, roEnablePaging, roEnableSorting,
    roReturnNewEntityKey, roReturnNewEntityValue, roAppendOnPut);
  ...
  TEMSFDDataSetAdaptorHelper = class helper for TEMSFDDataSetAdaptor
  protected
    function ProcessGet: Boolean;
  end;
...
var
  LVal: string;
  LPage, LStart, LLimit: Integer;

  function StrToVal(const AStr: string; AAllowZero: Boolean): Integer;
  begin
    Result := StrToInt(AStr);
    if (Result < 0) or not AAllowZero and (Result = 0) then
      EEMSHTTPError.RaiseError(500, sResourceErrorMessage,
        Format(sDataSetAdaptorInvalidPaging, [Result]));
  end;
  ...
  begin
    if FContext.Request.Params.TryGetValue(FResource.PageParamName, LVal) then
      if LVal <> '' then begin
        ...
        SetPaging((LPage - 1) * FResource.PageSize, FResource.PageSize);
      end
      else
        SetPaging(-1, -1);
  end;
...
begin
  if Self is TEMSFDDataSetAdaptor then
    if TEMSFDDataSetAdaptor(Self).ProcessGet then
      Exit;
...
{$R-}
function TEMSFDDataSetAdaptorHelper.ProcessGet: Boolean;
var
  LFormat: TFDStorageFormat;
  LDataSet: TFDDataSet;
  LStr: TStream;
...
begin
  ...
  try
    ...
    ACharset := LValues.Negotiate('utf-8, *;q=0.5', LWeight,
      ...
      begin
        try
          TEncoding.GetEncoding(AName).DisposeOf;
          Result := True;
        except
          Result := False;
        end;
...
begin
  ...
  begin
    ...
    try
      ...
      AContext.Request.Body.JSONReader.Rewind;
    finally
      LJSON.Free;
    end;
  end;
