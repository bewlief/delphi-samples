...
uses Data.DBConsts, Data.FmtBcd, System.AnsiStrings
{$IFDEF MACOS}
    ...
    case p^ of
      ...
      begin
        ...
        begin
          ...
          Token := System.AnsiStrings.AnsiExtractQuotedStr(Mark, Literal);
          if DotStart then
            Result := stFieldName else
          if p^ = '.' then
            Result := stTableName else
    ...
    case p^ of
      ...
      begin
        ...
        begin
          ...
          while ((p^ = ' ') or (p^ = #10) or (p^ = #13) or (p^ = ',') or (p^ = '(') or (p^ = ';')) do Inc(p);
      end;
      '.':
      begin
        if Assigned(TokenStart) then
        ...
        if SQLToken in SQLSections then
          ...
          case SQLToken of
            ...
            begin
              ...
              else if DataSet.FieldDefs.Count >= FieldIndex then
...
begin
  case FToken of
    etSymbol:
      ...
      else if TokenSymbolIs(FStrTrue) or TokenSymbolIs('TRUE') then
        begin
          Result := FFilter.NewNode(enConst, coNOTDEFINED, 1, nil, nil);
          Result.FDataType := ftBoolean;
          Result.FScopeKind := skConst;
        end
      else if TokenSymbolIs(FStrFalse) or TokenSymbolIs('FALSE') then
        begin
