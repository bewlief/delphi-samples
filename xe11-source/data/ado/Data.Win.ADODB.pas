...
uses
  Winapi.Windows, System.Types, System.Variants, Winapi.ActiveX, System.SysUtils, 
  System.Classes, System.Generics.Collections, System.TypInfo, Data.DB, Winapi.OleDB,
  Winapi.ADOInt;
...
{ Redclare ADO types exposed by this unit }
  ...
  _Connection = Winapi.ADOInt._Connection;
{$EXTERNALSYM _Connection}
  _Command = Winapi.ADOInt._Command;
{$EXTERNALSYM _Command}
  _Recordset = Winapi.ADOInt.Recordset;
{$EXTERNALSYM _Recordset}
  Error = Winapi.ADOInt.Error;
{$EXTERNALSYM Error}
  Errors = Winapi.ADOInt.Errors;
{$EXTERNALSYM Errors}
  _Parameter = Winapi.ADOInt._Parameter;
{$EXTERNALSYM _Parameter}
  Parameters = Winapi.ADOInt.Parameters;
{$EXTERNALSYM Parameters}
  Property_ = Winapi.ADOInt.Property_;
{$EXTERNALSYM Property_}
  Properties = Winapi.ADOInt.Properties;
...
{ TADOConnection }
  ...
  published
    ...
    [Stored('IsDefaultDatabaseStored')]
...
{ TCustomADODataSet }
  ...
  ///<summary>
  /// Auto incremental fields detection mode.
  /// Some OLEDB providers, like SQL Server one, will exchange additional data
  /// packets with a DB server to detect is a field an uto-incrementing one.
  /// On a large SELECT list this may affect performance critically.
  ///</summary>
  TAutoIncFieldsMode = (
    /// <summary>
    /// afAll - try to detect every one field is it auto-incremental or not.
    /// </summary>
    afAll,
    /// <summary>
    /// afFirst - stop detection of auto-incrementing after the first detected field.
    /// </summary>
    afFirst,
    /// <summary>
    /// afNone - do not detect auto-incremental fields.
    /// </summary>
    afNone);
  ...
  protected
    ...
    function AreParamsMasterDependent: Boolean; virtual;
    ...
    procedure InitializeMasterFields; virtual;
    ...
    procedure UpdateRecordSetPosition(Buffer: PByte);
  ...
  public
    ...
    function GetFieldData(Field: TField; Buffer: Pointer; NativeFormat: Boolean): Boolean; overload; override; deprecated 'Use overloaded method instead';
  ...
  private
    function GetAutoIncFieldsMode: TAutoIncFieldsMode;
    procedure SetAutoIncFieldsMode(const Value: TAutoIncFieldsMode);
  public
    /// <summary>
    /// Use AutoIncFieldsMode property to optimise dataset opening time by
    /// avoiding extra data packet exchange to detect auto incremental fields.
    /// </summary>
    property AutoIncFieldsMode: TAutoIncFieldsMode read GetAutoIncFieldsMode
      write SetAutoIncFieldsMode default afAll;
  end;
...
uses
  Data.DBCommon, Winapi.Mtx, Data.DBConsts, System.Win.ComObj, Data.Win.ADOConst,
  System.WideStrUtils, Data.FmtBcd, System.VarUtils;
...
const
  ...
  DataTypeValues: array[TDataType] of TOleEnum = (
    ...
    adBoolean, adDouble, adDouble, adCurrency, adDate, adDBTime, adDate, // 5..11
    ...
    adBigInt, adEmpty, adEmpty, adEmpty, adEmpty, adLongVarBinary, adLongVarChar, // 25..31
    adVariant, adIUnknown, adIDispatch, adGuid, adDBTimeStamp, adDecimal, // 32..37
    adWChar, adLongVarWChar, adDBTimeStamp, adEmpty, // 38..41
    adUnsignedBigInt, adTinyInt, adUnsignedTinyInt, adDouble, adEmpty, adEmpty, adEmpty, // 42..48
    adDBTimeStamp, adEmpty, adSingle); // 49..51
...
function PropertyExists(const PropList: Winapi.ADOInt.Properties; const PropName: WideString): Boolean;
var
  I: Integer;
begin
...
begin
  ...
  for I := 0 to DataSet.Fields.Count - 1 do
    Result := Result + (IntPtr(Dataset.Fields[I]) shr (I mod 16));
end;
...
begin
  ...
  try
    ...
    LDataSet.First;
    SourcesName := LDataSet.FieldByName('SOURCES_NAME'); { do not localize }
    SourcesType := LDataSet.FieldByName('SOURCES_TYPE'); { do not localize }
    Names.BeginUpdate;
    try
...
begin
  ...
  begin
    ...
    for I := DataSetCount - 1 downto 0 do
      if DataSets[I].State in dsEditModes then DataSets[I].Cancel;
...
begin
  ...
  try
    ...
    begin
      List.Add(VarToStr(Fields.Fields[COLUMN_NAME].Value));
      Fields.MoveNext;
    end;
  finally
    List.EndUpdate;
...
begin
  ...
  for I := 0 to DataSetCount - 1 do
    if stClosed in DataSets[I].RecordsetState then DataSets[I].Close;
end;
...
begin
  ...
  begin
    ...
    if (adStatus <> adStatusCancel) and Assigned(OnLogin) then
...
function TParameters.InternalRefresh: Boolean;
      ...
      try
        ...
        if OLEDBParameters.GetParameterInfo(ParamCount, PDBPARAMINFO(ParamInfo), @NamesBuffer) = S_OK then
          ...
          begin
            ...
            Parameter.Attributes := ParamInfo[I].dwFlags and (adParamSigned or adParamNullable or adParamLong); { Mask out Input, Output, etc flags }
  ...
  begin
    try
      Command.CommandObject.Parameters.Refresh;
      for I := 0 to Command.CommandObject.Parameters.Count - 1 do
      begin
      { We can't use the instance of the parameter in the ADO collection because
        ...
        Parameter := Command.CommandObject.CreateParameter(Command.CommandObject.Parameters[I].Name,
                                     Command.CommandObject.Parameters[I].Type_,
                                     Command.CommandObject.Parameters[I].Direction,
                                     Command.CommandObject.Parameters[I].Size, EmptyParam);
        Parameter.Precision := Command.CommandObject.Parameters[I].Precision;
        Parameter.NumericScale := Command.CommandObject.Parameters[I].NumericScale;
        Parameter.Attributes := Command.CommandObject.Parameters[I].Attributes;
        AddParameter.FParameter := Parameter;
      end;
    except
      { do nothing }
...
begin
  ...
  if not ((TVarData(Value).VType = varError) and
    (LongWord(TVarData(Value).VError) = $80020004)) then
...
function TParameters.ParseSQL(const SQL: WideString; DoCreate: Boolean): WideString;
    ...
    case CurPos^ of
      ...
      ':': // parameter
        begin
          ...
          begin
            Result := Result + Copy(SQL, StartPos - BeginPos + 1, CurPos - StartPos - 1) + '?';
  ...
  case DataType of
    ftString, ftFixedChar, ftMemo:
      begin
        ...
        Stream.ReadBuffer(Pointer(StrData)^, Len);
        Self.Value := StrData;
      end;
    ftWideString, ftFixedWideChar, ftWideMemo:
      begin
        SetLength(WStrData, Len div 2);
        Stream.ReadBuffer(Pointer(WStrData)^, Len);
        Self.Value := WStrData;
      end;
...
begin
  Result := ADOTypeToFieldType(ParameterObject.Type_, ParameterObject.Precision,
    ParameterObject.NumericScale);
end;
...
procedure TADOCommand.SetCommandType(const Value: TCommandType);
begin
...
begin
  case Field.DataType of
    ftVariant:
      { TVariantField should work for any field type }
      Compatible := True;
    ftFloat, ftCurrency, ftBCD, ftFMTBCD:
      { Numeric and Doubles are interchangeable }
      Compatible := FieldDef.DataType in [ftFloat, ftCurrency, ftBCD, ftFMTBCD];
    ftString, ftFixedChar, ftWideString, ftFixedWideChar:
      { As are string and widestring }
      Compatible := FieldDef.DataType in [ftString, ftFixedChar, ftWideString, ftFixedWideChar];
  else
    Compatible := False;
  end;
...
var
  ...
  begin
    ...
    begin
      ...
      I := 0;
      LName := F.Name;
      while (LName = '') or (FieldDefs.IndexOf(LName) >= 0) do
      begin
        Inc(I);
        if F.Name = '' then
          LName := WideFormat('COLUMN%d', [I]) else { Do not localize }
          LName := WideFormat('%s_%d', [F.Name, I]);
      end;
      ...
      case FieldType of
        ...
        ftBCD, ftFMTBCD:
          begin
            LPrecision := F.Precision;
            LSize := ShortInt(F.NumericScale);
        ...
        ftInteger:
          ...
          begin
            FieldType := ftAutoInc;
            if AutoIncFieldsMode = afFirst then
      ...
      FieldDef.Size := LSize;
      FieldDef.Precision := LPrecision;
      if (FieldDef.DataType = ftDataSet) and (Fields.Count = 0) then
        ObjectView := True;
    end;
  end;
...
begin
  ...
  if (Count > 0) and (AutoIncFieldsMode <> afNone) then
...
procedure TCustomADODataSet.InitOleDBAccess(Initializing: Boolean);
  ...
  begin
    ...
    Binding.iOrdinal := ColInfo.iOrdinal;
    Binding.wType := ColInfo.wType;
    Binding.obStatus := Offset;
    Binding.obLength := Offset + 4;
    Binding.obValue := Offset + 8;

    if (Binding.wType = DBTYPE_WSTR) and (ColInfo.ulColumnSize <> $FFFFFFFF) then
      Binding.cbMaxLen := ColInfo.ulColumnSize * SizeOf(WideChar)
    else
      Binding.cbMaxLen := ColInfo.ulColumnSize;
    Inc(Offset, (Binding.cbMaxLen + 15) and not 7); {cbMaxLen+Status+Length, rounded up to nearest 8 byte boundry};
  end;
...
begin
  ...
  if MasterDataLink.Active and not AreParamsMasterDependent then
    SetDetailFilter;
...
function TCustomADODataSet.GetClonedDataSet(WithSettings: Boolean): TDataSet;
var
  DS: TCustomADODataSet;
begin
  Result := NestedDataSetClass.Create(nil);
  try
    DS := Result as TCustomADODataSet;
    if WithSettings then
    begin
      DS.Filter := Filter;
      DS.OnFilterRecord := OnFilterRecord;
      DS.FilterOptions := FilterOptions;
      DS.Filtered := Filtered;
      if IndexName <> '' then
        DS.IndexName := IndexName
      else
        DS.IndexFieldNames := IndexFieldNames;
      DS.DataSource := DataSource;
      DS.MasterFields := MasterFields;
    end;
    DS.Clone(Self, ltReadOnly);
  except
    Result.Free;
    raise;
  end;
...
procedure TCustomADODataSet.DataEvent(Event: TDataEvent; Info: NativeInt);
begin
  case Event of
    dePropertyChange:
    ...
    deLayoutChange:
      if Active and Assigned(Reserved) and
        (FieldListCheckSum(Self) <> IntPtr(Reserved)) then
        Reserved := nil;
  end;
  inherited;
end;
...
function TCustomADODataSet.AreParamsMasterDependent: Boolean;
var
  I: Integer;
begin
  Result := False;
  if (Parameters.Count > 0) and Assigned(MasterDataLink.DataSet) then
    for I := 0 to Parameters.Count - 1 do
      if (Parameters[I].Direction in [pdInput, pdInputOutput]) and
         (MasterDataLink.DataSet.FindField(Parameters[I].Name) <> nil) then
      begin
        Result := True;
        Break;
      end;
...
begin
  ...
  if not AreParamsMasterDependent then
  begin
    CheckBrowseMode;
...
var
  ...
  begin
    ...
    if (VarType(V1) = VT_DECIMAL) or (VarType(V2) = VT_DECIMAL) then
      Result := not SameStr(VarToStr(V1), VarToStr(V2)) else
      Result := V1 <> V2;
  end;
...
begin
  if MasterDataLink.DataSource <> nil then
    for I := 0 to MasterDataLink.Fields.Count - 1 do
      Parameters.ParamByName(TField(MasterDataLink.Fields[I]).FieldName).Assign(MasterDataLink.Fields[I]);
end;
...
procedure TCustomADODataSet.InternalGotoBookmark(Bookmark: TBookmark);
begin
...
procedure TCustomADODataSet.InternalSetToRecord(Buffer: TRecordBuffer);
begin
  if PRecInfo(Buffer)^.BookmarkFlag in [bfCurrent, bfInserted] then
    InternalGotoBookmark(@PRecInfo(Buffer)^.Bookmark);
end;

function TCustomADODataSet.GetBookmarkFlag(Buffer: TRecordBuffer): TBookmarkFlag;
begin
  Result := PRecInfo(Buffer)^.BookmarkFlag;
end;

procedure TCustomADODataSet.SetBookmarkFlag(Buffer: TRecordBuffer; Value: TBookmarkFlag);
begin
  PRecInfo(Buffer).BookmarkFlag := Value;
end;

procedure TCustomADODataSet.GetBookmarkData(Buffer: TRecordBuffer; Data: TBookmark);
begin
...
procedure TCustomADODataSet.SetBookmarkData(Buffer: TRecordBuffer; Data: TBookmark);
begin
  if Assigned(Data) then
    PRecInfo(Buffer).Bookmark := POleVariant(Data)^
  else
    PRecInfo(Buffer).BookmarkFlag := bfNA;
end;

procedure TCustomADODataSet.SetBookmarkData(Buffer: TRecordBuffer; Data: Pointer);
begin
  if Assigned(Data) then
...
function TCustomADODataSet.AllocRecordBuffer: TRecordBuffer;
begin
...
procedure TCustomADODataSet.FreeRecordBuffer(var Buffer: TRecordBuffer);
begin
...
function TCustomADODataSet.InternalGetRecord(Buffer: TRecordBuffer;
...
begin
  if (Assigned(FParentDataSet) and FParentDataSet.Active and
     ...
     (MasterDataLink.Active and (DataSource.DataSet.IsEmpty or
                                (DataSource.DataSet.State = dsInsert))) or
     (Recordset.State = adStateClosed) then
  begin
    Result := grEOF;
    Exit;
  end;
  try
      ...
      gmNext:
        begin
          ...
          if BookmarkSize = 0 then
            Finalize(PVariantList(PByte(ActiveBuffer)+SizeOf(TRecInfo))^, Fields.Count);
        end;
      gmPrior:
        begin
    ...
    if Result = grOK then
    begin
      PRecInfo(Buffer)^.RecordStatus := Recordset.Status;
      if (BookmarkSize > 0) and ((adRecDeleted and PRecInfo(Buffer)^.RecordStatus) = 0) then
      begin
        PRecInfo(Buffer)^.BookmarkFlag := bfCurrent;
        PRecInfo(Buffer)^.Bookmark := Recordset.Bookmark;
        if ControlsDisabled then
          PRecInfo(Buffer)^.RecordNumber := -2
        else
          PRecInfo(Buffer)^.RecordNumber := Recordset.AbsolutePosition;
      end
      else
        PRecInfo(Buffer)^.BookmarkFlag := bfNA;
      Finalize(PVariantList(PByte(Buffer)+SizeOf(TRecInfo))^, Fields.Count);
...
function TCustomADODataSet.GetRecord(Buffer: TRecordBuffer; GetMode: TGetMode;
...
procedure TCustomADODataSet.InternalInitRecord(Buffer: TRecordBuffer);
var
  I: Integer;
begin
  for I := 0 to Fields.Count - 1 do
    PVariantList(PByte(Buffer)+SizeOf(TRecInfo))[I] := Null;
end;

procedure TCustomADODataSet.ClearCalcFields(Buffer: PByte);
var
  I: Integer;
begin
  if CalcFieldsSize > 0 then
    for I := 0 to Fields.Count - 1 do
      if Fields[I].FieldKind in [fkCalculated, fkLookup] then
        PVariantList(PByte(Buffer)+SizeOf(TRecInfo))[Fields[I].Index] := Null;
end;

function TCustomADODataSet.GetActiveRecBuf(var RecBuf: PByte): Boolean;
begin
  case State of
    dsBlockRead,
    dsBrowse:
    ...
    dsEdit, dsInsert, dsNewValue: RecBuf := TRecordBuffer(ActiveBuffer);
    dsCalcFields,
    dsInternalCalc: RecBuf := PByte(CalcBuffer);
    dsFilter: RecBuf := @FFilterBuffer[0];
  else
    RecBuf := nil;
  end;
  Result := RecBuf <> nil;
...
procedure TCustomADODataSet.UpdateRecordSetPosition(Buffer: PByte);
begin
  ...
  begin
    if Assigned(FParentDataSet) and (FParentDataSet.Active) then
      FParentDataSet.UpdateRecordSetPosition(TRecordBuffer(FParentDataSet.ActiveBuffer));
    InternalSetToRecord(Buffer);
    CursorPosChanged;
  end;
end;

function TCustomADODataSet.GetCalcFieldTypes: TFieldTypes;
begin
  Result := inherited GetCalcFieldTypes + [ftVariant];
end;
...
function TCustomADODataSet.GetFieldData(Field: TField; var Buffer: TValueBuffer): Boolean;
begin
  Result := GetFieldData(Field, Buffer, True);
end;

function TCustomADODataSet.GetFieldData(Field: TField; var Buffer: TValueBuffer;
    ...
    case Field.DataType of
      ...
      ftBytes, ftVarBytes:
        ...
        begin
          ...
          TDBBitConverter.UnsafeFromBAVariant(Data, Buffer);
        end;
      ftInterface:
        begin
...
begin
  ...
  Data := PVariantList(PByte(RecBuf)+SizeOf(TRecInfo))[Field.Index];
...
procedure TCustomADODataSet.GetDetailLinkFields(MasterFields, DetailFields: TList);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if Idx = nil then
          GetFieldList(DetailFields, IndexFieldNames);
      end
      else
...
function TCustomADODataSet.GetFieldData(FieldNo: Integer; Buffer: Pointer): Boolean;
begin
  Result := GetFieldData(FieldByNumber(FieldNo), Buffer);
end;
...
procedure TCustomADODataSet.SetFieldData(Field: TField; Buffer: TValueBuffer);
begin
  SetFieldData(Field, Buffer, True);
end;

procedure TCustomADODataSet.SetFieldData(Field: TField; Buffer: TValueBuffer; NativeFormat: Boolean);
...
begin
  ...
  begin
    ...
    if FieldNo > 0 then
    begin
      if not (State in [dsSetKey, dsFilter]) then
      begin
        if ReadOnly then
...
begin
  if (Recordset.State <> adStateClosed) and not Recordset.BOF then
  begin
    Recordset.MoveFirst;
...
procedure TCustomADODataSet.InternalAddRecord(Buffer: TRecordBuffer; Append: Boolean);
begin
...
procedure TCustomADODataSet.InternalPost;
  ...
  begin
    try
      ...
      begin
        Data := Unassigned;
        Data := FieldData[TField(FModifiedFields[I]).Index];
        if not VarIsClear(Data) and
           (((adFldUpdatable+adFldUnknownUpdatable) and
           Recordset.Fields[TField(FModifiedFields[I]).FieldNo-1].Attributes) <> 0) then
...
begin
  try
    DataSet.Recordset.Delete(AffectRecordsValues[AffectRecords]);
    ...
    on E: Exception do
    begin
      DataSet.Recordset.CancelUpdate;
      DatabaseError(E.Message);
    end;
  end;
end;
...
begin
  ...
  begin
    if not AreParamsMasterDependent and (Value <> '') and (MasterFields <> '') then
      ...
      try
        ...
        if FieldCount = 1 then
          if VarIsNull(KeyValues) then
            FLookupCursor.Filter := GetFilterStr(Fields[0], KeyValues, Partial)
          else
            FLookupCursor.Find(GetFilterStr(Fields[0], KeyValues, Partial), 0,
        ...
        begin
          ...
          begin
            ...
            if LocateFilter <> '' then
              LocateFilter := LocateFilter + ' AND ' + FieldExpr    { Do not localize }
            else
...
begin
  ...
  begin
    ...
    try
      CalculateFields(PByte(TempBuffer));
  ...
  try
    ...
    begin
      ...
      try
        ...
        begin
          // In some cases the IndexInfo recordset contains a row of null values.
          // If we find no index name then skip the record.
          if not VarIsNull(IndexInfo.Fields[SIndexName].Value) then
          begin
            if TagVariant(IndexInfo.Fields[SOrdinalPosition].Value).ulVal > 1 then
            begin
              LIndexDef := IndexDefs.Find(IndexInfo.Fields[SIndexName].Value);
              LIndexDef.Fields := Format('%s;%s', [LIndexDef.Fields, IndexInfo.Fields[SColumnName].Value])
            end
            else
            begin
              LIndexDef := IndexDefs.AddIndexDef;
              LIndexDef.Name := VarToWideStr(IndexInfo.Fields[SIndexName].Value);
              LIndexDef.Fields := VarToWideStr(IndexInfo.Fields[SColumnName].Value);
              if IndexInfo.Fields[SPrimaryKey].Value = True then
                LIndexDef.Options := LIndexDef.Options + [ixPrimary];
              if IndexInfo.Fields[SUnique].Value = True then
                LIndexDef.Options := LIndexDef.Options + [ixUnique];
              if IndexInfo.Fields[SAutoUpdate].Value = False then
                LIndexDef.Options := LIndexDef.Options + [ixNonMaintained];
            end;
          end;
          IndexInfo.MoveNext;
        end;
...
begin
  ...
  begin
    ...
    if Active and (Value = nil) then
      RecordSet._Set_ActiveConnection(nil);
...
function TCustomADODataSet.PSExecuteStatement(const ASQL: string; AParams: TParams): Integer;
var
  I: Integer;
  Cmd: TADOCommand;
  InProvider: Boolean;
...
function TCustomADODataSet.PSExecuteStatement(const ASQL: string; AParams: TParams;
...
function TCustomADODataSet.PSGetKeyFields: string;
var
  I, Pos: Integer;
  IndexFound: Boolean;
begin
...
function TCustomADODataSet.PSGetTableName: string;
begin
  case CommandType of
...
function TCustomADODataSet.PSGetQuoteChar: string;
begin
  Result := '';
end;
...
procedure TCustomADODataSet.PSSetCommandText(const CommandText: string);
begin
  if CommandText <> '' then
...
begin
  ...
  begin
    if Prev <> nil then
      ...
      PrevErr := 0;
    Result := EUpdateError.Create(E.Message, '', EOleException(E).ErrorCode, PrevErr, E);
  end else
...
var
  FAutoIncFieldsModeDict: TDictionary<TCustomADODataSet, TAutoIncFieldsMode> = nil;

function TCustomADODataSetHelper.GetAutoIncFieldsMode: TAutoIncFieldsMode;
begin
  if FAutoIncFieldsModeDict = nil then
    Exit(afAll);
  TMonitor.Enter(FAutoIncFieldsModeDict);
  try
    if not FAutoIncFieldsModeDict.TryGetValue(Self, Result) then
      Result := afAll;
  finally
    TMonitor.Exit(FAutoIncFieldsModeDict);
  end;
end;

procedure TCustomADODataSetHelper.SetAutoIncFieldsMode(const Value: TAutoIncFieldsMode);
begin
  if FAutoIncFieldsModeDict = nil then
    Exit;
  TMonitor.Enter(FAutoIncFieldsModeDict);
  try
    FAutoIncFieldsModeDict.AddOrSetValue(Self, Value);
  finally
    TMonitor.Exit(FAutoIncFieldsModeDict);
  end;
end;
...
procedure TADODataSet.CreateDataSet;
  ...
  begin
    ...
    begin
      ...
      if (FieldDefs[I].DataType in [ftMemo, ftWideMemo, ftBlob]) and (FieldDefs[I].Size = 0) then
        FieldDefs[I].Size := High(Integer);
      Recordset.Fields._Append(FieldDefs[I].Name, FieldTypeToADOType(FieldDefs[I].DataType), FieldDefs[I].Size, Options);
    end;
  end;
...
procedure TADODataSet.DataEvent(Event: TDataEvent; Info: NativeInt);
begin
  case Event of
    deParentScroll:
      begin
    ...
    deConnectChange:
      if Active and not Bool(Info) and Assigned(RDSConnection) then
        if (CursorLocation = clUseClient) and not (csDestroying in ComponentState) then
          Recordset._Set_ActiveConnection(nil) else
          Close;
  end;
  inherited;
end;
...
function TADOQuery.GetSQL: TStrings;
begin
  Result := FSQL;
end;
...
begin
  ...
  FData := Null;
  LBuffer := @FBuffer[0];
  if not FDataSet.GetActiveRecBuf(LBuffer) then Exit;
  if Mode <> bmRead then
  begin
...
begin
  if FModified then
  try
    if FWritten and VarIsNull(FData) then
      if FField.DataType = ftMemo then
        FData := AnsiString('')
      else if FField.DataType = ftWideMemo then
        FData := WideString('');
...
function TADOBlobStream.Realloc(var NewCapacity: NativeInt): Pointer;
...
initialization
  ...
  FAutoIncFieldsModeDict := TDictionary<TCustomADODataSet, TAutoIncFieldsMode>.Create;

finalization
  GlobalMalloc := nil;
  FreeAndNil(FAutoIncFieldsModeDict);
