...
type
  ...
  public type
    ...
    public
      ...
      ReadTimeout: Integer;
      class operator Initialize(out Dest: TConnectionInfo);
...
uses System.DateUtils, System.Rtti, System.Net.HttpClient, REST.JSON.Types, System.TypInfo,
  REST.Backend.Consts;
...
begin
  ...
  FRESTClient.SecureProtocols := [THTTPSecureProtocol.TLS1 .. THTTPSecureProtocol.TLS12];
...
begin
  ...
  FRESTClient.ConnectTimeout := FConnectionInfo.ConnectTimeout;
  FRESTClient.ReadTimeout := FConnectionInfo.ReadTimeout;
end;
...
begin
  ...
  begin
    ...
    if (LJSONCode <> nil) and (LJSONMessage <> nil) then
      Result := AClass.Create(LCode, LMessage)
    else if LJSONMessage <> nil then
      Result := AClass.Create(LMessage);
  end;
  if Result = nil then
    if ARequest.Response.Content <> '' then
      Result := AClass.Create(ARequest.Response.Content)
    else
      Result := AClass.CreateFmt('%d: %s',
        [ARequest.Response.StatusCode, ARequest.Response.StatusText]);
end;
...
class operator TParseApi.TConnectionInfo.Initialize(out Dest: TConnectionInfo);
begin
  Dest.ProxyPort := 0;
  Dest.ConnectTimeout := CRestDefaultTimeout;
  Dest.ReadTimeout := CRestDefaultTimeout;
end;
