...
{$ENDIF}
  ...
  System.Net.HttpClient, REST.HttpClient,
...
begin
  ...
  RESTRequest.ResetToDefaults;
  RESTResponse.ResetToDefaults;
  UpdateComponentProperties(False);
  ...
  if FRESTParams.CustomBody.Size > 0 then
    ...
    begin
      ...
      try
        ...
        RESTRequest.AddBody(string(s), ContentTypeFromString(FRESTParams.ContentType));
      finally
...
begin
  ...
  FRESTParams.RootElement := EditRootElementTab.Text;
  FRESTParams.NestedFields := cb_NestedFields.IsChecked;
end;
...
begin
  ...
  EditRootElementTab.Text := FRESTParams.RootElement;
  cb_NestedFields.IsChecked := FRESTParams.NestedFields;
  UpdateRootElement;
end;
...
begin
  ...
  cbSSL2.IsChecked := False;
  cbSSL3.IsChecked := False;
  cbTLS1.IsChecked := False;
  cbTLS11.IsChecked := False;
  cbTLS12.IsChecked := False;
  cbTLS13.IsChecked := False;
...
begin
  try
    ...
    dlg_LoadRequestSettings.InitialDir := '';
    dlg_SaveRequestSettings.InitialDir := '';
  end;
end;
...
procedure Tfrm_Main.ConfigureHTTPConnection;
var
  LSecProts: THTTPSecureProtocols;
begin
  if cbProxy.IsChecked then
  begin
  ...
  if cbTimeout.IsChecked then
    RESTRequest.Timeout := Trunc(edt_Timeout.Value)
  else
    RESTRequest.Timeout := 0;
  LSecProts := [];
  if cbSSL2.IsChecked then
    Include(LSecProts, THTTPSecureProtocol.SSL2);
  if cbSSL3.IsChecked then
    Include(LSecProts, THTTPSecureProtocol.SSL3);
  if cbTLS1.IsChecked then
    Include(LSecProts, THTTPSecureProtocol.TLS1);
  if cbTLS11.IsChecked then
    Include(LSecProts, THTTPSecureProtocol.TLS11);
  if cbTLS12.IsChecked then
    Include(LSecProts, THTTPSecureProtocol.TLS12);
  if cbTLS13.IsChecked then
    Include(LSecProts, THTTPSecureProtocol.TLS13);
  RESTClient.SecureProtocols := LSecProts;
end;
...
begin
  ...
  TRY
    ...
    edt_ProxyUser.Text := SimpleDecryptStr(LINI.ReadString('proxy', 'username',
      SimpleEncryptStr('', AUTH_CRYPTO_VALUE)), AUTH_CRYPTO_VALUE);
    edt_ProxyPass.Text := SimpleDecryptStr(LINI.ReadString('proxy', 'password',
      SimpleEncryptStr('', AUTH_CRYPTO_VALUE)), AUTH_CRYPTO_VALUE);

    cbTimeout.IsChecked := LINI.ReadBool('timeout', 'enabled', True);
    edt_Timeout.Value := LINI.ReadInteger('timeout', 'value', 30000);

    cbSSL2.IsChecked := LINI.ReadBool('https', 'ssl2', False);
    cbSSL3.IsChecked := LINI.ReadBool('https', 'ssl3', False);
    cbTLS1.IsChecked := LINI.ReadBool('https', 'tls1', False);
    cbTLS11.IsChecked := LINI.ReadBool('https', 'tls11', False);
    cbTLS12.IsChecked := LINI.ReadBool('https', 'tls12', False);
    cbTLS13.IsChecked := LINI.ReadBool('https', 'tls13', False);
  FINALLY
    FreeAndNil(LINI);
  END;
...
begin
  if not Accepted then
    if MessageDlg(Format(RSUnableToValidateCertifcate, [Certificate.CertName]),
...
begin
  ...
  TRY
    ...
    LINI.WriteBool('timeout', 'enabled', cbTimeout.IsChecked);
    LINI.WriteInteger('timeout', 'value', Trunc(edt_Timeout.Value));

    LINI.WriteBool('https', 'ssl2', cbSSL2.IsChecked);
    LINI.WriteBool('https', 'ssl3', cbSSL3.IsChecked);
    LINI.WriteBool('https', 'tls1', cbTLS1.IsChecked);
    LINI.WriteBool('https', 'tls11', cbTLS11.IsChecked);
    LINI.WriteBool('https', 'tls12', cbTLS12.IsChecked);
    LINI.WriteBool('https', 'tls13', cbTLS13.IsChecked);
