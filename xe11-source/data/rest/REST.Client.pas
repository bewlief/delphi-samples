...
type
  ...
  private
    ...
    FContentTypeStr: TRESTContentType;
    ...
    procedure SetContentTypeStr(const AValue: TRESTContentType);
    procedure ReleaseStream;
    procedure NotifyParameterChanged(AValueChanged: Boolean);
    function GetBytes: TBytes;
    procedure ReadContentType(Reader: TReader);
    function ContentTypeStrIsStored: Boolean;
  protected
  ...
  published
    ...
    /// <summary>
    /// <para>
    /// If ContentTypeStr is set to a specific value, then this will be used as actual content-type for the
  ...
  TRESTRequestParameterHelper = class helper for TRESTRequestParameter
  private
    function GetContentType: TRESTContentType; inline;
    procedure SetContentType(const AValue: TRESTContentType); inline;
  public
    /// <summary>
    /// Duplicates ContentTypeStr for backward compatibility.
    /// </summary>
  ...
  public
    ...
    /// <summary>
    /// Cancels asynchronous REST request. This will not raise an exception.
  ...
  public type
    TAssignedValue = (rvAccept, rvHandleRedirects, rvAcceptCharset, rvAcceptEncoding,
      rvAllowCookies, rvConnectTimeout, rvReadTimeout);
    ...
    public
      ...
      procedure ParameterValuesNeeded(Sender: TObject); virtual;
    end;
  ...
  private
    ...
    function AcceptEncodingIsStored: Boolean;
    function AllowCookiesIsStored: Boolean;
    function HandleRedirectsIsStored: Boolean;
    ...
    procedure DoReadTimeout(Reader: TReader);
    ...
    function ConnectTimeoutIsStored: Boolean;
    function ReadTimeoutIsStored: Boolean;
  protected
    ...
    procedure ParameterValuesNeeded; virtual;
  ...
  public
    ...
    *)
    ...
    /// <summary>
    /// Builds the final request body that will be executed.
    /// </summary>
    function GetFullRequestBody: string;

    /// <summary>
    /// Specifies which property values were overriden.
    /// </summary>
    property AssignedValues: TAssignedValues read FAssignedValues write FAssignedValues default [];
    ...
    /// <summary>
    /// Specifies the acceptance of server cookies.
    /// </summary>
    ...
    /// </remarks>
    property HandleRedirects: Boolean read GetHandleRedirects write SetHandleRedirects stored HandleRedirectsIsStored default True;
    ...
    /// Deprecated, please use ConnectTimeout and ReadTimeout instead.
    /// </summary>
    property Timeout: Integer read GetTimeout write SetTimeout stored False default CRestDefaultTimeout;
    /// <summary>
    /// Connection timeout in milliseconds to be used for the request.
    /// </summary>
    property ConnectTimeout: Integer read GetConnectTimeout write SetConnectTimeout stored ConnectTimeoutIsStored default CRestDefaultTimeout;
    /// <summary>
    /// Response reading timeout in milliseconds to be used for the request.
    /// </summary>
    property ReadTimeout: Integer read GetReadTimeout write SetReadTimeout stored ReadTimeoutIsStored default CRestDefaultTimeout;
    ...
    /// <summary>
    /// Specifies if Events (such as OnAfterExecute) and ResponseDataSetAdapter's should run in the context of the main
  ...
  private
    ...
    property HTTPClient: TRESTHTTP read FHttpClient;
  protected
    property NotifyList: TNotifyList read FNotifyList;
    procedure ParameterListChanged; virtual;
    procedure ParameterValueChanged; virtual;
  ...
  public
    ...
    /// </remarks>
    property FallbackCharsetEncoding: string read FFallbackCharsetEncoding
  ...
  TCustomRESTRequestHelper = class helper for TCustomRESTRequest
  private
    function GetBodyCodePage: Integer;
    procedure SetBodyCodePage(const Value: Integer);
    procedure DeleteCodePage;
  public
    function GetBodyEncoding: TEncoding;
    /// <summary>
    /// Specifies request body code page. The default value is -1, which means use
    /// platform default encoding. Other standard values are: 0 - current active
    /// code page, 65001 - UTF8.
    /// </summary>
    property BodyCodePage: Integer read GetBodyCodePage write SetBodyCodePage default -1;
  end;

const
  sRequestDefaultAccept = ctAPPLICATION_JSON + ', ' +
    ctTEXT_PLAIN + '; q=0.9, ' + ctTEXT_HTML + ';q=0.8,';
...
{$ENDIF}
  ...
  System.NetEncoding, System.JSON.Types, System.Net.Mime, System.NetConsts,
...
begin
  ...
  begin
    ...
    FContentTypeStr := LParam.ContentTypeStr;
  end
  else
...
begin
  ...
  FOptions := [];
  FContentTypeStr := DefaultRESTContentType;
end;
...
procedure TRESTRequestParameter.ReadContentType(Reader: TReader);
var
  S: string;
  I: Integer;
begin
  S := Reader.ReadIdent.ToLower;
  if S.StartsWith('ct') then
    S := S.Substring(2);
  S := S.Replace('_', '/', []);
  if S.Contains('application/vnd') then
  begin
    S := S.Replace('_', '.', [rfReplaceAll]);
    I := S.LastIndexOf('.');
    if I >= 0 then
      S[Low(S) + I] := '+';
  end
...
procedure TRESTRequestParameter.DefineProperties(Filer: TFiler);
begin
  inherited DefineProperties(Filer);
  Filer.DefineProperty('ContentType', ReadContentType, nil, False);
end;
...
procedure TRESTRequestParameter.SetContentTypeStr(const AValue: TRESTContentType);
begin
  if FContentTypeStr <> AValue then
  begin
    FContentTypeStr := AValue;
    Changed(False);
  end;
end;

function TRESTRequestParameter.ContentTypeStrIsStored: Boolean;
begin
  Result := ContentTypeStr <> DefaultRESTContentType;
end;
...
procedure TRESTRequestParameter.Clear;
begin
  if (Stream <> nil) or (Value <> '') then
  begin
    ReleaseStream;
    FValue := '';
    Changed(False);
...
function TRESTRequestParameterHelper.GetContentType: TRESTContentType;
begin
  Result := ContentTypeStr;
end;

procedure TRESTRequestParameterHelper.SetContentType(const AValue: TRESTContentType);
begin
  ContentTypeStr := AValue;
end;
...
procedure TRESTExecutionThread.Cancel;
begin
  FRequest.Cancel;
end;
...
var
  FRequestPages: TDictionary<TCustomRESTRequest, Integer> = nil;

function TCustomRESTRequestHelper.GetBodyCodePage: Integer;
begin
  if FRequestPages = nil then
    Exit(-1);
  TMonitor.Enter(FRequestPages);
  try
    if not FRequestPages.TryGetValue(Self, Result) then
      Result := -1;
  finally
    TMonitor.Exit(FRequestPages);
  end;
end;

procedure TCustomRESTRequestHelper.SetBodyCodePage(const Value: Integer);
begin
  if FRequestPages = nil then
    Exit;
  TMonitor.Enter(FRequestPages);
  try
    FRequestPages.AddOrSetValue(Self, Value);
  finally
    TMonitor.Exit(FRequestPages);
  end;
end;

procedure TCustomRESTRequestHelper.DeleteCodePage;
begin
  if FRequestPages = nil then
    Exit;
  TMonitor.Enter(FRequestPages);
  try
    FRequestPages.Remove(Self);
  finally
    TMonitor.Exit(FRequestPages);
  end;
end;

function TCustomRESTRequestHelper.GetBodyEncoding: TEncoding;
var
  LPage: Integer;
begin
  LPage := BodyCodePage;
  ...
  else if LPage = CP_ACP then
    Result := TEncoding.ANSI
  else if LPage = CP_UTF8 then
    Result := TEncoding.UTF8
  else
    Result := TEncoding.GetEncoding(LPage);
end;
...
begin
  ...
  try
    ...
    // the goal is to embed all relevant params into a stream
    if SameText(AContentType, TRESTContentType.ctAPPLICATION_X_WWW_FORM_URLENCODED) then
      ABodyStream := TStringStream.Create('', GetBodyEncoding)
    else if SameText(AContentType, TRESTContentType.ctMULTIPART_FORM_DATA) then
    begin
    ...
    begin
      ...
      begin
        ...
        else if SameText(AContentType, TRESTContentType.ctAPPLICATION_X_WWW_FORM_URLENCODED) then
        begin
          // WWWForm
...
begin
  ...
  try
    ...
    HandleEvent(FResponse.EndUpdate);
  end;
end;

procedure TCustomRESTRequest.Cancel;
begin
  if FClient <> nil then
    FClient.FHttpClient.Cancel;
end;

function TCustomRESTRequest.GetIsCancelled: Boolean;
begin
  Result := (FClient <> nil) and FClient.FHttpClient.IsCancelled;
end;
  ...
  Result :=
    (AParam.Kind = TRESTRequestParameterKind.pkGETorPOST) and (
      ...
      (Method in [TRESTRequestMethod.rmPOST, TRESTRequestMethod.rmPUT, TRESTRequestMethod.rmPATCH]) and
        ((AParam.ContentType = TRESTContentType.ctNone) and
         not IsContentType(AContentType, [TRESTContentType.ctAPPLICATION_X_WWW_FORM_URLENCODED,
                              TRESTContentType.ctMULTIPART_FORM_DATA]) or
         not IsContentType(AContentType, [TRESTContentType.ctAPPLICATION_X_WWW_FORM_URLENCODED,
                              TRESTContentType.ctMULTIPART_FORM_DATA,
                              TRESTContentType.ctAPPLICATION_JSON,
                              TRESTContentType.ctAPPLICATION_XML]))) or
...
begin
  ...
  begin
    ...
    if (LParam.ContentType <> ctNone) and
       not IsContentType(LSpecificContentType, [ctMULTIPART_FORM_DATA, ctAPPLICATION_X_WWW_FORM_URLENCODED]) then
  ...
  // "multipart" is necessary if we have more than one
  // body-parameter or body-parameters as well as post/get-parameters
  // (which should be quite unusual, but "multipart" is the only
...
begin
  // the goal is to embed all relevant params into the url.
  for LParam in AParamList do
...
function TCustomRESTRequest.GetFullRequestBody: string;
var
  LParamList: TRESTRequestParameterArray;
  LContentType: TRESTContentType;
  LBodyStream: TStream;
  LBodyStreamOwner: Boolean;
...
begin
  Result := GetFullURL;
  LParamList := CreateUnionParameterList;
  DoApplyURLSegments(LParamList, Result);
  LContentType := ContentType(LParamList);
  LBodyStream := nil;
  LBodyStreamOwner := False;
  LReader := nil;
  LEncoding := GetBodyEncoding;
  try
    DoPrepareRequestBody(LParamList, LContentType, LBodyStream, LBodyStreamOwner);
    LBodyStream.Position := 0;
    LReader := TStreamReader.Create(LBodyStream, LEncoding, False);
...
begin
  ...
  for i := 0 to LResponse.Cookies.Count - 1 do
    FResponse.Cookies.Add(LResponse.Cookies[i]);
  if (FResponse.Headers.IndexOfName(sSetCookie) = -1) and (LResponse.Cookies.Count > 0) then
    for i := 0 to LResponse.Cookies.Count - 1 do
      FResponse.Headers.Add(sSetCookie + ': ' + LResponse.Cookies[i].GetServerCookie);
  FResponse.SetContent(AContent);
  FResponse.SetRawBytes(AResponseStream);
end;

procedure TCustomRESTRequest.HandleEvent(AEventHandler: TMethod);
begin
  TCustomRESTClient.HandleEvent(SynchronizedEvents, AEventHandler);
end;

procedure TCustomRESTRequest.DoReadTimeout(Reader: TReader);
begin
  Timeout := Reader.ReadInteger;
end;

procedure TCustomRESTRequest.DefineProperties(AFiler: TFiler);
begin
  inherited DefineProperties(AFiler);
  AFiler.DefineProperty('Timeout', DoReadTimeout, nil, False);
end;
...
procedure TCustomRESTRequest.ParameterValuesNeeded;
begin
  if NotifyList <> nil then
    NotifyList.Notify(
      procedure(ANotify: TNotify)
      begin
        ANotify.ParameterValuesNeeded(Self);
      end);
end;
...
begin
  ...
  Resource := '';
  ResourceSuffix := '';
  ConnectTimeout := CRestDefaultTimeout; // Some servers may be slow. Esp if they just recycled and need to start up on their first request
  ReadTimeout := CRestDefaultTimeout;
...
begin
  Result := TAssignedValue.rvAccept in FAssignedValues;
end;

function TCustomRESTRequest.AcceptCharSetIsStored: Boolean;
begin
  Result := TAssignedValue.rvAcceptCharset in FAssignedValues;
end;

function TCustomRESTRequest.AcceptEncodingIsStored: Boolean;
begin
  Result := TAssignedValue.rvAcceptEncoding in FAssignedValues;
end;

function TCustomRESTRequest.AllowCookiesIsStored: Boolean;
begin
  Result := TAssignedValue.rvAcceptEncoding in FAssignedValues;
end;

function TCustomRESTRequest.HandleRedirectsIsStored: Boolean;
begin
  Result := TAssignedValue.rvHandleRedirects in FAssignedValues;
end;

function TCustomRESTRequest.ConnectTimeoutIsStored: Boolean;
begin
  Result := TAssignedValue.rvConnectTimeout in FAssignedValues;
end;

function TCustomRESTRequest.ReadTimeoutIsStored: Boolean;
begin
  Result := TAssignedValue.rvReadTimeout in FAssignedValues;
end;

function TCustomRESTRequest.GetAccept: string;
begin
  if (TAssignedValue.rvAccept in FAssignedValues) or
     (FClient = nil) or FClient.Accept.IsEmpty then
    Result := FAccept
  else
    Result := FClient.Accept;
end;

function TCustomRESTRequest.GetAcceptCharset: string;
begin
  if (TAssignedValue.rvAcceptCharset in FAssignedValues) or
     (FClient = nil) or FClient.AcceptCharset.IsEmpty then
    Result := FAcceptCharset
  else
    Result := FClient.AcceptCharset;
end;

function TCustomRESTRequest.GetAcceptEncoding: string;
begin
  if (TAssignedValue.rvAcceptEncoding in FAssignedValues) or
     (FClient = nil) or FClient.AcceptEncoding.IsEmpty then
    Result := FAcceptEncoding
  else
    Result := FClient.AcceptEncoding;
end;

function TCustomRESTRequest.GetAllowCookies: Boolean;
begin
  if (TAssignedValue.rvAllowCookies in FAssignedValues) or
     (FClient = nil) then
    Result := FAllowCookies
  else
    Result := FClient.AllowCookies;
end;

function TCustomRESTRequest.GetHandleRedirects: Boolean;
begin
  if (TAssignedValue.rvHandleRedirects in FAssignedValues) or
     (FClient = nil) then
...
function TCustomRESTRequest.GetConnectTimeout: Integer;
begin
  if (TAssignedValue.rvConnectTimeout in FAssignedValues) or
     (FClient = nil) then
...
function TCustomRESTRequest.GetReadTimeout: Integer;
begin
  if (TAssignedValue.rvReadTimeout in FAssignedValues) or
     (FClient = nil) then
    Result := FReadTimeout
  else
...
begin
  if not AcceptIsStored or (FAccept <> AValue) then
  begin
    FAccept := AValue;
    Include(FAssignedValues, TAssignedValue.rvAccept);
    PropertyValueChanged;
  end;
end;
...
begin
  if not AcceptCharSetIsStored or (FAcceptCharset <> AValue) then
  begin
    FAcceptCharset := AValue;
    Include(FAssignedValues, TAssignedValue.rvAcceptCharset);
    PropertyValueChanged;
  end;
end;
...
begin
  if not AcceptEncodingIsStored or (FAcceptEncoding <> AValue) then
  begin
    FAcceptEncoding := AValue;
    Include(FAssignedValues, TAssignedValue.rvAcceptEncoding);
    PropertyValueChanged;
  end;
end;

procedure TCustomRESTRequest.SetAllowCookies(const AValue: Boolean);
begin
  if not AllowCookiesIsStored or (FAllowCookies <> AValue) then
  begin
    FAllowCookies := AValue;
    Include(FAssignedValues, TAssignedValue.rvAllowCookies);
    PropertyValueChanged;
  end;
end;
...
begin
  if not HandleRedirectsIsStored or (FHandleRedirects <> AValue) then
  begin
    FHandleRedirects := AValue;
    Include(FAssignedValues, TAssignedValue.rvHandleRedirects);
    PropertyValueChanged;
  end;
end;
...
begin
  ConnectTimeout := AValue;
  ReadTimeout := AValue;
end;

procedure TCustomRESTRequest.SetConnectTimeout(const AValue: Integer);
begin
  if not ConnectTimeoutIsStored or (FConnectTimeout <> AValue) then
  begin
    FConnectTimeout := AValue;
    Include(FAssignedValues, TAssignedValue.rvConnectTimeout);
    PropertyValueChanged;
  end;
end;

procedure TCustomRESTRequest.SetReadTimeout(const AValue: Integer);
begin
  if not ReadTimeoutIsStored or (FReadTimeout <> AValue) then
  begin
    FReadTimeout := AValue;
    Include(FAssignedValues, TAssignedValue.rvReadTimeout);
    PropertyValueChanged;
  end;
end;
...
procedure TCustomRESTRequest.TNotify.ParameterValuesNeeded(Sender: TObject);
begin
  //
end;
...
begin
  Result := False;
  // plain text
  if SameText(ContentType, ctTEXT_HTML) or SameText(ContentType, ctTEXT_PLAIN) then
  begin
  ...
  // Json
  else if SameText(ContentType, ctAPPLICATION_JSON) then
  begin
...
begin
  HandleEvent(SynchronizedEvents, AEventHandler);
end;

class procedure TCustomRESTClient.HandleEvent(ASynchronizedEvents: Boolean; AEventHandler: TMethod);
begin
...
var
  LPrevValidCert: TValidateCertificateEvent;
  LPrevNeedCert: TNeedClientCertificateEvent;
  LPrevAuth: TCredentialsStorage.TCredentialAuthevent;
begin
  if FHttpClient <> nil then
  begin
    LPrevValidCert := OnValidateCertificate;
    LPrevNeedCert := OnNeedClientCertificate;
    LPrevAuth := OnAuthEvent;
    CreateHttpClient;
    OnValidateCertificate := LPrevValidCert;
    OnNeedClientCertificate := LPrevNeedCert;
    OnAuthEvent := LPrevAuth;
  end
  else
    CreateHttpClient;
  BaseURL := '';
  ...
  FAutoCreateParams := True;
  ConnectTimeout := CRestDefaultTimeout;
  ReadTimeout := CRestDefaultTimeout;
  FParams.Clear;
  FTransientParams.Clear;
end;
...
function TCustomRESTClient.GetSecureFailureReasons: THTTPSecureFailureReasons;
begin
  Result := HTTPClient.SecureFailureReasons;
end;
...
function TCustomRESTClient.GetConnectTimeout: Integer;
begin
  Result := HTTPClient.ConnectTimeout;
end;

function TCustomRESTClient.GetReadTimeout: Integer;
begin
  Result := HTTPClient.ReadTimeout;
end;

procedure TCustomRESTClient.SetConnectTimeout(const AValue: Integer);
begin
  HTTPClient.ConnectTimeout := AValue;
end;

procedure TCustomRESTClient.SetReadTimeout(const AValue: Integer);
begin
  HTTPClient.ReadTimeout := AValue;
end;
...
begin
  ...
  begin
    ...
    CreateReadOnlyField<TCookies>(sCookies, LGetMemberObject, TScopeMemberType.mtObject,
      function: TCookies
      begin
...
initialization
  FRequestPages := TDictionary<TCustomRESTRequest, Integer>.Create;

finalization
  FreeAndNil(FRequestPages);
