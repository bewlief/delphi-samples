...
type
  ...
  private
    ...
    procedure MarshalVarValue(Value: TValue; fieldRTTI: TRttiField);
  protected
...
begin
  ...
  LType := LRTTICtx.GetType(AObject.ClassType);
...
function TJSONConverter.ConvertFieldNameToJson(const AField: TRttiField): string;
var
...
begin
  ...
  begin
    ...
    if Result.StartsWith('F', True) then
...
begin
  ...
  if rttiField <> nil then
    try
      ...
      if (Result = nil) and (rttiField.FieldType <> nil) then
      begin
        // TDateTime
        if rttiField.FieldType.IsPublicType and (
             (rttiField.FieldType.QualifiedName = 'System.TDateTime') or // Do not localize
             (JSONSerializationVersion >= 34) and
               (rttiField.FieldType.QualifiedName = 'System.TDate')) then // Do not localize
        begin
          case DateFormat of
        ...
        // TBytes and similar
        if (rttiField.FieldType is TRttiDynamicArrayType) and
           (TRttiDynamicArrayType(rttiField.FieldType).ElementType.Handle = TypeInfo(Byte)) then
          case BytesFormat of
          jbfArray: ;
...
begin
  ...
  rttiType := FRTTICtx.GetType(AObj.ClassType);
  for attr in rttiType.GetAttributes do
...
procedure TTypeMarshaller<TSerial>.MarshalVarValue(Value: TValue; fieldRTTI: TRttiField);
begin
  ...
  case rttiField.FieldType.TypeKind of
    ...
    TTypeKind.tkEnumeration:
      begin
        ...
        if string.CompareText('Boolean', rttiField.FieldType.Name) = 0 then
    ...
    TTypeKind.tkSet, TTypeKind.tkMethod, TTypeKind.tkInterface, TTypeKind.tkPointer,
...
begin
  ...
  if Field <> nil then
    ...
    try
      ...
      if (Result = nil) and (Field.FieldType <> nil) then
      begin
        // TDateTime
        if (Field.FieldType.IsPublicType) and (
             (Field.FieldType.QualifiedName = 'System.TDateTime') or // Do not localize
             (JSONSerializationVersion >= 34) and
               (Field.FieldType.QualifiedName = 'System.TDate')) then // Do not localize
        begin
          case DateFormat of
            jdfISO8601: Result := TISODateTimeInterceptor.Create(DateTimeIsUTC);
            jdfUnix: Result := TUnixDateTimeInterceptor.Create(DateTimeIsUTC);
        ...
        // TBytes and similar
        if (Field.FieldType is TRttiDynamicArrayType) and
           (TRttiDynamicArrayType(Field.FieldType).ElementType.Handle = TypeInfo(Byte)) then
          case BytesFormat of
          jbfArray: ;
          jbfBase64: Result := TBase64BytesInterceptor.Create;
          end;
...
begin
  ...
  begin
    ...
    Result := 'F' + AFieldName;
    LRTTIField := LRttiType.GetField(Result);
    ...
    begin
      for LAttribute in LRTTIField.GetAttributes do
      begin
        if (LAttribute is JsonNameAttribute) or (LAttribute is JSONMarshalledAttribute) and (not JSONMarshalledAttribute(LAttribute).Value) then
        begin
...
begin
  ...
  begin
    ...
    raise EConversionError.Create(Format(SNoConversionAvailableForValue, [JsonValue.ClassName, 'Variant']));
...
procedure TJSONUnMarshal.SetField(Data: TObject; Field, Value: string; IsNum: Boolean);
var
  rField: TRttiField;
