...
begin
  ...
  AClient.Params.AddHeader(THeaderNames.ApiVersion, FConnectionInfo.ApiVersion);
  ...
  AClient.ConnectTimeout := FConnectionInfo.ConnectTimeout;
  AClient.ReadTimeout := FConnectionInfo.ReadTimeout;
end;
...
begin
  ...
  FRequest.Execute;
  CheckForResponseError;
  if (AJSONArray <> nil) and (FRequest.Response.JSONValue <> nil) then
  begin
...
{ TEMSClientAPI.TConnectionInfo }

class operator TEMSClientAPI.TConnectionInfo.Initialize(out Dest: TConnectionInfo);
begin
  Dest.ProxyPort := 0;
  Dest.ConnectTimeout := CRestDefaultTimeout;
  Dest.ReadTimeout := CRestDefaultTimeout;
  Dest.OnValidateCertificate := nil;
end;

constructor TEMSClientAPI.TConnectionInfo.Create(const AApiVersion,
