unit Xml.Internal.XmlRulesUtils;
...
//
// The Initial Developer of the Original Code is Dieter Köèler (Heidelberg,
// Germany, "http://www.philo.de/"). Portions created by the Initial Developer
// are Copyright (C) 2003-2009 Dieter Köèler. All Rights Reserved.
//
...
{$IFDEF NEXTGEN}
function GetXmlWhitespaceWideString: string;
{$ELSE !NEXTGEN}
function GetXmlWhitespaceWideString: WideString;
{$ENDIF !NEXTGEN}

{$IFDEF NEXTGEN}
function XmlPredefinedEntityToStr(const S: string): string;
{$ELSE !NEXTGEN}
function XmlPredefinedEntityToStr(const S: WideString): WideString;
{$ENDIF NEXTGEN}
...
{$IFDEF NEXTGEN}
function IsXmlAttValue(const S: string): Boolean;
function IsXmlCData(const S: string): Boolean;
function IsXmlCharData(const S: string): Boolean;
function IsXmlCharRef(const S: string): Boolean;
function IsXmlChars(const S: string): Boolean;
function IsXmlComment(const S: string): Boolean;
function IsXmlEncName(const S: string): Boolean;
function IsXmlEntityRef(const S: string): Boolean;
function IsXmlEntityValue(const S: string): Boolean;
function IsXmlEntityValueChars(const S: string): Boolean;
function IsXmlName(const S: string): Boolean;
function IsXmlNames(const S: string): Boolean;
function IsXmlNmtoken(const S: string): Boolean;
function IsXmlNmtokens(const S: string): Boolean;
function IsXmlPEReference(const S: string): Boolean;
function IsXmlPITarget(const S: string): Boolean;
function IsXmlPredefinedEntityName(const S: string): Boolean;
function IsXmlPubidChars(const S: string): Boolean;
function IsXmlPubidLiteral(const S: string): Boolean;
function IsXmlReference(const S: string): Boolean;
function IsXmlS(const S: string): Boolean;
function IsXmlStringType(const S: string): Boolean;
function IsXmlSystemChars(const S: string): Boolean;
function IsXmlSystemLiteral(const S: string): Boolean;
function IsXmlTokenizedType(const S: string): Boolean;
function IsXmlVersionNum(const S: string): Boolean;

function IsXmlDefaultAttName(const S: string): Boolean;
function IsXmlLocalPart(const S: string): Boolean;
function IsXmlNCName(const S: string): Boolean;
function IsXmlNCNameChar(const S: WideChar): Boolean;
function IsXmlNCNameStartChar(const S: WideChar): Boolean;
function IsXmlNSAttName(const S: string): Boolean;
function IsXmlPrefix(const S: string): Boolean;
function IsXmlPrefixedAttName(const S: string): Boolean;
function IsXmlQName(const S: string): Boolean;
{$ELSE !NEXTGEN}
...
begin
  Result := True;
  if S.Length > 3 then
    if (S[FirstIndex+0] = '&') and (S[FirstIndex+High(S)] = ';') then
      if (S[FirstIndex+1] = '#') and (S[FirstIndex+2] = 'x') then
        for I := 3 to High(S)-1 do
        begin
          C := S[FirstIndex+I];
          if not ((C >= '0') and (C <= '9'))
              and not ((C >= 'a') and (C <= 'f'))
        ...
        if (S[FirstIndex+1] = 'x') then
          for I := 2 to High(S)-1 do
          begin
            C := S[FirstIndex+I];
...
begin
  if S.StartsWith('&') and S.EndsWith(';') then
    Result := IsXmlName(S.Substring(1, S.Length - 2))
  else
    Result := False;
end;
{$ELSE !NEXTGEN}
...
begin
  ...
  if (S[Length(S)-AdjustIndex] = SQ) and (S[FirstIndex] = SQ) {single quotes} then
    ForbittenQuote := Ord(SQ)
  else if (S[Length(S)-AdjustIndex] = DQ) and (S[FirstIndex] = DQ) {double quotes} then
...
begin
  ...
  if (S[Length(S)-AdjustIndex] = SQ) and (S[FirstIndex] = SQ) then
  begin // single quotes
  ...
  else if (S[Length(S)-AdjustIndex] = DQ) and (S[FirstIndex] = DQ) then
  begin // double quotes
    for I := 2 to Length(S) - 1 do
    begin
      SChar := S[I-AdjustIndex];
...
begin
  Result := IsXmlName(S);
  if Length(S) = 3 then
    if ((S[FirstIndex  ] = 'X') or (S[FirstIndex  ] = 'x')) and
       ((S[FirstIndex+1] = 'M') or (S[FirstIndex+1] = 'm')) and
       ((S[FirstIndex+2] = 'L') or (S[FirstIndex+2] = 'l')) then
      Result := False;
end;
...
begin
  ...
  if (S[FirstIndex] <> '1') or (S[FirstIndex+1] = ',') then
  begin
    Result := False;
    Exit;
  end;
  for I := FirstIndex+2 to Length(S)-AdjustIndex do
  begin
...
begin
  ...
  if not IsXmlEncNameLeadingCharCodePoint(ord(S[FirstIndex])) then
  begin
    Result := False;
    Exit;
  end;
  for I := FirstIndex+1 to Length(S)-AdjustIndex do
  begin
...
begin
  ...
  for I := 1+FirstIndex to Length(S)-AdjustIndex do
