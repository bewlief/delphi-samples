...
unit Xml.Internal.OTextReadWrite;

interface

uses
  System.SysUtils, System.Classes, Xml.Internal.OBufferedStreams;
...
type
  ...
  private
    ...
    fStreamStartPosition: NativeInt;
    fOwnsStream: Boolean;
    ...
    function GetApproxStreamPosition: NativeInt;
  ...
  public
    ...
    procedure InitString(const aString: string);
  ...
  public
    ...
    function ReadNextChar(var outChar: Char): Boolean;
    //read text
    function ReadString(const aMaxChars: Integer; const aBreakAtNewLine: Boolean = False): string;
    ...
    //     e.g. for errors etc.
    function ReadPreviousString(const aMaxChars: Integer; const aBreakAtNewLine: Boolean = False): string;
  ...
  public
    ...
    property EOFi: Boolean read fEOFi;
    ...
    property ApproxStreamPosition: NativeInt read GetApproxStreamPosition;
    ...
    property StreamSize: NativeInt read fStreamSize;
  end;
  ...
  private
    ...
    procedure WriteStringToStream(const aString: string; const aMaxLength: Integer);
    procedure SetEncoding(const Value: TEncoding);
  protected
...
function GetEncodingFromStream(const aStream: TStream;
  var {%H-}ioTempStringPosition: NativeInt;
  const aLastPosition: NativeInt;
...
function GetEncodingFromStream(const aStream: TStream;
  var ioTempStringPosition: NativeInt;
  const aLastPosition: NativeInt;
...
function TOTextReader.GetApproxStreamPosition: NativeInt;
begin
...
procedure TOTextReader.InitString(const aString: string);
var
  xLength: Integer;
  xNewStream: TStream;
begin
  ...
  if xLength > 0 then
    xNewStream.WriteBuffer(aString[1], xLength * SizeOf(Char));
  xNewStream.Position := 0;
...
function TOTextReader.ReadNextChar(var outChar: Char): Boolean;
begin
...
function TOTextReader.ReadPreviousString(const aMaxChars: Integer;
  const aBreakAtNewLine: Boolean): string;
var
  xReadChars: Integer;
  I: Integer;
begin
...
procedure TOTextWriter.WriteChar(const aChar: Char);
begin
...
procedure TOTextWriter.WriteString(const aString: string);
var
...
begin
  ...
  if xStringLength > fTempStringLength then begin
    ...
    Move(aString[1], fTempString[fTempStringPosition], xStringLength*SizeOf(Char));
    fTempStringPosition := fTempStringPosition + xStringLength;
  end;
end;

procedure TOTextWriter.WriteStringToStream(const aString: string; const aMaxLength: Integer);
var
  xBytes: TBytes;
