...
begin
  ...
  begin//check on prefixed attribute
    if NativeElement.HasNamespaceURI(ExtractPrefix(name)) then
      Result := NativeElement.hasAttributeNS(NativeElement.LookupNamespaceURI(ExtractPrefix(name)), ExtractLocalName(name))
    else
      Result := False;
  end else
...
begin
  if IsPrefixed(name) and NativeElement.HasNamespaceURI(ExtractPrefix(name)) then
    NativeElement.setAttributeNS(NativeElement.LookupNamespaceURI(ExtractPrefix(name)), name, value)
  else
...
begin
  ...
  try
    if Self.QueryInterface(IDOMXMLProlog, PrologInfo) = S_OK then
      parser.IncludeXmlDecl := not ((Trim(PrologInfo.Version) = '') and (Trim(PrologInfo.Encoding) = ''))
    else
      parser.IncludeXmlDecl := False;

    parser.UseByteOrderMark := [];
    parser.IncludeXmlEnc := False;
