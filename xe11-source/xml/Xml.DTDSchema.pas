...
uses System.SysUtils, System.Classes, Xml.XMLSchema, Xml.xmldom;
...
begin
  ...
  begin
    ...
    while (Length(Str) > 0) and (Str[FirstIndex] = ' ') do
      Str := Tail(Str, ' '); //remove leading spaces
    while (Length(Str) > 0) and (Str[Length(Str)-AdjustIndex]=' ') do
      SetLength(Str, Length(Str) - 1);
    while (Length(Str) > 0) and (Str[Length(Str)-AdjustIndex] = #0) do
      SetLength(Str, Length(Str) - 1);
    Result := Str;
  end
  else
...
begin
  Str := Trim(Str);
  Ch := ' ';
  for I := FirstIndex to Length(Str)-AdjustIndex do
  begin
    //if CharInSet(Str[I], ['(','#',' ']) then
    if Ord(Str[I]) in [Ord('('),Ord('#'),Ord(' ')] then
    begin
      Ch := Str[I];
      Break;
    end
...
procedure TDTDImportTranslator.Translate(const FileName: DOMString;
  ...
  begin
    ...
    begin
      ...
      begin  //Terminating
        ...
        if Length(Str)> 0 then
        begin
           Ch := Str[FirstIndex]; //Should ignore whitespaces
           //if CharInSet(Ch, ['?','*','+']) then
           if Ord(Ch) in [Ord('?'),Ord('*'),Ord('+')] then
           begin
              Suffix := Ch;
              Str := Tail(Str, Ch);
           end;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        //if CharInSet(AttrToken[1], ['"', '''']) then
        if Ord(AttrToken[FirstIndex]) in [Ord('"'), Ord('''')] then
        begin
          CurToken := CurToken+1;
          AttributeDefault := UnQuote(AttrToken, AttrToken[FirstIndex]);
        end;
      end;
...
procedure TDTDImportTranslator.ParseChildElements(ComplexDef: IXMLComplexTypeDef;
  ...
  begin
    ...
    Ch := Name[Length(Name)-AdjustIndex];
    //if CharInSet(Ch, ['?','*','+','!', ')']) then
    if Ord(Ch) in [Ord('?'),Ord('*'),Ord('+'),Ord('!'), Ord(')')] then
    begin
  ...
  function IncStrProp(const Val: string): string;
  begin
    //if CharInSet(Val[1], ['0'..'9']) then
    if Ord(Val[FirstIndex]) in [Ord('0')..Ord('9')] then
      Result := IntToStr(StrToInt(Val) + 1)
    else
      Result := Val;
  end;
  ...
  begin
    ...
    begin
      ...
      begin  //Terminating
        ...
        begin ///
          ElementDef := ElementDefs.Find(Name);
          if not Assigned(ElementDef) then
          begin
            ElementDef := ElementDefs.Add(Name);
            CountChildNames := CountChildNames + 1;
            if (Ch = ')') and (CountChildNames = 1) and
              //not (CharInSet(NameSuffix, ['?','*','+'])) then
              not (Ord(NameSuffix) in [Ord('?'),Ord('*'),Ord('+')]) then
            begin  // (x)*
              if Length(Str) > 0 then
              begin
                NameSuffix := Str[FirstIndex];
                Str := Tail(Str, NameSuffix);
              end
              else NameSuffix := ' ';
            end;
            //if (CharInSet(NameSuffix, ['?','*','+'])) then
            if (Ord(NameSuffix) in [Ord('?'),Ord('*'),Ord('+')]) then
            begin
          ...
          begin
            // Duplicate entries i.e. (StreetAddress, StreetAddress?, City ...
            if NameSuffix = '' then
            begin
              ElementDef.MinOccurs := IncStrProp(ElementDef.MinOccurs);
              ElementDef.MaxOccurs := IncStrProp(ElementDef.MaxOccurs);
            end
            else if (NameSuffix = '*') or (NameSuffix = '+') then
            begin
              ElementDef.MinOccurs := IncStrProp(ElementDef.MinOccurs);
              ElementDef.MaxOccurs := 'unbounded';
            end
            else if (NameSuffix = '?') then
              ElementDef.MaxOccurs := IncStrProp(ElementDef.MaxOccurs);
          end;
        end; ///
        if Ch = ')' then
...
initialization
  TranslatorFactory := TXMLSchemaTranslatorFactory.Create(TDTDImportTranslator,
    nil, DTDExtension, SDTDTransDesc);
