...
function TODOMNode.appendChild(const newChild: IDOMNode): IDOMNode;
  ...
  begin
    ...
    if FNativeNode.HasChildNodes then
      for i := 0 to NativeNode.ChildNodes.Length - 1 do
        if NativeNode.ChildNodes.Item[i].NodeType = 1 then
        begin
          Result := True;
          Break;
        end;
...
begin
  // when NativeNode.ParentNode = nil it means that build the root node
  // when _hasRootElement = True it means that we have the root node
  // when newChild.nodeType = 1 it means that we want to add a new root node
  if (NativeNode.ParentNode = nil) and HasRootElement and (newChild.nodeType = 1) {ntElement} then
