...
uses
  ...
  Xml.Internal.XmlRulesUtils, System.Generics.Collections;
...
type
  ...
  public
    ...
    property Encoding: TEncoding read GetEncoding write SetEncoding;
    property BOMFound: Boolean read GetBOMFound;
    property IncludeXmlEnc: Boolean read FIncludeXmlEnc;
    constructor Create(const Stream: TStream; const Mode: TStreamMode; const Encoding: TEncoding; const WriteBOM: Boolean; const IncludeXmlEnc: Boolean);
...
uses
  Xml.Internal.OmniXMLXPath, System.Character, Xml.Internal.xmlInputSource;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if SameText(Encoding, 'UTF-16') then
            OutEncoding := TEncoding.Unicode
          else
...
constructor OmniTXMLTextStream.Create(const Stream: TStream; const Mode: TStreamMode;
  const Encoding: TEncoding; const WriteBOM: Boolean; const IncludeXmlEnc: Boolean);
begin
  FStreamMode := Mode;
...
begin
  ...
  try
    US := OmniTXMLTextStream.Create(Stream, smWrite, TEncoding.Unicode, False, True);
...
procedure OmniTXMLProcessingInstruction.InternalWriteToStream(const OutputStream: IUnicodeStream);
  ...
  function deleteEncoding(const aData: string): string;
  var
    LEncodingStart,
    LEncodingEnd: Integer;
  begin
    Result := aData;
    LEncodingStart := Pos('encoding', aData);

    if LEncodingStart > 0 then
    begin
      LEncodingEnd := System.Pos(' ', aData, LEncodingStart);

      if LEncodingEnd = 0 then
      begin
        LEncodingEnd := aData.Length;
        if (LEncodingStart > 1) and (aData[LEncodingStart - 1] = ' ') then
          Dec(LEncodingStart);
      end;

      Delete(Result, LEncodingStart, LEncodingEnd - LEncodingStart + 1);
    end
  end;
...
begin
  ...
  if (OutputStream is OmniTXMLTextStream) and
     (OutputStream as OmniTXMLTextStream).IncludeXmlEnc and
     not FData.ToLower.Contains('"utf-16"')
  then
    OutputStream.WriteString('<?' + NodeName + ' ' + deleteEncoding(FData) + '?>')
  else
...
var
  LStrEncoding,
  LVersion: String;
  LStandalone: TDomStandalone;
  LEncoding: TEncoding;
begin
  Result := True;
  ClearChildNodes;

  TXmlInputSource.GetXMLProlog(Stream, LStrEncoding, LStandalone, LVersion);

  if not LStrEncoding.IsEmpty then
    LEncoding := TEncoding.GetEncoding(LStrEncoding)
  else
    LEncoding := TEncoding.UTF8;

  Stream.Position := 0;
  XTS := OmniTXMLTextStream.Create(Stream, smRead, LEncoding, False, False);
  try
    try
    ...
    if (LEncoding <> XTS.Encoding) and not LStrEncoding.IsEmpty then
      LEncoding.Free;
    FreeAndNil(XTS);
    FURL := '';
  end;
...
begin
  US := OmniTXMLTextStream.Create(OutputStream, smWrite, InternalFindEncoding, True, False);
  try
      ...
      psTagName:  // one-time stop
        ...
        begin
          case ReadChar of
            ...
            begin
              // it's an element
              if(Parent.ParentNode = nil) and (GetDocumentElement <> nil) then
                // when Parent.ParentNode = nil it means that build the root node
                // when GetDocumentElement <> nil it means that we have the root node
                raise EXMLException.CreateParseError(WRONG_DOCUMENT_ERR, MSG_E_MULTIPLEROOTS, [])
              else
              begin
                InputStream.WriteOutputChar(ReadChar);
