...
uses
  System.SysUtils, System.Classes, Xml.xmldom;
...
function XmlStrToInt(const Value: string): Integer; inline;
function XmlIntToStr(const Value: Integer): string; inline;
function XmlStrToInt64(const Value: string): Int64; inline;
function XmlInt64ToStr(const Value: Int64): string; inline;
function XmlStrToBool(const Value: string): Boolean;
function XmlBoolToStr(const Value: Boolean): string; inline;
function XmlStrToDateTime(const Value: string): TDateTime;
function XmlDateTimeToStr2(const Value: TDateTime): string;
...
const
  ...
  XSDBoolStrs: array[Boolean] of string = ('false', 'true');
  XSDBoolIntStrs: array[Boolean] of string = ('0', '1');

implementation

uses Xml.XMLConst, System.Math, System.DateUtils, System.Variants;
...
try
  if Format <> '' then
  begin
    Store := FormatSettings.ShortDateFormat;
  ...
  if Format <> '' then
  begin
    FormatSettings.ShortDateFormat := Store;
  end;
end;
end;
...
begin
  try
    if (Format <> '') then
    begin
      Store := FormatSettings.LongTimeFormat;
      FormatSettings.LongTimeFormat := Format;
    end;
    try
    ...
    if Format <> '' then
    begin
      FormatSettings.LongTimeFormat := Store;
    end;
  end;
end;
...
function XmlFloatToStr(const Value: Extended): string;
begin
  Result := FloatToStr(Value, XmlFormatSettings);
end;

function XmlStrToFloat(const Value: string): Extended;
begin
  Result := StrToFloat(Value, XmlFormatSettings);
end;
...
begin
  ...
  begin
    ...
    Result := XmlFloatToStr(Value);
end;

function XmlStrToInt(const Value: string): Integer;
begin
  Result := StrToIntDef(Value, 0);
end;

function XmlIntToStr(const Value: Integer): string;
begin
  Result := IntToStr(Value);
end;

function XmlStrToInt64(const Value: string): Int64;
begin
  Result := StrToInt64Def(Value, 0);
end;

function XmlInt64ToStr(const Value: Int64): string;
begin
  Result := IntToStr(Value);
end;
...
begin
  if Trim(Value) = '' then
    Result := False
  else if (Value = XSDBoolStrs[True]) or (Value = XSDBoolIntStrs[True]) then
    Result := True
  else if (Value = XSDBoolStrs[False]) or (Value = XSDBoolIntStrs[False]) then
    Result := False
  else
  begin
    var V: Variant := Value;
...
function XmlBoolToStr(const Value: Boolean): string;
begin
  Result := XSDBoolStrs[Value];
end;
...
begin
  if Trim(Value) = '' then
    Result := 0.0
  else if not TryISO8601ToDate(Value, Result, True) then
    Result := VarToDateTime(Value);
end;

function XmlDateTimeToStr2(const Value: TDateTime): string;
begin
  Result := DateToISO8601(Value, True);
end;

initialization
  XmlFormatSettings := TFormatSettings.Invariant;
