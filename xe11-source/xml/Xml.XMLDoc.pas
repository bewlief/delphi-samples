...
uses
  System.Classes, Xml.xmldom, Xml.XMLIntf;
...
(*$HPPEMIT '};'                               *)
(*$HPPEMIT '};'                               *)
(*$HPPEMIT 'using Xml::Xmldoc::TNodeClassInfo;'    *)
...
{ TXMLNode }
  ...
  private
    ...
    [Weak] FDocument: TXMLDocument;
    FDOMNode: IDOMNode;
    FHostNode: TXMLNode;
    [Weak] FParentNode: TXMLNode;
  ...
  protected
    ...
    function NodeValueToText(const Value: OleVariant): string;
    { Data member access }
...
{$IFNDEF NEXTGEN}
    procedure SaveToUTF8String(var XML: UTF8String);
{$ENDIF !NEXTGEN}
...
function XMLStringToWideString(const XMLString: AnsiString): WideString; deprecated;
{$ENDIF !NEXTGEN}
...
uses
  System.SyncObjs, System.Variants, System.SysUtils,
  Xml.XMLSchema, Xml.XMLConst, Xml.XMLUtil;

const
  AdjustIndex= 1-Low(string);

const
  DefaultNodeIndent: DOMString = '  '; { 2 spaces }
...
begin
  case SourceNode.nodeType of
    ntElement:
      begin
        Result := TargetDoc.CreateElement(SourceNode.NodeName, SourceNode.NamespaceURI);
        for I := 0 to SourceNode.AttributeNodes.Count - 1 do
          Result.AttributeNodes.Add(CloneNodeToDoc(SourceNode.AttributeNodes[I], TargetDoc, False));
        if Deep then
          for I := 0 to SourceNode.ChildNodes.Count - 1 do
            Result.ChildNodes.Add(CloneNodeToDoc(SourceNode.ChildNodes[I], TargetDoc, Deep));
      end;
    ntAttribute:
      begin
        Result := TargetDoc.CreateNode(SourceNode.NodeName, ntAttribute, SourceNode.NamespaceURI);
        Result.NodeValue := SourceNode.NodeValue;
      end;
    ntText, ntCData, ntComment:
        Result := TargetDoc.CreateNode(SourceNode.Text, SourceNode.NodeType);
    ntEntityRef:
        Result := TargetDoc.createNode(SourceNode.nodeName, SourceNode.NodeType);
    ntProcessingInstr:
        Result := TargetDoc.CreateNode(SourceNode.NodeName, ntProcessingInstr, SourceNode.NodeValue);
    ntDocFragment:
      begin
...
//    QuoteChars = ['''', '"'];
  begin
    //Result := AnsiChar(C) in QuoteChars;
    Result := (C = '''') or (C = '"');
  end;
...
begin
  ItemPos := AttrLine.IndexOf(AttrName) + 1;
  LineLen := AttrLine.Length;
  if ItemPos > 0 then
  begin
    Inc(ItemPos, AttrName.Length);
    while (ItemPos < LineLen) and not IsQuoteChar(AttrLine.Chars[ItemPos-1]) do
      Inc(ItemPos);
    if ItemPos < LineLen then
    begin
      ItemEnd := ItemPos + 1;
      while (ItemEnd < LineLen) and not IsQuoteChar(AttrLine.Chars[ItemEnd-1]) do
        Inc(ItemEnd);
      Result := AttrLine.Substring(ItemPos, ItemEnd-ItemPos-1);
    end;
  end
  else
    Result := Default;
...
function EncodingMatches(const Encoding: string; const EncodingList: array of string): Boolean;
var
  I: Integer;
begin
...
function XMLStringToWideString(const XMLString: AnsiString): WideString; deprecated;
begin
  Result := XMLStringToUnicodeString(XMLString);
end;
{$ENDIF !NEXTGEN}
...
function SplitString(const S: DOMString; Delimiter: WideChar;
   const StringSplitOptions: TStringSplitOptions = []): TDOMStringDynArray;
var
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        {Set the string}
        SetString(Result[LResultCount], PWideChar(@S[LSplitStartPos-AdjustIndex]),
...
procedure TXMLNodeList.Insert(Index: Integer; const Node: IXMLNode);
  ...
  begin
    ...
    if Break then
      IndentStr := #10 + IndentStr;
    IndentNode := TXMLNode.Create(CreateDOMNode(Owner.OwnerDocument.DOMDocument,
      IndentStr, ntText), nil, Owner.OwnerDocument);
...
function TXMLNode.NodeValueToText(const Value: OleVariant): string;
begin
  case VarType(Value) of
    varBoolean:
      // The Variant conversion of boolean to strings is not W3C XML Schema
      // boolean type compatible, so we have to manually override it here.
      if UseXSDBooleanStrings then
        Result := XmlBoolToStr(Boolean(Value))
      else
        Result := VarToStr(Value);
    varDouble, varSingle, varCurrency:
      Result := XmlFloatToStrExt(Value);
    else
      Result := VarToStr(Value);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      AttrNode.nodeValue := NodeValueToText(Value);
  ...
  begin
    if ChangeType = ncAddAttribute then
      DOMElement.setAttribute(AttrName, NodeValueToText(Value))
    else
...
{ This method uses an explicit namespaceURI. }
  ...
  DoNodeChange(ChangeType, True);
  if ChangeType = ncAddAttribute then
    DOMElement.setAttributeNS(NamespaceURI, NodeName, NodeValueToText(Value))
  else
...
var
  NodeName, NamespaceURI: DOMString;
begin
...
{ This method uses an explicit namespaceURI. }
  ...
  if (NamespaceURI <> '') and Assigned(OwnerDocument) and
     (doNamespaceDecl in OwnerDocument.Options) and not Assigned(NSDecl) then
...
begin
  ...
  if ItemNS <> '' then
    Result.ItemTag := GetPrefixedName(ItemTag, ItemNS)
  else
  begin
    if IsPrefixed(ItemTag) then
      ItemNS := FindNamespaceURI(ExtractPrefix(ItemTag));
    ...
    case Operation of
      ...
      nlInsert:
        begin
          ...
          if Cardinal(IndexOrName) >= Cardinal(DOMNode.ChildNodes.length) then
  ...
  begin
    ...
    begin
      ...
      for I := 0 to Length(ChildNodeClasses) - 1 do
        if NodeMatches(ADOMNode, ChildNodeClasses[I].NodeName, ChildNodeClasses[I].NamespaceURI) then
        begin
          ChildNodeClass := ChildNodeClasses[I].NodeClass;
          Break;
        end;
    end;
...
var
  ...
  NSAttrName: string;
begin
  NSDecl := FindNamespaceDecl(URI);
  if Assigned(NSDecl) and (NSDecl.Prefix <> Prefix) then
    NSDecl := nil;
  if not Assigned(NSDecl) then
  begin
    NSAttrName := SXMLNS;
    if Prefix <> '' then
      NSAttrName := NSAttrName+NSDelim+Prefix;
    SetAttributeNS(NSAttrName, SXMLNamespaceURI, URI);
  end;
end;
...
begin
  if VarType(IndexOrName) = varInteger then
    Node := ChildNodes.Get(IndexOrName)
  else
...
begin
  ...
  FListInst := TXMLNodeList.Create(Self, FItemNS, nil);
  FList := FListInst;
  for I := 0 to ChildNodes.Count - 1 do
    if IsCollectionItem(ChildNodes[I]) then
    begin
      (ChildNodes[I] as IXMLNodeAccess).SetCollection(Self);
      FListInst.InternalInsert(-1, ChildNodes[I]);
    end;
end;
...
begin
  ...
  begin
    if isPrefixed(Node.NodeName) or isPrefixed(FItemTag) then
    begin
      LocalName := Node.LocalName;
      Result := SameNamespace(Node.NamespaceURI, FItemNS) and (LocalName = ExtractLocalName(FItemTag));
    end
    else
    begin
    ...
    begin
      ...
      for I := Low(FItemTags) to High(FItemTags) do
        if LocalName = ExtractLocalName(FItemTags[I]) then
        begin
          Result := True;
          Break;
        end;
...
begin
  ...
  if (FindNamespaceDecl(ItemNS) <> nil) or IsPrefixed(ItemTag) then
    Result := AddChild(ItemTag, ItemNS, True, Index)
  else
...
begin
  ...
  begin
    Status := DOMPersist.loadxml(FXMLStrings.Text);
  ...
  begin
    ...
    if Word(FXMLData[Low(string)]) = $FEFF then
...
{$IFNDEF NEXTGEN}
procedure TXMLDocument.LoadFromXML(const XML: AnsiString);
var
...
begin
  ...
  begin
    ...
    try
      ...
      if DetectCharEncoding(MemoryStream) in [xetUTF_8, xetUTF_8Like, xetUnknown] then
      begin
        SetLength(XML, MemoryStream.Size);
        MemoryStream.Position := 0;
        MemoryStream.Read(PByte(XML)^, MemoryStream.Size);
      end
      else
...
{$ELSE NEXTGEN}
  XML := StringOf(TEncoding.UTF8.GetBytes(UnicodeXML));
{$ENDIF !NEXTGEN}
  ...
  if Copy(XML, 1, 5) = '<?xml' then
    System.Delete(XML, 1, Pos(AnsiString('>'), XML))
  else
    PrologNodeStr := PrologNodeStr + SLineBreak;
  XML := UTF8String(PrologNodeStr) + XML;
end;
{$ENDIF !NEXTGEN}
...
{$ELSE}
            Result := VarToStr(Node.AttributeNodes[J].NodeValue);
{$ENDIF !NEXTGEN}
            Break;
          end;
        Break;
  ...
  begin
    ...
    begin
      ...
      begin
        if SchemaLoc[HintStart-AdjustIndex] <= ' ' then
        begin
