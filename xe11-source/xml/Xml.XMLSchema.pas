...
uses System.SysUtils, System.Variants, System.Classes, Xml.xmldom, Xml.XMLDoc, Xml.XMLIntf;
...
{ IXMLSchemaDocRef }
  ...
  IXMLSchemaDocRef = interface(IXMLAnnotatedItem)
    ...
    function GetSchemaRefType: TSchemaRefType;
    procedure SetSchemaLocation(const Value: DOMString);
    procedure SetSchemaRefType(const Value: TSchemaRefType);
    { Public Properties }
    ...
    property SchemaRefType: TSchemaRefType read GetSchemaRefType write SetSchemaRefType;
  end;
...
{ IXMLSimpleTypeContent }

  IXMLSimpleContent = interface(IXMLSchemaNode)
    ['{D355AA00-A9C9-4945-B97D-219A6E7E026F}']
  end;

{ IXMLSimpleTypeRestriction }

  IXMLSimpleTypeRestriction = interface(IXMLSimpleContent)
...
{ IXMLSimpleTypeList }
  ...
  IXMLSimpleTypeList = interface(IXMLSimpleContent)
    ['{0E61C20B-9773-4716-AF81-1833000F15BC}']
    { Property Accessors }
    function GetItemType: DOMString;
    function GetSimpleTypeNode: IXMLSimpleTypeDef;
    procedure SetItemType(const Value: DOMString);
    { Public Properties }
    property ItemType: DOMString read GetItemType write SetItemType;
    property SimpleTypeNode: IXMLSimpleTypeDef read GetSimpleTypeNode;
  end;

{ IXMLSimpleTypeUnion }

  IXMLSimpleTypeUnion = interface(IXMLSimpleContent)
    ['{EA433957-14EE-4ACA-8B95-65060923EB56}']
    { Property Accessors }
    function GetMemberTypes: DOMString;
    function GetSimpleTypes: IXMLSimpleTypeDefs;
    procedure SetMemberTypes(const Value: DOMString);
    { Public Properties }
    property MemberTypes: DOMString read GetMemberTypes write SetMemberTypes;
...
{ IXMLSimpleTypeDef }
  ...
  IXMLSimpleTypeDef = interface(IXMLTypeDef)
    ...
    function GetFacetParentNode(const Required: Boolean=True): IXMLNode;
    { Property Accessors }
    function GetFinal: DOMString;
    function GetContentNode: IXMLSimpleContent;
    function GetDerivationMethod: TSimpleDerivationMethod;
    function GetRestrictionNode: IXMLSimpleTypeRestriction;
    procedure SetFinal(const Value: DOMString);
    { Public Methods & Properties }
    function IsBuiltInType: Boolean;
    property ContentNode: IXMLSimpleContent read GetContentNode;
    property DerivationMethod: TSimpleDerivationMethod read GetDerivationMethod;
    property Final: DOMString read GetFinal write SetFinal;
    property RestrictionNode: IXMLSimpleTypeRestriction read GetRestrictionNode;
  end;
...
{ IXMLSchemaDef }
  ...
  IXMLSchemaDef = interface(IXMLNOde)
    ...
    function GetParentSchemaDef: IXMLSchemaDef;
    function GetParentRefType: TSchemaRefType;
    function GetQualifyAnonymousTypeNames: Boolean;
    ...
    procedure SetParentSchemaDef(const Value: IXMLSchemaDef);
    procedure SetParentRefType(const Value: TSchemaRefType);
    procedure SetQualifyAnonymousTypeNames(const Value: Boolean);
    procedure SetStrictFormat(const Value: Boolean);
    procedure SetVersion(const Value: DOMString);
    { Public Methods }
    function FindNamespacePrefix(const NamespaceURI: DOMString): DOMString;
    ...
    property ParentSchema: IXMLSchemaDef read GetParentSchemaDef write SetParentSchemaDef;
    property ParentRefType: TSchemaRefType read GetParentRefType write SetParentRefType;
    property QualifyAnonymousTypeNames: Boolean read GetQualifyAnonymousTypeNames write SetQualifyAnonymousTypeNames;
...
{$IFDEF MSWINDOWS}
(*$HPPEMIT 'namespace Xml       {'              *)
(*$HPPEMIT 'namespace Xmlschema {'              *)
(*$HPPEMIT 'using Xml::Xmlintf::IXMLDocument;'  *)
(*$HPPEMIT '};'                                 *)
(*$HPPEMIT '};'                                 *)
{$ENDIF}
...
{ IXMLSchemaTranslatorFactory }
  ...
  IXMLSchemaTranslatorFactory = interface(IInterface)
    ...
    { Public Methods }
    function CanExportFile(const FileName: DOMString): Boolean;
    function CanImportFile(const FileName: DOMString): Boolean;
    function GetExportTranslator: IXMLSchemaTranslator;
    function GetImportTranslator: IXMLSchemaTranslator;
    function GetTranslatorDescription: DOMString;
  end;
...
{ TXMLSimpleTypeContent }
  ...
  TXMLSimpleTypeContent = class(TXMLSchemaNode, IXMLSimpleContent)
  end;

{ TXMLTypeRestriction }

  TXMLSimpleTypeRestriction = class(TXMLSimpleTypeContent, IXMLSimpleTypeRestriction)
  protected
    { IXMLSimpleTypeRestriction }
    function GetBaseName: DOMString;
    procedure SetBaseName(const Value: DOMString);
...
{ TXMLSimpleTypeList }
  ...
  TXMLSimpleTypeList = class(TXMLSimpleTypeContent, IXMLSimpleTypeList)
  protected
    { IXMLSimpleTypeList }
    function GetItemType: DOMString;
    function GetSimpleTypeNode: IXMLSimpleTypeDef;
...
{ TXMLSimpleTypeUnion }
  ...
  TXMLSimpleTypeUnion = class(TXMLSimpleTypeContent, IXMLSimpleTypeUnion)
  private
...
{ TXMLSchemaDef }
  ...
  private
    ...
    FParentSchemaDef: TXMLSchemaDef;
    FParentRefType: TSchemaRefType;
    ...
    FQualifyAnonymousTypeNames: Boolean;
  protected
    ...
    function LoadSchemaRef(const SchemaDocRef: IXMLSchemaDocRef): IXMLSchemaDef;
...
{ TXMLSchemaTranslator }
  ...
  protected
    { IXMLSchemaTranslator }
    procedure Translate(const FileName: DOMString; const SchemaDef: IXMLSchemaDef); virtual;
    { Data member access }
...
{ TXMLSchemaTranslatorFactory }
  ...
  private
    FDescription: DOMString;
    FExtension: DOMString;
    FExportTranslatorClass: TSchemaTranslatorClass;
    FImportTranslatorClass: TSchemaTranslatorClass;
  protected
    { IXMLSchemaTranslatorFactory }
    function CanExportFile(const FileName: DOMString): Boolean; virtual;
    function CanImportFile(const FileName: DOMString): Boolean; virtual;
    function GetExportTranslator: IXMLSchemaTranslator;
    function GetImportTranslator: IXMLSchemaTranslator;
    function GetTranslatorDescription: DOMString;
  public
    constructor Create(ImportClass, ExportClass: TSchemaTranslatorClass;
      const Extension, Description: DOMString);
  end;
...
{$IFDEF MSWINDOWS}
  Winapi.Windows,
{$ENDIF}
  Xml.XMLSchemaTags, Xml.XMLConst;
...
function IsHosted(const Node: IXMLNode): Boolean;
begin
  Result := (Node as IXMLNodeAccess).HostNode <> nil;
end;

function GetDocumentationText(const Documentation: IXMLDocumentation): DOMString;
var
  Child: IXMLNode;
  I: Integer;
begin
...
begin
  Result := VarToStr(GetAttribute(SSchemaLocation));
end;

function TXMLSchemaDocRef.GetSchemaRef: IXMLSchemaDef;
begin
  if not Assigned(FSchemaRef) then
    FSchemaRef := SchemaObject.LoadSchemaRef(Self);
  Result := FSchemaRef;
end;

function TXMLSchemaDocRef.GetSchemaRefType: TSchemaRefType;
begin
  Result := FSchemaRefType;
end;
...
procedure TXMLSchemaDocRef.SetSchemaRefType(const Value: TSchemaRefType);
begin
  FSchemaRefType := Value;
end;

{ TXMLSchemaImport }

procedure TXMLSchemaImport.AfterConstruction;
begin
  inherited;
  SetSchemaRefType(srImport);
end;

{ TXMLSchemaInclude }

procedure TXMLSchemaInclude.AfterConstruction;
begin
  inherited;
  SetSchemaRefType(srInclude);
end;
...
var
  ...
  TypeNode: IXMLNode;
  TypeDef: IXMLComplexTypeDef;
begin
  { For unnamed local types, fabricate a name base on owning elements and types }
  if Assigned(ParentNode) and (ParentNode is TXMLTypedSchemaItem) then
  begin
    Result := (ParentNode as TXMLTypedSchemaItem).GetName;
    if SchemaDef.QualifyAnonymousTypeNames then
    begin
      TypeNode := ParentNode;
      while Assigned(TypeNode) and not Supports(TypeNode, IXMLComplexTypeDef, TypeDef) do
        TypeNode := TypeNode.ParentNode;
      if Assigned(TypeDef) then
        Result := TypeDef.Name + '_' + Result;
      BaseType := GetBaseType;
    end;
    // Avoid name clash when an element has an anonymous type and the
    // element name is the same as the base type of the anonymous type
    if Assigned(BaseType) and BaseType.IsComplex and (Result = BaseType.Name) then
      Result := Result + '_';
  end
  else
  begin
...
var
  node: IXMLNode;
begin
  node := GetFacetParentNode;
  if HasChildNode(FacetName) then
    node.ChildNodes[FacetName].Attributes[SValue] := Value else
    node.AddChild(FacetName).Attributes[SValue] := Value;
end;
...
procedure TXMLSimpleTypeRestriction.AfterConstruction;
begin
  RegisterChildNode(xsfEnumeration, TXMLEnumeration);
  inherited;
end;

function TXMLSimpleTypeRestriction.GetBaseName: DOMString;
begin
  Result := VarToStr(GetAttribute(SBase));
end;

procedure TXMLSimpleTypeRestriction.SetBaseName(const Value: DOMString);
begin
...
procedure TXMLSimpleTypeList.AfterConstruction;
begin
  RegisterChildNode(SSimpleType, TXMLSimpleTypeDef);
  inherited;
end;

function TXMLSimpleTypeList.GetSimpleTypeNode: IXMLSimpleTypeDef;
begin
  Result := ChildNodes.FindNode(IXMLSimpleTypeDef) as IXMLSimpleTypeDef;
end;

function TXMLSimpleTypeList.GetItemType: DOMString;
begin
  Result := VarToStr(GetAttribute(SItemType));
end;

procedure TXMLSimpleTypeList.SetItemType(const Value: DOMString);
begin
  SetAttribute(SItemType, SchemaDef.PrefixedTypeName(Value));
end;

{ TXMLSimpleTypeUnion }

procedure TXMLSimpleTypeUnion.AfterConstruction;
begin
...
function TXMLSimpleTypeUnion.GetMemberTypes: DOMString;
begin
  Result := VarToStr(GetAttribute(SMemberTypes));
end;

procedure TXMLSimpleTypeUnion.SetMemberTypes(const Value: DOMString);
begin
  SetAttribute(SMemberTypes, SchemaDef.PrefixedTypeName(Value));
end;

function TXMLSimpleTypeUnion.GetSimpleTypes: IXMLSimpleTypeDefs;
begin
  Result := FSimpleTypes;
end;
...
begin
  RegisterChildNode(SRestriction, TXMLSimpleTypeRestriction);
  RegisterChildNode(SList, TXMLSimpleTypeList);
  RegisterChildNode(SUnion, TXMLSimpleTypeUnion);
...
var
  RestNode: IXMLSimpleTypeRestriction;
begin
  Result := nil;
...
begin
  ...
  begin
    ...
    if not Assigned(RestNode) then
      RestNode := AddChild(SRestriction) as IXMLSimpleTypeRestriction;
...
begin
  if Assigned(SchemaDef) and SchemaObject.IsLegacyFormat then
    Result := Self
  else if GetDerivationMethod = sdmList then
    Result := (GetContentNode as IXMLSimpleTypeList).SimpleTypeNode.GetFacetParentNode
  else
    Result := ChildNodes.FindNode(IXMLSimpleTypeRestriction);
  if Required and not Assigned(Result) then
    SchemaValidationError(SNoFacetsAllowed, [GetName]);
end;

function TXMLSimpleTypeDef.GetRestrictionNode: IXMLSimpleTypeRestriction;
begin
  Result := ChildNodes.FindNode(IXMLSimpleTypeRestriction) as IXMLSimpleTypeRestriction;
end;

function TXMLSimpleTypeDef.GetContentNode: IXMLSimpleContent;
begin
  Result := ChildNodes.FindNode(IXMLSimpleContent) as IXMLSimpleContent;
end;

function TXMLSimpleTypeDef.GetDerivationMethod: TSimpleDerivationMethod;
var
  ContentNode: IXMLSimpleContent;
begin
  Result := sdmNone;
  ContentNode := GetContentNode;
  if Assigned(ContentNode) then
  begin
    if Supports(ContentNode, IXMLSimpleTypeRestriction) then
      Result := sdmRestriction
    else if Supports(ContentNode, IXMLSimpleTypeList) then
      Result := sdmList
    else if Supports(ContentNode, IXMLSimpleTypeUnion) then
      Result := sdmUnion;
  end;
end;

function TXMLSimpleTypeDef.GetFinal: DOMString;
begin
  Result := VarToStr(GetAttribute(SFinal));
end;

procedure TXMLSimpleTypeDef.SetFinal(const Value: DOMString);
begin
  SetAttribute(SFinal, SchemaDef.PrefixedTypeName(Value));
end;
...
begin
  ...
  begin
    if Assigned(FCompositorNode) then
    begin
      { Delete any existing compositor, but make sure it's not a dummy hosted node first }
      if not IsHosted(FCompositorNode) and Assigned(CompositorNode.ParentNode) then
...
procedure TXMLComplexTypeDef.CheckContent;
  ...
  function FindDerivation(const DerivMeth: DOMString;
  ...
  begin
    ...
    if Result then
    begin
      nodeList := ChildNodes.FindNode(DerivMeth, GetNamespaceURI).ChildNodes;
      if SetDerivationMeth(nodeList.FindNode(SExtension, GetNamespaceURI), ExtType) then Exit;
      if SetDerivationMeth(nodeList.FindNode(SRestriction, GetNamespaceURI), RestType) then Exit;
...
function TXMLComplexTypeDef.GetElementDefList: IXMLElementDefList;
  ...
  function AlreadyInList(ElementDef: IXMLElementDef): Boolean;
  var
    I: Integer;
    Name: DOMString;
  begin
    ...
    for I := 0 to FElementDefList.Count - 1 do
    begin
      if FElementDefList[I].Name = Name then
      begin
        Result := True;
        Break;
      end;
  ...
  begin
    ...
    begin
      Node := NodeList[I];
      if Supports(Node, IXMLElementDef, ElementDef) and not AlreadyInList(ElementDef) then
...
begin
  ...
  if not SchemaObject.IsLegacyFormat then
  begin
    if GetDerivationMethod <> dmNone then
    begin
...
begin
  Result := ContentNode.GetCompositorNode;
end;
...
var
  I: Integer;
  WasHosted: Boolean;
  BaseTypeIndicatorNode: IXMLNode;
  OldAttributes: IXMLAttributeDefs;
begin
  OldAttributes := nil;
  WasHosted := False;
  if Value <> GetDerivationMethod then
  begin
    { If there are existing AttributesDefs, we need to move them }
    if GetAttributeDefs.Count > 0 then
    begin
      OldAttributes := GetAttributeDefs;
      { Here we touch the attributes datatype to cache it, otherwise we can't read them later }
      for I := 0 to OldAttributes.Count - 1 do
        OldAttributes[I].DataTypeName;
    end;
    { Delete any existing "complexContent" or "simpleContent" node }
    if not IsHosted(FContentNode) then
    begin
      WasHosted := True;
      { Delete the existing "simpleContent" or "complexContent" node }
      BaseTypeIndicatorNode := ChildNodes.FindNode(IXMLBaseTypeIndicator);
      if Assigned(BaseTypeIndicatorNode) then
        ChildNodes.Remove(BaseTypeIndicatorNode);
      FContentNode := nil;
    end;
    ...
    begin
      if not SchemaObject.IsLegacyFormat then
      begin
        BaseTypeIndicatorNode := AddChild(BaseTypeIndMap[Value]);
        FContentNode := BaseTypeIndicatorNode.AddChild(ExtRestTypeMap[Value]) as IXMLComplexContent;
      end
      else
        SetAttribute(SDerivedBy, ExtRestTypeMap[Value]);
    end;
    if FContentNode = nil then
      CheckContent;
    { Copy the attributes over now }
    if Assigned(OldAttributes) then
    begin
      for I := 0 to OldAttributes.Count - 1 do
        FContentNode.GetAttributeDefs.Add(OldAttributes[I].Name, OldAttributes[I].DataTypeName);
      { When the AttributeDefs are parented directly to the typedef, we will need to remove them }
      if not WasHosted then
        OldAttributes.Clear;
    end;
    FDerivationMethod := Value;
  end;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if SameNamespace(TypeNS, SXMLSchemaURI_2001) then
          FTypeDef := SchemaDef.BuiltInTypes.Find(TypeName);
        if not Assigned(FTypeDef) then
        begin
          ...
          begin
            ...
            begin
              ...
              if not Assigned(FTypeDef) then
                FTypeDef := FindItemNS(((SchemaDef.ComplexTypes as IXMLNodeAccess).GetNodeObject as TXMLSchemaItems), TypeName, '') as IXMLTypeDef;
              if not Assigned(FTypeDef) then
...
begin
  if not IsGlobal and (SchemaObject <> nil) then
    Result := SchemaObject.GetAttributeDefs else
    Result := nil;
end;
...
begin
  if not IsGlobal and (SchemaObject <> nil) then
    Result := SchemaObject.GetElementDefs else
    Result := nil;
end;
...
function TXMLElementDef.IsRepeating: Boolean;
  ...
  function IsRepeatingValue(const Value: Variant): Boolean;
  var
    S: string;
  begin
    S := VarToStr(Value);
  ...
  function ParentRepeats(const Node: IXMLNode): Boolean;
  var
    CompositorNode: IXMLElementCompositor;
  begin
    if Assigned(Node) and Supports(Node, IXMLElementCompositor, CompositorNode) then
    begin
      Result := IsRepeatingValue(CompositorNode.GetMaxOccurs);
      if not Result then
        Result := ParentRepeats(CompositorNode.ParentNode);
    end
    else
      Result := False;
  end;
...
begin
  Result := IsRepeatingValue(GetMaxOccurs);
  if not Result then
    Result := ParentRepeats(ParentNode);
end;
...
const
  SAllCompositorNames = SSequence + ';' + SChoice + ';' + SAll;
begin
  ...
  FCompositors := CreateCollection(TXMLElementCompositors,
    IXMLElementCompositor, SAllCompositorNames) as IXMLElementCompositors;
...
begin
  if not IsGlobal and (SchemaObject <> nil) then
    Result := SchemaObject.GetElementGroups else
    Result := nil;
end;
...
type
  TSchemaIterator = class
  strict private
    FSearchingSiblings: Boolean;
    FVisitedSchemas: TStringList;
    FCollectionClassType: TXMLNodeCollectionClass;
    FSchemaDepth: Integer;
  strict protected
    function SearchCollection(const SchemaNode: IXMLNode; const Name: DOMString): IXMLSchemaItem;
    function SchemaVisited(const SchemaDoc: IXMLSchemaDef): Boolean;
    function SearchSchemaDocRefs(const SchemaDocRefs: IXMLSchemaDocRefs;
      const Name, ItemNS: DOMString): IXMLSchemaItem;
    function SearchParentSchema(const ParentSchema: IXMLSchemaDef; const Name: DOMString): IXMLSchemaItem;
    function SearchSiblingSchemas(const ParentNode: IXMLNode; const Name, ItemNS: DOMString): IXMLSchemaItem;
  public
    constructor Create(SchemaItems: TXMLSchemaItems);
    destructor Destroy; override;
    function SearchItems(SchemaItems: TXMLSchemaItems; const Name, ItemNS: DOMString): IXMLSchemaItem;
    property SchemaDepth: Integer read FSchemaDepth;
  end;

var
  GSchemaIterator: TSchemaIterator;

constructor TSchemaIterator.Create(SchemaItems: TXMLSchemaItems);
begin
  FCollectionClassType := SchemaItems.FCollectionClassType;
  FVisitedSchemas := TStringList.Create;
  FVisitedSchemas.CaseSensitive := False;
  FVisitedSchemas.Sorted := True;
  SchemaVisited(SchemaItems.SchemaDef);
  inherited Create;
end;

destructor TSchemaIterator.Destroy;
begin
  inherited;
  FVisitedSchemas.Free;
end;

// Ensure we don't search the same schema document more than one time.
function TSchemaIterator.SchemaVisited(const SchemaDoc: IXMLSchemaDef): Boolean;
var
  SchemaLocation: string;
begin
  SchemaLocation := SchemaDoc.OwnerDocument.FileName;
  Result := FVisitedSchemas.IndexOf(SchemaLocation) <> -1;
  if not Result then
    FVisitedSchemas.Add(SchemaLocation);
end;

function TSchemaIterator.SearchCollection(const SchemaNode: IXMLNode; const Name: DOMString): IXMLSchemaItem;
var
  SchemaObj: IXMLNodeAccess;
  Collection: IXMLSchemaItems;
begin
  SchemaObj := (SchemaNode as IXMLNodeAccess);
  Collection := SchemaObj.FindHostedNode(FCollectionClassType) as IXMLSchemaItems;
  Assert(Assigned(Collection), 'Internal error obtaining schema type collection'); // Do not localize
  Result := Collection.FindItem(ExtractLocalName(Name));
end;

// Search schema "import" and "include" references
function TSchemaIterator.SearchSchemaDocRefs(const SchemaDocRefs: IXMLSchemaDocRefs;
  const Name, ItemNS: DOMString): IXMLSchemaItem;
var
  I: Integer;
  SchemaDocRef: IXMLSchemaDef;
begin
  Result := nil;
  for I := 0 to SchemaDocRefs.Count - 1 do
  begin
    SchemaDocRef := SchemaDocRefs[I].SchemaRef;
    if Assigned(SchemaDocRef) and
       SameNamespace(VarToStr(SchemaDocRef.TargetNamespace), ItemNS) and
       not SchemaVisited(SchemaDocRef) then
    begin
      Result := SearchCollection(SchemaDocRef, Name);
      if Assigned(Result) then Break;
    end;
  end;
end;

// For "included" schemas, look for types in the parent.
function TSchemaIterator.SearchParentSchema(const ParentSchema: IXMLSchemaDef;
  const Name: DOMString): IXMLSchemaItem;
begin
  Result := nil;
  if not SchemaVisited(ParentSchema) then
    Result := SearchCollection(ParentSchema, Name);
end;

// Search other schema documents in a WSDL document
function TSchemaIterator.SearchSiblingSchemas(const ParentNode: IXMLNode;
  const Name, ItemNS: DOMString): IXMLSchemaItem;
var
  I: Integer;
  SchemaDef: IXMLSchemaDef;
begin
  Result := nil;
  if not FSearchingSiblings then
  begin
    FSearchingSiblings := True;
    try
      for I := 0 to ParentNode.ChildNodes.Count - 1 do
        if Supports(ParentNode.ChildNodes[I], IXMLSchemaDef, SchemaDef) and
           SameNamespace(VarToStr(SchemaDef.TargetNamespace), ItemNS) then
        begin
          Result := SearchCollection(SchemaDef, Name);
          if Assigned(Result) then Break;
        end;
    finally
      FSearchingSiblings := False;
    end;
  end;
end;

function TSchemaIterator.SearchItems(SchemaItems: TXMLSchemaItems; const Name, ItemNS: DOMString): IXMLSchemaItem;
begin
  Inc(FSchemaDepth);
  try
    Result := SearchSchemaDocRefs(SchemaItems.SchemaDef.SchemaIncludes, Name, ItemNS);
    if not Assigned(Result) then
      Result := SearchSchemaDocRefs(SchemaItems.SchemaDef.SchemaImports, Name, ItemNS);
    if not Assigned(Result) and (SchemaItems.SchemaDef.ParentRefType = srInclude) then
      Result := SearchParentSchema(SchemaItems.SchemaDef.ParentSchema, Name);
    if not Assigned(Result) and (SchemaItems.SchemaDef.ParentNode.NodeType = ntElement) then
      Result := SearchSiblingSchemas(SchemaItems.SchemaDef.ParentNode, Name, ItemNS);
  finally
    Dec(FSchemaDepth);
  end;
end;
...
begin
  ...
  begin
    if not Assigned(GSchemaIterator) then
      GSchemaIterator := TSchemaIterator.Create(SchemaItems);
    Result := GSchemaIterator.SearchItems(SchemaItems, Name, ItemNS);
    if GSchemaIterator.SchemaDepth = 0 then
      FreeAndNil(GSchemaIterator);
  end;
end;
...
function TXMLSchemaDef.FindNamespacePrefix(const NamespaceURI: DOMString): DOMString;
var
  NSDecl: IXMLNode;
begin
  Result := '';
  // Try to find a prefix for the given NamespaceURI.  If we don't find one in this schema
  // then traverse any parent schemas documents as well.
  NSDecl := FindNamespaceDecl(NamespaceURI);
  if ((NSDecl = nil) or (NSDecl.NodeName = SXMLNS)) and (FParentSchemaDef <> nil) then
  begin
    NSDecl := FParentSchemaDef.FindNamespaceDecl(NamespaceURI);
    if (NSDecl <> nil) and (NSDecl.NodeName <> SXMLNS) then
      Result := NSDecl.LocalName; // LocalName is the prefix (e.g. xmlns:pre)
  end;
end;
...
function TXMLSchemaDef.GetParentSchemaDef: IXMLSchemaDef;
begin
  Result := FParentSchemaDef as IXMLSchemaDef;
end;

function TXMLSchemaDef.GetQualifyAnonymousTypeNames: Boolean;
begin
  Result := FQualifyAnonymousTypeNames;
end;

function TXMLSchemaDef.GetParentRefType: TSchemaRefType;
begin
  Result := FParentRefType;
end;
...
var
  NamespaceAttr: OleVariant;
begin
  NamespaceAttr := GetAttribute(STargetNamespace);
  if (FParentRefType = srInclude) and Assigned(FParentSchemaDef) then
  begin
    Result := FParentSchemaDef.TargetNamespace;
    if not VarIsNull(NamespaceAttr) and
       not SameNamespace(VarToStr(Result), VarToStr(NamespaceAttr)) then
    begin
      if VarIsNull(Result) then
        Result := SUndefinedTargetNS;
       SchemaValidationError(SInvalidTargetNS, [GetSchemaDoc.FileName, string(Result), string(NamespaceAttr)]);
    end
  end
  else
    Result := NamespaceAttr;
end;
...
procedure TXMLSchemaDef.SetParentSchemaDef(const Value: IXMLSchemaDef);
begin
  FParentSchemaDef := (Value as IXMLNodeAccess).GetNodeObject as TXMLSchemaDef;
end;

procedure TXMLSchemaDef.SetQualifyAnonymousTypeNames(const Value: Boolean);
begin
  FQualifyAnonymousTypeNames := Value;
end;

procedure TXMLSchemaDef.SetParentRefType(const Value: TSchemaRefType);
begin
  FParentRefType := Value;
end;
...
begin
  Result := VarToStr(GetAttribute(SVersion));
end;
...
function TXMLSchemaDef.LoadSchemaRef(const SchemaDocRef: IXMLSchemaDocRef): IXMLSchemaDef;

  { Simple check for whether a URI starts with 'http://' or 'https://' }
  function isHTTP(const URI: DOMString): boolean;
  const
    sHTTPPrefix = 'http://';
    sHTTPsPrefix= 'https://';
  begin
    Result := SameText(Copy(URI, 1, Length(sHTTPPrefix)), sHTTPPrefix) or
              SameText(Copy(URI, 1, Length(sHTTPsPrefix)),sHTTPsPrefix);
  end;

  { Enhanced ExtractFilePath that handles http URI }
  function ExtractFilePathEx(const W: DOMString): DOMString;
  const
    FirstIndex = Low(string);
    AdjustIndex= 1-Low(string);
  const
    URLSeparator: WideChar = '/';
  var
    Len: Integer;
  begin
    Result := '';
    ...
    begin
      ...
      while (Len > 0) do
      begin
        if W[Len-AdjustIndex] = URLSeparator then
        begin
          Result := Copy(W, 0, Len);
          Exit;
        end;
      ...
      Result := ExtractFilePath(W);
  end;

var
  I: Integer;
  SchemaLoc, FilePath, ImportPath: string;
begin
  Result := nil;
  SchemaLoc := SchemaDocRef.SchemaLocation;
  if SchemaLoc <> '' then
  begin
    ImportPath := ExtractFilePathEx(SchemaLoc);
    FilePath := OwnerDocument.FileName;
    if not isHTTP(FilePath) then
      FilePath := ExpandFileName(FilePath);
    FilePath := ExtractFilePathEx(FilePath);
    if (FilePath <> '') and (ImportPath = '') then
    begin
      SchemaLoc := FilePath + SchemaLoc;
    end;
    Result := LoadXMLSchema(SchemaLoc).SchemaDef;
    Result.ParentSchema := Self;
    Result.ParentRefType := SchemaDocRef.SchemaRefType;
    for I := 0 to Length(FAppInfoClasses) - 1 do
      Result.RegisterAppInfo(FAppInfoClasses[I].NamespaceURI, FAppInfoClasses[I].Prefix, FAppInfoClasses[I].NodeClass);
  end;
end;
...
procedure TXMLSchemaDoc.SaveToFile(const AFileName: DOMString);
var
  Translator: IXMLSchemaTranslator;
begin
...
procedure TXMLSchemaTranslator.Translate(const FileName: DOMString;
...
function TXMLSchemaTranslatorFactory.CanExportFile(const FileName: DOMString): Boolean;
begin
  Result := Assigned(FExportTranslatorClass) and
    (AnsiCompareText(ExtractFileExt(FileName), FExtension) = 0);
end;

function TXMLSchemaTranslatorFactory.CanImportFile(const FileName: DOMString): Boolean;
begin
...
constructor TXMLSchemaTranslatorFactory.Create(ImportClass, ExportClass: TSchemaTranslatorClass; const
  Extension, Description: DOMString);
begin
  inherited Create;
...
function TXMLSchemaTranslatorFactory.GetTranslatorDescription: DOMString;
begin
  Result := FDescription;
end;
