...
uses System.SysUtils, System.Classes, Xml.XMLSchema, Xml.xmldom, Xml.XMLDoc, Xml.XMLIntf;
...
procedure TXDRImportTranslator.Translate(const FileName: DOMString;
...
begin
  ...
  begin
    ...
    if DtMaxLength <> '' then
      SimpleTypeDef.MaxLength:= DtMaxLength;
    if DtMin <> '' then
      SimpleTypeDef.MinInclusive:= DtMin;
    if DtMax <> '' then
      SimpleTypeDef.MaxInclusive:= DtMax;
    if DtValues <> '' then
    begin
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        AttributeDef.DataType.Enumerations.Add(OList[I]);
      end;
    finally
      OList.Free;
    end;
...
begin
  ...
  Attribute := XsdAttributeDefs.Add(Name);
  if DefValue <> '' then
  begin
