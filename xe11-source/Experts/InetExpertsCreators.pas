...
uses DCCStrs, CommonOptionStrs, ToolsApi, InetExpertsTemplateProperties, StrUtils, 
  PlatformAPI, Vcl.Dialogs;
...
procedure TInetExpertsCreatorsModule.SetProjectOptions;
var
  LProject: IOTAProject;
  LPlatforms: IOTAProjectPlatforms160;
...
begin
  ...
  if Personality = sCBuilderPersonality then
    if Supports(LProject.ProjectOptions, IOTAProjectOptionsConfigurations, LOptionsConfigurations) then
      LOptionsConfigurations.BaseConfiguration.AsBoolean[sUsingDelphiRTL] := True;

  if (osfLinux in WebProject.PlatformTypes) and (osfLinux in WebProject.ProjectPlatformTypes) then
  begin
    if Supports(LProject, IOTAProjectPlatforms160, LPlatforms) and
       LPlatforms.Supported[cLinux64Platform] then
      LPlatforms.AddPlatform(cLinux64Platform);
    //'AllowUndefined' is limited to Apache
    if FProjectType = ptApacheTwo then
      if Supports(LProject.ProjectOptions, IOTAProjectOptionsConfigurations, LOptionsConfigurations) then
      begin
        LBuildConfiguration := LOptionsConfigurations.BaseConfiguration.PlatformConfiguration[cLinux64Platform];
        LBuildConfiguration.SetBoolean(sAllowUndefined, True);
      end;
  end;
end;
...
begin
  ...
  case FProjectType of
    ...
    begin
      ...
      WebProject.ProjectPlatformTypes := WebProject.PlatformTypes - [osfLinux];
    end;
    ptCGI:
    begin
...
(*
  if (Personality = sCBuilderPersonality) and
     (WebProject.FrameWorkType = sFrameworkTypeNone) then
    WebProject.FrameWorkType := sFrameworkTypeVCL;
*)
