...
procedure TDSServerExpertsUIModule.FrameWorkWizardPageEnterPage(
  Sender: TCustomExpertsWizardPage;
  PageTransition: TExpertsWizardPageTransition);
begin
  if (FProjectType in [ptVCL, ptService]) and
...
begin
  ...
  else if FProjectType = ptService then
    LFrameWorkType := frameworkVCL
  else
...
procedure TDSServerExpertsUIModule.ServerClassWizardPageEnterPage(Sender: TCustomExpertsWizardPage;
  PageTransition: TExpertsWizardPageTransition);
begin
  TDSServerClassFrame(ServerClassWizardPage.Frame).rbTComponent.Visible := true; // not (osfLinux in FPlatformTypes);
  TDSServerClassFrame(ServerClassWizardPage.Frame).rbDataModule.Visible := true; // not (osfLinux in FPlatformTypes);
  TDSServerClassFrame(ServerClassWizardPage.Frame).rbDSServerModule.Visible := true; // not (osfLinux in FPlatformTypes);
end;
