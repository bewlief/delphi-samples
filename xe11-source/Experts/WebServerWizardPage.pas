...
begin
  ...
  FCGICheckboxTop := CreateNewCGIApp.Top;
end;
...
begin
  Result := not(CreateNewISAPIApp.Checked or CreateNewIndyApp.Checked) or
    (CreateNewISAPIApp.Visible and CreateNewIndyApp.Visible);
  if not Result then
...
begin
  ...
  begin
    ...
    // Only implemented for CGI, Indy GUI and ISAPI
    Assert(FHiddenProjectTypes - [ptCGI, ptIndyForm, ptISAPI] = []);
    CreateNewCGIApp.Visible := not (ptCGI in FHiddenProjectTypes);
    CreateNewIndyApp.Visible := not (ptIndyForm in FHiddenProjectTypes);
    CreateNewISAPIApp.Visible := not (ptISAPI in FHiddenProjectTypes);
    if FHiddenProjectTypes * [ptIndyForm, ptISAPI] = [] then
      CreateNewCGIApp.Top := FCGICheckboxTop
    else
      CreateNewCGIApp.Top := CreateNewIndyApp.Top;
    if not CreateNewIndyApp.Visible and CreateNewIndyApp.Checked or
       not CreateNewISAPIApp.Visible and CreateNewISAPIApp.Checked then
      SetProjectType(ptIndyConsole)
    else if not CreateNewCGIApp.Visible and CreateNewCGIApp.Checked then
      SetProjectType(ptIndyConsole);
  end;
end;
