...
type
  ...
  private
    ...
    class procedure AddPreparedWrappers(
      const StartScopeEnumerable: IScopeEnumerable; const AList: TObject);
  public
    ...
    class procedure EnumerateResetWrappersArguments(const StartScopeEnumerable: IScopeEnumerable;
      const ACallback: TConstProc<IArguments>);
    ...
    class procedure EnumerateReattachWrappers(const StartScopeEnumerable: IScopeEnumerable;
        const AReattachCallback: TConstProc<IPlaceholder>;
    const AReattachRecordCallback: TConstProc<IRecordPlaceholder>);
    ...
    class procedure EnumeratePrepareWrappersForEvaluation(const StartScopeEnumerable: IScopeEnumerable;
      const AResetCallback: TConstProc<IArguments>;
      const AReattachCallback: TConstProc<IPlaceholder>;
      const AReattachRecordCallback: TConstProc<IRecordPlaceholder>);
    class function GetPreparedWrappers(const StartScopeEnumerable: IScopeEnumerable): IPreparedWrappers;
...
begin
  ...
  begin
    ...
    Result := LTypeKind in [tkClass, tkRecord, tkMRecord];
  end;
end;
...
type
  ...
  TPreparedWrappers = class(TInterfacedObject, IPreparedWrappers)
  private
    FList: TList<TPrepare>;
    FFirstEvaluate: Boolean;
    FScopeEnumerable: IScopeEnumerable;
    procedure Prepare;
    procedure Refresh;
  public
    constructor Create(const AScopeEnumerable: IScopeEnumerable);
    destructor Destroy; override;
    { IPreparedWrappers }
    procedure BeforeEvaluate;
    procedure AfterEvaluate;
  end;


  TPrepareArgs = class(TPrepare)
  private
    FIntf: IArguments;
  public
    constructor Create(const AIntf: IArguments);
    procedure Prepare; override;
  end;

  TPreparePlaceholder = class(TPrepare)
  private
    FIntf: IPlaceholder;
  public
    constructor Create(const AIntf: IPlaceholder);
    procedure Prepare; override;
  end;

  TPrepareRecordPlaceholder = class(TPrepare)
  private
    FIntf: IRecordPlaceholder;
  public
    constructor Create(const AIntf: IRecordPlaceholder);
    procedure Prepare; override;
  end;

{ TPreparedWrappers }
procedure TPreparedWrappers.AfterEvaluate;
begin
  if FFirstEvaluate then
    Refresh; // Get new wrappers
  FFirstEvaluate := False;
end;

procedure TPreparedWrappers.BeforeEvaluate;
begin
  if FFirstEvaluate then
    Refresh;
  Prepare;
...
constructor TPreparedWrappers.Create(const AScopeEnumerable: IScopeEnumerable);
begin
  FScopeEnumerable := AScopeEnumerable;
  FFirstEvaluate := True;
  FList := TObjectList<TPrepare>.Create;
end;

destructor TPreparedWrappers.Destroy;
begin
  FList.Free;
  inherited;
end;

procedure TPreparedWrappers.Prepare;
var
  LPrepare: TPrepare;
begin
  for LPrepare in FList do
    LPrepare.Prepare;
end;

procedure TPreparedWrappers.Refresh;
begin
  TBindingSearch.AddPreparedWrappers(FScopeEnumerable, FList);
end;

{ TPrepareArgs }

constructor TPrepareArgs.Create(const AIntf: IArguments);
begin
  FIntf := AIntf;
end;

procedure TPrepareArgs.Prepare;
begin
  FIntf.SetArgs(nil);
...
constructor TPreparePlaceholder.Create(const AIntf: IPlaceholder);
begin
  FIntf := AIntf;
end;

procedure TPreparePlaceholder.Prepare;
begin
  FIntf.Attach(FIntf.Attachment);

end;

{ TPrepareRecordPlaceholder }

constructor TPrepareRecordPlaceholder.Create(const AIntf: IRecordPlaceholder);
begin
  FIntf := AIntf;
end;

procedure TPrepareRecordPlaceholder.Prepare;
begin
  FIntf.Attach(FIntf.Attachment);
...
var
  LList: TList<TPrepare>;
  LPrepare: TPrepare;
begin
  if not Assigned(StartScopeEnumerable) then
    Exit;
  LList := TObjectList<TPrepare>.Create;
  try
    EnumeratePrepareWrappersForEvaluation(StartScopeEnumerable,
      procedure(const Intf: IArguments)
      begin
        LList.Add(TPrepareArgs.Create(Intf))
      end,
      procedure(const Intf: IPlaceholder)
      begin
        LList.Add(TPreparePlaceholder.Create(Intf))
      end,
      procedure(const Intf: IRecordPlaceholder)
      begin
        LList.Add(TPrepareRecordPlaceholder.Create(Intf))
      end);

    for LPrepare in LList do
      LPrepare.Prepare;
  finally
    LList.Free;
  end;
...
class function TBindingSearch.GetPreparedWrappers(
  const StartScopeEnumerable: IScopeEnumerable): IPreparedWrappers;
var
  LPreparedWrappers: TPreparedWrappers;
begin
  LPreparedWrappers := TPreparedWrappers.Create(StartScopeEnumerable);
  Result := LPreparedWrappers;
end;

class procedure TBindingSearch.AddPreparedWrappers(
  const StartScopeEnumerable: IScopeEnumerable; const AList: TObject);
var
  LList: TList<TPrepare>;
begin
  LList := AList as TList<TPrepare>;
  LList.Clear;
  if not Assigned(StartScopeEnumerable) then
    Exit;
  EnumeratePrepareWrappersForEvaluation(StartScopeEnumerable,
...
class procedure TBindingSearch.EnumeratePrepareWrappersForEvaluation(
  const StartScopeEnumerable: IScopeEnumerable;
  const AResetCallback: TConstProc<IArguments>;
  const AReattachCallback: TConstProc<IPlaceholder>;
  const AReattachRecordCallback: TConstProc<IRecordPlaceholder>);
begin
  ...
  EnumerateResetWrappersArguments(StartScopeEnumerable, AResetCallback);
  ...
  EnumerateReattachWrappers(StartScopeEnumerable, AReattachCallback, AReattachRecordCallback);
end;
...
var
  ...
  Wrpr: IInterface;
begin
  if not Assigned(StartScopeEnumerable) or
     not StartScopeEnumerable.HasItemsSupporting([IPlaceholder, IRecordPlaceholder, IScopeEnumerable]) then
    Exit;
...
class procedure TBindingSearch.EnumerateReattachWrappers(
  const StartScopeEnumerable: IScopeEnumerable;
  const AReattachCallback: TConstProc<IPlaceholder>;
  const AReattachRecordCallback: TConstProc<IRecordPlaceholder>);
...
begin
  ...
  begin
    if Supports(Wrpr, IPlaceholder, WrprPlacehld) then
      //WrprPlacehld.Attach(WrprPlacehld.Attachment)
      AReattachCallback(WrprPlacehld)
    else
    if Supports(Wrpr, IRecordPlaceholder, WrprRecPlacehld) then
      // WrprRecPlacehld.Attach(WrprRecPlacehld.Attachment);
      AReattachRecordCallback(WrprRecPlacehld);
    ...
    if not Supports(Wrpr, IWrapper) and
       Supports(Wrpr, IScopeEnumerable, WrprScopeEnumerable) then
      EnumerateReattachWrappers(WrprScopeEnumerable, AReattachCallback, AReattachRecordCallback);
  end;
end;
...
begin
  if not Assigned(StartScopeEnumerable) or
     not StartScopeEnumerable.HasItemsSupporting([IArguments, IScopeEnumerable]) then
    Exit;
  for Wrpr in StartScopeEnumerable do
  begin
...
class procedure TBindingSearch.EnumerateResetWrappersArguments(
  const StartScopeEnumerable: IScopeEnumerable; const ACallback: TConstProc<IArguments>);
var
  Wrpr: IInterface;
  WrprArguments: IArguments;
  WrprScopeEnumerable: IScopeEnumerable;
begin
  ...
  begin
    ...
    if Supports(Wrpr, IArguments, WrprArguments) then
      // WrprArguments.SetArgs(nil);
      ACallback(WrprArguments);

    // go in-depth in its subwrappers
    if Supports(Wrpr, IScopeEnumerable, WrprScopeEnumerable) then
      EnumerateResetWrappersArguments(WrprScopeEnumerable, ACallback);
  end;
end;
