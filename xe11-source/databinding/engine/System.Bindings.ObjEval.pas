...
type
  ...
  { TObjectWrapper
    ...
    Wraps around a specified object and stores an internal dictionary scope to
  ...
  public
    ...
    procedure Attach(const Rec: TValue);
    procedure Detach;
  ...
  public
    ...
    procedure AttachRecordOnly(const Rec: TValue);
...
begin
  if AObject = nil then
    raise EWrapperError.CreateRes(@sScopeObjNull);
...
begin
  ...
  if not Assigned(MetaClass) then
    raise EWrapperError.CreateResFmt(@SParamIsNil, ['MetaClass']);
...
begin
  ...
  begin
    ...
    if Assigned(LMember) then
      FTypeInfo := TRttiProperty(LMember).PropertyType.Handle
    else
                                                                                
      FTypeInfo := GetParent.ClassInfo;
  end;
  Result := FTypeInfo;
end;
...
begin
  ...
  LHasStructResult := LMethodResult.IsObject or (LMethodResult.TypeInfo^.Kind in [tkRecord, tkMRecord]);
...
begin
  ...
  begin
    ...
    if Assigned(Result) and Assigned(LType) then
      case CacheType of
        ctObject:
          if LType^.Kind in [tkRecord, tkMRecord] then
...
begin
  ...
  if not ResVal.IsEmpty and (ResVal.TypeInfo^.Kind in [tkClass, tkRecord, tkMRecord]) then
    if CanCache then
    begin
      case CacheType of
        ctObject:
...
begin
  ...
  raise EWrapperError.CreateRes(@sVirtualMemberReadOnly);
end;
...
begin
  ...
  if Supports(FromWrpr, IDynamicGroup, FromDynGrp) then
    if Supports(ToWrpr, IDynamicGroup, ToDynGrp) then
      ...
      raise EWrapperError.CreateResFmt(@sConvertWrapperFail, ['IDynamicGroup'])
  else
  ...
  if Supports(FromWrpr, IGroup, FromGroup) then
    if Supports(ToWrpr, IGroup, ToGroup) then
      ...
      raise EWrapperError.CreateResFmt(@sConvertWrapperFail, ['IGroup']);
...
begin
  ...
  if Supports(FromWrpr, IArguments, FromArguments) then
    if Supports(ToWrpr, IArguments, ToArguments) then
      ...
      raise EWrapperError.CreateResFmt(@sConvertWrapperFail, ['IArguments']);
...
begin
  if not Supports(Wrapper, ICustomWrapper) then
    raise EWrapperError.CreateResFmt(@sInvalidCustomWrapper, [IntfName]);
end;
...
function TAbstractStructuredWrapper.HasItemsSupporting(const IIDs: array of TGUID): Boolean;
begin
  Result := FCache.HasItemsSupporting(IIDs);
end;

{ TRecordWrapper }

procedure TRecordWrapper.Attach(const Rec: TValue);
var
  LWrpr: IInterface;
...
function TAbstractMemberGroup.HasItemsSupporting(const IIDs: array of TGUID): Boolean;
begin
  Result := FCache.HasItemsSupporting(IIDs);
end;
...
procedure TPhysicalRecordMemberGroup.Attach(const Rec: TValue);
var
  LResultWrapper: IInterface;
  LPlaceholder: IRecordPlaceholder;
begin
...
procedure TPhysicalRecordMemberGroup.AttachRecordOnly(const Rec: TValue);
begin
  FRecord := Rec;
end;
...
begin
  ...
  if not Assigned(Group) then
    raise EWrapperError.CreateResFmt(@SParamIsNil, ['Group']);
  ...
  begin
    ...
    if Assigned(LType) then
      case LType^.Kind of
        ...
        tkRecord, tkMRecord: // use a TRecordWrapper cache
          begin
...
begin
  ...
  Result := Assigned(LTypInf) and
           (LTypInf^.Kind in [tkClass, tkRecord, tkMRecord]);
end;
...
function TAbstractMemberInstance.HasItemsSupporting(const IIDs: array of TGUID): Boolean;
begin
  Result := Cache.HasItemsSupporting(IIDs);
end;
...
procedure TPhysicalRecordMemberInstance.Attach(const Rec: TValue);
begin
  if not FAttaching then
  begin
    FAttaching := True;
