...
type
  ...
  IScopeEnumerator = interface
    ...
    /// <returns>True when the jump to the next wrapper has been successfully done.
  ...
  /// <summary>Persists prepared wrappers between repeated evaluations of same expression</summary>
  IPreparedWrappers = interface
...
begin
  ...
  if (IID = IArguments) and ((FWorkMode <> TWorkMode.wmObjectProperty) or not Assigned(FIndexedProperty)) or
     (IID = IChild) and (FWorkMode <> TWorkMode.wmObjectProperty) then
...
function TLocationWrapper.GetMemberName: String;
begin
  if FWorkMode = wmObjectProperty then
    if Assigned(FProperty) then
      Result := FProperty.Name
    else
      Result := FIndexedProperty.Name
  else
    Result := '';
end;

function TLocationWrapper.GetParent: TObject;
begin
  if FWorkMode = wmObjectProperty then
    Result := FObject
  else
...
constructor TWrapperDictionary.Create;
begin
  inherited Create(40);
end;
