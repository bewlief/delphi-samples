...
begin
  if not Assigned(FMethodScope) then
    FMethodScope := GetMethodScope([]);
  Result := FMethodScope;
end;
  ...
  if Length(MethodNames) > 0 then
    ...
    begin
      if not FRegisteredMethods.ContainsKey(LMethod) then
        raise EBindMethodError.CreateResFmt(@sMethodNotFound, [LMethod]);
...
begin
  ...
  begin
    if Length(Args) <> 1 then
      raise EEvaluatorError.CreateResFmt(@sUnexpectedArgCount, [1, Length(Args)]);
    v := Args[0];
  ...
  begin
    ...
    for I := 0 to Length(FmtArgs) - 1 do
      try
        ...
        ArgVals[I] := Args[I+1].GetValue.CastToVarRec;
        FmtArgs[I] := ArgVals[I].AsVarRec;
      except
        on E: EInvalidCast do
          raise EBindMethodError.CreateResFmt(@sFormatUnexpectedType, [Args[I+1].GetType.NameFld.ToString]);
      end;
  ...
  var
    ...
    begin
      try
        ...
        raise EBindMethodError.CreateResFmt(@sConversionUnavailable, [v.GetType.Name, 'Variant'])
      end;
    end;
  begin
    if Length(Args) <> 1 then
...
begin
  ...
  begin
    if Length(Args) <> 1 then
      raise EEvaluatorError.CreateRes(@sToStrArgCount);
    v := Args[0];
    // Non-bidi ToStr()
    if v.GetValue.IsEmpty then
...
begin
  ...
  begin
    if Length(Args) > sMaxArgs then
      raise EEvaluatorError.CreateResFmt(@sMaxArgsExceeded, [sMaxArgs]);
...
begin
  ...
  begin
    if Length(Args) <> 3 then
      raise EEvaluatorError.CreateResFmt(@sUnexpectedArgCount, [3, Length(Args)]);
    v1 := Args[0];
    v2 := Args[1];
    v3 := Args[2];
