...
type
  TCollectionInterfacesTracker = class(TList<TGUID>)
  private
    procedure ItemChanged(Sender: TObject; const Item: IInterface;
      Action: TCollectionNotification);
  public
    function GetSupported(const IIDs: array of TGUID): Boolean;
  end;
  ...
  public
    constructor Create(const MethodName: string; const MethodInvokable: IInvokable);
  ...
  private
    FWrappers: TWrappers;
    FSupportedIntfs: TCollectionInterfacesTracker;
  public
    constructor Create;
    ...
    function HasItemsSupporting(const IIDs: array of TGUID): Boolean;
  ...
  public
    ...
    type
      ...
      private
        ...
        FMapEnumerator: TMap.TValueEnumerator;
      public
...
// Supports(FScope, IScopeEnumerable, ScopeEnumerable) and ScopeEnumerable.GetEnumerator()
  ...
  public
    constructor Create;
    destructor Destroy; override;
    procedure AddConversion(AFrom, ATo: PTypeInfo; const AFunc: TFunc<TValue,TValue>);
...
{ TCollectionInterfacesTracker }

procedure TCollectionInterfacesTracker.ItemChanged(Sender: TObject;
...
begin
  ...
  begin
    if (Item.QueryInterface(IArguments, LIntf) = 0) and not Contains(IArguments) then
      Add(IArguments);
    if (Item.QueryInterface(IScopeEnumerable, LIntf) = 0) and not Contains(IScopeEnumerable) then
      Add(IScopeEnumerable);
    if (Item.QueryInterface(IPlaceholder, LIntf) = 0) and not Contains(IPlaceholder) then
      Add(IPlaceholder);
    if (Item.QueryInterface(IRecordPlaceholder, LIntf) = 0) and not Contains(IRecordPlaceholder) then
      Add(IPlaceholder);
    if (Item.QueryInterface(IWrapperBinding, LIntf) = 0) and not Contains(IWrapperBinding) then
      Add(IWrapperBinding);
  end;
end;

function TCollectionInterfacesTracker.GetSupported(const IIDs: array of TGUID): Boolean;
var
  LIID: TGUID;
begin
  for LIID in IIDs do
    if Contains(LIID) then
      Exit(True);
  Result := False;
end;
...
begin
  ...
  FSupportedIntfs := TCollectionInterfacesTracker.Create;
  FMap := TMap.Create;
  FMap.OnValueNotify := FSupportedIntfs.ItemChanged;
end;

destructor TDictionaryScope.Destroy;
begin
  FMap.Free;
  FSupportedIntfs.Free;
  inherited;
end;
...
function TDictionaryScope.HasItemsSupporting(const IIDs: array of TGUID): Boolean;
begin
  Result := FSupportedIntfs.GetSupported(IIDs);
end;
...
function TNestedScope.HasItemsSupporting(const IIDs: array of TGUID): Boolean;
begin
  Result := True;
end;
...
procedure TValueConverter.AddConversion(AFrom, ATo: PTypeInfo;
  const AFunc: TFunc<TValue, TValue>);
begin
...
begin
  if FList.Count = 0 then
    ...
    raise EEvaluatorError.CreateResFmt(@sOverloadGroupArgCount, [Name, FArity]);
  FList.Add(AOverload);
end;
...
begin
  if Length(Args) <> FArity then
    raise EEvaluatorError.CreateResFmt(@sNamedMismatchArgCount,
  ...
  try
    ...
    if candidates.Count = 0 then
      raise EEvaluatorError.CreateResFmt(@sOverloadNotFound, [Name]);
    if candidates.Count > 1 then
      raise EEvaluatorError.CreateResFmt(@sAmbiguousOverloadCall, [Name]);
...
type
  TValueInvokableBody = reference to function(const Args: TArray<TValue>): TValue;
...
function GetBasicOperatorValue(const Value: TValue; out AOutValue: string): Boolean; overload;
var
  f: Extended;
  i: Int64;
  s: string;
...
function GetBasicOperatorValue(const Value: TValue; out AOutValue: Extended): Boolean; overload;
var
  f: Extended;
  i: Int64;
  s: string;
...
begin
  ...
  scope.Map.Add(BuiltinOpName[boAdd], TValueInvokable.Create(2, function(const Args: TArray<TValue>): TValue
  var
    fx, fy: Extended;
    sx, sy: string;
  begin
    ...
    begin
      if GetBasicOperatorValue(Args[0], sx) and GetBasicOperatorValue(Args[1], sy) then
        ...
        raise EEvaluatorError.CreateResFmt(@sAddOpFail, [Args[0].ToString, Args[1].ToString]);
    end;
  ...
  scope.Map.Add(BuiltinOpName[boSubtract], TValueInvokable.Create(2, function(const Args: TArray<TValue>): TValue
  var
    fx, fy: Extended;
    sy: string;
  begin
    if not GetBasicOperatorValue(Args[0], fx) then
      raise EEvaluatorError.CreateResFmt(@sSubtractOpFail, [Args[0].ToString, Args[1].ToString]);
    if not GetBasicOperatorValue(Args[1], fy) then
    begin
  ...
  scope.Map.Add(BuiltinOpName[boMultiply], TValueInvokable.Create(2, function(const Args: TArray<TValue>): TValue
  var
    fx, fy: Extended;
  begin
    if not GetBasicOperatorValue(Args[0], fx) then
      raise EEvaluatorError.CreateResFmt(@sMultiplyOpFail, [Args[0].ToString, Args[1].ToString]);
    if not GetBasicOperatorValue(Args[1], fy) then
      raise EEvaluatorError.CreateResFmt(@sMultiplyOpFail, [Args[0].ToString, Args[1].ToString]);
    Result := fx * fy;
  end));

  scope.Map.Add(BuiltinOpName[boDivide], TValueInvokable.Create(2, function(const Args: TArray<TValue>): TValue
  var
    fx, fy: Extended;
  begin
    if not GetBasicOperatorValue(Args[0], fx) then
      raise EEvaluatorError.CreateResFmt(@sDivideOpFail, [Args[0].ToString, Args[1].ToString]);
    if not GetBasicOperatorValue(Args[1], fy) then
      raise EEvaluatorError.CreateResFmt(@sDivideOpFail, [Args[0].ToString, Args[1].ToString]);
    Result := fx / fy;
  end));

  scope.Map.Add(BuiltinOpName[boNegate], TValueInvokable.Create(1, function(const Args: TArray<TValue>): TValue
  var
    ix: Int64;
    fx: Extended;
  begin
    if Args[0].TryAsType<Int64>(ix) then
      ...
      raise EEvaluatorError.CreateResFmt(@sNegateOpFail, [Args[0].ToString]);
  end));

  scope.Map.Add(BuiltinOpName[boEqualTo], TValueInvokable.Create(2, function(const Args: TArray<TValue>): TValue
  var
    ix, iy: Int64;
    sx, sy: string;
    fx, fy: Extended;
  ...
  begin
    if Args[0].TryAsType<Int64>(ix) and Args[1].TryAsType<Int64>(iy) then
      ...
      raise EEvaluatorError.CreateResFmt(@sCompareOpFail,
        [Args[0].ToString, Args[1].ToString]);
  end));

  scope.Map.Add(BuiltinOpName[boNotEqual], TValueInvokable.Create(2, function(const Args: TArray<TValue>): TValue
  var
    ix, iy: Int64;
    sx, sy: string;
    fx, fy: Extended;
  ...
  scope.Map.Add(BuiltinOpName[boLessThan], TValueInvokable.Create(2, function(const Args: TArray<TValue>): TValue
  var
    ix, iy: Int64;
    sx, sy: string;
    fx, fy: Extended;
  ...
  scope.Map.Add(BuiltinOpName[boLessEqual], TValueInvokable.Create(2, function(const Args: TArray<TValue>): TValue
  var
    ix, iy: Int64;
    sx, sy: string;
    fx, fy: Extended;
  ...
  scope.Map.Add(BuiltinOpName[boGreaterThan], TValueInvokable.Create(2, function(const Args: TArray<TValue>): TValue
  var
    ix, iy: Int64;
    sx, sy: string;
    fx, fy: Extended;
  ...
  scope.Map.Add(BuiltinOpName[boGreaterEqual], TValueInvokable.Create(2, function(const Args: TArray<TValue>): TValue
  var
    ix, iy: Int64;
    sx, sy: string;
    fx, fy: Extended;
  ...
  scope.Map.Add(BuiltinOpName[boAnd], TValueInvokable.Create(2, function(const Args: TArray<TValue>): TValue
  var
    bx, by: Boolean;
  begin
    if Args[0].TryAsType<Boolean>(bx) and Args[1].TryAsType<Boolean>(by) then
      Result := bx and by
    else
      raise EEvaluatorError.CreateResFmt(@sBoolOpFail,
        [Args[0].ToString, Args[1].ToString]);
  end));
  scope.Map.Add(BuiltinOpName[boOr], TValueInvokable.Create(2, function(const Args: TArray<TValue>): TValue
  var
    bx, by: Boolean;
  begin
    if Args[0].TryAsType<Boolean>(bx) and Args[1].TryAsType<Boolean>(by) then
  ...
  scope.Map.Add(BuiltinOpName[boXor], TValueInvokable.Create(2, function(const Args: TArray<TValue>): TValue
  var
    bx, by: Boolean;
  begin
    if Args[0].TryAsType<Boolean>(bx) and Args[1].TryAsType<Boolean>(by) then
  ...
  scope.Map.Add(BuiltinOpName[boNot], TValueInvokable.Create(1, function(const Args: TArray<TValue>): TValue
  var
    bx: Boolean;
  begin
    if Args[0].TryAsType<Boolean>(bx) then
      Result := not bx
    else
      raise EEvaluatorError.CreateResFmt(@sBoolNegateOpFail, [Args[0].ToString]);
  end));
...
begin
  if Length(Args) <> FArgCount then
    raise EEvaluatorError.CreateResFmt(@sMismatchArgCount,
...
{ TDictionaryScope.TWrappersEnumerator }
...
begin
  ...
  FScope := Scope;
  FMapEnumerator := TMap.TValueEnumerator.Create(FScope.Map);
end;
...
constructor TPairScope.Create(const MethodName: string;
  const MethodInvokable: IInvokable);
begin
...
{ TObjectMemberGroupScope.TWrappersEnumerator }
...
begin
  ...
  FWrappers.OnNotify := FSupportedIntfs.ItemChanged;
end;
...
function TObjectMemberGroupScope.HasItemsSupporting(const IIDs: array of TGUID): Boolean;
begin
  Result := FSupportedIntfs.GetSupported(IIDs);
end;

{ TNestedScope.TWrapperEnumerator }
...
begin
  ...
  FScope := Scope;
  SetLength(FWrappers, 2);
  FWrappers[0] := FScope.FInner;
  FWrappers[1] := FScope.FOuter;
...
function TNamespaceScope.HasItemsSupporting(const IIDs: array of TGUID): Boolean;
begin
  Result := False;
end;
