...
type
  ...
  public
    ...
    function Evaluate(const ARoot: IScope;
      const ASubscriptionCallback: TSubscriptionNotification;
      {out} Subscriptions: TList<ISubscription>; var APreparedWrappers: IPreparedWrappers): IValue; overload;
  ...
  private type
    ...
    TPrio = (prNon = -1, prTop = 10, prAssign = 20, prBoolOr = 30, prBoolAnd = 40,
      prRelOp = 50, prAddOp = 60, prMulOp = 70, prDotOp = 80);
  private type
    TParser = class
    private type
...
begin
  ...
  case cp^ of
    ...
    begin
      ...
      begin
        ...
        if not ((cp^ >= '0') and (cp^ <= '9')) then
          raise EEvaluatorError.CreateRes(@sInvalidFloatingPt);
        while (cp^ >= '0') and (cp^ <= '9') do
          Inc(cp);
      end;
      ...
      begin
        ...
        if (cp^ = '-') or (cp^ = '+') then
          Inc(cp);
        if not ((cp^ >= '0') and (cp^ <= '9')) then
          raise EEvaluatorError.CreateRes(@sInvalidFloatingPtExpt);
        while (cp^ >= '0') and (cp^ <= '9') do
          Inc(cp);
      end;
    ...
    begin
      ...
      SetString(FStringToken, start, cp - start);
      if SameText('and', FStringToken) then
        SetToken(Self, tkAnd, cp)
      else if SameText('or', FStringToken) then
        SetToken(Self, tkOr, cp)
      else if SameText('xor', FStringToken) then
        SetToken(Self, tkXor, cp)
      else if SameText('not', FStringToken) then
        SetToken(Self, tkNot, cp)
      else
    ...
    begin
      ...
      begin
        case cp^ of
          #0:
            raise EEvaluatorError.CreateRes(@sUnterminatedString);
    ...
    begin
      if cp[1] <> '=' then
        raise EEvaluatorError.CreateResFmt(@sInvalidOperator, [cp[0], cp[1]]);
    ...
    raise EEvaluatorError.CreateResFmt(@sInvalidOperatorNum, [Ord(cp^)]);
  end;
end;
...
begin
  ...
  try
    ...
    if FCurrTok <> tkEof then
      raise EEvaluatorError.CreateRes(@sExpectedEOF);
  finally
...
procedure TEvaluator.TParser.ParsePrio(Prio: TPrio; Skip: Boolean);
  ...
  begin
    ...
    if count > High(Word) then
      raise EEvaluatorError.CreateRes(@sTooManyArgs);
    Result := count;
  end;
    ...
    case FCurrTok of
      ...
      begin
        ...
        FBinding.AddOpArg2(opInvokeDirect,
          FBinding.AddConst(BuiltinOpName[LOper]), 1); // 1 arg to unary
        Exit; // must not skip token again
      end;
      ...
      raise EEvaluatorError.CreateRes(@sExpectedIdentifier);
    end;
    NextToken;
  end;
...
const
  ...
  BuiltinPrio: array[TToken] of TPrio = (
    ...
    prNon, prNon, prNon, prNon,
    // tkAnd, tkOr, tkXor, tkNot
    prBoolAnd, prBoolOr, prBoolOr, prBoolAnd
  );
var
  operatorPrio: TPrio;
  builtin: TBuiltinOp;
  ...
  begin
    ...
    begin
      ...
      case FCurrTok of
        ...
        begin
          tokenPair := Succ(FCurrTok); // select the appropriate closing parenthesis/bracket
...
begin
  ...
  if FConstCount = High(Word) then
    raise EEvaluatorError.CreateRes(@sExpressionTooComplex);
...
var
  LPreparedWrappers: IPreparedWrappers;
begin
  Result := Evaluate(ARoot, ASubscriptionCallback, Subscriptions, LPreparedWrappers);
end;

function TCompiledBinding.Evaluate(
  const ARoot: IScope;
  const ASubscriptionCallback: TSubscriptionNotification;
  Subscriptions: TList<ISubscription>;
  var APreparedWrappers: IPreparedWrappers): IValue;
var
  ...
  begin
    if tos < 0 then
      raise EEvaluatorError.CreateRes(@sEvalStackOverflow);
    Result := stack[tos];
    Dec(tos);
  end;
  ...
  begin
    ...
    raise EEvaluatorError.CreateResFmt(@sLookupError, [Name]);
  end;
  ...
  begin
    if Supports(Value, IGroup, LGroup) then
      ...
      raise EEvaluatorError.CreateRes(@sUnsupportedGroup);
  end;
  ...
  begin
    if not Supports(Value, IInvokable, LInvokable) and
      ...
      raise EEvaluatorError.CreateRes(@sInvokeError);
    ...
    begin
      if not Supports(Value, IValue, Result) then
        raise EEvaluatorError.CreateRes(@sUnsupportedWrapper);
...
begin
  try
    try
      ...
      if (APreparedWrappers = nil) and Supports(ARoot, IScopeEnumerable, RootEnumerable) then
        APreparedWrappers := TBindingSearch.GetPreparedWrappers(RootEnumerable);

      if APreparedWrappers <> nil then
        APreparedWrappers.BeforeEvaluate;
    ...
    if APreparedWrappers <> nil then
      APreparedWrappers.AfterEvaluate;
  end;
end;
