...
begin
  try
    ...
    for LScope in FScopes do
      LParentScope := TNestedScope.Create(LParentScope, LScope);
    if Associations.Count > 0 then
      LParentScope := TNestedScope.Create(LParentScope, CreateScope(Associations));
...
//    raise EBindingExpressionError.CreateRes(@sManagerNotFound);
...
begin
  if not FCompiled then
    raise EBindingExpressionError.CreateRes(@sUncompiledExpression);
...
procedure TBindingExpressionDefault.SetBinding(
  ...
  begin
    if not Assigned(StartScopeEnumerable) or
       not StartScopeEnumerable.HasItemsSupporting([IScopeEnumerable, IWrapperBinding]) then
      Exit;
