...
procedure TBaseLinkGridToDataSourceControlManager.DoUpdateColumn(const AColumn: TColumn;
  const ADescription: TCreateColumnDescription; const DefaultColumnWidth: Integer);
begin
  if AColumn = nil then
  ...
  try
    ...
    else if ADescription.TextWidth <> -1 then
      AColumn.Width := FGrid.TextWidthToColWidth(ADescription.TextWidth, ADescription.Header)
    else
      AColumn.Width := DefaultColumnWidth;
    AColumn.Visible := ADescription.Visible;
    AColumn.ReadOnly := ADescription.ReadOnly;
    AColumn.HorzAlign := AlignmentToTTextAlign[ADescription.Alignment];
    if AColumn is TCustomNumberColumn then
    begin
      TCustomNumberColumn(AColumn).ShowThousandSeparator := ADescription.ShowThousandSeparator;
      TCustomNumberColumn(AColumn).DecimalDigits := ADescription.DecimalDigits;
    end;
  finally
    AColumn.EndUpdate;
  end;
...
begin
  ...
  begin
    ...
    try
      ...
      if (ADescription.ColumnStyle = '') and not SupportsMemberType(LMemberType, LLiteralExpression) then
      begin
...
begin
  Result := [mtUnknown, mtObject, mtBitmap];
end;
{$ENDREGION}
