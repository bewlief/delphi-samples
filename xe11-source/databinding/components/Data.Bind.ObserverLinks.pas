...
type
  ...
  TBindIteratorLinkObserver = class(TBindMultiCastObserver, IIteratorLinkObserver)
  private
    FEnumerableBuffered: IScopeRecordEnumerableBuffered;
    FEnumerable: IScopeRecordEnumerable;
    FRecordEnum: IScopeRecordEnumerator;
    FRecordUpdator: IScopeRecordControlUpdator;
    constructor CreateBase(ABindLink: TActivatedContainedBindComponent);
  public
    constructor Create(ABindLink: TCustomBindGridLink); overload;
    constructor Create(ABindLink: TCustomBindListLink); overload;
    constructor Create(ABindLink: TCustomBindGridListLink); overload;
...
function TBindEditLinkObserver.GetFormatLink: IEditFormatLink;
begin
  Result := FBindLink.GetFormatLink;
end;
...
{ TBindIteratorLinkObserver }

constructor TBindIteratorLinkObserver.CreateBase(ABindLink: TActivatedContainedBindComponent);
var
  LBindLink: IBindLink;
  LEnumerableBuffered: IScopeRecordEnumerableBuffered;
  LEnumerable: IScopeRecordEnumerable;
  LRecordUpdator: IScopeRecordControlUpdator;
  LSup1, LSup2: Boolean;
begin
  if not Supports(ABindLink, IBindLink, LBindLink) then
    raise EObserverException.CreateFmt(sBindLinkIncompatible,
      [ABindLink.DisplayName]);
  // We need to get both EnumerableBuffered and Enumerable. Even if BindLink supports
  // EnumerableBuffered, the underlying data source may not, and that will be known
  // at GetEnumerator call.
  LSup1 := Supports(ABindLink, IScopeRecordEnumerableBuffered, LEnumerableBuffered);
  LSup2 := Supports(ABindLink, IScopeRecordEnumerable, LEnumerable);
  if not LSup1 and not LSup2 then
    raise EObserverException.CreateFmt(sScopeRecordEnumIncompatible,
      [ABindLink.DisplayName]);
  if not Supports(ABindLink, IScopeRecordControlUpdator, LRecordUpdator) then
    raise EObserverException.CreateFmt(sScopeRecordEnumIncompatible,
      [ABindLink.DisplayName]);
  inherited Create(LBindLink);
  FEnumerableBuffered := LEnumerableBuffered;
  FEnumerable := LEnumerable;
  FRecordUpdator := LRecordUpdator;
end;

constructor TBindIteratorLinkObserver.Create(ABindLink: TCustomBindGridLink);
begin
  CreateBase(ABindLink);
end;

constructor TBindIteratorLinkObserver.Create(ABindLink: TCustomBindListLink);
begin
  CreateBase(ABindLink);
end;

constructor TBindIteratorLinkObserver.Create(ABindLink: TCustomBindGridListLink);
begin
  CreateBase(ABindLink);
end;

procedure TBindIteratorLinkObserver.StartFrom(APosition: Integer);
begin
  FRecordEnum := nil;
  if FEnumerableBuffered <> nil then
    FRecordEnum := FEnumerableBuffered.GetEnumerator('', True);
  if FRecordEnum = nil then
    FRecordEnum := FEnumerable.GetEnumerator('', -1);
  FRecordEnum.First;
  while (APosition > 0) and FRecordEnum.MoveNext do
    Dec(APosition);
end;

function TBindIteratorLinkObserver.MoveNext: Boolean;
begin
  Result := FRecordEnum.MoveNext;
end;

procedure TBindIteratorLinkObserver.UpdateControlComponent(AControl: TComponent);
begin
  FRecordUpdator.UpdateControlComponent(AControl);
end;

procedure TBindIteratorLinkObserver.Finish;
begin
  FRecordEnum := nil;
end;
