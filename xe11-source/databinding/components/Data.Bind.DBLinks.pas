...
uses Data.Bind.Consts, System.Generics.Collections, System.Bindings.Methods,
...
procedure TBaseBindDBFieldLink.Reactivate;
begin
  DoReactivate(
...
begin
  Result := True;
  if GetDataSet <> nil then Result := lcAutomatic in GetDataSet.Fields.LifeCycles;
end;
...
begin    { Returns Nil if FieldName can't be found in dataset }
  ...
  if (FField = nil) and (FFieldName.Length > 0) and Assigned(Grid) and
    ...
    if Grid.Dataset.Active or (lcPersistent in Grid.Dataset.Fields.LifeCycles) then
...
procedure TBaseBindDBControlLink.UpdateColumns;
begin
  //
end;
