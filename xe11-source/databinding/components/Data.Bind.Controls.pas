...
var
  ...
  begin
    if FCanRefreshState = stUnknown then
      if ADataSourceEnabled and Self.CanRefresh then
        FCanRefreshState := stEnable
      else
        FCanRefreshState := stDisable;
    Result := FCanRefreshState = stEnable;
  end;
  ...
  function LastEnable: Boolean;
  begin
    if FLastState = stUnknown then
      if ADataSourceEnabled and (Selected and not Self.EOF) or ((not Selected) and not (BOF and EOF)) then
        FLastState := stEnable
      else
        FLastState := stDisable;
    Result := FLastState = stEnable;
  end;

  function FirstEnable: Boolean;
  begin
    if FFirstState = stUnknown then
      if ADataSourceEnabled and (Selected and not Self.BOF) or ((not Selected) and not (BOF and EOF)) then
        FFirstState := stEnable
      else
        FFirstState := stDisable;
    Result := FFirstState = stEnable;
  end;
var
...
begin
  FUpState := stUnknown;
  FDnState := stUnknown;
  FFirstState := stUnknown;
  FLastState := stUnknown;
  FCanModifyState := stUnknown;
  FCanRefreshState := stUnknown;
  FStateSelected := stUnknown;
  for LButton in AButtons do
  begin
    case LButton of
      nbFirst:
        ASetEnabled(nbFirst,  FirstEnable);
      nbPrior:
        ASetEnabled(nbPrior,  UpEnable);
      nbNext:
        ASetEnabled(nbNext,  DnEnable);
      nbLast:
        ASetEnabled(nbLast,  LastEnable);
      nbInsert:
        ASetEnabled(nbInsert, CanModify and Self.CanInsert);
      nbDelete:
        ASetEnabled(nbDelete, Selected and CanModify and Self.CanDelete);
      nbEdit:
      ...
      nbRefresh:
        ASetEnabled(nbRefresh, CanRefresh);
      nbApplyUpdates:
...
function TBindNavigatorController.GetCanRefresh: Boolean;
begin
  if Assigned(FScopeState) then
    Result := FScopeState.CanRefresh
  else
    Result := False;
end;
