...
uses
  ...
  FMX.Controls, FMX.StdCtrls, FMX.ExtCtrls, Data.Bind.Components, Data.Bind.Controls, FMX.Graphics,
...
{ TvgDBNavigator }
  ...
  private
    ...
    procedure SetSize(var W: single;var H: single); reintroduce;
    procedure SetVisibleButtons(Value: TBindNavButtonSet);
  ...
  public
    ...
    property VisibleButtons: TBindNavButtonSet read FVisibleButtons write SetVisibleButtons
...
uses System.Math, Data.Bind.Consts, FMX.Dialogs, FMX.DialogService;
...
//    Btn.NumGlyphs := 2;
    ...
    Btn.FPath.Width := 18;
    Btn.FPath.Height := 18;
    Btn.FPath.Align := TAlignLayout.Center;
    Btn.FPath.HitTest := false;
    Btn.FPath.Locked := true;
    Btn.FPath.Stored := false;
    Btn.FPath.Data.Data := BtnTypePath[I];
    Btn.FPath.WrapMode := TPathWrapMode.Fit;
    Btn.FPath.Stroke.Kind := TBrushKind.None;

    RasterEffect := TRasterEffect.Create(Self);
    RasterEffect.Parent := Btn.FPath;
  end;
...
procedure TCustomBindNavigator.SetVisibleButtons(Value: TBindNavButtonSet);
var
  I: TBindNavigateBtn;
  W, H: single;
begin
...
begin
  ...
  if Align = TAlignLayout.None then W := Temp;
  ...
  begin
    ...
    begin
      if X = 0 then
        Buttons[I].Corners := [TCorner.TopLeft] * FCorners + [TCorner.BottomLeft] * FCorners
      else
      if X > Width - (ButtonWidth * 1.5) then
        Buttons[I].Corners := [TCorner.TopRight] * FCorners + [TCorner.BottomRight] * FCorners
      else
        Buttons[I].Corners := [];
...
begin
  ...
  begin
    ...
    if (Index = TNavigateButton.nbDelete) and (FConfirmDelete) then
      TDialogService.MessageDialog(TranslateText(SDeleteRecordQuestion), TMsgDlgType.mtConfirmation,
        mbOKCancel, TMsgDlgBtn.mbOK, 0,
        procedure(const AResult: TModalResult)
        begin
          if AResult = mrOk then
            FController.ExecuteButton(Index, nil);
...
begin
  InvokeController(Target,
    procedure(AController: TBindNavigatorController)
    begin
      if (FButton = TNavigateButton.nbDelete) and (FConfirmDelete) then
        TDialogService.MessageDialog(TranslateText(SDeleteRecordQuestion), TMsgDlgType.mtConfirmation,
        mbOKCancel, TMsgDlgBtn.mbOK, 0,
          procedure(const AResult: TModalResult)
          begin
