...
type
  ...
  IDependencyList = Interface
    ...
    procedure Link(AComponent: TComponent; const ALocation: ILocation);
    function GetLink(const ALocation: ILocation): TComponent;
    procedure ClearLink(const ALocation: ILocation);
  End;
  ...
  private
    procedure AutoActivateOnPropertyChange;
  protected
  ...
  TBaseBindExpression = class(TActivatedContainedBindComponent)
  private
  ...
  IScopeRecordControlUpdator = interface
  ['{E961A08B-7FC3-4FE2-819C-79F3D0A230BE}']
  ...
  IBindTimeEditEditor = interface
  ['{21F446F9-5C99-47F2-8029-5D7BD0A40563}']
    function GetIsEmpty: Boolean;
    procedure SetIsEmpty(Value: Boolean);
    function GetTime: TTime;
    procedure SetTime(ATime: TTime);
    property IsEmpty: Boolean read GetIsEmpty write SetIsEmpty;
    property SelectedTime: TTime read GetTime write SetTime;
  end;
  ...
  IBindListEditor = interface(IBindListEditorCommon)
    ...
    function MoveNext: Boolean;
    procedure SetExpectedRowCount(ARowCount: Integer);
  ...
  IBindListRecordVirtualEditor = interface
    ['{3DE056BC-0DC9-40B0-AB32-2B4FE7AFDCF5}']
    procedure BeforeRecordUpdate;
    procedure AfterRecordUpdate;
  end;
  ...
  TCustomBindGridList = class(TActivatedContainedBindComponent)
  public
  type
...
{ TExpressions }
  ...
  ///  <remarks>If the direction is bidirectional then the control must support IEditLinkObserver</remarks>
  ...
  private
    ...
    function GetNotifyOutputs: Boolean;
    procedure SetManaged(const Value: Boolean);
    procedure SetNotifyOutputs(const Value: Boolean);
  protected
    procedure Reactivate; override;
  ...
  public
    ...
    property NotifyOutputs: Boolean read GetNotifyOutputs write SetNotifyOutputs default False;
    property Track: Boolean read GetTrack write SetTrack default False;
  end;
...
{ TBindArtifacts }
  ...
  public
    const
      ...
      IDIteratorLinkObserver = Integer(TObserverMapping.IteratorLinkID);
  private
...
type
  ...
  TValuePropertyNameRec = record
    ...
    FAuto: Boolean;
    constructor Create(const APropertyName, AFrameworkExt: string;
      AOptions: TValuePropertyOptions; AAuto: Boolean);
  end;
...
var
  ...
  FValuePropertyNamesAutoLoaded: Integer;
  FListItemValueExpressions: TDictionary<TClass, TListItemExpressionRec>;

constructor TValuePropertyNameRec.Create(const APropertyName: string;
  const AFrameworkExt: string; AOptions: TValuePropertyOptions; AAuto: Boolean);
begin
...
function GetControlValuePropertyName(AComponentClass: TClass; out APropertyName: string; out AOptions: TValuePropertyOptions): Boolean; overload;
var
...
begin
  Result := False;
  if (AComponentClass = nil) or not AComponentClass.InheritsFrom(TPersistent) then
    Exit;

  LClass := AComponentClass;
  while (LClass <> TPersistent) and
        not FValuePropertyNames.TryGetValue(LClass, LValuePropertyNameRec) do
    LClass := LClass.ClassParent;

  if LClass <> TPersistent then
  begin
  ...
  LObservableMemberAttribute := GetObservableMemberAttribute(AComponentClass);
  ...
  begin
    ...
    if AComponentClass.Unitname.StartsWith('VCL.', True) then
      LFramework := 'DFM'
    else if AComponentClass.Unitname.StartsWith('FMX.', True) then
      LFramework := 'FMX'
    else
      LFramework := '';
    if LObservableMemberAttribute.Track then
      Include(AOptions, TValuePropertyOption.vpTrack);
    if not LObservableMemberAttribute.Framework.IsEmpty then
      LFramework := LObservableMemberAttribute.Framework;
    FValuePropertyNames.Add(AComponentClass,
      TValuePropertyNameRec.Create(APropertyName, LFramework, AOptions, True));
    Exit(True);
  end;
end;
...
begin
  Result := GetControlValuePropertyName(AComponent.ClassType, APropertyName, AOptions);
end;
...
var
  ...
  LType: TRttiType;
  LProp: string;
  LOpts: TValuePropertyOptions;
  LClass: TClass;
  LValuePropertyNameRec: TValuePropertyNameRec;
begin
  if FValuePropertyNamesAutoLoaded = -1 then
  begin
    for LClass in FValuePropertyNames.Keys.ToArray do
      if FValuePropertyNames.TryGetValue(LClass, LValuePropertyNameRec) and
         LValuePropertyNameRec.FAuto then
        FValuePropertyNames.Remove(LClass);
    FValuePropertyNamesAutoLoaded := 0;
  end;

  if FValuePropertyNamesAutoLoaded = 0 then
  begin
    for LType in LCtx.GetTypes do
      if LType is TRttiInstanceType then
        GetControlValuePropertyName(TRttiInstanceType(LType).MetaclassType, LProp, LOpts);
    FValuePropertyNamesAutoLoaded := 1;
  end;
...
begin
  if GetControlComponent = nil then
    raise TBindCompException.CreateResFmt(@sNoControl, [Self.GetDisplayName]);
end;
...
procedure TContainedBindComponent.DoReactivate(AIsActive: TFunc<Boolean>;
  ASetActive: TProc<Boolean>; ASetAutoActivate: TProc<Boolean>);
begin
  Assert(not Loading);
  if AIsActive() then
...
begin
  ...
  if Operation = opRemove then
    if (AComponent is TContainedBindComponent) and Assigned(FBindComps) then
...
var
  LScopeComp: IScopeComponent;
  LObj: TObject;
  LIntf: IBindingNotifier;
begin
  if Supports(AObject, IScopeComponent, LScopeComp) then
    LObj := LScopeComp.GetScopeObject
  else
    LObj := AObject;
  LIntf := TBindings.CreateNotifier(LObj, FBindingsManager);
  LIntf.Notify(AProperty);
end;
...
var
  LComp: TContainedBindComponent;
begin
  if FUseAppManager <> Value then
  begin
    if Value then
    begin
    ...
    for LComp in FBindComps do
      if LComp is TActivatedContainedBindComponent then
        TActivatedContainedBindComponent(LComp).Active := False;
  end;
end;
...
begin
  if FBindComps.Extract(ABindComp) <> nil then
  begin
...
begin
  ...
  begin
    if (Value <> nil) and (Value = FSourceComponent) then
      raise TBindCompException.CreateResFmt(@sSameSourceAndControlComponent, [GetDisplayName]);
...
begin
  if (AComponent = nil) or (csDestroying in AComponent.ComponentState) then
    Exit(nil);
...
begin
  if Assigned(BindingsList) then
    ...
    Result := TValueRefConverterFactory.GetOutputConverter;
end;
...
type
  ...
  private
    ...
    FCollType: TRttiType;
    FCollGetEnum: TRttiMethod;
    FEnumerator: TValue;
    FEnumerating: Boolean;
    FEnumType: TRttiType;
    FEnumCurr: TRttiProperty;
    FEnumNext: TRttiMethod;
    FCurrObject: TObject;
    procedure CheckEnumerator;
    procedure Reset;
  public
    constructor Create(AObject: TObject; const AParentScope: IScope = nil);
    destructor Destroy; override;
    procedure First;
    function GetCurrent: IScope;
    function GetMemberCurrent(const AMemberName: string): IScope;
    ...
    function GetRecordCount: Integer;
    //
    property Current: TObject read FCurrObject;
  end;
...
begin
  if FExpressions <> nil then
    if FExpressions.Remove(AExpression) >= 0 then
            ...
            if not Designing then
              ...
              begin
                ...
                if not FControlObserver.TrySetActive(True) then
                  raise TBindCompException.CreateResFmt(@sNoControlObserverSupport, [Self.GetDisplayName, SafeClassName(ControlComponent)]);
              end;
            EvaluateFormat('');
          finally
            DoOnActivated;
...
function TCustomBindLink.GetFormatLink: IEditFormatLink;
var
  LScopeEditor: IScopeEditor;
begin
  Result := nil;
  if Supports(SourceComponent, IScopeEditor, LScopeEditor) then
    Result := LScopeEditor.GetFormatLink(SourceMemberName);
end;
...
function TBindLink.CanActivate: Boolean;
begin
  Result := inherited CanActivate and (
    (ParseExpressions.Count > 0) or (FormatExpressions.Count > 0) or (ClearExpressions.Count > 0));
end;
...
begin
  ...
  if not TryGetBindListEditor(Result) then
    if Editor = '' then
      raise TBindCompException.CreateResFmt(@sNoListEditor, [Self.GetDisplayName, SafeClassName(ControlComponent)])
    else
      raise TBindCompException.CreateResFmt(@sNoNamedListEditor, [Self.GetDisplayName, SafeClassName(ControlComponent), Editor]);
end;
...
begin
  if not Supports(GetBindListEditor, IBindListEditor, LEditor) then
    raise TBindCompException.CreateResFmt(@sNoInsertItem, [Self.GetDisplayName, ControlComponent.ClassName]);
  if LEditor <> nil then
  begin
    ...
    begin
      ...
      raise TBindCompException.CreateResFmt(@sNoInsertItem, [Self.GetDisplayName, SourceComponent.ClassName]);
  end
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      raise TBindCompException.CreateResFmt(@sNoEnumerator, [Self.GetDisplayName, SourceComponent.ClassName]);
  end
end;
...
function TCustomBindPosition.CanActivate: Boolean;
begin
  Result := inherited CanActivate and (
    (PosControlExpressions.Count > 0) or (PosSourceExpressions.Count > 0) or (PosClearExpressions.Count > 0));
end;
...
procedure TCustomBindControlValue.SetManaged(const Value: Boolean);
begin
  if FManaged <> Value then
  begin
    SetActive(False);
...
procedure TCustomBindControlValue.SetNotifyOutputs(const Value: Boolean);
begin
  if FNotifyOutputs <> Value then
  begin
    SetActive(False);
...
begin
  ...
  LControlScopes := inherited GetComponentScopes(ControlComponent);
  LSourceScopes := inherited GetComponentScopes(SourceComponent, SourceMemberName);
  ...
  begin
    ...
    begin
      LBindingExpression := TBindings.CreateManagedBinding(
        LControlScopes, // input
        LExpressionItem.ControlExpression,
        LSourceScopes,  // output
        LExpressionItem.SourceExpression,
        LOutputConverters,
        TBindingEventRec.Create(DoOnEvalError, DoOnAssigningValue,  DoOnAssignedValue, DoOnLocationUpdated),
        LManager,
...
begin
  ...
  begin
    ...
    FValuePropertyNames.Add(AClass, TValuePropertyNameRec.Create(APropertyName, AFrameworkExt, [vpObservable], False));
end;
...
begin
  ...
  begin
    ...
    FValuePropertyNames.Add(AClass, TValuePropertyNameRec.Create('', '', LValuePropertyOptions, False));
end;
...
begin
  if not FValuePropertyNames.ContainsKey(AClass) then
    FValuePropertyNames.Add(AClass, TValuePropertyNameRec.Create(APropertyName, AFrameWorkExt, [], False)); // Not observerable
end;
...
begin
  if AName = '' then
    ...
    raise EBindCompError.CreateResFmt(@sUnknownEditor, [AName]);
end;
...
begin
  ...
  if not TryGetBindListEditor(Result) then
    if ListItemStyle = '' then
      ...
      raise TBindCompException.CreateResFmt(@sNoNamedListEditor, [Self.GetDisplayName, SafeClassName(ControlComponent), ListItemStyle]);
end;
...
begin
  CheckControlComponent;
  if not TryGetBindListEditor(Result) then
    raise TBindCompException.CreateResFmt(@sNoListEditor, [Self.GetDisplayName, ControlComponent.ClassName]);
end;
...
var
  ...
  LEditor: IBindListEditor;
  LScopeLocks: IScopeLocks;
  LBindListRecordVirtualEditor: IBindListRecordVirtualEditor;
begin
  if not Active then
    Exit;
  ...
  begin
    ...
    begin
      Supports(LEditorCommon, IBindListRecordVirtualEditor, LBindListRecordVirtualEditor);
      if LBindListRecordVirtualEditor <> nil then
        LBindListRecordVirtualEditor.BeforeRecordUpdate;
      try
        UpdateRow(LBindListVirtualEditor);
      finally
        if LBindListRecordVirtualEditor <> nil then
          LBindListRecordVirtualEditor.AfterRecordUpdate;
      end;
    end
    else if Supports(LEditorCommon, IBindListEditor, LEditor) then
    begin
...
begin
  ...
  FControlObserver.EnsureObserving(TControlObserver.IDIteratorLinkObserver,
    function: IObserver
    begin
      Result := TBindIteratorLinkObserver.Create(Self);
    end);
end;
...
begin
  ...
  LFieldName := UpdateColumnCurrent;
  if LFieldName = '' then
    raise TBindCompException.CreateResFmt(@sLinkFieldNotFound, [GetDisplayName]);
  if Supports(SourceComponent, IScopeEditor, LScopeEditor) then
    Result := LScopeEditor.IsValidChar(LFieldName, AKey);
end;

function TCustomBindGridLink.GetFormatLink: IEditFormatLink;
var
  LScopeEditor: IScopeEditor;
  LFieldName: string;
begin
  ...
  if LFieldName = '' then
    raise TBindCompException.CreateResFmt(@sLinkFieldNotFound, [GetDisplayName]);
  if Supports(SourceComponent, IScopeEditor, LScopeEditor) then
    Result := LScopeEditor.GetFormatLink(LFieldName);
end;
...
begin
  ...
  begin
    ...
    case FEditColumnCurrent.VType of
      vtInteger:
        if (FEditColumnCurrent.VInteger >= 0) and (FEditColumnCurrent.VInteger < ColumnExpressions.Count) then
        begin
          LItem := ColumnExpressions[FEditColumnCurrent.VInteger];
          FEditColumnIndex := TColumnLinkExpressionItem(LItem).Index;
          Result := TColumnLinkExpressionItem(LItem).SourceMemberName;
        end;
      else
        raise TBindCompException.CreateResFmt(@sLinkUnexpectedGridCurrentType, [GetDisplayName]);
    end;
  end;
end;
...
function TCustomBindGridLink.GetEnumerator(const AMemberName: string;
  ABuffered: Boolean): IScopeRecordEnumerator;
var
  LScopeRecordEnum: IScopeRecordEnumerableBuffered;
begin
  if Supports(SourceComponent, IScopeRecordEnumerableBuffered, LScopeRecordEnum) then
    Result := LScopeRecordEnum.GetEnumerator(AMemberName, ABuffered)
  else
    Result := nil;
end;
...
var
  LScopeRecordEnum: IScopeRecordEnumerable;
begin
  if Supports(SourceComponent, IScopeRecordEnumerable, LScopeRecordEnum) then
    Result := LScopeRecordEnum.GetEnumerator(AMemberName, ABufferCount)
  else
    Result := nil;
end;

procedure TCustomBindGridLink.UpdateControlComponent(AControl: TComponent);
var
  LScopeRecordControlUpdator: IScopeRecordControlUpdator;
begin
  if Supports(SourceComponent, IScopeRecordControlUpdator, LScopeRecordControlUpdator) then
    LScopeRecordControlUpdator.UpdateControlComponent(AControl);
end;
...
begin
  Result := inherited CanActivate and
    (FControlExpression <> '') and (FSourceExpression <> '');
end;
...
function TCustomBindExprItems.CanActivate: Boolean;
begin
  Result := inherited CanActivate and (
    (FormatExpressions.Count > 0) or (ClearExpressions.Count > 0));
end;
...
function TCustomBindListLink.GetFormatLink: IEditFormatLink;
var
  LScopeEditor: IScopeEditor;
  LFieldName: string;
begin
...
function TCustomBindGridListLink.GetFormatLink: IEditFormatLink;
var
  LScopeEditor: IScopeEditor;
  LFieldName: string;
begin
...
begin
  ...
  begin
    if Value then
    begin
      if GetAutoActivate and CanActivate then
        SetActive(True)
    end
    else
      SetActive(False)
...
procedure TActivatedContainedBindComponent.AutoActivateOnPropertyChange;
var
  LScopeActive: IScopeActive;
begin
  ...
  begin
    ...
    if (LScopeActive = nil) or (LScopeActive.Active) then
      if (not Active) and (not Loading) and AutoActivate then
        if CanActivate then
          SetActive(True);
  end;
end;
...
begin
  if (not Loading) and CanActivate then
...
constructor TEnumerableWrapper.Create(AObject: TObject; const AParentScope: IScope);
begin
  inherited Create;
  FObject := AObject;
  FParentScope := AParentScope;
  FCollType := FContext.GetType(FObject.ClassType);
  FCollGetEnum := FCollType.GetMethod('GetEnumerator');
end;
...
procedure TEnumerableWrapper.Reset;
begin
  if FEnumerator.IsObjectInstance then
    FEnumerator.AsObject.Free;
  FEnumerator := TValue.Empty;
  FEnumerating := False;
  FEnumType := nil;
  FEnumNext := nil;
  FEnumCurr := nil;
...
procedure TEnumerableWrapper.CheckEnumerator;
var
  LValue: TValue;
begin
  if FEnumerator.IsEmpty and Assigned(FCollGetEnum) then
  begin
    LValue := FCollGetEnum.Invoke(FObject, []);
    if not LValue.IsEmpty and (LValue.Kind in [tkClass, tkRecord, tkMRecord]) then
    begin
      FEnumerator := LValue;
      FEnumType := FContext.GetType(FEnumerator.TypeInfo);
      if FEnumType <> nil then
      begin
        FEnumNext := FEnumType.GetMethod('MoveNext');
        FEnumCurr := FEnumType.GetProperty('Current');
      end;
    end;
  end;
end;
...
var
  LEnum: IScopeRecordEnumerator;
begin
  Result := 0;
  LEnum := TEnumerableWrapper.Create(FObject, FParentScope) as IScopeRecordEnumerator;
...
begin
  ...
  FCurrObject := nil;
  CheckEnumerator;
  if (FEnumNext = nil) or (FEnumCurr = nil) then
    Exit;
  try
    LValue := FEnumNext.Invoke(FEnumerator, []);
    LValue.TryAsType<Boolean>(Result);
    if Result then
    begin
      LValue := FEnumCurr.GetValue(PPointer(FEnumerator.GetReferenceToRawData)^);
      if LValue.IsObjectInstance then
        FCurrObject := LValue.AsObject;
    end;
  finally
    if Result then
      FEnumerating := True;
...
begin
  Result := inherited CanActivate and not SourceMemberName.IsEmpty and
...
begin
  ...
  if LookupDataSource <> nil then
    raise TBindCompException.CreateRes(@sNoBidiLookup);
end;
        ...
        begin
          ...
          begin
            ...
            if not GetFieldValueMemberNames(BindList.SourceComponent, LFillExpressionItem.SourceMemberName,
              ...
              raise TBindCompException.CreateResFmt(@sMemberNotFound, [LFillExpressionItem.SourceMemberName,
...
procedure TCustomLinkFillControlToProperty.SetFillValueFieldName(const Value: string);
begin
  FBindList.LookupValueSourceMemberName := Value;
end;
...
begin
  Result := (Component <> nil) and (ComponentProperty <> '') and
    (DataSource <> nil) and (FieldName <> '');
  if Result then
    Result := CheckActiveScope(LookupDataSource)
end;
...
var
  LDeleg: TCommonBindComponent;
begin
  LDeleg := GetDelegates[0];
  if (LDeleg <> nil) and (LDeleg.SourceComponent <> nil) then
    Result := LDeleg.SourceComponent as TBaseLinkingBindSource
  else
    Result := nil;
end;
...
begin
  ...
  if LDeleg = nil then
    ...
    Result := LDeleg.SourceMemberName;
end;
...
begin
  LDeleg := GetDelegates[0];
  if LDeleg.SourceComponent <> nil then
    Result := LDeleg.SourceComponent as TBaseLinkingBindSource
  else
    Result := nil;
end;
...
var
  LDeleg: TCommonBindComponent;
begin
  if FComponentProperty <> Value then
  begin
    LDeleg := GetDelegates[0];
    LDeleg.UpdateControlChanging;
    FComponentProperty := Value;
    LDeleg.UpdateControlChanged;
  end;
end;
...
function TCustomLinkControlToProperty.GetManaged: Boolean;
begin
  Result := FBindLink.Managed;
end;

function TCustomLinkControlToProperty.GetNotifyOutputs: Boolean;
begin
  Result := FBindLink.NotifyOutputs;
end;
...
procedure TCustomLinkControlToProperty.SetManaged(const Value: Boolean);
begin
  FBindLink.Managed := Value;
end;

procedure TCustomLinkControlToProperty.SetNotifyOutputs(const Value: Boolean);
begin
  FBindLink.NotifyOutputs := Value;
end;
...
procedure TDependencyList.ClearLink(const ALocation: ILocation);
begin
...
function TDependencyList.GetLink(const ALocation: ILocation): TComponent;
begin
  FLocationDict.TryGetValue(ALocation, Result);
end;

procedure TDependencyList.Link(AComponent: TComponent; const ALocation: ILocation);
begin
  FLocationDict.AddOrSetValue(ALocation, AComponent);
end;

procedure TDependencyList.Notify(ASender: TObject; const ALocation: ILocation;
  const AExpr: string);
var
  LComp: TComponent;
begin
  if FLocationDict.TryGetValue(ALocation, LComp) then
    Notify(ASender, LComp, AExpr);
end;

procedure TDependencyList.Notify(ASender: TObject; const AComponent: TComponent; const AValue: string);
var
    ...
    begin
      ...
      begin
        ...
        if LValue <> nil then
         ...
         raise TBindCompException.CreateResFmt(@sErrorNoLookupSupport, [Self.GetDisplayName, SafeClassName(ControlComponent)]);
    ...
    if Self.ControlComponent <> nil then
      if Self.TryGetBindListEditor(LEditorCommon) then
        if not Supports(LEditorCommon, IBindListLookupEditor) then
          raise TBindCompException.CreateResFmt(@sNoLookupSupport, [Self.GetDisplayName, SafeClassName(ControlComponent)]);
end;
...
begin
  if IsUnidirectionalDataSource(ADataSource) then
    raise TBindCompException.CreateResFmt(@SDataSourceUnidirectional, [ABindComponent.GetDisplayName]);
end;
