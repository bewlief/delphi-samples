...
var
  LDateTimeEditor: IBindDateTimeEditEditor;
  LTimeEditor: IBindTimeEditEditor;
begin
  Result := TValue.Empty;
  if Supports(GetBindEditor(AObject, IBindTimeEditEditor), IBindTimeEditEditor, LTimeEditor) then
  begin
    if not LTimeEditor.IsEmpty then
      Result := TValue.From<TTime>(LTimeEditor.SelectedTime);
  end
  else if Supports(GetBindEditor(AObject, IBindDateTimeEditEditor), IBindDateTimeEditEditor, LDateTimeEditor) then
  begin
    if not LDateTimeEditor.IsEmpty then
      Result := TValue.From<TDateTime>(LDateTimeEditor.SelectedDateTime);
  end;
end;
...
procedure SetTimeValue(const ATimeEditor : IBindTimeEditEditor; const AValue: TValue);
var
  LDateTime: TDateTime;
begin
  if (AValue.ToString = '') or (AValue.IsEmpty) then
    ATimeEditor.IsEmpty := True
  else if TryStrToTime(AValue.ToString, LDateTime) then
  begin
    ATimeEditor.SelectedTime := LDateTime;
    ATimeEditor.IsEmpty := False;
  end
  else
    raise Exception.Create(AValue.ToString + ' Invalid Time');
end;

procedure SetDateTimeValue(const ADateTimeEditor : IBindDateTimeEditEditor; const AValue: TValue);
var
  LDateTime: TDateTime;
begin
  if (AValue.ToString = '') or (AValue.IsEmpty) then
    ADateTimeEditor.IsEmpty := True
  else if TryStrToDateTime(AValue.ToString, LDateTime) then
  begin
    ADateTimeEditor.SelectedDateTime := LDateTime;
    ADateTimeEditor.IsEmpty := False;
  end
  else
...
begin
  ...
  begin
    if Supports(GetBindEditor(AObject, IBindTimeEditEditor), IBindTimeEditEditor, LTimeEditor) then
      SetTimeValue(LTimeEditor, AValue)
    else if Supports(GetBindEditor(AObject, IBindDateTimeEditEditor), IBindDateTimeEditEditor, LDateTimeEditor) then
      SetDateTimeValue(LDateTimeEditor, AValue);
  end;
end;
...
begin
  ...
  TBindingMethodsFactory.UnRegisterMethod(sIDListItemIndex);
end;
