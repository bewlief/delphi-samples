...
uses 
  System.Classes, Data.Bind.Components,
    System.RTTI,
    System.Generics.Collections, System.SysUtils,  Data.Bind.ObjectScope,
...
type
  ...
  private
    class function GetLoremIpsum: TArray<string>; static;
    class function GetIntegers: TArray<Integer>; static;
    class function GetUIntegers: TArray<UInt32>; static;
    class function GetSingles: TArray<Single>; static;
    class function GetCurrency: TArray<Currency>; static;
    class function GetDateTime: TArray<TDateTime>; static;
    class function GetDate: TArray<TDate>; static;
    class function GetTime: TArray<TTime>; static;
  protected
    function CreateDelegate: TValueGeneratorDelegate; override;
  end;
  ...
  private
    function MakeStrings(AIndex: Integer; const AValues: TArray<string>): TStrings;
  protected
    function CreateDelegate: TValueGeneratorDelegate; override;
  end;
...
class function TSimpleValueGenerator.GetLoremIpsum: TArray<string>;
var
  I, J: Integer;
begin
...
class function TSimpleValueGenerator.GetIntegers: TArray<Integer>;
var
  I: Integer;
begin
  SetLength(Result, 201);
  for I := -100 to 100 do
    Result[I + 100] := I;
end;

class function TSimpleValueGenerator.GetUIntegers: TArray<UInt32>;
var
  I: Integer;
begin
  SetLength(Result, 201);
...
class function TSimpleValueGenerator.GetSingles: TArray<Single>;
var
  I: Integer;
begin
  SetLength(Result, 201);
...
class function TSimpleValueGenerator.GetCurrency: TArray<Currency>;
var
  I: Integer;
begin
  SetLength(Result, 201);
...
class function TSimpleValueGenerator.GetDateTime: TArray<TDateTime>;
var
  I: Integer;
  LDateTime: TDateTime;
begin
...
class function TSimpleValueGenerator.GetDate: TArray<TDate>;
var
  LDateTimes: TArray<TDateTime>;
  I: Integer;
begin
  LDateTimes := GetDateTime;
  SetLength(Result, Length(LDateTimes));
  for I := 0 to Length(LDateTimes) - 1 do
    Result[I] := DateOf(LDateTimes[I]);
end;

class function TSimpleValueGenerator.GetTime: TArray<TTime>;
var
  LDateTimes: TArray<TDateTime>;
  I: Integer;
begin
...
begin
  ...
  begin
    case FieldType of
      ftString:
        Result := TTypedListValueGeneratorDelegate2<string, uint32>.Create(Options, GetUIntegers);
      ftInteger:
        Result := TTypedListValueGeneratorDelegate2<Integer, uint32>.Create(Options, GetUIntegers);
      ftUInteger:
        Result := TTypedListValueGeneratorDelegate<uint32>.Create(Options, GetUIntegers);
      ftSingle:
        Result := TTypedListValueGeneratorDelegate2<Single, uint32>.Create(Options, GetUIntegers);
    else
      Assert(False);
    end
  end
  ...
  begin
    case FieldType of
      ftString:
        Result := TTypedListValueGeneratorDelegate2<string, Integer>.Create(Options, GetIntegers);
      ftInteger:
        Result := TTypedListValueGeneratorDelegate<Integer>.Create(Options, GetIntegers);
      ftSingle:
        Result := TTypedListValueGeneratorDelegate2<Single, Integer>.Create(Options, GetIntegers);
      ftUInteger:
        Result := TTypedListValueGeneratorDelegate2<UInt32, Integer>.Create(Options, GetIntegers);
    else
      Assert(False);
    end;
  end
  ...
  begin
    case FieldType of
      ftString:
        Result := TTypedListValueGeneratorDelegate2<string, single>.Create(Options, GetSingles);
      ftSingle:
        Result := TTypedListValueGeneratorDelegate<Single>.Create(Options, GetSingles);
    else
      Assert(False);
    end;
  end
  ...
  begin
    case FieldType of
      ftString:
        Result := TTypedListValueGeneratorDelegate2<string, boolean>.Create(Options, TArray<Boolean>.Create(false, true));
      ftBoolean:
        Result := TTypedListValueGeneratorDelegate<Boolean>.Create(Options, TArray<Boolean>.Create(false, true));
    else
      Assert(False);
    end;
  end
  ...
  begin
    case FieldType of
      ftString:
        Result := TTypedListValueGeneratorDelegate<string>.Create(Options, GetLoremIpsum);
    else
      Assert(False);
    end;
  end
  ...
  begin
    case FieldType of
      ftString:
        Result := TTypedListValueGeneratorDelegate2<string, Currency>.Create(Options, GetCurrency);
      ftCurrency:
        Result := TTypedListValueGeneratorDelegate<Currency>.Create(Options, GetCurrency);
    else
      Assert(False);
    end;
  end
  ...
  begin
    case FieldType of
      ftString:
        Result := TTypedListValueGeneratorDelegate2<string, TDateTime>.Create(Options, GetDateTime);
      ftDateTime:
        Result := TTypedListValueGeneratorDelegate<TDateTime>.Create(Options, GetDateTime);
    else
      Assert(False);
    end;
  end
  ...
  begin
    case FieldType of
      ftString:
        Result := TTypedListValueGeneratorDelegate2<string, TTime>.Create(Options, GetTime);
      ftTime:
        Result := TTypedListValueGeneratorDelegate<TTime>.Create(Options, GetTime);
    else
      Assert(False);
    end;
  end
  ...
  begin
    case FieldType of
      ftString:
        Result := TTypedListValueGeneratorDelegate2<string, TDate>.Create(Options, GetDate);
      ftDate:
        Result := TTypedListValueGeneratorDelegate<TDate>.Create(Options, GetDate);
    else
      Assert(False);
    end;
  end
...
begin
  ...
  case FieldType of
    ftString:
    begin
      SetLength(LStrings, Length(CAlphaColorValues));
      for I := Low(CAlphaColorValues) to High(CAlphaColorValues) do
        LStrings[I] := UIntToStr(CAlphaColorValues[I]);
      Result := TTypedListValueGeneratorDelegate<string>.Create(Options, LStrings);
    end;
    ftUInteger:
      Result := TTypedListValueGeneratorDelegate<TAlphaColor>.CreateFromOA(Options, CAlphaColorValues);
  else
    Assert(False);
  end;
end;
...
begin
  ...
  case FieldType of
    ftString:
    begin
      SetLength(LStrings, Length(CColorValues));
      for I := Low(CColorValues) to High(CColorValues) do
        LStrings[I] := IntToStr(CColorValues[I]);
      Result := TTypedListValueGeneratorDelegate<string>.Create(Options, LStrings);
    end;
    ftUInteger:
      Result := TTypedListValueGeneratorDelegate<TColor>.CreateFromOA(Options, CColorValues);
  else
    Assert(False);
  end;
end;
...
begin
  ...
  case FieldType of
    ftString:
      Result := TTypedListValueGeneratorDelegate<string>.CreateFromOA(Options, CColorNames);
  else
    Assert(False);
  end;
end;
...
begin
  ...
  case FieldType of
    ftString:
      Result := TTypedListValueGeneratorDelegate<string>.CreateFromOA(Options, cPaths);
  else
    Assert(False);
  end;
end;
...
begin
  ...
  case FieldType of
    ftString:
      Result := TTypedListValueGeneratorDelegate<string>.CreateFromOA(Options, cPathNames);
  else
    Assert(False);
  end;
end;
...
begin
  ...
  case FieldType of
    ftString:
    begin
      SetLength(LStrings, cCount);
      for I := 0 to cCount - 1 do
        LStrings[I] := Format('Strings%d', [I + 1]);
      Result := TTypedListValueGeneratorDelegate<string>.Create(Options, LStrings);
    end;
    ftTStrings:
    begin
      SetLength(LIntegers, cCount);
      for I := 0 to cCount - 1 do
        LIntegers[I] := I + 1;
      LStrings := TSimpleValueGenerator.GetLoremIpsum;
      Result := TTypedListValueGeneratorDelegate<Integer>.Create(Options, LIntegers,
...
function TStringsGenerator.MakeStrings(AIndex: Integer; const AValues: TArray<string>): TStrings;
var
  I: Integer;
  LText: string;
  LCount: Integer;
  ...
  case FieldType of
    ftString:
      if SameText(GeneratorName, sContactNames) then
        Result := TTypedListValueGeneratorDelegate<string>.CreateFromOA(Options, cContactNames)
      else if SameText(GeneratorName, sContactTitles) then
        Result := TTypedListValueGeneratorDelegate<string>.CreateFromOA(Options, cContactTitles);
  else
    Assert(False);
  end;
end;
...
begin
  ...
  RegisterValueGenerator(sFloats, [ftSingle, ftString],
    TValueGeneratorDescription.Create(TSimpleValueGenerator, sFloatField, sThisUnit));
  RegisterValueGenerator(sBooleans, [ftBoolean, ftString],
  ...
  RegisterValueGenerator(sCurrency, [ftCurrency, ftString],
