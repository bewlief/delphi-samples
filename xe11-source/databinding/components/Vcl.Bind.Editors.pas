...
uses
  ...
  Vcl.Controls, System.Generics.Collections, System.Rtti, System.SysUtils, System.UITypes;
...
type
  ...
  TBaseBindListListBoxEditor = class(TBindListLookupEditorImpl, IBindListEditorEvalShortcut)
  private
  ...
  public
    ...
    /// <summary>Bind to the imageindex of the current list view fitem</summary>
...
uses System.Math, System.StrUtils, Vcl.Bind.Consts,
...
const
  sRow = 'Row';
  sListItemIndex = 'ListItemIndex(Self)';
...
procedure SetCurrentListBoxEditorValue(const AEditor: TBaseBindListListBoxEditor; const AMemberName: string;
  const AValue: TValue; AType: TScopeMemberType);
begin
  if AMemberName = 'Text' then
    AEditor.FEditorObject.Text := AValue.ToString
end;
...
function TBaseBindListListBoxEditor.CanSetCurrentValue(const AMemberName: string; out AType: TScopeMemberType): Boolean;
begin
  Result := CanSetCurrentListBoxEditorValue(AMemberName, AType);
end;

procedure TBaseBindListListBoxEditor.SetCurrentValue(const AMemberName: string;
  const AValue: TValue; AType: TScopeMemberType);
begin
  if FEditorObject.FIndex = -1 then
    FEditorObject.FIndex := FEditorObject.ControlItemIndex;
  SetCurrentListBoxEditorValue(Self, AMemberName, AValue, AType);
end;
...
begin
  ...
  begin
    Result := (FUpdateCounter = 0) // While is being updated the row is empty (Text = '')
      and (FEditorObject.FStringGrid.RowCount = FEditorObject.FStringGrid.FixedRows + 1)
...
begin
  if AIndex < FEditorObject.FStringGrid.RowCount then
    FEditorObject.FStringGrid.Row := AIndex;
end;
...
begin
  Result := FEditorObject.FStringGrid.Row;
end;
...
begin
  ASetter := sListItemIndex;    // Will call Self.SetItemIndex
  ABase := 1;
  Exit(True);
end;
...
const
  sCellsPrefix = 'Cells[';
  sSubitemsPrefix = 'SubItems.Strings[';

function TryParseCellExpression(const APrefix: string; const AMemberName: string; out AIndex: Integer): Boolean;
var
  L, Col: Integer;
  C: Char;
  P: PChar;
begin
  L := Length(APrefix);
  Result := StrLComp(PChar(AMemberName), PChar(APrefix), L) = 0;
  if Result then
  begin
    P := PChar(AMemberName) + L;
    Col := 0;
    while P <> #0 do
    begin
      C := P^;
...
begin
  if FEditorObject.FIndex = -1 then
    FEditorObject.FIndex := Max(1, FEditorObject.FStringGrid.Row);
  if TryParseCellExpression(sCellsPrefix, AMemberName, Col) then
  begin
    if Col < FEditorObject.FStringGrid.ColCount then
      FEditorObject.Cells[Col] := AValue.ToString
    else
      raise Exception.CreateRes(@sColOutOfRange);
  end;
...
begin
  AType := mtText;
  Result := TryParseCellExpression(sCellsPrefix, AMemberName, Col);
end;
...
function TBindListListViewEditor.GetPositionSetter(var ASetter: string; var ABase: Integer): Boolean;
begin
  Result := inherited;
  ASetter := sListItemIndex;  // Will call Self.SetItemIndex
end;

procedure TBindListListViewEditor.SetItemIndex(AIndex: Integer);
begin
  if AIndex <  FEditorObject.FListView.Items.Count then
...
type
  ...
  TListViewItemNames = record
  const
    Caption = 'Caption';
    FormatSubItemName = 'SubItems[%d]';
    FormatSubItemExpression = 'SubItems.Strings[%d]';  // Default properties not supported by expression engine
    Header = 'Header';
    Subtitle = 'Subtitle';
    Footer = 'Footer';
  end;
...
function TBindListListViewEditor.GetItemIndex: Integer;
begin
...
begin
  for I := 0 to TListViewCracker(FEditorObject.FListView).Columns.Count - 2 do
    if SameText(AName, Format(TListViewItemNames.FormatSubItemName, [I])) then
      Exit(Format(TListViewItemNames.FormatSubItemExpression, [I]));
  Result := AName;
end;
...
var
  I: Integer;
begin
  ANames.Add(TListViewItemNames.Caption);
  for I := 0 to TListViewCracker(FEditorObject.FListView).Columns.Count - 2 do
    ANames.Add(Format(TListViewItemNames.FormatSubItemName, [I]))
end;
...
begin
  ANames.Clear;
  ANames.Add(TListViewItemNames.Header);
  ANames.Add(TListViewItemNames.Subtitle);
  ANames.Add(TListViewItemNames.Footer);
end;
...
procedure TBindListListViewEditor.SetCurrentValue(const AMemberName: string;
  const AValue: TValue;  AType: TScopeMemberType);
var
  Col: Integer;
  LItems: TStrings;
...
begin
  if FEditorObject.FItemIndex = -1 then
    FEditorObject.FItemIndex := FEditorObject.FListView.ItemIndex;
  LVal := AValue.AsString;
  if AMemberName = TListViewItemNames.Caption then
    FEditorObject.Caption := LVal
  else if AMemberName = TListViewItemNames.Header then
    FGroupEditorObject.Header := LVal
  else if AMemberName = TListViewItemNames.Subtitle then
    FGroupEditorObject.Subtitle := LVal
  else if TryParseCellExpression(sSubItemsPrefix, AMemberName, Col) then
  begin
    LItems := FEditorObject.SubItems;
    while LItems.Count < Col do
      LItems.Add('');
    if LItems.Count = Col then
      LItems.Add(LVal)
    else
      LItems[Col] := LVal;
  end;
...
function TBindListListViewEditor.CanSetCurrentValue(const AMemberName: string; out AType: TScopeMemberType): Boolean;
var
  Col: Integer;
begin
  if AMemberName = TListViewItemNames.Caption then
...
initialization
  ...
  Data.Bind.Components.RegisterListItemValueExpression(TArray<TClass>.Create(
    ...
    TListViewItemNames.Caption, TListViewItemNames.Header);
