...
uses
  System.Classes, System.Bindings.EvalProtocol, Data.Bind.Components, Data.Bind.Editors, System.Bindings.ObjEval,
    ...
    FMX.Memo, FMX.StdCtrls, System.RTTI, FMX.Graphics, System.Generics.Collections, System.SysUtils, Data.Bind.ObjectScope,
...
type
  ...
  public
    ...
    procedure SetTime(ATime: TTime); override;
  end;
  ...
  protected
    ...
    function GetPositionSetter(var ASetter: string;
      var ABase: Integer): Boolean; override;
    procedure SetExpectedRowCount(ARowCount: Integer); override;
    { IBindListItemIndexEditor }
    function GetItemIndex: Integer; override;
    procedure SetItemIndex(AIndex: Integer); override;
...
uses System.Math, FMX.Edit, System.Types, FMX.Utils, Fmx.Bind.Consts,
...
const
  CPrefix = 'Cells[';
  CPrefixLen = Length(CPrefix);
var
  Col: Integer;
  C: Char;
  P: PChar;
begin
  Result := StrLComp(PChar(AMemberName), PChar(CPrefix), CPrefixLen) = 0;
  if Result then
  begin
    P := PChar(AMemberName) + CPrefixLen;
    Col := 0;
    while P <> #0 do
    begin
      C := P^;
...
procedure TBindListStringGridEditor.SetExpectedRowCount(ARowCount: Integer);
begin
  FEditorObject.FStringGrid.Model.SetExpectedRowCount(ARowCount);
end;
...
begin
  Result := False;
  if (AObject <> nil) then
    if ((AIntf = IBindTimeEditEditor) and AObject.InheritsFrom(TCustomTimeEdit)) or
       ((AIntf = IBindDateTimeEditEditor) and AObject.InheritsFrom(TCustomDateTimeEdit))
    then begin
      Result := True;
    end;
end;
...
function TBindEmptyDateTimeEditEditor.GetTime: TTime;
begin
  Result := FCustomDateTimeEdit.DateTime;
end;

procedure TBindEmptyDateTimeEditEditor.SetTime(ATime: TTime);
begin
  FCustomDateTimeEdit.DateTime := ATime;
end;
...
{$WARNINGS ON}
  ...
  UnregisterBindEditorFactory([TBindListBoxEditorFactory, TBindMetroListBoxEditorFactory, TBindComboBoxEditorFactory,
    TBindMetroComboBoxEditorFactory,
    TBindStringGridEditorFactory, TBindGridEditorFactory, TBindCheckBoxEditorFactory,
    TBindComboEditEditorFactory, TBindListViewEditorFactory, TBindDateTimeEditEditorFactory]);
