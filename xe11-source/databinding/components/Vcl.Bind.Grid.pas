...
begin
  if ADescription.Width <> -1 then
    FCustomGrid.ColWidths[AColumn] := ADescription.Width
  else if ADescription.TextWidth <> -1 then
    FCustomGrid.ColWidths[AColumn] := FCustomGrid.TextWidthToColWidth(
      ADescription.TextWidth, ADescription.Header, nil)
  else
    FCustomGrid.ColWidths[AColumn] := GetDefaultColumnWidth;
  FCustomGrid.ColAlignments[AColumn] := ADescription.Alignment;
end;
...
begin
  inherited ApplyDescription(ADescription, AColumn);
...
//        end;
    ...
    begin
      LPair := TLinkGridColumnExpressionPair.Create(
        Format('Cells[%d]', [AIndex]), LCellExpression, ADescription.CustomFormat <> '');
...
//            LCellExpression := '';
    ...
    begin
      ...
      LPair := TLinkGridColumnExpressionPair.Create(
      LSelectedText, LCellExpression, ADescription.CustomParse <> ''); // do not localize
      FPairsList.Add(LPair);
    end;
