...
//    procedure InvalidateField(AField: TField);
  ...
  protected
    ...
    function GetFormatLink(const AMemberName: string): IEditFormatLink; virtual;
  ...
  protected
    function GetFormatLink(const AMemberName: string): IEditFormatLink; override;
  ...
  TLinkGridToDataSourceColumnValue = (cvWidth, cvTextWidth, cvHeader, cvVisible,
    cvReadOnly, cvDecimalDigits, cvShowThousandSeparator, cvAlignment);
  ...
  private
    ...
    FDecimalDigits: Integer;
    FShowThousandSeparator: Boolean;
    FFormatLink: IEditFormatLink;
    FFormatLinkInit: Boolean;
    FAlignment: TAlignment;
    ...
    procedure SetCustomParse(const AValue: string);
    procedure SetColumnStyle(const AValue: string);
    procedure SetDecimalDigits(const AValue: Integer);
    procedure SetShowThousandSeparator(const AValue: Boolean);
    function TryGetDefaultInfo: Boolean;
    function IsVisibleStored: Boolean;
    function IsReadonlyStored: Boolean;
    function GetTextWidth: Integer;
    function IsTextWidthStored: Boolean;
    procedure SetTextWidth(const AValue: Integer);
    function GetDecimalDigits: Integer;
    function IsDecimalDigitsStored: Boolean;
    function GetShowThousandSeparator: Boolean;
    function IsShowThousandSeparatorStored: Boolean;
    function GetAlignment: TAlignment;
    function IsAlignmentStored: Boolean;
    procedure SetAlignment(const AValue: TAlignment);
  protected
    procedure Initialize; override;
    procedure ResetDefaultInfo;
  public
  ...
  published
    ...
    property TextWidth: Integer read GetTextWidth write SetTextWidth stored IsTextWidthStored;
    ...
    property DecimalDigits: Integer read GetDecimalDigits write SetDecimalDigits stored IsDecimalDigitsStored;
    property ShowThousandSeparator: Boolean read GetShowThousandSeparator write SetShowThousandSeparator stored IsShowThousandSeparatorStored;
...
var
  LActive: IScopeActive;
begin
  Result := (GetControlComponent <> nil) and (DataSource <> nil);
  if Result and Supports(DataSource, IScopeActive, LActive) then
    Result := LActive.Active;
end;
...
function TBaseLinkGridToDataSource.GetFormatLink(const AMemberName: string): IEditFormatLink;
begin
  Result := nil;
end;
...
procedure TBaseLinkGridToDataSource.Reactivate;
begin
  DoReactivate(
...
function TBaseLinkGridToDataSource.RequiresControlHandler: Boolean;
begin
  Result := TInternalBindGridLink(FBindGridLink).RequiresControlHandler;
end;
...
begin
  ...
  FDecimalDigits := AColumn.DecimalDigits;
  if (cvShowThousandSeparator in AColumn.AssignedValues) or (FMemberType <> mtCurrency) then
    FShowThousandSeparator := AColumn.ShowThousandSeparator
  else
    FShowThousandSeparator := True;
  if cvAlignment in AColumn.AssignedValues then
  begin
    FDefaultAlignment := False;
    FAlignment := AColumn.Alignment;
  end
  else
  begin
    FDefaultAlignment := True;
    if FMemberType in [mtInteger, mtFloat, mtBCD, mtUInteger, mtCurrency] then
...
begin
  ...
  if FMemberName <> '' then
  begin
    if Supports(FDataSource, IScopeMemberScripting, LScopeMemberScripting) then
    begin
      LScopeMemberScripting.GetMemberType(FMemberName, FMemberType);
      if FMemberType = mtCurrency then
        FShowThousandSeparator := True;
      if FMemberType in [mtInteger, mtFloat, mtBCD, mtUInteger, mtCurrency] then
        FAlignment := taRightJustify;
      LScopeMemberScripting.GetMemberGetter(FMemberName, FMemberGetter);
    ...
    if Supports(FDataSource, IScopeEditor, LScopeEditor) then
    begin
      LFormatLink := LScopeEditor.GetFormatLink(FMemberName);
      if LFormatLink <> nil then
      begin
        FHeader := LFormatLink.DisplayName;
        FReadOnly := LFormatLink.ReadOnly;
        FVisible := LFormatLink.Visible;
        FWidth := LFormatLink.DisplayWidth;
        FTextWidth := LFormatLink.DisplayTextWidth;
        FDefaultAlignment := False;
        FAlignment := LFormatLink.Alignment;
      end;
    end;
  end;
end;
...
procedure TLinkGridToDataSourceColumn.ResetDefaultInfo;
begin
  FFormatLink := nil;
  FFormatLinkInit := False;
end;
...
begin
  ...
  begin
    ...
    try
      RestoreDefaults;
      FAssignedValues := TLinkGridToDataSourceColumn(Source).FAssignedValues;
      FMemberName := TLinkGridToDataSourceColumn(Source).FMemberName;
      FReadOnly := TLinkGridToDataSourceColumn(Source).FReadOnly;
      FHeader := TLinkGridToDataSourceColumn(Source).FHeader;
      FWidth := TLinkGridToDataSourceColumn(Source).FWidth;
      FTextWidth := TLinkGridToDataSourceColumn(Source).FTextWidth;
      FVisible := TLinkGridToDataSourceColumn(Source).FVisible;
      FColumnStyle := TLinkGridToDataSourceColumn(Source).FColumnStyle;
      FCustomParse := TLinkGridToDataSourceColumn(Source).FCustomParse;
      FDecimalDigits := TLinkGridToDataSourceColumn(Source).FDecimalDigits;
      FShowThousandSeparator := TLinkGridToDataSourceColumn(Source).FShowThousandSeparator;
      FFormatLink := TLinkGridToDataSourceColumn(Source).FFormatLink;
      FFormatLinkInit := TLinkGridToDataSourceColumn(Source).FFormatLinkInit;
      FAlignment := TLinkGridToDataSourceColumn(Source).FAlignment;
    finally
      if Assigned(Collection) then Collection.EndUpdate;
    end;
  end
...
function TLinkGridToDataSourceColumn.TryGetDefaultInfo: Boolean;
var
  LLink: TPersistent;
begin
  if not FFormatLinkInit and (MemberName <> '') then
  begin
    FFormatLink := nil;
    FFormatLinkInit := True;
    if Collection is TBaseLinkGridToDataSourceColumns then
    begin
      LLink := TBaseLinkGridToDataSourceColumns(Collection).Owner;
      if LLink is TBaseLinkGridToDataSource then
        FFormatLink := TBaseLinkGridToDataSource(LLink).GetFormatLink(MemberName);
    end;
  end;
  Result := FFormatLink <> nil;
end;
...
begin
  if cvWidth in FAssignedValues then
    Result := FWidth
  else if (cvTextWidth in FAssignedValues) and (FTextWidth <> -1) then
    Result := -1
  else if TryGetDefaultInfo then
    Result := FFormatLink.DisplayWidth
  else
  begin
    Result := 0;
    if Collection is TBaseLinkGridToDataSourceColumns then
...
procedure TLinkGridToDataSourceColumn.SetWidth(const AValue: Integer);
begin
  if not (cvWidth in FAssignedValues) or (AValue <> Width) then
  begin
    FWidth := AValue;
    Include(FAssignedValues, cvWidth);
    Exclude(FAssignedValues, cvTextWidth);
    Changed(False);
  end;
end;

function TLinkGridToDataSourceColumn.IsTextWidthStored: Boolean;
begin
  Result := cvTextWidth in FAssignedValues;
end;

function TLinkGridToDataSourceColumn.GetTextWidth: Integer;
begin
  if cvTextWidth in FAssignedValues then
    Result := FTextWidth
  else if (cvWidth in FAssignedValues) and (FWidth <> -1) then
    Result := -1
  else if TryGetDefaultInfo then
    Result := FFormatLink.DisplayTextWidth
  else
    Result := -1;
end;

procedure TLinkGridToDataSourceColumn.SetTextWidth(const AValue: Integer);
begin
  if not (cvTextWidth in FAssignedValues) or (AValue <> TextWidth) then
  begin
    FTextWidth := AValue;
    Include(FAssignedValues, cvTextWidth);
    Exclude(FAssignedValues, cvWidth);
    Changed(False);
  end;
end;
...
begin
  Result := cvHeader in FAssignedValues;
end;
...
begin
  if cvHeader in FAssignedValues then
    ...
    Result := FFormatLink.DisplayName
  else
    Result := MemberName;
end;

procedure TLinkGridToDataSourceColumn.SetHeader(const AValue: string);
begin
  if not (cvHeader in FAssignedValues) or (AValue <> Header) then
  begin
    FHeader := AValue;
...
function TLinkGridToDataSourceColumn.IsVisibleStored: Boolean;
begin
  Result := cvVisible in FAssignedValues;
end;
...
var
  ...
  LVisible: Boolean;
begin
  if cvVisible in FAssignedValues then
    Result := FVisible
  else if TryGetDefaultInfo then
    Result := FFormatLink.Visible
  else
    Result := True;
  if Result then
  begin
...
procedure TLinkGridToDataSourceColumn.SetVisible(const AValue: Boolean);
begin
  if not (cvVisible in FAssignedValues) or (AValue <> Visible) then
  begin
    FVisible := AValue;
    Include(FAssignedValues, cvVisible);
    Changed(True);
  end;
end;

function TLinkGridToDataSourceColumn.IsReadonlyStored: Boolean;
begin
...
begin
  if cvReadOnly in FAssignedValues then
    ...
    Result := FFormatLink.ReadOnly
  else
    Result := False;
end;

procedure TLinkGridToDataSourceColumn.SetReadOnly(const AValue: Boolean);
begin
  if not (cvReadOnly in FAssignedValues) or (AValue <> ReadOnly) then
  begin
    FReadOnly := AValue;
...
function TLinkGridToDataSourceColumn.IsDecimalDigitsStored: Boolean;
begin
  Result := cvDecimalDigits in FAssignedValues;
end;

function TLinkGridToDataSourceColumn.GetDecimalDigits: Integer;
begin
  if cvDecimalDigits in FAssignedValues then
    Result := FDecimalDigits
  else
    Result := 2;
end;

procedure TLinkGridToDataSourceColumn.SetDecimalDigits(const AValue: Integer);
begin
  if not (cvDecimalDigits in FAssignedValues) or (AValue <> DecimalDigits) then
  begin
    FDecimalDigits := AValue;
    Include(FAssignedValues, cvDecimalDigits);
    Changed(False);
  end;
end;

function TLinkGridToDataSourceColumn.IsShowThousandSeparatorStored: Boolean;
begin
  Result := cvShowThousandSeparator in FAssignedValues;
end;

function TLinkGridToDataSourceColumn.GetShowThousandSeparator: Boolean;
begin
  if cvShowThousandSeparator in FAssignedValues then
    Result := FShowThousandSeparator
  else if TryGetDefaultInfo then
    Result := FFormatLink.Currency
  else
    Result := False;
end;

procedure TLinkGridToDataSourceColumn.SetShowThousandSeparator(const AValue: Boolean);
begin
  if not (cvShowThousandSeparator in FAssignedValues) or (AValue <> ShowThousandSeparator) then
  begin
    FShowThousandSeparator := AValue;
    Include(FAssignedValues, cvShowThousandSeparator);
    Changed(False);
  end;
end;

function TLinkGridToDataSourceColumn.IsAlignmentStored: Boolean;
begin
  Result := cvAlignment in FAssignedValues;
end;

function TLinkGridToDataSourceColumn.GetAlignment: TAlignment;
begin
  if cvAlignment in FAssignedValues then
    Result := FAlignment
  else if TryGetDefaultInfo then
    Result := FFormatLink.Alignment
  else
    Result := taLeftJustify;
end;

procedure TLinkGridToDataSourceColumn.SetAlignment(const AValue: TAlignment);
begin
  if not (cvAlignment in FAssignedValues) or (AValue <> Alignment) then
  begin
    FAlignment := AValue;
...
procedure TLinkGridToDataSourceColumn.SetColumnStyle(const AValue: string);
var
  Grid: ILinkGridToDataSource;
begin
  if AValue <> FColumnStyle then
  begin
    Grid := GetGridIntf;
    Grid.BeginLayout;
    try
...
procedure TLinkGridToDataSourceColumn.SetCustomFormat(const AValue: string);
begin
  if AValue <> FCustomFormat then
  begin
    FCustomFormat := AValue;
    Changed(True);
  end;
end;

procedure TLinkGridToDataSourceColumn.SetCustomParse(const AValue: string);
begin
  if AValue <> FCustomParse then
  begin
    FCustomParse := AValue;
    Changed(True);
  end;
...
function TCustomLinkGridToDataSource.GetFormatLink(const AMemberName: string): IEditFormatLink;
var
  LScopeEditor: IScopeEditor;
begin
  Result := nil;
  if Supports(GetDataSource, IScopeEditor, LScopeEditor) then
    Result := LScopeEditor.GetFormatLink(AMemberName);
end;
