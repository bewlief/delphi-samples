...
type
  ...
  private
    ...
    FEditFormatLinks: TDictionary<TField, IEditFormatLink>;
...
begin
  ...
  UpdateField;
  if (FBindLayoutChanged = nil) and (FBindLink <> nil) and FBindLink.Active then
  begin
    FBindLink.Active := False;
    FBindLink.Active := True;
  end;
end;

procedure TBindLinkDataLink.DataEvent(Event: TDataEvent; Info: NativeInt);
begin
  if (Event = deDataSetChange) and (FOwner.FEnumerating = 0) then
    RefreshNeeded;
  inherited;
  case Event of
    deFieldChange:
...
{ TEditFormatFieldLink }

constructor TEditFormatFieldLink.Create(AOwner: TCustomBindSourceDB; AField: TField);
begin
  inherited Create;
  FOwner := AOwner;
  FField := AField;
  FCurrency := FOwner.IsCurrencyField(FField);
end;

function TEditFormatFieldLink.GetAlignment: TAlignment;
begin
  Result := FField.Alignment;
end;

function TEditFormatFieldLink.GetCurrency: Boolean;
begin
  Result := FCurrency;
end;

function TEditFormatFieldLink.GetDisplayName: string;
begin
  Result := FField.DisplayName;
end;

function TEditFormatFieldLink.GetDisplayTextWidth: Integer;
begin
  Result := FField.DisplayWidth;
end;

function TEditFormatFieldLink.GetDisplayWidth: Integer;
begin
  Result := -1;
end;

function TEditFormatFieldLink.GetEditMask: string;
begin
  Result := FField.EditMask;
end;

function TEditFormatFieldLink.GetMaxLength: Integer;
begin
  if FField.DataType in [TFieldType.ftString, TFieldType.ftFixedChar,
                         TFieldType.ftWideString, TFieldType.ftFixedWideChar] then
    Result := FField.Size
  else
    Result := 0;
end;

function TEditFormatFieldLink.GetReadOnly: Boolean;
begin
  Result := FField.ReadOnly;
end;

function TEditFormatFieldLink.GetVisible: Boolean;
begin
...
begin
  ...
  FEditFormatLinks := TDictionary<TField, IEditFormatLink>.Create;
end;
...
procedure TCustomBindSourceDB.EmptyDataLink;
begin
...
begin
  FEditFormatLinks.Clear;
  if (FDataLink <> nil) and (not FDataLink.Active) then
    EmptyDataLink;
...
function TCustomBindSourceDB.IsCurrencyField(AField: TField): Boolean;
var
  LType: TRttiType;
  LProp: TRttiProperty;
begin
  Result := False;
  LType := FRttiContext.GetType(AField.ClassType);
  if LType <> nil then
  begin
    LProp := LType.GetProperty('currency');
    Result := (LProp <> nil) and LProp.GetValue(AField).AsBoolean;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      Result := True;
      if IsCurrencyField(LField) then
        LDataType := TFieldType.ftCurrency
      else
        LDataType := LField.DataType;
      AType := DefaultMemberType[LDataType];
    end;
  end;
end;
...
function TCustomBindSourceDB.GetFormatLink(const AFieldName: string): IEditFormatLink;
var
  LField: TField;
begin
  Result := nil;
...
{$HINTS OFF}
  TBindLinkDataLinkHack = class(TBaseDataLink)
  private
    [weak] FOwner: TCustomBindSourceDB;
    FBindLink: IBindLink;
  end;
...
procedure TCustomBindSourceDB.UpdateControlComponent(AControl: TComponent);
var
  LDS: TDataSourceHack;
  I: Integer;
  LLink: TBindLinkDataLinkHack;
  LType: TRttiType;
  LProp: TRttiProperty;
begin
  ...
  LDS := TDataSourceHack(DataSource);
  for I := 0 to LDS.DataLinks.Count - 1 do
    if LDS.DataLinks[I] is TBindLinkDataLink then
    begin
      LLink := TBindLinkDataLinkHack(LDS.DataLinks[I]);
      LType := FRttiContext.GetType(LLink.FBindLink.BindComp.ClassType);
      if LType <> nil then
      begin
        LProp := LType.GetProperty('ControlComponent');
        if (LProp <> nil) and (LProp.GetValue(LLink.FBindLink.BindComp).AsObject = AControl) then
          LLink.RecordChanged(nil);
      end;
    end;
end;
