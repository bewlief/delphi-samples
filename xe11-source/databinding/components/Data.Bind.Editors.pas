...
type
  ...
  TBindListEditor = class(TBindListEditorCommon, IBindListEditor, IBindListEditorItem, IBindListItemIndexEditor)
  private
  protected
    ...
    function GetPositionSetter(var ASetter: string;
      var ABase: Integer): Boolean; override;
    procedure SetExpectedRowCount(ARowCount: Integer); virtual;
    { IBindListEditorItem }
    function CurrentObject: TObject; virtual;
    function CurrentIndex: Integer; virtual;
    { IBindListItemIndexEditor }
    // Support binding engine method ListItemIndex.
    // Editors should override these if ListItemIndex will be used in an expression to set or get the item index.
  ...
  TBindDateTimeEditEditor = class(TInterfacedObject, IBindDateTimeEditEditor, IBindTimeEditEditor)
  protected
    function GetIsEmpty: Boolean; virtual; abstract;
    procedure SetIsEmpty(Value: Boolean); virtual; abstract;
    function GetDateTime: TDateTime; virtual; abstract;
    procedure SetDateTime(ADateTime: TDateTime); virtual; abstract;
    function GetTime: TTime; virtual; abstract;
    procedure SetTime(ADateTime: TTime); virtual; abstract;
  end;
  ...
  public
    ...
    function ReleaseEditor(const AEditor: IInterface): Boolean;
...
function TBindListEditor.GetItemIndex: Integer;
begin
  raise ENotImplemented.Create('GetItemIndex'); // Do not localize
end;
...
// Descendents may override this method and set to 'ListItemIndex(Self)' instead.  ListItemIndex will
// call IBindListItemIndexEditor.SetItemIndex.  This allows descendents to implement SetItemIndex.  For example,
// descendents can check that ItemIndex is in range before setting actual control ItemIndex property.
...
procedure TBindListEditor.SetExpectedRowCount(ARowCount: Integer);
begin
  // nothing
end;

procedure TBindListEditor.SetItemIndex(AValue: Integer);
begin
  raise ENotImplemented.Create('SetItemIndex'); // Do not localize
end;
...
begin
  ...
  try
    ...
    begin
      ...
      // Do not use IScopeRecordEnumeratorCount.RecordCount here because it can change the current record position.
      // If the record position is changed, then ARecordEnumerator.MoveNext may not return expected value
      // Delete remaining items, if any
      // if Supports(ARecordEnumerator, IScopeRecordEnumeratorCount, LRecordCount) and (LRecordCount.RecordCount = 0) then
      //  LEditor.ClearList
      // else
      LEditor.DeleteToEnd;
      // Add new items
...
var
  LPairs: TPair<TComponent, IInterface>;
begin
  for LPairs in FDictionary do
    LPairs.Key.RemoveFreeNotification(Self);
  inherited;
...
function TEditors.ReleaseEditor(const AEditor: IInterface): Boolean;
var
  LPairs: TPair<TComponent, IInterface>;
  LKey: TComponent;
begin
  LKey := nil;
  for LPairs in FDictionary do
    if (LPairs.Value as IInterface) = (AEditor as IInterface) then
      LKey := LPairs.Key;
  Result := LKey <> nil;
  if Result then
  begin
    LKey.RemoveFreeNotification(Self);
