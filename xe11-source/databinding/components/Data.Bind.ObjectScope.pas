...
type
  ...
  public
    ...
    function GetFormatLink(const AFieldName: string): IEditFormatLink; virtual;
...
{ TBindFieldDef }
  ...
  private
    ...
    FValuesList: TList<T>;
  public
    constructor CreateFromOA(const AOptions: TGeneratorOptions; const AValuesList: array of T); overload;
    constructor CreateFromOA(const AOptions: TGeneratorOptions; const AValuesList: array of T;
      const AOnGetValue: TValueGeneratorOnGetValue); overload;
    constructor Create(const AOptions: TGeneratorOptions; const AValuesList: TArray<T>); overload;
    constructor Create(const AOptions: TGeneratorOptions; const AValuesList: TArray<T>;
      const AOnGetValue: TValueGeneratorOnGetValue); overload;
    destructor Destroy; override;
...
begin
  ...
  for LBindComponent in Expressions do
    if (LBindComponent is TCustomBindExpression) and
       TCustomBindExpression(LBindComponent).Active and
       (TCustomBindExpression(LBindComponent).Direction in [dirSourceToControl, dirBidirectional]) then
      TCustomBindExpression(LBindComponent).Evaluate;
end;
...
procedure TBaseObjectBindSource.UpdateControlComponent(AControl: TComponent);
var
  LLink: TBindSourceAdapterLink;
begin
  for LLink in FAdapterLinks.Values do
    LLink.DataSetScrolled(0);
end;
...
var
  LWillCreate: Boolean;
begin
  LWillCreate := FCheckRuntimeAdapter and (FRuntimeAdapter = nil);
  if CheckRuntimeAdapter then
  begin
    if LWillCreate and not FCheckRuntimeAdapter and (FRuntimeAdapter <> nil) then
    begin
      FRuntimeAdapter.AutoEdit := AutoEdit;
      FRuntimeAdapter.AutoPost := AutoPost;
    end;
    Result := GetRuntimeAdapter;
  end
...
begin
  ...
  if FRuntimeAdapter <> nil then
    FRuntimeAdapter.AutoEdit := Value;
end;
...
begin
  ...
  if FRuntimeAdapter <> nil then
    FRuntimeAdapter.AutoPost := Value;
end;
...
begin
  if (Value <> nil) and not FScopes.Contains(Value) then
  begin
    FScopes.Add(Value);
...
begin
  if (Value <> nil) and FScopes.Contains(Value) then
  begin
...
{$ENDIF}
    ...
    begin
      ...
      LCollectionEditorField.FIndex := AFieldsList.Add(LCollectionEditorField);
    end;
  end;
...
var
  ...
  LFieldDef: TGeneratorFieldDef;
begin
  SetLength(ARecord.FValues, FieldDefs.Count);
  ...
  begin
    ...
    case LFieldDef.FieldType of
      ...
      ftUInteger:
        ARecord.FValues[I] := 0;
      ftSingle:
        ARecord.FValues[I] := 0.0;
      ftBoolean:
        ARecord.FValues[I] := False;
      ftTStrings:
        ARecord.FValues[I] := TStringList.Create;
      ftBitmap:
        ARecord.FValues[I] := nil;
      ftCurrency:
        ARecord.FValues[I] := TValue.Empty.AsType<Currency>;
      ftDateTime:
        ARecord.FValues[I] := TValue.Empty.AsType<TDateTime>;
      ftTime:
        ARecord.FValues[I] := TDateTime(TValue.Empty.AsType<TTime>);
      ftDate:
        ARecord.FValues[I] := TDateTime(TValue.Empty.AsType<TDate>);
    else
      Assert(False);
    end;
  end;
...
procedure TCustomDataGeneratorAdapter.GenerateExistingRecords(AList: TList<TGeneratorRecord>);
  ...
  begin
    ...
    begin
      ...
      ARecord.FValues[I] := LValue;
      if LFree and LValue.IsObject then
      begin
        if ARecord.FFreeObjects = nil then
          ARecord.FFreeObjects := TObjectList<TObject>.Create;
          ...
          begin
            if LFieldDef.CustomFormat <> '' then
              Self.CreateCustomFormatExpression<TTime>(LFieldDef.CustomFormat,
                LCustomFormatExpression, LExpressionObject);
            LDefaultValue := TDateTime(TValue.Empty.AsType<TDate>);
          end;
          ftDate:
          begin
            if LFieldDef.CustomFormat <> '' then
              Self.CreateCustomFormatExpression<TDate>(LFieldDef.CustomFormat,
                LCustomFormatExpression, LExpressionObject);
            LDefaultValue := TDateTime(TValue.Empty.AsType<TTime>);
          end;
        else
        ...
        case LField.FieldType.TypeKind of
          tkEnumeration:
            if IsBoolType(LTypeInfo) then
              ...
              case LField.FieldType.TypeSize of
              1:
...
begin
  ...
  begin
    ...
    if LRttiField <> nil then
      if (LRttiField.FieldType.TypeKind = tkEnumeration) and
         not IsBoolType(LRttiField.FieldType.Handle) then
...
begin
  LObject := FField.GetMemberObject;
  if LObject <> nil then
    if FField.Index >= 0 then
      Result := (LObject as TGeneratorRecord).FValues[FField.Index].AsType<T>()
    else
    begin
      Assert(False);
      Result := TValue.Empty.AsType<T>;
...
begin
  ...
  begin
    ...
    if LRttiField <> nil then
      ...
      begin
        var V: TValue;
        TValue.Make(@AValue, LRttiField.FieldType.Handle, V);
...
begin
  ...
  begin
    ...
    if LRttiField <> nil then
      if (LRttiField.PropertyType.TypeKind = tkEnumeration) and
         not IsBoolType(LRttiField.PropertyType.Handle) then
        Result := T(LRttiField.GetValue(LObject).GetReferenceToRawData^)
      else
        Result := LRttiField.GetValue(LObject).AsType<T>
    else
...
begin
  ...
  if LObject <> nil then
    if FField.Index >= 0 then
      (LObject as TGeneratorRecord).FValues[FField.Index] := TValue.From<T>(AValue)
    else
      Assert(False);
end;
...
// Copy from System.Random
{$Q-}{$R-}
function TInternalTypedListValueGeneratorDelegate.PRNGenerator(const ARange: Integer; var Seed: Integer): Integer;
var
  Temp: Integer;
begin
  Temp := Seed * $08088405 + 1;
  Seed := Temp;
...
constructor TTypedListValueGeneratorDelegate<T>.CreateFromOA(const AOptions: TGeneratorOptions; const AValuesList: array of T);
var
  I: Integer;
  Seed: Integer;
begin
  ...
  begin
    ...
    Seed := 123456789; // Shuffle always results in same order, so that generated and names match corresponding generated values
    for I := 0 to FValuesList.Count - 1 do
      FValuesList.Exchange(I, PRNGenerator(FValuesList.Count, Seed));
  end;
end;

constructor TTypedListValueGeneratorDelegate<T>.CreateFromOA(const AOptions: TGeneratorOptions; const AValuesList: array of T;
  const AOnGetValue: TValueGeneratorOnGetValue);
begin
  CreateFromOA(AOptions, AValuesList);
  OnGetValue := AOnGetValue;
end;

constructor TTypedListValueGeneratorDelegate<T>.Create(const AOptions: TGeneratorOptions; const AValuesList: TArray<T>);
begin
  CreateFromOA(AOptions, AValuesList);
end;
...
begin
  ...
  if LObject <> nil then
  begin
    LValues := (LObject as TGeneratorRecord).FValues;
    if FField.Index >= 0 then
    begin
      Assert(not LValues[FField.Index].IsEmpty);
      LNewValue := TValue.From<T>(AValue);
      LOldValue := LValues[FField.Index];
      if LOldValue.IsObject and (LOldValue.AsObject is TPersistent) then
        if (LNewValue.IsObject) and (LNewValue.AsObject is TPersistent) then
          TPersistent(LOldValue.AsObject).Assign(TPersistent(LNewValue.AsObject))
        else
          TPersistent(LOldValue.AsObject).Assign(nil) // Clear
      else
...
function TBaseObjectBindSourceDelegate.GetFormatLink(const AFieldName: string): IEditFormatLink;
begin
  Result := FBindSource.GetFormatLink(AFieldName);
end;
