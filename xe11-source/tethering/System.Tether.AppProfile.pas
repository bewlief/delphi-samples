...
type
  ...
  private
    type
      TTetheringAppCommand = (Unknown, Connect, Run, GetActions, GetResources, GetResourceValue, SubscribeResource,
        UnSubscribeResource, SendStream, SendString, ReverseConnect, RunAsync, GetRemoteActionState,
        DisconnectProfile);
      TTetheringAppCommandPair = TPair<string, TTetheringAppCommand>;
  ...
  private
    ...
    FAppCommandMap: TDictionary<string, TTetheringAppCommand>;

    function GetRemoteActions(const AProfile: TTetheringProfileInfo): Boolean;
    procedure GetRemoteResources(const AProfile: TTetheringProfileInfo; const AProtocolType: TTetheringProtocolType;
      const AnAdapterType: TTetheringAdapterType);
  ...
  protected
    ...
    procedure InternalConsistencyCheck; override;
  public
...
begin
  ...
  try
    LCommand := SendCommandWithResponse(LConnection, TTetheringCommand.Create(SendStreamCommand, Version, [Description, Identifier]));
    ...
    begin
      try
        ...
        begin
          ...
          Result := LCommand.Command = SendStreamContentOKResponse;
        end;
      except
...
begin
  CheckProfileIsConnected(AProfile);
  LCommand := TTetheringCommand.Create(SendStringCommand, Version, [Description, AString, Identifier]);
  Result := SendCommandWithResponse(AProfile, LCommand).Command = SendStringContentOKResponse;
end;

function TTetheringAppProfile.SendLongString(const AProfile: TTetheringProfileInfo;
  const Description, AString: string): Boolean;
var
  LStream: TStream;
begin
  ...
  try
    Result := SendStream(AProfile, Description, LStream);
  finally
    LStream.Free;
  end;
end;
...
begin
  ...
  begin
    ...
    for I := FSubscribedRemoteProfiles.Count - 1 downto 0  do
    begin
      LSubscribedPair := FSubscribedRemoteProfiles[I];
      if (LSubscribedPair.Key.ManagerIdentifier = AProfile.ManagerIdentifier) and
         (LSubscribedPair.Key.ProfileIdentifier = AProfile.ProfileIdentifier) and
         (LSubscribedPair.Value.ItemType = ARemoteItem.ItemType) and
         (LSubscribedPair.Value.Name = ARemoteItem.Name) then
...
function TTetheringAppProfile.Connect(const AProfile: TTetheringProfileInfo; ATimeout: Integer;
  const AProtocolType: TTetheringProtocolType; const AnAdapterType: TTetheringAdapterType): Boolean;
var
  LCommand: TTetheringCommand;
  LConnection: TTetheringConnection;
begin
  ...
  Result := LConnection.Protocol.Connect(LConnection, ATimeout);
  if Result then
  begin
    TMonitor.Enter(LConnection);
    try
    ...
    begin
      ...
      GetRemoteResources(AProfile, AProtocolType, AnAdapterType);
...
begin
  Result := Connect(AProfile, TTetheringCustomComm.ProtocolSpecificTimeout, AProtocolType, AnAdapterType);
end;
...
begin
  ...
  FAppCommandMap := TDictionary<string, TTetheringAppCommand>.Create(Ord(High(TTetheringAppCommand)) * 2, TIStringComparer.Ordinal);
  FAppCommandMap.Add(RunActionCommand, TTetheringAppCommand.Run);
  FAppCommandMap.Add(GetActionsCommand, TTetheringAppCommand.GetActions);
  FAppCommandMap.Add(GetResourceValueCommand, TTetheringAppCommand.GetResourceValue);
  FAppCommandMap.Add(GetResourcesCommand, TTetheringAppCommand.GetResources);
  FAppCommandMap.Add(ConnectCommand, TTetheringAppCommand.Connect);
  FAppCommandMap.Add(SubscribeResourceCommand, TTetheringAppCommand.SubscribeResource);
  FAppCommandMap.Add(UnSubscribeResourceCommand, TTetheringAppCommand.UnsubscribeResource);
  FAppCommandMap.Add(SendStreamCommand, TTetheringAppCommand.SendStream);
  FAppCommandMap.Add(SendStringCommand, TTetheringAppCommand.SendString);
  FAppCommandMap.Add(ReverseConnectCommand, TTetheringAppCommand.ReverseConnect);
  FAppCommandMap.Add(RunActionAsyncCommand, TTetheringAppCommand.RunAsync);
  FAppCommandMap.Add(GetRemoteActionAsyncStateCommand, TTetheringAppCommand.GetRemoteActionState);
  FAppCommandMap.Add(DisconnectProfileCommand, TTetheringAppCommand.DisconnectProfile);
end;
...
var
  ...
  LSubscribedPair: TSubscribedResPair;
  LSubscribedProfile: TPair<TCustomLocalItem, TStringList>;
  LPos: Integer;
begin
  if IsProfileConnected(AProfile) then
  begin
  ...
  // Always remove the subscribed remote resources...
  for I := FSubscribedRemoteProfiles.Count - 1 downto 0  do
  begin
    LSubscribedPair := FSubscribedRemoteProfiles[I];
    if (LSubscribedPair.Key.ManagerIdentifier = AProfile.ManagerIdentifier) and
       (LSubscribedPair.Key.ProfileIdentifier = AProfile.ProfileIdentifier) then
         FSubscribedRemoteProfiles.Delete(I);
  end;

  // Remove from subscribed profiles
  for LSubscribedProfile in FSubscribedProfiles do
  begin
    LPos := LSubscribedProfile.Value.IndexOf(AProfile.ProfileIdentifier);
    if LPos >= 0 then
      LSubscribedProfile.Value.Delete(LPos);
  end;
...
var
  LCommand: TTetheringCommand;
  LAppCommand: TTetheringAppCommand;
  ...
  begin
    ...
    LRemoteID := '';
    if Length(ACommand.Parameters) > 1 then
      LRemoteID := ACommand.Parameters[1];
    ...
    begin
      ...
      OnStreamReceived(Self, LRemoteID, LStream, LParameter);
    end
    else
  ...
  var
    ...
    LRemoteID: string;
  begin
    if (Length(ACommand.Parameters) < 2) or (Length(ACommand.Parameters) > 3) then
    ...
    begin
      ...
      begin
        ...
        if SizeOf(LHandle) > 4 then
          LHandle := TRemoteActionHandle(UInt64(LHandle) + (UInt64(Random(MaxInt)) shl 32));
        LThread := TThread.CreateAnonymousThread(procedure
          var
            Res: Boolean;
          begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if LRemote is TLocalAction then
          TLocalAction(LRemote).Broadcast;
      end
      else
...
begin
  ...
  FAppCommandMap.TryGetValue(LCommand.Command, LAppCommand);
...
var
  ...
  JSonPos: Integer;
begin
  LConnection := GetConnectionTo(AProfile, AProtocolType, AnAdapterType);
  TMonitor.Enter(LConnection);
  try
...
begin
  ...
  begin
    ...
    if LFoundRemoteRes <> nil then
      ...
      raise ETetheringException.CreateFmt(SRemoteResNotPublished,
        [ARemoteResName, AProfile.ProfileIdentifier]);
  end
  else
    raise ETetheringException.CreateFmt(SRemoteProfileNotSpecifiedRes, [AProfile.ProfileIdentifier]);
end;
...
procedure TTetheringAppProfile.Notify(const ANotification, Data: string);
  ...
  begin
    Result := TEncoding.UTF8.GetString(TBytes(OnAfterReceiveData(Self, TClassicByteDynArray(TNetEncoding.Base64.DecodeStringToBytes(AData)))));
  end;
...
procedure TTetheringAppProfile.NotifyUpdate(ALocalItem: TCustomLocalItem; const Data: string);
  ...
  begin
    Result := TNetEncoding.Base64.EncodeBytesToString(TBytes(OnBeforeSendData(Self, TClassicByteDynArray(TEncoding.UTF8.GetBytes(AData)))));
  end;
...
procedure TTetheringAppProfile.InternalConsistencyCheck;
var
  I: Integer;
  LName: string;
  LNames: TStringList;
begin
  ...
  for I := 0 to FSharedActions.Count - 1 do
    if FSharedActions.Items[I].Name = '' then
      raise ETetheringException.CreateRes(@SActionNameEmpty);

  // Check for empty resource name and for duplicate resource names
  LNames := TStringList.Create;
  try
    LNames.Sorted := true;
    for I := 0 to FSharedResources.Count - 1 do
    begin
      LName := FSharedResources.Items[I].Name;
      if LName = '' then
        raise ETetheringException.CreateRes(@SResourceNameEmpty)
      else if LNames.IndexOf(LName) >= 0 then
        raise ETetheringException.CreateResFmt(@SDuplicateResourceName, [LName])
      else
        LNames.Add(LName);
    end;
  finally
...
begin
  if FindProfileFromRemoteItem(ARemoteRes, LProfile) then
    ...
    raise ETetheringException.CreateFmt(SProfileNotFoundRes, [ARemoteRes.Name]);
end;
...
begin
  ...
  begin
    ...
    if TargetAction <> nil then
      ...
      raise ETetheringException.CreateFmt(SRemoteActNotPublished, [AnActionName, AProfile.ProfileIdentifier]);
  end
  else
    raise ETetheringException.CreateFmt(SRemoteProfileNotSpecifiedAct, [AProfile.ProfileIdentifier]);
end;
...
begin
  if FindProfileFromRemoteItem(AnAction, LProfile) then
    ...
    raise ETetheringException.CreateFmt(SProfileNotFoundAct, [AnAction.Name]);
end;
...
function TResourceValue.GetString: string;
var
  LBytes: TBytes;
  LSize: Int64;
begin
...
begin
  if NotifyUpdates and ((FActionEnabled <> FAction.Enabled) or (FActionCaption <> FAction.Caption) or (FHint <> FAction.Hint)) then
  begin
    FActionEnabled := FAction.Enabled;
    FActionCaption := FAction.Caption;
    FHint := FAction.Hint;
...
begin
  if Assigned(FOnResourceReceived) then
  begin
    if Profile.SynchronizeEvents then
      TThread.Synchronize(nil,
        procedure
        begin
          FOnResourceReceived(Self, AResource);
...
{$IFNDEF AUTOREFCOUNT}
  ...
  begin
    ...
    FResValue.FStream.CopyFrom(Value.FStream, 0);
  end;
{$ENDIF AUTOREFCOUNT}
  Broadcast;
end;
