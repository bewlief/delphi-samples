...
{ Abstract class for TetheringAdapters }
  ...
  public
    ...
    property ReconnectTimeout: Integer read FReconnectTimeout write FReconnectTimeout default TTetheringCustomComm.ProtocolSpecificTimeout;
  end;
  ...
  private
    ...
    class function IsInArray(const AItem: string; const AnArray: TArray<string>): Boolean; static;
  protected
    /// <summary> Manages a connection error</summary>
    procedure GetGroupsAndTexts(var AProfileGroups, AProfileTexts: TArray<string>);
    /// <summary> Check if Manager's profiles are compatible with Given groups or texts</summary>
  ...
  public
    ...
    procedure DiscoverManagersFiltered(const AProfileGroups, AProfileTexts: TArray<string>; const ATarget: string = ''); overload;
    procedure DiscoverManagersFiltered(const AProfileGroups, AProfileTexts: TArray<string>; Timeout: Cardinal; const ATarget: string = ''); overload;
    procedure DiscoverManagersFiltered(const AProfileGroups, AProfileTexts: TArray<string>; const ATargetList: TTetheringTargetHosts); overload;
    procedure DiscoverManagersFiltered(const AProfileGroups, AProfileTexts: TArray<string>; Timeout: Cardinal; const ATargetList: TTetheringTargetHosts); overload;
    ...
    property Version: Integer read FVersion;
    /// <summary>Network (UDP) adapter binds to this address when the property is not empty. When it is empty (default),
    /// Network adapter tries to discover all tethering applications on all IPv4 subnets accessible from this computer.</summary>
  ...
  published
    ...
    /// <summary>Property to control event synchronization with UI thread.</summary>
  ...
  protected
    ...
    /// <summary> Placeholder where derived classes can implement internal consistency check.</summary>
    procedure InternalConsistencyCheck; virtual;
...
begin
  ...
  if TTetheringAdapter.IsLoggingItem(TTetheringAdapter.TTetheringLogItem.Manager) then
    TTetheringAdapter.Log(Format('TTetheringManagerInfoHelper.CreateArrayString("%s", "%s", "%s")="%s"', [AValue, AnAdapters, AnAdapterType, Result]));
end;
...
begin
  ...
  if TTetheringAdapter.IsLoggingItem(TTetheringAdapter.TTetheringLogItem.Manager) then
    TTetheringAdapter.Log(Format('TTetheringManagerInfoHelper.MergeArrayString("%s", "%s") = "%s"', [OldString, NewString, Result]));
end;

function TTetheringManagerInfoHelper.MergeManagerInfo(const AManagerInfo: TTetheringManagerInfo): TTetheringManagerInfo;
begin
  if TTetheringAdapter.IsLoggingItem(TTetheringAdapter.TTetheringLogItem.Manager) then
    TTetheringAdapter.Log(Format('TTetheringManagerInfoHelper.MergeManagerInfo("%s", "%s")',
      [Self.ManagerAdapters, AManagerInfo.ManagerAdapters]));

  Self.ConnectionString := TTetheringManagerInfo.MergeArrayString(Self.ConnectionString, AManagerInfo.ConnectionString);
  Result := Self;
  if TTetheringAdapter.IsLoggingItem(TTetheringAdapter.TTetheringLogItem.Manager) then
    TTetheringAdapter.Log(Format('TTetheringManagerInfoHelper.MergeManagerInfo("%s", "%s") = "%s"',
      [Self.ManagerAdapters, AManagerInfo.ManagerAdapters, Result.ManagerAdapters]));
end;
...
var
  I: Integer;
  LGroups, LTexts: TArray<string>;
begin
  GetGroupsAndTexts(LGroups, LTexts);
  for I := 0 to FAdapters.Count - 1 do
    FAdapters[I].StartManagersAutoConnect(Timeout, ATargetList, LGroups, LTexts);
end;
...
begin
  GetGroupsAndTexts(LGroups, LTexts);
  for I := 0 to FAdapters.Count - 1 do
    FAdapters[I].StartManagersAutoConnect(FAdapters[I].GetAutomaticTimeout, ATargetList, LGroups, LTexts);
end;
...
begin
  ...
  begin
    for I := 0 to FAdapters.Count - 1 do
      FAdapters[I].StartManagersDiscovery(FAdapters[I].GetAutomaticTimeout, ATargetList, nil, nil);
  end
  else
    raise ETetheringException.CreateFmt(SDisabledManager, [Identifier]);
end;

procedure TTetheringManager.DiscoverManagersFiltered(const AProfileGroups, AProfileTexts: TArray<string>;
  Timeout: Cardinal; const ATarget: string);
begin
  DiscoverManagersFiltered(AProfileGroups, AProfileTexts, Timeout, ATarget.Split([';']));
end;

procedure TTetheringManager.DiscoverManagersFiltered(const AProfileGroups, AProfileTexts: TArray<string>;
  const ATarget: string);
begin
  DiscoverManagersFiltered(AProfileGroups, AProfileTexts, ATarget.Split([';']));
end;

procedure TTetheringManager.DiscoverManagersFiltered(const AProfileGroups, AProfileTexts: TArray<string>;
  Timeout: Cardinal; const ATargetList: TTetheringTargetHosts);
var
  I: Integer;
begin
  if Enabled then
  begin
    for I := 0 to FAdapters.Count - 1 do
      FAdapters[I].StartManagersDiscovery(Timeout, ATargetList, AProfileGroups, AProfileTexts);
  end
  else
    raise ETetheringException.CreateFmt(SDisabledManager, [Identifier]);
end;
...
begin
  ...
  begin
    for I := 0 to FAdapters.Count - 1 do
      FAdapters[I].StartManagersDiscovery(FAdapters[I].GetAutomaticTimeout, ATargetList, AProfileGroups, AProfileTexts);
  end
  else
    raise ETetheringException.CreateFmt(SDisabledManager, [Identifier]);
end;
  ...
  begin
    ...
    begin
      ...
      try
        ...
        ManagerIndex := FindAutoManager(ARemoteManagers[I].ManagerIdentifier);
        if ManagerIndex >= 0 then
          FRemoteAutoManagers.Delete(ManagerIndex);
        if I = ARemoteManagers.Count - 1 then
        begin
          FAutoConnect := False;
          if Assigned(FOnEndAutoConnect) then
            if SynchronizeEvents then
...
procedure TTetheringManager.PairManager(const ARemoteManager: TTetheringManagerInfo; ATimeout: Integer);
var
  I: Integer;
begin
  for I := 0 to FPairedManagers.Count - 1 do
    if FPairedManagers[I].ManagerIdentifier = ARemoteManager.ManagerIdentifier then
      Exit;
  ARemoteManager.Adapter.PairManager(ARemoteManager, ATimeout);
end;
...
begin
  ...
  try
    for I := 0 to ARemoteManagers.Count - 1 do
    begin
      LIndex := TTetheringManagerInfo.FindManagerIndex(FRemoteManagers, ARemoteManagers[I].ManagerIdentifier);
      if LIndex = -1 then
        FRemoteManagers.Add(ARemoteManagers[I])
      else
        FRemoteManagers[LIndex] := FRemoteManagers[LIndex].MergeManagerInfo(ARemoteManagers[I])
    end;
  finally
    TMonitor.Exit(FRemoteManagers);
  end;
...
procedure TTetheringManager.RegisterProfiles(const ARemoteProfiles: TTetheringProfileInfoList);
  ...
  begin
    ...
    for J := 0 to FRemoteProfiles.Count - 1 do
      if (FRemoteProfiles[J].ManagerIdentifier = AProfile.ManagerIdentifier) and
         (FRemoteProfiles[J].ProfileIdentifier = AProfile.ProfileIdentifier) then
        Exit(J);
  end;
...
procedure TTetheringManager.GetGroupsAndTexts(var AProfileGroups, AProfileTexts: TArray<string>);
var
  LProfile: TTetheringProfile;
begin
  AProfileGroups := [];
  ...
  for LProfile in FRegisteredProfiles do
  begin
    if (LProfile.FGroup <> '') and not IsInArray(LProfile.FGroup, AProfileGroups) then
      AProfileGroups := AProfileGroups + [LProfile.FGroup];

    if (LProfile.FText <> '') and not IsInArray(LProfile.FText, AProfileTexts) then
      AProfileTexts := AProfileTexts + [LProfile.FText];
  end;
end;

function TTetheringManager.IsCompatible(const AProfileGroups, AProfileTexts: TArray<string>): Boolean;
var
  LProfile: TTetheringProfile;
begin
  Result := False;
  for LProfile in FRegisteredProfiles do
  begin
    if (LProfile.FGroup <> '') and IsInArray(LProfile.FGroup, AProfileGroups) then
      Exit(True);
    if (LProfile.FText <> '') and IsInArray(LProfile.FText, AProfileTexts) then
      Exit(True);
  end;
end;

class function TTetheringManager.IsInArray(const AItem: string; const AnArray: TArray<string>): Boolean;
var
  I: Integer;
begin
  if Length(AnArray) > 0 then
    for I := Low(AnArray) to High(AnArray) do
      if AItem = AnArray[I] then
        Exit(True);
  Result := False;
end;
...
procedure TTetheringProfile.InternalConsistencyCheck;
begin
...
procedure TTetheringAdapter.AddManagerInfo(const AManagerIdentifier, AManagerName, AManagerText, AConnectionString,
  ...
  function FindManagerIndex(const AManagerList: TTetheringManagerInfoList; const AManagerId: string): Integer;
  var
    I: Integer;
  begin
    Result := -1;
...
begin
  ...
  TMonitor.Enter(FRemoteManagers);
  try
    LIndex := FindManagerIndex(FRemoteManagers, AManagerIdentifier);
  if LIndex = -1 then
  begin
...
begin
  TMonitor.Enter(FRemoteManagers);
  try
    LIndex := FindManagerIndex(FRemoteManagers, AManagerInfo.ManagerIdentifier);
    if LIndex = -1 then
...
var
  Temp: TTetheringManagerInfoList;
  Item: TTetheringManagerInfo;
begin
  if FTimerManager <> nil then
    FTimerManager.Cancel;

  Temp := TTetheringManagerInfoList.Create;
  try
    TMonitor.Enter(FRemoteManagers);
    try
      Temp.Capacity := FRemoteManagers.Count;

      for Item in FRemoteManagers do
        Temp.Add(Item);

    finally
      TMonitor.Exit(FRemoteManagers);
    end;
    FManager.DoEndAutoConnectDiscover(Temp);
  finally
    Temp.Free;
  end;
end;
...
begin
  FCommThread := DoCreateCommunicationThread(Self, Manager.BindToAddress);
...
procedure TTetheringAdapter.StartManagersAutoConnect(Timeout: Cardinal; const ATargetList: TTetheringTargetHosts;
  const AProfileGroups, AProfileTexts: TArray<string>);
begin
  TMonitor.Enter(FRemoteManagers);
  try
  ...
  TThread.CreateAnonymousThread(procedure
    begin
      DoDiscoverManagers(Timeout, ATargetList, AProfileGroups, AProfileTexts);
    end).Start;
end;

procedure TTetheringAdapter.StartManagersAutoConnect(const ATargetList: TTetheringTargetHosts);
begin
  StartManagersAutoConnect(GetAutomaticTimeout, ATargetList, nil, nil);
end;
...
procedure TTetheringAdapter.PairManager(const ARemoteManager: TTetheringManagerInfo; ATimeout: Integer);
var
  LOldTimeout: Integer;
begin
  if FCommThread <> nil then
  begin
    LOldTimeout := FCommThread.ReconnectTimeout;
    try
      FCommThread.ReconnectTimeout := ATimeout;
      PairManager(ARemoteManager);
    finally
      FCommThread.ReconnectTimeout := LOldTimeout;
    end;
  end;
end;
...
begin
  ...
  FReconnectTimeout := TTetheringCustomComm.ProtocolSpecificTimeout;
end;
...
function TTetheringManagerCommunicationThread.ProcessResponse(const AData: string; var RemoteConnectionString: string): string;
  ...
  function IsPreferredAdapter(const ARemoteProfileId, ARemoteAdapters: string): Boolean;
  var
  ...
  function IsManagerCompatible(const ACommand: TTetheringManagerCommand): Boolean;
  var
    LGroupList: TArray<string>;
    LTextList: TArray<string>;
  begin
    if ACommand.NumParams = 3 then
    begin
      if ACommand.Params[1] <> EMPTYTOKEN then
        LGroupList := TTetheringManagerCommand.Split(ACommand.Params[1], ';');
      if ACommand.Params[2] <> EMPTYTOKEN then
        LTextList := TTetheringManagerCommand.Split(ACommand.Params[2], ';');

      Result := FAdapter.Manager.IsCompatible(LGroupList, LTextList);
    end
    else
      // If it is a Discover without filters, it's always compatible.
      Result := True;
  end;
    ...
    TPairingState.Waiting:
      begin
        ...
        begin
          ...
          LNetworkInfo.ManagerVersion := LVersion;
          if LCommand.Version = 0 then
            Result := CommandStr(TetheringServerDiscovered, LVersion,
              [FAdapter.Manager.Identifier, FAdapter.Manager.Name, FAdapter.Manager.Text])
          else
            if IsManagerCompatible(LCommand) then
        ...
        begin
          LProcessCommand := (LCommand.NumParams = 4) or ((LCommand.NumParams = 5) and IsPreferredAdapter(LCommand.Params[0], LCommand.Params[4]));
          ...
          begin
            ...
            begin
              ...
              if TTetheringAdapter.IsLoggingItem(TTetheringAdapter.TTetheringLogItem.Adapter) then
                FAdapter.Log(Format('Send(%s): state: %s; Resp: <%s>', [RemoteConnectionString,
...
begin
  // If we have an error pairing in AutoConnect mode we have to remove the manager from the list
  if FAdapter.FManager.FAutoConnect and (AState = TPairingState.Waiting) and
    (GetNetworkInfo(Connection).State <> TPairingState.Waiting) then
  begin
    LManagerIndex := FAdapter.FManager.FindAutoManager(GetNetworkInfo(Connection).ManagerId);
    if LManagerIndex >= 0 then
      FAdapter.FManager.DeleteRemoteAutoManagers(LManagerIndex);

    if FAdapter.FManager.FRemoteAutoManagers.Count = 0 then
    begin
      FAdapter.FManager.FAutoConnect := False;
      if Assigned(FAdapter.FManager.FOnEndAutoConnect) then
        TThread.Synchronize(nil,
          procedure
          begin
            FAdapter.FManager.FOnEndAutoConnect(FAdapter.FManager);
          end);
    end;
  end;
