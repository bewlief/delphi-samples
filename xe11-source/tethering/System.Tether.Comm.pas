...
{ Network classes }
  ...
  protected
    ...
    function DoConnect(const ATarget: string; ATimeout: Integer): Boolean; override;
  ...
  private
    ...
    FOnUDPException: TNetworkCommUDPException;

    procedure DoUDPException(AThread: IIPUDPListenerThread; ABinding: IIPSocketHandle; const
      AMessage : String; const AExceptionClass: TClass);
  ...
  public
    ...
    /// <summary>Property to specify a procedure that is executed when unhandled exception occurs in the UDP server </summary>
    property OnUDPException: TNetworkCommUDPException read FOnUDPException write FOnUDPException;
  end;
...
function TTetheringCustomComm.Connect(const ATarget: string; ATimeout: Integer): Boolean;
var
  Res: Boolean;
begin
  ...
  begin
    ...
    FTarget := ATarget;
    try
      Res := DoConnect(ATarget, ATimeout);
    except
      Res := False;
    end;
    Result := Res;
...
begin
  Result := Connect(ATarget, ProtocolSpecificTimeout);
end;
...
begin
  if Assigned(FOnAfterReceiveData) then
    Result := TBytes(FOnAfterReceiveData(Self, TClassicByteDynArray(DoReadData)))
  else
    Result := DoReadData;
...
begin
  ...
  if Assigned(FOnBeforeSendData) then
    DoWriteData(TBytes(FOnBeforeSendData(Self, TClassicByteDynArray(AData))))
  else
    DoWriteData(AData);
end;
...
function TTetheringNetworkComm.DoConnect(const ATarget: string; ATimeout: Integer): Boolean;
var
  Pos: Integer;
  LHost: string;
  LPort: Integer;
begin
  ...
  if ATimeout = ProtocolSpecificTimeout then
    ATimeout := 5000;
...
procedure TTetheringNetworkServerCommUDP.DoUDPException(AThread: IIPUDPListenerThread;
  ABinding: IIPSocketHandle; const AMessage : String; const AExceptionClass: TClass);
begin
  if Assigned(FOnUDPException) then
    FOnUDPException(AMessage, AExceptionClass);
end;
...
begin
  ...
  for I := 0 to FUDPServer.Bindings.Count - 1 do
    FUDPServer.Bindings.Sockets[I].Port := Value.ToInteger;
end;
...
function TTetheringBTComm.DoConnect(const ATarget: string; ATimeout: Integer): Boolean;
type
