...
function TTetheringTCPProtocol.Connect(const AProfile: TTetheringConnection; ATimeout: Integer): Boolean;
const
...
begin
  ...
  if SepPos > 0 then
  begin
    Result := FTCPClient.Connect(AProfile.ConnectionString, ATimeout);
    if Result then
    begin
...
begin
  Result := Connect(AProfile, TTetheringCustomComm.ProtocolSpecificTimeout);
end;
