...
type
  ...
  TShortIntDynArray     = TArray<ShortInt>;
  {$EXTERNALSYM TShortIntDynArray 'System::TInt8DynArray'}
  TByteDynArray         = TArray<Byte>;
  {$EXTERNALSYM TByteDynArray 'System::TByteDynArray'}
  TSmallIntDynArray     = TArray<SmallInt>;
  {$EXTERNALSYM TSmallIntDynArray 'System::TSmallIntDynArray'}
  TWordDynArray         = TArray<Word>;
  ...
  TUInt64DynArray        = TArray<UInt64>;
  {$EXTERNALSYM TUInt64DynArray 'System::TUInt64DynArray'}

{$IF SizeOf(LongWord) = SizeOf(Cardinal)}
  TLongIntDynArray      = TIntegerDynArray;
  TLongWordDynArray     = TCardinalDynArray;
{$ENDIF}
{$IF SizeOf(LongWord) = SizeOf(UInt64)}
  TLongIntDynArray      = TInt64DynArray;
  TLongWordDynArray     = TUInt64DynArray;
{$ENDIF}
  {$EXTERNALSYM TLongIntDynArray}
...
{$IFNDEF NEXTGEN}
  TWideStringDynArray   = TArray<WideString>;
...
type
  ...
  {.$DEFINE SYSTEM_TYPES_HPP_DEFINES_TYPES}
  {$IFNDEF SYSTEM_TYPES_HPP_DEFINES_TYPES}
    {$NODEFINE TSize}
    {$NODEFINE PSize}
  {$ENDIF}
...
// them is also called LOCK_WRITE, which produces error with the defined here.
  LOCK_WRITE     = 1;
{$IF defined(MSWINDOWS) or defined(ANDROID) or defined(LINUX)}
...
type
  ...
  protected
    ...
    ///  </summary>
    class function GetTickCount64: UInt64; virtual; abstract;
    ///  <summary>
...
begin
  ...
  begin
    CurrentTicks := FMultiEventType.GetTickCount64;
      ...
      try
        ...
        begin
          ...
          begin
            ...
            begin
              Now := FMultiEventType.GetTickCount64;
