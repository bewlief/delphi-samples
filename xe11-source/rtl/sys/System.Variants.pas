...
{$IFDEF NEXTGEN}
    procedure VarDataFromLStr(var V: TVarData; const Value: RawByteString);
{$ELSE  NEXTGEN}
...
{$ENDIF !NEXTGEN}
procedure _VarToLStr(var S: RawByteString; const V: TVarData; CodePage: Word = 0);
...
procedure _VarFromCurr; // var V: TVarData; const Value: Currency
{$ENDIF !X86ASM}
procedure _VarFromLStr(var V: TVarData; const Value: RawByteString);
{$IFNDEF NEXTGEN}
...
procedure _OleVarFromLStr(var V: TVarData; const Value: RawByteString);
{$IFNDEF NEXTGEN}
...
{$ELSE NEXTGEN}
  TStringRef = record
    ...
    function FromUTF8(U8: PUTF8String): PString;
...
procedure InitVariantConstants;
...
var
  VarBooleanStr: array[Boolean, TBooleanToStringRule] of string;
  ByteShortToStr: array[-128..255] of string;
...
function _VarToLongWord(const V: TVarData): Cardinal; forward;
...
procedure _VarFromLongWord(var V: TVarData; const Value: Cardinal); forward;
...
procedure VarCastErrorV(var V: TVarData);
begin
  VarCastError;
end;
...
{$IFDEF EXTERNALLINKER}
  DispInvokeCore(Dest, Src^, CallDesc, Pointer(@VAList));
{$ELSE !EXTERNALLINKER}
...
function TStringRef.FromUTF8(U8: PUTF8String): PString;
begin
  UTF8 := U8;
  Unicode := nil;
  Wide := string(U8^);
...
{$IFDEF NEXTGEN}
          ...
          Temp := VarArgGetValue(VAList, Pointer);
          if UTF8String(Temp) <> '' then
          begin
            PVarParm^.VPointer := PWideChar(Strings[StringCount].FromUTF8(@UTF8String(Temp))^);
  ...
  begin
    ...
    begin
      ...
      begin
        PVarData(PVarParm)^.VType := varByRef or varOleStr;
{$IFDEF NEXTGEN}
        PVarData(PVarParm)^.VPointer := Strings[StringCount].FromUTF8(PUTF8String(Params^));
{$ELSE !NEXTGEN}
...
{$ENDIF}
        ...
        begin
          ...
          begin
            PVarParm^.VType := varInteger;
            PVarParm^.VInteger := Integer(PCardinal(Params)^);
          end else
            PVarParm^.VUInt32 := PCardinal(Params)^;
        end;
...
{$IFDEF NEXTGEN}
          PVarParm^.VType := varOleStr;
          if PUTF8String(Params)^ <> '' then
          begin
            PVarParm^.VPointer := PWideChar(Strings[StringCount].FromUTF8(PUTF8String(Params))^);
            Strings[StringCount].UTF8 := nil;
            Inc(StringCount);
          end
          else
...
begin
  ...
  begin
    ...
    Dest.VString := nil;
    RawByteString(Dest.VString) := RawByteString(Source.VString);
  end
...
begin
  case Source.VType and not varByRef of
    ...
    varUInt32:   Variant(Dest) := PCardinal(Source.VPointer)^;
...
begin
  ...
  begin
    OleStrPtr := StringToOleStr(RawByteString(Source.VString))
  end
  else //varUString
...
begin
  if Source.VType = AVarType then
    ...
    case AVarType of
      ...
      varUInt32:   _VarFromLongWord(Dest, _VarToLongWord(Source));
...
function _VarToLongWord(const V: TVarData): Cardinal;
begin
  case V.VType of
    varEmpty:    Result := 0;
    varNull:
      begin
        if NullStrictConvert then
          VarCastError(varNull, varUInt32);
        Result := 0;
      end;
    varBoolean:  Result := Cardinal(V.VBoolean);
    varByte:     Result := V.VByte;
    varWord:     Result := V.VWord;
    varUInt32:   Result := V.VUInt32;
  else
    try
      ...
      HandleConversionException(V.VType, varUInt32);
      Result := 0;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if LResult = VAR_TYPEMISMATCH then
      begin
        if TryStrToBool(S, B) then
          Result := Int64(B)
        else
...
begin
  case V.VType of
    varString:
      begin
        if RawByteString(V.VString) = '0' then Exit(False)
        else if RawByteString(V.VString) = '-1' then Exit(True);
        S := WideString(RawByteString(V.VString));
        LResult := VarBoolFromStr(PWideChar(S), VAR_LOCALE_USER_DEFAULT, 0, LValue);
      end;
    varUString:
      begin
        if UnicodeString(V.VUString) = '0' then Exit(False)
        else if UnicodeString(V.VUString) = '-1' then Exit(True);
        S := UnicodeString(V.VUString);
        LResult := VarBoolFromStr(PWideChar(V.VUString), VAR_LOCALE_USER_DEFAULT, 0, LValue);
      end;
    varOleStr:
      begin
        S := V.VOleStr;
        LResult := VarBoolFromStr(V.VOleStr, VAR_LOCALE_USER_DEFAULT, 0, LValue);
      end;
  else
    _VarToWStr(S, V);
    LResult := VarBoolFromStr(PWideChar(S), VAR_LOCALE_USER_DEFAULT, 0, LValue);
  ...
  case V.VType of
    ...
    if V.VType and varByRef <> 0 then
      case V.VType and not varByRef of
        ...
        varUInt32:   Result := PCardinal(V.VPointer)^ <> 0;
...
procedure _VarToLStr(var S: RawByteString; const V: TVarData; CodePage: Word);

  procedure SetIntToStr(var S: RawByteString; const Value: Int32);
  begin
    S := IntToStr(Value);
  end;

  procedure SetUIntToStr(var S: RawByteString; const Value: UInt32);
  begin
    S := UIntToStr(Value);
  end;

  procedure SetInt64ToStr(var S: RawByteString; const Value: Int64);
  begin
    S := IntToStr(Value);
  end;

  procedure SetUInt64ToStr(var S: RawByteString; const Value: UInt64);
  begin
    S := UIntToStr(Value);
  end;

  procedure SetFloatToStr(var S: RawByteString; const Value: Extended);
  begin
    S := FloatToStr(Value);
  end;

  procedure SetCurrToStr(var S: RawByteString; const Value: Currency);
  begin
    S := CurrToWStrViaOS(Value);
  end;

  procedure SetDateToStr(var S: RawByteString; const Value: TDateTime);
  begin
    S := DateToWStrViaOS(Value);
  end;

  procedure SetVarToStr(var S: RawByteString; const Value: TVarData);
  begin
    S := VarToWStrViaOS(Value);
  end;

  function SetCustomToStr(var S: RawByteString; const Value: TVarData): Boolean;
  var
    LHandler: TCustomVariantType;
    LTemp: TVarData;
  begin
    Result := FindCustomVariantType(Value.VType, LHandler);
    if Result then
    begin
      VariantInit(LTemp);
      try
        LHandler.CastTo(LTemp, Value, varString);
        S := RawByteString(LTemp.VString);
      finally
        _VarClear(LTemp);
      end;
    end;
  ...
  procedure SetAnyToStr(var S: RawByteString; const Value: TVarData);
  var
    LTemp: TVarData;
  begin
    VariantInit(LTemp);
    try
      ...
      ChangeAnyProc(LTemp);
      _VarToLStr(S, LTemp, CodePage);
    finally
      _VarClear(LTemp);
    end;
  end;

  procedure SetByRef(var S: RawByteString; const V: TVarData);
  begin
    case V.VType and not varByRef of
      varSmallInt: SetIntToStr(S, PSmallInt(V.VPointer)^);
      varInteger:  SetIntToStr(S, PInteger(V.VPointer)^);
      varSingle:   SetFloatToStr(S, PSingle(V.VPointer)^);
      varDouble:   SetFloatToStr(S, PDouble(V.VPointer)^);
      varCurrency: SetCurrToStr(S, PCurrency(V.VPointer)^);
      varDate:     SetDateToStr(S, PDate(V.VPointer)^);
      varOleStr:   SetAnsiString(@S, PWideChar(V.VPointer^), Length(WideString(V.VPointer^)), CodePage);
      varBoolean:  S := VarBooleanStr[Ord(PWordBool(V.VPointer)^) <> 0, BooleanToStringRule];
      varShortInt: SetIntToStr(S, PShortInt(V.VPointer)^);
      varByte:     SetUIntToStr(S, PByte(V.VPointer)^);
      varWord:     SetUIntToStr(S, PWord(V.VPointer)^);
      varUInt32:   SetUIntToStr(S, PCardinal(V.VPointer)^);
      varInt64:    SetInt64ToStr(S, PInt64(V.VPointer)^);
      varUInt64:   SetUInt64ToStr(S, PUInt64(V.VPointer)^);
      varString:   S := PUTF8Char(V.VPointer^);
{$IFDEF NEXTGEN}
      varUString:  S := UTF8String(PWideChar(V.VPointer^));
{$ELSE  NEXTGEN}
      varUString:  S := AnsiString(PWideChar(V.VPointer^));
{$ENDIF NEXTGEN}
      varVariant:  _VarToLStr(S, PVarData(V.VPointer)^);
    else           SetVarToStr(S, V);
    end;
  end;
...
begin
  case V.VType of
    ...
    varSmallInt: if (V.VSmallInt >= -128) and (V.VSmallInt < 256) then S := ByteShortToStr[V.VSmallInt] else SetIntToStr(S, V.VSmallInt);
    varInteger:  if (V.VInteger >= -128) and (V.VInteger < 256) then S := ByteShortToStr[V.VInteger] else SetIntToStr(S, V.VInteger);
    varSingle:   SetFloatToStr(S, V.VSingle);
    varDouble:   SetFloatToStr(S, V.VDouble);
    varCurrency: SetCurrToStr(S, V.VCurrency);
    varDate:     SetDateToStr(S, V.VDate);
    varOleStr:   SetAnsiString(@S, V.VOleStr, Length(WideString(Pointer(V.VOleStr))), CodePage);
    varBoolean:  S := VarBooleanStr[Ord(V.VBoolean) <> 0, BooleanToStringRule];
    varShortInt: S := ByteShortToStr[V.VShortInt];
    varByte:     S := ByteShortToStr[V.VByte];
    varWord:     if V.VWord < 256 then S := ByteShortToStr[V.VWord] else SetUIntToStr(S, V.VWord);
    varUInt32:   if V.VLongWord < 256 then S := ByteShortToStr[V.VUInt32] else SetUIntToStr(S, V.VUInt32);
    varInt64:    SetInt64ToStr(S, V.VInt64);
    varUInt64:   SetUInt64ToStr(S, V.VUInt64);
    varVariant:  _VarToLStr(S, PVarData(V.VPointer)^);
    varDispatch,
    varUnknown:  SetVarToStr(S, V);
    varString:   S := RawByteString(V.VString);
    varUString:  SetAnsiString(@S, PWideChar(UnicodeString(V.VUString)), Length(UnicodeString(V.VUString)), CodePage);
    varAny:      SetAnyToStr(S, V);
    else
      if V.VType and varByRef <> 0 then
        SetByRef(S, V)
      else
        if not SetCustomToStr(S, V) then
          SetVarToStr(S, V);
  end;
end;
...
begin
  _VarToLStr(LTemp, V);
  ShortString(S) := AnsiString(LTemp);
end;

procedure _VarToWStr(var S: WideString; const V: TVarData);

  procedure SetIntToStr(var S: WideString; const Value: Int32);
  begin
    S := IntToStr(Value);
  end;

  procedure SetUIntToStr(var S: WideString; const Value: UInt32);
  begin
    S := UIntToStr(Value);
  end;

  procedure SetInt64ToStr(var S: WideString; const Value: Int64);
  begin
    S := IntToStr(Value);
  end;

  procedure SetUInt64ToStr(var S: WideString; const Value: UInt64);
  begin
    S := UIntToStr(Value);
  end;

  procedure SetFloatToStr(var S: WideString; const Value: Extended);
  begin
    S := FloatToStr(Value);
  end;

  procedure SetCurrToStr(var S: WideString; const Value: Currency);
  begin
    S := CurrToWStrViaOS(Value);
  end;

  procedure SetDateToStr(var S: WideString; const Value: TDateTime);
  begin
    S := DateToWStrViaOS(Value);
  end;

  procedure SetVarToStr(var S: WideString; const Value: TVarData);
  begin
    S := VarToWStrViaOS(Value);
  end;

  function SetCustomToStr(var S: WideString; const Value: TVarData): Boolean;
  var
    LHandler: TCustomVariantType;
    LTemp: TVarData;
  begin
    ...
    begin
      VariantInit(LTemp);
      try
        LHandler.CastTo(LTemp, Value, varOleStr);
        S := WideString(Pointer(LTemp.VOleStr));
      finally
        _VarClear(LTemp);
      end;
    end;
  ...
  procedure SetAnyToStr(var S: WideString; const Value: TVarData);
  var
    LTemp: TVarData;
  begin
    VariantInit(LTemp);
  ...
  procedure SetByRef(var S: WideString; const V: TVarData);
  begin
    case V.VType and not varByRef of
      varSmallInt: SetIntToStr(S, PSmallInt(V.VPointer)^);
      varInteger:  SetIntToStr(S, PInteger(V.VPointer)^);
...
procedure _VarToUStr(var S: UnicodeString; const V: TVarData);
  ...
  procedure SetIntToStr(var S: UnicodeString; const Value: Int32);
  begin
    S := IntToStr(Value);
  end;

  procedure SetUIntToStr(var S: UnicodeString; const Value: UInt32);
  begin
    S := UIntToStr(Value);
  end;

  procedure SetInt64ToStr(var S: UnicodeString; const Value: Int64);
  begin
    S := IntToStr(Value);
  end;

  procedure SetUInt64ToStr(var S: UnicodeString; const Value: UInt64);
  begin
    S := UIntToStr(Value);
  end;

  procedure SetFloatToStr(var S: UnicodeString; const Value: Extended);
  begin
    S := FloatToStr(Value);
  end;

  procedure SetCurrToStr(var S: UnicodeString; const Value: Currency);
  begin
    S := CurrToWStrViaOS(Value);
  end;

  procedure SetDateToStr(var S: UnicodeString; const Value: TDateTime);
  begin
    S := DateToWStrViaOS(Value);
  end;

  procedure SetVarToStr(var S: UnicodeString; const Value: TVarData);
  begin
    S := VarToWStrViaOS(Value);
  end;

  function SetCustomToStr(var S: UnicodeString; const Value: TVarData): Boolean;
  var
    LHandler: TCustomVariantType;
    LTemp: TVarData;
  begin
    ...
    begin
      VariantInit(LTemp);
      try
        LHandler.CastTo(LTemp, Value, varUString);
        _VarToUStr(S, LTemp);
      finally
        _VarClear(LTemp);
      end;
  ...
  procedure SetAnyToStr(var S: UnicodeString; const Value: TVarData);
  var
    LTemp: TVarData;
  begin
    VariantInit(LTemp);
  ...
  procedure SetByRef(var S: UnicodeString; const V: TVarData);
  begin
    case V.VType and not varByRef of
    varSmallInt: SetIntToStr(S, PSmallInt(V.VPointer)^);
    varInteger:  SetIntToStr(S, PInteger(V.VPointer)^);
    ...
    varOleStr:   S := PWideChar(V.VPointer^);
    varBoolean:  S := VarBooleanStr[Ord(PWordBool(V.VPointer)^) <> 0, BooleanToStringRule];
    varString:   S := UnicodeString(PUTF8String(V.VPointer^));
    varUString:  S := PWideChar(V.VPointer^);
    else         SetVarToStr(S, V);
    end;
  end;
...
const
  CRangeToType: array [-4..4] of TVarType = (varInteger, varError, varSmallInt, varShortInt,
                                             varError,
                                             varByte, varWord, varError, varUInt32);
begin
  if (V.VType and varDeepData) <> 0 then
    VarClearDeep(V);
...
procedure _VarFromLongWord(var V: TVarData; const Value: Cardinal);
begin
  if (V.VType and varDeepData) <> 0 then
    VarClearDeep(V);
  V.VType := varUInt32;
...
begin
  ...
  V.VString := nil;
  V.VType := varString;
  RawByteString(V.VString) := Value;
end;

{$IFNDEF NEXTGEN}
procedure _VarFromPStr(var V: TVarData; const Value: ShortString);
begin
  _VarFromLStr(V, AnsiString(Value));
end;
...
begin
  ...
  V.VOleStr := nil;
  V.VType := varOleStr;
  WideString(Pointer(V.VOleStr)) := Value;
end;
{$ENDIF !NEXTGEN}
...
procedure _OleVarFromLStr(var V: TVarData {OleVariant}; const Value: RawByteString);
begin
...
begin
  ...
  begin
    ...
    case Source.VType of
      ...
      varUInt32:
        if Source.VUInt32 and $80000000 <> 0 then
      ...
      varInt64:
        if (Source.VInt64 <= High(Integer)) and (Source.VInt64 >= Low(Integer)) then
          ...
          if not OleVariantInt64AsDouble then
            ...
            _VarCast(Dest, Source, varDouble);
      varString:
        _OleVarFromLStr(Dest, RawByteString(Source.VString));
      varUString:
...
procedure NegateCardinal(const Value: Cardinal; var Dest: TVarData);
begin
...
begin
  case Dest.VType of
    ...
    varUInt32:   NegateCardinal(Dest.VUInt32, Dest);
    ...
    if (Dest.VType and varByRef) <> 0 then
      case Dest.VType and not varByRef of
        ...
        varUInt32:   NegateCardinal(PCardinal(Dest.VPointer)^, Dest);
...
begin
  case Dest.VType of
    ...
    varUInt32:   Dest.VUInt32 := not Dest.VUInt32;
    ...
    if (Dest.VType and varByRef) <> 0 then
      case Dest.VType and not varByRef of
        ...
        varUInt32:   Variant(Dest) := not PCardinal(Dest.VPointer)^;
...
begin
  Result := AVarType in [varSmallInt, varInteger, varBoolean, varShortInt,
                         varByte, varWord, varUInt32, varInt64, varUInt64];
end;
    ...
    repeat
      ...
      begin
        case DAVarType of
          ...
          varUInt32:    Value := PCardinal(PDAData)^;
          varInt64:     Value := PInt64(PDAData)^;
          varUInt64:    Value := PUInt64(PDAData)^;
{$IFDEF NEXTGEN}
          varString:    Value := PRawByteString(PDAData)^;
{$ELSE !NEXTGEN}
    ...
    repeat
      ...
      begin
        ...
        case DAVarType of
          ...
          varUInt32:    PCardinal(PDAData)^     := Value;
...
{$IFDEF NEXTGEN}
procedure TCustomVariantType.VarDataFromLStr(var V: TVarData; const Value: RawByteString);
{$ELSE  NEXTGEN}
...
var
  ...
  I: Integer;
begin
  if not Assigned(VarBStrFromBool) then
    Exit;

  VarResultCheck(VarBStrFromBool(False, VAR_LOCALE_USER_DEFAULT, 0, S),
                 varBoolean, varOleStr);
  VarBooleanStr[False, bsrAsIs] := S;
  VarBooleanStr[False, bsrLower] := AnsiLowerCase(S);
  VarBooleanStr[False, bsrUpper] := AnsiUpperCase(S);

  VarResultCheck(VarBStrFromBool(True, VAR_LOCALE_USER_DEFAULT, 0, S),
                 varBoolean, varOleStr);
  VarBooleanStr[True, bsrAsIs] := S;
  VarBooleanStr[True, bsrLower] := AnsiLowerCase(S);
  VarBooleanStr[True, bsrUpper] := AnsiUpperCase(S);

  for I := Low(ByteShortToStr) to High(ByteShortToStr) do
    ByteShortToStr[I] := IntToStr(I);
end;
