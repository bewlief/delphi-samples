...
{$IF Defined(CPUX64)}
procedure SignalConverterUnAligned(ExceptionEIP: NativeUInt; FaultAddr: NativeUInt; ErrorCode: NativeUInt); cdecl;
  external 'librtlhelper.a' name 'SignalConverterUnAligned';
...
begin
  ...
  KRes := thread_get_state(Thread, x86_EXCEPTION_STATE,
                           thread_state_t(@ExceptionState), ExceptionStateCount);
  Assert(ExceptionState.esh.flavor = {$IFDEF CPUX86}x86_EXCEPTION_STATE32{$ELSE}x86_EXCEPTION_STATE64{$ENDIF});
  Assert(ExceptionState.esh.count  = sizeof({$IFDEF CPUX86}x86_exception_state32_t{$ELSE}x86_exception_state64_t{$ENDIF}) div sizeof(natural_t));
  if KRes <> KERN_SUCCESS then
  begin
    // Fatal
    Result := KERN_INVALID_ARGUMENT;
...
{$ELSE !CPUX86}
  if (ExceptionState.es64.__trapno = $E) and
       ((ThreadState^.ts64.rsp and $FFFFFFFFFFFFF000) = (ExceptionState.es64.__faultvaddr and $FFFFFFFFFFFFF000)) then
{$ENDIF CPUX86}
  begin
//    writeln('stack fault');
     {
...
{$ELSEIF Defined(CPUX64)}
  ThreadState^.ts64.rdi := ThreadState^.ts64.rip;
  ThreadState^.ts64.rsi := ExceptionState.es64.__faultvaddr;
  ThreadState^.ts64.rdx := ExcCode;

  // Check and set stack alignment
  if ThreadState^.ts64.rsp mod $10 = $8 then
    // Unaligned stack pointer.
    ThreadState^.ts64.rip := UIntPtr(@SignalConverterUnAligned)
  else
    // normal case
    ThreadState^.ts64.rip := UIntPtr(@SignalConverter);
  // Emulate CALL SignalConverter
  ThreadState^.ts64.rsp := ThreadState^.ts64.rsp - sizeof(Pointer);
  PUInt64(ThreadState^.ts64.rsp)^ := ThreadState^.ts64.rip;

  Px86_thread_state_t(NewState)^ := ThreadState^;
{$ELSE}
  {$MESSAGE ERROR 'Unknown CPU'}
{$ENDIF}
