...
{$IF Defined(LINUX64) or Defined(OSX64) or (Defined(IOSSIMULATOR) and Defined(CPUX64)) }
procedure SignalConverterUnAligned(ExceptionEIP: NativeUInt; FaultAddr: NativeUInt; ErrorCode: NativeUInt); cdecl;
  external 'librtlhelper.a' name _PU + 'SignalConverterUnAligned';
{$ENDIF LINUX64 or OSX64 or Simulator64}
...
{$IFDEF ANDROID}
 ...
 {$ELSE ANDROID64}
    begin
      ...
      UserContext.uc_mcontext.arm_lr := UserContext.uc_mcontext.arm_pc;
      NextInst := UIntPtr(@SignalConverter);
    end;
 {$ENDIF ANDROID64}
  end;
...
{$IFDEF MACOS}
 ...
 {$IF defined(CPUX64)}
  ...
  PUInt64(mcontext^.__ss.__rsp)^ := mcontext^.__ss.__rip;
  ...
  if mcontext^.__ss.__rsp mod $10 = 0 then
    mcontext^.__ss.__rip := UIntPtr(@SignalConverterUnAligned)
  else
...
{$IFDEF LINUX}
 ...
 {$IFDEF CPUX64}
  ...
  begin
    ...
    if mcontext^.gregs[REG_RSP] mod $10 = 0 then
      mcontext^.gregs[REG_RIP] := greg_t(@SignalConverterUnAligned)
    else
