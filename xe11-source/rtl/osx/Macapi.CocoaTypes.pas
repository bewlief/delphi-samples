...
{$M+}
  ...
  NSPoint = record
    ...
    class operator Explicit(Point: NSPoint):TPointF;
    class operator Explicit(Point: TPointF):NSPoint;
  end;
  PNSPoint = ^NSPoint;
  CGPoint = NSPoint;
  PCGPoint = ^CGPoint;
  ...
  NSSize = record
    ...
    class operator Explicit(Size: NSSize):TSizeF;
    class operator Explicit(Size: TSizeF):NSSize;
    class operator Explicit(Size: NSSize):TPointF;
    class operator Explicit(Size: TPointF):NSSize;
  end;
  PNSSize = ^NSSize;
  CGSize = NSSize;
  PCGSize = ^CGSize;
...
function MakeNSRect(ox, oy, sx, sy: CGFloat): NSRect;
...
class operator NSPoint.Explicit(Point: NSPoint):TPointF;
begin
  Result.x := Point.x;
  Result.y := Point.y;
end;

class operator NSPoint.Explicit(Point: TPointF):NSPoint;
begin
  Result.x := Point.x;
  Result.y := Point.y;
end;
...
class operator NSSize.explicit(Size: NSSize):TSizeF;
begin
  Result.cx := Size.width;
  Result.cy := Size.height;
end;

class operator NSSize.explicit(Size: TSizeF):NSSize;
begin
  Result.width := Size.cx;
  Result.height := Size.cy;
end;

class operator NSSize.explicit(Size: NSSize):TPointF;
begin
  Result.x := Size.width;
  Result.y := Size.height;
end;

class operator NSSize.explicit(Size: TPointF):NSSize;
begin
  Result.width := Size.x;
