...
var
  ...
  begin
    if (InvokeData.RetKind = rkObject) or (InvokeData.RetKind = rkConstructor) then
    begin
    ...
    mkObjectMethod:
      begin
        ...
        case InvokeData.RetKind of
          ...
          rkByte:
            PShortInt(PResult)^ := JNIEnv^.CallByteMethodA(JNIEnv, JNIObject(LInstance), InvokeData.MethodID, DstParams);
          rkChar:
          ...
          rkInstance:
            begin
              AJNIObject := JNIEnv^.CallObjectMethodA(JNIEnv, JNIObject(LInstance), InvokeData.MethodID, DstParams);
              HandleJNIException(JNIEnv);
              PPointer(PResult)^ := WrapJNIArray(AJNIObject, InvokeData.JavaType);
            end;
          rkConstructor:
            begin
...
procedure ExecJNI(SrcParams, DstParams, PResult: Pointer; InvokeData: PJNIMethodInvokeData; FPURegs: Pm128); cdecl;
var
  Src: PByte;
  JArraySrc: TArray<Pointer>;
  CntJArraySrc: Integer;
...
//    if (InvokeData.RetKind = rkObject) or (InvokeData.RetKind = rkConstructor) or (InvokeData.RetKind = rkInstance) then
    // ARM64 use X8 register for indirect  return value address.
  end;
  ...
  begin
    // Src and ARMRegister points next of user parameter
    // Get Object instance from top of user parameter.
    LInstance := Pointer(PNativeInt(Src)^);
    LInstance := (IJava(LInstance) as ILocalObject).GetObjectID;
    Inc(Src, SizeOf(Pointer));
  end;
  ...
  begin
    ...
    begin
      case InvokeData.RetKind of
        rkObject, rkConstructor:
          begin
            Res := Pointer(PNativeInt(SrcParams)^); // x8 -> destination var
            IInterface(Res^) := WrappedIntf;
          end;
      end;
    end;
...
begin
  ...
  JNIEnv := TJNIResolver.GetJNIEnv;
  Src := SrcParams;
  Inc(Src, SizeOf(Pointer)); // Skip X8
  ARMRegister := 1;
  PInt64(PResult)^ := 0; // Initialize result buffer
  case InvokeData.MethodType of
    mkObjectMethod:
    ...
    mkClassMethod:
      begin
        SkipResultField;
        //CopyObjectInstance; // No Instance
        Inc(Src, SizeOf(Pointer)); // Skip Delphi instance.
        ProccessParameters;
        case InvokeData.RetKind of
          rkBoolean:
            PByte(PResult)^ := JNIEnv^.CallStaticBooleanMethodA(JNIEnv, JNIClass(InvokeData.Instance), InvokeData.MethodID, DstParams);
