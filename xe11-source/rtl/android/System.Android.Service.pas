...
{$IFNDEF ANDROID}
  ...
  Japp_JobIntentService = interface end;
...
{$ENDIF ANDROID}
  ...
  /// <remarks>Only available in JobIntentService</remarks>
  ...
  private
    ...
    FJavaJobIntentService: Japp_JobIntentService;
  ...
  public
    ...
    /// <remarks>This var will be nil if it's a JobIntentService</remarks>
    property JavaService: JService read FJavaService;
    /// <summary> Property to access to the instance of the java job intent service </summary>
    /// <remarks>This var will be nil if it's a Service</remarks>
    property JavaIntentService: Japp_JobIntentService read FJavaJobIntentService;
...
{$ENDIF ANDROID}
  ...
  /// <summary>Base class to use android JobIntentService</summary>
  /// <remarks>More info in https://developer.android.com/reference/android/support/v4/app/JobIntentService.html</remarks>
  ...
  published
    ...
    /// <summary>Property to manage the Service TrimMemory Event</summary>
    property OnTrimMemory;
    /// <summary>Property to manage the Service OnHandleIntent Event</summary>
...
{$IFDEF ANDROID}
  ...
  public
    ...
    /// <summary>Start a JobIntentService</summary>
    class procedure StartJobIntentService(const AServiceName: string; AJobID: Integer = 1; const AIntent: JIntent = nil); static;
    /// <summary>Bind to service</summary>
    procedure BindService(const AServiceName: string; AFlags: Integer = 1{TJContext.JavaClass.BIND_AUTO_CREATE});
  ...
  public
    ...
    destructor Destroy; override;
    /// <summary>Bind to service</summary>
    procedure BindService(const APackageName, AServiceName: string; AFlags: Integer = 1{TJContext.JavaClass.BIND_AUTO_CREATE});
    /// <summary>Unbind to service</summary>
    procedure UnbindService;
    /// <summary>Event fired when you are connected</summary>
...
begin
  if (FJavaService <> nil) or (FJavaJobIntentService <> nil) then
    inherited;
end;
...
begin
  ...
  if FIntentService <> nil then
  begin
    FIntentService.FJavaJobIntentService := {$IFDEF ANDROID}TJapp_JobIntentService.Wrap(AJavaService){$ELSE}nil{$ENDIF ANDROID};
...
procedure TLocalServiceConnection.TJavaServiceConnection.onBindingDied(name: JComponentName);
begin
end;
...
const
  EMBTJavaServicePrefix = 'com.embarcadero.services.'; // Do not localize

procedure TLocalServiceConnection.BindService(const AServiceName: string; AFlags: Integer);
var
  LIntent: JIntent;
  LService: string;
begin
...
class procedure TLocalServiceConnection.StartJobIntentService(const AServiceName: string; AJobID: Integer; const AIntent: JIntent);
var
  LService: string;
  LActivity: JFMXNativeActivity;
  LIntent: JIntent;
begin
  LService := AServiceName;
  if not LService.StartsWith(EMBTJavaServicePrefix) then
  ...
  begin
    LActivity := TJFMXNativeActivity.Wrap(System.JavaContext);
    LIntent := AIntent;
    if LIntent = nil then
      LIntent := TJIntent.Create;
    LActivity.startJobIntentService(StringToJString(LService), AJobID, LIntent);
  end;
end;
...
procedure TRemoteServiceConnection.TJavaServiceConnection.onBindingDied(name: JComponentName);
begin
end;
...
procedure TRemoteServiceConnection.BindService(const APackageName, AServiceName: string; AFlags: Integer);
var
  LIntent: JIntent;
begin
  if FConnection = nil then
