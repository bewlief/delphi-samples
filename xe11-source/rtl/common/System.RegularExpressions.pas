...
procedure RaiseERegularExpressionError(const AIndex: Integer);
begin
  raise ERegularExpressionError.CreateResFmt(@SRegExIndexOutOfBounds, [AIndex]) at ReturnAddress;
end;

procedure StringRangeCheck(const S: string; Index: Integer); inline;
begin
  if (Index > Length(S)) or (Index <= 0) then
    RaiseERegularExpressionError(Index);
end;
...
begin
  case VarType(Index) of
    ...
    varByte, varSmallint, varInteger, varShortInt, varWord, varUInt32:
      LIndex := Index;
  else
...
begin
  ...
  Result := TMatchCollection.Create(FNotifier, Input, FOptions,
    {$IFDEF PCRE16}StartPos{$ELSE}UnicodeIndexToUTF8(Input, StartPos) + 1{$ENDIF});
end;
...
begin
  StringRangeCheck(Input, StartPos);
  StringRangeCheck(Input, StartPos+Length-1);
  FRegEx.Subject := Input;
  FRegEx.Start := {$IFDEF PCRE16}StartPos{$ELSE}UnicodeIndexToUTF8(Input, StartPos) + 1{$ENDIF};
  FRegEx.Stop := {$IFDEF PCRE16}StartPos + Length - 1{$ELSE}UnicodeIndexToUTF8(Input, StartPos + Length){$ENDIF};
  LSuccess := FRegEx.MatchAgain;
  if LSuccess then
    Result := TMatch.Create(FNotifier, FRegEx.Subject,
      FRegEx.MatchedOffset, FRegEx.MatchedLength, LSuccess)
...
begin
  ...
  begin
    ...
    try
      FRegEx.Subject := Input;
      FRegEx.SplitCapture(List, Count, {$IFDEF PCRE16}StartPos{$ELSE}UnicodeIndexToUTF8(Input, StartPos) + 1{$ENDIF});
