...
type
  ...
  private
    ...
    FViewerList: TList<THelpViewerNode>;
    FExtendedViewerList: TList<THelpViewerNode>;
    FMinCookie: Integer;
    FHandle: THandle;
    FHelpFile: string;
    ...
    function CallSpecialWinHelp(Handle: THandle; const HelpFile: string;
      Command: Word; Data: NativeInt): Boolean;
  public
    constructor Create;
    ...
    { IHelpSystem }
    procedure ShowHelp(const HelpKeyword, HelpFileName: string);
    procedure ShowContextHelp(const ContextID: THelpContext;
      const HelpFileName: string);
    procedure ShowTableOfContents;
    procedure ShowTopicHelp(const Topic, HelpFileName: string); overload;
    procedure AssignHelpSelector(const Selector: IHelpSelector);
                                                        
    function Hook(Handle: THandle; const HelpFile: string;
      Command: Word; Data: NativeInt): Boolean;
    { IHelpSystem2 }
    function UnderstandsKeyword(const HelpKeyword, HelpFileName: string): Boolean;
    { IHelpSystem3 }
...
begin
  inherited Create;
  FViewerList := TList<THelpViewerNode>.Create;
  FExtendedViewerList := TList<THelpViewerNode>.Create;
  FHelpFile := '';
  FMinCookie := 1;
end;
...
begin
  ...
  begin
    HelpNode := FViewerList[FViewerList.Count - 1];
...
function THelpManager.CallSpecialWinHelp(Handle: THandle; const HelpFile: string;
  Command: Word; Data: NativeInt): Boolean;
var
...
begin
  ...
  begin
    ...
    if Supports(FViewerList[I].Viewer, ISpecialWinHelpViewer,
      SpecialViewer) then
    begin
...
begin
  ...
  begin
    Viewers := TList<THelpViewerNode>.Create;

    try
      for I := 0 to FViewerList.Count - 1 do
      begin
        //HelpNode := THelpViewerNode(FViewerList[I]);
        HelpNode := FViewerList[I];
...
procedure THelpManager.ShowContextHelp(const ContextID: THelpContext; const HelpFileName: string);
var
  I: Integer;
  HelpCount: Integer;
  ...
  procedure DefaultContextHelp(const ContextId: THelpContext;
                               const HelpFileName: string);
  var
    View: ICustomHelpViewer;
  begin
...
begin
 ...
 { if multiple people handle context-sensitive help, hand it off to the first
  ...
  begin
    ...
    begin
      View := FExtendedViewerList[I].Viewer;
  ...
  begin
    if ContextID = 0 then
      Exit;
    raise EHelpSystemException.CreateResFmt(@SNoContextFound, [ContextID]);
  end;
    ...
    begin
      ...
      begin
        ...
        try
          for I := 0 to FExtendedViewerList.Count - 1 do
          begin
            HelpNode := FExtendedViewerList[I];
...
begin
  ...
  if Viewer <> nil then
    ...
    raise EHelpSystemException.CreateRes(@SNoTableOfContents);
end;
...
begin
  ...
  if FExtendedViewerList.Count = 0 then
    raise EHelpSystemException.CreateRes(@SNoTopics);
...
function THelpManager.Hook(Handle: THandle; const HelpFile: string;
  Command: Word; Data: NativeInt): Boolean;
begin
  if HelpFile <> '' then
...
begin
  ...
  begin
    Viewer := FViewerList[Index].Viewer;
...
begin
  ...
  if Viewer <> nil then
    ...
    raise EHelpSystemException.CreateRes(@SNoIndex);
end;
...
begin
  ...
  if Viewer <> nil then
    ...
    raise EHelpSystemException.CreateRes(@SNoSearch);
end;
...
begin
  ...
  for I := 0 to FExtendedViewerList.Count - 1 do
  begin
    Viewer := FExtendedViewerList[I].Viewer;
...
function THelpManager.GetHandle: THandle;
begin
  Result := Handle;
end;
...
begin
  ...
  begin
    if FExtendedViewerList[I].ViewerID = ViewerID then
    begin
