...
uses
  System.Sensors, Winapi.ActiveX, Winapi.Windows, System.RTLConsts;
...
uses
  System.Variants, System.SysUtils, System.Math, System.Win.ComObj, Winapi.Portabledevicetypes, Winapi.Sensors,
  Winapi.Sensorsapi, Winapi.Locationapi, System.Generics.Collections;
...
type
  ...
  private
    ...
    procedure SensorEvent(Sender: TObject; const Sensor: ISensor; const EventID: TGUID;
      const EventData: IPortableDeviceValues);
  ...
  public
    ...
    class function GetCustomSensorData(const Sensor: ISensor; const Data: ISensorDataReport;
      const Prop: TPropertyKey): Variant; static;
    ...
    class procedure SetCustomSensorProperty(const Sensor: ISensor; const Prop: TPropertyKey;
      const Value: Variant); static;
...
function GetSensorData(const Sensor: ISensor; Data: ISensorDataReport; const Prop: TPropertyKey): TPropVariant;
begin
  if Data = nil then
...
function GetIntegerSensorData(const Sensor: ISensor; const Data: ISensorDataReport; const Prop: TPropertyKey): Integer;
var
  Value: TPropVariant;
begin
...
function GetDoubleSensorData(const Sensor: ISensor; const Data: ISensorDataReport; const Prop: TPropertyKey): Double;
var
  Value: TPropVariant;
begin
  Value := GetSensorData(Sensor, Data, Prop);
...
function GetStringSensorData(const Sensor: ISensor; const Data: ISensorDataReport; const Prop: TPropertyKey): string;
var
  Value: TPropVariant;
begin
  Value := GetSensorData(Sensor, Data, Prop);
...
class function TWindowsSensorManager.GetCustomSensorData(const Sensor: ISensor; const Data: ISensorDataReport;
  const Prop: TPropertyKey): Variant;
var
  Res: OleVariant;
  Value: TPropVariant;
begin
...
class function TWindowsSensorManager.GetHasCustomSensorProperty(const Sensor: ISensor;
  const Prop: TPropertyKey): Boolean;
var
  Value: TPropVariant;
begin
...
begin
  ...
  try
    OleCheck(CoCreateInstance(CLSID_PortableDeviceValues, nil, CLSCTX_INPROC_SERVER, IID_IPortableDeviceValues,
      Values));
...
begin
  ...
  FSensor := ASensor;
  if FSensor <> nil then
    OleCheck(FSensor.SetEventSink(TSensorEventSink.Create(DataUpdatedEvent, SensorEvent, LeaveEvent,
      StateChangeEvent)));
end;

procedure TWindowsMotionSensor.DataUpdatedEvent(Sender: TObject; const Sensor: ISensor;
  const NewData: ISensorDataReport);
begin
  FDataReport := NewData;
  try
    DataChanged;
...
function TWindowsMotionSensor.DoGetInterface(const IID: TGUID; out Obj): HResult;
begin
  Result := E_NOTIMPL;
  if FSensor <> nil then
    Result := FSensor.QueryInterface(IID, Obj);
  if (Result <> S_OK) and (FDataReport <> nil) then
    Result := FDataReport.QueryInterface(IID, Obj);
end;
...
begin
  ...
  begin
    case Prop of
      ...
      TProperty.AngleAccelX: Result := GetDoubleSensorData(FSensor, FDataReport,
        SENSOR_DATA_TYPE_ANGULAR_ACCELERATION_X_DEGREES_PER_SECOND_SQUARED);
      TProperty.AngleAccelY: Result := GetDoubleSensorData(FSensor, FDataReport,
        SENSOR_DATA_TYPE_ANGULAR_ACCELERATION_Y_DEGREES_PER_SECOND_SQUARED);
      TProperty.AngleAccelZ: Result := GetDoubleSensorData(FSensor, FDataReport,
...
procedure TWindowsMotionSensor.SensorEvent(Sender: TObject; const Sensor: ISensor; const EventID: TGUID;
  const EventData: IPortableDeviceValues);
begin
...
procedure TWindowsLightSensor.DataUpdatedEvent(Sender: TObject; const Sensor: ISensor;
  const NewData: ISensorDataReport);
begin
  FDataReport := NewData;
  try
...
function TWindowsLightSensor.DoGetInterface(const IID: TGUID; out Obj): HResult;
begin
  Result := E_NOTIMPL;
  if FSensor <> nil then
    Result := FSensor.QueryInterface(IID, Obj);
...
begin
  ...
  begin
    case Prop of
      ...
      TProperty.Temperature: Result := GetDoubleSensorData(FSensor, FDataReport,
...
procedure TWindowsLightSensor.SensorEvent(Sender: TObject; const Sensor: ISensor; const EventID: TGUID;
  const EventData: IPortableDeviceValues);
begin
...
procedure TWindowsMechanicalSensor.DataUpdatedEvent(Sender: TObject; const Sensor: ISensor;
  const NewData: ISensorDataReport);
begin
  FDataReport := NewData;
  try
...
function TWindowsMechanicalSensor.DoGetInterface(const IID: TGUID; out Obj): HResult;
begin
  Result := E_NOTIMPL;
  if FSensor <> nil then
    Result := FSensor.QueryInterface(IID, Obj);
...
begin
  ...
  begin
    case Prop of
      TProperty.MultiValueState: Result := GetDoubleSensorData(FSensor, FDataReport,
      ...
      TProperty.AbsPressure: Result := GetDoubleSensorData(FSensor, FDataReport,
        SENSOR_DATA_TYPE_ABSOLUTE_PRESSURE_PASCAL);
      TProperty.GaugePressure: Result := GetDoubleSensorData(FSensor, FDataReport,
...
begin
  ...
  begin
    case Prop of
      TProperty.SwitchArrayState: Result := GetOrdinalSensorData(FSensor, FDataReport,
        SENSOR_DATA_TYPE_BOOLEAN_SWITCH_ARRAY_STATE);
    else
...
procedure TWindowsMechanicalSensor.SensorEvent(Sender: TObject; const Sensor: ISensor; const EventID: TGUID;
  const EventData: IPortableDeviceValues);
begin
...
procedure TWindowsLocationSensor.DataUpdatedEvent(Sender: TObject; const Sensor: ISensor;
  const NewData: ISensorDataReport);
var
...
function TWindowsLocationSensor.DoGetInterface(const IID: TGUID; out Obj): HResult;
begin
  Result := E_NOTIMPL;
  if FSensor <> nil then
    Result := FSensor.QueryInterface(IID, Obj);
  if (Result <> S_OK) and (FSensorReport <> nil) then
    Result := FSensorReport.QueryInterface(IID, Obj);
  if (Result <> S_OK) and (FSensorEventSink <> nil) then
    Result := FSensorEventSink.QueryInterface(IID, Obj);
  if (Result <> S_OK) and (FLocation <> nil) then
    Result := FLocation.QueryInterface(IID, Obj);
  if (Result <> S_OK) and (FLocationReport <> nil) then
    Result := FLocationReport.QueryInterface(IID, Obj);
  if (Result <> S_OK) and (FLocationEventSink <> nil) then
    Result := FLocationEventSink.QueryInterface(IID, Obj);
end;
...
function TWindowsLocationSensor.GetActivityType: TLocationActivityType;
begin
...
begin
  case Prop of
    ...
    TProperty.MagneticHeading: Result := GetDoubleSensorData(FSensor, Report,
...
function TWindowsLocationSensor.GetUsageAuthorization: TLocationUsageAuthorization;
begin
...
procedure TWindowsLocationSensor.SensorEvent(Sender: TObject; const Sensor: ISensor; const EventID: TGUID;
  const EventData: IPortableDeviceValues);
begin
...
procedure TWindowsLocationSensor.SetActivityType(const Value: TLocationActivityType);
begin
  FActivityType := Value;
end;
...
procedure TWindowsLocationSensor.SetUsageAuthorization(const Value: TLocationUsageAuthorization);
begin
...
procedure TWindowsEnvironmentalSensor.DataUpdatedEvent(Sender: TObject; const Sensor: ISensor;
  const NewData: ISensorDataReport);
begin
  FDataReport := NewData;
  try
...
function TWindowsEnvironmentalSensor.DoGetInterface(const IID: TGUID; out Obj): HResult;
begin
  Result := E_NOTIMPL;
  if FSensor <> nil then
    Result := FSensor.QueryInterface(IID, Obj);
...
begin
  ...
  begin
    case Prop of
      ...
      TProperty.Pressure: Result := GetDoubleSensorData(FSensor, FDataReport,
        SENSOR_DATA_TYPE_ATMOSPHERIC_PRESSURE_BAR);
      TProperty.Humidity: Result := GetDoubleSensorData(FSensor, FDataReport,
        SENSOR_DATA_TYPE_RELATIVE_HUMIDITY_PERCENT);
      TProperty.WindDirection: Result := GetDoubleSensorData(FSensor, FDataReport,
        SENSOR_DATA_TYPE_WIND_DIRECTION_DEGREES_ANTICLOCKWISE);
      TProperty.WindSpeed: Result := GetDoubleSensorData(FSensor, FDataReport,
...
procedure TWindowsEnvironmentalSensor.SensorEvent(Sender: TObject; const Sensor: ISensor; const EventID: TGUID;
  const EventData: IPortableDeviceValues);
begin
...
procedure TWindowsOrientationSensor.DataUpdatedEvent(Sender: TObject; const Sensor: ISensor;
  const NewData: ISensorDataReport);
begin
  FDataReport := NewData;
  try
...
function TWindowsOrientationSensor.DoGetInterface(const IID: TGUID; out Obj): HResult;
begin
  Result := E_NOTIMPL;
  if FSensor <> nil then
    Result := FSensor.QueryInterface(IID, Obj);
...
begin
  ...
  begin
    case Prop of
      ...
      TProperty.HeadingX: Result := GetDoubleSensorData(FSensor, FDataReport,
        SENSOR_DATA_TYPE_MAGNETIC_HEADING_X_DEGREES);
      TProperty.HeadingY: Result := GetDoubleSensorData(FSensor, FDataReport,
        SENSOR_DATA_TYPE_MAGNETIC_HEADING_Y_DEGREES);
      TProperty.HeadingZ: Result := GetDoubleSensorData(FSensor, FDataReport,
        SENSOR_DATA_TYPE_MAGNETIC_HEADING_Z_DEGREES);
      TProperty.MagHeading: Result := GetDoubleSensorData(FSensor, FDataReport,
        SENSOR_DATA_TYPE_MAGNETIC_HEADING_MAGNETIC_NORTH_DEGREES);
      TProperty.TrueHeading: Result := GetDoubleSensorData(FSensor, FDataReport,
        SENSOR_DATA_TYPE_MAGNETIC_HEADING_TRUE_NORTH_DEGREES);
      TProperty.CompMagHeading: Result := GetDoubleSensorData(FSensor, FDataReport,
        SENSOR_DATA_TYPE_MAGNETIC_HEADING_COMPENSATED_MAGNETIC_NORTH_DEGREES);
      TProperty.CompTrueHeading: Result := GetDoubleSensorData(FSensor, FDataReport,
...
procedure TWindowsOrientationSensor.SensorEvent(Sender: TObject; const Sensor: ISensor; const EventID: TGUID;
  const EventData: IPortableDeviceValues);
begin
...
procedure TWindowsElectricalSensor.DataUpdatedEvent(Sender: TObject; const Sensor: ISensor;
  const NewData: ISensorDataReport);
begin
  FDataReport := NewData;
  try
...
function TWindowsElectricalSensor.DoGetInterface(const IID: TGUID; out Obj): HResult;
begin
  Result := E_NOTIMPL;
  if FSensor <> nil then
    Result := FSensor.QueryInterface(IID, Obj);
...
procedure TWindowsElectricalSensor.SensorEvent(Sender: TObject; const Sensor: ISensor; const EventID: TGUID;
  const EventData: IPortableDeviceValues);
begin
...
procedure TWindowsBiometricSensor.DataUpdatedEvent(Sender: TObject; const Sensor: ISensor;
  const NewData: ISensorDataReport);
begin
  FDataReport := NewData;
  try
...
function TWindowsBiometricSensor.DoGetInterface(const IID: TGUID; out Obj): HResult;
begin
  Result := E_NOTIMPL;
  if FSensor <> nil then
    Result := FSensor.QueryInterface(IID, Obj);
...
begin
  ...
  begin
    case Prop of
      TCustomBiometricSensor.TProperty.HumanPresence: Result := GetBooleanSensorData(FSensor, FDataReport,
        SENSOR_DATA_TYPE_HUMAN_PRESENCE);
      TCustomBiometricSensor.TProperty.Touch: Result := GetBooleanSensorData(FSensor, FDataReport,
...
procedure TWindowsBiometricSensor.SensorEvent(Sender: TObject; const Sensor: ISensor; const EventID: TGUID;
  const EventData: IPortableDeviceValues);
begin
...
procedure TWindowsScannerSensor.DataUpdatedEvent(Sender: TObject; const Sensor: ISensor;
  const NewData: ISensorDataReport);
begin
  FDataReport := NewData;
  try
...
function TWindowsScannerSensor.DoGetInterface(const IID: TGUID; out Obj): HResult;
begin
  Result := E_NOTIMPL;
  if FSensor <> nil then
    Result := FSensor.QueryInterface(IID, Obj);
...
procedure TWindowsScannerSensor.SensorEvent(Sender: TObject; const Sensor: ISensor; const EventID: TGUID;
  const EventData: IPortableDeviceValues);
begin
