...
{$IF Defined(CPUX86) and Defined(ASSEMBLER)}
  {$DEFINE X86ASM}
{$ELSE !(CPUX86 && ASSEMBLER)}
  {$DEFINE PUREPASCAL}
{$ENDIF (CPUX86 && ASSEMBLER)}
...
{$IF Defined(PIC) and Defined(ASSEMBLER)}
...
var
  ...
  LenS: Integer;
  ArrLen: Integer;
  LPResult, LPSource, LPNewPattern: PAnsiChar;
  LReplaceAll: Boolean;
begin
  ...
  if Length(Str) <> Length(Source) then
  begin
    Result := '';
    I := Low(AnsiString);
    while I <= High(Source) do
    begin
      if System.AnsiStrings.AnsiStrLIComp(PAnsiChar(Pointer(Source)) + I - Low(AnsiString), PAnsiChar(Pointer(OldPattern)), LenOP) = 0 then
      begin
        Result := Result + NewPattern;
        Inc(I, LenOP);
        if not (rfReplaceAll in Flags) then
        begin
          Result := Result + Copy(Source, I - Low(AnsiString) + 1, MaxInt);
          Break;
        end;
      end
      else
  ...
  begin
    ...
    begin
      ...
      begin                           // special case where Length(OldPattern) = Length(NewPattern)
        ...
        Move(LPSource^, LPResult^, LenS * SizeOf(AnsiChar));
        if LenNP = 1 then
          ...
          for I := 0 to FindCount - 1 do
            Move(LPNewPattern^, LPResult[PosArray[I]], LenNP * SizeOf(AnsiChar));
      end
      else
      begin
        ...
        if LenNP = 0 then
          ...
          begin
            ...
            begin
              ...
              begin
                Move(LPSource[SourceIdx], LPResult[DestIdx], LCharsToCopy * SizeOf(AnsiChar));
                Inc(SourceIdx, LCharsToCopy);
                Inc(DestIdx, LCharsToCopy);
              end;
            end;
          ...
          begin
            ...
            Move(LPNewPattern^, LPResult[DestIdx], LenNP * SizeOf(AnsiChar));
            Inc(DestIdx, LenNP);
            Inc(SourceIdx, LenOP);
          end;
...
{$ELSEIF Defined(POSIX)}
    Result := AnsiCompareStr(String(ASubText), String(AnsiString(PAnsiChar(@AText[SubTextLocation])))) = 0
{$ELSE}
...
{$ELSEIF Defined(POSIX)}
      Result := System.SysUtils.AnsiCompareStr(String(AnsiString(PAnsiChar(P1))), String(AnsiString(PAnsiChar(P2))))
{$ELSE}
{$MESSAGE ERROR 'Unknown platform'}
{$ENDIF}
    else
    ...
    if (P1 <> @Buf1) and (P1 <> S1) then
      FreeMem(P1);
    if (P2 <> @Buf2) and (P2 <> S2) then
      FreeMem(P2);
  end;
end;
...
{$IFNDEF NEXTGEN}
    if Unicode then
      ...
      Result := FloatToText(PAnsiChar(Buf), Value, ValueType, ffGeneral, 15, 0, AFormatSettings)
{$ELSE NEXTGEN}
    Result := FloatToText(PWideChar(Buf), Value, ValueType, ffGeneral, 15, 0, AFormatSettings)
{$ENDIF !NEXTGEN}
  else
...
var
  ps: mbstate_t;
begin
  FillChar(ps, sizeof(mbstate_t), 0);
  Result := mbrlen(MarshaledAString(Str), MB_CUR_MAX, @ps);
...
begin
  Result := nil;
  if (BufLen <= 0) or (SearchString = '') then Exit;
  ...
  begin
    Direction := -1;
    Dec(SelStart, Length(SearchString) - 1);
...
{$ENDIF POSIX}
  ...
  begin
    if (soWholeWord in Options) and ((Result <> @Buf[SelStart]) or ((Byte(Result^)) in WordDelimiters)) then
    ...
    begin
      ...
      begin
        if not (soWholeWord in Options) or
           ((Result + I >= Buf + BufLen) or ((Byte(Result[I])) in WordDelimiters)) and
           ((Result <= Buf) or ((Byte(Result[-1])) in WordDelimiters)) then
          Exit;
        Break;
      end;
    end;
