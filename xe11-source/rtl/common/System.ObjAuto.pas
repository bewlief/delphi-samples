...
{$ENDIF PIC}
  ...
  if ReturnInfo^.CallingConvention = System.TypInfo.TCallConv(ccCdecl) then
  asm
            ADD     ESP,ParamBytes
  end;
...
{$IF Defined(CPUX86) or Defined(CPUX64)}
  if ReturnInfo^.CallingConvention = System.TypInfo.TCallConv.ccPascal then
...
{$IFNDEF NEXTGEN}
        ...
        tkLString:
          Args[K] := TValue.From<AnsiString>( AnsiString(Params[I]) );
{$ENDIF NEXTGEN}
        tkWChar:
          begin
            if (TVarData(Params[I]).VType = varString) or
        ...
        tkInt64:
          begin
            TypeData := ParamInfos[K].ParamType^.TypeData;
            if TypeData^.MinInt64Value > TypeData^.MaxInt64Value then
              Args[K] := TValue.From<UInt64>( TVarData(Params[I]).VUInt64 )
            else
              Args[K] := TValue.From<Int64>( TVarData(Params[I]).VInt64 );
          end;
        tkInteger:
...
asm
        MOV     RAX,[RAX]
        JMP     QWORD PTR [RAX] + VMTOFFSET TMethodHandlerInstance.Handler
end;
{$ELSEIF defined(CPUARM32) or defined(CPUARM64)}
begin
