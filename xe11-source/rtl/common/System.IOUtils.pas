...
{$ENDIF}
  ...
  public
    ...
    [HPPGEN('static void __fastcall Copy(const System::UnicodeString SourceDirName, const System::UnicodeString DestDirName);'  + #13#10 +
            'static void __fastcall Copy(const System::UnicodeString SourceDirName, const System::UnicodeString DestDirName, bool IgnoreErrors) _ALWAYS_INLINE {' + #13#10 +
            '  DirectoryCopyCPP(SourceDirName, DestDirName, IgnoreErrors);' + #13#10 +
            '}')]
  ...
  public
    class procedure Copy(const SourceDirName, DestDirName: string; IgnoreErrors: Boolean = True); static;
  end;
...
{$ENDIF MSWINDOWS}
    ...
    class function DoIsPathRooted(const Path: string;
        const ValidateParam, UseWildcards: Boolean): Boolean; static;
...
{$ENDIF LINUX}
  public
    ...
    class function Combine(const Path1, Path2: string; const ValidateParams: Boolean = True): string; inline; static;
...
{$NODEFINE DirectoryCopyCPP}
procedure DirectoryCopyCPP(const SourceDirName, DestDirName: string; IgnoreErrors: Boolean);
(*$HPPEMIT OPENNAMESPACE*)
(*$HPPEMIT 'extern DELPHI_PACKAGE void __fastcall DirectoryCopyCPP(const System::UnicodeString SourceDirName, const System::UnicodeString DestDirName, bool IgnoreErrors);' *)
...
begin
  ...
  if TFile.Exists(LPath) then
    raise EInOutError.CreateRes(@SDirectoryInvalid, LPath);
  ...
  if (not Recursive) and (not IsEmpty(LPath)) then
    raise EInOutError.CreateRes(@SDirectoryNotEmpty, LPath);
end;
...
begin
  ...
  if Trim(SearchPattern) = '' then // DO NOT LOCALIZE
    raise EInOutArgumentException.CreateRes(@SEmptySearchPattern);
  if not TPath.HasValidFileNameChars(SearchPattern, True) then
    raise EInOutArgumentException.CreateRes(@SInvalidCharsInSearchPattern, SearchPattern);
...
begin
  ...
  if SameFileName(SourceDirName, DestDirName) then
    raise EInOutError.CreateRes(@SSourceDirIsDestDir, SourceDirName);
end;
...
{$IFDEF MSWINDOWS}
  ...
  if not TPath.IsUNCRooted(Path) then
    if not TPath.DriveExists(TPath.DoGetPathRoot(Path)) then
      raise EDirectoryNotFoundException.CreateRes(@SDriveNotFound, Path);
{$ENDIF}
end;
...
begin
  ...
  if Exists(DestDirName) then
    raise EInOutError.CreateRes(@SDirectoryAlreadyExists, DestDirName);
...
{$IFDEF MSWINDOWS}
  ...
  if not SameFileName(TPath.GetPathRoot(SourceDirName),
    ...
    raise EInOutError.CreateRes(@SSameRootDrive, SourceDirName);
{$ENDIF}
...
begin
  ...
  if UseLocalTimeZone then
    TzSpecificLocalTimeToSystemTime(nil, SysTime, UtcTime) // DateTime is Local Time
  else
    UtcTime := SysTime; // DateTime is UTC
  SystemTimeToFileTime(UtcTime, Result);
end;
{$ENDIF}
{$IFDEF POSIX}
class function TDirectory.ConvertDateTimeToFileTime(const DateTime: TDateTime;
...
procedure DirectoryCopyCPP(const SourceDirName, DestDirName: string; IgnoreErrors: Boolean);
begin
  TDirectory.Copy(SourceDirName, DestDirName, IgnoreErrors);
end;

class procedure TDirectoryHelper.Copy(const SourceDirName, DestDirName: string; IgnoreErrors: Boolean);
var
...
begin
      ...
      case FileInfo.Attr and System.SysUtils.faDirectory of
        ...
        0: // move files from source to destination
            begin
              ...
              if not TFile.DoCopy(CompleteSrc, CompleteDest, True) and not IgnoreErrors then
                Failures.Add(CompleteSrc + ': ' + SysErrorMessage(GetLastError)); // DO NOT LOCALIZE
            end;
      end;
    end;
  ...
  begin
    ...
    Failures.TrailingLineBreak := False;
  end;
  try
    WalkThroughDirectory(SourceDirName, '*', PreCallback, nil, True); // DO NOT LOCALIZE
    if not IgnoreErrors and (Failures.Count > 0) then
      raise EInOutError.Create(SFailedToCopyDirectory, Failures.Text);
  finally
    if not IgnoreErrors then
      Failures.Free;
  end;
...
{$ENDIF MSWINDOWS}
  ...
  if Trim(Path) = '' then // DO NOT LOCALIZE
    raise EInOutArgumentException.CreateRes(@SEmptyPath);
  if not TPath.HasValidPathChars(Path, False) then
    raise EInOutArgumentException.CreateRes(@SInvalidCharsInPath, Path);
  if ExistsCheck and (not Exists(Path)) then
    raise EDirectoryNotFoundException.CreateRes(@SPathNotFound, Path);
end;
...
begin
  ...
  begin
    ...
    if (ErrCode <> ERROR_FILE_NOT_FOUND) and
      ...
      raise EInOutError.Create(SysErrorMessage(ErrCode), Path);
  end;
end;
{$ENDIF}
{$IFDEF POSIX}
  ...
  try
    try
      ...
      on E: EFileStreamError do
        raise EInOutError.Create(E.Message, Path);
    end;
  finally
    LFileStream.Free;
  end;
...
begin
  if NeedEncoding and (not Assigned(Encoding)) then
    raise EInOutArgumentException.CreateResFmt(@SParamIsNil, ['Encoding']); // DO NOT LOCALIZE
...
begin
  ...
  if (not Overwrite) and Exists(DestFileName) then
    raise EInOutError.CreateRes(@SFileAlreadyExists, DestFileName);
end;
...
begin
  ...
  if not SameFileName(TPath.DoGetPathRoot(SourceFileName),
    ...
    raise EInOutError.CreateRes(@SSameRootDrive, SourceFileName);

  if SameFileName(SourceFileName, DestinationFileName) then
    raise EInOutError.CreateRes(@SSourceFileIsDestFile, SourceFileName);
end;
{$ENDIF MSWINDOWS}
...
begin
  ...
  if not DoCopy(SourceFileName, DestFileName, Overwrite) then
    raise EInOutError.Create(SysErrorMessage(GetLastError), SourceFileName);
end;
...
begin
  ...
  if not DecryptFile(PChar(Path), 0) then
    raise EInOutError.Create(SysErrorMessage(GetLastError), Path);
end;
{$ENDIF}
  ...
  try
   ...
   {$IFDEF CPU32BITS}
    if LFileSize > MaxInt then
      raise EInOutError.CreateRes(@SFileTooLong, Path);
   {$ENDIF}
...
{$ENDIF MSWINDOWS}
  ...
  if FileExistsCheck and (not Exists(Path)) then
    raise EFileNotFoundException.CreateRes(@SSpecifiedFileNotFound, Path)
end;
...
{$ENDIF}
  ...
  case Mode of
    TFileMode.fmCreateNew:
      begin
        if Exists(Path) then
          raise EInOutError.CreateRes(@SFileAlreadyExists, Path);
...
begin
{$IFDEF MSWINDOWS}
  // Windows 10 1607 has 14393 as the build number.
  if (Length(Path) >= MaxLength) and (not TPath.IsExtendedPrefixed(Path)) and
   ((TOSVersion.Major < 10) or ((TOSVersion.Major = 10) and (TOSVersion.Build < 14393))) then // Check the length in Chars on Win32
{$ENDIF MSWINDOWS}
{$IFDEF POSIX}
  if (Length(UTF8Encode(Path)) >= MaxLength) then // Check the length in bytes on POSIX
{$ENDIF POSIX}
    raise EPathTooLongException.CreateRes(@SPathTooLong, Path);
end;

class function TPath.Combine(const Path1, Path2: string; const ValidateParams: Boolean): string;
begin
    ...
    begin
      ...
      begin
        if not HasValidPathChars(Path1, True) then
          raise EInOutArgumentException.CreateRes(@SInvalidCharsInPath, Path1);
        if not HasValidPathChars(Path2, True) then
          raise EInOutArgumentException.CreateRes(@SInvalidCharsInPath, Path2);
      end;
...
begin
  ...
  begin
    if ValidateParam and (not HasValidPathChars(FileName, True)) then
      raise EInOutArgumentException.CreateRes(@SInvalidCharsInPath, FileName);
...
begin
  if Trim(Path) = '' then
    raise EInOutArgumentException.CreateRes(@SEmptyPath);
  if Length(Path) <> Integer(StrLen(PChar(Path))) then
    raise EInOutArgumentException.CreateRes(@SInvalidCharsInPath, Path);
  SetLastError(ERROR_SUCCESS);
...
class function TPath.DoIsPathRooted(const Path: string;
  const ValidateParam, UseWildcards: Boolean): Boolean;
{$IFDEF MSWINDOWS}
var
  PPath: PChar;
  Prefix: TPathPrefixType;
...
begin
  ...
  if Trim(FileName) = '' then // DO NOT LOCALIZE
    raise EInOutArgumentException.CreateRes(@SEmptyFileName);
  if not HasValidPathChars(FileName, True) then
    raise EInOutArgumentException.CreateRes(@SInvalidCharsInFileName, FileName);
...
var
  LStr: PChar;
begin
  if SHGetKnownFolderPath(FOLDERID_Downloads, 0, 0, LStr) = 0 then
  begin
    Result := LStr;
    CoTaskMemFree(LStr);
  end
  else
    Result := GetDocumentsPath;
end;
{$ENDIF}
{$IFDEF POSIX}
{$IFDEF MACOS}
...
var
  LStr: PChar;
begin
  if SHGetKnownFolderPath(FOLDERID_PublicDownloads, 0, 0, LStr) = 0 then
  begin
    Result := LStr;
    CoTaskMemFree(LStr);
  end
...
begin
  ...
  Len := Winapi.Windows.GetTempPath(MAX_PATH, Tmp);
  if Len <> 0 then
  begin
...
{$ENDIF MSWINDOWS}
  ...
  if ExistsCheck and (not TFile.Exists(Path)) and (not TDirectory.Exists(Path)) then
    raise EPathNotFoundException.CreateRes(@SPathNotFound, Path);
end;
...
begin
  Result := DoIsPathRooted(Path, True, False);
end;
