...
{$IFDEF MACOS}
    ...
    function WaitNoReset(Timeout: Cardinal): TWaitResult;
{$ENDIF MACOS}
{$ENDIF POSIX}
  public
...
{$ENDIF MSWINDOWS}
  ...
  /// <summary>Wraps a system implementation of a reader-writer locking mechanism. </summary>
  /// <remarks>This mechanism (also known as readers-writer lock, MRSW lock, or MREW lock) allows two different "modes" of access.
  ///  Shared access allows multiple readers to access the resource at the same time while it also prevents
  ///  any thread from acquiring an exclusive (write) access.
  ///  Shared access can be requested recursively. A single thread can acquire it multiple
  ///  times in a row. Each BeginRead/TryBeginRead call must be paired with a call to EndRead.
  ///  Exclusive (write) access can only be granted to one thread at a time. While a TLighweightMREW lock
  ///  is in exclusive mode, it will prevent any thread from getting shared (read) access.
  ///  Exclusive access cannot be requested recursively.
  ///  Shared access cannot be upgraded to exclusive access.
  ///  TLightweightMREW is much faster than the TMultiReadExclusiveWriteSynchronizer and implements
  ///  a reader-writer locking mechanism on all platforms while the TMultiReadExclusiveWriteSynchronizer
  ///  is implemented as a critical section on non-Windows platforms.
  ///  This implementation is designed to be included in a larger class and not as an object that you can copy around.
  /// </remarks>
{$HINTS OFF}
...
{$ENDIF MACOS}
    ...
    /// <summary>Acquires a lock in shared (reader) mode. Blocks until a lock can be acquired. A single thread can recursively acquire shared lock multiple times. Each BeginRead call must be paired with an EndRead call.</summary>
    procedure BeginRead;
    /// <summary>Tries to acquire a lock in shared (reader) mode. Returns True if lock was acquired. Does not block.</summary>
    function TryBeginRead: Boolean; {$IF defined(LINUX) or defined(ANDROID)}overload;{$ENDIF}
{$IF defined(LINUX) or defined(ANDROID)}
    /// <summary>Tries to acquire a lock in shared (reader) mode. If a lock failed to be acquired in Timeout milliseconds, the function returns False. INFINITE timeout is supported.</summary>
    function TryBeginRead(Timeout: Cardinal): Boolean; overload;
{$ENDIF LINUX or ANDROID}
    /// <summary>Releases a lock that was opened in shared (reader) mode.</summary>
    procedure EndRead;
    /// <summary>Acquires a lock in exclusive (writer) mode. Blocks until a lock can be acquired.</summary>
    procedure BeginWrite;
    /// <summary>Tries to acquire a lock in exclusive (writer) mode. Returns True if lock was acquired. Does not block. A single thread cannot recursively acquire exclusive lock multiple times.</summary>
    function TryBeginWrite: Boolean; {$IF defined(LINUX) or defined(ANDROID)}overload;{$ENDIF}
{$IF defined(LINUX) or defined(ANDROID)}
    /// <summary>Tries to acquire a lock in exclusive (writer) mode. If a lock failed to be acquired in Timeout milliseconds, the function returns False.</summary>
    function TryBeginWrite(Timeout: Cardinal): Boolean; overload;
{$ENDIF LINUX or ANDROID}
    /// <summary>Releases a lock that was opened in shared (writer) mode.</summary>
    procedure EndWrite;
  end;
{$HINTS ON}
...
type
  ...
  TCoWaitForMultipleHandlesProc = function (dwFlags: DWORD; dwTimeOut: DWORD;
    cHandles: Cardinal; var Handles; var lpdwIndex: DWORD): HRESULT; stdcall;
...
function CoWaitForMultipleHandles(dwFlags: DWORD; dwTimeOut: DWORD;
  cHandles: Cardinal; var Handles; var lpdwIndex: DWORD): HRESULT;
...
{$IFDEF POSIX}
procedure TSynchroObject.GetPosixEndTime(var EndTime: timespec; TimeOut: Cardinal);
{$IFDEF MACOS}
var
  Now: mach_timespec_t;
...
class function THandleObject.WaitForMultiple(
  const HandleObjs: THandleObjectArray; Timeout: Cardinal; AAll: Boolean;
...
begin
  ...
  FEvent := dispatch_semaphore_create(0);
  if FEvent = nil then
...
{$IFDEF MACOS}
function TEvent.WaitNoReset(Timeout: Cardinal): TWaitResult;
begin
  ...
  begin
    Result := wrSignaled;
    while dispatch_semaphore_wait(FEvent, DISPATCH_TIME_NOW) = 0 do
      ;
  end
  else
    Result := wrTimeout;
...
constructor TMutex.Create(DesiredAccess: Cardinal; InheritHandle: Boolean;
...
function TMutex.WaitFor(Timeout: Cardinal): TWaitResult;
var
  Err: Integer;
  EndTime: timespec;
begin
...
constructor TSemaphore.Create(DesiredAccess: Cardinal; InheritHandle: Boolean;
  const Name: string; UseCOMWait: Boolean);
{$IFDEF MSWINDOWS}
var
  lpName: PChar;
...
{$IFDEF POSIX}
function TSemaphore.WaitFor(Timeout: Cardinal): TWaitResult;
{$IF Defined(LINUX) or Defined(ANDROID)}
var
  EndTime: timespec;
begin
...
function TConditionVariableMutex.WaitFor(AExternalMutex: TMutex; TimeOut: Cardinal): TWaitResult;
{$IFDEF MSWINDOWS}
var
  LastWaiter: Boolean;
begin
...
var
  ...
  BitMask: Integer;
begin
  BitMask := 1 shl (BitOffset and 31);
  repeat
    OldValue := Target;
    until AtomicCmpExchange(Target, OldValue or BitMask, OldValue) = OldValue;
  Result := (OldValue and BitMask) = BitMask;
end;
{$ENDIF}
...
var
  ...
  BitMask: Integer;
begin
  BitMask := not(1 shl (BitOffset and 31));
  repeat
    OldValue := Target;
    until AtomicCmpExchange(Target, OldValue and BitMask, OldValue) = OldValue;
  BitMask := not BitMask;
  Result := (OldValue and BitMask) = BitMask;
end;
{$ENDIF}
...
class function TSpinWait.SpinUntil(const ACondition: TFunc<Boolean>; Timeout: Cardinal): Boolean;
var
  Timer: TStopwatch;
  Wait: TSpinWait;
begin
...
function TSpinLock.InternalTryEnter(Timeout: Cardinal): Boolean;
var
  CurLock: Integer;
  NewLock: Integer;
  Timer: TStopwatch;
...
begin
  ...
  begin
    ...
    Timeout := Timeout - Cardinal(Timer.ElapsedMilliseconds);
    Result := TSpinWait.SpinUntil(
      function: Boolean
      begin
...
begin
  ...
  if (Total < 0) or (Total > $7FFFFFFF) then
    raise EArgumentOutOfRangeException.CreateResFmt(@sInvalidTimeoutValue, [string(Timeout)]);
  Result := TryEnter(Cardinal(Total));
end;
  ...
  begin
    ...
    begin
      ...
      if (Elapsed > $7FFFFFFF) or (Elapsed >= Timeout) then
        Exit(wrTimeout);
      Timeout := Timeout - Cardinal(Elapsed);
    end;
    TMonitor.Enter(FLock);
    try
...
function TLightweightSemaphore.WaitFor(Timeout: Cardinal): TWaitResult;
var
  Timer: TStopwatch;
  Spinner: TSpinWait;
  CountDown: Integer;
...
function TCountdownEvent.WaitFor(Timeout: Cardinal): TWaitResult;
begin
  if not IsSet then
...
destructor TMultiWaitEventImpl.Destroy;
begin
  inherited Destroy;
  FreeAndNil(FEvent);
end;
...
{$IFDEF MACOS}
class constructor TLightweightMREW.Create;
begin
  host_get_clock_service(mach_host_self, CALENDAR_CLOCK, CalendarClock);
end;

class destructor TLightweightMREW.Destroy;
begin
  mach_port_deallocate(mach_task_self, CalendarClock);
end;
{$ENDIF MACOS}

class operator TLightweightMREW.Initialize(out Dest: TLightweightMREW);
begin
{$IFDEF MSWINDOWS}
  InitializeSRWLock(Dest.FNativeRW);
{$ENDIF MSWINDOWS}
{$IFDEF POSIX}
  Dest.FNativeRW := PTHREAD_RWLOCK_INITIALIZER;
{$ENDIF POSIX}
end;

{$IFDEF POSIX}
procedure TLightweightMREW.GetPosixEndTime(var EndTime: timespec; TimeOut: Cardinal);
{$IFDEF MACOS}
var
  Now: mach_timespec_t;
  NanoSecTimeout: Int64;
...
begin
{$IFDEF MSWINDOWS}
  AcquireSRWLockShared(FNativeRW);
{$ENDIF MSWINDOWS}
{$IFDEF POSIX}
...
begin
{$IFDEF MSWINDOWS}
  AcquireSRWLockExclusive(FNativeRW);
{$ENDIF MSWINDOWS}
{$IFDEF POSIX}
...
begin
{$IFDEF MSWINDOWS}
  ReleaseSRWLockShared(FNativeRW);
{$ENDIF MSWINDOWS}
{$IFDEF POSIX}
...
begin
{$IFDEF MSWINDOWS}
  ReleaseSRWLockExclusive(FNativeRW);
{$ENDIF MSWINDOWS}
{$IFDEF POSIX}
  CheckOSError(pthread_rwlock_unlock(FNativeRW));
{$ENDIF POSIX}
...
begin
{$IFDEF MSWINDOWS}
  Result := TryAcquireSRWLockShared(FNativeRW);
{$ENDIF MSWINDOWS}
{$IFDEF POSIX}
  Err := pthread_rwlock_tryrdlock(FNativeRW);
  Result := Err = 0;
  if (not Result) and (Err <> EBUSY) then
    CheckOSError(Err);
{$ENDIF POSIX}
end;
...
begin
  ...
  begin
    GetPosixEndTime(EndTime, Timeout);
    Err := pthread_rwlock_timedrdlock(FNativeRW, EndTime);
    Result := Err = 0;
...
begin
{$IFDEF MSWINDOWS}
  Result := TryAcquireSRWLockExclusive(FNativeRW);
{$ENDIF MSWINDOWS}
{$IFDEF POSIX}
  Err := pthread_rwlock_trywrlock(FNativeRW);
  Result := Err = 0;
  if (not Result) and (Err <> EBUSY) and (Err <> EDEADLK) then
    CheckOSError(Err);
{$ENDIF POSIX}
end;
...
begin
  ...
  begin
    GetPosixEndTime(EndTime, Timeout);
    Err := pthread_rwlock_timedwrlock(FNativeRW, EndTime);
    Result := Err = 0;
    if (not Result) and (Err <> ETIMEDOUT) and (Err <> EDEADLK) then
      CheckOSError(Err);
  end else if Timeout = INFINITE then
  begin
    CheckOSError(pthread_rwlock_wrlock(FNativeRW));
