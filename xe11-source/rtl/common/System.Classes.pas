...
{ TParser special tokens }
  ...
  pidAllPlatforms = pidWin32 or pidWin64 or
                    ...
                    pidiOSSimulator32 or pidiOSSimulator64 or // pidiOSSimulatorArm64
  ...
  pfidiOS         = pidiOSDevice32 or pidiOSDevice64 or
                    pidiOSSimulator32 or pidiOSSimulator64;  // pidiOSSimulatorArm64
...
{ TList class }
  ...
  end {$IFDEF AUTOREFCOUNT} deprecated 'Use System.Generics.Collections.TList<T>' {$ENDIF};
...
{ TCollection class }
  ...
  {$NODEFINE TCollectionNotification}
  ...
  public
    ...
    /// <summary>Sorts collection items using comparator <c>AComparer</c>.</summary>
...
{ TStrings class }
  ...
  public
    ...
    // Workaround for RSP-35503, caused by RSP-34286
    [HPPGEN('void __fastcall AddStrings(const System::UnicodeString *Strings, const int Strings_High);'+ #13#10 +
            'void __fastcall AddStrings(const System::DynamicArray<System::UnicodeString> Strings) {'  + #13#10 +
            '   if (Strings.Length)'                                                                   + #13#10 +
            '     AddStrings(Strings.data(), Strings.High);'                                           + #13#10 +
            '}')]
    procedure AddStrings(const Strings: array of string); overload;

    [HPPGEN('void __fastcall AddStrings(const System::UnicodeString *Strings, const int Strings_High,' + #13#10 +
            '                 System::TObject* const *Objects, const int Objects_High);'               + #13#10 +
            'void __fastcall AddStrings(const System::DynamicArray<System::UnicodeString> Strings,'    + #13#10 +
            '                 const System::DynamicArray<System::TObject*> Objects) {'                 + #13#10 +
            '  if (Strings.Length && Objects.Length)'                                                  + #13#10 +
            '    AddStrings(Strings.data(), Strings.High, Objects.data(), Objects.High);'              + #13#10 +
            '}')]
...
{ TStringList class }
  ...
  private type
    TOverridden = set of (sloCompareStrings, sloGetObject);
  private
    ...
    FOwnsObject: Boolean;
    FOverridden: TOverridden;
    procedure Grow;
    procedure QuickSort(L, R: Integer; SCompare: TStringListSortCompare);
    procedure SetSorted(Value: Boolean);
    procedure SetCaseSensitive(const Value: Boolean);
    class function InternalStrComp(L, R: PChar; Len, Mode: Integer): Integer; inline; static;
    function LinearIndexOfName(const Name: string): Integer;
    function SortedIndexOfName(const Name: string): Integer;
  protected
  ...
  public
    ...
    // Workaround for RSP-35503, caused by RSP-34286
    [HPPGEN('virtual void __fastcall AddStrings(TStrings* Strings)/* overload */;'+ #13#10 +
            'void __fastcall AddStrings(const System::DynamicArray<System::UnicodeString> Strings) {'  + #13#10 +
            '   TStrings::AddStrings(Strings);'                                                        + #13#10 +
            '}'                                                                                        + #13#10 +
            'void __fastcall AddStrings(const System::DynamicArray<System::UnicodeString> Strings,'    + #13#10 +
            '                           const System::DynamicArray<System::TObject*> Objects) {'       + #13#10 +
            '   TStrings::AddStrings(Strings, Objects);'                                               + #13#10 +
            '}')]
...
{ TStream abstract class }
  ...
  protected
    ...
    function GetByteCount(const Buffer: TBytes; Offset, Count: Longint): Longint;
...
{ TPointerStream }
  ...
  public
    constructor Create(Ptr: Pointer; const Size: NativeInt; ReadOnly: Boolean = False);
    procedure SetPointer(Ptr: Pointer; const Size: NativeInt);
    function Write(const Buffer; Count: LongInt): LongInt; override;
  end;
...
{ TBaseAsyncResult }
  ...
  private
    {$IFDEF AUTOREFCOUNT} [Unsafe] {$ENDIF} FOwner: TComponent;
  ...
  TComponentInterfaceDelegate = class(TObject, IInterface, IVCLComObject)
  private
...
function GetComponentFieldAddress(const Instance: TComponent; const Name: string): Pointer;
procedure SetComponentFieldAddressCache(Clear: Boolean; MaxClassesInCache: Integer);
...
procedure BinToHex(Buffer: PAnsiChar; Text: PWideChar; BufSize: Integer); overload; inline;
...
function HexToBin(const Text: PWideChar; var Buffer; BufSize: Integer): Integer; overload; inline;
{$IFNDEF NEXTGEN}
function HexToBin(Text: PAnsiChar; Buffer: PAnsiChar; BufSize: Integer): Integer; overload;
function HexToBin(Text: PWideChar; Buffer: PAnsiChar; BufSize: Integer): Integer; overload; inline;
function HexToBin(Text: PAnsiChar; var Buffer; BufSize: Integer): Integer; overload; inline;
function HexToBin(const Text: PAnsiChar; Buffer: Pointer; BufSize: Integer): Integer; overload; inline;
{$ENDIF !NEXTGEN}
...
function SwapHexEndianness(const Text: string): string;
...
{  Call CheckSynchronize periodically within the main thread in order for
   ...
   is mainly for applications that have an event driven GUI such as Windows
   VCL or multi-platform FMX ones.  The best place this can be called is during
   Idle processing.  This guarantees that the main thread is in a known "good"
...
{ SyncEvent is an Event handle that is signaled every time a thread wishes to
  synchronize with the main thread or is terminating.  This handle is suitable
...
const
  cnAdding = System.Generics.Collections.TCollectionNotification.cnAdding;
  cnAdded = System.Generics.Collections.TCollectionNotification.cnAdded;
  cnExtracting = System.Generics.Collections.TCollectionNotification.cnExtracting;
  cnExtracted = System.Generics.Collections.TCollectionNotification.cnExtracted;
  cnDeleting = System.Generics.Collections.TCollectionNotification.cnDeleting;
  cnRemoved = System.Generics.Collections.TCollectionNotification.cnRemoved;
...
{$IF Defined(POSIX)}
  ...
  Posix.SysSelect, Posix.SysTime, Posix.Time, Posix.Errno,
{$IF Defined(LINUX)}
  Posix.Fcntl,
...
{$IFDEF USE_LIBICU}
  System.Internal.ICU,
{$ENDIF USE_LIBICU}
  System.Variants, System.Character, System.RTLConsts, System.SysConst, System.Diagnostics;
...
type
  TFieldsCache = class (TObject)
  private type
    TFields = class (TObject)
    private
      FHits: NativeUInt;
      FOffsets: TDictionary<string, Cardinal>;
    public
      constructor Create;
      destructor Destroy; override;
    end;
    TClasses = TObjectDictionary<TClass, TFields>;
  private
    class var FGlobal: TFieldsCache;
    class function GetGlobal: TFieldsCache; static; inline;
  private
    FClasses: TClasses;
    FMaxClassesInCache: Integer;
    class procedure ModuleUnload(HInstance: NativeInt); static;
    class constructor Create;
    class procedure InitFields; static;
    class destructor Destroy;
    function PopClass(AClass: TClass): TFields;
    procedure PushLessUsedClasses;
  public
    constructor Create;
    destructor Destroy; override;
    procedure Clear;
    function GetAddress(const AObj: TObject; const AName: string): Pointer;
    property MaxClassesInCache: Integer read FMaxClassesInCache write FMaxClassesInCache default 100;
    class property Global: TFieldsCache read GetGlobal;
  end;

constructor TFieldsCache.TFields.Create;
begin
  inherited Create;
  FOffsets := TDictionary<string, Cardinal>.Create(DictComparer);
end;

destructor TFieldsCache.TFields.Destroy;
begin
  FOffsets.Free;
  inherited Destroy;
end;

class procedure TFieldsCache.ModuleUnload(HInstance: NativeInt);
begin
  if FGlobal <> nil then
    FGlobal.Clear;
end;

class constructor TFieldsCache.Create;
begin
  InitFields;
end;

class procedure TFieldsCache.InitFields;
var
  LFieldsCache: TFieldsCache;
begin
  if FGlobal = nil then
  begin
    LFieldsCache := TFieldsCache.Create;
    if AtomicCmpExchange(Pointer(FGlobal), Pointer(LFieldsCache), nil) <> nil then
      LFieldsCache.Free
    else
    begin
{$IFDEF AUTOREFCOUNT}
...
class destructor TFieldsCache.Destroy;
begin
  FreeAndNil(FGlobal);
...
begin
  inherited Create;
  FClasses := TClasses.Create([doOwnsValues]);
  FMaxClassesInCache := 100;
end;
...
class function TFieldsCache.GetGlobal: TFieldsCache;
begin
  if FGlobal = nil then
    TFieldsCache.InitFields;
  Result := FGlobal;
end;
...
begin
  ...
  FClasses.Add(AClass, Result);
  while (AClass <> nil) and (AClass <> TComponent) do
  begin
    LFieldTable := PPointer(PByte(AClass) + vmtFieldTable)^;
    if LFieldTable <> nil then
    begin
      LField := PVmtFieldEntry(NativeUInt(LFieldTable) + SizeOf(TVmtFieldTable));
      for I := 0 to LFieldTable.Count - 1 do
      begin
        // only the names from the descendant classes
        Result.FOffsets.TryAdd(LField.NameFld.ToString, LField.FieldOffset);
        LField := PVmtFieldEntry(LField.NameFld.Tail);
      end;
    end;
...
procedure TFieldsCache.PushLessUsedClasses;
const
  CDelta = 1.1;
var
  LPair: TPair<TClass, TFields>;
  LMinHits: NativeUInt;
  LMaxClasses: Integer;
  LRemovals: TList<TClass>;
  LRemove: TClass;
begin
  LMinHits := High(NativeUInt);
  for LPair in FClasses do
    if LMinHits > LPair.Value.FHits then
      LMinHits := LPair.Value.FHits;
  LMinHits := Round(LMinHits * CDelta);
  LMaxClasses := Round(MaxClassesInCache / CDelta);
  LRemovals := TList<TClass>.Create;
  try
    for LPair in FClasses do
      if LMinHits >= LPair.Value.FHits then
      begin
        LRemovals.Add(LPair.Key);
        if FClasses.Count - LRemovals.Count <= LMaxClasses then
          Break;
      end;
    for LRemove in LRemovals do
      FClasses.Remove(LRemove);
  finally
    LRemovals.Free;
  end;
end;

function TFieldsCache.GetAddress(const AObj: TObject; const AName: string): Pointer;
var
  LClass: TClass;
  LFields: TFields;
  LOffset: Cardinal;
begin
  if MaxClassesInCache = 0 then
    Exit(AObj.FieldAddress(AName));

  LClass := AObj.ClassType;
  if (AName = '') or (LClass.ClassName = 'TApplication') then
    Exit(nil);

  TMonitor.Enter(Self);
  try
    if not FClasses.TryGetValue(LClass, LFields) then
    begin
      if FClasses.Count >= MaxClassesInCache then
        PushLessUsedClasses;
      LFields := PopClass(LClass);
    end;
    if LFields.FHits < High(NativeUInt) then
      Inc(LFields.FHits);
  finally
    TMonitor.Exit(Self);
  end;

  if not LFields.FOffsets.TryGetValue(AName, LOffset) then
    Result := nil
  else
    Result := PByte(AObj) + LOffset;
end;

function GetComponentFieldAddress(const Instance: TComponent; const Name: string): Pointer;
begin
  Result := TFieldsCache.Global.GetAddress(Instance, Name);
end;

procedure SetComponentFieldAddressCache(Clear: Boolean; MaxClassesInCache: Integer);
begin
  if Clear then
    TFieldsCache.Global.Clear;
  if MaxClassesInCache >= 0 then
    TFieldsCache.Global.MaxClassesInCache := MaxClassesInCache;
end;
...
{$HINTS OFF}
// Inline function 'TList`1.GetList' has not been expanded because contained unit 'System.Generics.Collections' uses compiling unit 'System.Classes' indirectly
function TCollectionEnumerator.GetCurrent: TCollectionItem;
begin
  Result := FCollection.FItems.List[FIndex];
end;
{$HINTS ON}
...
begin
  Inc(FIndex);
  Result := FIndex < FCollection.FItems.Count;
end;
...
procedure TCollection.Sort(const AComparer: IComparer<TCollectionItem>);
begin
  BeginUpdate;
  try
...
procedure TStrings.AddStrings(const Strings: array of string);
var
  I: Integer;
begin
  BeginUpdate;
...
procedure TStrings.AddStrings(const Strings: array of string; const Objects: array of TObject);
var
  I: Integer;
begin
...
begin
  ...
  begin
    ...
    begin
      LDelimiters := [Char(#0)];
      LUniDelimiters := (QuoteChar >= #256) or (Delimiter >= #256);
      if not LUniDelimiters then
        LDelimiters := LDelimiters + [Char(QuoteChar), Char(Delimiter)];
    ...
    try
      ...
      begin
        ...
        begin
          P := PChar(S);
          while not ((P^ in LDelimiters) or
                     LUniDelimiters and ((P^ = QuoteChar) or (P^ = Delimiter))) do
            P := NextChar(P);
...
procedure TStringList.AddStrings(Strings: TStrings);
var
  I: Integer;
begin
  if not Sorted or
     (Count = 0) and (Strings is TStringList) and
     TStringList(Strings).Sorted and
     (CaseSensitive = TStringList(Strings).CaseSensitive) and
     (UseLocale = TStringList(Strings).UseLocale) and
     (Duplicates = TStringList(Strings).Duplicates) then
  begin
    BeginUpdate;
    try
      for I := 0 to Strings.Count - 1 do
        InsertItem(Count, Strings[I], Strings.Objects[I]);
    finally
      EndUpdate;
    end;
  end
...
begin
  ...
  begin
    Count := GetCount;
    SLen := Length(S);
    PItem := PStringItem(FList);
    if sloCompareStrings in FOverridden then
      for Result := 0 to Count - 1 do
      begin
        if CompareStrings(PItem^.FString, S) = 0 then
          Exit;
        Inc(PItem);
      end
    else
      for Result := 0 to Count - 1 do
      begin
        if (Length(PItem^.FString) = SLen) and (CompareStrings(PItem^.FString, S) = 0) then
          Exit;
        Inc(PItem);
      end;
    Result := -1;
...
{$WARNINGS OFF}
class function TStringList.InternalStrComp(L, R: PChar; Len, Mode: Integer): Integer;
begin
  case Mode of
  0: Result := StrLIComp(L, R, Len);
  1: Result := AnsiStrLIComp(L, R, Len);
  2: Result := StrLComp(L, R, Len);
  3: Result := AnsiStrLComp(L, R, Len);
  end;
end;
{$WARNINGS ON}

function TStringList.LinearIndexOfName(const Name: string): Integer;
var
  Count: Integer;
  PItem: PStringItem;
  PName, PStr: PChar;
...
begin
  ...
  NameLen := Length(Name);
  PName := PChar(Name);
  M := Ord(CaseSensitive) * 2 + Ord(UseLocale);
  PItem := PStringItem(FList);
  for Result := 0 to Count - 1 do
  begin
    PStr := PChar(PItem^.FString);
    if (InternalStrComp(PStr, PName, NameLen, M) = 0) and
       ((PStr + NameLen)^ = NameValueSeparator) then
      Exit;
    Inc(PItem);
  end;
  Result := -1;
...
function TStringList.SortedIndexOfName(const Name: string): Integer;
var
  Count: Integer;
  PName, PStr: PChar;
  NameLen, M, L, H, I, C: Integer;
begin
  Count := GetCount;
  NameLen := Length(Name);
  PName := PChar(Name);
  ...
  begin
    I := (L + H) shr 1;
    PStr := PChar(FList[I].FString);
    C := InternalStrComp(PStr, PName, NameLen, M);
    if C = 0 then
      if (PStr + NameLen)^ <> FNameValueSeparator then
        C := InternalStrComp(PStr + NameLen, @FNameValueSeparator, 1, M and 1);
    if C = 0 then
      Exit(I)
    else if C < 0 then
      L := I + 1
...
begin
  if not (sloCompareStrings in FOverridden) then
    if Sorted and not UseLocale and (Duplicates <> dupAccept) and (Count > 5) then
      Result := SortedIndexOfName(Name)
    else
      Result := LinearIndexOfName(Name)
  else
    Result := inherited IndexOfName(Name);
end;

function TStringList.IndexOfObject(AObject: TObject): Integer;
var
  Count: Integer;
  PItem: PStringItem;
begin
  if sloGetObject in FOverridden then
...
begin
  ...
  begin
    ...
    P := (L + R) shr 1;
    repeat
      while (I <> P) and (SCompare(Self, I, P) < 0) do Inc(I);
      while (J <> P) and (SCompare(Self, J, P) > 0) do Dec(J);
      if I <= J then
      begin
        if I <> J then
          ExchangeItems(I, J);
...
function TStream.GetByteCount(const Buffer: TBytes; Offset, Count: Longint): Longint;
begin
  Result := Count;
  if (Offset < 0) or (Count < 0) then
    Result := 0
  else if Count > Length(Buffer) - Offset then
  begin
    Result := Length(Buffer) - Offset;
    if Result < 0 then
      Result := 0;
  end;
end;

function TStream.GetByteCount64(const Buffer: TBytes; Offset, Count: Int64): Int64;
begin
  Result := Count;
  if (Offset < 0) or (Count < 0) then
    Result := 0
...
begin
  Result := Read(Buffer[Offset], GetByteCount(Buffer, Offset, Count));
end;

function TStream.Write(const Buffer: TBytes; Offset, Count: Longint): Longint;
begin
  Result := Write(Buffer[Offset], GetByteCount(Buffer, Offset, Count));
end;
...
begin
  Count := GetByteCount64(Buffer, Offset, Count);
  Result := 0;
...
function UsePreallocateOptimization(Self: TStream): Boolean;
begin
  Result :=
    (Self.ClassType = TMemoryStream) or
    (Self.ClassType = TStringStream) or
    (Self.ClassType = TBytesStream);
end;
...
begin
  ...
  begin
    ...
    SetLength(Buffer, BufferSize);
    AdjustSize := False;
    if UsePreallocateOptimization(Self) then
      try
        CurSize := Size;
        CurPos := Position;
        NewSize := CurPos + Count;
        if (CurSize >= 0) and (CurSize < NewSize) then
        begin
          Size := NewSize;
          Position := CurPos;
          AdjustSize := True;
...
{$ELSEIF Defined(POSIX)}
  if ftruncate(FHandle, Position) = -1 then
    raise EStreamError.CreateRes(@SStreamSetSize);
{$ENDIF POSIX}
end;
...
function TBytesStream.Realloc(var NewCapacity: NativeInt): Pointer;
begin
  if (NewCapacity > 0) and (NewCapacity <> FSize) then
    NewCapacity := (NewCapacity + (MemoryDelta - 1)) and not (MemoryDelta - 1);
...
var
  Preamble: Integer;
begin
  Preamble := TEncoding.GetBufferEncoding(Bytes, FEncoding);
  Result := FEncoding.GetString(Bytes, Preamble, Size - Preamble);
end;
...
constructor TPointerStream.Create(Ptr: Pointer; const Size: NativeInt; ReadOnly: Boolean);
begin
  inherited Create;
  SetPointer(Ptr, Size);
  FReadOnly := ReadOnly;
...
procedure TPointerStream.SetPointer(Ptr: Pointer; const Size: NativeInt);
begin
  inherited SetPointer(Ptr, Size);
  FPosition := 0;
end;

procedure TPointerStream.SetSize(const NewSize: Int64);
begin
  if NewSize <> Size then
    raise EStreamError.CreateRes(@SCantWritePointerStreamError);
end;

procedure TPointerStream.SetSize(NewSize: Longint);
begin
  if NewSize <> Size then
    raise EStreamError.CreateRes(@SCantWritePointerStreamError);
end;
            ...
            begin
              ...
              begin
                ...
                if Root <> nil then
                  ...
                  if aPropFixup.FPropInfo^.PropType^^.Kind = tkInterface then
                  begin
                    var IntValue: IInterface := GetInterfaceProp(aPropFixup.FInstance, aPropFixup.FPropInfo);
                    TryResolve := IntValue <> nil;
                  end
                  else
                    TryResolve := GetOrdProp(aPropFixup.FInstance, aPropFixup.FPropInfo) <> 0;
                if TryResolve then
                begin
                  if Root <> nil then
...
var
  L: Integer;
  LResult: TBytes;
  LStatResult: array [0..254] of Byte;
begin
  L := 0;
  case ReadValue of
    vaWString:
    ...
    vaString:
      begin
        Read(L, SizeOf(Byte));
        Read(LStatResult[0], L);
        Result := TMarshal.ReadStringAsAnsi(DefaultSystemCodePage,
          TPtrWrapper.Create(@LStatResult[0]), L);
      end;
    vaLString:
      begin
        Read(L, SizeOf(Integer));
...
var
  ...
  I: Integer;
begin
  if (csDestroying in Component.FComponentState) and (csDesigning in Component.FComponentState) then
    Exit;
  OldAncestor := Ancestor;
...
begin
  ...
  begin
    L := LocaleCharsFromUnicode(CP_UTF8, 0, PWideChar(Value), Length(Value), MarshaledAString(@U[1]), 255, nil, nil);
    if L > 255 then L := 255;
    U[0] := Byte(L);
    Write(U, 1 + L);
  end;
...
var
  ...
  PBuf: PByte;
begin
  PText := Text + TextOffset;
  PBuf := PByte(Buffer) + BufOffset;
  I := Count;
  while I > 0 do
  begin
    if (Ord(PText[0]) > 255) or (Ord(PText[1]) > 255) then
      Break;
    b1 := H2BConvert[Ord(PText[0])];
    b2 := H2BConvert[Ord(PText[1])];
...
var
  ...
  PBuf: PByte;
begin
  PText := PByte(Text) + TextOffset;
  PBuf := PByte(Buffer) + BufOffset;
  I := Count;
  while I > 0 do
  begin
    b1 := H2BConvert[PText[0]];
    b2 := H2BConvert[PText[1]];
    if (b1 = $FF) or (b2 = $FF) then
      Break;
    PBuf[0] := (b1 shl 4) + b2;
    Inc(PBuf);
...
begin
  ...
  begin
    Text[0] := WideChar(B2HConvert[PByte(Buffer)[I] shr 4]);
    Text[1] := WideChar(B2HConvert[PByte(Buffer)[I] and $F]);
    Inc(Text, 2);
  end;
end;
...
begin
  Result := HexToBin(Text, @Buffer, BufSize);
end;
...
begin
  for I := 0 to BufSize - 1 do
  begin
    Text[0] := AnsiChar(B2HConvert[Byte(Buffer[I]) shr 4]);
    Text[1] := AnsiChar(B2HConvert[Byte(Buffer[I]) and $F]);
    Inc(Text, 2);
  end;
end;
...
begin
  BinToHex(Pointer(Buffer), Text, BufSize);
end;
...
begin
  Result := HexToBin(Text, Pointer(Buffer), BufSize);
end;
...
begin
  ...
  PRight := PChar(Result) + Length(Result) - 2;
  PLeft := PChar(Result);
  while L > 0 do
  begin
    c := PRight^;
...
procedure TParser.Error(const IdentRes: PResStringRec);
begin
  ErrorStr(LoadResString(IdentRes));
end;
...
procedure TParser.ErrorFmt(const IdentRes: PResStringRec; const Args: array of const);
begin
  ErrorStr(Format(LoadResString(IdentRes), Args));
end;
...
begin
  ...
  // Try optimistic fast ASCII to Unicode conversion
  SetLength(Result, L);
  D := PChar(Result);
  S := @FBuffer[FTokenPtr];
  I := L;
  C := S^;
  while (I > 0) and ((C and $80) = 0) do
  begin
    D^ := Char(C);
    Inc(S);
    C := S^;
...
begin
  ...
  try
    ...
    for I := Low(LName) to High(LName) do
      ...
      begin
        AName := AnsiUpperCase(TEncoding.UTF8.GetString(LName));
        Result := True;
        Exit;
      end;
...
{$ENDIF POSIX}
  ...
  try
    ...
    try
      Result := False;
      if (LocalSyncList <> nil) and (LocalSyncList.Count > 0) then
      begin
        for I := 0 to LocalSyncList.Count - 1 do
        begin
          SyncProc := LocalSyncList[I];
          if (SyncProc.SyncRec.FExecuteAfterTimestamp > 0) and (SyncProc.SyncRec.FExecuteAfterTimestamp > SyncTimer.ElapsedMilliseconds) then
            continue;
          Result := True;
        ...
        for I := 0 to LocalSyncList.Count - 1 do
          if LocalSyncList[I] <> nil then
          begin
            if SyncList = nil then
              SyncList := TList.Create;
            SyncList.Add(LocalSyncList[I]);
          end;
      end;
    finally
      LocalSyncList.Free;
...
procedure TThread.TSynchronizeRecord.Init(AThread: TObject; const AMethod: TThreadMethod);
begin
  FThread := AThread;
  FMethod := AMethod;
  FProcedure := nil;
  FSynchronizeException := nil;
  FExecuteAfterTimestamp := 0;
end;

procedure TThread.TSynchronizeRecord.Init(AThread: TObject; const AProcedure: TThreadProcedure);
begin
  FThread := AThread;
...
{$ELSEIF Defined(POSIX)}
  ...
  begin
    ...
    if FThreadID <> 0 then pthread_detach(pthread_t(FThreadID));
    pthread_mutex_destroy(FCreateSuspendedMutex);
    sem_destroy(FResumeEvent);
  end;
{$ENDIF POSIX}
  inherited Destroy;
...
procedure TThread.ShutdownThread;
begin
...
{$IF Defined(MSWINDOWS)}
    if ResumeThread(FHandle) <> 1 then
      raise EThread.CreateRes(@SThreadStartError);
{$ELSEIF Defined(POSIX)}
...
class procedure TThread.InitializeExternalThreadsList;
var
  LExternalThreads: TThreadList<TThread>;
begin
...
procedure TThread.Schedule(APolicy, APriority: Integer);
var
  P: sched_param;
begin
  P.sched_priority := APriority;
  CheckThreadError(pthread_setschedparam(pthread_t(FThreadID), APolicy, P));
end;
...
{
  Note, to set Posix thread priority, Policy must be SCHED_FIFO or SCHED_RR.
  To set both Policy and Priority use Schedule method.
}
...
{
  Note, to set Posix thread policy to SCHED_FIFO or SCHED_RR, Priority must be > 0.
  To set both Policy and Priority use Schedule method.
}
procedure TThread.SetPolicy(Value: Integer);
begin
  if Value <> Policy then
    Schedule(Value, Priority);
end;
{$ENDIF POSIX}
...
function InitGetTickCount64: UInt64; stdcall; forward;

// Intel 64 and IA-32 Architectures Software Developer Manual : 8.1.1
// the following basic memory operations will always be carried out atomically
//   Reading and writing a doubleword aligned on a 32-bit boundary
//   Reading and writing a quadword aligned on a 64-bit boundary
var
  [Align(8)]
  GetTickCount64Func: TGetTickCount64Func  = InitGetTickCount64;
...
class function TThread.IsTimeout(AStartTime: Cardinal; ATimeout: Integer): Boolean;
var
  LNow: Cardinal;
begin
  if ATimeout <= 0 then
  ...
  if LNow < AStartTime then
    Result := (High(Cardinal) - AStartTime + LNow) >= Cardinal(ATimeout)
  else
...
begin
  New(LSynchronize);
  try
    LSynchronize.Init(AThread, AThreadProc);
    Synchronize(LSynchronize, True);
  finally
    if MainThreadID = CurrentThread.ThreadID then
      Dispose(LSynchronize);
...
begin
  if Assigned(FOnSynchronize) then
    FOnSynchronize(CurrentThread.ThreadID, QueueEvent, ForceQueue, ASyncRec^.FMethod, ASyncRec^.FProcedure);
...
begin
  LSynchronize.Init(AThread, AThreadProc);
  TThread.Synchronize(@LSynchronize);
end;

class procedure TThread.ForceQueue(const AThread: TThread; const AMethod: TThreadMethod; ADelay: Integer = 0);
var
  LSynchronize: PSynchronizeRecord;
begin
  New(LSynchronize);
  LSynchronize.Init(AThread, AMethod);
  if ADelay > 0 then
    LSynchronize.FExecuteAfterTimestamp := SyncTimer.ElapsedMilliseconds + ADelay;
  Synchronize(LSynchronize, True, True);
end;

class procedure TThread.ForceQueue(const AThread: TThread; const AThreadProc: TThreadProcedure; ADelay: Integer = 0);
var
  LSynchronize: PSynchronizeRecord;
begin
  New(LSynchronize);
...
{$ELSEIF Defined(POSIX)}
{
  Suspend in POSIX is NOT implemented, so the thread can not be suspended after start.
  But when Suspend is called from the thread Execute, it will wait for event set in Resume.
}
    if GetCurrentThreadId = ThreadId then
      sem_wait(FResumeEvent);
{$ENDIF POSIX}
  except
...
{$ELSEIF Defined(POSIX)}
{
  Resume in POSIX is NOT implemented because the thread can not be suspended after start.
  The follow code resumes a thread created suspended, or a thread suspended in it Execute.
}
var
  I: Integer;
begin
  ...
  else if GetCurrentThreadId <> ThreadId then
  begin
    sem_getvalue(FResumeEvent, I);
    if I = 0 then
      sem_post(FResumeEvent);
  end;
  FSuspended := False;
end;
{$ELSE OTHERPLATFORM}
...
begin
  if FExternalThread then
    raise EThread.CreateRes(@SThreadExternalWait);
  if FThreadID = 0 then Exit(FReturnValue);
  ...
  FThreadID := 0;
  X := nil;
  JoinResult := pthread_join(ID, X);
  if (JoinResult <> EINVAL) and (JoinResult <> ESRCH) then
    CheckThreadError(JoinResult);
  Result := FReturnValue;
end;
{$ENDIF POSIX}
...
constructor ObservableMemberAttribute.Create(const AMemberName, AFramework: string;
  ATrack: Boolean);
begin
  inherited Create;
...
begin
  inherited Create;
  if AComponent.FComponents = nil then
    FIndex := -MaxInt
  else
    FIndex := -1;
...
begin
  Result := FComponent.FComponents.List[FIndex];
end;
{$HINTS ON}
...
begin
  Inc(FIndex);
  Result := (FIndex >= 0) and (FIndex < FComponent.FComponents.Count);
end;
...
{$HINTS OFF}
// Inline function 'TList`1.Remove' has not been expanded because contained unit 'System.Generics.Collections' uses compiling unit 'System.Classes' indirectly
...
begin
  if (FOwner <> nil) then
  begin
    Field := TFieldsCache.Global.GetAddress(FOwner, FName);
    if Field <> nil then
...
constructor TComponentInterfaceDelegate.Create(AOwner: TComponent);
begin
  inherited Create;
  FOwner := AOwner;
  FOwner.FVCLComObject := Self as IVCLComObject;
end;

destructor TComponentInterfaceDelegate.Destroy;
begin
  FOwner.FVCLComObject := nil;
  inherited Destroy;
end;

function TComponentInterfaceDelegate._AddRef: Integer;
begin
  Result := -1;
end;

function TComponentInterfaceDelegate._Release: Integer;
begin
  Result := -1;
end;

function TComponentInterfaceDelegate.QueryInterface(const IID: TGUID; out Obj): HResult;
begin
...
procedure TComponentInterfaceDelegate.FreeOnRelease;
begin
  Destroy;
end;

function TComponentInterfaceDelegate.GetIDsOfNames(const IID: TGUID; Names: Pointer;
  NameCount, LocaleID: Integer; DispIDs: Pointer): HResult;
begin
  Result := E_NOTIMPL;
end;

function TComponentInterfaceDelegate.GetTypeInfo(Index, LocaleID: Integer;
  out TypeInfo): HResult;
begin
  Result := E_NOTIMPL;
end;

function TComponentInterfaceDelegate.GetTypeInfoCount(out Count: Integer): HResult;
begin
  Result := E_NOTIMPL;
end;

function TComponentInterfaceDelegate.Invoke(DispID: Integer; const IID: TGUID;
...
begin
  try
    ...
    if NewPos < 0 then
      Exit(E_FAIL);
    if @libNewPosition <> nil then libNewPosition := LargeUInt(NewPos);
    Result := S_OK;
  except
...
begin
  ...
  {$IFDEF MSWINDOWS}
  Filer.DefineProperty('PixelsPerInch', ReadPixelsPerInch, WritePixelsPerInch, PixelsPerInch <> Winapi.Windows.USER_DEFAULT_SCREEN_DPI);
  {$ELSE}
...
procedure TDataModule.ReadPixelsPerInch(Reader: TReader);
begin
...
procedure TDataModule.WritePixelsPerInch(Writer: TWriter);
begin
...
{$IF Declared(RTLVersion112)}
  if (pidiOSSimulator and FPlatforms) <> 0 then
    FPlatforms := FPlatforms or pidiOSSimulatorArm64;
{$ENDIF Declared(RTLVersion112)}
end;
...
begin
  ...
  if AObservers.IsObserving(LID) then
    ...
    raise EObserverException.CreateRes(@sObserverNotAvailable);
end;
...
begin
  ...
  begin
    if Supports(LList[I], IIteratorLinkObserver, LIteratorLinkObserver) then
      LIteratorLinkObserver.StartFrom(APosition);
  end;
end;
...
begin
  ...
  begin
    if Supports(LList[I], IIteratorLinkObserver, LIteratorLinkObserver) then
      Result := Result or LIteratorLinkObserver.MoveNext;
  end;
end;
...
var
  ...
  begin
    ...
    begin
      ...
      case Info.Kind of
        ...
        tkRecord,
        tkMRecord: WalkStructure(FldRef, Info);
  ...
  begin
    ...
    if (AArray <> nil) and (Data.elType^.Kind in [tkClass, tkInterface, tkRecord, tkMRecord, tkPointer, tkDynArray, tkArray]) then
    begin
      PElement := AArray;
      ...
      begin
        case Data.elType^.Kind of
          ...
          tkRecord,
          tkMRecord: WalkStructure(PElement, Data.elType^);
    ...
    begin
      ...
      begin
        case Data.ArrayData.ElType^.Kind of
          ...
          tkInterface: WalkInterface(IInterface(PPointer(PElement)^));
          tkRecord,
          tkMRecord: WalkStructure(PElement, Data.ArrayData.ElType^);
  ...
  begin
    ...
    case ATypeInfo.Kind of
      ...
      tkRecord,
      tkMRecord: WalkStructure(Ref, ATypeInfo);
...
type
  ...
  protected
    ...
    class function GetTickCount64: UInt64; override;
...
class function TMultiWaitEventImpl.GetTickCount64: UInt64;
begin
...
begin
  // By default, all Async cannot be cancelled. Descendants can override this behaviour and do the required processing;
  Result := False;
end;
...
{$ENDIF POSIX}
  ...
  SyncTimer := TStopwatch.StartNew;
  TThread.InitializeExternalThreadsList;
...
finalization
  ...
  if TFieldsCache.Global <> nil then
  begin
    TFieldsCache.Global.Clear;
    RemoveModuleUnloadProc(TFieldsCache.ModuleUnload);
  end;
