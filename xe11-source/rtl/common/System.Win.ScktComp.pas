...
uses System.Types, System.RTLConsts, System.Generics.Collections;
...
begin
  ...
  begin
    ...
    Socket := TSocket(AtomicExchange(Pointer(FSocket), Pointer(INVALID_SOCKET)));
...
begin
  ...
  Event(Self, seListen);
  CheckSocketResult(Winapi.Winsock.listen(FSocket, QueueSize), 'listen');
  FLookupState := lsIdle;
  FConnected := True;
end;
...
begin
  ...
  try
    ...
    Len := SizeOf(Addr);
    ClientWinSocket := Winapi.WinSock.accept(Socket, @Addr, @Len);
