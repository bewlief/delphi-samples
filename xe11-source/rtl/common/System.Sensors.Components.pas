...
const
  ...
  pidSensorSupportPlatforms = (pfidWindows or pfidOSX or pfidiOS or pfidAndroid);
...
{ TSensor<T: TCustomSensor> }
  ...
  protected
    ...
    procedure DoStateChanged(Sender: TObject); virtual;
...
{ TLocationSensor }
  [ComponentPlatformsAttribute(pidSensorSupportPlatforms)]
  ...
  strict private
    ...
    procedure SetOnLocationUpdatesPausedChanged(const Value: TLocationUpdatesPausedChangedEvent);
  ...
  protected
    ...
    procedure DoStateChanged(Sender: TObject); override;
    function GetRegion(const AIndex: Integer): TLocationRegion;
    function GetRegionCount: Integer;
  public
    constructor Create(AOwner: TComponent); override;
    function Authorized: TAuthorizationType;
  ...
  published
    ...
    property ActivityType: TLocationActivityType read FActivityType write SetActivityType;
    property Distance: TLocationDistance read FDistance write SetDistance;
    property UsageAuthorization: TLocationUsageAuthorization read FUsageAuthorization write SetUsageAuthorization;
    { Events }
    ...
    property OnLocationUpdatesPausedChanged: TLocationUpdatesPausedChangedEvent read FOnLocationUpdatesPausedChanged write SetOnLocationUpdatesPausedChanged;
  end;
...
procedure TSensor<T>.DoStateChanged(Sender: TObject);
begin
  if Assigned(FOnStateChanged) then
    FOnStateChanged(Sender);
end;
...
function TSensor<T>.QueryInterface(const IID: TGUID; out Obj): HResult;
begin
  Result := inherited QueryInterface(IID, Obj);
  if (Result <> S_OK) and (FSensor <> nil) then
    Result := FSensor.GetInterface(IID, Obj);
end;
...
begin
  ...
  begin
    ...
    FSensor.OnStateChanged := DoStateChanged;
  end;
end;
...
begin
  ...
  begin
    ...
    FSensor.UsageAuthorization := FUsageAuthorization;
    FSensor.ActivityType := FActivityType;
    FSensor.OnLocationUpdatesPausedChanged := FOnLocationUpdatesPausedChanged;
  end;
end;
...
procedure TLocationSensor.SetOnLocationUpdatesPausedChanged(const Value: TLocationUpdatesPausedChangedEvent);
begin
  FOnLocationUpdatesPausedChanged := Value;
  UpdateSensorProperties;
end;
...
function TLocationSensor.Authorized: TAuthorizationType;
var
  LSensor: TCustomLocationSensor;
begin
  Result := TAuthorizationType.atNotSpecified;
  LSensor := GetSensor;
  if LSensor <> nil then
    Result := LSensor.Authorized;
end;
...
procedure TLocationSensor.SetActivityType(const Value: TLocationActivityType);
begin
  FActivityType := Value;
  UpdateSensorProperties;
end;
...
procedure TLocationSensor.SetUsageAuthorization(const Value: TLocationUsageAuthorization);
begin
  FUsageAuthorization := Value;
  UpdateSensorProperties;
end;

procedure TLocationSensor.DoStateChanged(Sender: TObject);
begin
  if Sensor <> nil then
    FUsageAuthorization := Sensor.UsageAuthorization;
  inherited;
end;
