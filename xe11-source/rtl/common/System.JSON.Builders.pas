...
type
  ...
  public
    ...
    /// <summary> Specifies Extended JSON mode. </summary>
  ...
  public
    ...
    /// <summary> Returns current item full name or path starting from specified depth. </summary>
    function GetPath(AFromDepth: Integer): String; overload; inline;
    /// <summary> Associated reader object. </summary>
...
uses
  System.TypInfo, System.Variants, System.JSONConsts;
...
begin
  ...
  try
    ...
    try
      ...
      try
        LJson.ExtendedJsonMode := ExtendedJsonMode;
        LJson.DateTimeZoneHandling := DateTimeZoneHandling;
...
begin
  ...
  begin
    ...
    try
      ...
      try
        LRdr.ExtendedJsonMode := Owner.ExtendedJsonMode;
        LRdr.DateTimeZoneHandling := Owner.DateTimeZoneHandling;
...
begin
  if FPath = '' then
    FPath := FReader.GetPath(0);
  Result := FPath;
end;

function TJSONIterator.GetPath(AFromDepth: Integer): String;
begin
  Result := FReader.GetPath(AFromDepth);
end;
...
begin
  ...
  repeat
    ...
    if FReader.Read then
      ...
      begin
        FKey := '';
        Inc(FStack.List[FStack.Count - 1].FIndex);
      ...
      else if FReader.IsPrimitiveToken(FReader.TokenType) then
      begin
        FKey := '';
        FType := FReader.TokenType;
        Result := True;
      end
      else if FReader.TokenType = TJsonToken.Comment then
        Continue
      else
        Error
    else
