...
begin
  ...
  begin
    SetLength(Result, GetLongPathName(PChar(FileName), nil, 0));
    SetLength(Result, GetLongPathName(PChar(FileName), PChar(Result), Length(Result)));
  end
  else
  begin
    Result := '';
...
begin
  ...
  while True do
  begin
    Len := Winapi.Windows.GetModuleFileName(HInstance, PChar(Result), Length(Result) + 1);
