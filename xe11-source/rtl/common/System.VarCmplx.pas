...
uses
  {$IFDEF MSWINDOWS}Winapi.Windows,{$ENDIF} System.Variants;
...
uses
  {Windows,} System.VarUtils, System.SysUtils, System.Math, System.SysConst, System.RTLConsts, System.TypInfo, System.Classes;
...
var
  ...
  begin
    ...
    begin
      while AText[LAt] = ' ' do
        Inc(LAt);
      ARest := AText.SubString( LAt-Low(string), MaxInt);
    end;
    Result := ARest <> '';
  end;
...
begin
  ...
  begin
    ...
    LSign := 1;
    if LLeftover[Low(string)] = '-' then
      LSign := -1
    else if LLeftover[Low(string)] <> '+' then
      ParseError(SCmplxCouldNotParsePlus);
    LPart := LLeftover.SubString( 1, MaxInt);
...
begin
  ...
  if System.Math.IsZero(LDenominator) then
...
begin
  FReal := AValue;
  if ComplexNumberDefuzzAtZero and System.Math.IsZero(FReal) then
    FReal := 0;
end;
...
begin
  FImaginary := AValue;
  if ComplexNumberDefuzzAtZero and System.Math.IsZero(FImaginary) then
    FImaginary := 0;
end;
...
begin
  ...
  begin
    Img := 2.0 * Imaginary;
    LDenominator := Cos(2.0 * Real) + CosH(Img);
    if System.Math.IsZero(LDenominator) then
      raise EZeroDivide.Create(SDivByZero);
    SetValue(Sin(2.0 * Real) / LDenominator, SinH(Img) / LDenominator);
  end;
end;
...
begin
  Result := System.Math.IsZero(Real) and System.Math.IsZero(Imaginary);
end;

function TComplexData.Equals(const Right: TComplexData): Boolean;
begin
  Result := System.Math.SameValue(Real, Right.Real) and
            System.Math.SameValue(Imaginary, Right.Imaginary);
end;

function TComplexData.Equals(const AReal, AImaginary: Double): Boolean;
begin
  Result := System.Math.SameValue(Real, AReal) and System.Math.SameValue(Imaginary, AImaginary);
end;
...
begin
  if System.Math.IsZero(GetAbsSqr) then
    if System.Math.IsZero(APower.GetAbsSqr) then
      SetValue(1)
    else
      SetValue(0)
  else
  ...
  if Right.VType = VarType then
    case Left.VType of
      varString:
        case Operator of
          opAdd:
            Variant(Left) := Variant(Left) + TComplexData(TComplexVarData(Right).VComplex).AsString;
        else
          RaiseInvalidOp;
        end;
    else
      if Left.VType = VarType then
        case Operator of
          opAdd:
            TComplexData(TComplexVarData(Left).VComplex).DoAdd(TComplexData(TComplexVarData(Right).VComplex));
          opSubtract:
            TComplexData(TComplexVarData(Left).VComplex).DoSubtract(TComplexData(TComplexVarData(Right).VComplex));
          opMultiply:
            TComplexData(TComplexVarData(Left).VComplex).DoMultiply(TComplexData(TComplexVarData(Right).VComplex));
          opDivide:
            TComplexData(TComplexVarData(Left).VComplex).DoDivide(TComplexData(TComplexVarData(Right).VComplex));
        else
          RaiseInvalidOp;
        end
      else
...
begin
  ...
  try
    ...
    if VarDataIsStr(LSource) then
      TObject(TComplexVarData(Dest).VComplex) := TComplexData.Create(VarDataToStr(LSource))
    else
    begin
      VarDataInit(LTemp);
      try
        VarDataCastTo(LTemp, LSource, varDouble);
        TObject(TComplexVarData(Dest).VComplex) := TComplexData.Create(LTemp.VDouble);
      finally
        VarDataClear(LTemp);
      end;
    end;
...
begin
  if Source.VType = VarType then
    ...
    {$IFNDEF NEXTGEN}
      varOleStr:
        VarDataFromOleStr(Dest, TComplexData(TComplexVarData(Source).VComplex).AsString);
      varString:
        VarDataFromLStr(Dest, AnsiString(TComplexData(TComplexVarData(Source).VComplex).AsString));
    {$ENDIF !NEXTGEN}
      varUString:
        VarDataFromStr(Dest, TComplexData(TComplexVarData(Source).VComplex).AsString);
    else
      VarDataInit(LTemp);
      try
        LTemp.VType := varDouble;
        LTemp.VDouble := TComplexData(TComplexVarData(Source).VComplex).Real;
...
begin
  ...
  if (Left.VType = VarType) and (Right.VType = VarType) then
    case Operator of
      opCmpEQ:
        Result := TComplexData(TComplexVarData(Left).VComplex).Equals(TComplexData(TComplexVarData(Right).VComplex));
      opCmpNE:
        Result := not TComplexData(TComplexVarData(Left).VComplex).Equals(TComplexData(TComplexVarData(Right).VComplex));
    else
      RaiseInvalidOp;
    end
  else
...
begin
  ...
  begin
    TComplexVarData(Dest).VType := VarType;
    TObject(TComplexVarData(Dest).VComplex) := TComplexData.Create(TComplexData(TComplexVarData(Source).VComplex));
  end;
end;

function TComplexVariantType.GetInstance(const V: TVarData): TObject;
begin
  Result := TObject(TComplexVarData(V).VComplex);
end;

function TComplexVariantType.IsClear(const V: TVarData): Boolean;
var
  ComplexData : TComplexData;
begin
  ComplexData := TComplexData(TComplexVarData(V).VComplex);
  Result := (ComplexData = nil) or
            ComplexData.IsZero;
end;
...
begin
  ...
  try
    ...
    TObject(TComplexVarData(Dest).VComplex) := TComplexData.Create;
    TComplexData(TComplexVarData(Dest).VComplex).Real := Reader.ReadFloat;
    TComplexData(TComplexVarData(Dest).VComplex).Imaginary := Reader.ReadFloat;
  finally
    Reader.Free;
  end;
end;
...
begin
  ...
  ComplexData := TComplexData(TComplexVarData(Source).VComplex);
  try
    Writer.WriteFloat(ComplexData.Real);
    Writer.WriteFloat(ComplexData.Imaginary);
  finally
    Writer.Free;
  end;
end;
...
begin
  if Right.VType = VarType then
    case Operator of
      opNegate:
        TComplexData(TComplexVarData(Right).VComplex).DoNegate;
    else
      RaiseInvalidOp;
    end
  else
...
begin
  ...
  TObject(TComplexVarData(ADest).VComplex) := AComplex;
end;
...
begin
  if VarIsComplex(AValue) and
     System.Math.IsZero(TComplexData(TComplexVarData(AValue).VComplex).Imaginary) then
    Result := TComplexData(TComplexVarData(AValue).VComplex).Real
  else
    Result := AValue;
end;
...
begin
  ...
  Result := TComplexData(TComplexVarData(LTemp).VComplex).GetAbsSqr;
end;
...
begin
  VarCast(LTemp, AValue, VarComplex);
  Result := TComplexData(TComplexVarData(LTemp).VComplex).GetAbs;
end;
...
begin
  VarCast(LTemp, AValue, VarComplex);
  Result := TComplexData(TComplexVarData(LTemp).VComplex).GetAngle;
end;

function VarComplexSign(const AValue: Variant): Variant;
begin
  VarCast(Result, AValue, VarComplex);
  TComplexData(TComplexVarData(Result).VComplex).DoSign;
end;

function VarComplexConjugate(const AValue: Variant): Variant;
begin
  VarCast(Result, AValue, VarComplex);
  TComplexData(TComplexVarData(Result).VComplex).DoConjugate;
end;

function VarComplexInverse(const AValue: Variant): Variant;
begin
  VarCast(Result, AValue, VarComplex);
  TComplexData(TComplexVarData(Result).VComplex).DoInverse;
end;

function VarComplexExp(const AValue: Variant): Variant;
begin
  VarCast(Result, AValue, VarComplex);
  TComplexData(TComplexVarData(Result).VComplex).DoExp;
end;

function VarComplexLn(const AValue: Variant): Variant;
begin
  VarCast(Result, AValue, VarComplex);
  TComplexData(TComplexVarData(Result).VComplex).DoLn;
end;

function VarComplexLog10(const AValue: Variant): Variant;
begin
  VarCast(Result, AValue, VarComplex);
  TComplexData(TComplexVarData(Result).VComplex).DoLog10;
end;

function VarComplexLog2(const AValue: Variant): Variant;
begin
  VarCast(Result, AValue, VarComplex);
  TComplexData(TComplexVarData(Result).VComplex).DoLog2;
end;

function VarComplexLogN(const AValue: Variant; const X: Double): Variant;
begin
  VarCast(Result, AValue, VarComplex);
  TComplexData(TComplexVarData(Result).VComplex).DoLogN(X);
end;

function VarComplexSqr(const AValue: Variant): Variant;
begin
  VarCast(Result, AValue, VarComplex);
  TComplexData(TComplexVarData(Result).VComplex).DoSqr;
end;

function VarComplexSqrt(const AValue: Variant): Variant;
begin
  VarCast(Result, AValue, VarComplex);
  TComplexData(TComplexVarData(Result).VComplex).DoSqrt;
end;

function VarComplexTimesPosI(const AValue: Variant): Variant;
begin
  VarCast(Result, AValue, VarComplex);
  TComplexData(TComplexVarData(Result).VComplex).DoTimesImaginary(1.0);
end;
...
begin
  VarCast(Result, AValue, VarComplex);
  TComplexData(TComplexVarData(Result).VComplex).DoTimesImaginary(AFactor);
end;

function VarComplexTimesReal(const AValue: Variant; const AFactor: Double): Variant;
begin
  VarCast(Result, AValue, VarComplex);
  TComplexData(TComplexVarData(Result).VComplex).DoTimesReal(AFactor);
end;
...
begin
  ...
  TComplexData(TComplexVarData(Result).VComplex).DoPower(TComplexData(TComplexVarData(LTemp).VComplex));
end;
...
begin
  VarCast(Result, AValue, VarComplex);
  TComplexData(TComplexVarData(Result).VComplex).DoCos;
end;

function VarComplexSin(const AValue: Variant): Variant;
begin
  VarCast(Result, AValue, VarComplex);
  TComplexData(TComplexVarData(Result).VComplex).DoSin;
end;

function VarComplexTan(const AValue: Variant): Variant;
begin
  VarCast(Result, AValue, VarComplex);
  TComplexData(TComplexVarData(Result).VComplex).DoTan;
end;

function VarComplexCot(const AValue: Variant): Variant;
begin
  VarCast(Result, AValue, VarComplex);
  TComplexData(TComplexVarData(Result).VComplex).DoCot;
end;

function VarComplexSec(const AValue: Variant): Variant;
begin
  VarCast(Result, AValue, VarComplex);
  TComplexData(TComplexVarData(Result).VComplex).DoSec;
end;

function VarComplexCsc(const AValue: Variant): Variant;
begin
  VarCast(Result, AValue, VarComplex);
  TComplexData(TComplexVarData(Result).VComplex).DoCsc;
end;

function VarComplexArcCos(const AValue: Variant): Variant;
begin
  VarCast(Result, AValue, VarComplex);
  TComplexData(TComplexVarData(Result).VComplex).DoArcCos;
end;

function VarComplexArcSin(const AValue: Variant): Variant;
begin
  VarCast(Result, AValue, VarComplex);
  TComplexData(TComplexVarData(Result).VComplex).DoArcSin;
end;

function VarComplexArcTan(const AValue: Variant): Variant;
begin
  VarCast(Result, AValue, VarComplex);
  TComplexData(TComplexVarData(Result).VComplex).DoArcTan;
end;

function VarComplexArcCot(const AValue: Variant): Variant;
begin
  VarCast(Result, AValue, VarComplex);
  TComplexData(TComplexVarData(Result).VComplex).DoArcCot;
end;

function VarComplexArcSec(const AValue: Variant): Variant;
begin
  VarCast(Result, AValue, VarComplex);
  TComplexData(TComplexVarData(Result).VComplex).DoArcSec;
end;

function VarComplexArcCsc(const AValue: Variant): Variant;
begin
  VarCast(Result, AValue, VarComplex);
  TComplexData(TComplexVarData(Result).VComplex).DoArcCsc;
end;

function VarComplexCosH(const AValue: Variant): Variant;
begin
  VarCast(Result, AValue, VarComplex);
  TComplexData(TComplexVarData(Result).VComplex).DoCosH;
end;

function VarComplexSinH(const AValue: Variant): Variant;
begin
  VarCast(Result, AValue, VarComplex);
  TComplexData(TComplexVarData(Result).VComplex).DoSinH;
end;

function VarComplexTanH(const AValue: Variant): Variant;
begin
  VarCast(Result, AValue, VarComplex);
  TComplexData(TComplexVarData(Result).VComplex).DoTanH;
end;

function VarComplexCotH(const AValue: Variant): Variant;
begin
  VarCast(Result, AValue, VarComplex);
  TComplexData(TComplexVarData(Result).VComplex).DoCotH;
end;

function VarComplexSecH(const AValue: Variant): Variant;
begin
  VarCast(Result, AValue, VarComplex);
  TComplexData(TComplexVarData(Result).VComplex).DoSecH;
end;

function VarComplexCscH(const AValue: Variant): Variant;
begin
  VarCast(Result, AValue, VarComplex);
  TComplexData(TComplexVarData(Result).VComplex).DoCscH;
end;

function VarComplexArcCosH(const AValue: Variant): Variant;
begin
  VarCast(Result, AValue, VarComplex);
  TComplexData(TComplexVarData(Result).VComplex).DoArcCosH;
end;

function VarComplexArcSinH(const AValue: Variant): Variant;
begin
  VarCast(Result, AValue, VarComplex);
  TComplexData(TComplexVarData(Result).VComplex).DoArcSinH;
end;

function VarComplexArcTanH(const AValue: Variant): Variant;
begin
  VarCast(Result, AValue, VarComplex);
  TComplexData(TComplexVarData(Result).VComplex).DoArcTanH;
end;

function VarComplexArcCotH(const AValue: Variant): Variant;
begin
  VarCast(Result, AValue, VarComplex);
  TComplexData(TComplexVarData(Result).VComplex).DoArcCotH;
end;

function VarComplexArcSecH(const AValue: Variant): Variant;
begin
  VarCast(Result, AValue, VarComplex);
  TComplexData(TComplexVarData(Result).VComplex).DoArcSecH;
end;

function VarComplexArcCscH(const AValue: Variant): Variant;
begin
  VarCast(Result, AValue, VarComplex);
  TComplexData(TComplexVarData(Result).VComplex).DoArcCscH;
end;
...
begin
  ...
  TComplexData(TComplexVarData(Temp).VComplex).GetAsPolar(ARadius, ATheta, AFixTheta);
end;

function VarComplexFromPolar(const ARadius, ATheta: Double): Variant;
begin
  Result := VarComplexCreate;
  TComplexData(TComplexVarData(Result).VComplex).SetAsPolar(ARadius, ATheta);
end;
