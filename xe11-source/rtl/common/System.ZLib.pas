...
*                                                                            *
*  revision history                                                          *
*    2022.04.01  updated Zlib library version 1.2.12 and review              *
*                  header translation                                        *
...
function deflateGetDictionary(var strm: z_stream; dictionary: PByte;
  dictLength: PCardinal): Integer; cdecl;
...
(*
function deflatePending(var strm: z_stream; pending: PCardinal; bits: PInteger): Integer; cdecl;
...
function inflateGetDictionary(var strm: z_stream;
  dictionary: PByte; dictLength: PCardinal): Integer; cdecl;
...
function uncompress2(dest: PByte; var destLen: LongWord;
  source: PByte; var sourceLen: LongWord): Integer; cdecl;
...
(*
function gzfread(const buf; size, ntimes: NativeUInt; file_: gzFile): NativeUInt; cdecl;
...
(*
function gzfwrite(const buf; size, ntimes: NativeUInt; file_: gzFile): NativeUInt; cdecl;
...
type
  ...
  TZCompressionStreamExt = class(TZCompressionStream)
  protected
    ...
    begin
      ...
      begin
        ...
        repeat
          ...
          zresult := ZDecompressCheckWithoutBufferError(inflate(zstream, Z_NO_FLUSH));

          outSize := bufferSize - zstream.avail_out;

          outStream.Write(outBuffer, outSize);
        until ((zresult = Z_STREAM_END) or (zstream.avail_in = 0)) and (zstream.avail_out > 0);
      ...
      repeat
        ...
        outSize := bufferSize - zstream.avail_out;
        if (outSize = 0) and (zresult = Z_BUF_ERROR) then
          raise EZDecompressionError.Create(string(_z_errmsg[2 - Z_BUF_ERROR]));
...
function TZCompressionStreamExt.GetSize: Int64;
begin
  Result := FZStream.total_in;
end;
...
var
  ...
  localOffset: Int64;
begin
  if (NativeUInt(offset) < FZStream.total_out) and (Origin = soBeginning) then
  begin
  ...
  if ((Offset >= 0) and (Origin = soCurrent)) or
    (((NativeUInt(offset) >= FZStream.total_out)) and (Origin = soBeginning)) then
  begin
    localOffset := Offset;
...
procedure TZDecompressionStreamExt.AfterConstruction;
begin
  inherited AfterConstruction;
  FSize := -1;
end;

function TZDecompressionStreamExt.GetSize: Int64;
begin
  if FSize = -1 then
    FSize := inherited GetSize;
  Result := FSize;
end;
