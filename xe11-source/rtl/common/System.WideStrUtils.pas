...
function LoadWideStr(Ident: NativeUInt): WideString; inline;

{ System.pas }

{ Wide version of resourcestring loader. }

function LoadResWideString(ResStringRec: PResStringRec): WideString; inline;
...
begin
  ...
  Result := '';
  wlen := UnicodeFromLocaleChars(cp, 0, PAnsiChar(S), length(S), NIL, 0); // wlen is the number of UCS2 without NULL terminater.
  if wlen = 0 then exit;
  wbuf := GetMemory(wlen * sizeof(WideChar));
  try
    UnicodeFromLocaleChars(cp, 0, PAnsiChar(S), length(S), wbuf, wlen);

    ulen := LocaleCharsFromUnicode(CP_UTF8, 0, wbuf, wlen, NIL, 0, NIL, NIL);
    setlength(Result, ulen);
    LocaleCharsFromUnicode(CP_UTF8, 0, wbuf, wlen, PAnsiChar(Result), ulen, NIL, NIL);
  finally
    FreeMemory(WBuf);
  end;
end;
...
begin
  ...
  Result := '';
  wlen := UnicodeFromLocaleChars(CP_UTF8, 0, PAnsiChar(S), length(S), NIL, 0); // wlen is the number of UCS2 without NULL terminater.
  if wlen = 0 then exit;
  wbuf := GetMemory(wlen * sizeof(WideChar));
  try
    UnicodeFromLocaleChars(CP_UTF8, 0, PAnsiChar(S), length(S), wbuf, wlen);

    ulen := LocaleCharsFromUnicode(cp, 0, wbuf, wlen, NIL, 0, NIL, NIL);
    setlength(Result, ulen);
    LocaleCharsFromUnicode(cp, 0, wbuf, wlen, PAnsiChar(Result), ulen, NIL, NIL);
  finally
    FreeMemory(WBuf);
  end;
end;
...
//----------------------------------------------------------
// Convert from ANSI encoding to UTF8 encoding in Stream.
// Stream's position set to the begin of the stream.
...
begin
  ...
  begin
    SUTF8 := TStringStream.Create('', TEncoding.UTF8);
    try
      Src.Position := 0;
...
begin
  Result := System.SysUtils.LoadStr(Ident);
end;
