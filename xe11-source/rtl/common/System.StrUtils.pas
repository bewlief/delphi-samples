...
begin
  if ASubText = '' then
    ...
    Result := StrLComp(PChar(AText), Pointer(ASubText), ASubText.Length) = 0;
end;
...
begin
  ...
  if (SubTextLocation > 0) and (ASubText <> '') and
    ...
    Result := StrComp(Pointer(ASubText), PChar(@AText[SubTextLocation - StrAdjust])) = 0
  else
    Result := False;
end;
...
begin
  Result := nil;
  if (BufLen <= 0) or (SearchString.IsEmpty) then Exit;
  if soDown in Options then
  begin
    Direction := 1;
    Inc(SelStart, SelLength);  { start search past end of selection }
  ...
  begin
    Direction := -1;
    Dec(SelStart, SearchString.Length - 1);
    SearchCount := SelStart + 1;
  end;
  if (SelStart < 0) or (SelStart > BufLen) then Exit;
  Result := @Buf[SelStart];
  ...
  while SearchCount > 0 do
  begin
    if (soWholeWord in Options) and ((Result <> @Buf[SelStart]) or not Result^.IsLetterOrDigit) then
      if not FindNextWordStart(Result) then Break;
    I := 0;
    ...
    begin
      ...
      begin
        if not (soWholeWord in Options) or
           ((Result + I >= Buf + BufLen) or not Result[I].IsLetterOrDigit) and
           ((Result <= Buf) or not Result[-1].IsLetterOrDigit) then
          Exit;
        Break;
      end;
    end;
