...
type
  ...
  protected
    ...
    /// <remarks> When the token to be written is the last end token, the document it will be flushed. See <see cref="Flush">Flush</see></remarks>
  ...
  public
    ...
    /// <summary> Close the writer and flushes the document. See <see cref="Flush">Flush</see>.</summary>
...
begin
  ...
  WriteKey;
  WriteBytes(Value.ToByteArray, TJsonBinaryType.Uuid);
end;
...
begin
  ...
  WriteInt64(DateTimeToMilliseconds(Value) - DateTimeToMilliseconds(UnixDateDelta));
end;
...
//    TBsonType.BsonDocument: ;
    ...
    TBsonType.DateTime:
      begin
        LDate := IncMilliSecond(UnixDateDelta, ReadInt64);
        if DateTimeZoneHandling = TJsonDateTimeZoneHandling.Local then
          LDate := TTimeZone.Local.ToLocalTime(LDate);
        SetToken(TJsonToken.Date, LDate);
      end;
    TBsonType.Null:
...
begin
  ...
  begin
    LLength := Length(FByteBuffer);
    while ALength > LLength do
      LLength := LLength * 2;
    SetLength(FByteBuffer, LLength);
  end;
