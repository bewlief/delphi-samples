...
begin
  ...
  if IsLatin1(C) then
    ...
    Result := CheckLetter(InternalGetUnicodeCategory(ConvertToUtf32(S, Index)));
end;
...
{$ELSEIF Defined(MSWINDOWS)}
  if C < $10000 then
    Result := UCS4Char(CharLower(WideChar(C)))
  else
  begin
    var s1: string := ConvertFromUtf32(C);
    var s2: string := ToLower(s1);
    Result := ConvertToUtf32(s2, Low(string));
  end;
{$ENDIF}
end;
...
{$ELSEIF Defined(MSWINDOWS)}
  if C < $10000 then
    Result := UCS4Char(CharUpper(WideChar(C)))
  else
  begin
    var s1: string := ConvertFromUtf32(C);
    var s2: string := ToUpper(s1);
    Result := ConvertToUtf32(s2, Low(string));
  end;
{$ENDIF}
end;
