...
{$SCOPEDENUMS ON}
  ...
  TLocationUsageAuthorization = (WhenInUse, Always);
  TLocationActivityType = (Other, Automotive, Fitness, Navigation);
...
// region monitoring will not work
  ...
  protected
    ...
    /// <summary>
    /// Method for overriding in heritor for getting native interface of sensor
    /// </summary>
    function DoGetInterface(const IID: TGUID; out Obj): HResult; virtual; abstract;
  public
    /// <summary>
    /// Method for getting native interface of sensor
    /// </summary>
  ...
  public type
    ...
    TRegionList = class(TList<TLocationRegion>)
    public
      constructor Create;
    end;
  private
    ...
    FOnLocationUpdatesPausedChanged: TLocationUpdatesPausedChangedEvent;
  protected
    ...
    function GetActivityType: TLocationActivityType; virtual; abstract;
    function GetDistance: TLocationDistance; virtual; abstract;
    function GetPowerConsumption: TPowerConsumption; virtual; abstract;
    function GetUsageAuthorization: TLocationUsageAuthorization; virtual; abstract;
    procedure SetAccuracy(const Value: TLocationAccuracy); virtual; abstract;
    procedure SetActivityType(const Value: TLocationActivityType); virtual; abstract;
    ...
    procedure SetUsageAuthorization(const Value: TLocationUsageAuthorization); virtual; abstract;
    ...
    procedure DoLocationUpdatesPausedChanged(const APaused: Boolean); virtual;
  ...
  public
    ...
    property OnLocationUpdatesPausedChanged: TLocationUpdatesPausedChangedEvent read FOnLocationUpdatesPausedChanged write FOnLocationUpdatesPausedChanged;
  end;
...
uses
  System.Variants, System.Math, System.Character, System.Generics.Defaults,
{$IFDEF ANDROID}
...
type
  TLocationRegionComparer = class(TComparer<TLocationRegion>)
  public
    function Compare(const Left, Right: TLocationRegion): Integer; override;
  end;

{ TLocationRegionComparer }

function TLocationRegionComparer.Compare(const Left, Right: TLocationRegion): Integer;
begin
...
function TCustomSensor.GetInterface(const IID: TGUID; out Obj): HResult;
begin
  Result := DoGetInterface(IID, Obj);
end;
...
begin
  raise ErrorClass.CreateFmt(Msg, Params);
end;
...
{ TCustomLocationSensor.TRegionList }

constructor TCustomLocationSensor.TRegionList.Create;
begin
  inherited Create(TLocationRegionComparer.Create);
end;
...
procedure TCustomLocationSensor.DoLocationChanged(const OldLocation, NewLocation: TLocationCoord2D);
begin
...
procedure TCustomLocationSensor.DoLocationUpdatesPausedChanged(const APaused: Boolean);
begin
  if Assigned(FOnLocationUpdatesPausedChanged) then
    FOnLocationUpdatesPausedChanged(Self, APaused);
end;
...
procedure TCivicAddress.Reset;
begin
  FCoord := TLocationCoord2D.Create(0, 0);
  FAddress := string.Empty;
  FAdminArea := string.Empty;
  FCountryCode := string.Empty;
  FCountryName := string.Empty;
  FFeatureName := string.Empty;
  FLocale := string.Empty;
  FLocality := string.Empty;
  FPhone := string.Empty;
  FPostalCode := string.Empty;
  FPremises := string.Empty;
  FSubAdminArea := string.Empty;
  FSubLocality := string.Empty;
  FSubThoroughfare := string.Empty;
  FThoroughfare := string.Empty;
  FURL := string.Empty;
end;
