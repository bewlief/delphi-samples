...
var
  ClassID, ProgID, ServerKeyName, RegPrefix: string;
  RootKey: HKEY;
begin
  if FInstancing = ciInternal then
    Exit;
  ...
  begin
    ...
    CreateRegKey(ServerKeyName, '', FComServer.ServerFileName, RootKey);
...
begin
  ...
  begin
    ...
    var LSilencer: IUnknown := TDispatchSilencer.Create(Sink, FAutoFactory.FEventIID);
    EventSinkChanged(LSilencer);
  end
  else
  begin
    FEventSink := nil;
...
{$ENDIF}
  ...
  begin
    case DispDesc.ResType of
      ...
      varUInt32:    PCardinal(Result)^ := Res.VUInt32;
...
{$ENDIF}
      ...
      varUInt32:    _DispReturnInteger(Res.VUInt32);
