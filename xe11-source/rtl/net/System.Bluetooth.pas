...
{$HPPEMIT CLOSENAMESPACE}
  ...
  protected
    ...
    procedure DoRemoteRequestPair(const ADevice: TBluetoothDevice); virtual;
    { Discovery process }
    /// <summary> Starts a discovery for Bluetooth devices. This is a really heavy Bluetooth action.</summary>
    /// <summary> Suggestion of cancellation before starting another Bluetooth/BLE action.</summary>
    procedure StartDiscovery(Timeout: Cardinal);
    procedure CancelDiscovery;
    procedure StartDiscoverable(Timeout: Cardinal);
  public
...
{$HPPEMIT CLOSENAMESPACE}
  ...
  protected
    ...
    /// <summary> Reset the devices of the AllDiscoveredLEDevices list</summary>
  ...
  protected
    ...
    /// <summary> TBluetooth Devices List of all discovered BLE Devices </summary>
    FAllDiscoveredLEDevices: TBluetoothLEDeviceList;
    /// <summary> TBluetooth Devices List of active discovered BLE Devices </summary>
    FLastDiscoveredLEDevices: TBluetoothLEDeviceList;
  ...
  protected
    ...
    /// <summary>Implementation for StartDiscoveryRaw function. Refresh to true read the new ScanFilterList.</summary>
    ...
    procedure DoDiscoveryEnd(const Sender: TObject; const ADeviceList: TBluetoothLEDeviceList); virtual;
    /// <summary> Raised once a device is found </summary>
    ...
    /// <summary> Raises OnDiscoverLEDevice event </summary>
    procedure DoDiscoverDevice(const Sender: TObject; const ADevice: TBluetoothLEDevice; Rssi: Integer;
      const ScanResponse: TScanResponse); virtual;
    { Discovery process }
    /// <summary> Starts a scan for Bluetooth LE devices. If AFilterUUIDList is provided, only peripherals that advertise the services
    /// you specify will be discovered. If you set ForceConnect to True, the scan process will connect with all discovered devices to search for
  ...
  protected
    ...
    /// <summary> FScanned is the var that holds if the device has been detected (scanned) in last scan period. </summary>
    FScanned: Boolean;
    /// <summary> FPaired is the var that holds if the device is paired. </summary>
    FPaired: Boolean;
    ...
    /// <summary>Implementation for RequestMtu function </summary>
  ...
  public
    ...
    /// <summary>This property only has effect in iOS/MacOS/Android.
    /// Setting to True, updates the services tree when a previously connected device is re-connected (Reading or Writing).
    ...
    /// <summary>
    ///  Scanned indicates if the device has been detected (scanned) in last scan period.
    /// </summary>
...
class procedure TBluetoothLEManager.ResetDevicesFromList(const ADeviceList: TBluetoothLEDeviceList);
var
  I: Integer;
begin
  for I := 0 to  ADeviceList.Count - 1 do
    ADeviceList[I].FScanned := False;
end;
...
begin
  inherited;
  FLastDiscoveredLEDevices := TBluetoothLEDeviceList.Create(False);
  FAllDiscoveredLEDevices := TBluetoothLEDeviceList.Create;
...
begin
  FFilterUUIDList := nil;
  FLastDiscoveredLEDevices.Free;
  FAllDiscoveredLEDevices.Free;
  if FCurrentManager = Self then
    FCurrentManager := nil;
  inherited;
end;
...
procedure TBluetoothLEManager.DoDiscoveryEnd(const Sender: TObject; const ADeviceList: TBluetoothLEDeviceList);
  ...
  function FilterDiscoveredDevices(Filter: Boolean; const ADeviceList: TBluetoothLEDeviceList): Boolean;
  var
    I: Integer;
  begin
    Result := True;
  ...
  if not FServicesFilterScan then
    try
      if Assigned(FOnDiscoverLEDevice) then
        FOnDiscoverLEDevice(Self, ADevice, Rssi, ScanResponse);
    except
      if Assigned(ApplicationHandleException) then
        ApplicationHandleException(Self)
      else
...
begin
  if ADeviceList <> nil then
    ...
    FManager.DoDiscoveryEnd(Sender, FManager.LastDiscoveredDevices);
end;

procedure TBluetoothLEAdapter.DoDeviceDiscovered(const ADevice: TBluetoothLEDevice; ANewDevice: Boolean;
...
begin
  ...
  begin
    if (ABluetoothLEScanFilterList <> nil) then
      LDiscovered := DoDeviceOvercomesFilters(ADevice, ABluetoothLEScanFilterList);

    if LDiscovered then
    begin
      if ANewDevice then
        TBluetoothLEManager.AddDeviceToList(ADevice, FManager.ALLDiscoveredDevices);
      TBluetoothLEManager.AddDeviceToList(ADevice, FManager.LastDiscoveredDevices);
      ADevice.FScanned := True;
    end;
  end;
...
begin
  FManager.DoDiscoverDevice(Self, ADevice, Rssi, ScanResponse);
end;
...
begin
  if (State <>  TBluetoothAdapterState.Discovering) then
  begin
    FManager.ClearDevicesFromList(FManager.LastDiscoveredDevices);
    FManager.ResetDevicesFromList(FManager.AllDiscoveredDevices);
...
constructor TBluetoothGattService.Create;
begin
  inherited Create;
...
begin
  ...
  Result := '';
  if Assigned(ACharacteristic.Descriptors) then
    for var LDescriptor in ACharacteristic.Descriptors do
      if LDescriptor.Kind = TBluetoothDescriptorKind.UserDescription then
      begin
        Result := LDescriptor.GetUserDescription;
        Break;
      end;
end;
