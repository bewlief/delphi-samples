...
uses
  ...
  System.Mac.Bluetooth, Macapi.Bluetooth;
...
type
  ...
  private
    ...
    FManagerState: CBManagerState;
    FIsWaitingToScan: Boolean;
  private
     FScanning: Boolean;
     ...
     procedure OnManagerStateChangedHandler(AManagerState: CBManagerState);
  protected
    ...
    function DoStartScanHelper: Boolean;
...
begin
  ...
  TBluetoothLEManagerHelper.Current.OnManagerStateChanged := OnManagerStateChangedHandler;
end;
...
procedure TiOSBeaconManager.OnManagerStateChangedHandler(AManagerState: CBManagerState);
begin
  FManagerState := AManagerState;

  if FIsWaitingToScan and (AManagerState = CBManagerStatePoweredOn) then
    DoStartScanHelper;
end;

function TiOSBeaconManager.DoStartScanHelper: Boolean;
...
begin
  if FManagerState = CBManagerStatePoweredOn then
    Result := DoStartScanHelper
  else
  begin
    Result := True;
    FIsWaitingToScan := True;
  end;
end;
