...
begin
  if (not FClosed) and Assigned(FBluetoothLEDevice) and (FBluetoothLEDevice.QueryInterface(IClosable, LBLEClosable) = 0) then
  begin
    CloseServices;
    LBLEClosable.Close;
    FClosed := True;
...
begin
  ...
  begin
    if TAsyncOperation<IAsyncOperation_1__GenericAttributeProfile_IGattCharacteristicsResult>.Wait(
          service3.GetCharacteristicsAsync(BluetoothCacheMode.Uncached), res3) = AsyncStatus.Completed
    then
    begin
