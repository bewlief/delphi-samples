...
begin
  ...
  FAdapter.DoDeviceDiscovered(LDevice, LNew, FAdapter.FBluetoothLEScanFilterList);
end;
...
begin
  Result := TAndroidBluetoothLEManager.AddDeviceToList(TAndroidBluetoothLEDevice.Create(AJDevice, True, True),
...
var
  LCharacteristicBytes: TJavaArray<Byte>;
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      TThread.Sleep(BLEDiscoverRefreshTimeout);
    end;
...
var
  LDescriptorBytes: TJavaArray<Byte>;
begin
...
var
  LDevice: TAndroidBluetoothLEDevice;
  LNew: Boolean;
  LScanRecordBytes: TJavaArray<Byte>;
begin
  FAdapter.FStartScanFailed := 0;
  FAdapter.FEventStartScan.SetEvent;
  LNew := False;

  LDevice := TAndroidBluetoothLEDevice(TAndroidBluetoothLEManager.GetDeviceInList(JStringToString(result.getDevice.getAddress),
    FAdapter.FManager.AllDiscoveredDevices));
  if LDevice = nil then
  begin
  ...
  LScanRecordBytes := result.getScanRecord.getBytes;
  try
    LDevice.FAdvertisedData := ScanRecordToTScanResponse(LScanRecordBytes, LDevice.FAdvertisedData);
  finally
