...
uses
  System.SysUtils, System.Bluetooth, Macapi.Bluetooth;
...
{$IFDEF BLUETOOTH_LE}
  ...
  TBluetoothLEManagerStateChangedEvent = procedure(AManagerState: CBManagerState) of object;

  TBluetoothLEManagerHelper = class
  strict private
    FOnManagerStateChanged: TBluetoothLEManagerStateChangedEvent;
    class var FCurrent: TBluetoothLEManagerHelper;

    class constructor Create;
    class destructor Destroy;
  private
    procedure NotifyManagerStateChange(AManagerState: CBManagerState);
  public
    property OnManagerStateChanged: TBluetoothLEManagerStateChangedEvent read FOnManagerStateChanged write FOnManagerStateChanged;

    class property Current: TBluetoothLEManagerHelper read FCurrent;
  end;
{$ENDIF BLUETOOTH_LE}
...
uses
  ...
  System.NetConsts, System.Generics.Defaults, System.Generics.Collections, Macapi.CoreFoundation,
{$IFDEF IOS}
  iOSapi.Foundation,
  iOSapi.CocoaTypes,
{$ELSE}
...
{ TBluetoothLEManagerHelper }

class constructor TBluetoothLEManagerHelper.Create;
begin
  FCurrent := TBluetoothLEManagerHelper.Create;
end;

class destructor TBluetoothLEManagerHelper.Destroy;
begin
  FCurrent.Free;
end;

procedure TBluetoothLEManagerHelper.NotifyManagerStateChange(AManagerState: CBManagerState);
begin
  if Assigned(FOnManagerStateChanged) then
    FOnManagerStateChanged(AManagerState);
end;
...
begin
  ...
  begin
    LEDevice := TMacBluetoothLEDevice(TMAcBluetoothLEManager.GetDeviceInList(UUID.ToString, LEManager.AllDiscoveredDevices));
    ...
    DoDeviceDiscovered(LEDevice, LNew, FBluetoothLEScanFilterList);
  end;
end;

function TMacBluetoothLEAdapter.GetLEDevice(const ADevice: CBPeripheral): TBluetoothLEDevice;
begin
  Result := TMacBluetoothLEManager.GetDeviceInList(TMacBluetoothLEManager(FManager).GetDeviceIdentifier(ADevice).ToString,
...
begin
  if FLEManager.FIsDiscovering then
...
begin
  ...
  if AFilterUUIDList <> nil then
    LFilterUUIDList := AFilterUUIDList
  else
  begin
...
begin
  ...
  begin
    ...
    LNumber := TNSNumber.Wrap(TNSNumber.OCClass.numberWithBool(True));
    FLEManager.FScanOptions.setValue((LNumber as ILocalObject).GetObjectID, CBCentralManagerScanOptionAllowDuplicatesKey);
  end;

  if FLEManager.FCentralManager.state = CBManagerStatePoweredOn then
  begin
...
begin
  if (central.state = CBManagerStatePoweredOn) and FWaitingToDiscover then
  begin
  ...
  TBluetoothLEManagerHelper.Current.NotifyManagerStateChange(central.state);
end;
...
begin
  ...
  if FCentralManager.state = CBManagerStatePoweredOn then
  begin
    FWaitingToDiscover := False;
...
begin
  LMacBluetoothLEAdapter := CurrentAdapter as TMacBluetoothLEAdapter;
  if LMacBluetoothLEAdapter.FLEManager.FCentralManager.state = CBManagerStatePoweredOn then
    Result := TBluetoothConnectionState.Connected
  else
  begin
    LTotal := 0;
    repeat
      InternalWaitMessage(0.05);
      LTotal := LTotal + 0.05;
    until (LTotal >= 1) or (LMacBluetoothLEAdapter.FLEManager.FCentralManager.state = CBManagerStatePoweredOn);
    if LMacBluetoothLEAdapter.FLEManager.FCentralManager.state = CBManagerStatePoweredOn then
      Result := TBluetoothConnectionState.Connected
    else
      Result := TBluetoothConnectionState.Disconnected;
