...
{$ENDIF}
  ...
  public
    ...
    /// <summary>Lookup and return an ip address based on passed in the domain name.
    ///  When lookup failed, then if RaiseException is True then exception will be raised,
    ///  otherwise empty address will be returned. </summary>
    class function LookupName(const Name: string; RaiseException: Boolean = False): TIPAddress; static;
...
{$ENDIF}
  ...
  private
    ...
    procedure DoSetReceiveTimeout(Value: Integer);
    procedure SetSendTimeout(const Value: Integer);
    procedure DoSetSendTimeout(Value: Integer);
    procedure WaitToReceiveData;
  protected type
  ...
  public
    ...
    /// <summary>
    ///  Perform a DNS lookup of 'name'. When lookup failed, then if RaiseException is True
    ///  then exception will be raised, otherwise empty address will be returned.
    /// </summary>
    function LookupName(const name: string; RaiseException: Boolean = False): TIPAddress;
    /// <summary>
    ...
    /// <summary>
    ///    Property specifies receiving timeout for blocking calls, eg Receive, in msecs.
    ///    0 and -1 means inifnite timeout. Default value is 0.
    /// </summary>
    property ReceiveTimeout: Integer read FReceiveTimeout write SetReceiveTimeout default 0;
    /// <summary>
    ///    Property specifies sending timeout for blocking calls, eg Send, in msecs.
    ///    0 and -1 means inifnite timeout. Default value is 0.
    /// </summary>
    property SendTimeout: Integer read FSendTimeout write SetSendTimeout default 0;
  end;
...
function acceptsocket(Socket: TSocketHandle; var Addr: sockaddr_in; var Len: Integer): TSocketHandle; inline;
begin
{$IFDEF MSWINDOWS}
  Result := accept(Socket, @Addr, @Len);
{$ENDIF}
{$IFDEF POSIX}
  Result := accept(Socket, sockaddr(Addr), socklen_t(Len));
{$ENDIF}
end;
...
begin
  ...
  begin
    ...
    if Result = 0 then
      raise ESocketError.CreateResFmt(@sSocketFailure, [Op])
    else if Result <> WSAEWOULDBLOCK then
    begin
      if Result = WSAETIMEDOUT then
        Exc := ESocketError.CreateResFmt(@sSocketTimeout, [Op])
      else
        Exc := ESocketError.CreateResFmt(@sSocketError, [SysErrorMessage(Result), Result, Op]);
      Exc.FCode := Result;
      raise Exc;
    end;
  end
...
begin
  ...
  begin
    ...
    if Result = 0 then
      raise ESocketError.CreateResFmt(@sSocketFailure, [Op])
    else if (Result <> EWOULDBLOCK) or
            (Result = EWOULDBLOCK) and (Op.StartsWith('recv') or Op.StartsWith('send')) then
    begin
      if (Result = EWOULDBLOCK) or (Result = EAGAIN) then
        Exc := ESocketError.CreateResFmt(@sSocketTimeout, [Op])
      else
        Exc := ESocketError.CreateResFmt(@sSocketError, [SysErrorMessage(Result), Result, Op]);
      Exc.FCode := Result;
...
begin
  Result.FAddr.S_addr := htonl(InAddrAny);
end;

class function TIPAddress.GetBroadcast: TIPAddress;
begin
  Result.FAddr.S_addr := htonl(InAddrBroadcast);
end;

class function TIPAddress.GetLocalHost: TIPAddress;
begin
  Result.FAddr.S_addr := htonl(InAddrLoopback);
end;
...
var
  M: TMarshaller;
begin
  Result.FAddr.S_addr := inet_addr(M.AsAnsi(Address).ToPointer);
end;

class function TIPAddress.LookupName(const Name: string; RaiseException: Boolean): TIPAddress;
...
begin
  ...
  SetLength(AsciiStr, IdnToAscii(0, PChar(Name), Length(Name), nil, 0));
  if Length(AsciiStr) = 0 then
    if RaiseException then CheckSocketResult(-1, 'IdnToAscii') else Exit;
  SetLength(AsciiStr, IdnToAscii(0, PChar(Name), length(Name), PChar(AsciiStr), Length(AsciiStr)));
  ...
  begin
    ...
    HostInfo := nil;
    res := GetAddrInfoW(PChar(AsciiStr), nil, Hint, HostInfo);
    if res <> 0 then
      if RaiseException then CheckSocketResult(-1, 'GetAddrInfoW') else Exit;
    if HostInfo <> nil then
    begin
      Result.FAddr := Psockaddr_in(HostInfo.ai_addr).sin_addr;
      FreeAddrInfoW(HostInfo^);
    end;
  end

  else
  begin
    HostEnt := gethostbyname(M.AsAnsi(AsciiStr).ToPointer);
    if HostEnt = nil then
      if RaiseException then CheckSocketResult(-1, 'gethostbyname') else Exit;
...
{$ENDIF}
  ...
  HostInfo := nil;
{$IF Defined(MSWINDOWS)}
  res := getaddrinfo(M.AsAnsi(Name).ToPointer, nil, Hint, HostInfo);
{$ELSEIF Defined(ANDROID)}
  // ANDROID BIONIC doens't support IDN
  res := getaddrinfo(M.AsUTF8(TURI.UnicodeToIDNA(Name)).ToPointer, nil, Hint, HostInfo);
{$ELSE}
  res := getaddrinfo(M.AsUTF8(Name).ToPointer, nil, Hint, HostInfo);
{$ENDIF}
  if res <> 0 then
    if RaiseException then CheckSocketResult(-1, 'getaddrinfo') else Exit;
  if HostInfo <> nil then
  begin
    Result.FAddr := Psockaddr_in(HostInfo.ai_addr).sin_addr;
...
begin
  ...
  Result.FEndPoint.sin_addr.S_addr := htonl(Address);
end;
...
begin
  SetSize(Result);
  Result.FEndPoint.sin_family := AF_INET;
  Result.FEndPoint.sin_addr := TIPAddress.LookupName(Name, True);
...
var
  ...
  M: TMarshaller;
begin
  ServEnt := getservbyname(M.AsAnsi(Service).ToPointer, Proto[SocketType]);
  if ServEnt <> nil then
...
begin
  SetAddress(TIPAddress.LookupName(Address, True));
end;
...
begin
  ...
  begin
    FDR := TFDSet.Create(Self);
    if Select(@FDR, nil, nil, Timeout * 1000) = TWaitResult.wrTimeout then
      Exit;
  end;
  Len := SizeOf(Addr);
  ClientHandle := acceptsocket(FSocket, Addr, Len);
  CheckSocketResult(Integer(ClientHandle), 'accept');
...
begin
  ...
  if not ForceClosed and (FSocketType in [TSocketType.TCP, TSocketType.RAW, TSocketType.RDM, TSocketType.SEQ]) then
  begin
    Res := shutdown(FSocket, BOTH);
{$IFDEF POSIX}
    // if other side closed connection, then just ignore error
    if (Res = 0) or (GetLastError <> ENOTCONN) then
{$ENDIF}
    begin
      CheckSocketResult(Res, 'shutdown');
...
begin
  Result := socket(PF_INET, _Type[FSocketType], Proto[FSocketType]);
...
const
  CMaxHostLen = 256;
var
  LocalName: TPtrWrapper;
begin
  Result := '';
  if FSocket = InvalidSocket then Exit;
  LocalName := TMarshal.AllocMem(CMaxHostLen);
  try
    if gethostname(MarshaledAString(LocalName.ToPointer), CMaxHostLen) = 0 then
{$IF defined(MSWINDOWS)}
      Result := TMarshal.ReadStringAsAnsi(LocalName);
{$ELSEIF defined(POSIX)}
      Result := TMarshal.ReadStringAsUtf8(LocalName);
{$ENDIF}
  finally
    TMarshal.FreeMem(LocalName);
  end;
end;
...
begin
  ...
  if Name <> '' then
    Result.sin_addr := LookupName(name, True)
  else if Address <> '' then
  begin
{$IFDEF MSWINDOWS}
    // Windows Vista and later only support inet_pton
    if Win32MajorVersion >= 6 then
    begin
{$ENDIF}
      i := inet_pton(2, MarshaledAString(AddressToBytes(Address)), @Result.sin_addr);
      if i <> 1 then
      begin
        Result.sin_addr.s_addr := INADDR_NONE;
...
begin
  ...
  if FSocketType in [TSocketType.TCP, TSocketType.RAW, TSocketType.RDM, TSocketType.SEQ] then
    DoListen(QueueSize)
  else
    DoBind;
end;
...
begin
  if TSocketState.Connected in FState then
    raise ESocketError.CreateRes(@sSocketAlreadyOpen);
  if FSocketType <> TSocketType.UDP then
...
function TSocket.LookupName(const name: string; RaiseException: Boolean): TIPAddress;
begin
  Result := TIPAddress.LookupName(name, RaiseException);
end;

function TSocket.LookupService(const service: string): Integer;
begin
  Result := TNetEndpoint.LookupService(service, FSocketType);
end;
...
begin
  ...
  begin
    ...
    begin
      if Available < 0 then
        Exit(-1)
      else if Available > Count then
        Available := Count
      else if Available = 0 then
        Available := 1;
      Dec(Count, Receive(Bytes[Result - Count], Available, Flags));
      if Count > 0 then
      begin
        Available := ReceiveLength;
        if (Available = 0) and TThread.IsTimeout(StartTime, ReceiveTimeout) then
          raise ESocketError.CreateRes(@sReceiveTimeout);
      end;
    end;
  end else
    Result := -1;
  ...
  begin
    ...
    begin
      if Available < 0 then
        ...
        Available := 1;
      Dec(Count, Receive(Bytes[Offset + (Result - Count)], Available, Flags));
      if Count > 0 then
      begin
        Available := ReceiveLength;
        if (Available = 0) and TThread.IsTimeout(StartTime, ReceiveTimeout) then
...
//    ValidSocketTypes([TSocketType.UDP]);
    ...
    begin
      if Available < 0 then
        ...
        Available := 1;
      Dec(Count, ReceiveFrom(Bytes[Offset + (Result - Count)], Available, Endpoint, Flags));
      if Count > 0 then
      begin
        Available := ReceiveFrom(Pointer(nil)^, -1);
        if (Available = 0) and TThread.IsTimeout(StartTime, ReceiveTimeout) then
...
//    ValidSocketTypes([TSocketType.UDP]);
    ...
    begin
      if Available < 0 then
        ...
        Available := 1;
      Dec(Count, ReceiveFrom(Bytes[Result - Count], Available, Endpoint, Flags));
      if Count > 0 then
      begin
        Available := ReceiveFrom(Pointer(nil)^, -1);
        if (Available = 0) and TThread.IsTimeout(StartTime, ReceiveTimeout) then
...
procedure TSocket.SetReceiveTimeout(const Value: Integer);
begin
  if FReceiveTimeout <> Value then
  begin
    if Value < -1 then
      raise EArgumentOutOfRangeException.CreateResFmt(@sSocketTimeoutValue, ['ReceiveTimeout']);
    FReceiveTimeout := Value;
    if FSocket <> InvalidSocket then
      DoSetReceiveTimeout(Value);
  end;
end;

procedure TSocket.DoSetReceiveTimeout(Value: Integer);
{$IFDEF POSIX}
var
  time: timeval;
{$ENDIF}
...
{$IFDEF MSWINDOWS}
  setsocketoption(FSocket, SOL_SOCKET, SO_RCVTIMEO, Value, SizeOf(Value));
{$ENDIF}
{$IFDEF POSIX}
  time.tv_sec := Value div 1000;
  time.tv_usec := (Value mod 1000) * 1000;
  setsocketoption(FSocket, SOL_SOCKET, SO_RCVTIMEO, time, SizeOf(time));
{$ENDIF}
end;

procedure TSocket.SetSendTimeout(const Value: Integer);
begin
  if FSendTimeout <> Value then
  begin
    if Value < -1 then
      raise EArgumentOutOfRangeException.CreateResFmt(@sSocketTimeoutValue, ['SendTimeout']);
    FSendTimeout := Value;
    if FSocket <> InvalidSocket then
      DoSetSendTimeout(Value);
  end;
end;

procedure TSocket.DoSetSendTimeout(Value: Integer);
{$IFDEF POSIX}
var
  time: timeval;
{$ENDIF}
begin
  if Value < 0 then
    Value := Integer(INFINITE);
{$IFDEF MSWINDOWS}
  setsocketoption(FSocket, SOL_SOCKET, SO_SNDTIMEO, Value, SizeOf(Value));
{$ENDIF}
{$IFDEF POSIX}
  time.tv_sec := Value div 1000;
  time.tv_usec := (Value mod 1000) * 1000;
  setsocketoption(FSocket, SOL_SOCKET, SO_SNDTIMEO, time, SizeOf(time));
{$ENDIF}
end;
...
begin
  if not (FSocketType in SocketTypes) then
...
function TSocketStream.Seek(const Offset: Int64; Origin: TSeekOrigin): Int64;
  ...
  procedure Error;
  begin
    raise ERangeError.CreateRes(@sSeekNotImplemented);
  end;
