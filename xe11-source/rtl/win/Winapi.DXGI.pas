...
const
  ...
  DXGI_PRESENT_DO_NOT_WAIT            = UINT($00000008);
  {$EXTERNALSYM DXGI_PRESENT_DO_NOT_WAIT}
  DXGI_PRESENT_STEREO_PREFER_RIGHT    = UINT($00000010);
  {$EXTERNALSYM DXGI_PRESENT_STEREO_PREFER_RIGHT}
  DXGI_PRESENT_STEREO_TEMPORARY_MONO  = UINT($00000020);
  {$EXTERNALSYM DXGI_PRESENT_STEREO_TEMPORARY_MONO}
  DXGI_PRESENT_RESTRICT_TO_OUTPUT     = UINT($00000040);
...
type
  ...
  IDXGISwapChain = interface(IDXGIDeviceSubObject)
    ...
    function GetBuffer(
            (* [in] *) Buffer: UINT;
            (* [in] *) const riid: TIID;
            (* [out][in] *)out ppSurface): HResult; stdcall;
  ...
  DXGI_ADAPTER_FLAG = (
    ...
    //DXGI_ADAPTER_FLAG_FORCE_DWORD	= 0xffffffff // todo: add this as const?
  );
...
function CreateDXGIFactory1(const riid: TIID; out ppFactory): HResult; stdcall; external DXGI_dll delayed;
