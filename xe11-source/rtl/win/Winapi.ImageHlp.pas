...
function MapFileAndCheckSum(Filename: LPWSTR; var HeaderSum,
  CheckSum: DWORD): DWORD; stdcall;
{$EXTERNALSYM MapFileAndCheckSumA}
function MapFileAndCheckSumA(Filename: LPSTR; var HeaderSum,
  CheckSum: DWORD): DWORD; stdcall;
{$EXTERNALSYM MapFileAndCheckSumW}
function MapFileAndCheckSumW(Filename: LPWSTR; var HeaderSum,
  CheckSum: DWORD): DWORD; stdcall;
...
function BindImageEx(Flags: DWORD; ImageName, DllPath, SymbolPath: LPSTR;
  var StatusRoutine: TImagehlpStatusRoutine): Bool; stdcall;
...
function ImageRvaToVa(NtHeaders: PImageNtHeaders; Base: Pointer;
  Rva: ULONG; var LastRvaSection: PImageSectionHeader): Pointer; stdcall; overload;

{$EXTERNALSYM ImageRvaToVa}
function ImageRvaToVa(NtHeaders: PImageNtHeaders; Base: Pointer;
  Rva: ULONG; LastRvaSection: PPImageSectionHeader): Pointer; stdcall; overload;
...
type
  ...
  {$EXTERNALSYM PimageDebugInformation}
  ...
  {$EXTERNALSYM TImageDebugInformation}
...
type
  ...
  _tagADDRESS = record
    Offset: NativeUInt;   { DWORD in x32, DWORD64 in x64 }
    Segment: Word;
    Mode: TAddressMode;
  end;
  ADDRESS = _tagADDRESS;
...
{ from the DBGKD_GET_VERSION and WAIT_STATE_CHANGE packets. }
  ...
  PKdHelp = ^TKdHelp;
  {$EXTERNALSYM PKdHelp}
  _KDHELP = record      { address of kernel thread object, as provided }
...
{$ENDIF}
    ...
    KiCallUserMode: NativeUInt;
    { Address of the user mode dispatcher function }
    KeUserCallbackDispatcher: NativeUInt;
    { Lowest kernel mode address }
    SystemRangeStart: NativeUInt;
{$IFDEF WIN32}
    { offset in thread object to pointer to the current callback backing }
    { store frame in kernel stack. }
    ThCallbackBStore: DWORD;
{$ENDIF}
    ...
    KiUserExceptionDispatcher: NativeUInt;
    { Stack bounds, added in API version 11. }
    StackBase: NativeUInt;
    StackLimit: NativeUInt;

    Reserved: array [0..4] of NativeUInt;
  end;
  KDHELP = _KDHELP;
  {$EXTERNALSYM KDHELP}
  TKdHelp = _KDHELP;
...
{$ENDIF}
    ...
    Params: packed array[0..3] of NativeUInt;{ possible arguments to the function }
    _Far: Bool;                        { WOW far call }
    _Virtual: Bool;                    { is this a virtual frame? }
    Reserved: packed array[0..2] of NativeUInt;{ used internally by StackWalk api }
    KdHelp: TKdHelp;
{$IFDEF WIN32}
...
{ symbol data structure }
  ...
  {$EXTERNALSYM PImagehlpSymbol}
  _IMAGEHLP_SYMBOL = record
    SizeOfStruct: DWORD;                                { set to sizeof(IMAGEHLP_SYMBOL) }
    Address: NativeUInt;                                { virtual address including dll base address }
...
{$ELSE}
    ModuleName: packed array[0..31] of Byte;            { module name }
    ImageName: packed array[0..255] of Byte;            { image name }
    LoadedImageName: packed array[0..255] of Byte;      { symbol file name }
{$ENDIF}
{$IFDEF WIN64}
    { new elements: 07-Jun-2002 }
{$IF DECLARED(AnsiChar)}
    LoadedPdbName: packed array[0..255] of AnsiChar;    { pdb file name }
    CVSig: DWORD;                                       { Signature of the CV record in the debug directories}
    CVData: packed array[0..(MAX_PATH * 3)-1] of AnsiChar; { Contents of the CV record }
{$ELSE}
    LoadedPdbName: packed array[0..255] of Byte;        { pdb file name }
    CVSig: DWORD;                                       { Signature of the CV record in the debug directories}
    CVData: packed array[0..(MAX_PATH * 3)-1] of Byte;  { Contents of the CV record }
{$ENDIF}
...
type
  ...
  {$EXTERNALSYM PImagehlpDeferredSymbolLoad}
  _IMAGEHLP_DEFERRED_SYMBOL_LOAD = record
    ...
    TimeDateStamp: DWORD;                               { date/time stamp from pe header }
{$IF DECLARED(AnsiChar)}
    FileName: packed array[0..MAX_PATH-1] of AnsiChar;  { symbols file or image name }
{$ELSE}
    FileName: packed array[0..MAX_PATH-1] of Byte;      { symbols file or image name }
{$ENDIF}
    Reparse: BOOL;                                      { load failure reparse }
    hFile: THandle;                                     { file handle, if passed }
{$IFDEF WIN64}
    Flags: DWORD;
{$ENDIF}
  end;
  ...
  {$EXTERNALSYM TImagehlpDeferredSymbolLoad}

  PImagehlpDuplicateSymbol = ^TImagehlpDuplicateSymbol;
  {$EXTERNALSYM PImagehlpDuplicateSymbol}
  ...
  {$EXTERNALSYM TImagehlpDuplicateSymbol}
...
function ImageRvaToVa(NtHeaders: PImageNtHeaders; Base: Pointer;
  Rva: ULONG; var LastRvaSection: PImageSectionHeader): Pointer; external ImagehlpLib name 'ImageRvaToVa';
function ImageRvaToVa(NtHeaders: PImageNtHeaders; Base: Pointer;
  Rva: ULONG; LastRvaSection: PPImageSectionHeader): Pointer; external ImagehlpLib name 'ImageRvaToVa';
...
function SymGetModuleInfo;              external ImagehlpLib name 'SymGetModuleInfoW';
...
function SymGetSymFromAddr;             external ImagehlpLib name 'SymGetSymFromAddrW';
function SymGetSymFromName;             external ImagehlpLib name 'SymGetSymFromNameW';
function SymGetSymNext;                 external ImagehlpLib name 'SymGetSymNextW';
function SymGetSymPrev;                 external ImagehlpLib name 'SymGetSymPrevW';
