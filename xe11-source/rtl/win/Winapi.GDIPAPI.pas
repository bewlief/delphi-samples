...
type
  ...
  NotificationHookProc = function(out token: ULONG_PTR): Status; stdcall;
  {$EXTERNALSYM NotificationUnhookProc}
  NotificationUnhookProc = procedure(token: ULONG_PTR); stdcall;
...
{$ALIGN ON}
 ...
 function GdiplusStartup(out token: ULONG_PTR; input: PGdiplusStartupInput;
...
{  TGPColor = class
  ...
  {$EXTERNALSYM PGPColor 'DWORD*'} // should be 'ARGB*'
  {$ELSE}
  {$EXTERNALSYM PGPColor}
  {$ENDIF}
  TGPColor = ARGB;
  {$IF defined(WEAK_NATIVEINT)}
  {$EXTERNALSYM TGPColor 'DWORD'} // should be 'ARGB'
  {$ELSE}
