...
const
  ...
  {$EXTERNALSYM TBSTYLE_EX_MULTICOLUMN}
  TBSTYLE_EX_MULTICOLUMN = $00000002;              { conflicts w/ TBSTYLE_WRAPABLE }
  {$EXTERNALSYM TBSTYLE_EX_VERTICAL}
  TBSTYLE_EX_VERTICAL = $00000004;
...
function StateImageMaskToIndex(I: Integer): Longint; inline;
...
const
  ...
  {$EXTERNALSYM TVS_EX_NOSINGLECOLLAPSE}
...
const
  {$EXTERNALSYM TVM_SETBORDER}
  TVM_SETBORDER = TV_FIRST + 35;

{$EXTERNALSYM TreeView_SetBorder}
function TreeView_SetBorder(hwnd: HWND; dwFlags: DWORD; xBorder, yBorder: Smallint): COLORREF; inline;

const
  {$EXTERNALSYM TVSBF_XBORDER}
  TVSBF_XBORDER = $00000001;
  {$EXTERNALSYM TVSBF_YBORDER}
...
{$EXTERNALSYM Pager_SetScrollInfo}
procedure Pager_SetScrollInfo(hwnd: HWND; cTimeOut, cLinesPer, cPixelsPerLine: UINT); inline;
...
// Edit Control Extended Styles to use with EM_SETEXTENDEDSTYLE/EM_GETEXTENDEDSTYLE
  ES_EX_ALLOWEOL_CR = $0001;
  ES_EX_ALLOWEOL_LF = $0002;
  ES_EX_ALLOWEOL_ALL = ES_EX_ALLOWEOL_CR or ES_EX_ALLOWEOL_LF;
  ES_EX_CONVERT_EOL_ON_PASTE = $0004;
...
const
  {$EXTERNALSYM EM_NOSETFOCUS}
  EM_NOSETFOCUS = ECM_FIRST + 7;
  {$EXTERNALSYM EM_TAKEFOCUS}
  EM_TAKEFOCUS = ECM_FIRST + 8;

{$EXTERNALSYM Edit_NoSetFocus}
function Edit_NoSetFocus(hwndCtl: HWND): DWORD; inline;
{$EXTERNALSYM Edit_TakeFocus}
function Edit_TakeFocus(hwndCtl: HWND): DWORD; inline;

type
  {$EXTERNALSYM EC_ENDOFLINE}
  EC_ENDOFLINE = type Integer;
...
{$EXTERNALSYM Edit_SetExtendedStyle}
function Edit_SetExtendedStyle(hwndCtl: HWND; dw: LPARAM; dwMask: WPARAM): DWORD; inline;
{$EXTERNALSYM Edit_GetExtendedStyle}
function Edit_GetExtendedStyle(hwndCtl: HWND): DWORD; inline;
{$EXTERNALSYM Edit_SetEndOfLine}
function Edit_SetEndOfLine(hwndCtl: HWND; eolType: WPARAM): DWORD; inline;
{$EXTERNALSYM Edit_GetEndOfLine}
function Edit_GetEndOfLine(hwndCtl: HWND): EC_ENDOFLINE; inline;
{$EXTERNALSYM Edit_EnableSearchWeb}
function Edit_EnableSearchWeb(hwndCtl: HWND; enable: WPARAM): BOOL; inline;
{$EXTERNALSYM Edit_SearchWeb}
function Edit_SearchWeb(hwndCtl: HWND): BOOL; inline;

// Form codes are internal-only so keep the api internal

{$EXTERNALSYM Edit_SetCaretIndex}
function Edit_SetCaretIndex(hwndCtl: HWND; newCaretPosition: WPARAM): BOOL; inline;
{$EXTERNALSYM Edit_GetCaretIndex}
function Edit_GetCaretIndex(hwndCtl: HWND): DWORD; inline;


// We want to reuse the same messages as richedit.h
// which is why these are outside of the ECM_FIRST-ECM_LAST range.

{$EXTERNALSYM Edit_GetZoom}
function Edit_GetZoom(hwndCtl: HWND; numerator: WPARAM; denominator : LPARAM): BOOL; inline;
{$EXTERNALSYM Edit_SetZoom}
function Edit_SetZoom(hwndCtl: HWND; numerator: WPARAM;  denominator: LPARAM): BOOL; inline;
{$EXTERNALSYM Edit_GetFileLineFromChar}
function Edit_GetFileLineFromChar(hwndCtl: HWND; characterIndex: WPARAM): DWORD; inline;
{$EXTERNALSYM Edit_GetFileLineIndex}
function Edit_GetFileLineIndex(hwndCtl: HWND; lineNumber: WPARAM): DWORD; inline;
{$EXTERNALSYM Edit_GetFileLineLength}
function Edit_GetFileLineLength(hwndCtl: HWND; characterIndex: WPARAM): DWORD; inline;
{$EXTERNALSYM Edit_GetFileLine}
function Edit_GetFileLine(hwndCtl: HWND; lineNumber: WPARAM; textBuffer: LPARAM): DWORD; inline;
{$EXTERNALSYM Edit_GetFileLineCount}
function Edit_GetFileLineCount(hwndCtl: HWND): DWORD; inline;

//const
//  {$EXTERNALSYM EN_SEARCHWEB}
//  EN_SEARCHWEB = EN_FIRST - 0;

type
  EC_SEARCHWEB_ENTRYPOINT = type Integer;
const
...
type
  ...
  TNMSEARCHWEB = NMSEARCHWEB;
  PNMSEARCHWEB = ^TNMSEARCHWEB;
...
const
  ...
  {$EXTERNALSYM TDF_NO_SET_FOREGROUND}
  TDF_NO_SET_FOREGROUND               = $00010000; // Don't call SetForegroundWindow() when activating the dialog - WIN8
  {$EXTERNALSYM TDF_SIZE_TO_CONTENT}
  TDF_SIZE_TO_CONTENT                 = $01000000; // used by ShellMessageBox to emulate MessageBox sizing behavior
...
function TaskDialog(hwndOwner: HWND; hInstance: HINST; pszWindowTitle,
...
type
  ...
  tagTRACKMOUSEEVENT = Winapi.Windows.tagTRACKMOUSEEVENT;
  {$EXTERNALSYM TTrackMouseEvent}
  {$EXTERNALSYM PTrackMouseEvent}
  {$HPPEMIT OPENNAMESPACE}
  {$HPPEMIT 'using ::Winapi::Windows::TTrackMouseEvent;'}
  {$HPPEMIT 'typedef TTrackMouseEvent *PTrackMouseEvent;'}
...
type
  ...
  TWMNotifyTV = record { TWMNotify }
    ...
    case Integer of
      ...
      6: (NMTVItemChange: PNMTVItemChange;
          Result: LRESULT);
  end;
  ...
  TWMNotifyTRB = record { TWMNotify }
    ...
    case Integer of
      0: (NMHdr: PNMHdr);
      1: (NMReBarChevron: PNMReBarChevron);
      2: (NMCustomDraw: PNMCustomDraw;
          Result: LRESULT);
  end;
...
function StateImageMaskToIndex(I: Integer): Longint;
begin
  Result := I shr 12
end;
...
function TreeView_SetBorder(hwnd: HWND; dwFlags: DWORD; xBorder, yBorder: Smallint): COLORREF;
begin
  Result := COLORREF(SendMessage(hwnd, TVM_SETBORDER, dwFlags, MAKELPARAM(xBorder, yBorder)));
end;
...
procedure Pager_SetScrollInfo(hwnd: HWND; cTimeOut, cLinesPer, cPixelsPerLine: UINT);
begin
  SendMessage(hwnd, PGM_SETSCROLLINFO, cTimeOut, MAKELONG(cLinesPer, cPixelsPerLine));
end;
...
function Edit_NoSetFocus(hwndCtl: HWND): DWORD;
begin
  Result := DWORD(SendMessage(hwndCtl, EM_NOSETFOCUS, 0, 0));
end;

function Edit_TakeFocus(hwndCtl: HWND): DWORD;
begin
  Result := DWORD(SendMessage(hwndCtl, EM_TAKEFOCUS, 0, 0));
end;

function Edit_SetExtendedStyle(hwndCtl: HWND; dw: LPARAM; dwMask: WPARAM): DWORD;
begin
  Result := DWORD(SendMessage(hwndCtl, EM_SETEXTENDEDSTYLE, dwMask, dw));
end;

function Edit_GetExtendedStyle(hwndCtl: HWND): DWORD;
begin
  Result := DWORD(SendMessage(hwndCtl, EM_GETEXTENDEDSTYLE, 0, 0));
end;

function Edit_SetEndOfLine(hwndCtl: HWND; eolType: WPARAM) : DWORD;
begin
  Result := DWORD(SendMessage(hwndCtl, EM_SETENDOFLINE, eolType, 0));
end;

function Edit_GetEndOfLine(hwndCtl: HWND): EC_ENDOFLINE;
begin
  Result := EC_ENDOFLINE(SendMessage(hwndCtl, EM_GETENDOFLINE, 0, 0));
end;

function Edit_EnableSearchWeb(hwndCtl: HWND; enable: WPARAM): BOOL;
begin
  Result := BOOL(SendMessage(hwndCtl, EM_ENABLESEARCHWEB, enable, 0));
end;

function Edit_SearchWeb(hwndCtl: HWND): BOOL;
begin
  Result := BOOL(SendMessage(hwndCtl, EM_SEARCHWEB, 0, 0));
end;

function Edit_SetCaretIndex(hwndCtl: HWND; newCaretPosition: WPARAM): BOOL;
begin
  Result := BOOL(SendMessage(hwndCtl, EM_SETCARETINDEX, newCaretPosition, 0));
end;

function Edit_GetCaretIndex(hwndCtl: HWND): DWORD;
begin
  Result := DWORD(SendMessage(hwndCtl, EM_GETCARETINDEX, 0, 0));
end;

function Edit_GetZoom(hwndCtl: HWND; numerator: WPARAM; denominator: LPARAM): BOOL;
begin
  Result := BOOL(SendMessage(hwndCtl, EM_GETZOOM, numerator, denominator));
end;

function Edit_SetZoom(hwndCtl: HWND; numerator: WPARAM; denominator: LPARAM): BOOL;
begin
  Result := BOOL(SendMessage(hwndCtl, EM_SETZOOM, numerator, denominator))
end;

function Edit_GetFileLineFromChar(hwndCtl: HWND; characterIndex: WPARAM): DWORD;
begin
  Result := DWORD(SendMessage(hwndCtl, EM_FILELINEFROMCHAR, characterIndex, 0))
end;

function Edit_GetFileLineIndex(hwndCtl: HWND; lineNumber: WPARAM): DWORD;
begin
  Result := DWORD(SendMessage(hwndCtl, EM_FILELINEINDEX, lineNumber, 0));
end;

function Edit_GetFileLineLength(hwndCtl: HWND; characterIndex: WPARAM): DWORD;
begin
  Result := DWORD(SendMessage(hwndCtl, EM_FILELINELENGTH, characterIndex, 0));
end;

function Edit_GetFileLine(hwndCtl: HWND; lineNumber: WPARAM; textBuffer: LPARAM): DWORD;
begin
  Result := DWORD(SendMessage(hwndCtl, EM_GETFILELINE, lineNumber, textBuffer));
end;

function Edit_GetFileLineCount(hwndCtl: HWND): DWORD;
begin
  Result := DWORD(SendMessage(hwndCtl, EM_GETFILELINECOUNT, 0, 0));
end;
...
begin
  if Assigned(_TaskDialog) then
    Result := _TaskDialog(hwndOwner, hInstance, pszWindowTitle, pszMainInstruction,
