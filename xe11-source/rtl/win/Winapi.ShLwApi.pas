...
function StrCmpLogicalW(psz1, psz2: LPCWSTR): Integer; stdcall;
...
const
  ...
  ASSOCSTR_QUICKTIP                     = 12; //  list of property names in the form of an IPropertyDescriptionList
  {$EXTERNALSYM ASSOCSTR_QUICKTIP}
  ASSOCSTR_TILEINFO                     = 13; //  list of properties to be displayed for a particular file type in a Windows Explorer window that is in tile view.
  {$EXTERNALSYM ASSOCSTR_TILEINFO}
  ASSOCSTR_CONTENTTYPE                  = 14; //  Describes a general type of MIME file association, such as image and bmp, so that applications can make general assumptions about a specific file type.
  ...
  ASSOCSTR_SHELLEXTENSION               = 16; //  Retrieves the CLSID of that Shell extension object by passing a string representation of the IID of the interface you want to retrieve.
  {$EXTERNALSYM ASSOCSTR_SHELLEXTENSION}
  ASSOCSTR_DROPTARGET                   = 17; //  You can use this flag to retrieve the IDropTarget object's CLSID. This CLSID is registered in the DropTarget subkey.
  {$EXTERNALSYM ASSOCSTR_DROPTARGET}
  ASSOCSTR_DELEGATEEXECUTE              = 18; //  You can use this flag to retrieve the IExecuteCommand object's CLSID. This CLSID is registered in the verb's command subkey as the DelegateExecute entry.
  {$EXTERNALSYM ASSOCSTR_DELEGATEEXECUTE}
  ASSOCSTR_SUPPORTED_URI_PROTOCOLS      = 19; //
  {$EXTERNALSYM ASSOCSTR_SUPPORTED_URI_PROTOCOLS}
  ASSOCSTR_MAX                          = 20; //  last item in enum...
