...
uses
  Winapi.Windows, Winapi.ActiveX, Winapi.CommCtrl, Winapi.ShellAPI, Winapi.RegStr,
  Winapi.Messages, Winapi.WinInet, Winapi.UrlMon, Winapi.ObjectArray,
  Winapi.PropSys, Winapi.StructuredQueryCondition, Winapi.MSXMLIntf;
...
{$HPPEMIT 'DECLARE_DINTERFACE_TYPE_UUID("3D8B0590-F691-11d2-8EA9-006097DF5BD4", IDataObjectAsyncCapability);'}
{$HPPEMIT ''}
...
{$ALIGN ON}
  ...
  PPIDLIST_ABSOLUTE = ^PIDLIST_ABSOLUTE;
...
function SHCreateShellItemArray(pidlParent: PItemIDList; const psf: IShellFolder;
  cidl: UINT; ppidl: PCUIDLIST_CHILD_ARRAY;
...
function SHCreateShellItemArrayFromIDLists(cidl: UINT;
  rgpidl: PCUIDLIST_ABSOLUTE_ARRAY;
  out ppsiItemArray: IShellItemArray): HResult; stdcall;
...
function SHAddDefaultPropertiesByExt(pszExt: LPCWSTR;
  const pPropStore: Winapi.PropSys.IPropertyStore): HResult; stdcall;
...
{ interface IFileSaveDialog }
  IFileSaveDialog = interface(IFileDialog)
    ...
    function SetProperties(const pStore: Winapi.PropSys.IPropertyStore): HResult; stdcall;
    function SetCollectedProperties(const pList: Winapi.PropSys.IPropertyDescriptionList;
      fAppendDefault: BOOL): HResult; stdcall;
    function GetProperties(out ppStore: Winapi.PropSys.IPropertyStore): HResult; stdcall;
    function ApplyProperties(const psi: IShellItem;
      const pStore: Winapi.PropSys.IPropertyStore; hwnd: HWND;
...
{ interface IInitializeWithPropertyStore }
  IInitializeWithPropertyStore = interface(IUnknown)
    ...
    function Initialize(const pps: Winapi.PropSys.IPropertyStore): HRESULT; stdcall;
  end;
...
function SHCreateLibrary(const riid: TIID; out ppv): HResult;  inline;
{$EXTERNALSYM SHCreateLibrary}
function SHLoadLibraryFromItem(const psiLibrary: IShellItem; grfMode: DWORD;
  const riid: TIID; out ppv): HResult; inline;
...
type
  IProgressDialog = interface(IUnknown)
    [SID_IProgressDialog]
    function StartProgressDialog(hwndParent: HWND; const punkEnableModless: IUnknown; dwFlags: DWORD; pvResevered: Pointer): HRESULT; stdcall;
...
type
  ...
  SHARDAPPIDINFOIDLIST = record
    pidl: PItemIDList;      // The idlist for the shell item that should be added to the recent docs folder.
    pszAppID: LPCWSTR;      // The id of the application that should be associated with this recent doc.
  end;
...
function SHChangeNotification_Lock(hChange: THandle; dwProcId: DWORD;
  out pppidl: PPIDLIST_ABSOLUTE; out plEvent: LONG): THANDLE; stdcall;
...
//
// NOTE: 'IAsyncOperation' has been renamed 'IDataObjectAsyncCapability'.
//        https://msdn.microsoft.com/en-us/library/windows/desktop/bb776309(v=vs.85).aspx
  ...
  {$EXTERNALSYM IDataObjectAsyncCapability}

  // For backward compatibility
  IAsyncOperation = IDataObjectAsyncCapability;
  {$EXTERNALSYM IAsyncOperation}
  {$HPPEMIT 'typedef _di_IDataObjectAsyncCapability _di_IAsyncOperation;'}
