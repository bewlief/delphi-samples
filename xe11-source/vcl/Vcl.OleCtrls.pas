...
uses System.Variants, System.Types, Winapi.Windows, Winapi.Messages, Winapi.ActiveX, System.SysUtils, System.Classes,
  Vcl.Controls, Vcl.Forms, Vcl.Menus, Vcl.Graphics, System.Win.ComObj, Vcl.AxCtrls;
...
type
  ...
  TOleEnum = Winapi.ActiveX.TOleEnum;
  {$NODEFINE TOleEnum}
...
{$ENDIF}
  protected
    ...
    function CreateInstance: IOleObject; virtual;
    ...
    property OleInPlaceActiveObject: IOleInPlaceActiveObject read FOleInPlaceActiveObject;
    { IPropertyNotifySink }
...
uses Vcl.OleConst, System.RTLConsts;
...
function TOleControl.CreateInstance: IOleObject;
var
...
begin
  ...
  begin
    ...
    LicenseCheck(ClassFactory2.CreateInstanceLic(nil, nil, IOleObject,
      LicKeyStr, Result), SInvalidLicense);
  end else
...
begin
  if FMiscStatus and OLEMISC_INVISIBLEATRUNTIME = 0 then
  begin
    SetWindowLong(WindowHandle, GWL_WNDPROC, NativeInt(DefWndProc));
    if FOleObject <> nil then FOleObject.Close(OLECLOSE_NOSAVE);
    WindowHandle := 0;
  end else
...
{$IFDEF PUREPASCAL}
procedure Exchange(var A, B: ITypeInfo); register;
var
  L: Pointer;
begin
  L := Pointer(A);
...
begin
  ...
  begin
    ...
    SetWindowLong(WindowHandle, GWL_WNDPROC, NativeInt(@InitWndProc));
...
begin
  ...
  if NativeInt(EventMethod.Code) < $10000 then Exit;
    ...
    try
      asm
        ...
        @@3:    CMP     AX, varInt64
                ...
                CMP     AX, varUInt64
                JE      @@4
                CMP     AX, varDate                    // 8 bytes to push
                JA      @@2
...
begin
  CreateControl;
  dispIDs[0] := TDispInfo(DispInfo).DispID;
  DispatchInvoke(FControlDispatch, @TDispInfo(DispInfo).CallDesc, @dispIDs, nil, @res);
end;
{$ELSE !PUREPASCAL}
{$IFDEF CPUX86}
asm
...
const
  ...
  //DispInfo: array[0..7] of Byte = ($64,$00,$00,$00,$00,$01,$00,$00); -- GoBack dispid 100
begin
...
function TOleControl.PreMessageFilter(wnd: HWnd; msg: UInt; wp: WPARAM; lp: LPARAM;
  out res: LRESULT; out Cookie: DWORD): HResult;
begin
  Result := S_OK;
end;

function TOleControl.PostMessageFilter(wnd: HWnd; msg: UInt; wp: WPARAM; lp: LPARAM;
...
initialization
  GroupDescendentsWith(TOleControl, Vcl.Controls.TControl);
