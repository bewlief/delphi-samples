...
uses Winapi.Windows, Winapi.Messages, System.Classes, Vcl.Forms, Vcl.Graphics, Vcl.Menus, Vcl.Grids,
  Vcl.Controls, Vcl.StdCtrls, System.SysUtils;
...
{$IF DEFINED(CLR)}
    procedure WriteNode(Buffer: TBytes; Stream: TStream); overload; deprecated;
{$ELSE}
    procedure WriteNode(Buffer: PChar; Stream: TStream); overload; deprecated;
{$ENDIF}
    procedure WriteNode(Stream: TStream; Encoding: TEncoding); overload;
...
//    FMaskColor: TColor;
    ...
    function AttachNode(Index: LongInt; Str: string;
      Ptr: TCustomData; AttachMode: TAttachMode): LongInt;
...
{$ENDIF}
  System.Types, System.UITypes, Vcl.Consts, Vcl.Themes;

{$IF DEFINED(CLR)}
{$R Borland.Vcl.Outline.resources}
{$ELSE}
{$R Outline.res}
{$ENDIF}
...
const
  ...
  BitmapHeight = 14;
{$IF DEFINED(CLR)}
  ResourceBaseName = 'Borland.Vcl.Outline';
{$ENDIF}
...
{$IF DEFINED(CLR)}
// Provided for backwards compatibility with obsolete/deprecated WriteNode method
function GetStrAndLevel(Buffer: string; var Level: Cardinal): string; overload;
begin
  Level := 0;
  while (Level < Cardinal(Length(Buffer))) and (CharInSet(Buffer[Level + 1], [' ', #9])) do
    Inc(Level);
  Result := Copy(Buffer, Level + 1, Cardinal(Length(Buffer)) - Level);
end;
{$ENDIF}

{$IF DEFINED(CLR)}
// Provided for backwards compatibility with obsolete/deprecated WriteNode method
function GetBufStart(Buffer: TBytes; Offset: Integer; var Level: Cardinal): Integer; overload;
begin
  Level := 0;
  while (Buffer[Offset] = Ord(' ')) or (Buffer[Offset] = $09) do
  begin
    Inc(Offset);
    Inc(Level);
  end;
...
{$IF DEFINED(CLR)}
// Provided for backwards compatibility with obsolete/deprecated WriteNode method
function PutString(Buffer: TBytes; BufPtr: Integer; const S: string): Integer;
var
  I: Integer;
  B: TBytes;
begin
...
begin
  ...
  begin
    Node := TOutlineNode(FList.Items[I]);
    Node.FParent := nil;
    Node.Destroy;
  end;
  FList.Free;
...
begin
  Result := TOutlineNode(List[Index]);
end;
...
begin
  Level := Cardinal(Integer(Level) + Integer(Value));
end;
...
function TOutlineNode.GetDataItem(Value: TCustomData): LongInt;
var
  I: Integer;
begin
  if Value = Data then
...
{$IF DEFINED(CLR)}
// No longer used when streaming TOutline.
// Provided for backwards compatibility only
procedure TOutlineNode.WriteNode(Buffer: TBytes; Stream: TStream);
var
  BufPtr: Integer;
  NodeLevel: Word;
  I: Integer;
begin
  ...
  begin
    ...
    begin
      Buffer[BufPtr] := Ord(Tab);
      Dec(NodeLevel);
      Inc(BufPtr);
    end;
    BufPtr := PutString(Buffer, BufPtr, Text);
    Stream.WriteBuffer(Buffer, BufPtr);
  end;
  for I := 0 to List.Count - 1 do
...
procedure TOutlineNode.WriteNode(Stream: TStream; Encoding: TEncoding);
var
  I: Integer;
  LText: string;
  LBytes: TBytes;
begin
  if Parent <> nil then
  begin
    LText := StringOfChar(#9, Level - 1) + Text;
    LBytes := Encoding.GetBytes(LText);
    Stream.WriteBuffer(LBytes{$IFNDEF CLR}[0]{$ENDIF}, Length(LBytes));
    LBytes := Encoding.GetBytes(SLineBreak);
    Stream.WriteBuffer(LBytes{$IFNDEF CLR}[0]{$ENDIF}, Length(LBytes));
  end;
  for I := 0 to List.Count - 1 do
    Items[I].WriteNode(Stream, Encoding);
end;
...
begin
  ...
  try
    ...
    try
      StringList.SaveToStream(MemStream, TEncoding.UTF8);
      MemStream.Position := 0;
      Outline.LoadFromStream(MemStream, TEncoding.UTF8);
    finally
      MemStream.Free;
    end;
  finally
...
begin
  ...
  MemStream := TMemoryStream.Create;
  try
    Outline.SaveToStream(MemStream, TEncoding.UTF8);
    MemStream.Position := 0;
    StringList := TStringList.Create;
    try
      StringList.LoadFromStream(MemStream, TEncoding.UTF8);
...
const
{$IF DEFINED(CLR)}
  Images: array[TBitmapArrayRange] of string = ('PLUS', 'MINUS', 'OPEN', 'CLOSED', 'LEAF');
{$ELSE}
...
{$IF DEFINED(CLR)}
    FPictures[I].LoadFromResourceName(Images[I],
...
procedure TCustomOutline.InvalidateCell(ACol, ARow: Longint);
begin
  inherited;
end;
...
var
  B: TBytes;
begin
  SetLength(B, SizeOf(FOldBitmaps));
  Stream.ReadBuffer(B, SizeOf(FOldBitmaps));
  FOldBitmaps := TOutlineBitmaps(B[0]);
{$ELSE}
begin
...
var
  B: TBytes;
begin
  SetLength(B, SizeOf(FUserBitmaps));
  B[0] := Byte(FUserBitmaps);
  Stream.WriteBuffer(B, SizeOf(FUserBitmaps));
{$ELSE}
begin
...
function TCustomOutline.InsertObject(Index: LongInt; const Text: string; const Data: TCustomData): LongInt;
begin
  Result := -1;
...
function TCustomOutline.AddObject(Index: LongInt; const Text: string; const Data: TCustomData): LongInt;
begin
  Result := -1;
...
function TCustomOutline.AddChildObject(Index: LongInt; const Text: string; const Data: TCustomData): LongInt;
begin
  Result := -1;
...
function TCustomOutline.GetDataItem(Value: TCustomData): Longint;
begin
...
begin
  ...
  begin
    ...
    begin
      if TStyleManager.IsCustomStyleActive and (seClient in StyleElements) then
        Brush.Color := StyleServices.GetStyleColor(scGrid)
      else
        Brush.Color := Color;
      FillRect(ARect);
    end;
  ...
  begin
    ...
    begin
      ...
      if seFont in StyleElements then
        Font.Color := StyleServices.GetStyleFontColor(sfGridItemNormal);
    end
    else
      Brush.Color := Color;
    FillRect(ARect);
    ...
    begin
      ...
      begin
        ...
        Font.Color := StyleServices.GetStyleFontColor(sfGridItemSelected);
      end
      else
      begin
  ...
  begin
    ...
    if OutlineStyle = osPlusMinusPictureText then
      Inc(ARect.Left, DefaultRowHeight);
    if (GetFocus = Self.Handle) and (ooDrawFocusRect in Options) and
...
begin
  ...
  try
    ...
    begin
      if TStyleManager.IsCustomStyleActive and (seClient in StyleElements) then
        Pen.Color := StyleServices.GetStyleFontColor(sfEditBoxTextDisabled)
      else
        Pen.Color := clBlack;
      Pen.Width := 1;
      try
        ...
        begin
          ...
          LineTo(ARect.Left + Offset, Height);
          MoveTo(ARect.Left + Offset, ARect.Top + Offset);
          LineTo(ARect.Left + Offset + FFontSize div 2, ARect.Top + Offset);
        end;
      finally
        Pen.Assign(OldPen);
      end;
...
begin
  Winapi.Windows.DrawText(Canvas.Handle, Node.Text, Length(Node.Text),
    Rect, DT_LEFT or DT_SINGLELINE or DT_VCENTER or DT_NOPREFIX);
end;
...
var
  Bitmap: {$IFNDEF CLR}^{$ENDIF}TBitmap;
begin
  Bitmap := {$IFNDEF CLR}@{$ENDIF}FPictures[Ord(Kind)];
  Include(FUserBitmaps, Kind);
  if Value = nil then ClearBitmap(Bitmap{$IFNDEF CLR}^{$ENDIF}, Kind)
...
procedure TCustomOutline.LoadFromFile(const FileName: string; Encoding: TEncoding);
var
  Stream: TStream;
begin
...
{$IF DEFINED(CLR)}
function LineStart(Buffer: TBytes; BufPos: Integer): Integer;
begin
  if BufPos > 2 then
  begin
    Dec(BufPos, 2);
    while (Buffer[BufPos] <> $0D) and (BufPos > 0) do
      Dec(BufPos);
    if BufPos > 0 then
    begin
      Inc(BufPos);
      if (Buffer[BufPos] = $0A) then Inc(BufPos);
    end;
    Result := BufPos;
  end
  else Result := 0;
...
{$IF DEFINED(CLR)}
// Provided for backwards compatibility with obsolete/deprecated WriteNode method
function GetString(Buffer: TBytes; BufPtr: Integer; var S: string): Integer;
var
  Start: Integer;
begin
  Start := BufPtr;
  while (BufPtr < Length(Buffer)) and (Buffer[BufPtr] <> $0D) and (Buffer[BufPtr] <> $1A) do
    Inc(BufPtr);
  S := System.Text.Encoding.Default.GetString(Buffer, Start, BufPtr - Start);
  if Buffer[BufPtr] = $0D then
    Inc(BufPtr);
  if Buffer[BufPtr] = $0A then
    Inc(BufPtr);
  Result := BufPtr;
end;
{$ELSE}
...
begin
  Start := BufPtr;
  while not CharInSet(BufPtr^, [#13, #26]) do Inc(BufPtr);
  SetString(S, Start, BufPtr - Start);
...
procedure TCustomOutline.LoadFromStream(Stream: TStream; Encoding: TEncoding);
var
  S: string;
  I, Len, Level, OldLevel: Integer;
  LStrings: TStrings;
...
begin
  ...
  try
    ...
    LStrings.LoadFromStream(Stream, Encoding);
    for S in LStrings do
    begin
      Level := 0;
      Len := Length(S);
      while (Level + 1 <= Len) and CharInSet(S[Level + 1], [' ', #9]) do
        Inc(Level);

      NewNode := TOutlineNode.Create(Self);
      try
        NewNode.Text := Copy(S, Level + 1, Len);
        if (Level > OldLevel) or (ParentNode = FRootNode) then
        begin
          if Level - OldLevel > 1 then
            Error(SOutlineFileLoad);
        end
        else
        begin
...
begin
  ...
  begin
    ...
    begin
      ...
      FSelectedItem := TOutlineNode(List.First);
    end;
...
procedure TCustomOutline.SaveToFile(const FileName: string; Encoding: TEncoding);
var
  Stream: TStream;
begin
...
procedure TCustomOutline.SaveToStream(Stream: TStream; Encoding: TEncoding);
var
  Preamble: TBytes;
begin
  ...
  FRootNode.WriteNode(Stream, Encoding);
end;
