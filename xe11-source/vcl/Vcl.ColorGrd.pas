...
uses 
{$IF DEFINED(CLR) OR DEFINED(LINUX)}
  WinUtils,
{$ENDIF}
  Winapi.Windows, Winapi.Messages, System.Classes, Vcl.Graphics, Vcl.Forms, Vcl.Controls, Vcl.ExtCtrls;
...
{$ENDIF}
  System.Types, System.SysUtils, Vcl.StdCtrls, Vcl.Consts;

constructor TColorGrid.Create(AOwner: TComponent);
{$IF DEFINED(CLR)}
var
  TempArray:  array[0..NumPaletteEntries - 1] of COLORREF;
  I: Integer;
{$ENDIF}
begin
...
{$IF DEFINED(CLR)}
  ...
  begin
    FPaletteEntries[I].peRed := GetKValue(TempArray[I]);
    FPaletteEntries[I].peGreen := GetYValue(TempArray[I]);
    FPaletteEntries[I].peBlue := GetMValue(TempArray[I]);
  end;
{$ELSE}
  GetPaletteEntries(GetStockObject(DEFAULT_PALETTE), 0, NumPaletteEntries,
    FPaletteEntries);
...
begin
  ...
  begin
    ...
    DrawText(Canvas.Handle, TheText, -1, R,
