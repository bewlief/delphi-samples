...
function CreateMessageDialog(const Msg: string; DlgType: TMsgDlgType;
  Buttons: TMsgDlgButtons; DefaultButton: TMsgDlgBtn; CustomButtonCaptions: array of string): TForm; overload;
function CreateMessageDialog(const Msg: string; DlgType: TMsgDlgType;
...
function MessageDlgPos(const Msg: string; DlgType: TMsgDlgType;
  ...
  TaskDlgFlags: TTaskDialogFlags): Integer; overload; inline;
function MessageDlgPos(const Msg: string; DlgType: TMsgDlgType;
  Buttons: TMsgDlgButtons; HelpCtx: Longint; X, Y: Integer;
  DefaultButton: TMsgDlgBtn): Integer; overload; inline;
function MessageDlgPos(const Msg: string; DlgType: TMsgDlgType;
  Buttons: TMsgDlgButtons; HelpCtx: Longint; X, Y: Integer;
  DefaultButton: TMsgDlgBtn; TaskDlgFlags: TTaskDialogFlags): Integer; overload; inline;
function MessageDlgPos(const Msg: string; DlgType: TMsgDlgType;
  Buttons: TMsgDlgButtons; HelpCtx: Longint; X, Y: Integer;
  DefaultButton: TMsgDlgBtn; TaskDlgFlags: TTaskDialogFlags; CustomButtonCaptions: array of string): Integer; overload;   
...
function MessageDlgPosHelp(const Msg: string; DlgType: TMsgDlgType;
  Buttons: TMsgDlgButtons; HelpCtx: Longint; X, Y: Integer;
  const HelpFileName: string; TaskDlgFlags: TTaskDialogFlags): Integer; overload;
function MessageDlgPosHelp(const Msg: string; DlgType: TMsgDlgType;
  Buttons: TMsgDlgButtons; HelpCtx: Longint; X, Y: Integer;
  const HelpFileName: string; DefaultButton: TMsgDlgBtn): Integer; overload;
function MessageDlgPosHelp(const Msg: string; DlgType: TMsgDlgType;
  Buttons: TMsgDlgButtons; HelpCtx: Longint; X, Y: Integer;
  const HelpFileName: string; DefaultButton: TMsgDlgBtn; TaskDlgFlags: TTaskDialogFlags): Integer; overload;
function MessageDlgPosHelp(const Msg: string; DlgType: TMsgDlgType;
  Buttons: TMsgDlgButtons; HelpCtx: Longint; X, Y: Integer;
...
function TaskMessageDlgPosHelp(const Title, Msg: string; DlgType: TMsgDlgType;
  Buttons: TMsgDlgButtons; HelpCtx: Longint; X, Y: Integer;
  const HelpFileName: string; DefaultButton: TMsgDlgBtn; CustomButtonCaptions: array of string): Integer; overload;

procedure ShowMessage(const Msg: string); overload; inline;
procedure ShowMessage(const Msg: string; TaskDlgFlags: TTaskDialogFlags); overload; inline;
procedure ShowMessageFmt(const Msg: string; Params: array of const); overload;
procedure ShowMessageFmt(const Msg: string; Params: array of const; TaskDlgFlags: TTaskDialogFlags); overload;
procedure ShowMessagePos(const Msg: string; X, Y: Integer); overload; inline;
procedure ShowMessagePos(const Msg: string; X, Y: Integer; TaskDlgFlags: TTaskDialogFlags); overload; inline;
...
begin
  if FCallOnClose then
    FCommonDialog.DoClose;
  Free;
end;
...
begin
  if (Message.Msg = WM_STYLEDLGDESTROY) and not (Self is TFindDialog) then
  begin
    DoClose;
    Exit;
  end;
...
begin
  inherited;
  if not (Self is TFindDialog) then
    DoClose;
end;
...
{$ENDIF}
          ...
          begin
            Message.Result := 1;
            SetWindowLong(Handle, DWLP_MSGRESULT, Message.Result);
            Exit;
          end;
        end;
      CDN_INITDONE: DoShow;
...
{$IFDEF CLR}[UIPermission(SecurityAction.LinkDemand, Window=UIPermissionWindow.SafeSubWindows)]{$ENDIF}
{$IFDEF CLR}[PrintingPermission(SecurityAction.LinkDemand, Level=PrintingPermissionLevel.SafePrinting)]{$ENDIF}
...
procedure TFileTypeItem.Assign(Source: TPersistent);
begin
  if Source is TFileTypeItem then
  begin
    DisplayName := TFileTypeItem(Source).DisplayName;
    FileMask := TFileTypeItem(Source).FileMask;
  end
  else
...
procedure TFavoriteLinkItem.Assign(Source: TPersistent);
begin
  if Source is TFavoriteLinkItem then
    Location := TFavoriteLinkItem(Source).Location
  else
    inherited Assign(Source);
end;
...
procedure TTaskDialogBaseButtonItem.Assign(Source: TPersistent);
begin
  if Source is TTaskDialogBaseButtonItem then
  begin
    Caption := TTaskDialogBaseButtonItem(Source).Caption;
    Default := TTaskDialogBaseButtonItem(Source).Default;
    Enabled := TTaskDialogBaseButtonItem(Source).Enabled;
    ModalResult := TTaskDialogBaseButtonItem(Source).ModalResult;
  end
  else
    inherited Assign(Source);
end;
...
procedure TTaskDialogButtonItem.Assign(Source: TPersistent);
begin
  if Source is TTaskDialogButtonItem then
  begin
    CommandLinkHint := TTaskDialogButtonItem(Source).CommandLinkHint;
    ElevationRequired := TTaskDialogButtonItem(Source).ElevationRequired;
  end;
  inherited Assign(Source);
end;
...
begin
  ...
  ScaleForPPI(GetPPIForActiveForm);
end;
...
var
  ...
  IconSize: Integer;
  MsgB, B, CancelButton: TMsgDlgBtn;
{$IF DEFINED(CLR)}
  IconID: Integer;
{$ELSE}
  IconID: PChar;
    ...
    begin
      ...
      begin
        ...
        if LIndex <= High(CustomButtonCaptions) then
          S := CustomButtonCaptions[LIndex]
        else
          S := LoadResString(ButtonCaptions[B]);
...
{$ENDIF}
        ...
        if TextRect.Width + 8 > ButtonWidth then 
...
{$ENDIF}
    ...
    LIndex := -1;
    for MsgB := Low(TMsgDlgBtn) to High(TMsgDlgBtn) do
    begin
      if Result.BiDiMode = bdRightToLeft then
        B := TMsgDlgBtn(Ord(High(TMsgDlgBtn)) - Ord(MsgB))
      else
        B := MsgB;
      if B in Buttons then
      begin
        ...
        begin
          ...
          Inc(LIndex);
          if LIndex <= High(CustomButtonCaptions) then
            Caption := CustomButtonCaptions[LIndex]
          else
{$IF DEFINED(CLR)}
...
function MessageDlg(const Msg: string; DlgType: TMsgDlgType;
  Buttons: TMsgDlgButtons; HelpCtx: Longint; DefaultButton: TMsgDlgBtn; CustomButtonCaptions: array of string): Integer; overload; 
begin
  Result := MessageDlgPosHelp(Msg, DlgType, Buttons, HelpCtx, -1, -1, '', DefaultButton, [], CustomButtonCaptions);
end;
...
begin
  Result := TaskMessageDlgPosHelp(Title, Msg, DlgType, Buttons, HelpCtx,
    -1, -1, '', DefaultButton, CustomButtonCaptions);
end;
...
function MessageDlgPos(const Msg: string; DlgType: TMsgDlgType;
  Buttons: TMsgDlgButtons; HelpCtx: Longint; X, Y: Integer;
  TaskDlgFlags: TTaskDialogFlags): Integer;
begin
  Result := MessageDlgPosHelp(Msg, DlgType, Buttons, HelpCtx, X, Y, '', TaskDlgFlags);
end;
...
function MessageDlgPos(const Msg: string; DlgType: TMsgDlgType;
  Buttons: TMsgDlgButtons; HelpCtx: Longint; X, Y: Integer;
  DefaultButton: TMsgDlgBtn; TaskDlgFlags: TTaskDialogFlags): Integer; overload;
begin
  Result := MessageDlgPosHelp(Msg, DlgType, Buttons, HelpCtx, X, Y, '', DefaultButton, TaskDlgFlags);
end;
...
function MessageDlgPos(const Msg: string; DlgType: TMsgDlgType;
  ...
  DefaultButton: TMsgDlgBtn; TaskDlgFlags: TTaskDialogFlags; CustomButtonCaptions: array of string): Integer; overload;
begin
  Result := MessageDlgPosHelp(Msg, DlgType, Buttons, HelpCtx, X, Y, '', DefaultButton, TaskDlgFlags, CustomButtonCaptions);
end;
...
{ Helper function to get the default button }

function GetDefaultButton(const Buttons: TMsgDlgButtons): TMsgDlgBtn;
begin
  if mbOk in Buttons then Result := mbOk else
    if mbYes in Buttons then Result := mbYes else
      Result := mbRetry;
end;
...
function InternalDoTaskMessageDlgPosHelp(const Instruction, Msg: string; DlgType: TMsgDlgType;
  ...
  AlternateTaskDlgFlags: TTaskDialogFlags; CustomButtonCaptions: array of string): Integer; forward;
...
function DoTaskMessageDlgPosHelp(const Instruction, Msg: string; DlgType: TMsgDlgType;
  Buttons: TMsgDlgButtons; HelpCtx: Longint; X, Y: Integer;
  const HelpFileName: string; AlternateTaskDlgFlags: TTaskDialogFlags): Integer; overload; inline;
begin
  Result := InternalDoTaskMessageDlgPosHelp(Instruction, Msg, DlgType,
    Buttons, HelpCtx, X, Y, HelpFileName, GetDefaultButton(Buttons), True, AlternateTaskDlgFlags, []);
end;

function DoTaskMessageDlgPosHelp(const Instruction, Msg: string; DlgType: TMsgDlgType;
  Buttons: TMsgDlgButtons; HelpCtx: Longint; X, Y: Integer;
  const HelpFileName: string; DefaultButton: TMsgDlgBtn): Integer; overload; inline;
begin
  Result := InternalDoTaskMessageDlgPosHelp(Instruction, Msg, DlgType,
...
function DoTaskMessageDlgPosHelp(const Instruction, Msg: string; DlgType: TMsgDlgType;
  Buttons: TMsgDlgButtons; HelpCtx: Longint; X, Y: Integer;
  const HelpFileName: string;
  DefaultButton: TMsgDlgBtn; AlternateTaskDlgFlags: TTaskDialogFlags): Integer; overload; inline;
begin
  Result := InternalDoTaskMessageDlgPosHelp(Instruction, Msg, DlgType,
    Buttons, HelpCtx, X, Y, HelpFileName, DefaultButton, True, AlternateTaskDlgFlags, []);
end;

function DoTaskMessageDlgPosHelp(const Instruction, Msg: string; DlgType: TMsgDlgType;
  Buttons: TMsgDlgButtons; HelpCtx: Longint; X, Y: Integer;
  const HelpFileName: string;
  DefaultButton: TMsgDlgBtn; AlternateTaskDlgFlags: TTaskDialogFlags; CustomButtonCaptions: array of string): Integer; overload;
begin
  Result := InternalDoTaskMessageDlgPosHelp(Instruction, Msg, DlgType,
    Buttons, HelpCtx, X, Y, HelpFileName, DefaultButton, False, AlternateTaskDlgFlags, CustomButtonCaptions);
end;
...
begin
  if TOSVersion.Check(6) and UseLatestCommonDialogs and
     IsNewCommonCtrl and not ((shDialogs in TStyleManager.SystemHooks) and not StyleServices(Screen.ActiveCustomForm).IsSystemStyle) then
    Result := DoTaskMessageDlgPosHelp('', Msg, DlgType, Buttons,
      HelpCtx, X, Y, HelpFileName, DefaultButton, TaskDlgFlags)
  else
    Result := DoMessageDlgPosHelp(CreateMessageDialog(Msg, DlgType, Buttons, DefaultButton),
      HelpCtx, X, Y, HelpFileName);
end;
...
begin
  if TOSVersion.Check(6) and UseLatestCommonDialogs and
     IsNewCommonCtrl and not ((shDialogs in TStyleManager.SystemHooks) and not StyleServices(Screen.ActiveCustomForm).IsSystemStyle) then
    Result := DoTaskMessageDlgPosHelp('', Msg, DlgType, Buttons,
      HelpCtx, X, Y, HelpFileName, DefaultButton, TaskDlgFlags, CustomButtonCaptions)
  else
    Result := DoMessageDlgPosHelp(CreateMessageDialog(Msg, DlgType, Buttons, DefaultButton, CustomButtonCaptions),
      HelpCtx, X, Y, HelpFileName);
end;
...
begin
  if TOSVersion.Check(6) and IsNewCommonCtrl then
    Result := DoTaskMessageDlgPosHelp(Title, Msg, DlgType, Buttons,
      HelpCtx, X, Y, HelpFileName, DefaultButton, [], CustomButtonCaptions)
  else
    Result := DoMessageDlgPosHelp(CreateMessageDialog(Msg, DlgType, Buttons, DefaultButton, CustomButtonCaptions),
      HelpCtx, X, Y, HelpFileName);
end;
...
begin
  ShowMessagePos(Msg, -1, -1, TaskDlgFlags);
end;
...
begin
  ShowMessage(Format(Msg, Params), TaskDlgFlags);
end;
...
begin
  MessageDlgPos(Msg, mtCustom, [mbOK], 0, X, Y, TaskDlgFlags);
end;
...
function InternalDoTaskMessageDlgPosHelp(const Instruction, Msg: string; DlgType: TMsgDlgType;
  Buttons: TMsgDlgButtons; HelpCtx: Longint; X, Y: Integer;
  const HelpFileName: string; DefaultButton: TMsgDlgBtn;
  UseAlternateTaskDlgFlags: Boolean; AlternateTaskDlgFlags: TTaskDialogFlags; CustomButtonCaptions: array of string): Integer;
const
  ...
  with Form do
    try
      ...
      begin
        ...
        Form.ClientHeight := Top + Height + ScaleFromDefaultPPI(13);
        Form.Height := Min(Form.Height, Form.ClientHeight + ScaleFromDefaultPPI(30));
      end;
