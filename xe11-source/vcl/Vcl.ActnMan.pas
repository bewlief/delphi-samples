...
{$ENDIF}
  ...
  Vcl.ActnList, Vcl.Graphics, Vcl.ToolWin, Vcl.ImgList, Vcl.Controls, Vcl.Themes, Vcl.Buttons, Vcl.StdCtrls,
  System.UITypes;
...
{$ELSE}
  System.Character,
{$ENDIF}
  Vcl.Dialogs, Winapi.CommCtrl, System.Math, System.Types,
  Vcl.GraphUtil, Vcl.ActnMenus, Vcl.Consts,
  Vcl.ActnCtrls, Vcl.ActnColorMaps, Winapi.UxTheme;


function ScaleRect(const Source: TRect; M, D: Integer): TRect;
begin
  Result := Source;
  Result.Left := MulDiv(Result.Left, M, D);
  Result.Top := MulDiv(Result.Top, M, D);
  Result.Right := MulDiv(Result.Right, M, D);
  Result.Bottom := MulDiv(Result.Bottom, M, D);
end;
...
begin
  ...
  begin
    ...
    try
      ...
      ImageName := TActionClientItem(Source).ImageName;
...
function TActionClientItem.GetImages: TCustomImageList;
begin
  Result := nil;
  if (ActionClients <> nil) and (ActionClients.ActionManager <> nil) then
    Result := ActionClients.ActionManager.Images;
end;

procedure TActionClientItem.UpdateImageName(Index: TImageIndex; var Name: TImageName);
var
...
procedure TActionClientItem.UpdateImageIndex(Name: TImageName; var Index: TImageIndex);
var
  LImages: TCustomImageList;
begin
  LImages := GetImages;
...
procedure TActionClientItem.CheckImageIndexAndName;
var
  LImages: TCustomImageList;
begin
  LImages := GetImages;
...
procedure TActionClientItem.SetImageName(const Value: TImageName);
begin
...
procedure TCustomActionBar.AdjustItems;
var
  I: Integer;
begin
  if ItemCount > 0 then
...
begin
  if not (csLoading in ComponentState) and (not FIsRecreating) then
  begin
    FIsRecreating := True;
    try
      AdjustSize;
...
procedure TCustomActionBar.SetBounds(ALeft, ATop, AWidth, AHeight: Integer);
var
  NewWidth: Integer;
begin
...
begin
  ...
  begin
    LShadowColor := LStyle.GetSystemColor(clBtnShadow);
    LHighlightColor := LStyle.GetSystemColor(clBtnHighLight);
  end
  else
  begin
...
function TCustomActionBar.IsCustomStyleActive: Boolean;
var
  LStyle: TCustomStyleServices;
begin
  LStyle := StyleServices;
...
function TCustomActionBar.StyleServices: TCustomStyleServices;
begin
  Result := Vcl.Themes.StyleServices(Self);
end;

procedure TCustomActionBar.SetTransparent(const Value: Boolean);
begin
...
begin
  ...
  OldDC := 0;
  if csGlassPaint in ControlState then
    PaintBuffer := BeginBufferedPaint(Canvas.Handle, PaintRect, BPBF_COMPOSITED, nil, MemDC);
  try
    if (PaintBuffer <> 0) and (csGlassPaint in ControlState) then
    begin
      OldDC := Canvas.Handle;
      Canvas.Handle := MemDC;
    end;
...
begin
  ...
  begin
    LImageList := nil;
    if (ActionClient.Action <> nil) and (ActionClient.Action.ActionList <> nil) and
      (ActionClient.Action.ActionList is TCustomActionList) then
        LImageList := TCustomActionList(ActionClient.Action.ActionList).Images;
    if (LImageList = nil) and (ActionClient.OwningCollection <> nil) then
      LImageList := ActionClient.OwningCollection.ActionManager.Images;
    if LImageList <> nil then
      Result := Point(LImageList.Width, LImageList.Height);
  end
  else
  begin
    Result := Point(32, 32);
    if ActionClient.OwningCollection.ActionManager.LargeImages <> nil then
...
function TCustomActionControl.IsCustomStyleActive: Boolean;
var
  LStyle: TCustomStyleServices;
begin
  LStyle := StyleServices;
...
function TCustomActionControl.StyleServices: TCustomStyleServices;
begin
  if FActionBar <> nil then
    Result := FActionBar.StyleServices
  else
...
procedure TCustomActionControl.ChangeScale(M, D: Integer; isDpiChange: Boolean);
begin
  inherited;
  Margins := ScaleRect(Margins, M, D);
  Spacing := MulDiv(Spacing, M, D);
end;
