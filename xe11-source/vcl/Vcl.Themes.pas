...
type
  ...
  const
    cDesignerStyleName: array[Boolean] of string = ('Windows Designer', 'Windows Designer Dark');
...
begin
  ...
  begin
    ...
    if TStyleManager.IsSystemStyleDefault(AControl) or (AControl.StyleElements = []) then
...
begin
  Result := TStyleManager.Style[cDesignerStyleName[ADark]];
end;
  ...
  case TThemedGrid(State) of
    tgCellSelected..tgCellRowSelectedRight:
      begin
        ...
        Result := Style.DrawElement(DC, Style.GetElementDetails(tmPopupItemHot), LRect, ClipRect, DPI);
      end;
