...
begin
  ...
  begin
    ...
    FImage.Reference;
    if TJPEGImage(Source).Bitmap <> nil then
    begin
      NewBitmap;
      FBitmap.Assign(TJPEGImage(Source).Bitmap);
    end;
  end
  ...
  begin
    ...
    try
      ...
      try
        TWICImage(Source).ImageFormat := wifJpeg;
...
begin
  if Dest is TBitmap then
    ...
    try
      SaveToStream(Stream);
      Stream.Position := 0;
      TWICImage(Dest).ImageFormat := wifJpeg;
...
procedure ProgressCallback(const cinfo: jpeg_common_struct);
  function getElapsedMs(current, last: DWORD): DWORD;
  begin
    if current >= last then
      Result := current - last
    else
      Result := MAXDWORD - last + current;
  end;
var
  Ticks: DWORD;
  R: TRect;
...
begin
  ...
  begin
    Ticks := GetTickCount;
    if getElapsedMs(Ticks, last_time) < 500 then Exit;
    if last_time = 0 then
    begin
      last_time := Ticks;
      Exit;
    ...
    if cinfo.is_decompressor then
      ...
      R := Rect(0,0,0,0);
    curr_pct := Byte(Trunc(100.0*(completed_passes + (pass_counter/pass_limit))/total_passes));
    if curr_pct = last_pct then Exit;
    last_pct := curr_pct;
    if cinfo.is_decompressor then
      last_scanline := j_decompress_ptr(@cinfo)^.output_scanline;
    instance.Progress(instance, psRunning, curr_pct, (R.Bottom - R.Top) >= 4, R, '');
  end;
end;
...
begin
  ...
  try
    try
      ...
      GetBitmap;   // Ensure that FData is loaded into FBitmap
...
function TJPEGImage.GetCanvas: TCanvas;
begin
...
class function TJPEGImage.CanLoadFromStream(Stream: TStream): Boolean;
var
  soi, marker: Word;
  P: Int64;
begin
