...
uses
  System.Variants, Winapi.Windows, Winapi.Messages, Winapi.ActiveX, {$IFDEF LINUX} WinUtils, {$ENDIF}
  System.Win.ComObj, System.Classes, Vcl.Graphics, Vcl.Controls, Vcl.Forms, Vcl.ExtCtrls, System.Win.StdVCL;
...
type
  ...
  TConnectionKind = System.Win.ComObj.TConnectionKind;
  ...
  private
    ...
    FOleLinkStub: TInterfacedObject; // Pointer to a TOleLinkStub instance
  ...
  protected
    ...
    function GetUserType(dwFormOfType: Longint; out pszUserType: POleStr): HResult;
      stdcall;
{$IFDEF WIN64}
    function SetExtent(dwDrawAspect: Longint; size: PPoint): HResult;
      stdcall;
{$ELSE}
...
{$ENDIF}
    ...
    function PreMessageFilter(wnd: HWnd; msg: UInt; wp: WPARAM; lp: LPARAM;
      out res: LRESULT; out Cookie: DWORD): HResult; stdcall;
...
{$IFDEF WIN64}
    function SetContentExtent(sizel: PPoint): HResult; stdcall;
{$ELSE}
...
{$ENDIF}
    ...
    procedure PreInitializeControl; virtual;
    ...
    function DoPropRequestEdit(DispID: TDispID): HResult; 
  public
    constructor Create;
  ...
  public
    ...
    destructor Destroy; override;
    function CreateControl(Parent: HWND): TWinControl; virtual;
...
uses 
  Vcl.Consts, System.SysUtils, System.Types, System.UITypes;
...
type
  ...
  IStdEvents = dispinterface
    ...
    procedure KeyUp(var KeyCode: Smallint;
      Shift: Smallint); dispid DISPID_KEYUP;
...
const
  ShiftMap: array[0..9] of Byte = (0, 1, 4, 5, 2, 3, 6, 7, 8, 9);
begin
...
begin
  if FControl is TWinControl then
    Winapi.Windows.SetFocus(TWinControl(FControl).Handle) else
    inherited;
end;
...
type
  ...
  private
    FController: Pointer; // Pointer to IUnknown
  public
...
constructor TActiveXControl.Create;
begin
  inherited;
end;

constructor TActiveXControl.CreateAggregated(const Controller: IInterface);
begin
  inherited;
end;

constructor TActiveXControl.CreateFromFactory(Factory: TComObjectFactory;
...
begin
  ...
  if FControlFactory.EventTypeInfo <> nil then
    FConnectionPoint := FConnectionPoints.CreateConnectionPoint(FControlFactory.EventIID,
                                                                ckSingle, EventConnect)
  else
    FConnectionPoint := nil;
  FPropertySinks := FConnectionPoints.CreateConnectionPoint(IPropertyNotifySink,
    ckMulti, nil);
  FControl := FControlFactory.CreateControl(ParkingWindow);
...
begin
  ...
  if FOleLinkStub <> nil then
  begin
    TOleLinkStub(FOleLinkStub).FController := nil;
    TOleLinkStub(FOleLinkStub)._Release;
  end;
  inherited Destroy;
end;
...
begin
  ...
  if PropBag.Read(PWideChar(PropName), PropValue, ErrorLog) = S_OK then
    System.Win.ComObj.SetDispatchPropValue(Self as IDispatch, DispatchID, PropValue);
end;

procedure TActiveXControl.PutPropInBag(const PropName: WideString;
  DispatchID: Integer; PropBag: IPropertyBag);
begin
  PropBag.Write(PWideChar(PropName), System.Win.ComObj.GetDispatchPropValue(Self as IDispatch,
    DispatchID));
end;
...
procedure TActiveXControl.PreInitializeControl;
begin
end;
  ...
  begin
    ...
    if Result <> 0 then
      ...
      begin
        ...
        if FOleLinkStub = nil then
        begin
          FOleLinkStub := TOleLinkStub.Create(Self);
          TOleLinkStub(FOleLinkStub)._AddRef;
        end;

        IOleLink(Obj) := FOleLinkStub as IOleLink;
      end;
  end;
end;
...
begin
  Result := DoPropRequestEdit(DispID) = S_OK;
end;

function TActiveXControl.DoPropRequestEdit(DispID: TDispID): HResult;
var
  Enum: IEnumConnections;
  ConnectData: TConnectData;
  Fetched: Longint;
begin
  ...
  while Enum.Next(1, ConnectData, @Fetched) = S_OK do
  begin
    Result := (ConnectData.pUnk as IPropertyNotifySink).OnRequestEdit(DispID);
    ConnectData.pUnk := nil;
...
begin
  ...
  begin
    ...
    PrevWnd := Winapi.Windows.GetWindow(FWinControl.Handle, GW_HWNDPREV);
    InPlaceDeactivate;
...
begin
  if EventSink <> nil then
    IStdEvents(EventSink).MouseMove((Word(Shift) shr 3) and 7,
      GetEventShift(Shift), X, Y);
end;
  ...
  begin
    ...
    if FilterMessage then
      if FSimpleFrameSite.PreMessageFilter(Handle, Msg, WParam, LParam,
        LRESULT(Result), Cookie) = S_FALSE then Exit;
    case Msg of
...
{$IFDEF WIN64}
function TActiveXControl.SetExtent(dwDrawAspect: Longint; size: PPoint): HResult;
{$ELSE}
...
begin
  Result := InPlaceActivate(fActivate);
end;
...
function TActiveXControl.PreMessageFilter(wnd: HWnd; msg: UInt; wp: WPARAM; lp: LPARAM;
...
function TActiveXControl.PostMessageFilter(wnd: HWnd; msg: UInt; wp: WPARAM; lp: LPARAM;
...
{$IFDEF WIN64}
function TActiveXControl.SetContentExtent(sizel: PPoint): HResult; stdcall;
{$ELSE}
...
function TActiveXControlFactory.CreateControl(Parent: HWND): TWinControl;
begin
  Result := WinControlClass.CreateParented(Parent);
end;
...
begin
  ...
  begin
    ...
    CreateRegKey(ClassKey + '\MiscStatus', '', '0', RootKey);
    CreateRegKey(ClassKey + '\MiscStatus\1', '', IntToStr(FMiscStatus), RootKey);
    CreateRegKey(ClassKey + '\ToolboxBitmap32', '',
      ComServer.ServerFileName + ',' + IntToStr(FToolboxBitmapID), RootKey);
    CreateRegKey(ClassKey + '\Control', '', '', RootKey);
    CreateRegKey(ClassKey + '\Verb', '', '', RootKey);
    for I := 0 to FVerbs.Count - 1 do
      CreateRegKey(ClassKey + '\Verb\' + IntToStr(Integer(FVerbs.Objects[I])),
        '', FVerbs[I] + ',0,2', RootKey);
  end else
  begin
    for I := 0 to FVerbs.Count - 1 do
      DeleteRegKey(ClassKey + '\Verb\' + IntToStr(Integer(FVerbs.Objects[I])), RootKey);
    DeleteRegKey(ClassKey + '\Verb', RootKey);
    DeleteRegKey(ClassKey + '\Control', RootKey);
    DeleteRegKey(ClassKey + '\ToolboxBitmap32', RootKey);
    DeleteRegKey(ClassKey + '\MiscStatus\1', RootKey);
    DeleteRegKey(ClassKey + '\MiscStatus', RootKey);
    inherited UpdateRegistry(Register);
  end;
end;
...
constructor TActiveFormControl.Create;
begin
  inherited;
end;

constructor TActiveFormControl.CreateAggregated(const Controller: IInterface);
begin
  inherited;
end;

constructor TActiveFormControl.CreateFromFactory(Factory: TComObjectFactory;
  const Controller: IInterface);
begin
  inherited;
end;

destructor TActiveFormControl.Destroy;
begin
  inherited;
end;
...
begin
  ...
  begin
    ...
    TActiveForm(FControl).InitializeControl;
  end;
end;
...
begin
  ...
  Result := TAutoObjectFactory(Factory).DispTypeInfo.Invoke(Pointer(
    PByte(Control) + TAutoObjectFactory(Factory).DispIntfEntry.IOffset),
...
procedure TActiveForm.InitializeControl;
begin
end;

function TActiveForm.GetConnectionPoint: TConnectionPoint;
begin
  Result := nil;
  if Assigned(ActiveFormControl) then
    Result := ActiveFormControl.ConnectionPoint;
end;

{ TActiveFormFactory }

constructor TActiveFormFactory.Create(ComServer: TComServerObject;
                                      ActiveXControlClass: TActiveXControlClass;
                                      WinControlClass: TWinControlClass; 
                                      const ClassID: TGUID; 
                                      ToolboxBitmapID: Integer; 
                                      const LicStr: string; MiscStatus: Integer;
                                      ThreadingModel: TThreadingModel = tmSingle);
begin
  inherited;
...
destructor TActiveFormFactory.Destroy; 
begin
  inherited;
end;
...
constructor TPropertyPageImpl.Create(const Controller: IInterface);
begin
  inherited;
end;

destructor TPropertyPageImpl.Destroy;
begin
  inherited;
end;
...
const
  MaxStrings = MaxInt div 16;
type
  TPWideCharArray = array[0..MaxStrings] of PWideChar;
var
  Size, I: Integer;
  S: WideString;
begin
  ...
  begin
    S := FStrings.Item[FIndex];
    Size := Length(S) * SizeOf(WideChar);
    TPWideCharArray(elt)[I] := CoTaskMemAlloc(Size + SizeOf(WideChar));
    FillChar(TPWideCharArray(elt)[I]^, Size + SizeOf(WideChar), 0);
    CopyMemory(TPWideCharArray(elt)[I], PWideChar(S), Size);
    Inc(I);
    Inc(FIndex);
  end;
...
{$IFDEF MSWINDOWS}
 {$IFDEF WIN64}
  OlePro32Dll := SafeLoadLibrary('oleaut32.dll');
 {$ELSE}  
