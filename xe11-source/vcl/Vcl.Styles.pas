...
uses
  ...
  System.Generics.Collections, System.ZLib, {$NOINCLUDE System.ZLib} // Vcl.Styles.hpp need not include System.Zlib.hpp
...
type
  ...
  public
    class function DrawEdge(Style: TCustomStyleServices; DC: HDC; Part, State: Integer;
      const R: TRect; Edges: TElementEdges; Flags: TElementEdgeFlags; ContentRect: PRect; DPI: Integer = 0): Boolean; override;
    ...
    class function DrawIcon(Style: TCustomStyleServices; DC: HDC; Part, State: Integer;
      const R: TRect; himl: HIMAGELIST; Index: Integer; DPI: Integer = 0): Boolean; override;
    ...
    class function GetElementMargins(Style: TCustomStyleServices; DC: HDC; Part, State: Integer;
      Rect: PRect; Margin: TElementMargin; out ElementMargins: TElementMargins; DPI: Integer = 0): Boolean; override;
  ...
  // Implements specifics for teMPlayerButtons elements
  TCustomStyleMPlayerButtonElements = class(TCustomElementServices)
  public
    class function DrawElement(Style: TCustomStyleServices; DC: HDC; Part, State: Integer;
      const R: TRect; ClipRect: PRect; DPI: Integer = 0): Boolean; override;
    class function GetElementContentRect(Style: TCustomStyleServices; DC: HDC; Part, State: Integer;
  ...
  /// <summary>
  ///    Special class to draw search indicator element from style
  /// </summary>
  // Implements specifics for teSearchIndicators elements
  TCustomStyleSearchIndicatorElements = class(TCustomElementServices)
  public
    class function DrawElement(Style: TCustomStyleServices; DC: HDC; Part, State: Integer;
      const R: TRect; ClipRect: PRect; DPI: Integer = 0): Boolean; override;
    class function GetElementContentRect(Style: TCustomStyleServices; DC: HDC; Part, State: Integer;
  ...
  /// <summary>
  ///    Special class to draw swicth elements from style
  /// </summary>
  // Implements specifics for teToggleSwitch elements
  TCustomStyleToggleSwitchElements = class(TCustomElementServices)
  public
    class function DrawElement(Style: TCustomStyleServices; DC: HDC; Part, State: Integer;
      const R: TRect; ClipRect: PRect; DPI: Integer = 0): Boolean; override;
  end;
...
uses
  System.Types, System.UITypes, System.SysUtils, System.StrUtils, Winapi.UxTheme, Vcl.StdCtrls, Vcl.ComCtrls,
  Vcl.Mask, Vcl.GraphUtil, Vcl.ImgList, Vcl.Menus, Vcl.Grids, Vcl.CategoryButtons,
  Vcl.ButtonGroup, Vcl.ExtCtrls, Vcl.Consts, Vcl.SysStyles, Winapi.D2D1, Vcl.Direct2D
{$IFDEF USE_VCL_STYLE_API}
  , Vcl.StyleAPI, Vcl.StyleBitmap
  ;
{$ELSE}
  ;
{$I StyleUtils.inc}
...
function DrawButtonElement(Style: TCustomStyle; DC: HDC; const R: TRect; ClipRect: PRect;
  const ControlName, ElementName: string; State: TSeState; DPI: Integer = 0): Boolean;
var
  ...
  begin
    ...
    try
      if ClipRect <> nil then
        LObject.Draw(Canvas, ClipRect^, DPI)
      else
        LObject.Draw(Canvas, NullRect, DPI);
    finally
      Free;
    end;
    Result := True;
...
function DrawBitmapElement(Style: TCustomStyle; DC: HDC; const R: TRect; ClipRect: PRect;
  const ControlName, ElementName: string; DPI: Integer = 0): Boolean;
var
  LObject: TseStyleObject;
begin
  Result := False;
...
begin
  ...
  FCustomElements[teMPlayerButtons] := TCustomStyleMPlayerButtonElements;
  FCustomElements[teToggleSwitch] := TCustomStyleToggleSwitchElements;
  FCustomElements[teSearchIndicators] := TCustomStyleSearchIndicatorElements;
end;
...
function TCustomStyle.GetThemeForDPI(Element: TThemedElement; DPI: Integer = 0): THandle;
begin
  Result := 0;
end;

procedure TCustomStyle.UnloadThemeDataForDPI;
begin
end;
...
begin
  ...
  if FCustomElements[Details.Element] <> nil then
    Result := FCustomElements[Details.Element].GetElementSize(Self, DC, Details.Part,
      Details.State, Rect, ElementSize, Size, DPI)
  else
  begin
    Result := False;
      ...
      case Details.Part of
        THEME_BP_CHECKBOX:
          begin
            ...
            if not GetElementSize(DC, GetElementDetails(tbRadioButtonCheckedNormal),
                LRect, ElementSize, Size, DPI) then
            begin
              Size.cx := 13;
              Size.cy := 13;
            end;
        ...
        THEME_BP_RADIOBUTTON:
         begin
           LBitmapLink := nil;
           case TSeCheckDrawState(Details.State) of
             kcdsUncheckedNormal..kcdsUncheckedDisabled:
               begin
                 LStyleObject := GetElementObject(Self, 'RadioButton', 'Unchecked'); // do not localize
                 if LStyleObject is TSeButtonObject then
                   case TSeCheckDrawState(Details.State) of
                     kcdsUncheckedNormal: LBitmapLink := TSeButtonObject(LStyleObject).Bitmap;
                     kcdsUncheckedHot: LBitmapLink := TSeButtonObject(LStyleObject).BitmapHot;
             ...
             kcdsCheckedNormal..kcdsCheckedDisabled:
               begin
                 LStyleObject := GetElementObject(Self, 'RadioButton', 'Checked'); // do not localize
                 if LStyleObject is TSeButtonObject then
                   case TSeCheckDrawState(Details.State) of
                     kcdsCheckedNormal: LBitmapLink := TSeButtonObject(LStyleObject).Bitmap;
                     kcdsCheckedHot: LBitmapLink := TSeButtonObject(LStyleObject).BitmapHot;
...
begin
  ...
  if FCustomElements[Details.Element] <> nil then
    Result := FCustomElements[Details.Element].GetElementMargins(Self, DC, Details.Part,
      Details.State, Rect, ElementMargin, Margins, DPI)
  else
    Result := False;
end;
      ...
      case Element of
        teButton:
          begin
            ...
            if Part = THEME_BP_CHECKBOX then
              CheckDraw(kcscCheckBox, LCanvas, CheckInfo(R, TSeCheckDrawState(State)), DPI)
            else
            if Part = THEME_BP_RADIOBUTTON then
              CheckDraw(kcscRadioButton, LCanvas, CheckInfo(R, TSeCheckDrawState(State)), DPI)
            else
            ...
            begin
              DrawButtonElement(Self, LCanvas.Handle, R, ClipRect,
                'Button', 'CommandLinkGlyph', TseState(State), DPI);
            end
            else
              Result := False;
          end;
              ...
              try
                ...
                EditDrawButton(kescEdit, Buffer.Canvas,
                  EditButtonInfo(Rect(0, 0, Buffer.Width, Buffer.Height),
                    TSeEditButtonDrawState(State), kebcDropDown), DPI);
        ...
        teTrackBar:
          begin
            ...
            if Part = THEME_TKP_TRACK then
              TrackBarDraw(ktscTrackBar, LCanvas, TrackBarInfo(R, ktbcHorizontal, ktrbdsNormal), DPI)
            else
            if Part = THEME_TKP_TRACKVERT then
              TrackBarDraw(ktscTrackBar, LCanvas, TrackBarInfo(R, ktbcVertical, ktrbdsNormal), DPI)
            else
            if (Part = THEME_TKP_THUMB) then
              TrackDrawThumb(ktscTrackBar, LCanvas, TrackThumbInfo(R, ktbmHorzBoth, TSeTrackThumbDrawState(State)), DPI)
            else
            if Part = THEME_TKP_THUMBVERT then
              TrackDrawThumb(ktscTrackBar, LCanvas, TrackThumbInfo(R, ktbmVertBoth, TSeTrackThumbDrawState(State)), DPI)
            else
            if Part = THEME_TKP_THUMBBOTTOM then
              TrackDrawThumb(ktscTrackBar, LCanvas, TrackThumbInfo(R, ktbmHorzBottom, TSeTrackThumbDrawState(State)), DPI)
            else
            if Part = THEME_TKP_THUMBTOP then
              TrackDrawThumb(ktscTrackBar, LCanvas, TrackThumbInfo(R, ktbmHorzTop, TSeTrackThumbDrawState(State)), DPI)
            else
            if Part = THEME_TKP_THUMBLEFT then
              TrackDrawThumb(ktscTrackBar, LCanvas, TrackThumbInfo(R, ktbmVertLeft, TSeTrackThumbDrawState(State)), DPI)
            else
            if Part = THEME_TKP_THUMBRIGHT then
              TrackDrawThumb(ktscTrackBar, LCanvas, TrackThumbInfo(R, ktbmVertRight, TSeTrackThumbDrawState(State)), DPI)
            else
        ...
        teStatus:
          begin
            ...
            begin
              StatusDrawPanel(ksscStatusBar, LCanvas,
                StatusPanelInfo(R, kspcGripperPanel), DPI);
            end;
            if Part = THEME_SP_GRIPPER then
            begin
              StatusDrawPanel(ksscStatusBar, LCanvas,
                StatusPanelInfo(R, kspcGripper), DPI);
            end
            else
              Result := False;
          end;
        teScrollBar:
          begin
            ...
            if Part = THEME_SBP_LOWERTRACKVERT then
              ScrollDrawArea(ksscScrollBar, LCanvas,
                ScrollAreaInfo(R, TSeScrollAreaDrawState(State), ksacVertical), DPI)
            else
            ...
            begin
              WindowDrawGripper(kwscStandard, LCanvas, WindowGripperInfo(R, true), DPI);
            end;
          end;
        teSpin:
          begin
            ...
            if Part = THEME_SPNP_UP then
              ButtonDraw(kbscSpinUp, LCanvas, ButtonInfo(R, TSeButtonDrawState(State)), DPI)
            else
            if Part = THEME_SPNP_DOWN then
              ButtonDraw(kbscSpinDown, LCanvas, ButtonInfo(R, TSeButtonDrawState(State)), DPI)
            else
            if Part = THEME_SPNP_DOWNHORZ then
              ButtonDraw(kbscSpinLeft, LCanvas, ButtonInfo(R, TSeButtonDrawState(State)), DPI)
            else
            if Part = THEME_SPNP_UPHORZ then
              ButtonDraw(kbscSpinRight, LCanvas, ButtonInfo(R,TSeButtonDrawState(State)), DPI)
            else
              Result := False;
          end;
        teRebar:
          begin
            ...
            if Part = THEME_RP_GRIPPER then
              ToolDrawItem(ktscToolbar, LCanvas,
                ToolItemInfo(R, kticGripper, mtidsNormal), DPI)
            else
            if Part = THEME_RP_GRIPPERVERT then
              ToolDrawItem(ktscToolbar, LCanvas,
                ToolItemInfo(R, kticGripperVert, mtidsNormal), DPI)
            else
            if Part = THEME_RP_BAND then
              ToolDraw(ktscToolbar, LCanvas, ToolInfo(R), DPI)
            else
            if Part = THEME_RP_CHEVRONVERT then
              ToolDrawItem(ktscToolbar, LCanvas,
                      ToolItemInfo(R, kticExpandButton, TSeToolItemDrawState(State)), DPI)
            else
         ...
         teWindow:
          begin
            ...
            begin
              ...
              begin
                MenuDrawItem(kmscMenuBar, LCanvas, MenuItemInfo(R, kmicMin, TSeMenuItemDrawState(State)), DPI);
                Exit;
              end
              else
              if (Part = THEME_WP_MDIRESTOREBUTTON) then
              begin
                MenuDrawItem(kmscMenuBar, LCanvas, MenuItemInfo(R, kmicRestore, TSeMenuItemDrawState(State)), DPI);
                Exit;
              end
              else
              if (Part = THEME_WP_MDICLOSEBUTTON) then
              begin
                MenuDrawItem(kmscMenuBar, LCanvas, MenuItemInfo(R, kmicClose, TSeMenuItemDrawState(State)), DPI);
                Exit;
              end
              else
              ...
              begin
                ...
                WindowDrawButton(kwscStandard, LCanvas, WindowButtonInfo(WI.Rect, kwbSysMenu, TSeWindowButtonDrawState(State)), DPI);
                Exit;
              end
              else
              ...
              begin
                ...
                WI.Rect := R;
                WI.ClipRect := Rect(0, 0, 0, 0);
                WindowDrawButton(kwscStandard, LCanvas, WindowButtonInfo(WI.Rect, kwbMin, TSeWindowButtonDrawState(State)), DPI);
                Exit;
              end
              else
              ...
              begin
                ...
                WI.Rect := R;
                WI.ClipRect := Rect(0, 0, 0, 0);
                WindowDrawButton(kwscStandard, LCanvas, WindowButtonInfo(WI.Rect, kwbMax, TSeWindowButtonDrawState(State)), DPI);
                Exit;
              end
              else
              ...
              begin
                ...
                WI.Rect := R;
                WI.ClipRect := Rect(0, 0, 0, 0);
                WindowDrawButton(kwscStandard, LCanvas, WindowButtonInfo(WI.Rect, kwbClose, TSeWindowButtonDrawState(State)), DPI);
                Exit;
              end
              else
              ...
              begin
                ...
                WI.Rect := R;
                WI.ClipRect := Rect(0, 0, 0, 0);
                WindowDrawButton(kwscStandard, LCanvas, WindowButtonInfo(WI.Rect, kwbMaxRestore, TSeWindowButtonDrawState(State)), DPI);
                Exit;
              end
              else
              ...
              begin
                ...
                WI.Rect := R;
                WI.ClipRect := Rect(0, 0, 0, 0);
                WindowDrawButton(kwscStandard, LCanvas, WindowButtonInfo(WI.Rect, kwbHelp, TSeWindowButtonDrawState(State)), DPI);
                Exit;
              end
              else
              ...
              begin
                ...
                WI.Rect := R;
                WI.ClipRect := Rect(0, 0, 0, 0);
                WindowDrawButton(kwscToolWindow, LCanvas, WindowButtonInfo(WI.Rect, kwbClose, TSeWindowButtonDrawState(State)), DPI);
                Exit;
              end;
              ...
              try
                ...
                begin
                  ...
                  if DPI > Screen.DefaultPixelsPerInch then
                    WindowDraw(WC, Buffer.Canvas, WI, DPI, 'Title')
                  else
                ...
                begin
                  ...
                  WindowDraw(WC, Buffer.Canvas, WI, DPI);
                  MoveWindowOrg(Buffer.Canvas.Handle, 0, MR.Top);
                end
                else
...
begin
  ...
  if C <> nil then
  begin
    if not (TStyleManager.UseParentPaintBuffers and (C.Parent <> nil) and
       (C.Parent.Perform(CM_PAINTPARENTBUFFER, Winapi.Windows.WPARAM(Target), Winapi.Windows.LPARAM(C)) > 0)) then
...
begin
  ...
  if FCustomElements[Details.Element] <> nil then
    ...
    Result := InternalDrawText(DC, Details, S, R, Flags, Options.TextColor, DPI);
end;
    ...
    case Element of
      ...
      teWindow:
        begin
          ...
          if DPI > Screen.DefaultPixelsPerInch then
            R1.Bottom := MulDiv(R1.Bottom, DPI, Screen.DefaultPixelsPerInch);

          if (Details.Part <> THEME_WP_SMALLCAPTION) or
             ((Details.Part = THEME_WP_SMALLCAPTION) and (R1.Left < R.Left))
          then
            R1.Left := R.Left;

          R1.Right := R.Right;
          WindowDrawText(WC, LCanvas, WI, R1, tfRtlReading in Flags, DPI);
        end
      else
        begin
          DrawStyleText(LCanvas, TextInfo(R, S, TextFlags), DPI);
          Result := True;
        end;
    end;
  finally
          ...
          case Details.Part of
            THEME_WP_SMALLCLOSEBUTTON:
              begin
                ...
                ContentRect := TseStyle(FSource).WindowGetButtonRect(kwscToolWindow, WI, kwbClose, DPI);
              end;
            THEME_WP_CLOSEBUTTON:
              begin
                THEME_WP_BUTTONS := [kwbClose];
                WI.Buttons := THEME_WP_BUTTONS;
                WI.Rect := BoundingRect;
                ContentRect := TseStyle(FSource).WindowGetButtonRect(kwscStandard, WI, kwbClose, DPI);
              end;
            THEME_WP_HELPBUTTON:
              begin
                ...
                WI.Buttons := THEME_WP_BUTTONS;
                WI.Rect := BoundingRect;
                ContentRect := TseStyle(FSource).WindowGetButtonRect(kwscStandard, WI, kwbHelp, DPI);
              end;
            THEME_WP_MAXBUTTON, THEME_WP_RESTOREBUTTON:
              begin
                ...
                WI.Buttons := THEME_WP_BUTTONS;
                WI.Rect := BoundingRect;
                ContentRect := TseStyle(FSource).WindowGetButtonRect(kwscStandard, WI, kwbMax, DPI);
              end;
            THEME_WP_MINBUTTON:
              begin
                ...
                WI.Buttons := THEME_WP_BUTTONS;
                WI.Rect := BoundingRect;
                ContentRect := TseStyle(FSource).WindowGetButtonRect(kwscStandard, WI, kwbMin, DPI);
              end;
            THEME_WP_SYSBUTTON:
              begin
                ...
                WI.Buttons := THEME_WP_BUTTONS;
                WI.Rect := BoundingRect;
                ContentRect := TseStyle(FSource).WindowGetButtonRect(kwscStandard, WI, kwbSysMenu, DPI);
              end;
          else
            Result := False;
          end;
...
begin
  ...
  begin
    case Detail of
      tbCommandLinkGlyphNormal..tbCommandLinkGlyphDefaulted:
      begin
        ...
        case Detail of
          ...
          tbCommandLinkGlyphDefaulted: State := Ord(ssFocused);
        end;
      end;
...
function TCustomStyle.GetElementDetails(Detail: TThemedMPlayerButtons): TThemedElementDetails;
begin
...
function TCustomStyle.GetElementDetails(Detail: TThemedSearchIndicators): TThemedElementDetails;
begin
...
function TCustomStyle.GetElementDetails(Detail: TThemedToggleSwitch): TThemedElementDetails;
begin
      ...
      tsUpNormal..tsUpDisabled:
        begin
          ...
          case Detail of
            ...
            tsUpPressed: State := Ord(kbdsPressed);
      ...
      tsDownNormal..tsDownDisabled:
        begin
          ...
          case Detail of
            ...
            tsDownPressed: State := Ord(kbdsPressed);
      ...
      tsUpHorzNormal..tsUpHorzDisabled:
        begin
          ...
          case Detail of
            ...
            tsUpHorzPressed: State := Ord(kbdsPressed);
      ...
      tsDownHorzNormal..tsDownHorzDisabled:
        begin
          ...
          case Detail of
            ...
            tsDownHorzPressed: State := Ord(kbdsPressed);
...
begin
  ...
  begin
    LClass := Control.ClassType;
    while LClass <> TControl do
    begin
      if RegisteredStyleHooks.ContainsKey(LClass) then
      begin
        LStyleHook := CreateStyleHook(RegisteredStyleHooks[LClass]);
        Break;
      end;
...
begin
  case TThemedGrid(State) of
    tgIndicatorArrow:
      Result := DrawBitmapElement(TCustomStyle(Style), DC, R, ClipRect, ControlName, 'curArrow', DPI);
    tgIndicatorEdit:
      Result := DrawBitmapElement(TCustomStyle(Style), DC, R, ClipRect, ControlName, 'curEdit', DPI);
    tgIndicatorInsert:
      Result := DrawBitmapElement(TCustomStyle(Style), DC, R, ClipRect, ControlName, 'curInsert', DPI);
    tgIndicatorMultiDot:
      Result := DrawBitmapElement(TCustomStyle(Style), DC, R, ClipRect, ControlName, 'curMultiDot', DPI);
    tgIndicatorMultiArrow:
      Result := DrawBitmapElement(TCustomStyle(Style), DC, R, ClipRect, ControlName, 'curMultiArrow', DPI);
  else
    Result := inherited DrawElement(Style, DC, Part, State, R, ClipRect, DPI);
  end;
end;
...
begin
  case TThemedCategoryPanelGroup(State) of
    tcpThemedHeader:
      Result := DrawBitmapElement(TCustomStyle(Style), DC, R, ClipRect, CControlName, CHeaderElement, DPI);
    tcpThemedChevronOpenedNormal:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect, CControlName, CChevronOpenedElement, ssNormal, DPI);
    tcpThemedChevronOpenedHot:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect, CControlName, CChevronOpenedElement, ssHot, DPI);
    tcpThemedChevronOpenedPressed:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect, CControlName, CChevronOpenedElement, ssPressed, DPI);
    tcpThemedChevronClosedNormal:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect, CControlName, CChevronClosedElement, ssNormal, DPI);
    tcpThemedChevronClosedHot:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect, CControlName, CChevronClosedElement, ssHot, DPI);
    tcpThemedChevronClosedPressed:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect, CControlName, CChevronClosedElement, ssPressed, DPI);
  else
    Result := inherited DrawElement(Style, DC, Part, State, R, ClipRect, DPI);
  end;
end;
...
begin
  case TThemedCategoryButtons(State) of
    tcbButtonNormal:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect, CControlName, CButtonFace, ssNormal, DPI);
    tcbButtonHot:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect, CControlName, CButtonFace, ssHot, DPI);
    tcbButtonSelected:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect, CControlName, CButtonFace, ssPressed, DPI);
    tcbCategoryGlyphOpened:
      Result := DrawBitmapElement(TCustomStyle(Style), DC, R, ClipRect, CControlName, CChevronOpenedElement, DPI);
    tcbCategoryGlyphClosed:
      Result := DrawBitmapElement(TCustomStyle(Style), DC, R, ClipRect, CControlName, CChevronClosedElement, DPI);
  else
    Result := inherited DrawElement(Style, DC, Part, State, R, ClipRect, DPI);
  end;
end;
...
class function TCustomStyleTextLabelElements.DrawText(Style: TCustomStyleServices;
  ...
  Options: TStyleTextOptions; DPI: Integer = 0): Boolean;
var
  LRect: TRect;
  LFlags: Cardinal;
  LColor: TColor;
...
{ TCustomStyleSearchIndicatorElements }

class function TCustomStyleSearchIndicatorElements.DrawElement(Style: TCustomStyleServices;
  DC: HDC; Part, State: Integer; const R: TRect; ClipRect: PRect; DPI: Integer = 0): Boolean;
...
begin
  Result := True;
  case TThemedSearchIndicators(State) of
    tsiTextNormal, tsiTextHot, tsiTextPressed, tsiTextDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect,
        ControlName, 'btnText', GetState(State - Integer(tsiTextNormal)), DPI);

    tsiAudioNormal, tsiAudioHot, tsiAudioPressed, tsiAudioDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect,
        ControlName, 'btnAudio', GetState(State - Integer(tsiAudioNormal)), DPI);
  end;
end;

class function TCustomStyleSearchIndicatorElements.GetElementContentRect(Style: TCustomStyleServices;
  DC: HDC; Part, State: Integer; const BoundingRect: TRect; out ContentRect: TRect; DPI: Integer = 0): Boolean;
begin
  Result := True;
  ContentRect := TRect.Create(0, 0, 24, 16);
end;

{ TCustomStyleToggleSwitchElements }

class function TCustomStyleToggleSwitchElements.DrawElement(Style: TCustomStyleServices;
  DC: HDC; Part, State: Integer; const R: TRect; ClipRect: PRect; DPI: Integer = 0): Boolean;
const
  CControlName = 'ToggleSwitch';
  CTrackOffElement = 'TrackOff';
  CTrackOnElement = 'TrackOn';
  CThumbElement = 'Thumb';
begin
  case TThemedToggleSwitch(State) of
    ttsTrackOffNormal:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect, CControlName, CTrackOffElement, ssNormal, DPI);
    ttsTrackOffHot:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect, CControlName, CTrackOffElement, ssHot, DPI);
    ttsTrackOffDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect, CControlName, CTrackOffElement, ssDisabled, DPI);

    ttsTrackOnNormal:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect, CControlName, CTrackOnElement, ssNormal, DPI);
    ttsTrackOnHot:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect, CControlName, CTrackOnElement, ssHot, DPI);
    ttsTrackOnDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect, CControlName, CTrackOnElement, ssDisabled, DPI);

    ttsThumbNormal:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect, CControlName, CThumbElement, ssNormal, DPI);
    ttsThumbHot:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect, CControlName, CThumbElement, ssHot, DPI);
    ttsThumbDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect, CControlName, CThumbElement, ssDisabled, DPI);
  else
    Result := inherited DrawElement(Style, DC, Part, State, R, ClipRect, DPI);
  end;
end;
...
var
  ...
  begin
    LSource.ToolDrawItem(ktscToolbar, LFontRecall.Canvas, ToolItemInfo(R, APart, AState), DPI);
  end;
begin
  Result := True;
    ...
    case TThemedToolBar(State) of
      ...
      ttbSeparatorVertNormal..ttbSeparatorVertCheckedHot:
        LSource.ToolDrawItem(ktscToolbar, LFontRecall.Canvas,
          ToolItemInfo(R, kticSeparator, mtidsNormal), DPI, 'ToolBar');
        ...
        case TThemedToolBar(State) of
          ttbDropDownButtonGlyphNormal:
            DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect,
              'SpeedButton\Chevron', 'Glyph', ssNormal, DPI); // do not localize
          ttbDropDownButtonGlyphHot:
            DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect,
              'SpeedButton\Chevron', 'Glyph', ssHot, DPI); // do not localize
          ttbDropDownButtonGlyphPressed:
            DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect,
              'SpeedButton\Chevron', 'Glyph', ssPressed, DPI); // do not localize
        else
          Result := False;
        end;
    else
      LSource.ToolDraw(ktscToolbar, LFontRecall.Canvas, ToolInfo(R), DPI, 'ToolBar');
    end;
  finally
    LFontRecall.Free;
  end;
...
var
  ...
  begin
    LSource.MenuDrawItemGlyph(kmscPopupMenu, LFontRecall.Canvas,
      ...
      MenuGlyphInfo(R, nil, kgfaCenter, AGlyph), DPI);
  end;
...
begin
  ...
  case TThemedMenu(State) of
    ...
    tmPopupItemHot:
      case ElementColor of
        ecTextColor: Color := LSource.Fonts[ktfPopupMenuItemTextSelected].Color;
      else
        Result := False;
      end;
    tmPopupItemDisabled,
    tmPopupItemDisabledHot:
      case ElementColor of
        ecTextColor: Color := LSource.Fonts[ktfPopupMenuItemTextDisabled].Color;
      else
        Result := False;
      end;
  else
...
class function TCustomStyleMenuElements.GetElementMargins(Style: TCustomStyleServices;
  DC: HDC; Part, State: Integer; Rect: PRect; Margin: TElementMargin; out ElementMargins: TElementMargins; DPI: Integer = 0): Boolean;
var
  LObject: TseStyleObject;
begin
  Result := True;
        ...
        emSizing:
          begin
            ...
            if LObject <> nil then
              ...
              ElementMargins := TElementMargins.Create(2, 2, 2, 2);
          end;
  ...
  if Result and (DPI > Screen.DefaultPixelsPerInch) then
    ElementMargins := TElementMargins.Create(
    MulDiv(ElementMargins.cxLeftWidth, DPI, Screen.DefaultPixelsPerInch),
    MulDiv(ElementMargins.cyTopHeight, DPI, Screen.DefaultPixelsPerInch),
    MulDiv(ElementMargins.cxRightWidth, DPI, Screen.DefaultPixelsPerInch),
    MulDiv(ElementMargins.cyBottomHeight, DPI, Screen.DefaultPixelsPerInch));
end;
...
begin
  ...
  if Result and (DPI > Screen.DefaultPixelsPerInch) then
    Size := TSize.Create(MulDiv(Size.cx, DPI, Screen.DefaultPixelsPerInch),
    MulDiv(Size.cy, DPI, Screen.DefaultPixelsPerInch));
end;
...
begin
  ...
  case TThemedDataNavButtons(State) of
    tdnbFirstNormal, tdnbFirstHot, tdnbFirstPressed, tdnbFirstDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect,
        ControlName, 'btnFirst', GetState(State - Integer(tdnbFirstNormal)), DPI);
    tdnbPriorNormal, tdnbPriorHot, tdnbPriorPressed, tdnbPriorDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect,
        ControlName, 'btnPrior', GetState(State - Integer(tdnbPriorNormal)), DPI);
    tdnbNextNormal, tdnbNextHot, tdnbNextPressed, tdnbNextDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect,
        ControlName, 'btnNext', GetState(State - Integer(tdnbNextNormal)), DPI);
    tdnbLastNormal, tdnbLastHot, tdnbLastPressed, tdnbLastDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect,
        ControlName, 'btnLast', GetState(State - Integer(tdnbLastNormal)), DPI);
    tdnbInsertNormal, tdnbInsertHot, tdnbInsertPressed, tdnbInsertDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect,
        ControlName, 'btnInsert', GetState(State - Integer(tdnbInsertNormal)), DPI);
    tdnbDeleteNormal, tdnbDeleteHot, tdnbDeletePressed, tdnbDeleteDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect,
        ControlName, 'btnDelete', GetState(State - Integer(tdnbDeleteNormal)), DPI);
    tdnbEditNormal, tdnbEditHot, tdnbEditPressed, tdnbEditDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect,
        ControlName, 'btnEdit', GetState(State - Integer(tdnbEditNormal)), DPI);
    tdnbPostNormal, tdnbPostHot, tdnbPostPressed, tdnbPostDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect,
        ControlName, 'btnPost', GetState(State - Integer(tdnbPostNormal)), DPI);
    tdnbCancelNormal, tdnbCancelHot, tdnbCancelPressed, tdnbCancelDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect,
        ControlName, 'btnCancel', GetState(State - Integer(tdnbCancelNormal)), DPI);
    tdnbRefreshNormal, tdnbRefreshHot, tdnbRefreshPressed, tdnbRefreshDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect,
        ControlName, 'btnRefresh', GetState(State - Integer(tdnbRefreshNormal)), DPI);
    tdnbApplyUpdatesNormal, tdnbApplyUpdatesHot, tdnbApplyUpdatesPressed, tdnbApplyUpdatesDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect,
        ControlName, 'btnApplyUpdates', GetState(State - Integer(tdnbApplyUpdatesNormal)), DPI);
    tdnbCancelUpdatesNormal, tdnbCancelUpdatesHot, tdnbCancelUpdatesPressed, tdnbCancelUpdatesDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect,
        ControlName, 'btnCancelUpdates', GetState(State - Integer(tdnbCancelUpdatesNormal)), DPI);
  end;
end;
...
{ TCustomStyleMPlayerButtonElements }

class function TCustomStyleMPlayerButtonElements.DrawElement(Style: TCustomStyleServices;
  DC: HDC; Part, State: Integer; const R: TRect; ClipRect: PRect; DPI: Integer = 0): Boolean;
...
const
  ControlName = 'MediaPlayer';
begin
  Result := True;
  case TThemedMPlayerButtons(State) of
    tmpbPlayNormal, tmpbPlayHot, tmpbPlayPressed, tmpbPlayDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect,
        ControlName, 'btnPlay', GetState(State - Integer(tmpbPlayNormal)), DPI);
    tmpbPauseNormal, tmpbPauseHot, tmpbPausePressed, tmpbPauseDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect,
        ControlName, 'btnPause', GetState(State - Integer(tmpbPauseNormal)), DPI);
    tmpbStopNormal, tmpbStopHot, tmpbStopPressed, tmpbStopDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect,
        ControlName, 'btnStop', GetState(State - Integer(tmpbStopNormal)), DPI);
    tmpbNextNormal, tmpbNextHot, tmpbNextPressed, tmpbNextDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect,
        ControlName, 'btnNext', GetState(State - Integer(tmpbNextNormal)), DPI);
    tmpbPrevNormal, tmpbPrevHot, tmpbPrevPressed, tmpbPrevDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect,
        ControlName, 'btnPrev', GetState(State - Integer(tmpbPrevNormal)), DPI);
    tmpbStepNormal, tmpbStepHot, tmpbStepPressed, tmpbStepDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect,
        ControlName, 'btnStep', GetState(State - Integer(tmpbStepNormal)), DPI);
    tmpbBackNormal, tmpbBackHot, tmpbBackPressed, tmpbBackDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect,
        ControlName, 'btnBack', GetState(State - Integer(tmpbBackNormal)), DPI);
    tmpbRecordNormal, tmpbRecordHot, tmpbRecordPressed, tmpbRecordDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect,
        ControlName, 'btnRecord', GetState(State - Integer(tmpbRecordNormal)), DPI);
    tmpbEjectNormal, tmpbEjectHot, tmpbEjectPressed, tmpbEjectDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect,
        ControlName, 'btnEject', GetState(State - Integer(tmpbEjectNormal)), DPI);
  end;
end;

class function TCustomStyleMPlayerButtonElements.GetElementContentRect(Style: TCustomStyleServices;
  DC: HDC; Part, State: Integer; const BoundingRect: TRect; out ContentRect: TRect; DPI: Integer = 0): Boolean;
begin
  Result := True;
  ContentRect := TRect.Create(0, 0, 16, 16);
...
class function TUxThemeStyleElements.DrawIcon(Style: TCustomStyleServices;
  DC: HDC; Part, State: Integer; const R: TRect; himl: HIMAGELIST; Index: Integer; DPI: Integer = 0): Boolean;
var
  Details: TThemedElementDetails;
begin
  Details := TThemedElementDetails.Create(GetThemedElement, Part, State);
...
begin
  Details := TThemedElementDetails.Create(GetThemedElement, Part, State);
  if Rect <> nil then
    Result := TStyleManager.SystemStyle.GetElementMargins(DC, Details, Rect^, Margin, ElementMargins, DPI)
  else
    Result := TStyleManager.SystemStyle.GetElementMargins(DC, Details, Margin, ElementMargins, DPI);
end;
...
begin
  Details := TThemedElementDetails.Create(GetThemedElement, Part, State);
  if Rect <> nil then
    Result := TStyleManager.SystemStyle.GetElementSize(DC, Details, Rect^, ElementSize, Size, DPI)
  else
    Result := TStyleManager.SystemStyle.GetElementSize(DC, Details, ElementSize, Size, DPI);
end;
