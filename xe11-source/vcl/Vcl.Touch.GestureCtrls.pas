...
begin
  Result := Point;
  Inc(Result.X, FOffsetX + ScaleValue(5));
  Inc(Result.Y, FOffsetY + ScaleValue(5));
end;
  ...
  case FAnimateState of
    asAnimating:
      begin
        ...
        if (FAnimateIndex >= 0) and (FAnimateIndex <= Length(FAnimatePoints) - 1) then
        begin
...
begin
  ...
  DrawGesture(Canvas, FDrawPoints, FPreviewLineColor, Point(FOffsetX + ScaleValue(5), FOffsetY + ScaleValue(5)));
...
begin
  ...
  begin
    ...
    LRect := FLegendRect;
    LRect.Left := LRect.Left + ScaleValue(10);
    Canvas.TextRect(LRect, FLegendText, [tfSingleLine, tfLeft, tfVerticalCenter, tfNoClip]);
  end;
end;
...
var
  LRect: TRect;
begin
  LRect := Rect(P.X - ScaleValue(3), P.Y - ScaleValue(3),
    P.X + ScaleValue(4), P.Y + ScaleValue(4));
  Canvas.Pen.Width := 1;
...
var
  LSize: TSize;
  LMargin, LWidth, LHeight,
  LPointWidth, LPreviewSize: Integer;
begin
  if HandleAllocated then
  begin
    ...
    FOffsetX := 0;
    FOffsetY := 0;
    LMargin := ScaleValue(CMargin);
    LPointWidth := ScaleValue(CPointWidth);
    ...
    begin
      ...
      FLegendRect := Rect(0, 0, LSize.cx + ScaleValue(10), LSize.cy);

      case FLegendPosition of
        lpTop:
          begin
            OffsetRect(FLegendRect, (Width - (LSize.cx + LPointWidth)) div 2, LMargin);
            Inc(FOffsetY, LSize.cy + LMargin * 2);
          end;
        lpBottom:
          OffsetRect(FLegendRect, (Width - (LSize.cx + LPointWidth)) div 2,
            Height - LSize.cy - LMargin);
      end;
    end
    else
    begin
    ...
    LHeight := Height - LSize.cy - (LMargin * 2);
    LWidth := Width - (LMargin * 2);
    if (FLegendPosition <> lpNone) then
      Dec(LHeight, LMargin);
