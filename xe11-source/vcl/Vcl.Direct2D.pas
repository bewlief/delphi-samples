...
type
  ...
  public
    ...
    property DrawTextOption: D2D1_DRAW_TEXT_OPTIONS read FDrawTextOption write FDrawTextOption;
    property Brush: TDirect2DBrush read GetBrush write SetBrush;
    property Pen: TDirect2DPen read GetPen write SetPen;
    property Font: TDirect2DFont read GetFont write SetFont;
  end;

  TD2DScaledGraphicDrawer = class(TScaledGraphicDrawer)
  private
    FScaledWICBuffer: TWICImage;
    FScaledBuffer: TBitmap;
  protected
...
begin
  ...
  begin
    ...
    if TBrush(Source).Style in [bsSolid, bsClear] then
...
class constructor TDirect2DCanvas.Create;
begin
...
begin
  ...
  begin
    ...
    HR := D2DFactory.CreateHwndRenderTarget(D2D1RenderTargetProperties(),
      D2D1HwndRenderTargetProperties(FHwnd, D2D1SizeU(Rect.Width, Rect.Height)),
...
procedure TD2DScaledGraphicDrawer.Initialize;
var
  LStream: TStream;
begin
  if FScaledWICBuffer <> Graphic then
    FreeAndNil(FScaledWICBuffer)
  else
    FScaledWICBuffer := nil;
  ...
  if (Graphic is TBitmap) and (TBitmap(Graphic).PixelFormat = pf32bit) then
    FScaledBuffer := TBitmap(Graphic)
  else if Graphic is TWICImage then
    FScaledWicBuffer := TWICImage(Graphic)
  else
  begin
    FScaledWicBuffer := TWICImage.Create;
    if Graphic is TBitmap then
      FScaledWICBuffer.Assign(Graphic)
    else
    begin
      ...
      try
        ...
        try
          FScaledWicBuffer.LoadFromStream(LStream);
        except
          FreeAndNil(FScaledWicBuffer);
          raise;
        end;
...
destructor  TD2DScaledGraphicDrawer.Destroy;
begin
  if FScaledWICBuffer <> Graphic then
    FreeAndNil(FScaledWICBuffer);
  inherited;
...
function  TD2DScaledGraphicDrawer.GetInitialized: Boolean;
begin
  Result := (FScaledBuffer <> nil) or (FScaledWICBuffer <> nil);
end;

procedure  TD2DScaledGraphicDrawer.Draw(ACanvas: TCanvas; const ARect: TRect);
var
...
begin
  if not Initialized or ARect.IsEmpty then
    Exit;

  D2DCanvas := TDirect2DCanvas.Create(ACanvas, ARect);
  try
    RenderTarget := D2DCanvas.RenderTarget;

    if FScaledBuffer <> nil then
      D2DBitmap := D2DCanvas.CreateBitmap(FScaledBuffer)
    else
    begin
      Factory := FScaledWicBuffer.ImagingFactory;
      Factory.CreateBitmapScaler(Scaler);
      Scaler.Initialize(FScaledWicBuffer.Handle, ARect.Width, ARect.Height, WICBitmapInterpolationModeHighQualityCubic);
      Factory.CreateFormatConverter(FormatConverter);
      FormatConverter.Initialize(Scaler, GUID_WICPixelFormat32bppPBGRA,
        WICBitmapDitherTypeNone, nil, 0, WICBitmapPaletteTypeCustom);
      RenderTarget.CreateBitmapFromWicBitmap(FormatConverter, nil, D2DBitmap);
    end;
    ...
    RenderTarget.BeginDraw;
    if D2DBitmap <> nil then
      RenderTarget.DrawBitmap(D2DBitmap, @DestRect);
...
begin
  ...
  begin
    ...
    if FpDefaultDrawTextOption <> nil then
