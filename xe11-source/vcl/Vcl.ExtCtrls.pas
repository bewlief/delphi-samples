...
{$ENDIF}
  Winapi.Messages, Winapi.Windows, System.SysUtils, System.Classes, System.Contnrs, System.Types, Vcl.ImgList,
  System.UITypes, Vcl.Controls, Vcl.Forms, Vcl.Menus, Vcl.Graphics, Vcl.StdCtrls, Vcl.GraphUtil, Vcl.Themes, Winapi.ShellAPI,
  Vcl.Mask;
...
{$ENDIF}
  ...
  TPanelStyleHook = class(TStyleHook)
  strict protected
...
{$ENDIF}
  ...
  private
    ...
    function GetControlBounds(AControl: TControl; AUseExplicitSize: Boolean; var AIntPos: TRect): TRect;
{$IF NOT DEFINED(CLR)}
...
{ THeader
  ...
  private
    ...
    procedure SetRadioTabStop(Value: Boolean);
  ...
  protected
    ...
    procedure CheckDefaultHeaderFont; override;
...
{$ENDIF}
  ...
  TCustomLabeledEdit = class(TCustomMaskEdit)
  private
...
type
  ...
  private
    ...
    FCollapsedHotImageName: TImageName;
    FCollapsedImageIndex: TImageIndex;
    FCollapsedImageName: TImageName;
    FCollapsedPressedImageIndex: TImageIndex;
    FCollapsedPressedImageName: TImageName;
    FExpandedHeight: Integer;
    FExpandedHotImageIndex: TImageIndex;
    FExpandedHotImageName: TImageName;
    FExpandedImageIndex: TImageIndex;
    FExpandedImageName: TImageName;
    FExpandedPressedImageIndex: TImageIndex;
    FExpandedPressedImageName: TImageName;
    FGlyphRect: TRect;
    ...
    procedure SetCollapsedHotImageName(const Value: TImageName);
    procedure SetCollapsedImageIndex(const Value: TImageIndex);
    procedure SetCollapsedImageName(const Value: TImageName);
    procedure SetCollapsedPressedImageIndex(const Value: TImageIndex);
    procedure SetCollapsedPressedImageName(const Value: TImageName);
    procedure SetExpandedHotImageIndex(const Value: TImageIndex);
    procedure SetExpandedHotImageName(const Value: TImageName);
    procedure SetExpandedImageIndex(const Value: TImageIndex);
    procedure SetExpandedImageName(const Value: TImageName);
    procedure SetExpandedPressedImageIndex(const Value: TImageIndex);
    procedure SetExpandedPressedImageName(const Value: TImageName);
...
{$ENDIF}
  ...
  public
    ...
    property CollapsedHotImageName: TImageName read FCollapsedHotImageName write SetCollapsedHotImageName;
    property CollapsedImageIndex: TImageIndex read FCollapsedImageIndex write SetCollapsedImageIndex default -1;
    property CollapsedImageName: TImageName read FCollapsedImageName write SetCollapsedImageName;
    ...
    property CollapsedPressedImageName: TImageName read FCollapsedPressedImageName write SetCollapsedPressedImageName;
    property ExpandedHotImageIndex: TImageIndex read FExpandedHotImageIndex write SetExpandedHotImageIndex default -1;
    property ExpandedHotImageName: TImageName read FExpandedHotImageName write SetExpandedHotImageName;
    property ExpandedImageIndex: TImageIndex read FExpandedImageIndex write SetExpandedImageIndex default -1;
    property ExpandedImageName: TImageName read FExpandedImageName write SetExpandedImageName;
    ...
    property ExpandedPressedImageName: TImageName read FExpandedPressedImageName write SetExpandedPressedImageName;
  ...
  published
    ...
    property CollapsedHotImageName;
    property CollapsedImageIndex;
    property CollapsedImageName;
    property CollapsedPressedImageIndex;
    property CollapsedPressedImageName;
    property Ctl3D;
    property UseDockManager default True;
    property DockSite;
    property DoubleBuffered;
    ...
    property ExpandedHotImageName;
    property ExpandedImageIndex;
    property ExpandedImageName;
    property ExpandedPressedImageName;
    property FullRepaint;
    property Font;
...
{$ENDIF}
  ...
  strict protected
    procedure WndProc(var Message: TMessage); override;
    /// <summary>
    ///    Override background painting and font color
    /// </summary>
    procedure CNCtlColorStatic(var Message: TWMCtlColorStatic); message CN_CTLCOLORSTATIC;
  public
    constructor Create(AControl: TWinControl); override;
...
{$ENDIF}
  Vcl.Consts, Vcl.Dialogs, System.Math, Winapi.UxTheme, Winapi.CommCtrl;
...
procedure TImage.FindGraphicClass(Sender: TObject;
...
procedure TImage.CMStyleChanged(var Message: TMessage);
var
  G: TGraphic;
begin
  inherited;
...
begin
  ...
  begin
    ...
    begin
      ...
      Color2 := LStyle.GetSystemColor(clBtnHighlight);
    end
    else
    begin
      Color1 := LStyle.GetSystemColor(clBtnHighlight);
...
class constructor TCustomPanel.Create;
begin
  TCustomStyleEngine.RegisterStyleHook(TCustomPanel, TPanelStyleHook);
end;

class destructor TCustomPanel.Destroy;
begin
  TCustomStyleEngine.UnRegisterStyleHook(TCustomPanel, TPanelStyleHook);
end;
...
procedure TCustomPanel.WMEraseBkgnd(var Message: TWmEraseBkgnd);
begin
  if IsCustomStyleActive and (seClient in StyleElements) and not ParentBackground then
    Message.Result := 1
  else
    inherited;
end;
...
{$ELSE}
      ...
      if not Control.Visible and not (csDesigning in ComponentState) then
        Continue;
{$ENDIF}
      if (csSubComponent in Control.ComponentStyle) and (FControlList.IndexOf(Control.Owner) >= 0) then
        Continue;
      R := TRect.Create(TPoint.Create(Control.Margins.ControlLeft, Control.Margins.ControlTop),
        Control.Margins.ControlWidth, Control.Margins.ControlHeight);
      IntPos := R;
      for J := 0 to Control.ComponentCount - 1 do
        if Control.Components[J] is TControl then
        begin
          Owned := TControl(Control.Components[J]);
          if (csSubComponent in Owned.ComponentStyle) and
             (Owned.Parent = Control.Parent) then
            UnionRect(R, R, TRect.Create(TPoint.Create(Owned.Margins.ControlLeft, Owned.Margins.ControlTop),
              Owned.Margins.ControlWidth, Owned.Margins.ControlHeight));
        end;
      Size.cx := R.Width;
      Size.cy := R.Height;
      IntPos.Offset(-R.Left, -R.Top);
      case FFlowStyle of
        fsLeftRightTopBottom,
        fsLeftRightBottomTop:
      ...
      if Size.cx > MaxWidth then
        MaxWidth := Size.cx;
      Control.Margins.SetControlBounds(Position.X + XDelta + IntPos.Left, Position.Y + YDelta + IntPos.Top, IntPos.Width, IntPos.Height);
...
procedure TCustomFlowPanel.CMShowingChanged(var Message: TMessage);
begin
  inherited;
  if Showing then AdjustSize;
end;
...
procedure TCustomFlowPanel.FlipChildren(AllLevels: Boolean);
const
  CFlippedStyle: array [TFlowStyle] of TFlowStyle =
    (fsRightLeftTopBottom, fsLeftRightTopBottom,
     fsRightLeftBottomTop, fsLeftRightBottomTop,
     fsTopBottomRightLeft, fsBottomTopRightLeft,
     fsTopBottomLeftRight, fsBottomTopLeftRight);
begin
  FlowStyle := CFlippedStyle[FlowStyle];
  inherited FlipChildren(AllLevels);
end;

{ TCustomGridPanel }

function TCustomGridPanel.GetControlBounds(AControl: TControl; AUseExplicitSize: Boolean;
...
procedure TCustomGridPanel.ScaleForPPI(NewPPI: Integer);
var
  I: Integer;
begin
  if newPPI = 0 then
...
var
  ...
  PercentX, PercentY, Delta, NewValue: Double;
  ControlItem: TControlItem;
  IntPos: TRect;
begin
    ...
    for I := 0 to FColumnCollection.Count - 1 do
      with FColumnCollection[I] do
        ...
        begin
          ...
          Inc(PercentXCount);
          // When all items are percentual and difference is relatively small then
          // it is probably due to rounding errors. Set the total to 100% to avoid
          // recalculation and growing of rounding errors.
          if (PercentXCount = FColumnCollection.Count) and
             SameValue(PercentX, 100, 0.05 * FColumnCollection.Count) and
             (Value > (PercentX - 100)) then
          begin
            Value := Value - (PercentX - 100);
            PercentX := 100;
          end;
        end
        else
        begin
          ...
          begin
            ...
            if (ControlItem <> nil) and (ControlItem.Control <> nil) and
               (ControlItem.Column = I) and (ControlItem.Row = J) then
            begin
              LSize := GetControlBounds(ControlItem.Control, False, IntPos).Width + Self.Padding.Left + Self.Padding.Right;
    ...
    for I := 0 to FRowCollection.Count - 1 do
      with FRowCollection[I] do
        ...
        begin
          ...
          // it is probably due to rounding errors. Set the total to 100% to avoid
          // recalculation and growing of rounding errors.
          if (PercentYCount = FRowCollection.Count) and
             SameValue(PercentY, 100, 0.05 * FRowCollection.Count) and
             (Value > (PercentY - 100)) then
          begin
            Value := Value - (PercentY - 100);
            PercentY := 100;
          end;
        end else
        begin
          ...
          begin
            ...
            begin
              LSize := GetControlBounds(ControlItem.Control, False, IntPos).Height + Self.Padding.Top + Self.Padding.Bottom;
              if LSize > MaxSize then
                MaxSize := LSize;
            end;
          end;
    ...
    for I := 0 to FColumnCollection.Count - 1 do
      with FColumnCollection[I] do
        ...
        begin
          ...
          else if RecalcCellPercentsEnabled and not SameValue(PercentX, 100.0, 0.0001) then
            Value := (Value / PercentX) * 100.0;
          NewValue := XSize * (Value / 100.0);
          Size := Round(NewValue + Delta);
          Delta := Delta + NewValue - Size;
          Dec(RemainingX, Size);
    ...
    for I := 0 to FRowCollection.Count - 1 do
      with FRowCollection[I] do
        ...
        begin
          ...
          else if RecalcCellPercentsEnabled and not SameValue(PercentY, 100.0, 0.0001) then
            Value := (Value / PercentY) * 100.0;
          NewValue := YSize * (Value / 100.0);
          Size := Round(NewValue + Delta);
          Delta := Delta + NewValue - Size;
          Dec(RemainingY, Size);
...
procedure TCustomGridPanel.WMSize(var Message: TWMSize);
begin
  FRecalcCellPercentsDisabled := True;
  inherited;
end;

function TCustomGridPanel.GetRecalcCellSizesEnabled: Boolean;
begin
  Result := FRecalcCellSizesDisabled = 0;
end;

function TCustomGridPanel.GetRecalcCellPercentsEnabled: Boolean;
begin
  Result := not FRecalcCellPercentsDisabled;
end;

procedure TCustomGridPanel.BeginUpdate;
begin
...
procedure TCustomGridPanel.EndUpdate;
begin
...
procedure TCustomGridPanel.FlipChildren(AllLevels: Boolean);
var
  I, J, K: Integer;
  LCol: TCellItem;
  LCtrl: TControl;
...
begin
  if (Owner.RowCollection.Count = 0) and (Owner.ExpandStyle = emAddColumns) or
     (Owner.ColumnCollection.Count = 0) and (Owner.ExpandStyle = emAddRows) then
...
begin
  if Dest is TControlItem then
    ...
    begin
      ...
      FRow := Self.Row;
      FColumn := Self.Column;
      FRowSpan := Self.RowSpan;
      FColumnSpan := Self.ColumnSpan;
      Changed(False);
    end;
end;
...
procedure THeader.ChangeScale(M, D: Integer; isDpiChange: Boolean);
var
  I: Integer;
begin
  inherited ChangeScale(M, D, isDpiChange);
  for I := 0 to FSections.Count - 1 do
    THeaderSectionType(THeaderStrings(FSections).FList[I]).Width :=
      MulDiv(THeaderSectionType(THeaderStrings(FSections).FList[I]).Width, M, D);
end;
...
procedure TGroupButton.SetChecked(Value: Boolean);
begin
  inherited;
  if Value and not TCustomRadioGroup(Parent).RadioTabStop then
    TabStop := False
end;
...
begin
  ...
  begin
    DC := GetDC(0);
    SaveFont := SelectObject(DC, HeaderFont.Handle);
...
procedure TCustomRadioGroup.SetRadioTabStop(Value: Boolean);
begin
  if Value <> FRadioTabStop then
  begin
    FRadioTabStop := Value;
    if [csDesigning, csLoading, csReading] * ComponentState = [] then
      if FItemIndex >= 0 then
        TGroupButton(FButtons[FItemIndex]).TabStop := Value;
  end;
end;
...
begin
  ...
  begin
    ...
    TGroupButton(FButtons[I]).StyleElements := StyleElements;
  end;
  if FItemIndex >= 0 then
  begin
    FUpdating := True;
...
procedure TCustomRadioGroup.CheckDefaultHeaderFont;
begin
  inherited;
  if DefaultHeaderFont then
    UpdateButtons;
end;
  ...
  begin
    ...
    begin
      ...
      if IsCustomStyleActive then
        FBrush.Bitmap := AllocPatternBitmap(clBlack, StyleServices(Self).GetStyleColor(scSplitter))
      else
...
begin
  ...
  begin
    ...
    if Vertical then
      ...
      InflateRect(R, 2, -1);
    OffsetRect(R, 1, 1);
    if not (csDesigning in ComponentState) and TStyleManager.IsCustomStyleActive and (seBorder in StyleElements) then
      FrameBrush := CreateSolidBrush(ColorToRGB(LStyle.GetSystemColor(clBtnHighlight)))
    else
      FrameBrush := CreateSolidBrush(ColorToRGB(clBtnHighLight));
    if Vertical and (R.Width = 2) then
      FrameRect(Canvas.Handle, Rect(R.Left + 1, R.Top, R.Right, R.Bottom), FrameBrush)
    else
    if not Vertical and (R.Height = 2) then
      FrameRect(Canvas.Handle, Rect(R.Left, R.Top + 1, R.Right, R.Bottom), FrameBrush)
    else
      FrameRect(Canvas.Handle, R, FrameBrush);
    DeleteObject(FrameBrush);
    OffsetRect(R, -2, -2);
    if not (csDesigning in ComponentState) and TStyleManager.IsCustomStyleActive and (seBorder in StyleElements) then
      FrameBrush := CreateSolidBrush(ColorToRGB(LStyle.GetSystemColor(clBtnShadow)))
    else
...
procedure TCustomControlBar.ResetDockItems;
var
  I: Integer;
begin
...
procedure TCustomControlBar.ScaleControlsForDpi(NewPPI: Integer);
begin
  FAligning := True;
  try
    inherited;
...
procedure TCustomControlBar.ChangeScale(M, D: Integer; isDpiChange: Boolean);
begin
  FRowSize := MulDiv(FRowSize, M, D);
  inherited;
end;
...
begin
  ...
  LPPI := CurrentPPI;
  Insets.Left := MulDiv(Insets.Left, LPPI, Screen.DefaultPixelsPerInch);
...
var
  ...
  begin
    ...
    if not TStyleManager.IsCustomStyleActive or (csDesigning in ComponentState) then
...
begin
  ...
  begin
    ...
    begin
      if LStyle.Enabled then
      begin
        ElemDetails := LStyle.GetElementDetails(trGripper);
        if not (csDesigning in ComponentState) and TStyleManager.IsCustomStyleActive then
          LRect := Rect(ARect.Left + 2, ARect.Top + 2, ARect.Left + MulDiv(10, LPPI, Screen.DefaultPixelsPerInch), ARect.Bottom - 3)
        else
          LRect := Rect(ARect.Left + 2, ARect.Top + 2, ARect.Left + MulDiv(7, LPPI, Screen.DefaultPixelsPerInch), ARect.Bottom - 3);
        SaveIndex := SaveDC(Canvas.Handle);
        try
          LStyle.DrawElement(Canvas.Handle, ElemDetails, LRect, nil, LPPI);
        finally
...
begin
  ...
  begin
    if not (csDesigning in ComponentState) and TStyleManager.IsCustomStyleActive then
      Canvas.Brush.Color := StyleServices(Self).GetStyleColor(scWindow)
    else
      Canvas.Brush.Color := Color;
...
begin
  ...
  if (Owner is TCustomLabeledEdit) and AutoSize then
    TCustomLabeledEdit(Owner).UpdateLabelPosition;
end;
...
procedure TBoundLabel.CMTextChanged(var Message: TMessage);
begin
  inherited;
...
class function TCustomLabeledEdit.WithLabel: Boolean;
begin
  Result := True;
end;

class function TCustomLabeledEdit.GetLabelClass: TBoundLabelClass;
begin
  Result := TBoundLabelEx;
end;
...
procedure TCustomLabeledEdit.CMAllChildrenFlipped(var Message: TMessage);
begin
  inherited;
...
procedure TCustomLabeledEdit.UpdateLabelPosition;
var
  P: TPoint;
  S: TSize;
  L: Integer;
  ...
  case LabelPosition of
    lpAbove:
      case AdjustedAlignment(UseRightToLeftAlignment, Alignment) of
        ...
        taRightJustify: P := Point(Left + Width - S.Width,
          Top - S.Height - LSpacing);
        taCenter: P := Point(Left + (Width - S.Width) div 2,
          Top - S.Height - LSpacing);
      end;
    lpBelow:
      case AdjustedAlignment(UseRightToLeftAlignment, Alignment) of
...
const
  cColorStates: array[Boolean] of TStyleColor = (scEditDisabled, scEdit);
var
  LIndex: Integer;
begin
  inherited;
  if FButton.FEditControl.IsCustomStyleActive and (seClient in FButton.FEditControl.StyleElements) then
    Canvas.Brush.Color := StyleServices(FButton.FEditControl).GetStyleColor(cColorStates[FButton.FEditControl.Enabled])
  else
    Canvas.Brush.Color := FButton.FEditControl.Color;
  Canvas.FillRect(ClientRect);
  ...
  begin
    ...
    if LIndex <> -1 then
    begin
      if FButton.Images.Height < FButton.FEditControl.ClientHeight then
        Top := (FButton.FEditControl.ClientHeight - FButton.Images.Height) div 2;
...
procedure TEditButton.CheckImageIndexes;
begin
...
procedure TEditButton.UpdateImageIndex(Name: TImageName; var Index: TImageIndex);
begin
  if (Images <> nil) and Images.IsImageNameAvailable then
    Index := Images.GetIndexByName(Name);
end;

procedure TEditButton.UpdateImageName(Index: TImageIndex; var Name: TImageName);
begin
  if (Images <> nil) and Images.IsImageNameAvailable then
...
procedure TEditButton.SetDisabledImageName(const Value: TImageName);
begin
...
procedure TEditButton.SetHotImageName(const Value: TImageName);
begin
...
procedure TEditButton.SetImageName(const Value: TImageName);
begin
...
procedure TEditButton.SetPressedImageName(const Value: TImageName);
begin
...
begin
  if FGlyph <> nil then
  begin
    LStyle := StyleServices(FEditControl);
    if Images <> nil then
    begin
...
begin
  if HandleAllocated then
  begin
    FLeftButton.CheckImageIndexes;
    FLeftButton.UpdateBounds;
    FRightButton.CheckImageIndexes;
    ...
    CN_CTLCOLOREDIT:
      ...
      begin
        ...
        begin
          ...
          if StyleServices(Self).Enabled and Ctl3D then
          begin
            Inc(LLeft);
            Inc(LTop);
          end;
...
procedure TCustomButtonedEdit.ChangeScale(M, D: Integer; isDpiChange: Boolean);
begin
  inherited;
  LeftButton.UpdateBounds;
  RightButton.UpdateBounds;
...
begin
  with Canvas do
  begin
    if TStyleManager.IsCustomStyleActive and (odSelected in State) and
      (seFont in StyleElements) then
    begin
    ...
    if TStyleManager.IsCustomStyleActive and (odSelected in State) and
      (seFont in StyleElements) then
      Brush.Color := StyleServices(Self).GetSystemColor(clHighlightText)
    else
    ...
    if TStyleManager.IsCustomStyleActive and (odFocused in State) then
      DrawFocusRect(OrgRect);
  end;
end;
...
begin
  ...
  LStyle := StyleServices(Self);
  LPPI := CurrentPPI;
  Offset := MulDiv(5, LPPI, Screen.DefaultPixelsPerInch);
  if (FPanelGroup.HeaderStyle = hsThemed) and LStyle.Enabled then
  begin
    LStyle.GetElementSize(Canvas.Handle,
      LStyle.GetElementDetails(ThemedChevronState[FHeaderState, FCollapsed]), esActual, LSize, LPPI);
    LHeight := LSize.cy;
    LWidth := LSize.cx;
  end
  else
  begin
    ...
    begin
      LHeight := Round(16 * LPPI / Screen.DefaultPixelsPerInch);
      LWidth := Round(16 * LPPI / Screen.DefaultPixelsPerInch);
    end;
  end;
...
begin
  ...
  if LStyle.Enabled then
  begin
    if LStyle.GetElementColor(LStyle.GetElementDetails(Parts[FPanelGroup.HeaderStyle = hsThemed]),
        ecTextColor, LTextColor) then
...
begin
  ...
  if (FPanelGroup.HeaderStyle = hsThemed) and LStyle.Enabled then
    LStyle.DrawElement(ACanvas.Handle,
      LStyle.GetElementDetails(ThemedChevron[FHeaderState, FCollapsed]), FGlyphRect, nil, LPPI)
  else
  begin
    if UsingImageList then
    begin
    ...
    begin
      // draw chevron
      LChevronSize := Round(6 * LPPI / Screen.DefaultPixelsPerInch);
      LPoint.X := FGlyphRect.Left + ((FGlyphRect.Right - FGlyphRect.Left  - LChevronSize) div 2) - 1;
      LPoint.Y := (HeaderHeight - LChevronSize) div 2;
      if FCollapsed then
        Inc(LPoint.X, LChevronSize div 3)
      else
        Inc(LPoint.Y, LChevronSize div 3);
      ...
      begin
        ...
        DrawChevron(ACanvas, ChevronDirection[AdjustedAlignment(UseRightToLeftAlignment,
          FPanelGroup.ChevronAlignment), FCollapsed], LPoint, LChevronSize div 2);
      end;
    end;
  end;
end;
...
begin
  FPanelSurface.GetChildren(Proc, Root);
end;
...
procedure TCustomCategoryPanel.CheckImageIndexes;
begin
  if (FPanelGroup = nil) or (FPanelGroup.Images = nil) or not FPanelGroup.Images.IsImageNameAvailable then
    Exit;
  FPanelGroup.Images.CheckIndexAndName(FCollapsedImageIndex, FCollapsedImageName);
  FPanelGroup.Images.CheckIndexAndName(FCollapsedHotImageIndex, FCollapsedHotImageName);
  FPanelGroup.Images.CheckIndexAndName(FCollapsedPressedImageIndex, FCollapsedPressedImageName);
  FPanelGroup.Images.CheckIndexAndName(FExpandedImageIndex, FExpandedImageName);
  FPanelGroup.Images.CheckIndexAndName(FExpandedHotImageIndex, FExpandedHotImageName);
  FPanelGroup.Images.CheckIndexAndName(FExpandedPressedImageIndex, FExpandedPressedImageName);
end;

procedure TCustomCategoryPanel.UpdateImageIndex(Name: TImageName; var Index: TImageIndex);
begin
  if (FPanelGroup <> nil) and (FPanelGroup.Images <> nil) and FPanelGroup.Images.IsImageNameAvailable then
    Index := FPanelGroup.Images.GetIndexByName(Name);
end;

procedure TCustomCategoryPanel.UpdateImageName(Index: TImageIndex; var Name: TImageName);
begin
  if (FPanelGroup <> nil) and (FPanelGroup.Images <> nil) and FPanelGroup.Images.IsImageNameAvailable then
    Name := FPanelGroup.Images.GetNameByIndex(Index);
end;
...
begin
  ...
  begin
    ...
    UpdateImageName(Value, FCollapsedHotImageName);
...
procedure TCustomCategoryPanel.SetCollapsedHotImageName(const Value: TImageName);
begin
  if Value <> FCollapsedHotImageName then
  begin
    FCollapsedHotImageName:= Value;
    UpdateImageIndex(Value, FCollapsedHotImageIndex);
    if not (csLoading in ComponentState) and (PanelGroup.HeaderStyle = hsImage) and
       Collapsed and (FHeaderState = hsHot) then
      if FCollapsedHotImageIndex <> -1 then
        UpdateButtonState
...
begin
  ...
  begin
    ...
    UpdateImageName(Value, FCollapsedImageName);
    if not (csLoading in ComponentState) and (PanelGroup.HeaderStyle = hsImage) and
...
procedure TCustomCategoryPanel.SetCollapsedImageName(const Value: TImageName);
begin
  if Value <> FCollapsedImageName then
  begin
    FCollapsedImageName := Value;
    UpdateImageIndex(Value, FCollapsedImageIndex);
    if not (csLoading in ComponentState) and (PanelGroup.HeaderStyle = hsImage) and
       Collapsed and (FHeaderState = hsNormal) then
      if FCollapsedImageIndex <> -1 then
        UpdateButtonState
      else
        UpdateHeader;
  end;
...
begin
  ...
  begin
    ...
    UpdateImageName(Value, FCollapsedPressedImageName);
    if not (csLoading in ComponentState) and (PanelGroup.HeaderStyle = hsImage) and
...
procedure TCustomCategoryPanel.SetCollapsedPressedImageName(const Value: TImageName);
begin
  if Value <> FCollapsedPressedImageName then
  begin
    FCollapsedPressedImageName := Value;
    UpdateImageIndex(Value, FCollapsedPressedImageIndex);
    if not (csLoading in ComponentState) and (PanelGroup.HeaderStyle = hsImage) and
       Collapsed and (FHeaderState = hsPressed) then
      if FCollapsedPressedImageIndex <> -1 then
        UpdateButtonState
...
begin
  ...
  begin
    ...
    UpdateImageName(Value, FExpandedHotImageName);
    if not (csLoading in ComponentState) and (PanelGroup.HeaderStyle = hsImage) and
...
procedure TCustomCategoryPanel.SetExpandedHotImageName(const Value: TImageName);
begin
  if Value <> FExpandedHotImageName then
  begin
    FExpandedHotImageName := Value;
    UpdateImageIndex(Value, FExpandedHotImageIndex);
    if not (csLoading in ComponentState) and (PanelGroup.HeaderStyle = hsImage) and
       not Collapsed and (FHeaderState = hsHot) then
      if FExpandedHotImageIndex <> -1 then
        UpdateButtonState
...
begin
  ...
  begin
    ...
    UpdateImageName(Value, FExpandedImageName);
    if not (csLoading in ComponentState) and (PanelGroup.HeaderStyle = hsImage) and
...
procedure TCustomCategoryPanel.SetExpandedImageName(const Value: TImageName);
begin
  if Value <> FExpandedImageName then
  begin
    FExpandedImageName := Value;
    UpdateImageIndex(Value, FExpandedImageIndex);
    if not (csLoading in ComponentState) and (PanelGroup.HeaderStyle = hsImage) and
       not Collapsed and (FHeaderState = hsNormal) then
      if FExpandedImageIndex <> -1 then
        UpdateButtonState
      else
        UpdateHeader;
  end;
...
begin
  ...
  begin
    ...
    UpdateImageName(Value, FExpandedPressedImageName);
    if not (csLoading in ComponentState) and (PanelGroup.HeaderStyle = hsImage) and
...
procedure TCustomCategoryPanel.SetExpandedPressedImageName(const Value: TImageName);
begin
  if Value <> FExpandedPressedImageName then
  begin
    FExpandedPressedImageName := Value;
    UpdateImageIndex(Value, FExpandedPressedImageIndex);
    if not (csLoading in ComponentState) and (PanelGroup.HeaderStyle = hsImage) and
       not Collapsed and (FHeaderState = hsPressed) then
      if FExpandedPressedImageIndex <> -1 then
        UpdateButtonState
...
var
  LPoint: TPoint;
begin
  if (Message.DC = 0) or (not (csDesigning in ComponentState) and TStyleManager.IsCustomStyleActive) then
  ...
  case Message.Msg of
    ...
    begin
      if not (csDesigning in ComponentState) and TStyleManager.IsCustomStyleActive and
        StyleServices(Self).Enabled then
      begin
        ...
        begin
          ...
          try
            ...
            begin
              ...
              if LStyle.GetElementColor(LStyle.GetElementDetails(tpPanelBackground),
...
procedure TCustomCategoryPanelGroup.ChangeScale(M, D: Integer; isDpiChange: Boolean);
var
  I: Integer;
...
begin
  ...
  HeaderFont.Height := MulDiv(HeaderFont.Height, M, D);
  for I := 0 to FPanels.Count - 1 do
  begin
    LPanel := TCustomCategoryPanel(FPanels[I]);
    if LPanel.Collapsed then
      LPanel.FExpandedHeight := MulDiv(LPanel.FExpandedHeight, M, D);
  end;
end;
...
begin
  LStyle := StyleServices(Self);
  if LStyle.Enabled and not (IsCustomStyleActive and not (seClient in StyleElements)) and
     LStyle.GetElementColor(LStyle.GetElementDetails(tcpBackground),
       ecFillColor, LColor) and (LColor <> clNone) then
  begin
...
begin
  inherited CreateParams(Params);
  if StyleServices(Self).Available then
  begin
...
procedure TLinkLabelStyleHook.CNCtlColorStatic(var Message: TWMCtlColorStatic);
var
  LStyle: TCustomStyleServices;
begin
  LStyle := StyleServices;
  SetBkMode(TWMCtlColorStatic(Message).ChildDC, Winapi.Windows.TRANSPARENT);
  LStyle.DrawParentBackground(Handle, TWMCtlColorStatic(Message).ChildDC, nil, False);
  SetTextColor(TWMCtlColorStatic(Message).ChildDC, LStyle.GetSystemColor(clBtnText));
  Message.Result := GetStockObject(NULL_BRUSH);
  Handled := True;
end;
...
constructor TPanelStyleHook.Create(AControl: TWinControl);
begin
  inherited;
  OverridePaint := True;
...
function TPanelStyleHook.AcceptMessage(var Message: TMessage): Boolean;
begin
  Result := inherited AcceptMessage(Message);
  if Result then
    Result := TStyleManager.UseParentPaintBuffers and (Control is TCustomPanel) and TCustomPanel(Control).ParentBackground;
end;

procedure TPanelStyleHook.WndProc(var Message: TMessage);
begin
  // Reserved for potential updates
  inherited;
end;

procedure TPanelStyleHook.PaintBackground(Canvas: TCanvas);
var
...
procedure TPanelStyleHook.Paint(Canvas: TCanvas);
begin
  if not TStyleManager.UseParentPaintBuffers then
    PaintBackground(Canvas);

  if Control is TCustomPanel then
    TCustomPanel(Control).PaintWindow(Canvas.Handle);
end;
