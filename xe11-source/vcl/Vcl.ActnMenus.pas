...
uses
  ...
  Vcl.Buttons, Vcl.Graphics, Vcl.Forms, Vcl.Controls, Vcl.Themes,
...
{ TCustomActionMenuBar }
  ...
  public
    ...
    function GetDevicePPI: Integer; virtual;
...
{ TCustomActionPopupMenu }
  ...
  public
    ...
    function GetDevicePPI: Integer; override;
    function StyleServices: TCustomStyleServices; override;
...
{ TCustomMenuItem }
  ...
  public
    ...
    function GetDevicePPI: Integer; virtual;
    function StyleSerives: TCustomStyleServices; virtual;
...
var
  EnableWMGETOBJECT: Boolean = True;
...
procedure TCustomActionMenuBar.ScaleControlsForDpi(NewPPI: Integer);
begin
// currently do nothing, just hides inherited TWinControl.ScaleControlsForDpi and prevents it be executed avoiding the inproper scaling.
end;
...
function TCustomActionMenuBar.GetDevicePPI: Integer;
begin
  Result := CurrentPPI;
end;
...
{$IF NOT DEFINED(CLR)}
    WM_GETOBJECT:
      if EnableWMGETOBJECT and (Message.LParam = Winapi.Windows.LPARAM(OBJID_CLIENT)) and (ActionClient <> nil) then
      begin
...
procedure TCustomActionPopupMenu.AdjustFont;
begin
end;
...
function TCustomActionPopupMenu.GetDevicePPI: Integer;
begin
  if Owner is TCustomActionMainMenuBar then
    Result := TCustomActionMainMenuBar(Owner).CurrentPPI
  else
...
function TCustomActionPopupMenu.StyleServices: TCustomStyleServices;
var
  LControl: TControl;
begin
  if Owner is TCustomActionMainMenuBar then
    LControl := TCustomActionMainMenuBar(Owner)
  else
    LControl := Screen.ActiveCustomForm;
  if csDesigning in LControl.ComponentState then
    LControl := Application.MainForm;

  Result := Vcl.Themes.StyleServices(LControl);
end;
...
begin
  ...
  if LStyle.Enabled and not LStyle.IsSystemStyle then
    LStyle.DrawElement(Canvas.Handle, LStyle.GetElementDetails(tmPopupBackground),
...
begin
  ...
  begin
    ...
    try
      LStyle.DrawElement(Canvas.Handle, LStyle.GetElementDetails(tmPopupBorders),
       Rect(0, 0, Width, Height));
    finally
...
procedure TCustomActionMainMenuBar.ChangeScale(M, D: Integer; isDpiChange: Boolean);
var
  I: Integer;
begin
  inherited;
  ...
  begin
    for I := 0 to ItemCount - 1 do
      TCustomActionControlClass(ActionControls[I]).ChangeScale(M, D);
    AdjustItems;
  end;
end;
...
begin
  ...
  end else if LStyle.Enabled and not LStyle.IsSystemStyle then
  begin
...
function TCustomMenuItem.GetDevicePPI: Integer;
begin
  if Assigned(Menu) then
    Result := Menu.GetDevicePPI
  else
    Result := Screen.PixelsPerInch;
end;

function TCustomMenuItem.StyleSerives: TCustomStyleServices;
var
  LControl: TControl;
begin
  if Assigned(Menu) then
...
begin
  ...
  TempHeight := MulDiv(FCYMenu, GetDevicePPI, Screen.PixelsPerInch);
  AHeight := TempHeight;
  if Separator then
    AHeight := TempHeight div 3 * 2
  else
    // 16 is standard image size so adjust for larger images
    if ImageSize.Y > 16 then
      AHeight := ImageSize.Y + cEdgeSize;
  ...
  if (AHeight > 0) and (AHeight <= TextBounds.Height) then
    AHeight := TextBounds.Height + cEdgeSize;
...
begin
  ...
  if not ActionBar.DesignMode and not Selected and Menu.InMenuLoop and Enabled then
  begin
    FDoClick := True;
    Click;
  end;
