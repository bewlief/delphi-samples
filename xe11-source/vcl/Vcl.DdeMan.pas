...
{$ENDIF}
  System.Types, System.SysUtils, Vcl.Dialogs, Vcl.Controls, Vcl.Consts, Vcl.Clipbrd, System.AnsiStrings;
...
{$ELSE}
  if FHszApp = 0 then
  begin
    System.AnsiStrings.StrPCopy(CharVal, AnsiString(DdeService));
    FHszApp := DdeCreateStringHandleA(ddeMgr.DdeInstId, CharVal, CP_WINANSI);
  end;
  if FHszTopic = 0 then
  begin
    System.AnsiStrings.StrPCopy(CharVal, AnsiString(DdeTopic));
...
{$ELSE}
        System.AnsiStrings.StrPCopy(CharVal, AnsiString(ServiceApplication))
      else
        System.AnsiStrings.StrPCopy(CharVal, AnsiString(DdeService + ' ' + DdeTopic));
...
begin
  Result := False;
  if (FConv = 0) or FWaitStat then Exit;
  hszCmd := DdeCreateDataHandle(ddeMgr.DdeInstId, Cmd, System.AnsiStrings.StrLen(Cmd) + 1,
    0, 0, FDdeFmt, 0);
...
begin
  ...
  hszDat := DdeCreateDataHandle(ddeMgr.DdeInstId, Data, System.AnsiStrings.StrLen(Data) + 1,
    0, hszItem, FDdeFmt, 0);
  if hszDat <> 0 then
  begin
