...
uses
  System.SysUtils, Vcl.GraphUtil, Winapi.UxTheme, Vcl.Consts, Vcl.Forms;
...
const
  ...
  cEnterIconWidth = 36;
  cEnterIconHeight = 12;
  cTallEnterIconWidth = 24;
  cTallEnterIconHeight = 12;
  cBackspaceIconWidth = 24;
  cBackspaceIconHeight = 12;
  cTabIconWidth = 36;
  cTabIconHeight = 24;
  cArrowIconSize = 8;

var
  LIconLeftOffset, LIconTopOffset: Integer;

  procedure DrawArrowImage(ACanvas: TCanvas; AX, AY: Integer; ADirection: TKeyImage);
  var
    i: Integer;
    LSize: Integer;
    LSavePenWidth: Integer;
  begin
    LSize := Round(2 * FParent.ScaleFactor);
    if not Odd(Trunc(FParent.ScaleFactor)) then
      LSize := LSize + 1;

    LSavePenWidth := ACanvas.Pen.Width;
    ACanvas.Pen.Width := 1;
    with ACanvas do
    begin
      case ADirection of
  ...
  ACanvas.Pen.Width := LSavePenWidth;
end;

  procedure PaintTab(ACanvas: TCanvas; ARect: TRect);
  var
    R: TRect;
    LArrowSize: Integer;
  begin
    R := Rect(0, 0, FParent.ScaleValue(cTabIconWidth), FParent.ScaleValue(cTabIconHeight));
    R.Offset(ARect.Left + LIconLeftOffset, ARect.Top + LIconTopOffset);
    LArrowSize := FParent.ScaleValue(2);

    ACanvas.MoveTo(R.Left + LArrowSize + ACanvas.Pen.Width, R.Top + R.Height div 6);
    ACanvas.LineTo(R.Right, R.Top + R.Height div 6);
    ACanvas.MoveTo(R.Left, R.Top + R.Height div 6 - R.Height div 5);
    ACanvas.LineTo(R.Left, R.Top + R.Height div 6 + R.Height div 5 + 1);
    DrawArrowImage(ACanvas, R.Left + ACanvas.Pen.Width, R.Top + R.Height div 6, kiLeft);

    ACanvas.MoveTo(R.Left, R.Bottom - R.Height div 2);
    ACanvas.LineTo(R.Right - LArrowSize - ACanvas.Pen.Width, R.Bottom - R.Height div 2);

    ACanvas.MoveTo(R.Right, R.Bottom - R.Height div 2 - R.Height div 5);
    ACanvas.LineTo(R.Right, R.Bottom - R.Height div 2 + R.Height div 5);
    DrawArrowImage(ACanvas, R.Right - ACanvas.Pen.Width , R.Bottom - R.Height div 2, kiRight);
  end;


  procedure PaintEnter(ACanvas: TCanvas; ARect: TRect);
  var
    R: TRect;
  begin
    R := Rect(0, 0, FParent.ScaleValue(cEnterIconWidth), FParent.ScaleValue(cEnterIconHeight));
    R.Offset(ARect.Left + LIconLeftOffset, ARect.Top + LIconTopOffset);

    ACanvas.MoveTo(R.Left + FParent.ScaleValue(2), R.Top + R.Height div 2);
    ACanvas.LineTo(R.Right, R.Top + R.Height div 2);
    ACanvas.LineTo(R.Right, R.Top);
    DrawArrowImage(ACanvas, R.Left, R.Top + R.Height div 2, kiLeft);
  end;

  procedure PaintTallEnter(ACanvas: TCanvas; ARect: TRect);
  var
    R: TRect;
  begin
    R := Rect(0, 0, FParent.ScaleValue(cTallEnterIconWidth), FParent.ScaleValue(cTallEnterIconHeight));
    R.Offset(ARect.Left + LIconLeftOffset, ARect.Top + LIconTopOffset);
  ...
  procedure PaintBackspace(ACanvas: TCanvas; ARect: TRect);
  var
    R: TRect;
  begin
    R := Rect(0, 0, FParent.ScaleValue(cBackspaceIconWidth), FParent.ScaleValue(cBackspaceIconHeight));
    R.Offset(ARect.Left + LIconLeftOffset, ARect.Top + LIconTopOffset);
  ...
  procedure PaintShift(ACanvas: TCanvas; ARect: TRect);
  begin
    ACanvas.MoveTo(ARect.Left + LIconLeftOffset, ARect.Top + FParent.ScaleValue(4 + 12));
    ACanvas.LineTo(ARect.Left + LIconLeftOffset + FParent.ScaleValue(10), ARect.Top + FParent.ScaleValue(6));
    ACanvas.LineTo(ARect.Left + LIconLeftOffset + FParent.ScaleValue(20), ARect.Top + FParent.ScaleValue(4 + 12));
    ACanvas.LineTo(ARect.Left + LIconLeftOffset + FParent.ScaleValue(14), ARect.Top + FParent.ScaleValue(4 + 12));
    ACanvas.LineTo(ARect.Left + LIconLeftOffset + FParent.ScaleValue(14), ARect.Top + FParent.ScaleValue(4 + 18));
    ACanvas.LineTo(ARect.Left + LIconLeftOffset + FParent.ScaleValue(6), ARect.Top + FParent.ScaleValue(4 + 18));
    ACanvas.LineTo(ARect.Left + LIconLeftOffset + FParent.ScaleValue(6), ARect.Top + FParent.ScaleValue(4 + 12));
    ACanvas.LineTo(ARect.Left + LIconLeftOffset, ARect.Top + FParent.ScaleValue(4 + 12));
  end;

  procedure PaintArrow(ACanvas: TCanvas; ARect: TRect; ADirection: TKeyImage);
  const
    ICON_WIDTH = 8;
    ICON_HEIGHT = 8;
  var
  ...
  begin
    ...
    LIconSize := FParent.ScaleValue(cArrowIconSize);
    LArrowSize := FParent.ScaleValue(2);
    case ADirection of
      kiUp:
      begin
        // Vertical line
        ACanvas.MoveTo(X, Y - LIconSize + LArrowSize);
        ACanvas.LineTo(X, Y + LIconSize);
        Y := Y - LIconSize;
      end;
      kiDown:
      begin
        // Vertical line
        ACanvas.MoveTo(X, Y - LIconSize);
        ACanvas.LineTo(X, Y + LIconSize - LArrowSize);
        Y := Y + LIconSize;
      end;
      kiLeft:
      begin
        // Vertical line
        ACanvas.MoveTo(X - LIconSize + LArrowSize, Y);
        ACanvas.LineTo(X + LIconSize, Y);
        X := X - LIconSize;
      end;
      kiRight:
      begin
        ACanvas.MoveTo(X - LIconSize, Y);
        ACanvas.LineTo(X + LIconSize - LArrowSize, Y);
        X := X + LIconSize;
      end;
    end;
    DrawArrowImage(ACanvas, X, Y, ADirection);
  end;
...
begin
  ...
  try
    ...
    try
      ...
      LCanvas.Font.Height := MulDiv(LCanvas.Font.Height, FParent.CurrentPPI, Screen.PixelsPerInch);
      LCanvas.Pen.Width := Trunc(FParent.ScaleFactor);
      if not Odd(LCanvas.Pen.Width) then
        LCanvas.Pen.Width := LCanvas.Pen.Width + 1;
      LIconLeftOffset := FParent.ScaleValue(ICON_LEFT_OFFSET);
      LIconTopOffset := FParent.ScaleValue(ICON_TOP_OFFSET);
      ...
      case KeyImage of
        ...
        begin
          if not Parent.GetOverrideCaption(Key, LCaption) then
            LCaption := Caption;
          LCanvas.TextOut(LRect.Left + LIconLeftOffset, LRect.Top +  LIconTopOffset, LCaption);
        end;
        kiText: LCanvas.TextOut(LRect.Left +  LIconLeftOffset, LRect.Top +  LIconTopOffset, Caption);
...
var
  Index: Integer;
  Button: TCustomKeyboardButton;
  CapsLock: TCustomKeyboardButton;
begin
  FDeadKey := VKey(-1, -1);
  FUnclick.Clear;
  ...
  begin
    ...
    if ((Button.Key.Vk = VK_CAPITAL) or (Button.Key.ScanCode = CapsLockScanCode)) then
      CapsLock := Button;
    Button.Down := False;
  ...
  FPressedKeys.Clear;
  if Assigned(CapsLock) then
  begin
    CapsLock.Down := True;
    CapsLock.SetState(TCustomKeyboardButton.TDrawState.dsPressed, False);
    FUnclick.Add(CapsLock);
    FPressedKeys.Add(CapsLock);
  end;
end;
