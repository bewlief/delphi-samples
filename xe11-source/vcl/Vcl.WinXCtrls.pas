...
const
  ...
  DefaultSearchButtonImageSize = 16;
...
type
  ...
  private
    ...
    // Align all controls relative to the panel
    procedure AlignControlsToPanel(APanelRect: TRect);
    // Align all controls relative to each other
    procedure AlignControlsToSibling;
  protected
...
begin
  ...
  if StyleServices(Self).Enabled and (M <> D) then
  begin
  ...
  begin
    ...
    case FState of
      tssOff:
        begin
          if Enabled then
            Details := LStyle.GetElementDetails(ttsTrackOffNormal)
          else
            Details := LStyle.GetElementDetails(ttsTrackOffDisabled);
        end;

      tssOn:
        begin
          if Enabled then
            Details := LStyle.GetElementDetails(ttsTrackOnNormal)
          else
            Details := LStyle.GetElementDetails(ttsTrackOnDisabled);
        end;
    end;
    ...
    // Draw Thumb
    if Enabled then
      Details := LStyle.GetElementDetails(ttsThumbNormal)
    else
      Details := LStyle.GetElementDetails(ttsThumbDisabled);
    if FState = tssOff then
  ...
  try
    ...
    begin
      ...
      begin
        ...
        if not LStyle.GetElementColor(LStyle.GetElementDetails(tbCheckBoxUncheckedNormal),
          ecTextColor, TextColor) then
        begin
          if Enabled then
            TextColor := LStyle.GetSystemColor(clBtnText)
          else
            TextColor := LStyle.GetSystemColor(clGrayText);
        end;

        LStyle.DrawText(MemImage.Canvas.Handle, LStyle.GetElementDetails(tbCheckBoxUncheckedNormal),
      ...
      begin
        ...
        if LStyle.Enabled then
          MemImage.Canvas.Brush.Color := LStyle.GetSystemColor(clBtnFace)
        else
...
procedure InitButtonImageCollection(var AImageCollection: TImageCollection);

  procedure LoadItem(const AName: String);
  begin
    AImageCollection.Add(AName, HInstance, AName, ['', '_20X']);
  end;

begin
  if AImageCollection <> nil then
    Exit;
  AImageCollection := TImageCollection.Create(nil);
...
begin
  InitButtonImageCollection(FButtonImageCollection);
  inherited Create(AOwner);
...
begin
  FButtonImages.SetSize(DefaultSearchButtonImageSize, DefaultSearchButtonImageSize);
  FButtonImages.AutoFill := True;
  FButtonImages.ImageCollection := FButtonImageCollection;
end;

procedure TSearchBox.ChangeScale(M, D: Integer; isDpiChange: Boolean);
begin
...
procedure TSearchBox.CMWantSpecialKey(var Msg: TCMWantSpecialKey);
begin
  if (Msg.CharCode = VK_RETURN) and (SearchIndicator = sbiText) then
    Msg.Result := 1
  else
    inherited;
end;
...
begin
  ...
  AlignControlsToPanel(Rect);
  // Then align all controls relative to each other
  AlignControlsToSibling;
end;

procedure TCustomRelativePanel.AlignControlsToPanel(APanelRect: TRect);
var
  I: Integer;
  Item: TRelativePanelControlItem;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        ItemControl.Margins.SetControlBounds(APanelRect.Left, ItemControl.Margins.ControlTop,
          APanelRect.Width, ItemControl.Margins.ControlHeight);
      end
      else
      begin
        Item.CheckSaveWidth;
        ItemControl.Margins.SetControlBounds(APanelRect.Left, ItemControl.Margins.ControlTop,
          ItemControl.Margins.ControlWidth, ItemControl.Margins.ControlHeight);
      end;
    end
    else if Item.AlignRightWithPanel then
    begin
      Item.CheckSaveWidth;
      X := APanelRect.Right - ItemControl.Margins.ControlWidth;
      ItemControl.Margins.SetControlBounds(X, ItemControl.Margins.ControlTop,
        ItemControl.Margins.ControlWidth, ItemControl.Margins.ControlHeight);
    end;
    ...
    begin
      ...
      begin
        ...
        ItemControl.Margins.SetControlBounds(ItemControl.Margins.ControlLeft, APanelRect.Top,
          ItemControl.Margins.ControlWidth, APanelRect.Height);
      end
      else
      begin
    ...
    begin
      Item.CheckSaveHeight;
      Y := APanelRect.Bottom - ItemControl.Margins.ControlHeight;
      ItemControl.Margins.SetControlBounds(ItemControl.Margins.ControlLeft, Y,
        ItemControl.Margins.ControlWidth, ItemControl.Margins.ControlHeight);
    end;

    if Item.AlignHorizontalCenterWithPanel then
    begin
      X := (APanelRect.Width - ItemControl.Width) div 2;
      ItemControl.SetBounds(X, ItemControl.Top, ItemControl.Width, ItemControl.Height);
    end;

    if Item.AlignVerticalCenterWithPanel then
    begin
      Y := (APanelRect.Height - ItemControl.Height) div 2;
...
procedure TCustomRelativePanel.AlignControlsToSibling;
var
  I: Integer;
  Item: TRelativePanelControlItem;
  ItemControl: TControl;
  LAlignedList: TList;
  LDependenciesDictionary, LIterationDictionary: TDictionary<TControl, TRelativePanelControlItem>;
  LDependenciesEnum: TPair<TControl, TRelativePanelControlItem>;
  ...
  var
    ...
    // This procedure fill LDependenciesDictionary with controls that should be realigned
    // if controls from their alignment fields like AlignLeftWith, AlignRightWith, etc.
    // were not aligned yet or should be realigned.
    procedure CheckDependencies(AControl: TControl);
    begin
      if ((LAlignedList.IndexOf(AControl) = -1) or LDependenciesDictionary.ContainsKey(AControl)) and
        (not LDependenciesDictionary.ContainsKey(ItemControl)) then
        LDependenciesDictionary.Add(ItemControl, Item);
    end;

  begin
    if LAlignedList.IndexOf(ItemControl) = -1 then
      LAlignedList.Add(ItemControl);
    if Item.AlignLeftWith <> nil then
    begin
      CheckDependencies(Item.AlignLeftWith);
      if Item.AlignRightWith <> nil then
      begin
        CheckDependencies(Item.AlignRightWith);
    ...
    begin
      ...
      R := Item.AlignRightWith.BoundsRect;
      if Item.AlignLeftWithPanel then
        W := R.Right - ItemControl.Left
      else
        W := ItemControl.Width;
      X := R.Right - W;
      ItemControl.SetBounds(X, ItemControl.Top, W, ItemControl.Height);
    end;

    if Item.AlignTopWith <> nil then
    begin
      CheckDependencies(Item.AlignTopWith);
      if Item.AlignBottomWith <> nil then
      begin
        CheckDependencies(Item.AlignBottomWith);
    ...
    begin
      ...
      R := Item.AlignBottomWith.BoundsRect;
      if Item.AlignTopWithPanel then
        H := R.Bottom - ItemControl.Top
      else
        H := ItemControl.Height;
      Y := R.Bottom - H;
      ItemControl.SetBounds(ItemControl.Left, Y, ItemControl.Width, H);
    end;

    if Item.LeftOf <> nil then
    begin
      CheckDependencies(Item.LeftOf);
      X := Item.LeftOf.Margins.ControlLeft - ItemControl.Margins.ControlWidth;
      ItemControl.Margins.SetControlBounds(X, ItemControl.Margins.ControlTop,
        ItemControl.Margins.ControlWidth, ItemControl.Margins.ControlHeight);
    end
    else if Item.RightOf <> nil then
    begin
      CheckDependencies(Item.RightOf);
      X := Item.RightOf.Margins.ControlLeft + Item.RightOf.Margins.ControlWidth;
      ItemControl.Margins.SetControlBounds(X, ItemControl.Margins.ControlTop,
        ItemControl.Margins.ControlWidth, ItemControl.Margins.ControlHeight);
    end;
    ...
    begin
      CheckDependencies(Item.Above);
      Y := Item.Above.Margins.ControlTop - ItemControl.Margins.ControlHeight;
      ItemControl.Margins.SetControlBounds(ItemControl.Margins.ControlLeft, Y,
        ItemControl.Margins.ControlWidth, ItemControl.Margins.ControlHeight);
    end
    else if Item.Below <> nil then
    begin
      CheckDependencies(Item.Below);
        Y := Item.Below.Margins.ControlTop + Item.Below.Margins.ControlHeight;
      ItemControl.Margins.SetControlBounds(ItemControl.Margins.ControlLeft, Y,
        ItemControl.Margins.ControlWidth, ItemControl.Margins.ControlHeight);
    end;
    ...
    begin
      CheckDependencies(Item.AlignHorizontalCenterWith);
    ...
    begin
      CheckDependencies(Item.AlignVerticalCenterWith);
...
begin
  LAlignedList := TList.Create;
  LDependenciesDictionary := TDictionary<TControl, TRelativePanelControlItem>.Create;
  LIterationDictionary := TDictionary<TControl, TRelativePanelControlItem>.Create;
  try
    // Then align all controls relative to each other
    for I := 0 to FControlCollection.Count - 1 do
    begin
    ...
    // Realign all controls whose positions depend on positions of controls
    // from alignment fields like AlignLeftWith, AlignRightWith, etc. and
    // were aligned before these controls
    while LDependenciesDictionary.Count > 0 do
    begin
      LIterationDictionary.Clear;
      for LDependenciesEnum in LDependenciesDictionary do
      begin
        LAlignedList.Remove(LDependenciesEnum.Key);
        LIterationDictionary.Add(LDependenciesEnum.Key, LDependenciesEnum.Value);
      end;
      LDependenciesDictionary.Clear;
      for Item in LIterationDictionary.Values do
      begin
        ItemControl := Item.Control;
        if ItemControl <> nil then
          AlignControlToSibling;
      end;
      if LIterationDictionary.Count = LDependenciesDictionary.Count then
        if csUpdating in ComponentState then
          Exit
        else
          raise ERelativePanelException.Create(SRelativePanelCircularDependency);
    end;
  finally
    LIterationDictionary.Free;
    LAlignedList.Free;
    LDependenciesDictionary.Free;
  end;
end;
...
procedure TCustomSplitView.ConstrainedResize(var MinWidth, MinHeight, MaxWidth,
  MaxHeight: Integer);
var
  LMaxWidth, LMinWidth: TConstraintSize;
begin
  LMaxWidth := Constraints.MaxWidth;
  LMinWidth := Constraints.MinWidth;

  if (LMaxWidth > 0) and (FOpenedWidth > LMaxWidth) then
    FOpenedWidth := LMaxWidth
  else if (LMinWidth > 0) and (FOpenedWidth < LMinWidth) then
    FOpenedWidth := Constraints.MinWidth;

  if (LMaxWidth > 0) and (FCompactWidth > LMaxWidth) then
    FCompactWidth := LMaxWidth
  else if (LMinWidth > 0) and (FCompactWidth < LMinWidth) then
    FCompactWidth := LMinWidth;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (FState in [svsClosed, svsClosing]) then
        inherited Width := FCompactWidth;
    end;
  end;
end;
