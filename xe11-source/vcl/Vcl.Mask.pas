...
{ TCustomMaskEdit }
  ...
  protected
    ...
    procedure ObserverToggle(const AObserver: IObserver; const Value: Boolean); override;
...
procedure TCustomMaskEdit.ObserverToggle(const AObserver: IObserver;
  const Value: Boolean);
var
  LEditLinkObserver: IEditLinkObserver;
  LEditFormatLink: IEditFormatLink;
begin
  inherited ObserverToggle(AObserver, Value);
  if Supports(AObserver, IEditLinkObserver, LEditLinkObserver) then
    if Supports(LEditLinkObserver.FormatLink, IEditFormatLink, LEditFormatLink) then
    begin
      if Value then
      begin
        FSavedEditMask := EditMask;
        EditMask := LEditFormatLink.EditMask;
      end
      else
        if not (csDestroying in ComponentState) then
          EditMask := FSavedEditMask;
    end;
end;
...
{$ELSE}
  SendMessage(Handle, EM_GETSEL, WPARAM(@SelStart), LPARAM(@SelStop));
{$ENDIF}
end;
