...
{$IF DEFINED(LINUX)}
  WinUtils,
{$ENDIF}
  Winapi.Messages, Winapi.Windows, System.SysUtils, System.Classes, Vcl.ImgList,
  System.UITypes, Vcl.Controls, Vcl.Forms, Vcl.Menus, Vcl.Graphics, Winapi.CommCtrl,
...
type
  ...
  protected
    ...
    procedure CheckDefaultHeaderFont; virtual;
    procedure CreateParams(var Params: TCreateParams); override;
    procedure Paint; override;
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    property DefaultHeaderFont: Boolean read FDefaultHeaderFont write SetDefaultHeaderFont default True;
...
type
  ...
  private
    ...
    FSavedAlignment: TAlignment;
    FSavedMaxLength: Integer;
    FCanUndoSelText: Boolean;
    class constructor Create;
    class destructor Destroy;
  ...
  protected
    ...
    procedure ObserverToggle(const AObserver: IObserver; const Value: Boolean); virtual;
    ...
    procedure UpdateEditMargins; virtual;
...
type
  ...
  private
    FLeft, FRight: TMarginSize;
    FAuto: Boolean;
    FOnChange: TNotifyEvent;
    procedure SetLeftMargin(Value: TMarginSize);
    procedure SetRightMargin(Value: TMarginSize);
    procedure SetAutoMargin(Value: Boolean);
  protected
  ...
  private
    ...
    FWantTabs: Boolean;
    FEditMargins: TEditMargins;
    class constructor Create;
    class destructor Destroy;
    procedure DoEditMarginChange(Sender: TObject);
    procedure CMFontChanged(var Message: TMessage); message CM_FONTCHANGED;
...
{$ENDIF}
  ...
  private
    ...
    procedure SetTextHint(const Value: string);
    procedure SetExtendedUI(Value: Boolean);
  ...
  public
    ...
    property ExtendedUI: Boolean read FExtendedUI write SetExtendedUI default False;
    property CanUndoSelText: Boolean read FCanUndoSelText write FCanUndoSelText default False;
    property SelText: string read GetSelText write SetSelText;
    property TextHint: string read FTextHint write SetTextHint;
  end;
...
{ TButtonControl }
  ...
  private
    ...
    procedure SetStylusHotImageName(const Value: TImageName);
...
{$ENDIF}
  protected
    ...
    procedure SetImageList(AHandle: HIMAGELIST); virtual;
...
type
  ...
  TCustomComboHelper = class helper for TCustomCombo
  private
    class var FLastComboMousePositions: TDictionary<TCustomCombo, TSmallPoint>;
    function GetLastComboMousePos: TSmallPoint;
    procedure SetLastComboMousePos(const Value: TSmallPoint);
    property LastComboMousePos: TSmallPoint read GetLastComboMousePos write SetLastComboMousePos;

    class var FLastEditMousePositions: TDictionary<TCustomCombo, TSmallPoint>;
    function GetLastEditMousePos: TSmallPoint;
    procedure SetLastEditMousePos(const Value: TSmallPoint);
    property LastEditMousePos: TSmallPoint read GetLastEditMousePos write SetLastEditMousePos;

    procedure RemoveFromDictionaries;
  end;
...
protected
  ...
  strict protected
    ...
    function GetCaptionRect(Canvas: TCanvas): TRect; virtual;
    procedure Paint(Canvas: TCanvas); override;
    procedure PaintBackground(Canvas: TCanvas); override;
    procedure WndProc(var Message: TMessage); override;
  public
...
{$ENDIF}
  ...
  System.Types, System.StrUtils, Vcl.ExtCtrls, Vcl.GraphUtil;

const
  CanUndoAfterEM_REPLACESEL: array[Boolean] of WPARAM = (0, 1);
...
function _Min(const A, B: Integer): Integer; inline;
begin
  if A < B then
    Result := A
  else
...
destructor TCustomGroupBox.Destroy;
begin
  inherited;
  FHeaderFont.Free;
end;
...
procedure TCustomGroupBox.SetHeaderFont(Value: TFont);
begin
  FHeaderFont.Assign(Value);
end;

procedure TCustomGroupBox.SetShowFrame(Value: Boolean);
begin
...
procedure TCustomGroupBox.SetDefaultHeaderFont(Value: Boolean);
begin
  if FDefaultHeaderFont <> Value then
  begin
    FDefaultHeaderFont := Value;
...
procedure TCustomGroupBox.CMFontChanged(var Message: TMessage);
begin
  inherited;
  CheckDefaultHeaderFont;
  Realign;
...
procedure TCustomGroupBox.CheckDefaultHeaderFont;
begin
  if FDefaultHeaderFont then
  begin
    // Restore header font without a font change event
...
procedure TCustomGroupBox.HeaderFontChanged(Sender: TObject);
begin
  FDefaultHeaderFont := False;
  Perform(CM_FONTCHANGED, 0, 0);
end;

function TCustomGroupBox.IsHeaderFontStored: Boolean;
begin
  Result := not DefaultHeaderFont;
end;
...
begin
  ...
  LStyle := StyleServices(Self);
  LStyle.DrawText(DC,
    LStyle.GetElementDetails(CStates[Enabled]), Text, TextRect, LFormat, LOptions);
end;
...
begin
    ...
    begin
      ...
      begin
        repeat
          ...
          if (Height > ClientHeight) and (Height > Canvas.TextHeight('Wq')) then
          begin
  ...
  {$ENDIF}
    ...
    if Enabled or StyleServices(Self).Enabled then
...
begin
  ...
  if not Transparent then
    ControlStyle := ControlStyle + [csOpaque]
  else if StyleServices(Self).Enabled and (seClient in StyleElements) then
    ControlStyle := ControlStyle - [csOpaque];
  UpdateDrawTextProc;
  Invalidate;
end;
...
begin
  ...
  begin
    if Enabled and IsCustomStyleActive and not (seFont in StyleElements) then
...
procedure TEditMargins.Change;
begin
  if Assigned(FOnChange) then
    FOnChange(Self);
end;

procedure TEditMargins.SetAutoMargin(Value: Boolean);
begin
  if Value <> FAuto then
  begin
    FAuto := Value;
...
procedure TEditMargins.SetLeftMargin(Value: TMarginSize);
begin
  // Take new value of left margin if Auto is False
  if (Value <> FLeft) and not FAuto then
  begin
    FLeft := Value;
    Change;
  end;
...
procedure TEditMargins.SetRightMargin(Value: TMarginSize);
begin
  // Take new value of right margin if Auto is False
  if (Value <> FRight) and not FAuto then
  begin
    FRight := Value;
    Change;
  end;
...
const
  EditStyle = [csClickEvents, csSetCaption, csDoubleClicks, csFixedHeight, csPannable, csNeedsDesignDisabledState];
begin
  inherited Create(AOwner);
...
begin
  SendMessage(Handle, EM_REPLACESEL, CanUndoAfterEM_REPLACESEL[FCanUndoSelText], LPARAM(Buffer));
end;
{$ENDIF}
...
{$IF DEFINED(CLR)}
  if not Assigned(Value) then
    SendTextMessage(Handle, EM_REPLACESEL, CanUndoAfterEM_REPLACESEL[FCanUndoSelText], '')
  else
{$ENDIF}
    SendTextMessage(Handle, EM_REPLACESEL, CanUndoAfterEM_REPLACESEL[FCanUndoSelText], Value);
end;
...
begin
  ...
  Modified := FModified;
  if (FPasswordChar <> #0) and not (StyleServices(Self).Enabled and ((FPasswordChar = '*'))) then
...
procedure TCustomEdit.UpdateEditMargins;
begin
...
begin
  inherited;
  if NewStyleControls and (GetWindowLong(Handle, GWL_STYLE) and ES_MULTILINE = 0) then
    UpdateEditMargins;
end;
...
begin
  inherited;
  if (csFixedHeight in ControlStyle) and not ((csDesigning in ComponentState) and (csLoading in ComponentState)) then
    AdjustHeight;
end;
...
begin
  ...
  begin
    ...
    SendTextMessage(Memo.Handle, EM_REPLACESEL, CanUndoAfterEM_REPLACESEL[Memo.CanUndoSelText], S);
  end;
end;
...
begin
  ...
  begin
    ...
    SendMessage(Memo.Handle, EM_SETSEL, SelStart, SelStart);
    SendTextMessage(Memo.Handle, EM_REPLACESEL, CanUndoAfterEM_REPLACESEL[Memo.CanUndoSelText], Line);
  end;
end;
...
begin
  ...
  begin
    ...
    SendTextMessage(Memo.Handle, EM_REPLACESEL, CanUndoAfterEM_REPLACESEL[Memo.CanUndoSelText], '');
  end;
end;
...
begin
  ...
  FEditMargins := TEditMargins.Create;
  FEditMargins.OnChange := DoEditMarginChange;
end;
...
procedure TCustomMemo.DoEditMarginChange(Sender: TObject);
begin
  UpdateEditMargins;
end;
...
procedure TCustomMemo.CreateWnd;
begin
  inherited;
  UpdateEditMargins;
end;

procedure TCustomMemo.DoUpdateEditMargins;
begin
  if HandleAllocated then
  begin
    if EditMargins.Auto then
      SendMessage(Handle, EM_SETMARGINS, EC_LEFTMARGIN or EC_RIGHTMARGIN, MakeLParam(EC_USEFONTINFO, EC_USEFONTINFO))
    else
      SendMessage(Handle, EM_SETMARGINS, EC_LEFTMARGIN or EC_RIGHTMARGIN,
        MakeLParam(ScaleValue(EditMargins.Left), ScaleValue(EditMargins.Right)));
  end;
end;
...
procedure TCustomMemo.SetEditMargins(Value: TEditMargins);
begin
  FEditMargins.Assign(Value);
end;
...
procedure TCustomMemo.UpdateEditMargins;
begin
  DoUpdateEditMargins;
  // Multiline EDIT window class controls, (not anything else, like richedits, though!)
  if HandleAllocated and (GetWindowLong(Handle, GWL_STYLE) and ES_MULTILINE <> 0) then
    SendMessage(Handle, EM_SETRECT, 0, 0)
end;
...
const
  ComboBoxStyle = [csCaptureMouse, csSetCaption, csDoubleClicks,
    csFixedHeight, csReflector, csPannable, csNeedsDesignDisabledState];
begin
  inherited Create(AOwner);
  if NewStyleControls then
...
begin
  ...
  case Message.Msg of
    ...
    WM_MOUSEMOVE:
    begin
      if LastEditMousePos <> TWMMouseMove(Message).Pos then
      begin
        LastEditMousePos := TWMMouseMove(Message).Pos;
...
function TCustomComboBox.GetDropDownWidth: Integer;
begin
...
procedure TCustomComboBox.SetDropDownWidth(Value: Integer);
begin
  if FDropDownWidth <> Value then
  begin
    FDropDownWidth := Value;
    if (Style <> csSimple) and HandleAllocated then
      SendMessage(Handle, CB_SETDROPPEDWIDTH, WPARAM(FDropDownWidth), 0);
  end;
end;
...
{$ENDIF}
        ...
        WM_CHAR:
          begin
            if (TWMKey(Message).CharCode = VK_BACK) and (FEditHandle <> 0) and
               (GetWindowLong(FEditHandle, GWL_STYLE) and ES_READONLY = ES_READONLY) then
              Exit;
{$IF DEFINED(CLR)}
            KeyMsg := TWMKey.Create(Message);
...
{$ENDIF}
        ...
        WM_MOUSEMOVE:
          if LastComboMousePos <> TWMMouseMove(Message).Pos then
          begin
            LastComboMousePos := TWMMouseMove(Message).Pos;
...
const
  ...
  function DidMouseLeaveWindow(Wnd: HWnd): Boolean;
  begin
    Result := not IsChild(Wnd, WindowFromPoint(SmallPointToPoint(System.Types.SmallPoint(GetMessagePos))));
  end;
  ...
  with Message do
    case Msg of
      ...
      WM_CTLCOLORMSGBOX..WM_CTLCOLORSTATIC:
        if (not IsCustomStyleActive) or
           (IsCustomStyleActive and not (seBorder in StyleElements)) then
        begin
...
{$ENDIF}
      ...
      WM_TIMER, WM_MOUSELEAVE:
        begin
          if DidMouseLeaveWindow(Handle) then
          begin
            KillTimer(Handle, CComboBoxTimerID);
            // If the timer message tells us the mouse has left the control, then send a
            // final WM_MOUSELEAVE back to TControl.WndProc so the hint can be removed
          ...
          begin
            // We are over the edit window embedded in a combobox, so WM_MOUSELEAVE is not technically 'valid'.
            // Windows sent WM_MOUSELEAVE as we are over the edit, even though we are still within the combobox.
            // However, in case the mouse gets moved out really fast we need to use a timer to capture that scenario
            SetTimer(Handle, CComboBoxTimerID, 10, nil);
            Exit;
          end;
        end;
      WM_NCCALCSIZE:
  ...
  begin
    ...
    if Index >= 0 then
      if odComboBoxEdit in State then
        FCanvas.TextOut(Rect.Left + 1, Rect.Top + 1, Items[Index]) //Visual state of the text in the edit control
      else
        FCanvas.TextOut(Rect.Left + 2, Rect.Top, Items[Index]); //Visual state of the text(items) in the deployed list
  end;
end;
...
begin
  ...
  begin
    var MaxItemWidth := 0;
    // Identify maximum item width
    for var I: Integer := 0 to -1 + Items.Count do
    begin
      var ItemWidth := Canvas.TextWidth(Items[I]);
      Inc(ItemWidth, ScaleValue(Spacing));
...
begin
  ...
  begin
    FocusedWnd := GetFocus;
    Result := (FocusedWnd <> 0) and ((FocusedWnd = FEditHandle) or (FocusedWnd = FListHandle) or (FocusedWnd = Handle));
  end;
end;
...
procedure TCustomCombo.ChangeScale(M, D: Integer; isDpiChange: Boolean);
begin
  if (Self is TCustomComboBox) and (TCustomComboBox(Self).Style = csSimple) then
...
begin
  ...
  FDroppingDown := True;
  try
    SetWindowPos(FDropHandle, 0, 0, 0, Width, ItemHeight * Count + Height + 2,
      SWP_NOMOVE or SWP_NOZORDER or SWP_NOACTIVATE or SWP_NOREDRAW or SWP_HIDEWINDOW);
  finally
    FDroppingDown := False;
  end;
  SetWindowPos(FDropHandle, 0, 0, 0, 0, 0,
...
{$IF DEFINED(CLR)}
    if not Assigned(Value) then
      SendTextMessage(FEditHandle, EM_REPLACESEL, CanUndoAfterEM_REPLACESEL[FCanUndoSelText], '')
    else
{$ENDIF}
      SendTextMessage(FEditHandle, EM_REPLACESEL, CanUndoAfterEM_REPLACESEL[FCanUndoSelText], Value);
  end;
end;
...
procedure TCustomComboBox.SetExtendedUI(Value: Boolean);
begin
  if FExtendedUI <> Value then
  begin
    FExtendedUI := Value;
    if not (csDesigning in ComponentState) and TOSVersion.Check(6) and HandleAllocated then
      SendMessage(Handle, CB_SETEXTENDEDUI, Ord(FExtendedUI), 0)
  end;
end;
...
function TCustomComboBox.IsDropDownWidthStored: Boolean;
begin
...
begin
  Result := (FStyle in [csOwnerDrawFixed, csOwnerDrawVariable]) and inherited IsItemHeightStored;
end;
...
{$ENDIF}
  ...
  if FExtendedUI and TOSVersion.Check(6) then
    SendMessage(Handle, CB_SETEXTENDEDUI, 1, 0);

  if (Style <> csSimple) and (FDropDownWidth <> 0) and (FDropDownWidth <> Width) then
    SendMessage(Handle, CB_SETDROPPEDWIDTH, WPARAM(FDropDownWidth), 0);
...
begin
  if StyleServices(Self).Enabled and HandleAllocated then
...
procedure TCustomComboBox.PerformAutoActions(var Key: Char);
  ...
  begin
    ...
    begin
      ...
      if seClient in StyleElements then
        FCanvas.Brush.Color := LStyle.GetStyleColor(ColorStates[Enabled])
      else
        FCanvas.Brush := Brush;
      if seFont in StyleElements then
        FCanvas.Font.Color := LStyle.GetStyleFontColor(FontStates[Enabled]);
    end
    else
    begin
      FCanvas.Brush := Brush;
    ...
    if odFocused in State then DrawFocusRect(FCanvas.Handle, rcItem);
    FCanvas.Handle := 0;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      var ComboRect, ListRect: TRect;
      // Get combobox bounds
      GetWindowRect(Handle, ComboRect);
      // Get combobox dropdown list bounds
      GetWindowRect(Message.LParam, ListRect);
      // Take action if list spills off the right side of the screen
      if ComboRect.Left + ListRect.Width > Screen.MonitorFromWindow(Handle).Width then
        MoveWindow(Message.LParam, ComboRect.Right - ListRect.Width, ListRect.Top, ListRect.Width,
          ListRect.Height, True);
    end;
  end;
  if not (csDesigning in ComponentState) then
    if Observers.IsObserving(TObserverMapping.EditLinkID) then
...
function TCustomComboBox.GetEditHeight: Integer;
begin
  Result := ItemHeight + 8;
end;
...
procedure TCustomComboBox.CMFontChanged(var Message: TMessage);
var
  LNeedAdjustHeight: Boolean;
begin
  LNeedAdjustHeight := (Style = csSimple) and ((Height = GetEditHeight) or (FOldHeight = GetEditHeight));
  inherited;
  if LNeedAdjustHeight and (GetEditHeight > 8) then
  begin
    FOldHeight := 0;
...
function TPushButtonActionLink.IsImageNameLinked: Boolean;
begin
  Result := inherited IsImageNameLinked and
    (TCustomButton(FClient).ImageName = TCustomAction(Action).ImageName);
end;
...
begin
  inherited Create(AOwner);
  ControlStyle := [csSetCaption, csDoubleClicks, csNeedsDesignDisabledState];
  Width := 75;
  Height := 25;
  TabStop := True;
...
procedure TCustomButton.CheckImageIndexes;
begin
...
procedure TCustomButton.UpdateImageName(Index: TImageIndex; var Name: TImageName);
begin
...
procedure TCustomButton.UpdateImageIndex(Name: TImageName; var Index: TImageIndex);
begin
  if (FImages <> nil) and FImages.IsImageNameAvailable then
...
procedure TCustomButton.UpdateImage;
begin
end;
  ...
  begin
    ...
    begin
      if FDisabledImageIndex <> -1 then
        FInternalImageList.AddImage(FDisabledImages, FDisabledImageIndex)
      else
        FInternalImageList.AddImage(FDisabledImages, FImageIndex);
    end
    else
    if FDisabledImageIndex <> -1 then
      FInternalImageList.AddImage(FImages, FDisabledImageIndex)
    else
      FInternalImageList.AddDisabledImage(FImages, FImageIndex);
    // PBS_DEFAULTED
...
procedure TCustomButton.SetDisabledImageName(const Value: TImageName);
begin
...
procedure TCustomButton.SetHotImageName(const Value: TImageName);
begin
...
procedure TCustomButton.SetImageName(const Value: TImageName);
var
  LOldIndex: TImageIndex;
begin
...
procedure TCustomButton.SetDisabledImages(const Value: TCustomImageList);
begin
...
procedure TCustomButton.SetPressedImageName(const Value: TImageName);
begin
...
procedure TCustomButton.SetSelectedImageName(const Value: TImageName);
begin
  if Value <> FSelectedImageName then
  begin
    FSelectedImageName := Value;
    UpdateImageIndex(Value, FSelectedImageIndex);
  end;
end;
...
begin
  ...
  begin
    ...
    UpdateImageName(Value, FStylusHotImageName);
  end;
end;

procedure TCustomButton.SetStylusHotImageName(const Value: TImageName);
begin
  if Value <> FStylusHotImageName then
  begin
    FStylusHotImageName := Value;
    UpdateImageIndex(Value, FStylusHotImageIndex);
  end;
end;
...
function TCustomButton.IsImageNameStored: Boolean;
begin
  Result := (ActionLink = nil) or not TPushButtonActionLink(ActionLink).IsImageNameLinked;
end;
...
const
  ListBoxStyle = [csSetCaption, csDoubleClicks, csOpaque, csPannable, csGestures, csNeedsDesignDisabledState];
begin
  inherited Create(AOwner);
  if NewStyleControls then
...
begin
  ...
  begin
    ...
    Style := Style or (WS_HSCROLL or WS_VSCROLL or
      Data[Self.Style in [lbVirtual, lbVirtualOwnerDraw]] or LBS_NOTIFY) or
      Styles[FStyle] or Sorteds[FSorted] or Select or IntegralHeights[FIntegralHeight] or
...
procedure TCustomListBox.ChangeScale(M, D: Integer; isDpiChange: Boolean);
begin
  ItemHeight := MulDiv(FItemHeight, M, D);
  inherited;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (ItemNo >= 0) and (ItemNo <> ItemIndex) then
    ...
    begin
      ...
      begin
        ...
        begin
          MouseDown(TMouseButton.mbLeft, ShiftState, Message.XPos, Message.YPos);
...
procedure TCustomListBox.WMPaint(var Message: TWMPaint);
  ...
  var
    ...
    MeasureItemStruct: TMeasureItemStruct;
    R: TRect;
    Y, I, H, W, IH, LocItemHeight: Integer;
{$IF DEFINED(CLR)}
    LMessage: TMessage;
...
{$ENDIF}
        ...
        MeasureItemStruct.itemHeight := LocItemHeight;
...
{$ENDIF}
        if Style = lbStandard then
          ...
          IH := MeasureItemStruct.itemHeight;
...
{$ENDIF}
  ...
  DefaultHandler(M);
  M.Msg := WM_LBUTTONUP;
  FDragCancelling := True;
  try
    DefaultHandler(M);
  finally
    FDragCancelling := False;
  end;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (Integer(itemID) >= 0) and (odSelected in State) then
        begin
          FCanvas.Brush.Color := LStyle.GetSystemColor(clHighlight);
          FCanvas.Font.Color := LStyle.GetStyleFontColor(sfListItemTextSelected);
        end
        else
        begin
          FCanvas.Brush.Color := LStyle.GetStyleColor(scListBox);
          if seFont in StyleElements then
            FCanvas.Font.Color := LStyle.GetStyleFontColor(sfListItemTextNormal);
        end;
      end
      else
      begin
      ...
      if Integer(itemID) >= 0 then
        ...
        FCanvas.FillRect(rcItem);
      if (odFocused in State) and not IsCustomStyleActive then
...
begin
  ...
  TabStop := True;
  ControlStyle := [csFramed, csDoubleClicks, csOpaque, csNeedsDesignDisabledState];
  FKind := sbHorizontal;
  FMax := 100;
  FSmallChange := 1;
  FLargeChange := 1;
...
procedure TScrollBar.WMPaint(var Message: TWMPaint);
begin
  if DoubleBuffered then
    DefaultHandler(Message)
  else
    ...
    begin
      ...
      try
        ...
        if LStyle.Enabled then
          ...
          begin
            Brush.Style := bsSolid;
            Brush.Color := LStyle.ColorToRGB(clBtnFace);
...
begin
  ...
  if Result and (csDesigning in Control.ComponentState) then
    case Message.Msg of
...
procedure TScrollBarStyleHook.CMShowingChanged(var Message: TMessage);
begin
  if Control.HandleAllocated and (csDesigning in Control.ComponentState) and
    (FScrollWnd <> nil) and FScrollWnd.HandleAllocated then
  begin
...
begin
  ...
  if (FScrollWnd <> nil) and FScrollWnd.HandleAllocated and (Control.Parent <> nil) then
  begin
    R := ControlBounds;
    SetWindowPos(FScrollWnd.Handle, HWND_TOP,
...
var
  Info: TScrollInfo;
  LVertTrackRect, LVertDownButtonRect: TRect;
begin
  ...
  begin
    ...
    begin
      Result.Top := LVertTrackRect.Top + Round(((Info.nPos - Info.nMin) / (Info.nMax - Info.nMin)) * (LVertTrackRect.Height - GetSystemMetrics(SM_CYVTHUMB)));
      Result.Bottom := Result.Top + GetSystemMetrics(SM_CYVTHUMB);
    end
    else
    begin
      Result.Top := LVertTrackRect.Top + Round(((Info.nPos - Info.nMin) / (Info.nMax - Info.nMin)) * LVertTrackRect.Height);
      Result.Bottom := LVertTrackRect.Top + Round(((Info.nPos + Integer(Info.nPage) - Info.nMin - 1) / (Info.nMax - Info.nMin)) * LVertTrackRect.Height);
      if Result.Bottom - Result.Top < GetSystemMetrics(SM_CYVTHUMB) then
      begin
        if Info.nMax - Info.nMin - Integer(Info.nPage) = 0 then
          Result.Top := LVertTrackRect.Top
        else
          Result.Top := LVertTrackRect.Top + Round(((Info.nPos - Info.nMin) / (Info.nMax - Info.nMin - Integer(Info.nPage))) * (LVertTrackRect.Height - GetSystemMetrics(SM_CYVTHUMB)));
        Result.Bottom := Result.Top + GetSystemMetrics(SM_CYVTHUMB);
      end;
    end;
    if Result.Bottom > LVertDownButtonRect.Top then
        Result.Bottom := LVertDownButtonRect.Top;
    if LVertDownButtonRect.Top - VertUpButtonRect.Bottom < GetSystemMetrics(SM_CYVTHUMB) then
        Result := TRect.Empty;
  end;
end;
...
begin
  ...
  if Result.Width > 0 then
  begin
    LThumbSize := GetSystemMetrics(SM_CYVTHUMB);
    Result.Top := Result.Bottom - LThumbSize;
    if VertScrollRect.Height < LThumbSize * 2 then
    Inc(Result.Top, (LThumbSize * 2 - VertScrollRect.Height) div 2);
    if Result.Bottom - Result.Top < LThumbSize div 2 then
      Result.Top := Result.Bottom -LThumbSize div 2;
  end
  else
    Result := TRect.Empty;
end;
...
var
  Info: TScrollInfo;
  LHorzTrackRect, LHorzDownButtonRect: TRect;
  LThumbSize: Integer;
begin
  if not Control.Enabled then
    Result := TRect.Empty
  ...
  begin
    ...
    LThumbSize := GetSystemMetrics(SM_CXHTHUMB);

    if Info.nPage = 0 then
    begin
      Result.Left := LHorzTrackRect.Left + Round(((Info.nPos - Info.nMin) / (Info.nMax - Info.nMin)) * (LHorzTrackRect.Width - LThumbSize));
      Result.Right := Result.Left + LThumbSize;
    end
    else
    begin
      Result.Left := LHorzTrackRect.Left + Round(((Info.nPos - Info.nMin) / (Info.nMax - Info.nMin)) * LHorzTrackRect.Width);
      Result.Right := LHorzTrackRect.Left + Round(((Info.nPos + Integer(Info.nPage) - Info.nMin - 1) / (Info.nMax - Info.nMin)) * LHorzTrackRect.Width);
      if Result.Right - Result.Left < LThumbSize then
      begin
        if Info.nMax - Info.nMin - Integer(Info.nPage) = 0 then
          Result.Left := LHorzTrackRect.Left
        else
          Result.Left := LHorzTrackRect.Left + Round(((Info.nPos - Info.nMin) / (Info.nMax - Info.nMin - Integer(Info.nPage))) * (LHorzTrackRect.Width - LThumbSize));
        Result.Right := Result.Left + LThumbSize;
      end;
    end;
    if Result.Right > LHorzDownButtonRect.Left then
        Result.Right := LHorzDownButtonRect.Left;
    if LHorzDownButtonRect.Left - HorzUpButtonRect.Right < LThumbSize then
        Result := TRect.Empty;
  end;
end;
...
begin
  ...
  begin
    LThumbSize := GetSystemMetrics(SM_CXHTHUMB);
    Result.Left := Result.Left + LThumbSize;
    Result.Right := Result.Right - LThumbSize;
  end
  else
    Result := TRect.Empty;
end;
...
begin
  ...
  begin
    ...
    if HorzScrollRect.Width < LThumbSize * 2 then
    Dec(Result.Right, (LThumbSize * 2 - HorzScrollRect.Width) div 2);
    if Result.Right - Result.Left < LThumbSize div 2 then
      Result.Right := Result.Left + LThumbSize div 2;
  end
  else
    Result := TRect.Empty;
end;
...
begin
  ...
  begin
    LThumbSize := GetSystemMetrics(SM_CXHTHUMB);
    Result.Left := Result.Right - LThumbSize;
    if HorzScrollRect.Width < LThumbSize * 2 then
    Inc(Result.Left, (LThumbSize * 2 - HorzScrollRect.Width) div 2);
    if Result.Right - Result.Left < LThumbSize div 2 then
      Result.Left := Result.Right - LThumbSize div 2;
  end
  else
    Result := TRect.Empty;
end;
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        LDetails := LStyle.GetElementDetails(tsUpperTrackVertNormal);
      ...
      // Important: Must get handle after call to VertSliderRect
      LHandle := LBitmap.Canvas.Handle;

      if Control.Enabled then
      begin
        LDetails := LStyle.GetElementDetails(FVSliderState);
        LStyle.DrawElement(LHandle, LDetails, LRect, nil, LPPI);
      end;

      if Control.Enabled then
        LDetails := LStyle.GetElementDetails(FVUpState)
      else
        LDetails := LStyle.GetElementDetails(tsArrowBtnUpDisabled);

      LStyle.DrawElement(LHandle, LDetails, VertUpButtonRect, nil, LPPI);

      if Control.Enabled then
        LDetails := LStyle.GetElementDetails(FVDownState)
      else
        LDetails := LStyle.GetElementDetails(tsArrowBtnDownDisabled);

      LStyle.DrawElement(LHandle, LDetails, VertDownButtonRect, nil, LPPI);
...
begin
  ...
  begin
    ...
    try
      ...
      // Important: Must get handle after call to HorzSliderRect
      LHandle := LBitmap.Canvas.Handle;
      ...
      LStyle.DrawElement(LHandle, LDetails, HorzUpButtonRect, nil, LPPI);

      if Control.Enabled then
        LDetails := LStyle.GetElementDetails(FHDownState)
      else
        LDetails := LStyle.GetElementDetails(tsArrowBtnRightDisabled);
      LStyle.DrawElement(LHandle, LDetails, HorzDownButtonRect, nil, LPPI);
...
var
  ...
  LHorzSliderRect, LVertSliderRect: TRect;
begin
  inherited;
  CallDefaultProc(TMessage(Message));
  P := Point(Message.XPos, Message.YPos);
  ...
  begin
    ...
    if LHorzSliderRect.Width > 0 then
      if PtInRect(LHorzSliderRect, P) and (FHSliderState = tsThumbBtnHorzNormal) then
      begin
        FHSliderState := tsThumbBtnHorzHot;
        MustUpdateScroll := True;
      end
      else if not PtInRect(LHorzSliderRect, P) and (FHSliderState = tsThumbBtnHorzHot) then
      begin
        FHSliderState := tsThumbBtnHorzNormal;
        MustUpdateScroll := True;
      end;
  ...
  begin
    ...
    if LVertSliderRect.Height > 0 then
      if PtInRect(LVertSliderRect, P) and (FVSliderState = tsThumbBtnVertNormal) then
      begin
        FVSliderState := tsThumbBtnVertHot;
        MustUpdateScroll := True;
      end
      else if not PtInRect(LVertSliderRect, P) and (FVSliderState = tsThumbBtnVertHot) then
      begin
        FVSliderState := tsThumbBtnVertNormal;
        MustUpdateScroll := True;
      end;
...
begin
  LStyle := StyleServices;
  if LStyle.Available and HasBorder then
  begin
...
begin
  ...
  try
    ...
    Details := LStyle.GetElementDetails(DrawState);
    ...
    begin
      ...
      LStyle.DrawElement(Buffer.Canvas.Handle, Details, R);
    ...
    if Style <> csSimple then
    begin
      Details := LStyle.GetElementDetails(BtnDrawState);
      LStyle.DrawElement(Buffer.Canvas.Handle, Details, ButtonRect, nil, LPPI);
    end;
...
begin
  ...
  try
    ...
    R := Control.ClientRect;
    if csGlassPaint in Control.ControlState then
      PaintBuffer := BeginBufferedPaint(Canvas.Handle, R, BPBF_COMPOSITED, nil, MemDC)
    else
      PaintBuffer := 0;
    try
      if csGlassPaint in Control.ControlState then
...
begin
  ...
  if Result.Height > 0 then
    Result.Top := Result.Bottom - _Min(GetSystemMetrics(SM_CYVTHUMB), Result.Height div 2)
  else
    Result := TRect.Empty;
 end;
...
begin
  Result := ListBoxVertScrollRect;
  if Result.Height > 0 then
    Result.Bottom := Result.Top + _Min(GetSystemMetrics(SM_CYVTHUMB), Result.Height div 2)
  else
    Result := TRect.Empty;
end;
...
begin
  ...
  if Result.Height < MinSize then
  begin
    Dec(LSize, MinSize - Result.Height + 1);
    Result.Top := ListBoxVertUpButtonRect.Bottom +
      Round(LTopIndex / LItemCount * LSize);
    Result.Bottom := Result.Top + MinSize;
  end;
...
var
  ...
  LPPI: Integer;
begin
  if not DroppedDown or (GetWindowLong(FListHandle, GWL_STYLE) and WS_VSCROLL = 0) then
    Exit;
  LPPI := Control.CurrentPPI;
    ...
    begin
      ...
      try
        ...
        begin
          ...
          StyleServices.DrawElement(B.Canvas.Handle, Details, ListBoxVertSliderRect, nil, LPPI);
          Details := StyleServices.GetElementDetails(FVUpState);
          StyleServices.DrawElement(B.Canvas.Handle, Details, ListBoxVertUpButtonRect, nil, LPPI);
          Details := StyleServices.GetElementDetails(FVDownState);
          StyleServices.DrawElement(B.Canvas.Handle, Details, ListBoxVertDownButtonRect, nil, LPPI);
        end;
...
var
  ...
  begin
    if TOSVersion.Check(6) then
      SendMessage(FListHandle, LB_SETTOPINDEX, AValue, 0)
    else
    begin
  ...
  begin
    ...
    begin
      ...
      begin
        SetTopindex(SendMessage(FListHandle, LB_GETTOPINDEX, 0, 0) - 1);
        Exit;
      end;
...
begin
  if (Message.Msg = WM_ERASEBKGND) and (Control.StyleName <> '') and not FStyleColorsChecked then
  begin
...
begin
  ...
  begin
    ...
    ACanvas.Font.Height := MulDiv(ACanvas.Font.Height, LPPI, ACanvas.Font.PixelsPerInch);
    R := DrawRect;
...
begin
 ...
 if AnimationEnabled then
   case Message.Msg of
     ...
     CM_FOCUSCHANGED:
       if TCMFocusChanged(Message).Sender = Self.Control then
         BufferedPaintStopAllAnimations(Handle);
     WM_SIZE,
 ...
 case Message.Msg of
   ...
   CM_FOCUSCHANGED:
     if AnimationEnabled and (TCMFocusChanged(Message).Sender = Self.Control) then
     begin
       if Control.HandleAllocated and IsWindowVisible(Handle) then
          RedrawWindow(Handle, nil, 0, RDW_INVALIDATE or RDW_UPDATENOW);
     end;
...
function TButtonStyleHook.GetDropDownWidth: Integer;
var
  LSplitInfo: TButtonSplitinfo;
begin
  LSplitInfo.mask := BCSIF_SIZE;
  if Button_GetSplitInfo(Handle, LSplitInfo) then
    Result := LSplitInfo.size.cx
  else
    Result := 0;
end;
...
begin
  ...
  if ((GetWindowLong(Handle, GWL_STYLE) and BS_SPLITBUTTON) = BS_SPLITBUTTON) and
     (Message.XPos >= Control.Width - GetDropDownWidth) and (TCustomButton(Control).DropDownMenu <> nil)
  then
    begin
      Handled := False;
    end
...
begin
  ...
  begin
    ...
    Details := LStyle.GetElementDetails(GetDrawState(State));
    LPPI := Control.CurrentPPI;
    ...
    R := Control.ClientRect;
    if not LStyle.GetElementSize(Canvas.Handle, LStyle.GetElementDetails(tbCheckBoxCheckedNormal),
      LRect, ElementSize, BoxSize, LPPI) then
    begin
      BoxSize.cx := 13;
      BoxSize.cy := 13;
    end;
    ...
    LStyle.DrawElement(Canvas.Handle, Details, R, nil, LPPI);
    ...
    begin
      ...
      Canvas.Brush.Color := LStyle.GetSystemColor(clBtnFace);
...
var
  ...
  OldVisible: Boolean;
begin
  OldBoundsRect := Control.BoundsRect;
  ...
  FPressed := False;
  Invalidate;
  if ((OldBoundsRect <> Control.BoundsRect) or (OldVisible <> Control.Visible)) and (Control.Parent <> nil) then
    InvalidateRect(Control.Parent.Handle, OldBoundsRect, True);
  Handled := True;
end;
...
constructor TGroupBoxStyleHook.Create(AControl: TWinControl);
begin
  inherited;
  OverridePaint := True;
  OverrideEraseBkgnd := True;
...
procedure TGroupBoxStyleHook.DrawControlText(Canvas: TCanvas; Details: TThemedElementDetails;
...
var
  ...
  LStyle: TCustomStyleServices;
begin
  Canvas.Font := TCustomGroupBox(Control).HeaderFont;
...
begin
  ...
  begin
    ...
    if Control is TCustomStaticText then
      S := TCustomStaticText(Control).Caption
    else
      S := Text;
...
var
  ...
  LTextRect: TRect;
begin
  LDC := HDC(Message.WParam);
  LSaveIndex := SaveDC(LDC);
  try
    LCanvas := TCanvas.Create;
    LCanvas.Handle := LDC;
    try
      SendMessage(Handle, EM_GETRECT, 0, LPARAM(@LTextRect));
      ExcludeClipRect(LCanvas.Handle, LTextRect.Left, LTextRect.Top, LTextRect.Right, LTextRect.Bottom);
      PaintBackground(LCanvas);
    finally
      LCanvas.Handle := 0;
      LCanvas.Free;
    end;
...
function TCustomComboHelper.GetLastComboMousePos: TSmallPoint;
begin
  Result := Default(TSmallPoint);
  if FLastComboMousePositions <> nil then
    FLastComboMousePositions.TryGetValue(Self, Result)
end;

function TCustomComboHelper.GetLastEditMousePos: TSmallPoint;
begin
  Result := Default(TSmallPoint);
  if FLastEditMousePositions <> nil then
    FLastEditMousePositions.TryGetValue(Self, Result)
end;

procedure TCustomComboHelper.SetLastComboMousePos(const Value: TSmallPoint);
begin
  if GetLastComboMousePos <> Value then
  begin
    if FLastComboMousePositions = nil then
      FLastComboMousePositions := TDictionary<TCustomCombo, TSmallPoint>.Create;
    FLastComboMousePositions.AddOrSetValue(Self, Value);
  end;
end;

procedure TCustomComboHelper.SetLastEditMousePos(const Value: TSmallPoint);
begin
  if GetLastEditMousePos <> Value then
  begin
    if FLastEditMousePositions = nil then
      FLastEditMousePositions := TDictionary<TCustomCombo, TSmallPoint>.Create;
    FLastEditMousePositions.AddOrSetValue(Self, Value);
  end;
end;

procedure TCustomComboHelper.RemoveFromDictionaries;
begin
  if FLastComboMousePositions <> nil then
    FLastComboMousePositions.Remove(Self);
  if FLastEditMousePositions <> nil then
    FLastEditMousePositions.Remove(Self);
end;

initialization
finalization
  TCustomCombo.FLastComboMousePositions.Free;
  TCustomCombo.FLastEditMousePositions.Free;
end.
