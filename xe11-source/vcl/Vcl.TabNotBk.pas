...
{$IF DEFINED(CLR)}
  ExtCtrls,
{$ENDIF}
  Winapi.Windows, System.Classes, Vcl.StdCtrls, Vcl.Forms, Winapi.Messages, Vcl.Graphics, Vcl.Controls,
  Vcl.ComCtrls;
...
begin
  If (FPageIndex >= 0) and (FPageIndex < FPageList.Count) then
    inherited AlignControls(TControl(FPageList[FPageIndex]), Rect);
end;
...
procedure TTabbedNotebook.ChangeScale(M, D: Integer);
begin
  inherited ChangeScale(M, D);
  TabFont.Height := MulDiv(TabFont.Height, M, D);
end;

procedure TTabbedNotebook.WMPaint(var Message: TWMPaint);
begin
  SendMessage(Handle, WM_SETFONT, WPARAM(TabFont.Handle), 0);
  inherited;
end;
