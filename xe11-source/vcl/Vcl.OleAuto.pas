...
uses Winapi.Windows, Winapi.Ole2, Winapi.OleCtl, System.SysUtils;
...
uses Vcl.OleConst, System.Win.ComObj, System.Classes;
...
begin
  ...
  KeyPath := TStringList.Create;
  KeyPath.Delimiter := '\';
  KeyPath.DelimitedText := Key;

  RegHandle := HKEY_CLASSES_ROOT;
  for i := 0 to KeyPath.Count - 1 do
    RegCreateKeyEx(RegHandle, PChar(KeyPath.Strings[i]),
          0, nil, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, nil, RegHandle, nil);

  // set default value
  RegSetValueEx(RegHandle, nil, 0, REG_SZ, PChar(Value), (1+Length(Value))*SizeOf(Char));
  ...
  repeat
    ...
    begin
      ...
      repeat
        ...
        Inc(PByte(Result), SizeOf(TAutoEntry));
        Dec(I);
      until I = 0;
    end;
    ClassRef := ClassRef.ClassParent;
...
begin
  ...
  try
    ...
    begin
      ...
      I := 0;
      repeat
        Dec(PByte(ArgPtr), SizeOf(TVarData));
    ...
    begin
      ...
      Dec(PByte(DispParams.rgdispidNamedArgs), SizeOf(Integer));
...
//      WideCharToStrVar(bstrDescription, Message);
      Len := Length(Message);
      while (Len > 0) and CharInSet(Message[Len], [#0..#32, '.']) do Dec(Len);
  ...
  try
    try
      ...
      begin
        ...
        ArgIndex := 0;
        repeat
          Dec(PByte(ParamPtr), SizeOf(Variant));
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                OleStrToStrVar(BStr^, AnsiString(PStr));
          ...
          begin
            ...
            begin
              StringPtr := nil;
              OleStrToStrVar(ArgPtr^.VOleStr, AnsiString(StringPtr));
              VariantClear(PVariant(ArgPtr)^);
              ArgPtr^.VType := varString;
              ArgPtr^.VString := StringPtr;
            end;
          end;
          Inc(PByte(ArgPtr), SizeOf(Variant));
          Inc(ArgIndex);
