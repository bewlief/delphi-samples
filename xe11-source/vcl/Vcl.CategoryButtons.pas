...
{$IF DEFINED(CLR)}
  WinUtils,
{$ENDIF}
  Winapi.Windows, Winapi.Messages, Vcl.ImgList, System.Classes, System.UITypes, Vcl.Forms,
...
{$ENDIF}
  strict private
    ...
    function GetDropDownSize: Integer;
  ...
  protected
    ...
    property SideBufferSize: Integer read FSideBufferSize;
    property DownButton: TButtonItem read FDownButton;
    property HotButton: TButtonItem read FHotButton;
    property InsertLeft: TBaseItem read FInsertLeft;
    property InsertTop: TBaseItem read FInsertTop;
    property InsertRight: TBaseItem read FInsertRight;
    property InsertBottom: TBaseItem read FInsertBottom;
  public
...
{ TBaseButtonItem }
  ...
  protected
    ...
    procedure UpdateImageName(Index: TImageIndex; var Name: TImageName); virtual;
    procedure UpdateImageIndex(Name: TImageName; var Index: TImageIndex); virtual;
  public
...
{ TButtonItem }
  ...
  protected
    ...
    procedure UpdateImageName(Index: TImageIndex; var Name: TImageName); override;
    procedure UpdateImageIndex(Name: TImageName; var Index: TImageIndex); override;
...
{ TButtonCategory }
  ...
  protected
    ...
    procedure SetVisibleGutter(const Value: Boolean);
  protected // friend
  ...
  published
    ...
    property VisibleGutter: Boolean read FVisibleGutter write SetVisibleGutter default True;
  end;
...
{$ENDIF}
  Vcl.ExtCtrls, System.SysUtils, System.Types;
...
procedure TCategoryButtons.ChangeScale(M, D: Integer; isDpiChange: Boolean);
begin
  inherited;
...
begin
  ...
  begin
    ...
    if not (boVerticalCategoryCaptions in ButtonOptions) then
      if Category.VisibleGutter then
        Inc(Result, FGutterSize);
  end;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        if boVerticalCategoryCaptions in ButtonOptions then
        begin
          Left := XStart + cBorderBuffer;
          if Category.VisibleGutter then
            Inc(Left, FGutterSize);
          Top := StartingPos;
        end
        else
        begin
...
function TCategoryButtons.GetDropDownSize: Integer;
begin
...
begin
  Result.Left := CategoryBounds.Left + ScaleValue(2);
  Result.Top := CategoryBounds.Top + ScaleValue(2);
  Result.Right := Result.Left + GetDropDownSize;
  Result.Bottom := Result.Top + GetDropDownSize;
end;
...
var
  ...
  var
    ...
    begin
      ...
      begin
        if Collapsed then
          LDetails := LStyle.GetElementDetails(tcbCategoryGlyphClosed)
        else
          LDetails := LStyle.GetElementDetails(tcbCategoryGlyphOpened);
        if not LStyle.GetElementSize(Canvas.Handle, LDetails, esActual, LSize, FCurrentPPI) then
        begin
          LSize.cx := ScaleValue(9);
          LSize.cy := LSize.cx;
        end;
        LStyle.DrawElement(Canvas.Handle, LDetails, Rect(X, Y, X + LSize.cx, Y + LSize.cy), nil, FCurrentPPI);
      end
      else
      begin
  ...
  begin
    ...
    begin
      Middle := GetDropDownSize div 2;
      LPoint := TPoint.Create(X + Middle - ScaleValue(ChevronXPosAdjust[Collapsed]),
        Y + Middle - ScaleValue(ChevronYPosAdjust[Collapsed]));
      if not IsStyleEnabled or
         not LStyle.DrawElement(Canvas.Handle, LStyle.GetElementDetails(Elements[Collapsed]),
           TRect.Create(LPoint, ScaleValue(3), 0), nil, CurrentPPI) then
      begin
        if IsStyleEnabled and
          ...
          Canvas.Pen.Color := clBtnShadow;
        DrawChevron(Canvas, ChevronDirection[Collapsed], LPoint, ScaleValue(3));
      end;
    end;
  end;
...
begin
  ...
  begin
    ...
    begin
      if IsCustomStyleActive and (seClient in StyleElements) and
    ...
    begin
      ...
      if BackgroundGradientColor <> clNone then
        ...
        begin
          LDetails := LStyle.GetElementDetails(tcbBackground);
          if IsCustomStyleActive and LStyle.GetElementColor(LDetails, ecFillColor, LColor) and
             (LColor <> clNone)
          then
            TempColor := LColor
          else
    ...
    if VerticalCaption then
      CatHeight := CategoryBounds.Bottom - CategoryBounds.Top - 3 - GetDropDownSize
    else
      CatHeight := CategoryBounds.Right - CategoryBounds.Left - 2 - GetDropDownSize;
    ...
    begin
      CaptionRect.Left := CategoryBounds.Left + 4 + GetDropDownSize;
...
begin
  ...
  begin
    ...
    if IsStyleEnabled and IsCustomStyleActive and not (seFont in StyleElements) then
      FontColor := Font.Color;
...
begin
  ...
  begin
    ...
    begin
      ...
      if DownCategory.VisibleGutter then
      begin
...
begin
  ...
  begin
    ...
    begin
      DownCategory := GetCategoryAt(X, Y);
      if ((DownCategory <> nil) and
          (DownCategory.Items <> nil) and
          (DownCategory.Items.Count > 0) and
          DownCategory.VisibleGutter) then
      begin
...
begin
  ...
  begin
    ...
    begin
      ...
      if (Category.Items <> nil) and (ResIndex >= 0) and (ResIndex < Category.Items.Count) then
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        else if Category.VisibleGutter then
          Result := Category;
      end
    end
...
begin
  ...
  begin
    ...
    begin
      ...
      if not CurrentCat.Collapsed and CurrentCat.VisibleGutter then { exclude the button area on the right }
...
begin
  ...
  begin
    ...
    begin
      ...
      Button := TButtonItem(FSelectedItem);
      if Button.Category.Collapsed then
    ...
    if FSelectedItem is TButtonCategory then
    begin
      var ButtonCategory := TButtonCategory(FSelectedItem);
      if not ButtonCategory.VisibleGutter and (ButtonCategory.Items <> nil) and (ButtonCategory.Items.Count > 0) then
        SetSelectedItem(ButtonCategory.Items[0]);
    end
  end;
end;
...
begin
  ...
  if not Category.VisibleGutter then
  begin
...
function TBaseButtonItem.IsImageNameStored: Boolean;
begin
  Result := (FImageName <> '') and ((ActionLink = nil) or not ActionLink.IsImageNameLinked);
end;
...
procedure TBaseButtonItem.UpdateImageIndex(Name: TImageName; var Index: TImageIndex);
begin
end;

procedure TBaseButtonItem.UpdateImageName(Index: TImageIndex; var Name: TImageName);
begin
end;
...
procedure TBaseButtonItem.SetImageName(const Value: TImageName);
begin
...
procedure TButtonItem.UpdateImageName(Index: TImageIndex; var Name: TImageName);
var
...
begin
  ...
  if (LImages <> nil) and LImages.IsImageNameAvailable then
    Name := LImages.GetNameByIndex(Index);
end;

procedure TButtonItem.UpdateImageIndex(Name: TImageName; var Index: TImageIndex);
var
  LImages: TCustomImageList;
begin
  if CategoryButtons <> nil then
    ...
    LImages := nil;
  if (LImages <> nil) and LImages.IsImageNameAvailable then
    Index := LImages.GetIndexByName(Name);
end;
...
procedure TButtonCategory.SetVisibleGutter(const Value: Boolean);
begin
  if FVisibleGutter <> Value then
  begin
    FVisibleGutter := Value;
    Changed(False);
  end;
end;
...
function TButtonItemActionLink.IsImageNameLinked: Boolean;
begin
