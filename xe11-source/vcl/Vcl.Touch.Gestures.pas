  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if csRecreating in Control.ControlState then
          FStylusGestureEngine.Handle := 0;
      end;
    end;
  end;
end;
...
begin
  ...
  try
    ...
    if (goUniDirectional in Options) and
       not TPoint.PointInCircle(LPoints[0], GesturePoints[0], Deviation) then
      Exit;

    PointCount := Length(LPoints);
    if (goEndpoint in Options) and
      (not TPoint.PointInCircle(LPoints[PointCount - 1], GesturePoints[GesturePointCount - 1], Deviation)) then
    begin
      Exit;
    end;
    ...
    if (Result < LErrorMargin) and not (goUniDirectional in Options) and
      TPoint.PointInCircle(LPoints[0], GesturePoints[GesturePointCount - 1], Deviation) then
    begin
...
begin
  ...
  begin
    ...
    while (PointsIndex + 1 < PointsCount) and
          TPoint.PointInCircle(GesturePoints[Index], Points[PointsIndex + 1], Deviation) do
      Inc(PointsIndex);
  ...
  begin
    ...
    while (GestureIndex + 1 < GestureCount) and
          TPoint.PointInCircle(Points[Index], GesturePoints[GestureIndex + 1], Deviation) do
      Inc(GestureIndex);
