...
{ TFileListBox }
  ...
  {$IFDEF CLR}[SecurityPermission(SecurityAction.Demand, Unrestricted=True)]{$ENDIF}
...
{$ENDIF}
  System.Types, System.UITypes, Vcl.Consts, System.RTLConsts, Vcl.Dialogs, Winapi.ShlObj, Winapi.ActiveX, Vcl.Themes;
...
{$ENDIF}
  ...
  try
    if System.SysUtils.DirectoryExists(Root) then
      ChDir(Root);
    ...
    if System.SysUtils.DirectoryExists(SaveDir) then
...
begin
  ...
  if System.SysUtils.DirectoryExists(FDirectory) then
    ChDir(FDirectory);
...
begin
  ...
  begin
    ...
    begin
      ...
      raise EInvalidOperation.CreateFmt(SInvalidKnownFilename, [EditText]);
  end;
end;
...
var
  LPPIActiveForm: Integer;

function ScaleV(const AValue: Integer): Integer;
begin
  Result := MulDiv(AValue, LPPIActiveForm, Screen.DefaultPixelsPerInch);
end;

begin
  inherited CreateNew(AOwner);
  LPPIActiveForm := GetPPIForActiveForm;
  ScaleForPPI(LPPIActiveForm);
  Font.Assign(Screen.MessageFont);
  Font.Height := MulDiv(Font.Height, LPPIActiveForm, Screen.PixelsPerInch);

  Caption := SSelectDirCap;
  if TStyleManager.IsCustomStyleActive then
    ClientWidth := ScaleV(430)
  else
    ClientWidth := ScaleV(424);
  ClientHeight := ScaleV(255);
  ...
  begin
    Parent := Self;
    SetBounds(ScaleV(8), ScaleV(24), ScaleV(313), ScaleV(20));
    Visible := False;
    TabOrder := 1;
  end;
  ...
  begin
    Parent := Self;
    SetBounds(ScaleV(8), ScaleV(8), ScaleV(92), ScaleV(13));
  ...
  begin
    Parent := Self;
    SetBounds(ScaleV(232), ScaleV(192), ScaleV(185), ScaleV(19));
    DriveList.ItemHeight := ScaleV(17);
    TabOrder := 2;
  ...
  begin
    Parent := Self;
    SetBounds(ScaleV(232), ScaleV(176), ScaleV(41), ScaleV(13));
    Caption := SDrivesCap;
  ...
  begin
    Parent := Self;
    SetBounds(ScaleV(120), ScaleV(8), ScaleV(213), ScaleV(13));
  end;
  ...
  begin
    Parent := Self;
    ItemHeight := ScaleV(17);
    SetBounds(ScaleV(8), ScaleV(70), ScaleV(213), ItemHeight * 8 + 4);
    TabOrder := 0;
    TabStop := True;
  ...
  begin
    Parent := Self;
    SetBounds(ScaleV(8), ScaleV(54), ScaleV(66), ScaleV(13));
    Caption := SDirsCap;
  ...
  begin
    ...
    IntegralHeight := False;
    ItemHeight := ScaleV(17);
    SetBounds(ScaleV(232), ScaleV(70), ScaleV(185), ItemHeight * 6 + 4);
  end;

  with TLabel.Create(Self) do
  begin
    Parent := Self;
    SetBounds(ScaleV(232), ScaleV(54), ScaleV(57), ScaleV(13));
    Caption := SFilesCap;
  ...
  begin
    Parent := Self;
    SetBounds(ScaleV(8), ScaleV(224), ScaleV(77), ScaleV(27));
    Visible := False;
    TabOrder := 3;
    Caption := SNetworkCap;
    OnClick := NetClick;
  ...
  begin
    Parent := Self;
    SetBounds(ScaleV(172), ScaleV(224), ScaleV(77), ScaleV(27));
    TabOrder := 4;
    OnClick := OKClick;
    Caption := SOKButton;
    ModalResult := 1;
  ...
  begin
    Parent := Self;
    SetBounds(ScaleV(256), ScaleV(224), ScaleV(77), ScaleV(27));
    TabOrder := 5;
    Cancel := True;
  ...
  begin
    Parent := Self;
    SetBounds(ScaleV(340), ScaleV(224), ScaleV(77), ScaleV(27));
    TabOrder := 7;
    Caption := SHelpButton;
...
begin
  if AllowCreate and Prompt and (not System.SysUtils.DirectoryExists(Directory)) and
  ...
  try
    ...
    begin
      ...
      if sdPerformCreate in Options then
        System.SysUtils.ForceDirectories(Directory);
    end;
  finally
    D.Free;
  end;
...
{$IFDEF CLR}[SecurityPermission(SecurityAction.Demand, UnmanagedCode=True)]{$ENDIF}
function SelectDirectory(const Caption: string; const Root: WideString;
...
begin
  ...
  if sdHidePinnedPlaces in Options then
    LOptions := LOptions + [fdoHidePinnedPlaces];
  if sdNoDereferenceLinks in Options then
    LOptions := LOptions + [fdoNoDereferenceLinks];
  if sdForceShowHidden in Options then
    LOptions := LOptions + [fdoForceShowHidden];
  if sdAllowMultiselect in Options then
  ...
  try
    ...
    if Title <> '' then
      LFileDialog.Title := Title;
    if StartDirectory <> '' then
      LFileDialog.DefaultFolder := StartDirectory;
    ...
    begin
      ...
      begin
        ...
        begin
          SetLength(Directories, LSelectedItemCount);
          LIndex := 0;
          ...
          begin
            ...
            begin
              Directories[LIndex] := LShellItemName;
              Inc(LIndex);
            end
            else
      ...
      begin
        SetLength(Directories, 1);
        Directories[0] := LFileDialog.FileName;
      end;
    end;
  finally
    LFileDialog.Free;
