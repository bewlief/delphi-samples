...
type
  ...
  var
    ...
    FMouseDown, FMouseTracking: Boolean;
...
implementation
  uses System.UITypes, WinApi.DWMApi;
...
begin
  ...
  FIsDWMEnabled := DWMCompositionEnabled;
...
var
  ...
  var
    ...
    LSubMenuRect: TRect;
  begin
    LSubMenuRect := TRect.Empty;
    LSubMenuDetail := tmPopupSubMenuNormal;
    if isDisabled in State then
      LSubMenuDetail := tmPopupSubMenuDisabled;
    LSubMenuDetails := LStyle.GetElementDetails(LSubMenuDetail);
    LStyle.GetElementSize(DC, LSubMenuDetails, esActual, SubMenuSize, LPPI);
    if not RightToLeft then
      LSubMenuRect := Rect(ItemRect.Right - SubMenuSize.cx,
        ItemRect.Top + (ItemRect.Height - SubMenuSize.cy) div 2 - 4,
        ItemRect.Right,
        ItemRect.Top + (ItemRect.Height - SubMenuSize.cy) div 2 - 4 + SubMenuSize.cy)
    else
    ...
    try
      ...
      LStyle.DrawElement(Bmp.Canvas.Handle, LSubMenuDetails,
        Rect(0, 0, SubMenuSize.Width, SubMenuSize.Height), nil, LPPI);
  ...
  begin
    ...
    if LStyle.GetElementColor(LDetails, ecTextColor, LColor) then
...
begin
  ...
  LDetails := LStyle.GetElementDetails(Detail);
  ...
  begin
    ...
    try
      ...
      LStyle.DrawElement(Buffer.Canvas.Handle, LDetails, SrcRect);
      DstRect := R;
  ...
  if (Detail <> tmPopupItemNormal) and (Detail <> tmPopupItemDisabled) then
    LStyle.DrawElement(DC, LDetails, R);
    ...
    begin
      ...
      if SysItem.RadioCheck then
        Detail := TThemedMenu(Integer(tmPopupBulletNormal) +
          Integer(SysItem.Disabled));
      LDetails := LStyle.GetElementDetails(Detail);
      LStyle.GetElementSize(DC, LDetails, esActual, LSize);
      LSize.cx := MulDiv(LSize.cx, LPPI, Screen.DefaultPixelsPerInch);
      LSize.cy := LSize.cx;
      ...
      LStyle.DrawElement(DC, LDetails, LImageRect, nil, LPPI);
    end;
  ...
  begin
    ...
    begin
      ...
      if TStyleManager.SystemStyle.Enabled then
        Inc(LTextRect.Left, MulDiv(30, LPPI, Screen.DefaultPixelsPerInch))
      else
        Inc(LTextRect.Left, MulDiv(20, LPPI, Screen.DefaultPixelsPerInch));
  end
  else
    begin
      ...
      begin
        ...
        if TStyleManager.SystemStyle.Enabled then
          Dec(LTextRect.Right, MulDiv(30, LPPI, Screen.DefaultPixelsPerInch))
        else
          Dec(LTextRect.Right, MulDiv(20, LPPI, Screen.DefaultPixelsPerInch));
      Exclude(LTextFormat, tfLeft);
      Include(LTextFormat, tfRtlReading);
      Include(LTextFormat, tfRight);
    end;

  LDetails := LStyle.GetElementDetails(tmPopupItemNormal);
  if isHot in State then
    LDetails := LStyle.GetElementDetails(tmPopupItemHot);
  if isDisabled in State then
    LDetails := LStyle.GetElementDetails(tmPopupItemDisabled);
...
begin
  LStyle := StyleServices;
  LDetails := LStyle.GetElementDetails(tmPopupBorders);
  LStyle.DrawElement(Canvas.Handle, LDetails, SysControl.ClientRect);
end;
...
begin
  ...
  case Message.Msg of
    ...
    MN_SELECTITEM, WM_PRINT:
      begin
        if (Message.Msg = MN_SELECTITEM) and (GetKeyState(VK_LBUTTON) < 0)
           and FItemsPainted and not FMouseDown and FMouseTracking
        then
        begin
          GetCursorPos(P);
    ...
    WM_NCCALCSIZE, WM_NCPAINT:
     ...
     begin
       ...
       begin
         S := Items[i].Text;
         if Items[i].HasSubMenu and (S <> '-') and (S <> '') and IsAccel(TWMChar(Message).CharCode, S) then
         begin
           j := i;
           Break;
         end;
       ...
       begin
         FCallFromKeyboard := True;
         FMouseDown := True;
         SendMessage(Handle, MN_SELECTITEM, j, 0);
         FPreviousHotItemIndex := -1;
...
type
  ...
  private
    function GetMenuHandle: TMenuHandle;
    property MenuHandle:TMenuHandle read GetMenuHandle;
  end;
...
function TMenuItemHelper.GetMenuHandle: TMenuHandle;
begin
  with Self do
    Exit(FHandle);
end;

function TPopupMenuHelper.GetMenuHandle: TMenuHandle;
begin
  if Assigned(Items) then
    Result := Items.GetMenuHandle
  else
    Result := 0;
end;
...
begin
  ...
  for I := 0 to PopupList.Count - 1 do
    if TPopupMenu(PopupList[I]).MenuHandle = FMenu then
...
begin
  ...
  begin
    ...
    try
      ...
      LDetails := LStyle.GetElementDetails(Detail);
      LStyle.DrawElement(BmpDC, LDetails, R);
      { Draw Left Button }
      ...
      R := Rect(0, 0, cx, cy);
      LDetails := LStyle.GetElementDetails(Detail);
      LStyle.DrawElement(BmpDC, LDetails, R, nil, LPPI);
      { Draw Slider Button }
      ...
      P := NormalizePoint(P);
      BitBlt(DC, P.X, P.Y, HorzScrollRect.Width,
        HorzScrollRect.Height, B.Canvas.Handle, 0, 0, SRCCOPY);
      B.Free;
      if LDC = 0 then
        ReleaseDC(Handle, DC);
    end;
...
begin
  if Assigned(Application) and (SysControl.Style and DS_CONTROL <> DS_CONTROL) and
    (Application.DialogHandle = Handle) then
  begin
...
begin
  ...
  begin
    if not UseSmallBorder then
      LDetails := LStyle.GetElementDetails(twCaptionActive)
    else
  ...
  begin
    if not UseSmallBorder then
      LDetails := LStyle.GetElementDetails(twCaptionInActive)
    else
      LDetails := LStyle.GetElementDetails(twSmallCaptionInActive);
  end;
  LStyle.GetElementSize(0, LDetails, esActual, ElementSize, CurrentPPI);
...
var
  ...
  LDetails: TThemedElementDetails;
begin
  if FCloseButtonRect.Width > 0 then
    Exit(FCloseButtonRect);
  ...
  begin
    ...
    FCloseButtonRect := Result;

    if FPPI > Screen.DefaultPixelsPerInch then
    begin
      FCloseButtonRect.Left := FCloseButtonRect.Right - MulDiv(Result.Width, FPPI, Screen.DefaultPixelsPerInch);
      FCloseButtonRect.Bottom := FCloseButtonRect.Top + MulDiv(Result.Height, FPPI, Screen.DefaultPixelsPerInch);
...
begin
  ...
  begin
    ...
    FHelpButtonRect := Result;

    if FPPI > Screen.DefaultPixelsPerInch then
    begin
      FHelpButtonRect.Left := FHelpButtonRect.Right - MulDiv(Result.Width, FPPI, Screen.DefaultPixelsPerInch);
      FHelpButtonRect.Bottom := FHelpButtonRect.Top + MulDiv(Result.Height, FPPI, Screen.DefaultPixelsPerInch);
...
var
  ...
  LDetails: TThemedElementDetails;
begin
 if FMaxButtonRect.Width > 0 then
    Exit(FMaxButtonRect);
  ...
  begin
    ...
    FMaxButtonRect := Result;

    if FPPI > Screen.DefaultPixelsPerInch then
    begin
      FMaxButtonRect.Left := FMaxButtonRect.Right - MulDiv(Result.Width, FPPI, Screen.DefaultPixelsPerInch);
      FMaxButtonRect.Bottom := FMaxButtonRect.Top + MulDiv(Result.Height, FPPI, Screen.DefaultPixelsPerInch);
...
begin
  ...
  begin
    ...
    FMinButtonRect := Result;

    if FPPI > Screen.DefaultPixelsPerInch then
    begin
      FMinButtonRect.Left := FMinButtonRect.Right - MulDiv(Result.Width, FPPI, Screen.DefaultPixelsPerInch);
      FMinButtonRect.Bottom := FMinButtonRect.Top + MulDiv(Result.Height, FPPI, Screen.DefaultPixelsPerInch);
...
begin
  ...
  LStyle.GetElementSize(0, Details, esActual, Size, PPI);
...
begin
  ...
  DeleteObject(FRegion);
  LDetails := LStyle.GetElementDetails(Detail);
  if not LStyle.GetElementRegion(LDetails, R, Result) then
    FRegion := 0;
end;
...
var
  ...
  R: TRect;
begin
  if FSysMenuButtonRect.Width > 0 then
    Exit(FSysMenuButtonRect);
  ...
  begin
    ...
    if not StyleServices.GetElementContentRect(0, IconDetails, CaptionRect, FSysMenuButtonRect) then
      FSysMenuButtonRect := TRect.Empty;
    if FCaptionHeight > 0 then
    begin
      FSysMenuButtonRect.Top := 0;
      FSysMenuButtonRect.Bottom := FCaptionHeight;
    end;
    R := Rect(0, 0, GetSystemMetrics(SM_CXSMICON), GetSystemMetrics(SM_CYSMICON));
    RectVCenter(R, FSysMenuButtonRect);
...
begin
  ...
  ButtonRect := TRect.Empty;;
  FCaptionRect := TRect.Empty;
  FCloseButtonRect := TRect.Empty;
  FSysMenuButtonRect := TRect.Empty;
  FMinButtonRect:= TRect.Empty;
  FMaxButtonRect := TRect.Empty;
  FHelpButtonRect := TRect.Empty;
  LPrevButtonRect := TRect.Empty;
  ...
  LStyle.DrawElement(DC, LDetails, LCaptionRect, nil, FPPI);
  ...
  begin
    ...
    ButtonRect := GetSysMenuButtonRect;
    ...
    FSysMenuButtonRect := Rect(R.Left, R.Top, R.Left + IW, R.Top + IH);
    TextRect.Left := FSysMenuButtonRect.Right + 5;
  end
  else
    Inc(TextRect.Left, 7);
  ...
  begin
    ...
    begin
      ...
      LDetails := LStyle.GetElementDetails(FButtonState);
    ButtonRect := CloseButtonRect;
    if (ButtonRect.Width > 0) then
    begin
      LPrevButtonRect := ButtonRect;
      LStyle.DrawElement(CaptionBmp.Canvas.Handle, LDetails, ButtonRect, nil, FPPI);
    end;
  ...
  begin
    ...
    if FPPI > Screen.DefaultPixelsPerInch then
    begin
      if LPrevButtonRect.Width > 0 then
        ButtonRect := Rect(LPrevButtonRect.Left - FMaxButtonRect.Width,
          ButtonRect.Top, LPrevButtonRect.Left, ButtonRect.Bottom)
      else
        ButtonRect := Rect(CaptionBmp.Width - FMaxButtonRect.Right - MulDiv(CaptionBmp.Width - FMaxButtonRect.Right, FPPI, Screen.DefaultPixelsPerInch),
          ButtonRect.Top, MulDiv(CaptionBmp.Width - FMaxButtonRect.Right, FPPI, Screen.DefaultPixelsPerInch), ButtonRect.Bottom);
  ...
  begin
    ...
    begin
      if LPrevButtonRect.Width > 0 then
        ButtonRect := Rect(LPrevButtonRect.Left - FMinButtonRect.Width,
          ButtonRect.Top, LPrevButtonRect.Left, ButtonRect.Bottom)
      else
        ButtonRect := Rect(CaptionBmp.Width - FMinButtonRect.Right - MulDiv(CaptionBmp.Width - FMinButtonRect.Right, FPPI, Screen.DefaultPixelsPerInch),
          ButtonRect.Top, MulDiv(CaptionBmp.Width - FMinButtonRect.Right, FPPI, Screen.DefaultPixelsPerInch), ButtonRect.Bottom);
      FMinButtonRect := ButtonRect;
    end;

    if ButtonRect.Width > 0 then
    begin
      LStyle.DrawElement(CaptionBmp.Canvas.Handle, LDetails, ButtonRect);
      LPrevButtonRect := ButtonRect;
    end;
    if ButtonRect.Left > 0 then
      TextRect.Right := ButtonRect.Left;
  ...
  begin
    ...
    if not LStyle.GetElementContentRect(0, LDetails, LCaptionRect, ButtonRect) then
      ButtonRect := TRect.Empty;

    if FPPI > Screen.DefaultPixelsPerInch then
    begin
      if LPrevButtonRect.Width > 0 then
        ButtonRect := Rect(LPrevButtonRect.Left - FHelpButtonRect.Width,
          ButtonRect.Top, LPrevButtonRect.Left, ButtonRect.Bottom)
      else
        ButtonRect := Rect(CaptionBmp.Width - FHelpButtonRect.Right - MulDiv(CaptionBmp.Width - FHelpButtonRect.Right, FPPI, Screen.DefaultPixelsPerInch),
          ButtonRect.Top, MulDiv(CaptionBmp.Width - FHelpButtonRect.Right, FPPI, Screen.DefaultPixelsPerInch), ButtonRect.Bottom);
  ...
  begin
    ...
    LStyle.DrawText(CaptionBmp.Canvas.Handle, CaptionDetails, LText, TextRect, TextFormat, clNone, FPPI);
    MoveWindowOrg(CaptionBmp.Canvas.Handle, 0, -TextTopOffset);
  end
  else
    LStyle.DrawText(CaptionBmp.Canvas.Handle, CaptionDetails, LText, TextRect, TextFormat, clNone, FPPI);
  ...
  begin
    if not UseSmallBorder then
      LDetails := LStyle.GetElementDetails(twFrameLeftActive)
    else
      LDetails := LStyle.GetElementDetails(twSmallFrameLeftActive);
  end
  else
  begin
    if not UseSmallBorder then
      LDetails := LStyle.GetElementDetails(twFrameLeftInActive)
    else
      LDetails := LStyle.GetElementDetails(twSmallFrameLeftInActive);
  end;
  ...
  begin
    if not UseSmallBorder then
      LDetails := LStyle.GetElementDetails(twFrameRightActive)
    else
      LDetails := LStyle.GetElementDetails(twSmallFrameRightActive);
  end
  else
  begin
    if not UseSmallBorder then
      LDetails := LStyle.GetElementDetails(twFrameRightInActive)
    else
      LDetails := LStyle.GetElementDetails(twSmallFrameRightInActive);
  end;
  ...
  begin
    if not UseSmallBorder then
      LDetails := LStyle.GetElementDetails(twFrameBottomActive)
    else
      LDetails := LStyle.GetElementDetails(twSmallFrameBottomActive);
  end
  else
  begin
    if not UseSmallBorder then
      LDetails := LStyle.GetElementDetails(twFrameBottomInActive)
    else
      LDetails := LStyle.GetElementDetails(twSmallFrameBottomInActive);
  end;
    ...
    WM_CREATE:
      begin
        ...
        if (FWidth >= CXMin) and (FHeight >= CYMin) then
          SetWindowPos(Handle, 0, 0, 0, FWidth + DFBW + 5, FHeight + DFBW + 5,
        ...
        begin
          ...
          try
            ...
            try
              ...
              begin
                LDetails := LStyle.GetElementDetails(tsSizeBoxLeftAlign);
                LStyle.DrawElement(DC, LDetails, SysControl.ClientRect, nil, CurrentPPI);
              end;
            finally
              ReleaseDC(Handle, DC);
            end;
...
begin
  ...
  LStyle.DrawElement(Canvas.Handle, LDetails, LRect);
  ...
  begin
    if (GetWindowLong(Handle, GWL_STYLE) and BS_COMMANDLINK) = BS_COMMANDLINK then
      IY := DrawRect.Top + MulDiv(15, LPPI, Screen.DefaultPixelsPerInch)
    else
  ...
  begin
      ...
      if LStyle.GetElementColor(LDetails, ecTextColor, ThemeTextColor) then
         Canvas.Font.Color := ThemeTextColor;
      LStyle.DrawText(Canvas.Handle, LDetails, LText, DrawRect, TextFormat, Canvas.Font.Color);
      ...
      begin
        ...
        begin
          ...
          Canvas.Font.Size := 8;
          LStyle.DrawText(Canvas.Handle, LDetails, Buffer, DrawRect,
      ...
      begin
        if MouseDown then
          LDetails := LStyle.GetElementDetails(tbCommandLinkGlyphPressed)
        else if MouseInControl then
          LDetails := LStyle.GetElementDetails(tbCommandLinkGlyphHot)
        else if SysControl.Enabled then
          LDetails := LStyle.GetElementDetails(tbCommandLinkGlyphNormal)
        else
          LDetails := LStyle.GetElementDetails(tbCommandLinkGlyphDisabled);
        DrawRect.Right := IW;
        DrawRect.Left := 3;
        DrawRect.Top := 10;
        DrawRect.Bottom := DrawRect.Top + IW;
        LStyle.DrawElement(Canvas.Handle, LDetails, DrawRect);
      end;
  ...
  if SplitButton then
    ...
    begin
      { draw vertical line }
      IW := MulDiv(IW, LPPI, Screen.DefaultPixelsPerInch);
      Pen.Color := LStyle.GetSystemColor(clBtnShadow);
      MoveTo(Width - IW, 3);
      LineTo(Width - IW, Height - 3);
      if Enabled then
        Pen.Color := LStyle.GetSystemColor(clBtnHighLight)
      else
        Pen.Color := Font.Color;
      MoveTo(Width - IW - 1, 3);
      LineTo(Width - IW - 1, Height - 3);
      { Draw arrow }
      Pen.Color := Font.Color;
      X := Width - IW div 2 + 1;
      Y := Height div 2 + 1;
      IH := MulDiv(3, LPPI, Screen.DefaultPixelsPerInch);
      for i := IH downto 0 do
      begin
        MoveTo(X - i, Y - i);
        LineTo(X + i + 1, Y - i);
      end;
...
begin
  ...
  if not LStyle.GetElementSize(DC, LDetails, esActual, Size, LPPI) then
  begin
  ...
  LStyle.DrawElement(DC, LDetails, BoxRect, nil, LPPI);
...
begin
  ...
  R := GetBoxRect;
  R.Top := CaptionRect.Height div 2 + 2;
  LStyle := StyleServices;

  if SysControl.Enabled then
    LDetails := LStyle.GetElementDetails(tbGroupBoxNormal)
  else
    LDetails := LStyle.GetElementDetails(tbGroupBoxDisabled);
  ...
  { Paint Text }
  LStyle.DrawText(Canvas.Handle, LDetails, SysControl.Text, CaptionRect,
...
begin
  ...
  begin
    ...
    if LStyle.Available then
    begin
      Color := LStyle.GetStyleColor(ColorStates[SysControl.Enabled]);
...
begin
  ...
  try
    ...
    begin
      ...
      LStyle.DrawElement(Buffer.Canvas.Handle, Details, ButtonRect, nil, CurrentPPI);
    end;
...
begin
  ...
  begin
    ...
    begin
      Color := LStyle.GetStyleColor(ColorStates[SysControl.Enabled]);
      FontColor := LStyle.GetStyleFontColor(FontColorStates[True]);
    end;
  end;
end;
{$HINTS ON}
...
begin
  ...
  begin
    LDetails := LStyle.GetElementDetails(tbCheckBoxUncheckedNormal);
    LStyle.DrawParentBackground(Handle, Canvas.Handle, LDetails, False);
  ...
  LDetails := LStyle.GetElementDetails(States[SysControl.Enabled]);
  Canvas.Font := SysControl.Font;
  DrawText(Canvas.Handle, LDetails, SysControl.Text, LRect, TextFormat);
end;
...
begin
  ...
  begin
    ...
    try
      ...
      Frame3D(LBitMap.Canvas, LRect, LStyle.ColorToRGB(clBtnShadow),
        LStyle.ColorToRGB(clBtnHighLight), 1);
...
begin
  ...
  begin
    ...
    FontColor := LStyle.GetSystemColor(clBtnText);
  end;
...
begin
  ...
  begin
    ...
    if LStyle.Available then
    begin
      Color := LStyle.GetStyleColor(scListView);
      FontColor := LStyle.GetSystemColor(clWindowText);
...
begin
  ...
  Bmp := TBitmap.Create;
  try
    Bmp.Canvas.Font := SysControl.Font;
    Bmp.SetSize(SysControl.Width, SysControl.Height);
    ...
    LDetails := LStyle.GetElementDetails(thHeaderItemNormal);
    LStyle.DrawElement(DC, LDetails, R);
    ...
    begin
      ...
      LDetails := LStyle.GetElementDetails(thHeaderItemNormal);
      if SectionHot then
        LDetails := LStyle.GetElementDetails(thHeaderItemHot);
      if FPressedSection = I then
        LDetails := LStyle.GetElementDetails(thHeaderItemPressed);
      LStyle.DrawElement(DC, LDetails, LSectionRect);
      ...
      begin
        LSplitDetails := LStyle.GetElementDetails(ttbDropDownButtonGlyphHot);;
        R := LDropDownRect;
        if SectionHot then
        begin
          Inc(R.Left, 3);
          LStyle.DrawElement(DC, LSplitDetails, R, nil, LPPI);
          Dec(R.Left, 3);
          with Bmp.Canvas do
          begin
            Pen.Color := LStyle.GetSystemColor(clBtnShadow);
      ...
      LStyle.DrawText(DC, LDetails, LText, TxtRect, LTextFormat);
    end;
    Canvas.Draw(0, 0, Bmp);
  finally
    Bmp.Free;
...
begin
  ...
  begin
    ...
    if LStyle.Available then
    begin
      Color := LStyle.GetStyleColor(scTreeView);
      FontColor := LStyle.GetSystemColor(clWindowText)
    end;
  end;
end;
...
var
  LOldTimerMode: Integer;
begin
  LOldTimerMode := FTimerMode;
  inherited;
  if (FScrollKind = sbVertical) and (LOldTimerMode = 3) then
  begin
...
begin
  ...
  LStyle := StyleServices;
  Details := LStyle.GetElementDetails(ttPane);
...
begin
  ...
  begin
    ...
    if LStyle.Available then
      LStyle.DrawIcon(Canvas.Handle, Details, GlyphR, Images.Handle, FImageIndex);
  end;
  ...
  begin
    ...
    begin
      ...
      LStyle.DrawText(Canvas.Handle, Details, Tabs[Index], LayoutR, [tfSingleLine, tfVerticalCenter, tfCenter, tfNoClip]);
...
var
  ...
  LButtonHot: Boolean;
  P: TPoint;
  LToolStyle: TSysToolbarButtonStyle;
  LToolState: TSysToolbarButtonState;
  Bmp: TBitmap;
...
begin
  ...
  try
    ...
    Bmp.SetSize(SysControl.Width, SysControl.Height);
    R := Rect(0, 0, Bmp.Width, Bmp.Height);
    // Bmp.Canvas.Brush.Color := LStyle.GetStyleColor(scWindow);
    // Bmp.Canvas.FillRect(R);
    DC := Bmp.Canvas.Handle;
    ...
    begin
      ...
      if LStyle.HasTransparentParts(LDetails) then
        LStyle.DrawParentBackground(Handle, DC, LDetails, False);
      LStyle.DrawElement(DC, LDetails, R);
    end;
  except
    Bmp.Free;
  ...
  try
    ...
    begin
      ...
      LButtonHot := False;
      if not(bsHidden in LToolState) then
      begin
        if MouseInControl then
        begin
          GetCursorPos(P);
        ...
        if (bsEnabled in LToolState) then
          LDetails := LStyle.GetElementDetails(ttbButtonNormal)
        else
          LDetails := LStyle.GetElementDetails(ttbButtonDisabled);
        if (LButtonHot) and (bsEnabled in LToolState) then
        begin
          LDetails := LStyle.GetElementDetails(ttbButtonHot);
        end;
        if (bsPressed in LToolState) and (bsEnabled in LToolState) then
          LDetails := LStyle.GetElementDetails(ttbButtonPressed);

        if bsChecked in LToolState then
          LDetails := LStyle.GetElementDetails(ttbButtonChecked);

        if not(bsSep in LToolStyle) then
        begin
          if Flat then
          begin
            // Bmp.Canvas.FillRect(ItemRect);
            DrawParentBackground(DC, @ItemRect);
            if (LButtonHot or (bsPressed in LToolState) or (bsChecked in LToolState)) and (bsEnabled in LToolState) then
            begin
              LStyle.DrawElement(DC, LDetails, ItemRect);
            end;
          end
          else
            LStyle.DrawElement(DC, LDetails, ItemRect);
        ...
        begin
          LDetails := LStyle.GetElementDetails(ttbSeparatorNormal);
          LStyle.DrawElement(DC, LDetails, ItemRect);
        end;
        ...
        begin
          ...
          if bsDropDown in LToolStyle then
          begin
            { If button is DropDown then draw the button glyph. }
            R := ItemRect;
            R := Rect(R.Right - LDropDownWidth, R.Top, R.Right, R.Bottom);
            if bsEnabled in LToolState then
              LDetails := LStyle.GetElementDetails(ttbDropDownButtonGlyphNormal)
            else
              LDetails := LStyle.GetElementDetails(ttbDropDownButtonGlyphDisabled);
            if (LButtonHot and (bsEnabled in LToolState)) then
              LDetails := LStyle.GetElementDetails(ttbDropDownButtonGlyphHot);
            if ((bsPressed in LToolState) and (bsEnabled in LToolState)) then
              LDetails := LStyle.GetElementDetails(ttbDropDownButtonGlyphPressed);
            LStyle.DrawElement(DC, LDetails, R, nil, LPPI);
          ...
          begin
            ...
            begin
              ...
              if (ImgRect.Width > 0) and (LImageIndex > -1) then
                ...
                TxtRect := ItemRect;
                if LToolState = [bsEnabled] then
                  LDetails := LStyle.GetElementDetails(tbCheckBoxUncheckedNormal)
                else
                if LToolState = [] then
                  LDetails := LStyle.GetElementDetails(tbCheckBoxUncheckedDisabled);
                if LText <> '' then
            ...
            begin
              ...
              if LText <> '' then
                LStyle.DrawText(DC, LDetails, LText, TxtRect, TxtFormat);
            end;
          end;
        end;
      end;
...
function TSysToolbarStyleHook.TSysToolbarButton.GetCurrentPPI: Integer;
begin
  if CheckPerMonitorV2SupportForWindow(FParent.Handle) then
    Result := GetDPIForWindow(FParent.Handle)
  else
...
begin
  if BOOL(SendMessage(FParent.Handle, TB_GETITEMDROPDOWNRECT, FIndex, LParam(@R))) then
    ...
    Result := MulDiv(15, CurrentPPI, Screen.DefaultPixelsPerInch); // default width when runtime themes are enabled
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      LDetails := LStyle.GetElementDetails(tpChunk);
    end
    else
    begin
      FillR.Top := FillR.Bottom - LPos;
      LDetails := LStyle.GetElementDetails(tpChunkVert);
    end;

    FillR.SetLocation(FStep * FillR.Width, FillR.Top);
    LStyle.DrawElement(Canvas.Handle, LDetails, FillR);
  end
  else
  begin
    InflateRect(LRect, -2, -2);
...
begin
  ...
  if LStyle.Available then
  begin
    Color := ColorToRGB(LStyle.GetStyleColor(scEdit));
    FontColor := ColorToRGB(LStyle.GetStyleFontColor(TextColor[SysControl.Enabled]));
    BackColor := ColorToRGB(LStyle.GetStyleColor(BkColor[SysControl.Enabled]));
  end;
...
begin
  ...
  begin
    ...
    { band }
    Details := LStyle.GetElementDetails(trBand);
    LStyle.DrawElement(Canvas.Handle, Details, R);
    { text }
    ...
    Details := LStyle.GetElementDetails(trGripper);
    LStyle.DrawElement(Canvas.Handle, Details, R);
  end;
end;
...
begin
  ...
  Canvas.Brush.Color := LStyle.ColorToRGB(clBtnFace);
  Canvas.FillRect(Rect(0, 0, SysControl.Width, SysControl.Height));
  LDetails.Element := teToolBar;
  LDetails.Part := 0;
  LStyle.DrawElement(Canvas.Handle, LDetails, Rect(0, 0, SysControl.Width, SysControl.Height));
end;
...
begin
  ...
  Details := LStyle.GetElementDetails(tsStatusRoot);
  LStyle.DrawElement(Canvas.Handle, Details, Rect(0, 0, SysControl.Width, SysControl.Height));
  ...
  begin
    ...
    Details := LStyle.GetElementDetails(tsPane);
    LStyle.DrawElement(Canvas.Handle, Details, R);

    R1 := SysControl.ClientRect;
    R1.Left := R1.Right - R.Height;
    Details := LStyle.GetElementDetails(tsGripper);
...
begin
  ...
  begin
    LDetails := LStyle.GetElementDetails(ttbTrack);
    LStyle.DrawElement(Canvas.Handle, LDetails, LRect);
  end
  else
  begin
    ...
    LDetails := LStyle.GetElementDetails(ttbTrackVert);
    LStyle.DrawElement(Canvas.Handle, LDetails, LRect);
  end;
  ...
  begin
    ...
    Canvas.Pen.Color := LStyle.ColorToRGB(clBtnText);
  ...
  begin
    ...
    LDetails := LStyle.GetElementDetails(LThemedTrackBar);
    LStyle.DrawElement(Canvas.Handle, LDetails, LRect);
  end;
