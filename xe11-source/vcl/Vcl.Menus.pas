...
{$IF DEFINED(LINUX)}
  WinUtils, 
{$ENDIF}
  Winapi.Windows, Winapi.Messages, System.SysUtils, System.Classes, System.Contnrs, Vcl.ImgList,
  System.UITypes, Vcl.Graphics, Vcl.ActnList;
...
{$ENDIF}
  System.Types, Vcl.Controls, Vcl.Forms, Vcl.Consts, Vcl.Themes, Winapi.UxTheme;
...
function TMenuActionLink.IsImageNameLinked: Boolean;
begin
...
procedure DoThemedDrawText(MenuItem: TMenuItem; ACanvas: TCanvas;
  const ACaption: string; var Rect: TRect; Selected: Boolean; Flags: TTextFormat; TopLevel: Boolean);
const
  MenuStates: array[Boolean, Boolean, Boolean] of TThemedMenu = ( // TopLevel, Selected, MenuItem.Enabled
    // TopLevel = False
    ((tmPopupItemDisabled, tmPopupItemNormal),  // Selected = False
     (tmPopupItemDisabledHot, tmPopupItemHot)), // Selected = True
    // TopLevel = True
    ((tmMenuBarItemDisabled, tmMenuBarItemNormal),    // Selected = False
     (tmMenuBarItemDisabledHot, tmMenuBarItemHot)) ); // Selected = True
var
  LCaption: string;
  ParentMenu: TMenu;
begin
  ...
  begin
    ...
    TStyleManager.SystemStyle.DrawText(ACanvas.Handle,
      TStyleManager.SystemStyle.GetElementDetails(MenuStates[TopLevel, Selected, MenuItem.Enabled]), LCaption, Rect, Flags);
  end;
end;
...
var
  ...
  var
    ...
    LGutterRect, LCheckRect, LBitmapRect, R: TRect;
  ...
  begin
    ...
    LStyle.GetElementSize(ACanvas.Handle, LDetails, esActual, LCheckSize, LPPI);
    LStyle.GetElementMargins(ACanvas.Handle, LDetails, emContent, LCheckMargins, LPPI);
    ...
    begin
      LCheckRect.Bottom := ARect.Bottom;
      LCheckRect.Right := LCheckRect.Left + ARect.Height;
    end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              LStyle.DrawElement(ACanvas.Handle, LStyle.GetElementDetails(CheckMarkBkgStates[Enabled]), LCheckRect);
              LStyle.GetElementSize(ACanvas.Handle, LStyle.GetElementDetails(CheckMarkStates[Enabled, RadioItem]), esActual, LCheckSize, LPPI);
              R := Rect(0, 0, LCheckSize.cx, LCheckSize.cy);
              OffsetRect(R, LCheckRect.Left + (LCheckRect.Width - LCheckSize.cx) div 2,
                LCheckRect.Top + (LCheckRect.Height - LCheckSize.cy) div 2);
              LStyle.DrawElement(ACanvas.Handle, LStyle.GetElementDetails(CheckMarkStates[Enabled, RadioItem]), R, nil, LPPI);
            end;
          end;
        end
        else
    ...
    LStyle.GetElementMargins(ACanvas.Handle, LStyle.GetElementDetails(tmPopupItemNormal), emSizing, LMargins, LPPI);
    if not TopLevel then
    begin
      if not RightToLeft then
      begin
      ...
      if Caption <> cLineCaption then
      begin
        LStyle.GetElementSize(ACanvas.Handle, LStyle.GetElementDetails(SubMenuStates[Enabled]), esActual, LSize, LPPI);
        if not RightToLeft then
    ...
    begin
      ...
      SaveRect := ARect;
      DoThemedDrawText(Self, ACanvas, Caption, ARect, Selected, DrawStyle + [tfCalcRect, tfNoClip], TopLevel);
      if RightToLeft then
      begin
      ...
      DoThemedDrawText(Self, ACanvas, Caption, ARect, Selected, DrawStyle, TopLevel);
      if (ShortCut <> scNone) and not TopLevel and (FItems = nil) then
      begin
        ...
        DoThemedDrawText(Self, ACanvas, LShortCut, ARect, Selected, DrawStyle + [tfRight], TopLevel);
      end;
    end
    else
    begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if DrawImage and (ImageList <> nil) then
          begin
...
{$ENDIF}
      ...
      if (ShortCut <> scNone) and not TopLevel and not (WinVista and (FItems <> nil)) then
      begin
        ARect.Left := ARect.Right;
...
{$ENDIF}
  ...
  begin
    Height := 0;
    Width := 0;
    LSPPI := Screen.PixelsPerInch;
    var LCheckOrGlyphOnlyWidth := 0;
    var LStyle := TStyleManager.SystemStyle;
    // Calculate check/glyph size
    LStyle.GetElementSize(ACanvas.Handle, LStyle.GetElementDetails(CheckMarkStates[Enabled, RadioItem]),
      esActual, GlyphSize);

    GlyphSize.cy := MulDiv(GlyphSize.cy, LPPI, LSPPI);
    GlyphSize.cx := MulDiv(GlyphSize.cx, LPPI, LSPPI);
    ACanvas.Font.Size := MulDiv(ACanvas.Font.Size, LPPI, LSPPI);
    ...
    begin
      ...
      LStyle.GetElementSize(ACanvas.Handle, LStyle.GetElementDetails(tmMenuBarItemNormal), esActual, LSize, LPPI);
      Inc(Height, LSize.cy);
    end
    else
    begin
      ...
      LStyle.GetElementMargins(ACanvas.Handle, LStyle.GetElementDetails(CheckMarkStates[Enabled, RadioItem]), emContent, LMargins, LPPI);
      ...
      LStyle.GetElementSize(ACanvas.Handle, LStyle.GetElementDetails(tmPopupGutter), esActual, LSize);
      Inc(Width, LSize.cx);
      LStyle.GetElementMargins(ACanvas.Handle, LStyle.GetElementDetails(tmPopupGutter), emSizing, LMargins);
      Inc(Width, LMargins.cxLeftWidth + LMargins.cxRightWidth);
      ...
      LStyle.GetElementSize(ACanvas.Handle, LStyle.GetElementDetails(tmPopupSubMenuNormal), esActual, LSize, LPPI);
      Inc(Width, LSize.cx);
      LCheckOrGlyphOnlyWidth := LSize.cx;

      // Adjust size if separator
      if Caption = cLineCaption then
      begin
        LStyle.GetElementSize(ACanvas.Handle, LStyle.GetElementDetails(tmPopupSeparator), esActual, LSize);
        LStyle.GetElementMargins(ACanvas.Handle, LStyle.GetElementDetails(tmPopupSeparator), emSizing, LMargins);
        Height := LSize.cy + LMargins.cyBottomHeight;
        Inc(Width, LSize.cx);
      end
    end;

    if (Caption <> '') or (ShortCut <> scNone) then
    begin
      R := Rect(0, 0, 0, 0);
      if ParentMenu is TMenu then
        Alignment := paLeft
      ...
      DoThemedDrawText(Self, ACanvas, Text, R, False, LDrawStyle, TopLevel);
      Inc(Width, R.Right - R.Left);
      if not TopLevel then
      begin
        if (Caption <> '') and (Caption <> cLineCaption) and (ShortCut <> scNone) then
          Width := Width + ShortCutGap + MulDiv(ShortCutGap, LPPI, OriginalPPI) - 1; { 96 DPI: 23, 192 DPI: 35 }
        // Adjust the size if the text height is bigger than the current height
        if Caption <> cLineCaption then
        begin
          LStyle.GetElementMargins(ACanvas.Handle, LStyle.GetElementDetails(CheckMarkStates[Enabled, RadioItem]), emContent, LMargins, LPPI);
          if R.Height > Height - LMargins.cyTopHeight - LMargins.cyBottomHeight then
            Height := R.Height + LMargins.cyTopHeight + LMargins.cyBottomHeight;
        end;
      end
    end
    else
      Width := LCheckOrGlyphOnlyWidth;
  end;
...
begin
  ...
  begin
    ...
    Height := MulDiv(Height, LPPI, OriginalPPI);
    Width := MulDiv(Width, LPPI, OriginalPPI);
  end;
...
begin
  ...
  begin
    FImageIndex := Value;
    IL := GetImageList;
    if (IL <> nil) and IL.IsImageNameAvailable then
      FImageName := IL.GetNameByIndex(FImageIndex);
    MenuChanged(True);
  end;
end;

procedure TMenuItem.SetImageName(const Value: TImageName);
var
  IL: TCustomImageList;
begin
  ...
  begin
    ...
    IL := GetImageList;
    if (IL <> nil) and IL.IsImageNameAvailable then
      FImageIndex := IL.GetIndexByName(FImageName);
    MenuChanged(True);
  end;
end;

procedure TMenuItem.CheckImageIndex;
var
  IL: TCustomImageList;
begin
  IL := GetImageList;
...
begin
  if Sender is TCustomAction then
    ...
    begin
      ...
      if not CheckDefaults or (Self.ImageName = '') then
...
function TMenuItem.IsImageNameStored: Boolean;
begin
...
begin
  ...
  begin
    ...
    begin
      if TWinControl(LParent.Owner).Parent = nil then
        LMonitor := Screen.MonitorFromPoint(LPlacement.rcNormalPosition.CenterPoint)
      else
        LMonitor := Screen.MonitorFromPoint(TWinControl(LParent.Owner).ClientToScreen(LPlacement.rcNormalPosition.CenterPoint))
    end
    else
...
begin
  ...
  begin
    ...
    LCurrentDPI := Item.GetDevicePPI;
    if FPreviousDPI <> 0 then
      LRebuild := FPreviousDPI <> LCurrentDPI
    else
      LRebuild := False;
    FPreviousDPI := LCurrentDPI;

    LRebuild := Item.InternalRethinkHotkeys(False) or LRebuild;
...
{$IF DEFINED(CLR)}
  ...
  Result := (ShortCutItem.Value <> nil) and (ClickResult = crClicked) or (MyItems.Value = nil);
{$ELSE}
  Result := False;
  ...
  Result := (ShortCutItem <> nil) and (ClickResult = crClicked) or (MyItems = nil);
{$ENDIF}
end;
...
begin
  ...
  begin
    ...
    Font.Size := MulDiv(Font.Size, MenuItem.GetDevicePPI, Screen.PixelsPerInch);
