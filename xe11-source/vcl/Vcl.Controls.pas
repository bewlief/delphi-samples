...
{$ENDIF}
  ...
  CM_PARENTVISIBLECHANGED        = CM_BASE + 91;
  CM_SYSCOMMAND                  = CM_BASE + 92;
  CM_REMOTESESSIONSTATUSCHANGED  = CM_BASE + 93;
...
{$IF DEFINED(CLR)}
  ...
  private
    function GetNotifyRec: TDockNotifyRec;
    procedure SetNotifyRec(Value: TDockNotifyRec);
  public
    class operator Explicit(Message: TMessage): TCMDockNotification;
    property NotifyRec: TDockNotifyRec read GetNotifyRec write SetNotifyRec;
...
{$ENDIF}
  {$IFDEF CLR}[StructLayout(LayoutKind.Sequential)]{$ENDIF}
...
{$IF NOT DEFINED(CLR)}
  ...
  TCMRemoteSessionStatusChanged = record
    Msg: Cardinal;
    MsgFiller: TDWordFiller;
    InRemoteSession: LongBool;
    UnusedFiller: TDWordFiller;
    Unused: LPARAM;
    Result: LRESULT;
  end;
...
{$ENDIF}
  ...
  protected
    ...
    procedure ScaleBy(M, D: Integer; AUpdate: Boolean = False);
...
{$ENDIF}
  ...
  // TOriginalParentCalcType indicates how FOriginalParentSize was calculated.
  // ctWinApi - Parent has handle and FOriginalParentSize was calculated using
  //            Winapi.Windows.GetClientRect function;
  // ctNative - Parent does not have handle and FOriginalParentSize was
  //            calculated using Width and Height properties;
  TOriginalParentCalcType = (ctWinApi, ctNative);
...
{$ENDIF}
  private
    class var FRaiseOnNonMainThreadUsage: Boolean;
  private
    FParent: TWinControl;
...
{$ENDIF}
  protected
    ...
    FOriginalParentCalcType: TOriginalParentCalcType;
    ...
    FScaleFactor: Single;
    /// <summary>   Obtains Design dpi for the form where the control was designed </summary>
    function GetDesignDpi: Integer; virtual;
    /// <summary>
    /// Obtains CurrentDpi for parent control, or current control dpi if there is no parent
    /// </summary>
    function GetParentCurrentDpi: Integer; virtual;
    function GetCurrentPPI: Integer; virtual;
    function GetDPIForDesigner: Integer; virtual;
    ...
    function DefaultScalingFlags: TScalingFlags; virtual;
...
{$ELSE}
    ...
    function GetDragImages: TDragImageList; virtual;
{$ENDIF}
    function GetAllocatedWindowHandle: HWnd; virtual;
...
{$ENDIF}
  ...
  public
    ...
    destructor Destroy; override;
    procedure CheckNonMainThreadUsage;
...
{$ENDIF}
    ...
    function GetStyleName: string; virtual;
...
{$ENDIF}
    ...
    class property RaiseOnNonMainThreadUsage: Boolean read FRaiseOnNonMainThreadUsage write FRaiseOnNonMainThreadUsage default False;
  published
...
{$ENDIF}
    ...
    function GetIsDrawingLocked: Boolean;
    function GetRedrawDisabled: Boolean;
    ...
    procedure CMRemoteSessionStatusChanged(var Message: TCMRemoteSessionStatusChanged); message CM_REMOTESESSIONSTATUSCHANGED;
...
{$ENDIF}
    ...
    FAsyncList: TList;
    /// <summary>
    ///    Called from a specific message posted to this control's window handle in order to process pending
    ///    Asynchronous method calls within the thread context of this control's window.
    /// </summary>
    ...
    function DoWritePixelsPerInch(Filer: TFiler): Boolean; virtual;
    ...
    function GetDPIForDesigner: Integer; override;
    ...
    /// <summary>   Scale all contained controls to the NewPPI value </summary>
    procedure ScaleControlsForDpi(NewPPI: Integer); virtual;
    function GetCurrentPPIScreen(const AControl: TWinControl = nil): Integer;
    ...
    procedure UpdateTIPStatus; virtual;
...
{$ENDIF}
    function GetAllocatedWindowHandle: HWnd; override;
...
{$ENDIF}
    ...
    destructor Destroy; override;
    /// <summary>   Scale current control for the newPPI </summary>
...
{ TDockTree }
  ...
  protected
    ...
    procedure MouseLeave(var Handled: Boolean); virtual;
    procedure MouseUp(Button: TMouseButton; Shift: TShiftState;
      X, Y: Integer; var Handled: Boolean); virtual;
...
{ Mouse support }
  ...
  private
    class var FWheelRouting: TWheelRouting;
  protected
    class procedure GetWheelData;
  public
    /// <summary>
    ...
    /// </summary>
    class property WheelRouting: TWheelRouting read FWheelRouting;
  end;
...
function InitWndProc(HWindow: HWND; Msg: UINT; WParam: WPARAM;
  LParam: LPARAM): LRESULT; {$IF NOT DEFINED(CLR)}stdcall;{$ENDIF}
...
function Win32NLSEnableIME(hWnd: HWND; Enable: Boolean): Boolean; deprecated;
...
procedure InvalidControlOperation(AMessage: PResStringRec; AControl: TControl);
...
{$ENDIF}
  Winapi.MSCTF,
  System.UIConsts, System.RTLConsts, Vcl.Consts, Vcl.Forms, Winapi.ActiveX, System.Math, Vcl.Themes, Winapi.UxTheme,
  Winapi.DwmApi, System.TypInfo, Vcl.GraphUtil, Winapi.PenInputPanel, System.Win.ComObj, Winapi.ShellAPI;
...
begin
  if CheckPerMonitorV2SupportForWindow(AHandle) then
    Result := SystemParametersInfoForDpi(uiAction, uiParam, pvParam, fWinIni, GetDPIForWindow(AHandle))
  else
...
procedure InvalidControlOperation(AMessage: PResStringRec; AControl: TControl);
  ...
  function GetControlName(AComponent: TComponent): string;
  begin
    if AComponent = nil then
      Result := 'nil'  // Do not localize
    else if AComponent.Name <> '' then
      Result := AComponent.Name
    else
      Result := AComponent.ClassName + '($' + IntToHex(NativeUInt(AComponent)) + ')';  // Do not localize
  end;

  function GetControlPath(AComponent: TComponent): string;
  begin
    Result := GetControlName(AComponent);
    if AComponent <> nil then
    begin
      if csDestroying in AComponent.ComponentState then
        Result := '~' + Result;  // Do not localize
      if (AComponent is TControl) and (TControl(AComponent).Parent <> nil) then
        Result := GetControlPath(TControl(AComponent).Parent) + sLineBreak + '  ^' + Result  // Do not localize
      else if (AComponent.Owner <> nil) and not (AComponent.Owner is TApplication) then
        Result := GetControlPath(AComponent.Owner) + sLineBreak + '  .' + Result;  // Do not localize
    end;
  end;

begin
  raise EInvalidOperation.CreateFmt(LoadResString(AMessage) + SControlPath,
    [GetControlName(AControl), GetControlPath(AControl)]);
end;
...
{$IF DEFINED(CLR)}
function TCMDockNotification.GetNotifyRec: TDockNotifyRec;
begin
...
{$ELSE}
function IsBeforeTargetWindow(Window: HWND; Data: LPARAM): Bool; stdcall;
{$ENDIF}
var
  R: TRect;
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      if CanDock and R.Contains(MousePos) then
...
{$ELSE}
function DragMessage(Handle: HWND; Msg: TDragMessage;
  Source: TDragObject; Target: Pointer; const Pos: TPoint): NativeInt;
var
  DragRec: TDragRec;
begin
  Result := 0;
...
procedure TSizeConstraints.ScaleBy(M, D: Integer; AUpdate: Boolean = False);
begin
...
begin
  ...
  FCurrentPPI := Winapi.Windows.USER_DEFAULT_SCREEN_DPI;
  FScaleFactor := 1;
end;
...
procedure TControl.CheckNonMainThreadUsage;
begin
  if RaiseOnNonMainThreadUsage and (TThread.Current.ThreadID <> MainThreadID) then
    try
      InvalidControlOperation(@SControlNonMainThreadUsage, Self);
    except
      on E: Exception do
      begin
...
begin
  if (Parent <> nil) and not IScaling then
...
begin
  ...
  begin
    ...
    begin
      ...
      FOriginalParentCalcType := ctNative;
      Exit;
    end;
...
function TControl.DefaultScalingFlags: TScalingFlags;
begin
  Result := [sfLeft, sfTop, sfWidth, sfHeight, sfFont];
end;
...
function TControl.ScreenToClient(const Rect: TRect): TRect;
var
  Origin: TPoint;
begin
  Result := Rect;
...
begin
  ...
  if (LForm <> nil) then
    Result := LForm.PixelsPerInch
  else
    Result := Winapi.Windows.USER_DEFAULT_SCREEN_DPI;
end;

function TControl.GetDPIForDesigner: Integer;
var
  LForm: TCustomForm;
begin
  LForm := GetParentForm(Self);
  if (LForm <> nil) and (LForm.Designer <> nil) and (LForm.Scaled) then
    Result := LForm.Designer.DesignPPI(LForm)
  else if (LForm <> nil) then
    Result := LForm.PixelsPerInch
  else
    Result := Winapi.Windows.USER_DEFAULT_SCREEN_DPI;
end;
...
function TControl.GetParentCurrentDpi: Integer;
begin
  if Parent <> nil then
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        LForm := GetParentForm(Self);
        var IsRootControl := (csDesignInstance in ComponentState) and
          not (Self is TCustomForm) and (AParent is TCustomForm);
        var IsDesignChildControl := (LForm <> nil) and (LForm.Scaled) and
          ((csDesigning in ComponentState) or (csDesigning in Parent.ComponentState));
        if IsDesignChildControl then
          ScaleForPPI(GetDPIForDesigner)
        else if not IsRootControl then
...
procedure TControl.AsyncSchedule(const AsyncResult: TBaseAsyncResult);
var
  LParent: TWinControl;
begin
  LParent := Parent;
  while (LParent <> nil) and not LParent.HandleAllocated do
    LParent := LParent.Parent;
  if LParent = nil then
    raise EInvalidOperation.CreateRes(@sBeginInvokeNoHandle);
  LParent.AsyncSchedule(ASyncResult);
end;
...
begin
  ...
  begin
    ...
    if not ClientRect.Contains(Temp) then
    begin
      inherited;
      Exit;
    end;
...
begin
  ...
  if LParent = nil then
    InvalidControlOperation(@SParentGivenNotAParent, Self);
end;
...
procedure TControl.UpdateStyleElements;
begin
  Invalidate;
end;
...
procedure TControl.SetStyleName(const Value: string);
begin
  if not SameText(FStyleName, Value) then
  begin
    FStyleName := Value;
    if not (csLoading in ComponentState) and TStyleManager.IsCustomStyleAvailable(Self) then
      UpdateStyleElements;
  end;
end;
...
function TControl.GetAllocatedWindowHandle: HWnd;
begin
  if Parent <> nil then
    Result := Parent.GetAllocatedWindowHandle
  else
    Result := 0;
end;
...
function TControl.GetStyleName: string;
begin
  Result := FStyleName;
  if (Result = '') and (Parent <> nil) and
     (not (csDesigning in ComponentState) or
     ((csDesigning in ComponentState) and (csDesigning in Parent.ComponentState))) then
    Result := Parent.GetStyleName;
end;

function TControl.IsCustomStyleActive: Boolean;
var
  LStyleName: string;
begin
  Result := TStyleManager.IsCustomStyleAvailable(Self);
  if Result then
  begin
    LStyleName:= GetStyleName;
    Result := (StyleElements <> []) and not SameText(LStyleName, TStyleManager.SystemStyleName) and
      not ((LStyleName = '') and TStyleManager.IsSystemStyleDefault(Self));
  end;
end;

function TControl.IsLightStyleColor(AColor: TColor): Boolean;
var
...
begin
  ...
  if LStyle.Enabled and not LStyle.IsSystemStyle and (seClient in StyleElements) then
    AColor := LStyle.GetSystemColor(AColor)
  else
    AColor := ColorToRGB(AColor);
  R := GetRValue(AColor);
  G := GetGValue(AColor);
  B := GetBValue(AColor);
  Result := 0.299 * R + 0.587 * G + 0.114 * B >= 127;
end;

function TControl.ScaleValue(const Value: Integer): Integer;
begin
  Result := MulDiv(Value, Round(FScaleFactor * 100), 100);
end;

function TControl.ScaleValue(const Value: Double): Double;
begin
  Result := Value * FScaleFactor;
end;

function TControl.ScaleValue(const Value: TPoint): TPoint;
begin
  Result.X := ScaleValue(Value.X);
  Result.Y := ScaleValue(Value.Y);
end;

function TControl.ScaleRectSize(const Value: TRect): TRect;
begin
  Result.TopLeft := Value.TopLeft;
  Result.BottomRight := ScaleValue(Value.BottomRight);
end;

function TControl.ScaleValue(const Value: TRect): TRect;
begin
  Result.TopLeft := ScaleValue(Value.TopLeft);
  Result.BottomRight := ScaleValue(Value.BottomRight);
end;

function TControl.ScaleValue(const Value: TSize): TSize;
begin
  Result.cx := ScaleValue(Value.cx);
  Result.cy := ScaleValue(Value.cy);
end;
...
{$ENDIF}
  ...
  if Assigned(CustomStyleNotification) then
    CustomStyleNotification(snControlDestroyed, Self);
...
var
  ...
  begin
    ...
    if (((csDesigning in ComponentState) and (Control.FOriginalParentCalcType = ctWinApi)) or
        not (csDesigning in ComponentState)) and
       LParent.HandleAllocated then
      Result := LParent.ClientRect.BottomRight
    else
  ...
  begin
    ArrangeControl(Control, GetParentClientSize(Control), AAlign, AlignInfo, Rect);
  end;
...
(*  procedure DoPosition(Control: TControl; AAlign: TAlign; AlignInfo: TAlignInfo);
  ...
  begin
    ...
    begin
      ...
      if (Width = Rect.Right-Rect.Left) and (Height = Rect.Bottom-Rect.Top)
...
{$IFDEF WIN64} //FIXME: Temporary to workaround bad dcc64 codegen for @DefWindowProc (BB)
var
  UserLibrary: THandle;
{$ENDIF}
...
{$ENDIF}
  ...
  begin
    ...
    if (not (csDesigning in ComponentState) or (csNeedsDesignDisabledState in ControlStyle)) and not Enabled then
...
procedure TWinControl.ReadPixelsPerInch(Reader: TReader);
begin
...
procedure TWinControl.WritePixelsPerInch(Writer: TWriter);
begin
...
begin
  inherited;
  Filer.DefineProperty('PixelsPerInch', ReadPixelsPerInch, WritePixelsPerInch, DoWritePixelsPerInch(Filer));
  Filer.DefineProperty('DesignSize', ReadDesignSize, WriteDesignSize,
    DoWriteDesignSize);
end;

function TWinControl.DoWritePixelsPerInch(Filer: TFiler): Boolean;
begin
  Result := (Filer.Root = Self) and (csDesignInstance in ComponentState) and (PixelsPerInch <> Screen.DefaultPixelsPerInch);
end;
...
{$ENDIF}
    if IsDrawingLocked and Visible then
...
procedure TWinControl.CMRemoteSessionStatusChanged(var Message: TCMRemoteSessionStatusChanged);
begin
  if Message.InRemoteSession then
  begin
    // Save user-requested True value if we need to disable it in remote sessions and we haven't saved it yet
    if Application.SingleBufferingInRemoteSessions and not FDoubleBufferedSaved then
    begin
      FDoubleBufferedSaved := FDoubleBuffered;
      FDoubleBuffered := False;
    end
  end
  else
    FDoubleBuffered := FDoubleBufferedSaved;
end;

procedure TWinControl.CMSysCommand(var Message: TWMKeyDown);
begin
  Broadcast(&Message)
end;

procedure TWinControl.LockDrawing;
begin
...
procedure TWinControl.UnlockDrawing;
begin
...
function TWinControl.GetIsDrawingLocked: Boolean;
begin
  Result := FDrawLockCount > 0;
end;

function TWinControl.GetRedrawDisabled: Boolean;
begin
  ...
  begin
    if IsDrawingLocked then
      ...
      Result := Parent.RedrawDisabled
    else
      Result := False;
  end
  else if TOSVersion.Check(10) then
    Exit(GetProp(WindowHandle, PChar('SysSetRedraw')) <> 0)
  else
  begin
    Result := not GetUpdateRect(WindowHandle, nil, False);
    if Result then
    begin
...
function TWinControl.ControlAtPos(const Pos: TPoint; AllowDisabled,
  ...
  function InternalControlAtPos(AWinControl: TWinControl; const Pos: TPoint): TControl;
  var
    I: Integer;
    P: TPoint;
    LControl: TControl;
    ...
    begin
      P := Point(Pos.X - AControl.Left, Pos.Y - AControl.Top);
      Result := AControl.ClientRect.Contains(P) and
                ((csDesigning in AControl.ComponentState) and (AControl.Visible or
                not (csNoDesignVisible in AControl.ControlStyle)) or
                (AControl.Visible and (AControl.Enabled or AllowDisabled) and
                (AControl.Perform(CM_HITTEST, 0, PointToLParam(P)) <> 0)));
      if Result then
        LControl := AControl;
    end;
  ...
  begin
    ...
    if AllowWinControls and (AWinControl.FWinControls <> nil) then
      for I := AWinControl.FWinControls.Count - 1 downto 0 do
      begin
        LWinControl := TWinControl(AWinControl.FWinControls[I]);
        if AllLevels then
          LControl := InternalControlAtPos(LWinControl, LWinControl.ParentToClient(Pos));
        if (LControl <> nil) or GetControlAtPos(LWinControl) then
          Break;
      end;

    if (AWinControl.FControls <> nil) and (LControl = nil) then
      for I := AWinControl.FControls.Count - 1 downto 0 do
        if GetControlAtPos(TControl(AWinControl.FControls[I])) then
          Break;
...
begin
  Result := InternalControlAtPos(Self, Pos);
end;
...
begin
  if (FHandle <> 0) {HandleAllocated} and
     ...
     IsCustomStyleActive and
     DoHandleStyleMessage(Message) then
    Exit;

  if Assigned(CustomStyleHandleMessage) and
     (FHandle <> 0) {HandleAllocated} and
     not (csDestroying in ComponentState) and
     not (csDestroyingHandle in ControlState) and
     not (csOverrideStylePaint in ControlStyle) and
     CustomStyleHandleMessage(Self, Message, FWindowProc) then
    Exit;
...
{$ENDIF}
    ...
    CM_STYLECHANGED:
      Broadcast(Message);
    CM_PARENTVISIBLECHANGED:
      Broadcast(Message);
    CM_REMOTESESSIONSTATUSCHANGED:
      Broadcast(Message);
    WM_TOUCH:
      with FTouchManager do
  ...
  if (Message.Msg = Cardinal(RM_AsyncMessage)) and (Message.Msg <> 0) then
  begin
    InvokeAsyncCalls;
    Exit;
  end;
...
{$IF NOT DEFINED(CLR)}
        if (Msg = RM_GetObjectInstance) and (WParam = GetCurrentProcessId) then
...
procedure TWinControl.PaintTo(DC: HDC; X, Y: Integer);
  ...
  begin
    ...
    try
      ...
      LStyle := StyleServices(Self);
      Details := LStyle.GetElementDetails(teEditTextNormal);
      LStyle.DrawElement(DC, Details, DrawRect);
    finally
      RestoreDC(DC, Save);
    end;
...
begin
  ...
  try
    ...
    if (EdgeFlags = EDGE_SUNKEN) and StyleServices(Self).Enabled and
    ...
    begin
      ...
      try
        ...
        if PaintBuffer <> 0 then
          try
            FillRect(MemDC, PS.rcPaint, FBrush.Handle);
...
begin
  if StyleServices(Self).Enabled and Assigned(Parent) and (csParentBackground in FControlStyle) then
  begin
    { Get the parent to draw its background into the control's background. }
    if Parent.DoubleBuffered then
      PerformEraseBackground(Self, Message.DC)
    else
      StyleServices(Self).DrawParentBackground(Handle, Message.DC, nil, False);
  end
  else
  begin
...
procedure TWinControl.SetIme(hWnd: HWND);
var
  I: Integer;
  HandleToSet: HKL;
begin
...
procedure TWinControl.ResetIme(hWnd: HWND);
begin
  if not SysLocale.FarEast then Exit;
  if FImeName <> '' then
  begin
...
begin
  ...
  begin
    FDoubleBufferedSaved := Value;
    if not (Application.InRemoteSession and Application.SingleBufferingInRemoteSessions) or
...
{$ENDIF}
  ...
  inherited;
  // The main form only sees these min/max messages when Application.MainFormOnTaskBar
  // is True otherwise TApplication.WndProc picks them up
...
begin
  if SysLocale.MiddleEast then
    if UseRightToLeftReading then
      ...
      Application.ApplyNonBiDiKeyboardLayout;
  DoEnter;
end;
...
begin
  ...
  if not (csDesigning in ComponentState) and not (csDestroying in ComponentState) then
    Perform(CM_PARENTVISIBLECHANGED, Ord(FVisible), 0);
 end;
...
begin
  ...
  if IsDrawingLocked and FShowing then
...
begin
  ...
  if HandleAllocated and (not (csDesigning in ComponentState) or (csNeedsDesignDisabledState in ControlStyle)) then
...
procedure TWinControl.CMSysFontsAllChanged(var Message: TMessage);
begin
  Broadcast(Message);
end;
...
begin
  ...
  if (csDesignInstance in ComponentState) and (Parent is TCustomForm) and
    (FCurrentPPI <> FPixelsPerInch) then
  begin
    FScaleFactor := FScaleFactor * FPixelsPerInch / Screen.DefaultPixelsPerInch;
    FCurrentPPI := FPixelsPerInch;
    for var I := 0 to ControlCount - 1 do
      Controls[I].ChangeScale(FCurrentPPI, FCurrentPPI, True);
  end;
...
begin
  ...
  try
    for I := 0 to ControlCount - 1 do
    begin
      if (csDesignInstance in Controls[I].ComponentState) and
       (Controls[I] is TForm) and (TForm(Controls[I]).Designer <> nil) then
      begin
        LForm := TForm(Controls[I]);
        if LForm.Scaled then
          LForm.ScaleForPPI(LForm.Designer.DesignPPI(LForm));
      end
      else
...
function TWinControl.GetCurrentPPIScreen(const AControl: TWinControl = nil): Integer;
var
  LMonitor: TMonitor;
...
function TWinControl.GetDPIForDesigner: Integer;
begin
  if (csDesignInstance in ComponentState) and not (Self is TCustomForm) and
    (Parent is TCustomForm) then
  begin
    var LForm := GetParentForm(Self, False);
    if (LForm <> nil) and (LForm.Designer <> nil) and (LForm.Scaled) then
      Exit(LForm.Designer.DesignPPI(LForm))
    else
      Exit(Self.PixelsPerInch);
  end;
  Result := Inherited GetDPIForDesigner;
end;
...
function TWinControl.GetPixelsPerInch: Integer;
begin
  Result := FPixelsPerInch;
end;

procedure TWinControl.SetPixelsPerInch(Value: Integer);
begin
  if (Value <> FPixelsPerInch) then
...
procedure TWinControl.UpdateStyleElements;
begin
  RecreateWnd;
end;

procedure TWinControl.InvokeAsyncCalls;
var
  LAsyncResult: TBaseAsyncResult;

  function GetAsyncResult: TBaseAsyncResult;
  begin
    System.TMonitor.Enter(FAsyncList);
    try
      if FAsyncList.Count > 0 then
      begin
        Result := FAsyncList.First;
        FAsyncList.Delete(0);
      end else
        Result := nil;
    finally
      System.TMonitor.Exit(FAsyncList);
    end;
  end;

begin
  if FAsyncList <> nil then
  begin
    LAsyncResult := GetAsyncResult;
    while LAsyncResult <> nil do
    begin
      TBaseAsyncResult.Dispatch(LAsyncResult);
      LAsyncResult := GetAsyncResult;
    end;
  end;
end;

procedure TWinControl.AsyncSchedule(const AsyncResult: TBaseAsyncResult);
var
  LParent: TWinControl;
  LList: TList;
  LState: TComponentState;
begin
  LState := ComponentState;
  LParent := Self;
  while (LParent <> nil) and not LParent.HandleAllocated do
    LParent := LParent.Parent;
  if LParent = nil then
    raise EInvalidOperation.CreateRes(@sBeginInvokeNoHandle);
  LState := LParent.ComponentState + LState; // union the sets in case one or more are destroying
  if csDestroying in LState then
    raise EInvalidOperation.CreateRes(@sBeginInvokeDestroying);
  if GetWindowThreadProcessId(FHandle, nil) <> TThread.CurrentThread.ThreadId then
  begin
    if FAsyncList = nil then
    begin
      LList := TList.Create;
      if TInterlocked.CompareExchange<TList>(FAsyncList, LList, nil) <> nil then
        LList.Free;
    end;
    System.TMonitor.Enter(FAsyncList);
    try
      FAsyncList.Add(AsyncResult);
    finally
      System.TMonitor.Exit(FAsyncList);
    end;
    if RM_AsyncMessage = 0 then
      RM_AsyncMessage := RegisterWindowMessage(PChar(Format('%d_AsyncMessage', [Trunc(System.RTLVersion)]))); // do not localize
    PostMessage(FHandle, RM_AsyncMessage, 0, 0);
  end else
    TBaseAsyncResult.Dispatch(AsyncResult);
end;
...
begin
  ...
  begin
    ...
    try
      ...
      if IsCustomStyleActive then
      begin
        FBrush.Color := StyleServices(Self).GetStyleColor(scWindow);
  ...
  if ThemeControl(Self) and (csNeedsBorderPaint in ControlStyle) then
    StyleServices(Self).PaintBorder(Self, False);
end;
...
{$IF NOT DEFINED(CLR)}
  ...
  if not ClientRect.Contains(LPoint) then
  begin
    SetLength(Configs, 1);
    Configs[0].dwID := 0;
    Configs[0].dwWant := 0;
...
begin
  ...
  begin
    ...
    begin
      ...
      AControl.FOriginalParentCalcType := ctWinApi;
    end;
  end
  ...
  begin
    ...
    AControl.FOriginalParentCalcType := ctNative;
  end;
...
function TWinControl.GetAllocatedWindowHandle: HWnd;
var
  Control: TWinControl;
begin
  Control := Self;
  while (Control <> nil) and not Control.HandleAllocated do
    Control := Control.Parent;
  if Control <> nil then
    Result := Control.WindowHandle
  else
    Result := 0;
end;
...
begin
  ...
  LStyle := StyleServices(Screen.ActiveForm);
  if not LStyle.Enabled then
    Winapi.Windows.DrawEdge(DC, R, BDR_RAISEDOUTER, BF_RECT)
  else
  begin
    Details := LStyle.GetElementDetails(twWindowRoot);
    LStyle.DrawEdge(DC, Details, R, [eeRaisedOuter], [efRect]);
  end;
end;
...
procedure TDockTree.PaintDockFrame(Canvas: TCanvas; Control: TControl;
  ...
  var
    ...
    LStyle: TCustomStyleServices;
  begin
    LStyle := StyleServices(FDockSite);
    DrawRect := System.Types.Rect(Left, Top, Left+FGrabberSize-2,
      Top+FGrabberSize-2);
    if LStyle.Enabled then
    begin
      Details := LStyle.GetElementDetails(twCloseButtonNormal);
      LStyle.DrawElement(Canvas.Handle, Details, DrawRect, nil);
    end
    else
  ...
  begin
    ...
    Details := LStyle.GetElementDetails(GripperType);
...
begin
  ...
  case Message.Msg of
    ...
    WM_MOUSELEAVE: MouseLeave(Handled);
    WM_LBUTTONDBLCLK:
      with TWMMouse(Message) do
...
procedure TDockTree.MouseLeave(var Handled: Boolean);
begin
//
end;
...
begin
  if not Syslocale.FarEast then Exit;
  if IMM32DLL = 0 then
    IMM32DLL := SafeLoadLibrary(Imm32ModName);
end;
...
begin
  ...
  if IMM32DLL = 0 then Exit; // No IMM32.DLL. done.
  ...
  begin
    if Mode in [imOpen, imClose] then
    begin
...
begin
  ...
  begin
    ...
    for I := 0 to ClientCount - 1 do
      ...
      begin
        Link := TControlActionLink(Clients[I]);
...
begin
  ...
  if HintParent.Style = bhsBalloon then
    Left := Left + MulDiv(cBalloonStemHeight, LPPI, Screen.DefaultPixelsPerInch);

  FPopAbove := Top > Screen.Height div 2;
  if FPopAbove then
    Top := Top - Height - RectHeight(Rect)
  else
    Top := Top + MulDiv(Screen.CursorHeightMargin, LPPI, Screen.PixelsPerInch);
end;
...
function TCustomHint.GetCurrentPPI: Integer;
begin
  if Screen.ActiveCustomForm <> nil then
    Result := Screen.ActiveCustomForm.CurrentPPI
  else
    Result := Screen.PixelsPerInch;
end;
...
begin
  ...
  begin
    ...
    TextHeight := 0;
    Canvas.Font := Screen.HintFont;
    Canvas.Font.Height := MulDiv(Canvas.Font.Height, Self.CurrentPPI, Screen.PixelsPerInch);
...
begin
  ...
  begin
    ...
    if HintParent.Style = bhsBalloon then
      LHeight := MulDiv(cBalloonStemHeight, LPPI, Screen.DefaultPixelsPerInch)
    else
      LHeight := 0;

    Theme := StyleServices.ThemeForDPI[teToolTip, LPPI];

    Canvas.Font := Screen.HintFont;
    Canvas.Font.Height := MulDiv(Canvas.Font.Height, LPPI, Screen.PixelsPerInch);
    ...
    begin
      ...
      begin
        ...
        TextWidth := RectWidth(MeasureRect) + cTextHorizontalMargin * 2;
        TextHeight := TextHeight + RectHeight(MeasureRect) + cTextVerticalMargin * 2;
      end;
...
const
  cDefaultTextColor = $00575757;
  cDefaultGradientEndColor = $EFE4E3;
  cDefaultBkColor = $00767676;
var
  Theme: HTHEME;
...
begin
  ...
  begin
    ...
    LScaledStemHeight := MulDiv(cBalloonStemHeight, LPPI, Screen.DefaultPixelsPerInch);
    Canvas.Font := Screen.HintFont;
    Canvas.Font.Height := MulDiv(Canvas.Font.Height, LPPI, Screen.PixelsPerInch);
    ...
    begin
      if FPopAbove then
        CRect := SplitRect(ClientRect, srTop, Height - LScaledStemHeight)
      else
        CRect := SplitRect(ClientRect, srBottom, Height - LScaledStemHeight);
    end
    else
    begin
      CRect := SplitRect(ClientRect, srBottom, Height);
    ...
    begin
      ...
      begin
        if FPopAbove then
        begin
          MeasureRect := CenteredRect(SplitRect(ClientRect, srBottom, LScaledStemHeight + 1), Rect(0, 0, LScaledStemHeight, LScaledStemHeight));
        ...
        begin
          MeasureRect := CenteredRect(SplitRect(ClientRect, srTop, LScaledStemHeight), Rect(0, 0, LScaledStemHeight, LScaledStemHeight));
          StemPts[0] := MeasureRect.TopLeft;
      ...
      BkColor := cDefaultBkColor;
      GradientStartColor := clWhite;
      GradientEndColor := cDefaultGradientEndColor;
      TextColor := cDefaultTextColor;
      LStyle := StyleServices(Screen.ActiveForm);
      if LStyle.Enabled then
      begin
      ...
      Canvas.Font.Style := Canvas.Font.Style + [fsBold];
      Canvas.TextRect(TextSize, FTitle, [tfCalcRect]);
      TopText := SplitRect(CRect, srTop,  TextSize.Height + cTextVerticalMargin * 2);
      TopText := CenteredRect(TopText, TextSize);
      Canvas.TextRect(TopText, FTitle, [tfLeft, tfTop]);

      Canvas.Font.Style := Canvas.Font.Style - [fsBold];
      Canvas.TextRect(TextSize, FDescription, [tfCalcRect]);
      BottomText := SplitRect(CRect, srBottom, TextSize.Height + cTextVerticalMargin * 2);
      BottomText := CenteredRect(BottomText, TextSize);
    ...
    begin
      ...
      begin
        if FPopAbove then
        begin
          MeasureRect := SplitRect(ClientRect, srBottom, LScaledStemHeight + 1);
          DrawThemeBackground(Theme, Canvas.Handle, TTP_BALLOONSTEM, TTBSS_POINTINGDOWNLEFTWALL, SplitRect(ClientRect, srBottom, LScaledStemHeight + MulDiv(11, LPPI, Screen.DefaultPixelsPerInch)), {$IFNDEF CLR}@{$ENDIF}MeasureRect) //This +11 is a vista hack till I can find how to measure the Stem properly
        end
        else
        begin
          MeasureRect := SplitRect(ClientRect, srTop, LScaledStemHeight + 1);
          DrawThemeBackground(Theme, Canvas.Handle, TTP_BALLOONSTEM, TTBSS_POINTINGUPLEFTWALL, SplitRect(ClientRect, srTop, LScaledStemHeight + 1), {$IFNDEF CLR}@{$ENDIF}MeasureRect);
        end;
      end;
      ...
      TopText := SplitRect(CRect, srTop, MeasureRect.Height + cTextVerticalMargin * 2);
      ...
      BottomText := SplitRect(CRect, srBottom, MeasureRect.Height + cTextVerticalMargin * 2);
      ...
      if not StyleServices.GetElementColor(Details, ecTextColor, TextColor) then
        TextColor := cDefaultTextColor;
      StyleServices.DrawText(Canvas.Handle, Details, FTitle, TopText, [], TextColor, LPPI);

      Details := StyleServices.GetElementDetails(tttStandardNormal);
      if not StyleServices.GetElementColor(Details, ecTextColor, TextColor) then
        TextColor := cDefaultTextColor;
      StyleServices.DrawText(Canvas.Handle, Details, FDescription, BottomText, [], TextColor, LPPI);
...
begin
  ...
  begin
    ...
    if (FGestureEngine <> nil) and TWinControl(FControl).HandleAllocated and (csGestures in FControl.ControlStyle) then
...
class procedure TMouseHelper.GetWheelData;
const
  SPI_GETMOUSEWHEELROUTING = $201C;
begin
  // This SystemParametersInfo call was introduced in Windows 8
  // If earlier, we leave FWheelRouting set to wrFocusedWindow (0)
  if TOSVersion.Check(6,2) then
    SystemParametersInfo(SPI_GETMOUSEWHEELROUTING, 0, @FWheelRouting, 0);
end;
...
{$ELSE}
  ...
  GroupDescendentsWith(TCustomGestureManager, TControl);
