...
{$ENDIF}
  Winapi.Windows, System.SysUtils, System.Classes, Vcl.Controls, Vcl.Tabs, Winapi.Messages, System.Types, Vcl.ExtCtrls, System.Contnrs,
  Vcl.CaptionedDockTree, Vcl.Graphics;

const
  WM_PINDOCKCLIENT = WM_USER + 112;
...
type
  ...
  public
    constructor Create(AOwner: TComponent; DockClientInfo: TDockClientInfo); reintroduce; virtual; 
  ...
  public
    ...
    class function GetTabDockPanelClass: TTabDockPanelClass; virtual;
...
{$ENDIF}
    procedure WMPinDockClient(var Message: TMessage); message WM_PINDOCKCLIENT;
  protected
...
uses
  Vcl.Consts, Vcl.Forms, Vcl.GraphUtil, Vcl.Themes, Winapi.DwmApi;
...
procedure TDockTabSet.WMPinDockClient(var Message: TMessage);
var
  Client: TControl;
begin
  { Undock from this site, and redock to the other site }
  with FCurrentClient do
  begin
    Client := DockClient;
...
{$IF DEFINED(CLR)}
function TDockTabSet.DockClient(DockSource: TDragDockObject;
...
begin
  ...
  DockClient.Dock(Self, DockSource.DockRect);
end;
{$ENDIF}

{$IF DEFINED(CLR)}
function TDockTabSet.UndockClient(NewTarget, Client: TControl): Boolean;
begin
  Client.Align := alNone;
  Result := False;
end;
...
begin
  ...
  FImageIndex := -1;
  CreateTab;
  FParentPanel := GetTabDockPanelClass.Create(nil, Self);
...
class function TDockClientInfo.GetTabDockPanelClass: TTabDockPanelClass;
begin
  Result := TTabDockPanel;
end;
...
begin
  ...
  begin
    ...
    // Vista Aero doesn't handle AnimateWindow's AW_HIDE with AW_SLIDE properly
    if (not Showing) and (Win32MinorVersion = 0) and DwmCompositionEnabled then
      inherited
    else
    begin
      BeginAnimation;
...
begin
  ...
  FGrabberSize := FDockCaptionDrawer.GetGrabberSize(Self);
...
begin
  ...
  begin
    ...
    PostMessage(FDockClientInfo.DockTabSet.Handle, WM_PINDOCKCLIENT, 0, 0);
end;
...
begin
  if FIsNotXP or
    ((not FInAnimation or FFirstErase) or (not StyleServices(Self).Enabled)) then
  begin
    inherited;
    FFirstErase := False;
  end;
...
begin
  ...
  with FDockClientInfo do
    if Application.Active and { Not in the panel rect }
       (not Rect(0, 0, Width, Height).Contains(ScreenToClient(Mouse.CursorPos))) and { Not in the tab dock }
       (not Rect(0, 0, DockTabSet.Width, DockTabSet.Height).Contains(DockTabSet.ScreenToClient(Mouse.CursorPos))) then
