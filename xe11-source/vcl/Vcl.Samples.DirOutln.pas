...
unit Vcl.Samples.DirOutln;

{ Directory outline component }

interface

uses System.Classes, Vcl.Forms, Vcl.Controls, Vcl.Outline, System.SysUtils, Vcl.Graphics, Vcl.Grids, Vcl.StdCtrls,
     Vcl.Menus;
...
begin
  case TextCase of
    tcLowerCase: FCaseFunction := AnsiLowerCase;
    tcUpperCase: FCaseFunction := AnsiUpperCase;
...
begin
  ...
  try
    ...
    begin
      ...
      if (RootName <> '') and (RootName[Length(RootName)] <> '\') then
...
begin
  ...
  begin
    ...
    if (Length(TempPath) > 3) and (TempPath[Length(TempPath)] = '\') then
...
var
  TempPath: string;
begin
  if CharInSet(UpCase(NewDrive), ['A'..'Z']) then { disallow all but drive letters}
  begin
