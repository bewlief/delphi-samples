...
{ TDownLoadURL }
  ...
  TURLDownloadStatus = (dsFindingResource, dsConnecting, dsRedirecting,
    ...
    dsPersistentCookieReceived, dsSessionCookiesAllowed, dsCacheControl,
    dsContentDispositionFileName, dsMimeTextPlainMismatch, dsPublisherAvailable,
    dsDisplayNameAvailable, dsSSLUXNavBlocked, dsServerMimeTypeAvailable,
    dsSniffedClassIDAvailable, ds64BitProgress, dsReserved0, dsReserved1,
    dsReserved2, dsReserved3, dsReserved4, dsReserved5, dsReserved6,
    dsReserved7, dsReserved8, dsReserved9, dsReservedA, dsReservedB,
    dsReservedC, dsReservedD, dsReservedE, dsReservedF, dsReserved10,
    dsReserved11, dsReserved12, dsReserved13);
...
{$ELSE}
  System.IOUtils,
{$ENDIF}
  Vcl.Forms, Winapi.CommCtrl, Vcl.Consts, System.AnsiStrings;
...
begin
  ...
  Enabled := (Target is TCustomRichEdit) and not TCustomRichEdit(Target).ReadOnly;
...
{$IFDEF CLR}[WebPermission(SecurityAction.Demand, Unrestricted=True)]{$ENDIF}
...
{$IFDEF CLR}[WebPermission(SecurityAction.LinkDemand, Unrestricted=True)]{$ENDIF}
...
begin
  ...
  begin
    if UTF8Encoded then
      Result := System.AnsiStrings.StrNew(PAnsiChar(UTF8Encode(Value)))
    else
      Result := System.AnsiStrings.StrNew(PAnsiChar(AnsiString(Value)));
  end;
end;
{$ENDIF}
...
begin
  System.AnsiStrings.StrDispose(Value);
  Value := nil;
end;
{$ENDIF}
...
const
  ...
  BindStatusCode: array[0..BINDSTATUS_LAST_PRIVATE - 1] of DWORD =
