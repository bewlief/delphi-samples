  ...
  try
   if LStyle.Enabled and not LStyle.IsSystemStyle then
    begin
      ...
      if not LStyle.GetElementColor(LDetails, ecTextColor, LColor) then
        if Enabled then
          LColor := LStyle.GetSystemColor(clBtnText)
        else
          LColor := LStyle.GetSystemColor(clGrayText);
...
begin
  ...
  try
    ...
    DrawArrow(TempCanvas, sdDown,
      Point((ClientWidth - GetDropDownButtonWidth div 2) - ScaleValue(2) - 1,
            ClientHeight div 2 - 1), ScaleValue(2));
  finally
    TempCanvas.Handle := 0;
    TempCanvas.Free;
  end;
...
begin
  Dec(ARect.Right, GetDropDownButtonWidth);
  inherited DrawFrame(ARect, Down and not MouseOverDropDown);
  ARect.Left := ARect.Right;
  Inc(ARect.Right, GetDropDownButtonWidth);
...
begin
  Result := ScaleValue(cDropDownSize); // default drop down button width
end;
...
begin
  ...
  LClass := (Style as TActionBarStyleEx).GetScrollBtnClass(Self);
