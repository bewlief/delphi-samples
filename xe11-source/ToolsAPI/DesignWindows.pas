...
uses
  DeskStrs, Winapi.Windows, Math, Vcl.Clipbrd, DesignEditors, VCLEditors,
  IDETheme.Utils;
...
var L, T, W, H, MI: Integer;
    ...
    P: TPoint;
begin
  L := Form.ScaleValue(Reg.ReadInteger(SectionName, ivLeft, 0));
  T := Form.ScaleValue(Reg.ReadInteger(SectionName, ivTop, 0));
  W := Form.ScaleValue(Reg.ReadInteger(SectionName, ivWidth, 0));
  H := Form.ScaleValue(Reg.ReadInteger(SectionName, ivHeight, 0));
...
begin
  ...
  Reg.WriteInteger(SectionName, ivLeft, ConvertToDefaultPPI(Form, L));
  Reg.WriteInteger(SectionName, ivTop, ConvertToDefaultPPI(Form, T));
  Reg.WriteInteger(SectionName, ivWidth, ConvertToDefaultPPI(Form, Form.Width));
  Reg.WriteInteger(SectionName, ivHeight, ConvertToDefaultPPI(Form, Form.Height));
  if M <> nil then
