...
uses
  Winapi.Messages, System.Types, System.UITypes, System.Classes, System.Actions,
  Winapi.Windows, Vcl.Graphics, Vcl.Menus, Vcl.Controls, Vcl.Forms, Vcl.StdCtrls,
  Vcl.Dialogs, DesignIntf, DesignEditors, DesignMenus, Vcl.ActnList, Vcl.ComCtrls;
...
{ TFontNameProperty
  ...
  the fonts known by Winapi.Windows.  The following global variable will make
...
{ TFontCharsetProperty
  ...
  character-set by Winapi.Windows.}
...
{ TImeNameProperty
  ...
  the IME names known by Winapi.Windows.}
...
{ Spacing
  the Spacing parameter is used to specify the space between center-divider of the OI 
  and the property value. }
  
procedure DefaultPropertyDrawName(Prop: TPropertyEditor; Canvas: TCanvas;
  const Rect: TRect; Spacing : Integer = 0);
...
procedure DefaultPropertyListDrawValue(const Value: string; Canvas: TCanvas;
  const Rect: TRect; Selected: Boolean; Spacing : Integer = 0); overload;
...
var
  PropertyDrawingOffset: Integer = 8;
//{ Action Registration }
//
//type
//
...
uses Vcl.Consts, System.RTLConsts, System.SysUtils, System.Math, System.Win.Registry, System.TypInfo,
     Vcl.Clipbrd, Vcl.ImgList, Winapi.CommCtrl, Vcl.Themes, System.Generics.Collections, DsnConst,
     Winapi.UxTheme, Vcl.GraphUtil, DesignConst, ComponentDesigner, BrandingAPI,
     IDETheme.Utils;
...
//    end;
//  RegisteredActions.RegisterActions(ACategory, lClasses, AResource);
//
...
//    end;
//  RegisteredActions.UnregisterActions(lClasses);
//
...
//    var Image, Mask: Vcl.Graphics.TBitmap);
//  begin
...
//            Vcl.Graphics.TBitmap(THackAction(Result).FImage),
//            Vcl.Graphics.TBitmap(THackAction(Result).FMask));
//        end;
//      end;
//    end;
//  end;
...
var
  ...
  LRect: TRect;
begin
  s := Prop.GetVisualValue;
  LRect := Rect;
  Inc(LRect.Left, 1 + Spacing);
  Canvas.TextRect(LRect, s, [tfLeft, tfSingleLine, tfVerticalCenter]);
end;

procedure DefaultPropertyListDrawValue(const Value: string; Canvas: TCanvas;
  const Rect: TRect; Selected: Boolean; Spacing : Integer = 0);
var
  s: String;
  LRect: TRect;
begin
...
begin
  ...
  begin
    ...
    Result.cX := 0;
    Result.cY := 0;
    Winapi.Windows.GetTextExtentPoint32W(Handle, PWideChar(Text), Length(Text), Result);
    Changed;
  end;
end;
...
begin
  ...
  begin
    ...
    Winapi.Windows.ExtTextOutW(Handle, X, Y, Options, @Rect, PWideChar(Text),
      Length(Text), nil);
    Changed;
  end;
end;
...
procedure DefaultPropertyListDrawValue(const Value: WideString; Canvas: TCanvas;
  const Rect: TRect; Selected: Boolean; Spacing : Integer = 0);
begin
  TextRectW(Canvas, Rect, Rect.Left + 1 + Spacing, Rect.Top + 1, Value);
end;
...
begin
  if FontNamePropertyDisplayFontNames then
    ...
    begin
      ...
      Font.Name := Value;
      TextRect(ARect, ARect.Left + 2 + PropertyDrawingOffset, ARect.Top + 1, Value);

      // restore things
      Font.Name := OldFontName;
    end
  else
    DefaultPropertyListDrawValue(Value, ACanvas, ARect, ASelected, PropertyDrawingOffset);
end;
...
begin
  ...
  begin
    ...
    DefaultPropertyListDrawValue(Value, ACanvas, Rect(Right, ARect.Top,
      ARect.Right, ARect.Bottom), ASelected, PropertyDrawingOffset);
  end;
end;
...
procedure TCursorProperty.ListDrawValue(const Value: string;
  ACanvas: TCanvas; const ARect: TRect; ASelected: Boolean);
const
  cColors : Array[Boolean] of TColor = (clWindow, clHighlight);
var
  Right: Integer;
  CursorIndex: Integer;
  CursorHandle: THandle;
begin
  var LFormDPI := GetFormDPI(nil);
  Right := ARect.Left + LFormDPI.ScaleValue(cCursorSize + TIDEThemeMetrics.AlignFactor);
  with ACanvas do
  begin
    ...
    begin
      var LSize := GetSystemMetrics(SM_CXCURSOR);
      var LBitmap := TBitmap.Create(LSize, LSize);
      try
        LBitmap.PixelFormat := pf32bit;
        if ThemeProperties <> nil then
          LBitmap.Canvas.Brush.Color := ThemeProperties.StyleServices.GetSystemColor(cColors[ASelected])
        else
          LBitmap.Canvas.Brush.Color := cColors[ASelected];
        LBitmap.Canvas.FillRect(Rect(0, 0, LSize, LSize));
        DrawIconEx(LBitmap.Canvas.Handle, 0, 0, CursorHandle, LSize, LSize, 0, 0, DI_NORMAL);
        var LFactor := LFormDPI.ScaleValue(cCursorSize) / LSize;

        var NBitmap := TBitmap.Create(LSize, LSize);
        try
          NBitmap.Assign(LBitmap);
          // DrawIconEx don't scale well all the icons using custom sizes,
          // so we request the default size icon and then we scale the image
          ScaleImage(LBitmap, NBitmap, LFactor, LBitmap.PixelFormat);
          ACanvas.Draw(ARect.Left + LFormDPI.ScaleValue(2) + PropertyDrawingOffset,
            ARect.Top + LFormDPI.ScaleValue(2), NBitmap);
        finally
          NBitmap.Free;
        end;
      finally
...
procedure TCursorProperty.ListMeasureWidth(const Value: string;
  ACanvas: TCanvas; var AWidth: Integer);
begin
  AWidth := AWidth + GetFormDPI(nil).ScaleValue(cCursorSize + TIDEThemeMetrics.AlignFactor);
end;

procedure TCursorProperty.ListMeasureHeight(const Value: string;
  ACanvas: TCanvas; var AHeight: Integer);
begin
  AHeight := TIDEThemeMetrics.AdjustSize(Max(ACanvas.TextHeight('Wg'),
    GetFormDPI(nil).ScaleValue(cCursorSize + TIDEThemeMetrics.AlignFactor)));
end;
...
var
  ...
  SaveIndex, PPI: Integer;


  function FormDPI: TCustomForm;
  begin
    for var I := 0 to Screen.FormCount - 1 do
      if SameText(Screen.Forms[i].ClassName, 'TPropertyInspector') then
        Exit(Screen.Forms[i]);
...
begin
  ...
  begin
    ...
    try
      PPI := 0;
      var LHandle := WindowFromDC(ACanvas.Handle);
      if LHandle > 0 then
      begin
        var LControl := FindControl(LHandle);
        if Assigned(LControl) then
          PPI := LControl.CurrentPPI;
      end
      else
      begin
        var LControl := FindControl(GetForegroundWindow);
        if Assigned(LControl) then
          PPI := LControl.CurrentPPI;
      end;
      ...
      if PPI = 0 then
      begin
        var LFormDPI := FormDPI;
        if (LFormDPI <> nil) then
          PPI := LFormDPI.CurrentPPI
        else
        if Screen.ActiveControl <> nil then
          PPI := Screen.ActiveControl.CurrentPPI
        else
          PPI := Application.MainForm.CurrentPPI;
      end;

      LStyle.DrawElement(ACanvas.Handle,
        LStyle.GetElementDetails(ThemeStyles[State][AEnabled]), ARect, nil, PPI);
    finally
...
var
  X, W : Integer;
begin
  X := ItemRect.Right + cBorder;
  W := ItemRect.Bottom - ItemRect.Top - cBorder;
  Result := Rect(X, ItemRect.Top,  X + W, ItemRect.Bottom);
end;
...
begin
  Handled := False;
  if (paReadOnly in GetAttributes) or InNameRect then Exit;
...
begin
  ...
  if ThemeProperties <> nil then
    FControl.StyleName := ThemeProperties.StyleServices.Name;
  FControl.OnKeyPress := CalendarKeyPress;
  FControl.OnDblClick := CalendarDblClick;
  var LFormDPI := GetFormDPI(nil);
  if TIDEThemeMetrics.Font.Enabled then
    FControl.Font.Assign(TIDEThemeMetrics.Font.GetFont);
  TIDEThemeMetrics.Font.AdjustDPISize(FControl.Font, TIDEThemeMetrics.Font.Size, LFormDPI.CurrentPPI);

  FControl.Align := alNone;
  FControl.Visible := False;
  FControl.ParentWindow := LFormDPI.Handle;
...
begin
  ...
  if SelectionEditorList <> nil then
...
// prevent scaling the TMonthCalendar
procedure TMonthCalendar.ChangeScale(M, D: Integer; isDpiChange: Boolean);
begin
  //
end;
