...
uses System.RTLConsts, System.Math, System.TypInfo, ToolsAPI, BrandingAPI,
  DsnConst, DesignConst, ComponentDesigner, Winapi.Windows, Winapi.UxTheme,
  Vcl.GraphUtil, Vcl.Themes, Vcl.Forms, Vcl.Dialogs, Vcl.ImageCollection, IDETheme.Utils,
  Vcl.ActnList;

var vGetIDEActions: TStringList;

type
  TOpenActionList = class (Vcl.ActnList.TCustomActionList);
...
var
  LImageCollection: TImageCollection;
begin
  inherited;
  if Application.MainForm.FindComponent('ImageList1') <> nil then
    LImageCollection  := TVirtualImageList(Application.MainForm.FindComponent('ImageList1')).ImageCollection as TImageCollection
  else LImageCollection := nil;
  ...
  FImageList.Width := LControlDPI.ScaleValue(TIDEThemeMetrics.Images.DefaultSize);
  FImageList.Height := FImageList.Width;
  FImageList.ImageCollection := LImageCollection;
  ...
  if ThemeProperties <> nil then
    Color := ThemeProperties.StyleServices.GetSystemColor(clWindow);
  UpdateFont;
end;

procedure TActionListView.UpdateFont;
begin
  if TIDEThemeMetrics.Font.Enabled then
  begin
    Font.Assign(BrandingAPI.TIDEThemeMetrics.Font.GetFont());
    TIDEThemeMetrics.Font.AdjustDPISize(Font, TIDEThemeMetrics.Font.Size, GetControlForDPI.CurrentPPI);
...
procedure TActionListView.CNMeasureItem(var Message: TWMMeasureItem);
begin
  inherited;
  Message.MeasureItemStruct.itemHeight := TIDEThemeMetrics.AdjustSize(Message.MeasureItemStruct.itemHeight);
end;

function TActionListView.GetControlForDPI: TWinControl;
begin
  if Parent = nil then
    Result := GetFormDPI(nil) as TWinControl
  else
    Result := Parent;
end;
...
begin
  ...
  LSelected := (Item.Index <> cSeparatorIndex) and ([cdsSelected, cdsFocused, cdsHot] * State <> []);
  case Stage of
    cdPostPaint:
      begin
        ...
        // Draw separator
        if Item.Index = cSeparatorIndex then
        begin
          Canvas.Pen.Color := GetGrayBlendColor(LStyle);
        ...
        begin
          if Item.ImageIndex <> - 1 then
            FImageList.Draw(Canvas, LRect.Left + LControlDPI.ScaleValue(TIDEThemeMetrics.AlignFactor),
              LRect.Top, Item.ImageIndex);

          Inc(LRect.Left, FImageList.Width + LControlDPI.ScaleValue(TIDEThemeMetrics.AlignFactor) * 3);
          s := Item.Caption;
        ...
        begin
          LRect.Left := LRect.Right - LControlDPI.ScaleValue(20);
          if ThemeProperties <> nil then
          begin
            Canvas.Pen.Color := Canvas.Font.Color;
            LChevronSize := LRect.Height div 4;
            LPoint.X := LRect.Left + LChevronSize + (LRect.Width - LChevronSize) div 2;
            LPoint.Y := LRect.Top + (LRect.Height - 2 * LChevronSize) div 2;
            TIDEThemeDrawers.DrawChevron(Canvas, TScrollDirection.sdRight, LPoint, LChevronSize);
          end
          else
          if StyleServices.Enabled and (Win32MajorVersion >= 6) then
            DrawThemeBackground(StyleServices.Theme[teMenu], Canvas.Handle, MENU_POPUPSUBMENU,
              MSM_NORMAL, LRect, nil)
          else
            DrawArrow(Canvas, sdRight, Point(LRect.Right - LControlDPI.ScaleValue(15),
              LRect.Top + ((LRect.Bottom - LRect.Top - LControlDPI.ScaleValue(8)) div 2)), LControlDPI.ScaleValue(4));
        end;
      end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      Form.StyleName := ThemeProperties.StyleServices.Name;
    end;
    SetupIDEDialogTitleBar(Form, nil, True);
    try
...
var
  ...
  begin
    ...
    begin
      ...
      if (LImages <> nil) and (Action.ImageIndex <> -1) then
      begin
        LIcon := TIcon.Create;
        try
          LImages.GetIcon(Action.ImageIndex, LIcon);
          if (LIcon <> nil) then
            ListItem.ImageIndex := IDEImageCollection.AddIconToVirtualImageList('ActionEditors', 'ActionIcon' + IntToStr(Action.ImageIndex),
              LIcon, FImageList, True);
        finally
          LIcon.Free;
        end;
      end;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        MinWidth := Max(Width, Canvas.TextWidth(SNoSetProp) + LControlDPI.ScaleValue(25));
          ...
          try
            ...
            begin
              ...
              begin
                ...
                if (LWidth + LControlDPI.ScaleValue(40)) > MinWidth then
                  MinWidth := LWidth + LControlDPI.ScaleValue(40);
...
procedure TActionListView.Resize;
begin
  inherited;
...
begin
  ...
  begin
    ...
    if N > 0 then
      Height := (Min(N, 11) + 3) * GetFormDPI(nil).ScaleValue(FDefItemHeight)
    else
      Height := GetFormDPI(nil).ScaleValue(FDefItemHeight);
...
begin
  ...
  try
    ...
    LImages := nil;
    if ActionList is Vcl.ActnList.TCustomActionList then
      LImages := TOpenActionList(ActionList).Images;
    // This pair of BeginUpdate/EndUpdate calls ensures that the EndUpdate called in CopyImageIfAvailable (below),
    // has no immediate effect. Without it, that EndUpdate triggers a call to the design surface's Modified, which
    // ultimately destroys this TActionProperty object, making the call to SelectAction crash. Instead, the
    // EndUpdate below now allows that designer interaction to proceed after all the method code has executed.
    if LImages <> nil then
      LImages.BeginUpdate;
    try
