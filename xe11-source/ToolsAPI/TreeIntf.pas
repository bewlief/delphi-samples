...
uses
  Winapi.Windows, Winapi.Messages, Winapi.CommCtrl, Winapi.ActiveX,
  System.Win.ComObj, System.SysUtils, System.Classes, System.Variants,
  System.TypInfo, System.Contnrs, System.IniFiles, System.Types,
  Vcl.Graphics, Vcl.Controls, Vcl.Forms, Vcl.Dialogs, Vcl.ExtCtrls,
  Vcl.ComCtrls, Vcl.Menus, Vcl.ImgList, Vcl.AppEvnts,
  DesignIntf, DesignEditors, DesignMenus, StructureViewAPI;
...
{ sprig \Sprig\, n.
  ...
  public
    ...
    property IconIndex: TImageIndex read GetIconIndex;
...
uses
  System.Generics.Collections, System.Math, ToolsAPI, DesignConst, ComponentDesigner, PaletteAPI;

type
  TIconGeneratedClasses = TDictionary<TClass, TImageIndex>;

var
  _IconGeneratedClassesList: TIconGeneratedClasses;
...
type
  ...
  TInternalDragSprigs = class(System.Classes.TStreamAdapter, IDragSprigs)
  private
    FDragSprigs: TList;
  protected
    function Add(ASprig: TSprig): Integer;
...
const
  ...
  cTComponentBasedItem = 'TComponentBased'; { Do not localize }

  function GenIDString(const AClassName, AGroupName: string): string;
  begin
    Result := AClassName;
    if AGroupName <> '' then
      Result := AGroupName + '.' + Result;
    Result := cTComponentBasedItem + '.' + Result;
  end;

var
  LService280: IOTAStructureView280;
  LPalServices: IOTAPaletteServices;
  LGroupClass: TClass;
  LIDString: string;
  LPainter280: INTAPalettePaintIcon280;
  LPaletteItem: IOTABasePaletteItem;
  LGraphicArray: TGraphicArray;
  i: Integer;
begin
  inherited Create;
  FItem := AItem;
  ...
  if (FItem <> nil) then
  begin
    if not _IconGeneratedClassesList.TryGetValue(FItem.ClassType, FIconIndex) then
    begin
      if Supports(BorlandIDEServices, IOTAStructureView280, LService280) and
        Supports(BorlandIDEServices, IOTAPaletteServices, LPalServices) then
      begin
        // let's try to get a icon of the represented component
        LGroupClass := ClassGroupOf(FItem);
        repeat
          LIDString := GenIDString(FItem.ClassType.ClassName, LGroupClass.UnitName + '.' + LGroupClass.ClassName);
          LPaletteItem := LPalServices.BaseGroup.FindItem(LIDString, True);
          LGroupClass := LGroupClass.ClassParent;
        until (LPaletteItem <> nil) or (LGroupClass = nil) or (LGroupClass = TPersistent.ClassParent);

        if Supports(LPaletteItem, INTAPalettePaintIcon280, LPainter280) then
        begin
          if LPaletteItem.ImageIndex = -1 then
            LPaletteItem.ImageIndex := LPainter280.LoadIconFromResource;

          with LPalServices.ImageCollection.Images.Items[LPaletteItem.ImageIndex] do
          begin
            SetLength(LGraphicArray, SourceImages.Count);

            for i := 0 to SourceImages.Count -1 do
            begin
              LGraphicArray[i] := TWicImage.Create;
              LGraphicArray[i].Assign(SourceImages.Items[i].Image);
            end;
          end;

          FIconIndex := LService280.AddImage(FItem.ClassType.ClassName, LGraphicArray);
          _IconGeneratedClassesList.Add(FItem.ClassType, FIconIndex);

          for i := 0 to Length(LGraphicArray) - 1 do
            LGraphicArray[i].Free;
        end;
      end;
    end;
  end;
...
function TSprig.GetIconIndex: TImageIndex;
begin
  Result := FIconIndex;
end;
...
begin
  if (FIconIndex > 0) then
    Result := FIconIndex - GImageOffset
  else
  begin
    Result := FImageIndex;
    if Ghosted then
...
var
  LParent: TSprig;
begin
  if (AItem <> nil) and not AItem.Invalid then
  begin
...
var
  LStructureView: IOTAStructureView280;
  LDesigner: IDesigner;
begin
  if FPendingClearSelection then
  begin
    SetPendingClearSelection(False);
    LStructureView := BorlandIDEServices as IOTAStructureView280;
...
initialization
  ...
  _IconGeneratedClassesList := TIconGeneratedClasses.Create;

finalization
  _IconGeneratedClassesList.Free;
