...
{ TCustomModule
  ...
  protected
    function GetFinderClass: TPersistentClass; virtual;
  public
...
var
  ...
  HasStoredProc, ProcAsInt, Mask: NativeUInt;
begin
  ...
  begin
    ...
    ProcAsInt := NativeUInt(PPropInfo(GetPropInfo)^.StoredProc);
    Mask := $FF;
    HasStoredProc := ProcAsInt and (not Mask);
...
function TPropInfoList.Contains(P: PPropInfo): Boolean;
  ...
  function CompareShortStrings(const S1, S2: ShortString): Boolean;
  begin
    Result := (Length(S1) = Length(S2)) and
      (StrLIComp(PAnsiChar(@S1[1]), PAnsiChar(@S2[1]), Length(S1)) = 0);
  end;
...
begin
  ...
  for I := 0 to FCount - 1 do
    if (FList^[I]^.PropType^ = LType) and CompareShortStrings(FList^[I]^.Name, P^.Name) then
    begin
      Result := True;
      Exit;
    end;
...
const
  PropClassMap: array[System.TypInfo.TTypeKind] of TPropertyEditorClass = (
    nil,                // tkUnknown
...
function TCustomModule.GetFinderClass: TPersistentClass;
var
  Base: TComponent;
begin
  Base := Root;
  if Base.Owner <> nil then
    ...
    Base := Base.Owner;
  Result := TPersistentClass(Base.ClassType);
end;
...
var
  FinderClass: TPersistentClass;
begin
  FinderClass := GetFinderClass;
  if (FFinder = nil) or (FFinder.FinderClass <> FinderClass) then
  begin
    FreeAndNil(FFinder);
    FFinder := TClassFinder.Create(FinderClass);
  end;
...
begin
  ...
  case VarType(Value) and varTypeMask of
    varDispatch:
      Result := DesignConst.SDispatch;
    varUnknown:
      Result := DesignConst.SUnknown;
    else
...
procedure TVariantProperty.SetValue(const Value: string);
  ...
  begin
    ...
    if NewType = varCurrency then
      Result := AnsiPos(FormatSettings.CurrencyString, Value) > 0;
    if Result then
    try
...
begin
  ...
  if Value = '' then
    ...
    V := NULL
  else if (CompareText(Value, DesignConst.SError) = 0) or
          (CompareText(Value, DesignConst.SDispatch) = 0) or
          (CompareText(Value, DesignConst.SUnknown) = 0) then
    Exit
...
initialization
  ...
  DesignIntf.RegisterComponentGuidelinesProc := RegisterComponentGuidelines;
...
finalization
  ...
  DesignIntf.RegisterComponentGuidelinesProc := nil;
