...
uses Winapi.Windows, System.Classes, System.Variants, System.Win.StdVCL, System.Generics.Collections,
     Vcl.Graphics, Vcl.OleCtrls, Vcl.OleServer, Winapi.ActiveX, WinApi.Messages, Winapi.Mshtmhst,
     Vcl.Controls, Vcl.StdCtrls, Vcl.Forms, Vcl.Edge, Winapi.WebView2;
...
// *********************************************************************//
  ...
  private
    class var FEdgeBrowserExecutableFolders: TDictionary<TWebBrowser, string>;
    class var FEdgeUserDataFolders: TDictionary<TWebBrowser, string>;
    function GetEdgeBrowserExecutableFolder: string;
    function GetEdgeUserDataFolder: string;
  ...
  begin
    ...
    case FSelectedEngine of
      ...
      begin
        ...
        FEdge.UserDataFolder := EdgeUserDataFolder;
        FEdge.Parent := Self;
      end;
    end;
  end;
...
function TWebBrowserHelper.GetEdgeBrowserExecutableFolder: string;
begin
  Result := '';
  if FEdgeBrowserExecutableFolders <> nil then
    FEdgeBrowserExecutableFolders.TryGetValue(Self, Result)
end;

function TWebBrowserHelper.GetEdgeUserDataFolder: string;
begin
  Result := '';
  if FEdgeUserDataFolders <> nil then
    FEdgeUserDataFolders.TryGetValue(Self, Result)
end;

procedure TWebBrowserHelper.SetEdgeBrowserExecutableFolder(const Value: string);
begin
  if GetEdgeBrowserExecutableFolder <> Value then
  begin
    if FEdgeBrowserExecutableFolders = nil then
      FEdgeBrowserExecutableFolders := TDictionary<TWebBrowser, string>.Create;
    FEdgeBrowserExecutableFolders.AddOrSetValue(Self, Value);
    if FEdge <> nil then
    begin
      FEdge.BrowserExecutableFolder := Value;
...
procedure TWebBrowserHelper.SetEdgeUserDataFolder(const Value: string);
begin
  if GetEdgeUserDataFolder <> Value then
  begin
    if FEdgeUserDataFolders = nil then
      FEdgeUserDataFolders := TDictionary<TWebBrowser, string>.Create;
    FEdgeUserDataFolders.AddOrSetValue(Self, Value);
    if FEdge <> nil then
    begin
      FEdge.UserDataFolder := Value;
      // Without this, the specified user data folder probably won't be applied
      FEdge.ReinitializeWebView;
    end;
  end;
...
finalization
  TWebBrowser.FEdgeBrowserExecutableFolders.Free;
  TWebBrowser.FEdgeUserDataFolders.Free;
end.
