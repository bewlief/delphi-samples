...
type
  ...
  TISAPIServerExceptionProc = procedure(E: TObject; var ECB: TEXTENSION_CONTROL_BLOCK);

  TISAPIApplication = class(TWebApplication)
  private
    FFactory: TISAPIFactory;
    FTerminateProc: TISAPITerminateProc;
    FServerExceptionProc: TISAPIServerExceptionProc;
  ...
  public
    ...
    /// <summary>Event handler for custom processing of exceptions that are raised while processing a request</summary>
...
uses System.SysUtils, Web.BrkrConst, System.AnsiStrings;
...
begin
  try
    ...
    System.AnsiStrings.StrLCopy(Ver.lpszExtensionDesc, PUTF8Char(UTF8String(Title)), HSE_MAX_EXT_DLL_NAME_LEN);
    Integer(Result) := 1; // This is so that the web server will know what "True" really is
  except
    Result := False;
  end;
end;
...
begin
  try
    ...
    if Assigned(OnServerException) then
      OnServerException(ExceptObject, ECB)
    else
