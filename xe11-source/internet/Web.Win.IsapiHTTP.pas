...
function TISAPIRequest.GetIntegerVariable(Index: Integer): Int64;
var
  Value: string;
begin
  Value := GetStringVariable(Index);
...
begin
  ...
  if ECB.ServerSupportFunction(ECB.ConnID, HSE_REQ_MAP_URL_TO_PATH,
    @LBytes[0], @Size, nil) then
      Result := DefaultCharSetEncoding.GetString(LBytes, 0, Size - 1);
end;
...
function TISAPIResponse.GetIntegerVariable(Index: Integer): Int64;
begin
...
procedure TISAPIResponse.SetIntegerVariable(Index: Integer; Value: Int64);
begin
  if (Index >= Low(FIntegerVariables)) and (Index <= High(FIntegerVariables)) then
