...
{ TWebRequestFiles }
  ...
  private
    FList: TObjectList<TWebRequestFile>;
  protected
...
begin
  FList := TObjectList<TWebRequestFile>.Create(True {Owned});
  inherited Create;
end;
...
function TWebRequestFileStream.Write(const Buffer; Count: Longint): Longint;
begin
  // Write not supported
  Assert(False);
  Result := 0;
