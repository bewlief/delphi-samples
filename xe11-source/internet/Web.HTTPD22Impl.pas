...
unit Web.HTTPD22Impl;

// Implement the HTTPD interface for version 2.2
interface
...
type
  ...
  THTTPMethods22 = class(TCommonHTTPDMethods)
  public
    constructor Create;
...
procedure THTTPMethods22.InitModule;
var
  LModule: PModule;
begin
...
constructor THTTPMethods22.Create;
begin
...
function THTTPMethods22.get_Content(
...
function THTTPMethods22.get_client_block(const ARequestRec: PHTTPDRequest; Buffer: PByte; Count: Integer): Integer;
const
  cMaxChunkSize = 1024 * 10;
var
  LBytesRead: Integer;
...
function THTTPMethods22.get_ContentType(
  const ARequestRec: PHTTPDRequest): UTF8String;
begin
  Result := Prequest_rec(ARequestRec)^.content_type
end;

function THTTPMethods22.get_content_encoding(
  const ARequestRec: PHTTPDRequest): UTF8String;
begin
  Result := Prequest_rec(ARequestRec)^.content_encoding
end;

function THTTPMethods22.get_filename(
  const ARequestRec: PHTTPDRequest): UTF8String;
begin
  Result := Prequest_rec(ARequestRec)^.filename
end;

function THTTPMethods22.get_handler(
  const ARequestRec: PHTTPDRequest): UTF8String;
begin
  Result := Prequest_rec(ARequestRec)^.handler
end;

function THTTPMethods22.get_headers_in(
...
function THTTPMethods22.get_hostname(
  const ARequestRec: PHTTPDRequest): UTF8String;
begin
  Result := Prequest_rec(ARequestRec)^.hostname
end;

function THTTPMethods22.get_method(
  const ARequestRec: PHTTPDRequest): UTF8String;
begin
  Result := Prequest_rec(ARequestRec)^.method
end;

function THTTPMethods22.get_path_info(
  const ARequestRec: PHTTPDRequest): UTF8String;
var
  LSplit:  TStrings;
  I, IStart: Integer;
...
function THTTPMethods22.get_protocol(
  const ARequestRec: PHTTPDRequest): UTF8String;
begin
  Result := Prequest_rec(ARequestRec)^.protocol
end;

function THTTPMethods22.get_ServerPort(
  const ARequestRec: PHTTPDRequest): UTF8String;
begin
...
function THTTPMethods22.get_unparsed_uri(
  const ARequestRec: PHTTPDRequest): UTF8String;
begin
  Result := Prequest_rec(ARequestRec)^.unparsed_uri
end;

procedure THTTPMethods22.hook_handler(const AHandler: THTTPDMethods.THandlerProc;
  const APre, ASucc: PPChar; AOrder: Integer);
begin
  ap_hook_handler(ap_hook_handler_t(AHandler), APre, ASucc, AOrder);
end;

procedure THTTPMethods22.hook_child_init(const AInitiation: THTTPDMethods.TInitiationProc;
  const APre, ASucc: PPChar; AOrder: Integer);
begin
  ap_hook_child_init(ap_hook_child_init_t(AInitiation), APre, ASucc, AOrder);
end;

procedure THTTPMethods22.pool_cleanup_register(const p: PHTTPDPool;
...
procedure THTTPMethods22.set_ContentType(const ARequestRec: PHTTPDRequest;
  const AValue: UTF8String);
begin
  Prequest_rec(ARequestRec)^.content_type := 'text/html';
end;

procedure THTTPMethods22.add_headers_out(const ARequestRec: PHTTPDRequest; const Key, Value: UTF8String);
begin
  apr_table_add(Prequest_rec(ARequestRec)^.headers_out, PUTF8Char(Key), PUTF8Char(Value));
end;

procedure THTTPMethods22.set_headers_out(const ARequestRec: PHTTPDRequest;
...
procedure THTTPMethods22.set_headers_out_ContentEncoding(
...
procedure THTTPMethods22.set_headers_out_ContentType(
...
procedure THTTPMethods22.set_status(const ARequestRec: PHTTPDRequest;
  ACode: Integer);
begin
  Prequest_rec(ARequestRec)^.status := ACode;
end;

function THTTPMethods22.write_buffer(const ARequestRec: PHTTPDRequest;
...
function THTTPMethods22.write_string(const ARequestRec: PHTTPDRequest;
...
initialization
  RegisterHTTPD(THTTPMethods22.Create);
