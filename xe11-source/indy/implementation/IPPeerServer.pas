...
type
  ...
  public
    ...
    function GetIPv4Subnets: TArray<TIPv4Subnet>;
  ...
  public
    ...
    function GetSocket(Index: Integer): IIPSocketHandle;
...
{$ENDIF}
  ...
  private
    ...
    ARequestInfo: TIdHTTPRequestInfo; AResponseInfo: TIdHTTPResponseInfo);
    procedure LOnParseAuthentication(AContext: TIdContext; const AAuthType,
      AAuthData: String; var VUsername, VPassword: String;
      var VHandled: Boolean);
    procedure LOnQuerySSLPort(APort: TIdPort; var VUseSSL: Boolean);
  protected
    function GetActive: Boolean;
    procedure SetActive(Val: Boolean);
    function GetDefaultPort: TIPPortPeer;
...
function TIdStackPeer.GetIPv4Subnets: TArray<TIPv4Subnet>;
var
  I: Integer;
  LCount: Integer;
...
begin
  ...
  try
    ...
    begin
      if LList[I].IPVersion = Id_IPv4 then
      begin
        LMask := IPv4ToUInt32((LList[I] as TIdStackLocalAddressIPv4).SubNetMask);
        Result[LCount] := TIPv4Subnet.Create(LList[I].IPAddress, IPv4ToUInt32(LList[I].IPAddress) and LMask, LMask);
        Inc(LCount);
      end;
    end;
  finally
...
function TIdStackPeer.IPv4ToUInt32(const AIPAddress: string): UInt32;
begin
  Result := IdGlobal.IPv4ToUInt32(AIPAddress);
end;
...
function TIdSocketHandlesPeer.GetCount: Integer;
begin
...
function TIdSocketHandlesPeer.GetSocket(Index: Integer): IIPSocketHandle;
begin
  Result := PeerFactory.CreatePeer(GetIPImplementationID, IIPSocketHandle, FHandles[Index]) as IIPSocketHandle;
end;
...
begin
  ...
  FHTTPServer.OnParseAuthentication := LOnParseAuthentication;
  FHTTPServer.OnQuerySSLPort := LOnQuerySSLPort;
end;
...
procedure TIdHTTPServerPeer.LOnParseAuthentication(AContext: TIdContext;
  const AAuthType, AAuthData: String; var VUsername, VPassword: String; var VHandled: Boolean);
var
  s: String;
  LDecoder: TIdDecoderMIME;
begin
  if (not VHandled) and TextIsSame(AAuthType, 'Basic') then begin    {Do not Localize}
...
procedure TIdHTTPServerPeer.LOnQuerySSLPort(APort: TIdPort; var VUseSSL: Boolean);
begin
  VUseSSL := True;
end;
