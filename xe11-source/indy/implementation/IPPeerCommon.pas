...
type
  ...
  public
    ...
    procedure AddFormFile(const AFieldName, AFieldValue, AContentType: string);
...
{$ENDIF}
  ...
  public
    function GetObject: TObject;
    function GetIPImplementationID: string;
    procedure Broadcast(const AData: TIPBytesPeer; const APort: TIPPortPeer; const AIP: String = '');
...
{$IF not Defined(NEXTGEN) or not Defined(MSWINDOWS)}
  ...
  TIdProc = class(TInterfacedObject, IIPPeerProcs, IIPeerProcsGlobals)
  public
    { IIPeerProcsGlobals }
    procedure RSA_register_global(APtr: PRSAPeer);
    procedure RSA_free_global(APtr: PRSAPeer);
  ...
  public
    constructor Create;
    destructor Destroy; override;
    procedure Add(APtr: PRSAPeer);
    procedure Remove(APtr: PRSAPeer);
  end;
...
begin
  ...
  FList.Duplicates := TDuplicates.dupError;
end;
...
procedure TGlobals.Remove(APtr: PRSAPeer);
var
  LList: TList;
  I: Integer;
begin
...
procedure TGlobals.Add(APtr: PRSAPeer);
begin
  FList.Add(APtr);
end;
...
procedure TIdSocketHandlePeer.Broadcast(const AData: TIPBytesPeer;
  const APort: TIPPortPeer; const AIP: String);
begin
  FHandle.Broadcast(TIdBytes(AData), APort, AIP);
end;
...
procedure TIdProc.RSA_register_global(APtr: PRSAPeer);
begin
  FGlobals.Add(APtr);
end;

procedure TIdProc.RSA_free_global(APtr: PRSAPeer);
begin
  FGlobals.Remove(APtr);
end;
...
procedure TIdMultipartFormDataStreamPeer.AddFormFile(const AFieldName, AFieldValue, AContentType: string);
begin
  FStream.AddFile(AFieldName, AFieldValue, AContentType);
end;
...
function TIdMultipartFormDataStreamPeer.GetRequestContentType : string;
begin
  Result := FStream.RequestContentType;
end;
