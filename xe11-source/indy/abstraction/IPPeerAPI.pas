...
type
  ...
  ///<summary> Manage global objects.  Global objects are automatically freed
  /// when the implementation is unloaded. </summary>
  IIPeerProcsGlobals = interface
    ['{2CD26FAB-193A-43E0-9783-67E2FE777D0F}']
    /// <summary>Register an object to be freed on shutdown </summary>
    procedure RSA_register_global(APtr: PRSAPeer);
    /// <summary>Free an object that has been registered.</summary>
  ...
  /// <summary>Information on one IPv4 subnet.</summary>
  TIPv4Subnet= record
  public
    /// <summary>IP address.</summary>
    Address: string;
    /// <summary>Subtnet mask.</summary>
    Subnet : UInt32;
    /// <summary>Masked Address, contains only bits selected by the Subnet mask.</summary>
    Mask   : UInt32;
    constructor Create(const AAddress: string; ASubnet, AMask: UInt32);
  end;
  ...
  IIPSocketHandles = Interface
    ...
    property Sockets[Index: Integer]: IIPSocketHandle read GetSocket;
  End;
...
constructor TIPv4Subnet.Create(const AAddress: string; ASubnet, AMask: UInt32);
begin
  Address := AAddress;
  Mask := AMask;
  Subnet := ASubnet;
