...
procedure TBitmapLinks.TPropertyLoader.ReadCapInsets;
  ...
  begin
    ...
    try
      ...
      Filer.DefineProperty(S.ToString(True), ReadData, nil, False);
    finally
      S.Free;
    end;
  end;
...
var
  ...
  BRGAValue, PremTint: TAlphaColor;
  Pixel: PAlphaColor;
  I, J: Integer;
begin
  ...
  try
    ...
    begin
      ...
      begin
        BRGAValue := PixelToAlphaColor(Pixel, Bitmap.PixelFormat);
        BRGAValue := ModulatePixel(BRGAValue, PremTint);
        AlphaColorToPixel(BRGAValue, Pixel, Bitmap.PixelFormat);
        Inc(Pixel);
      end;
    end;
  finally
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Canvas.DrawBitmap(FTintBuffer, FTintBuffer.BoundsF, ARect, AOpacity, True);
      end;
    end;
...
procedure TActiveStyleObject.SetActive(const Value: Boolean);
begin
  FActive := Value;
end;
...
procedure TActiveStyleObject.SetNewScene(AScene: IScene);
begin
  inherited;
  if AScene = nil then
    Active := False;
...
procedure TActiveStyleTextObject.SetNewScene(AScene: IScene);
begin
  inherited;
  if AScene = nil then
    Active := False;
