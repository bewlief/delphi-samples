...
uses
  System.SysUtils, System.UITypes, System.Types, System.Math.Vectors, System.Classes, FMX.Types;
...
uses
  System.UIConsts, System.RTLConsts, System.Math, FMX.Consts;
...
begin
  ...
  begin
    if FormatSettings.ThousandSeparator <> FormatSettings.DecimalSeparator then
      Result := Result.Replace(FormatSettings.ThousandSeparator, '', [rfReplaceAll]);
    if string.Compare(Result, NaN.ToString, [TCompareOption.coIgnoreCase]) = 0 then
      Result := NaN.ToString
    else if string.Compare(Result, Infinity.ToString, [TCompareOption.coIgnoreCase]) = 0 then
      Result := Infinity.ToString
    else if string.Compare(Result, '+' + Infinity.ToString, [TCompareOption.coIgnoreCase]) = 0 then
      Result := Infinity.ToString
    else if string.Compare(Result, NegInfinity.ToString, [TCompareOption.coIgnoreCase]) = 0 then
      Result := NegInfinity.ToString
    else
      for I := 0 to PossibleDecimalDelimiters.Length - 1 do
        if (PossibleDecimalDelimiters.Chars[I] <> FormatSettings.DecimalSeparator) and
          (PossibleDecimalDelimiters.Chars[I] <> FormatSettings.ThousandSeparator) then
          Result := Result.Replace(PossibleDecimalDelimiters.Chars[I], FormatSettings.DecimalSeparator, [rfReplaceAll]);
  end;
end;
...
class function TFMXObjectHelper.FindNearestParentOfClass<T>(const AObject: TFMXObject; var AResult: T): Boolean;
var
  ResultObject: TFmxObject;
begin
  Result := FindParent(AObject, function (AObject: TFmxObject): Boolean
    begin
      Result := AObject is T;
    end, ResultObject);
  if ResultObject is T then
    AResult := T(ResultObject)
  else
    AResult := nil;
end;

class function TFMXObjectHelper.FindParent(const AObject: TFMXObject; const AFilter: TFilterFunc;
  var AResult: TFMXObject): Boolean;
var
  CurrentObject: TFmxObject;
begin
  if not Assigned(AFilter) or (AObject = nil) then
    AResult := nil
  else
  begin
    ...
    until (CurrentObject = nil) or AFilter(CurrentObject);
    AResult := CurrentObject;
  end;
  Result := AResult <> nil;
end;

class function TFMXObjectHelper.GetNearestParentOfClass<T>(const AObject: TFMXObject): T;
begin
  if not FindNearestParentOfClass<T>(AObject, Result) then
    raise ESearchException.Create(SCannotFindParentBySpecifiedCriteria);
end;

class function TFMXObjectHelper.GetParent(const AObject: TFMXObject; const AFilter: TFilterFunc): TFMXObject;
begin
  if not FindParent(AObject, AFilter, Result) then
    raise ESearchException.Create(SCannotFindParentBySpecifiedCriteria);
end;
