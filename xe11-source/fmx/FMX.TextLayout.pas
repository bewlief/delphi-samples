...
function TTextRange.InRange(const AIndex: Integer): Boolean;
begin
  Result := System.Math.InRange(AIndex, Pos, Pos + Length - 1);
end;

{ TTextAttribute }

constructor TTextAttribute.Create(const AFont: TFont; const AColor: TAlphaColor);
begin
  Self.Font := AFont;
  Self.Color := AColor;
end;

constructor TTextAttribute.Create(const AExisting: TTextAttribute; const ANewFont: TFont);
begin
  Self := AExisting;
  Self.Font := ANewFont;
end;

constructor TTextAttribute.Create(const AExisting: TTextAttribute; const ANewColor: TAlphaColor);
begin
  Self := AExisting;
...
constructor TTextAttributedRange.Create(const ARange: TTextRange; const AAttribute: TTextAttribute);
begin
  Self.Range := ARange;
...
procedure TTextLayout.DoColorChanaged;
begin
end;
...
function TTextLayout.GetSize: TSizeF;
begin
...
function TTextLayout.PositionAtPoint(const APoint: TPointF; const RoundToWord: Boolean): Integer;
var
...
procedure TTextLayout.AttributesChanged(Sender: TObject; const Item: TTextAttributedRange; Action: TCollectionNotification);
begin
{$IFNDEF AUTOREFCOUNT}
...
function TTextLayout.RegionForRange(const ARange: TTextRange; const RoundToWord: Boolean): TRegion;
var
  LRange: TTextRange;
begin
...
procedure TTextLayout.SetColor(const Value: TAlphaColor);
begin
