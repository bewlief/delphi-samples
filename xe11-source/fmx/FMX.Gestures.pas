...
type
  ...
  strict private
    ...
    FSaved: Boolean;
    FStreamCollections: TStack<TCollection>;
    FUpdating: Boolean;
...
type
  ...
  private
    ...
    procedure SetGestureCollection(const Value: TGestureCollection);
  public
  ...
  published
    ...
    property Collection: TGestureCollection read FCollection write SetGestureCollection;
  end;
...
destructor TGestureStreamData.Destroy;
begin
...
procedure TGestureStreamData.SetGestureCollection(const Value: TGestureCollection);
begin
...
begin
  ...
  else if FStreamCollections <> nil then
  begin
    MissingGestures := TList<TCustomGestureCollectionItem>.Create;
    try
      LStreamCollection := TGestureStreamList(FStreamCollections.Peek);
      for I := 0 to LStreamCollection.Count - 1 do
      begin
        ...
        LStreamCollection[I].OwnsCollection := False;
    ...
    while FStreamCollections.Count > 0 do
      FStreamCollections.Pop.Free;
    FreeAndNil(FStreamCollections);
  end;
end;
...
begin
  if FStreamCollections = nil then
    FStreamCollections := TStack<TCollection>.Create;
  FStreamCollections.Push(TGestureStreamList.Create(csReading in ComponentState));
  Reader.ReadValue;
  Reader.ReadCollection(FStreamCollections.Peek);
end;
