...
{ Shaders }
  ...
  TContextShaderArch = (Undefined, DX9, DX10, DX11_level_9, DX11, Metal, GLSL, Mac, IOS, Android);
...
procedure GetVertexOffsets(Buffer: TVertexBuffer);
begin
  Buffer.FTexCoord0 := GetVertexOffset(TVertexFormat.TexCoord0, Buffer.FFormat);
  Buffer.FTexCoord1 := GetVertexOffset(TVertexFormat.TexCoord1, Buffer.FFormat);
  Buffer.FTexCoord2 := GetVertexOffset(TVertexFormat.TexCoord2, Buffer.FFormat);
  Buffer.FTexCoord3 := GetVertexOffset(TVertexFormat.TexCoord3, Buffer.FFormat);
  Buffer.FColor0 := GetVertexOffset(TVertexFormat.Color0, Buffer.FFormat);
  Buffer.FColor1 := GetVertexOffset(TVertexFormat.Color1, Buffer.FFormat);
  Buffer.FColor2 := GetVertexOffset(TVertexFormat.Color2, Buffer.FFormat);
  Buffer.FColor3 := GetVertexOffset(TVertexFormat.Color3, Buffer.FFormat);
  Buffer.FNormal := GetVertexOffset(TVertexFormat.Normal, Buffer.FFormat);
  Buffer.FBiNormal := GetVertexOffset(TVertexFormat.BiNormal, Buffer.FFormat);
  Buffer.FTangent := GetVertexOffset(TVertexFormat.Tangent, Buffer.FFormat);
  Buffer.FColorF0 := GetVertexOffset(TVertexFormat.ColorF0, Buffer.FFormat);
  Buffer.FColorF1 := GetVertexOffset(TVertexFormat.ColorF1, Buffer.FFormat);
  Buffer.FColorF2 := GetVertexOffset(TVertexFormat.ColorF2, Buffer.FFormat);
  Buffer.FColorF3 := GetVertexOffset(TVertexFormat.ColorF3, Buffer.FFormat);
end;
...
begin
  ...
  begin
    FreeMem(FBuffer);
    if FFormat <> TVertexBuffer(Source).FFormat then
    begin
...
begin
  ...
  begin
    if TPlatformServices.Current.SupportsPlatformService(IFMXContextService, ContextSrv) then
...
begin
  Result := ((APoint.X > Left) or SameValue(APoint.X, Left, TEpsilon.Vector)) and (APoint.X < Right) and
    ...
    ((APoint.Z > Near) or SameValue(APoint.Z, Near, TEpsilon.Vector)) and (APoint.Z < Near);
end;
