...
uses
  System.Types, System.Messaging, System.Classes, System.SysUtils, System.Generics.Collections, System.UITypes,
...
type
  ...
  public
    ...
    procedure onRequestPermissionsResult(ARequestCode: Integer; APermissions: TJavaObjectArray<JString>;
      AGrantResults: TJavaArray<Integer>); cdecl;
  end;
...
{ TPlatformAndroid }
  ...
  private
    ...
    FVirtualKeyboardService: TAndroidVirtualKeyboard;
    FWindowService: TWindowServiceAndroid;
    FRecognizerServiceMap: TRecognizerServiceMap;
    FSystemAppearanceService: TAndroidSystemAppearanceService;
    ...
    FTitle: string;
    FLastThemeKind: TSystemThemeKind;
    procedure CheckSystemThemeChanges;
  ...
  public
    ...
    property VirtualKeyboard: TAndroidVirtualKeyboard read FVirtualKeyboardService;
    property WindowService: TWindowServiceAndroid read FWindowService;
    property SystemAppearanceService: TAndroidSystemAppearanceService read FSystemAppearanceService;
  end;
...
begin
  ...
  if DeviceManager.GetDeviceClass <> TDeviceInfo.TDeviceClass.Watch then
    FVirtualKeyboardService := TAndroidVirtualKeyboard.Create;
  FGraphicServices := TAndroidGraphicsServices.Create;
  FWindowService := TWindowServiceAndroid.Create;
  FRecognizerServiceMap := TRecognizerServiceMap.Create; (FRecognizerServiceMap as IInterface)._AddRef;
  FSystemAppearanceService := TAndroidSystemAppearanceService.Create;
  ...
  Activity.addListener(FActivityListener);
  // Releasing the ownership of the current window's surface is required for the
  // native activity to be able to present native controls.
  Activity.getWindow.takeSurface(nil);
  ...
  RegisterIdleHandler;
  FLastThemeKind := SystemAppearanceService.GetSystemThemeKind;
end;

destructor TPlatformAndroid.Destroy;
begin
  ReleaseAndNil(FTextInputManager);
  ReleaseAndNil(FSystemAppearanceService);
  ReleaseAndNil(FRecognizerServiceMap);
  ReleaseAndNil(FWindowService);
  ReleaseAndNil(FGraphicServices);
  ReleaseAndNil(FVirtualKeyboardService);
  ReleaseAndNil(FDeviceServices);
  ReleaseAndNil(FMetricsServices);
  ReleaseAndNil(FScreenServices);
  ReleaseAndNil(FSaveStateService);
  ReleaseAndNil(FTimerService);
  ReleaseAndNil(FLoggerService);
  ...
  MainActivity.removeListener(FActivityListener);

  FreeAndNil(FActivityListener);
...
procedure TPlatformAndroid.CheckSystemThemeChanges;
var
  Message: TSystemAppearanceChangedMessage;
  NewThemeKind: TSystemThemeKind;
begin
  NewThemeKind := SystemAppearanceService.GetSystemThemeKind;
  if NewThemeKind <> FLastThemeKind then
  begin
    Message := TSystemAppearanceChangedMessage.Create(TSystemAppearance.Create, True);
    TMessageManager.DefaultManager.SendMessage(SystemAppearanceService, Message);
    FLastThemeKind := NewThemeKind;
  end;
end;
...
begin
  ...
  if not TPlatformServices.Current.SupportsPlatformService(IFMXSystemAppearanceService) then
    TPlatformServices.Current.AddPlatformService(IFMXSystemAppearanceService, FSystemAppearanceService);
...
begin
  ...
  begin
    ...
    TPlatformServices.Current.RemovePlatformService(IFMXSystemAppearanceService);
  end;
end;
