...
var
  ...
  /// </summary>
  OpenGlErrorReporting: TOpenGlErrorReporting = TOpenGlErrorReporting.Strict;
...
type
  ...
  protected class var
    ...
    FThreadDictionary: TDictionary<TThreadID, NSOpenGLContext>;
  ...
  public
    ...
    property FormContext: NSOpenGLContext read FFormContext;
  end;
...
uses
  ...
  System.Classes, Macapi.ObjectiveC, Macapi.Foundation,  Macapi.Helpers, FMX.Consts, FMX.Utils, FMX.Platform, FMX.Forms,
  FMX.Platform.Mac;
...
const
  SharedAttributes: array[0..1] of NSOpenGLPixelFormatAttribute = (NSOpenGLPFADoubleBuffer, 0);
begin
...
class procedure TCustomContextOpenGL.CreateThreadDictionary;
begin
  FThreadDictionary := TDictionary<TThreadID, NSOpenGLContext>.Create;
end;

class procedure TCustomContextOpenGL.DestroyThreadDictionary;
var
  LContext: NSOpenGLContext;
begin
...
begin
  ...
  begin
    ...
    begin
      Result := TNSOpenGLContext.Wrap(TNSOpenGLContext.Alloc.initWithFormat(FSharedPixelFormat, FSharedContext));
...
class function TCustomContextOpenGL.Valid: Boolean;
var
  LContext: NSOpenGLContext;
begin
  if (CurrentContext <> nil) and (TCustomContextOpenGL(CurrentContext).FormContext <> nil) then
    LContext := TCustomContextOpenGL(CurrentContext).FormContext
  else
    LContext := SharedContext;

  if LContext <> nil then
  begin
    if TNSOpenGLContext.OCClass.currentContext <> NSObjectToID(LContext) then
      LContext.makeCurrentContext;
    Result := True;
  end
  else
    Result := False;
...
begin
  ...
  FFormContext := TNSOpenGLContext.Wrap(TNSOpenGLContext.Alloc.initWithFormat(LPixelFormat, FSharedContext));
  CreateBuffer;
end;
  ...
  begin
    ...
    begin
      ...
      if Status <> GL_FRAMEBUFFER_COMPLETE_EXT then
        raise EContext3DException.CreateResFmt(@SCannotCreateRenderBuffers, [ClassName]);
...
begin
  ...
  begin
    ...
    if (GLHasAnyErrors()) then
      raise EContext3DException.CreateResFmt(@SCannotBeginRenderingScene, [ClassName]);
  end;
end;
...
begin
  ...
  begin
    ...
    if (GLHasAnyErrors()) then
      raise EContext3DException.CreateResFmt(@SErrorInContextMethod, ['DoSetContextState']);
  end;
end;
...
begin
  ...
  begin
    ...
    if (GLHasAnyErrors()) then
      raise EContext3DException.CreateResFmt(@SErrorInContextMethod, ['DoClear']);
  end;
end;
...
begin
  ...
  begin
    ...
    if (GLHasAnyErrors()) then
      raise EContext3DException.CreateResFmt(@SErrorInContextMethod, ['DoDrawPrimitive']);
  {$WARNINGS ON}
  end;
end;
...
begin
  ...
  begin
    ...
    if (GLHasAnyErrors()) then
      raise EContext3DException.CreateResFmt(@SErrorInContextMethod, ['DoSetStencilOp']);
  end;
end;
...
begin
  ...
  begin
    ...
    if (GLHasAnyErrors()) then
      raise EContext3DException.CreateResFmt(@SErrorInContextMethod, ['DoSetStencilFunc']);
  end;
end;
...
begin
  ...
  begin
    ...
    if (GLHasAnyErrors()) then
      raise EContext3DException.CreateResFmt(@SCannotCreateTexture, [ClassName]);
  end;
end;
...
begin
  ...
  begin
    ...
    glBindTexture(GL_TEXTURE_2D, 0);
    if (GLHasAnyErrors()) then
      raise EContext3DException.CreateResFmt(@SCannotUploadTexture, [ClassName]);
  end;
end;
...
var
  I, UniformCount, UniformSize: GLint;
  UniformType: GLenum;
...
begin
  ...
  begin
    ...
    begin
      ...
      raise EContext3DException.CreateResFmt(@SCannotActivateShaderProgram, [ClassName]);
    end;
