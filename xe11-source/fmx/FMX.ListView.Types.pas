...
type
  TDrawableElement = (FontSize, FontFamily, FontStyle, TextColor, SelectedTextColor, TextShadowColor, PressedTextColor);
  TDrawableElements = set of TDrawableElement;

  TDefaultSettingsHelper = class
  private
    class constructor Create;
    class destructor Destroy;
    class var FItems: TDictionary<TListItemDrawable, TDrawableElements>;
  public
    class procedure Register(const ADrawable: TListItemDrawable);
    class procedure Unregister(const ADrawable: TListItemDrawable);

    class function IsDefault(const ADrawable: TListItemDrawable; const AElement: TDrawableElement): Boolean;
    class procedure SetDefault(const ADrawable: TListItemDrawable; const AElement: TDrawableElement; const ADefault: Boolean);
  end;
...
begin
  ...
  TDefaultSettingsHelper.Register(Self);
end;

destructor TListItemDrawable.Destroy;
begin
  TDefaultSettingsHelper.Unregister(Self);
  FController := nil;
  FCallback := nil;
  FPlaceOffsetX.Free;
  inherited;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (TStyleResource.TextColor in FStyleValuesNeedUpdate) and (Resources.HeaderTextColor > 0) and
         TDefaultSettingsHelper.IsDefault(Self, TDrawableElement.TextColor) then
        FTextColor := Resources.HeaderTextColor;
      if (TStyleResource.TextShadowColor in FStyleValuesNeedUpdate) and (Resources.HeaderTextShadowColor > 0) and
         TDefaultSettingsHelper.IsDefault(Self, TDrawableElement.TextShadowColor) then
    ...
    begin
      ...
      if (TStyleResource.TextColor in FStyleValuesNeedUpdate) and (Resources.DetailTextColor > 0) and
         TDefaultSettingsHelper.IsDefault(Self, TDrawableElement.TextColor) then
        FTextColor := Resources.DetailTextColor;
      if (TStyleResource.SelectedTextColor in FStyleValuesNeedUpdate) and (Resources.DefaultTextSelectedColor > 0) and
         TDefaultSettingsHelper.IsDefault(Self, TDrawableElement.SelectedTextColor) then
        FSelectedTextColor := Resources.DefaultTextSelectedColor;
    end
    else
    begin
      ResourceFont := Resources.DefaultTextFont;
      if (TStyleResource.TextColor in FStyleValuesNeedUpdate) and (Resources.DefaultTextColor > 0) and
         TDefaultSettingsHelper.IsDefault(Self, TDrawableElement.TextColor) then
  ...
  begin
    ...
    begin
      ...
      try
        if (TStyleResource.FontFamily in FStyleValuesNeedUpdate) and TDefaultSettingsHelper.IsDefault(Self, TDrawableElement.FontFamily) then
          FFontX.Family := ResourceFont.Family;
        if (TStyleResource.FontSize in FStyleValuesNeedUpdate) and TDefaultSettingsHelper.IsDefault(Self, TDrawableElement.FontSize) then
          FFontX.Size := ResourceFont.Size;
        if (TStyleResource.FontStyle in FStyleValuesNeedUpdate) and TDefaultSettingsHelper.IsDefault(Self, TDrawableElement.FontStyle) then
...
begin
  ...
  begin
    TDefaultSettingsHelper.SetDefault(Self, TDrawableElement.FontFamily, False);
  ...
  begin
    TDefaultSettingsHelper.SetDefault(Self, TDrawableElement.FontSize, False);
    Exclude(FStyleValuesNeedUpdate, TStyleResource.FontSize);
  end;
  if NewSettings.Style <> FFontSettings.Style then
  begin
    TDefaultSettingsHelper.SetDefault(Self, TDrawableElement.FontStyle, False);
...
begin
  ...
  begin
    ...
    TDefaultSettingsHelper.SetDefault(Self, TDrawableElement.TextShadowColor, Value = TAlphaColorRec.Null);
...
begin
  ...
  begin
    FTextColor := Value;
    TDefaultSettingsHelper.SetDefault(Self, TDrawableElement.TextColor, Value = TAlphaColorRec.Null);
...
begin
  ...
  begin
    ...
    TDefaultSettingsHelper.SetDefault(Self, TDrawableElement.SelectedTextColor, Value = TAlphaColorRec.Null);
...
begin
  ...
  else if Value.TryAsType<string>(LSIndex) then
  begin
    Val(LSIndex, LIndex, LCode);
    if LCode = 0 then
      SetImageIndex(LIndex);
  end
    ...
    case FButtonType of
      TTextButtonType.Normal:
        begin
          ...
          begin
            if TDefaultSettingsHelper.IsDefault(Self, TDrawableElement.TextColor) then
              FTextColor := Resources.ButtonTextColor;
            if TDefaultSettingsHelper.IsDefault(Self, TDrawableElement.PressedTextColor) then
...
begin
  ...
  begin
    ...
    TDefaultSettingsHelper.SetDefault(Self, TDrawableElement.PressedTextColor, Value = TAlphaColorRec.Null);
...
function TListItemControlScene.LocalToScreen(const P: TPointF): TPointF;
begin
  Result := RealScene.LocalToScreen(P);
end;

function TListItemControlScene.ScreenToLocal(const P: TPointF): TPointF;
begin
...
procedure TListItemControlScene.AddUpdateRect(const R: TRectF);
begin
  if FDisableUpdating = 0 then
...
{ TDrawableDefaultSettings }

class constructor TDefaultSettingsHelper.Create;
begin
  FItems := TDictionary<TListItemDrawable, TDrawableElements>.Create;
end;

class function TDefaultSettingsHelper.IsDefault(const ADrawable: TListItemDrawable; const AElement: TDrawableElement): Boolean;
begin
  Result := AElement in FItems[ADrawable];
end;

class procedure TDefaultSettingsHelper.SetDefault(const ADrawable: TListItemDrawable; const AElement: TDrawableElement;
  const ADefault: Boolean);
var
  Elements: TDrawableElements;
begin
  Elements := FItems[ADrawable];
  if ADefault then
    Include(Elements, AElement)
  else
    Exclude(Elements, AElement);
  FItems[ADrawable] := Elements;
end;

class procedure TDefaultSettingsHelper.Register(const ADrawable: TListItemDrawable);
begin
  FItems.Add(ADrawable, [Low(TDrawableElement)..High(TDrawableElement)])
end;

class procedure TDefaultSettingsHelper.Unregister(const ADrawable: TListItemDrawable);
begin
  FItems.Remove(ADrawable);
end;

class destructor TDefaultSettingsHelper.Destroy;
begin
  FreeAndNil(FItems);
end;
