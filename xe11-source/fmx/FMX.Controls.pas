...
begin
  if not TPlatformServices.Current.SupportsPlatformService(IFMXVirtualKeyboardService, VKbSvc) then
    Exit;

  if Displayed and (Caret <> nil) and not Caret.ReadOnly then
  begin
    ...
    begin
      if VKbSvc.ShowVirtualKeyboard(Caret.Owner) then
        Include(VirtualKeyboardState, TVirtualKeyboardState.Visible)
      else
        Exclude(VirtualKeyboardState, TVirtualKeyboardState.Visible);
    end;
  end
  else
  begin
...
procedure TControl.CreateTouchManagerIfRequired;
begin
...
begin
  ...
  try
    ...
    begin
      ...
      ChildControl.SetNewScene(FScene);
...
begin
  ...
  begin
    ...
    else if FUpdating = 1 then // 1 - because we increased this value in this method.
      Disappear;
  end;
  Dec(FUpdating);
end;
...
function TControl.AbsoluteToLocal(const ARect: TRectF): TRectF;
begin
  Result.TopLeft := AbsoluteToLocal(ARect.TopLeft);
  Result.BottomRight := AbsoluteToLocal(ARect.BottomRight);
end;
...
begin
  ...
  else if FSimpleTransform then
    Result := Vector
  else
    Result := Vector * InvertAbsoluteMatrix;
end;

function TControl.LocalToAbsolute(const ARect: TRectF): TRectF;
begin
...
function TControl.AbsoluteToLocal(const APoint: TPointF): TPointF;
begin
  if FInPaintTo then
    Result := APoint * FInPaintToInvMatrix
  else if FSimpleTransform then
  begin
    Result.X := APoint.X + InvertAbsoluteMatrix.m31;
    Result.Y := APoint.Y + InvertAbsoluteMatrix.m32;
  end
  else
    Result := APoint * InvertAbsoluteMatrix;
end;

function TControl.LocalToAbsolute(const APoint: TPointF): TPointF;
begin
  if FInPaintTo then
    Result := APoint * FInPaintToAbsMatrix
  else if FSimpleTransform then
  begin
    Result.X := APoint.X + AbsoluteMatrix.m31;
    Result.Y := APoint.Y + AbsoluteMatrix.m32;
  end
  else
    Result := APoint * AbsoluteMatrix;
end;
...
var
  P: TPointF;
begin
  P := AbsoluteToLocal(TPointF.Create(X, Y));
...
function TControl.ScreenToLocal(const AScreenPoint: TPointF): TPointF;
begin
  if Scene = nil then
    Result := AbsoluteToLocal(AScreenPoint)
  else
    Result := AbsoluteToLocal(Scene.ScreenToLocal(AScreenPoint));
end;

function TControl.LocalToScreen(const ALocalPoint: TPointF): TPointF;
begin
  if Scene = nil then
    Result := LocalToAbsolute(ALocalPoint)
  else
    Result := Scene.LocalToScreen(LocalToAbsolute(ALocalPoint));
end;
...
function TControl.ObjectAtPoint(AScreenPoint: TPointF): IControl;
var
  I: Integer;
  NewObj: IControl;
  Control: TControl;
...
begin
  LControls := Controls;
  if LControls = nil then
    Exit;
  for I := 0 to LControls.Count - 1 do
    LControls[I].AncestorParentChanged;
end;
...
function TControl.CanShowHint: Boolean;
begin
  if FParentShowHint then
  begin
    if ParentControl <> nil then
      Result := ParentControl.CanShowHint
    else if Parent is TCommonCustomForm then
...
begin
  if AScene <> nil then
  begin
    // We have to reset control state, when it's change scene. Otherwise, the component will have an invalid state
    // And may be mistakenly highlighted or stuck in the pressed state
    if FPressed then
    begin
      FPressed := False;
  ...
  Controller := GetTabStopController;
  if (Controller <> nil) and (FTabOrder <> -1) then
    Controller.TabList.Update(Self, FTabOrder);
end;
...
function TControl.ConvertLocalPointTo(const AControl: TControl; const ALocalPoint: TPointF): TPointF;
begin
  Assert(AControl <> nil);
...
begin
  ...
  begin
    ...
    if Visible and (ParentContent <> nil) then
      ParentContent.Changed
    else if Supports(Parent, IAlignRoot, AlignRoot) then
    begin
  ...
  begin
    ...
    try
      ...
      begin
        ...
        FAdjustSizeValue := TSizeF.Create(StyleControl.Width, StyleControl.Height);
...
var
  SavedDisableAlign: Boolean;
  Link: TFmxObject;
begin
  Link := FResourceLink;
  if (Link <> nil) and (Link is TControl) and TStyleCache.Initialized then
  begin
    SavedDisableAlign := FDisableAlign;
    FDisableAlign := True;
    try
      InternalFreeStyle;
      TStyleCache.Current.FreeResource(TControl(Link));
    finally
      FDisableAlign := SavedDisableAlign;
    end;
  end
  else
    KillResourceLink;
...
begin
  ...
  begin
    ...
    {$ELSE}
      // RSP-27656. It's very important to remove link immediately in IDE mode. Because style can contain custom
      // user's components, which could be reinstalled in IDE. Therefore, the asynchronous version may try to remove
      // a component that does not already have a valid class type info.
      if csDesigning in ComponentState then
        Link.Free
      else
...
begin
  if FStyleState in [TStyleState.Unapplied] then
    Exit;
...
procedure TStyledControl.FreeStyle;
  ...
  procedure SynchronizeStyleData;
  var
    Entry: TPair<string, TValue>;
    ValueFromStyle: TValue;
    ValueFromData: TValue;
  begin
    for Entry in FStylesData do
    begin
      ValueFromStyle := StylesData[Entry.Key];
      ValueFromData := FStylesData[Entry.Key];
      if ValueFromStyle.IsInstanceOf(TPersistent) and ValueFromData.IsInstanceOf(TPersistent) then
        ValueFromData.AsType<TPersistent>.Assign(ValueFromData.AsType<TPersistent>)
      else
        FStylesData.AddOrSetValue(Entry.Key, ValueFromStyle);
    end;
  end;
...
begin
  ...
  FHideWhenPlacementTargetInvisible := True;
...
begin
  ...
  begin
    if Parent = FPopupForm then
      Parent := nil;
    // TPopup is being embedded into FPopupForm, So if PopupForm is destroying, it initiates destroing TPopup.
    if not (csDestroying in FPopupForm.ComponentState) then
...
begin
  ...
  try
    ...
    try
      ...
      TCustomPopupForm(FPopupForm).HideWhenPlacementTargetInvisible := HideWhenPlacementTargetInvisible;
...
procedure TPopup.SetHideWhenPlacementTargetInvisible(const Value: Boolean);
begin
  if FHideWhenPlacementTargetInvisible <> Value then
  begin
    FHideWhenPlacementTargetInvisible := Value;
    if HasPopupForm then
      TCustomPopupForm(FPopupForm).HideWhenPlacementTargetInvisible := FHideWhenPlacementTargetInvisible;
  end;
end;
