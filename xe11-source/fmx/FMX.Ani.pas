...
uses
  System.Types, System.Math, System.SysUtils, System.StrUtils, System.SyncObjs, System.TypInfo,
...
class procedure TAnimator.StartTriggerAnimation(const Target: TFmxObject; const AInstance: TFmxObject; const APropertyName: string);
var
...
class procedure TAnimator.DefaultStartTriggerAnimation(const Target: TFmxObject; const AInstance: TFmxObject; const APropertyName: string);
var
  I: Integer;
  Control: IControl;
begin
  if (Target <> nil) and (Target.Children <> nil) then
    ...
    begin
      if Target.Children[I] is TAnimation then
        TAnimation(Target.Children[I]).StartTrigger(AInstance, APropertyName);
...
class procedure TAnimator.StartTriggerAnimationWait(const Target: TFmxObject; const AInstance: TFmxObject; const APropertyName: string);
var
  Animatable: ITriggerAnimation;
begin
  StopTriggerAnimation(Target, AInstance, APropertyName);
...
class procedure TAnimator.DefaultStartTriggerAnimationWait(const Target, AInstance: TFmxObject; const APropertyName: string);
var
  I: Integer;
  Control: IControl;
begin
...
class procedure TAnimator.StopTriggerAnimation(const Target: TFmxObject; const AInstance: TFmxObject; const APropertyName: string);
var
  Item: TFmxObject;
  Control: IControl;
begin
  if Target.Children <> nil then
    ...
    begin
      if TFmxObject(Item) is TAnimation then
        TAnimation(Item).StopTrigger(AInstance, APropertyName);
...
function TAnimationTrigger.CanExecute(const ATarget: TObject): Boolean;
var
  I: Integer;
...
procedure TAnimationTrigger.ClearRttiInfo;
begin
  FreeAndNil(FRttiInfo);
  FTargetClass := nil;
end;

procedure TAnimationTrigger.CollectRttiInfo(const ATarget: TObject);
var
  T: TRttiType;
...
destructor TAnimationTrigger.Destroy;
begin
  FreeAndNil(FRttiInfo);
  FreeAndNil(FNames);
  inherited;
...
function TAnimationTrigger.HasProperty(const APropertyName: string): Boolean;
begin
  Result := (FNames <> nil) and FNames.ContainsKey(APropertyName.ToLower);
end;

procedure TAnimationTrigger.ParseTriggerNames(const ATrigger: TTrigger);
...
procedure TAnimationTrigger.SetTrigger(const ATrigger: TTrigger);
begin
...
begin
  ...
  for TriggerType := Low(TTriggerType) to High(TTriggerType) do
    FTriggers[TriggerType] := TAnimationTrigger.Create;
end;
...
procedure TAnimation.SetTrigger(const Index: TTriggerType; const Value: TTrigger);
begin
...
function TAnimation.GetTrigger(const Index: TTriggerType): TTrigger;
begin
...
procedure TAnimation.ProcessTick(const ATime, ADeltaTime: Single);
var
  Control: IControl;
begin
  inherited;
...
procedure TAnimation.StartTrigger(const AInstance: TFmxObject; const APropertyName: string);
var
  LTrigger: TAnimationTrigger;
begin
...
procedure TAnimation.StopTrigger(const AInstance: TFmxObject; const APropertyName: string);
begin
  if AInstance = nil then
    Exit;
