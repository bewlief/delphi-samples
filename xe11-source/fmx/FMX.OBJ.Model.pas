...
function GetFormatColor(const AR, AG, AB: string): TAlphaColorF;
var
...
begin
  ...
  if (AR + AG + AB).Contains(',') then
    LFormatSettings.DecimalSeparator := ',';

  Result := TAlphaColorF.Create(StrToFloat(AR, LFormatSettings), StrToFloat(AG, LFormatSettings),
    StrToFloat(AB, LFormatSettings));
end;
