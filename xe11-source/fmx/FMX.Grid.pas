...
type
  ...
  private
    ...
    FHorzAlignModified: Boolean;
    FHorzAlign: TTextAlign;
    procedure SaveOriginalIndex;
    ...
    procedure SetGetHorzAlign(const Value: TTextAlign);
  protected
    { ICaption }
    ...
    function GetDefaultHorzAlign: TTextAlign; virtual;
  ...
  public
    ...
    /// <summary> This property returns the column index as it was before first movement of any column in grid. </summary>
    property OriginalIndex: Integer read GetOriginalIndex;
    /// <summary> This property returns True when HorzAlign was explicitly set. </summary>
    property HorzAlignModified: Boolean read FHorzAlignModified;
  published
    property Action;
    /// <summary> This property specifies the horizontal alignment of text in the column </summary>
  ...
  TDateTimeColumnBase = class(TColumn)
  private type
    TType = (DateTime, Date, Time);
  private
  ...
  protected
    ...
    /// <see cref="FMX.Grid|TDateTimeColumnBase.Format">Format</see> property in fmx-file </summary>
  ...
  TDateTimeColumn = class(TDateTimeColumnBase)
  end;
  ...
  TDateColumn = class(TDateTimeColumnBase)
  public
    constructor Create(AOwner: TComponent); override;
  end;
  ...
  protected
    function GetDefaultHorzAlign: TTextAlign; override;
  public
    constructor Create(AOwner: TComponent); override;
  ...
  public
    ...
    procedure SetExpectedRowCount(const Value: Integer);
  ...
  public
    ...
    /// <summary> Returns available unused space of viewport on the right from columns. </summary>
    /// <remarks> If content is bigger than <c>ViewportSize</c>, then returns 0.</remarks>
    function GetUnusedViewportWidth: Single;
    ...
    function TextWidthToColWidth(const ATextLength: Integer; const ACaption: string): Single;
...
begin
  if TStyledSetting.Other in Model.StyledSettings then
    ...
    Result := Source.PlaceInto(Bounds, HorzAlign.AsHorzRectAlign,
...
procedure TColumn.SaveOriginalIndex;
begin
  if FOriginalIndex = -1 then
    FOriginalIndex := Index;
end;

function TColumn.GetOriginalIndex: Integer;
begin
  if FOriginalIndex = -1 then
    Result := Index
  else
    Result := FOriginalIndex;
end;
...
begin
  ...
  try
    ...
    FLayout.HorizontalAlign := HorzAlign;
...
begin
  Result := Value.IsEmpty or
    ((Value.Kind in [tkString, tkChar, tkWChar, tkLString, tkWString, tkUString]) and Value.AsString.IsEmpty) or
    ((Value.Kind = tkFloat) and Value.TryAsType<Double>(D) and IsNan(D)) or
...
begin
  if Value.IsEmpty then
    Result := string.Empty
  else if Value.Kind in [tkString, tkWString, tkLString, tkUString] then
...
function TColumn.IsHorzAlignStored: Boolean;
begin
  Result := FHorzAlignModified;
end;

function TColumn.GetDefaultHorzAlign: TTextAlign;
begin
  if (Model <> nil) and (DefaultTextSettings <> nil) then
    Result := DefaultTextSettings.HorzAlign
  else
    Result := TTextAlign.Leading;
end;

function TColumn.GetHorzAlign: TTextAlign;
begin
  if FHorzAlignModified then
    Result := FHorzAlign
  else
    Result := GetDefaultHorzAlign;
end;

procedure TColumn.SetGetHorzAlign(const Value: TTextAlign);
begin
  if Value <> HorzAlign then
  begin
    FHorzAlignModified := True;
    FHorzAlign := Value;
  end;
end;
...
begin
  ...
  TestString := string.LowerCase(Format(StrTrigger, [BoolToStr(Value, True)]));
...
constructor TDateTimeColumnBase.Create(AOwner: TComponent);
begin
  inherited;
  FShowClearButton := False;
  FType := TType.DateTime;
end;

procedure TDateTimeColumnBase.SetShowClearButton(const Value: Boolean);
begin
...
function TDateTimeColumnBase.ValueToString(const Value: TValue): string;
  ...
  begin
    if Format.IsEmpty then
      case FType of
        TDateTimeColumnBase.TType.DateTime:
          Result := DateTimeToStr(Date);
        TDateTimeColumnBase.TType.Date:
          Result := DateToStr(Date);
        TDateTimeColumnBase.TType.Time:
          begin
...
begin
  ...
  begin
    if Value.Kind in [tkString, tkWString, tkLString, tkUString] then
    begin
      S := Value.AsString;
      LValid := False;
      case FType of
        TDateTimeColumnBase.TType.DateTime:
          LValid := TryStrToDateTime(S, LDate);
        TDateTimeColumnBase.TType.Date:
          LValid := TryStrToDate(S, LDate) or TryStrToDateTime(S, LDate);
        TDateTimeColumnBase.TType.Time:
          LValid := TryStrToTime(S, LDate) or TryStrToDateTime(S, LDate);
      end;
      if LValid then
...
function TDateTimeColumnBase.IsFormatStored: Boolean;
begin
  Result := not Format.Trim.IsEmpty;
end;

procedure TDateTimeColumnBase.SetFormat(const Value: string);
begin
...
constructor TTimeColumn.Create(AOwner: TComponent);
begin
  inherited;
  FType := TType.Time;
end;
...
function TCustomNumberColumn.GetDefaultHorzAlign: TTextAlign;
begin
...
begin
  ...
  begin
    LHaveValue := False;
    FOnCallbackGetValue(Self.Owner, Columns[Col].OriginalIndex, Row,
...
begin
  ...
  try
    ...
    UpdateColumnsIndex;
    FCol := EnsureRange(FCol, -1, FColumns.Count - 1);
...
begin
  ...
  begin
    ...
    try
      OldValue := FRowCount;
      FRowCount := Value;
      FRow := EnsureRange(FRow, -1, FRowCount - 1);
...
procedure TGridModel.SetExpectedRowCount(const Value: Integer);
var
  LCapacity: Integer;
begin
  LCapacity := (Value * FColumns.Count * 4) div 3;
  if LCapacity + 1 > Cache.Count then
    Cache.Capacity := LCapacity;
end;
...
function TCustomGrid.GetUnusedViewportWidth: Single;
begin
  Result := Max(0, ViewportSize.Width - ContentSize.Width);
end;
...
begin
  ...
  Result.VInteger := Columns[Col].OriginalIndex;
end;
...
function TCustomGrid.TextWidthToColWidth(const ATextLength: Integer; const ACaption: string): Single;
var
  W: Single;
begin
  if Canvas <> nil then
  begin
    Result := Canvas.TextWidth(StringOfChar('0', ATextLength)) + TColumn.HorzTextMargin * 2;
    if ACaption <> '' then
    begin
      W := Canvas.TextWidth(ACaption) + TColumn.HorzTextMargin * 2;
      if Result < W then
        Result := W;
    end;
  end
  ...
  begin
    Result := 7 * ATextLength + TColumn.HorzTextMargin * 2;
    if ACaption <> '' then
    begin
      W := Length(ACaption) + TColumn.HorzTextMargin * 2;
      if Result < W then
        Result := W;
    end;
  end;
...
initialization
  ...
  TColumnClasses.Register([TColumn, TStringColumn, TProgressColumn, TCheckColumn,
    TDateTimeColumn, TDateColumn, TTimeColumn, TPopupColumn, TImageColumn,
    TCurrencyColumn, TFloatColumn, TIntegerColumn, TGlyphColumn]);
