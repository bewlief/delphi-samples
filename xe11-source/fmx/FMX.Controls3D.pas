...
{ TControl3D }
  ...
  private
    ...
    function GetAbsoluteOpacity: Single;
  ...
  protected
    ...
    function CalculateAbsoluteOpacity: Single; virtual;
    { events }
    procedure Capture;
...
begin
  ...
  begin
    FAbsoluteMatrix := CalculateAbsoluteMatrix;
...
function TControl3D.ScreenToLocal(const AScreenPoint: TPointF): TPointF;
begin
  if FViewport = nil then
    Result := AScreenPoint
  else
    Result := FViewport.ScreenToLocal(AScreenPoint);
end;

function TControl3D.LocalToScreen(const ALocalPoint: TPointF): TPointF;
begin
  if FViewport = nil then
   Result := ALocalPoint
  else
    Result := FViewport.LocalToScreen(ALocalPoint);
end;
  ...
  if Children <> nil then
    for i := 0 to Children.Count - 1 do
      if Children[i] is TControl3D then
        TControl3D(Children[i]).RecalcAbsolute;
end;
...
begin
  ...
  begin
    FAbsoluteOpacity := CalculateAbsoluteOpacity;
...
function TControl3D.CalculateAbsoluteMatrix: TMatrix3D;
begin
  if FParent is TControl3D then
    Result := FLocalMatrix * TControl3D(FParent).AbsoluteMatrix
  else
    Result := FLocalMatrix;
end;

function TControl3D.CalculateAbsoluteOpacity: Single;
begin
  if FParent is TControl3D then
    Result := FOpacity * TControl3D(FParent).AbsoluteOpacity
  else
    Result := FOpacity;
end;
