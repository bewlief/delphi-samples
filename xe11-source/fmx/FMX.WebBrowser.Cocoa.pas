...
{ TCommonWebBrowserService }
  ...
  private
    ...
    FNavigationDelegate: WebNavigationDelegate;
...
begin
  ...
  try
    if TNativeWebViewHelper.NativeViewToSurface(FWebView, Surface) then
    begin
...
procedure TCommonWebBrowserService.LoadFromStrings(const AContent: string; const ABaseUrl: string);
begin
  LoadFromStrings(AContent, nil, ABaseUrl);
end;

procedure TCommonWebBrowserService.LoadFromStrings(const AContent: string; const AContentEncoding: TEncoding; const ABaseUrl: string);
var
  LContent: NSString;
  LURL: NSURL;
  LBase: NSString;
begin
  LContent := StrToNSStr(AContent);
  LBase := StrToNSStr(ABaseUrl);
...
{$IFDEF IOS}
  ...
  begin
    ...
    for I := 0 to Integer(AView.subviews.count) - 1 do
