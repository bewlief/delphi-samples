...
procedure TCustomBannerAd.AncestorVisibleChanged(const Visible: Boolean);
begin
  inherited;
  UpdateContent;
end;
...
procedure TCustomBannerAd.DoAbsoluteChanged;
begin
  inherited;
  UpdateContent;
end;
...
procedure TCustomBannerAd.ParentChanged;
begin
  inherited;
...
function TCustomBannerAd.QueryInterface(const IID: TGUID; out Obj): HResult;
begin
  Result := inherited QueryInterface(IID, Obj);
  if (FAd <> nil) and (Result <> S_OK) then
    Result := FAd.QueryInterface(IID, Obj);
end;
