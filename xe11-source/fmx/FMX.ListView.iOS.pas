...
begin
  if ZOrderManager <> nil then
    ZOrderManager.RemoveLink(TControl(FParent));
...
begin
  if (ZOrderManager <> nil) and (FMainView <> nil) then
  begin
    ZOrderManager.AddOrSetLink(TControl(FParent), FMainView, nil);
    ZOrderManager.UpdateOrderAndBounds(TControl(FParent));
    FMainView.setAlpha(FParent.GetControlOpacity);
  end;
end;

function TListViewPresentation.GetZOrderManager: TiOSZOrderManager;
var
  Root: TObject;
begin
  Root := FParent.GetRootObject;
  if (Root <> nil) and (TCommonCustomForm(Root).Handle <> nil) then
    Result := WindowHandleToPlatform(TCommonCustomForm(Root).Handle).ZOrderManager
  else
    Result := nil;
end;
