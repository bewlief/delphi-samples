...
begin
  ...
  begin
    ...
    try
      ...
      begin
        CurrentChar := PathString.Chars[Pos];
        if CurrentChar.ToUpper = ExponentChar then
        begin
          StringBuilder.Append(ExponentChar);
          Inc(Pos);
          Continue;
        end;
        if (CurrentChar = '-') and (StringBuilder.Length > 0) and
          (StringBuilder.Chars[StringBuilder.Length - 1] = ExponentChar) then
        begin
          StringBuilder.Append(CurrentChar);
          Inc(Pos);
          Continue;
        end;
        if (StringBuilder.Length > 0) and (CurrentChar = '-') then
          Break;
        if not '0123456789.'.Contains(CurrentChar) and not (CurrentChar = '-') then
          Break;
        StringBuilder.Append(CurrentChar);
        Inc(Pos);
      end;
...
begin
  ...
  if (LastToken <> 'Z') and (FPathData.Count > 0) then
    ClosePath;
  DoChanged;
end;
