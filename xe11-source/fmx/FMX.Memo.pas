...
uses
  System.Classes, System.UITypes, System.Types, System.Rtti, FMX.Memo.Types, FMX.Graphics, FMX.Controls.Model,
  FMX.Controls.Presentation, FMX.ScrollBox, FMX.Controls, FMX.Types, FMX.Text;
...
uses
  System.SysUtils, System.Math, System.Character, {$IFDEF MACOS}Macapi.CoreFoundation, {$ENDIF}
  FMX.Platform, FMX.Consts, FMX.Clipboard
  {$IF Defined(MSWINDOWS) OR (Defined(MACOS) AND NOT Defined(IOS))}
  , FMX.Memo.Style.New // Only for Windows and macOS at this moment
  {$ELSE}
  , FMX.Memo.Style
  {$ENDIF}
  {$IFDEF MSWINDOWS}, FMX.Memo.Win{$ENDIF}
  {$IFDEF IOS}, FMX.Memo.iOS{$ENDIF}
  {$IFDEF ANDROID}, FMX.Memo.Android{$ENDIF};
...
procedure TCustomMemoModel.DeleteFrom(const APosition: TCaretPosition; const ALength: Integer; const Options: TDeleteOptions);
var
  LText: string;
  TmpPos, TmpLength, TmpLine, RemainLength: Integer;
  TmpS: string;
begin
  if (Lines.Count <= 0) or (ALength <= 0) then
    Exit;

  if not InRange(APosition.Line, 0, Lines.Count - 1) then
    raise EArgumentOutOfRangeException.CreateRes(@SCaretLineIncorrect);
...
function TCustomMemoModel.GetCaretPositionByPoint(const AHitPoint: TPointF;
  const ARoundToWord: Boolean): TCaretPosition;
var
  Message: TGetCaretPositionInfo;
begin
  Message.HitPoint := AHitPoint;
  Message.RoundToWord := ARoundToWord;
  Message.CaretPosition := TCaretPosition.Invalid;
  SendMessageWithResult<TGetCaretPositionInfo>(MM_MEMO_GET_CARET_POSITION_BY_POINT, Message);
  Result := Message.CaretPosition;
end;

function TCustomMemoModel.GetCount: Integer;
begin
  Result := FLines.Count;
end;

function TCustomMemoModel.GetLine(const ALineIndex: Integer): string;
begin
  Result := FLines[ALineIndex];
end;

function TCustomMemoModel.GetLineBreak: string;
begin
  Result := FLines.LineBreak;
end;

function TCustomMemoModel.GetNextWordBegin(const APosition: TCaretPosition): TCaretPosition;
var
  CurrentLine: string;
begin
...
function TCustomMemoModel.GetPrevWordBegin(const APosition: TCaretPosition): TCaretPosition;
var
  CurrentLine: string;
begin
  if FLines.Count = 0 then
  ...
  else if (APosition.Line - 1 >= 0) and (APosition.Line - 1 <= Lines.Count - 1) then
  begin
    Result.Line := APosition.Line - 1;
...
function TCustomMemoModel.GetPositionShift(const APosition: TCaretPosition; const ADelta: Integer): TCaretPosition;
begin
  Result := APosition;
  ...
  if Result.Pos < 0 then
    ...
    begin
      Inc(Result.Pos, Lines[Result.Line].Length + 1);
      Dec(Result.Line);
    ...
    while Result.Pos > Lines[Result.Line].Length do
    begin
      Result.IncrementLine;
      if Result.Line >= Lines.Count then
        Result := TCaretPosition.Create(Lines.Count - 1, Lines[Lines.Count - 1].Length)
      else
        Dec(Result.Pos, Lines[Result.Line - 1].Length + 1);
    end;
end;
...
procedure TCustomMemoModel.InsertAfter(const APosition: TCaretPosition; const AFragment: string;
  ...
  begin
    ...
    if MaxLength > 0 then
      AFragment := AFragment.Substring(0, MaxLength - LText.Length);
  end;

  procedure InsertFragment(const AFragment: TStringList; const APosition: TCaretPosition);
  var
    StartPos: Integer;
    I: Integer;
    FirstLine: string;
    Tail: string;
  begin
    StartPos := IfThen(Lines[APosition.Line].Length > 0, APosition.Pos, -1);

    FirstLine := Lines[APosition.Line];
    if AFragment.Count = 1 then
      Lines[APosition.Line] := FirstLine.Insert(StartPos, AFragment[0])
    else
    begin
      Tail := FirstLine.Substring(StartPos);
      Lines[APosition.Line] := FirstLine.Remove(StartPos) + AFragment[0];
      for I := 1 to AFragment.Count - 1 do
        if I = AFragment.Count - 1 then
          Lines.Insert(APosition.Line + I, AFragment[I] + Tail)
        else
          Lines.Insert(APosition.Line + I, AFragment[I]);
    end;
  end;

  procedure InsertLineBreak(const APosition: TCaretPosition);
  var
    TextAfter: string;
    Line: string;
    TextBefore: string;
  begin
    Line := Lines[APosition.Line];
    TextBefore := Line.Substring(0, APosition.Pos);
    TextAfter := Line.Substring(APosition.Pos);
    Lines[APosition.Line] := TextBefore;
    Lines.Insert(APosition.Line + 1, TextAfter);
  end;
...
begin
  ...
  TextPosition := PosToTextPos(APosition);
  ...
  try
    // TStringList doesn't present last line, if it includes only one line break characters.
    // So we have to add additional LineBreak for correct line counting.
    if Insertion.EndsWith(sLineBreak) then
      Fragment.Text := Insertion + sLineBreak
    else
      Fragment.Text := Insertion;
    Lines.BeginUpdate;
    try
      ...
      begin
        ...
        begin
          ...
          Lines.AddStrings(Fragment);
      end
      else if Insertion = Lines.LineBreak then
      begin
        ...
        InsertFragment(Fragment, LPosition);
      ...
      begin
        if Insertion = Lines.LineBreak then
          CaretPosition := TextPosToPos(TextPosition + Lines.LineBreak.Length)
        else
          CaretPosition := TextPosToPos(TextPosition + Insertion.Length);
      end;
    finally
      Lines.EndUpdate;
    end;
...
procedure TCustomMemoModel.MoveCaretHorizontal(const ADelta: Integer);
begin
  CaretPosition := GetPositionShift(CaretPosition, ADelta);
end;

procedure TCustomMemoModel.MoveCaretLeft;
var
  Offset: Integer;
begin
  if FLines.Count = 0 then
  ...
  if (Lines[CaretPosition.Line].Length > 1) and (CaretPosition.Pos > 0) and Lines[CaretPosition.Line].Chars[CaretPosition.Pos - 1].IsLowSurrogate then
    Offset := -2
  else
    Offset := -1;
...
procedure TCustomMemoModel.MoveCaretRight;
var
  Offset: Integer;
begin
  if FLines.Count = 0 then
  ...
  if (Lines[CaretPosition.Line].Length > CaretPosition.Pos) and Lines[CaretPosition.Line].Chars[CaretPosition.Pos].IsHighSurrogate then
    Offset := 2
  else
    Offset := 1;
...
procedure TCustomMemoModel.Replace(const APosition: TCaretPosition; const ALength: Integer; const AFragment: string);
begin
  if not InRange(APosition.Line, 0, Lines.Count - 1) then
    raise EArgumentOutOfRangeException.CreateRes(@SCaretLineIncorrect);
  if not InRange(APosition.Pos, 0, Lines[APosition.Line].Length - 1) then
    raise EArgumentOutOfRangeException.CreateRes(@SCaretPosIncorrect);

  Lines[APosition.Line] := Lines[APosition.Line].Remove(APosition.Pos, ALength).Insert(APosition.Pos, AFragment);
end;
...
begin
  ...
  begin
    ...
    try
      ...
      Line := SelBeg.Line;
      LSelLength := Min(SelLength, Lines.Text.Length);
...
begin
  Model.CaretPosition := TCaretPosition.Zero;
end;
...
begin
  if Model.Lines.Count = 0 then
    NewCaretPosition := TCaretPosition.Zero
  else
    NewCaretPosition := TCaretPosition.Create(Model.Lines.Count - 1, Model.Lines[Model.Lines.Count - 1].Length);

  Model.CaretPosition := NewCaretPosition;
end;
...
procedure TCustomMemo.PasteFromClipboard;
  ...
  procedure BulkPaste(const APastingText: string; var ACaret: TCaretPosition);
  var
  ...
  begin
    ...
    if Model.MaxLength > 0 then
      Insertion := APastingText.Substring(0, Model.MaxLength - LText.Length)
    else
  ...
  procedure LightPaste(const APastingStringList: TStrings; var ACaret: TCaretPosition);
  var
    I : Integer;
  begin
...
begin
  if not Model.ReadOnly and TPlatformServices.Current.SupportsPlatformService(IFMXExtendedClipboardService, ClipService) then
  begin
    if Observers.IsObserving(TObserverMapping.EditLinkID) then
      if not TLinkObservers.EditLinkEdit(Observers) then
      begin
    ...
    try
      ...
      DeleteSelection;
      ClipBoardText := ClipService.GetText;
...
begin
  if Model.Lines.Count > 0 then
  begin
    if FindWordBound(Model.Lines[CaretPosition.Line], CaretPosition.Pos, WordBeginIndex, WordEndIndex) and
      InRange(CaretPosition.Pos, WordBeginIndex, WordEndIndex + 1) then
...
constructor TMemoSettingsInfo.Create(AOwner: TPersistent; ATextSettingsClass: TTextSettingsInfo.TCustomTextSettingsClass);
begin
  inherited;
...
function TMemoLines.AddObject(const S: string; AObject: TObject): Integer;
var
  Lines: TArray<string>;
  I: Integer;
begin
  if (csLoading in FModel.Owner.ComponentState) or S.IsEmpty then
    Result := inherited
  else
  begin
    Result := -1;
    Lines := S.Split([SLineBreak]);
...
procedure TMemoLines.InsertObject(Index: Integer; const S: string; AObject: TObject);
var
  Lines: TArray<string>;
  I: Integer;
begin
