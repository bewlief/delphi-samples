...
type
  TCameraComponentHelper = class
  strict private
    class var FCurrent: TCameraComponentHelper;
    FCompletionHandler: TProc;

    class constructor Create;
    class destructor Destroy;
    procedure PermissionRequestHandler(ASender: TObject; const AMessage: string; const AAccessGranted: Boolean);
  public
    procedure RequestPermission(const ACaptureDevice: TVideoCaptureDevice; const ACompletionHandler: TProc);

    class property Current: TCameraComponentHelper read FCurrent;
  end;
...
begin
  ...
  begin
    ...
    {$ENDIF}
    {$IFDEF LINUX}
    FCurrentManager := TLinuxCaptureDeviceManager.Create;
    {$ENDIF}
    if FCurrentManager = nil then
...
procedure TCaptureDevice.DoPermissionRequest(const AMessage: string; const AAccessGranted: Boolean);
begin
  if Assigned(FOnPermissionRequest) then
    FOnPermissionRequest(Self, AMessage, AAccessGranted);
end;

procedure TCaptureDevice.DoRequestPermission;
begin
  // Stub implementation.
  DoPermissionRequest(string.Empty, True);
end;

procedure TCaptureDevice.RequestPermission;
begin
  DoRequestPermission;
end;
...
class constructor TCameraComponentHelper.Create;
begin
  FCurrent := TCameraComponentHelper.Create;
end;

class destructor TCameraComponentHelper.Destroy;
begin
  FCurrent.Free;
end;

procedure TCameraComponentHelper.PermissionRequestHandler(ASender: TObject; const AMessage: string; const AAccessGranted: Boolean);
begin
  if AAccessGranted and Assigned(FCompletionHandler) then
  begin
    FCompletionHandler;
...
procedure TCameraComponentHelper.RequestPermission(const ACaptureDevice: TVideoCaptureDevice; const ACompletionHandler: TProc);
begin
  FCompletionHandler := ACompletionHandler;

  ACaptureDevice.OnPermissionRequest := PermissionRequestHandler;
  ACaptureDevice.RequestPermission;
end;
...
begin
  TCameraComponentHelper.Current.RequestPermission(Device,
    procedure
    begin
      Device.StartCapture;
      if FDevice <> nil then
  ...
  begin
    ...
    if FMedia <> nil then
      try
        ...
        // If something wrong with FMedia control initialization, we need to release it.
        FreeAndNil(FMedia)
      end;
  end;
end;
...
procedure TMediaPlayerControl.FormHandleAfterCreated(const Sender: TObject; const Msg: TMessage);
  ...
  begin
    Result := (FSavedParent = AForm) or
              (FSavedParent <> nil) and (FSavedParent.Root <> nil) and (FSavedParent.Root.GetObject = AForm) and (AForm is TCommonCustomForm);
  end;

begin
  if (FMediaPlayer <> nil) and IsMediaRootForm(Sender) then
  begin
    Parent := FSavedParent;
    UpdateMedia;
  end;
