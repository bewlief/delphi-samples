...
type
  ...
  IScrollableContent = interface
    ...
    property ContentSize: TSizeF read GetContentSize write SetContentSize;
    property ViewportPosition: TPointF read GetViewportPosition;
    property ViewportSize: TSizeF read GetViewportSize;
  end;

  ///<summary>Styled presentation for <c>TScrollBox</c></summary>
  TStyledCustomScrollBox = class(TStyledPresentation, IScrollableContent)
  private const
...
procedure TStyledCustomScrollBox.InternalAlign;
  ...
  begin
    ...
    begin
      ...
      VScrollBar.Enabled := ((CompareValue(Model.ContentSize.Height, AContentRect.Height) = GreaterThanValue) or
        FAniCalculations.AutoShowing) and Model.EnabledScroll;
      VScrollBar.Visible := Model.ShowScrollBars and (CompareValue(Model.ContentSize.Height, AContentRect.Height) = GreaterThanValue) and
    ...
    begin
      ...
      HScrollBar.Enabled := ((CompareValue(Model.ContentSize.Width, AContentRect.Width) = GreaterThanValue) or
        FAniCalculations.AutoShowing) and Model.EnabledScroll;
      HScrollBar.Visible := Model.ShowScrollBars and (CompareValue(Model.ContentSize.Width, AContentRect.Width) = GreaterThanValue) and
  ...
  begin
    AbsoluteContentLayoutRect := FContentLayout.AbsoluteRect;
    Result := TRectF.Create(AbsoluteToLocal(AbsoluteContentLayoutRect.TopLeft), AbsoluteToLocal(AbsoluteContentLayoutRect.BottomRight));
...
procedure TStyledCustomScrollBox.FreeStyle;
  ...
  procedure ResetScrollInfo(var AScrollInfo: TScrollInfo);
  begin
    if AScrollInfo.Scroll <> nil then
    begin
      AScrollInfo.Scroll.Margins.Rect := AScrollInfo.Margins;
      AScrollInfo.Scroll := nil;
    end;
    AScrollInfo.Margins := TRectF.Empty;
  end;

begin
  inherited;
  ResetScrollInfo(FHScrollInfo);
  ResetScrollInfo(FVScrollInfo);
...
function TStyledCustomScrollBox.GetContentSize: TSizeF;
begin
  Result := Model.ContentSize.Size;
end;
...
function TStyledCustomScrollBox.GetViewportSize: TSizeF;
var
  Rect: TRectF;
  AlignRoot: IAlignRoot;
begin
...
begin
  if FContentLayout <> nil then
  begin
    LPoint := FContentLayout.ConvertLocalPointFrom(Self, TPointF.Create(X, Y));
    X := LPoint.X;
    Y := LPoint.Y;
  end;
end;
...
procedure TStyledCustomScrollBox.SetContentSize(const ASize: TSizeF);
begin
  Model.ContentSize.Size := ASize;
end;
...
begin
  ...
  if (FContentLayout <> nil) and (EventInfo.GestureID = igiPan) then
  begin
    FMouseEvents := False;
