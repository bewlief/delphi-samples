...
uses
  System.Classes, System.Generics.Collections, System.Types, System.Rtti, System.UITypes, FMX.Types, FMX.TextLayout,
  FMX.Controls, FMX.StdCtrls, FMX.VirtualKeyboard, FMX.Menus, FMX.Platform, FMX.Graphics, FMX.Pickers, FMX.DateTimeCtrls.Types;
...
function TCustomTimeEdit.GetData: TValue;
begin
  if IsEmpty then
    Result := TValue.Empty
  else
...
procedure TCustomTimeEdit.SetData(const Value: TValue);
var
  S: string;
...
begin
  ...
  begin
    ...
    Settings.ShortTimeFormat := GetFormat;
    if TryStrToTime(S, LTime, Settings) or TryStrToDateTime(S, LTime, Settings) then
    begin
      IsEmpty := False;
      DateTime := LTime;
    end
...
begin
  ...
  begin
    ...
    if (csDesigning in ComponentState) and UseNowTime then
      Time := System.SysUtils.Time;
  end;
end;
...
begin
  ...
  TmpItem.Text := Translate(ATitleName);
...
procedure TCustomDateTimeEdit.KeyDown(var Key: Word; var KeyChar: System.WideChar; Shift: TShiftState);
  ...
  procedure TrySkipUnknownFormatPart(const AKeyChar: WideChar);
  var
    NextFormatPart: Integer;
    FormatSection: TDTFormatPart;
  begin
    NextFormatPart := IndexCurrentFormatPart + 1;
    if InRange(NextFormatPart, 0, FDTFormatter.Count - 1) then
    begin
      FormatSection := FDTFormatter[NextFormatPart];
      if (FormatSection.Part = TDTPart.Unknown) and FormatSection.DisplayFormat.StartsWith(AKeyChar) then
        GoToNextFormatPart;
    end;
  end;
...
begin
  ...
  case Key of
    ...
    if KeyChar <> #0 then
      TrySkipUnknownFormatPart(KeyChar);
    Exit;
  end;
  Key := 0;
end;
...
begin
  if HasTextObject then
  begin
    ContentMousePos:= ConvertLocalPointTo(TextObject, TPointF.Create(X, Y));
    SetIndexFormatPart(FindDateTimeSection(ContentMousePos));
  end;
  inherited;
end;

procedure TCustomDateTimeEdit.MouseUp(Button: TMouseButton; Shift: TShiftState; X, Y: Single);
begin
...
var
  ...
  BufferString: string;
  LClipboardService: IFMXExtendedClipboardService;
begin
  if not CanInput or not TPlatformServices.Current.SupportsPlatformService(IFMXExtendedClipboardService, LClipboardService) or not LClipboardService.HasText then
    Exit;

  BufferString := LClipboardService.GetText;
...
procedure TCustomDateTimeEdit.SetData(const Value: TValue);
var
  LDateTime: TDateTime;
begin
  if Value.IsEmpty or ((Value.Kind = TTypeKind.tkUString) and Value.AsString.Trim.IsEmpty) then
    IsEmpty := True
  else if Value.TryAsType<TDateTime>(LDateTime) then
  begin
    IsEmpty := False;
    DateTime := LDateTime;
  end
...
begin
  ...
  begin
    // DoDateTimeChanged will be generated in FDTFormatter.OnChange (FormatterDateTimeChanged).
...
function TCustomDateEdit.GetData: TValue;
begin
  if IsEmpty then
    Result := TValue.Empty
  else
...
procedure TCustomDateEdit.SetData(const Value: TValue);
var
  S: string;
  Settings: TFormatSettings;
  LDate: TDateTime;
begin
  ...
  begin
    S := Value.AsString;
    Settings := FormatSettings;
    Settings.ShortDateFormat := GetFormat;
