...
function CGImageRefToSurface(const AImageRef: CGImageRef; const ASurface: TBitmapSurface; const AMaxSize: Integer = 0): Boolean;
...
function MainScreen: NSScreen;
function MainScreenFrame: NSRect;
function MainScreenHeight: CGFloat;

{ Color convertions }

function AlphaColorToNSColor(const AColor: TAlphaColor): NSColor;
function NSColorToAlphaColor(const AColor: NSColor): TAlphaColor;

implementation

uses
  System.RTLConsts, System.SysUtils, System.Types, System.Math, System.Math.Vectors, System.DateUtils, FMX.Consts, FMX.Forms,
  Macapi.CoreGraphics, FMX.Types, MacApi.Helpers;
...
function CGImageRefToSurface(const AImageRef: CGImageRef; const ASurface: TBitmapSurface; const AMaxSize: Integer): Boolean;

  procedure AdjustSize(var ASize: TSize; const AMaxSize: Integer);
  begin
    if (AMaxSize > 0) and ((ASize.Width > AMaxSize) or (ASize.Height > AMaxSize)) then
    begin
      var R := TRectF.Create(TPointF.Zero, ASize.Width, ASize.Height);
      R.Fit(TRectF.Create(TPointF.Zero, AMaxSize, AMaxSize));
      ASize.Width := Trunc(R.Width);
      ASize.Height := Trunc(R.Height);
    end;
  end;
...
begin
  ...
  Size.Width := CGImageGetWidth(AImageRef);
  Size.Height := CGImageGetHeight(AImageRef);
  AdjustSize(Size, AMaxSize);
  ASurface.SetSize(Size.Width, Size.Height, TPixelFormat.RGBA);

  var CtxRef := CGBitmapContextCreate(ASurface.Bits, ASurface.Width, ASurface.Height, 8, ASurface.Width * 4, ColorSpace,
                                      kCGImageAlphaPremultipliedLast);
  if CtxRef = nil then
    Result := False
  else
  try
    CGContextDrawImage(CtxRef, CGRectMake(0, 0, ASurface.Width, ASurface.Height), AImageRef);
    Result := True;
  finally
...
function MainScreen: NSScreen;
begin
  if TNSScreen.OCClass.screens.count > 0 then
...
function MainScreenFrame: NSRect;
begin
  Result := MainScreen.frame;
end;

function MainScreenHeight: CGFloat;
begin
  Result := MainScreen.frame.size.height;
end;
...
begin
  ...
  Result := TNSColor.Wrap(TNSColor.OCClass.colorWithRed(Red, Green, Blue, Alpha));
end;
...
begin
  ...
  C := AColor.colorUsingColorSpaceName(StrToNSStr('NSDeviceRGBColorSpace'));
  C.getRed(@R, @G, @B, @A);
  AlphaColorF.R := C.redComponent;
  AlphaColorF.G := C.greenComponent;
  AlphaColorF.B := C.blueComponent;
  AlphaColorF.A := C.alphaComponent;
