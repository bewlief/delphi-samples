...
function GregorianFirstWeekDaysCount(const AYear: Word; const AFirstDayOfWeek: Word): Word;
var
  FirstJanuary: TDateTime;
  FirstJanuaryDay: Word;
begin
  FirstJanuary := EncodeDate(AYear, 1, 1);
  FirstJanuaryDay := FirstJanuary.DayOfTheWeek;
  Result := DaysPerWeek - ((FirstJanuaryDay - AFirstDayOfWeek + 7) mod DaysPerWeek);
end;

function GregorianWeeksInAYear(const AYear: Word; const AFirstDayOfWeek: Word): Word;
var
  DaysInFirstWeek: Word;
begin
  Result := DaysInAYear(AYear) div 7;
  DaysInFirstWeek := GregorianFirstWeekDaysCount(AYear, AFirstDayOfWeek);
...
function GregorianWeekOfTheYear(const ADateTime: TDateTime; const AFirstDayOfWeek: Word): Word;
var
  DaysInFirstWeek: Word;
begin
  DaysInFirstWeek := GregorianFirstWeekDaysCount(ADateTime.Year, AFirstDayOfWeek);
  if DaysInFirstWeek >= 4 then
    // 1 January - 1 week number
    Result := (DayOfTheYear(ADateTime) + DaysPerWeek - DaysInFirstWeek - 1) div 7 + 1
  else if DayOfTheYear(ADateTime) < 4 then
    // 1 January - previous last week number
    Result := GregorianWeeksInAYear(ADateTime.Year - 1, AFirstDayOfWeek)
  else
    Result := (DayOfTheYear(ADateTime) + DaysPerWeek - DaysInFirstWeek - 1) div 7;
end;

procedure TStyledCalendar.FillWeeksNumbers;

  function GenerateWeekNumbersForAnotherMonthes: TArray<Word>;
  var
    FirstWeekOfMonth: Word;
    WeekCorrection: Integer;
  begin
    SetLength(Result, FWeeks.ControlsCount);

    FirstWeekOfMonth := GregorianWeekOfTheYear(RecodeDay(FDateTime, 1), FirstDayOfWeek);
    ...
    for var I := 0 to FWeeks.ControlsCount - 1 do
      Result[I] := FirstWeekOfMonth + I - WeekCorrection;
  end;

  function GenerateWeekNumbersForJanuary: TArray<Word>;
  var
    FirstWeekOfJanuary: Word;
    WeekCorrection: Integer;
  begin
    FirstWeekOfJanuary := GregorianWeekOfTheYear(RecodeDay(FDateTime, 1), FirstDayOfWeek);
    if FirstWeekOfJanuary <> 1 then
    begin
      SetLength(Result, FWeeks.ControlsCount);
      Result[0] := FirstWeekOfJanuary;
      // First week in FDay may be a last week of previous month. So we need to keep offset for correct counting of weeks.
      WeekCorrection := (DefineItemIndexOfFirstDayInCurrentMonth - 1) div 7;
      for var I := 1 to FWeeks.ControlsCount - 1 do
        Result[I] := I - WeekCorrection;
    end
    else
      Result := GenerateWeekNumbersForAnotherMonthes;
  end;

  function GenerateWeekNumbersForDecember: TArray<Word>;
  var
    FirstWeekOfDecember: Word;
    LastWeekOfDecember: Word;
    WeeksCount: Word;
    WeekCorrection: Integer;
  begin
    SetLength(Result, FWeeks.ControlsCount);

    FirstWeekOfDecember := GregorianWeekOfTheYear(RecodeDay(FDateTime, 1), FirstDayOfWeek);
    LastWeekOfDecember := GregorianWeekOfTheYear(RecodeDay(FDateTime, 31), FirstDayOfWeek);
    WeeksCount := LastWeekOfDecember - FirstWeekOfDecember + 1;

    WeekCorrection := (DefineItemIndexOfFirstDayInCurrentMonth - 1) div 7;
    for var I := 0 to Min(WeeksCount - 1, FWeeks.ControlsCount - 1) do
      Result[I] := FirstWeekOfDecember + I - WeekCorrection;

    for var I := WeeksCount to FWeeks.ControlsCount - 1 do
      Result[I] := I - WeeksCount + 1;
  end;

  procedure SetWeekNumbers(const AWeekNumbers: TArray<Word>);
  var
    CaptionControl: ICaption;
  begin
    Assert(Length(AWeekNumbers) = FWeeks.ControlsCount);

    for var I := 0 to FWeeks.ControlsCount - 1 do
      if Supports(FWeeks.Controls[I], ICaption, CaptionControl) then
        CaptionControl.Text := AWeekNumbers[I].ToString;
  end;

var
  WeekNumbers: TArray<Word>;
begin
  if FWeeks = nil then
    Exit;
  ...
  if not Model.WeekNumbers then
    Exit;

  case MonthOf(FDateTime) of
    MonthJanuary:
      WeekNumbers := GenerateWeekNumbersForJanuary;
    MonthDecember:
      WeekNumbers := GenerateWeekNumbersForDecember;
  else
    WeekNumbers := GenerateWeekNumbersForAnotherMonthes;
  end;
...
begin
  ...
  try
    ...
    try
      ...
      BeginYear := Max(1, SelectedYear - HalfCountOfYearsInPopupBox);
      for Year := BeginYear to BeginYear + 2 * HalfCountOfYearsInPopupBox do
...
begin
  ...
  if FDays <> nil then
  begin
    FDays.OnMouseDown := nil;
    FDays.OnMouseUp := nil;
    FDays.OnChange := nil;
    FDays.OnItemClick := nil;
  end;
  FDays := nil;
end;
