...
procedure TCustomLayer3D.AddUpdateRect(const R: TRectF);
var
  RoundedRect: TRectF;
begin
  if (FDisableUpdating = 0) and not (csDestroying in ComponentState) then
  begin
    RoundedRect := TRectF.Create(R.TopLeft.Truncate, R.BottomRight.Ceiling);
    if IntersectRect(RoundedRect, TRectF.Create(0, 0, FLayerWidth, FLayerHeight)) then
    begin
      SetLength(FUpdateRects, Length(FUpdateRects) + 1);
      FUpdateRects[High(FUpdateRects)] := RoundedRect;
      Repaint;
    end;
  end;
end;
...
function TCustomLayer3D.ScreenToLocal(const P: TPointF): TPointF;
var
  P3: TPoint3D;
  RayPos, RayDir: TVector3D;
begin
...
function TCustomLayer3D.LocalToScreen(const P: TPointF): TPointF;
var
  P3: TPoint3D;
begin
  if Context = nil then
