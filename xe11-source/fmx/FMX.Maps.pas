...
type
  ...
  TMapEpsilon = record
  const
    /// <summary>Coordinate tolerance</summary>
    Coordinate = 0.000001;
  ...
  TMapCoordinate = record
    ...
    class operator Equal(const ACoordinate1, ACoordinate2: TMapCoordinate): Boolean;
    class operator NotEqual(const ACoordinate1, ACoordinate2: TMapCoordinate): Boolean;
  ...
  ICustomMapView = interface
    ...
    /// <summary>Get current camera location, set by SetLocation</summary>
...
class operator TMapCoordinate.Equal(const ACoordinate1, ACoordinate2: TMapCoordinate): Boolean;
begin
  Result := SameValue(ACoordinate1.Latitude, ACoordinate2.Latitude, TMapEpsilon.Coordinate) and
            SameValue(ACoordinate1.Longitude, ACoordinate2.Longitude, TMapEpsilon.Coordinate);
end;

class operator TMapCoordinate.NotEqual(const ACoordinate1, ACoordinate2: TMapCoordinate): Boolean;
begin
  Result := not (ACoordinate1 = ACoordinate2);
end;

function TMapCoordinate.ToString: string;
begin
  Result := Format('%.6f,%.6f', [Latitude, Longitude], TFormatSettings.Create('en-US'));
end;
...
function TCustomMapView.QueryInterface(const IID: TGUID; out Obj): HResult;
begin
  Result := inherited QueryInterface(IID, Obj);
  if (Result <> S_OK) and (FMapView <> nil) then
