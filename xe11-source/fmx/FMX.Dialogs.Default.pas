...
begin
  ...
  try
    ...
    Button := CreateButton(LocalizedButtonCaption(TMsgDlgBtn.mbOK), mrOk);
    Button.Default := True;
    Button := CreateButton(LocalizedButtonCaption(TMsgDlgBtn.mbCancel), mrCancel);
    Button.Default := True;
    Form.ClientHeight := Min(Round(WorkArea.Height * 5) div 6, Trunc(TotalHeight +
      Layout.Margins.MarginRect(Layout.BoundsRect).Height));
    Form.Left := Round(WorkArea.Left) + Round(WorkArea.Width - Form.ClientWidth) div 2;
    Form.Top := Round(WorkArea.Top) + Round(WorkArea.Height - Form.ClientHeight) div 2;
