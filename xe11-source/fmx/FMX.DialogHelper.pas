...
type
  ...
  private
    class function GetWorkArea: TRectF;
    ...
    class function CreateButton(const AOwner: TComponent; const ABorderSize: TPoint; const ALayout: TLayout;
      const ADlgButton: TMsgDlgBtn; const AOnClick: TNotifyEvent = nil): TButton; overload;
    class function CreateButton(const AOwner: TComponent; const ABorderSize: TPoint; const ALayout: TLayout;
      const AText: string; const AModalResult: TModalResult; const AOnClick: TNotifyEvent = nil): TButton; overload;

    class function GetMaxWidth(const AWorkArea: TRect): Integer;
    class function GetMaxHeight(const AWorkArea: TRect): Integer;
  public
    class function MessageDialog(const AMessage: string; const ADialogType: TMsgDlgType; const AButtons: TMsgDlgButtons;
      ...
      const ACloseDialogProc: TInputCloseDialogProc; const ForceCanCancel: Boolean): TDialogForm;

    class function InputQuery(const ACaption: string; const APrompts: array of string;
      const ADefaultValues: array of string; const ACloseQueryProc: TInputCloseQueryProc): TDialogForm;
  end;
...
class function TDialogBuilder.CreateButton(const AOwner: TComponent; const ABorderSize: TPoint;
  const ALayout: TLayout; const ADlgButton: TMsgDlgBtn; const AOnClick: TNotifyEvent): TButton;
begin
  Result := CreateButton(AOwner, ABorderSize, ALayout, ButtonCaptions[ADlgButton], ModalResults[ADlgButton], AOnClick);
end;

class function TDialogBuilder.CreateButton(const AOwner: TComponent; const ABorderSize: TPoint;
  const ALayout: TLayout; const AText: string; const AModalResult: TModalResult;
...
begin
  ...
  Result.Position.X := ALayout.Width;
  Result.Text := AText;
  Result.ModalResult := AModalResult;
  Result.OnClick := AOnClick;
end;

class function TDialogBuilder.GetMaxWidth(const AWorkArea: TRect): Integer;
const
...
class function TDialogBuilder.GetMaxHeight(const AWorkArea: TRect): Integer;
const
...
class function TDialogBuilder.GetWorkArea: TRectF;
begin
...
begin
  ...
  try
    ...
    begin
      Button := CreateButton(LForm, BorderSize, Layout, DlgBtn, LForm.ButtonOnClick);
  ...
  try
    ...
    try
      ...
      begin
        ...
        Prompt.TextAlign := TTextAlign.Leading;
        Prompt.VertTextAlign := TTextAlign.Leading;
        Prompt.Align := TAlignLayout.Left;
        Prompt.VertTextAlign := TTextAlign.Leading;
        Prompt.TextSettings.Trimming := TTextTrimming.Character;
        Prompt.WordWrap := False;
        Prompt.Parent := Layout;
        Prompt.Width := MaxPromptWidth;
    ...
    { Footer }
    Layout := CreateLayout(LForm, BorderSize, ButtonHeight, TAlignLayout.Bottom, LForm);
    Button := CreateButton(LForm, BorderSize, Layout, SMsgDlgOK, mrOk, LForm.ButtonOnClick);
    Button.Default := True;
    Button := CreateButton(LForm, BorderSize, Layout, SMsgDlgCancel, mrCancel, LForm.ButtonOnClick);

    LForm.ClientWidth := Max(Ceil(Button.Width + BorderSize.X) * 2, Min(GetMaxWidth(WorkArea),
      Round((120 + MaxPromptWidth) * DialogUnits.X / 4)));
