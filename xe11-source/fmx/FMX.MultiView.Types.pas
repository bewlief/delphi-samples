...
const
  ...
  MM_COLOR_CHANGED = MM_USER + 3;
  MM_ENABLED_SHADOW_CHANGED = MM_USER + 4;
  MM_SHADOW_OPACITY_CHANGED = MM_USER + 5;
  MM_SHADOWED_OVERLAY_LAYOUT_USER = MM_USER + 6;
...
type
  ...
  TTouchInterceptingLayoutModel = class(TDataModel)
  public const
  ...
  private
    ...
    function GetModel: TTouchInterceptingLayoutModel; overload;
  protected
  ...
  public
    ...
    /// <summary>Enable of shadow</summary>
  ...
  TOverlayLayout = TTouchInterceptingLayout deprecated 'Use TTouchInterceptingLayout instead';
...
{ TDrawerAppearance }
  ...
  TPanelPlacement = (Left, Right, Top, Bottom);
...
uses
  ...
  {$IFDEF ANDROID}, FMX.MultiView.Types.Android{$ENDIF};

{ TOverlayLayout }

constructor TCustomTouchInterceptingLayout.Create(AOwner: TComponent);
begin
  inherited;
  HitTest := True;
  AutoCapture := True;
...
function TCustomTouchInterceptingLayout.DefineModelClass: TDataModelClass;
begin
  Result := TTouchInterceptingLayoutModel;
end;

procedure TCustomTouchInterceptingLayout.DoMouseLeave;
begin
  inherited;
  Pressed := False;
end;

function TCustomTouchInterceptingLayout.GetMode: TOverlayMode;
begin
  Result := Model.Mode;
end;

function TCustomTouchInterceptingLayout.GetModel: TTouchInterceptingLayoutModel;
begin
  Result := inherited GetModel<TTouchInterceptingLayoutModel>;
end;

function TCustomTouchInterceptingLayout.GetOnPointInObjectEvent: TOnPointInObjectEvent;
begin
  Result := Model.OnPointInObjectEvent;
end;

function TCustomTouchInterceptingLayout.GetColor: TAlphaColor;
begin
  Result := Model.ShadowColor;
end;

function TCustomTouchInterceptingLayout.GetEnabledShadow: Boolean;
begin
  Result := Model.EnabledShadow;
end;

function TCustomTouchInterceptingLayout.GetInterceptionSize: Single;
begin
  Result := Model.InterceptionSize;
end;

function TCustomTouchInterceptingLayout.IsInterceptionSizeStored: Boolean;
begin
  Result := not SameValue(InterceptionSize, TTouchInterceptingLayoutModel.DefaultSizeInterception, TEpsilon.Scale);
end;

procedure TCustomTouchInterceptingLayout.MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Single);
begin
  FMousePressedPos := TPointF.Create(X, Y);
  inherited;
end;

procedure TCustomTouchInterceptingLayout.SetMode(const Value: TOverlayMode);
begin
  Model.Mode := Value;
end;

procedure TCustomTouchInterceptingLayout.SetOnPointInObjectEvent(const Value: TOnPointInObjectEvent);
begin
  Model.OnPointInObjectEvent := Value;
end;

procedure TCustomTouchInterceptingLayout.SetColor(const Value: TAlphaColor);
begin
  Model.ShadowColor := Value;
end;

procedure TCustomTouchInterceptingLayout.SetEnabledShadow(const Value: Boolean);
begin
  Model.EnabledShadow := Value;
end;

procedure TCustomTouchInterceptingLayout.SetInterceptionSize(const Value: Single);
begin
...
var
  AbsoluteRect: TRectF;
begin
  AbsoluteRect := ASourceControl.LocalToAbsolute(ASourceLocal);
  Result := ADestControl.AbsoluteToLocal(AbsoluteRect);
end;
...
{ TTouchInterceptingLayoutModel }

constructor TTouchInterceptingLayoutModel.Create;
begin
  inherited Create;
  FMode := DefaultMode;
...
procedure TTouchInterceptingLayoutModel.SetMode(const Value: TOverlayMode);
begin
  if Mode <> Value then
  begin
    FMode := Value;
...
procedure TTouchInterceptingLayoutModel.SetColor(const Value: TAlphaColor);
begin
  FColor := Value;
  SendMessage<TAlphaColor>(MM_COLOR_CHANGED, FColor);
end;

procedure TTouchInterceptingLayoutModel.SetEnabledShadow(const Value: Boolean);
begin
...
procedure TTouchInterceptingLayoutModel.SetInterceptionSize(const Value: Single);
begin
...
function TMultiViewBorderOptions.AreDefaultValues: Boolean;
begin
  Result := (Visible = DefaultVisible) and (Color = DefaultColor);
end;

procedure TMultiViewBorderOptions.AssignTo(Dest: TPersistent);
var
  DestOptions: TMultiViewBorderOptions;
begin
  if Dest is TMultiViewBorderOptions then
  begin
    DestOptions := TMultiViewBorderOptions(Dest);
    DestOptions.Visible := Visible;
    DestOptions.Color := Color;
  end
  else
    inherited;
end;

constructor TMultiViewBorderOptions.Create(AOwner: TComponent; const AInternalChange: TNotifyEvent);
begin
  inherited;
...
procedure TMultiViewBorderOptions.SetColor(const Value: TAlphaColor);
begin
...
procedure TMultiViewBorderOptions.SetVisible(const Value: Boolean);
begin
...
initialization
  RegisterFmxClasses([TCustomTouchInterceptingLayout, TTouchInterceptingLayout]);
end.
