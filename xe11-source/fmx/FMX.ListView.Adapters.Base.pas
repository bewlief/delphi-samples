...
procedure TAbstractListViewAdapter.DoEndUpdate;
begin
end;
...
procedure TAbstractListViewAdapter.EndUpdate;
begin
...
procedure TAbstractListViewAdapter.BeginUpdate;
begin
  Inc(FUpdatingCount);
end;
...
function TAbstractListViewAdapter.IsUpdating: Boolean;
begin
...
procedure TFilterableListViewItems.Changed;
begin
  inherited;
  FNeedApplyFilter := True;
  ApplyFilter;
end;
...
procedure TFilterableListViewItems.DoEndUpdate;
begin
  inherited;
  if FNeedApplyFilter then
    ApplyFilter;
end;
...
begin
  ...
  if TNonReentrantHelper.EnterSection(FIsFiltering) then
  try
    ItemsMayChange;
    try
      ...
      FNeedApplyFilter := False;
      ItemsCouldHaveChanged;
    end;
  finally
    TNonReentrantHelper.LeaveSection(FIsFiltering);
  end;
end;
