...
type
  ...
  public
    ...
    /// <summary>Generates default presentation name for specified control type.</summary>
    ...
    /// <summary>Tries to register default presentation proxy class. If factory already has default presentation proxy
    /// for specified control type, it will rise exception <c>EPresentationProxy</c></summary>
    ...
    /// <summary>Unregisters default presentation</summary>
    procedure UnregisterDefault(const AControlType: TControlType);
...
class function TPresentationProxyFactory.GenerateDefaultPresentationName(const AControlType: TControlType): string;
begin
  Result := Format('default-%s', [AControlType.ToString]);
end;
...
procedure TPresentationProxyFactory.RegisterDefault(const AControlType: TControlType;
  const APresentationProxyClass: TPresentationProxyClass);
begin
  if APresentationProxyClass = nil then
    raise EPresentationProxy.Create(SPresentationProxyClassIsNil);
  if Supported(GenerateDefaultPresentationName(AControlType)) then
    raise EPresentationProxy.CreateFmt(SPresentationAlreadyRegistered, [APresentationProxyClass.ClassName,
      GenerateDefaultPresentationName(AControlType)]);

  FPresentations.Add(GenerateDefaultPresentationName(AControlType), APresentationProxyClass);
end;
...
procedure TPresentationProxyFactory.UnregisterDefault(const AControlType: TControlType);
begin
  FPresentations.Remove(GenerateDefaultPresentationName(AControlType));
end;
