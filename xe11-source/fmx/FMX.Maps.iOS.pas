...
type
  ...
  private
    ...
    procedure NotifyAboutCameraPositionChanged(const ACoordinate: TMapCoordinate);
  public
  ...
  protected
    ...
    procedure SetDragState(const APointAnnitation: MKPointAnnotation; const ANextState: Integer);
  ...
  public
    ...
    procedure UpdatePosition(const ACoordinate: CLLocationCoordinate2D);
  end;
...
function TMapKitMapView.QueryInterface(const IID: TGUID; out Obj): HResult;
begin
...
begin
  if (FMapView <> nil) and (ZOrderManager <> nil) then
  begin
    ZOrderManager.AddOrSetLink(FHostControl, FMapView, nil);
...
procedure TMapKitMapView.SetDragState(const APointAnnitation: MKPointAnnotation; const ANextState: Integer);
var
  Marker: TMapKitMapMarker;
begin
  Marker := GetMapObject<TMapKitMapMarker>(NSObjectToId(APointAnnitation));
  Marker.UpdatePosition(APointAnnitation.coordinate);
...
procedure TMapKitMapView.SetBearing(const ABearing: Single);
begin
  if not SameValue(GetBearing, ABearing, TMapEpsilon.Bearing) then
  begin
    SyncCameraPosition;
    FMapView.camera.setHeading(ABearing);
  end;
end;
...
procedure TMapKitMapView.SetLocation(const ALocation: TMapCoordinate);
begin
  if FLocation <> ALocation then
  begin
    SyncCameraPosition;
    FMapView.setCenterCoordinate(CLLocationCoordinate2D(ALocation));
...
procedure TMapKitMapView.SetTilt(const ATilt: Single);
begin
  if not SameValue(GetTilt, ATilt, TMapEpsilon.Tilt) then
  begin
    SyncCameraPosition;
    FMapView.camera.setPitch(ATilt);
  end;
end;
...
procedure TMapKitMapView.SetZoom(const AZoom: Single);
begin
  if not SameValue(AZoom, GetZoom, TMapEpsilon.Zoom) then
  begin
    SyncCameraPosition;
    FSetZoom := AZoom;
    SetZoomArea;
...
function TMapKitMapView.GetZOrderManager: TiOSZOrderManager;
var
...
begin
  ...
  if FView.GetLocation <> TMapCoordinate.Zero then
    NotifyAboutCameraPositionChanged(FView.GetLocation);
end;

procedure TMapKitDelegate.mapViewRegionDidChangeAnimated(mapView: MKMapView; regionDidChangeAnimated: Boolean); cdecl;
begin
  if not FView.Loaded then
    Exit;

  NotifyAboutCameraPositionChanged(TMapCoordinate(mapView.centerCoordinate));
end;
...
begin
  Result := nil;
  Marker := FView.GetMapObject<TMapKitMapMarker>(viewForAnnotation);
  if Marker <> nil then
    Result := Marker.AnnotationView;
end;

procedure TMapKitDelegate.NotifyAboutCameraPositionChanged(const ACoordinate: TMapCoordinate);
var
  Event: TMapEvent;
begin
  Event.Kind := TMapEventKind.CameraChanged;
  Event.LatLng := ACoordinate;
...
var
  PointAnnitation: MKPointAnnotation;
begin
  PointAnnitation := TMKPointAnnotation.Wrap(annotationView.annotation);
  FView.SetDragState(PointAnnitation, didChangeDragState);
...
procedure TMapKitMapMarker.UpdatePosition(const ACoordinate: CLLocationCoordinate2D);
begin
  FDescriptor.Position := TMapCoordinate.Create(ACoordinate.latitude, ACoordinate.longitude);
end;
