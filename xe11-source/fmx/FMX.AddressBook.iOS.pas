...
uses
  System.Generics.Collections, System.Permissions, iOSapi.AddressBook, iOSapi.Foundation, Macapi.CoreFoundation, FMX.AddressBook,
...
destructor TiOSAddressBookContact.Destroy;
begin
  CFRelease(PersonRef);
  inherited;
end;
...
procedure TiOSAddressBookContact.AllGroups(var AGroups: TAddressBookGroups);
var
  ...
  J: Integer;
  ContactID: ABRecordID;
  LPersonRef: ABRecordRef;
begin
  if AGroups = nil then
  ...
  try
    FAddressBook.AllGroups(Source, Groups);
    for I := 0 to Groups.Count - 1 do
      if Groups[I] is TiOSAddressBookGroup then
      begin
        Group := TiOSAddressBookGroup(Groups[I]);
        ...
        if AllPeople <> nil then
          try
            ...
            begin
              LPersonRef := CFArrayGetValueAtIndex(AllPeople, J);
              ContactID := ABRecordGetRecordID(LPersonRef);
              if ContactID = ID then
                AGroups.Add(TiOSAddressBookGroup.CreateExisted(FAddressBook, Group.GroupRef));
            end;
          finally
            CFRelease(AllPeople);
          end;
...
var
  ...
  Person: TiOSAddressBookContact;
  Group: TiOSAddressBookGroup;
  ContactsIDs: TList<ABRecordID>;
  PersonRef: ABRecordRef;
  ContactID: Integer;
begin
  if AGroups = nil then
  ...
  begin
    // Any contact can be included in several groups, so we need to avoid including 
    // this contact several times in result list
    ContactsIDs := TList<ABRecordID>.Create;
    try
        ...
        begin
          ...
          if AllPeople <> nil then
            try
              for J := 0 to CFArrayGetCount(AllPeople) - 1 do
              begin
                PersonRef := CFArrayGetValueAtIndex(AllPeople, J);
                ContactID := ABRecordGetRecordID(PersonRef);
                if not ContactsIDs.Contains(ContactID) then
                begin
                  Person := TiOSAddressBookContact.CreateFromExisting(Self, PersonRef);
                  AContacts.Add(Person);
...
var
  ...
  I: Integer;
  Group: TiOSAddressBookGroup;
begin                                                                           if (ASource <> nil) and not (ASource is TiOSAddressBookSource) then
...
procedure TiOSAddressBook.RequestPermission(ADisplayRationaleEvent: TDisplayRationaleEvent);
  // No permission implementation for iOS yet, so we just defer to a parameterless routine
var
  LTypeInfo: Pointer;
  LMethod: TMethod;
