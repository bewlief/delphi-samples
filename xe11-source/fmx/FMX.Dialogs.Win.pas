...
begin
  Result := FFileDialog.FDialog.GetFileTypeIndex(FFileDialog.FFileTypeIndex);
end;
...
begin
  ...
  begin
    ...
    if Printer is TPrinterWin then
      TPrinterWin(Printer).Copies := PrintDlgRec.nCopies;
  end
  else
  begin
...
const
  ...
  procedure PutToParentCenter(const ADialogHandle: HWND);
  var
    ParentHwnd: HWND;
    ParentRect: TRect;
    DialogRect: TRect;
    ...
    Y: Integer;
  begin
    ParentHwnd := GetParent(ADialogHandle);
    if (ParentHwnd = 0) or not GetWindowRect(ParentHwnd, ParentRect) or ParentRect.IsEmpty then
      Exit;

    if not GetWindowRect(ADialogHandle, DialogRect) then
      Exit;

    X := ParentRect.Left + (ParentRect.Width - DialogRect.Width) div 2;
    Y := ParentRect.Top + (ParentRect.Height - DialogRect.Height) div 2;
    SetWindowPos(ADialogHandle, 0, X, Y, 0, 0, SWP_NOSIZE or SWP_NOZORDER or SWP_NOACTIVATE);
  end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            CreateButton(Data, IDRETRY, LocalizedButtonCaption(TMsgDlgBtn.mbYesToAll));
            CreateButton(Data, IDIGNORE, LocalizedButtonCaption(TMsgDlgBtn.mbNoToAll));
            SetButtonText(Data, IDYES, LocalizedButtonCaption(TMsgDlgBtn.mbYes));
            SetButtonText(Data, IDNO, LocalizedButtonCaption(TMsgDlgBtn.mbNo));
            SetButtonText(Data, IDCANCEL, LocalizedButtonCaption(TMsgDlgBtn.mbCancel));
            SetButtonText(Data, IDHELP, LocalizedButtonCaption(TMsgDlgBtn.mbHelp));
          ...
          // MessageBoxIndirect has bug with dynamic adaptation to HiDPI changes. If we change scale of monitor,
          // when the application has already ran, Dialog windows is appeared with accounting old monitor settings.
          // As result message box is not appeared in the screen center.
          PutToParentCenter(AwParam);
        end;
      end;
    HCBT_DESTROYWND:
...
begin
  ...
  if ADialogType = TMsgDlgType.mtCustom then
    ...
    Params.lpszCaption := PChar(LocalizedMessageDialogTitle(ADialogType));
