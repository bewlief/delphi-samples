...
uses
  ...
  FMX.Types, FMX.ActnList, System.ImageList, FMX.Styles, FMX.Platform, FMX.Forms, FMX.Controls, FMX.DateTimeCtrls, FMX.Calendar;
...
procedure TPopupListPicker.DoMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Single);
  ...
  begin
    ...
    ContentRect := FListBox.Content.BoundsRect;
    Result := (Floor(ItemRect.Bottom + ContentRect.Top - FListBox.Content.Margins.Top) > 0) and
              (Ceil(ItemRect.Top + ContentRect.Top - FListBox.ClientHeight) <= 0)
  end;
...
var
  ...
  begin
    ...
    ScrollPos := ConvertLocalPointFrom(FListBox.ListItems[FListBox.ItemIndex], TPointF.Zero);
    if Parent is TControl then
      VerticalOffset := -TControl(Parent).Height - ScrollPos.Y;
  end;

begin
  FListBox.NeedStyleLookup;
