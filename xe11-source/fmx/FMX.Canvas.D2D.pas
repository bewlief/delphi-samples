...
uses
  System.Classes, Winapi.Windows, Winapi.DXGI, Winapi.D3D10, Winapi.D2D1, Winapi.Wincodec, FMX.Graphics;
...
type
  ...
  TD2DDeviceRemoveEvent = procedure (Sender: TObject; const ARes: HResult);
  ...
  private class var
    ...
    { Diagnostics }
    FOnDeviceRemoved: TD2DDeviceRemoveEvent;
  private
  ...
  public
    ...
    class property OnDeviceRemoved: TD2DDeviceRemoveEvent read FOnDeviceRemoved;
  end;
...
uses
  ...
  Winapi.D3DCommon, Winapi.D3D10_1, System.Types, System.Character, System.SysUtils, System.Math,
...
type
  ...
  private class var
    ...
    FSharedMultithread: ID2D1Multithread;
    class function SharedRenderTarget: ID2D1RenderTarget; static;
    class property SharedMultithread: ID2D1Multithread read FSharedMultithread;
  private
    FBrush: ID2D1Brush;
    ...
    procedure ContextLostHandler(const Sender: TObject; const Msg: TMessage);
    procedure HandleDeviceRemoved(const ARes: HResult);
    function CreatePathGeometry(const APath: TPathData): ID2D1PathGeometry;
    procedure RestoreDrawingState(const ABLockState: ID2D1DrawingStateBlock);
  ...
  public
    ...
    { Multithreading }
    class procedure MultithreadEnter;
    class procedure MultithreadLeave;
  end;
  ...
  private type
    TFontDescriptor = record
      ...
      constructor Create(const AFamilyName: string; AFont: IDWriteFont); overload;
      constructor Create(const AFont: TFont); overload;
    end;
  private class var
    FDefaultDrawTextOption: TD2D1DrawTextOptions;
    FFontCollection: IDWriteFontCollection;
    FLocaleName: string;
    FFontDescriptors: TDictionary<string, TFontDescriptor>;
  private
    ...
    class function GetDefaultDrawTextOption: TD2D1DrawTextOptions; static;
    class property DefaultDrawTextOption: TD2D1DrawTextOptions read GetDefaultDrawTextOption;
  private
    FLayout: IDWriteTextLayout;
    FBrush: ID2D1Brush;
    FDrawTextOption: TD2D1DrawTextOptions;
    FMetrics: TDWriteTextMetrics;
    FOverhangMetrics: TDwriteOverhangMetrics;
    FContextLostId: Integer;
    FTextRect: TRectF;
    function TryGetFontDescriptorFromGDI(const AFont: TFont; var ADescriptor: TFontDescriptor): Boolean;
    function GetFontStyles(const AFont: TFont): TFontDescriptor;
...
begin
  W := 0;
  H := 0;
  ImagingFactory.CreateDecoderFromFilename(PChar(AFileName), GUID_NULL, GENERIC_READ, WICDecodeMetadataCacheOnDemand,
    Decoder);
  if Decoder <> nil then
  begin
...
begin
  ...
  if ParentHandle.Transparency and not IsDesignTime then
...
const
  ...
  function TryCreateFromBitmap(const ABitmap: TBitmap; out ARenderTarget: ID2D1RenderTarget): Boolean;
  var
  ...
  begin
    ...
    Supports(TD2DBitmapHandle(Bitmap.Handle).Texture, IDXGISurface, Surface);
    Result := Succeeded(SharedFactory.CreateDxgiSurfaceRenderTarget(Surface, Prop, ARenderTarget)) and
              (ARenderTarget <> nil);
  end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          SwapDesc.OutputWindow := ParentHandle.Wnd;
          ...
          SharedDXGIFactory.MakeWindowAssociation(ParentHandle.Wnd, DXGI_MWA_NO_WINDOW_CHANGES);
        end;
      end;
    end;
    if FTarget <> nil then
...
begin
  ...
  begin
    ...
    begin
      ...
      RadialGradBrushProp.Center := TD2D1Point2F(TPointF.Create(ARect.Width * AGradient.RadialTransform.RotationCenter.X, ARect.Height * AGradient.RadialTransform.RotationCenter.Y));
...
procedure TCanvasD2D.DoClear(const Color: TAlphaColor);
begin
  if FTarget <> nil then
    FTarget.Clear(D2Color(Color, 1));
end;

procedure TCanvasD2D.DoClearRect(const ARect: TRectF; const AColor: TAlphaColor);
begin
...
function TCanvasD2D.DoBeginScene(AClipRects: PClipRects; AContextHandle: THandle): Boolean;
var
  Res: HResult;
begin
          ...
          begin
            for I := 0 to FBufferSize.Height - 1 do
              Move(PAlphaColorArray(Mapped.pData)[Cardinal(I) * (Mapped.RowPitch div 4)],
                PAlphaColorArray(ParentHandle.BufferBits)[I * FBufferSize.Width], FBufferSize.Width * 4);
          end
          else
            Move(Mapped.pData^, ParentHandle.BufferBits^, FBufferSize.Width * FBufferSize.Height * 4);
        finally
          FCopyBuffer.Unmap(0);
        end;
...
procedure TCanvasD2D.HandleDeviceRemoved(const ARes: HResult);
begin
  try
    try
      if Assigned(OnDeviceRemoved) then
        OnDeviceRemoved(Self, ARes);
    finally
...
function TCanvasD2D.IsDesignTime: Boolean;
var
  ParentHandle: TWinWindowHandle;
begin
  if Parent = nil then
  ...
  begin
    ParentHandle := WindowHandleToPlatform(Parent);
    Result := (ParentHandle.Form <> nil) and (csDesigning in ParentHandle.Form.ComponentState);
  end;
end;
...
procedure TCanvasD2D.DoIntersectClipRect(const ARect: TRectF);
var
...
procedure TCanvasD2D.DoExcludeClipRect(const ARect: TRectF);
var
  i: Integer;
...
class procedure TCanvasD2D.MultithreadEnter;
begin
  if SharedMultithread <> nil then
    SharedMultithread.Enter
  else
    Log.d(SCannotGetSharedMultithreadObject);
end;

class procedure TCanvasD2D.MultithreadLeave;
begin
  if SharedMultithread <> nil then
    SharedMultithread.Leave
  else
    Log.d(SCannotGetSharedMultithreadObject);
end;
...
class function TCanvasD2D.DoMapBitmap(const Bitmap: THandle; const Access: TMapAccess; var Data: TBitmapData): Boolean;
  ...
  function TryCreateTexture2DFor(const AHandle: TD2DBitmapHandle): HResult;
  var
    Desc: TD3D10_Texture2DDesc;
  begin
    FillChar(Desc, SizeOf(D3D10_TEXTURE2D_DESC), 0);
    Desc.Format := DXGI_FORMAT_B8G8R8A8_UNORM;
    Desc.Width := AHandle.FWidth;
    Desc.Height := AHandle.FHeight;
    Desc.MipLevels := 1;
    Desc.ArraySize := 1;
    Desc.SampleDesc.Count := 1;
    Desc.SampleDesc.Quality := 0;
    ...
    Result := SharedDevice.CreateTexture2D(Desc, nil, AHandle.FMapBuffer);
  end;
...
begin
  ...
  try
    ...
    H.FAccess := Access;
    IsJustCreated := H.FMapBuffer = nil;
    ...
    case Access of
      ...
      begin
        ...
        // If Map buffer wasn't allocated before, it can lead to problem, when content of buffer is lost.
        if IsJustCreated then
          SharedDevice.CopyResource(H.FMapBuffer, H.Texture);
      end;
    else
...
begin
  ...
  begin
    ...
    Geometry := CreatePathGeometry(APath);
...
procedure TCanvasD2D.RestoreDrawingState(const ABLockState: ID2D1DrawingStateBlock);
begin
  FCurrentSaveState := nil;
  FTarget.RestoreDrawingState(ABLockState);

  // Restoring drawing state resets antialiasing mode, so we should restore it.
  case Quality of
    TCanvasQuality.HighPerformance:
      FTarget.SetAntialiasMode(D2D1_ANTIALIAS_MODE_ALIASED);
    TCanvasQuality.HighQuality:
...
begin
  ...
  begin
    var DestCanvas := TCanvasD2D(Dest);
    DestCanvas.RestoreDrawingState(FStateBlock);
    if FLayer <> nil then
    begin
      DestCanvas.FTarget.PopLayer;
      FLayer := nil;
    end;
  end;
...
{ TTextLayoutD2D.TFontDescriptor }

constructor TTextLayoutD2D.TFontDescriptor.Create(const AFont: TFont);
begin
  FamilyName := AFont.Family;
  Weight := FontWeightToDWrite(AFont.StyleExt.Weight);
  Style := FontSlantToDWrite(AFont.StyleExt.Slant);
  Stretch := FontStretchToDWrite(AFont.StyleExt.Stretch);
end;

constructor TTextLayoutD2D.TFontDescriptor.Create(const AFamilyName: string; AFont: IDWriteFont);
begin
  FamilyName := AFamilyName;
  Weight := AFont.GetWeight;
  Style := AFont.GetStyle;
  Stretch := AFont.GetStretch;
end;
...
begin
  FFontCollection := nil;
  FreeAndNil(FFontDescriptors);
end;

class function TTextLayoutD2D.GetDefaultDrawTextOption: TD2D1DrawTextOptions;
begin
  if FDefaultDrawTextOption = D2D1_DRAW_TEXT_OPTIONS_NONE then
  begin
    FDefaultDrawTextOption := D2D1_DRAW_TEXT_OPTIONS_CLIP;
    if TOSVersion.Check(5, 2) and Supports(TCanvasD2D.SharedFactory, ID2D1Factory5) then
      FDefaultDrawTextOption := FDefaultDrawTextOption or D2D1_DRAW_TEXT_OPTIONS_ENABLE_COLOR_FONT;
  end;
  Result := FDefaultDrawTextOption;
end;

function TTextLayoutD2D.GetFontStyles(const AFont: TFont): TFontDescriptor;
...
begin
  Result := TFontDescriptor.Create(AFont);
  ...
  begin
    if FFontDescriptors = nil then
      FFontDescriptors := TDictionary<string, TFontDescriptor>.Create;
    if FFontDescriptors.ContainsKey(AFont.Family) then
      Result := FFontDescriptors[AFont.Family]
    else
    begin
      if TryGetFontDescriptorFromGDI(AFont, FontDescriptor) then
...
function TTextLayoutD2D.TryGetFontDescriptorFromGDI(const AFont: TFont; var ADescriptor: TFontDescriptor): Boolean;
  ...
  try
    ...
    try
      ...
      try
        ...
        try
          ADescriptor := TFontDescriptor.Create(GetDWriteLocalizedName(Names, FLocaleName), DWriteFont);
          Result := True;
        finally
          Names := nil;
        end;
...
begin
  ...
  if IsPointInRect(APoint, LRect) then
  begin
