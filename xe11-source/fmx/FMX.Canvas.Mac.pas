...
type
  ...
  protected
    ...
    /// <summary>Helper method that convert TBitmapSurface to CGContextRef.</summary>
    function SurfaceToCGBitmap(const ABitmap: TBitmapSurface; out ABitmapCtx: CGContextRef): Boolean;
...
begin
  ...
  if CtxRef <> nil then
  try
    CGContextDrawImage(CtxRef, CGRectMake(0, 0, Bitmap.Width, Bitmap.Height), Img);
...
begin
  ...
  try
    ...
    try
      if SurfaceToCGBitmap(Bitmap, ctxRef) then
      try
...
function TBitmapCodecQuartz.SurfaceToCGBitmap(const ABitmap: TBitmapSurface; out ABitmapCtx: CGContextRef): Boolean;
var
  Data: Pointer;
begin
  if ABitmap.PixelFormat = TPixelFormat.RGBA then
    ABitmapCtx := CGBitmapContextCreate(ABitmap.Bits, ABitmap.Width, ABitmap.Height, 8, ABitmap.Pitch, ColorSpace,
                                        kCGImageAlphaPremultipliedLast)
  else
  begin
    ABitmapCtx := CGBitmapContextCreate(nil, ABitmap.Width, ABitmap.Height, 8, ABitmap.Pitch, ColorSpace,
                                        kCGImageAlphaPremultipliedLast);
    Data := CGBitmapContextGetData(ABitmapCtx);
    ChangePixelFormat(ABitmap.Bits, Data, ABitmap.Width * ABitmap.Height, ABitmap.PixelFormat, TPixelFormat.RGBA);
  end;
  Result := ABitmapCtx <> nil;
end;
...
begin
  if not Bitmap.IsEmpty then
    for I := 0 to Trunc(DestRect.Width / Bitmap.Width)  do
      ...
      begin
        LRect := CGRectMake(DestRect.Left + (Bitmap.Width * I), DestRect.Top + (Bitmap.Height * J),
...
procedure TCanvasQuartz.DoClear(const Color: TAlphaColor);
var
  LColor: TAlphaColorF;
  ClearRect: CGRect;
begin
  if Context <> nil then
  begin
    ClearRect := CGRectMake(0, 0, FWidth, FHeight);
...
procedure TCanvasQuartz.DoClearRect(const ARect: TRectF; const AColor: TAlphaColor);
var
  LColor: TAlphaColorF;
  ClearRect: CGRect;
begin
  if Context <> nil then
  begin
    ClearRect := CGRectFromRect(ARect);
    CGContextClearRect(Context, ClearRect);
...
function TCanvasQuartz.DoBeginScene(AClipRects: PClipRects = nil; AContextHandle: THandle = 0): Boolean;
begin
...
procedure TCanvasQuartz.DoSetMatrix(const M: TMatrix);
  ...
  function CGAffineTransformToMatrix(const ATransformation: CGAffineTransform): TMatrix;
  begin
    Result := TMatrix.Identity;
    Result.m11 := ATransformation.a;
    Result.m12 := ATransformation.b;
    Result.m21 := ATransformation.c;
    Result.m22 := ATransformation.d;
    Result.m31 := ATransformation.tx;
    Result.m32 := ATransformation.ty;
  end;

  function MatrixToCGAffineTransform(const AMatrix: TMatrix): CGAffineTransform;
  begin
    Result := CGAffineTransformMake(AMatrix.m11, AMatrix.m12, AMatrix.m21, AMatrix.m22, AMatrix.m31, AMatrix.m32);
  end;

  procedure SetIdentityMatrixToContext;
  var
    LMatrix: TMatrix;
    Transform: CGAffineTransform;
  begin
    Transform := CGContextGetCTM(Context);
    LMatrix := CGAffineTransformToMatrix(Transform).Inverse;
    CGContextConcatCTM(Context, MatrixToCGAffineTransform(LMatrix));
  end;

begin
  if Context <> nil then
  begin
    // CoreGraphics works with matrix concatenation and reflected Coordinate system, so before setting matrix we need to
    // prepare context transformation for working in regular coordinate system.
    SetIdentityMatrixToContext;
    AdaptCoordinateSystem;

    CGContextConcatCTM(Context, MatrixToCGAffineTransform(M));
  end;
end;
...
procedure TCanvasQuartz.DoIntersectClipRect(const ARect: TRectF);
begin
  if Context <> nil then
    CGContextClipToRect(Context, CGRectFromRect(ARect));
end;

procedure TCanvasQuartz.DoExcludeClipRect(const ARect: TRectF);
var
...
begin
  ...
  Origin := CalcOrigin;
  // Canvas for bitmap always works with physical size, so Origin has already contains physical offset
  if Bitmap <> nil then
    CGContextTranslateCTM(Context, Origin.X, Origin.Y)
  else
    CGContextTranslateCTM(Context, Origin.X * Scale, Origin.Y * Scale);
...
constructor TMacColor.Create(const AColor: TAlphaColor);
var
  AlphaColorF: TAlphaColorF;
begin
  AlphaColorF := TAlphaColorF.Create(AColor);
  R := AlphaColorF.R;
  G := AlphaColorF.G;
  B := AlphaColorF.B;
  A := AlphaColorF.A;
...
type
  TGradientInfo = record
    ...
    Alpha: Single;
    constructor Create(const AGradient: TGradient; const AAlpha: Single);
    function InterpolateColor(const AOffset: Single): TAlphaColor;
  end;
  PGradientInfo = ^TGradientInfo;

{ TGradientInfo }

constructor TGradientInfo.Create(const AGradient: TGradient; const AAlpha: Single);
var
  Point: TGradientPoint;
  I: Integer;
begin
  SetLength(Colors, AGradient.Points.Count);
  SetLength(Offsets, AGradient.Points.Count);
  for I := Low(Colors) to High(Colors) do
  begin
    Point := AGradient.Points[I];
    Colors[I] := Point.Color;
    Offsets[I] := Point.Offset;
  end;
  Alpha := AAlpha;
end;

function TGradientInfo.InterpolateColor(const AOffset: Single): TAlphaColor;
var
  I: Integer;
  Offset: Single;
  Count: Integer;
begin
  ...
  Offset := EnsureRange(AOffset, 0, 1);
  if Offset < Offsets[0] then
  begin
    Result := Colors[0];
    Exit;
  end;
  if Offset > Offsets[High(Offsets)] then
  begin
  ...
  begin
    if Offset < Offsets[I] then
      Continue;
    if Offset > Offsets[I + 1] then
      Continue;
    if Offsets[I + 1] - Offsets[I] <= 0 then
      Result := Colors[I]
    else if (I = Count - 2) and (Offset > Offsets[Count - 1]) then // last
      Result := Colors[Count - 1]
    else
      Result := FMX.Utils.InterpolateColor(Colors[I], Colors[I + 1], (Offset - Offsets[I]) / (Offsets[I + 1] - Offsets[I]));
  end;
end;

procedure GradientEvaluateCallback(info: Pointer; inData: PCGFloat; outData: PMacColor); cdecl;
var
  AlphaColor: TAlphaColor;
  GradientInfo: PGradientInfo;
begin
  if info <> nil then
  begin
    GradientInfo := PGradientInfo(info);
    AlphaColor := MakeColor(GradientInfo.InterpolateColor(inData^), GradientInfo.Alpha);
    outData^ := TMacColor.Create(AlphaColor);
  end;
end;

procedure GradientReleaseInfoCallback(info: Pointer); cdecl;
begin
  FreeMem(info);
end;
...
const
  DefaultInputRange: array[0..1] of CGFloat = (0, 1);
var
  LColor: TAlphaColorF;
  RCenter: TPointF;
  BrushBitmap: TBitmap;
  ...
  case ABrush.Kind of
    ...
    TBrushKind.Gradient:
      begin
        ...
        FCallback.releaseInfo:= @GradientReleaseInfoCallback;
        GradientInfo := AllocMem(SizeOf(TGradientInfo));
        GradientInfo.Create(ABrush.Gradient, AOpacity);
        FFunc := CGFunctionCreate(GradientInfo, 1, @DefaultInputRange, 4, nil, @FCallback);
        if ABrush.Gradient.Style = TGradientStyle.Linear then
        begin
          StartPoint := CGPointMake(ARect.Left + ABrush.Gradient.StartPosition.X * ARect.Width,
                                    ARect.Top + ABrush.Gradient.StartPosition.Y * ARect.Height);
          StopPoint := CGPointMake(ARect.Left + ABrush.Gradient.StopPosition.X * ARect.Width,
                                   ARect.Top + ABrush.Gradient.StopPosition.Y * ARect.Height);
          FShading := CGShadingCreateAxial(ColorSpace, StartPoint, StopPoint, FFunc, True, True);
        end
        else
        begin
          ...
          FShading := CGShadingCreateRadial(ColorSpace, CGPointMake(RCenter.X, RCenter.Y), ARect.Width / 2,
                                            CGPointMake(RCenter.X, RCenter.Y), 0, FFunc, True, True);
        end;
      end;
    TBrushKind.Bitmap:
      begin
...
type
  TMacDashArray = TArray<CGFloat>;

  function DashArrayToMacDashArray(const ADash: TDashArray): TMacDashArray;
  var
    I: Integer;
  begin
    SetLength(Result, Length(ADash));
    for I := Low(ADash) to High(ADash) do
      Result[I] := ADash[I];
  end;
...
begin
  ...
  begin
    ...
    MacDash := DashArrayToMacDashArray(Dash);
    CGContextSetLineDash(Context, AStroke.DashOffset, @MacDash[0], Length(AStroke.DashArray));
  end
  else
...
begin
  if Context <> nil then
  begin
    ApplyStroke(ABrush, TRectF.Create(APt1, APt2), AOpacity);
  ...
  begin
    ...
    case ABrush.Bitmap.WrapMode of
      ...
      TWrapMode.TileOriginal:
        begin
          CGContextScaleCTM(Context, 1, -1);
          CGContextDrawImage(Context, CGRectFromRect(TRectF.Create(ARect.Left, ARect.Top, ARect.Left + BrushBitmap.Width,
...
begin
  ...
  try
    ...
    Result := CGContextPathContainsPoint(ContextRef, CGPointMake(APoint.X, APoint.Y), kCGPathFillStroke);
  finally
...
procedure TCanvasQuartz.DoDrawBitmap(const ABitmap: TBitmap; const SrcRect, DstRect: TRectF; const AOpacity: Single;
  ...
  function BitmapDataToCGBitmap(const ABitmapData: TBitmapData; out ABitmapCtx: CGContextRef): Boolean;
  var
    Data: Pointer;
  begin
    if ABitmapData.PixelFormat = TPixelFormat.RGBA then
      ABitmapCtx := CGBitmapContextCreate(ABitmapData.Data, ABitmapData.Width, ABitmapData.Height, 8, ABitmapData.Pitch, ColorSpace,
                                          kCGImageAlphaPremultipliedLast)
    else
    begin
      ABitmapCtx := CGBitmapContextCreate(nil, ABitmapData.Width, ABitmapData.Height, 8, ABitmapData.Pitch, ColorSpace,
                                          kCGImageAlphaPremultipliedLast);
      Data := CGBitmapContextGetData(ABitmapCtx);
      ChangePixelFormat(ABitmapData.Data, Data, ABitmapData.Width * ABitmapData.Height, ABitmapData.PixelFormat, TPixelFormat.RGBA);
    end;
    Result := ABitmapCtx <> nil;
  end;

  function BitmapToContextAndImageRef(const ABitmap: TBitmap; out AContextRef: CGContextRef; out AImageRef: CGImageRef): Boolean;
  var
    Data: TBitmapData;
  begin
    if ABitmap.Map(TMapAccess.Read, Data) then
    try
      if BitmapDataToCGBitmap(Data, AContextRef) then
      begin
        AImageRef := CGBitmapContextCreateImage(AContextRef);
        if AImageRef = nil then
          CGContextRelease(AContextRef)
      end;
    finally
      ABitmap.Unmap(Data);
    end;
    Result := AImageRef <> nil;
  end;
...
var
  ...
  NeedReleaseImageRef: Boolean;
begin
  if (Context = nil) or (ABitmap = nil) or not ABitmap.HandleAllocated then
    Exit;

  if (FPrinter <> nil) and not TCanvasManager.DefaultCanvas.InheritsFrom(TCanvasQuartz) then
  begin
    // In printer
    if BitmapToContextAndImageRef(ABitmap, CtxRef, ImageRef) then
      NeedReleaseImageRef := True
    else
      Exit;
  end
  else
  begin
    ...
    NeedReleaseImageRef := False;
  end;

  try
    NewDestRect := CGRect.Create(DstRect);
    NewDestRect.origin.y := -DstRect.Bottom;
    if SrcRect = ABitmap.BoundsF then
    begin
      CGContextSaveGState(Context);
      CGContextSetAlpha(Context, AOpacity);
...
begin
  ...
  Rgba[2] := LColor.B;
  Rgba[3] := LColor.A;
  TextColor := CGColorCreate(ColorSpace, @Rgba);
...
procedure TTextLayoutCT.CreateCT;
  ...
  begin
    ...
    try
      CFAttributedStringSetAttribute(AAttribute, CFRangeMake(AStart, ALength), kCTFontAttributeName, LFontRef);
    finally
      CFRelease(LFontRef);
    end;
    // Underline
    ...
    begin
      ...
      try
        CFAttributedStringSetAttribute(AAttribute, CFRangeMake(AStart, ALength), kCTUnderlineStyleAttributeName, LUnderline);
      finally
...
var
  ...
  i: Integer;
  LText: string;
  Attribute: TTextAttributedRange;
  AttributeFont: TFont;
begin
  ...
  Bounds := CGRectMake(0, 0, MaxSize.X - Padding.Left - Padding.Right, $FFFF);
  ...
  if Text.Length = 0 then
    ...
    LText := Text;
  LStringRef := CFStringCreateWithCharacters(kCFAllocatorDefault, PChar(LText), LText.Length);
  try
    ...
    try
      ...
      try
        CFAttributedStringSetAttribute(FCTAttr, CFRangeMake(0, LText.Length), kCTLigatureAttributeName, Ligature);
        ...
        if WordWrap then
          ...
          case Trimming of
            ...
            // MacOS doesn't support Word trancation
        ...
        begin
          ...
          AttributeFont := Attribute.Attribute.Font;
          if AttributeFont = nil then
            Continue;

          // Checking for changes or we will get an SIGSERV error
          if (AttributeFont.Family <> Font.Family) or
             not SameValue(AttributeFont.Size, Font.Size, Epsilon) or
             (AttributeFont.Style <> Font.Style) then
            AddFontAttribute(FCTAttr, AttributeFont, Attribute.Range.Pos, Attribute.Range.Length);
        end;
      finally
        CFRelease(Ligature);
      end;
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        Attribute := Attributes[i];
        if Color <> Attribute.Attribute.Color then
          AddColorAttribute(LAttr, Attribute.Attribute.Color, Opacity, Attribute.Range.Pos, Attribute.Range.Length);
      end;
    finally
...
begin
  if (FCTAttr = nil) or Text.IsEmpty then
    Exit;

  Context := TCanvasQuartz(ACanvas).Context;
  if Context = nil then
    Exit;
  ...
  CGContextSaveGState(Context);
  try
    CGContextClipToRect(Context, CGRectMake(TopLeft.X, TopLeft.Y, MaxSize.X, MaxSize.Y));
    CGContextSetTextMatrix(Context, CGAffineTransformMakeScale(1.0, 1.0));
    CGContextTranslateCTM(Context, TopLeft.X, TopLeft.Y + MaxSize.Y);
    CGContextScaleCTM(Context, 1, -1);
    CGContextTranslateCTM(Context, 0, -($FFFF - MaxSize.Y));

    // Applying vertical align
    case VerticalAlign of
      TTextAlign.Center:
        CGContextTranslateCTM(Context, 0, -(MaxSize.Y - FTextHeight + Padding.Top - Padding.Bottom) / 2);
      TTextAlign.Leading:
        CGContextTranslateCTM(Context, 0, -Padding.Top);
      TTextAlign.Trailing:
        CGContextTranslateCTM(Context, 0, -(MaxSize.Y - FTextHeight - Padding.Bottom));
    end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          bounds := CGRectMake(lineBounds.origin.x + offset, lineBounds.origin.y,
                               rwidth, Font.Size * FFontScale - descent);
...
begin
  ...
  try
    ...
    begin
      NewFontRef := CTFontCreateCopyWithSymbolicTraits(LFontRef, 0, nil, kCTFontItalicTrait, kCTFontItalicTrait);
...
function TTextLayoutCT.DoPositionAtPoint(const APoint: TPointF): Integer;
  ...
  function IsIndexInEllipsisGlyph(const ALine: CTLineRef; const AIndex: CFIndex): Boolean;
  var
    Range: CFRange;
  begin
    ...
    Range := CTLineGetStringRange(ALine);
    if AIndex < Range.location + Range.Length then
      Exit(True);

    Result := False;
  end;

  function DefineStringIndexForPositionInTruncatedLine(const ALine: CTLineRef; const AHitPoint: TPointF): CFIndex;
  const
    UndefinedPosition = -$FFFF;
  var
    Runs: CFArrayRef;
    RunsCount: Int64;
    I: Integer;
    ...
    Run: CTRunRef;
    GlyphsCount: Int64;
    GlyphsPositionsPtr: PCGPoint;
    GlyphPos: CGPoint;
    Index: Integer;
    PreviousGlyphX: Single;
  begin
    Index := 0;
    PreviousGlyphX := UndefinedPosition;
    Runs := CTLineGetGlyphRuns(ALine);
    RunsCount := CFArrayGetCount(Runs);
    for I := 0 to RunsCount - 1 do
    begin
      Run := CTRunRef(CFArrayGetValueAtIndex(Runs, I));
      GlyphsCount := CTRunGetGlyphCount(Run);
      GlyphsPositionsPtr := CTRunGetPositionsPtr(Run);
      for J := 0 to GlyphsCount - 1 do
      begin
        GlyphPos := GlyphsPositionsPtr^;
        if GlyphPos.x >= AHitPoint.X then
        begin
          if not SameValue(PreviousGlyphX, UndefinedPosition) and (AHitPoint.X >= PreviousGlyphX + (GlyphPos.x - PreviousGlyphX) / 2) then
            Inc(Index);
          Break;
        end
        else
          Inc(Index);
        PreviousGlyphX := GlyphPos.x;
        Inc(GlyphsPositionsPtr);
      end;
    end;

    // CoreText internally inserts truncate char '...'. We need to ignore it. CoreText doesn't provide any good way
    // to catch it. So we determinate it based on range values.
    if IsIndexInEllipsisGlyph(ALine, Index) then
      Dec(Index);
    Result := Index;
  end;
...
begin
  ...
  begin
    ...
    if InRange(APoint.Y, TopY + LinesBeforeHeight, TopY + LinesBeforeHeight + LineHeight) then
    begin
      // CTLineGetStringIndexForPosition has specific logic of handling getting position in truncated string, namely if
      // APoint falls into ellipsis character "...", it automatically returns last index of all string.
      // But in FMX we use regular behavior, when we ignore "...". So only or this case we use a scanning position of
      // all glyphs in line and define manually index.
      if not WordWrap and (Trimming <> TTextTrimming.None) then
        Index := DefineStringIndexForPositionInTruncatedLine(Line, APoint)
      else
        Index := CTLineGetStringIndexForPosition(Line, CGPointMake(APoint.X - Padding.Left - LineOrigin.x - TopLeft.X,
          LineOrigin.Y - APoint.Y - Padding.Top - TopLeft.Y));
      if Index >= 0 then
      begin
        Result := Index;
        if Result > 0 then
...
begin
  ...
  begin
    ...
    if LineRange.location + RemainsLength < 0 then
      Continue;
    if LineRange.location > LRange.Pos + RemainsLength then
      Break;
    Width := CTLineGetTypographicBounds(Line, @Ascent,  @Descent, @Leading);
    if Width = 0 then
      Continue;
    CTFrameGetLineOrigins(FCTFrame, CFRangeMake(I, 1), @LineOrigin);
    BaseLineY := $FFFF - LineOrigin.y;
    Runs := CTLineGetGlyphRuns(Line);
    ...
    begin
      ...
      SetLength(Indicies, RunGlyphCount);
      CTRunGetStringIndices(Run, CFRangeMake(0, RunGlyphCount), @Indicies[0]);
      for K := 0 to RunGlyphCount - 1 do
      begin
        GlyphRect := TRectF.Create(LineOrigin.x + Positions[K].x, BaseLineY - Ascent,
                                   LineOrigin.x + Positions[K].x + Advances[K].width, BaseLineY + Descent);
        if InRange(Indicies[K], LRange.Pos, LRange.Pos + LRange.Length - 1) then
        begin
          SetLength(CurLine, Length(CurLine) + 1);
          CurLine[High(CurLine)] := GlyphRect;
          // Resetting width if selection length is 0 (i.e. only position need to be calculated, not a region)
          if RangeLength = 0 then
            CurLine[High(CurLine)].Width := 0;
        end;

        if K = RunGlyphCount - 1 then
          NextIndex := RunRange.location + RunRange.length
        else
          NextIndex := Indicies[K + 1];

        if (Indicies[K] < LRange.Pos) and (LRange.Pos < NextIndex) then
        begin
          SetLength(CurLine, Length(CurLine) + 1);
          CurLine[High(CurLine)] := GlyphRect;
          // Resetting width if selection length is 0 (i.e. only position need to be calculated, not a region)
    ...
    // Caret in the end of line
    if (Length(CurLine) = 0) and (RemainsLength = 0) then
      CurLine := [TRectF.Create(GlyphRect.Right, GlyphRect.Top, GlyphRect.Right + 1, GlyphRect.Bottom)];
