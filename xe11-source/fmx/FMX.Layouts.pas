...
uses
  System.Generics.Collections, System.Math, System.RTLConsts, System.Generics.Defaults, System.TypInfo, FMX.Styles, FMX.Consts, 
  FMX.Effects, FMX.Ani, FMX.Graphics;
...
const
  ...
  begin
    ...
    if VScrollBar <> nil then
      UpdateScrollbarVisibility(VScrollBar, CompareValue(FContentBounds.Height, AContentRect.Height) = GreaterThanValue, Reset);
    if HScrollBar <> nil then
      UpdateScrollbarVisibility(HScrollBar, CompareValue(FContentBounds.Width, AContentRect.Width) = GreaterThanValue, Reset);
  end;
...
begin
  if ContentLayout <> nil then
  begin
    LPoint := ConvertLocalPointTo(ContentLayout, TPointF.Create(X, Y));
    X := LPoint.X;
    Y := LPoint.Y;
  end;
end;
  ...
  for I := 0 to FRowCollection.Count - 1 do
    ...
    begin
      ...
      begin
        ...
        begin
          Control := ControlItem.Control;
          LSize := Control.Height + Control.Margins.Top + Control.Margins.Bottom + Self.Padding.Top + Self.Padding.Bottom;
          if LSize > MaxSize then
            MaxSize := LSize;
        end;
      end;
...
procedure TGridPanelLayout.DoRealign;
  ...
  procedure ArrangeControlInCell(const AControl: TControl; const ACellRect: TRectF);
  var
    NewBounds: TRectF;
  ...
  begin
    ...
    begin
      ...
      if AnchorSubset = [TAnchorKind.akLeft] then
        NewBounds.Left := ACellRect.Left
      else if AnchorSubset = [TAnchorKind.akRight] then
        NewBounds.Left := Max(ACellRect.Left, ACellRect.Right - AControl.Width) (* was: Margins.ExplicitWidth *)
      else
        NewBounds.Left := Max(ACellRect.Left, ACellRect.Left + ((ACellRect.Right - ACellRect.Left) - AControl.Width (* was: Margins.ControlWidth *)) / 2);
      NewBounds.Right := NewBounds.Left + Min(ACellRect.Right - ACellRect.Left, AControl.Width); (* was: Margins.ExplicitWidth *)
      AnchorSubset := AControl.Anchors * [TAnchorKind.akTop, TAnchorKind.akBottom];
      if AnchorSubset = [TAnchorKind.akTop] then
        NewBounds.Top := ACellRect.Top
      else if AnchorSubset = [TAnchorKind.akBottom] then
        NewBounds.Top := Max(ACellRect.Top, ACellRect.Bottom - AControl.Height) (* was: Margins.ExplicitHeight *)
      else
        NewBounds.Top := Max(ACellRect.Top, ACellRect.Top + ((ACellRect.Bottom - ACellRect.Top) - AControl.Height) / 2); (* was: Margins.ControlHeight *)
      NewBounds.Bottom := NewBounds.Top + Min(ACellRect.Bottom - ACellRect.Top, AControl.Height); (* was: Margins.ExplicitHeight *)
    ...
    begin
      ...
      ArrangeControl(AControl, AControl.Align,
                     ACellRect.Width, ACellRect.Height,
                     ACellRect.Width, ACellRect.Height,
                     NewBounds);
    end;
  end;
