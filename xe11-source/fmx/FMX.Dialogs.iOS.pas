...
type
  ...
  private
    ...
    class function CountDialogButtons(AButtons: TMsgDlgButtons): UInt8; static;
    class procedure AddDialogButtons(const AAlertView: UIAlertView; AButtons: TMsgDlgButtons); static;
  protected
...
begin
  ...
  Result.initWithTitle(StrToNSStr(ACaption), StrToNSStr(Prompt), Delegate.GetObjectID,
    StrToNSStr(LocalizedButtonCaption(TMsgDlgBtn.mbCancel)), nil);
  Result.addButtonWithTitle(StrToNSStr(LocalizedButtonCaption(TMsgDlgBtn.mbOK)));
...
begin
  ...
  try
    AlertView := TUIAlertView.Wrap(AlertView.initWithTitle(StrToNSStr(LocalizedMessageDialogTitle(ADialogType)),
                                                           StrToNSStr(AMessage), Delegate.GetObjectID, nil, nil));
    AddDialogButtons(AlertView, AButtons);
    AlertView.Show;
...
class function TCocoaDialogsService.CountDialogButtons(AButtons: TMsgDlgButtons): UInt8;
var
  Button: TMsgDlgBtn;
begin
  Result := 0;
  ...
  begin
    if Button in AButtons then
      Inc(Result);
  end;
end;

class procedure TCocoaDialogsService.AddDialogButtons(const AAlertView: UIAlertView; AButtons: TMsgDlgButtons);
var
  Count: UInt8;
  Button: TMsgDlgBtn;
begin
  Count := CountDialogButtons(AButtons);

  if (Count <= 2) and (TMsgDlgBtn.mbCancel in AButtons) then
    AAlertView.addButtonWithTitle(StrToNSStr(LocalizedButtonCaption(TMsgDlgBtn.mbCancel)));

  for Button := Low(TMsgDlgBtn) to High(TMsgDlgBtn) do
  begin
    if (Button <> TMsgDlgBtn.mbCancel) and (Button in AButtons) then
      AAlertView.addButtonWithTitle(StrToNSStr(LocalizedButtonCaption(Button)));
  end;
...
begin
  ...
  Count := TCocoaDialogsService.CountDialogButtons(AButtons);
  Index := 0;
  ...
  if (Count <= 2) and (TMsgDlgBtn.mbCancel in AButtons) then
  begin
    FResults[Index] := ModalResults[TMsgDlgBtn.mbCancel];
    Inc(Index);
  end;
  ...
  begin
    if (Button <> TMsgDlgBtn.mbCancel) and (Button in AButtons) then
    begin
      FResults[Index] := ModalResults[Button];
      Inc(Index);
    end;
  end;
