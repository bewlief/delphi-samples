...
type
  ...
  public
    constructor Create(const AWidth, AHeight: Integer; const AScale: Single;
      const APixelFormat: TPixelFormat = TPixelFormat.None);
...
uses
  ...
  FMX.Platform.Win,
  {$ENDIF}
  System.Math, System.Math.Vectors, System.SysUtils, FMX.Consts, FMX.Platform, FMX.Forms, FMX.TextLayout,
  FMX.TextLayout.GPU, FMX.StrokeBuilder;
...
type
  ...
  protected
    function TransformPoint(const P: TPointF): TPointF; reintroduce;
    function TransformRect(const Rect: TRectF): TRectF; reintroduce;
...
begin
  ...
  begin
    ...
    LDest.FCanvasHelper.DrawingMode := TDrawingMode.Normal;
  end;
...
begin
  ...
  begin
    ...
    FTexture.PixelFormat := FPixelFormat;
...
constructor TCanvasGpu.CreateFromBitmap(const ABitmap: TBitmap; const AQuality: TCanvasQuality = TCanvasQuality.SystemDefault);
var
  Multisample: TMultisample;
  H: TBitmapCtx;
begin
...
procedure TCanvasGpu.DoClear(const Color: TAlphaColor);
begin
...
procedure TCanvasGpu.DoClearRect(const ARect: TRectF; const AColor: TAlphaColor);
begin
  FCanvasHelper.Flush;
  try
...
function TCanvasGpu.DoBeginScene(AClipRects: PClipRects; AContextHandle: THandle): Boolean;
begin
  if FGlobalBeginScene = 0 then
  begin
    FCanvasHelper.SetContext(Context);
    FCanvasHelper.BeginRender;
    TGlyphsManager.Default.BeginRender;
  end
  else
  begin
    FCanvasHelper.Flush;
...
begin
  ...
  begin
    ...
    begin
      ...
      TGlyphsManager.Default.EndRender;
...
procedure TCanvasGpu.DoExcludeClipRect(const ARect: TRectF);
begin
  if FContext <> nil then
  begin
    FCanvasHelper.DrawingMode := TDrawingMode.WriteStencilInvert;
    FCanvasHelper.FillRect(TransformBounds(CornersF(ARect)), TAlphaColors.White);

    FCanvasHelper.DrawingMode := TDrawingMode.ReadStencil;
    FContext.SetStencilFunc(TStencilFunc.Equal, 0, $FF);  // Invert test
  end;
end;
...
var
  H: TBitmapCtx;
begin
  H := TBitmapCtx.Create(Width, Height, Scale, PixelFormat);
{$IFDEF AUTOREFCOUNT}
  H.__ObjAddRef;
{$ENDIF}
  Result := THandle(H);
...
begin
  ...
  TTextLayoutManager.RegisterTextLayout(TGPUTextLayout, TCanvasGpu);
