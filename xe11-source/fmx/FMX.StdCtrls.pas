...
procedure TLabel.MouseMove(Shift: TShiftState; X, Y: Single);
var
  IsPressed: Boolean;
begin
  inherited;
  ...
  begin
    ...
    if FIsPressed <> IsPressed then
    begin
      FIsPressed := IsPressed;
      StartTriggerAnimation(Self, 'IsPressed');
      ApplyTriggerEffect(Self, 'IsPressed');
    end;
  end;
...
procedure TLabel.SetNewScene(AScene: IScene);
begin
...
procedure TCustomButton.SetNewScene(AScene: IScene);
begin
  if (AScene <> nil) and FIsPressed then
  begin
    FIsPressed := False;
...
procedure TCheckBox.SetNewScene(AScene: IScene);
begin
  if (AScene <> nil) and FIsPressed then
  begin
    FIsPressed := False;
...
procedure TRadioButton.SetNewScene(AScene: IScene);
begin
  if (AScene <> nil) and FIsPressed then
  begin
    FIsPressed := False;
...
begin
  ...
  begin
    case Orientation of
      TOrientation.Horizontal:
        if FTrackHighlight.Align = TAlignLayout.Right then
          FTrackHighlight.Width := Round(FTrack.width - (GetThumbRect.Left + GetThumbRect.Right) / 2)
        else
...
procedure TBitmapTrackBar.DoRealign;
  ...
  function IsBitmapChangedSize: Boolean;
  begin
    Result := (FBitmap <> nil) and (FBackground <> nil) and ((FBitmap.Width <> Trunc(FBackground.Width)) or (FBitmap.Height <> Trunc(FBackground.Height)));
  end;

begin
  inherited;
  if (FBitmap = nil) or IsBitmapChangedSize then
    UpdateBitmap;
end;
...
procedure TAniIndicator.ApplyStyle;
  ...
  function CreateStyleReplacement: TFloatAnimation;
  begin
    ...
    Result.Parent := FLayout;
    Result.Loop := True;
    Result.StartValue := 0;
    Result.StopValue := 360;
    Result.Duration := 10;
...
begin
  ...
  begin
    ...
    FFill.Color := TextObject.Color;
  end;
  if FindStyleResource<TFmxObject>('fill', O) and Supports(O, IBrushObject, BO) then
    FFill.Assign(BO.Brush);
  if not FindStyleResource<TAnimation>('ani', FAni) then
    FAni := CreateStyleReplacement;
  if csDesigning in ComponentState then
  begin
    FAni.Start;
    FAni.Stop;
