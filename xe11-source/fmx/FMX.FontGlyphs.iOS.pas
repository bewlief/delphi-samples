...
function TIOSFontGlyphManager.DoGetGlyph(const ACharacter: UCS4String; const Settings: TFontGlyphSettings;
  ...
  function GetPixelFormat(const ABitmapContext: CGContextRef): TPixelFormat;
  var
    AlphaInfo: CGImageAlphaInfo;
    BitmapInfo: CGBitmapInfo;
    IsAlphaFirst: Boolean;
    IsAlphaLast: Boolean;
    IsEndianLittle: Boolean;
  begin
    AlphaInfo := CGBitmapContextGetAlphaInfo(ABitmapContext);
    BitmapInfo := CGBitmapContextGetBitmapInfo(ABitmapContext);
    IsAlphaFirst := AlphaInfo in [kCGImageAlphaPremultipliedFirst, kCGImageAlphaFirst, kCGImageAlphaNoneSkipFirst];
    IsAlphaLast := AlphaInfo in [kCGImageAlphaPremultipliedLast, kCGImageAlphaLast, kCGImageAlphaNoneSkipLast];
    IsEndianLittle := BitmapInfo and kCGBitmapByteOrder32Little = kCGBitmapByteOrder32Little;

    if IsAlphaFirst and IsEndianLittle then
      Result := TPixelFormat.BGRA
    else if IsAlphaLast and not IsEndianLittle then
...
begin
  ...
  begin
    ...
    try
      ...
      Result.Bitmap.SetSize(GlyphRect.Width, GlyphRect.Height, GetPixelFormat(Context));

      if TFontGlyphSetting.PremultipliedAlpha in Settings then
        for I := GlyphRect.Top to GlyphRect.Bottom - 1 do
          Move(Bits^[I * ContextSize.Width + Max(GlyphRect.Left, 0)], Result.Bitmap.GetPixelAddr(0, I - GlyphRect.Top)^, Result.Bitmap.Pitch)
      else
...
const
  ...
  function IsDigitWithoutSelector(const ACharacter: UCS4String): Boolean; inline;
  begin
    Result := (Length(ACharacter) = 2) and InRange(ACharacter[0], ZeroCode, NineCode) and (ACharacter[1] = 0);
  end;
  ...
  begin
    ...
    begin
      // User's font doesn't have specified character. We are tring to find it in the Apple Emoji font.
      // The AppleColorEmoji font has digit presentations for Unicode code without presentation mode selector.
      // It's not ok for us, because we are interested in digits only with presentation mode selector, so we
      // reject digits without this selector
      if IsDigitWithoutSelector(ACharacter) then
        Result := False
      else
