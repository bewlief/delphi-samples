...
uses
  ...
  System.Math.Vectors, FMX.Consts, FMX.Ani, FMX.Utils, FMX.BehaviorManager, FMX.Forms 
  {$IFDEF IOS}, FMX.ListView.iOS{$ENDIF} {$IFDEF ANDROID}, FMX.ListView.Android{$ENDIF};
...
begin
  ...
  if not InRange(FTapSelectItemIndex, -1, ItemCount - 1) then
    FTapSelectItemIndex := -1;
...
function TListViewBase.GetControlType: TControlType;
begin
  Result := FControlType;
end;
...
begin
  ...
  if FDeleteButtonIndex = -1 then
  begin
    // This method can be invoked from event handler of these controls, so we have to remove it later after exiting
    // from their event handlers
    if FDeleteButton <> nil then
    begin
      FDeleteButton.Release;
      FDeleteButton := nil
    end;
    if FDeleteLayout <> nil then
    begin
      FDeleteLayout.Release;
      FDeleteLayout := nil
    end;
  end;
end;
...
begin
  ...
  if FDeleteButtonIndex = -1 then
    LTargetItemIndex := Min(FPrevDeleteButtonIndex, ItemCount - 1)
  else
    LTargetItemIndex := Min(FDeleteButtonIndex, ItemCount - 1);

  if LTargetItemIndex = -1 then
    RelRect := TRectF.Create(LocalRect.TopLeft + TPointF.Create(FSideSpace, FSideSpace - FScrollViewPos), 0, 0)
  else
    RelRect := GetItemRelRect(LTargetItemIndex, LocalRect);
...
begin
  ...
  if (NewIndex <> OldItemIndex) and AllowChange then
  begin
    DoChange;
    Invalidate;
  end;
...
procedure TListViewBase.ApplyStyle;
  ...
  procedure UpdateColorsInItemsObjects;
  var
    I: Integer;
    ListItem: TListItem;
    J: Integer;
    Drawable: TListItemDrawable;
    StyleResources: TListItemStyleResources;
  begin
    StyleResources := GetStyleResources;

    for I := 0 to Adapter.Count - 1 do
    begin
      ListItem := Adapter[I];
      for J := 0 to ListItem.View.Count - 1 do
      begin
        Drawable := ListItem.View.Drawables[J];
        Drawable.UpdateValuesFromStyle;
        if ControlType = TControlType.Platform then
          Drawable.UpdateValuesFromResources(StyleResources, ListItem.Purpose);
      end;
    end;
  end;
...
begin
  ...
  UpdateColorsInItemsObjects;
  // We don't use active notification model for native presentation and colors are set in presentation construction.
  // So we recreate native presentation in this case.
  if ControlType = TControlType.Platform then
  begin
    RecreateNativePresentation;
    Invalidate;
...
var
  AccessoryType: TAccessoryType;
begin
  // Control Colors
  FBackgroundStyleColor := TAlphaColorRec.Null;
  FItemStyleFillColor := TAlphaColorRec.Null;
  FItemStyleFillAltColor := TAlphaColorRec.Null;
  FItemStyleFrameColor := TAlphaColorRec.Null;

  // Item Colors
  FStyleResources.DefaultTextColor := TAlphaColorRec.Null;
  FStyleResources.DefaultTextSelectedColor := TAlphaColorRec.Null;
  FStyleResources.DetailTextColor := TAlphaColorRec.Null;
  FStyleResources.HeaderTextColor := TAlphaColorRec.Null;
  FStyleResources.HeaderTextShadowColor := TAlphaColorRec.Null;

  FStyleResources.ButtonTextColor := TAlphaColorRec.Null;
  FStyleResources.ButtonTextPressedColor := TAlphaColorRec.Null;
  FStyleResources.DeleteButtonTextColor := TAlphaColorRec.Null;
  FStyleResources.DeleteButtonTextPressedColor := TAlphaColorRec.Null;

  FStyleResources.ButtonAddItemStyleImage.Normal := nil;
  FStyleResources.ButtonAddItemStyleImage.Pressed := nil;
  FStyleResources.ButtonDeleteItemStyleImage.Normal := nil;
  FStyleResources.ButtonDeleteItemStyleImage.Pressed := nil;
  FStyleResources.ButtonNormalStyleImage.Normal := nil;
  FStyleResources.ButtonNormalStyleImage.Pressed := nil;
  FStyleResources.ButtonDeleteStyleImage.Normal := nil;
  FStyleResources.ButtonDeleteStyleImage.Pressed := nil;
  FStyleResources.ButtonCheckboxStyleImage.Normal := nil;
  FStyleResources.ButtonCheckboxStyleImage.Pressed := nil;

  // Acessory Images
  for AccessoryType in [Low(TAccessoryType)..High(TAccessoryType)] do
  begin
    FStyleResources.AccessoryImages[AccessoryType].Normal := nil;
    FStyleResources.AccessoryImages[AccessoryType].Selected := nil;
  end;

  FStyleResources.ScrollingStretchGlowColor := TAlphaColorRec.Null;
  FStyleResources.PullRefreshIndicatorColor := TAlphaColorRec.Null;
  FStyleResources.PullRefreshStrokeColor := TAlphaColorRec.Null;
...
begin
  ...
  begin
    ...
    begin
      Checkable.CheckAll(False);
      // It's important to reset link on IListViewCheckProvider, because RecreateNativePresentation can recreate Adapter
      // and as result release IListViewCheckProvider.
      Checkable := nil;
    end;
    RecreateNativePresentation;
  end;
...
var
  ...
  LDoUpdatingItemViewP: procedure (const AListItem: TListItem; var AHandled: Boolean) of object;
  LDoUpdateItemViewP: procedure (const AListItem: TListItem) of object;
  LSync: Boolean;

  function IsIndexValid(AIndex: Integer): Boolean;
  begin
    Result := (AIndex >= 0) and (AIndex < Adapter.Count);
  end;

  function GetSelectableIndex(AIndex: Integer): Integer;
  var
    LItem: TListItem;
  begin
    Result := AIndex;
    if not IsIndexValid(AIndex) then
      Exit;
    LItem := Adapter[AIndex];
    case LItem.Purpose of
      TListItemPurpose.Header:
        while (Result < Adapter.Count) and (Adapter[Result].Purpose <> TListItemPurpose.None) do
          Inc(Result);
      TListItemPurpose.Footer:
        while (Result >= 0) and (Adapter[Result].Purpose <> TListItemPurpose.None) do
          Dec(Result);
    end;
  end;
...
begin
  ...
  try
    ...
    begin
      ...
      LDoUpdatingItemViewP := DoUpdatingItemView;
      LDoUpdateItemViewP := DoUpdateItemView;
      LSync := IsIndexValid(AItem.Index) and Observers.IsObserving(TObserverMapping.PositionLinkID) and (
        (TMethod(LDoUpdatingItemViewP).Code <> @TAppearanceListView.DoUpdatingItemView) or
        (TMethod(LDoUpdateItemViewP).Code <> @TAppearanceListView.DoUpdateItemView) or
        Assigned(OnUpdatingItemView) or Assigned(OnUpdatingObjects) or
        Assigned(OnUpdateItemView) or Assigned(OnUpdateObjects));
      try
        if LSync then
        begin
          FItemIndex := GetSelectableIndex(AItem.Index);
...
begin
  ...
  if FAppearanceViewItems <> nil then
    FAppearanceViewItems.BeginUpdate;
end;
...
begin
  ...
  if FAppearanceViewItems <> nil then
    FAppearanceViewItems.EndUpdate;
end;
