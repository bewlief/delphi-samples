...
procedure TStyledPresentation.FreeStyle;
  ...
  begin
    for Entry in FModel.DataSource do
    begin
      ValueFromStyle := FModel.Data[Entry.Key];
      ValueFromData := FModel.DataSource[Entry.Key];
      if ValueFromStyle.IsInstanceOf(TPersistent) and ValueFromData.IsInstanceOf(TPersistent) then
        ...
        FModel.DataSource.AddOrSetValue(Entry.Key, ValueFromStyle);
    end;
  end;
...
begin
  ...
  { Models Data }
  if (FModel <> nil) and (FModel.DataSource <> nil) then
