...
uses
  ...
  Macapi.CoreServices, Macapi.CoreFoundation, iOSapi.Foundation,
  iOSapi.CocoaTypes, iOSapi.UIKit,

  FMX.Printer, FMX.Types, FMX.Consts, FMX.Graphics;

type
  TPrinteriOS = class(TPrinter)
{
  private
...
procedure TPrinteriOS.ActivePrinterChanged;
begin
  inherited;
...
//procedure TPrinteriOS.CheckError(Error: OSStatus; const Msg: String);
//begin
//  if Error <> noErr then
//    RaiseError(Msg);
//end;

//function TPrinteriOS.ConvCFString(const Value: CFStringRef): String;
//begin
...
//function TPrinteriOS.ConvDriverCreator(const Value: OSType): String;
//var
//  P: PChar;
//begin
...
//function TPrinteriOS.ConvNSString(const Value: NSString): String;
//begin
//  if Assigned(Value) then
...
constructor TPrinteriOS.Create;
{var
  Changed: Boolean;}
begin
  inherited;
 ...
 
{
  ...
  FCanvas.ResizeBuffer(PageWidth, PageHeight);
...
destructor TPrinteriOS.Destroy;
begin
{
  FCanvas.Free;
...
procedure TPrinteriOS.DoAbortDoc;
begin
  inherited;
...
procedure TPrinteriOS.DoBeginDoc;
{var
  Changed: Boolean;
  Err: OSStatus;}
begin
...
procedure TPrinteriOS.DoEndDoc;
//var
//  Err: OSStatus;
begin
  inherited;
...
procedure TPrinteriOS.DoNewPage;
//var
//  Err: OSStatus;
begin
  inherited;
...
function TPrinteriOS.GetCanvas: TCanvas;
begin
  Result := nil; //FCanvas;
end;

function TPrinteriOS.GetCapabilities: TPrinterCapabilities;
begin
...
{
function TPrinteriOS.GetCoordFactor: TPointF;
const
  CPointsPerInch = 72;
begin
...
function TPrinteriOS.GetNumCopies: Integer;
begin
//  PMGetCopies(PrintInfo.PMPrintSettings, @Result);
  Result := 0;
end;

function TPrinteriOS.GetOrientation: TPrinterOrientation;
begin
...
function TPrinteriOS.GetPageHeight: Integer;
var
//  PageRect: PMRect;
  Height: Double;
//  Err: OSStatus;
...
function TPrinteriOS.GetPageWidth: Integer;
var
//  PageRect: PMRect;
  Width: Double;
//  Err: OSStatus;
...
{
function TPrinteriOS.GetPaperMargins: TRectF;
var
  PaperRect: PMRect;
  Err: OSStatus;
  Ok: Boolean;
...
procedure TPrinteriOS.RefreshFonts;
begin
  inherited;

end;

procedure TPrinteriOS.RefreshPrinterDevices;
//var
...
procedure TPrinteriOS.SetDefaultPrinter;
//var
...
procedure TPrinteriOS.SetNumCopies(Value: Integer);
//var
//  Err: OSStatus;
begin
  inherited;
...
procedure TPrinteriOS.SetOrientation(Value: TPrinterOrientation);
//var
...
//  Canvas.ResizeBuffer(PageWidth, PageHeight);
end;
...
begin
  ...
  PMPrinterSetOutputResolution(CarbonPrinter,
    TPrinteriOS(Printer).PrintInfo.PMPrintSettings, @Resolution);

  // resize the canvas
  Printer.Canvas.ResizeBuffer(Printer.PageWidth, Printer.PageHeight);
end;
