...
unit FMX.Platform.SaveState.Win;
...
type
  /// <summary>Implements <c>IFMXSaveStateService</c> for Windows.</summary>
  TWinSaveStateService = class(TInterfacedObject, IFMXSaveStateService)
  private
...
{ TWinSaveStateService }

function TWinSaveStateService.GetBlock(const ABlockName: string; const ABlockData: TStream): Boolean;
...
function TWinSaveStateService.GetNotifications: Boolean;
begin
  Result := False;
end;

function TWinSaveStateService.GetSaveStateFileName(const ABlockName: string): string;
const
  Prefix = '~';
  Separator = '_';
var
...
begin
  ...
  try
    S.Append(FilePath);
    S.Append(Prefix);
    S.Append(ChangeFileExt(ExtractFileName(ParamStr(0)), string.Empty));
    S.Append(Separator);
    S.Append(ABlockName);
...
function TWinSaveStateService.GetStoragePath: string;
begin
  Result := FSaveStateStoragePath;
end;

function TWinSaveStateService.SetBlock(const ABlockName: string; const ABlockData: TStream): Boolean;
...
procedure TWinSaveStateService.SetStoragePath(const ANewPath: string);
begin
