...
type
  ...
  TReleasePool = class(TNoRefCountObject, IFreeNotification)
  private
    class var FInstance: TReleasePool;
    class destructor Destroy;
    class function GetInstance: TReleasePool; static;
  private
    FPool: TList<TObject>;
    FIsReleaseInQueue: Boolean;
  ...
  public
    ...
    procedure Release(const AObject: TObject);
    function IsMarkedForReleasing(const AObject: TObject): Boolean;         
    class property Instance: TReleasePool read GetInstance;
  end;
...
begin
  Result := AText;
  // Checking for & allows us to speed up this function by 7-8 times for lines without &. In most cases,
  // this function is invoked for string without &.
  if AText.IsEmpty or not AText.Contains('&') then
    Exit;

  SetLength(SB, AText.Length);
  I := 0;
  SBIndex := 0;
  LastIndex := Length(AText) - 1;
  while I <= LastIndex do
  begin
    CurrentChar := AText.Chars[I];
    if CurrentChar = '&' then
    begin
      if (I < LastIndex) and (AText.Chars[I + 1] = '&') then
      begin
        SB[SBIndex] := CurrentChar;
        Inc(SBIndex);
        Inc(I);
      end;
    end
  ...
  Result := string.Create(Sb, 0, SBIndex);
end;
...
var
  ...
  Color: TAlphaColor;
begin
  if AInputFormat = AOutputFormat then
    Exit;

  SrcPitch := PixelFormatBytes[AInputFormat];
  DestPitch := PixelFormatBytes[AOutputFormat];
  if (DestPitch < 1) or (SrcPitch < 1) then
    Exit;

  InData := AInput;
  OutData := AOutput;

  for Index := 0 to APixelCount - 1 do
  begin
    Color := PixelToAlphaColor(InData, AInputFormat);
    AlphaColorToPixel(Color, OutData, AOutputFormat);

    Inc(NativeInt(InData), SrcPitch);
...
var
  ResourceDict: TObjectDictionary<string,TFmxObjectList> = nil;
...
begin
  if ResourceDict = nil then
    ResourceDict := TObjectDictionary<string,TFmxObjectList>.Create([doOwnsValues], 100);
...
var
  ...
  FitScale: Single;
begin
  MR := TRectF.Create(R.Left, R.Top, AParentWidth, AParentHeight);
...
begin
  ...
  if (Control.AdjustType in [TAdjustType.FixedSize, TAdjustType.FixedHeight]) and (LAV.Height > 0)
    ...
    TAlignLayout.VertCenter, TAlignLayout.HorzCenter, TAlignLayout.Center, TAlignLayout.Vertical, 
  ...
  if (Control.AdjustType in [TAdjustType.FixedSize, TAdjustType.FixedWidth]) and (LAV.Width > 0)
    ...
    TAlignLayout.VertCenter, TAlignLayout.HorzCenter, TAlignLayout.Center, TAlignLayout.Horizontal,
    TAlignLayout.Client]) then
  begin
...
begin
  ...
  begin
    Scale.X := Max(MinScale, Rotated.Scale.X);
    Scale.Y := Max(MinScale, Rotated.Scale.Y);
...
begin
  ...
  if Application <> nil then
    Application.UnregisterActionClient(Self);
  Parent := nil;
  TReleasePool.Instance.Release(Self);
end;
...
function TFmxObject.ObjectState: TObjectState;
begin
  Result := [];
end;
...
begin
  if FStyleIndexer <> nil then
    FStyleIndexer.NeedRebuild;
end;
...
function TFmxObject.FindStyleResource(const AStyleLookup: string; const AClone: Boolean = False): TFmxObject;
  ...
  function CloneStyle(const AStyle: TFmxObject): TFmxObject; inline;
  var
    SavedStyleName: string;
  begin
    SavedStyleName := AStyle.StyleName;
    try
      AStyle.FStyleName := string.Empty;
      Result := AStyle.Clone(nil);
    finally
      AStyle.FStyleName := SavedStyleName;
    end;
  end;

begin
  Result := nil;
  if not AStyleLookup.IsEmpty and (FChildren <> nil) and (FChildren.Count > 0) then
  begin
    if FStyleIndexer = nil then
      FStyleIndexer := TStyleIndexer.Create(Self);
    if FStyleIndexer.FindStyleObject(AStyleLookup, Result) and AClone then
      Result := CloneStyle(Result);
  end;
end;
...
begin
  ...
  begin
    AObject.FParent := nil;
    AObject.SetRoot(nil);
    // We use lazy calculation of TfmxObject.Index. However, we need to normalize First index in case,
    // if user doesn't get access to Index. Because FIndex = -1 by default.
    ResetChildrenIndicesSpan(Max(0, AObject.FIndex), FChildren.Count - 1);
...
function TTabList.FindNextTabStop(const ACurrent: IControl; const AMoveForward: Boolean; const AClimb: Boolean): IControl;
  ...
  begin
    ...
    begin
      ...
      if (Result < 0) or (Result >= ASize) then
        Result := -1;
  end;
...
begin
  ...
  try
    ...
    Exclude(VirtualKeyboardStates, TVirtualKeyboardState.Transient);
...
constructor TDisplay.Create(const AIndex: Integer; const APrimary: Boolean; const ABounds, AWorkArea: TRectF);
begin
  Id := 0;
  Index := AIndex;
  Primary := APrimary;
  ...
  WorkArea := AWorkArea;
  Scale := 1;
  PhysicalBounds := Bounds.Round;
  PhysicalWorkarea := Workarea.Round;
end;

function TDisplay.BoundsRect: TRectF;
begin
  Result := Bounds;
end;

function TDisplay.WorkareaRect: TRectF;
begin
  Result := Workarea;
end;
...
begin
  ...
  try
    ...
    begin
      ...
      d(Format('%s times: %s; total=%d', [Tag, Output.ToString(True), Total]));  // do not localize
    end;
  finally
    Token.Free;
    Output.Free;
...
constructor TStyleIndexer.Create(const AStyle: TFmxObject);
begin
  if AStyle = nil then
    raise EArgumentNilException.Create(SArgumentNil);
  FStyle := AStyle;
...
destructor TStyleIndexer.Destroy;
begin
  FreeAndNil(FIndex);
  inherited;
end;

function TStyleIndexer.FindStyleObject(const AStyleLookup: string; var AObject: TFmxObject): Boolean;
begin
  RebuildIfNeeded;
  Result := FIndex.TryGetValue(AStyleLookup.ToLowerInvariant, AObject);
end;

procedure TStyleIndexer.NeedRebuild;
begin
  FreeAndNil(FIndex);
end;

procedure TStyleIndexer.Rebuild;

  procedure Inventory(const ARoot: TFmxObject; const APrefix: string = '');
  var
    Child: TFmxObject;
    I: Integer;
    StyleName: string;
  begin
    if ARoot.Children = nil then
      Exit;

    for I := 0 to ARoot.Children.Count - 1 do
    begin
      Child := ARoot.Children[I];
      StyleName := Child.StyleName;
      if not StyleName.IsEmpty then
      begin
        StyleName := StyleName.ToLowerInvariant;
        FIndex.AddOrSetValue(StyleName, Child);
        if not APrefix.IsEmpty then
          FIndex.AddOrSetValue(APrefix + '.' + StyleName, Child);
      end;

      if Child.SearchInto then
        Inventory(Child, StyleName);
    end;
  end;

begin
  if FIndex = nil then
    FIndex := TDictionary<string, TFmxObject>.Create
  else
    FIndex.Clear;
  Inventory(FStyle);
end;

procedure TStyleIndexer.RebuildIfNeeded;
begin
  // Index was rebuild
  if FIndex <> nil then
    Exit;
...
constructor TReleasePool.Create;
begin
  FPool := TList<TObject>.Create;
end;

destructor TReleasePool.Destroy;
begin
  FreeAndNil(FPool);
  inherited;
end;

class destructor TReleasePool.Destroy;
begin
  FreeAndNil(FInstance);
end;

procedure TReleasePool.DestroyObjects;
var
  Obj: TObject;
begin
  try
    // We are using while cycle for guarding from destroying nested components.
    while FPool.Count > 0 do
    try
      Obj := FPool.ExtractAt(FPool.Count - 1);
      Obj.Free;
    except on E: Exception do
      Application.HandleException(Self);
    end;
  finally
    FIsReleaseInQueue := False;
  end;
end;

procedure TReleasePool.FreeNotification(AObject: TObject);
begin
  FPool.Remove(AObject);
end;

class function TReleasePool.GetInstance: TReleasePool;
begin
  if FInstance = nil then
    FInstance := TReleasePool.Create;

  Result := FInstance;
end;

function TReleasePool.IsMarkedForReleasing(const AObject: TObject): Boolean;
begin
  Result := FPool.Contains(AObject);
end;

procedure TReleasePool.Release(const AObject: TObject);

  procedure CheckUIThread; inline;
  begin
    if TThread.Current.ThreadID <> MainThreadID then
      raise EInvalidOperation.CreateResFmt(@SObjectNonMainThreadUsage, ['Release']);           
  end;

begin
  CheckUIThread;                        
  if IsMarkedForReleasing(AObject) then
    Exit;

  FPool.Add(AObject);
  if AObject is TFmxObject then
  begin
    var Obj := TFmxObject(AObject);
    Obj.AddFreeNotify(Self);
  end;

  if not FIsReleaseInQueue then
  begin
    FIsReleaseInQueue := True;
    TThread.ForceQueue(nil, DestroyObjects);
  end;
end;

initialization
  ClonePropertiesCache := TObjectDictionary<string , TList<TRttiProperty>>.Create([doOwnsValues]);
