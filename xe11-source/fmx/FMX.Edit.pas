...
begin
  ...
  if TPlatformServices.Current.SupportsPlatformService(IFMXExtendedClipboardService, LClipBoardSvc) and LClipBoardSvc.HasText then
  begin
    OldText := Model.Text;
    OldText := OldText.Remove(SelStart, SelLength);

    InsertableString := LClipBoardSvc.GetText;
    ...
    OldText := OldText.Insert(SelStart, InsertableString);
    if MaxLength > 0 then
      OldText := OldText.Substring(0, MaxLength);
    LCaretPosition := SelStart + InsertableString.Length;
    Model.Text := OldText;
