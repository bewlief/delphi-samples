...
type
  ...
  /// <summary>Static class provides methods for converting FMX images into menu Windows bitmaps.</summary>
  TWinMenuHelper = class
  public
    class function BitmapToMenuBitmap(const AIsTopLevel: Boolean; const ABitmap: FMX.Graphics.TBitmap; const AScale: Single = 1): HBITMAP; static;
    class function ImageListToMenuBitmap(const AIsTopLevel: Boolean; const AImages: TCustomImageList; const AIndex: TImageIndex; const AScale: Single = 1): HBITMAP; static;
  end;

function BitmapToWinBitmap(const ABitmap: FMX.Graphics.TBitmap; const ARemoveBlackPixel: Boolean): HBITMAP; overload;
function BitmapToWinBitmap(const ABitmap: FMX.Graphics.TBitmap; const ABackground: TAlphaColor): HBITMAP; overload;
function BitmapToMenuBitmap(const TopLevel: Boolean; const Bitmap: FMX.Graphics.TBitmap): HBITMAP; deprecated 'Use TWinMenuHelper.BitmapToMenuBitmap';
function ImageListToMenuBitmap(const TopLevel: Boolean; const Images: TCustomImageList; const Index: Integer): HBITMAP; overload; deprecated 'Use TWinMenuHelper.ImageListToMenuBitmap';
...
function CheckWinapiHandle(const AHandle: Winapi.Windows.THandle; const AdditionalInfo: string = ''): Winapi.Windows.THandle;
...
function GetSystemMetricsForWindow(const AIndex: Integer; const AHandle: HWnd): Integer;

implementation

uses
  System.SysConst, Winapi.UxTheme, FMX.Consts, FMX.Platform, FMX.Menus, FMX.Utils, FMX.Forms,
...
function BitmapToWinBitmap(const ABitmap: FMX.Graphics.TBitmap; const ARemoveBlackPixel: Boolean): HBITMAP;
var
  BitmapBits: Pointer;
  ...
  C: PAlphaColorRec;
begin
  Result := CreateBitmapHandler(ABitmap, BitmapBits);
  try
    if (BitmapBits <> nil) and ABitmap.Map(TMapAccess.Read, BitmapData) then
      try
        if BitmapData.PixelFormat = TPixelFormat.BGRA then
          for I := 0 to ABitmap.Height - 1 do
            Move(PAlphaColorArray(BitmapData.Data)[I * (BitmapData.Pitch div 4)],
              PAlphaColorArray(BitmapBits)[I * ABitmap.Width], ABitmap.Width * 4)
        else
          for I := 0 to ABitmap.Height - 1 do
            for J := 0 to ABitmap.Width - 1 do
              PAlphaColorArray(BitmapBits)[I * ABitmap.Width + J] := BitmapData.GetPixel(J, I);
        // Remove black pixel that didn't show
        if ARemoveBlackPixel then
          for I := 0 to ABitmap.Height - 1 do
            for J := 0 to ABitmap.Width - 1 do
            begin
              C := @PAlphaColorRecArray(BitmapBits)[I * ABitmap.Width + J];
...
function BitmapToWinBitmap(const ABitmap: FMX.Graphics.TBitmap; const ABackground: TAlphaColor): HBITMAP;
var
  TempBitmap: TBitmap;
  Rect: TRectF;
  Color: TAlphaColor;
  ...
  try
    ...
    begin
      try
        ...
        Rect := TRectF.Create(0, 0, ABitmap.Width, ABitmap.Height);
        TempBitmap.Canvas.DrawBitmap(ABitmap, Rect, Rect, 1, False);
      finally
...
procedure MenuItemMetrics(const TopLevel: Boolean; const AScale: Single; var Themed: Boolean; var BitmapSize, ImageSize: TSize); overload;
...
var
  ItemHeight: Integer;
  TargetDPI: Integer;
  RelatedScale: Extended;
begin
  Themed := IsThemeActive;
  if TOSVersion.Check(10) then
  begin
    TargetDPI := Round(StandardDpi * AScale);
    ItemHeight := GetSystemMetricsForDpi(SM_CYMENU, TargetDPI);
    ImageSize.cx := MultipleOf(GetSystemMetricsForDpi(SM_CXMENUCHECK, TargetDPI), 4);
    ImageSize.cy := MultipleOf(GetSystemMetricsForDpi(SM_CYMENUCHECK, TargetDPI), 4);
  end
  else
  begin
    RelatedScale := AScale / GetScaleFromDC;
    ItemHeight := Round(GetSystemMetrics(SM_CYMENU) * RelatedScale);
    ImageSize.cx := MultipleOf(Round(GetSystemMetrics(SM_CXMENUCHECK) * RelatedScale), 4);
    ImageSize.cy := MultipleOf(Round(GetSystemMetrics(SM_CYMENUCHECK) * RelatedScale), 4);
  end;
...
procedure MenuItemMetrics(const TopLevel: Boolean; var Themed: Boolean; var BitmapSize, ImageSize: TSize); overload;
begin
  MenuItemMetrics(Toplevel, 1, Themed, BitmapSize, ImageSize);
end;

function BitmapToMenuBitmap(const TopLevel: Boolean; const Bitmap: FMX.Graphics.TBitmap): HBITMAP; overload;
begin
  Result := TWinMenuHelper.BitmapToMenuBitmap(TopLevel, Bitmap, 1);
end;

function ImageListToMenuBitmap(const TopLevel: Boolean; const Images: TCustomImageList; const Index: Integer): HBITMAP;
begin
  Result := TWinMenuHelper.ImageListToMenuBitmap(TopLevel, Images, Index);
end;
...
begin
  ...
  try
    ImmSetCompositionString(ImmContext, SCS_SETSTR, PChar(''), 0, PChar(''), 0);
  finally
...
function GetSystemMetricsForWindow(const AIndex: Integer; const AHandle: HWnd): Integer;
  ...
  function CheckPerMonitorV2SupportForWindow(const AHandle: HWnd): Boolean;
  begin
...
begin
  if CheckPerMonitorV2SupportForWindow(AHandle) then
    Result := WinApi.Windows.GetSystemMetricsForDPI(AIndex, GetDPIForWindow(AHandle))
  else
  begin
    RelatedScale := GetWndScale(AHandle) / GetScaleFromDC;
    Result := Round(GetSystemMetrics(AIndex) * RelatedScale);
  end;
end;
...
class function TWinMenuHelper.BitmapToMenuBitmap(const AIsTopLevel: Boolean; const ABitmap: FMX.Graphics.TBitmap;
  const AScale: Single): HBITMAP;
var
...
begin
  ...
  MenuItemMetrics(AIsTopLevel, AScale, Themed, BmpSize, ImgSize);
  ...
  try
    ...
    begin
      try
        ...
        InitSrcDstRects(TSizeF.Create(ABitmap.Width, ABitmap.Height), TSizeF.Create(ImgSize.cx, ImgSize.cy), Src, Dst);
        Dst.Location := TPointF.Create(Dst.Left, Round((BmpSize.cy - Dst.Height) / 2));
        if not Themed then
          Dst.Location := TPointF.Create(0, Dst.Top);
        TempBitmap.Canvas.DrawBitmap(ABitmap, Src, Dst, 1, False);
      finally
        TempBitmap.Canvas.EndScene;
      end;
      Result := BitmapToWinBitmap(TempBitmap, False);
...
class function TWinMenuHelper.ImageListToMenuBitmap(const AIsTopLevel: Boolean; const AImages: TCustomImageList;
  const AIndex: TImageIndex; const AScale: Single): HBITMAP;
var
  ImgSize, BmpSize: TSize;
  Themed: Boolean;
begin
  if (AImages = nil) or (AIndex = -1) then
    Result := 0
  else
  begin
    MenuItemMetrics(AIsTopLevel, AScale, Themed, BmpSize, ImgSize);
    Result := TWinMenuHelper.BitmapToMenuBitmap(AIsTopLevel, AImages.Bitmap(TSizeF.Create(ImgSize.cx, ImgSize.Height), AIndex), AScale);
  end;
end;
