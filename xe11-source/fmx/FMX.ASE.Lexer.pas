...
begin
  ...
  if FToken = TASEToken.EndOfFile then
    Exit(TASEToken.EndOfFile);
  ...
  case CurrentChar of
    ...
    '"':
      begin
        ...
        FToken := TASEToken.String;
      end;
    '{':
      begin
        FToken := TASEToken.OpenBracket;
        Inc(FIdxCurChar);
      end;
    '}':
      begin
        FToken := TASEToken.CloseBracket;
        Inc(FIdxCurChar);
      end;
    ':':
      begin
        FToken := TASEToken.Colon;
        Inc(FIdxCurChar);
      end;
    'a'..'z', 'A'..'Z', '_':
      begin
    ...
    '-', '0'..'9':
      begin
        ...
        begin
          ...
          FToken := TASEToken.Float;
        end
        else
          FToken := TASEToken.Integer;
      end;
    ',':
      if FUseCommaToken then
      begin
        FToken := TASEToken.Comma;
        Inc(FIdxCurChar);
      end;
    else
      Inc(FIdxCurChar);
...
begin
  Assert(Token = TASEToken.KeyWord);

  if NextToken = TASEToken.OpenBracket then
  begin
    LBracketCount := 1;
    repeat
      case NextToken of
        TASEToken.OpenBracket: Inc(LBracketCount);
        TASEToken.CloseBracket: Dec(LBracketCount);
        TASEToken.EndOfFile: Exit;
      end;
    until (Token = TASEToken.EndOfFile) or (LBracketCount = 0);
    // NextToken;
  end
  else
  begin
    while not (NextToken in [TASEToken.KeyWord, TASEToken.EndOfFile, TASEToken.CloseBracket]) do
    {none};
...
var
  ...
  LHash: Integer;
begin
  LHash := THashBobJenkins.GetHashValue(PChar(FString)^, FString.Length * SizeOf(Char), 2004);

  FKeyWord := TKeyWord.kw_UNKNOWN;
...
begin
  for kw := Succ(Low(TKeyWord)) to High(TKeyWord) do
    KEY_WORDS[kw].Hash := THashBobJenkins.GetHashValue(PChar(KEY_WORDS[kw].Str)^,
      KEY_WORDS[kw].Str.Length * SizeOf(Char), 2004);
end;
