...
uses
  ...
  System.SysUtils, FMX.ActnList, FMX.Types, FMX.Controls, FMX.MultiResBitmap, FMX.ImgList, FMX.InertialMovement,
  FMX.BehaviorManager, FMX.Graphics;
...
type
  ...
  public const
    ...
    DefaultSlidingDuration = 0.3;
    DefaultSlidingAnimationType = TAnimationType.In;
    DefaultSlidingInterpoation = TInterpolationType.Linear;
  private
    FContent: TContent;
    ...
    FTransitionFinishedCallback: TProc;
  ...
  protected
    ...
    /// <summary>Forces to load all styles, measure size, build font-glyphs etc.</summary>
    procedure PreloadContent(const Control: TControl);
    /// <summary>Disables automatically unloading style, when control is not visible on screen for tree controls
    /// with root <c>AControl</c>.</summary>
    procedure DisableDisappear(const AControl: TControl);
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    /// <summary>Synchronously changes the current active tab to a new one <c>ATab</c>.</summary>
    /// <remarks>Do not use this method on the Android. Synchronous animation is not supported on the Android.
    /// Instead, use a <c>SetActiveTabWithTransitionAsync</c>.</remarks>
    procedure SetActiveTabWithTransition(const ATab: TTabItem; const ATransition: TTabTransition;
      const ADirection: TTabTransitionDirection = TTabTransitionDirection.Normal);
    /// <summary>Asynchronously changes the current active tab to a new one <c>ATab</c>.</summary>
    /// <remarks>When the animation ends, callback <c>AOnFinish</c> is called.</remarks>
    procedure SetActiveTabWithTransitionAsync(const ATab: TTabItem; const ATransition: TTabTransition;
      const ADirection: TTabTransitionDirection = TTabTransitionDirection.Normal; const AOnFinish: TProc = nil); overload;
    /// <summary>Asynchronously changes the current active tab to a new one <c>ATab</c>.</summary>
    /// <remarks>When the animation ends, event AOnFinish is called</remarks>
    procedure SetActiveTabWithTransitionAsync(const ATab: TTabItem; const ATransition: TTabTransition;
      const ADirection: TTabTransitionDirection = TTabTransitionDirection.Normal; const AOnFinish: TNotifyEvent = nil); overload;
    /// <summary>Finishes current tabs transition, if it is.</summary>
...
uses
  System.Math, System.Math.Vectors, System.Rtti, System.TypInfo, {$IFDEF MACOS}Macapi.CoreFoundation, {$ENDIF}
...
procedure TTabControl.DisableDisappear(const AControl: TControl);
var
  I: Integer;
begin
...
begin
  ...
  TAnimation(Sender).Parent := nil;

  // We don't need to remove animation instance from his handler.
  ...
  if Assigned(FTransitionFinishedCallback) then
  begin
    FTransitionFinishedCallback;
    FTransitionFinishedCallback := nil
  end;
  ...
  try
    ...
    if FContent.ControlsCount > 0 then
      ...
      begin
        ...
        ItemContentRect.TopLeft := ConvertLocalPointTo(LItem, ItemContentRect.TopLeft);
        ItemContentRect.BottomRight := ConvertLocalPointTo(LItem, ItemContentRect.BottomRight);
...
procedure TTabControl.FinishCurrentTabTransition;
var
  TabContent1: TContent;
  TabContent2: TContent;
begin
  if not FTransitionRunning then
    Exit;

  TabContent1 := FTransitionTabs[0].Content;
  TabContent2 := FTransitionTabs[1].Content;

  TAnimator.StopPropertyAnimation(TabContent1, 'Position.X');
  TAnimator.StopPropertyAnimation(TabContent2, 'Position.X');
  FTransitionRunning := False;
end;
...
begin
  ...
  begin
    ...
    {$IFDEF ANDROID}
      SetActiveTabWithTransitionAsync(Tabs[Index], ATransition, LDirection, nil);
    {$ELSE}
...
function TTabControl.ObjectAtPoint(P: TPointF): IControl;
  ...
  begin
    ...
    if Result then
    begin
      LocalRect := AbsoluteToLocal(Control.AbsoluteRect);
...
procedure TTabControl.PreloadContent(const Control: TControl);
var
  Buffer: TBitmap;
begin
...
procedure TTabControl.SetActiveTabWithTransitionAsync(const ATab: TTabItem; const ATransition: TTabTransition;
  const ADirection: TTabTransitionDirection; const AOnFinish: TProc);

  procedure AnimateControlPositionX(AParent: TFmxObject; const NewValue: Integer; const AProgressHandler: TNotifyEvent;
  ...
  case ATransition of
    TTabTransition.Slide:
      begin
        ...
        PreviousTabContent := ActiveTab.Content;
        ActiveTab := ATab;
        NewTab := ATab;
        NewTabContent := ActiveTab.Content;
...
procedure TTabControl.SetActiveTabWithTransitionAsync(const ATab: TTabItem; const ATransition: TTabTransition;
  const ADirection: TTabTransitionDirection; const AOnFinish: TNotifyEvent);
begin
  SetActiveTabWithTransitionAsync(ATab, ATransition, ADirection, procedure begin
