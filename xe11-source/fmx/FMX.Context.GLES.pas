...
type
  ...
  /// <summary>Diagnostic information about current Open GLES state.</summary>
  TGlesDiagnostic = class
  private type
    TErrorBuffer = record
      Errors: TArray<GLEnum>;
      Count: Integer;
      procedure AddError(const AError: Integer);
      procedure Clear;
    end;
  private
    class var FErrorBuffer: TErrorBuffer;
    class constructor Create;
    class procedure CollectErrors;
    class function GetDescription(const AErrors: TArray<GLenum>): string;
  public
    /// <summary>Is specified a error code <c>ACode</c> a warning?</summary>
    /// <remarks>Logic of this method depends on <c>OpenGlErrorReporting</c>.</remarks>
    class function IsWarning(const ACode: Integer): Boolean;
    /// <summary>Returns string presentation of error code. If code is unknown, returns 'Unknown code'.</summary>
    class function TranslateErrorCode(const AErrorCode: GLenum): string;
    /// <summary>Returns list of current errors codes in GLES and resets error flag in GLES.</summary>
    class function GetLastErrors: TArray<GLenum>;
    /// <summary>Raises <c>EContext3DException</c>, if GLES has active errors.</summary>
    class procedure RaiseIfHasError(const ARes: PResStringRec; const AArgs: array of const); overload;
  end;
...
begin
  ...
  repeat
    ...
    if (OpenGlErrorReporting = TOpenGlErrorReporting.Strict) or not IsWarning(Code) then
      FErrorBuffer.AddError(Code);
    Dec(StopLock);
  until (Code = GL_NO_ERROR) or (StopLock <= 0);
end;

class constructor TGlesDiagnostic.Create;
const
  InitialCapacity = 8; // 7 known GLES errors code + 1 reserved unknown code
begin
  SetLength(FErrorBuffer.Errors, InitialCapacity);
  FErrorBuffer.Count := 0;
end;

class function TGlesDiagnostic.GetDescription(const AErrors: TArray<GLenum>): string;
var
  I: Integer;
  ErrorCode: GLenum;
begin
  ...
  begin
    ErrorCode := AErrors[I];
    Result := Result + Format('%d (%s)', [ErrorCode, TranslateErrorCode(ErrorCode)]);
    if I < High(AErrors) then
      Result := Result + ', ';
  end;
end;

class function TGlesDiagnostic.TranslateErrorCode(const AErrorCode: GLenum): string;
begin
  case AErrorCode of
    GL_INVALID_ENUM:
...
class function TGlesDiagnostic.IsWarning(const ACode: Integer): Boolean;
begin
  case ACode of
    GL_INVALID_ENUM,
    GL_INVALID_VALUE,
...
class function TGlesDiagnostic.GetLastErrors: TArray<GLenum>;
var
  Count: Integer;
  Error: Integer;
begin
  ...
  SetLength(Result, 16);
  Error := glGetError;
  while Error <> GL_NO_ERROR do
  begin
    if (Count < Length(Result)) and (OpenGlErrorReporting = TOpenGlErrorReporting.Strict) or not IsWarning(Error) then
    begin
      Result[Count] := Error;
      Inc(Count);
    end;
...
class procedure TGlesDiagnostic.RaiseIfHasError(const ARes: PResStringRec; const AArgs: array of const);
var
  Message: string;
  Error: Integer;
begin
  ...
  while Error <> GL_NO_ERROR do
  begin
    if (OpenGlErrorReporting = TOpenGlErrorReporting.Strict) or not IsWarning(Error) then
    begin
      if Message = '' then
        Message := Format(LoadResString(ARes), AArgs)
      else
        Message := Message + ',';
      Message := Message + Format(' %d (%s)', [Error, TranslateErrorCode(Error)]);
    end;
    Error := glGetError;
  end;
  if Message <> '' then
  begin
  {$IFDEF ANDROID}
    Log.d('[Context Exception]: ' + Message);
  {$ENDIF}
    raise EContext3DException.Create(Message) at ReturnAddress;
  end;
end;
...
begin
  ...
  repeat
    ...
    if Code <> GL_NO_ERROR then
    begin
      Log.d(SOpenGLErrorFlag, [Code, TGlesDiagnostic.TranslateErrorCode(Code)]);

      if (OpenGlErrorReporting = TOpenGlErrorReporting.Strict) or not TGlesDiagnostic.IsWarning(Code) then
...
begin
  ...
  begin
    ...
    TGlesDiagnostic.RaiseIfHasError(@SErrorInContextMethod, ['DoSetContextState']);
  end;
end;
...
begin
  ...
  begin
    ...
    TGlesDiagnostic.RaiseIfHasError(@SErrorInContextMethod, ['DoClear']);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      TGlesDiagnostic.RaiseIfHasError(@SErrorInContextMethod, ['DoCopyToBitmap']);
    end else begin
...
begin
  ...
  begin
    ...
    TGlesDiagnostic.RaiseIfHasError(@SErrorInContextMethod, ['DoDrawPrimitivesBatch']);
  end;
end;
...
begin
  ...
  TGlesDiagnostic.RaiseIfHasError(@SErrorInContextMethod, ['DoDrawPrimitives']);
end;
...
begin
  ...
  begin
    ...
    TGlesDiagnostic.RaiseIfHasError(@SErrorInContextMethod, ['DoSetStencilOp']);
  end;
end;
...
begin
  ...
  begin
    ...
    TGlesDiagnostic.RaiseIfHasError(@SErrorInContextMethod, ['DoSetStencilFunc']);
  end;
end;
...
begin
  ...
  begin
    ...
    TGlesDiagnostic.RaiseIfHasError(@SErrorInContextMethod, ['DoFinalizeTexture']);
  end;
...
begin
  ...
  begin
    ...
    TGlesDiagnostic.RaiseIfHasError(@SCannotUploadTexture, [ClassName]);
  end;
end;
...
const
  GLESHeaderHigh: array [0..24] of byte =
    (Byte('p'), Byte('r'), Byte('e'), Byte('c'), Byte('i'), Byte('s'), Byte('i'), Byte('o'), Byte('n'), Byte(' '),
     ...
     Byte('o'), Byte('a'), Byte('t'), Byte(';'), Byte(#10));

class function TCustomContextOpenGL.BuildShader(AType: Integer; const ACode: TContextShaderCode): Integer;

  function AddPrecisionHeader(const ACode: TContextShaderCode): TContextShaderCode;
  begin
    SetLength(Result, Length(GLESHeaderHigh) + Length(ACode));
    Move(GLESHeaderHigh, Result[0], SizeOf(GLESHeaderHigh));
    Move(ACode[0], Result[Length(GLESHeaderHigh)], Length(ACode)); // Length(ACode) in bytes
  end;

  function BeginsWithDirective(const ACode: TContextShaderCode): Boolean;
  begin
    // #extension GL_OES_EGL_image_external : require
    Result := (Length(ACode) > 0) and (ACode[0] = $23 {'#'});
  end;
...
begin
  ...
  if Valid then
  begin
    if (AType = GL_FRAGMENT_SHADER) and not BeginsWithDirective(ACode) then
      code := AddPrecisionHeader(ACode)
    else
      code := ACode;
    ...
    TGlesDiagnostic.RaiseIfHasError(@SCannotCreateShader, [ClassName]);
  end
end;
...
begin
  ...
  begin
    ...
    TGlesDiagnostic.RaiseIfHasError(@SCannotActivateShaderProgram, [ClassName]);
...
begin
  ...
  begin
    ...
    TGlesDiagnostic.RaiseIfHasError(@SErrorInContextMethod, ['DoFinalizeShader']);
  end;
  Shader.Handle := 0;
end;
...
begin
  ...
  begin
    ...
    TGlesDiagnostic.RaiseIfHasError(@SCannotFindShaderVariable, [Name]);
  end;
end;
...
{ TGlesDiagnostic.TErrorBuffer }

procedure TGlesDiagnostic.TErrorBuffer.AddError(const AError: Integer);
begin
  Inc(Count);
  if Count > Length(Errors) then
    SetLength(Errors, Count);
  Errors[Count - 1] := AError;
end;

procedure TGlesDiagnostic.TErrorBuffer.Clear;
begin
  Count := 0;
end;
