...
begin
  if SameValue(Opacity, 1, Single.Epsilon) then
    Opacity := 0
  else
    Opacity := 1;
end;
...
function TText.TextStored: Boolean;
begin
...
procedure TText.SetPrefixStyle(const Value: TPrefixStyle);
begin
...
procedure TText.DoSetText(const Value: string);
var
  NewText: string;
  LKey: Char;
  LKeyIndex: Integer;
...
begin
  ...
  if (FPrefixStyle = TPrefixStyle.HidePrefix) and TPlatformServices.Current.SupportsPlatformService(IFMXAcceleratorKeyRegistryService, AccelKeyService) then
  begin
...
procedure TImage.SetDisableInterpolation(const Value: Boolean);
begin
...
begin
  ...
  begin
    P := ConvertLocalPointTo(ParentControl, TPointF.Create(X, Y));
