...
begin
  FObjectsCollection.BeginUpdate; // prevent EndUpdate from firing on dead appearance
  FreeAndNil(FObjectsCollection);
  inherited;
end;
...
begin
  if Source.ClassType = FAppearanceClass then
    ...
    if Source is TAppearanceObjectItem then
    begin
      Self.AppearanceClassName := TAppearanceObjectItem(Source).AppearanceClassName;
      Self.AppearanceObjectName := TAppearanceObjectItem(Source).AppearanceObjectName;
      FAppearance.Assign(TAppearanceObjectItem(Source).Appearance);
    end
    else
      inherited;
end;
...
begin
  // The inherited destructor notifies collection about destroying item.
  // And Collection need to know about FAppearance instance for correct removing instance from internal lists.
  // So we invoke destructor first here. 
  inherited;
...
begin
  ...
  else if Action in [TCollectionNotification.cnExtracting, TCollectionNotification.cnDeleting] then
    TAppearanceObjectItem(Item).FOwner.ObjectRemoving(TAppearanceObjectItem(Item).Appearance);
end;

procedure TAppearanceObjectItemCollection.BeginUpdate;
begin
  inherited;
  FDynamicAppearance.BeginUpdate;
end;

procedure TAppearanceObjectItemCollection.EndUpdate;
begin
  inherited;
  FDynamicAppearance.EndUpdate;
end;
