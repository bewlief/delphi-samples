...
begin
  ...
  begin
    CreateWnd;
    SetProp(WindowHandle, MakeIntAtom(ControlAtom), THandle(Self));
    if Parent <> nil then
...
begin
  try
    if (Message.Msg = FM_GET_OBJECT_INSTANCE) and (Message.WParam = GetCurrentProcessId) then
      Message.Result := Winapi.Windows.LRESULT(Self)
    else
      WindowProc(Message);
  except
  ...
  begin
    ...
    begin
      ...
      if TPlatformServices.Current.SupportsPlatformService(IFMXMultiDisplayService, DisplaySvc) then
        try
          MaxTooltipWidth := Round(DisplaySvc.DisplayFromWindow(FWindowHandle).Bounds.Width);
        except
