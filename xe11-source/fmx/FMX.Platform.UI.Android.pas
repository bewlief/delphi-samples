...
uses
  ...
  AndroidApi.JNI.App, AndroidApi.JNI.Os, Androidapi.JNI.Util,
...
begin
  ...
  try
    ...
    begin
      Shift := 0;
      if FLines.Count > 0 then
        for I := 0 to CaretPosition.Y - 1 do
...
begin
  ...
  begin
    ...
    begin
      ...
      FTextView.setNeededToShowSoftKeyboardOnTouch(not ReadOnly and (TVirtualKeyboardState.AutoShow in PlatformAndroid.VirtualKeyboard.VirtualKeyboardState));
...
begin
  if (FText <> AText) or (FCaretPosition.X <> APos) then
  begin
    FText := AText;
    ReadLines;
...
procedure TAndroidMotionManager.HandleMultiTouch;
  ...
  begin
    ...
    for I := 0 to FMotionEvents.Count - 1 do
      Result[I].Location := FMotionEvents[I].Position;
  end;
