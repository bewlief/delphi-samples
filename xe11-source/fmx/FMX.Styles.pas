...
uses
  ...
  FMX.Controls, FMX.BehaviorManager, FMX.Graphics {$IFDEF MSWINDOWS}, FMX.Forms{$ENDIF};
...
class function TStyleStreaming.IsSupportedPlatformTarget(const PlatformTarget: string): Boolean;
begin
  Result := True;
...
procedure TStyleManager.TStyleManagerNotification.FreeNotification(AObject: TObject);
var
  Styles: TDictionary<string, TFmxObject>;
  Pair: TPair<string, TFmxObject>;
begin
  inherited;
  Styles := TStyleManager.FStyleResources;
  if Styles = nil then
    Exit;
  ...
  for Pair in Styles do
    ...
    begin
      Styles.Remove(Pair.Key);
      Break;
    end;
end;

class function TStyleManager.StyleManagerNotification: TStyleManagerNotification;
begin
  if FStyleManagerNotification = nil then
    FStyleManagerNotification := TStyleManagerNotification.Create(nil);
  Result := FStyleManagerNotification;
end;
...
begin
  ...
  FSelections.Free;
  FreeAndNil(FStyleManagerNotification);
end;

class function TStyleManager.ActiveStyleForScene(const AScene: IInterface): TFmxObject;

  function FindSystemStyleFromBehaviorService(const AContext: TFmxObject): TFmxObject;
  var
    Service: IInterface;
    StyleBehavior: IStyleBehavior;
  begin
    Service := TBehaviorServices.Current.GetBehaviorService(IStyleBehavior, AContext);
    if Supports(Service, IStyleBehavior, StyleBehavior) then
      StyleBehavior.GetSystemStyle(AContext, Result)
    else
      Result := nil;
  end;

  function IsDesignTime(const AScene: IScene): Boolean;
  begin
    Result := csDesigning in AScene.GetObject.ComponentState;
  end;
...
var
  ...
  Context: TFmxObject;
{$IFDEF MSWINDOWS}
  SceneObject: TFmxObject;
{$ENDIF}
begin
  if not Supports(AScene, IScene, Scene) then
  begin
    Result := FindSystemStyleFromBehaviorService(nil);
    Exit;
  end;

{$IFDEF MSWINDOWS}
  if IsDesignTime(Scene) then
  begin
    SceneObject := Scene.GetObject;

    if SceneObject is TCommonCustomForm then
      Context := SceneObject // Form
    else if (SceneObject.Root <> nil) and (SceneObject.Root.GetObject is TCommonCustomForm) then
      Context := SceneObject.Root.GetObject // Viewport, Layer3D and etc
    else
      Context := SceneObject;
  end
  else
{$ENDIF}
    Context := Scene.GetObject;

  Result := FindSystemStyleFromBehaviorService(Context);
end;

class procedure TStyleManager.EnumStyleResources(Proc: TStyleResourceEnumProc);
var
  Pair: TPair<string, TOSPlatform>;
begin
  if FPlatformResources = nil then
    Exit;

  for Pair in FPlatformResources do
...
var
  ...
  StylePair: TPair<string, TFmxObject>;
begin
  StyleResource := StyleResourceForContext(Context);
  ...
  begin
    ...
    for StylePair in FStyleResources do
      if StylePair.Value = Style then
      begin
        Style.RemoveFreeNotify(StyleManagerNotification);
        FStyleResources.Remove(StylePair.Key);
        Break;
      end;
...
begin
  ...
  while (I < AObject.ChildrenCount) and not Founded do
    ...
    begin
      Founded := True;
      DescriptorTmp := TStyleDescription(AObject.Children[I]);
    end
    else
    begin
...
class function TStyleManager.GetStyleDescriptionForControl(const AObject: TFmxObject): TStyleDescription;

  function GetActiveStyleForScene(const AScene: IScene): TFmxObject;
  begin
    if (AScene <> nil) and (AScene.StyleBook <> nil) then
      Result := AScene.StyleBook.Style
    else
      Result := ActiveStyleForScene(AScene);
  end;
...
begin
  ...
  begin
    ...
    begin
      Scene := TControl(AObject).Scene;
      Resources := GetActiveStyleForScene(Scene);
