...
function TAVMedia.GetZOrderManager: TiOSZOrderManager;
var
...
begin
  if (ZOrderManager <> nil) and (FView <> nil) then
  begin
    ZOrderManager.AddOrSetLink(Control, FView, nil);
...
function TAVMedia.QueryInterface(const IID: TGUID; out Obj): HResult;
begin
  Result := E_NOTIMPL;
  if (FView <> nil) and (FView.QueryInterface(IID, Obj) = S_OK) then
    Exit(S_OK);
