...
constructor TPopupDialog.Create;
  ...
  begin
    ...
    Constraint := TNSLayoutConstraint.Wrap(TNSLayoutConstraint.OCClass.constraintWithItem(NSObjectToID(FUIToolBar), NSLayoutAttributeTop,
      NSLayoutRelationEqual, NSObjectToID(FUIContainerView), Attribute, 1, 0));
    Constraint.setActive(True);
...
begin
  ...
  if TOSVersion.Check(13) then
    SystemBackgroundColor := TUIColor.OCClass.tertiarySystemBackgroundColor
  else
    SystemBackgroundColor := iOSapi.UIKit.TUIColor.OCClass.whiteColor;
  FUIContainerView.setBackgroundColor(iOSapi.UIKit.TUIColor.Wrap(SystemBackgroundColor));
...
begin
  ...
  if TOSVersion.Check(13, 4) then
    FDateTimePicker.setPreferredDatePickerStyle(UIDatePickerStyleWheels);
...
begin
  // selectRow cannot accept -1 value
  FListBoxPicker.selectRow(Max(0, Value), 0, False);
end;
