...
type
  ...
  private type
    ...
    TClickIndex = (Single, Double);
    TCellClickInfo = record
      [Weak] Column: TColumn;
      Row: Integer;
    end;
  private
    ...
    FCellClickArray: array [TClickIndex] of TCellClickInfo;
    ...
    procedure StartSelectTimer(const ACol, ARow: Integer; const AbsolutePoint: TPointF; const DblTap: Boolean);
    procedure ClearCellClicks;
  protected
    ...
    function CreateDefaultCell: TStyledControl; virtual;
...
uses
  ...
  FMX.Styles.Objects, FMX.Presentation.Factory, FMX.Platform, FMX.Utils, FMX.TextLayout, FMX.StdActns, FMX.Objects,
  FMX.Ani, FMX.ExtCtrls, FMX.Edit, FMX.DateTimeCtrls, FMX.NumberBox, FMX.Text, FMX.ImgList;
...
type
  ...
  public type
    TTimerAction = (ShowEditor, Exit, SelectCell, MouseClick, DblTap);
...
{$REGION 'TNumberCell'}
type
  TNumberCell = class(TDefaultEditor)
  protected
    function DefineModelClass: TDataModelClass; override;
    procedure KeyDown(var Key: Word; var KeyChar: Char; Shift: TShiftState); override;
  end;

  TNumberModel = class(TCustomEditModel)
  protected
...
function TNumberModel.GetTextSettingsClass: TTextSettingsInfo.TCustomTextSettingsClass;
begin
...
function TNumberCell.DefineModelClass: TDataModelClass;
begin
  Result := TNumberModel;
end;

procedure TNumberCell.KeyDown(var Key: Word; var KeyChar: Char;
  Shift: TShiftState);
begin
  if (KeyChar <> #0) and CharInSet(KeyChar, ['e', 'E', FormatSettings.DecimalSeparator]) and
     Text.Contains(KeyChar) then
    KeyChar := #0;
  inherited;
end;
{$ENDREGION}
...
begin
  ...
  begin
    ...
    else if not (Value.TryAsType<Double>(ClampValue) or
      (TCustomNumberColumn.TryValueToString(Value, TNumValueType.Float, S) and TryStrToFloat(S, ClampValue))) then
      Exit;
...
begin
  ...
  begin
    ...
    ImgRect := FColumn.RectPlaceIntoBounds(ImgRect, Bounds).SnapToPixel(Scale, False);
...
var
  Background, ParentLayout: TControl;
  BitmapLink: TBitmapLink;
begin
  inherited;
  ...
  begin
    ...
    begin
      ...
      case FColumn.HorzAlign of
        TTextAlign.Center:
          FLayout.Align := TAlignLayout.HorzCenter;
        TTextAlign.Leading:
          FLayout.Align := TAlignLayout.Left;
        TTextAlign.Trailing:
          FLayout.Align := TAlignLayout.Right;
      end;
      if Background is TCheckStyleObject then
      begin
        ParentLayout := FLayout.ParentControl;
        BitmapLink := TCheckStyleObject(Background).HotLink.LinkByScale(1, False);
        if (ParentLayout <> nil) and (BitmapLink <> nil) and (BitmapLink.Scale > 0) then
        begin
          FLayout.Width := Ceil(BitmapLink.SourceRect.Width / BitmapLink.Scale);
          ParentLayout.Height := Ceil(BitmapLink.SourceRect.Height / BitmapLink.Scale);
          ParentLayout.Margins.Rect := FColumn.Padding.Rect;
          case FColumn.DefaultTextSettings.VertAlign of
            TTextAlign.Center:
              ParentLayout.Align := TAlignLayout.VertCenter;
            TTextAlign.Leading:
              ParentLayout.Align := TAlignLayout.Top;
            TTextAlign.Trailing:
              ParentLayout.Align := TAlignLayout.Bottom;
          end;
        end;
      end;
    end;
...
type
  ...
  private
    [Weak] FStyledGrid: TStyledGrid;
    [Weak] FColumn: TCustomNumberColumn;
    FContent: TControl;
    FBackground: TControl;
    FOldMargins: TRectF;
    { ICellControl }
...
function TCurrencyCell.GetAdjustType: TAdjustType;
begin
...
begin
  ValidateInheritance(Value, TCustomNumberColumn, True);
  FColumn := TCustomNumberColumn(Value);
  if FColumn <> nil then
  begin
    if FColumn.DecimalDigits >= 0 then
      DecimalDigits := FColumn.DecimalDigits;
    ValueType := FColumn.ValueType;
  end;
end;
...
begin
  ...
  else if (FColumn <> nil) and
    TCustomNumberColumn.TryValueToString(Value, FColumn.ValueType, S, FColumn.DecimalDigits) then
  begin
...
begin
  inherited;
  ClearCellClicks;
  FCellControl := CreateDefaultCell;
  ...
  Touch.DefaultInteractiveGestures := Touch.DefaultInteractiveGestures + [TInteractiveGesture.DoubleTap,
...
procedure TStyledGrid.ClearCellClicks;
var
  I: TClickIndex;
begin
  for I := Low(TClickIndex) to High(TClickIndex) do
  begin
    FCellClickArray[I].Row := -1;
    FCellClickArray[I].Column := nil;
  end;
end;
...
function TStyledGrid.CreateDefaultCell: TStyledControl;
begin
...
procedure TStyledGrid.InternalDrawCells(const Canvas: TCanvas; const AFrontLayer: Boolean;
  ...
  begin
    NeedBackGround := TGridOption.AlternatingRowBackground in Model.Options;
    for X := LeftColumn to RightColumn do
    if ((Columns[X].Enabled = AFrontLayer) or NeedBackGround) and Columns[X].EffectiveVisible and (Columns[X].Width > 0) then
    begin
          ...
          for Y := TopRow to BottomRow do
            ...
            begin
              ...
              begin
                ...
                BkState := State;
                if (X = Model.Col) and not (TGridPart.EditorShowing in FUpdating) then
                begin
  ...
  begin
    ...
    begin
      ...
      for X := LeftColumn to RightColumn + 1 do
      begin
...
begin
  ...
  begin
    ...
    begin
      Model.ReduceCache(LeftColumn, TopRow, RightColumn, BottomRow);
      for Y := TopRow to BottomRow do
        for X := LeftColumn to RightColumn do
...
var
  ...
  I: Integer;
begin
  if (ColumnCount > 0) and (RowCount > 0) and (RightColumn >= 0) and (BottomRow >= 0) then
  begin
...
begin
  if (FInplaceEdit is TControl) and
     (Model.Row >= 0) and (Model.Row < RowCount) and
     (Model.Col >= 0) and (Model.Col < ColumnCount) then
...
begin
  ...
  FTopRow := EnsureRange(FTopRow, 0, RowCount - 1);
...
var
  Height: Integer;
  SrcRect, DstRect, TmpRect: TRectF;
  Scale: Single;
  B: Boolean;
begin
  Scale := Scene.GetSceneScale;
  ...
  begin
    ...
    try
      ...
      TmpRect := SrcRect.PlaceInto(TRectF.Create(0, 0, Height, Height));
      FBitmaps[Index].SetSize(Ceil(TmpRect.Width), Ceil(TmpRect.Height));
      if FBitmaps[Index].HandleAllocated and FBitmaps[Index].Canvas.BeginScene then
      try
...
function TStyledGrid.GetRightColumn: Integer;
var
  I: Integer;
begin
...
begin
  ...
  begin
    if (Message.Value >= 0) and (Message.Value < Length(FColInfo)) then
      OldWidth := FColInfo[Message.Value].Width
    else
      OldWidth := -1;
    SetLength(FColInfo, 0);
...
procedure TStyledGrid.MMContentChanged(var Message: TDispatchMessage);
  ...
  procedure ValidateCellClickColumn(var Info: TCellClickInfo);
  var
    I: Integer;
  begin
    ...
    begin
      for I := 0 to ColumnCount - 1 do
        if Info.Column = Columns[I] then
          Exit;
      Info.Column := nil;
    end;
  end;

  function CalculateContentSize: TSizeF;
  var
    LastCellRect: TRect;
  begin
    LastCellRect := CellRect(Max(0, ColumnCount), Max(0, RowCount));
    Result := TSizeF.Create(LastCellRect.Right, LastCellRect.Bottom);
  end;
...
begin
  ...
  begin
    for I := Low(TClickIndex) to High(TClickIndex) do
      ValidateCellClickColumn(FCellClickArray[I]);
    ...
    ViewportRect := TRectF.Create(Model.ViewportPosition, Model.ViewportSize.Width, Model.ViewportSize.Height);
    LCellRect := CellRect(Max(0, Model.Col), Max(0, Model.Row));
    LContentSize := CalculateContentSize;
    // We have to adjust content offset only if viewport position is placed out of new content size.
    if (ViewportRect.Bottom > LContentSize.Height) and (ViewportRect.Right > LContentSize.Width) then
      SetTopLeft(Model.Col, Model.Row);
  end;
  UpdateContent;
end;
...
begin
  ...
  else if (Column is TFloatColumn) or (Column is TIntegerColumn) then
    Result := TNumberCell;
end;
...
begin
  ...
  if LeftColumn <= Min(RightColumn, ColumnCount) then
  begin
    Message.Value.TopLeft.Col := LeftColumn;
    Message.Value.BottomRight.Col := Min(RightColumn, ColumnCount);
  end;
end;
...
begin
  Result := HasTouchScreen and not (csDesigning in ComponentState);
end;

procedure TStyledGrid.PerformClick;
begin
  try
    if (FCellClickArray[TClickIndex.Single].Column <> nil) and (FCellClickArray[TClickIndex.Double].Column = nil) then
      CellClick(FCellClickArray[TClickIndex.Single].Column, FCellClickArray[TClickIndex.Single].Row);
  finally
    FCellClickArray[TClickIndex.Single].Column := nil;
    FCellClickArray[TClickIndex.Single].Row := -1;
  end;
  try
    if FCellClickArray[TClickIndex.Double].Column <> nil then
      CellDblClick(FCellClickArray[TClickIndex.Double].Column, FCellClickArray[TClickIndex.Double].Row);
  finally
    FCellClickArray[TClickIndex.Double].Column := nil;
    FCellClickArray[TClickIndex.Double].Row := -1;
  end;
end;
...
begin
  ...
  begin
    ...
    try
      ...
      if (LCol >= 0) and (LCol < ColumnCount) then
        if Columns[LCol].Enabled then
        begin
          ...
          if TSelectTimer.TTimerAction.SelectCell in TSelectTimer(Sender).FTimerActions then
            Model.SelectCell(LCol, LRow);
          PerformClick;
          if TSelectTimer.TTimerAction.DblTap in TSelectTimer(Sender).FTimerActions then
          begin
            if (LCol = Model.Col) and (LRow = Model.Row) and not EditorMode then
              ShowEditor;
          end
          else if TSelectTimer.TTimerAction.ShowEditor in TSelectTimer(Sender).FTimerActions then
            ShowEditor;
      ...
      TSelectTimer(Sender).Free;
    end;
  end;
end;

procedure TStyledGrid.StartSelectTimer(const ACol, ARow: Integer; const AbsolutePoint: TPointF; const DblTap: Boolean);
var
  LTimer: TSelectTimer;
  TimerActions: TSelectTimer.TTimerActions;
  Coord: TGridModel.TCoord;
begin
  ...
  if DblTap then
    TimerActions := TimerActions + [TSelectTimer.TTimerAction.DblTap]
  else if (ACol = Model.Col) and (ARow = Model.Row) and not EditorMode then
    TimerActions := TimerActions + [TSelectTimer.TTimerAction.ShowEditor];
  if (ACol >= 0) and (ARow >= 0) then
  begin
    FMouseDown := True;
    FMouseDownPos := AbsolutePoint;
    if not (TGridPart.EditorShowing in FUpdating) then
    begin
      Coord.Col := ACol;
      Coord.Row := ARow;
      LTimer := TSelectTimer.Create(Self, Coord, AbsolutePoint, ClickTimerProc, TimerActions);
      FUpdating := FUpdating + [TGridPart.EditorShowing];
      if DblTap or (AniCalculations.TouchTracking = []) then
        LTimer.Interval := 10;
      LTimer.Enabled := True;
    end;
  end;
...
procedure TStyledGrid.CMGesture(var EventInfo: TGestureEventInfo);
var
  P: TPointF;
  LCol, LRow: Integer;
  TimerActions: TSelectTimer.TTimerActions;
begin
  ...
  begin
    ...
    case EventInfo.GestureID of
      igiDoubleTap:
      begin
        P := EventInfo.Location;
        CellByPoint(P.X, P.Y, LCol, LRow);
        if InRange(LCol, 0, ColumnCount - 1) and InRange(LRow, 0, RowCount - 1) and (Columns[LCol] <> nil) then
        begin
          EventInfo.GestureID := 0;
          FCellClickArray[TClickIndex.Double].Column := Columns[LCol];
          FCellClickArray[TClickIndex.Double].Row := LRow;
          StartSelectTimer(LCol, LRow, P, True);
        end;
      end;
    end;
  end;
...
begin
  ...
  begin
    ...
    if InRange(LCol, 0, ColumnCount - 1) and InRange(LRow, 0, RowCount - 1) and (Columns[LCol] <> nil) then
    begin
      if (FCellClickArray[TClickIndex.Single].Column <> nil) or (ssDouble in Shift) then
        ClickIndex := TClickIndex.Double
      else
        ClickIndex := TClickIndex.Single;
      FCellClickArray[ClickIndex].Column := Columns[LCol];
      FCellClickArray[ClickIndex].Row := LRow;
      StartSelectTimer(LCol, LRow, P, False);
    end;
  end;
  inherited;
end;
...
begin
  ...
  begin
    P := LocalToAbsolute(TPointF.Create(X, Y));
    if not FMouseDownPos.EqualsTo(P, RowHeight / 4 * Scene.GetSceneScale) and (AniCalculations.TouchTracking <> []) then
    begin
      FUpdating := FUpdating - [TGridPart.EditorShowing];
      ClearCellClicks;
    end;
...
procedure TStyledGrid.CellClick(const Column: TColumn; const Row: Integer);
begin
  if (Model <> nil) and Assigned(Model.OnCellClick) then
    Model.OnCellClick(Column, Row);
end;

procedure TStyledGrid.CellDblClick(const Column: TColumn; const Row: Integer);
begin
...
begin
  ...
  begin
    if ContentLayout <> nil then
    begin
      R := ContentLayout.AbsoluteToLocal(Message.Value.AbsoluteRect);
...
begin
  if not FEditorMode and CanEdit and (Content <> nil) and (FInplaceEdit = nil) and
    ((FUpdating - [TGridPart.EditorShowing]) = []) then
  begin
