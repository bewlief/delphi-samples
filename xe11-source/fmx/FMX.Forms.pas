...
procedure TCommonCustomForm.Show;
  ...
  begin
    Result := AValue;
    if Result.Top < Screen.DesktopRect.Top then
      Result.Offset(0, Screen.DesktopRect.Top - Result.Top)
    else if Result.Bottom > Screen.DesktopRect.Bottom then
      Result.Offset(0, Screen.DesktopRect.Bottom - Result.Bottom);

    if Result.Left < Screen.DesktopRect.Left then
      Result.Offset(Screen.DesktopRect.Left - Result.Left, 0)
    else if Result.Right > Screen.DesktopRect.Right then
      Result.Offset(Screen.DesktopRect.Right - Result.Right, 0);
  end;
        ...
        begin
          ...
          case LPosition of
            ...
            TFormPosition.DesktopCenter:
              R := TRectF.Create(Screen.GetDesktopCenterRect(TSizeF.Create(Width, Height)));
          end;
          // set bounds
          case LPosition of
            ...
            TFormPosition.DefaultPosOnly:
              NewBounds := TRectF.Create(FDefaultWindowRect.TopLeft, FBounds.Width, FBounds.Height);
            TFormPosition.DefaultSizeOnly:
              NewBounds := TRectF.Create(FBounds.TopLeft, FDefaultWindowRect.Width, FDefaultWindowRect.Height);
            TFormPosition.ScreenCenter,
            TFormPosition.MainFormCenter,
            TFormPosition.OwnerFormCenter:
            begin
              NewBounds := TRectF.Create(TPointF.Create(R.Left + (R.Width - Width) / 2, R.Top + (R.Height - Height) / 2),
            ...
            NewBounds := FDefaultWindowRect;
          end;
          SetBoundsF(NewBounds);
        end;
        if FFullScreen then
        begin
          SetFullScreen(True);
...
begin
  if (Shift = [ssAlt]) and TPlatformServices.Current.SupportsPlatformService(IFMXAcceleratorKeyRegistryService, AccelKeySvc) and
