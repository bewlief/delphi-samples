...
uses
  ...
  FMX.ScrollBox.iOS, FMX.Memo.Types, FMX.Controls.Model, FMX.Text;
...
uses
  System.UITypes, System.Classes, System.Math, System.Messaging, MacApi.Helpers,
  {$IFDEF MACOS}Macapi.CoreFoundation,{$ENDIF} iOSapi.CoreText, FMX.Presentation.Factory, FMX.Consts, FMX.Graphics,
  FMX.Helpers.iOS, iOSapi.CocoaTypes;
...
procedure TiOSNativeMemo.HideCaret;
begin
  Model.Caret.Hide;
...
procedure TiOSNativeMemo.ResetFocus;
begin
  inherited;
  HideCaret;
end;

procedure TiOSNativeMemo.SetFocus;
begin
  inherited;
  ShowCaret;
end;

procedure TiOSNativeMemo.ShowCaret;
begin
...
begin
  if FNativeMemo.Model.MaxLength > 0 then
    Result := (FNativeMemo.View.Text.Length - shouldChangeTextInRange.length + replacementText.length) <=
      NSUInteger(FNativeMemo.Model.MaxLength)
  else
    Result := True;
...
type
  ...
  TiOSThemedNativeMemo = class(TiOSNativeMemo)
  private
...
{ TiOSThemedNativeMemo }

constructor TiOSThemedNativeMemo.Create;
begin
  inherited;
...
destructor TiOSThemedNativeMemo.Destroy;
begin
...
procedure TiOSThemedNativeMemo.PMApplyStyleLookup(var AMessage: TDispatchMessage);
var
...
procedure TiOSThemedNativeMemo.PMNeedStyleLookup(var AMessage: TDispatchMessage);
begin
  FIsStyledFontLoaded := False;
end;

procedure TiOSThemedNativeMemo.StyleChangedHandler(const Sender: TObject; const Msg: TMessage);
begin
  FIsStyledFontLoaded := False;
end;

initialization
  TPresentationProxyFactory.Current.Register(TMemo, TControlType.Platform, TiOSPresentationProxy<TiOSThemedNativeMemo>);
finalization
  TPresentationProxyFactory.Current.Unregister(TMemo, TControlType.Platform, TiOSPresentationProxy<TiOSThemedNativeMemo>);
end.
