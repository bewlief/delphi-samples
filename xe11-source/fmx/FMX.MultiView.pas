...
{ TCustomMultiView  }
  ...
  private
    ...
    FSavedMasterButtonVisible: Boolean;
    FTargetControl: TControl;
    FBorderOptions: TMultiViewBorderOptions;
    ...
    procedure SetBorderOptions(const Value: TMultiViewBorderOptions);
    ...
    function AreBorderOptionsStored: Boolean;
...
begin
  Result := HasPresenter and Presenter.Opened;
end;
...
begin
  ...
  FSavedMasterButtonVisible := True;

  // Display settings
  FBorderOptions := TMultiViewBorderOptions.Create(Self, DoOptionsChanged);
...
procedure TCustomMultiView.DoAnotherMultiViewDisplayed(const Sender: TObject; const M: TMessage);
  ...
  function HasSameRoot: Boolean;
  begin
    Result := (Sender is TControl) and (Root = TControl(Sender).Root);
  end;

begin
  if (csDesigning in ComponentState) and (Sender <> Self) and HasSameRoot and HasPresenter and Presenter.NeedHideInDesignTime then
    HideMaster;
end;
...
procedure TCustomMultiView.EnabledChanged;
begin
  inherited;
  if HasPresenter then
    Presenter.EnabledChanged;
end;
...
function TCustomMultiView.AreBorderOptionsStored: Boolean;
begin
  Result := not FBorderOptions.AreDefaultValues;
end;
...
begin
  inherited;
  if HasPresenter and not Presenter.Opened then
  begin
...
begin
  if HasPresenter and (ComponentState * [csLoading, csDestroying] = []) and not Presenter.IsInternalRealign then
  begin
    BeginUpdate;
    try
      Presenter.Realign;
...
procedure TCustomMultiView.SetBorderOptions(const Value: TMultiViewBorderOptions);
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      FMasterButton.Visible := FSavedMasterButtonVisible;
    end;
    FMasterButton := Value;
    if HasMasterButton then
    begin
      FSavedMasterButtonVisible := FMasterButton.Visible;
...
procedure TMultiViewPresentation.EnabledChanged;
begin
end;

procedure TMultiViewPresentation.EndInternalRealign;
begin
  if IsInternalRealign then
    Dec(FInternalRealign);
end;
...
function TMultiViewPresentation.GetParentHeight: Single;
begin
  if MultiView.ParentControl <> nil then
    Result := MultiView.ParentControl.Height
  else if MultiView.Parent is TCommonCustomForm then
    Result := TCommonCustomForm(MultiView.Parent).ClientHeight
  else
    Result := 0;
end;
...
procedure TMultiViewPresentation.BeginInternalRealign;
begin
  Inc(FInternalRealign);
end;

function TMultiViewPresentation.IsInternalRealign: Boolean;
begin
  Result := FInternalRealign > 0;
end;

function TMultiViewPresentation.Moving: Boolean;
begin
  Result := TPresentationState.Moving in FState;
end;
