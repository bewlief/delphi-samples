...
uses
  ...
  FMX.Edit, FMX.Types, FMX.Controls.Model, FMX.Controls;
...
uses
  Macapi.CoreFoundation, Macapi.Helpers, System.UITypes, System.Messaging, iOSapi.CoreImage, iOSapi.CoreText,
...
begin
  Result := UIView(super).canBecomeFirstResponder and Edit.CanFocus and Edit.HitTest;
end;
...
procedure TiOSNativeEdit.HideCaret;
begin
  Model.Caret.Hide;
...
procedure TiOSNativeEdit.MMSelLengthChanged(var AMessage: TDispatchMessageWithValue<Integer>);
begin
...
procedure TiOSNativeEdit.ResetFocus;
begin
  inherited;
  HideCaret;
end;
...
procedure TiOSNativeEdit.SetFocus;
begin
  inherited;
  ShowCaret
end;

procedure TiOSNativeEdit.ShowCaret;
begin
...
begin
  ...
  begin
    StartPosition := View.positionFromPosition(View.beginningOfDocument, Model.CaretPosition);
    EndPosition := View.positionFromPosition(StartPosition, 0);
  end;
...
begin
  ...
  begin
    if (replacementString.length = 0) and (shouldChangeCharactersInRange.length > 0) then
      Result := FNativeEdit.PressKey(vkBack, #0, [])
    else if (replacementString.length = 1) and (shouldChangeCharactersInRange.length = 0) then
    begin
      Shift := [];
      if replacementString.characterAtIndex(0) <> LowerCase(replacementString.characterAtIndex(0)) then
        Shift := [ssShift];
      Result := FNativeEdit.PressKey(0, replacementString.characterAtIndex(0), Shift);
    end
    else if replacementString.length > 1 then
...
begin
  if FNativeEdit.PressKey(vkReturn, #0, []) then
  begin
...
type
  ...
  TiOSThemedNativeEdit = class(TiOSNativeEdit)
  private
...
{ TiOSThemedNativeEdit }

constructor TiOSThemedNativeEdit.Create;
begin
  inherited;
...
destructor TiOSThemedNativeEdit.Destroy;
begin
  TMessageManager.DefaultManager.Unsubscribe(TStyleChangedMessage, StyleChangedHandler);
  inherited;
end;

procedure TiOSThemedNativeEdit.PMApplyStyleLookup(var AMessage: TDispatchMessage);
var
...
begin
  ...
  begin
    ...
    begin
      ...
      StyleObject := Style.FindStyleResource('foreground'); // Do not localize
      if StyleObject is TBrushObject then
        Model.TextSettingsInfo.DefaultTextSettings.FontColor := TBrushObject(StyleObject).Brush.Color;
    end;
...
procedure TiOSThemedNativeEdit.PMNeedStyleLookup(var AMessage: TDispatchMessage);
begin
  FIsStyledFontLoaded := False;
end;

procedure TiOSThemedNativeEdit.StyleChangedHandler(const Sender: TObject; const Msg: TMessage);
begin
  FIsStyledFontLoaded := False;
end;

initialization
  TPresentationProxyFactory.Current.Register(TEdit, TControlType.Platform, TiOSPresentationProxy<TiOSThemedNativeEdit>);
finalization
  TPresentationProxyFactory.Current.Unregister(TEdit, TControlType.Platform, TiOSPresentationProxy<TiOSThemedNativeEdit>);
end.
