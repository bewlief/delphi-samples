...
begin
  if not PermissionsService.IsEveryPermissionGranted(
    ...
    raise EPermissionException.CreateFmt(SRequiredPermissionsAreAbsent, ['CAMERA, WRITE_EXTERNAL_STORAGE']);

  FParams := AParams;
  FParams.RequiredResolution := ApproximateAdmissibleImageSize(FParams.RequiredResolution);
  MainActivity.getMediaLibrary.takeImageFromCamera(FParams.RequiredResolution.Width, FParams.RequiredResolution.Height,
