...
{$ENDIF}
  System.SysUtils, System.RTLConsts, System.Math.Vectors, System.Math, System.Character, FMX.Utils, FMX.Consts,
  FMX.Platform;
...
function TMultiResBitmap.GetDefaultSizeKind: TSizeKind;
begin
  Result := TSizeKind.Source;
end;
...
var
  ...
  begin
    ...
    begin
      Index := S.Substring(I + 1).ToInteger;
      S := S.Substring(0, I + 1);
    end;
  end;

begin
  OriginalName := AName.Trim;
  Result := OriginalName;
  if Collection <> nil then
  begin
    ...
    for I := 0 to Collection.Count - 1 do
      ...
      begin
        S := TCustomSourceItem(Collection.Items[I]).Name;
    ...
    begin
      ...
      Result := Result + MaxIndex.ToString;
    end
    else
      Result := OriginalName;
  end;
...
begin
  ...
  try
    ...
    try
      if (CurrentIndex = -1) and not SourceName.Trim.IsEmpty then
        Result.Name := SourceName.Trim;
...
procedure TSourceCollection.Update(Item: TCollectionItem);
  ...
  begin
    ImageList.InvalidateDestination(Item.Name);
    if string.Compare(Item.Name, Item.FOldName, True) <> 0 then
      ImageList.InvalidateDestination(Item.FOldName);
  end;
var
  I: Integer;
begin
...
begin
  LValue := Value.Trim;
  if string.Compare(FName, LValue, True) <> 0 then
  begin
    FName := LValue;
    FMultiResBitmapIsValid := False;
    Changed(False);
...
begin
  ...
  begin
    ...
    for I := 0 to ImageList.Source.Count - 1 do
      if string.Compare(Name, ImageList.Source[I].Name, True) = 0 then
      begin
...
var
  I: Integer;
  Tmp: TCustomDestinationItem;
begin
  FreeAndNil(FLayers);
  if (Destination <> nil) and (Destination.ImageList <> nil) and
    not (csDestroying in Destination.ImageList.ComponentState) then
  begin
    for I := Index to Destination.Count - 1 do
      if not Destination.ImageList.FChangedList.TryGetValue(I, Tmp) then
        Destination.ImageList.FChangedList.Add(I, nil);
    Destination.ImageList.Change;
  end;
  inherited;
end;
...
begin
  ...
  FChangedList := TDictionary<Integer, TCustomDestinationItem>.Create;
{$IF Defined(ANDROID) OR Defined(IOS)}
  // Mobile platforms have limited resources. Therefore, we need to use them sparingly for avoiding out of memory.
  CacheSize := 32;
{$ELSE}
  CacheSize := 256;
{$ENDIF}
...
begin
  for I := 0 to Destination.Count - 1 do
    ...
    begin
      if not FChangedList.TryGetValue(I, Tmp) then
        FChangedList.Add(I, Destination[I]);
      Destination[I].FIsChanged := False;
    end;
  for I in FChangedList.Keys do
    ClearCache(I);
  if (FChangedList.Count > 0) or IsIgnoreIndex then
  begin
    StartTimer;
...
var
  I: Integer;
  Tmp: TCustomDestinationItem;
  LChangedLinks: TList<TChangedLink>;
  LLink: TChangedLink;
begin
  StopTimer;
  DoChanged;
  if LinkCount > 0 then
  begin
    LChangedLinks := TList<TChangedLink>.Create;
    try
      for I := 0 to LinkCount - 1 do
        if (Links[I] <> nil) and (Links[I].IgnoreIndex or ((FChangedList <> nil) and
          FChangedList.TryGetValue(Links[I].ImageIndex, Tmp))) then
        begin
          LLink.Link := Links[I];
          LLink.Index := I;
          LChangedLinks.Add(LLink);
        end;
      for I := 0 to LChangedLinks.Count - 1 do
      begin
        LLink := LChangedLinks[I];
        if LinkContains(LLink.Link, LLink.Index) then
          LLink.Link.Change;
      end;
    finally
      LChangedLinks.Free;
...
function TCustomImageList.InvalidateDestination(const Name: string): Boolean;
var
  I, J: Integer;
begin
  Result := False;
  if (Destination <> nil) and not Name.IsEmpty then
    for I := 0 to Destination.Count - 1 do
      for J := 0 to Destination[I].Layers.Count - 1 do
        if string.Compare(Name, Destination[I].Layers[J].Name, True) = 0 then
        begin
          Destination[I].FIsChanged := True;
          Destination[I].Layers[J].FMultiResBitmapIsValid := False;
          Result := True;
        end;
end;

function TCustomImageList.IsIgnoreIndex: Boolean;
var
  I: Integer;
begin
  for I := 0 to LinkCount - 1 do
...
procedure TGlyph.SetDisableInterpolation(const Value: Boolean);
begin
  if FDisableInterpolation <> Value then
  begin
    FDisableInterpolation := Value;
    Repaint;
  end;
end;
