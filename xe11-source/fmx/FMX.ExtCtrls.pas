...
begin
  ...
  begin
    ...
    FilterParts := FFilter.Split(['|']);
    if FFilterIndex <= Length(FilterParts) div 2 then
      Result := FilterParts[(FFilterIndex - 1) * 2 + 1]
    else
      Result := string.Empty;
  end
  else
...
begin
  if (Content <> nil) and (ContentLayout <> nil) then
  begin
    NeedRealign := (VScrollBar <> nil) and (VScrollBar.Enabled) or (HScrollBar <> nil) and (HScrollBar.Enabled);
...
begin
  if ([csLoading, csDesigning] * ComponentState = []) and (FUpdating = 0) then
    try
      DoChange
    finally
      FChanged := False;
