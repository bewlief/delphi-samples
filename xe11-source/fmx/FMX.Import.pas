...
procedure TGEVertexSource.SetTextue0Source(const ASource : TSingleDynArray; AStride: Integer = 2);
  ...
  function Normalize(const AP: TPointF): TPointF; inline;
  begin
    Result := AP;
    if Result.X < 0 then
      Result.X := Result.X + 1;
...
begin
  ...
  begin
    Id  := I * AStride;
    P := TPointF.Create(ASource[Id + 0], 1 - ASource[Id + 1]); // change y value to firemonkey coordinate
    FTexture0Source[I] := Normalize(P);
  end;
end;
