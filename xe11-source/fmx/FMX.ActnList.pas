...
uses
  System.Character, FMX.Consts, FMX.Platform, FMX.Forms, FMX.Menus;
...
function TCustomAction.IsDialogKey(const Key: Word; const Shift: TShiftState): Boolean;
  ...
  function ExtractShifState(const AShortCut: System.Classes.TShortCut): TShiftState;
  begin
    Result := [];
  ...
  function IsSameShortCut(const AShortCut: System.Classes.TShortCut; const AKey: Word; const AShift: TShiftState): Boolean;
  var
    ShortCutKey: Word;
    ShortCutState: TShiftState;
    MenuService: IFMXMenuService;
    NormilizedKey: Word;
  begin
    ShortCutState := [];
    ShortCutKey := 0;
    NormilizedKey := Ord(Chr(AKey).ToUpper);

    if TPlatformServices.Current.SupportsPlatformService(IFMXMenuService, MenuService) then
      MenuService.ShortCutToKey(AShortCut, ShortCutKey, ShortCutState)
    else
    begin
      ShortCutKey := AShortCut and not (scCommand + scShift + scCtrl + scAlt);
      ShortCutState := ExtractShifState(AShortCut);
    end;
    Result := (ShortCutKey = NormilizedKey) and (ShortCutState = AShift);
  end;

  function FindShortCutInSecondaryShortCuts(const AKey: Word; const AShift: TShiftState): Boolean;
  var
    I: Integer;
  begin
    ...
    begin
      Result := IsSameShortCut(SecondaryShortCuts.ShortCuts[I], AKey, AShift);
      if Result then
        Break;
    end;
    Result := False;
...
begin
  ...
  begin
    Result := IsSameShortCut(ShortCut, Key, Shift);
    if not Result then
      Result := FindShortCutInSecondaryShortCuts(Key, Shift);
  end
  else
    Result := False;
end;
