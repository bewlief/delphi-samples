...
begin
  ...
  if not TBitmapCodecManager.CodecExists(ExtractFileExt(LName)) then
    raise EMultiResBitmap.CreateFMT(SInvalidKnownFilename, [LName]);
...
function TCustomMultiResBitmap.GetDefaultSizeKind: TSizeKind;
begin
  Result := TSizeKind.Custom;
end;
...
begin
  ...
  Filer.DefineProperty('LoadSize', ReadLoadSize, WriteLoadSize, SizeKind <> GetDefaultSizeKind);
  Filer.DefineProperty('TransparentColor', ReadColor, WriteColor, TransparentColor <> TColors.SysNone);
end;

type
  TScaleComparer = class(TInterfacedObject, IComparer<Single>)
    function Compare(const Left, Right: Single): Integer;
  end;

{ TScaleComparer }

function TScaleComparer.Compare(const Left, Right: Single): Integer;
begin
  Result := CompareValue(Left, Right, TEpsilon.Scale);
end;
...
var
  ...
  List: TList<Single>;
begin
  List := TList<Single>.Create(TScaleComparer.Create);
  try
    for I := 0 to Count - 1 do
      if IncludeEmpty or not Items[I].IsEmpty then
        List.Add(Items[I].Scale);
    List.Sort;
    Result := List.ToArray;
  finally
    FreeAndNil(List);
...
function TCustomMultiResBitmap.ItemByScale(const AScale: Single; const ExactMatch: Boolean; const IncludeEmpty: Boolean)
  ...
  function FindTheSameScale(const AScale: Single; out AItem: TCustomBitmapItem): Boolean;
  var
    I: Integer;
  begin
    for I := 0 to Count - 1 do
      if SameValue(AScale, Items[I].Scale, TEpsilon.Scale) and (IncludeEmpty or not Items[I].IsEmpty) then
      begin
        AItem := Items[I];
        Exit(True);
      end;
...
begin
  ...
  begin
    if FindTheSameScale(LScale, Result) then
      Exit;
