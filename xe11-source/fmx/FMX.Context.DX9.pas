...
class function TCustomDX9Context.Style: TContextStyles;
begin
  // This implies that TTexture.UpdateTexture will store raw bits of the image
  // in an internal cache and also that TTexture will not be usable in background thread
  Result := [TContextStyle.Fragile];
end;
...
class function TDX9Context.Valid: Boolean;
begin
...
begin
  ...
  begin
    var ScaledScissorRect := TRectF.Create(ScissorRect.Left * Scale, ScissorRect.Top * Scale,
                                           ScissorRect.Right * Scale, ScissorRect.Bottom * Scale).Round;
    SharedDevice.SetScissorRect(@ScaledScissorRect);
  end;
end;
