...
(*$HPPEMIT END 'T __fastcall TListViewItem::TListViewItemObjects::FindObjectT(const System::UnicodeString AName) {'*)
(*$HPPEMIT END '  Fmx::Listview::Types::TListItemDrawable* LObject = FindDrawable(AName);'*)
(*$HPPEMIT END '  if (T t = dynamic_cast<T>(LObject)) {'*)
(*$HPPEMIT END '    return t;'*)
(*$HPPEMIT END '  }'*)
(*$HPPEMIT END '  return 0;'*)
(*$HPPEMIT END '}'*)
...
begin
  ...
  if Result = -1 then
  begin
    Controller.RequestReindexing(Self); // = ReindexAndFindItem
    Result := inherited;
  end;
end;
...
begin
  for Dirty in FDirtyDrawables do
    case Dirty of
      ...
      TDirtyDrawable.Accessory:
        begin
          LAccessory := Objects.AccessoryObject;
          if LAccessory <> nil then
            LAccessory.AccessoryType := FAccessory;
        end;
      TDirtyDrawable.ImageIndex:
        begin
          LImage := Objects.ImageObject;
          if LImage <> nil then
            LImage.ImageIndex := FImageIndex;
        end;
      TDirtyDrawable.BitmapValue:
        begin
          LImage := Objects.ImageObject;
          if LImage <> nil then
          begin
            LImage.OwnsBitmap := False;
            LImage.Bitmap := FBitmap;
          end;
        end;
      TDirtyDrawable.BitmapRef:
        begin
          LImage := Objects.ImageObject;
          if LImage <> nil then
          begin
            LImage.OwnsBitmap := False;
            LImage.Bitmap := FBitmapRef;
          end;
        end;
    end;
...
begin
  FImageIndex := Value;
  Include(FDirtyDrawables, TDirtyDrawable.ImageIndex);
  Invalidate;
end;
...
begin
  ...
  begin
    if not FData.TryGetValue(AIndex, Result) then
      if (FDataBitmaps <> nil) and FDataBitmaps.TryGetValue(AIndex, DataBitmap) then
        Result := DataBitmap
      else
        Result := TValue.Empty;
  end
...
begin
  ...
  Include(FDirtyDrawables, TDirtyDrawable.BitmapValue);
  Invalidate;
end;
...
begin
  FBitmapRef := Value;
  Include(FDirtyDrawables, TDirtyDrawable.BitmapRef);
  Invalidate;
end;
...
begin
  FAccessory := Value;
  Include(FDirtyDrawables, TDirtyDrawable.Accessory);
  Invalidate;
end;

procedure TListViewItem.CreateObjects;
begin
  if (not View.Initialized) and TNonReentrantHelper.EnterSection(FCreatingObjectsGuard) then
    try
      Adapter.ResetView(Self);
...
begin
  FreeAndNil(FPlaceOffset);
  FreeAndNil(FInternalPlaceOffset);
  inherited;
end;
...
var
  ...
  LHandled: Boolean;
  LValue: TValue;
  LHasButtonText: Boolean;
  LHasButtonData: Boolean;
begin
  LItem := TListItemTextButton.Create(AListViewItem);
  LItem.BeginUpdate;
  try
    LItem.Name := FName;
    LHasButtonText := AListViewItem.GetHasButtonText;
    LHasButtonData := AListViewItem.HasData[FName];
    FHasButtonText := LHasButtonText or LHasButtonData;
    LItem.Assign(Self);
    if LHasButtonData then
    begin
      LHandled := False;
      LValue := AListViewItem.GetData(FName);
      FOwner.SetObjectData(AListViewItem, FName, LValue, LHandled);
...
begin
  if AListViewItem.GetHasButtonText or AListViewItem.GetHasData(FName) then
...
var
  ...
  OldAppearance: TItemAppearanceObjects;
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if FAppearanceCache.TryGetValue(TItemAppearanceObjectsClass(FAppearance.ClassType), OldAppearance) then
          OldAppearance.Assign(FAppearance)
        else
          FAppearanceCache.Add(TItemAppearanceObjectsClass(FAppearance.ClassType), FAppearance);
        FAppearance := nil;
      end
      else
        FreeAndNil(FAppearance);
...
begin
  ...
  if Item is TListViewItem then
  begin
    ListItem := TListViewItem(Item);
    if Filter(ListItem.Text) or Filter(ListItem.Detail) or Filter(ListItem.ButtonText) then
      Exit(True);
  end;

  for I := 0 to Item.View.Count - 1 do
  begin
    Drawable := Item.View[I];
    Result := (Drawable is TListItemText) and Filter(TListItemText(Drawable).Text);
    if Result then
      Break;
  end;
end;
...
begin
  ...
  try
    ...
    if TListViewItem(LItem).Checked then
      Dec(FCheckedCount);
