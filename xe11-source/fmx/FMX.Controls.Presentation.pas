...
uses
  ...
  {$IFDEF ANDROID}, FMX.Presentation.Android.Style, FMX.Presentation.Android{$ENDIF};
...
begin
  ...
  var
    MessageSendingCompatible: IMessageSendingCompatible;
    NeedNotifyChild: Boolean;
    MessageSender: TMessageSender;
  begin
    if Supports(AObject, IMessageSendingCompatible, MessageSendingCompatible) then
    begin
      MessageSender := MessageSendingCompatible.MessageSender;
      if MessageSender <> nil then
      begin
        NeedNotifyChild := True;
        MessageSender.SendMessageWithResult<Boolean>(PM_ANCESTOR_PRESENTATION_LOADED, NeedNotifyChild);
        if not NeedNotifyChild then
          Exit(TEnumProcResult.Discard);
      end;
    end;
...
procedure TPresentedControl.AncestorPresentationUnloading(const AControl: TPresentedControl);
begin
  EnumObjects(function (AObject: TFmxObject): TEnumProcResult
  var
    MessageSendingCompatible: IMessageSendingCompatible;
  ...
  begin
    ...
    begin
      MessageSender := MessageSendingCompatible.MessageSender;
      if MessageSender <> nil then
        MessageSendingCompatible.MessageSender.SendMessage<TControl>(PM_ANCESTOR_PRESENTATION_UNLOADING, AControl);
    end;
    Result := TEnumProcResult.Continue;
  end);
end;
...
procedure TPresentedControl.DoAddObject(const AObject: TFmxObject);
var
  LPresentationScene: TFmxObject;
begin
  LPresentationScene := PresentationScene;
  if LPresentationScene <> nil then
  begin
    if FSceneObjects = nil then
      FSceneObjects := TFmxObjectList.Create;
    LPresentationScene.AddObject(AObject);
    FSceneObjects.Add(AObject);
  end
  else
    inherited;
end;
...
begin
  if HasPresentationProxy then
    PresentationProxy.SendMessage(PM_DO_BEFORE_EXIT);
...
procedure TPresentedControl.DoInsertObject(Index: Integer; const AObject: TFmxObject);
var
  LPresentationScene: TFmxObject;
begin
  LPresentationScene := PresentationScene;
  ...
  begin
    if FSceneObjects = nil then
      FSceneObjects := TFmxObjectList.Create;
    LPresentationScene.InsertObject(Index, AObject);
    FSceneObjects.Insert(Index, AObject);
  end
  else
    inherited;
end;
...
procedure TPresentedControl.DoRemoveObject(const AObject: TFmxObject);
var
  LPresentationScene: TFmxObject;
begin
  inherited;
  ...
  if LPresentationScene <> nil then
  begin
    LPresentationScene.RemoveObject(AObject);
    if FSceneObjects <> nil then
      FSceneObjects.Remove(AObject);
  end;
end;
...
function TPresentedControl.GetMessageSender: TMessageSender;
begin
  Result := FPresentationProxy;
end;

function TPresentedControl.GetControlType: TControlType;
begin
  Result := FControlType;
end;
...
function TPresentedControl.GetPresentationScene: TFmxObject;
begin
  Result := nil;
  if PresentationProxy <> nil then
    PresentationProxy.SendMessageWithResult<TFmxObject>(PM_GET_SCENE, Result);
end;
...
var
  ...
  function StyledPresentationAlreadyLoaded: Boolean;
  var
    PrevControlType: TControlType;
  begin
    Result := False;
    if HasPresentationProxy then
    begin
      PrevControlType := FControlType;
      try
        ...
        Result := PresentationProxy is PresentationProxyClass;
      finally
        if not Result then
          FControlType := PrevControlType;
      end;
    end;
  end;

  procedure MoveChildrenControlsToScene;
  var
    I: Integer;
    Child: TFmxObject;
  begin
    if PresentationScene = nil then
      Exit;
    ...
    BeginUpdate;
    try
      while I <= ChildrenCount - 1 do
      begin
        Child := Children[I];
        if Child = PresentationScene then
        begin
          Inc(I);
          Continue;
        end;
        PresentationScene.AddObject(Child);
        FSceneObjects.Add(Child);
      end;
    finally
      EndUpdate;
    end;
...
begin
  ...
  begin
    ...
    begin
      if StyledPresentationAlreadyLoaded then
        FControlType := TControlType.Styled
      else
...
procedure TPresentedControl.PaintChildren;
var
  Result: Boolean;
begin
  Result := False;
  if HasPresentationProxy then
    PresentationProxy.SendMessageWithResult<Boolean>(PM_PAINT_CHILDREN, Result);
  if not Result then
    inherited;
end;
...
procedure TPresentedControl.UnloadPresentation;
  ...
  procedure MoveSceneControlsToSelf;
  var
    I: Integer;
  begin
    if FSceneObjects = nil then
    ...
    BeginUpdate;
    try
      for I := FSceneObjects.Count - 1 downto 0 do
        InsertObject(0, FSceneObjects[I]);
    finally
      EndUpdate;
    end;
    FSceneObjects.Clear;
...
begin
  ...
  begin
    ...
    try
      Model.Receiver := nil;
      MoveSceneControlsToSelf;
      FPresentationProxy.SendMessage(PM_UNLOAD);
      AncestorPresentationUnloading(Self);
...
procedure TPresentedControl.ChildWasRemoved(const AChild: TFmxObject);
begin
  if FSceneObjects <> nil then
    FSceneObjects.Remove(AChild);
end;
...
procedure TPresentedControl.BeforeDestruction;
begin
  inherited;
  UnloadPresentation;
end;
