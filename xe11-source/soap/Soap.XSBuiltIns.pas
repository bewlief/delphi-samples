...
begin
  ...
  begin
    Bias := Trunc(TTimeZone.Local.GetUTCOffset(Value).Negate.TotalMinutes);
    if Bias <> 0 then
    begin
...
begin
  Result := Trunc(FractionalSeconds*1000);
end;
...
begin
  Result := TTimeZone.Local.ToLocalTime(GetAsUTCDateTime());
end;
...
begin
  ...
  begin
    ...
    count := HexToBin(SourceBytes, 0, DestBytes, 0, Length(SourceBytes) div 2);
