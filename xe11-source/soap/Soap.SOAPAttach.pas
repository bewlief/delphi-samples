...
begin
  ...
  FWebRequest.ReadTotalContent; // Init data
...
begin
  ...
  begin
    ...
    SetLength(Buffer, CrPos + 1);
  end else
  begin
    { Check for split EOL }
    if (Length(Buffer) > 0 ) and (Buffer[Length(Buffer) - 1] = Ord(#13)) then
    begin
      SetLength(SplitLF, 1);
      Read(SplitLF, 0, 1 );
...
var
  ...
  begin
    Result := '';
    HeadersText := MimeStream.Headers.Text;
    HeadersText := StringReplace(HeadersText, ';', EOL, [rfReplaceAll]);
    HeadersList := TStringList.Create;
    try
      HeadersList.Text := HeadersText;
      for i := 0 to HeadersList.Count - 1 do
        if HeadersList.Names[i].ToLower.Trim = SCharset then
        begin
          Result := HeadersList.ValueFromIndex[i].ToLower.Trim;
          Break;
        end;
    finally
      HeadersList.Free;
  ...
  begin
    ...
    begin
      if CurPtr^ = Pattern[0] then
      begin
        Found := True;
        for J := 1 to PatternLength - 1 do
          if PByte(CurPtr + J)^ <> Pattern[J] then
          begin
            Found := False;
            Break;
          end;
  ...
  function ReadLine(const Stream: TStream; out IsEof: Boolean; AEncoding: TEncoding; BlockSize:
      Integer = 1014): string;
  var
    Buffer: TBytes;
    BytesRead: Integer;
  ...
  begin
    Result := '';
    BlockSize := Max(Length(EOL), BlockSize);
    ...
    try
      repeat
        ...
        FoundPosition := BytePos(EOLBytes, @Buffer[0], BytesRead);
        ...
        begin
          ...
          begin
            // seek back, EOL can be read partially
            LineSize := BytesRead - (Length(EOLBytes) - 1);
            Stream.Seek(-(Length(EOLBytes) - 1), soCurrent);
          end;
        end
        else
          LineSize := NativeUInt(FoundPosition) - NativeUInt(@Buffer[0]);

        if LineSize > 0 then
        begin
          if ResultStream = nil then
            ResultStream := TMemoryStream.Create;
          ResultStream.Write(Buffer, LineSize);
        end;

        if FoundPosition <> nil then
        begin
          // EOL found
          Stream.Seek(-(BytesRead - LineSize - Length(EOLBytes)), soCurrent); // correct stream pos EOL skipped
          Break;
        end;
      ...
      if ResultStream <> nil then
      begin
        LineSize := ResultStream.Size;
        // convert to Unicode
        SetLength(Buffer, LineSize);
        ResultStream.Position := 0;
        ResultStream.Read(Buffer[0], LineSize);
  ...
  begin
    Result := SameText(SFound, SMime) or
              ...
              SameText(SFound, '--' + SMime + '--');
  end;

  procedure ReadContent(ADestStream: TStream; AHeaders: TStrings);
  var
    SLine: string;
    BLine: TBytes;
    Encoding: TEncoding;
  ...
  begin
    Charset := GetCharset;
    if (Charset <> '') and (CompareText(Charset, 'utf-8') <> 0) then  // do not translate
    ...
    try
      ...
      repeat
        SLine := ReadLine(MimeStream.SourceStream, Eof, Encoding);
        if SameMimeBoundary(SLine, MimeBoundary) then
          Break;
        BLine := Encoding.GetBytes(SLine);
        ADestStream.Write(BLine, Length(BLine));
        if not Eof then
        begin
          BLine := Encoding.GetBytes(EOL);
          ADestStream.Write(BLine, Length(BLine));
        end;
      until Eof;
  ...
  begin
    ...
    begin
      ...
      begin
        ReadContent(ADestStream, MimeStream.Headers);
      end else
      begin
  ...
  { Boundary Headers values are ASCII symbols in range from 33 to 126) }
  procedure GetHeaders(AHeaders: TStrings);
  var
    ReadHeaders: TStringList;
    Eof: Boolean;
    Line: string;
    IsFirstEmpty: Boolean;
  begin
    ReadHeaders := TStringList.Create;
    try
      IsFirstEmpty := True;
      repeat
        Line := ReadLine(MimeStream.SourceStream, Eof, TEncoding.ASCII, 128);
        if Line = '' then
        begin
          if Eof then
            Break;
        ...
        ReadHeaders.Add(StringReplace(Line, ': ', '=', []));
        IsFirstEmpty := False;
      until Eof;

      AHeaders.Clear;
      AHeaders.AddStrings(ReadHeaders);
    finally
      ReadHeaders.Free;
    end;
  end;
  ...
  begin
    ...
    begin
      repeat
        MimeStr := ReadLine(MimeStream.SourceStream, Eof, TEncoding.ASCII);
      until Eof or (MimeStr <> '');
      Done := Eof;
    end
    else
...
begin
  ...
  begin
    ...
    if Attachment.CacheFile <> '' then
      Stream := TFileStream.Create(Attachment.CacheFile, fmOpenRead or fmShareDenyWrite)
