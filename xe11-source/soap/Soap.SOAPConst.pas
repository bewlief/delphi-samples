...
const
  ...
  MimeVersionName = 'MIME-Version';                        { Do not localize }
  MimeVersionValue = '1.0';                                { Do not localize }
  MimeVersion = MimeVersionName + ': ' + MimeVersionValue; { Do not localize }
  sTextHtml = 'text/html';                                 { Do not localize }
  sTextHtmlUTF8 = 'text/html; charset=utf-8';              { Do not localize }
  ...
  SSOAP12RPCNamespace = 'http://www.w3.org/2003/05/soap-rpc'; { do not localize }
  ...
  SSOAPXMLHREFs: array[Boolean] of string = (SXMLHREF, SSOAP12XMLHREF);
...
//  SSOAPHREFPres: array[Boolean] of WideString = (SHREFPre, SSOAP12HREFPre);
  SSOAPHREFPres: array[Boolean] of string = (SHREFPre, SSOAP12HREFPre);
  ...
  SCharset = 'charset';                      { do not localize }
...
{resourcestring - these are getting truncated as resources currently resulting in bad HTML pages!!} 
  ...
  HTMLTop                   = HTMLHead + HTMLStyles + '</head>' + '<body>';
  HTMLTopNoMargin           = HTMLHead + HTMLNoMargin+'</head>' + '<body>';
  HTMLTopTitleNoMargin      = HTMLHead + '<title>%s</title>'+HTMLNoMargin+'</head><body>';
  HTMLTopNoStyles           = HTMLHead + '</head><body>';
  HTMLTopTitle              = HTMLHead + '<title>%s</title>'+HTMLStyles+'</head><body>';
  HTMLTopTitleNoMarginWSIL  = HTMLHead + HTMLServiceInspection + '<title>%s</title>'+HTMLNoMargin+'</head><body>';
...
resourcestring
  ...
  SUnexpectedDataType = 'Internal error: data type kind %s cannot be converted to and from text';
