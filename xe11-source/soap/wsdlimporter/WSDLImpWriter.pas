...
uses
  ...
  UDDIHlprDesign, Contnrs, WSDLModelIntf, StrUtils;
...
type
  ...
  WSDLGenFlags = (wfDebug,
                  ...
                  wfMergeDuplicateTypes,          { Merge duplicate types }
                  wfLastOption                    { Marker }
                  );
...
const
  RevString = '- $Rev: 108085 $';  { Do not localize }
  { Default Settings }
...
const
  DateString= '$Date: 2022-01-02 23:58:47 -0800 (Sun, 02 Jan 2022) $'; { Do not localize }
...
{$ENDIF}
  ...
  public
    ...
    function    GetOutputUse: DOMString;
    function    GetOutputNamespace: DOMString;
...
{$IFDEF VALIDATE_MODEL}
  ValidateModel;
{$ENDIF}
  { Merge types to avoid Type2 = class(Type) begin end; }
  if wfMergeDuplicateTypes in Global_WSDLGenFlags then
    MergeTypes;
...
{$ENDIF}
  ...
  if TypeDef.HasAttribute(SMixed) and (TypeDef.ElementDefs.Count = 0)
  and (TypeDef.AttributeDefs.Count = 0) and (TypeDef.AttributeGroups.Count = 0) then
  begin
    {
      NOTE:
      ...
      simpleType. Of course, this would throw us off because it's in essence
...
begin
  ...
  begin
    ...
    begin
      if TypeDef.SchemaDef.HasAttribute('xmlns') then
        ANamespace := TypeDef.SchemaDef.GetAttribute('xmlns')
      else
        ANamespace := TypeDef.SchemaDef.TargetNamespace;
    end;
  end;
...
procedure TWSDLImporter.MergeTypes;
var
  WSDLType: IWSDLType;
begin
  ...
  begin
    if (WSDLType.DataKind = wtAlias) and
       (WSDLType.BaseType.DataKind = wtClass) and
       (WSDLType.TypeFlag = WSDLType.BaseType.TypeFlag) and
       (WSDLType.Name = WSDLType.BaseType.Name) and
       (WSDLType.Namespace = WSDLType.BaseType.Namespace) then
    begin
      if wfDebug in Global_WSDLGenFlags then
        WriteFeedback('MergeType %s@%s' + sLineBreak, [WSDLType.Name, WSDLType.Namespace]);
      WSDLType.DataKind := wtDuplicate;
    end;
  end;
end;
...
{$ENDIF}
  ...
  begin
    ...
    begin
      ...
      begin
        for I := Length(WSDLType.Members)-1 downto 0 do
        begin
...
procedure WriteXMLLog(OutputFile: String; SchemaFilename: String; Types: IWSDLTypeArray);
var
...
begin
  Count := Length(Types);
  DebugFile := TCheapTextFile.Create(OutputFile);
...
function TWSDLOperation.GetOutputUse: DOMString;
begin
  Result := FOutputUse;
end;

function TWSDLOperation.GetOutputNamespace: DOMString;
begin
...
function InParts(const Name, Parts: string): Boolean;
const
  WHITESPACE_CHARS: array[0..3] of Char = (#$20, #$9, #$A, #$D);
begin
  // https://www.w3.org/TR/2001/NOTE-wsdl-20010315#_soap:body
  // The 'parts' attribute is optional; it's a list of NMTokens
  // If omitted, then all parts defined by the message are assumed to be included...
  if Parts = '' then
    Exit(True);
  Result := MatchStr(Name, Parts.Split(WHITESPACE_CHARS, TStringSplitOptions.ExcludeEmpty));
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          if InParts(Parts[I].Name, FInputParts) then
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        for I := 0 to Parts.Count -1 do
        begin
          if InParts(Parts[I].Name, FOutputParts) then
...
begin
  ...
  begin
    ...
    NewOperation.FInputParts := WSDLImporter.GetWSDLItems.GetSoapBodyInputAttribute(FBinding, Operation.Name, 'parts', OpIndex, SOAPVersion);
    ...
    NewOperation.FOutputParts := WSDLImporter.GetWSDLItems.GetSoapBodyOutputAttribute(FBinding, Operation.Name, 'parts', OpIndex, SOAPVersion);
...
begin
  {
     <xsd:element name="xxx" type="soap-enc:Array"/>
  }
  if (Name = SArray) and ((Namespace = Soapns) or (Namespace = SSoap11EncodingS5)) then
  begin
    Result := GenArrayType(FindType('anyType', XMLSchemaNamespace, [xtiGlobal], [], True), False);
    Exit;
  end;
...
{$IFDEF DEBUG}
  Assert(InParts('param1', ''));
  Assert(InParts('param1', 'param1 param2'));
  Assert(InParts('param2', 'param1 param2'));
  Assert(InParts('param1', #13'param1'#9'param2'));
  Assert(InParts('param2', #13'param1'#9'param2'));
  Assert(not InParts('param3', 'param1 param2'));
{$ENDIF}
finalization
