...
type
  ...
  protected
    ...
    function  EscapeActions(const W: DOMString): DOMString;
  public
...
const
   ...
   CppReservedWords: array[0..106] of string =(
                        '__asm',
                        '_asm',
                        'asm',
                        'auto',
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        DataTypeName := Member.DataType.LangName + RefOp[IsComplex(Member.DataType)];
        if Member.DataType.DataKind = wtEnumeration then
        begin
          for MemberName in Members do
          begin
            if MemberName = Member then
              Break;
            if MemberName.LangName = Member.DataType.LangName then
              DataTypeName := 'enum ' + DataTypeName;
          end;
        end;
...
begin
  ...
  I := Length(Result);
  while I > cMaxLitteralLen do
  begin
    Dec(I, cMaxLitteralLen);
    while Result[I - 1] = '\' do
      Inc(I);
    Insert(cContinueLine, Result, I);
  end;

end;

function TWSDLCppWriter.EscapeActions(const W: DOMString): DOMString;
const
  cBackslash = '\';
  cContinueLine = '"'#13#10'    "';
  cMaxActionsLen = 128;
...
begin
  ...
  if Length(Result) > cMaxActionsLen then
  begin
    I := Pos('|', Result, 3);
    while I > 0 do
    begin
      Insert(cContinueLine, Result, I);
      I := Pos('|', Result, I + Length(cContinueLine) + 1);
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      if (not (wfServer in Global_WSDLGenFlags)) or
        ...
        WriteFmt(sRegSOAPAction, [WSDLPortType.LangName, EscapeActions(WSDLPortType.SOAPAction)]);
    end
    ...
    begin
      ...
      if (not (wfServer in Global_WSDLGenFlags)) then
      begin
        WriteFmt(sRegAllSOAPAction, [WSDLPortType.LangName, EscapeActions(WSDLPortType.SOAPAction)]);
      end;
    end;
    ...
    try
      ...
      begin
        ...
        begin
          ...
          DataType := WSDLPart.DataType;
          while DataType.DataKind = wtAlias  do
            DataType := DataType.BaseType;
          if DataType.RegInfo <> '' then
            ARegInfo.Load(DataType.RegInfo);
          if ARegInfo.HasAttr then
            RegInfoStr := ARegInfo.AsString(True);
