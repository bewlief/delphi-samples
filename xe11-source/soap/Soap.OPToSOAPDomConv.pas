...
uses
  ...
  Xml.XMLIntf, Xml.xmldom, System.Generics.Collections;
...
type
  ...
  private
    ...
    FHREFs: TDictionary<string, Integer>;
    FMHREFs: TDictionary<string, TObject>;
  public
    constructor Create;
...
{ The code for TRefCache was kindly provided in QC-26063. It greatly improves
  performance when serializing multiref objects and arrays. }
    ...
    procedure Clear;
    procedure FAttachmentsNotify(Sender: TObject; const Item: TSoapAttachment; Action: TCollectionNotification);
  protected
...
//  Xml.Win.msxmldom,
{$ENDIF}
  System.Character, System.Rtti, System.Variants, System.NetEncoding,
  Xml.XMLDoc,
  Soap.XSBuiltIns, Soap.HTTPUtil, Soap.SOAPConst,
...
{$ENDIF}
    function VariantToString(const V: Variant): InvString;
    function VariantToString2(const V: Variant; var Info: PTypeInfo): InvString;
    function StringToVariant(const S: InvString): Variant; inline;
    function IsVarArray(const Node: IXMLNode): Boolean;
...
function NodeHasText(const ANode: IXMLNode): Boolean;
begin
  if ANode.hasChildNodes then
  begin
                                                            
//    Result := ANode.IsTextElement
    Result := (ANode.NodeType = ntElement) and (ANode.ChildNodes.Count = 1) and
      (ANode.ChildNodes[0].NodeType in [ntText, ntCData]);
  end
  else
    Result := True;
end;

function GetNodeText(const ANode: IXMLNode): InvString; inline;
begin
  if ANode.hasChildNodes then
    Result := ANode.Text
  else
...
begin
  ...
  FHREFs := TDictionary<string, Integer>.Create;
  FMHREFs := TDictionary<string, TObject>.Create;
end;
...
function TSOAPDomConvHelper.IsVarArray(const Node: IXMLNode): Boolean;
var
  ChildCount: Integer;
  VT: TVarType;
begin
  ChildCount := ntElementChildCount(Node);
  Result := ChildCount > 0;
  if ChildCount = 1 then
  begin
    Result := IsNodeAVarArray(Node, VT)
  end;
end;
...
begin
  ...
  begin
    ...
    if not VarIsNull(V) then
      RefCache.FHREFs.Add(SSOAPHREFPres[soSOAP12 in Converter.Options] + V, RefCache.FNodes.Add(Node));
  end;
...
begin
  ...
  begin
    ...
    { NOTE: SOAP 1.2 do not allows to put this attribute on the body node }
      Node := MethNode
    else
      Node := BodyNode;
    Node.SetAttributeNS(SSoapEncodingAttr,
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            if GetRegInfo(ParamNamespace, InfoAttribute.iaArrayItemName, ParamChildNamespace) then
...
begin
  ...
  FAttachments.OnNotify := FAttachmentsNotify;
  FRefCache := TRefCache.Create();
end;

destructor TSOAPDomConv.Destroy;
begin
  FAttachments.OnNotify := nil;
  Clear;
  FRefCache.Free;
  FAttachments.Free;
  inherited;
...
function IsXMLStringTypeInfo(const Info: PTypeInfo): Boolean;
begin
  Result := Info.Kind in [tkString, tkLString, tkUString, tkWString];
end;

function TSOAPDomConvHelper.VariantToString(const V: Variant): string;
var
  Info: PTypeInfo;
begin
  Result := VariantToString2(V, Info);
end;

function TSOAPDomConvHelper.VariantToString2(const V: Variant; var Info: PTypeInfo): string;
const
...
begin
  ...
  if IsXMLDateTimeTypeInfo(Info) then
    ...
    Result := sBoolean[Boolean(V)]
  else if VarIsFloat(V) then
    Result := FloatToStrEx(V)
  else
    Result := V;
end;

function TSOAPDomConvHelper.StringToVariant(const S: InvString): Variant;
begin
  Result := TypeTranslator.CastSoapToVariant(TypeInfo(System.string), S);
end;
...
begin
  ...
  P := VarArrayLock(V);
  try
    Encd := InvString(TNetEncoding.Base64.EncodeBytesToString(P, VSize));
  finally
    VarArrayUnlock(V);
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (SoapTypeInfo <> nil) and (not IsXMLStringTypeInfo(SoapTypeInfo)) then
        begin
          { Handle 'dateTime' }
          if IsXMLDateTimeTypeInfo(SoapTypeInfo) and NodeHasText(ChildNode) then
          begin
            Result[VarIndex] := XMLTimeToDateTime(GetNodeText(ChildNode));
          end else
                                                                                                          
            Result[VarIndex] := TypeTranslator.CastSoapToVariant(SoapTypeInfo, GetNodeText(ChildNode));
        end
        else
        begin
          Result[VarIndex] := StringToVariant(GetNodeText(ChildNode));
        end;
      end else
...
begin
  ...
  begin
    ...
    { Handle dates }
    if IsXMLDateTimeTypeInfo(Info) and NodeHasText(Node) then
    begin
      Variant(PVarData(InvData)^) := XMLTimeToDateTime(GetNodeText(Node));
    end else
...
begin
  Result := nil;
  if FRefCache.FHREFs.TryGetValue(HREF, I) then
    Result := FRefCache.FNodes[I] as IXMLNode;end;
...
function ByteArrayElemInfo(ElemInfo: PTypeInfo): Boolean;
begin
...
function ByteArrayInfo(PInfo: PTypeInfo): Boolean;
begin
  Result := (PInfo^.Kind = tkDynArray) and ByteArrayElemInfo(GetTypeData(PInfo)^.elType2^);
end;


{$DEFINE COPY_ARRAY}
function Base64Binary(TypeInfo: PTypeInfo; Node: IXMLNode; DataP: Pointer): Pointer;
var
  Bytes: TBytes;
{$IFDEF COPY_ARRAY}
  ArrayLen: NativeInt;
...
begin
{$IFDEF NEXTGEN}
  Bytes := TNetEncoding.Base64.DecodeStringToBytes(Node.Text);
{$ELSE !NEXTGEN}
  Bytes := TNetEncoding.Base64.DecodeStringToBytes(string(Node.Text));
{$ENDIF NEXTGEN}
{$IFDEF COPY_ARRAY}
...
{$ELSE}
  ...
  if Result <> nil then
    Inc(PInteger(PByte(Result)-(Sizeof(NativeInt) + Sizeof(Integer)))^);                            
{$ENDIF}
end;
...
begin
  ...
  if (Dim = 1) and ByteArrayElemInfo(ElemInfo) then
  begin
...
begin
  ...
  begin
   ...
   if IsXMLDateTimeTypeInfo(SoapTypeInfo) and NodeHasText(Node) then
   begin
     Variant(PVarData(P)^) := XMLTimeToDateTime(GetNodeText(Node));
   end else
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        { Skip child Nodes that do not match the desired name }
        if (ElementName <> '') and (ExtractLocalName(N.NodeName) <> ElementName) then
          continue;
  ...
  begin
    if ByteArrayElemInfo(ElemInfo) then  // for an array of TByteSOAPArray
    begin
      Result := Base64Binary(ArrayInfo, Node, DataP);
    end else
    begin
...
begin
  ...
  begin
    Result := Base64Binary(TypeInfo, Node, DataP);
  end else
  begin
...
procedure TSOAPDomConv.FAttachmentsNotify(Sender: TObject;
  const Item: TSoapAttachment; Action: TCollectionNotification);
begin
...
{$ENDIF}
            ...
            begin
              ...
              ConvertNativeArrayToSoap(RootNode, InstNode, ExtPropName,
                                       ...
                                       InlineProp);
            { Variant property }
            end else if (Kind = tkVariant) and (not IsAttribute) then
            begin
  ...
  case (PropInfo.PropType)^.Kind of
    tkInteger:
      begin
        ...
        if (PropInfo.PropType)^.TypeData^.OrdType in [otUByte, otUWord, otULong] then
          Result := UIntToStr(NativeUInt(I))
        else
          Result := IntToStr(I);
      end;
    tkFloat:
...
{$ENDIF}
    ...
    tkDynArray:
      begin
        ...
        begin
          ...
          if (Dims <> 1) or not ByteArrayElemInfo(ElemInfo) then
          ...
          Len := GetDynArrayLength(P);
          Result := InvString(TNetEncoding.Base64.EncodeBytesToString(P, Len));
    ...
    tkVariant:
      begin
        V := GetVariantProp(Instance, PropInfo);
        if VarIsNull(V) or VarIsEmpty(V) then
          Result := ''
        else
...
begin
  case (PropInfo.PropType)^.Kind of
    tkInteger:
      begin
        if (PropInfo.PropType)^.TypeData^.OrdType in [otUByte, otUWord, otULong] then
          I := NativeInt(StrToUInt(SoapData))
        else
          I := StrToInt(SoapData);
...
{$ENDIF}
    ...
    tkWChar:
      if SoapData <> '' then
        SetOrdProp(Instance, PropInfo, Integer(SoapData[Low(string)]));
...
{$IFDEF NEXTGEN}
        ByteArray := TNetEncoding.Base64.DecodeStringToBytes(SoapData);
{$ELSE !NEXTGEN}
        ByteArray := TNetEncoding.Base64.DecodeStringToBytes(string(SoapData));
{$ENDIF NEXTGEN}

        SetDynArrayProp(Instance, PropInfo, Pointer(ByteArray));
      end;
    tkVariant:
      SetVariantProp(Instance, PropInfo, StringToVariant(SoapData));
    tkSet,
    tkMethod,
    tkArray,
    tkRecord,
...
begin
  ...
  begin
            ...
            begin
              ...
              begin
                ...
                begin
                  ...
                  ArrayPtr := nil;
                  { Here if the object we're writing to inlines array members [i.e. its array
                  ...
                  if Assigned(ArrayPtr) then
                    L := PInteger(PByte(ArrayPtr)-(Sizeof(NativeInt) + Sizeof(Integer)))^
                  else
                    L := 0;
                  SetDynArrayProp(Instance, PropList[I], ArrayPtr);
                  if (L > 0) and ((PInteger(PByte(ArrayPtr)-(Sizeof(NativeInt) + Sizeof(Integer)))^) > L) then
                    Dec(PInteger(PByte(ArrayPtr)-(Sizeof(NativeInt) + Sizeof(Integer)))^);
                end;
              ...
              begin
                ...
                begin
                  if IsVarArray(ChildNode) then
                    V := ReadVarArrayDim(ChildNode)
                  else
                  begin
                    GetElementType(ChildNode, URI, TypeName);
                    SoapTypeInfo := RemTypeRegistry.XSDToTypeInfo(URI, TypeName);
                    if NodeHasText(ChildNode) then
                    begin
                      if SoapTypeInfo <> nil then
                      begin
                        if IsXMLDateTimeTypeInfo(SoapTypeInfo) then
                          V := XMLTimeToDateTime(GetNodeText(ChildNode))
                        else
                          V := TypeTranslator.CastSoapToVariant(SoapTypeInfo, GetNodeText(ChildNode));
                      end
                      else
                      begin
                        V := StringToVariant(GetNodeText(ChildNode));
                      end
                    end
                    else
                      { Case of anyType mapping to a complex type; TXMLData would be better than Variant }
                      V := StringToVariant(ChildNode.XML)
                  end;
                end;
              ...
              begin
                ...
                if IsNull then
                begin
                  if (Kind = tkFloat) and (PropList[I] = TypeInfo(TDateTime)) then
                    TextData := ''
                  else
    ...
    Assert(NodeCount = Node.ChildNodes.Count, 'extra nodes in TSOAPDomConv.InitObjectFromSOAP');
  end;
end;
...
begin
  ...
  begin
    Len := GetDynArrayLength(P);
    S1 := TNetEncoding.Base64.EncodeBytesToString(P, Len); 
...
begin
  ...
  begin
    ...
    if (Dims = 1) and ByteArrayElemInfo(ElemInfo) then
...
begin
  { New ID }
  if not FRefcache.FMHREFs.ContainsKey(ID) then
    FRefCache.FMHREFs.AddOrSetValue(ID, Instance)
  else
  begin
    { Same object added with new ID?? }
    if not FRefCache.FMHREFs.ContainsValue(Instance) then
      FRefCache.FMHREFs.AddOrSetValue(ID, Instance)
  end;
end;
...
var
  P: TPair<string, TObject>;
begin
  Result := '';

  for P in FRefCache.FMHREFs do
  begin
    if P.Value = Instance then
    begin
      Result := P.Key;
      Break;
    end;
...
var
  LObj: TObject;
begin
  FRefCache.FMHREFs.TryGetValue(ID, LObj);
  Result := Pointer(LObj);
end;
...
begin
  ID := '';
  Result := Node;
  if (Result = nil) or (UsingLiteralEncoding) then
    Exit;
