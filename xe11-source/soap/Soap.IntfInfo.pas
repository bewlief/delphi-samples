...
begin
  ...
  P := Pointer(IntPtr(P) + 17);   // Intf.flags and GUID
  B := Byte(P^);                     // Length
  P := Pointer(IntPtr(P) + B + 1);// Unit name  and count
...
begin
  ...
  P := Pointer(IntPtr(P) + B + 1); // Skip sym name  and count
...
begin
  ...
  { Interface flags }
  ReadByte(P);
  IntfMD.IID.D1 := Cardinal(ReadLong(P));
