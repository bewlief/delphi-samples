...
uses
  System.Math, System.Types, System.Variants, System.NetEncoding,
  Soap.IntfInfo, Soap.SOAPConst, Soap.XSBuiltIns;
...
{$ENDIF}
    ...
    tkDynArray:
    begin
      if SameTypeInfo(TypeInfo(System.Types.TByteDynArray), SoapInfo) or
         SameTypeInfo(TypeInfo(Soap.InvokeRegistry.TByteSOAPArray), SoapInfo) then
  ...
  case Info^.Kind of
    tkInteger:
      case ParamTypeData^.OrdType of
        otSByte:
          Int8(NatData^) := StrToInt(Check(SoapData));
        otUByte:
          UInt8(NatData^) := StrToUInt(Check(SoapData));
        otSWord:
          Int16(NatData^) := StrToInt(Check(SoapData));
        otUWord:
          UInt16(NatData^) := StrToUInt(Check(SoapData));
        otSLong:
          Int32(NatData^) := StrToInt(Check(SoapData));
        otULong:
          UInt32(NatData^) := StrToUInt(Check(SoapData));
      end;
    tkFloat:
  ...
  case Info.Kind of
    tkInteger:
      case TypeData.OrdType of
        otSByte:
          SoapData := IntToStr(Int8(NatData^));
        otUByte:
          SoapData := UIntToStr(UInt8(NatData^));
        otSWord:
          SoapData := IntToStr(Int16(NatData^));
        otUWord:
          SoapData := UIntToStr(UInt16(NatData^));
        otSLong:
          SoapData := IntToStr(Int32(NatData^));
        otULong:
          SoapData := UIntToStr(UInt32(NatData^));
      end;
    tkFloat:
...
begin
{$IFDEF NEXTGEN}
  Base64Dec := TNetEncoding.Base64.DecodeStringToBytes(SoapData);
{$ELSE !NEXTGEN}
  Base64Dec := TNetEncoding.Base64.DecodeStringToBytes(string(SoapData));
{$ENDIF NEXTGEN}
