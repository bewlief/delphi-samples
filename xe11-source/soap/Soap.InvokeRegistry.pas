...
{$IFNDEF NEXTGEN}
  TWideStringSOAPArray = array of WideString;
{$ENDIF}
...
{$ENDIF}
  ...
  public
    ...
    function  URIToClass(const URI, Name: InvString; var IsScalar: Boolean; AInheritsFrom: TClass = nil): TClass; overload;
...
begin
  ...
  begin
    ...
    if SourceFile = '' then
      Exit;
    Result := TFileStream.Create(SourceFile, fmOpenRead or fmShareDenyWrite);
...
function TRemotableTypeRegistry.URIToClass(const URI, Name: InvString; var IsScalar: Boolean; AInheritsFrom: TClass): TClass;
var
  I: Integer;
begin
  Result := nil;
  ...
  try
    for I := 0 to Length(URIMap) - 1 do
    begin
      if (URIMap[I].URI = URI) and ((AInheritsFrom = nil) or ((URIMap[I].ClassType <> nil) and (URIMap[I].ClassType.InheritsFrom(AInheritsFrom))))then
      begin
...
{$ENDIF}
  ...
  RemClassRegistry.RegisterXSInfo(TypeInfo(TByteSOAPArray), SoapEncodingNamespaces[False], 'base64');
  RemClassRegistry.RegisterXSInfo(TypeInfo(System.Types.TByteDynArray), SoapEncodingNamespaces[False], 'base64');

  RemClassRegistry.RegisterXSInfo(TypeInfo(TIntegerSOAPArray), SBorlandTypeNamespace);
  RemClassRegistry.RegisterXSInfo(TypeInfo(TIntegerDynArray), SBorlandTypeNamespace);
  RemClassRegistry.RegisterXSInfo(TypeInfo(TCardinalSOAPArray), SBorlandTypeNamespace);
  RemClassRegistry.RegisterXSInfo(TypeInfo(TCardinalDynArray), SBorlandTypeNamespace);
  RemClassRegistry.RegisterXSInfo(TypeInfo(TWordSOAPArray), SBorlandTypeNamespace);
  RemClassRegistry.RegisterXSInfo(TypeInfo(TWordDynArray), SBorlandTypeNamespace);
  RemClassRegistry.RegisterXSInfo(TypeInfo(TSmallIntSOAPArray), SBorlandTypeNamespace);
  RemClassRegistry.RegisterXSInfo(TypeInfo(TSmallIntDynArray), SBorlandTypeNamespace);
  RemClassRegistry.RegisterXSInfo(TypeInfo(TByteSOAPArray), SBorlandTypeNamespace);
  RemClassRegistry.RegisterXSInfo(TypeInfo(System.Types.TByteDynArray), SBorlandTypeNamespace);
  RemClassRegistry.RegisterXSInfo(TypeInfo(TShortIntSOAPArray), SBorlandTypeNamespace);
  RemClassRegistry.RegisterXSInfo(TypeInfo(TShortIntDynArray), SBorlandTypeNamespace);
  RemClassRegistry.RegisterXSInfo(TypeInfo(TInt64SOAPArray), SBorlandTypeNamespace);
  RemClassRegistry.RegisterXSInfo(TypeInfo(TInt64DynArray), SBorlandTypeNamespace);
  RemClassRegistry.RegisterXSInfo(TypeInfo(TLongWordSOAPArray), SBorlandTypeNamespace);
  RemClassRegistry.RegisterXSInfo(TypeInfo(TLongWordDynArray), SBorlandTypeNamespace);
  RemClassRegistry.RegisterXSInfo(TypeInfo(TSingleSOAPArray), SBorlandTypeNamespace);
  RemClassRegistry.RegisterXSInfo(TypeInfo(TSingleDynArray), SBorlandTypeNamespace);
  RemClassRegistry.RegisterXSInfo(TypeInfo(TDoubleSOAPArray), SBorlandTypeNamespace);
  RemClassRegistry.RegisterXSInfo(TypeInfo(TDoubleDynArray), SBorlandTypeNamespace);
  RemClassRegistry.RegisterXSInfo(TypeInfo(TBooleanSOAPArray), SBorlandTypeNamespace);
  RemClassRegistry.RegisterXSInfo(TypeInfo(TBooleanDynArray), SBorlandTypeNamespace);
  RemClassRegistry.RegisterXSInfo(TypeInfo(TStringSOAPArray), SBorlandTypeNamespace);
  RemClassRegistry.RegisterXSInfo(TypeInfo(TStringDynArray), SBorlandTypeNamespace);
{$IFNDEF NEXTGEN}
  RemClassRegistry.RegisterXSInfo(TypeInfo(TWideStringSOAPArray), SBorlandTypeNamespace);
