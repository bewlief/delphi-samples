...
begin
  ...
  begin
    YPos := (Y + ScaleValue(1)) * CharHeight;
    XPos := SQLCanvas.TextWidth(Copy(SQLMemo.Lines[Y], 1, X)) + ScaleValue(TIDEThemeMetrics.AlignFactor);
    SQLCanvas.StretchDraw(Rect(XPos, YPos, XPos + ScaleValue(cWidthImage), YPos + ScaleValue(cHeightImage)), Image1.Picture.Graphic);
  end;
end;
