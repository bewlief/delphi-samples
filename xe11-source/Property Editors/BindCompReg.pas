...
type
  ...
  TLinkFillControlToFieldFieldNamePropertyEditor = class(TSourceMemberNamePropertyEditor)
  protected
    function GetSourceComponentName: string; override;
  end;
...
uses 
  DsnConst, Vcl.Forms, Winapi.Windows,Vcl.Controls, IDETheme.Utils,
  Vcl.Themes, Winapi.UxTheme, VisualizationServicesAPI, BrandingAPI,
  BindCompEdit, BindCompDsnResStrs, Vcl.Graphics, Vcl.ComCtrls, Vcl.Menus,
  System.Math, ComponentDesigner,  Vcl.GraphUtil,System.TypInfo, System.UITypes,
...
begin
  ...
  begin
    ...
    begin
      ...
      if LContext.Owner <> nil then
        LContext.Owner.RemoveComponent(LBindCompList);
      LBindCompList.Free;
    end;
  end;
end;
...
procedure TBindCompFactorySelectionEditor.RequiresUnits(Proc: TGetStrProc);
var
  I: Integer;
  LService: TDataSource;
begin
  for I := 0 to Designer.Root.ComponentCount - 1 do
  begin
    if Designer.Root.Components[I] is TCustomBindSourceDB then
    begin
      LService := TBindSourceDB(Designer.Root.Components[I]).DataSource;
      if (LService <> nil) and Assigned(GetMethodProp(LService, 'OnDataChange').Code) then
      begin
        Proc(TDataSource.UnitName);
        break;
      end;
    end;
  end;
...
begin
  ...
  BorderStyle := bsNone;
  Columns.Add;
  Height := GetControlForDPI.ScaleValue(FDefItemHeight);
  ReadOnly := True;
  RowSelect := True;
  ShowColumnHeaders := False;
  SmallImages := FImageList;
  ViewStyle := vsReport;
  Width := GetControlForDPI.ScaleValue(200);
  UpdateFont;
end;
...
function TBindCompListView.GetControlForDPI: TWinControl;
begin
  if Parent = nil then
...
procedure TBindCompListView.UpdateFont;
begin
...
begin
  ...
  LSelected := (Item.Caption <> '') and ([cdsSelected, cdsFocused, cdsHot] * State <> []);
  case Stage of
    cdPostPaint:
      begin
        if LSelected then
...
begin
  ...
  begin
    ...
    Self.Width := MinWidth + LWidth + LControlDPI.ScaleValue(40); //+ FImageList.Width;
...
begin
  ...
  begin
    ...
    if FTempStringList.Count > 0 then
      Height := (Min(FTempStringList.Count, 11) + 3) * GetFormDPI(nil).ScaleValue(FDefItemHeight)
    else
...
//  RegisterComponentEditor(TBaseLinkingBindSource, TBaseLinkingBindScopeEditor);
  ...
  RegisterPropertyEditor(TypeInfo(string), TCustomLinkFillControlToField, 'FieldName', TLinkFillControlToFieldFieldNamePropertyEditor);
  ...
  RegisterPropertyEditor(TypeInfo(string), TFormatExpressionItem , sSourceMemberName, TFieldNamePropertyEditor);
...
{ TLinkFillControlToFieldFieldNamePropertyEditor }

function TLinkFillControlToFieldFieldNamePropertyEditor.GetSourceComponentName: string;
const
  sDataSource = 'DataSource';
begin
  Result := sDataSource;
