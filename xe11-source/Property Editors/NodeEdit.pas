...
uses
  ...
  StdCtrls, ComCtrls, Consts, Vcl.ExtCtrls;
...
begin
  ...
  try
    ...
    if AItems.Owner is TTreeView then
    begin
      LTreeViewItems.TreeView.CheckBoxes := TTreeView(AItems.Owner).CheckBoxes;
      LTreeViewItems.TreeView.CheckStyles := TTreeView(AItems.Owner).CheckStyles;
    end;
...
begin
  var LPPI := GetFormDPI(nil).CurrentPPI;
  ScaleForPPI(LPPI);
  if TIDEThemeMetrics.Font.Enabled then
  begin
    Font.Assign(TIDEThemeMetrics.Font.GetFont);
    TIDEThemeMetrics.Font.AdjustDPISize(Font, TIDEThemeMetrics.Font.DefaultSize, LPPI);
  end;

  if ThemeProperties <> nil then
  begin
    IDEThemeManager.RegisterFormClass(TTreeViewItems);
  ...
  if SavedWidth <> 0 then
    Width := ScaleValue(SavedWidth);
  if SavedHeight <> 0 then
    Height := ScaleValue(SavedHeight);
  if SavedLeft <> 0 then
    Left := ScaleValue(SavedLeft)
  else
    Left := (Screen.Width - Width) div 2;
  if SavedTop <> 0 then
    Top := ScaleValue(SavedTop)
  else
...
begin
  SavedWidth := ConvertToDefaultPPI(Self, Width);
  SavedHeight := ConvertToDefaultPPI(Self, Height);
  SavedLeft := ConvertToDefaultPPI(Self, Left);
  SavedTop := ConvertToDefaultPPI(Self, Top);
end;
