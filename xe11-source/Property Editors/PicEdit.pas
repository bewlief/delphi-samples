...
uses
  System.TypInfo, System.SysUtils, DesignConst, LibHelp, BrandingAPI,
  Vcl.Themes, Vcl.GraphUtil, IDETheme.Utils;
...
begin
  ...
  begin
    IDEThemeManager.RegisterFormClass(TPictureEditorDlg);
    ThemeProperties.ApplyTheme(Self);
    SetupIDEDialogTitleBar(Self, nil, True);
    LStyle := ThemeProperties.StyleServices;
    Shape1.Pen.Color := ColorBlendRGB(LStyle.GetSystemColor(clBtnText),
    LStyle.GetSystemColor(clBtnFace), 0.7);
  end;
end;
