...
uses
  ToolsAPI, CommCtrl, System.TypInfo, DsnConst, BrandingAPI,
...
begin
  if (AImageList.Width > 256) or (AImageList.Height > 256) then
    raise ENonAIRException.Create(SImageListTooBig);
  ...
  with ListItems do
    try
      ...
      try
        ...
        StretchImageList(FImages, FScaledImages, 24, 24);                          
...
begin
  ...
  try
    ...
    ImageView.AlphaSort;
    MainImage.Picture.Bitmap.Canvas.Brush.Color := Self.Color;
...
begin
  ...
  begin
    ...
    Shape1.Pen.Color := GetGrayBlendColor(LStyle);
    IDEThemeManager.RegisterFormClass(TImageListEditor);
    ThemeProperties.ApplyTheme(Self);
  end;

  FImages := TImageList.Create(nil);
  FScaledImages := TImageList.CreateSize(24, 24);
...
begin
  ...
  begin
    ...
    if (Graphic.Width < LWidth) and (Graphic.Height < LHeight) then
      ...
      FOperation := ioCenter;
    FOwnerForm.Replace(OwnerIndex, Graphic, FTransparentColor, FFillColor,
      FOperation);
