...
uses DsnConst, System.Classes,
  ...
  Vcl.Samples.Bind.Editors, Vcl.Bind.ControlList;
    ...
    begin
      ...
      if Assigned(LContainedBindComponent.ControlComponent) then
        ...
        begin
          ...
          if LContainedBindComponent.ControlComponent.UnitName.StartsWith('Vcl.Samples.', True) then
            Proc('Vcl.Samples.Bind.Editors')
          else if LContainedBindComponent.ControlComponent.UnitName.StartsWith('Vcl.ControlList', True) then
            Proc('Vcl.Bind.ControlList');
        end;
    end;
  end;
end;
...
begin
  ...
  try
    LList.AddRange(Data.Bind.Components.GetControlValueClasses(sDfm, False)); // Do not localize
...
type
  TBindNavigatorSelectionEditor = class(TSelectionEditor)
  public
    procedure RequiresUnits(Proc: TGetStrProc); override;
  end;

procedure TBindNavigatorSelectionEditor.RequiresUnits(Proc: TGetStrProc);
begin
  Proc('Data.Bind.Controls');
end;
...
begin
  ...
  RegisterSelectionEditor(TBindNavigator, TBindNavigatorSelectionEditor);
