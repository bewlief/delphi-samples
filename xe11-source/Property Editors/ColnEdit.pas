...
uses
  Winapi.Windows, Winapi.Messages, System.SysUtils, System.Classes, System.Actions, System.Types,
  Vcl.Graphics, Vcl.Controls, Vcl.Forms, Vcl.StdCtrls, Vcl.Menus, Vcl.ExtCtrls, Vcl.ComCtrls,
  Vcl.ImgList, Vcl.ActnList, Vcl.ToolWin, Vcl.ActnPopup, Vcl.PlatformDefaultStyleActnCtrls,
  DesignIntf, DesignWindows, ToolWnds, DesignEditors, System.ImageList, Vcl.VirtualImageList;
...
uses
  System.Win.Registry, System.TypInfo, DesignConst, ComponentDesigner,
  BrandingAPI, IDETheme.Utils, Winapi.CommCtrl;
...
procedure TCollectionEditor.FormAfterMonitorDpiChanged(Sender: TObject; OldDPI,
  NewDPI: Integer);
begin
  inherited;
  if ListView1.Columns.Count > 0 then
    ListView1.Column[0].Width := FListViewWidth;
end;

procedure TCollectionEditor.FormBeforeMonitorDpiChanged(Sender: TObject; OldDPI,
  NewDPI: Integer);
begin
  if ListView1.Columns.Count > 0 then
  begin
    FListViewWidth := ListView1.Column[0].Width;
    if FListViewWidth = LVSCW_AUTOSIZE then
      ListView1.Column[0].Width := LVSCW_AUTOSIZE_USEHEADER
    else if FListViewWidth = LVSCW_AUTOSIZE_USEHEADER then
      ListView1.Column[0].Width := LVSCW_AUTOSIZE
  end;
  inherited;
end;
...
begin
  ...
  begin
    IDEThemeManager.RegisterFormClass(TCollectionEditor);
