...
uses
  ...
  BindCompExprVis, System.Actions, Vcl.VirtualImageList, System.ImageList;
...
uses System.Win.Registry, System.TypInfo, Winapi.CommCtrl, StdConst, DsnConst,
  ...
  Vcl.Dialogs, DesignConst, System.UITypes, BrandingAPI, Vcl.Themes, IDETheme.Utils;
...
begin
  ...
  begin
    IDEThemeManager.RegisterFormClass(TBindCompExprDesigner);
...
begin
  ...
  begin
    ...
    ButtonEvalControl.Top := ButtonEvalControl.Top + (2 * LDiff);
    ButtonAssignToSource.Top := ButtonAssignToSource.Top + (2 * LDiff);
    ButtonAssignToControl.Top := ButtonAssignToControl.Top + (2 * LDiff);
    ButtonEvalSource.Top := ButtonEvalSource.Top + (2 * LDiff);
  end;
end;
...
begin
  ...
  try
    FVisualizer.Width := ScaleValue(ReadInteger(LSubKey, sVisWidth, ConvertToDefaultPPI(Self, FVisualizer.Width)));
    FVisualizer.Height := ScaleValue(ReadInteger(LSubKey, sVisHeight, ConvertToDefaultPPI(Self, FVisualizer.Height)));
    FVisualizer.PanelVisualizerList.Width := ScaleValue(ReadInteger(LSubKey, sVisListWidth,
      ConvertToDefaultPPI(Self, FVisualizer.PanelVisualizerList.Width)));
  finally
    Free;
  end;
end;
...
begin
  ...
  with TRegIniFile.Create(GetRegKey) do
  try
    Width := ScaleValue(ReadInteger(LSubKey, sWidth, ConvertToDefaultPPI(Self, Width)));
    Height := ScaleValue(ReadInteger(LSubKey, sHeight, ConvertToDefaultPPI(Self, Height)));
    ...
    for LIndex := 0 to ListView1.Columns.Count - 1 do
      with ListView1.Columns.Items[LIndex] do
        Width := ScaleValue(ReadInteger(LSubKey, sWidth + IntToStr(LIndex), ConvertToDefaultPPI(Self, Width)));
    PanelCollections.Width := ScaleValue(ReadInteger(LSubKey, sPanelCollectionsWidth, ConvertToDefaultPPI(Self, PanelCollections.Width)));
...
begin
  ...
  try
    ...
    WriteInteger(LSubKey, sWidth, ConvertToDefaultPPI(Self, Width));
    WriteInteger(LSubKey, sHeight, ConvertToDefaultPPI(Self, Height));
    if FVisualizer <> nil then
    begin
      WriteInteger(LSubKey, sVisWidth, ConvertToDefaultPPI(Self, FVisualizer.Width));
      WriteInteger(LSubKey, sVisHeight, ConvertToDefaultPPI(Self, FVisualizer.Height));
      WriteInteger(LSubKey, sVisListWidth, ConvertToDefaultPPI(Self, FVisualizer.PanelVisualizerList.Width));
    end;
    for LIndex := 0 to ListView1.Columns.Count - 1 do
      with ListView1.Columns.Items[LIndex] do
        WriteInteger(LSubKey, sWidth + IntToStr(LIndex), ConvertToDefaultPPI(Self, Width));
    if PanelCollections.Visible then
      WriteInteger(LSubKey, sPanelCollectionsWidth, ConvertToDefaultPPI(Self, PanelCollections.Width));
...
begin
  ...
  begin
    ...
    LabelControlScope.Font.Color := LStyle.GetSystemColor(clWindowText);
    LabelSourceScope.Caption := Format(sSourceExpressionFor, [LExpression.SourceScope]);
    LabelSourceScope.Font.Color := LStyle.GetSystemColor(clWindowText);
  ...
  begin
    ...
    LabelControlScope.Font.Color := LStyle.GetSystemColor(clGrayText);
    LabelSourceScope.Caption := sSourceExpression;
    LabelSourceScope.Font.Color := LStyle.GetSystemColor(clGrayText);
    MemoControl.Text := '';
    MemoSource.Text := '';
    MemoSource.Enabled := False;
    MemoControl.Enabled := False;
