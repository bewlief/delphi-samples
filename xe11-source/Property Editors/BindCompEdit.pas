...
uses
  ...
  Data.Bind.Components, System.Actions, BindCompRelationships, 
...
uses System.Win.Registry, System.TypInfo, Winapi.CommCtrl, StdConst, DsnConst,
  ...
  Vcl.Dialogs, Generics.Collections, BindExpertsUIUnit, System.UITypes, BrandingAPI, IDETheme.Utils;
...
{  var
  ...
  begin
    ...
    try
      Width := ScaleValue(ReadInteger(FDataBindingListClassName, 'Width', ConvertToDefaultPPI(Self, Width)));
      Height := ScaleValue(ReadInteger(FDataBindingListClassName, 'Height', ConvertToDefaultPPI(Self, Height)));
      ...
      for LIndex := 0 to ListView1.Columns.Count - 1 do
        with ListView1.Columns.Items[LIndex] do
          Width := ScaleValue(ReadInteger(FDataBindingListClassName, 'Width' + IntToStr(LIndex), ConvertToDefaultPPI(Self, Width)));
{      Kind := ReadInteger(FActionListClassName, 'CategoryKind', Ord(ckNone));
      ...
      CategoryPanel.Width := ScaleValue(ReadInteger(FDataBindingListClassName, 'CategoryWidth', ConvertToDefaultPPI(Self, CategoryPanel.Width)));
...
begin
  ...
  try
    ...
    WriteInteger(FDataBindingListClassName, 'Width', ConvertToDefaultPPI(Self, Width));
    WriteInteger(FDataBindingListClassName, 'Height', ConvertToDefaultPPI(Self, Height));
    for I := 0 to ListView1.Columns.Count - 1 do
      with ListView1.Columns.Items[I] do
        WriteInteger(FDataBindingListClassName, 'Width' + IntToStr(I), ConvertToDefaultPPI(Self, Width));
    ...
    WriteInteger(FDataBindingListClassName, 'CategoryWidth', ConvertToDefaultPPI(Self, CategoryPanel.Width));
...
begin
  ...
  begin
    IDEThemeManager.RegisterFormClass(TDataBindingListDesigner);
...
begin
  ...
  try
    ScaleForPPI(GetFormDPI(nil).CurrentPPI);
