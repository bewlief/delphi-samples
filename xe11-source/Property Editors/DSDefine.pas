...
uses
  Menus, DsnDBCst, DBConsts, Dialogs, DSDesign, LibHelp, TypInfo, BrandingAPI;
...
// Get the owner of a sub component
...
begin
  ...
  begin
    ...
    IDEThemeManager.RegisterFormClass(TDefineField);
...
begin
  ...
  FField := FieldClass.Create(DataSetOwner(Dataset));
  try
...
begin
  ...
  try
    ...
    if Assigned(FieldClass) and FieldClass.InheritsFrom(TGuidField) then
    begin
      SizeEdit.Text := '38';
      SizeEdit.Enabled := False;
    end
    else if Assigned(FieldClass) then
    begin
