...
uses
  ...
  FMX.Presentation.Style, FMX.ScrollBox, FMX.ScrollBox.Style, FMX.AddressBook, FMX.ListView.Types, FMX.BufferedLayout,
  ...
  FmxStrEdit,
  {$ENDIF}
  FMX.Editor.Items, FMX.Editor.AlignmentOrder, FmxSprigs, FmxStyleLookup, FMX.Types, FMX.Design.Font, FMX.Canvas.D2D,
  Winapi.D2D1;
...
type
  ...
  private
    class function EnumFontsCallback(const LogFont: TLogFont; const Metrics: TNewTextMetric; FontType: DWORD;
      Param: LPARAM): Integer; stdcall; static;
  public
    function GetAttributes: TPropertyAttributes; override;
...
type
  ...
  TMemoSelectionEditor = class(TSelectionEditor)
    procedure RequiresUnits(Proc: TGetStrProc); override;
  end;
...
//  RegisterClass(TMenuItem);
  ...
  RegisterComponents(srAdditional, [TSpeedButton, TCornerButton, TArcDial,
    TAniIndicator, TMediaPlayer, TMediaPlayerControl, TCameraComponent, TBannerAd, TFirebaseBannerAd]);
...
//                         TCustomOptionsProperty);
  ...
  RegisterComponents(SLayouts, [TLayout, TScaledLayout, TGridLayout, TGridPanelLayout, TFlowLayoutBreak, TFlowLayout,
    ...
    TFramedVertScrollBox, TPresentedScrollBox, TBufferedLayout]);
  ...
  RegisterComponentEditor(TFlowLayout, TAlignmentOrderEditor);
...
//  RegisterPropertyEditor(TypeInfo(TBounds), TFmxObject, '', TRectAnimationProperty);
  ...
  // TMemo
  RegisterSelectionEditor(TMemo, TMemoSelectionEditor);
  ...
  RegisterComponents(srWinX, [FMX.WindowsStore.TWindowsStore]);
  GroupDescendentsWith(TWindowsStore, FMX.Controls.TControl);
...
class function TFontNameProperty.EnumFontsCallback(const LogFont: TLogFont; const Metrics: TNewTextMetric;
  FontType: DWORD; Param: LPARAM): Integer;
begin
  TStringList(Param).Add(LogFont.lfFaceName);
  Result := 1;
end;
...
var
  ...
  LogFont: TLogFont;
begin
  if TCanvasManager.DefaultCanvas.InheritsFrom(TCustomCanvasD2D) then
  begin
    if FFontCollection = nil then
      TCustomCanvasD2D.SharedDWriteFactory.GetSystemFontCollection(FFontCollection, True);
    for I := 0 to FFontCollection.GetFontFamilyCount - 1 do
      if Succeeded(FFontCollection.GetFontFamily(I, FontFamily)) then
      begin
        if Succeeded(FontFamily.GetFamilyNames(Names)) then
        begin
          Names.FindLocaleName(DefaultLocale, Index, Exists);
          if not Exists then
            Index := 0;
          Names.GetStringLength(Index, Length);
          Names.GetString(Index, FontName, Min(Length + 1, High(FontName)));
          Proc(FontName);
        end;
      end;
  end
  ...
  begin
    ...
    try
      ...
      EnumFontFamiliesEx(GetDC(0), LogFont, @EnumFontsCallback, LPARAM(Pointer(Fonts)), 0);
...
begin
  ...
  try
    EditDlg.PathData.Lines.Text := TPathData(GetOrdValue).Data;
...
{ TMemoSelectionEditor }

procedure TMemoSelectionEditor.RequiresUnits(Proc: TGetStrProc);
begin
  inherited;
  Proc('FMX.Memo.Types');
end;
