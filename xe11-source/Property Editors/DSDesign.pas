...
uses Windows, SysUtils, Messages, Classes, Graphics, Controls, Forms,
  ...
  DsnDBCst, DesignWindows, Menus, DrpCtrls, ActnPopup,
...
uses
  Dialogs, TypInfo, Math, LibHelp, DSAdd, DSDefine, DesignConst, ToolsAPI,
  StructureViewAPI, Character, BrandingAPI, IDETheme.Utils;

// Find the owner of a subcomponent
...
// Find the name of the owner of a subcomponent
function DataSetName(ADataSet: TComponent): string;
begin
  if csSubComponent in ADataSet.ComponentStyle then
    Result := DataSetName(ADataSet.Owner)
  else
...
procedure TDSDesigner.DataEvent(Event: TDataEvent; Info: NativeInt);
begin
...
begin
  ...
  begin
    ...
    ShortName := FieldName;
  Result := FieldDef.CreateField(DataSetOwner(DataSet), ParentField as TObjectField, ShortName, False);
  try
...
begin
  ...
  Result := Format(Fmt, [DatasetName(DataSet), FieldName, Number]);
end;
...
begin
  if (DataSet = nil) or (csDestroying in Dataset.ComponentState) then
    Exit;
  if DataSetOwner(DataSet) <> nil then
    NewCaption := Format(SDatasetEditor, [DataSetOwner(DataSet).Name, DotSep,
      DatasetName(DataSet)]);
      ...
      try
        ...
        begin
          ...
          if not (csDestroying in Field.ComponentState) and
             (Field.Owner = DataSetOwner(DataSet)) then
          begin
...
begin
  Inc(DesignerCount);
  FMinWidth := ConvertToDefaultPPI(Self, Width);
  FMinHeight := ConvertToDefaultPPI(Self, Height);
  HelpContext := hcDataSetDesigner;
  if ThemeProperties <> nil then
  begin
    Font.Assign(TIDEThemeMetrics.Font.GetFont);
    IDEThemeManager.RegisterFormClass(TFieldsEditor);
...
procedure TFieldsEditor.Notification(AComponent: TComponent; Operation: TOperation);
begin
  inherited;
    ...
    for I := 0 to DataSet.FieldDefList.Count - 1 do
      with Dataset.FieldDefList[I] do
        ...
        begin
          ...
          if (Field = nil) or (Field.Owner <> DataSetOwner(DataSet)) then
...
begin
  ...
  begin
    X := ScaleValue(FMinWidth);
    Y := ScaleValue(FMinHeight);
  end;
end;
...
begin
  ...
  try
    ...
    CopyComponents(DataSetOwner(DataSet), ComponentList);
  finally
  end;
end;
  ...
  try
    ...
    try
      ...
      try
        FDSDesigner.BeginDesign;
        try
          PasteComponents(DataSetOwner(DataSet), Dataset, ComponentList);
        finally
          FDSDesigner.EndDesign;
        end;
      finally
...
begin
  if (FDataset <> nil) and (DataSetOwner(FDataSet) <> nil) and
    (csInline in DataSetOwner(FDataSet).ComponentState) then
