...
procedure TListItemShim.FreeNotification(AObject: TObject);
begin
  if (AObject is TListItemDrawableShim) and (FShims <> nil) then
    FShims.Remove(TListItemDrawableShim(AObject));
end;
...
begin
  FreeAndNil(FPresentations);
  FreeAndNil(FEnabled);
  FreeAndNil(FCachedAdapters);
  FreeAndNil(FPrototypeData);
end;
