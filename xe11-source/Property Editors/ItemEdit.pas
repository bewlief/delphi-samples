...
uses
  ...
  StdCtrls, ComCtrls, DsnConst, Vcl.ExtCtrls;
...
uses
  CommCtrl, BrandingAPI, IDETheme.Utils;
  ...
  with TListViewItems.Create(Application)do
    try
      ...
      for I := 0 to ListView.Groups.Count - 1 do
        cbGroupID.AddItem(Format(SItemEditGroupIDStr, [ListView.Groups[I].GroupID,
        ListView.Groups[I].Header]), ListView.Groups[I]);
...
begin
  ...
  begin
    ...
    GetWindowText(EditHand, TextBuf, Length(TextBuf));
...
begin
  ...
  begin
    IDEThemeManager.RegisterFormClass(TListViewItems);
