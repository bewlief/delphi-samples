...
begin
  LList := TList<TComponentClass>.Create;
  try
    LList.AddRange(Data.Bind.Components.GetControlValueClasses(sFmx, False)); // Do not localize
...
begin
  ...
  begin
    ...
    if LControl is TMemo then
      // Memo clips owned labels
      Exclude(AOptions, TControlClassFactory.TOption.optLabel);
...
begin
  ...
  case AOption of
    ...
    optLabel:
      ...
      Result := not AClass.InheritsFrom(TCustomGrid) and
          //Adding a label to a Memo interferes with the memo text
          not AClass.InheritsFrom(TMemo);
    optSuffix:
      Result := True;
  else
    Assert(False);
